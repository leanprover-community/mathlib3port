...
theorem univ_pi_side (c : Cube n) : pi univ (side c) = c.to_set :=
  ext fun x => mem_univ_pi
#align theorems_100.«82».cube.univ_pi_side Theorems100.«82».Cube.univ_pi_side

theorem toSet_subset {c c' : Cube n} : c.to_set ⊆ c'.to_set ↔ ∀ j, c.side j ⊆ c'.side j := by
...
@[simp]
theorem side_unitCube {j : Fin n} : unitCube.side j = Ico 0 1 := by norm_num [unit_cube, side]
...
structure Correct (cs : ι → Cube n) : Prop where
  ...
  iUnion_eq : (⋃ i : ι, (cs i).to_set) = unitCube.to_set
...
theorem side_subset {i j} : (cs i).side j ⊆ Ico 0 1 := by
  simpa only [side_unit_cube] using to_set_subset.1 h.to_set_subset_unit_cube j
...
theorem zero_le_of_mem {i p} (hp : p ∈ (cs i).to_set) (j) : 0 ≤ p j :=
...
theorem w_ne_one [Nontrivial ι] (i : ι) : (cs i).w ≠ 1 :=
  ...
  let p := (cs i').b
  have hp : p ∈ (cs i').to_set := (cs i').b_mem_toSet
...
def Valley (cs : ι → Cube (n + 1)) (c : Cube (n + 1)) : Prop :=
  (c.bottom ⊆ ⋃ i : ι, (cs i).bottom) ∧
    (∀ i,
        (cs i).b 0 = c.b 0 →
          (∃ x, x ∈ (cs i).tail.to_set ∩ c.tail.to_set) → (cs i).tail.to_set ⊆ c.tail.to_set) ∧
...
theorem valley_unitCube [Nontrivial ι] (h : Correct cs) : Valley cs unitCube :=
  ...
  · intro v
    ...
    intro j; exact hi _
  · intro i hi h'; rw [to_set_subset]; intro j; convert h.side_subset using 1; simp [side_tail]
  · intro i hi; exact h.w_ne_one i
...
def OnBoundary (hi : i ∈ bcubes cs c) (j : Fin n) : Prop :=
...
theorem nontrivial_bcubes : (bcubes cs c).Nontrivial :=
  ...
  have hp : p ∈ c.bottom := by
    constructor; · simp only [bottom, p, if_neg hj]
...
theorem smallest_onBoundary {j} (bi : OnBoundary (mi_mem_bcubes : mi h v ∈ _) j) :
    ∃ x : ℝ,
      x ∈ c.side j.succ \ (cs <| mi h v).side j.succ ∧
        ∀ ⦃i'⦄ (hi' : i' ∈ bcubes cs c),
          i' ≠ mi h v → (cs <| mi h v).b j.succ ∈ (cs i').side j.succ → x ∈ (cs i').side j.succ :=
  by
  ...
  refine' ⟨x, ⟨_, _⟩, _⟩
  · simp only [side, x, -add_comm, -add_assoc, neg_lt_zero, hw, add_lt_iff_neg_left, and_true_iff,
      mem_Ico, sub_eq_add_neg]
    rw [add_assoc, le_add_iff_nonneg_right, ← sub_eq_add_neg, sub_nonneg]
    apply le_of_lt (w_lt_w h v hi')
  · simp only [side, not_and_or, not_lt, add_comm, not_le, mem_Ico]; left; exact hx
...
theorem mi_not_onBoundary (j : Fin n) : ¬OnBoundary (mi_mem_bcubes : mi h v ∈ _) j :=
  ...
  rcases v.1 hp with ⟨_, ⟨i', rfl⟩, hi'⟩
  have h2i' : i' ∈ bcubes cs c := ⟨hi'.1.symm, v.2.1 i' hi'.1.symm ⟨tail p, hi'.2, hp.2⟩⟩
  have i_i' : i ≠ i' := by rintro rfl; simpa [p, side_tail, i, h2x] using hi'.2 j
  have : Nonempty ↥((cs i').tail.side j' \ (cs i).tail.side j') := by apply nonempty_Ico_sdiff;
...
theorem mi_not_on_boundary' (j : Fin n) :
  ...
  simp [hw]
#align theorems_100.«82».mi_not_on_boundary' Theorems100.«82».mi_not_on_boundary'
...
theorem cannot_cube_a_cube :
    ∀ {n : ℕ},
      n ≥ 3 →-- In ℝ^n for n ≥ 3
        ∀ {s : Set (Cube n)},
          s.Finite →-- given a finite collection of (hyper)cubes
  ...
  · cases hn
  exact
    @not_correct n s coe hfin.to_subtype h2.coe_sort
