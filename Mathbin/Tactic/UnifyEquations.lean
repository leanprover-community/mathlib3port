...
-/
unsafe def unify_heterogeneous : unification_step :=
...
-/
unsafe def unify_defeq : unification_step :=
  fun equ lhs_type _ _ _ lhs_whnf rhs_whnf _ =>
    (do 
...
-/
unsafe def unify_var : unification_step :=
  fun equ type _ lhs rhs lhs_whnf rhs_whnf u =>
    (do 
...
-/
unsafe def unify_constructor_headed : unification_step :=
  fun equ _ _ _ _ _ _ _ =>
    (do 
...
-/
unsafe def get_sizeof (type : expr) : tactic pexpr :=
  do 
...
-/
unsafe def match_n_plus_m md : ℕ → expr → tactic (ℕ × expr) :=
  fun n e =>
    do 
      let e ← whnf e md 
      match e with 
...
-/
unsafe def contradict_n_eq_n_plus_m (md : transparency) (equ lhs rhs : expr) : tactic expr :=
  do 
...
-/
unsafe def unify_cyclic : unification_step :=
...
unsafe def orelse_step (s t : unification_step) : unification_step :=
  fun equ lhs_type rhs_type lhs rhs lhs_whnf rhs_whnf u =>
    do 
...
Otherwise it fails.
-/
unsafe def unify_equation_once (equ : Name) : tactic unification_step_result :=
  do 
    let eque ← get_local equ 
    let t ← infer_type eque 
    match t with 
      | app (app (app (const `eq [u]) type) lhs) rhs =>
        do 
