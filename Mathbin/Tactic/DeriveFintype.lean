...
#align_import tactic.derive_fintype from "leanprover-community/mathlib"@"6ca1a09bc9aa75824bf97388c9e3b441fc4ccf3f"
...
def FinsetAbove.cons {α} {enum : α → ℕ} (n) (a : α) (h : enum a = n)
  ...
  · intro x h'; rcases Finset.mem_cons.1 h' with (rfl | h')
    · exact ge_of_eq h
...
def FinsetAbove.union {α} {enum : α → ℕ} (n) (s : FinsetIn fun a => enum a = n)
  ...
  · rw [Finset.disjoint_left]
    ...
    exact Nat.not_succ_le_self n this
  · intro x h'; rcases Finset.mem_disjUnion.1 h' with (h' | h')
