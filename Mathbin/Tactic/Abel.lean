...
unsafe def mk_context (red : transparency) (e : expr) : tactic context :=
  do 
    let α ← infer_type e 
...
-/
unsafe def context.mk_app (c : context) (n inst : Name) (l : List expr) : tactic expr :=
  do 
...
unsafe def normal_expr.pp (e : normal_expr) : tactic format :=
  do 
    let l ←
      (to_list e).mmap
          fun ⟨n, e⟩ =>
...
theorem const_add_term {α} [AddCommMonoidₓ α] k n x a a' (h : (k+a) = a') : (k+@term α _ n x a) = term n x a' :=
  by 
    simp [h.symm, term] <;> acRfl

theorem const_add_termg {α} [AddCommGroupₓ α] k n x a a' (h : (k+a) = a') : (k+@termg α _ n x a) = termg n x a' :=
  by 
    simp [h.symm, termg] <;> acRfl

theorem term_add_const {α} [AddCommMonoidₓ α] n x a k a' (h : (a+k) = a') : (@term α _ n x a+k) = term n x a' :=
  by 
    simp [h.symm, term, add_assocₓ]

theorem term_add_constg {α} [AddCommGroupₓ α] n x a k a' (h : (a+k) = a') : (@termg α _ n x a+k) = termg n x a' :=
  by 
...
theorem term_add_term {α} [AddCommMonoidₓ α] n₁ x a₁ n₂ a₂ n' a' (h₁ : (n₁+n₂) = n') (h₂ : (a₁+a₂) = a') :
  ...
  by 
    simp [h₁.symm, h₂.symm, term, add_nsmul] <;> acRfl

theorem term_add_termg {α} [AddCommGroupₓ α] n₁ x a₁ n₂ a₂ n' a' (h₁ : (n₁+n₂) = n') (h₂ : (a₁+a₂) = a') :
  (@termg α _ n₁ x a₁+@termg α _ n₂ x a₂) = termg n' x a' :=
  by 
    simp [h₁.symm, h₂.symm, termg, add_zsmul] <;> acRfl

theorem zero_term {α} [AddCommMonoidₓ α] x a : @term α _ 0 x a = a :=
  by 
    simp [term, zero_nsmul, one_nsmul]

theorem zero_termg {α} [AddCommGroupₓ α] x a : @termg α _ 0 x a = a :=
  by 
    simp [termg]
...
theorem term_neg {α} [AddCommGroupₓ α] n x a n' a' (h₁ : -n = n') (h₂ : -a = a') : -@termg α _ n x a = termg n' x a' :=
  by 
    simp [h₂.symm, h₁.symm, termg] <;> acRfl
...
theorem zero_smul {α} [AddCommMonoidₓ α] c : smul c (0 : α) = 0 :=
  by 
    simp [smul, nsmul_zero]

theorem zero_smulg {α} [AddCommGroupₓ α] c : smulg c (0 : α) = 0 :=
  by 
    simp [smulg]
...
theorem term_atom {α} [AddCommMonoidₓ α] (x : α) : x = term 1 x 0 :=
  by 
    simp [term]

theorem term_atomg {α} [AddCommGroupₓ α] (x : α) : x = termg 1 x 0 :=
  by 
    simp [termg]

unsafe def eval_atom (c : context) (e : expr) : tactic (normal_expr × expr) :=
  do 
...
theorem subst_into_smulg {α} [AddCommGroupₓ α] l r tl tr t (prl : l = tl) (prr : r = tr) (prt : @smulg α _ tl tr = t) :
  smulg l r = t :=
  by 
    simp [prl, prr, prt]
