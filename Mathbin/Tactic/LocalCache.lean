...
unsafe def load_data (dn : Name) : tactic α :=
  do 
    let e ← tactic.get_env 
    let d ← e.get dn 
    tactic.eval_expr α d.value

unsafe def poke_data (dn : Name) : tactic Bool :=
  do 
    let e ← tactic.get_env 
...
unsafe def present (ns : Name) : tactic Bool :=
  do 
    let o ← tactic.get_options 
...
unsafe def hash_context : tactic Stringₓ :=
  do 
...
unsafe def get_root_name (ns : Name) : tactic Name :=
  do 
    let hc ← hash_context 
...
| tag =>
  do 
    ...
    if ¬present then fail f! "{rn} never seen in cache!" else
        do 
