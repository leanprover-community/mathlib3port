...
unsafe def refl_conv (e : expr) : tactic (expr × expr) :=
  do 
    let p ← mk_eq_refl e 
    return (e, p)
...
protected unsafe def of_nat (α : expr) : ℕ → tactic expr :=
  Nat.binaryRec (tactic.mk_mapp `` HasZero.zero [some α, none])
    fun b n tac =>
      if n = 0 then mk_mapp `` HasOne.one [some α, none] else
        do 
          let e ← tac 
...
unsafe def kreplace (e old new : expr) (md := semireducible) (unify := tt) : tactic expr :=
  do 
...
unsafe def set_state (state : σ) : interaction_monad σ Unit :=
  fun _ => success () State
...
| mappings, var :: rest =>
  do 
    ...
    if is_dependent then add_local_consts_as_local_hyps_aux mappings (rest ++ [var]) else
        do 
...
-/
unsafe def get_app_fn_const_whnf (e : expr) (md := semireducible) (unfold_ginductive := tt) : tactic Name :=
  do 
...
| e@(expr.local_const uniq pp info _) :: es, f =>
  do 
    let t ← infer_type e 
    let f' ← pis es f 
...
unsafe def add_theorem_by (n : Name) (ls : List Name) (type : expr) (tac : tactic Unit) : tactic expr :=
  do 
...
unsafe def mk_user_fresh_name : tactic Name :=
  do 
...
unsafe def local_decls : tactic (name_map declaration) :=
  do 
...
-/
unsafe def get_decls_from (fs : List (Option Stringₓ)) : tactic (name_map declaration) :=
  do 
...
-/
unsafe def decl_mk_const (d : declaration) : tactic (expr × expr) :=
  do 
...
-/
unsafe def replace_univ_metas_with_univ_params (e : expr) : tactic expr :=
  do 
    e.list_univ_meta_vars.enum.mmap
        fun n =>
          do 
...
| n+1, e, hs, ns =>
  do 
    let t ← infer_type e 
    if t.is_app_of `eq then return (hs.reverse, e, ns) else
        do 
...
-/
unsafe def elim_gen_sum (n : Nat) (e : expr) : tactic (List expr) :=
  do 
...
unsafe def extract_def (n : Name) (trusted : Bool) (elab_def : tactic Unit) : tactic Unit :=
  do 
...
unsafe def retrieve {α} (tac : tactic α) : tactic α :=
  fun s => result.cases_on (tac s) (fun a s' => result.success a s) result.exception

/-- Runs a tactic for a result, reverting the state after completion or error. -/
unsafe def retrieve' {α} (tac : tactic α) : tactic α :=
  fun s => result.cases_on (tac s) (fun a s' => result.success a s) fun msg pos s' => result.exception msg Pos s
...
-/
unsafe def replace_at (tac : expr → tactic (expr × expr)) (hs : List expr) (tgt : Bool) : tactic Bool :=
  do 
    let to_remove ←
      hs.mfilter$
          fun h =>
...
unsafe def revert_after (e : expr) : tactic ℕ :=
  do 
    let l ← local_context 
...
unsafe def revert_target_deps : tactic ℕ :=
  do 
    let tgt ← target 
    let ctx ← local_context 
    let l ← ctx.mfilter (kdepends_on tgt)
    let n ← revert_lst l 
    if l = [] then return n else
        do 
...
unsafe def get_univ_level (t : expr) (md := semireducible) (unfold_ginductive := tt) : tactic level :=
  do 
...
unsafe def is_local_def (e : expr) : tactic Unit :=
  do 
...
unsafe def local_defs : tactic (List expr) :=
  do 
    let ctx ← unsafe.type_context.get_local_context.run 
    let ctx' ← local_context 
    ctx'.mfilter$
...
unsafe def partition_local_deps (vs : List expr) : tactic (List (List expr)) :=
  do 
...
unsafe def clear_value (vs : List expr) : tactic Unit :=
  do 
        ...
        fun vs =>
          do 
            ...
            let expr.elet v t d b ← target |
              fail f! "Cannot clear the body of {vs.head }. It is not a local definition."
...
the context.
-/
unsafe def context_has_local_def : tactic Bool :=
  do 
    let ctx ← local_context 
...
-/
unsafe def context_upto_hyp_has_local_def (h : expr) : tactic Bool :=
  do 
  ...
  do 
    let e ←
      do 
          ...
          if f.const_name = `eq ∨ f.const_name = `heq then
              do 
                let lhs := args.inth 1
                let rhs := args.ilast 
                if rhs.is_local_constant then return rhs else
                    if lhs.is_local_constant then return lhs else
                      fail "subst tactic failed, hypothesis '{h.local_pp_name}' is not of the form (x = t) or (t = x)."
            else return h 
    success_if_fail (is_local_def e) <|>
        fail
          f!
            "Cannot substitute variable {e.local_pp_name
...
unsafe def mk_instance_cache (α : expr) : tactic instance_cache :=
  do 
    let u ← mk_meta_univ 
...
| pi _ BinderInfo.inst_implicit (app (const n _) (var _)) body, c, l =>
  do 
    let (c, p) ← c.get n 
    return (c, p :: l)
| _, c, l => return (c, l)
...
-/
unsafe def mk_app (c : instance_cache) (n : Name) (l : List expr) : tactic (instance_cache × expr) :=
  do 
    let d ← get_decl n 
...
protected unsafe def of_nat (c : instance_cache) (n : ℕ) : tactic (instance_cache × expr) :=
  if n = 0 then c.mk_app `` HasZero.zero [] else
    do 
      let (c, ai) ← c.get `` Add 
...
unsafe def subobject_names (struct_n : Name) : tactic (List Name × List Name) :=
  do 
    let env ← get_env 
    let c ←
      match env.constructors_of struct_n with 
        | [c] => pure c
        | [] =>
          if env.is_inductive struct_n then fail f! "{struct_n} does not have constructors" else
            fail f! "{struct_n} is not an inductive type"
...
-/
unsafe def mk_conditional_instance (cond tgt : expr) : tactic (expr × expr) :=
  do 
...
unsafe def get_goal : tactic expr :=
  do 
    let gs ← get_goals 
    match gs with 
      | [a] => return a
...
a lemma from the list until it can't.
 -/
unsafe def apply_rules (hs : List pexpr) (n : Nat) (opt : apply_cfg) : tactic Unit :=
  do 
...
unsafe def replace (h : Name) (p : pexpr) : tactic Unit :=
  do 
    let h' ← get_local h 
    let p ← to_expr p 
    note h none p 
...
unsafe def iff_mp (e : expr) : tactic expr :=
  do 
    let t ← infer_type e 
...
unsafe def iff_mpr (e : expr) : tactic expr :=
  do 
    let t ← infer_type e 
...
unsafe def apply_any_thunk (lemmas : List (tactic expr)) (opt : apply_any_opt := {  }) (tac : tactic Unit := skip)
  (on_success : expr → tactic Unit := fun _ => skip) (on_failure : tactic Unit := skip) : tactic Unit :=
  do 
...
-/
unsafe def change_with_at (olde newe : pexpr) (hyp : Name) : tactic Unit :=
  do 
    let h ← get_local hyp 
    let tp ← infer_type h 
...
unsafe def propositional_goal : tactic Unit :=
  do 
    let g :: _ ← get_goals 
    is_proof g >>= guardb
...
unsafe def subsingleton_goal : tactic Unit :=
  do 
    let g :: _ ← get_goals 
...
unsafe def iterate1 (t : tactic α) : tactic (List α) :=
  do 
...
unsafe def under_binders {α : Type} (t : tactic α) : tactic α :=
  do 
    let v ← intros 
    let r ← t 
    revert_lst v 
...
unsafe def injections_and_clear : tactic Unit :=
  do 
    let l ← local_context 
...
unsafe def case_bash : tactic Unit :=
  do 
    let l ← local_context 
...
unsafe def dependent_pose_core (l : List (expr × expr)) : tactic Unit :=
  do 
...
unsafe def mk_protected (n : Name) : tactic Unit :=
  do 
    let env ← get_env 
...
current line. -/
unsafe def emit_command_here (str : Stringₓ) : lean.parser Stringₓ :=
  do 
...
@[user_command]
unsafe def run_parser_cmd (_ : interactive.parse$ tk "run_parser") : lean.parser Unit :=
  do 
...
unsafe def get_current_namespace : lean.parser Name :=
  do 
...
unsafe def get_included_variables : lean.parser (List (Name × BinderInfo × expr)) :=
  do 
...
unsafe def instance_stub : hole_command :=
  { Name := "Instance Stub", descr := "Generate a skeleton for the structure under construction.",
    action :=
      fun _ =>
        do 
          let tgt ← target >>= whnf 
...
unsafe def resolve_name' (n : Name) : tactic pexpr :=
  do 
...
unsafe def mk_patterns (t : expr) : tactic (List format) :=
  do 
...
unsafe def match_stub : hole_command :=
  { Name := "Match Stub", descr := "Generate a list of equations for a `match` expression.",
    action :=
      fun es =>
        do 
...
unsafe def eqn_stub : hole_command :=
  { Name := "Equations Stub", descr := "Generate a list of equations for a recursive definition.",
    action :=
      fun es =>
        do 
          let t ←
...
unsafe def list_constructors_hole : hole_command :=
  { Name := "List Constructors", descr := "Show the list of constructors of the expected type.",
    action :=
      fun es =>
        do 
          let t ← target >>= whnf 
...
unsafe def classical : tactic Unit :=
  do 
...
unsafe def higher_order_attr : user_attribute Unit (Option Name) :=
  { Name := `higher_order, parser := optionalₓ ident,
    descr :=
...
attribute [higherOrder map_comp_pure] map_pure
...
unsafe def apply_at (e h : expr) : tactic Unit :=
  do 
    let ht ← infer_type h 
    let et ← infer_type e 
...
unsafe def symmetry_hyp (h : expr) (md := semireducible) : tactic Unit :=
  do 
...
unsafe def finally {β} (tac : tactic α) (finalizer : tactic β) : tactic α :=
  fun s =>
    match tac s with 
...
unsafe def retrieve_or_report_error {α : Type u} (t : tactic α) : tactic (Sum α Stringₓ) :=
  fun s =>
    match t s with 
...
unsafe def try_or_report_error {α : Type u} (t : tactic α) : tactic (Sum α Stringₓ) :=
  fun s =>
    match t s with 
    | interaction_monad.result.success a s' => result.success (Sum.inl a) s'
    | interaction_monad.result.exception msg' _ s' => result.success (Sum.inr (msg'.iget ()).toString) s
...
-/
unsafe def succeeds_or_fails_with_msg {α : Type} (t : tactic α) (p : Stringₓ → Bool) : tactic Unit :=
  do 
...
and fails otherwise.
-/
unsafe def success_if_fail_with_msg {α : Type u} (t : tactic α) (msg : Stringₓ) : tactic Unit :=
  fun s =>
    match t s with 
    | interaction_monad.result.exception msg' _ s' =>
      let expected_msg := (msg'.iget ()).toString 
      if msg = expected_msg then result.success () s else
        mk_exception
...
-/
unsafe def tactic_statement (g : expr) : tactic Stringₓ :=
  do 
...
unsafe def with_local_goals {α} (gs : List expr) (tac : tactic α) : tactic (α × List expr) :=
  do 
    let gs' ← get_goals 
    set_goals gs 
...
unsafe def get_packaged_goal : tactic packaged_goal :=
  do 
    let ls ← local_context 
...
-/
unsafe def get_proof_state : tactic proof_state :=
  do 
    let gs ← get_goals 
    gs.mmap$
        fun g =>
...
unsafe def get_project_dir (n : Name) (k : ℕ) : tactic Stringₓ :=
  do 
    let e ← get_env 
    let s ←
      e.decl_olean n <|>
          throwError
            "Did not find declaration {( ← n)}. This command does not work in the file where { ← n } is declared."
...
unsafe def is_in_mathlib (n : Name) : tactic Bool :=
  do 
...
-/
unsafe def name_to_tactic (n : Name) : tactic Stringₓ :=
  do 
    let d ← get_decl n 
    let e ← mk_const n 
    let t := d.type 
...
unsafe def find_private_decl (n : Name) (fr : Option Name) : tactic Name :=
  do 
    let env ← get_env 
    let fn ←
      OptionTₓ.run
...
When possible, make `foo` non-private rather than using this feature.
 -/
@[user_command]
unsafe def import_private_cmd (_ : parse$ tk "import_private") : lean.parser Unit :=
  do 
    let n ← ident 
...
-/
@[user_command]
unsafe def mk_simp_attribute_cmd (_ : parse$ tk "mk_simp_attribute") : lean.parser Unit :=
  do 
    let n ← ident 
    let d ← parser.pexpr 
...
unsafe def get_user_attribute_name (attr_name : Name) : tactic Name :=
  do 
...
unsafe def set_attribute (attr_name : Name) (c_name : Name) (persistent := tt) (prio : Option Nat := none) :
  ...
  do 
    ...
    if msg = (f! "set_basic_attribute tactic failed, '{attr_name}' is not a basic attribute").toString then
        do 
