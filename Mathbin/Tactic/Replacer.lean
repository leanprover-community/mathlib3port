...
`α → β → option (tactic γ) → tactic γ` analogously to the previous cases.
 -/
@[user_command]
unsafe def def_replacer_cmd (_ : parse$ tk "def_replacer") : lean.parser Unit :=
  do 
    let ntac ← ident 
...
@[user_attribute]
unsafe def replaceable_attr : user_attribute :=
  { Name := `replaceable, descr := "make definition replaceable in dependent modules",
    after_set :=
      some$
        fun n' _ _ =>
          do 
