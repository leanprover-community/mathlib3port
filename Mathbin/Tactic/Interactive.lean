...
unsafe def try_for (max : parse parser.pexpr) (tac : itactic) : tactic Unit :=
  do 
...
unsafe def substs (l : parse (ident)*) : tactic Unit :=
...
unsafe def unfold_aux : tactic Unit :=
  do 
    let tgt ← target 
    let name ← decl_name 
...
unsafe def continue (tac : itactic) : tactic Unit :=
  fun s => result.cases_on (tac s) (fun a => result.success ()) fun e ref => result.success ()
...
-/
unsafe def swap (n := 2) : tactic Unit :=
  do 
    let gs ← get_goals 
...
of Coq. -/
unsafe def replace (h : parse (ident)?) (q₁ : parse (tk ":" *> texpr)?) (q₂ : parse$ (tk ":=" *> texpr)?) :
  tactic Unit :=
  do 
...
omit the goal.
-/
unsafe def generalize_hyp (h : parse (ident)?) (_ : parse$ tk ":") (p : parse generalize_arg_p) (l : parse location) :
  tactic Unit :=
  do 
...
| ns, bi, t, v'@(local_const n pp bi' t') :: xs =>
  do 
    let t' ← infer_type v' 
    if bi = bi' ∧ t = t' then compact_decl_aux (pp :: ns) bi t xs else
        do 
...
-/
unsafe def guard_hyp' (n : parse ident) (p : parse$ tk ":" *> texpr) : tactic Unit :=
  do 
...
We use this tactic for writing tests.
-/
unsafe def match_hyp (n : parse ident) (p : parse$ tk ":" *> texpr) (m := reducible) : tactic (List expr) :=
  do 
    let h ← get_local n >>= infer_type >>= instantiate_mvars 
    match_expr p h m
...
We use this tactic for writing tests.
-/
unsafe def guard_expr_strict (t : expr) (p : parse$ tk ":=" *> texpr) : tactic Unit :=
  do 
    let e ← to_expr p 
    guardₓ (t = e)
...
We use this tactic for writing tests.
-/
unsafe def guard_target_strict (p : parse texpr) : tactic Unit :=
  do 
    let t ← target 
...
/-- `guard_proof_term { t } e` applies tactic `t` and tests whether the resulting proof term
  unifies with `p`. -/
unsafe def guard_proof_term (t : itactic) (p : parse texpr) : itactic :=
  do 
    let g :: _ ← get_goals 
    let e ← to_expr p 
    t 
...
unsafe def get_current_field : tactic Name :=
  do 
    let [_, field, str] ← get_main_tag 
    expr.const_name <$> resolve_name (field.update_prefix str)

unsafe def field (n : parse ident) (tac : itactic) : tactic Unit :=
  do 
    let gs ← get_goals 
...
-/
unsafe def h_generalize (rev : parse (tk "!")?) (h : parse (ident_)?) (_ : parse (tk ":"))
  (arg : parse h_generalize_arg_p) (eqs_h : parse (tk "with" >> pure <$> ident_ <|> pure [])) : tactic Unit :=
  do 
    let (e, n) := arg 
...
```
-/
unsafe def Set (h_simp : parse (tk "!")?) (a : parse ident) (tp : parse (tk ":" >> texpr)?) (_ : parse (tk ":="))
...
-/
unsafe def clear_except (xs : parse (ident)*) : tactic Unit :=
  do 
...
private unsafe def partition_vars : tactic (List expr × List expr) :=
  do 
    let ls ← local_context 
...
-/
unsafe def extract_goal (print_use : parse$ (tt <$ tk "!" <|> pure ff)) (n : parse (ident)?)
  (vs : parse (tk "with" *> (ident)*)?) : tactic Unit :=
  do 
    let tgt ← target 
    solve_aux tgt$
        do 
...
```
-/
unsafe def inhabit (t : parse parser.pexpr) (inst_name : parse (ident)?) : tactic Unit :=
  do 
    let ty ← i_to_expr t 
...
unsafe def revert_deps (ns : parse (ident)*) : tactic Unit :=
...
unsafe def clear_value (ns : parse (ident)*) : tactic Unit :=
