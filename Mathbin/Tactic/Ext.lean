...
-/
unsafe def derive_struct_ext_lemma (n : Name) : tactic Name :=
  do 
    let e ← get_env 
            ...
            do 
              ...
              if s ≠ quote.1 Prop then
                  do 
                    ...
                    let t' ← infer_type y 
                    some <$>
                        if t = t' then mk_app `eq [x, y] >>= mk_local_def `h else
                          mk_mapp `heq [none, x, none, y] >>= mk_local_def `h
                else pure none 
...
| expr.app _ e =>
  do 
    ...
    if t.get_app_fn.is_constant then pure$ t.get_app_fn.const_name else
        if t.is_pi then pure$ Name.mk_numeral 0 Name.anonymous else
          if t.is_sort then pure$ Name.mk_numeral 1 Name.anonymous else
            do 
              let t ← pp t 
...
unsafe def equiv_type_constr (n n' : Name) : tactic Unit :=
  do 
...
@[ext]
theorem ext {P : Prop} (a b : Plift P) : a = b :=
  by 
    cases a 
    cases b 
    rfl
...
@[ext]
theorem PUnit.extₓ {x y : PUnit} : x = y :=
  by 
    cases x 
    cases y 
    rfl
...
/-- Helper function for `try_intros`. Additionally populates the `trace_msg` field
  of `ext_state`. -/
private unsafe def try_intros_core : StateTₓ ext_state tactic Unit :=
  do 
...
/-- Apply one extensionality lemma, and destruct the arguments using the patterns
  in the ext_state. -/
unsafe def ext1_core (cfg : apply_cfg := {  }) : StateTₓ ext_state tactic Unit :=
  do 
...
unsafe def ext_core (cfg : apply_cfg := {  }) : StateTₓ ext_state tactic Unit :=
  do 
...
unsafe def ext1 (xs : List rcases_patt) (cfg : apply_cfg := {  }) (trace : Bool := ff) : tactic (List rcases_patt) :=
  do 
