...
-/
unsafe def distrib_not : tactic Unit :=
  do 
...
unsafe def add_refl (r : tauto_state) (e : expr) : tactic (expr × expr) :=
  do 
    let m ← read_ref r 
...
/--
  ...
  store it.
-/
unsafe def add_symm_proof (r : tauto_state) (e : expr) : tactic (expr × expr) :=
  do 
    let env ← get_env 
