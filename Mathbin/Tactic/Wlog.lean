...
unsafe def wlog (vars' : List expr) (h_cases fst_case : expr) (perms : List (List expr)) : tactic Unit :=
  do 
      ...
      match cases with 
        | some r =>
          do 
            ...
            let perms ←
              if perms.length = 1 then
                  do 
                    return
                        (perms'.map$
                          fun p => p ++ vars.filter fun v => p.all fun v' => v'.local_uniq_name ≠ v.local_uniq_name)
                else
                  do 
