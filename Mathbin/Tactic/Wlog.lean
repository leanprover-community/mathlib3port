...
private unsafe
  def
    match_perms
    ( pat : pattern ) : expr → tactic ( List <| List expr )
    |
      t
      =>
      (
    ...
    :=
      do
        let perms ← parse_permutations perms
          let
            ( pat , cases_pr , cases_goal , vars , perms )
              ←
              match
                cases
                with
                        ...
                        let
                          perms
                            ...
                            if
                              perms . length = 1
                              then
                              do return ( perms' fun p => p ++ vars fun v => p fun v' => v' ≠ v )
                              else
                              do
                                guard ( perms = perms' )
                                    <|>
          ...
          let
            name_fn
              ...
              if
                ...
                fun i => mkSimpleName ( "invariant_" ++ toString ( i + 1 ) )
          with_enable_tags
            <|
            tactic.focus1
              do
