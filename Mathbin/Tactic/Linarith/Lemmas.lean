...
theorem mul_eq {α} [OrderedSemiring α] {a b : α} (ha : a = 0) (hb : 0 < b) : (b*a) = 0 :=
  by 
    simp 
...
@[nolint unused_arguments]
theorem mul_zero_eq {α} {R : α → α → Prop} [Semiringₓ α] {a b : α} (_ : R a 0) (h : b = 0) : (a*b) = 0 :=
  by 
    simp [h]

@[nolint unused_arguments]
theorem zero_mul_eq {α} {R : α → α → Prop} [Semiringₓ α] {a b : α} (h : a = 0) (_ : R b 0) : (a*b) = 0 :=
  by 
    simp [h]
