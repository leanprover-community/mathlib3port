...
| a@(n1, z1) :: t1, b@(n2, z2) :: t2 =>
  if n1 < n2 then b :: add (a :: t1) t2 else
    if n2 < n1 then a :: add t1 (b :: t2) else
      let sum := z1+z2 
...
| _, [] => Ordering.gt
| (n1, z1) :: t1, (n2, z2) :: t2 =>
  if n1 < n2 then Ordering.lt else
    if n2 < n1 then Ordering.gt else if z1 < z2 then Ordering.lt else if z2 < z1 then Ordering.gt else cmp t1 t2
    ...
    transform :=
      fun l =>
        do 
          ...
          singleton <$> Prod.mk g <$> pp.transform l }
...
-/
unsafe def mk_single_comp_zero_pf (c : ℕ) (h : expr) : tactic (ineq × expr) :=
  do 
    let tp ← infer_type h 
