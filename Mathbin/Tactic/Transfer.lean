...
private unsafe def analyse_rule (u' : List Name) (pr : expr) : tactic rule_data :=
  do 
    let t ← infer_type pr 
    ...
    let args ←
      (enum arg_rels).mmap$
          fun ⟨n, a⟩ => Prod.mk <$> mk_local_def (mkSimpleName ("a_" ++ reprₓ n)) a.in_type <*> pure a 
    ...
    let (e, m) ←
      match s with 
        ...
        | none =>
          ...
          let ty := pis ctxt' t 
          if bi = BinderInfo.inst_implicit then
            do 
...
unsafe def tactic.transfer (ds : List Name) : tactic Unit :=
  do 
