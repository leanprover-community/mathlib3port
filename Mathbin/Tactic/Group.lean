...
unsafe def aux_group₁ (locat : Loc) : tactic Unit :=
  simp_core { failIfUnchanged := false } skip true
      [expr ``(commutatorElement_def), expr ``(mul_one), expr ``(one_mul), expr ``(one_pow),
        ...
        expr ``(Int.ofNat_zero), expr ``(Int.ofNat_one), expr ``(Int.coe_nat_bit0),
        expr ``(Int.coe_nat_bit1), expr ``(Int.mul_neg_eq_neg_mul_symm),
        expr ``(Int.neg_mul_eq_neg_mul_symm), symm_expr ``(zpow_natCast),
