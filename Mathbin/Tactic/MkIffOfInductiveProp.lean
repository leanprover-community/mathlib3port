...
unsafe def constr_to_prop (univs : List level) (g : List expr) (idxs : List expr) (c : Name) :
  tactic ((List (Option expr) × Sum expr ℕ) × expr) :=
  do 
    let e ← get_env 
    let decl ← get_decl c 
...
@[nolint doc_blame]
unsafe def to_cases (s : List$ List (Option expr) × Sum expr ℕ) : tactic Unit :=
  do 
    let h ← intro1 
    let i ← induction h 
    focus
        ((s.zip i).enum.map$
          fun ⟨p, (shape, t), _, vars, _⟩ =>
            do 
...
```
-/
unsafe def mk_iff_of_inductive_prop (i : Name) (r : Name) : tactic Unit :=
  do 
    let e ← get_env 
    ...
    add_theorem_by r univ_names ((mk_iff lhs (mk_or_lst rhss)).pis g)
        do 
