...
unsafe def clear_goals (ugs : List uncleared_goal) : tactic Unit :=
  do 
    let gs ←
      ugs.mmap
          fun ⟨cs, g⟩ =>
...
unsafe def rcases (h : Option Name) (p : pexpr) (pat : rcases_patt) : tactic Unit :=
  do 
    let p :=
      match pat with 
      ...
      match h with 
        | some h =>
          do 
            ...
            get_local x
        | none => i_to_expr p 
    if e.is_local_constant then focus1 (rcases_core pat e >>= clear_goals) else
        do 
...
unsafe def rcases_many (ps : listΠ pexpr) (pat : rcases_patt) : tactic Unit :=
  do 
...
unsafe def rintro (ids : listΠ rcases_patt) : tactic Unit :=
  do 
    let l ←
      ids.mmap
          fun id =>
...
unsafe def rcases_hint (p : pexpr) (depth : Nat) : tactic rcases_patt :=
  do 
    let e ← i_to_expr p 
...
* `rcases? ⟨e1, e2, e3⟩ : n` can be used to control the depth of case splits
  ...
  times as you like). -/
unsafe def rcases_hint_many (ps : List pexpr) (depth : Nat) : tactic (listΠ rcases_patt) :=
  do 
    let es ←
      ps.mmap
          fun p =>
            do 
              let e ← i_to_expr p 
              if e.is_local_constant then pure e else
                  do 
                    let x ← mk_fresh_name 
                    let n ← revert_kdependencies e semireducible 
                    tactic.generalize e x <|>
...
recursive types like `nat`, which can be cased as many times as you like). -/
unsafe def rintro_hint (depth : Nat) : tactic (listΠ rcases_patt) :=
  do 
    let l ← intros 
    focus1$
        do 
...
unsafe def rcases_parse_depth : parser Nat :=
  do 
    let o ← (tk ":" *> small_nat)?
...
unsafe def rcases_parse : parser rcases_args :=
  with_desc "('?' expr (: n)?) | ((h :)? expr (with patt)?)"$
    do 
      let hint ← (tk "?")?
          ...
          do 
            let p ←
              (do 
                  ...
                  pure (Sum.inr p)
            let ids ← (tk "with" *> rcases_patt_parse)?
  ...
  with_desc "patt? (: expr)? (:= expr)?"$
    do 
      ...
      Prod.mk (pat, tp) <$>
          (do 
              tk ":="
              guardₓ tp.is_none >> Sum.inr <$> brackets "⟨" "⟩" (sep_by (tk ",") (parser.pexpr 0)) <|>
                  Sum.inl <$> texpr)?
