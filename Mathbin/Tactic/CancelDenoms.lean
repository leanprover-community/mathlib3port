...
theorem neg_subst {α} [Ringₓ α] {n e t : α} (h1 : (n*e) = t) : (n*-e) = -t :=
  by 
    simp 
...
-/
unsafe def derive_div (e : expr) : tactic (ℕ × expr) :=
  do 
    let (n, p) ← derive e 
    let tp ← infer_type e 
    let n' ← tp.of_nat n 
...
-/
unsafe def cancel_denominators_in_type (h : expr) : tactic (expr × expr) :=
  do 
...
```
-/
unsafe def tactic.interactive.cancel_denoms (l : parse location) : tactic Unit :=
  do 
