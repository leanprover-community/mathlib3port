...
private unsafe def transform_negation_step (cfg : auto_config) (e : expr) : tactic (Option (expr × expr)) :=
  do 
...
private unsafe def transform_negation (cfg : auto_config) : expr → tactic (Option (expr × expr)) :=
  fun e =>
    do 
...
unsafe def normalize_negations (cfg : auto_config) (h : expr) : tactic Unit :=
  do 
    let t ← infer_type h 
    let (_, e, pr) ←
      simplify_top_down ()
...
unsafe def normalize_hyps (cfg : auto_config) : tactic Unit :=
  do 
...
unsafe def do_subst : tactic Unit :=
  do 
    let ctx ← local_context 
    first$
        ctx.map$
...
unsafe def add_conjuncts : expr → expr → tactic Bool :=
  fun pr t =>
...
unsafe def split_hyp (h : expr) : tactic Bool :=
  do 
    let t ← infer_type h 
...
/-- Eagerly apply all the preprocessing rules -/
unsafe def preprocess_hyps (cfg : auto_config) : tactic Unit :=
  do 
    repeat (intro1 >> skip)
    preprocess_goal 
    normalize_hyps cfg 
...
private unsafe def add_hinst_lemma_from_name (md : transparency) (lhs_lemma : Bool) (n : Name) (hs : hinst_lemmas)
  (ref : pexpr) : smt_tactic hinst_lemmas :=
  do 
...
-/
unsafe def done (ps : List pexpr) (cfg : auto_config := {  }) : tactic Unit :=
  do 
...
unsafe def case_hyp (h : expr) (s : case_option) (cont : case_option → tactic Unit) : tactic Bool :=
  do 
    let t ← infer_type h 
    match t with 
