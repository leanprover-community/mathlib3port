...
-/
unsafe def prove_reassoc (h : expr) : tactic (expr × expr) :=
  do 
...
-/
unsafe def reassoc_axiom (n : Name) (n' : Name := n.append_suffix "_assoc") : tactic Unit :=
  do 
    let d ← get_decl n 
...
unsafe def reassoc_attr : user_attribute Unit (Option Name) :=
  { Name := `reassoc, descr := "create a companion lemma for associativity-aware rewriting", Parser := optionalₓ ident,
    after_set :=
      some
        fun n _ _ =>
          do 
...
-/
@[user_command]
unsafe def reassoc_cmd (_ : parse$ tk "reassoc_axiom") : lean.parser Unit :=
  do 
    let n ← ident 
    of_tactic$
        do 
...
in this way.)
-/
unsafe def reassoc (del : parse (tk "!")?) (ns : parse (ident)*) : tactic Unit :=
  do 
    ns.mmap'
        fun n =>
          do 
