...
unsafe def library_defs (hs : name_set) : tactic (List decl_data) :=
  do 
...
unsafe def suggest_opt.mk_accept (o : suggest_opt) : opt :=
  { o with
    accept :=
      fun gs => o.accept gs >> (guardₓ$ o.compulsory_hyps.all fun h => gs.any fun g => g.contains_expr_or_mvar h) }
...
unsafe def apply_and_solve (close_goals : Bool) (opt : suggest_opt := {  }) (e : expr) : tactic ℕ :=
  do 
    ...
    let ng ← num_goals 
    try (guardₓ (opt.compulsory_hyps = []) >> any_goals (independent_goal >> solve_by_elim opt.to_opt))
    done >> return ng <|>
        do 
          (guardₓ (opt.compulsory_hyps ≠ []) <|> any_goals (success_if_fail independent_goal) >> skip) >>
                solve_by_elim { opt.mk_accept with backtrack_all_goals := tt } <|>
              guardₓ ¬close_goals 
          let ng' ← num_goals 
          return (ng - ng')
  ...
  retrieve$
    focus1$
      do 
        ...
        return { State := s, decl := d.d, script := m, num_goals := ng, hyps_used := hyps.filter fun h => h.occurs g }
...
-/
unsafe def suggest (limit : Option ℕ := none) (opt : suggest_opt := {  }) : tactic (List application) :=
  do 
    let results := suggest_core opt 
    let L ← if h : limit.is_some then results.take (Option.get h) else results.force 
    return$
        L.qsort
          fun d₁ d₂ =>
...
from the library.
-/
unsafe def suggest_scripts (limit : Option ℕ := none) (opt : suggest_opt := {  }) : tactic (List Stringₓ) :=
  do 
...
unsafe def library_search (opt : suggest_opt := {  }) : tactic Stringₓ :=
  (suggest_core opt).mfirst
    fun a =>
      do 
        guardₓ (a.num_goals = 0)
        write a.state 
        return a.script
...
unsafe def suggest (n : parse (with_desc "n" small_nat)?) (hs : parse simp_arg_list)
  ...
  do 
    ...
    let L ←
      tactic.suggest_scripts (n.get_or_else 50)
          { opt with compulsory_hyps := use, lemma_thunks := some lemma_thunks, ctx_thunk }
    if !opt.try_this || is_trace_enabled_for `silence_suggest then skip else
        if L.length = 0 then fail "There are no applicable declarations" else L.mmap trace >> skip
  ...
  do 
    ...
    (tactic.library_search
            { opt with compulsory_hyps := use, backtrack_all_goals := tt, lemma_thunks := some lemma_thunks, ctx_thunk,
              md :=
                if semireducible.is_some then Tactic.Transparency.semireducible else Tactic.Transparency.reducible } >>=
          if !opt.try_this || is_trace_enabled_for `silence_library_search then fun _ => skip else trace) <|>
        fail
