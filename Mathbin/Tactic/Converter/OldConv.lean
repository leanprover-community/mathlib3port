...
unsafe def lhs : old_conv expr :=
  fun r e => return ⟨e, e, none⟩

unsafe def change (new_p : pexpr) : old_conv Unit :=
  fun r e =>
    do 
...
protected unsafe def pure {α : Type} : α → old_conv α :=
  fun a r e => return ⟨a, e, none⟩
...
protected unsafe def fail {α β : Type} [has_to_format β] (msg : β) : old_conv α :=
  fun r e => tactic.fail msg

protected unsafe def failed {α : Type} : old_conv α :=
  fun r e => tactic.failed

protected unsafe def orelse {α : Type} (c₁ : old_conv α) (c₂ : old_conv α) : old_conv α :=
  fun r e => c₁ r e <|> c₂ r e

protected unsafe def map {α β : Type} (f : α → β) (c : old_conv α) : old_conv β :=
  fun r e =>
    do 
      let ⟨a, e₁, pr⟩ ← c r e 
      return ⟨f a, e₁, pr⟩
...
unsafe def trace {α : Type} [has_to_tactic_format α] (a : α) : old_conv Unit :=
  fun r e => tactic.trace a >> return ⟨(), e, none⟩

unsafe def trace_lhs : old_conv Unit :=
  lhs >>= trace

unsafe def apply_lemmas_core (s : simp_lemmas) (prove : tactic Unit) : old_conv Unit :=
  fun r e =>
    do 
...
private unsafe def mk_refl_proof (r : Name) (e : expr) : tactic expr :=
  do 
    let env ← get_env 
...
unsafe def to_tactic (c : old_conv Unit) : Name → expr → tactic (expr × expr) :=
  fun r e =>
    do 
      let ⟨u, e₁, o⟩ ← c r e 
      match o with 
...
unsafe def lift_tactic {α : Type} (t : tactic α) : old_conv α :=
  fun r e =>
    do 
      let a ← t 
      return ⟨a, e, none⟩
...
unsafe def match_pattern (p : pattern) : old_conv Unit :=
  fun r e => tactic.match_pattern p e >> return ⟨(), e, none⟩

unsafe def mk_match_expr (p : pexpr) : tactic (old_conv Unit) :=
  do 
    let new_p ← pexpr_to_pattern p 
    return fun r e => tactic.match_pattern new_p e >> return ⟨(), e, none⟩

unsafe def match_expr (p : pexpr) : old_conv Unit :=
  fun r e =>
    do 
      let new_p ← pexpr_to_pattern p 
      tactic.match_pattern new_p e >> return ⟨(), e, none⟩

unsafe def funext (c : old_conv Unit) : old_conv Unit :=
  fun r lhs =>
    do 
      guardₓ (r = `eq)
...
unsafe def congr_core (c_f c_a : old_conv Unit) : old_conv Unit :=
  fun r lhs =>
    do 
      guardₓ (r = `eq)
...
unsafe def bottom_up (c : old_conv Unit) : old_conv Unit :=
  fun r e =>
    do 
      let s ← simp_lemmas.mk_default 
      let (a, new_e, pr) ←
...
unsafe def top_down (c : old_conv Unit) : old_conv Unit :=
  fun r e =>
    do 
      let s ← simp_lemmas.mk_default 
      let (a, new_e, pr) ←
...
unsafe def find (c : old_conv Unit) : old_conv Unit :=
  fun r e =>
    do 
      let s ← simp_lemmas.mk_default 
      let (a, new_e, pr) ←
...
unsafe def find_pattern (pat : pattern) (c : old_conv Unit) : old_conv Unit :=
  fun r e =>
    do 
      let s ← simp_lemmas.mk_default 
      let (a, new_e, pr) ←
...
unsafe def findp : pexpr → old_conv Unit → old_conv Unit :=
  fun p c r e =>
    do 
      let pat ← pexpr_to_pattern p 
      find_pattern pat c r e

unsafe def conversion (c : old_conv Unit) : tactic Unit :=
  do 
