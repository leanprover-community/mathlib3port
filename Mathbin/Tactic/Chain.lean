...
unsafe def tactic_script.to_string : tactic_script Stringₓ → Stringₓ
...
unsafe def abstract_if_success (tac : expr → tactic α) (g : expr) : tactic α :=
  do 
    let type ← infer_type g 
    let is_lemma ← is_prop type 
    if is_lemma then tac g else
        do 
          let m ← mk_meta_var type 
          let a ← tac m
          (do 
                let val ← instantiate_mvars m 
                guardₓ (val.list_meta_vars = [])
...
unsafe def chain_core {α : Type} [HasToString (tactic_script α)] (tactics : List (tactic α)) : tactic (List Stringₓ) :=
  do 
...
unsafe def trace_output (t : tactic α) : tactic α :=
  do 
    let tgt ← target 
    let r ← t 
    let name ← decl_name 
