...
an auto_param.
-/
unsafe def restate_axiom (d : declaration) (new_name : Name) : tactic Unit :=
  do 
    ...
    let new_decl :=
      if prop then declaration.thm new_name levels new_type (task.pure value) else
...
-/
@[user_command]
unsafe def restate_axiom_cmd (_ : parse$ tk "restate_axiom") : lean.parser Unit :=
  do 
