...
#align tactic.eliminate.constructor_argument_info tactic.eliminate.constructor_argument_info
...
unsafe structure constructor_info where
  ...
  num_rec_args : ℕ
  deriving has_reflect
#align tactic.eliminate.constructor_info tactic.eliminate.constructor_info
...
unsafe structure inductive_info where
  ...
  num_indices : ℕ
  deriving has_reflect
#align tactic.eliminate.inductive_info tactic.eliminate.inductive_info
...
unsafe structure major_premise_info where
  ...
  type : expr
  args : rb_map ℕ expr
#align tactic.eliminate.major_premise_info tactic.eliminate.major_premise_info
...
#align tactic.eliminate.get_constructor_argument_info tactic.eliminate.get_constructor_argument_info
...
-/
unsafe def get_constructor_info (iname : Name) (num_params : ℕ) (c : Name) :
...
list.
-/
unsafe def constructor_argument_names (info : constructor_argument_naming_info) :
    tactic (List Name) :=
...
#align tactic.eliminate.constructor_argument_names tactic.eliminate.constructor_argument_names
...
#align tactic.eliminate.intron_fresh tactic.eliminate.intron_fresh
...
unsafe def constructor_intros (generate_induction_hyps : Bool) (cinfo : constructor_info) :
    tactic
      (List (Name × constructor_argument_info) × List (Name × Name × constructor_argument_info)) :=
  do
  ...
  pure (args, ihs)
#align tactic.eliminate.constructor_intros tactic.eliminate.constructor_intros

/-- `ih_name arg_name` is the name `ih_<arg_name>`.
-/
unsafe def ih_name (arg_name : Name) : Name :=
  mkSimpleName ("ih_" ++ arg_name.toString)
#align tactic.eliminate.ih_name tactic.eliminate.ih_name
...
unsafe inductive with_pattern
  ...
  | exact (n : Name)
  deriving has_reflect
#align tactic.eliminate.with_pattern tactic.eliminate.with_pattern
...
protected unsafe def parser : lean.parser with_pattern :=
  tk "-" *> pure with_pattern.clear <|>
    tk "_" *> pure with_pattern.auto <|> with_pattern.exact <$> ident
#align tactic.eliminate.with_pattern.parser tactic.eliminate.with_pattern.parser

/-- Parser for a `with` clause. -/
unsafe def clause_parser : lean.parser (List with_pattern) :=
  tk "with" *> many with_pattern.parser <|> pure []
#align tactic.eliminate.with_pattern.clause_parser tactic.eliminate.with_pattern.clause_parser
...
-/
unsafe def to_name_spec (auto_candidates : tactic (List Name)) :
    with_pattern → tactic (Option (Sum Name (List Name)))
...
-/
unsafe def constructor_renames (generate_induction_hyps : Bool) (mpinfo : major_premise_info)
    (iinfo : inductive_info) (cinfo : constructor_info) (with_patterns : List with_pattern)
  ...
  let new_ih_hyps :=
    ih_hyp_map.filterMap fun ⟨old, new⟩ =>
...
inductive GeneralizationMode
  ...
  deriving has_reflect
#align tactic.eliminate.generalization_mode Tactic.Eliminate.GeneralizationMode
...
unsafe def generalize_hyps (major_premise : expr) (gm : GeneralizationMode) : tactic ℕ := do
  ...
  pure n
#align tactic.eliminate.generalize_hyps tactic.eliminate.generalize_hyps
...
- The number of hypotheses which were reverted because they contain complex
  indices.
-/
unsafe def generalize_complex_index_args (major_premise : expr) (num_params : ℕ)
  ...
  focus1 do
    ...
    let ⟨major_premise_param_args, major_premise_index_args⟩ :=
      major_premise_args.splitAt num_params
...
#align tactic.eliminate.generalize_complex_index_args tactic.eliminate.generalize_complex_index_args
...
unsafe def process_index_equation : expr → tactic (expr × Option expr)
  ...
  h@(local_const uname ppname binfo
        T@(app (app (app (app (const `heq [u]) lhs_type) lhs) rhs_type) rhs)) =>
    do
  ...
  | e =>
    throwError "process_index_equation: expected a local constant, but got:
      {← e}"
#align tactic.eliminate.process_index_equation tactic.eliminate.process_index_equation
...
-/
unsafe def assign_local_to_unassigned_mvar (mv : expr) (pp_name : Name) (binfo : BinderInfo) :
    tactic (Option expr) := do
  let ff ← is_assigned mv |
    pure none
...
-/
unsafe def simplify_ih (num_leading_pis : ℕ) (num_generalized : ℕ) (num_index_vars : ℕ)
    (ih : expr) : tactic expr := do
  let T ← infer_type ih
  ...
  clear ih
  pure ih'
#align tactic.eliminate.simplify_ih tactic.eliminate.simplify_ih
...
unsafe def eliminate_hyp (generate_ihs : Bool) (major_premise : expr)
    (gm := GeneralizationMode.generalize_all_except []) (with_patterns : List with_pattern := []) :
    tactic Unit :=
  focus1 do
    ...
    interactive.apply (rec_app rec_suffix) <|>
        interactive.apply (rec_app drec_suffix) <|>
          throwError
            "Failed to apply the (dependent) recursor for {(← iname)} on {← major_premise}."
    let-- Prepare the "with" names for each constructor case.
    with_patterns :=
      Prod.fst <|
        with_patterns.takeList (iinfo.constructors.map constructor_info.num_nameable_hypotheses)
    ...
    pure ()
#align tactic.eliminate_hyp tactic.eliminate_hyp
...
-/
unsafe def eliminate_expr (generate_induction_hyps : Bool) (major_premise : expr)
    (eq_name : Option Name := none) (gm := GeneralizationMode.generalize_all_except [])
...
#align tactic.eliminate_expr tactic.eliminate_expr
...
```
-/
unsafe def cases' (major_premise : parse cases_arg_p)
    (with_patterns : parse with_pattern.clause_parser) : tactic Unit := do
  let ⟨eq_name, e⟩ := major_premise
  let e ← to_expr e
