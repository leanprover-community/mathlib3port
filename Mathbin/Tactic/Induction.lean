...
unsafe structure constructor_argument_info where 
  ...
  recursive_leading_pis : Option ℕ deriving [anonymous]
...
unsafe structure constructor_info where 
  ...
  num_rec_args : ℕ deriving [anonymous]
...
unsafe structure inductive_info where 
  ...
  type : expr 
  num_params : ℕ 
  num_indices : ℕ deriving [anonymous]
...
unsafe inductive with_pattern
  | auto
  | clear
  | exact (n : Name)deriving [anonymous]
...
inductive generalization_mode
  ...
  | generalize_only (hs : List Name) : generalization_mode deriving [anonymous]
