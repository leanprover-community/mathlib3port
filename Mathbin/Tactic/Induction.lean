...
-/
unsafe def trace_state_eliminate_hyp {α} [has_to_format α] (msg : Thunkₓ α) : tactic Unit :=
  do 
    let state ← read 
...
meta
structure constructor_argument_info :=
  (aname : name)
  (type : expr)
  (dependent : bool)
  (index_occurrences : list exprℕ())
  (recursive_leading_pis : option exprℕ())
...
@[derive #[expr has_reflect]]
meta
structure constructor_info :=
  (cname : name)
  (non_param_args : list constructor_argument_info)
  (num_non_param_args : exprℕ())
...
@[derive #[expr has_reflect]]
meta
structure inductive_info :=
  (iname : name)
  (constructors : list constructor_info)
  (num_constructors : exprℕ())
  (type : expr)
  (num_params : exprℕ())
  (num_indices : exprℕ())
...
-/
unsafe def get_index_occurrences (num_params : ℕ) (ret_type : expr) : tactic (rb_lmap expr ℕ) :=
  do 
    let ret_args ← get_app_args_whnf ret_type 
    ret_args.mfoldl_with_index
        (fun i occ_map ret_arg =>
          do 
            if i < num_params then pure occ_map else
                do 
                  let ret_arg_consts := ret_arg.list_local_consts' 
                  ret_arg_consts.mfold occ_map$
                      fun c occ_map =>
                        do 
                          let ret_arg_type ← infer_type ret_arg 
                          let eq ← index_occurrence_type_match c.local_type ret_arg_type 
...
-/
unsafe def match_recursive_constructor_arg (I : Name) (T : expr) : tactic (Option ℕ) :=
  do 
...
unsafe def get_constructor_argument_info (inductive_name : Name) (num_params : ℕ) (T : expr) :
  tactic (List constructor_argument_info) :=
  do 
...
-/
unsafe def get_constructor_info (iname : Name) (num_params : ℕ) (c : Name) : tactic constructor_info :=
  do 
    let env ← get_env 
    when ¬env.is_constructor c$ throwError "Expected { ← c } to be a constructor."
    let decl ← env.get c 
...
-/
unsafe def get_inductive_info (I : Name) : tactic inductive_info :=
  do 
    let env ← get_env 
    when ¬env.is_inductive I$ throwError "Expected { ← I } to be an inductive type."
    let decl ← env.get I 
    let type := decl.type 
...
hypothesis.
-/
unsafe def get_major_premise_info (major_premise : expr) : tactic major_premise_info :=
  do 
    let type ← infer_type major_premise 
    let ⟨f, args⟩ ← get_app_fn_args_whnf type 
    pure { mpname := major_premise.local_pp_name, mpexpr := major_premise, type, args := args.to_rb_map }
...
-/
unsafe def constructor_argument_naming_rule_rec : constructor_argument_naming_rule :=
  fun i => pure$ if i.ainfo.is_recursive then [i.mpinfo.mpname] else []

/--
Naming rule for constructor arguments associated with an index.
-/
unsafe def constructor_argument_naming_rule_index : constructor_argument_naming_rule :=
  fun i =>
    let index_occs := i.ainfo.index_occurrences 
    let major_premise_args := i.mpinfo.args 
    let get_major_premise_arg_local_names : ℕ → Option (Name × Name) :=
      fun i =>
        do 
...
declaration.
-/
unsafe def constructor_argument_naming_rule_named : constructor_argument_naming_rule :=
  fun i =>
...
-/
unsafe def constructor_argument_naming_rule_type : constructor_argument_naming_rule :=
  fun i => typical_variable_names i.ainfo.type <|> pure []

/--
Naming rule for constructor arguments whose type is in `Prop`.
-/
unsafe def constructor_argument_naming_rule_prop : constructor_argument_naming_rule :=
  fun i =>
    do 
...
-/
unsafe def constructor_argument_naming_rule_fallback : constructor_argument_naming_rule :=
  fun _ => pure [`x]
...
unsafe def constructor_intros (generate_induction_hyps : Bool) (cinfo : constructor_info) :
  tactic (List (Name × constructor_argument_info) × List (Name × Name × constructor_argument_info)) :=
  do 
...
-/ @[derive #[expr has_reflect]] meta inductive with_pattern
| auto
| clear
| exact (n : name)
...
-/
unsafe def clear_dependent_if_exists (h : Name) : tactic Unit :=
  do 
...
unsafe def constructor_renames (generate_induction_hyps : Bool) (mpinfo : major_premise_info) (iinfo : inductive_info)
  ...
  (ihs : List (Name × Name × constructor_argument_info)) : tactic (List expr × List expr) :=
  do 
    let arg_pp_name_set := name_set.of_list$ args.map Prod.fst 
    let iname := iinfo.iname 
    let ⟨args, with_patterns⟩ := args.map₂_left' (fun arg p => (arg, p.get_or_else with_pattern.auto)) with_patterns 
    let arg_renames ←
      args.mmap_filter$
          fun ⟨⟨old_ppname, ainfo⟩, with_pat⟩ =>
            do 
              ...
              let some old ← try_core$ get_local old_ppname | pure none 
    ...
    let new_arg_hyps :=
      arg_hyp_map.filter_map$ fun ⟨old, new⟩ => if arg_pp_name_set.contains old.local_pp_name then some new else none 
    let arg_hyp_map : name_map expr := rb_map.of_list$ arg_hyp_map.map$ fun ⟨old, new⟩ => (old.local_pp_name, new)
    let tt ← pure generate_induction_hyps | pure (new_arg_hyps, [])
    let ih_pp_name_set := name_set.of_list$ ihs.map Prod.fst 
    let ihs := ihs.map₂_left (fun ih p => (ih, p.get_or_else with_pattern.auto)) with_patterns 
    let single_ih := ihs.length = 1
    let ih_renames ←
      ihs.mmap_filter$
          fun ⟨⟨ih_hyp_ppname, arg_hyp_ppname, _⟩, with_pat⟩ =>
            do 
              ...
              let some new ←
                with_pat.to_name_spec
                    (pure$
                      if single_ih then [`ih, ih_name arg_hyp.local_pp_name] else [ih_name arg_hyp.local_pp_name]) |
    ...
    let new_ih_hyps :=
      ih_hyp_map.filter_map$ fun ⟨old, new⟩ => if ih_pp_name_set.contains old.local_pp_name then some new else none 
...
- `generalize_only hs` means that only the `hs` are generalised. Exception:
  ...
  not appear in `hs`.
-/ @[derive #[expr has_reflect]] inductive generalization_mode
...
-/
unsafe def to_generalize (major_premise : expr) : generalization_mode → tactic name_set
| generalize_only ns =>
  do 
  ...
  do 
    ...
    let to_revert ←
      ctx.mmap_filter$
          fun h =>
            do 
              let h_depends_on_major_premise_deps ← hyp_depends_on_local_name_set h major_premise_dependencies 
...
-/
unsafe def generalize_hyps (major_premise : expr) (gm : generalization_mode) : tactic ℕ :=
  do 
...
unsafe def generalize_complex_index_args (major_premise : expr) (num_params : ℕ) (generate_induction_hyps : Bool) :
  ...
  focus1$
    do 
      ...
      let ⟨major_premise_param_args, major_premise_index_args⟩ := major_premise_args.split_at num_params 
        ...
        ctx.mfilter$
            fun h =>
              do 
                ...
                pure$ dep_on_major_premise ∧ h ≠ major_premise ∨ dep_of_index ∧ ¬dep_of_major_premise 
      ...
      let ks ← js.mfoldr go []
      let js_ks := js.zip ks 
      let new_ctx ← relevant_ctx.mmap$ fun h => js_ks.mfoldr (fun ⟨j, k⟩ h => kreplace h j k) h 
      let new_major_premise_type := major_premise_head.mk_app (major_premise_param_args ++ ks)
      ...
      let new_tgt ← js_ks.mfoldr (fun ⟨j, k⟩ tgt => kreplace tgt j k) tgt 
...
| h@(local_const _ ppname binfo T@(app (app (app (const `eq [u]) type) lhs) rhs)) =>
  do 
    let rhs_eq_lhs ← succeeds$ unify rhs lhs 
    if rhs_eq_lhs then pure ((const `eq.refl [u]) type lhs, none) else
        do 
          pure (h, some h)
| h@(local_const uname ppname binfo T@(app (app (app (app (const `heq [u]) lhs_type) lhs) rhs_type) rhs)) =>
  do 
    let lhs_type_eq_rhs_type ← succeeds$ is_def_eq lhs_type rhs_type 
    if ¬lhs_type_eq_rhs_type then
        do 
          pure (h, some h)
      else
        do 
          let lhs_eq_rhs ← succeeds$ unify rhs lhs 
          if lhs_eq_rhs then pure ((const `heq.refl [u]) lhs_type lhs, none) else
              do 
                let c ← mk_local' ppname binfo$ (const `eq [u]) lhs_type lhs rhs 
                let arg := (const `heq_of_eq [u]) lhs_type lhs rhs c 
                pure (arg, some c)
| local_const _ _ _ T =>
  throwError "process_index_equation: expected a homogeneous or heterogeneous equation, but got:
    { ← T}"
| e =>
...
-/
unsafe def simplify_ih (num_leading_pis : ℕ) (num_generalized : ℕ) (num_index_vars : ℕ) (ih : expr) : tactic expr :=
  do 
    let T ← infer_type ih 
    ...
    let new_generalized_arg_lcs ← assign_locals_to_unassigned_mvars generalized_arg_mvars 
    let new_generalized_arg_lcs ← new_generalized_arg_lcs.mmap instantiate_mvars 
    let new_index_eq_lcs ← new_index_eq_lcs.mmap instantiate_mvars 
    let b ← instantiate_mvars$ ih.mk_app (generalized_arg_mvars.map Prod.fst ++ new_args)
    let new_ih ← lambdas (new_generalized_arg_lcs ++ new_index_eq_lcs) b 
    type_check new_ih <|>
        throwError "internal error in simplify_ih: constructed term does not type check:
          { ← new_ih }"
    let ih' ← note ih.local_pp_name none new_ih 
    clear ih 
    pure ih'
...
-/
unsafe def set_cases_tags (in_tag : tag) (rs : List (Name × List expr)) : tactic Unit :=
  do 
    let gs ← get_goals 
    match gs with 
      | [g] => set_tag g in_tag
...
unsafe def eliminate_hyp (generate_ihs : Bool) (major_premise : expr)
  (gm := generalization_mode.generalize_all_except []) (with_patterns : List with_pattern := []) : tactic Unit :=
  focus1$
    do 
      ...
      let env ← get_env 
      let iname ←
        get_app_fn_const_whnf major_premise_type <|>
            throwError "The type of {( ← major_premise)} should be an inductive type, but it is
              { ← major_premise_type }"
      let iinfo ← get_inductive_info iname 
      trace_state_eliminate_hyp "State before complex index generalisation:"
      let (major_premise, num_index_vars, index_var_names, num_index_generalized) ←
      ...
      let with_patterns :=
        Prod.fst$ with_patterns.take_list (iinfo.constructors.map constructor_info.num_nameable_hypotheses)
      let constrs := iinfo.constructors.zip with_patterns 
      let cases : List (Option (Name × List expr)) ←
        focus$
            constrs.map$
              fun ⟨cinfo, with_patterns⟩ =>
                do 
                  trace_eliminate_hyp "============"
                  trace_eliminate_hyp$ f! "Case {cinfo.cname }"
                  ...
                  let (constructor_args, ihs) ← constructor_intros generate_ihs cinfo 
                  ...
                  let ff ← unify_equations index_equations |
                  ...
                  ihs.mmap'$
                      fun ⟨ih, _, arg_info⟩ =>
                        do 
                          let ih ← get_local ih 
                          let some num_leading_pis ← pure arg_info.recursive_leading_pis |
                            throwError "eliminate_hyp: internal error: unexpected non-recursive argument info"
  ...
  do 
    ...
    let hyp ←
      match eq_name with 
        ...
        | none =>
          do 
            if major_premise.is_local_constant then pure major_premise else
                do 
                  let x ← get_unused_name `x 
...
unsafe def induction' (major_premise : parse cases_arg_p) (gm : parse generalisation_mode_parser)
  (with_patterns : parse with_pattern.clause_parser) : tactic Unit :=
  do 
