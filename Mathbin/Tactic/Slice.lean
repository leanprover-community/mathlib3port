...
unsafe def repeat_count {α : Type} (t : tactic α) : tactic ℕ :=
  do 
...
unsafe def slice_lhs (a b : ℕ) (t : conv Unit) : tactic Unit :=
  do 
    conv.interactive.to_lhs 
    slice a b 
    t

unsafe def slice_rhs (a b : ℕ) (t : conv Unit) : tactic Unit :=
  do 
...
-/
unsafe def slice_lhs (a b : parse small_nat) (t : conv.interactive.itactic) : tactic Unit :=
  do 
...
composition as needed, zooms in on the `a`-th through `b`-th morphisms, and invokes `tac`.
-/
unsafe def slice_rhs (a b : parse small_nat) (t : conv.interactive.itactic) : tactic Unit :=
  do 
