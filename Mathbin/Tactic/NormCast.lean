...
private unsafe def count_internal_coes (e : expr) : tactic ℕ :=
  do 
...
-/
unsafe def classify_type (ty : expr) : tactic label :=
  do 
    ...
    if lhs_head_coes = 0 then return elim else
        if lhs_head_coes = 1 then
          do 
...
unsafe def add_elim (cache : norm_cast_cache) (e : expr) : tactic norm_cast_cache :=
  do 
    let new_up ← simp_lemmas.add cache.up e 
    return { up := new_up, down := cache.down, squash := cache.squash }

/-- `add_move cache e` adds `e` as a `move` lemma to `cache`. -/
unsafe def add_move (cache : norm_cast_cache) (e : expr) : tactic norm_cast_cache :=
  do 
    let ty ← infer_type e 
    let new_up ← cache.up.add e tt 
    let new_down ← simp_lemmas.add cache.down e 
    return { up := new_up, down := new_down, squash := cache.squash }

/-- `add_squash cache e` adds `e` as an `squash` lemma to `cache`. -/
unsafe def add_squash (cache : norm_cast_cache) (e : expr) : tactic norm_cast_cache :=
  do 
    let new_squash ← simp_lemmas.add cache.squash e 
    let new_down ← simp_lemmas.add cache.down e 
...
-/
unsafe def get_label_param (attr : norm_cast_attr_ty) (decl : Name) : tactic (Option label) :=
  do 
...
-/
unsafe def add_lemma (attr : norm_cast_attr_ty) (cache : norm_cast_cache) (decl : Name) : tactic norm_cast_cache :=
  do 
    let e ← mk_const decl 
...
-/
unsafe def mk_cache (attr : Thunkₓ norm_cast_attr_ty) (names : List Name) : tactic norm_cast_cache :=
  do 
...
unsafe def make_guess (decl : Name) : tactic label :=
  do 
    let e ← mk_const decl 
    let ty ← infer_type e 
    classify_type ty
...
-/
unsafe def get_label (decl : Name) : tactic label :=
  do 
    let param ← get_label_param norm_cast_attr decl 
...
unsafe def prove_eq_using (s : simp_lemmas) (a b : expr) : tactic expr :=
  do 
...
unsafe def exact_mod_cast (e : expr) : tactic Unit :=
  decorate_error "exact_mod_cast failed:"$
    do 
...
unsafe def apply_mod_cast (e : expr) : tactic (List (Name × expr)) :=
  decorate_error "apply_mod_cast failed:"$
    do 
      let new_e ← aux_mod_cast e 
      apply new_e
...
unsafe def assumption_mod_cast : tactic Unit :=
  decorate_error "assumption_mod_cast failed:"$
    do 
...
-/
unsafe def norm_cast (loc : parse location) : tactic Unit :=
  do 
...
-/
unsafe def exact_mod_cast (e : parse texpr) : tactic Unit :=
  do 
    let e ←
      i_to_expr e <|>
          do 
...
-/
unsafe def apply_mod_cast (e : parse texpr) : tactic Unit :=
  do 
...
attribute [normCast] Int.nat_abs_of_nat Int.coe_nat_subₓ Int.coe_nat_mul Int.coe_nat_zero Int.coe_nat_one
  Int.coe_nat_add

attribute [normCast] Int.coe_nat_succ
