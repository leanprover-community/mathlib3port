...
unsafe def get_open_namespaces (ns : Name) : tactic (List Name) :=
  do 
...
unsafe def build_str_open_namespaces (ns : Name) : tactic Stringₓ :=
  do 
...
unsafe def build_str_variables : lean.parser Stringₓ :=
  do 
    let l ← get_variables 
...
unsafe def build_str_includes : lean.parser Stringₓ :=
  do 
...
private unsafe def append_line (s : Stringₓ) (t : lean.parser Stringₓ) : lean.parser Stringₓ :=
  do 
    let v ← t 
    return$ s ++ v ++ if v.length = 0 then "" else "\n"

/-- `#where` output main function. -/
unsafe def build_msg : lean.parser Stringₓ :=
  do 
    let msg := ""
...
-/
@[user_command]
unsafe def where_cmd (_ : parse$ tk "#where") : lean.parser Unit :=
  do 
    let msg ← build_msg 
    trace msg
