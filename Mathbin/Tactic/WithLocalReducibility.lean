...
inductive decl_reducibility
  | reducible
  | semireducible
  | irreducible deriving [anonymous]
...
unsafe def get_decl_reducibility (n : Name) : tactic decl_reducibility :=
  do 
          ...
          if is_red then pure decl_reducibility.reducible else
              do 
                ...
                if e.contains n then pure decl_reducibility.semireducible else
                    fail f! "get_decl_reducibility: no declaration {n}"
...
unsafe def with_local_reducibility {α : Type _} (n : Name) (r : decl_reducibility) (body : tactic α) : tactic α :=
  do 
