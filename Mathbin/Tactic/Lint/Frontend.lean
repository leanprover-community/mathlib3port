...
unsafe def get_checks (slow : Bool) (extra : List Name) (use_only : Bool) : tactic (List (Name × linter)) :=
  do 
...
unsafe def sort_results {α} (e : environment) (results : rb_map Name α) : List (Name × α) :=
  List.reverse$
    rb_lmap.values$
      rb_lmap.of_list$
        results.fold []$
          fun decl linter_warning results =>
...
unsafe def grouped_by_filename (e : environment) (results : rb_map Name Stringₓ) (drop_fn_chars := 0)
  ...
  let results :=
    results.fold (rb_map.mk Stringₓ (rb_map Name Stringₓ))$
      fun decl_name linter_warning results =>
  ...
  let l :=
    results.to_list.reverse.map
      fun ⟨fn, results⟩ => ("-- " ++ fn.popn drop_fn_chars ++ "\n" ++ formatter results : format)
  format.intercalate "\n\n" l ++ "\n"
...
unsafe def format_linter_results (env : environment) (results : List (Name × linter × rb_map Name Stringₓ))
  ...
  (verbose : LintVerbosity) (num_linters : ℕ) : format :=
  do 
    let formatted_results :=
      results.map$
        fun ⟨linter_name, linter, results⟩ =>
    ...
    let s :=
      if verbose = LintVerbosity.low then s else
        (f!
            "/- Checking {non_auto_decls.length} declarations (plus {(decls.length -
...
unsafe def lint_aux (decls : List declaration) (group_by_filename : Option ℕ) (where_desc : Stringₓ) (slow : Bool)
  (verbose : LintVerbosity) (checks : List (Name × linter)) : tactic (name_set × format) :=
  do 
    let e ← get_env 
    let non_auto_decls := decls.filter fun d => ¬d.is_auto_or_internal e 
    let results ← lint_core decls non_auto_decls checks 
    let s :=
      format_linter_results e results decls non_auto_decls group_by_filename where_desc slow verbose checks.length 
    let ns :=
      name_set.of_list
        do 
          let (_, _, rs) ← results 
...
unsafe def lint (slow : Bool := tt) (verbose : LintVerbosity := LintVerbosity.medium) (extra : List Name := [])
  (use_only : Bool := ff) : tactic (name_set × format) :=
  do 
...
unsafe def lint_project_decls (proj_folder : Stringₓ) : tactic (List declaration) :=
  do 
    let e ← get_env 
...
-/
unsafe def lint_project (proj_folder proj_name : Stringₓ) (slow : Bool := tt)
  (verbose : LintVerbosity := LintVerbosity.medium) (extra : List Name := []) (use_only : Bool := ff) :
  tactic (name_set × format) :=
  do 
    let checks ← get_checks slow extra use_only 
...
unsafe def lint_cmd_aux (scope : Bool → LintVerbosity → List Name → Bool → tactic (name_set × format)) : parser Unit :=
  do 
    ...
    let verbosity ← if verbosity.is_some then return verbosity else parse_verbosity 
...
Use the command `#list_linters` to see all available linters. -/
@[user_command]
unsafe def lint_mathlib_cmd (_ : parse$ tk "#lint_mathlib") : parser Unit :=
  do 
...
@[user_command]
unsafe def list_linters (_ : parse$ tk "#list_linters") : parser Unit :=
  do 
    let env ← get_env 
    let ns :=
      env.decl_filter_map$
