  ...
  do 
    ...
    match ty with 
      ...
      | expr.pi n bi a b =>
        do 
          let l ← mk_local' n bi a 
          let some lhs ← simp_is_conditional_core (b.instantiate_var l) | pure none 
          if bi ≠ BinderInfo.inst_implicit ∧ ¬(lhs.abstract_local l.local_uniq_name).has_var then pure none else
              pure lhs
      | ty => pure ty
...
unsafe def simp_nf_linter (timeout := 200000) (d : declaration) : tactic (Option Stringₓ) :=
  do 
    let tt ← is_simp_lemma d.to_name | pure none 
    let tt ← is_valid_simp_lemma_cnst d.to_name | pure none 
    let [] ← get_eqn_lemmas_for ff d.to_name | pure none 
    try_for timeout$
        retrieve$
          do 
            let g ← mk_meta_var d.type 
            set_goals [g]
            unfreezing intros 
            ...
            if d.to_name ∈ prf1_lems then pure none else
                do 
                  let is_cond ← simp_is_conditional d.type 
                  let (rhs', prf2, ns2) ←
                  ...
                  if lhs'_eq_rhs' then
                      ...
                      if ¬lhs_in_nf then
                        do 
                          ...
                          pure$
                              format.to_string$
                                to_fmt "Left-hand side simplifies from" ++ lhs.group.indent 2 ++ format.line ++ "to" ++
                                            lhs'.group.indent 2 ++
                                          format.line ++
                                        "using " ++
...
@[linter]
unsafe def linter.simp_nf : linter :=
  { test := simp_nf_linter, auto_decls := tt,
    no_errors_found := "All left-hand sides of simp lemmas are in simp-normal form.",
    errors_found :=
      "SOME SIMP LEMMAS ARE NOT IN SIMP-NORMAL FORM.\nsee note [simp-normal form] for tips how to debug this.\nhttps://leanprover-community.github.io/mathlib_docs/notes.html#simp-normal%20form" }

private unsafe def simp_var_head (d : declaration) : tactic (Option Stringₓ) :=
  do 
    let tt ← is_simp_lemma d.to_name | pure none 
    let tt ← is_valid_simp_lemma_cnst d.to_name | pure none 
...
unsafe def linter.simp_var_head : linter :=
  { test := simp_var_head, auto_decls := tt,
...
private unsafe def simp_comm (d : declaration) : tactic (Option Stringₓ) :=
  do 
    let tt ← is_simp_lemma d.to_name | pure none 
    let tt ← is_valid_simp_lemma_cnst d.to_name | pure none 
    let (lhs, rhs) ← simp_lhs_rhs d.type 
    if lhs.get_app_fn.const_name ≠ rhs.get_app_fn.const_name then pure none else
        do 
          let (lhs', rhs') ← Prod.snd <$> open_pis_metas d.type >>= simp_lhs_rhs 
...
@[linter]
unsafe def linter.simp_comm : linter :=
  { test := simp_comm, auto_decls := tt, no_errors_found := "No commutativity lemma is marked simp.",
