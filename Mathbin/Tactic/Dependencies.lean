...
-/
unsafe def type_has_local_in_name_set (h : expr) (ns : name_set) : tactic Bool :=
  do 
...
`ns`.
-/
unsafe def local_def_value_has_local_in_name_set (h : expr) (ns : name_set) : tactic Bool :=
  do 
    let some h_val ← try_core$ local_def_value h | pure ff 
...
-/
unsafe def direct_dependency_set_of_hyp (h : expr) : tactic expr_set :=
  do 
    let t ← infer_type h 
...
-/
unsafe def direct_dependency_set_of_hyp_inclusive (h : expr) : tactic expr_set :=
  do 
...
unsafe def hyp_depends_on_local_name_set' : expr_set → expr → name_set → tactic (Bool × expr_set) :=
  fun cache h ns =>
    do 
      let ff ← pure$ cache.contains h | pure (ff, cache)
      let direct_deps ← direct_dependency_set_of_hyp h 
      let has_dep := direct_deps.fold ff fun d b => b || ns.contains d.local_uniq_name 
      let ff ← pure has_dep | pure (tt, cache)
      let (has_dep, cache) ←
        direct_deps.mfold (ff, cache)$
            fun d ⟨b, cache⟩ => if b then pure (tt, cache) else hyp_depends_on_local_name_set' cache d ns 
      if has_dep then pure (tt, cache) else pure (ff, cache.insert h)
...
-/
unsafe def hyp_depends_on_local_name_set (h : expr) (ns : name_set) : tactic Bool :=
  do 
    let ctx_has_local_def ← context_upto_hyp_has_local_def h 
    if ctx_has_local_def then Prod.fst <$> hyp_depends_on_local_name_set' mk_expr_set h ns else
...
every `h ∈ hs`.
-/
unsafe def hyps_depend_on_local_name_set (hs : List expr) (ns : name_set) : tactic (List Bool) :=
  do 
    ...
    if ctx_has_local then
      ...
      else hs.mmap$ fun h => hyp_directly_depends_on_local_name_set h ns
...
-/
unsafe def hyps_depend_on_local_name_set_inclusive (hs : List expr) (ns : name_set) : tactic (List Bool) :=
  do 
    let ctx_has_local ← context_has_local_def 
    if ctx_has_local then
        let go : expr → List Bool × expr_set → tactic (List Bool × expr_set) :=
          fun h ⟨deps, cache⟩ =>
            do 
              ...
              pure (h_dep :: deps, cache)
        Prod.fst <$> hs.mfoldr go ([], mk_expr_map)
      else hs.mmap$ fun h => hyp_directly_depends_on_local_name_set_inclusive h ns
...
-/
unsafe def dependency_set_of_hyp' : expr_map expr_set → expr → tactic (expr_set × expr_map expr_set) :=
  fun cache h =>
    do 
      match cache.find h with 
        ...
        | none =>
          do 
            let direct_deps ← direct_dependency_set_of_hyp h 
            let (deps, cache) ←
              direct_deps.mfold (direct_deps, cache)$
                  fun h' ⟨deps, cache⟩ =>
                    do 
...
-/
unsafe def dependency_set_of_hyp (h : expr) : tactic expr_set :=
  do 
...
-/
unsafe def dependency_sets_of_hyps (hs : List expr) : tactic (List expr_set) :=
  do 
    ...
    if ctx_has_def then
        ...
        Prod.fst <$> hs.mfoldr go ([], mk_expr_map)
      else hs.mmap direct_dependency_set_of_hyp
...
unsafe def dependency_set_of_hyp_inclusive' (cache : expr_map expr_set) (h : expr) :
  tactic (expr_set × expr_map expr_set) :=
  do 
    let (deps, cache) ← dependency_set_of_hyp' cache h 
...
-/
unsafe def dependency_set_of_hyp_inclusive (h : expr) : tactic expr_set :=
  do 
...
-/
unsafe def dependency_sets_of_hyps_inclusive (hs : List expr) : tactic (List expr_set) :=
  do 
    let ctx_has_def ← context_has_local_def 
    if ctx_has_def then
        let go : expr → List expr_set × expr_map expr_set → tactic (List expr_set × expr_map expr_set) :=
          fun h ⟨deps, cache⟩ =>
            do 
              ...
              pure (h_deps :: deps, cache)
        Prod.fst <$> hs.mfoldr go ([], mk_expr_map)
      else hs.mmap direct_dependency_set_of_hyp_inclusive
...
| H :: Hs, revdeps, ns =>
  do 
    ...
    if H_is_revdep then reverse_dependencies_of_hyp_name_set_aux Hs (H :: revdeps) (ns.insert H_uname) else
...
context.
-/
unsafe def reverse_dependencies_of_hyp_name_set (hs : name_set) : tactic (List expr) :=
  do 
...
| H :: Hs, revdeps, ns =>
  do 
    ...
    if H_is_revdep then reverse_dependencies_of_hyp_name_set_inclusive_aux Hs (H :: revdeps) (ns.insert H_uname) else
...
-/
unsafe def reverse_dependencies_of_hyp_name_set_inclusive (hs : name_set) : tactic (List expr) :=
  do 
    let ctx ← local_context 
    let ctx := ctx.drop_while fun h => ¬hs.contains h.local_uniq_name 
...
-/
unsafe def revert_name_set (hs : name_set) : tactic (ℕ × List expr) :=
  do 
