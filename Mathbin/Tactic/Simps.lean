...
unsafe instance  : has_to_tactic_format projection_data :=
  ⟨fun ⟨a, b, c, d, e⟩ =>
      (fun x =>
          group$
            nest 1$
              to_fmt "⟨" ++ to_fmt a ++ to_fmt "," ++ line ++ x ++ to_fmt "," ++ line ++ to_fmt c ++ to_fmt "," ++
                          line ++
                        to_fmt d ++
                      to_fmt "," ++
                    line ++
...
attribute [notationClass] HasZero HasOne Add Mul HasInv Neg Sub Div HasDvd Mod LE LT Append HasAndthen HasUnion HasInter
  HasSdiff HasEquivₓ HasSubset HasSsubset HasEmptyc HasInsert HasSingleton HasSep HasMem Pow
...
| str, proj, x, Pos, args =>
  do 
    ...
    if proj_rest.is_empty then return (new_x.lambdas args, new_pos) else
        do 
...
unsafe def get_composite_of_projections (str : Name) (proj : Stringₓ) : tactic (expr × List ℕ) :=
  do 
    let e ← get_env 
    let str_d ← e.get str 
...
-/
@[user_command]
unsafe def initialize_simps_projections_cmd (_ : parse$ tk "initialize_simps_projections") : parser Unit :=
  do 
    let env ← get_env 
    let trc ← is_some <$> (tk "?")?
...
unsafe def simps_get_projection_exprs (e : environment) (tgt : expr) (rhs : expr) (cfg : SimpsCfg) :
  tactic$ List$ expr × projection_data :=
  do 
...
| e, nm, type, lhs, rhs, args, univs, must_be_str, cfg, todo, to_apply =>
  do 
    ...
    if e.is_structure str ∧ ¬(todo = [] ∧ str ∈ cfg.not_recursive ∧ ¬must_be_str) then
        do 
          ...
          let (rhs_ap, todo_now) ←
            if ¬is_constant_of rhs_ap.get_app_fn intro ∧ is_constant_of rhs_whnf.get_app_fn intro then
                when ("" ∈ todo ∧ to_apply = [])
                    (if cfg.fully_applied then simps_add_projection nm tgt lhs_ap rhs_ap new_args univs cfg else
                      simps_add_projection nm type lhs rhs args univs cfg) >>
                  return (rhs_whnf, ff)
              else return (rhs_ap, "" ∈ todo ∧ to_apply = [])
          if is_constant_of (get_app_fn rhs_ap) intro then
              do 
                ...
                when ¬(to_apply ≠ [] ∨ todo = [""] ∨ eta.is_some ∧ todo = [])$
                    do 
                              ...
                              when (is_default ∧ todo = [] ∨ new_todo ≠ [])$
                                  do 
                                    ...
                                    let new_cfg :=
                                      { cfg with
                                        addAdditive :=
                                          cfg.add_additive.map$
                                            fun nm => nm.append_to_last (to_additive.guess_name proj.last) is_prefix }
                                    when_tracing `simps.debug
                                        ( ←
                                          do 
                                            dbg_trace "[simps] > Recursively add projections for:
                                                      >  { ← new_lhs}")
                                    simps_add_projections e new_nm new_type new_lhs new_rhs new_args univs ff new_cfg
                                        new_todo proj_nrs
            else
              if cfg.rhs_md = transparency.none ∧ (must_be_str ∨ todo_next ≠ [] ∨ to_apply ≠ []) then
                do 
                  when cfg.trace
                      ( ←
                        do 
...
unsafe def simps_tac (nm : Name) (cfg : SimpsCfg := {  }) (todo : List Stringₓ := []) (trc := ff) : tactic Unit :=
  do 
    let e ← get_env 
    let d ← e.get nm 
...
unsafe def simps_parser : parser (Bool × List Stringₓ × SimpsCfg) :=
  do 
    Prod.mk <$> is_some <$> (tk "?")? <*>
