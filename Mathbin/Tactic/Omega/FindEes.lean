...
@[reducible]
unsafe def eqelim :=
  StateTₓ ee_state tactic

unsafe def abort {α : Type} : eqelim α :=
  ⟨fun x => failed⟩

private unsafe def mk_eqelim_state (eqs les : List term) : tactic ee_state :=
...
unsafe def add_ee (e : ee) : eqelim Unit :=
  do 
    let es ← get_ees 
    set_ees (es ++ [e])
...
unsafe def head_eq : eqelim term :=
  do 
    let eqs ← get_eqs 
    match eqs with 
      | [] => abort
...
unsafe def ee_commit (t1 : eqelim α) (t2 : eqelim β) (t3 : α → eqelim β) : eqelim β :=
  do 
...
| [] => 0
| i :: is => Nat.gcdₓ i.nat_abs (gcd is)
...
| i :: is =>
  (do 
      ...
      if i ≠ 0 ∧ i.nat_abs ≤ j.nat_abs then pure (i, 0) else pure (j, n+1)) <|>
...
unsafe def find_min_coeff (t : term) : eqelim (Int × Nat × term) :=
  do 
    let (i, n) ← find_min_coeff_core t.snd 
    if 0 < i then pure (i, n, t) else add_ee ee.neg >> pure (-i, n, t.neg)

/-- Find an appropriate equality elimination step for the
    current state and apply it. -/
unsafe def elim_eq : eqelim Unit :=
  do 
    let t ← head_eq 
    let i ← get_gcd t 
    factor i
          ...
          find_min_coeff s !>>=add_ee ee.drop;fun ⟨i, n, u⟩ =>
              if i = 1 then
                ...
                let v : term := coeffs_reduce n u.fst u.snd 
                let r : term := rhs n u.fst u.snd 
                do 
                  let eqs ← get_eqs 
                  let les ← get_les 
