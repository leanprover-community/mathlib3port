...
attribute [sugar_nat] Ne not_leₓ not_ltₓ Nat.lt_iff_add_one_le Nat.succ_eq_add_one or_falseₓ false_orₓ and_trueₓ
  true_andₓ Ge Gt mul_addₓ add_mulₓ mul_commₓ one_mulₓ mul_oneₓ imp_iff_not_or iff_iff_not_or_and_or_not

-- ././Mathport/Syntax/Translate/Basic.lean:686:4: warning: unsupported (TODO): `[tacs]
unsafe def desugar :=
  sorry

theorem univ_close_of_unsat_neg_elim_not m (p : preform) : (neg_elim (¬* p)).Unsat → univ_close p (fun _ => 0) m :=
  by 
    intro h1 
...
negation-free. -/
unsafe def prove_unsat_sub_free (p : preform) : tactic expr :=
  do 
    let x ← prove_neg_free p 
...
unsafe def intro_nats_core : tactic Unit :=
  do 
    let x ← target 
    match x with 
