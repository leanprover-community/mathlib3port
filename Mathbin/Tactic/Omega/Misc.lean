...
theorem fun_mono_2 {p : α → β → γ} {a1 a2 : α} {b1 b2 : β} : a1 = a2 → b1 = b2 → p a1 b1 = p a2 b2 :=
  fun h1 h2 =>
    by 
      rw [h1, h2]

theorem pred_mono_2 {p : α → β → Prop} {a1 a2 : α} {b1 b2 : β} : a1 = a2 → b1 = b2 → (p a1 b1 ↔ p a2 b2) :=
  fun h1 h2 =>
    by 
      rw [h1, h2]
...
theorem update_eq (m : Nat) (a : α) (v : Nat → α) : (v ⟨m ↦ a⟩) m = a :=
  by 
...
unsafe def intro_fresh : tactic Unit :=
  do 
    let n ← mk_fresh_name 
    intro n 
    skip
...
unsafe def revert_cond_all (t : expr → tactic Unit) : tactic Unit :=
  do 
