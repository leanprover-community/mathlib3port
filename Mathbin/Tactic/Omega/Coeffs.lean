...
theorem valBetween_eq_of_le {as : List Int} {l : Nat} :
  ...
  | m + 1, h1 => by
    ...
    have h2 : List.length as ≤ l + m := by
      rw [← Nat.lt_succ_iff]
      apply h1
    simpa [get_eq_default_of_le _ h2, zero_mul, add_zero, valBetween] using
...
theorem val_eq_of_le {as : List Int} {k : Nat} : as.length ≤ k → val v as = valBetween v as 0 k :=
  by
  intro h1; unfold val
  rw [valBetween_eq_of_le k _]; rfl
...
theorem valBetween_set {a : Int} {l n : Nat} :
  ...
  | m + 1, h1, h2 => by
    ...
    · have h3 : valBetween v ([] {l + m ↦ a}) l m = 0 :=
        by
        apply @eq.trans _ _ (valBetween v [] l m)
        · apply valBetween_eq_valBetween
          · intros
            rfl
          · intro x h4 h5
            rw [get_nil, get_set_eq_of_ne, get_nil]
            apply ne_of_lt h5
        apply valBetween_nil
      rw [h2]
...
theorem val_set {m : Nat} {a : Int} : val v ([] {m ↦ a}) = a * v m :=
  by
  apply valBetween_set (zero_le _)
...
theorem val_neg {as : List Int} : val v (neg as) = -val v as := by
  simpa only [val, length_neg] using valBetween_neg
...
theorem valBetween_add {is js : List Int} {l : Nat} :
  ...
  | 0 => rfl
  | m + 1 => by
    simp only [valBetween, val_between_add m, List.Func.get, get_add]
    ring
...
theorem val_add {is js : List Int} : val v (add is js) = val v is + val v js :=
  by
  unfold val
  rw [valBetween_add]; apply fun_mono_2 <;> apply valBetween_eq_of_le <;> rw [zero_add, length_add]
...
theorem valBetween_sub {is js : List Int} {l : Nat} :
  ...
  | 0 => rfl
  | m + 1 => by
    simp only [valBetween, val_between_sub m, List.Func.get, get_sub]
    ring
...
theorem val_sub {is js : List Int} : val v (sub is js) = val v is - val v js :=
  ...
  unfold val
  rw [valBetween_sub]
  apply fun_mono_2 <;> apply valBetween_eq_of_le <;> rw [zero_add, length_sub]
  apply le_max_left
  apply le_max_right
...
theorem valExcept_eq_valExcept {k : Nat} {is js : List Int} {v w : Nat → Int} :
  ...
  apply fun_mono_2
  ·
    apply valBetween_eq_valBetween <;> intro x h3 h4 <;> [· apply h1, · apply h2] <;>
          apply ne_of_lt <;>
        rw [zero_add] at h4 <;>
      apply h4
  · repeat' rw [← valBetween_eq_of_le (max is.length js.length - (k + 1))]
    ·
      apply valBetween_eq_valBetween <;> intro x h3 h4 <;> [· apply h1, · apply h2] <;>
              apply Ne.symm <;>
            apply ne_of_lt <;>
...
theorem valExcept_update_set {n : Nat} {as : List Int} {i j : Int} :
  ...
  apply valExcept_eq_valExcept update_eq_of_ne (get_set_eq_of_ne _)
...
theorem valBetween_add_valBetween {as : List Int} {l m : Nat} :
  ...
  | 0 => by simp only [valBetween, add_zero]
  | n + 1 => by
    rw [← add_assoc]
    unfold val_between
    rw [add_assoc]
...
theorem valExcept_add_eq (n : Nat) {as : List Int} : valExcept n v as + get n as * v n = val v as :=
  ...
  · have h4 := @val_between_add_val_between v as 0 (n + 1) (as.length - (n + 1))
    ...
    rw [h5] at h4
    apply Eq.trans _ h4
    simp only [valBetween, zero_add]
    ring
  have h2 : List.length as - (n + 1) = 0 := tsub_eq_zero_iff_le.mpr h1
  have h3 : valBetween v as 0 (List.length as) = valBetween v as 0 (n + 1) := by
    simpa only [val] using @val_eq_of_le v as (n + 1) h1
  simp only [add_zero, valBetween, zero_add, h2, h3]
...
theorem valBetween_map_mul {i : Int} {as : List Int} {l : Nat} :
  ...
  | 0 => by simp only [valBetween, mul_zero, List.map]
  | m + 1 => by
    unfold val_between
...
theorem valBetween_map_div {as : List Int} {i : Int} {l : Nat} (h1 : ∀ x ∈ as, i ∣ x) :
  ...
  | 0 => by simp only [Int.zero_div, valBetween, List.map]
  | m + 1 => by
    unfold val_between
...
theorem val_map_div {as : List Int} {i : Int} :
  ...
  by
  intro h1
  simpa only [val, List.length_map] using valBetween_map_div h1
...
theorem valBetween_eq_zero {is : List Int} {l : Nat} :
  ...
  | m + 1, h1 => by
    ...
    simpa only [valBetween, h2 (l + m), zero_mul, add_zero] using @val_between_eq_zero m h1
