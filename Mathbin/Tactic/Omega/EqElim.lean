...
theorem rhs_correct {v : Nat → Int} {b : Int} {as : List Int} (n : Nat) :
  ...
  have h2 : (m*sgm v b as n) = symmod b m+coeffs.val v (as.map fun x => symmod x m) := by
    ...
    apply dvd_zero
  apply
    calc v n = ((-m*sgm v b as n)+symmod b m)+coeffs.val_except n v (as.map fun x => symmod x m) := by
...
theorem coeffs_reduce_correct {v : Nat → Int} {b : Int} {as : List Int} {n : Nat} :
  ...
  have h4 : 0 = term.val (v ⟨n ↦ sgm v b as n⟩) (coeffs_reduce n b as)*m :=
    calc 0 = term.val v (b, as) := h2
      ...
      _ = ((-(m*a_n)*sgm v b as n)+m*sym_sym m b)+coeffs.val_except n v (as.map fun a_i => m*sym_sym m a_i) := by
      repeat'
        rw [add_assocₓ]
      apply fun_mono_2
      rfl
