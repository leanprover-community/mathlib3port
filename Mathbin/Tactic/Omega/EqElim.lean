...
theorem rhs_correct {v : Nat → Int} {b : Int} {as : List Int} (n : Nat) :
  ...
  by 
    ...
    ·
      ...
      have h4 : ∃ c, ((m*c)+symmod b (get n as+1)+coeffs.val v (as.map fun x : ℤ => symmod x m)) = term.val v (b, as)
      ·
      ...
      apply dvd_zero 
    apply
      calc v n = ((-m*sgm v b as n)+symmod b m)+coeffs.val_except n v (as.map fun x => symmod x m) :=
        by 
...
theorem coeffs_reduce_correct {v : Nat → Int} {b : Int} {as : List Int} {n : Nat} :
  0 < get n as → 0 = term.val v (b, as) → 0 = term.val (v ⟨n ↦ sgm v b as n⟩) (coeffs_reduce n b as) :=
  by 
    intro h1 h2 
    let a_n := get n as 
    let m := a_n+1
    ...
    have h4 : 0 = term.val (v ⟨n ↦ sgm v b as n⟩) (coeffs_reduce n b as)*m :=
      calc 0 = term.val v (b, as) := h2 
        ...
        _ =
          ((-(m*a_n)*sgm v b as
                    n)+b+a_n*symmod b
                    m)+coeffs.val_except n v as+a_n*coeffs.val_except n v (as.map fun x => symmod x m) :=
        by 
          ...
          simp only [m]
          ring 
        _ = ((-(m*a_n)*sgm v b as n)+b+a_n*symmod b m)+coeffs.val_except n v (as.map fun a_i => a_i+a_n*symmod a_i m) :=
        by 
          apply fun_mono_2 rfl 
          simp only [coeffs.val_except, mul_addₓ]
          repeat' 
            rw [←coeffs.val_between_map_mul]
          have h4 : ∀ {a b c d : Int}, ((a+b)+c+d) = (a+c)+b+d
          ·
            intros 
            ring 
          rw [h4]
...
inductive ee : Type
  ...
  | neg : ee
  | reduce : Nat → ee
  | cancel : Nat → ee deriving [anonymous], [anonymous]
