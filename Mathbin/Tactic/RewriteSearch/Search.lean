...
-/
unsafe def mk_graph (conf : config) (rules : List (expr × Bool)) (eq : expr) : tactic graph :=
  do 
...
-/
private unsafe def solution_paths : tactic (List edge × List edge) :=
  do 
...
-/
private unsafe def add_rewrite (v : vertex) (rw : rewrite) : tactic graph :=
  do 
    ...
    match maybe_id with 
      | some id =>
        do 
          ...
          if v.side = existing_vertex.side then return g else
              return { g with solving_edge := some ⟨v.id, existing_vertex.id, rw.proof, rw.how⟩ }
      | none =>
        do 
...
-/
private unsafe def expand_vertex (v : vertex) : tactic graph :=
  do 
...
| g, vertex_id =>
  if vertex_id ≥ g.conf.max_iterations then fail "search failed: max iterations reached" else
    if h : vertex_id < g.vertices.size then
      do 
...
of the form a = a.
-/
private unsafe def find_trivial_proof : tactic (graph × expr × List proof_unit) :=
  do 
