...
-/
unsafe def prove_elementwise (h : expr) : tactic (expr × expr × Option Name) :=
  do 
...
-/
unsafe def elementwise_lemma (n : Name) (n' : Name := n.append_suffix "_apply") : tactic Unit :=
  do 
    let d ← get_decl n 
...
unsafe def elementwise_attr : user_attribute Unit (Option Name) :=
  { Name := `elementwise, descr := "create a companion lemma for a morphism equation applied to an element",
...
in this way.)
-/
unsafe def elementwise (del : parse (tk "!")?) (ns : parse (ident)*) : tactic Unit :=
  do 
    ns.mmap'
        fun n =>
          do 
