...
theorem NormalizeFinLt.castLEOrderEmb {n m} {nm} {a : Fin m} {a' : ℕ} (h : NormalizeFinLt m a a') :
    NormalizeFinLt n (Fin.castLEOrderEmb nm a) a' := by simpa [normalize_fin_lt] using h
#align tactic.norm_fin.normalize_fin_lt.cast_le Tactic.NormFin.NormalizeFinLt.castLEOrderEmb

theorem NormalizeFinLt.castOrderIso {n m} {nm} {a : Fin m} {a' : ℕ} (h : NormalizeFinLt m a a') :
    NormalizeFinLt n (Fin.castOrderIso nm a) a' := by simpa [normalize_fin_lt] using h
#align tactic.norm_fin.normalize_fin_lt.cast Tactic.NormFin.NormalizeFinLt.castOrderIso

theorem NormalizeFin.castOrderIso {n m} {nm} {a : Fin m} {a' : ℕ} (h : NormalizeFin m a a') :
    NormalizeFin n (Fin.castOrderIso nm a) a' := by convert ← normalize_fin_lt.cast h
#align tactic.norm_fin.normalize_fin.cast Tactic.NormFin.NormalizeFin.castOrderIso

theorem NormalizeFinLt.castAddOrderEmb {n m} {a : Fin n} {a' : ℕ} (h : NormalizeFinLt n a a') :
    NormalizeFinLt (n + m) (Fin.castAddOrderEmb m a) a' := by simpa [normalize_fin_lt] using h
#align tactic.norm_fin.normalize_fin_lt.cast_add Tactic.NormFin.NormalizeFinLt.castAddOrderEmb

theorem NormalizeFinLt.castSuccEmb {n} {a : Fin n} {a' : ℕ} (h : NormalizeFinLt n a a') :
    NormalizeFinLt (n + 1) (Fin.castSuccEmb a) a' :=
  NormalizeFinLt.castAddOrderEmb h
#align tactic.norm_fin.normalize_fin_lt.cast_succ Tactic.NormFin.NormalizeFinLt.castSuccEmb

theorem NormalizeFinLt.addNatOrderEmb {n m m'} (hm : m = m') {a : Fin n} {a' b : ℕ}
    (h : NormalizeFinLt n a a') (e : a' + m' = b) :
    NormalizeFinLt (n + m) (@Fin.addNatOrderEmb n m a) b := by
  simpa [normalize_fin_lt, ← e, ← hm] using h
#align tactic.norm_fin.normalize_fin_lt.add_nat Tactic.NormFin.NormalizeFinLt.addNatOrderEmb

theorem NormalizeFinLt.natAddOrderEmb {n m n'} (hn : n = n') {a : Fin m} {a' b : ℕ}
    (h : NormalizeFinLt m a a') (e : n' + a' = b) :
    NormalizeFinLt (n + m) (@Fin.natAddOrderEmb n m a) b := by
  simpa [normalize_fin_lt, ← e, ← hn] using h
#align tactic.norm_fin.normalize_fin_lt.nat_add Tactic.NormFin.NormalizeFinLt.natAddOrderEmb
...
unsafe def match_fin_coe_fn (a : expr) : expr → Option match_fin_result
  | q(@Fin.castLEOrderEmb $(n) $(m) $(h)) => some (cast_le n m h a)
  | q(@Fin.castOrderIso $(m) $(n) $(h)) => some (cast n m h a)
  | q(@Fin.castAddOrderEmb $(n) $(m)) => some (cast_add n m a)
  | q(@Fin.castSuccEmb $(n)) => some (cast_succ n a)
  | q(@Fin.addNatOrderEmb $(n) $(m)) => some (add_nat n m a)
  | q(@Fin.natAddOrderEmb $(n) $(m)) => some (nat_add n m a)
  | _ => none
  ...
  | n, a => do
    ...
    match e with
      ...
      | match_fin_result.cast_le _ m nm a => do
        let (a', pa) ← (eval_fin_lt' m a).reset
        pure (a', q(@NormalizeFinLt.castLEOrderEmb).mk_app [n, m, nm, a, a', pa])
      | match_fin_result.cast m _ nm a => do
        let (a', pa) ← (eval_fin_lt' m a).reset
        pure (a', q(@NormalizeFinLt.castOrderIso).mk_app [n, m, nm, a, a', pa])
      | match_fin_result.cast_add n m a => do
        let (a', pa) ← (eval_fin_lt' m a).reset
        pure (a', q(@NormalizeFinLt.castAddOrderEmb).mk_app [n, m, a, a', pa])
      ...
      | match_fin_result.add_nat n m a => do
        ...
        pure (b, q(@NormalizeFinLt.addNatOrderEmb).mk_app [n, m, m', pm, a, a', b, pa, pb])
      | match_fin_result.nat_add n m a => do
        ...
        pure (b, q(@NormalizeFinLt.natAddOrderEmb).mk_app [n, m, n', pn, a, a', b, pa, pb])
      | _ => do
  ...
  | a => do
    ...
    match m with
      ...
      | match_fin_result.cast m n nm a => do
        ...
        pure (a', q(@NormalizeFin.castOrderIso).mk_app [n, m, nm, a, a', pa])
      | _ => do
