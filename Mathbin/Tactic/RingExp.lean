...
-/
unsafe inductive ex : ex_type → Type
...
-/
unsafe def ex.orig {et : ex_type} (ps : ex et) : expr :=
  ps.info.orig

/--
Return the normalized version of this `ex`.
-/
unsafe def ex.pretty {et : ex_type} (ps : ex et) : expr :=
  ps.info.pretty
...
-/
unsafe def ex.proof {et : ex_type} (ps : ex et) : Option expr :=
  ps.info.proof
...
unsafe def ex_info.set (i : ex_info) (o : Option expr) (pf : Option expr) : ex_info :=
  { i with orig := o.get_or_else i.pretty, Proof := pf }
...
unsafe def ex.repr : ∀ {et : ex_type}, ex et → Stringₓ
| Sum, ex.zero _ => "0"
...
-/
unsafe def ex.eq : ∀ {et : ex_type}, ex et → ex et → Bool
...
-/
unsafe def ex.lt : ∀ {et : ex_type}, ex et → ex et → Bool
| Sum, _, ex.zero _ => ff
| Sum, ex.zero _, _ => tt
...
-/
unsafe def in_exponent {α} (mx : ring_exp_m α) : ring_exp_m α :=
  do 
    let ctx ← get_context 
...
-/
unsafe def mk_app_class (f : Name) (inst : expr) (args : List expr) : ring_exp_m expr :=
  do 
    let ctx ← get_context 
...
Should be faster because it can use the cached instances.
 -/
unsafe def mk_app_csr (f : Name) (args : List expr) : ring_exp_m expr :=
  do 
    let ctx ← get_context 
...
Should be faster because it can use the cached instances.
-/
unsafe def mk_add (args : List expr) : ring_exp_m expr :=
  do 
    let ctx ← get_context 
    mk_app_class `` Add.add ctx.info_b.ha_instance args

/--
Specialized version of `mk_app ``has_mul.mul`.
Should be faster because it can use the cached instances.
-/
unsafe def mk_mul (args : List expr) : ring_exp_m expr :=
  do 
    let ctx ← get_context 
    mk_app_class `` Mul.mul ctx.info_b.hm_instance args

/--
Specialized version of `mk_app ``has_pow.pow`.
Should be faster because it can use the cached instances.
-/
unsafe def mk_pow (args : List expr) : ring_exp_m expr :=
  do 
    let ctx ← get_context 
    pure$
...
/-- Construct a normalization proof term or return the cached one. -/
unsafe def ex.proof_term {et : ex_type} (ps : ex et) : ring_exp_m expr :=
  ps.info.proof_term
...
-/
unsafe def mk_proof (lem : Name) (args : List expr) (hs : List ex_info) : ring_exp_m expr :=
  do 
...
-/
unsafe def mk_proof_or_refl (term : expr) (lem : Name) (args : List expr) (hs : List ex_info) : ring_exp_m expr :=
  do 
...
theorem sum_congr {p p' ps ps' : α} : p = p' → ps = ps' → (p+ps) = p'+ps' :=
  by 
    cc

/-- Congruence lemma for constructing `ex.prod`. -/
theorem prod_congr {p p' ps ps' : α} : p = p' → ps = ps' → (p*ps) = p'*ps' :=
  by 
    cc

/-- Congruence lemma for constructing `ex.exp`. -/
theorem exp_congr {p p' : α} {ps ps' : ℕ} : p = p' → ps = ps' → p ^ ps = p' ^ ps' :=
  by 
    cc

/-- Constructs `ex.zero` with the correct arguments. -/
unsafe def ex_zero : ring_exp_m (ex Sum) :=
  do 
    let ctx ← get_context 
    pure$ ex.zero ⟨ctx.info_b.zero, ctx.info_b.zero, none⟩

/-- Constructs `ex.sum` with the correct arguments. -/
unsafe def ex_sum (p : ex Prod) (ps : ex Sum) : ring_exp_m (ex Sum) :=
  do 
...
-/
unsafe def ex_coeff (x : Rat) : ring_exp_m (ex Prod) :=
  do 
    let ctx ← get_context 
...
unsafe def ex_exp (p : ex base) (ps : ex Prod) : ring_exp_m (ex exp) :=
  do 
    let ctx ← get_context 
...
theorem base_to_exp_pf {p p' : α} : p = p' → p = p' ^ 1 :=
  by 
    simp 

/-- Conversion from `ex base` to `ex exp`. -/
unsafe def base_to_exp (p : ex base) : ring_exp_m (ex exp) :=
  do 
...
theorem exp_to_prod_pf {p p' : α} : p = p' → p = p'*1 :=
  by 
    simp 

/-- Conversion from `ex exp` to `ex prod`. -/
unsafe def exp_to_prod (p : ex exp) : ring_exp_m (ex Prod) :=
  do 
    let o ← ex_one 
    let ps ← ex_prod p o 
...
theorem prod_to_sum_pf {p p' : α} : p = p' → p = p'+0 :=
  by 
    simp 

/-- Conversion from `ex prod` to `ex sum`. -/
unsafe def prod_to_sum (p : ex Prod) : ring_exp_m (ex Sum) :=
  do 
    let z ← ex_zero 
    let ps ← ex_sum p z 
...
theorem atom_to_sum_pf (p : α) : p = ((p ^ 1)*1)+0 :=
  by 
    simp 
...
-/
unsafe def atom_to_sum (p : atom) : ring_exp_m (ex Sum) :=
  do 
    let p' ← ex_var p 
    let o ← in_exponent$ ex_one 
    let p' ← ex_exp p' o 
...
-/
unsafe def add_coeff (p_p q_p : expr) (p q : coeff) : ring_exp_m (ex Prod) :=
  do 
    let ctx ← get_context 
...
```
-/
unsafe def rewrite (ps_o : expr) (ps' : ex Sum) (pf : expr) : ring_exp_m (ex Sum) :=
  do 
...
theorem add_overlap_pf {ps qs pq} (p : α) : (ps+qs) = pq → ((p*ps)+p*qs) = p*pq :=
  fun pq_pf =>
    calc ((p*ps)+p*qs) = p*ps+qs := symm (mul_addₓ _ _ _)
      _ = p*pq :=
      by 
        rw [pq_pf]
      

theorem add_overlap_pf_zero {ps qs} (p : α) : (ps+qs) = 0 → ((p*ps)+p*qs) = 0 :=
  fun pq_pf =>
    calc ((p*ps)+p*qs) = p*ps+qs := symm (mul_addₓ _ _ _)
      _ = p*0 :=
      by 
...
theorem add_pf_z_sum {ps qs qs' : α} : ps = 0 → qs = qs' → (ps+qs) = qs' :=
  fun ps_pf qs_pf =>
    calc (ps+qs) = 0+qs' :=
      by 
        rw [ps_pf, qs_pf]
...
theorem add_pf_sum_z {ps ps' qs : α} : ps = ps' → qs = 0 → (ps+qs) = ps' :=
  fun ps_pf qs_pf =>
    calc (ps+qs) = ps'+0 :=
      by 
        rw [ps_pf, qs_pf]
...
theorem add_pf_sum_lt {pps p ps qqs pqs : α} : (pps = p+ps) → (ps+qqs) = pqs → (pps+qqs) = p+pqs :=
  by 
    cc

theorem add_pf_sum_gt {pps qqs q qs pqs : α} : (qqs = q+qs) → (pps+qs) = pqs → (pps+qqs) = q+pqs :=
  by 
    cc
...
| pps@(ex.sum pps_i p ps), qqs@(ex.sum qqs_i q qs) =>
  do 
    ...
    match ol with 
      ...
      | overlap.zero pq =>
        do 
          let pqs ← add ps qs 
          let pf ←
            mk_proof `` add_pf_sum_overlap_zero
      ...
      | overlap.none =>
        if p.lt q then
          do 
            ...
            let pf ←
              mk_proof `` add_pf_sum_lt [pps.orig, p.pretty, ps.pretty, qqs.orig, pqs.pretty] [pps.info, pqs.info]
          ...
          do 
            ...
            let pf ←
              mk_proof `` add_pf_sum_gt [pps.orig, qqs.orig, q.pretty, qs.pretty, pqs.pretty] [qqs.info, pqs.info]
            pure$ pqqs.set_info ppqqs_o pf
...
theorem mul_pp_pf_overlap {pps p_b ps qqs qs psqs : α} {p_e q_e : ℕ} :
  ...
  fun ps_pf qs_pf psqs_pf =>
    by 
      simp [symm psqs_pf, pow_addₓ, ps_pf, qs_pf] <;> acRfl

theorem mul_pp_pf_prod_lt {pps p ps qqs pqs : α} : (pps = p*ps) → (ps*qqs) = pqs → (pps*qqs) = p*pqs :=
  by 
    cc

theorem mul_pp_pf_prod_gt {pps qqs q qs pqs : α} : (qqs = q*qs) → (pps*qs) = pqs → (pps*qqs) = q*pqs :=
  by 
    cc
...
| ps@(ex.coeff _ x), qs@(ex.coeff _ y) =>
  do 
...
| ps@(ex.coeff _ x), qqs@(ex.prod _ q qs) =>
  do 
...
| pps@(ex.prod _ p ps), qs@(ex.coeff _ y) =>
  do 
    let pqs ← mul_pp ps qs 
...
theorem mul_p_pf_zero {ps qs : α} : ps = 0 → (ps*qs) = 0 :=
  fun ps_pf =>
    by 
      rw [ps_pf, zero_mul]
...
theorem mul_pf_zero {ps qs : α} : qs = 0 → (ps*qs) = 0 :=
  fun qs_pf =>
    by 
      rw [qs_pf, mul_zero]
...
with the proof of `expr.of_rat p ^ expr.of_rat q = expr.of_rat (p ^ q)`.
-/
unsafe def pow_coeff (p_p q_p : expr) (p q : coeff) : ring_exp_m (ex Prod) :=
  do 
    let ctx ← get_context 
...
theorem pow_pp_pf_one {ps : α} {qs : ℕ} : ps = 1 → ps ^ qs = 1 :=
  fun ps_pf =>
    by 
      rw [ps_pf, one_pow]

theorem pow_pf_c_c {ps ps' pq : α} {qs qs' : ℕ} : ps = ps' → qs = qs' → ps' ^ qs' = pq → ps ^ qs = pq :=
  by 
    cc

theorem pow_pp_pf_c {ps ps' pqs : α} {qs qs' : ℕ} : ps = ps' → qs = qs' → ps' ^ qs' = pqs → ps ^ qs = pqs*1 :=
  by 
    simp  <;> cc
...
theorem pow_p_pf_cons {ps ps' : α} {qs qs' : ℕ} : ps = ps' → qs = qs' → ps ^ qs = ps' ^ qs' :=
  by 
    cc
...
| ps@(ex.zero ps_i), qs@(ex.coeff qs_i ⟨⟨succ y, 1, _, _⟩⟩) =>
  do 
    ...
    pure$ z.set_info z_o pf
| pps@(ex.sum pps_i p (ex.zero _)), qqs =>
  do 
...
| ps, qs@(ex.coeff qs_i ⟨⟨Int.ofNat (succ n), 1, den_pos, _⟩⟩) =>
  do 
...
theorem pow_pf_zero {ps : α} {qs : ℕ} : qs = 0 → ps ^ qs = 1 :=
  fun qs_pf =>
    calc ps ^ qs = ps ^ 0 :=
      by 
        rw [qs_pf]
...
theorem simple_pf_sum_zero {p p' : α} : p = p' → (p+0) = p' :=
  by 
    simp 

theorem simple_pf_prod_one {p p' : α} : p = p' → (p*1) = p' :=
  by 
    simp 

theorem simple_pf_prod_neg_one {α} [Ringₓ α] {p p' : α} : p = p' → (p*-1) = -p' :=
  by 
    simp 

theorem simple_pf_var_one (p : α) : p ^ 1 = p :=
  by 
    simp 

theorem simple_pf_exp_one {p p' : α} : p = p' → p ^ 1 = p' :=
  by 
    simp 
...
| Sum, pps@(ex.sum pps_i p (ex.zero _)) =>
  do 
    let (p_p, p_pf) ← p.simple 
    Prod.mk p_p <$> mk_app_csr `` simple_pf_sum_zero [p.pretty, p_p, p_pf]
| Sum, ex.sum pps_i p ps =>
  do 
    let (p_p, p_pf) ← p.simple 
    let (ps_p, ps_pf) ← ps.simple
    (Prod.mk <$> mk_add [p_p, ps_p])<*>mk_app_csr `` sum_congr [p.pretty, p_p, ps.pretty, ps_p, p_pf, ps_pf]
| Prod, ex.prod pps_i p (ex.coeff _ ⟨⟨1, 1, _, _⟩⟩) =>
  do 
    let (p_p, p_pf) ← p.simple 
    Prod.mk p_p <$> mk_app_csr `` simple_pf_prod_one [p.pretty, p_p, p_pf]
| Prod, pps@(ex.prod pps_i p (ex.coeff _ ⟨⟨-1, 1, _, _⟩⟩)) =>
  do 
    let ctx ← get_context 
    match ctx.info_b.ring_instance with 
      ...
      | some ringi =>
        do 
          let (p_p, p_pf) ← p.simple
          (Prod.mk <$>
                lift (mk_app `` Neg.neg [p_p]))<*>mk_app_class `` simple_pf_prod_neg_one ringi [p.pretty, p_p, p_pf]
| Prod, ex.prod pps_i p ps =>
  do 
    let (p_p, p_pf) ← p.simple 
    let (ps_p, ps_pf) ← ps.simple
    (Prod.mk <$> mk_mul [p_p, ps_p])<*>mk_app_csr `` prod_congr [p.pretty, p_p, ps.pretty, ps_p, p_pf, ps_pf]
| base, ex.sum_b pps_i ps => ps.simple
| exp, ex.exp pps_i p (ex.coeff _ ⟨⟨1, 1, _, _⟩⟩) =>
  do 
    let (p_p, p_pf) ← p.simple 
    Prod.mk p_p <$> mk_app_csr `` simple_pf_exp_one [p.pretty, p_p, p_pf]
| exp, ex.exp pps_i p ps =>
  do 
    let (p_p, p_pf) ← p.simple 
...
| bas@(b :: as), n =>
  do 
    let ctx ← get_context 
    lift$ is_def_eq a b.value ctx.transp >> pure (b, bas) <|>
        do 
...
-/
unsafe def resolve_atom (a : expr) : ring_exp_m atom :=
  do 
...
theorem negate_pf {α} [Ringₓ α] {ps ps' : α} : ((-1)*ps) = ps' → -ps = ps' :=
  by 
    simp 
...
-/
unsafe def negate (ps : ex Sum) : ring_exp_m (ex Sum) :=
  do 
    let ctx ← get_context 
    match ctx.info_b.ring_instance with 
...
-/
unsafe def inverse (ps : ex Sum) : ring_exp_m (ex Sum) :=
  do 
    let ctx ← get_context 
    let dri ←
...
theorem sub_pf {α} [Ringₓ α] {ps qs psqs : α} (h : (ps+-qs) = psqs) : ps - qs = psqs :=
  by 
    rwa [sub_eq_add_neg]

theorem div_pf {α} [DivisionRing α] {ps qs psqs : α} (h : (ps*qs⁻¹) = psqs) : ps / qs = psqs :=
  by 
    rwa [div_eq_mul_inv]
...
-/
unsafe def eval_with_proof (e : expr) : ring_exp_m (ex Sum × expr) :=
  do 
    let e' ← eval e 
...
-/
unsafe def eval_simple (e : expr) : ring_exp_m (expr × expr) :=
  do 
...
unsafe def make_eval_info (α : expr) : tactic eval_info :=
  do 
    let u ← mk_meta_univ 
