...
unsafe structure atom : Type where
  value : expr
  index : ℕ
#align tactic.ring_exp.atom tactic.ring_exp.atom
...
unsafe def eq (a b : atom) : Bool :=
  a.index = b.index
#align tactic.ring_exp.atom.eq tactic.ring_exp.atom.eq

/-- We order `atom`s on the order of appearance in the main expression.
-/
unsafe def lt (a b : atom) : Bool :=
  a.index < b.index
#align tactic.ring_exp.atom.lt tactic.ring_exp.atom.lt

unsafe instance : Repr atom :=
  ⟨fun x => "(atom " ++ repr x.2 ++ ")"⟩
...
#align tactic.ring_exp.coeff Tactic.RingExp.Coeff
...
unsafe structure ex_info : Type where
  ...
  pretty : expr
  Proof : Option expr
#align tactic.ring_exp.ex_info tactic.ring_exp.ex_info
...
unsafe def ex.orig {et : ExType} (ps : ex et) : expr :=
  ps.info.orig
#align tactic.ring_exp.ex.orig tactic.ring_exp.ex.orig

/-- Return the normalized version of this `ex`.
-/
unsafe def ex.pretty {et : ExType} (ps : ex et) : expr :=
  ps.info.pretty
#align tactic.ring_exp.ex.pretty tactic.ring_exp.ex.pretty
...
instance coeffHasRepr : Repr Coeff :=
  ⟨fun x => repr x.1⟩
#align tactic.ring_exp.coeff_has_repr Tactic.RingExp.coeffHasRepr
...
unsafe structure eval_info where
  ...
  zero : expr
  one : expr
#align tactic.ring_exp.eval_info tactic.ring_exp.eval_info
...
unsafe structure context where
  ...
  info_e : eval_info
  transp : Transparency
#align tactic.ring_exp.context tactic.ring_exp.context
...
unsafe def get_context : ring_exp_m context :=
  ReaderT.read
#align tactic.ring_exp.get_context tactic.ring_exp.get_context
...
unsafe def lift {α} (m : tactic α) : ring_exp_m α :=
  ReaderT.lift (StateT.lift m)
#align tactic.ring_exp.lift tactic.ring_exp.lift
...
#align tactic.ring_exp.mk_app_class tactic.ring_exp.mk_app_class
...
#align tactic.ring_exp.mk_app_csr tactic.ring_exp.mk_app_csr
...
#align tactic.ring_exp.mk_add tactic.ring_exp.mk_add
...
#align tactic.ring_exp.mk_mul tactic.ring_exp.mk_mul
...
#align tactic.ring_exp.mk_pow tactic.ring_exp.mk_pow
...
unsafe def ex.proof_term {et : ExType} (ps : ex et) : ring_exp_m expr :=
  ps.info.proof_term
#align tactic.ring_exp.ex.proof_term tactic.ring_exp.ex.proof_term
  ...
  | x :: xs => do
    ...
    match (x, xs_pfs) with
      ...
      | (_, some xs_pfs) => do
        let x_pf ← x
        pure (some (x_pf :: xs_pfs))
#align tactic.ring_exp.none_or_proof_term tactic.ring_exp.none_or_proof_term
...
#align tactic.ring_exp.mk_proof tactic.ring_exp.mk_proof
...
#align tactic.ring_exp.mk_proof_or_refl tactic.ring_exp.mk_proof_or_refl
...
#align tactic.ring_exp.add_orig tactic.ring_exp.add_orig
...
#align tactic.ring_exp.mul_orig tactic.ring_exp.mul_orig
...
#align tactic.ring_exp.ex_zero tactic.ring_exp.ex_zero
...
#align tactic.ring_exp.ex_coeff tactic.ring_exp.ex_coeff
...
#align tactic.ring_exp.ex_one tactic.ring_exp.ex_one
...
#align tactic.ring_exp.ex_var tactic.ring_exp.ex_var
...
#align tactic.ring_exp.ex_sum_b tactic.ring_exp.ex_sum_b
...
#align tactic.ring_exp.base_to_exp_pf Tactic.RingExp.base_to_exp_pf
...
unsafe def base_to_exp (p : ex base) : ring_exp_m (ex exp) := do
  ...
  pure <| ps p pf
#align tactic.ring_exp.base_to_exp tactic.ring_exp.base_to_exp

theorem exp_to_prod_pf {p p' : α} : p = p' → p = p' * 1 := by simp
#align tactic.ring_exp.exp_to_prod_pf Tactic.RingExp.exp_to_prod_pf
...
unsafe def exp_to_prod (p : ex exp) : ring_exp_m (ex Prod) := do
  ...
  pure <| ps p pf
#align tactic.ring_exp.exp_to_prod tactic.ring_exp.exp_to_prod

theorem prod_to_sum_pf {p p' : α} : p = p' → p = p' + 0 := by simp
#align tactic.ring_exp.prod_to_sum_pf Tactic.RingExp.prod_to_sum_pf
...
unsafe def prod_to_sum (p : ex Prod) : ring_exp_m (ex Sum) := do
  ...
  pure <| ps p pf
#align tactic.ring_exp.prod_to_sum tactic.ring_exp.prod_to_sum

theorem atom_to_sum_pf (p : α) : p = p ^ 1 * 1 + 0 := by simp
#align tactic.ring_exp.atom_to_sum_pf Tactic.RingExp.atom_to_sum_pf
...
unsafe def atom_to_sum (p : atom) : ring_exp_m (ex Sum) := do
  ...
  pure <| p' p.1 pf
#align tactic.ring_exp.atom_to_sum tactic.ring_exp.atom_to_sum
...
#align tactic.ring_exp.mul_coeff tactic.ring_exp.mul_coeff
...
unsafe def rewrite (ps_o : expr) (ps' : ex Sum) (pf : expr) : ring_exp_m (ex Sum) := do
  ...
  pure <| ps' ps_o pf
#align tactic.ring_exp.rewrite tactic.ring_exp.rewrite
...
unsafe inductive overlap : Type
  ...
  | zero : ex Sum → overlap
#align tactic.ring_exp.overlap tactic.ring_exp.overlap
...
#align tactic.ring_exp.add_overlap tactic.ring_exp.add_overlap
    ...
    match ol with
      ...
      | overlap.none =>
        if p q then do
          ...
          pure <| pqqs ppqqs_o pf
#align tactic.ring_exp.add tactic.ring_exp.add
...
#align tactic.ring_exp.simple_pf_sum_zero Tactic.RingExp.simple_pf_sum_zero

theorem simple_pf_prod_one {p p' : α} : p = p' → p * 1 = p' := by simp
#align tactic.ring_exp.simple_pf_prod_one Tactic.RingExp.simple_pf_prod_one

theorem simple_pf_prod_neg_one {α} [Ring α] {p p' : α} : p = p' → p * -1 = -p' := by simp
#align tactic.ring_exp.simple_pf_prod_neg_one Tactic.RingExp.simple_pf_prod_neg_one

theorem simple_pf_var_one (p : α) : p ^ 1 = p := by simp
#align tactic.ring_exp.simple_pf_var_one Tactic.RingExp.simple_pf_var_one

theorem simple_pf_exp_one {p p' : α} : p = p' → p ^ 1 = p' := by simp
#align tactic.ring_exp.simple_pf_exp_one Tactic.RingExp.simple_pf_exp_one
...
unsafe def resolve_atom_aux (a : expr) : List atom → ℕ → ring_exp_m (atom × List atom)
  ...
  | bas@(b :: as), n => do
    ...
    (lift <| is_def_eq a b ctx >> pure (b, bas)) <|> do
        ...
        pure (atm, b :: as')
#align tactic.ring_exp.resolve_atom_aux tactic.ring_exp.resolve_atom_aux
...
unsafe def eval_base (ps : expr) : ring_exp_m (ex Sum) :=
  ...
  | none => do
    ...
    atom_to_sum a
#align tactic.ring_exp.eval_base tactic.ring_exp.eval_base

theorem negate_pf {α} [Ring α] {ps ps' : α} : -1 * ps = ps' → -ps = ps' := by simp
#align tactic.ring_exp.negate_pf Tactic.RingExp.negate_pf
...
/--
  ...
  def
    eval
    : expr → ring_exp_m ( ex Sum )
    | e @ q( $ ( ps ) + $ ( qs ) ) => do let ps' ← eval ps let qs' ← eval qs add ps' qs'
      |
        ps_o @ q( Nat.succ $ ( p_o ) )
        =>
        do
      ...
      |
        ...
        (
          ...
          <|>
          eval_base e
      | e @ q( - $ ( ps ) ) => do let ps' ← eval ps negate ps' <|> eval_base e
      | e @ q( $ ( ps ) * $ ( qs ) ) => do let ps' ← eval ps let qs' ← eval qs mul ps' qs'
      | e @ q( Inv.inv $ ( ps ) ) => do let ps' ← eval ps inverse ps' <|> eval_base e
      |
        e @ q( $ ( ps ) / $ ( qs ) )
        =>
        do
        ...
        do
          let ps' ← eval ps
            ...
            (
              ...
              eval_base e
      | ps => eval_base ps
#align tactic.ring_exp.eval tactic.ring_exp.eval
...
unsafe def make_eval_info (α : expr) : tactic eval_info := do
  ...
  let csr_instance ← mk_app `` CommSemiring [α] >>= mk_instance
  let ring_instance ← some <$> (mk_app `` Ring [α] >>= mk_instance) <|> pure none
...
#align tactic.ring_exp.make_eval_info tactic.ring_exp.make_eval_info
...
unsafe def normalize (transp : Transparency) (e : expr) : tactic (expr × expr) := do
  ...
  pure (e', pf')
#align tactic.ring_exp.normalize tactic.ring_exp.normalize
      ...
      (Command.optDeclSig
       [(Term.explicitBinder
         ...
         [":"
          (Term.app
           `parse
           [(Tactic.Interactive.Tactic.RingExp.parser.optional
             (Term.app `tk [(str "\"!\"")])
             "?")])]
         []
         ")")]
           ...
           (Term.doLetArrow
            ...
            (Term.doPatDecl
             (Qq.«termQ(__)»
              ...
              (Term.app
               `Eq
               [(term.pseudo.antiquot "$" [] (antiquotNestedExpr "(" `ps ")") [])
                (term.pseudo.antiquot "$" [] (antiquotNestedExpr "(" `qs ")") [])])
              []
              ")")
             "←"
             (Term.doExpr («term_>>=_» `target ">>=" `whnf))
             []))
           [])
          (Term.doSeqItem
           (Term.doLet
          ...
          (Term.doSeqItem
           (Term.doLetArrow
            ...
            (Term.doPatDecl
             ...
             (Term.doExpr
              («term_<|_»
               (Term.app `run_ring_exp [`transp `ps])
               "<|"
               («term_<*>_»
                («term_<$>_» `Prod.mk "<$>" (Term.app `eval_with_proof [`ps]))
                "<*>"
                (Term.app `eval_with_proof [`qs]))))
             []))
           [])
          (Term.doSeqItem
           (Term.doExpr
              ...
              (Term.doSeqIndent
               [(Term.doSeqItem
                 (Term.doLetArrow
                  "let"
                  []
                  (Term.doIdDecl `qs_pf_inv [] "←" (Term.doExpr (Term.app `mk_eq_symm [`qs_pf]))))
                 [])
                (Term.doSeqItem
                 (Term.doLetArrow
                  "let"
                  ...
                  (Term.doIdDecl
                   ...
                   []
                   "←"
                   (Term.doExpr (Term.app `mk_eq_trans [`ps_pf `qs_pf_inv]))))
                 [])
                (Term.doSeqItem
                 (Term.doExpr
                  (Term.app
                   ...
                   [(Term.precheckedQuot
                     ...
                     (Term.quot
                      "`("
                      (term.pseudo.antiquot "$" [] (antiquotNestedExpr "(" `pf ")") [])
                      ")"))]))
                 [])]))
             "else"
             (Term.app `fail [(str "\"ring_exp failed to prove equality\"")])))
           [])]))
       [])
      []
      []
         ...
         [(Term.doSeqItem
          ...
          (Term.doSeqItem
           (Term.doLetArrow
            "let"
            []
            (Term.doIdDecl `pf [] "←" (Term.doExpr (Term.app `mk_eq_trans [`ps_pf `qs_pf_inv]))))
           [])
          (Term.doSeqItem
           (Term.doExpr
            (Term.app
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (str "\"ring_exp failed to prove equality\"")
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.app
       ...
       [(Term.precheckedQuot
         "`"
         (Term.quot "`(" (term.pseudo.antiquot "$" [] (antiquotNestedExpr "(" `pf ")") []) ")"))])
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, doElem))
      (Term.app `mk_eq_trans [`ps_pf `qs_pf_inv])
...
[PrettyPrinter.parenthesize] ...precedences are 60 >? 100, (some 100, term) <=? (some 60, term)
[PrettyPrinter.parenthesize] ...precedences are 10 >? 60, (some 61, term) <=? (none, [anonymous])
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.tuple
       "("
       [(Term.tuple "(" [`ps' "," [`ps_pf]] ")") "," [(Term.tuple "(" [`qs' "," [`qs_pf]] ")")]]
       ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.tuple "(" [`qs' "," [`qs_pf]] ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.app
       `parse
       [(Tactic.Interactive.Tactic.RingExp.parser.optional (Term.app `tk [(str "\"!\"")]) "?")])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Tactic.Interactive.Tactic.RingExp.parser.optional', expected 'Lean.Parser.Term.namedArgument'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Tactic.Interactive.Tactic.RingExp.parser.optional', expected 'Lean.Parser.Term.ellipsis'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Tactic.Interactive.Tactic.RingExp.parser.optional (Term.app `tk [(str "\"!\"")]) "?")
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Tactic.Interactive.Tactic.RingExp.parser.optional', expected 'Tactic.Interactive.Tactic.RingExp.parser.optional._@.Tactic.RingExp._hyg.44'
...
#align tactic.interactive.ring_exp_eq tactic.interactive.ring_exp_eq
     ...
     (Command.def
      ...
      (Command.optDeclSig
       [(Term.explicitBinder
         ...
         []
         ")")
        (Term.explicitBinder "(" [`loc] [":" (Term.app `parse [`location])] [] ")")]
       [(Term.typeSpec ":" (Term.app `tactic [`Unit]))])
      (Command.declValSimple
       ":="
       («term_<|>_»
        (Term.match
         ...
         (Term.matchAlts
          [(Term.matchAlt
            "|"
            [[(Term.app `Interactive.Loc.ns [(«term[_]» "[" [`none] "]")])]]
            "=>"
            (Term.app `ring_exp_eq [`red]))
           (Term.matchAlt "|" [[(Term.hole "_")]] "=>" `failed)]))
        "<|>"
        (Term.do
         "do"
         (Term.doSeqIndent
          [(Term.doSeqItem
            (Term.doLetArrow
             "let"
             []
             (Term.doIdDecl `ns [] "←" (Term.doExpr (Term.proj `loc "." `get_locals))))
            [])
           (Term.doSeqItem
            (Term.doLet
             "let"
            ...
            (Term.doLetArrow
             ...
             (Term.doPatDecl
              ...
              (Term.doExpr
               (Term.app
                `tactic.replace_at
                [(Term.app `normalize [`transp]) `ns (Term.proj `loc "." `include_goal)]))
              ["|"
               (Term.doSeqIndent
                [(Term.doSeqItem
                  (Term.doExpr (Term.app `fail [(str "\"ring_exp failed to simplify\"")]))
                  [])])]))
            [])
           (Term.doSeqItem
            (Term.doExpr
             («term_<|_» (Term.app `when [`loc]) "<|" (Term.app `try [`tactic.reflexivity])))
            [])])))
       [])
      []
      []
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      («term_<|_» (Term.app `when [`loc]) "<|" (Term.app `try [`tactic.reflexivity]))
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.app `try [`tactic.reflexivity])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'Lean.Parser.Term.namedArgument'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'Lean.Parser.Term.ellipsis'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      `tactic.reflexivity
...
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.doSeqIndent', expected 'Lean.Parser.Term.doSeqBracketed'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, doElem))
      (Term.app `fail [(str "\"ring_exp failed to simplify\"")])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'str', expected 'Lean.Parser.Term.namedArgument'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'str', expected 'Lean.Parser.Term.ellipsis'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (str "\"ring_exp failed to simplify\"")
[PrettyPrinter.parenthesize] ...precedences are 1023 >? 1024, (none,
     [anonymous]) <=? (none, [anonymous])
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
      `fail
...
[PrettyPrinter.parenthesize] ...precedences are 1023 >? 1022, (some 1023,
     term) <=? (some 1024, term)
[PrettyPrinter.parenthesize] parenthesized: (Term.paren "(" (Term.app `normalize [`transp]) ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
      `tactic.replace_at
[PrettyPrinter.parenthesize] ...precedences are 1024 >? 1024, (none,
     [anonymous]) <=? (some 1022, term)
...
/--
  ...
  def
    ring_exp
    ( red : parse tk "!" ? ) ( loc : parse location ) : tactic Unit
    :=
      match loc with | Interactive.Loc.ns [ none ] => ring_exp_eq red | _ => failed
        <|>
        do
          let ns ← loc . get_locals
            ...
            let
              tt
                ←
                tactic.replace_at normalize transp ns loc . include_goal
     ...
     (Command.def
      ...
      (Command.optDeclSig
       [(Term.explicitBinder
         ...
         [":"
          (Term.app
           `parse
           [(Conv.Interactive.Tactic.RingExp.parser.optional
      ...
      (Command.declValSimple
       ...
       (Term.let
        ...
        []
        («term_<|>_»
         (Term.app `discharge_eq_lhs [(Term.app `ring_exp_eq [`red])])
         "<|>"
         («term_<|>_»
          (Term.app `replace_lhs [(Term.app `normalize [`transp])])
          "<|>"
          (Term.app `fail [(str "\"ring_exp failed to simplify\"")]))))
       [])
      []
...
[PrettyPrinter.parenthesize] ...precedences are 1023 >? 1022, (some 1023,
     term) <=? (none, [anonymous])
[PrettyPrinter.parenthesize] parenthesized: (Term.paren "(" (Term.app `ring_exp_eq [`red]) ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
      `discharge_eq_lhs
[PrettyPrinter.parenthesize] ...precedences are 1024 >? 1024, (none,
     [anonymous]) <=? (some 1022, term)
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.app
       ...
       [(Conv.Interactive.Tactic.RingExp.parser.optional
         (Term.app `lean.parser.tk [(str "\"!\"")])
         "?")])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Conv.Interactive.Tactic.RingExp.parser.optional', expected 'Lean.Parser.Term.namedArgument'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Conv.Interactive.Tactic.RingExp.parser.optional', expected 'Lean.Parser.Term.ellipsis'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Conv.Interactive.Tactic.RingExp.parser.optional
       (Term.app `lean.parser.tk [(str "\"!\"")])
       "?")
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Conv.Interactive.Tactic.RingExp.parser.optional', expected 'Conv.Interactive.Tactic.RingExp.parser.optional._@.Tactic.RingExp._hyg.101'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.explicitBinder', expected 'Lean.Parser.Term.strictImplicitBinder'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.explicitBinder', expected 'Lean.Parser.Term.implicitBinder'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.explicitBinder', expected 'Lean.Parser.Term.instBinder'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Command.def', expected 'Lean.Parser.Command.theorem'
