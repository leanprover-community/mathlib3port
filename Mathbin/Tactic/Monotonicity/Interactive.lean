...
| mono_function.non_assoc fn xs ys =>
  do 
    let fn' ← pp fn 
    let xs' ← mmap pp xs 
    let ys' ← mmap pp ys 
    return f! "{fn' } {xs' } _ {ys'}"
| mono_function.assoc fn xs ys =>
  do 
    let fn' ← pp fn 
    let xs' ← pp xs 
    let ys' ← pp ys 
    return f! "{fn' } {xs' } _ {ys'}"
| mono_function.assoc_comm fn xs =>
  do 
    let fn' ← pp fn 
    let xs' ← pp xs 
    return f! "{fn' } _ {xs'}"
...
unsafe def ac_mono_ctx.to_tactic_format (ctx : ac_mono_ctx) : tactic format :=
  do 
...
unsafe def as_goal (e : expr) (tac : tactic Unit) : tactic Unit :=
  do 
    let gs ← get_goals 
    set_goals [e]
    tac 
...
unsafe def match_assoc (l : List expr) (r : List expr) : tactic (List expr × List expr × List expr × List expr) :=
  do 
...
unsafe def parse_ac_mono_function' (l r : pexpr) :=
  do 
    let l' ← to_expr l 
    let r' ← to_expr r 
...
unsafe def mk_congr_args (fn : expr) (xs₀ xs₁ : List expr) (l r : expr) : tactic expr :=
  do 
...
unsafe def match_rule (pat : expr) (r : Name) : tactic expr :=
  do 
    let r' ← mk_const r 
    let t ← infer_type r' 
    let t ←
...
unsafe def side_conditions (e : expr) : tactic format :=
  do 
...
unsafe def solve_mvar (v : expr) (tac : tactic Unit) : tactic Unit :=
  do 
    let gs ← get_goals 
    set_goals [v]
...
unsafe def best_match {β} (xs : List expr) (tac : expr → tactic β) : tactic Unit :=
  do 
    let t ← target 
...
unsafe def mono_aux (dir : parse side) : tactic Unit :=
  do 
...
unsafe def mono (many : parse (tk "*")?) (dir : parse side)
  ...
  (simp_rules : parse$ (tk "using" *> simp_arg_list <|> pure [])) : tactic Unit :=
  do 
