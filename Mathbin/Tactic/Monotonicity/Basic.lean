...
unsafe def find_one_difference : List expr → List expr → tactic (List expr × expr × expr × List expr)
  | x :: xs, y :: ys => do
    ...
    if c.is_some then Prod.map (cons x) id <$> find_one_difference xs ys
      else do
...
unsafe instance mono_key.has_lt : LT mono_key where
  lt := Prod.Lex (· < ·) (· < ·)
    ...
    (do
        let (rel, l, r) ←
          if h.is_arrow then pure (none, h.binding_domain, h.binding_body)
            else guardₓ h.get_app_fn.is_constant >> Prod.mk (some h.get_app_fn.const_name) <$> last_two h.get_app_args
        Prod.mk <$> monotonicity.check_rel l r <*> pure rel) <|>
      match xs with
      | [] => fail f! "oh? {h}"
      | x :: xs => mono_head_candidates n xs (h.pis [x])

unsafe def monotonicity.check (lm_n : Name) : tactic mono_key := do
...
unsafe def get_monotonicity_lemmas (k : expr) (e : mono_selection) : tactic (List Name) := do
