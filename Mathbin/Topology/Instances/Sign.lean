...
theorem continuousAt_sign_of_pos {a : α} (h : 0 < a) : ContinuousAt sign a :=
  by
...
theorem continuousAt_sign_of_neg {a : α} (h : a < 0) : ContinuousAt sign a :=
  by
  refine' (continuousAt_const : ContinuousAt (fun x => (-1 : SignType)) a).congr _
  rw [Filter.EventuallyEq, eventually_nhds_iff]
...
theorem continuousAt_sign_of_ne_zero {a : α} (h : a ≠ 0) : ContinuousAt sign a :=
  by
