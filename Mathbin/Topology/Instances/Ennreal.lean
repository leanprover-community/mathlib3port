...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ≥ » (0 : exprℚ()))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ∈ » {q : exprℚ() | «expr ∧ »(«expr ≤ »(0, q), «expr < »(a, real.to_nnreal q))})
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ∈ » {q : exprℚ() | «expr ∧ »(«expr ≤ »(0, q), «expr < »(«expr↑ »(real.to_nnreal q), a))})
    ...
    ⟨
      ⟨
        ⋃
            ( q : _ ) ( _ : q ≥ ( 0 : ℚ ) )
            ,
            { { a : ℝ≥0∞ | a < Real.toNnreal q } , { a : ℝ≥0∞ | ↑ Real.toNnreal q < a } }
          ,
                              ...
                              [
                                show
                                  s
                                    =
                                    ⋃
                                      ( q : _ ) ( _ : q ∈ { q : ℚ | 0 ≤ q ∧ a < Real.toNnreal q } )
                                      ,
                                      { b | ↑ Real.toNnreal q < b }
                                  from
                                    Set.ext fun b => by simp [ hs , @ Ennreal.lt_iff_exists_rat_btwn a b , and_assoc ]
                                ]
                            ,
                            rw
                              [
                                show
                                  s
                                    =
                                    ⋃
                                      ( q : _ ) ( _ : q ∈ { q : ℚ | 0 ≤ q ∧ ↑ Real.toNnreal q < a } )
                                      ,
                                      { b | b < ↑ Real.toNnreal q }
                                  from
                                    Set.ext
...
/-- The set of finite `ℝ≥0∞` numbers is homeomorphic to `ℝ≥0`. -/
  def
    ne_top_homeomorph_nnreal
    : { a | a ≠ ∞ } ≃ₜ ℝ≥0
    :=
      {
        ne_top_equiv_nnreal with
...
theorem tendsto_nat_nhds_top : tendsto (fun n : ℕ => ↑n) at_top (𝓝 ∞) :=
...
protected theorem tendsto_nhds {f : Filter α} {u : α → ℝ≥0∞} {a : ℝ≥0∞} (ha : a ≠ ⊤) :
  tendsto u f (𝓝 a) ↔ ∀ ε _ : ε > 0, ∀ᶠ x in f, u x ∈ Icc (a - ε) (a+ε) :=
  by 
...
theorem le_of_forall_lt_one_mul_le {x y : ℝ≥0∞} (h : ∀ a _ : a < 1, (a*x) ≤ y) : x ≤ y :=
  by 
    ...
    rw [one_mulₓ] at this 
    have  : (𝓝[Iio 1] (1 : ℝ≥0∞)).ne_bot := nhds_within_Iio_self_ne_bot' ⟨0, Ennreal.zero_lt_one⟩
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » range s)
    ...
    :=
      by
        refine' forall_ennreal . 2 ⟨ fun a => _ , _ ⟩
          ...
          exact
            tendsto.congr'
                mem_of_superset lt_mem_nhds $ @ coe_lt_top r
                  $
                  by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) [ le_of_ltₓ ]
              tendsto_const_nhds
...
@[normCast]
protected theorem has_sum_coe {f : α →  ℝ≥0 } {r :  ℝ≥0 } : HasSum (fun a => (f a : ℝ≥0∞)) (↑r) ↔ HasSum f r :=
...
protected theorem tsum_sigma {β : α → Type _} (f : ∀ a, β a → ℝ≥0∞) : (∑' p : Σ a, β a, f p.1 p.2) = ∑' a b, f a b :=
  tsum_sigma' (fun b => Ennreal.summable) Ennreal.summable

protected theorem tsum_sigma' {β : α → Type _} (f : (Σ a, β a) → ℝ≥0∞) : (∑' p : Σ a, β a, f p) = ∑' a b, f ⟨a, b⟩ :=
  tsum_sigma' (fun b => Ennreal.summable) Ennreal.summable
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (p «expr ≤ » r)
        ...
        by
          ...
          by
            rintro ⟨ b , ⟨ b_bound , b_lim ⟩ ⟩
              ...
              exact ⟨ N , fun m n hm hn => calc edist s m s n ≤ b N := b_bound m n N hm hn _ < ε := hN _ le_reflₓ N ⟩
        ⟩
