...
theorem PiCat.uniformContinuous_proj (i : ι) : UniformContinuous fun a : ∀ i : ι, α i => a i :=
  uniformContinuous_pi.1 uniformContinuous_id i
#align Pi.uniform_continuous_proj PiCat.uniformContinuous_proj
  ...
  ⟨by
    ...
    have : ∀ i, ∃ x : α i, Filter.map (fun a : ∀ i, α i => a i) f ≤ 𝓝 x :=
      ...
      have key : Cauchy (map (fun a : ∀ i : ι, α i => a i) f) :=
        hf.map (PiCat.uniformContinuous_proj α i)
...
instance PiCat.separated [∀ i, SeparatedSpace (α i)] : SeparatedSpace (∀ i, α i) :=
  separated_def.2 fun x y H => by
    ext i
    apply eq_of_separated_of_uniform_continuous (PiCat.uniformContinuous_proj α i)
    apply H
