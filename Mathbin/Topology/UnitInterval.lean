...
@[simp, normCast]
theorem coe_eq_zero {x : I} : (x : ℝ) = 0 ↔ x = 0 :=
  by 
    symm 
...
@[simp, normCast]
theorem coe_eq_one {x : I} : (x : ℝ) = 1 ↔ x = 1 :=
  by 
    symm 
    exact Subtype.ext_iff
...
def symm : I → I :=
  fun t => ⟨1 - t.val, mem_iff_one_sub_mem.mp t.property⟩
...
theorem one_minus_nonneg (x : I) : 0 ≤ 1 - (x : ℝ) :=
  by 
    simpa using x.2.2
...
theorem one_minus_le_one (x : I) : 1 - (x : ℝ) ≤ 1 :=
  by 
    simpa using x.2.1
...
@[simp]
theorem proj_Icc_eq_zero {x : ℝ} : proj_Icc (0 : ℝ) 1 zero_le_one x = 0 ↔ x ≤ 0 :=
  proj_Icc_eq_left zero_lt_one

@[simp]
theorem proj_Icc_eq_one {x : ℝ} : proj_Icc (0 : ℝ) 1 zero_le_one x = 1 ↔ 1 ≤ x :=
...
theorem affine_homeomorph_image_I (a b : 𝕜) (h : 0 < a) :
  affineHomeomorph a b h.ne.symm '' Set.Icc 0 1 = Set.Icc b (a+b) :=
  by 
    simp [h]
