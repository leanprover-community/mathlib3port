...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (γ «expr ≠ » 0) -/
...
theorem isClosed_iff {s : Set Γ₀} : IsClosed s ↔ (0 : Γ₀) ∈ s ∨ ∃ (γ : _)(_ : γ ≠ 0), s ⊆ Ici γ :=
  by
  simp only [← isOpen_compl_iff, isOpen_iff, mem_compl_iff, Classical.not_not, ← compl_Ici,
    compl_subset_compl]
      ...
      ·
        exact
          has_basis_nhds_zero.lift'_closure_eq_self fun x hx =>
            isClosed_iff.2 <| Or.inl <| zero_lt_iff.2 hx
...
instance (priority := 100) : HasContinuousMul Γ₀ :=
  ⟨by
    ...
    · rw [ContinuousAt, zero_mul]
      refine'
        ((has_basis_nhds_zero.prod_nhds hasBasis_nhds_zero).tendsto_iff hasBasis_nhds_zero).2
    ...
    · rw [ContinuousAt, zero_mul, nhds_prod_eq, nhds_of_ne_zero hy, prod_pure, tendsto_map'_iff]
      refine' (has_basis_nhds_zero.tendsto_iff hasBasis_nhds_zero).2 fun γ hγ => _
