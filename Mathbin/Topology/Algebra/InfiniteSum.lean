...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (x «expr ∉ » set.range[set.range] g) -/
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (b «expr ∉ » s) -/
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (b' «expr ≠ » b) -/
...
theorem tsum_zero' (hz : IsClosed ({0} : Set α)) : (∑' b : β, (0 : α)) = 0 := by
  classical rw [tsum, dif_pos summable_zero]
...
theorem summable_nat_add_iff {f : ℕ → α} (k : ℕ) : (Summable fun n => f (n + k)) ↔ Summable f :=
  Iff.symm <|
    (Equiv.addRight (∑ i in range k, f i)).Surjective.summable_iff_of_has_sum_iff fun a => (has_sum_nat_add_iff k).symm
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (c «expr ∉ » set.range[set.range] i) -/
...
theorem Summable.tendsto_top_of_pos {α : Type _} [LinearOrderedField α] [TopologicalSpace α] [OrderTopology α]
  ...
  refine' ⟨Set.IoiCat 0, Ioi_mem_at_top _, fun _ _ => _⟩
