...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∉ » set.range g)
...
protected theorem Finset.has_sum (s : Finset β) (f : β → α) : HasSum (f ∘ coeₓ : (↑s : Set β) → α) (∑ b in s, f b) :=
  by 
    rw [←sum_attach]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b' «expr ≠ » b)
...
theorem
  ...
  { f : γ → β → α } { a : γ → α } { s : Finset γ }
    : ∀ i _ : i ∈ s , HasSum f i a i → HasSum fun b => ∑ i in s , f i b ∑ i in s , a i
  :=
    Finset.induction_on
      s
        by simp only [ has_sum_zero , sum_empty , forall_true_iff ]
        by
          simp
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » decode₂ γ n)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » decode₂ γ i)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (d «expr ∈ » t)
...
theorem HasSum.has_sum_compl_iff {s : Set β} (hf : HasSum (f ∘ coeₓ : s → α) a₁) :
  HasSum (f ∘ coeₓ : sᶜ → α) a₂ ↔ HasSum f (a₁+a₂) :=
  by 
...
theorem sum_add_tsum_compl {s : Finset β} (hf : Summable f) : ((∑ x in s, f x)+∑' x : (↑s : Set β)ᶜ, f x) = ∑' x, f x :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∉ » set.range i)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (e «expr ∈ » expr𝓝() (0 : α))
...
theorem
  summable_abs_iff
  [ LinearOrderedAddCommGroup β ] [ UniformSpace β ] [ UniformAddGroup β ] [ CompleteSpace β ] { f : α → β }
    : Summable fun x => | f x | ↔ Summable f
  :=
    have
      h1 : ∀ x : { x | 0 ≤ f x } , | f x | = f x := fun x => abs_of_nonneg x . 2
      have
        h2 : ∀ x : { x | 0 ≤ f x } ᶜ , | f x | = - f x := fun x => abs_of_neg not_leₓ . 1 x . 2
        calc
          Summable fun x => | f x |
                ↔
            ...
            _ ↔ Summable fun x : { x | 0 ≤ f x } => f x ∧ Summable fun x : { x | 0 ≤ f x } ᶜ => - f x
              :=
              by simp only [ h1 , h2 ]
