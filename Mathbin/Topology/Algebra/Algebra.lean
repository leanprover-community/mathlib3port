...
theorem Subalgebra.is_closed_topological_closure (s : Subalgebra R A) : IsClosed (s.topological_closure : Set A) :=
  by 
...
theorem Subalgebra.topological_closure_comap'_homeomorph (s : Subalgebra R A) {B : Type _} [TopologicalSpace B]
  ...
  s.topological_closure.comap' f = (s.comap' f).topologicalClosure :=
  by 
    apply SetLike.ext' 
