...
theorem compl_section_ordSeparatingSet_mem_nhdsWithin_Ici (hd : Disjoint s (closure t))
  ...
  · simp only [Set.disjoint_left, not_forall, Classical.not_not] at H
      ...
      hac.lt_of_ne
        (Ne.symm <|
          ne_of_mem_of_not_mem hc <|
            disjoint_left.1 (disjoint_left_ord_separating_set.mono_right ordConnectedSection_subset)
              ha)
    ...
    ·
      exact
        OrdConnected.uIcc_subset inferInstance (hsub' ⟨hx.1, hx.2.le.trans hcb⟩)
          (hsub' ⟨hac.le, hcb⟩)
