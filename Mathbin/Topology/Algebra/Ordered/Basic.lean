...
theorem is_open_iff_generate_intervals {s : Set α} : IsOpen s ↔ generate_open { s | ∃ a, s = Ioi a ∨ s = Iio a } s := by
...
theorem tendsto_of_tendsto_of_tendsto_of_le_of_le' {f g h : β → α} {b : Filter β} {a : α} (hg : tendsto g b (𝓝 a))
  ...
  tendsto_order.2
    ⟨fun a' h' => by
      ...
      filter_upwards [this, hgf] fun a => lt_of_lt_of_leₓ, fun a' h' => by
      have : ∀ᶠ b in b, h b < a' := (tendsto_order.1 hh).right a' h'
      filter_upwards [this, hfh] fun a h₁ h₂ => lt_of_le_of_ltₓ h₂ h₁⟩
...
theorem disjoint_nhds_at_top [NoMaxOrder α] (x : α) : Disjoint (𝓝 x) at_top := by
...
theorem disjoint_nhds_at_bot [NoMinOrder α] (x : α) : Disjoint (𝓝 x) at_bot :=
...
theorem exists_seq_strict_mono_tendsto' {α : Type _} [LinearOrderₓ α] [TopologicalSpace α] [DenselyOrdered α]
    ...
    ∃ u : ℕ → α, StrictMono u ∧ (∀ n, u n ∈ Ioo y x) ∧ tendsto u at_top (𝓝 x) := by
...
theorem exists_seq_strict_anti_tendsto' [DenselyOrdered α] [first_countable_topology α] {x y : α} (hy : x < y) :
    ∃ u : ℕ → α, StrictAnti u ∧ (∀ n, u n ∈ Ioo x y) ∧ tendsto u at_top (𝓝 x) := by
...
theorem exists_seq_strict_anti_strict_mono_tendsto [DenselyOrdered α] [first_countable_topology α] {x y : α}
    ...
    ∃ u v : ℕ → α,
      StrictAnti u ∧
        StrictMono v ∧
          (∀ k, u k ∈ Ioo x y) ∧
            (∀ l, v l ∈ Ioo x y) ∧ (∀ k l, u k < v l) ∧ tendsto u at_top (𝓝 x) ∧ tendsto v at_top (𝓝 y) :=
  by
...
theorem nhds_within_Ioi_ne_bot [NoMaxOrder α] {a b : α} (H : a ≤ b) : ne_bot (𝓝[Ioi a] b) :=
...
@[instance]
theorem nhds_within_Ioi_self_ne_bot [NoMaxOrder α] (a : α) : ne_bot (𝓝[>] a) :=
...
theorem nhds_within_Iio_ne_bot [NoMinOrder α] {a b : α} (H : a ≤ b) : ne_bot (𝓝[Iio b] a) :=
...
@[instance]
theorem nhds_within_Iio_self_ne_bot [NoMinOrder α] (a : α) : ne_bot (𝓝[<] a) :=
