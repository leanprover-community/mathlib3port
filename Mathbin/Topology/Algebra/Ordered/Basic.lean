...
@[instance]
theorem nhds_within_Ici_self_ne_bot (a : α) : ne_bot (𝓝[Ici a] a) :=
...
@[instance]
theorem nhds_within_Iic_self_ne_bot (a : α) : ne_bot (𝓝[Iic a] a) :=
...
theorem Ioo_mem_nhds_within_Ioi {a b c : α} (H : b ∈ Ico a c) : Ioo a c ∈ 𝓝[Ioi b] b :=
  mem_nhds_within.2
    ⟨Iio c, is_open_Iio, H.2,
      by 
        rw [inter_comm, Ioi_inter_Iio] <;> exact Ioo_subset_Ioo_left H.1⟩

theorem Ioc_mem_nhds_within_Ioi {a b c : α} (H : b ∈ Ico a c) : Ioc a c ∈ 𝓝[Ioi b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Ioi H) Ioo_subset_Ioc_self

theorem Ico_mem_nhds_within_Ioi {a b c : α} (H : b ∈ Ico a c) : Ico a c ∈ 𝓝[Ioi b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Ioi H) Ioo_subset_Ico_self

theorem Icc_mem_nhds_within_Ioi {a b c : α} (H : b ∈ Ico a c) : Icc a c ∈ 𝓝[Ioi b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Ioi H) Ioo_subset_Icc_self

@[simp]
theorem nhds_within_Ioc_eq_nhds_within_Ioi {a b : α} (h : a < b) : 𝓝[Ioc a b] a = 𝓝[Ioi a] a :=
  le_antisymmₓ (nhds_within_mono _ Ioc_subset_Ioi_self)$
    nhds_within_le_of_mem$ Ioc_mem_nhds_within_Ioi$ left_mem_Ico.2 h

@[simp]
theorem nhds_within_Ioo_eq_nhds_within_Ioi {a b : α} (h : a < b) : 𝓝[Ioo a b] a = 𝓝[Ioi a] a :=
...
theorem continuous_within_at_Ioc_iff_Ioi [TopologicalSpace β] {a b : α} {f : α → β} (h : a < b) :
  ContinuousWithinAt f (Ioc a b) a ↔ ContinuousWithinAt f (Ioi a) a :=
  by 
...
theorem continuous_within_at_Ioo_iff_Ioi [TopologicalSpace β] {a b : α} {f : α → β} (h : a < b) :
  ContinuousWithinAt f (Ioo a b) a ↔ ContinuousWithinAt f (Ioi a) a :=
  by 
...
theorem Ioo_mem_nhds_within_Iio {a b c : α} (H : b ∈ Ioc a c) : Ioo a c ∈ 𝓝[Iio b] b :=
  by 
    simpa only [dual_Ioo] using Ioo_mem_nhds_within_Ioi (show to_dual b ∈ Ico (to_dual c) (to_dual a) from H.symm)

theorem Ico_mem_nhds_within_Iio {a b c : α} (H : b ∈ Ioc a c) : Ico a c ∈ 𝓝[Iio b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Iio H) Ioo_subset_Ico_self

theorem Ioc_mem_nhds_within_Iio {a b c : α} (H : b ∈ Ioc a c) : Ioc a c ∈ 𝓝[Iio b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Iio H) Ioo_subset_Ioc_self

theorem Icc_mem_nhds_within_Iio {a b c : α} (H : b ∈ Ioc a c) : Icc a c ∈ 𝓝[Iio b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Iio H) Ioo_subset_Icc_self

@[simp]
theorem nhds_within_Ico_eq_nhds_within_Iio {a b : α} (h : a < b) : 𝓝[Ico a b] b = 𝓝[Iio b] b :=
  by 
    simpa only [dual_Ioc] using nhds_within_Ioc_eq_nhds_within_Ioi h.dual

@[simp]
theorem nhds_within_Ioo_eq_nhds_within_Iio {a b : α} (h : a < b) : 𝓝[Ioo a b] b = 𝓝[Iio b] b :=
  by 
...
theorem Ioo_mem_nhds_within_Ici {a b c : α} (H : b ∈ Ioo a c) : Ioo a c ∈ 𝓝[Ici b] b :=
  mem_nhds_within_of_mem_nhds$ IsOpen.mem_nhds is_open_Ioo H

theorem Ioc_mem_nhds_within_Ici {a b c : α} (H : b ∈ Ioo a c) : Ioc a c ∈ 𝓝[Ici b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Ici H) Ioo_subset_Ioc_self

theorem Ico_mem_nhds_within_Ici {a b c : α} (H : b ∈ Ico a c) : Ico a c ∈ 𝓝[Ici b] b :=
  mem_nhds_within.2
    ⟨Iio c, is_open_Iio, H.2,
      by 
        simp only [inter_comm, Ici_inter_Iio, Ico_subset_Ico_left H.1]⟩

theorem Icc_mem_nhds_within_Ici {a b c : α} (H : b ∈ Ico a c) : Icc a c ∈ 𝓝[Ici b] b :=
  mem_of_superset (Ico_mem_nhds_within_Ici H) Ico_subset_Icc_self

@[simp]
theorem nhds_within_Icc_eq_nhds_within_Ici {a b : α} (h : a < b) : 𝓝[Icc a b] a = 𝓝[Ici a] a :=
  le_antisymmₓ (nhds_within_mono _ Icc_subset_Ici_self)$
    nhds_within_le_of_mem$ Icc_mem_nhds_within_Ici$ left_mem_Ico.2 h

@[simp]
theorem nhds_within_Ico_eq_nhds_within_Ici {a b : α} (h : a < b) : 𝓝[Ico a b] a = 𝓝[Ici a] a :=
  le_antisymmₓ (nhds_within_mono _ fun x => And.left)$ nhds_within_le_of_mem$ Ico_mem_nhds_within_Ici$ left_mem_Ico.2 h
...
theorem continuous_within_at_Ico_iff_Ici [TopologicalSpace β] {a b : α} {f : α → β} (h : a < b) :
  ContinuousWithinAt f (Ico a b) a ↔ ContinuousWithinAt f (Ici a) a :=
  by 
...
theorem Ioo_mem_nhds_within_Iic {a b c : α} (H : b ∈ Ioo a c) : Ioo a c ∈ 𝓝[Iic b] b :=
  mem_nhds_within_of_mem_nhds$ IsOpen.mem_nhds is_open_Ioo H

theorem Ico_mem_nhds_within_Iic {a b c : α} (H : b ∈ Ioo a c) : Ico a c ∈ 𝓝[Iic b] b :=
  mem_of_superset (Ioo_mem_nhds_within_Iic H) Ioo_subset_Ico_self

theorem Ioc_mem_nhds_within_Iic {a b c : α} (H : b ∈ Ioc a c) : Ioc a c ∈ 𝓝[Iic b] b :=
  by 
    simpa only [dual_Ico] using Ico_mem_nhds_within_Ici (show to_dual b ∈ Ico (to_dual c) (to_dual a) from H.symm)

theorem Icc_mem_nhds_within_Iic {a b c : α} (H : b ∈ Ioc a c) : Icc a c ∈ 𝓝[Iic b] b :=
  mem_of_superset (Ioc_mem_nhds_within_Iic H) Ioc_subset_Icc_self

@[simp]
theorem nhds_within_Icc_eq_nhds_within_Iic {a b : α} (h : a < b) : 𝓝[Icc a b] b = 𝓝[Iic b] b :=
  by 
    simpa only [dual_Icc] using nhds_within_Icc_eq_nhds_within_Ici h.dual

@[simp]
theorem nhds_within_Ioc_eq_nhds_within_Iic {a b : α} (h : a < b) : 𝓝[Ioc a b] b = 𝓝[Iic b] b :=
  by 
...
theorem continuous_within_at_Icc_iff_Iic [TopologicalSpace β] {a b : α} {f : α → β} (h : a < b) :
  ContinuousWithinAt f (Icc a b) b ↔ ContinuousWithinAt f (Iic b) b :=
  by 
...
-- error in Topology.Algebra.Ordered.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
instance tendsto_Icc_class_nhds (a : α) : tendsto_Ixx_class Icc (expr𝓝() a) (expr𝓝() a) :=
begin
  simp [] [] ["only"] ["[", expr nhds_eq_order, ",", expr infi_subtype', "]"] [] [],
  refine [expr ((has_basis_infi_principal_finite _).inf (has_basis_infi_principal_finite _)).tendsto_Ixx_class (λ
    s hs, _)],
  refine [expr ((ord_connected_bInter _).inter (ord_connected_bInter _)).out]; intros ["_", "_"],
  exacts ["[", expr ord_connected_Ioi, ",", expr ord_connected_Iio, "]"]
end
...
theorem induced_order_topology' {α : Type u} {β : Type v} [PartialOrderₓ α] [ta : TopologicalSpace β] [PartialOrderₓ β]
  ...
  (H₁ : ∀ {a x}, x < f a → ∃ (b : _)(_ : b < a), x ≤ f b) (H₂ : ∀ {a x}, f a < x → ∃ (b : _)(_ : b > a), f b ≤ x) :
  @OrderTopology _ (induced f ta) _ :=
  by 
    letI this := induced f ta 
  ...
  by 
    ...
    ·
      ...
      ·
        ...
        exact mem_infi_of_mem ab (mem_principal_self (Ioi («expr↑ » b)))
      ·
...
theorem disjoint_nhds_at_top [NoTopOrder α] (x : α) : Disjoint (𝓝 x) at_top :=
  by 
...
theorem tfae_mem_nhds_within_Ioi {a b : α} (hab : a < b) (s : Set α) :
  tfae
    [s ∈ 𝓝[Ioi a] a, s ∈ 𝓝[Ioc a b] a, s ∈ 𝓝[Ioo a b] a, ∃ (u : _)(_ : u ∈ Ioc a b), Ioo a u ⊆ s,
      ∃ (u : _)(_ : u ∈ Ioi a), Ioo a u ⊆ s] :=
  by 
    tfaeHave 1 ↔ 2
    ·
...
theorem tfae_mem_nhds_within_Iio {a b : α} (h : a < b) (s : Set α) :
  tfae
    [s ∈ 𝓝[Iio b] b, s ∈ 𝓝[Ico a b] b, s ∈ 𝓝[Ioo a b] b, ∃ (l : _)(_ : l ∈ Ico a b), Ioo l b ⊆ s,
      ∃ (l : _)(_ : l ∈ Iio b), Ioo l b ⊆ s] :=
  by 
...
theorem tfae_mem_nhds_within_Ici {a b : α} (hab : a < b) (s : Set α) :
  tfae
    [s ∈ 𝓝[Ici a] a, s ∈ 𝓝[Icc a b] a, s ∈ 𝓝[Ico a b] a, ∃ (u : _)(_ : u ∈ Ioc a b), Ico a u ⊆ s,
      ∃ (u : _)(_ : u ∈ Ioi a), Ico a u ⊆ s] :=
  by 
    tfaeHave 1 ↔ 2
    ·
...
theorem mem_nhds_within_Ici_iff_exists_Ico_subset' {a u' : α} {s : Set α} (hu' : a < u') :
  s ∈ 𝓝[Ici a] a ↔ ∃ (u : _)(_ : u ∈ Ioi a), Ico a u ⊆ s :=
...
theorem tfae_mem_nhds_within_Iic {a b : α} (h : a < b) (s : Set α) :
  tfae
    [s ∈ 𝓝[Iic b] b, s ∈ 𝓝[Icc a b] b, s ∈ 𝓝[Ioc a b] b, ∃ (l : _)(_ : l ∈ Ico a b), Ioc l b ⊆ s,
      ∃ (l : _)(_ : l ∈ Iio b), Ioc l b ⊆ s] :=
  by 
...
theorem mem_nhds_within_Iic_iff_exists_Ioc_subset' {a l' : α} {s : Set α} (hl' : l' < a) :
  s ∈ 𝓝[Iic a] a ↔ ∃ (l : _)(_ : l ∈ Iio a), Ioc l a ⊆ s :=
...
theorem order_topology_of_nhds_abs {α : Type _} [TopologicalSpace α] [LinearOrderedAddCommGroup α]
  (h_nhds : ∀ a : α, 𝓝 a = ⨅(r : _)(_ : r > 0), 𝓟 { b | |a - b| < r }) : OrderTopology α :=
  by 
    refine' ⟨eq_of_nhds_eq_nhds$ fun a => _⟩
    rw [h_nhds]
    letI this := Preorderₓ.topology α 
    letI this : OrderTopology α := ⟨rfl⟩
...
-- error in Topology.Algebra.Ordered.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
-- error in Topology.Algebra.Ordered.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
@[simp]
theorem interior_Ico [NoBotOrder α] {a b : α} : Interior (Ico a b) = Ioo a b :=
  by 
    rw [←Ici_inter_Iio, interior_inter, interior_Ici, interior_Iio, Ioi_inter_Iio]

@[simp]
theorem interior_Ioc [NoTopOrder α] {a b : α} : Interior (Ioc a b) = Ioo a b :=
  by 
...
@[simp]
theorem frontier_Ico [NoBotOrder α] {a b : α} (h : a < b) : Frontier (Ico a b) = {a, b} :=
  by 
    simp [Frontier, h, le_of_ltₓ h, Icc_diff_Ioo_same]

@[simp]
theorem frontier_Ioc [NoTopOrder α] {a b : α} (h : a < b) : Frontier (Ioc a b) = {a, b} :=
  by 
    simp [Frontier, h, le_of_ltₓ h, Icc_diff_Ioo_same]
...
theorem comap_coe_Ioo_nhds_within_Iio (a b : α) : comap (coeₓ : Ioo a b → α) (𝓝[Iio b] b) = at_top :=
...
theorem comap_coe_Ioo_nhds_within_Ioi (a b : α) : comap (coeₓ : Ioo a b → α) (𝓝[Ioi a] a) = at_bot :=
  comap_coe_nhds_within_Ioi_of_Ioo_subset Ioo_subset_Ioi_self$ fun h => ⟨b, nonempty_Ioo.1 h, subset.refl _⟩

theorem comap_coe_Ioi_nhds_within_Ioi (a : α) : comap (coeₓ : Ioi a → α) (𝓝[Ioi a] a) = at_bot :=
  comap_coe_nhds_within_Ioi_of_Ioo_subset (subset.refl _)$ fun ⟨x, hx⟩ => ⟨x, hx, Ioo_subset_Ioi_self⟩

theorem comap_coe_Iio_nhds_within_Iio (a : α) : comap (coeₓ : Iio a → α) (𝓝[Iio a] a) = at_top :=
  @comap_coe_Ioi_nhds_within_Ioi (OrderDual α) _ _ _ _ a

@[simp]
theorem map_coe_Ioo_at_top {a b : α} (h : a < b) : map (coeₓ : Ioo a b → α) at_top = 𝓝[Iio b] b :=
  map_coe_at_top_of_Ioo_subset Ioo_subset_Iio_self$ fun _ _ => ⟨_, h, subset.refl _⟩

@[simp]
theorem map_coe_Ioo_at_bot {a b : α} (h : a < b) : map (coeₓ : Ioo a b → α) at_bot = 𝓝[Ioi a] a :=
  map_coe_at_bot_of_Ioo_subset Ioo_subset_Ioi_self$ fun _ _ => ⟨_, h, subset.refl _⟩

@[simp]
theorem map_coe_Ioi_at_bot (a : α) : map (coeₓ : Ioi a → α) at_bot = 𝓝[Ioi a] a :=
...
@[simp]
theorem tendsto_Ioo_at_bot {f : β → Ioo a b} : tendsto f l at_bot ↔ tendsto (fun x => (f x : α)) l (𝓝[Ioi a] a) :=
  by 
    rw [←comap_coe_Ioo_nhds_within_Ioi, tendsto_comap_iff]

@[simp]
theorem tendsto_Ioi_at_bot {f : β → Ioi a} : tendsto f l at_bot ↔ tendsto (fun x => (f x : α)) l (𝓝[Ioi a] a) :=
  by 
    rw [←comap_coe_Ioi_nhds_within_Ioi, tendsto_comap_iff]

@[simp]
theorem tendsto_Iio_at_top {f : β → Iio a} : tendsto f l at_top ↔ tendsto (fun x => (f x : α)) l (𝓝[Iio a] a) :=
  by 
...
theorem Monotone.tendsto_nhds_within_Iio {α : Type _} [LinearOrderₓ α] [TopologicalSpace α] [OrderTopology α]
  {f : α → β} (Mf : Monotone f) (x : α) : tendsto f (𝓝[Iio x] x) (𝓝 (Sup (f '' Iio x))) :=
  by 
...
theorem Monotone.tendsto_nhds_within_Ioi {α : Type _} [LinearOrderₓ α] [TopologicalSpace α] [OrderTopology α]
  {f : α → β} (Mf : Monotone f) (x : α) : tendsto f (𝓝[Ioi x] x) (𝓝 (Inf (f '' Ioi x))) :=
