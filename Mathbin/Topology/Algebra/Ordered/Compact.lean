...
/-- A closed interval in a conditionally complete linear order is compact. -/
  instance
    ( priority := 100 )
    ConditionallyCompleteLinearOrder.to_compact_Icc_space
    ( α : Type _ ) [ ConditionallyCompleteLinearOrder α ] [ TopologicalSpace α ] [ OrderTopology α ] : CompactIccSpace α
    :=
      by
        refine' ⟨ fun a b => _ ⟩
          ...
          contrapose! hf
          rw [ le_principal_iff ]
          have hpt : ∀ x _ : x ∈ Icc a b , { x } ∉ f
          exact fun x hx hxf => hf x hx le_pure_iff . 2 hxf . trans pure_le_nhds x
          ...
          set c := Sup s
          have hsc : IsLub s c
          exact is_lub_cSup ⟨ a , ha ⟩ sbd
          have hc : c ∈ Icc a b
          exact ⟨ hsc . 1 ha , hsc . 2 hsb ⟩
          specialize hf c hc
          have hcs : c ∈ s
          ·
            cases' hc . 1 . eq_or_lt with heq hlt
              ...
              rcases
                mem_nhds_within_Iic_iff_exists_Ioc_subset' hlt . 1 mem_nhds_within_of_mem_nhds hU
                with ⟨ x , hxc , hxU ⟩
              rcases
                hsc.frequently_mem ⟨ a , ha ⟩ . and_eventually Ioc_mem_nhds_within_Iic ⟨ hxc , le_rfl ⟩ . exists
                with ⟨ y , ⟨ hyab , hyf ⟩ , hy ⟩
              refine' mem_of_superset f.diff_mem_iff . 2 ⟨ hcf , hyf ⟩ subset.trans _ hxU
              rw [ diff_subset_iff ]
              exact
                subset.trans Icc_subset_Icc_union_Ioc union_subset_union subset.rfl $ Ioc_subset_Ioc_left hy . 1 . le
          cases' hc . 2 . eq_or_lt with heq hlt
          · rw [ ← HEq ] exact hcs . 2
          contrapose! hf
          intro U hU
          rcases
            mem_nhds_within_Ici_iff_exists_mem_Ioc_Ico_subset hlt . 1 mem_nhds_within_of_mem_nhds hU
            with ⟨ y , hxy , hyU ⟩
          refine' mem_of_superset _ hyU
          clear! U
          have hy : y ∈ Icc a b
          exact ⟨ hc . 1 . trans hxy . 1 . le , hxy . 2 ⟩
          byCases' hay : Icc a y ∈ f
          ·
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∉ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » insert (default β) s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » insert (default β) s)
...
theorem ContinuousOn.image_interval_eq_Icc
  (h : ContinuousOn f$ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
  f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)" =
    Icc (Inf (f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"))
      (Sup (f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")) :=
  by 
    cases' le_totalₓ a b with h2 h2
    ·
      simpRw [interval_of_le h2]  at h⊢
      exact h.image_Icc h2
    ·
      simpRw [interval_of_ge h2]  at h⊢
      exact h.image_Icc h2
