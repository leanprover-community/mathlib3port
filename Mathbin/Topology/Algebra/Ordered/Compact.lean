...
instance (priority := 100)ConditionallyCompleteLinearOrder.to_compact_Icc_space (α : Type _)
  [ConditionallyCompleteLinearOrder α] [TopologicalSpace α] [OrderTopology α] : CompactIccSpace α :=
  by 
    refine' ⟨fun a b => _⟩
...
-- error in Topology.Algebra.Ordered.Compact: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
{f : β → α}
(hf : continuous f)
(hlim : tendsto f (cocompact β) at_top) : «expr∃ , »((x), ∀ y, «expr ≤ »(f x, f y)) :=
begin
  inhabit [expr β] [],
  obtain ["⟨", ident s, ":", expr set β, ",", ident hsc, ":", expr is_compact s, ",", ident hsf, ":", expr ∀
   x «expr ∉ » s, «expr ≤ »(f (default β), f x), "⟩", ":=", expr (has_basis_cocompact.tendsto_iff at_top_basis).1 hlim «expr $ »(f, default β) trivial],
  obtain ["⟨", ident x, ",", "-", ",", ident hx, "⟩", ":", expr «expr∃ , »((x «expr ∈ » insert (default β) s), ∀
    y «expr ∈ » insert (default β) s, «expr ≤ »(f x, f y)), ":=", expr (hsc.insert (default β)).exists_forall_le (nonempty_insert _ _) hf.continuous_on],
  refine [expr ⟨x, λ y, _⟩],
  by_cases [expr hy, ":", expr «expr ∈ »(y, s)],
  exacts ["[", expr hx y (or.inr hy), ",", expr (hx _ (or.inl rfl)).trans (hsf y hy), "]"]
end
...
-- error in Topology.Algebra.Ordered.Compact: ././Mathport/Syntax/Translate/Basic.lean:545:47: unsupported (impossible)
