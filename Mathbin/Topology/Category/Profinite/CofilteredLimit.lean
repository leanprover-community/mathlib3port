...
theorem exists_clopen_of_cofiltered {U : Set C.X} (hU : IsClopen U) :
  ...
  by 
    ...
    ·
      ...
      ·
        intro hx 
        simpRw [Set.preimage_Union, Set.mem_Union]
      ...
      ·
        intro hx 
        simpRw [Set.preimage_Union, Set.mem_Union]  at hx 
        obtain ⟨s, hs, hx⟩ := hx 
        rw [h]
        refine' ⟨s.1, s.2, _⟩
        rw [(hV s).2]
...
theorem exists_locally_constant_fin_two (f : LocallyConstant C.X (Finₓ 2)) :
  ∃ (j : J)(g : LocallyConstant (F.obj j) (Finₓ 2)), f = g.comap (C.π.app _) :=
  by 
    let U := f ⁻¹' {0}
    ...
    convRHS => rw [Set.preimage_comp]
    rw [LocallyConstant.of_clopen_fiber_zero hV, ←h]

-- error in Topology.Category.Profinite.CofilteredLimit: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
theorem exists_locally_constant_fintype_nonempty {α : Type _} [Fintype α] [Nonempty α] (f : LocallyConstant C.X α) :
  ...
  by 
    ...
    ·
      intro a b hh 
      applyFun fun e => e a  at hh 
      dsimp [ι]  at hh 
      rw [if_pos rfl] at hh 
      splitIfs  at hh with hh1 hh1
      ·
        exact hh1.symm
      ·
