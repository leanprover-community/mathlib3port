    ...
    PreservesColimitsOfSize.{v, v}
      (forget :
        TopCat.{max v u} ⥤
          Type
            max v
              u) where PreservesColimitsOfShape J 𝒥 :=
...
@[simp, reassoc]
theorem pi_iso_pi_inv_π {ι : Type v} (α : ι → TopCat.{max v u}) (i : ι) : (piIsoPi α).inv ≫ Pi.π α i = piπ α i := by
  simp [pi_iso_pi]
...
@[simp, reassoc]
theorem prod_iso_prod_hom_fst (X Y : TopCat.{u}) : (prodIsoProd X Y).Hom ≫ prod_fst = limits.prod.fst := by
  simpa [← iso.eq_inv_comp, prod_iso_prod]
#align Top.prod_iso_prod_hom_fst TopCat.prod_iso_prod_hom_fst

@[simp, reassoc]
theorem prod_iso_prod_hom_snd (X Y : TopCat.{u}) : (prodIsoProd X Y).Hom ≫ prod_snd = limits.prod.snd := by
  simpa [← iso.eq_inv_comp, prod_iso_prod]
...
theorem prod_iso_prod_inv_fst (X Y : TopCat.{u}) : (prodIsoProd X Y).inv ≫ limits.prod.fst = prod_fst := by
  simp [iso.inv_comp_eq]
#align Top.prod_iso_prod_inv_fst TopCat.prod_iso_prod_inv_fst

@[simp, reassoc, elementwise]
theorem prod_iso_prod_inv_snd (X Y : TopCat.{u}) : (prodIsoProd X Y).inv ≫ limits.prod.snd = prod_snd := by
  simp [iso.inv_comp_eq]
...
theorem inducing_prod_map {W X Y Z : TopCat} {f : W ⟶ X} {g : Y ⟶ Z} (hf : Inducing f) (hg : Inducing g) :
...
theorem embedding_prod_map {W X Y Z : TopCat} {f : W ⟶ X} {g : Y ⟶ Z} (hf : Embedding f) (hg : Embedding g) :
...
then the induced morphism `W ×ₛ X ⟶ Y ×ₜ Z` is also an embedding.
  ...
  X  ⟶  Z
-/
theorem pullback_map_embedding_of_embeddings {W X Y Z S T : TopCat} (f₁ : W ⟶ S) (f₂ : X ⟶ S) (g₁ : Y ⟶ T) (g₂ : Z ⟶ T)
...
is mono, then the induced morphism `W ×ₛ X ⟶ Y ×ₜ Z` is also an open embedding.
  ...
  X  ⟶  Z
-/
theorem pullback_map_open_embedding_of_open_embeddings {W X Y Z S T : TopCat} (f₁ : W ⟶ S) (f₂ : X ⟶ S) (g₁ : Y ⟶ T)
...
theorem fst_embedding_of_right_embedding {X Y S : TopCat} (f : X ⟶ S) {g : Y ⟶ S} (H : Embedding g) :
...
theorem snd_open_embedding_of_left_open_embedding {X Y S : TopCat} {f : X ⟶ S} (H : OpenEmbedding f) (g : Y ⟶ S) :
...
theorem fst_open_embedding_of_right_open_embedding {X Y S : TopCat} (f : X ⟶ S) {g : Y ⟶ S} (H : OpenEmbedding g) :
...
theorem open_embedding_of_pullback_open_embeddings {X Y S : TopCat} {f : X ⟶ S} {g : Y ⟶ S} (H₁ : OpenEmbedding f)
...
theorem coequalizer_is_open_iff (F : walking_parallel_pair ⥤ TopCat.{u}) (U : Set ((colimit F : _) : Type u)) :
...
variable {J : Type v} [SmallCategory J] [IsCofiltered J] (F : J ⥤ TopCat.{max v u}) (C : Cone F) (hC : IsLimit C)
...
theorem partialSections.nonempty [IsCofiltered J] [h : ∀ j : J, Nonempty (F.obj j)] {G : Finset J}
    (H : Finset (FiniteDiagramArrow G)) : (partialSections F H).Nonempty := by classical
  use fun j : J =>
...
theorem partialSections.directed : Directed Superset fun G : FiniteDiagram J => partialSections F G.2 := by classical
  intro A B
...
theorem nonempty_sections_of_fintype_inverse_system {J : Type u} [Preorder J] [IsDirected J (· ≤ ·)] (F : Jᵒᵖ ⥤ Type v)
