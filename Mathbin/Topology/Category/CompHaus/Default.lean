...
theorem is_iso_of_bijective {X Y : CompHaus.{u}} (f : X ⟶ Y) (bij : Function.Bijective f) : is_iso f :=
  by 
...
noncomputable def iso_of_bijective {X Y : CompHaus.{u}} (f : X ⟶ Y) (bij : Function.Bijective f) : X ≅ Y :=
  by 
    letI this := is_iso_of_bijective _ bij <;> exact as_iso f

end CompHaus

-- error in Topology.Category.CompHaus.Default: ././Mathport/Syntax/Translate/Basic.lean:702:9: unsupported derive handler full
...
instance CompHaus.forget_reflects_isomorphisms : reflects_isomorphisms (forget CompHaus.{u}) :=
  ⟨by 
      introI A B f hf <;> exact CompHaus.is_iso_of_bijective _ ((is_iso_iff_bijective f).mp hf)⟩
...
`Top.limit_cone`. -/
def limit_cone {J : Type u} [small_category J] (F : J ⥤ CompHaus.{u}) : limits.cone F :=
  { x :=
      { toTop := (Top.limitCone (F ⋙ compHausToTop)).x,
        IsCompact :=
          by 
            show CompactSpace («expr↥ » { u : ∀ j, F.obj j | ∀ {i j : J} f : i ⟶ j, (F.map f) (u i) = u j })
        ...
        is_hausdorff :=
          show T2Space («expr↥ » { u : ∀ j, F.obj j | ∀ {i j : J} f : i ⟶ j, (F.map f) (u i) = u j }) from
            inferInstance },
    π :=
  ...
  by 
    ...
    ·
      ...
      ·
        ...
        rintro ⟨y', hy'⟩
        exact hy y' hy' 
      haveI  : NormalSpace («expr↥ » Y.to_Top) := normal_of_compact_t2 
...
theorem mono_iff_injective {X Y : CompHaus.{u}} (f : X ⟶ Y) : mono f ↔ Function.Injective f :=
  by 
    ...
    ·
      ...
      applyFun fun e => e PUnit.unit  at this 
      exact this
    ·
