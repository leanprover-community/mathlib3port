...
theorem is_basis_iff_cover {B : Set (opens α)} : is_basis B ↔ ∀ U : opens α, ∃ (Us : _)(_ : Us ⊆ B), U = Sup Us := by
  constructor
  · intro hB U
...
protected def Equivₓ (f : α ≃ₜ β) : opens α ≃ opens β where
  toFun := opens.comap f.symm.to_continuous_map
  invFun := opens.comap f.to_continuous_map
  left_inv := by
    intro U
    ext1
