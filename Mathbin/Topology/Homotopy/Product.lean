...
def pi (γ : ∀ i, Path.Homotopic.Quotient (as i) (bs i)) : Path.Homotopic.Quotient as bs :=
  (Quotientₓ.map Path.pi fun x y hxy => Nonempty.map (pi_homotopy x y) (Classical.nonempty_piₓ.mpr hxy))
    (Quotientₓ.choice γ)
...
def Prod (q₁ : Path.Homotopic.Quotient a₁ a₂) (q₂ : Path.Homotopic.Quotient b₁ b₂) :
  ...
  Quotientₓ.map₂ Path.prod (fun p₁ p₁' h₁ p₂ p₂' h₂ => Nonempty.map2 prod_homotopy h₁ h₂) q₁ q₂
...
@[simp]
theorem proj_left_prod : proj_left (Prod q₁ q₂) = q₁ := by
  apply Quotientₓ.induction_on₂ q₁ q₂
  intro p₁ p₂
  unfold proj_left
...
@[simp]
theorem proj_right_prod : proj_right (Prod q₁ q₂) = q₂ := by
  apply Quotientₓ.induction_on₂ q₁ q₂
  intro p₁ p₂
  unfold proj_right
  rw [prod_lift, ← Path.Homotopic.map_lift]
