...
theorem extend_apply_of_le_zero (F : homotopy f₀ f₁) {t : ℝ} (ht : t ≤ 0) (x : X) : F.extend t x = f₀ x :=
  by 
    rw [←F.apply_zero]
    exact ContinuousMap.congr_fun (Set.Icc_extend_of_le_left (@zero_le_one ℝ _) F.curry ht) x

theorem extend_apply_of_one_le (F : homotopy f₀ f₁) {t : ℝ} (ht : 1 ≤ t) (x : X) : F.extend t x = f₁ x :=
  by 
    rw [←F.apply_one]
...
theorem trans_apply {f₀ f₁ f₂ : C(X, Y)} (F : homotopy f₀ f₁) (G : homotopy f₁ f₂) (x : I × X) :
  (F.trans G) x =
    if h : (x.1 : ℝ) ≤ 1 / 2 then F (⟨2*x.1, (UnitInterval.mul_pos_mem_iff zero_lt_two).2 ⟨x.1.2.1, h⟩⟩, x.2) else
      G (⟨(2*x.1) - 1, UnitInterval.two_mul_sub_one_mem_iff.2 ⟨(not_leₓ.1 h).le, x.1.2.2⟩⟩, x.2) :=
  show ite _ _ _ = _ by 
    splitIfs <;>
      ·
...
theorem extend_prop (F : homotopy_with f₀ f₁ P) (t : ℝ) : P (F.to_homotopy.extend t) :=
  by 
    byCases' ht₀ : 0 ≤ t
    ·
      byCases' ht₁ : t ≤ 1
      ...
      ·
        convert F.prop 1 
        ext 
        rw [F.to_homotopy.extend_apply_of_one_le (le_of_not_leₓ ht₁), F.to_homotopy.curry_apply,
          F.to_homotopy.apply_one]
    ·
      convert F.prop 0 
      ext 
      rw [F.to_homotopy.extend_apply_of_le_zero (le_of_not_leₓ ht₀), F.to_homotopy.curry_apply,
