...
/- ./././Mathport/Syntax/Translate/Command.lean:388:30: infer kinds are unsupported in Lean 4: #[`hequiv_unit] [] -/
...
theorem contractible_iff_id_nullhomotopic (Y : Type _) [TopologicalSpace Y] :
  ...
  refine_struct
    {
      hequiv_unit :=
        ⟨{  toFun := continuous_map.const _ ()
            invFun := continuous_map.const _ p }⟩ }
  · exact h.symm;
