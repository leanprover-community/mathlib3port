...
theorem hcomp_apply (F : homotopy p₀ q₀) (G : homotopy p₁ q₁) (x : I × I) :
  F.hcomp G x =
    if h : (x.2 : ℝ) ≤ 1 / 2 then F.eval x.1 ⟨2*x.2, (UnitInterval.mul_pos_mem_iff zero_lt_two).2 ⟨x.2.2.1, h⟩⟩ else
      G.eval x.1 ⟨(2*x.2) - 1, UnitInterval.two_mul_sub_one_mem_iff.2 ⟨(not_leₓ.1 h).le, x.2.2.2⟩⟩ :=
  show ite _ _ _ = _ by 
    splitIfs <;> exact Path.extend_extends _ _
