...
instance
  : CategoryTheory.Groupoid ( FundamentalGroupoid X )
  where
    Hom x y := Path.Homotopic.Quotient x y
      id x := ⟦ Path.refl x ⟧
      comp x y z := Quotientₓ.map₂ Path.trans fun p₀ : Path x y p₁ hp q₀ q₁ hq => Path.Homotopic.hcomp hp hq
      id_comp'
        x y f
        :=
        Quotientₓ.induction_on
          f fun a => show ⟦ ( Path.refl x ) . trans a ⟧ = ⟦ a ⟧ from Quotientₓ.sound ⟨ Path.Homotopy.reflTrans a ⟩
      comp_id'
        x y f
        :=
        Quotientₓ.induction_on
          f fun a => show ⟦ a.trans ( Path.refl y ) ⟧ = ⟦ a ⟧ from Quotientₓ.sound ⟨ Path.Homotopy.transRefl a ⟩
      assoc'
        w x y z f g h
        :=
        Quotientₓ.induction_on₃
            ...
            fun
              p q r
                ...
                show
                  ...
                  from Quotientₓ.sound ⟨ Path.Homotopy.transAssoc p q r ⟩
      inv
        x y p
        :=
        Quotientₓ.lift
          ( fun l : Path x y => ⟦ l.symm ⟧ ) ( by rintro a b ⟨ h ⟩ rw [ Quotientₓ.eq ] exact ⟨ h.symm₂ ⟩ ) p
      inv_comp'
        x y f
        :=
        Quotientₓ.induction_on
          f
            fun
              a
                =>
                show
                  ⟦ a.symm.trans a ⟧ = ⟦ Path.refl y ⟧
                  from Quotientₓ.sound ⟨ ( Path.Homotopy.reflSymmTrans a ) . symm ⟩
      comp_inv'
        x y f
        :=
        Quotientₓ.induction_on
          f
            fun
              a
                =>
                show
                  ⟦ a.trans a.symm ⟧ = ⟦ Path.refl x ⟧
                  from Quotientₓ.sound ⟨ ( Path.Homotopy.reflTransSymm a ) . symm ⟩
