...
-/
def hom_to_index (V : OpensLeCover U) : V.val ⟶ U (index V) :=
...
-/
def opens_le_cover_cocone : Cocone (fullSubcategoryInclusion _ : OpensLeCover U ⥤ Opens X) where
  x := supr U
...
-/
@[simp]
def pairwise_to_opens_le_cover_obj : Pairwise ι → OpensLeCover U
...
-/
@[simps]
def pairwise_to_opens_le_cover : Pairwise ι ⥤ OpensLeCover U where
...
-/
def pairwise_diagram_iso : Pairwise.diagram U ≅ pairwiseToOpensLeCover U ⋙ fullSubcategoryInclusion _ where
  Hom :=
    { app := by
...
def is_limit_opens_le_equiv_generate₂ (R : Presieve Y) (hR : Sieve.generate R ∈ Opens.grothendieckTopology X Y) :
