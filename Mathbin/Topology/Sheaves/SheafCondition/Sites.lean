...
theorem is_sheaf_sites_of_is_sheaf_spaces (Fsh : F.is_sheaf) : presheaf.is_sheaf (Opens.grothendieckTopology X) F :=
  by 
    ...
    refine' ⟨_⟩
    apply
      (is_limit.of_cone_equiv (cones.postcompose_equivalence (covering_of_presieve.diagram_nat_iso F U R : _))).toFun 
...
def is_terminal_of_eq_empty (F : X.sheaf C) {U : opens X} (h : U = ∅) : limits.is_terminal (F.val.obj (op U)) :=
  by 
