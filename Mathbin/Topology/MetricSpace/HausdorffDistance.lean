...
theorem mem_iff_infEdist_zero_of_closed (h : IsClosed s) : x ∈ s ↔ infEdist x s = 0 :=
  by
  convert ← mem_closure_iff_infEdist_zero
  exact h.closure_eq
...
theorem IsOpen.exists_unionᵢ_isClosed {U : Set α} (hU : IsOpen U) :
  ...
  have F_subset : ∀ n, F n ⊆ U := by
    ...
    contrapose! this
    exact infEdist_zero_of_mem this
  ...
  · refine' Subset.antisymm (by simp only [unionᵢ_subset_iff, F_subset, forall_const]) fun x hx => _
    ...
    rw [mem_iff_infEdist_zero_of_closed hU.is_closed_compl] at this
...
theorem hausdorffEdist_def {α : Type u} [PseudoEmetricSpace α] (s t : Set α) :
    hausdorffEdist s t = (⨆ x ∈ s, infEdist x t) ⊔ ⨆ y ∈ t, infEdist y s := by rw [hausdorffEdist]
...
theorem hausdorffEdist_le_of_infEdist {r : ℝ≥0∞} (H1 : ∀ x ∈ s, infEdist x t ≤ r)
  ...
  by
  simp only [hausdorffEdist, sup_le_iff, supᵢ_le_iff]
  exact ⟨H1, H2⟩
...
theorem hausdorffEdist_le_of_mem_edist {r : ℝ≥0∞} (H1 : ∀ x ∈ s, ∃ y ∈ t, edist x y ≤ r)
  ...
  by
  refine' hausdorffEdist_le_of_infEdist _ _
  · intro x xs
...
theorem hausdorffEdist_zero_iff_closure_eq_closure :
  ...
  calc
    hausdorffEdist s t = 0 ↔ s ⊆ closure t ∧ t ⊆ closure s := by
      simp only [hausdorffEdist_def, Ennreal.sup_eq_zero, Ennreal.supᵢ_eq_zero, ←
        mem_closure_iff_infEdist_zero, subset_def]
...
theorem infDist_nonneg : 0 ≤ infDist x s := by simp [infDist]
...
theorem infEdist_ne_top (h : s.Nonempty) : infEdist x s ≠ ⊤ :=
  ...
  calc
    infEdist x s ≤ edist x y := infEdist_le_edist_of_mem hy
...
@[simp]
theorem infDist_singleton : infDist x {y} = dist x y := by simp [infDist, infEdist, dist_edist]
...
theorem infDist_le_dist_of_mem (h : y ∈ s) : infDist x s ≤ dist x y :=
  by
  rw [dist_edist, infDist, Ennreal.toReal_le_toReal (infEdist_ne_top ⟨_, h⟩) (edist_ne_top _ _)]
  exact infEdist_le_edist_of_mem h
...
theorem infDist_le_infDist_of_subset (h : s ⊆ t) (hs : s.Nonempty) : infDist x t ≤ infDist x s :=
  ...
  rw [infDist, infDist, Ennreal.toReal_le_toReal (infEdist_ne_top ht) (infEdist_ne_top hs)]
  exact infEdist_anti h
...
theorem infDist_le_infDist_add_dist : infDist x s ≤ infDist y s + dist x y :=
  ...
  cases' s.eq_empty_or_nonempty with hs hs
  · simp [hs, dist_nonneg]
  · rw [infDist, infDist, dist_edist, ← Ennreal.toReal_add (infEdist_ne_top hs) (edist_ne_top _ _),
      Ennreal.toReal_le_toReal (infEdist_ne_top hs)]
    · exact infEdist_le_infEdist_add_edist
    · simp [Ennreal.add_eq_top, infEdist_ne_top hs, edist_ne_top]
...
theorem dist_le_infDist_add_diam (hs : Bounded s) (hy : y ∈ s) : dist x y ≤ infDist x s + diam s :=
  by
  have A : infEdist x s ≠ ∞ := infEdist_ne_top ⟨y, hy⟩
  have B : Emetric.diam s ≠ ∞ := hs.ediam_ne_top
  rw [infDist, diam, ← Ennreal.toReal_add A B, dist_edist]
  apply (Ennreal.toReal_le_toReal _ _).2
  · exact edist_le_infEdist_add_ediam hy
  · rw [edist_dist]
...
theorem IsClosed.not_mem_iff_infDist_pos (h : IsClosed s) (hs : s.Nonempty) :
  ...
  rw [← not_iff_not]
  push_neg
  simp [h.mem_iff_inf_dist_zero hs, le_antisymm_iff, infDist_nonneg]
...
theorem IsClosed.exists_infDist_eq_dist [ProperSpace α] (h : IsClosed s) (hne : s.Nonempty)
  ...
  obtain ⟨y, ⟨hys, hyx⟩, hyd⟩ : ∃ y ∈ t, infDist x t = dist x y :=
...
theorem exists_mem_closure_infDist_eq_dist [ProperSpace α] (hne : s.Nonempty) (x : α) :
  ...
  simpa only [infDist_eq_closure] using is_closed_closure.exists_inf_dist_eq_dist hne.closure x
...
theorem infDist_le_hausdorffDist_of_mem (hx : x ∈ s) (fin : hausdorffEdist s t ≠ ⊤) :
  ...
  by
  have ht : t.nonempty := nonempty_of_hausdorffEdist_ne_top ⟨x, hx⟩ fin
  rw [hausdorffDist, infDist, Ennreal.toReal_le_toReal (infEdist_ne_top ht) fin]
  exact infEdist_le_hausdorffEdist_of_mem hx
...
theorem exists_dist_lt_of_hausdorffDist_lt {r : ℝ} (h : x ∈ s) (H : hausdorffDist s t < r)
  ...
  have : hausdorffEdist s t < Ennreal.ofReal r := by
    rwa [hausdorffDist, ← Ennreal.toReal_ofReal (le_of_lt r0),
      Ennreal.toReal_lt_toReal fin ennreal.of_real_ne_top] at H
...
theorem infDist_le_infDist_add_hausdorffDist (fin : hausdorffEdist s t ≠ ⊤) :
  ...
  by
  rcases empty_or_nonempty_of_hausdorffEdist_ne_top fin with (⟨hs, ht⟩ | ⟨hs, ht⟩)
  · simp only [hs, ht, hausdorffDist_empty, infDist_empty, zero_add]
  rw [infDist, infDist, hausdorffDist, ← Ennreal.toReal_add (infEdist_ne_top hs) fin,
    Ennreal.toReal_le_toReal (infEdist_ne_top ht)]
  · exact infEdist_le_infEdist_add_hausdorffEdist
  · exact Ennreal.add_ne_top.2 ⟨infEdist_ne_top hs, fin⟩
...
theorem hausdorffDist_triangle (fin : hausdorffEdist s t ≠ ⊤) :
  ...
  by
  by_cases hausdorffEdist s u = ⊤
  ·
    calc
      hausdorffDist s u = 0 + 0 := by simp [hausdorffDist, h]
      _ ≤ hausdorffDist s t + hausdorffDist t u :=
        add_le_add Hausdorff_dist_nonneg Hausdorff_dist_nonneg
      
  · have Dtu : hausdorffEdist t u < ⊤ :=
      calc
        hausdorffEdist t u ≤ hausdorffEdist t s + hausdorffEdist s u := hausdorffEdist_triangle
        _ = hausdorffEdist s t + hausdorffEdist s u := by simp [hausdorffEdist_comm]
        _ < ⊤ := lt_top_iff_ne_top.mpr <| ennreal.add_ne_top.mpr ⟨fin, h⟩
        
    rw [hausdorffDist, hausdorffDist, hausdorffDist, ← Ennreal.toReal_add fin Dtu.ne,
      Ennreal.toReal_le_toReal h]
    · exact hausdorffEdist_triangle
...
theorem hausdorffDist_zero_iff_closure_eq_closure (fin : hausdorffEdist s t ≠ ⊤) :
  ...
  simp [Hausdorff_edist_zero_iff_closure_eq_closure.symm, hausdorffDist, Ennreal.toReal_eq_zero_iff,
    fin]
...
theorem cthickening_eq_interᵢ_thickening' {δ : ℝ} (δ_nn : 0 ≤ δ) (s : Set ℝ) (hsδ : s ⊆ Ioi δ)
  ...
  · rw [cthickening_eq_interᵢ_cthickening' s hsδ hs E]
    exact interᵢ₂_mono fun ε hε => thickening_subset_cthickening ε E
...
theorem IsCompact.cthickening_eq_bUnion_closedBall {α : Type _} [PseudoMetricSpace α] {δ : ℝ}
  ...
  · simp only [cthickening_empty, unionᵢ_false, unionᵢ_empty]
  refine'
    Subset.antisymm (fun x hx => _) (unionᵢ₂_subset fun x hx => closedBall_subset_cthickening hx _)
  obtain ⟨y, yE, hy⟩ : ∃ y ∈ E, infEdist x E = edist x y := hE.exists_inf_edist_eq_edist hne _
...
theorem cthickening_eq_bUnion_closedBall {α : Type _} [PseudoMetricSpace α] [ProperSpace α]
  ...
  by
  rcases eq_empty_or_nonempty E with (rfl | hne)
  · simp only [cthickening_empty, unionᵢ_false, unionᵢ_empty, closure_empty]
