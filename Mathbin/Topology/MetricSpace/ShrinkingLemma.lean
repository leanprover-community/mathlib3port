...
theorem exists_subset_Union_ball_radius_lt {r : ι → ℝ} (hs : IsClosed s)
  ...
  by
  rcases exists_subset_Union_closed_subset hs (fun i => @is_open_ball _ _ (c i) (r i)) uf us with
    ⟨v, hsv, hvc, hcv⟩
...
theorem exists_locally_finite_subset_Union_ball_radius_lt (hs : IsClosed s) {R : α → ℝ}
    (hR : ∀ x ∈ s, 0 < R x) :
  ...
  rcases refinement_of_locally_compact_sigma_compact_of_nhds_basis_set hs this with
    ⟨ι, c, r', hr', hsub', hfin⟩
  rcases exists_subset_Union_ball_radius_pos_lt (fun i => (hr' i).2.1) hs
