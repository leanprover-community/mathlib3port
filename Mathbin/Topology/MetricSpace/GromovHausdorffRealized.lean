...
theorem HD_below_aux1 {f : Cb X Y} (C : ℝ) {x : X} :
  ...
  let ⟨cf, hcf⟩ := (isBounded_iff_bddBelow_bddAbove.1 f.isBounded_range).1
...
private theorem HD_bound_aux1 (f : Cb X Y) (C : ℝ) :
  ...
  by
  rcases(isBounded_iff_bddBelow_bddAbove.1 f.bounded_range).2 with ⟨Cf, hCf⟩
...
private theorem HD_lipschitz_aux1 (f g : Cb X Y) :
  ...
  by
  rcases(isBounded_iff_bddBelow_bddAbove.1 g.bounded_range).1 with ⟨cg, hcg⟩
  have Hcg : ∀ x, cg ≤ g x := fun x => hcg (mem_range_self x)
  rcases(isBounded_iff_bddBelow_bddAbove.1 f.bounded_range).1 with ⟨cf, hcf⟩
