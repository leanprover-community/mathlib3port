...
protected theorem disjoint (h : IsMetricSeparated s t) : Disjoint s t :=
  let ⟨r, r0, hr⟩ := h
  fun x hx => r0 <| by simpa using hr x hx.1 x hx.2
...
@[simp]
theorem finset_Union_left_iff {ι : Type _} {I : Finsetₓ ι} {s : ι → Set X} {t : Set X} :
...
@[simp]
theorem finset_Union_right_iff {ι : Type _} {I : Finsetₓ ι} {s : Set X} {t : ι → Set X} :
