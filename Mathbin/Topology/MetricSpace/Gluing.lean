...
private theorem glue_eq_of_dist_eq_zero (Φ : Z → X) (Ψ : Z → Y) (ε : ℝ) (ε0 : 0 < ε) :
  ...
  | inl x, inr y, h =>
    ...
    have : 0 + ε ≤ glueDist Φ Ψ ε (inl x) (inr y) := add_le_add this (le_refl ε)
    exfalso
    linarith
  | inr x, inl y, h =>
    by
    ...
    have : 0 + ε ≤ glueDist Φ Ψ ε (inr x) (inl y) := add_le_add this (le_refl ε)
    exfalso
    linarith
...
theorem Sum.dist_eq_glueDist {p q : Sum X Y} (x : X) (y : Y) :
  ...
  by
  cases p <;> cases q <;> first |rfl|simp [Sum.dist, glueDist, dist_comm, add_comm, add_left_comm]
...
def metricSpaceSum : MetricSpace (Sum X Y)
  ...
  dist_triangle p q r := by
    ...
    · simp only [dist, Sum.dist_eq_glueDist p r]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · simp only [dist, Sum.dist_eq_glueDist p q]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · simp only [dist, Sum.dist_eq_glueDist p q]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · simp only [dist, Sum.dist_eq_glueDist q p]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · simp only [dist, Sum.dist_eq_glueDist q p]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · simp only [dist, Sum.dist_eq_glueDist r p]
      exact glueDist_triangle _ _ _ (by norm_num) _ _ _
    · exact dist_triangle _ _ _
  eq_of_dist_eq_zero p q := by
    ...
    · intro h
      simp only [dist, Sum.dist_eq_glueDist p q] at h
      exact glue_eq_of_dist_eq_zero _ _ _ zero_lt_one _ _ h
    · intro h
      simp only [dist, Sum.dist_eq_glueDist q p] at h
      exact glue_eq_of_dist_eq_zero _ _ _ zero_lt_one _ _ h
    · simp only [Sum.dist, dist_eq_zero, imp_self]
...
protected def metricSpace : MetricSpace (Σi, E i) :=
  by
  refine' MetricSpace.ofMetrizable Sigma.dist _ _ Sigma.dist_triangle Sigma.isOpen_iff _
  · rintro ⟨i, x⟩
    simp [Sigma.dist]
  · rintro ⟨i, x⟩ ⟨j, y⟩
...
def toGlueL (hΦ : Isometry Φ) (hΨ : Isometry Ψ) (x : X) : GlueSpace hΦ hΨ :=
  letI : PseudoMetricSpace (Sum X Y) := gluePremetric hΦ hΨ
  ⟦inl x⟧
...
theorem to_glue_commute (hΦ : Isometry Φ) (hΨ : Isometry Ψ) :
  ...
  letI : PseudoMetricSpace (Sum X Y) := gluePremetric hΦ hΨ
  funext
  simp only [comp, toGlueL, toGlueR, Quotient.eq']
  exact glueDist_glued_points Φ Ψ 0 x
...
def inductivePremetric (I : ∀ n, Isometry (f n)) : PseudoMetricSpace (Σn, X n)
  ...
  dist_triangle x y z := by
    ...
    calc
      inductiveLimitDist f x z = dist (leRecOn hx f x.2 : X m) (leRecOn hz f z.2 : X m) :=
        inductiveLimitDist_eq_dist I x z m hx hz
      _ ≤
          dist (leRecOn hx f x.2 : X m) (leRecOn hy f y.2 : X m) +
            dist (leRecOn hy f y.2 : X m) (leRecOn hz f z.2 : X m) :=
        dist_triangle _ _ _
...
def toInductiveLimit (I : ∀ n, Isometry (f n)) (n : ℕ) (x : X n) : Metric.InductiveLimit I :=
  letI : PseudoMetricSpace (Σn, X n) := inductivePremetric I
  ⟦Sigma.mk n x⟧
...
theorem toInductiveLimit_commute (I : ∀ n, Isometry (f n)) (n : ℕ) :
  ...
  by
  funext
  simp only [comp, toInductiveLimit, Quotient.eq']
  show inductiveLimitDist f ⟨n.succ, f n x⟩ ⟨n, x⟩ = 0
  · rw [inductiveLimitDist_eq_dist I ⟨n.succ, f n x⟩ ⟨n, x⟩ n.succ, leRecOn_self, leRecOn_succ,
      leRecOn_self, dist_self]
    exact le_rfl
    exact le_rfl
    exact le_succ _
