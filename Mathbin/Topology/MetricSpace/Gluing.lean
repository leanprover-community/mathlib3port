...
theorem Sum.dist_eq_glue_dist {p q : X ⊕ Y} (x : X) (y : Y) :
...
theorem isometryInr : Isometry (Sum.inr : Y → X ⊕ Y) :=
  Isometry.ofDistEq $ fun x y => rfl
...
theorem isometryMk (i : ι) : Isometry (Sigma.mk i : E i → Σ k, E k) :=
...
#align metric.sigma.complete_space Metric.Sigma.complete_space
...
def toGlueL (hΦ : Isometry Φ) (hΨ : Isometry Ψ) (x : X) : GlueSpace hΦ hΨ :=
  letI : PseudoMetricSpace (X ⊕ Y) := glue_premetric hΦ hΨ
  ⟦inl x⟧
...
def toInductiveLimit (I : ∀ n, Isometry (f n)) (n : ℕ) (x : X n) : Metric.InductiveLimit I :=
  letI : PseudoMetricSpace (Σ n, X n) := inductive_premetric I
  ⟦Sigma.mk n x⟧
