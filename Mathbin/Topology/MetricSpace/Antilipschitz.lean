...
theorem antilipschitz_with_iff_le_mul_dist : AntilipschitzWith K f ↔ ∀ x y, dist x y ≤ K*dist (f x) (f y) :=
  by 
...
theorem mul_le_nndist (hf : AntilipschitzWith K f) (x y : α) : (K⁻¹*nndist x y) ≤ nndist (f x) (f y) :=
  by 
    simpa only [div_eq_inv_mul] using Nnreal.div_le_of_le_mul' (hf.le_mul_nndist x y)

theorem mul_le_dist (hf : AntilipschitzWith K f) (x y : α) : (K⁻¹*dist x y : ℝ) ≤ dist (f x) (f y) :=
  by 
    exactModCast hf.mul_le_nndist x y
...
protected theorem injective {α : Type _} {β : Type _} [EmetricSpace α] [PseudoEmetricSpace β] {K :  ℝ≥0 } {f : α → β}
  (hf : AntilipschitzWith K f) : Function.Injective f :=
  fun x y h =>
    by 
      simpa only [h, edist_self, mul_zero, edist_le_zero] using hf x y

theorem mul_le_edist (hf : AntilipschitzWith K f) (x y : α) : (K⁻¹*edist x y : ℝ≥0∞) ≤ edist (f x) (f y) :=
  by 
...
theorem cod_restrict (hf : AntilipschitzWith K f) {s : Set β} (hs : ∀ x, f x ∈ s) :
  AntilipschitzWith K (s.cod_restrict f hs) :=
  fun x y => hf x y

theorem to_right_inv_on' {s : Set α} (hf : AntilipschitzWith K (s.restrict f)) {g : β → α} {t : Set β}
  (g_maps : maps_to g t s) (g_inv : right_inv_on g f t) : LipschitzWith K (t.restrict g) :=
  fun x y =>
    by 
...
theorem of_subsingleton [Subsingleton α] {K :  ℝ≥0 } : AntilipschitzWith K f :=
  fun x y =>
    by 
...
theorem LipschitzWith.to_right_inverse [PseudoEmetricSpace α] [PseudoEmetricSpace β] {K :  ℝ≥0 } {f : α → β}
  (hf : LipschitzWith K f) {g : β → α} (hg : Function.RightInverse g f) : AntilipschitzWith K g :=
  fun x y =>
    by 
