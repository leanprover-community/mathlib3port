...
alias antilipschitz_with_iff_le_mul_nndist ↔
  AntilipschitzWith.le_mul_nndist AntilipschitzWith.ofLeMulNndist
...
alias antilipschitz_with_iff_le_mul_dist ↔
  AntilipschitzWith.le_mul_dist AntilipschitzWith.ofLeMulDist
...
theorem boundedPreimage (hf : AntilipschitzWith K f) {s : Set β} (hs : Bounded s) :
    Bounded (f ⁻¹' s) :=
...
theorem LipschitzWith.toRightInverse [PseudoEmetricSpace α] [PseudoEmetricSpace β] {K : ℝ≥0}
