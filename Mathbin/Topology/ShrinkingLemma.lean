...
theorem find_mem {c : Set (partial_refinement u s)} (i : ι) (ne : c.nonempty) : find c Ne i ∈ c := by
  rw [find]
  split_ifs
...
def chain_Sup (c : Set (partial_refinement u s)) (hc : chain (· ≤ ·) c) (ne : c.nonempty)
    (hfin : ∀, ∀ x ∈ s, ∀, finite { i | x ∈ u i }) (hU : s ⊆ ⋃ i, u i) : partial_refinement u s := by
  refine'
...
theorem exists_gt (v : partial_refinement u s) (hs : IsClosed s) (i : ι) (hi : i ∉ v.carrier) :
    ∃ v' : partial_refinement u s, v < v' := by
