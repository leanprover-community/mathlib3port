...
but is expected to have type
  forall {X : Type.{u2}} {Y : Type.{u1}} [_inst_1 : TopologicalSpace.{u2} X] [_inst_2 : TopologicalSpace.{u1} Y] {f : ContinuousMap.{u2, u1} X Y _inst_1 _inst_2} {A : DiscreteQuotient.{u2} X _inst_1} {B : DiscreteQuotient.{u1} Y _inst_2} (cond : DiscreteQuotient.LeComap.{u2, u1} X Y _inst_1 _inst_2 f A B) (x : X), Eq.{succ u1} (Quotient.{succ u1} Y (DiscreteQuotient.toSetoid.{u1} Y _inst_2 B)) (DiscreteQuotient.map.{u2, u1} X Y _inst_1 _inst_2 A B f cond (DiscreteQuotient.proj.{u2} X _inst_1 A x)) (DiscreteQuotient.proj.{u1} Y _inst_2 B (FunLike.coe.{max (succ u2) (succ u1), succ u2, succ u1} (ContinuousMap.{u2, u1} X Y _inst_1 _inst_2) X (fun (_x : X) => (fun (x._@.Mathlib.Topology.ContinuousFunction.Basic._hyg.669 : X) => Y) _x) (ContinuousMapClass.toFunLike.{max u2 u1, u2, u1} (ContinuousMap.{u2, u1} X Y _inst_1 _inst_2) X Y _inst_1 _inst_2 (ContinuousMap.instContinuousMapClassContinuousMap.{u2, u1} X Y _inst_1 _inst_2)) f x))
