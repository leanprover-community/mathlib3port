...
theorem is_compact_of_is_closed_subset (hs : IsCompact s) (ht : IsClosed t) (h : t ⊆ s) : IsCompact t :=
...
theorem Set.Finite.is_compact_bUnion {s : Set ι} {f : ι → Set α} (hs : s.Finite) (hf : ∀ i ∈ s, IsCompact (f i)) :
    IsCompact (⋃ i ∈ s, f i) :=
...
theorem is_compact_accumulate {K : ℕ → Set α} (hK : ∀ n, IsCompact (K n)) (n : ℕ) : IsCompact (accumulate K n) :=
  (finite_le_nat n).is_compact_bUnion fun k _ => hK k
#align is_compact_accumulate is_compact_accumulate

theorem is_compact_Union {f : ι → Set α} [Finite ι] (h : ∀ i, IsCompact (f i)) : IsCompact (⋃ i, f i) := by
...
theorem exists_subset_nhds_of_is_compact' {ι : Type _} [Nonempty ι] {V : ι → Set α} (hV : Directed (· ⊇ ·) V)
...
theorem cluster_point_of_compact [CompactSpace α] (f : Filter α) [NeBot f] : ∃ x, ClusterPt x f := by
  simpa using is_compact_univ (show f ≤ 𝓟 univ by simp)
...
/- ./././Mathport/Syntax/Translate/Command.lean:347:30: infer kinds are unsupported in Lean 4: #[`noncompact_univ] [] -/
...
theorem exists_subset_nhds_of_compact_space [CompactSpace α] {ι : Type _} [Nonempty ι] {V : ι → Set α}
...
theorem exists_compact_between [hα : LocallyCompactSpace α] {K U : Set α} (hK : IsCompact K) (hU : IsOpen U)
  ...
  refine' ⟨_, t.is_compact_bUnion fun x _ => hVc x, fun x hx => _, Set.Union₂_subset fun i _ => hKV i⟩
...
theorem SigmaCompactSpace.ofCountable (S : Set (Set α)) (Hc : S.Countable) (Hcomp : ∀ s ∈ S, IsCompact s)
...
theorem countable_cover_nhds_of_sigma_compact {f : α → Set α} (hf : ∀ x, f x ∈ 𝓝 x) :
  ...
  rcases countable_cover_nhds_within_of_sigma_compact isClosedUniv fun x _ => hf x with ⟨s, -, hsc, hsU⟩
...
theorem IsClopen.preimage {s : Set β} (h : IsClopen s) {f : α → β} (hf : Continuous f) : IsClopen (f ⁻¹' s) :=
...
theorem ContinuousOn.preimage_clopen_of_clopen {f : α → β} {s : Set α} {t : Set β} (hf : ContinuousOn f s)
  ...
  ⟨ContinuousOn.preimage_open_of_open hf hs.1 ht.1, ContinuousOn.preimageClosedOfClosed hf hs.2 ht.2⟩
...
theorem continuous_bool_indicator_iff_clopen (U : Set X) : Continuous U.boolIndicator ↔ IsClopen U := by
  constructor
  · intro hc
...
/- ./././Mathport/Syntax/Translate/Basic.lean:611:2: warning: expanding binder collection (s «expr ∈ » irreducible_components[irreducible_components] α) -/
...
theorem irreducible_component_mem_irreducible_components (x : α) : irreducibleComponent x ∈ irreducibleComponents α :=
...
/- ./././Mathport/Syntax/Translate/Command.lean:347:30: infer kinds are unsupported in Lean 4: #[`is_preirreducible_univ] [] -/
...
/- ./././Mathport/Syntax/Translate/Command.lean:347:30: infer kinds are unsupported in Lean 4: #[`to_nonempty] [] -/
...
theorem subset_closure_inter_of_is_preirreducible_of_is_open {S U : Set α} (hS : IsPreirreducible S) (hU : IsOpen U)
  ...
  by_contra h'
  obtain ⟨x, h₁, h₂, h₃⟩ :=
    hS _ (closure (S ∩ U)ᶜ) hU (is_open_compl_iff.mpr isClosedClosure) h (set.inter_compl_nonempty_iff.mpr h')
...
theorem IsPreirreducible.subset_irreducible {S U Z : Set α} (hZ : IsPreirreducible Z) (hU : U.Nonempty) (hU' : IsOpen U)
    (h₁ : U ⊆ S) (h₂ : S ⊆ Z) : IsIrreducible S := by classical
  obtain ⟨z, hz⟩ := hU
