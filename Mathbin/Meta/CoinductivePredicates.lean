...
unsafe def add_coinductive_predicate (u_names : List Name) (params : List expr) (preds : List$ expr × List expr) :
  ...
  do 
      ...
      (preds.zip pre_info).mmap
          fun ⟨⟨c, is⟩, ls, f₁, f₂⟩ =>
            do 
                ...
                is.mmap
                    fun i =>
                      do 
                        ...
                        let loc_args :=
                          args.map$
                            fun e => (fs₁.zip preds).foldl (fun e : expr ⟨f, c, _⟩ => e.replace_with (pred_g c) f) e 
              ...
              return
                  { pd_name := c.local_uniq_name, type := c.local_type, f₁, f₂, u_f, intros, locals := ls, params,
                    u_names }
    pds.mmap'
        fun pd : coind_pred =>
          do 
              ...
              pds.mmap
                  fun pd =>
                    do 
                      ...
                      return [pd.f₁, pd.f₂, h]
            pd.add_theorem (pd.func.const_name ++ "mono") ((pd.le func_f₁ func_f₂).pis$ params ++ mono_params.join)
                do 
                      ...
                      fun ⟨n, pp_n, t⟩ =>
                        solve1$
                          do 
                            let bs ← intros 
                            let ms ←
                              apply_core ((const n u_params).app_of_list$ ps ++ fs.map Prod.fst)
    ...
    pds.mmap'
        fun pd =>
          do 
            ...
            pd.add_theorem (pd.pred.const_name ++ "corec_functional") ((pd.le pd.f₁ pd.pred_g).pis$ params ++ fs₁ ++ hs)
                do 
    ...
    pds.enum.mmap'
        fun ⟨n, pd⟩ =>
          do 
            ...
            pd.add_theorem (pd.pred.const_name ++ "destruct") (destruct.pis params)
                do 
    ...
    pds.mmap'
        fun pd =>
          pd.add_theorem (pd.pred.const_name ++ "construct") ((pd.le (func_f pd) pd.pred_g).pis params)
            do 
              let ps ← intro_lst$ params.map local_pp_name 
              let func_pred_g :=
                fun pd : coind_pred => pd.func.app_of_list$ ps ++ pds.map fun pd : coind_pred => pd.pred.app_of_list ps 
    ...
    pds.mmap'
        fun pd =>
          pd.intros.mmap'
            fun r =>
              pd.add_theorem r.orig_nm (r.type.pis params)$
                do 
                  let ps ← intro_lst$ params.map local_pp_name 
                  let bs ← intros 
                  eapply$ pd.construct 
...
unsafe def coinduction (rule : expr) (ns : List Name) : tactic Unit :=
  focus1$
    do 
      let ctxts' ← intros 
      let ctxts ←
        ctxts'.mmap fun v => local_const v.local_uniq_name v.local_pp_name v.local_binding_info <$> infer_type v 
      ...
      solve1
          do 
            let eqs ←
              (mk_and_lst <$> eqs.mmap fun ⟨i, m⟩ => mk_app `eq [m, i] >>= instantiate_mvars) <|>
                  do 
