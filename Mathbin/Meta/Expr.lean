...
| nm, n =>
  let (pfx, height) := pop_nth_prefix_aux nm.get_prefix n 
  if height ≤ n then (anonymous, height+1) else (nm.update_prefix pfx, height+1)
...
unsafe def contains_constant (e : expr) (p : Name → Prop) [DecidablePred p] : Bool :=
  e.fold ff fun e' _ b => if p e'.const_name then tt else b
...
/-- Simplifies the expression `t` with the specified options.
  ...
  `pr : e = new_e`. -/
unsafe def simp (t : expr) (cfg : simp_config := {  }) (discharger : tactic Unit := failed) (no_defaults := ff)
  (attr_names : List Name := []) (hs : List simp_arg_type := []) : tactic (expr × expr × name_set) :=
  do 
...
unsafe def dsimp (t : expr) (cfg : dsimp_config := {  }) (no_defaults := ff) (attr_names : List Name := [])
  (hs : List simp_arg_type := []) : tactic expr :=
  do 
    let (s, to_unfold) ← mk_simp_set no_defaults attr_names hs 
...
| expr.local_const _ _ BinderInfo.default _ => tt
| _ => ff
...
unsafe def is_implicitly_included_variable (e : expr) (vs : List expr) : Bool :=
  if ¬e.local_pp_name.to_string.starts_with "_" then e ∈ vs else
    e.local_type.fold tt$ fun se _ b => if ¬b then ff else if ¬se.is_local_constant then tt else se ∈ vs
...
| [], vs, rs, ff => vs
| e :: rest, vs, rs, b =>
  let (vs, rs, b) := if e.is_implicitly_included_variable vs then (e :: vs, rs, tt) else (vs, e :: rs, b)
...
protected unsafe def simple_infer_type (env : environment) (e : expr) : exceptional expr :=
  do 
  ...
  e.replace$
    fun e _ =>
      do 
        ...
        if needed_n ≤ es.length then some e' else
            do 
      ...
      | app g x =>
        ...
        if n_args ∈ (reorder.find nm).iget ∧ test g.get_app_args.head then
          ...
          if n_args = (relevant.find nm).lhoare 0 ∧ f.is_constant ∧ ¬test x then
            some$ (f.mk_app$ g.get_app_args.map apply_replacement_fun) (apply_replacement_fun x) else none
      | _ => none
...
unsafe def is_eta_expansion_aux (val : expr) (l : List (Name × expr)) : tactic (Option expr) :=
  do 
...
unsafe def is_eta_expansion (val : expr) : tactic (Option expr) :=
  do 
    let e ← get_env 
...
unsafe def in_current_file (d : declaration) : tactic Bool :=
  do 
    let e ← get_env 
...
unsafe def is_auto_generated (e : environment) (d : declaration) : Bool :=
  e.is_constructor d.to_name ∨
    (e.is_projection d.to_name).isSome ∨
      e.is_constructor d.to_name.get_prefix ∧ d.to_name.last ∈ ["inj", "inj_eq", "sizeof_spec", "inj_arrow"] ∨
        e.is_inductive d.to_name.get_prefix ∧
          ...
          d.to_name.has_prefix fun nm => e.is_ginductive' nm
...
private unsafe def print_thm (nm : Name) (tp : expr) (body : task expr) : tactic format :=
  do 
    let tp ← pp tp 
...
private unsafe def print_defn (nm : Name) (tp : expr) (body : expr) (is_trusted : Bool) : tactic format :=
  do 
    let tp ← pp tp 
    let body ← pp body 
...
private unsafe def print_cnst (nm : Name) (tp : expr) (is_trusted : Bool) : tactic format :=
  do 
    let tp ← pp tp 
    return$ ("<" ++ if is_trusted then "constant " else "meta constant ") ++ to_fmt nm ++ " : " ++ tp ++ ">"

/-- formats the arguments of a `declaration.ax` -/
private unsafe def print_ax (nm : Name) (tp : expr) : tactic format :=
  do 
    let tp ← pp tp 
