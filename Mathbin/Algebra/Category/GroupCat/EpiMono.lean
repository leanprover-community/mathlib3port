...
theorem ker_eq_bot_of_cancel {f : A →* B} (h : ∀ u v : f.ker →* A, f.comp u = f.comp v → u = v) : f.ker = ⊥ := by
  simpa using _root_.congr_arg range (h f.ker.subtype 1 (by tidy))
...
theorem range_eq_top_of_cancel {f : A →* B} (h : ∀ u v : B →* B ⧸ f.range, u.comp f = v.comp f → u = v) : f.range = ⊤ :=
  ...
  · ext1
    ...
    rw [show (1 : B ⧸ f.range) = (1 : B) from QuotientGroup.coe_one _, QuotientGroup.eq, inv_one, one_mul]
    exact ⟨x, rfl⟩
...
theorem h_apply_infinity (x : B) (hx : x ∈ f.range) : (h x) ∞ = ∞ := by
  simp only [H, MonoidHom.coe_mk, Equiv.to_fun_as_coe, Equiv.coe_trans, Function.comp_app]
...
theorem h_apply_from_coset_nin_range (x : B) (hx : x ∈ f.range) (b : B) (hb : b ∉ f.range) :
  ...
  simp only [H, tau, MonoidHom.coe_mk, Equiv.to_fun_as_coe, Equiv.coe_trans, Function.comp_app]
  rw [Equiv.symm_swap,
...
theorem g_ne_h (x : B) (hx : x ∉ f.range) : g ≠ h := by
  ...
  simp only [MonoidHom.coe_range, Subtype.coe_mk, Equiv.symm_swap, Equiv.to_fun_as_coe, Equiv.coe_trans,
...
instance forget_Group_preserves_mono :
    (forget
        GroupCat).PreservesMonomorphisms where preserves X Y f e := by
    rwa [mono_iff_injective, ← CategoryTheory.mono_iff_injective] at e

@[to_additive]
instance forget_Group_preserves_epi :
    (forget
        GroupCat).PreservesEpimorphisms where preserves X Y f e := by
...
instance forget_CommGroup_preserves_mono :
    (forget
        CommGroupCat).PreservesMonomorphisms where preserves X Y f e := by
    rwa [mono_iff_injective, ← CategoryTheory.mono_iff_injective] at e

@[to_additive]
instance forget_CommGroup_preserves_epi :
    (forget
        CommGroupCat).PreservesEpimorphisms where preserves X Y f e := by
    rwa [epi_iff_surjective, ← CategoryTheory.epi_iff_surjective] at e
