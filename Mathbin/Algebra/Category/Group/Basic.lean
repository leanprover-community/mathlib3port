...
@[ to_additive ] instance one.unique : Unique ( 1 : Groupₓₓ ) where default := 1 uniq a := by cases a rfl
...
-- failed to format: format: uncaught backtrack exception
@[ to_additive ] instance one.unique : Unique ( 1 : CommGroupₓₓ ) where default := 1 uniq a := by cases a rfl
...
@[ to_additive ]
  instance
    Groupₓₓ.forget_reflects_isos
    : reflects_isomorphisms ( forget Groupₓₓ .{ u } )
    where
      reflects
        X Y f _
        :=
...
@[ to_additive ]
  instance
    CommGroupₓₓ.forget_reflects_isos
    : reflects_isomorphisms ( forget CommGroupₓₓ .{ u } )
    where
      reflects
        X Y f _
        :=
