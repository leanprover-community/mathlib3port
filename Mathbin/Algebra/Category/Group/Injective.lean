...
theorem injective_of_injective_as_module [Injective (⟨A⟩ : ModuleCat ℤ)] :
  ...
  { factors := fun X Y g f m => by
      ...
      have : mono F := by
        ...
        let α' : AddCommGroupₓₓ.of Z ⟶ X := α.to_add_monoid_hom
        let β' : AddCommGroupₓₓ.of Z ⟶ X := β.to_add_monoid_hom
        have eq2 : α' ≫ f = β' ≫ f := by
          ext
...
theorem injective_as_module_of_injective_as_Ab [Injective (⟨A⟩ : AddCommGroupₓₓ)] : Injective (⟨A⟩ : ModuleCat ℤ) :=
  { factors := fun X Y g f m => by
      skip
      let G : (⟨X⟩ : AddCommGroupₓₓ) ⟶ ⟨A⟩ := g.to_add_monoid_hom
      let F : (⟨X⟩ : AddCommGroupₓₓ) ⟶ ⟨Y⟩ := f.to_add_monoid_hom
      have : mono F := by
...
instance injective_of_divisible [DivisibleBy A ℤ] : CategoryTheory.Injective (⟨A⟩ : AddCommGroupₓₓ) :=
