...
def colimit_cocone_is_colimit : is_colimit colimit_cocone :=
  { desc :=
    ...
    uniq' :=
      fun t m h =>
        RingHom.coe_inj$
          (types.colimit_cocone_is_colimit (F ⋙ forget SemiRing)).uniq ((forget SemiRing).mapCocone t) m
            fun j => funext$ fun x => RingHom.congr_fun (h j) x }

instance forget₂_Mon_preserves_filtered_colimits : preserves_filtered_colimits (forget₂ SemiRing Mon.{v}) :=
...
def colimit_cocone : cocone F :=
  { x := colimit, ι := { (SemiRing.FilteredColimits.colimitCocone (F ⋙ forget₂ CommSemiRing SemiRing)).ι with  } }
...
def colimit_cocone_is_colimit : is_colimit colimit_cocone :=
  { desc :=
    ...
    uniq' :=
      fun t m h =>
        RingHom.coe_inj$
          (types.colimit_cocone_is_colimit (F ⋙ forget CommSemiRing)).uniq ((forget CommSemiRing).mapCocone t) m
            fun j => funext$ fun x => RingHom.congr_fun (h j) x }
...
def colimit_cocone : cocone F :=
  { x := colimit, ι := { (SemiRing.FilteredColimits.colimitCocone (F ⋙ forget₂ Ringₓₓ SemiRing)).ι with  } }
...
def colimit_cocone_is_colimit : is_colimit colimit_cocone :=
  { desc :=
    ...
    uniq' :=
      fun t m h =>
        RingHom.coe_inj$
          (types.colimit_cocone_is_colimit (F ⋙ forget Ringₓₓ)).uniq ((forget Ringₓₓ).mapCocone t) m
            fun j => funext$ fun x => RingHom.congr_fun (h j) x }

instance forget₂_SemiRing_preserves_filtered_colimits : preserves_filtered_colimits (forget₂ Ringₓₓ SemiRing.{v}) :=
  { PreservesFilteredColimits :=
      fun J _ _ =>
        by 
          exact
...
def colimit_cocone : cocone F :=
  { x := colimit, ι := { (Ringₓₓ.FilteredColimits.colimitCocone (F ⋙ forget₂ CommRingₓₓ Ringₓₓ)).ι with  } }
...
def colimit_cocone_is_colimit : is_colimit colimit_cocone :=
  { desc :=
    ...
    fac' :=
      fun t j =>
        RingHom.coe_inj$
          (types.colimit_cocone_is_colimit (F ⋙ forget CommRingₓₓ)).fac ((forget CommRingₓₓ).mapCocone t) j,
    uniq' :=
      fun t m h =>
        RingHom.coe_inj$
          (types.colimit_cocone_is_colimit (F ⋙ forget CommRingₓₓ)).uniq ((forget CommRingₓₓ).mapCocone t) m
            fun j => funext$ fun x => RingHom.congr_fun (h j) x }

instance forget₂_Ring_preserves_filtered_colimits : preserves_filtered_colimits (forget₂ CommRingₓₓ Ringₓₓ.{v}) :=
  { PreservesFilteredColimits :=
      fun J _ _ =>
        by 
          exact
