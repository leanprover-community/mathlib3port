...
instance semiring_obj (F : J ⥤ SemiRing) j : Semiringₓ ((F ⋙ forget SemiRing).obj j) := by
...
instance limit_semiring (F : J ⥤ SemiRing) : Semiringₓ (Types.limitCone (F ⋙ forget SemiRing.{u})).x :=
...
-/
def limit_cone (F : J ⥤ SemiRing.{u}) : Cone F where
...
(Internal use only; use the limits API.)
-/
def limit_cone_is_limit (F : J ⥤ SemiRing) : IsLimit (limitCone F) := by
  refine'
...
-/
instance forget₂_AddCommMon_preserves_limits : PreservesLimits (forget₂ SemiRing AddCommMon) where
...
/-- An auxiliary declaration to speed up typechecking.
-/
def forget₂_Mon_preserves_limits_aux (F : J ⥤ SemiRing) : IsLimit ((forget₂ SemiRing Mon).mapCone (limitCone F)) := by
  apply Mon.HasLimits.limitConeIsLimit (F ⋙ forget₂ SemiRing Mon)

/-- The forgetful functor from semirings to monoids preserves all limits.
-/
instance forget₂_Mon_preserves_limits : PreservesLimits (forget₂ SemiRing Mon) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F =>
...
-/
instance forget_preserves_limits : PreservesLimits (forget SemiRing) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F =>
...
instance comm_semiring_obj (F : J ⥤ CommSemiRing) j : CommSemiringₓ ((F ⋙ forget CommSemiRing).obj j) := by
  change CommSemiringₓ (F.obj j)
  infer_instance

instance limit_comm_semiring (F : J ⥤ CommSemiRing) : CommSemiringₓ (Types.limitCone (F ⋙ forget CommSemiRing.{u})).x :=
...
-/
def limit_cone_is_limit (F : J ⥤ CommSemiRing) : IsLimit (limitCone F) :=
...
-/
instance forget₂_SemiRing_preserves_limits : PreservesLimits (forget₂ CommSemiRing SemiRing) where
  PreservesLimitsOfShape := fun J 𝒥 =>
...
-/
instance forget_preserves_limits : PreservesLimits (forget CommSemiRing) where
  PreservesLimitsOfShape := fun J 𝒥 =>
...
instance ring_obj (F : J ⥤ Ringₓₓ) j : Ringₓ ((F ⋙ forget Ringₓₓ).obj j) := by
...
instance limit_ring (F : J ⥤ Ringₓₓ) : Ringₓ (Types.limitCone (F ⋙ forget Ringₓₓ.{u})).x :=
...
(Generally, you'll just want to use `limit.cone F`.)
-/
def limit_cone_is_limit (F : J ⥤ Ringₓₓ) : IsLimit (limitCone F) :=
  liftedLimitIsLimit _
...
/-- The forgetful functor from rings to semirings preserves all limits.
-/
instance forget₂_SemiRing_preserves_limits : PreservesLimits (forget₂ Ringₓₓ SemiRing) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F => by
        infer_instance }
...
-/
instance forget₂_AddCommGroup_preserves_limits : PreservesLimits (forget₂ Ringₓₓ AddCommGroupₓₓ) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F =>
...
types could have been computed instead as limits in the category of types.)
-/
instance forget_preserves_limits : PreservesLimits (forget Ringₓₓ) where
  PreservesLimitsOfShape := fun J 𝒥 =>
...
instance comm_ring_obj (F : J ⥤ CommRingₓₓ) j : CommRingₓ ((F ⋙ forget CommRingₓₓ).obj j) := by
  change CommRingₓ (F.obj j)
  infer_instance

instance limit_comm_ring (F : J ⥤ CommRingₓₓ) : CommRingₓ (Types.limitCone (F ⋙ forget CommRingₓₓ.{u})).x :=
...
(Generally, you'll just want to use `limit.cone F`.)
-/
def limit_cone_is_limit (F : J ⥤ CommRingₓₓ) : IsLimit (limitCone F) :=
  liftedLimitIsLimit _
...
-/
instance forget₂_Ring_preserves_limits : PreservesLimits (forget₂ CommRingₓₓ Ringₓₓ) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F => by
        infer_instance }
...
-/
instance forget₂_CommSemiRing_preserves_limits : PreservesLimits (forget₂ CommRingₓₓ CommSemiRing) where
  PreservesLimitsOfShape := fun J 𝒥 =>
    { PreservesLimit := fun F =>
...
-/
instance forget_preserves_limits : PreservesLimits (forget CommRingₓₓ) where
  PreservesLimitsOfShape := fun J 𝒥 =>
