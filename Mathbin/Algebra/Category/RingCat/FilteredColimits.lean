...
import Mathbin.Algebra.Category.GroupCat.FilteredColimits
...
def colimitCoconeIsColimit : IsColimit colimit_cocone where
  ...
  fac' t j := RingHom.coe_inj $ (Types.colimitCoconeIsColimit (F ⋙ forget RingCat)).fac ((forget RingCat).mapCocone t) j
  uniq' t m h :=
    RingHom.coe_inj $
...
instance forget₂RingPreservesFilteredColimits :
    PreservesFilteredColimits
      (forget₂ CommRingCat
        RingCat.{u}) where PreservesFilteredColimits J _ _ :=
    { PreservesColimit := fun F =>
        preserves_colimit_of_preserves_colimit_cocone (colimitCoconeIsColimit.{u, u} F)
