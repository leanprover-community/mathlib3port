...
instance bundled_hom : bundled_hom assoc_monoid_hom :=
  ⟨fun M N [Monoidₓ M] [Monoidₓ N] => by
    exact @MonoidHom.toFun M N _ _, fun M [Monoidₓ M] => by
    exact @MonoidHom.id M _, fun M N P [Monoidₓ M] [Monoidₓ N] [Monoidₓ P] => by
    exact @MonoidHom.comp M N P _ _ _, fun M N [Monoidₓ M] [Monoidₓ N] => by
...
@[ to_additive ]
  instance
    ...
    : reflects_isomorphisms ( forget Mon .{ u } )
    where
      reflects
        X Y f _
        :=
...
@[ to_additive ]
  instance
    ...
    : reflects_isomorphisms ( forget CommMon .{ u } )
    where
      reflects
        X Y f _
        :=
