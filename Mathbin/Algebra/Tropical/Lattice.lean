...
instance [SemilatticeInf R] : SemilatticeInf (Tropical R) :=
  { Tropical.hasInf, Tropical.partialOrder with le_inf := fun _ _ _ => le_inf,
    inf_le_left := fun _ _ => inf_le_left, inf_le_right := fun _ _ => inf_le_right }

instance [SemilatticeSup R] : SemilatticeSup (Tropical R) :=
  { Tropical.hasSup, Tropical.partialOrder with sup_le := fun _ _ _ => sup_le,
    le_sup_left := fun _ _ => le_sup_left, le_sup_right := fun _ _ => le_sup_right }
