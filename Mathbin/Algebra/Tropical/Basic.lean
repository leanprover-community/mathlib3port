...
-- failed to format: format: uncaught backtrack exception
instance
  : AddCommSemigroupₓ ( Tropical R )
  where
    add := Tropical.add
      add_assoc _ _ _ := untrop_injective ( min_assocₓ _ _ _ )
      add_comm _ _ := untrop_injective ( min_commₓ _ _ )
...
-- failed to format: format: uncaught backtrack exception
instance
  [ AddSemigroupₓ R ] : Semigroupₓ ( Tropical R )
  where mul := Tropical.mul mul_assoc _ _ _ := untrop_injective ( add_assocₓ _ _ _ )
...
-- failed to format: format: uncaught backtrack exception
instance { α : Type _ } [ HasScalar α R ] : Pow ( Tropical R ) α where pow x n := trop $ n • untrop x
...
-- failed to format: format: uncaught backtrack exception
instance
  [ AddZeroClass R ] : MulOneClass ( Tropical R )
  where one := 1 mul := · * · one_mul _ := untrop_injective $ zero_addₓ _ mul_one _ := untrop_injective $ add_zeroₓ _
