...
instance [Monoidₓ α] [Subsingleton α] : Unique (Associates α) :=
  { default := 1,
    uniq :=
      fun a =>
        by 
...
instance : Mul (Associates α) :=
  ⟨fun a' b' =>
      (Quotientₓ.liftOn₂ a' b' fun a b => ⟦a*b⟧)$
        fun a₁ a₂ b₁ b₂ ⟨c₁, h₁⟩ ⟨c₂, h₂⟩ =>
          Quotientₓ.sound$
            ⟨c₁*c₂,
              by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » p)
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
theorem
  prod_eq_one_iff
  { p : Multiset Associates α } : p.prod = 1 ↔ ∀ a _ : a ∈ p , ( a : Associates α ) = 1
  :=
      ...
      p
        ...
        by
          simp
            ...
            [ mul_eq_one_iff , or_imp_distrib , forall_and_distrib ]
