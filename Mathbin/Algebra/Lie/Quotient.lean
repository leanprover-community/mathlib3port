...
instance
  lie_quotient_lie_ring_module
  : LieRingModule L ( M ⧸ N )
  where
    bracket x n := ( action_as_endo_map N : L →ₗ[ R ] Module.End R ( M ⧸ N ) ) x n
      add_lie x y n := by simp only [ LinearMap.map_add , LinearMap.add_apply ]
      lie_add x m n := by simp only [ LinearMap.map_add , LinearMap.add_apply ]
      leibniz_lie
        x y m
        :=
        show
          action_as_endo_map _ _ _ = _
          by
            simp
              only
              [
...
/-- The quotient of a Lie module by a Lie submodule, is a Lie module. -/
  instance
    lie_quotient_lie_module
    : LieModule R L ( M ⧸ N )
    where
      smul_lie t x m := show ( _ : L →ₗ[ R ] Module.End R ( M ⧸ N ) ) _ _ = _ by simp only [ LinearMap.map_smul ] rfl
        lie_smul x t m := show ( _ : L →ₗ[ R ] Module.End R ( M ⧸ N ) ) _ _ = _ by simp only [ LinearMap.map_smul ] rfl
