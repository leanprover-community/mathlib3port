...
theorem is_cartan_subalgebra_iff_is_ucs_limit : H.IsCartanSubalgebra ↔ H.toLieSubmodule.IsUcsLimit := by
  ...
  · intro h
    ...
    replace hk : H.to_lie_submodule = LieSubmodule.ucs k ⊥ :=
      le_antisymm hk (LieSubmodule.ucs_le_of_centralizer_eq_self H.centralizer_eq_self_of_is_cartan_subalgebra k)
    refine' ⟨k, fun l hl => _⟩
    rw [← Nat.sub_add_cancel hl, LieSubmodule.ucs_add, ← hk, LieSubalgebra.ucs_eq_self_of_is_cartan_subalgebra]
...
@[simp]
theorem LieIdeal.normalizer_eq_top {R : Type u} {L : Type v} [CommRing R] [LieRing L] [LieAlgebra R L]
