...
private theorem ad_E_of_eq_eq_zero (i : B) (h : A i i = 2) : ad_E R A ⟨i, i⟩ = 0 := by
  have h' : (-2 : ℤ).toNat = 0 := by
    rfl
  simp [ad_E, h, h']

private theorem ad_F_of_eq_eq_zero (i : B) (h : A i i = 2) : ad_F R A ⟨i, i⟩ = 0 := by
  have h' : (-2 : ℤ).toNat = 0 := by
    rfl
  simp [ad_F, h, h']
...
-- ././Mathport/Syntax/Translate/Basic.lean:857:9: unsupported derive handler lie_algebra R
