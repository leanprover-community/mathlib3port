...
@[simp]
theorem bracket_tmul (s t : A) (x y : L) : ⁅s ⊗ₜ[R] x,t ⊗ₜ[R] y⁆ = (s*t) ⊗ₜ ⁅x,y⁆ :=
  by 
...
private theorem bracket_lie_self (x : A ⊗[R] L) : ⁅x,x⁆ = 0 :=
  by 
    ...
    ·
      ...
      ·
        rw [LinearMap.map_add, LinearMap.map_add, LinearMap.add_apply, LinearMap.add_apply, h₁, h₂, zero_addₓ,
          add_zeroₓ, add_commₓ, this]
...
instance  : LieRing (A ⊗[R] L) :=
  { add_lie :=
    ...
    lie_self := bracket_lie_self R A L, leibniz_lie := bracket_leibniz_lie R A L }
    ...
    ·
      ...
      ·
        ...
        simp only [bracket_def, bracket', TensorProduct.smul_tmul', mul_left_commₓ a₁ a a₂, TensorProduct.curry_apply,
          Algebra.lmul'_apply, Algebra.id.smul_eq_mul, Function.comp_app, LinearEquiv.coe_coe, LinearMap.coe_comp,
...
instance LieAlgebra : LieAlgebra A (A ⊗[R] L) :=
...
instance LieAlgebra [CommRingₓ R] [Algebra R A] : LieAlgebra R (RestrictScalars R A L) :=
  { (by 
      infer_instance :
    Module R (RestrictScalars R A L)) with
