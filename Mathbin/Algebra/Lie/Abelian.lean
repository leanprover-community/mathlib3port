...
theorem commutative_ring_iff_abelian_lie_ring {A : Type v} [Ring A] :
    IsCommutative A (· * ·) ↔ IsLieAbelian A :=
  by
...
theorem is_trivial_iff_max_triv_eq_top : IsTrivial L M ↔ maxTrivSubmodule R L M = ⊤ :=
  by
  constructor
  · rintro ⟨h⟩
    ext
...
theorem coe_max_triv_linear_map_equiv_lie_module_hom (f : maxTrivSubmodule R L (M →ₗ[R] N)) :
    (maxTrivLinearMapEquivLieModuleHom f : M → N) = f :=
  by
  ext
  rfl
#align
...
theorem coe_linear_map_max_triv_linear_map_equiv_lie_module_hom
    ...
    (maxTrivLinearMapEquivLieModuleHom f : M →ₗ[R] N) = (f : M →ₗ[R] N) :=
  by
  ext
  rfl
#align
