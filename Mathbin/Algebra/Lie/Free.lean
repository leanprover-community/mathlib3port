...
theorem liftAux_spec (f : X → L) (a b : lib R X) (h : FreeLieAlgebra.Rel R X a b) :
  ...
  case smul t a' b' h₁ h₂ => simp only [liftAux_map_smul, h₂]
  case add_right a' b' c' h₁ h₂ => simp only [liftAux_map_add, h₂]
  case mul_left a' b' c' h₁ h₂ => simp only [liftAux_map_mul, h₂]
  case mul_right a' b' c' h₁ h₂ => simp only [liftAux_map_mul, h₂]
