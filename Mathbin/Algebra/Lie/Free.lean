...
theorem Rel.sub_right {a b : lib R X} (c : lib R X) (h : Rel R X a b) : Rel R X (a - c) (b - c) :=
  by simpa only [sub_eq_add_neg] using h.add_right (-c)
#align free_lie_algebra.rel.sub_right FreeLieAlgebra.Rel.sub_right

theorem Rel.smul_of_tower {S : Type _} [Monoid S] [DistribMulAction S R] [IsScalarTower S R R]
    (t : S) (a b : lib R X) (h : Rel R X a b) : Rel R X (t • a) (t • b) := by
...
instance {S : Type _} [Monoid S] [DistribMulAction S R] [DistribMulAction Sᵐᵒᵖ R]
    [IsScalarTower S R R] [IsCentralScalar S R] :
    IsCentralScalar S
      (FreeLieAlgebra R
        X) where op_smul_eq_smul t := Quot.ind fun a => congr_arg (Quot.mk _) (op_smul_eq_smul t a)
...
instance :
    Add
      (FreeLieAlgebra R
        X) where add := Quot.map₂ (· + ·) (fun _ _ _ => Rel.add_left _) fun _ _ _ => Rel.add_right _
...
instance :
    Sub
      (FreeLieAlgebra R
        X) where sub := Quot.map₂ Sub.sub (fun _ _ _ => Rel.sub_left _) fun _ _ _ => Rel.sub_right _
