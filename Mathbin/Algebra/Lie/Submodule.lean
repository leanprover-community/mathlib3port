...
/- ././././Mathport/Syntax/Translate/Expr.lean:374:4: unsupported set replacement {((s : submodule R M)) | s «expr ∈ » S} -/
instance : InfSet (LieSubmodule R L M) :=
  ⟨fun S =>
    {
      sInf
        "././././Mathport/Syntax/Translate/Expr.lean:374:4: unsupported set replacement {((s : submodule R M)) | s «expr ∈ » S}" with
      lie_mem := fun x m h =>
        by
