...
@[simp]
theorem sub_lie : ⁅x - y,m⁆ = ⁅x,m⁆ - ⁅y,m⁆ :=
  by 
    simp [sub_eq_add_neg]

@[simp]
theorem lie_sub : ⁅x,m - n⁆ = ⁅x,m⁆ - ⁅x,n⁆ :=
  by 
    simp [sub_eq_add_neg]
...
theorem map_lie₂ (f : M →ₗ⁅R,L⁆ N →ₗ[R] P) (x : L) (m : M) (n : N) : ⁅x,f m n⁆ = f ⁅x,m⁆ n+f m ⁅x,n⁆ :=
  by 
...
theorem symm_symm (e : M ≃ₗ⁅R,L⁆ N) : e.symm.symm = e :=
  by 
    ext 
    applyFun e.symm using e.symm.injective 
    simp 
