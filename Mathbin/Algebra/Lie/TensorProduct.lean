      ...
      fun x y t =>
        by 
          ...
          simp only [has_bracket_aux, LieRing.of_associative_ring_bracket, LinearMap.mul_apply, mk_apply,
            ...
            LinearMap.rtensor_tmul, LieHom.map_lie, to_endomorphism_apply_apply, LinearMap.add_apply, LinearMap.map_add,
            LinearMap.rtensor_sub, LinearMap.sub_apply, LinearMap.ltensor_tmul]
          abel }
...
theorem lift_lie_apply (f : M →ₗ⁅R,L⁆ N →ₗ[R] P) (m : M) (n : N) : lift_lie R L M N P f (m ⊗ₜ n) = f m n :=
  by 
...
@[simp]
theorem to_module_hom_apply (x : L) (m : M) : to_module_hom R L M (x ⊗ₜ m) = ⁅x,m⁆ :=
  by 
