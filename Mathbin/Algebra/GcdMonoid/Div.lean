...
theorem coprime_of_div_gcd {β : Type _} {f : β → ℕ} (s : Finset β) {x : β} (hx : x ∈ s) (hfz : f x ≠ 0) :
...
then the `gcd` of `(f i) / d` is equal to `1`. -/
theorem coprime_of_div_gcd {β : Type _} {f : β → ℤ} (s : Finset β) {x : β} (hx : x ∈ s) (hfz : f x ≠ 0) :
    (s.gcd fun b => f b / s.gcd f) = 1 := by
  obtain ⟨g, he, hg⟩ := Finset.extract_gcd f ⟨x, hx⟩
  refine' ((Finset.gcd_congr rfl) fun a ha => _).trans hg
...
theorem gcd_div_gcd_id_eq_one (s : Finset ℤ) {x : ℤ} (hx : x ∈ s) (hnz : x ≠ 0) : (s.gcd fun b => b / s.gcd id) = 1 :=
  coprime_of_div_gcd s hx hnz
...
then the `gcd` of `(f i) / d` is equal to `1`. -/
theorem coprime_of_div_gcd {β : Type _} {f : β → K[X]} (s : Finset β) {x : β} (hx : x ∈ s) (hfz : f x ≠ 0) :
    (s.gcd fun b => f b / s.gcd f) = 1 := by
  obtain ⟨g, he, hg⟩ := Finset.extract_gcd f ⟨x, hx⟩
  refine' ((Finset.gcd_congr rfl) fun a ha => _).trans hg
