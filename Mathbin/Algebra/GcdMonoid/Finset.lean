...
@[simp]
theorem normalize_lcm : normalize (s.lcm f) = s.lcm f :=
  by 
    simp [lcm_def]
...
@[simp]
theorem normalize_gcd : normalize (s.gcd f) = s.gcd f :=
  by 
    simp [gcd_def]
...
theorem gcd_mul_right {a : α} : (s.gcd fun x => f x*a) = s.gcd f*normalize a :=
  by 
    classical 
    apply s.induction_on
    ·
