...
def lcm (s : Finsetₓ β) (f : β → α) : α :=
  s.fold GcdMonoid.lcm 1 f

variable {s s₁ s₂ : Finsetₓ β} {f : β → α}

theorem lcm_def : s.lcm f = (s.1.map f).lcm :=
  rfl

@[simp]
theorem lcm_empty : (∅ : Finsetₓ β).lcm f = 1 :=
  fold_empty
...
@[simp]
theorem lcm_insert [DecidableEq β] {b : β} : (insert b s : Finsetₓ β).lcm f = GcdMonoid.lcm (f b) (s.lcm f) := by
  by_cases h:b ∈ s
...
@[simp]
theorem lcm_singleton {b : β} : ({b} : Finsetₓ β).lcm f = normalize (f b) :=
...
theorem lcm_union [DecidableEq β] : (s₁ ∪ s₂).lcm f = GcdMonoid.lcm (s₁.lcm f) (s₂.lcm f) :=
  (Finsetₓ.induction_on s₁ (by rw [empty_union, lcm_empty, lcm_one_left, normalize_lcm])) fun a s has ih => by
    rw [insert_union, lcm_insert, lcm_insert, ih, lcm_assoc]

theorem lcm_congr {f g : β → α} (hs : s₁ = s₂) (hfg : ∀ a ∈ s₂, f a = g a) : s₁.lcm f = s₂.lcm g := by
  subst hs
  exact Finsetₓ.fold_congr hfg
...
@[simp]
theorem gcd_empty : (∅ : Finsetₓ β).gcd f = 0 :=
  fold_empty
...
@[simp]
theorem gcd_insert [DecidableEq β] {b : β} : (insert b s : Finsetₓ β).gcd f = GcdMonoid.gcd (f b) (s.gcd f) := by
  by_cases h:b ∈ s
...
@[simp]
theorem gcd_singleton {b : β} : ({b} : Finsetₓ β).gcd f = normalize (f b) :=
...
theorem gcd_union [DecidableEq β] : (s₁ ∪ s₂).gcd f = GcdMonoid.gcd (s₁.gcd f) (s₂.gcd f) :=
  (Finsetₓ.induction_on s₁ (by rw [empty_union, gcd_empty, gcd_zero_left, normalize_gcd])) fun a s has ih => by
...
theorem gcd_eq_of_dvd_sub {s : Finsetₓ β} {f g : β → α} {a : α} (h : ∀ x : β, x ∈ s → a ∣ f x - g x) :
