...
@[simp]
theorem i_mul_k : (q.i*q.k) = c₁ • q.j :=
  by 
    rw [←i_mul_j, ←mul_assocₓ, i_mul_i, smul_mul_assoc, one_mulₓ]

@[simp]
theorem k_mul_i : (q.k*q.i) = -c₁ • q.j :=
  by 
    rw [←i_mul_j, mul_assocₓ, j_mul_i, mul_neg_eq_neg_mul_symm, i_mul_k, neg_smul]

@[simp]
theorem k_mul_j : (q.k*q.j) = c₂ • q.i :=
  by 
    rw [←i_mul_j, mul_assocₓ, j_mul_j, mul_smul_comm, mul_oneₓ]

@[simp]
theorem j_mul_k : (q.j*q.k) = -c₂ • q.i :=
  by 
    rw [←i_mul_j, ←mul_assocₓ, j_mul_i, neg_mul_eq_neg_mul_symm, k_mul_j, neg_smul]

@[simp]
theorem k_mul_k : (q.k*q.k) = -((c₁*c₂) • 1) :=
  by 
    rw [←i_mul_j, mul_assocₓ, ←mul_assocₓ q.j _ _, j_mul_i, ←i_mul_j, ←mul_assocₓ, mul_neg_eq_neg_mul_symm, ←mul_assocₓ,
...
theorem lift_zero : q.lift (0 : ℍ[R,c₁,c₂]) = 0 :=
  by 
    simp [lift]

theorem lift_one : q.lift (1 : ℍ[R,c₁,c₂]) = 1 :=
  by 
    simp [lift]
...
theorem lift_smul (r : R) (x : ℍ[R,c₁,c₂]) : q.lift (r • x) = r • q.lift x :=
  by 
...
def lift : basis A c₁ c₂ ≃ (ℍ[R,c₁,c₂] →ₐ[R] A) :=
  { toFun := basis.lift_hom, invFun := (basis.self R).compHom,
    left_inv :=
      fun q =>
        by 
          ext <;> simp [basis.lift],
