...
theorem torsionBy_singleton_eq : torsionBySet R M {a} = torsionBy R M a :=
  by
  ext x
  simp only [mem_torsionBySet_iff, SetCoe.forall, Subtype.coe_mk, Set.mem_singleton_iff, forall_eq,
    mem_torsionBy_iff]
...
theorem torsionBy_le_torsionBy_of_dvd (a b : R) (dvd : a ∣ b) : torsionBy R M a ≤ torsionBy R M b :=
  by
  rw [← torsionBy_span_singleton_eq, ← torsionBy_singleton_eq]
...
theorem torsion_by_univ : torsionBySet R M Set.univ = ⊥ :=
  by
  rw [eq_bot_iff, ← torsionBy_one, ← torsionBy_singleton_eq]
...
theorem isTorsionBy_iff_torsionBy_eq_top : IsTorsionBy R M a ↔ torsionBy R M a = ⊤ := by
  rw [← torsionBy_singleton_eq, ← isTorsionBy_singleton_iff, isTorsionBySet_iff_torsionBySet_eq_top]
...
theorem isTorsionBySet_iff_is_torsion_by_span :
  ...
  rw [isTorsionBySet_iff_torsionBySet_eq_top, isTorsionBySet_iff_torsionBySet_eq_top,
    torsionBySet_eq_torsion_by_span]
...
theorem supᵢ_torsion_by_ideal_eq_torsion_by_infᵢ :
  ...
  · intro x hx
    rw [mem_supᵢ_finset_iff_exists_sum]
    obtain ⟨μ, hμ⟩ :=
      (mem_supᵢ_finset_iff_exists_sum _ _).mp
...
theorem supIndep_torsion_by_ideal : S.SupIndep fun i => torsionBySet R M <| p i :=
  ...
  rw [disjoint_iff, Finset.sup_eq_supᵢ,
    supᵢ_torsion_by_ideal_eq_torsion_by_infᵢ fun i hi j hj ij => hp (hT hi) (hT hj) ij]
  have :=
    @galois_connection.u_inf _ _ (OrderDual.toDual _) (OrderDual.toDual _) _ _ _ _ (torsion_gc R M)
  dsimp at this⊢
...
theorem supᵢ_torsionBy_eq_torsionBy_prod :
  ...
  by
  rw [← torsionBy_span_singleton_eq, Ideal.submodule_span_eq, ←
    Ideal.finset_inf_span_singleton _ _ hq, Finset.inf_eq_infᵢ, ←
    supᵢ_torsion_by_ideal_eq_torsion_by_infᵢ]
  · congr
    ext : 1
    congr
    ext : 1
    exact (torsionBy_span_singleton_eq _).symm
...
theorem supIndep_torsionBy : S.SupIndep fun i => torsionBy R M <| q i :=
  by
  convert
    supIndep_torsion_by_ideal fun i hi j hj ij =>
      (Ideal.sup_eq_top_iff_isCoprime (q i) _).mpr <| hq hi hj ij
  ext : 1; exact (torsionBy_span_singleton_eq _).symm
...
theorem isTorsion_by_ideal_of_finite_of_isTorsion [Module.Finite R M] (hM : Module.IsTorsion R M) :
  ...
  · refine' Set.Nonempty.ne_empty ⟨_, _, (∏ x in S, (@hM x).choose : R⁰).2⟩
...
theorem exists_isTorsionBy {p : R} (hM : IsTorsion' M <| Submonoid.powers p) (d : ℕ) (hd : d ≠ 0)
  ...
  have : pOrder hM (s i) ≤ pOrder hM (s <| Option.get hoj) :=
...
theorem torsionBy_eq_span_singleton {R : Type _} [CommRing R] (a b : R) (ha : a ∈ R⁰) :
  ...
  by
  ext x; rw [mem_torsionBy_iff, mem_span_singleton]
