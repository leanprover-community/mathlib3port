...
theorem CompleteLattice.Independent.linear_independent' {ι R M : Type _} {v : ι → M} [Ring R] [AddCommGroup M]
    [Module R M] (hv : CompleteLattice.Independent $ fun i => R ∙ v i)
...
theorem supIndepTorsionByIdeal : S.SupIndep fun i => torsionBySet R M $ p i := fun T hT i hi hiT => by
...
@[simp]
theorem pow_p_order_smul {p : R} (hM : IsTorsion' M $ Submonoid.powers p) (x : M) [∀ n : ℕ, Decidable (p ^ n • x = 0)] :
    p ^ pOrder hM x • x = 0 :=
...
theorem is_torsion_iff_is_torsion_nat [AddCommMonoid M] : AddMonoid.IsTorsion M ↔ Module.IsTorsion ℕ M := by
...
theorem is_torsion_iff_is_torsion_int [AddCommGroup M] : AddMonoid.IsTorsion M ↔ Module.IsTorsion ℤ M := by
  refine' ⟨fun h x => _, fun h x => _⟩
  · obtain ⟨n, h0, hn⟩ := (is_of_fin_add_order_iff_nsmul_eq_zero x).mp (h x)
