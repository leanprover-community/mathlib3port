...
theorem Submodule.isInternal_prime_power_torsion_of_pid [Module.Finite R M]
    ...
    ∃ (ι : Type u)(_ : Fintype ι)(_ : DecidableEq ι)(p : ι → R)(h : ∀ i, Irreducible <| p i)(e :
      ι → ℕ), DirectSum.IsInternal fun i => torsionBy R M <| p i ^ e i :=
  by
  obtain ⟨P, dec, hP, e, this⟩ := isInternal_prime_power_torsion hM
  refine' ⟨P, inferInstance, dec, fun p => IsPrincipal.generator (p : Ideal R), _, e, _⟩
  · rintro ⟨p, hp⟩
    haveI := Ideal.isPrime_of_prime (hP p hp)
    exact (IsPrincipal.prime_generator_of_isPrime p (hP p hp).ne_zero).irreducible
  · convert this
    ext p : 1
    rw [← torsionBy_span_singleton_eq, Ideal.submodule_span_eq, ← Ideal.span_singleton_pow,
...
theorem equiv_directSum_of_isTorsion [h' : Module.Finite R N] (hN : Module.IsTorsion R N) :
  ...
  have :
    ∀ i,
      ∃ (d : ℕ)(k : Fin d → ℕ),
        Nonempty <| torsionBy R N (p i ^ e i) ≃ₗ[R] ⨁ j, R ⧸ R ∙ p i ^ k j :=
    by
  ...
  classical
    refine'
      ⟨Σi, Fin (this i).choose, inferInstance, fun ⟨i, j⟩ => p i, fun ⟨i, j⟩ => hp i, fun ⟨i, j⟩ =>
...
theorem equiv_free_prod_directSum [h' : Module.Finite R N] :
  ...
  obtain ⟨I, fI, p, hp, e, ⟨h⟩⟩ := equiv_directSum_of_isTorsion (@torsion_is_torsion R N _ _ _)
