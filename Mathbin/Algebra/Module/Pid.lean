...
theorem _root_.ideal.torsion_of_eq_span_pow_p_order (x : M) : torsionOf R M x = span {p ^ pOrder hM x} := by
  ...
  have prop :
    (fun n : ℕ => p ^ n • x = 0) = fun n : ℕ => (Associates.mk $ generator $ torsion_of R M x) ∣ Associates.mk p ^ n :=
    by
    ext n
...
#align module._root_.ideal.torsion_of_eq_span_pow_p_order module._root_.ideal.torsion_of_eq_span_pow_p_order
