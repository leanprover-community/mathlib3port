...
are regular. -/
theorem is_regular_mul_and_mul_iff : IsRegular (a*b) ∧ IsRegular (b*a) ↔ IsRegular a ∧ IsRegular b :=
  by 
    refine' ⟨_, _⟩
    ·
      rintro ⟨ab, ba⟩
...
theorem not_is_left_regular_zero_iff : ¬IsLeftRegular (0 : R) ↔ Nontrivial R :=
  by 
...
theorem not_is_right_regular_zero_iff : ¬IsRightRegular (0 : R) ↔ Nontrivial R :=
  by 
...
theorem is_regular_iff_subsingleton : IsRegular (0 : R) ↔ Subsingleton R :=
  ⟨fun h => h.left.subsingleton,
    fun h => ⟨is_left_regular_zero_iff_subsingleton.mpr h, is_right_regular_zero_iff_subsingleton.mpr h⟩⟩
...
theorem not_is_regular_zero [Nontrivial R] : ¬IsRegular (0 : R) :=
  fun h => IsRegular.ne_zero h rfl
...
theorem is_regular_one : IsRegular (1 : R) :=
  ⟨fun a b ab => (one_mulₓ a).symm.trans (Eq.trans ab (one_mulₓ b)),
    fun a b ab => (mul_oneₓ a).symm.trans (Eq.trans ab (mul_oneₓ b))⟩
...
def mulLeftEmbedding {G : Type _} [LeftCancelSemigroup G] (g : G) : G ↪ G :=
  { toFun := fun h => g*h, inj' := mul_right_injective g }
