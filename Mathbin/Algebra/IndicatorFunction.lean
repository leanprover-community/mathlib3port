...
@[to_additive]
theorem mul_indicator_eq_self_of_superset (h1 : s.mul_indicator f = f) (h2 : s ⊆ t) : t.mul_indicator f = f := by
...
@[to_additive]
theorem mul_indicator_eq_one_iff (a : α) : s.mul_indicator f a ≠ 1 ↔ a ∈ s ∩ mul_support f := by
  constructor <;> intro h
  · by_contra hmem
...
@[to_additive]
theorem eq_on_mul_indicator : eq_on (mul_indicator s f) f s := fun x hx => mul_indicator_of_mem hx f
...
theorem mul_indicator_comp_right {s : Set α} (f : β → α) {g : α → M} {x : β} :
    mul_indicator (f ⁻¹' s) (g ∘ f) x = mul_indicator s g (f x) := by
  simp only [mul_indicator]
  split_ifs <;> rfl

@[to_additive]
theorem mul_indicator_comp_of_one {g : M → N} (hg : g 1 = 1) : mul_indicator s (g ∘ f) = g ∘ mul_indicator s f := by
  funext
  simp only [mul_indicator]
  split_ifs <;> simp [*]
...
theorem mul_indicator_rel_mul_indicator {r : M → M → Prop} (h1 : r 1 1) (ha : a ∈ s → r (f a) (g a)) :
    r (mul_indicator s f a) (mul_indicator s g a) := by
  simp only [mul_indicator]
...
theorem mul_indicator_mul (s : Set α) (f g : α → M) :
    (mul_indicator s fun a => f a * g a) = fun a => mul_indicator s f a * mul_indicator s g a := by
  funext
  simp only [mul_indicator]
  split_ifs
  · rfl
...
theorem mul_indicator_mul_eq_left {f g : α → M} (h : Disjoint (mul_support f) (mul_support g)) :
    (mul_support f).mulIndicator (f * g) = f := by
...
theorem mul_indicator_mul_eq_right {f g : α → M} (h : Disjoint (mul_support f) (mul_support g)) :
    (mul_support g).mulIndicator (f * g) = g := by
  refine' (mul_indicator_congr $ fun x hx => _).trans mul_indicator_mul_support
...
def mul_indicator_hom {α} M [MulOneClass M] (s : Set α) : (α → M) →* α → M where
  toFun := mul_indicator s
...
theorem _root_.finset.prod_mul_indicator_eq_prod_filter (s : Finset ι) (f : ι → α → M) (t : ι → Set α) (g : ι → α) :
    (∏ i in s, mul_indicator (t i) (f i) (g i)) = ∏ i in s.filter fun i => g i ∈ t i, f i (g i) := by
...
theorem mul_indicator_finset_bUnion {ι} (I : Finset ι) (s : ι → Set α) {f : α → M} :
    (∀, ∀ i ∈ I, ∀, ∀ j ∈ I, ∀, i ≠ j → Disjoint (s i) (s j)) →
      mul_indicator (⋃ i ∈ I, s i) f = fun a => ∏ i in I, mul_indicator (s i) f a :=
  by
