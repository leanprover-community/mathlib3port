...
theorem squashSeq_eq_self_of_terminated (terminated_at_succ_n : s.TerminatedAt (n + 1)) :
  ...
  cases s_nth_eq : s.nth n <;> simp only [*, squashSeq]
...
theorem squashSeq_nth_of_not_terminated {gp_n gp_succ_n : Pair K} (s_nth_eq : s.nth n = some gp_n)
...
theorem squashSeq_nth_of_lt {m : ℕ} (m_lt_n : m < n) : (squashSeq s n).nth m = s.nth m :=
  by
...
theorem squashSeq_succ_n_tail_eq_squashSeq_tail_n :
  ...
  case
    ...
    cases s_succ_nth_eq : s.nth (n + 1) <;>
      simp only [squashSeq, SeqCat.nth_tail, s_succ_nth_eq, s_succ_succ_nth_eq]
  case
    some =>
    ...
    · have : s.tail.nth m = s.nth (m + 1) := s.nth_tail m
      ...
      · simp only [*, squashSeq, SeqCat.nth_tail, SeqCat.nth_zipWith, Option.map₂_none_right]
      · simp [*, squashSeq]
...
theorem succ_succ_nth_convergent'_aux_eq_succ_nth_convergent'_aux_squashSeq :
  ...
  case some =>
    ...
    case zero =>
      ...
      simp [*, convergents'Aux, SeqCat.head, SeqCat.nth_tail]
    case succ =>
      obtain ⟨gp_head, s_head_eq⟩ : ∃ gp_head, s.head = some gp_head
      ...
      simp only [*, convergents'Aux, squashSeq_succ_n_tail_eq_squashSeq_tail_n]
...
theorem squashGcf_eq_self_of_terminated (terminated_at_n : TerminatedAt g n) : squashGcf g n = g :=
  ...
  case succ => cases g; simp [squashSeq_eq_self_of_terminated terminated_at_n, squashGcf]
...
/-- The values before the squashed position stay the same. -/
theorem squashGcf_nth_of_lt {m : ℕ} (m_lt_n : m < n) : (squashGcf g (n + 1)).s.nth m = g.s.nth m :=
  by simp only [squashGcf, squashSeq_nth_of_lt m_lt_n]
...
theorem succ_nth_convergent'_eq_squashGcf_nth_convergent' :
  ...
  case zero =>
    cases g_s_head_eq : g.s.nth 0 <;>
      simp [g_s_head_eq, squashGcf, convergents', convergents'Aux, SeqCat.head]
  case succ =>
      ...
      · cases' n with n'
        ...
        · cases' m' with m''
          ...
          · -- get some inequalities to instantiate the IH for m'' and m'' + 1
            ...
            have : (squashGcf g (n' + 1)).s.nth m'' = g.s.nth m'' :=
