...
theorem determinant_aux (hyp : n = 0 ∨ ¬(of v).TerminatedAt (n - 1)) :
  ...
  case nat.succ =>
    ...
    suffices ((pA*ppB+gp.b*pB) - pB*ppA+gp.b*pA) = -1 ^ n+1by
      ·
...
theorem sub_convergents_eq {ifp : int_fract_pair K} (stream_nth_eq : int_fract_pair.stream v n = some ifp) :
  ...
  ·
    ...
    calc ((pA+ifp.fr⁻¹*A) / pB+ifp.fr⁻¹*B) - A / B = (((pA+ifp.fr⁻¹*A)*B) - (pB+ifp.fr⁻¹*B)*A) / (pB+ifp.fr⁻¹*B)*B := by
      rw [div_sub_div _ _ this.symm zero_ne_B.symm]_ = (((pA*B)+(ifp.fr⁻¹*A)*B) - (pB*A)+(ifp.fr⁻¹*B)*A) / _ := by
      repeat'
