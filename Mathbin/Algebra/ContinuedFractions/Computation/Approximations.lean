...
theorem succ_nth_stream_b_le_nth_stream_fr_inv {ifp_n ifp_succ_n : IntFractPair K}
  ...
  suffices (⌊ifp_n.fr⁻¹⌋ : K) ≤ ifp_n.fr⁻¹
    ...
    have : ifp_n_fr ≠ 0 := by
      intro h
      simpa [h, IntFractPair.stream, nth_stream_eq] using succ_nth_stream_eq
...
theorem of_one_le_nth_part_denom {b : K}
  ...
  exact IntFractPair.exists_succ_nth_stream_of_gcf_of_nth_eq_some nth_s_eq
...
theorem of_part_num_eq_one_and_exists_int_part_denom_eq {gp : GeneralizedContinuedFraction.Pair K}
  ...
  have : gp = ⟨1, ifp.b⟩ :=
    ...
    have : (of v).s.nth n = some ⟨1, ifp.b⟩ :=
      nth_of_eq_some_of_succ_nth_intFractPair_stream stream_succ_nth_eq
...
theorem le_of_succ_nth_denom {b : K}
  ...
  by
  rw [denom_eq_conts_b, nth_cont_eq_succ_nth_cont_aux]
  exact le_of_succ_succ_nth_continuantsAux_b nth_part_denom_eq
  ...
  · -- more shorthand notation
    ...
    have n_eq_zero_or_not_terminated_at_pred_n : n = 0 ∨ ¬g.terminated_at (n - 1) :=
      ...
      · have : IntFractPair.stream v (n' + 1) ≠ none := by simp [stream_nth_eq]
        have : ¬g.terminated_at n' :=
          (not_congr of_terminatedAt_n_iff_succ_nth_intFractPair_stream_eq_none).mpr this
        exact Or.inr this
...
theorem abs_sub_convergents_le (not_terminated_at_n : ¬(of v).TerminatedAt n) :
  ...
  exact IntFractPair.exists_succ_nth_stream_of_gcf_of_nth_eq_some s_nth_eq
  ...
  · -- we can cancel multiplication by `conts.b` and addition with `pred_conts.b`
    ...
    exact (mul_le_mul_left zero_lt_conts_b).mpr <| (add_le_add_iff_left pred_conts.b).mpr this
