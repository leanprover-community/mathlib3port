...
theorem compExactValue_correctness_of_stream_eq_some :
  ...
  · intro ifp_zero stream_zero_eq
    ...
    · suffices v = ⌊v⌋ by simpa [continuantsAux, fract_eq_zero, compExactValue]
      calc
    ...
    ·
      field_simp [continuantsAux, nextContinuants, nextNumerator, nextDenominator, of_h_eq_floor,
        compExactValue, fract_ne_zero]
  · intro ifp_succ_n succ_nth_stream_eq
    ...
    · suffices v = conts.a / conts.b by simpa [compExactValue, ifp_succ_n_fr_eq_zero]
      ...
      have s_nth_eq : g.s.nth n = some ⟨1, ⌊ifp_n.fr⁻¹⌋⟩ :=
        nth_of_eq_some_of_nth_intFractPair_stream_fr_ne_zero nth_stream_eq nth_fract_ne_zero
    ...
    · -- use the IH to show that the following equality suffices
      ...
      have s_nth_eq : g.s.nth n = some ⟨1, (⌊ifp_n.fr⁻¹⌋ : K)⟩ :=
        nth_of_eq_some_of_nth_intFractPair_stream_fr_ne_zero nth_stream_eq ifp_n_fract_ne_zero
      ...
      let pB := pconts.b
      have :
        compExactValue ppconts pconts ifp_n.fr =
      ...
      field_simp [conts, compExactValue, continuantsAux_recurrence s_nth_eq ppconts_eq pconts_eq,
        nextContinuants, nextNumerator, nextDenominator, this, tmp_calc, tmp_calc']
      ac_rfl
...
theorem of_correctness_of_terminatedAt (terminated_at_n : (of v).TerminatedAt n) :
  ...
  have : IntFractPair.stream v (n + 1) = none :=
    of_terminatedAt_n_iff_succ_nth_intFractPair_stream_eq_none.mp terminated_at_n
