...
theorem is_sol_mk_sol (init : Finₓ E.order → α) : E.is_solution (E.mk_sol init) :=
  fun n =>
    by 
      rw [mk_sol] <;> simp 
...
| n =>
  if h' : n < E.order then
    by 
      rw [mk_sol] <;> simp only [h', dif_pos] <;> exactModCast HEq ⟨n, h'⟩
  else
    by 
...
def to_init : E.sol_space ≃ₗ[α] Finₓ E.order → α :=
...
theorem sol_eq_of_eq_init (u v : ℕ → α) (hu : E.is_solution u) (hv : E.is_solution v) :
  ...
  by 
    ...
    ext x 
    exactModCast h (mem_range.mpr x.2)
  ...
  { toFun := fun X i => if h : ((i : ℕ)+1) < E.order then X ⟨i+1, h⟩ else ∑i, E.coeffs i*X i,
    map_add' :=
      fun x y =>
        by 
          ext i 
          splitIfs <;> simp [h, mul_addₓ, sum_add_distrib],
    map_smul' :=
      fun x y =>
        by 
          ext i 
          splitIfs <;> simp [h, mul_sum]
          exact
            sum_congr rfl
              fun x _ =>
                by 
