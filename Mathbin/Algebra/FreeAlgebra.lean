...
instance :
    HasSmul R
      (FreeAlgebra R X) where smul r := Quot.map ((· * ·) ↑r) fun a b => Rel.mul_compat_right

instance : Algebra R (FreeAlgebra R
        X) where 
  toFun r := Quot.mk _ r
  map_one' := rfl
...
-/
def lift : (X → A) ≃ (FreeAlgebra R X →ₐ[R]
        A) where 
  toFun := liftAux R
  invFun F := F ∘ ι R
  left_inv f := by 
       ...
       (Term.typeSpec
        ...
        (Term.app
         ...
         [(Term.typeAscription
           ...
           (Term.app `ι [`R])
           ":"
           [(Term.arrow `X "→" (Term.app `FreeAlgebra [`R `X]))]
           ")")])))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
      (Term.proj (Term.proj `hfy1 "." `symm) "." `trans)
            ...
            hxy
              ...
              let
                f
                  ...
                  :=
                  lift R fun z => by skip <;> exact if x = z then ( 1 : R ) else 0
                have
