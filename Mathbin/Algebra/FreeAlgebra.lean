...
instance : HasSmul R (FreeAlgebra R X) where smul r := Quot.map ((· * ·) ↑r) fun a b => Rel.mul_compat_right
        ...
        (Term.let
         ...
         (Term.letDecl
          (Term.letIdDecl
           `f
           []
           [(Term.typeSpec ":" (Algebra.Algebra.Basic.«term_→ₐ[_]_» (Term.app `FreeAlgebra [`R `X]) " →ₐ[" `R "] " `R))]
           ":="
           (Term.app
            `lift
            [`R
