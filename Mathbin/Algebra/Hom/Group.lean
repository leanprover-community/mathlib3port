...
@[to_additive]
instance MonoidHom.hasCoeToOneHom {mM : MulOneClass M} {mN : MulOneClass N} :
    Coe (M →* N) (OneHom M N) :=
  ⟨MonoidHom.toOneHom⟩
#align monoid_hom.has_coe_to_one_hom MonoidHom.hasCoeToOneHom

@[to_additive]
instance MonoidHom.hasCoeToMulHom {mM : MulOneClass M} {mN : MulOneClass N} :
    Coe (M →* N) (M →ₙ* N) :=
  ⟨MonoidHom.toMulHom⟩
...
@[simp, to_additive]
theorem MonoidHom.to_one_hom_coe [MulOneClass M] [MulOneClass N] (f : M →* N) :
    (f.toOneHom : M → N) = f :=
  rfl
#align monoid_hom.to_one_hom_coe MonoidHom.to_one_hom_coe

@[simp, to_additive]
theorem MonoidHom.to_mul_hom_coe [MulOneClass M] [MulOneClass N] (f : M →* N) :
    (f.toMulHom : M → N) = f :=
  rfl
...
instance : Monoid (AddMonoid.EndCat
        A) where 
