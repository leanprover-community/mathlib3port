...
protected def Function.Injective.distrib {S} [Mul R] [Add R] [Distribₓ S] (f : R → S) (hf : Injective f)
  ...
  mul := (· * ·)
  add := (· + ·)
  left_distrib := fun x y z => hf <| by simp only [*, left_distrib]
  right_distrib := fun x y z => hf <| by simp only [*, right_distrib]
...
theorem add_one_mul [RightDistribClass α] (a b : α) : (a + 1) * b = a * b + b := by rw [add_mulₓ, one_mulₓ]

theorem mul_add_one [LeftDistribClass α] (a b : α) : a * (b + 1) = a * b + a := by rw [mul_addₓ, mul_oneₓ]

theorem one_add_mul [RightDistribClass α] (a b : α) : (1 + a) * b = b + a * b := by rw [add_mulₓ, one_mulₓ]

theorem mul_one_add [LeftDistribClass α] (a b : α) : a * (1 + b) = a + a * b := by rw [mul_addₓ, mul_oneₓ]
...
instance (priority := 100) MulZeroClassₓ.negZeroClass : NegZeroClass α :=
  { MulZeroClassₓ.toHasZero α, HasDistribNeg.toHasInvolutiveNeg α with
...
instance (priority := 100) NonUnitalNonAssocRing.toHasDistribNeg : HasDistribNeg α where
  neg := Neg.neg
  neg_neg := neg_negₓ
  neg_mul := fun a b => eq_neg_of_add_eq_zero_left <| by rw [← right_distrib, add_left_negₓ, zero_mul]
  mul_neg := fun a b => eq_neg_of_add_eq_zero_left <| by rw [← left_distrib, add_left_negₓ, mul_zero]
...
theorem mul_add_eq_mul_add_iff_sub_mul_add_eq : a * e + c = b * e + d ↔ (a - b) * e + c = d :=
  calc
    a * e + c = b * e + d ↔ a * e + c = d + b * e := by simp [add_commₓ]
    _ ↔ a * e + c - b * e = d :=
      Iff.intro
        (fun h => by
          rw [h]
...
theorem sub_one_mul (a b : α) : (a - 1) * b = a * b - b := by rw [sub_mul, one_mulₓ]

theorem mul_sub_one (a b : α) : a * (b - 1) = a * b - a := by rw [mul_sub, mul_oneₓ]

theorem one_sub_mul (a b : α) : (1 - a) * b = b - a * b := by rw [sub_mul, one_mulₓ]

theorem mul_one_sub (a b : α) : a * (1 - b) = a - a * b := by rw [mul_sub, mul_oneₓ]
...
instance (priority := 100) Ringₓ.toNonUnitalRing : NonUnitalRing α :=
  { ‹Ringₓ α› with
    zero_mul := fun a => add_left_cancelₓ <| show 0 * a + 0 * a = 0 * a + 0 by rw [← add_mulₓ, zero_addₓ, add_zeroₓ],
    mul_zero := fun a => add_left_cancelₓ <| show a * 0 + a * 0 = a * 0 + 0 by rw [← mul_addₓ, add_zeroₓ, add_zeroₓ] }
...
@[field_simps]
theorem divp_add_divp_same (a b : α) (u : αˣ) : a /ₚ u + b /ₚ u = (a + b) /ₚ u := by simp only [divp, add_mulₓ]
...
theorem dvd_add_iff_right (h : a ∣ b) : a ∣ c ↔ a ∣ b + c := by rw [add_commₓ] <;> exact dvd_add_iff_left h
...
theorem succ_ne_self [NonAssocRing α] [Nontrivial α] (a : α) : a + 1 ≠ a := fun h =>
  one_ne_zero ((add_right_injₓ a).mp (by simp [h]))

theorem pred_ne_self [NonAssocRing α] [Nontrivial α] (a : α) : a - 1 ≠ a := fun h =>
  one_ne_zero (neg_injective ((add_right_injₓ a).mp (by simpa [sub_eq_add_neg] using h)))
...
def NoZeroDivisors.toCancelMonoidWithZero [Ringₓ α] [NoZeroDivisors α] : CancelMonoidWithZero α :=
  { (by infer_instance : MonoidWithZeroₓ α) with
...
theorem mul_self_sub_mul_self_eq [NonUnitalNonAssocRing R] {a b : R} (h : Commute a b) :
    a * a - b * b = (a + b) * (a - b) := by rw [add_mulₓ, mul_sub, mul_sub, h.eq, sub_add_sub_cancel]

theorem mul_self_sub_mul_self_eq' [NonUnitalNonAssocRing R] {a b : R} (h : Commute a b) :
    a * a - b * b = (a - b) * (a + b) := by rw [mul_addₓ, sub_mul, sub_mul, h.eq, sub_add_sub_cancel]
