...
@[protect_proj, ancestor AddCommMonoidₓ Distrib MulZeroClass]
class NonUnitalNonAssocSemiring (α : Type u) extends AddCommMonoidₓ α, Distrib α, MulZeroClass α

/--  An associative but not-necessarily unital semiring. -/
@[protect_proj, ancestor NonUnitalNonAssocSemiring SemigroupWithZero]
class NonUnitalSemiring (α : Type u) extends NonUnitalNonAssocSemiring α, SemigroupWithZero α

/--  A unital but not-necessarily-associative semiring. -/
@[protect_proj, ancestor NonUnitalNonAssocSemiring MulZeroOneClass]
...
@[protect_proj, ancestor NonUnitalSemiring NonAssocSemiring MonoidWithZeroₓ]
...
@[to_additive]
theorem mul_ite {α} [Mul α] (P : Prop) [Decidable P] (a b c : α) : (a*if P then b else c) = if P then a*b else a*c := by
  split_ifs <;> rfl
     ...
     (Term.app
      ...
      [(Term.fun
        ...
        (Term.basicFun
         ...
         "=>"
         (Init.Logic.«term_*_» (numLit "2") "*" `x)))])
     "="
     (Set.«term{_|_}» "{" `a "|" (Term.app `Even [`a]) "}"))))
    ...
    (Tactic.tacticSeq
     (Tactic.tacticSeq1Indented
      [(group (Tactic.ext "ext" [(Tactic.rcasesPat.one `x)] []) [])
       (group
        (Tactic.simp "simp" [] [] ["[" [(Tactic.simpLemma [] [] `Even) "," (Tactic.simpLemma [] [] `eq_comm)] "]"] [])
        [])])))
   [])
  []
  []))
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `range_two_mul_add_one [])
  (Command.declSig
   [(Term.explicitBinder "(" [`α] [":" (Term.type "Type" [(Level.hole "_")])] [] ")")
    (Term.instBinder "[" [] (Term.app `Semiringₓ [`α]) "]")]
   (Term.typeSpec
     ...
     (Term.app
      ...
      [(Term.fun
        ...
        (Term.basicFun
         [(Term.simpleBinder [`x] [(Term.typeSpec ":" `α)])]
         "=>"
         (Init.Logic.«term_+_» (Init.Logic.«term_*_» (numLit "2") "*" `x) "+" (numLit "1"))))])
     "="
     (Set.«term{_|_}» "{" `a "|" (Term.app `Odd [`a]) "}"))))
  (Command.declValSimple
   ":="
   (Term.byTactic
    "by"
    (Tactic.tacticSeq
     (Tactic.tacticSeq1Indented
      [(group (Tactic.ext "ext" [(Tactic.rcasesPat.one `x)] []) [])
       (group
        (Tactic.simp "simp" [] [] ["[" [(Tactic.simpLemma [] [] `Odd) "," (Tactic.simpLemma [] [] `eq_comm)] "]"] [])
        [])])))
   [])
  []
  []))
...
theorem coe_add_monoid_hom_injective : Function.Injective (coeₓ : (α →+* β) → α →+ β) := fun f g h =>
  ext fun x => AddMonoidHom.congr_fun h x

theorem coe_monoid_hom_injective : Function.Injective (coeₓ : (α →+* β) → α →* β) := fun f g h =>
...
instance
  : Monoidₓ ( α →+* α )
  where one := id α mul := comp mul_one := comp_id one_mul := id_comp mul_assoc f g h := comp_assoc _ _ _
...
@[protect_proj, ancestor AddCommGroupₓ Monoidₓ Distrib]
...
protected def Function.Injective.ring [HasZero β] [HasOne β] [Add β] [Mul β] [Neg β] [Sub β] (f : β → α)
    (hf : injective f) (zero : f 0 = 0) (one : f 1 = 1) (add : ∀ x y, f (x+y) = f x+f y)
    (mul : ∀ x y, f (x*y) = f x*f y) (neg : ∀ x, f (-x) = -f x) (sub : ∀ x y, f (x - y) = f x - f y) : Ringₓ β :=
...
theorem mul_add_eq_mul_add_iff_sub_mul_add_eq : (((a*e)+c) = (b*e)+d) ↔ (((a - b)*e)+c) = d :=
  calc (((a*e)+c) = (b*e)+d) ↔ ((a*e)+c) = d+b*e := by
    simp [add_commₓ]
    _ ↔ (((a*e)+c) - b*e) = d :=
    Iff.intro
      (fun h => by
...
theorem Odd.neg {a : α} (hp : Odd a) : Odd (-a) := by
  obtain ⟨k, hk⟩ := hp
  use -k+1
  rw [mul_neg_eq_neg_mul_symm, mul_addₓ, neg_add, add_assocₓ, two_mul (1 : α), neg_add, neg_add_cancel_right, ← neg_add,
    hk]
...
protected def Function.Injective.commRing [HasZero β] [HasOne β] [Add β] [Mul β] [Neg β] [Sub β] (f : β → α)
    (hf : injective f) (zero : f 0 = 0) (one : f 1 = 1) (add : ∀ x y, f (x+y) = f x+f y)
    (mul : ∀ x y, f (x*y) = f x*f y) (neg : ∀ x, f (-x) = -f x) (sub : ∀ x y, f (x - y) = f x - f y) : CommRingₓ β :=
...
theorem Units.inv_eq_self_iff (u : Units α) : u⁻¹ = u ↔ u = 1 ∨ u = -1 := by
  ...
  { f with map_one' := h_one,
    map_mul' := by
      ...
      rw [mul_addₓ, add_mulₓ, add_mulₓ, f.map_add, f.map_add, f.map_add, f.map_add, h x, h y, add_mulₓ, mul_addₓ,
        mul_addₓ, ← sub_eq_zero, add_commₓ, ← sub_sub, ← sub_sub, ← sub_sub, mul_commₓ y x, mul_commₓ (f y) (f x)] at
        hxy
...
theorem AddMonoidHom.coe_add_monoid_hom_mk_ring_hom_of_mul_self_of_two_ne_zero [CommRingₓ β] (f : β →+ α)
    h h_two h_one : (f.mk_ring_hom_of_mul_self_of_two_ne_zero h h_two h_one : β →+ α) = f := by
  ext
  simp
