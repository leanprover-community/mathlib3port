...
@[simp]
theorem sub_eq_add : a - b = a+b :=
  by 
    rw [sub_eq_add_neg, add_right_injₓ, neg_eq]

@[simp]
theorem mul_one_add_self : (a*1+a) = 0 :=
  by 
...
theorem sup_inf_self (a b : α) : a⊔a⊓b = a :=
  by 
    dsimp only [·⊔·, ·⊓·]
    assocRw [mul_self, add_self, add_zeroₓ]

theorem inf_sup_self (a b : α) : a⊓(a⊔b) = a :=
  by 
    dsimp only [·⊔·, ·⊓·]
    assocRw [mul_addₓ, mul_addₓ, mul_self, mul_self, add_self, add_zeroₓ]

theorem le_sup_inf_aux (a b c : α) : (((a+b)+a*b)*(a+c)+a*c) = (a+b*c)+a*b*c :=
  calc (((a+b)+a*b)*(a+c)+a*c) = (((((a*a)+b*c)+a*b*c)+(a*b)+(a*a)*b)+(a*c)+(a*a)*c)+((a*b)*c)+((a*a)*b)*c :=
    by 
      ring 
    _ = (a+b*c)+a*b*c :=
    by 
...
theorem sup_inf_sdiff (a b : α) : a⊓b⊔a \ b = a :=
  calc (((a*b)+a*1+b)+(a*b)*a*1+b) = ((a*b)+a*1+b)+(a*a)*b*1+b :=
    by 
      acRfl 
    _ = (a*b)+a+a*b :=
    by 
...
def to_boolean_algebra : BooleanAlgebra α :=
  { Lattice.mk' sup_comm sup_assoc inf_comm inf_assoc sup_inf_self inf_sup_self, HasSdiff, HasBot with
    ...
    le_top :=
      fun a =>
        show ((a+1)+a*1) = 1by 
          assocRw [mul_oneₓ, add_commₓ, add_self, add_zeroₓ],
    bot_le :=
      fun a =>
        show ((0+a)+0*a) = a by 
    ...
    inf_compl_le_bot :=
      fun a =>
        show (((a*1+a)+0)+(a*1+a)*0) = 0 by 
          normNum [mul_addₓ, mul_self, add_self],
    top_le_sup_compl :=
      fun a =>
        by 
          change ((1+(a+1+a)+a*1+a)+1*(a+1+a)+a*1+a) = (a+1+a)+a*1+a 
          normNum [mul_addₓ, mul_self]
