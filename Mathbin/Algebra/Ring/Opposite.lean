...
def NonUnitalRingHom.toOpposite {R S : Type _} [NonUnitalNonAssocSemiring R] [NonUnitalNonAssocSemiring S]
...
@[simps (config := { fullyApplied := false })]
def NonUnitalRingHom.fromOpposite {R S : Type _} [NonUnitalNonAssocSemiring R] [NonUnitalNonAssocSemiring S]
...
@[simps (config := { fullyApplied := false })]
def RingHom.toOpposite {R S : Type _} [Semiring R] [Semiring S] (f : R →+* S) (hf : ∀ x y, Commute (f x) (f y)) :
    R →+* Sᵐᵒᵖ :=
...
@[simps (config := { fullyApplied := false })]
def RingHom.fromOpposite {R S : Type _} [Semiring R] [Semiring S] (f : R →+* S) (hf : ∀ x y, Commute (f x) (f y)) :
    Rᵐᵒᵖ →+* S :=
