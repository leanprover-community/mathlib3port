...
theorem Quotientₓ (R : Type u) [CommRingₓ R] (p : ℕ) [hp1 : Fact p.prime] (hp2 : «expr↑ » p ∈ Nonunits R) :
  CharP (Ideal.span {p} : Ideal R).Quotient p :=
...
theorem quotient' {R : Type _} [CommRingₓ R] (p : ℕ) [CharP R p] (I : Ideal R)
  ...
  ⟨fun x =>
      by 
        ...
        refine' quotient.eq'.trans (_ : «expr↑ » x - 0 ∈ I ↔ _)
        rw [sub_zero]
