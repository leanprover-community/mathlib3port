...
#align_import data.polynomial.degree.card_pow_degree from "leanprover-community/mathlib"@"31ca6f9cf5f90a6206092cd7f84b359dcb6d52e0"
...
noncomputable def cardPowDegree : AbsoluteValue Fq[X] ℤ :=
  ...
  { toFun := fun p => if p = 0 then 0 else Fintype.card Fq ^ p.natDegree
    nonneg' := fun p => by dsimp; split_ifs; · rfl; exact Nonneg.pow_nonneg (Int.ofNat_zero_le _) _
    eq_zero' := fun p =>
      ite_eq_left_iff.trans <| ⟨fun h => by contrapose! h; exact ⟨h, (pow_pos _).ne'⟩, absurd⟩
    add_le' := fun p q => by
      ...
      refine'
        le_trans (le_max_iff.mpr _)
          (max_le_add_of_nonneg (Nonneg.pow_nonneg (by linarith) _)
            (Nonneg.pow_nonneg (by linarith) _))
...
#print Polynomial.cardPowDegree_apply /-
...
#print Polynomial.cardPowDegree_zero /-
@[simp]
...
#print Polynomial.cardPowDegree_nonzero /-
@[simp]
...
#print Polynomial.cardPowDegree_isEuclidean /-
