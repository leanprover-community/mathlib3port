...
theorem nat_degree_sum_le (f : ι → Polynomial α) : nat_degree (∑ i in s, f i) ≤ s.fold max 0 (nat_degree ∘ f) := by
  simpa using nat_degree_multiset_sum_le (s.val.map f)

theorem degree_list_sum_le (l : List (Polynomial α)) : degree l.sum ≤ (l.map nat_degree).maximum := by
  by_cases' h : l.sum = 0
  · simp [h]
...
theorem nat_degree_list_prod_le (l : List (Polynomial α)) : nat_degree l.prod ≤ (l.map nat_degree).Sum := by
...
theorem degree_list_prod_le (l : List (Polynomial α)) : degree l.prod ≤ (l.map degree).Sum := by
  induction' l with hd tl IH
  · simp
...
theorem coeff_list_prod_of_nat_degree_le (l : List (Polynomial α)) (n : ℕ) (hl : ∀, ∀ p ∈ l, ∀, nat_degree p ≤ n) :
...
theorem leading_coeff_multiset_prod' (h : (t.map leading_coeff).Prod ≠ 0) :
    t.prod.leading_coeff = (t.map leading_coeff).Prod := by
...
theorem nat_degree_multiset_prod' (h : (t.map fun f => leading_coeff f).Prod ≠ 0) :
    t.prod.nat_degree = (t.map fun f => nat_degree f).Sum := by
  revert h
...
theorem nat_degree_multiset_prod_of_monic [Nontrivial R] (h : ∀, ∀ f ∈ t, ∀, monic f) :
    t.prod.nat_degree = (t.map nat_degree).Sum := by
...
theorem coeff_multiset_prod_of_nat_degree_le (n : ℕ) (hl : ∀, ∀ p ∈ t, ∀, nat_degree p ≤ n) :
...
theorem coeff_prod_of_nat_degree_le (f : ι → Polynomial R) (n : ℕ) (h : ∀, ∀ p ∈ s, ∀, nat_degree (f p) ≤ n) :
...
theorem multiset_prod_X_sub_C_next_coeff [Nontrivial R] (t : Multiset R) :
    next_coeff (t.map fun x => X - C x).Prod = -t.sum := by
...
theorem prod_X_sub_C_next_coeff [Nontrivial R] {s : Finset ι} (f : ι → R) :
    next_coeff (∏ i in s, X - C (f i)) = -∑ i in s, f i := by
...
theorem nat_degree_multiset_prod [Nontrivial R] (s : Multiset (Polynomial R)) (h : (0 : Polynomial R) ∉ s) :
    nat_degree s.prod = (s.map nat_degree).Sum := by
...
where additionally, the product of the leading coefficients must be nonzero.
-/
theorem leading_coeff_multiset_prod : t.prod.leading_coeff = (t.map fun f => leading_coeff f).Prod := by
