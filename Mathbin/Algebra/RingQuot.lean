...
-/
def mk_ring_hom (r : R → R → Prop) : R →+* RingQuot r where
...
theorem mk_ring_hom_surjective (r : R → R → Prop) : Function.Surjective (mk_ring_hom r) := by
  dsimp [mk_ring_hom]
  rintro ⟨⟨⟩⟩
  simp
...
theorem ring_quot_ext {T : Type u₄} [Semiringₓ T] {r : R → R → Prop} (f g : RingQuot r →+* T)
    (w : f.comp (mk_ring_hom r) = g.comp (mk_ring_hom r)) : f = g := by
  ext
...
theorem lift_unique (f : R →+* T) {r : R → R → Prop} (w : ∀ ⦃x y⦄, r x y → f x = f y) (g : RingQuot r →+* T)
    (h : g.comp (mk_ring_hom r) = f) : g = lift ⟨f, w⟩ := by
  ext
  simp [h]
...
def ring_quot_to_ideal_quotient (r : B → B → Prop) : RingQuot r →+* B ⧸ Ideal.ofRel r :=
  lift
    ⟨Ideal.Quotient.mk (Ideal.ofRel r), fun x y h =>
      Quot.sound (Submodule.mem_Inf.mpr fun p w => w ⟨x, y, h, sub_add_cancel x y⟩)⟩
...
theorem mk_alg_hom_surjective (s : A → A → Prop) : Function.Surjective (mk_alg_hom S s) := by
  dsimp [mk_alg_hom]
  rintro ⟨⟨a⟩⟩
  use a
  rfl
...
theorem ring_quot_ext' {s : A → A → Prop} (f g : RingQuot s →ₐ[S] B)
    (w : f.comp (mk_alg_hom S s) = g.comp (mk_alg_hom S s)) : f = g := by
  ext
...
-/
def lift_alg_hom {s : A → A → Prop} : { f : A →ₐ[S] B // ∀ ⦃x y⦄, s x y → f x = f y } ≃ (RingQuot s →ₐ[S] B) where
  toFun := fun f' =>
    let f := (f' : A →ₐ[S] B)
    { toFun := fun x =>
        Quot.lift f
  ...
  invFun := fun F =>
    ⟨F.comp (mk_alg_hom S s), fun _ _ h => by
      dsimp
...
theorem lift_alg_hom_unique (f : A →ₐ[S] B) {s : A → A → Prop} (w : ∀ ⦃x y⦄, s x y → f x = f y) (g : RingQuot s →ₐ[S] B)
    (h : g.comp (mk_alg_hom S s) = f) : g = lift_alg_hom S ⟨f, w⟩ := by
  ext
  simp [h]
