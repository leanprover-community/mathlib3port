...
theorem omega_le_cardinal_mk_of_char_zero (R A : Type _) [CommRingₓ R] [IsDomain R] [Ringₓ A] [Algebra R A]
    [CharZero A] : ω ≤ # { x : A | IsAlgebraic R x } :=
...
theorem cardinal_mk_lift_le_mul :
    Cardinal.lift.{u, v} (# { x : A | IsAlgebraic R x }) ≤ Cardinal.lift.{v, u} (# (Polynomial R)) * ω := by
  ...
  suffices Fintype (g ⁻¹' {f}) by
    exact @mk_le_omega _ (@Fintype.toEncodable _ this)
  by_cases' hf : f.1 = 0
...
theorem cardinal_mk_lift_le_max :
    Cardinal.lift.{u, v} (# { x : A | IsAlgebraic R x }) ≤ max (Cardinal.lift.{v, u} (# R)) ω :=
...
@[simp]
theorem cardinal_mk_of_encodable_of_char_zero [CharZero A] [IsDomain R] : # { x : A | IsAlgebraic R x } = ω :=
  le_antisymmₓ
    (by
      simp )
    (omega_le_cardinal_mk_of_char_zero R A)
...
theorem cardinal_mk_le_mul : # { x : A | IsAlgebraic R x } ≤ # (Polynomial R) * ω := by
  rw [← lift_id (# _), ← lift_id (# (Polynomial R))]
  exact cardinal_mk_lift_le_mul R A

theorem cardinal_mk_le_max : # { x : A | IsAlgebraic R x } ≤ max (# R) ω := by
