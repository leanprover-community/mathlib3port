...
theorem monotone_smul_left (hc : 0 ≤ c) : Monotone (HasScalar.smul c : M → M) :=
  fun a b h => smul_le_smul_of_nonneg h hc

theorem strict_mono_smul_left (hc : 0 < c) : StrictMono (HasScalar.smul c : M → M) :=
  fun a b h => smul_lt_smul_of_pos h hc
