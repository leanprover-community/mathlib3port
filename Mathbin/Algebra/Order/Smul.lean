...
instance [ HasScalar R M ] : HasScalar R ( OrderDual M ) where smul k x := OrderDual.rec ( fun x' => ( k • x' : M ) ) x
...
-- failed to format: format: uncaught backtrack exception
instance
  [ MonoidWithZeroₓ R ] [ AddMonoidₓ M ] [ DistribMulAction R M ] : DistribMulAction R ( OrderDual M )
  where
    smul_add k a := OrderDual.rec ( fun a' b => OrderDual.rec ( smul_add _ _ ) b ) a
      smul_zero r := OrderDual.rec smul_zero r

-- failed to format: format: uncaught backtrack exception
instance
  [ OrderedSemiring R ] [ OrderedAddCommMonoid M ] [ SmulWithZero R M ] [ OrderedSmul R M ]
...
-- failed to format: format: uncaught backtrack exception
instance
  LinearOrderedSemiring.to_ordered_smul
  { R : Type _ } [ LinearOrderedSemiring R ] : OrderedSmul R R
  where
    smul_lt_smul_of_pos := OrderedSemiring.mul_lt_mul_of_pos_left
      lt_of_smul_lt_smul_of_pos _ _ _ h hc := lt_of_mul_lt_mul_left h hc.le
...
@[simps]
def OrderIso.smulLeft {c : k} (hc : 0 < c) : M ≃o M :=
  { toFun := fun b => c • b, invFun := fun b => c⁻¹ • b, left_inv := inv_smul_smul₀ hc.ne',
    right_inv := smul_inv_smul₀ hc.ne', map_rel_iff' := fun b₁ b₂ => smul_le_smul_iff_of_pos hc }
