...
@[simp]
theorem inv_of_nonpos [Invertible a] : ⅟ a ≤ 0 ↔ a ≤ 0 := by simp only [← not_ltₓ, inv_of_pos]
...
@[simp]
theorem inv_of_lt_zero [Invertible a] : ⅟ a < 0 ↔ a < 0 := by simp only [← not_leₓ, inv_of_nonneg]
