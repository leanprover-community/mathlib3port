...
@[toAdditive]
instance Units.orderedCommGroup [OrderedCommMonoid α] : OrderedCommGroup (Units α) :=
...
/--  Uses `left` co(ntra)variant. -/
@[simp, toAdditive Left.nonneg_neg_iff]
...
theorem Left.one_lt_inv_iff : 1 < a⁻¹ ↔ a < 1 :=
  by 
    rw [←mul_lt_mul_iff_left a, mul_inv_selfₓ, mul_oneₓ]

/--  Uses `left` co(ntra)variant. -/
@[simp, toAdditive Left.neg_neg_iff]
theorem Left.inv_lt_one_iff : a⁻¹ < 1 ↔ 1 < a :=
  by 
    rw [←mul_lt_mul_iff_left a, mul_inv_selfₓ, mul_oneₓ]
...
/--  Uses `right` co(ntra)variant. -/
@[simp, toAdditive Right.nonneg_neg_iff]
...
theorem inv_le_iff_one_le_mul : a⁻¹ ≤ b ↔ 1 ≤ b*a :=
  (mul_le_mul_iff_right a).symm.trans$
    by 
      rw [inv_mul_selfₓ]

@[toAdditive]
theorem le_inv_iff_mul_le_one_right : a ≤ b⁻¹ ↔ (a*b) ≤ 1 :=
  (mul_le_mul_iff_right b).symm.trans$
    by 
      rw [inv_mul_selfₓ]
...
theorem Right.inv_lt_one_iff : a⁻¹ < 1 ↔ 1 < a :=
  by 
    rw [←mul_lt_mul_iff_right a, inv_mul_selfₓ, one_mulₓ]

/--  Uses `right` co(ntra)variant. -/
@[simp, toAdditive Right.neg_pos_iff]
theorem Right.one_lt_inv_iff : 1 < a⁻¹ ↔ a < 1 :=
  by 
    rw [←mul_lt_mul_iff_right a, inv_mul_selfₓ, one_mulₓ]

@[toAdditive]
theorem inv_lt_iff_one_lt_mul : a⁻¹ < b ↔ 1 < b*a :=
  (mul_lt_mul_iff_right a).symm.trans$
    by 
      rw [inv_mul_selfₓ]

@[toAdditive]
theorem lt_inv_iff_mul_lt_one : a < b⁻¹ ↔ (a*b) < 1 :=
  (mul_lt_mul_iff_right b).symm.trans$
    by 
      rw [inv_mul_selfₓ]

@[simp, toAdditive]
theorem mul_inv_lt_iff_lt_mul : (a*b⁻¹) < c ↔ a < c*b :=
  by 
...
attribute [toAdditive neg_le_of_neg_le] inv_le_of_inv_le'
...
@[toAdditive]
theorem mul_inv_le_inv_mul_iff : ((a*b⁻¹) ≤ d⁻¹*c) ↔ (d*a) ≤ c*b :=
  by 
    rw [←mul_le_mul_iff_left d, ←mul_le_mul_iff_right b, mul_inv_cancel_left, mul_assocₓ, inv_mul_cancel_right]

@[simp, toAdditive]
theorem div_le_self_iff (a : α) {b : α} : a / b ≤ a ↔ 1 ≤ b :=
  by 
    simp [div_eq_mul_inv]

@[simp, toAdditive]
theorem le_div_self_iff (a : α) {b : α} : a ≤ a / b ↔ b ≤ 1 :=
  by 
    simp [div_eq_mul_inv]
...
@[toAdditive neg_lt]
theorem inv_lt' : a⁻¹ < b ↔ b⁻¹ < a :=
  by 
    rw [←inv_lt_inv_iff, inv_invₓ]

@[toAdditive lt_neg]
theorem lt_inv' : a < b⁻¹ ↔ b < a⁻¹ :=
  by 
    rw [←inv_lt_inv_iff, inv_invₓ]

alias lt_inv' ↔ lt_inv_of_lt_inv _

attribute [toAdditive] lt_inv_of_lt_inv

alias inv_lt' ↔ inv_lt_of_inv_lt' _

attribute [toAdditive neg_lt_of_neg_lt] inv_lt_of_inv_lt'

@[toAdditive]
theorem mul_inv_lt_inv_mul_iff : ((a*b⁻¹) < d⁻¹*c) ↔ (d*a) < c*b :=
  by 
    rw [←mul_lt_mul_iff_left d, ←mul_lt_mul_iff_right b, mul_inv_cancel_left, mul_assocₓ, inv_mul_cancel_right]

@[simp, toAdditive]
theorem div_lt_self_iff (a : α) {b : α} : a / b < a ↔ 1 < b :=
  by 
    simp [div_eq_mul_inv]
...
@[simp, toAdditive]
theorem mul_inv_le_iff_le_mul' : (a*b⁻¹) ≤ c ↔ a ≤ b*c :=
  by 
    rw [←inv_mul_le_iff_le_mul, mul_commₓ]

@[toAdditive add_neg_le_add_neg_iff]
theorem mul_inv_le_mul_inv_iff' : ((a*b⁻¹) ≤ c*d⁻¹) ↔ (a*d) ≤ c*b :=
  by 
...
@[simp, toAdditive]
theorem mul_inv_lt_iff_le_mul' : (a*b⁻¹) < c ↔ a < b*c :=
  by 
    rw [←inv_mul_lt_iff_lt_mul, mul_commₓ]

@[toAdditive add_neg_lt_add_neg_iff]
theorem mul_inv_lt_mul_inv_iff' : ((a*b⁻¹) < c*d⁻¹) ↔ (a*d) < c*b :=
  by 
...
attribute [toAdditive] le_inv_of_le_inv

alias Left.inv_le_one_iff ↔ one_le_of_inv_le_one _

attribute [toAdditive] one_le_of_inv_le_one

alias Left.one_le_inv_iff ↔ le_one_of_one_le_inv _

attribute [toAdditive nonpos_of_neg_nonneg] le_one_of_one_le_inv

alias inv_lt_inv_iff ↔ lt_of_inv_lt_inv _

attribute [toAdditive] lt_of_inv_lt_inv

alias Left.inv_lt_one_iff ↔ one_lt_of_inv_lt_one _

attribute [toAdditive] one_lt_of_inv_lt_one

alias Left.inv_lt_one_iff ← inv_lt_one_iff_one_lt

attribute [toAdditive] inv_lt_one_iff_one_lt

alias Left.inv_lt_one_iff ← inv_lt_one'

attribute [toAdditive neg_lt_zero] inv_lt_one'

alias Left.one_lt_inv_iff ↔ inv_of_one_lt_inv _

attribute [toAdditive neg_of_neg_pos] inv_of_one_lt_inv

alias Left.one_lt_inv_iff ↔ _ one_lt_inv_of_inv

attribute [toAdditive neg_pos_of_neg] one_lt_inv_of_inv

alias le_inv_mul_iff_mul_le ↔ mul_le_of_le_inv_mul _

attribute [toAdditive] mul_le_of_le_inv_mul

alias le_inv_mul_iff_mul_le ↔ _ le_inv_mul_of_mul_le

attribute [toAdditive] le_inv_mul_of_mul_le

alias inv_mul_le_iff_le_mul ↔ _ inv_mul_le_of_le_mul

attribute [toAdditive] inv_mul_le_iff_le_mul

alias lt_inv_mul_iff_mul_lt ↔ mul_lt_of_lt_inv_mul _

attribute [toAdditive] mul_lt_of_lt_inv_mul

alias lt_inv_mul_iff_mul_lt ↔ _ lt_inv_mul_of_mul_lt

attribute [toAdditive] lt_inv_mul_of_mul_lt

alias inv_mul_lt_iff_lt_mul ↔ lt_mul_of_inv_mul_lt inv_mul_lt_of_lt_mul

attribute [toAdditive] lt_mul_of_inv_mul_lt

attribute [toAdditive] inv_mul_lt_of_lt_mul

alias lt_mul_of_inv_mul_lt ← lt_mul_of_inv_mul_lt_left

attribute [toAdditive] lt_mul_of_inv_mul_lt_left

alias Left.inv_le_one_iff ← inv_le_one'

attribute [toAdditive neg_nonpos] inv_le_one'

alias Left.one_le_inv_iff ← one_le_inv'

attribute [toAdditive neg_nonneg] one_le_inv'

alias Left.one_lt_inv_iff ← one_lt_inv'

attribute [toAdditive neg_pos] one_lt_inv'

alias mul_lt_mul_left' ← OrderedCommGroup.mul_lt_mul_left'

attribute [toAdditive OrderedAddCommGroup.add_lt_add_left] OrderedCommGroup.mul_lt_mul_left'

alias le_of_mul_le_mul_left' ← OrderedCommGroup.le_of_mul_le_mul_left

attribute [toAdditive OrderedAddCommGroup.le_of_add_le_add_left] OrderedCommGroup.le_of_mul_le_mul_left

alias lt_of_mul_lt_mul_left' ← OrderedCommGroup.lt_of_mul_lt_mul_left

attribute [toAdditive OrderedAddCommGroup.lt_of_add_lt_add_left] OrderedCommGroup.lt_of_mul_lt_mul_left

/-- Pullback an `ordered_comm_group` under an injective map.
See note [reducible non-instances]. -/
@[reducible,
  toAdditive Function.Injective.orderedAddCommGroup "Pullback an `ordered_add_comm_group` under an injective map."]
def Function.Injective.orderedCommGroup [OrderedCommGroup α] {β : Type _} [HasOne β] [Mul β] [HasInv β] [Div β]
  (f : β → α) (hf : Function.Injective f) (one : f 1 = 1) (mul : ∀ x y, f (x*y) = f x*f y) (inv : ∀ x, f (x⁻¹) = f x⁻¹)
...
@[toAdditive]
theorem le_div_iff_mul_le : a ≤ c / b ↔ (a*b) ≤ c :=
  by 
...
@[toAdditive]
theorem div_le_iff_le_mul : a / c ≤ b ↔ a ≤ b*c :=
  by 
...
@[toAdditive]
theorem le_div_iff_mul_le' : b ≤ c / a ↔ (a*b) ≤ c :=
  by 
...
@[toAdditive]
theorem div_le_iff_le_mul' : a / b ≤ c ↔ a ≤ b*c :=
  by 
...
@[toAdditive]
theorem inv_le_div_iff_le_mul' : a⁻¹ ≤ b / c ↔ c ≤ a*b :=
  by 
...
@[simp, toAdditive sub_neg]
theorem div_lt_one' : a / b < 1 ↔ a < b :=
  by 
    rw [←mul_lt_mul_iff_right b, one_mulₓ, div_eq_mul_inv, inv_mul_cancel_right]
...
@[toAdditive]
theorem lt_div_iff_mul_lt : a < c / b ↔ (a*b) < c :=
  by 
...
@[toAdditive]
theorem div_lt_iff_lt_mul : a / c < b ↔ a < b*c :=
  by 
...
@[simp, toAdditive]
theorem inv_lt_div_iff_lt_mul : a⁻¹ < b / c ↔ c < a*b :=
  by 
    rw [div_eq_mul_inv, lt_mul_inv_iff_mul_lt, inv_mul_lt_iff_lt_mul]

@[toAdditive sub_lt_sub_left]
...
@[toAdditive]
theorem lt_div_iff_mul_lt' : b < c / a ↔ (a*b) < c :=
  by 
...
@[toAdditive]
theorem div_lt_iff_lt_mul' : a / b < c ↔ a < b*c :=
  by 
...
@[simp, toAdditive]
theorem max_one_div_max_inv_one_eq_self (a : α) : max a 1 / max (a⁻¹) 1 = a :=
  by 
...
See note [reducible non-instances]. -/
@[reducible,
  toAdditive Function.Injective.linearOrderedAddCommGroup
...
@[toAdditive LinearOrderedAddCommGroup.add_lt_add_left]
theorem LinearOrderedCommGroup.mul_lt_mul_left' (a b : α) (h : a < b) (c : α) : (c*a) < c*b :=
  mul_lt_mul_left' h c

@[toAdditive min_neg_neg]
...
@[toAdditive min_sub_sub_right]
theorem min_div_div_right' (a b c : α) : min (a / c) (b / c) = min a b / c :=
  by 
    simpa only [div_eq_mul_inv] using min_mul_mul_right a b (c⁻¹)

@[toAdditive max_sub_sub_right]
theorem max_div_div_right' (a b c : α) : max (a / c) (b / c) = max a b / c :=
  by 
    simpa only [div_eq_mul_inv] using max_mul_mul_right a b (c⁻¹)

@[toAdditive min_sub_sub_left]
theorem min_div_div_left' (a b c : α) : min (a / b) (a / c) = a / max b c :=
  by 
    simp only [div_eq_mul_inv, min_mul_mul_left, min_inv_inv']

@[toAdditive max_sub_sub_left]
theorem max_div_div_left' (a b c : α) : max (a / b) (a / c) = a / min b c :=
  by 
    simp only [div_eq_mul_inv, max_mul_mul_left, max_inv_inv']

@[toAdditive eq_zero_of_neg_eq]
...
theorem eq_or_eq_neg_of_abs_eq {a b : α} (h : |a| = b) : a = b ∨ a = -b :=
  by 
...
theorem abs_sub_comm (a b : α) : |a - b| = |b - a| :=
  calc |a - b| = |-(b - a)| := congr_argₓ _ (neg_sub b a).symm 
    _ = |b - a| := abs_neg (b - a)
...
theorem abs_sub_le_iff : |a - b| ≤ c ↔ a - b ≤ c ∧ b - a ≤ c :=
  by 
    rw [abs_le, neg_le_sub_iff_le_add, sub_le_iff_le_add', and_comm, sub_le_iff_le_add']

theorem abs_sub_lt_iff : |a - b| < c ↔ a - b < c ∧ b - a < c :=
  by 
...
theorem abs_min_sub_min_le_max (a b c d : α) : |min a b - min c d| ≤ max |a - c| |b - d| :=
  by 
    simpa only [max_neg_neg, neg_sub_neg, abs_sub_comm] using abs_max_sub_max_le_max (-a) (-b) (-c) (-d)

theorem abs_max_sub_max_le_abs (a b c : α) : |max a c - max b c| ≤ |a - b| :=
  by 
...
@[toAdditive neg_nonneg_of_nonpos]
