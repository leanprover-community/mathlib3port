...
@[ to_additive ]
  instance
    OrderedCommMonoid.to_covariant_class_left
    ( M : Type _ ) [ OrderedCommMonoid M ] : CovariantClass M M ( · * · ) ( · ≤ · )
...
@[simp, norm_cast, to_additive]
theorem max_coe [Monoidₓ α] [LinearOrderₓ α] {a b : Units α} : (↑max a b : α) = max a b := by
  by_cases' b ≤ a <;> simp [max_def, h]

@[simp, norm_cast, to_additive]
theorem min_coe [Monoidₓ α] [LinearOrderₓ α] {a b : Units α} : (↑min a b : α) = min a b := by
...
@[ to_additive ]
  instance
    ( priority := 100 )
    CanonicallyOrderedMonoid.has_exists_mul_of_le
    ( α : Type u ) [ CanonicallyOrderedMonoid α ] : HasExistsMulOfLe α
    where exists_mul_of_le a b hab := le_iff_exists_mul . mp hab
...
@[ to_additive ]
  instance
    OrderedCancelCommMonoid.to_contravariant_class_left
    ( M : Type _ ) [ OrderedCancelCommMonoid M ] : ContravariantClass M M ( · * · ) ( · < · )
...
@[ to_additive OrderedCancelAddCommMonoid.to_contravariant_class ]
  instance
    ordered_cancel_comm_monoid.to_contravariant_class
    [ OrderedCancelCommMonoid α ] : ContravariantClass ( OrderDual α ) ( OrderDual α ) Mul.mul LE.le
    where elim a b c bc := OrderedCancelCommMonoid.le_of_mul_le_mul_left a c b ( dual_le . mp bc )
