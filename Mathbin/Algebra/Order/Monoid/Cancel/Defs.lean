...
lean 3 declaration is
  forall (M : Type.{u1}) [_inst_2 : OrderedCancelCommMonoid.{u1} M], ContravariantClass.{u1, u1} M M (HMul.hMul.{u1, u1, u1} M M M (instHMul.{u1} M (MulOneClass.toHasMul.{u1} M (Monoid.toMulOneClass.{u1} M (CommMonoid.toMonoid.{u1} M (OrderedCancelCommMonoid.toCommMonoid.{u1} M _inst_2)))))) (LT.lt.{u1} M (Preorder.toHasLt.{u1} M (PartialOrder.toPreorder.{u1} M (OrderedCancelCommMonoid.toPartialOrder.{u1} M _inst_2))))
but is expected to have type
