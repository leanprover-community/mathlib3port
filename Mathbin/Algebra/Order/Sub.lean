...
theorem le_mul_tsub {R : Type _} [Distrib R] [Preorderₓ R] [Sub R] [HasOrderedSub R] [CovariantClass R R (·*·) (· ≤ ·)]
...
theorem le_tsub_mul {R : Type _} [CommSemiringₓ R] [Preorderₓ R] [Sub R] [HasOrderedSub R]
  [CovariantClass R R (·*·) (· ≤ ·)] {a b c : R} : ((a*c) - b*c) ≤ (a - b)*c :=
  by 
...
theorem OrderIso.map_tsub {M N : Type _} [Preorderₓ M] [Add M] [Sub M] [HasOrderedSub M] [PartialOrderₓ N] [Add N]
  ...
  by 
    ...
    suffices  : e (e.symm (e a) - e.symm (e b)) ≤ e (e.symm (e a - e b))
    ·
      simpa 
...
theorem tsub_le_iff_tsub_le : a - b ≤ c ↔ a - c ≤ b :=
  by 
...
theorem tsub_right_comm : a - b - c = a - c - b :=
  by 
    simpRw [←tsub_add_eq_tsub_tsub, add_commₓ]
...
protected theorem tsub_eq_of_eq_add (hb : AddLeCancellable b) (h : a = c+b) : a - b = c :=
  le_antisymmₓ (tsub_le_iff_right.mpr h.le)$
    by 
      rw [h]
...
theorem add_tsub_add_eq_tsub_left (a b c : α) : ((a+b) - a+c) = b - c :=
  by 
...
theorem tsub_le_tsub_iff_right (h : c ≤ b) : a - c ≤ b - c ↔ a ≤ b :=
  by 
    rw [tsub_le_iff_right, tsub_add_cancel_of_le h]

theorem tsub_left_inj (h1 : c ≤ a) (h2 : c ≤ b) : a - c = b - c ↔ a = b :=
  by 
    simpRw [le_antisymm_iffₓ, tsub_le_tsub_iff_right h1, tsub_le_tsub_iff_right h2]
...
@[simp]
theorem tsub_eq_zero_iff_le : a - b = 0 ↔ a ≤ b :=
  by 
...
theorem tsub_inj_left (h₁ : a ≤ b) (h₂ : a ≤ c) (h₃ : b - a = c - a) : b = c :=
  by 
    rw [←tsub_add_cancel_of_le h₁, ←tsub_add_cancel_of_le h₂, h₃]

theorem tsub_pos_iff_not_le : 0 < a - b ↔ ¬a ≤ b :=
  by 
...
theorem tsub_add_tsub_cancel (hab : b ≤ a) (hbc : c ≤ b) : ((a - b)+b - c) = a - c :=
  by 
...
protected theorem tsub_eq_iff_eq_add_of_le (hb : AddLeCancellable b) (h : b ≤ a) : a - b = c ↔ a = c+b :=
  by 
    rw [eq_comm, hb.eq_tsub_iff_add_eq_of_le h, eq_comm]

protected theorem add_tsub_assoc_of_le (hc : AddLeCancellable c) (h : c ≤ b) (a : α) : (a+b) - c = a+b - c :=
  by 
    convLHS => rw [←add_tsub_cancel_of_le h, add_commₓ c, ←add_assocₓ, hc.add_tsub_cancel_right]

protected theorem tsub_add_eq_add_tsub (hb : AddLeCancellable b) (h : b ≤ a) : ((a - b)+c) = (a+c) - b :=
  by 
...
protected theorem tsub_lt_iff_right (hb : AddLeCancellable b) (hba : b ≤ a) : a - b < c ↔ a < c+b :=
  by 
    rw [add_commₓ]
...
protected theorem lt_tsub_iff_right_of_le (hc : AddLeCancellable c) (h : c ≤ b) : a < b - c ↔ (a+c) < b :=
  by 
...
protected theorem lt_tsub_iff_left_of_le (hc : AddLeCancellable c) (h : c ≤ b) : a < b - c ↔ (c+a) < b :=
  by 
    rw [add_commₓ]
...
protected theorem lt_of_tsub_lt_tsub_left_of_le [ContravariantClass α α (·+·) (· < ·)] (hb : AddLeCancellable b)
  ...
  by 
    convLHS at h => rw [←tsub_add_cancel_of_le hca]
...
protected theorem tsub_right_inj (ha : AddLeCancellable a) (hb : AddLeCancellable b) (hc : AddLeCancellable c)
  ...
  by 
    simpRw [le_antisymm_iffₓ, ha.tsub_le_tsub_iff_left hb hba, ha.tsub_le_tsub_iff_left hc hca, and_comm]

protected theorem tsub_lt_tsub_right_of_le (hc : AddLeCancellable c) (h : c ≤ a) (h2 : a < b) : a - c < b - c :=
  by 
...
protected theorem tsub_lt_tsub_iff_right (hc : AddLeCancellable c) (h : c ≤ a) : a - c < b - c ↔ a < b :=
  by 
...
theorem add_tsub_eq_max : (a+b - a) = max a b :=
  by 
...
@[simp]
theorem sub_top {a : WithTop α} : a - ⊤ = 0 :=
  by 
    cases a <;> rfl
