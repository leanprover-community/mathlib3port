...
protected theorem Decidable.add_le_mul_two_add [@DecidableRel α (· ≤ ·)] {a b : α} (a2 : 2 ≤ a) (b0 : 0 ≤ b) :
    (a+2+b) ≤ a*2+b :=
  calc (a+2+b) ≤ a+a+a*b := add_le_add_left (add_le_add a2 (Decidable.le_mul_of_one_le_left b0 (one_le_two.trans a2))) a
    _ ≤ a*2+b := by
...
theorem lt_of_mul_lt_mul_left (h : (c*a) < c*b) (hc : 0 ≤ c) : a < b := by
...
theorem lt_of_mul_lt_mul_right (h : (a*c) < b*c) (hc : 0 ≤ c) : a < b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
      lt_of_not_geₓ fun h1 : b ≤ a =>
...
@[simp]
theorem inv_of_le_one [Invertible a] (h : 1 ≤ a) : ⅟ a ≤ 1 := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact mul_inv_of_self a ▸ Decidable.le_mul_of_one_le_left (inv_of_nonneg.2 $ zero_le_one.trans h) h

theorem neg_of_mul_neg_left (h : (a*b) < 0) (h1 : 0 ≤ a) : b < 0 := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact lt_of_not_geₓ fun h2 : b ≥ 0 => (Decidable.mul_nonneg h1 h2).not_lt h

theorem neg_of_mul_neg_right (h : (a*b) < 0) (h1 : 0 ≤ b) : a < 0 := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact lt_of_not_geₓ fun h2 : a ≥ 0 => (Decidable.mul_nonneg h2 h1).not_lt h
...
@[simp]
theorem mul_lt_mul_right (h : 0 < c) : ((a*c) < b*c) ↔ a < b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
...
theorem mul_nonneg_iff_right_nonneg_of_pos (ha : 0 < a) : (0 ≤ a*b) ↔ 0 ≤ b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact ⟨fun h => nonneg_of_mul_nonneg_left h ha, fun h => Decidable.mul_nonneg ha.le h⟩

theorem mul_nonneg_iff_left_nonneg_of_pos (hb : 0 < b) : (0 ≤ a*b) ↔ 0 ≤ a := by
  have := @LinearOrderₓ.decidableLe α _ <;>
...
theorem monotone_mul_left_of_nonneg (ha : 0 ≤ a) : Monotone fun x => a*x := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact fun b c b_le_c => Decidable.mul_le_mul_of_nonneg_left b_le_c ha

theorem monotone_mul_right_of_nonneg (ha : 0 ≤ a) : Monotone fun x => x*a := by
...
theorem mul_nonneg_iff : (0 ≤ a*b) ↔ 0 ≤ a ∧ 0 ≤ b ∨ a ≤ 0 ∧ b ≤ 0 := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
...
theorem le_of_mul_le_of_one_le {a b c : α} (h : (a*c) ≤ b) (hb : 0 ≤ b) (hc : 1 ≤ c) : a ≤ b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
      have h' : (a*c) ≤ b*c :=
        calc (a*c) ≤ b := h
...
theorem nonneg_le_nonneg_of_sq_le_sq {a b : α} (hb : 0 ≤ b) (h : (a*a) ≤ b*b) : a ≤ b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact le_of_not_gtₓ fun hab => (Decidable.mul_self_lt_mul_self hb hab).not_le h

theorem mul_self_le_mul_self_iff {a b : α} (h1 : 0 ≤ a) (h2 : 0 ≤ b) : a ≤ b ↔ (a*a) ≤ b*b := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact ⟨Decidable.mul_self_le_mul_self h1, nonneg_le_nonneg_of_sq_le_sq h2⟩

theorem mul_self_lt_mul_self_iff {a b : α} (h1 : 0 ≤ a) (h2 : 0 ≤ b) : a < b ↔ (a*a) < b*b := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact ((@Decidable.strict_mono_on_mul_self α _ _).lt_iff_lt h1 h2).symm

theorem mul_self_inj {a b : α} (h1 : 0 ≤ a) (h2 : 0 ≤ b) : ((a*a) = b*b) ↔ a = b := by
  have := @LinearOrderₓ.decidableLe α _ <;> exact (@Decidable.strict_mono_on_mul_self α _ _).InjOn.eq_iff h1 h2

@[simp]
theorem mul_le_mul_left_of_neg {a b c : α} (h : c < 0) : ((c*a) ≤ c*b) ↔ b ≤ a := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
...
@[simp]
theorem mul_le_mul_right_of_neg {a b c : α} (h : c < 0) : ((a*c) ≤ b*c) ↔ b ≤ a := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
...
theorem mul_self_pos {a : α} (ha : a ≠ 0) : 0 < a*a := by
  rcases lt_trichotomyₓ a 0 with (h | h | h) <;> [exact mul_pos_of_neg_of_neg h h, exact (ha h).elim, exact mul_pos h h]
...
theorem pos_of_mul_neg_left {a b : α} (h : (a*b) < 0) (hb : b ≤ 0) : 0 < a := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact lt_of_not_geₓ fun ha => absurd h (Decidable.mul_nonneg_of_nonpos_of_nonpos ha hb).not_lt

theorem pos_of_mul_neg_right {a b : α} (h : (a*b) < 0) (ha : a ≤ 0) : 0 < b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
...
theorem max_mul_mul_le_max_mul_max (b c : α) (ha : 0 ≤ a) (hd : 0 ≤ d) : max (a*b) (d*c) ≤ max a c*max d b := by
  have := @LinearOrderₓ.decidableLe α _ <;>
    exact
...
theorem mul_lt_top [PartialOrderₓ α] {a b : WithTop α} (ha : a ≠ ⊤) (hb : b ≠ ⊤) : (a*b) < ⊤ := by
...
theorem bot_lt_mul [PartialOrderₓ α] {a b : WithBot α} (ha : ⊥ < a) (hb : ⊥ < b) : ⊥ < a*b := by
