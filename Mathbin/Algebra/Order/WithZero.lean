...
theorem ne_zero_of_lt (h : b < a) : a ≠ 0 :=
  fun h1 => not_lt_zero'$ show b < 0 from h1 ▸ h

theorem pow_pos_iff [NoZeroDivisors α] {n : ℕ} (hn : 0 < n) : 0 < a ^ n ↔ 0 < a :=
  by 
    ...
    if hc : c = 0 then
      ...
      show
        ((Units.mk0 a ha*Units.mk0 b hb⁻¹) ≤ Units.mk0 c hc*Units.mk0 d hd⁻¹) ↔
          (Units.mk0 a ha*Units.mk0 d hd) ≤ Units.mk0 c hc*Units.mk0 b hb from
        mul_inv_le_mul_inv_iff'

@[simp]
theorem Units.zero_lt (u : Units α) : (0 : α) < u :=
...
theorem pow_lt_pow_succ {x : α} {n : ℕ} (hx : 1 < x) : x ^ n < x ^ n.succ :=
  by 
    rw [←one_mulₓ (x ^ n), pow_succₓ]
    exact mul_lt_right₀ _ hx (pow_ne_zero _$ ne_of_gtₓ (lt_transₓ zero_lt_one₀ hx))

-- error in Algebra.Order.WithZero: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem pow_lt_pow₀
{x : α}
{m n : exprℕ()}
(hx : «expr < »(1, x))
(hmn : «expr < »(m, n)) : «expr < »(«expr ^ »(x, m), «expr ^ »(x, n)) :=
by { induction [expr hmn] [] ["with", ident n, ident hmn, ident ih] [],
  exacts ["[", expr pow_lt_pow_succ hx, ",", expr lt_trans ih (pow_lt_pow_succ hx), "]"] }
...
theorem map_neg (x : R) : f (-x) = f x :=
  calc f (-x) = f ((-1)*x) := congr_argₓ _ (neg_one_mul _).symm 
...
theorem map_sub_swap (x y : R) : f (x - y) = f (y - x) :=
  calc f (x - y) = f (-(y - x)) := congr_argₓ _ (neg_sub _ _).symm 
    _ = _ := map_neg _ _
