...
@[simp]
theorem inv_nonneg : 0 ≤ a⁻¹ ↔ 0 ≤ a :=
  by 
    simp only [le_iff_eq_or_lt, inv_pos, zero_eq_inv]

@[simp]
theorem inv_lt_zero : a⁻¹ < 0 ↔ a < 0 :=
  by 
    simp only [←not_leₓ, inv_nonneg]

@[simp]
theorem inv_nonpos : a⁻¹ ≤ 0 ↔ a ≤ 0 :=
  by 
...
theorem div_pos_iff : 0 < a / b ↔ 0 < a ∧ 0 < b ∨ a < 0 ∧ b < 0 :=
  by 
    simp [division_def, mul_pos_iff]

theorem div_neg_iff : a / b < 0 ↔ 0 < a ∧ b < 0 ∨ a < 0 ∧ 0 < b :=
  by 
    simp [division_def, mul_neg_iff]

theorem div_nonneg_iff : 0 ≤ a / b ↔ 0 ≤ a ∧ 0 ≤ b ∨ a ≤ 0 ∧ b ≤ 0 :=
  by 
    simp [division_def, mul_nonneg_iff]

theorem div_nonpos_iff : a / b ≤ 0 ↔ 0 ≤ a ∧ b ≤ 0 ∨ a ≤ 0 ∧ 0 ≤ b :=
  by 
...
theorem le_div_iff (hc : 0 < c) : a ≤ b / c ↔ (a*c) ≤ b :=
  ⟨fun h => div_mul_cancel b (ne_of_ltₓ hc).symm ▸ mul_le_mul_of_nonneg_right h hc.le,
    fun h =>
...
theorem le_div_iff' (hc : 0 < c) : a ≤ b / c ↔ (c*a) ≤ b :=
  by 
...
theorem div_le_iff' (hb : 0 < b) : a / b ≤ c ↔ a ≤ b*c :=
  by 
...
theorem lt_div_iff' (hc : 0 < c) : a < b / c ↔ (c*a) < b :=
  by 
...
theorem div_lt_iff' (hc : 0 < c) : b / c < a ↔ b < c*a :=
  by 
...
theorem inv_mul_le_iff' (h : 0 < b) : (b⁻¹*a) ≤ c ↔ a ≤ c*b :=
  by 
    rw [inv_mul_le_iff h, mul_commₓ]

theorem mul_inv_le_iff (h : 0 < b) : (a*b⁻¹) ≤ c ↔ a ≤ b*c :=
  by 
    rw [mul_commₓ, inv_mul_le_iff h]

theorem mul_inv_le_iff' (h : 0 < b) : (a*b⁻¹) ≤ c ↔ a ≤ c*b :=
  by 
    rw [mul_commₓ, inv_mul_le_iff' h]
...
theorem inv_mul_lt_iff' (h : 0 < b) : (b⁻¹*a) < c ↔ a < c*b :=
  by 
    rw [inv_mul_lt_iff h, mul_commₓ]

theorem mul_inv_lt_iff (h : 0 < b) : (a*b⁻¹) < c ↔ a < b*c :=
  by 
    rw [mul_commₓ, inv_mul_lt_iff h]

theorem mul_inv_lt_iff' (h : 0 < b) : (a*b⁻¹) < c ↔ a < c*b :=
  by 
    rw [mul_commₓ, inv_mul_lt_iff' h]
...
theorem div_le_iff_of_neg (hc : c < 0) : b / c ≤ a ↔ (a*c) ≤ b :=
  ⟨fun h => div_mul_cancel b (ne_of_ltₓ hc) ▸ mul_le_mul_of_nonpos_right h hc.le,
    fun h =>
...
theorem div_lt_iff_of_neg' (hc : c < 0) : b / c < a ↔ (c*a) < b :=
  by 
...
theorem lt_div_iff_of_neg' (hc : c < 0) : a < b / c ↔ b < c*a :=
  by 
...
theorem inv_le_inv_of_le (ha : 0 < a) (h : a ≤ b) : b⁻¹ ≤ a⁻¹ :=
  by 
...
theorem inv_le_inv (ha : 0 < a) (hb : 0 < b) : a⁻¹ ≤ b⁻¹ ↔ b ≤ a :=
  by 
...
theorem inv_le (ha : 0 < a) (hb : 0 < b) : a⁻¹ ≤ b ↔ b⁻¹ ≤ a :=
  by 
...
theorem le_inv (ha : 0 < a) (hb : 0 < b) : a ≤ b⁻¹ ↔ b ≤ a⁻¹ :=
  by 
...
theorem inv_le_inv_of_neg (ha : a < 0) (hb : b < 0) : a⁻¹ ≤ b⁻¹ ↔ b ≤ a :=
  by 
    rw [←one_div, div_le_iff_of_neg ha, ←div_eq_inv_mul, div_le_iff_of_neg hb, one_mulₓ]

theorem inv_le_of_neg (ha : a < 0) (hb : b < 0) : a⁻¹ ≤ b ↔ b⁻¹ ≤ a :=
  by 
    rw [←inv_le_inv_of_neg hb (inv_lt_zero.2 ha), inv_inv₀]

theorem le_inv_of_neg (ha : a < 0) (hb : b < 0) : a ≤ b⁻¹ ↔ b ≤ a⁻¹ :=
  by 
...
theorem div_lt_div_of_lt_left (hb : 0 < b) (h : b < a) (hc : 0 < c) : c / a < c / b :=
...
theorem one_le_div (hb : 0 < b) : 1 ≤ a / b ↔ b ≤ a :=
  by 
    rw [le_div_iff hb, one_mulₓ]

theorem div_le_one (hb : 0 < b) : a / b ≤ 1 ↔ a ≤ b :=
  by 
    rw [div_le_iff hb, one_mulₓ]

theorem one_lt_div (hb : 0 < b) : 1 < a / b ↔ b < a :=
  by 
    rw [lt_div_iff hb, one_mulₓ]

theorem div_lt_one (hb : 0 < b) : a / b < 1 ↔ a < b :=
  by 
    rw [div_lt_iff hb, one_mulₓ]

theorem one_le_div_of_neg (hb : b < 0) : 1 ≤ a / b ↔ a ≤ b :=
  by 
...
theorem one_lt_div_of_neg (hb : b < 0) : 1 < a / b ↔ a < b :=
  by 
    rw [lt_div_iff_of_neg hb, one_mulₓ]

theorem div_lt_one_of_neg (hb : b < 0) : a / b < 1 ↔ b < a :=
  by 
    rw [div_lt_iff_of_neg hb, one_mulₓ]

theorem one_div_le (ha : 0 < a) (hb : 0 < b) : 1 / a ≤ b ↔ 1 / b ≤ a :=
  by 
    simpa using inv_le ha hb

theorem one_div_lt (ha : 0 < a) (hb : 0 < b) : 1 / a < b ↔ 1 / b < a :=
  by 
    simpa using inv_lt ha hb

theorem le_one_div (ha : 0 < a) (hb : 0 < b) : a ≤ 1 / b ↔ b ≤ 1 / a :=
  by 
    simpa using le_inv ha hb

theorem lt_one_div (ha : 0 < a) (hb : 0 < b) : a < 1 / b ↔ b < 1 / a :=
  by 
    simpa using lt_inv ha hb

theorem one_div_le_of_neg (ha : a < 0) (hb : b < 0) : 1 / a ≤ b ↔ 1 / b ≤ a :=
  by 
    simpa using inv_le_of_neg ha hb

theorem one_div_lt_of_neg (ha : a < 0) (hb : b < 0) : 1 / a < b ↔ 1 / b < a :=
  by 
    simpa using inv_lt_of_neg ha hb

theorem le_one_div_of_neg (ha : a < 0) (hb : b < 0) : a ≤ 1 / b ↔ b ≤ 1 / a :=
  by 
    simpa using le_inv_of_neg ha hb

theorem lt_one_div_of_neg (ha : a < 0) (hb : b < 0) : a < 1 / b ↔ b < 1 / a :=
  by 
...
theorem one_div_le_one_div_of_neg_of_le (hb : b < 0) (h : a ≤ b) : 1 / b ≤ 1 / a :=
  by 
    rwa [div_le_iff_of_neg' hb, ←div_eq_mul_one_div, div_le_one_of_neg (h.trans_lt hb)]

theorem one_div_lt_one_div_of_neg_of_lt (hb : b < 0) (h : a < b) : 1 / b < 1 / a :=
  by 
...
/-- For the single implications with fewer assumptions, see `one_div_lt_one_div_of_neg_of_lt` and
  `lt_of_one_div_lt_one_div` -/
theorem one_div_le_one_div_of_neg (ha : a < 0) (hb : b < 0) : 1 / a ≤ 1 / b ↔ b ≤ a :=
  by 
...
theorem add_halves (a : α) : ((a / 2)+a / 2) = a :=
  by 
...
theorem add_self_div_two (a : α) : (a+a) / 2 = a :=
  by 
...
theorem add_sub_div_two_lt (h : a < b) : (a+(b - a) / 2) < b :=
  by 
...
theorem left_lt_add_div_two : a < (a+b) / 2 ↔ a < b :=
  by 
    simp [lt_div_iff, mul_two]

theorem add_div_two_lt_right : (a+b) / 2 < b ↔ a < b :=
  by 
...
theorem mul_sub_mul_div_mul_nonpos_iff (hc : c ≠ 0) (hd : d ≠ 0) : (((a*d) - b*c) / c*d) ≤ 0 ↔ a / c ≤ b / d :=
  by 
...
theorem abs_div (a b : α) : |a / b| = |a| / |b| :=
  (absHom : MonoidWithZeroHom α α).map_div a b
...
theorem one_div_pow_lt_one_div_pow_of_lt (a1 : 1 < a) {m n : ℕ} (mn : m < n) : 1 / a ^ n < 1 / a ^ m :=
  by 
