...
variable [Fintype α] [DecidableEq α] [CommMonoid β]

@[to_additive]
theorem IsCompl.prod_mul_prod {s t : Finset α} (h : IsCompl s t) (f : α → β) :
    ((∏ i in s, f i) * ∏ i in t, f i) = ∏ i, f i :=
  (Finset.prod_union h.Disjoint).symm.trans <| by rw [← Finset.sup_eq_union, h.sup_eq_top] <;> rfl
