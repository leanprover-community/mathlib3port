...
@[simp, to_additive]
theorem prod_sdiff_eq_div [DecidableEq α] (h : s₁ ⊆ s₂) : (∏ x in s₂ \ s₁, f x) = (∏ x in s₂, f x) / ∏ x in s₁, f x :=
  by
...
@[simp, to_additive]
theorem prod_erase_eq_div [DecidableEq α] {a : α} (h : a ∈ s) : (∏ x in s.erase a, f x) = (∏ x in s, f x) / f a := by
