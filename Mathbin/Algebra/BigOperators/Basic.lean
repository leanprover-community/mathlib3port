...
protected def prod [CommMonoidₓ β] (s : Finsetₓ α) (f : α → β) : β :=
  (s.1.map f).Prod

@[simp, to_additive]
theorem prod_mk [CommMonoidₓ β] (s : Multiset α) (hs : s.Nodup) (f : α → β) :
    (⟨s, hs⟩ : Finsetₓ α).Prod f = (s.map f).Prod :=
  rfl

@[simp, to_additive]
theorem prod_val [CommMonoidₓ α] (s : Finsetₓ α) : s.1.Prod = s.Prod id := by rw [Finsetₓ.prod, Multiset.map_id]
...
localized [BigOperators] notation3"∑ "(...)", "r:(scoped f => Finsetₓ.sum Finsetₓ.univ f) => r

-- mathport name: finset.prod_univ
localized [BigOperators] notation3"∏ "(...)", "r:(scoped f => Finsetₓ.prod Finsetₓ.univ f) => r

-- mathport name: finset.sum
localized [BigOperators] notation3"∑ "(...)" in "s", "r:(scoped f => Finsetₓ.sum s f) => r

-- mathport name: finset.prod
localized [BigOperators] notation3"∏ "(...)" in "s", "r:(scoped f => Finsetₓ.prod s f) => r

open BigOperators

namespace Finsetₓ

variable {s s₁ s₂ : Finsetₓ α} {a : α} {f g : α → β}

@[to_additive]
theorem prod_eq_multiset_prod [CommMonoidₓ β] (s : Finsetₓ α) (f : α → β) : (∏ x in s, f x) = (s.1.map f).Prod :=
  rfl

@[to_additive]
theorem prod_eq_fold [CommMonoidₓ β] (s : Finsetₓ α) (f : α → β) : (∏ x in s, f x) = s.fold (· * ·) 1 f :=
  rfl

@[simp]
theorem sum_multiset_singleton (s : Finsetₓ α) : (s.Sum fun x => {x}) = s.val := by
...
theorem map_prod [CommMonoidₓ β] [CommMonoidₓ γ] {G : Type _} [MonoidHomClass G β γ] (g : G) (f : α → β)
    (s : Finsetₓ α) : g (∏ x in s, f x) = ∏ x in s, g (f x) := by
  simp only [Finsetₓ.prod_eq_multiset_prod, map_multiset_prod, Multiset.map_map]

section Deprecated

/-- Deprecated: use `_root_.map_prod` instead. -/
@[to_additive "Deprecated: use `_root_.map_sum` instead."]
protected theorem MonoidHom.map_prod [CommMonoidₓ β] [CommMonoidₓ γ] (g : β →* γ) (f : α → β) (s : Finsetₓ α) :
    g (∏ x in s, f x) = ∏ x in s, g (f x) :=
  map_prod g f s

/-- Deprecated: use `_root_.map_prod` instead. -/
@[to_additive "Deprecated: use `_root_.map_sum` instead."]
protected theorem MulEquiv.map_prod [CommMonoidₓ β] [CommMonoidₓ γ] (g : β ≃* γ) (f : α → β) (s : Finsetₓ α) :
    g (∏ x in s, f x) = ∏ x in s, g (f x) :=
  map_prod g f s
...
/-- Deprecated: use `_root_.map_prod` instead. -/
protected theorem RingHom.map_prod [CommSemiringₓ β] [CommSemiringₓ γ] (g : β →+* γ) (f : α → β) (s : Finsetₓ α) :
    g (∏ x in s, f x) = ∏ x in s, g (f x) :=
  map_prod g f s

/-- Deprecated: use `_root_.map_sum` instead. -/
protected theorem RingHom.map_sum [NonAssocSemiringₓ β] [NonAssocSemiringₓ γ] (g : β →+* γ) (f : α → β)
...
@[to_additive]
theorem MonoidHom.coe_finset_prod [MulOneClassₓ β] [CommMonoidₓ γ] (f : α → β →* γ) (s : Finsetₓ α) :
...
@[simp, to_additive]
theorem MonoidHom.finset_prod_apply [MulOneClassₓ β] [CommMonoidₓ γ] (f : α → β →* γ) (s : Finsetₓ α) (b : β) :
...
@[to_additive]
theorem prod_pair [DecidableEq α] {a b : α} (h : a ≠ b) : (∏ x in ({a, b} : Finsetₓ α), f x) = f a * f b := by
  rw [prod_insert (not_mem_singleton.2 h), prod_singleton]

@[simp, to_additive]
theorem prod_const_one : (∏ x in s, (1 : β)) = 1 := by
  simp only [Finsetₓ.prod, Multiset.map_const, Multiset.prod_repeat, one_pow]

@[simp, to_additive]
theorem prod_image [DecidableEq α] {s : Finsetₓ γ} {g : γ → α} :
    (∀ x ∈ s, ∀ y ∈ s, g x = g y → x = y) → (∏ x in s.Image g, f x) = ∏ x in s, f (g x) :=
  fold_image

@[simp, to_additive]
theorem prod_map (s : Finsetₓ α) (e : α ↪ γ) (f : γ → β) : (∏ x in s.map e, f x) = ∏ x in s, f (e x) := by
  rw [Finsetₓ.prod, Finsetₓ.map_val, Multiset.map_map] <;> rfl

@[congr, to_additive]
theorem prod_congr (h : s₁ = s₂) : (∀ x ∈ s₂, f x = g x) → s₁.Prod f = s₂.Prod g := by rw [h] <;> exact fold_congr

attribute [congr] Finsetₓ.sum_congr
...
theorem prod_union [DecidableEq α] (h : Disjoint s₁ s₂) : (∏ x in s₁ ∪ s₂, f x) = (∏ x in s₁, f x) * ∏ x in s₂, f x :=
  by rw [← prod_union_inter, disjoint_iff_inter_eq_empty.mp h] <;> exact (mul_oneₓ _).symm

@[to_additive]
theorem prod_filter_mul_prod_filter_not (s : Finsetₓ α) (p : α → Prop) [DecidablePred p] [DecidablePred fun x => ¬p x]
...
@[simp, to_additive]
theorem prod_to_list (s : Finsetₓ α) (f : α → β) : (s.toList.map f).Prod = s.Prod f := by
  rw [Finsetₓ.prod, ← Multiset.coe_prod, ← Multiset.coe_map, Finsetₓ.coe_to_list]

end ToList

@[to_additive]
theorem _root_.equiv.perm.prod_comp (σ : Equivₓ.Perm α) (s : Finsetₓ α) (f : α → β) (hs : { a | σ a ≠ a } ⊆ s) :
    (∏ x in s, f (σ x)) = ∏ x in s, f x := by
  convert (prod_mapₓ _ σ.to_embedding _).symm
  exact (map_perm hs).symm

@[to_additive]
theorem _root_.equiv.perm.prod_comp' (σ : Equivₓ.Perm α) (s : Finsetₓ α) (f : α → α → β) (hs : { a | σ a ≠ a } ⊆ s) :
...
variable [Fintypeₓ α] [DecidableEq α] [CommMonoidₓ β]

@[to_additive]
theorem IsCompl.prod_mul_prod {s t : Finsetₓ α} (h : IsCompl s t) (f : α → β) :
    ((∏ i in s, f i) * ∏ i in t, f i) = ∏ i, f i :=
  (Finsetₓ.prod_union h.Disjoint).symm.trans <| by rw [← Finsetₓ.sup_eq_union, h.sup_eq_top] <;> rfl
...
theorem prod_mul_prod_compl [Fintypeₓ α] [DecidableEq α] (s : Finsetₓ α) (f : α → β) :
    ((∏ i in s, f i) * ∏ i in sᶜ, f i) = ∏ i, f i :=
  IsCompl.prod_mul_prod is_compl_compl f

@[to_additive]
theorem prod_compl_mul_prod [Fintypeₓ α] [DecidableEq α] (s : Finsetₓ α) (f : α → β) :
...
@[simp, to_additive]
theorem prod_sum_elim [DecidableEq (Sum α γ)] (s : Finsetₓ α) (t : Finsetₓ γ) (f : α → β) (g : γ → β) :
  ...
  · simp only [disjoint_left, Finsetₓ.mem_map, Finsetₓ.mem_map]
    rintro _ ⟨i, hi, rfl⟩ ⟨j, hj, H⟩
    cases H
    

@[simp, to_additive]
theorem prod_on_sum [Fintypeₓ α] [Fintypeₓ γ] (f : Sum α γ → β) :
...
@[to_additive]
theorem prod_bUnion [DecidableEq α] {s : Finsetₓ γ} {t : γ → Finsetₓ α} (hs : Set.PairwiseDisjoint (↑s) t) :
    (∏ x in s.bUnion t, f x) = ∏ x in s, ∏ i in t x, f i := by
  haveI := Classical.decEq γ
  induction' s using Finsetₓ.induction_on with x s hxs ih hd
  · simp_rw [bUnion_empty, prod_empty]
    
  · simp_rw [coe_insert, Set.pairwise_disjoint_insert, mem_coe] at hs
    have : Disjoint (t x) (Finsetₓ.bUnion s t) :=
...
theorem prod_sigma {σ : α → Type _} (s : Finsetₓ α) (t : ∀ a, Finsetₓ (σ a)) (f : Sigma σ → β) :
...
@[to_additive]
theorem prod_sigma' {σ : α → Type _} (s : Finsetₓ α) (t : ∀ a, Finsetₓ (σ a)) (f : ∀ a, σ a → β) :
...
theorem prod_bij {s : Finsetₓ α} {t : Finsetₓ γ} {f : α → β} {g : γ → β} (i : ∀ a ∈ s, γ) (hi : ∀ a ha, i a ha ∈ t)
...
@[to_additive]
theorem prod_finset_product (r : Finsetₓ (γ × α)) (s : Finsetₓ γ) (t : γ → Finsetₓ α)
...
@[to_additive]
theorem prod_finset_product_right (r : Finsetₓ (α × γ)) (s : Finsetₓ γ) (t : γ → Finsetₓ α)
...
@[to_additive]
theorem prod_fiberwise_of_maps_to [DecidableEq γ] {s : Finsetₓ α} {t : Finsetₓ γ} {g : α → γ} (h : ∀ x ∈ s, g x ∈ t)
...
@[to_additive]
theorem prod_image' [DecidableEq α] {s : Finsetₓ γ} {g : γ → α} (h : γ → β)
...
theorem prod_mul_distrib : (∏ x in s, f x * g x) = (∏ x in s, f x) * ∏ x in s, g x :=
  Eq.trans (by rw [one_mulₓ] <;> rfl) fold_op_distrib

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
@[to_additive]
theorem prod_product {s : Finsetₓ γ} {t : Finsetₓ α} {f : γ × α → β} :
...
-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
@[to_additive]
theorem prod_product_right {s : Finsetₓ γ} {t : Finsetₓ α} {f : γ × α → β} :
...
theorem prod_comm' {s : Finsetₓ γ} {t : γ → Finsetₓ α} {t' : Finsetₓ α} {s' : α → Finsetₓ γ}
...
@[to_additive]
theorem prod_comm {s : Finsetₓ γ} {t : Finsetₓ α} {f : γ → α → β} :
    (∏ x in s, ∏ y in t, f x y) = ∏ y in t, ∏ x in s, f x y :=
  prod_comm' fun _ _ => Iff.rfl

@[to_additive]
theorem prod_hom_rel [CommMonoidₓ γ] {r : β → γ → Prop} {f : α → β} {g : α → γ} {s : Finsetₓ α} (h₁ : r 1 1)
    (h₂ : ∀ a b c, r b c → r (f a * b) (g a * c)) : r (∏ x in s, f x) (∏ x in s, g x) := by
  delta Finsetₓ.prod
  apply Multiset.prod_hom_rel <;> assumption

@[to_additive]
theorem prod_eq_one {f : α → β} {s : Finsetₓ α} (h : ∀ x ∈ s, f x = 1) : (∏ x in s, f x) = 1 :=
  calc
    (∏ x in s, f x) = ∏ x in s, 1 := Finsetₓ.prod_congr rfl h
    _ = 1 := Finsetₓ.prod_const_one
...
@[to_additive]
theorem prod_eq_single_of_mem {s : Finsetₓ α} {f : α → β} (a : α) (h : a ∈ s) (h₀ : ∀ b ∈ s, b ≠ a → f b = 1) :
    (∏ x in s, f x) = f a := by
  haveI := Classical.decEq α
  calc
...
@[to_additive]
theorem prod_eq_single {s : Finsetₓ α} {f : α → β} (a : α) (h₀ : ∀ b ∈ s, b ≠ a → f b = 1) (h₁ : a ∉ s → f a = 1) :
    (∏ x in s, f x) = f a :=
  haveI := Classical.decEq α
...
@[to_additive]
theorem prod_eq_mul_of_mem {s : Finsetₓ α} {f : α → β} (a b : α) (ha : a ∈ s) (hb : b ∈ s) (hn : a ≠ b)
    (h₀ : ∀ c ∈ s, c ≠ a ∧ c ≠ b → f c = 1) : (∏ x in s, f x) = f a * f b := by
  haveI := Classical.decEq α <;> let s' := ({a, b} : Finsetₓ α)
  have hu : s' ⊆ s := by
  ...
  exact Finsetₓ.prod_pair hn

@[to_additive]
theorem prod_eq_mul {s : Finsetₓ α} {f : α → β} (a b : α) (hn : a ≠ b) (h₀ : ∀ c ∈ s, c ≠ a ∧ c ≠ b → f c = 1)
...
theorem prod_subtype_of_mem (f : α → β) {p : α → Prop} [DecidablePred p] (h : ∀ x ∈ s, p x) :
    (∏ x in s.Subtype p, f x) = ∏ x in s, f x := by simp_rw [prod_subtype_eq_prod_filter, filter_true_of_mem h]
...
theorem prod_subtype_map_embedding {p : α → Prop} {s : Finsetₓ { x // p x }} {f : { x // p x } → β} {g : α → β}
    (h : ∀ x : { x // p x }, x ∈ s → g x = f x) : (∏ x in s.map (Function.Embedding.subtype _), g x) = ∏ x in s, f x :=
  by
  rw [Finsetₓ.prod_map]
  exact Finsetₓ.prod_congr rfl h
...
@[to_additive]
theorem prod_finset_coe (f : α → β) (s : Finsetₓ α) : (∏ i : (s : Set α), f i) = ∏ i in s, f i :=
  prod_coe_sort s f

variable {f s}

@[to_additive]
theorem prod_subtype {p : α → Prop} {F : Fintypeₓ (Subtype p)} (s : Finsetₓ α) (h : ∀ x, x ∈ s ↔ p x) (f : α → β) :
...
theorem prod_congr_set {α : Type _} [CommMonoidₓ α] {β : Type _} [Fintypeₓ β] (s : Set β) [DecidablePred (· ∈ s)]
    (f : β → α) (g : s → α) (w : ∀ (x : β) (h : x ∈ s), f x = g ⟨x, h⟩) (w' : ∀ x : β, x ∉ s → f x = 1) :
    Finsetₓ.univ.Prod f = Finsetₓ.univ.Prod g := by
  rw [← @Finsetₓ.prod_subset _ _ s.to_finset Finsetₓ.univ f _ (by simp)]
...
@[to_additive]
theorem prod_apply_dite {s : Finsetₓ α} {p : α → Prop} {hp : DecidablePred p} [DecidablePred fun x => ¬p x]
...
@[to_additive]
theorem prod_apply_ite {s : Finsetₓ α} {p : α → Prop} {hp : DecidablePred p} (f g : α → γ) (h : γ → β) :
...
@[to_additive]
theorem prod_dite {s : Finsetₓ α} {p : α → Prop} {hp : DecidablePred p} (f : ∀ x : α, p x → β) (g : ∀ x : α, ¬p x → β) :
...
@[to_additive]
theorem prod_ite {s : Finsetₓ α} {p : α → Prop} {hp : DecidablePred p} (f g : α → β) :
...
@[to_additive]
theorem prod_extend_by_one [DecidableEq α] (s : Finsetₓ α) (f : α → β) :
    (∏ i in s, if i ∈ s then f i else 1) = ∏ i in s, f i :=
  (prod_congr rfl) fun i hi => if_pos hi

@[simp, to_additive]
theorem prod_dite_eq [DecidableEq α] (s : Finsetₓ α) (a : α) (b : ∀ x : α, a = x → β) :
    (∏ x in s, if h : a = x then b x h else 1) = ite (a ∈ s) (b a rfl) 1 := by
  split_ifs with h
  · rw [Finsetₓ.prod_eq_single a, dif_pos rfl]
    · intros
      rw [dif_neg]
      cc
...
@[simp, to_additive]
theorem prod_dite_eq' [DecidableEq α] (s : Finsetₓ α) (a : α) (b : ∀ x : α, x = a → β) :
...
@[simp, to_additive]
theorem prod_ite_eq [DecidableEq α] (s : Finsetₓ α) (a : α) (b : α → β) :
...
@[to_additive]
theorem prod_ite_index (p : Prop) [Decidable p] (s t : Finsetₓ α) (f : α → β) :
    (∏ x in if p then s else t, f x) = if p then ∏ x in s, f x else ∏ x in t, f x :=
  apply_iteₓ (fun s => ∏ x in s, f x) _ _ _

@[simp, to_additive]
theorem prod_ite_irrel (p : Prop) [Decidable p] (s : Finsetₓ α) (f g : α → β) :
    (∏ x in s, if p then f x else g x) = if p then ∏ x in s, f x else ∏ x in s, g x := by split_ifs with h <;> rfl

@[simp, to_additive]
theorem prod_dite_irrel (p : Prop) [Decidable p] (s : Finsetₓ α) (f : p → α → β) (g : ¬p → α → β) :
    (∏ x in s, if h : p then f h x else g h x) = if h : p then ∏ x in s, f h x else ∏ x in s, g h x := by
  split_ifs with h <;> rfl

@[simp]
theorem sum_pi_single' {ι M : Type _} [DecidableEq ι] [AddCommMonoidₓ M] (i : ι) (x : M) (s : Finsetₓ ι) :
...
theorem sum_pi_single {ι : Type _} {M : ι → Type _} [DecidableEq ι] [∀ i, AddCommMonoidₓ (M i)] (i : ι) (f : ∀ i, M i)
    (s : Finsetₓ ι) : (∑ j in s, Pi.single j (f j) i) = if i ∈ s then f i else 0 :=
  sum_dite_eq _ _ _

@[to_additive]
theorem prod_bij_ne_one {s : Finsetₓ α} {t : Finsetₓ γ} {f : α → β} {g : γ → β} (i : ∀ a ∈ s, f a ≠ 1 → γ)
...
theorem prod_range_succ' (f : ℕ → β) : ∀ n : ℕ, (∏ k in range (n + 1), f k) = (∏ k in range n, f (k + 1)) * f 0
  ...
  | n + 1 => by rw [prod_range_succ _ n, mul_right_commₓ, ← prod_range_succ', prod_range_succ]
...
theorem prod_range_add_div_prod_range {α : Type _} [CommGroupₓ α] (f : ℕ → α) (n m : ℕ) :
    ((∏ k in range (n + m), f k) / ∏ k in range n, f k) = ∏ k in Finsetₓ.range m, f (n + k) :=
...
@[to_additive]
theorem prod_list_count_of_subset [DecidableEq α] [CommMonoidₓ α] (m : List α) (s : Finsetₓ α) (hs : m.toFinset ⊆ s) :
...
theorem sum_filter_count_eq_countp [DecidableEq α] (p : α → Prop) [DecidablePred p] (l : List α) :
  ...
  simp [Finsetₓ.sum, sum_map_count_dedup_filter_eq_countp p l]
...
@[to_additive]
theorem prod_multiset_count_of_subset [DecidableEq α] [CommMonoidₓ α] (m : Multiset α) (s : Finsetₓ α)
...
theorem prod_mem_multiset [DecidableEq α] (m : Multiset α) (f : { x // x ∈ m } → β) (g : α → β) (hfg : ∀ x, f x = g x) :
  ...
  prod_bij (fun x _ => x.1) (fun x _ => Multiset.mem_to_finset.mpr x.2) (fun _ _ => hfg _)
    (fun _ _ _ _ h => by
      ext
      assumption)
    fun y hy => ⟨⟨y, Multiset.mem_to_finset.mp hy⟩, Finsetₓ.mem_univ _, rfl⟩
...
theorem prod_range_induction (f s : ℕ → β) (h0 : s 0 = 1) (h : ∀ n, s (n + 1) = s n * f n) (n : ℕ) :
    (∏ k in Finsetₓ.range n, f k) = s n := by
  induction' n with k hk
  · simp only [h0, Finsetₓ.prod_range_zero]
    
  · simp only [hk, Finsetₓ.prod_range_succ, h, mul_comm]
...
theorem eq_prod_range_div' {M : Type _} [CommGroupₓ M] (f : ℕ → M) (n : ℕ) :
  ...
  conv_lhs => rw [Finsetₓ.eq_prod_range_div f]
  simp [Finsetₓ.prod_range_succ', mul_comm]
...
theorem sum_range_tsub [CanonicallyOrderedAddMonoid α] [Sub α] [HasOrderedSub α]
    [ContravariantClass α α (· + ·) (· ≤ ·)] {f : ℕ → α} (h : Monotoneₓ f) (n : ℕ) :
...
@[to_additive]
theorem prod_pow (s : Finsetₓ α) (n : ℕ) (f : α → β) : (∏ x in s, f x ^ n) = (∏ x in s, f x) ^ n :=
...
@[to_additive]
theorem prod_involution {s : Finsetₓ α} {f : α → β} :
  ...
  haveI := Classical.decEq α <;>
    haveI := Classical.decEq β <;>
      exact
        Finsetₓ.strongInductionOn s fun s ih g h g_ne g_mem g_inv =>
...
@[to_additive]
theorem prod_piecewise [DecidableEq α] (s t : Finsetₓ α) (f g : α → β) :
    (∏ x in s, (t.piecewise f g) x) = (∏ x in s ∩ t, f x) * ∏ x in s \ t, g x := by
  rw [piecewise, prod_ite, filter_mem_eq_inter, ← sdiff_eq_filter]

@[to_additive]
theorem prod_inter_mul_prod_diff [DecidableEq α] (s t : Finsetₓ α) (f : α → β) :
    ((∏ x in s ∩ t, f x) * ∏ x in s \ t, f x) = ∏ x in s, f x := by
  convert (s.prod_piecewise t f f).symm
  simp [Finsetₓ.piecewise]

@[to_additive]
theorem prod_eq_mul_prod_diff_singleton [DecidableEq α] {s : Finsetₓ α} {i : α} (h : i ∈ s) (f : α → β) :
    (∏ x in s, f x) = f i * ∏ x in s \ {i}, f x := by
  convert (s.prod_inter_mul_prod_diff {i} f).symm
  simp [h]

@[to_additive]
theorem prod_eq_prod_diff_singleton_mul [DecidableEq α] {s : Finsetₓ α} {i : α} (h : i ∈ s) (f : α → β) :
    (∏ x in s, f x) = (∏ x in s \ {i}, f x) * f i := by rw [prod_eq_mul_prod_diff_singleton h, mul_comm]

@[to_additive]
theorem _root_.fintype.prod_eq_mul_prod_compl [DecidableEq α] [Fintypeₓ α] (a : α) (f : α → β) :
    (∏ i, f i) = f a * ∏ i in {a}ᶜ, f i :=
  prod_eq_mul_prod_diff_singleton (mem_univ a) f

@[to_additive]
theorem _root_.fintype.prod_eq_prod_compl_mul [DecidableEq α] [Fintypeₓ α] (a : α) (f : α → β) :
    (∏ i, f i) = (∏ i in {a}ᶜ, f i) * f a :=
  prod_eq_prod_diff_singleton_mul (mem_univ a) f

theorem dvd_prod_of_mem (f : α → β) {a : α} {s : Finsetₓ α} (ha : a ∈ s) : f a ∣ ∏ i in s, f i := by
  classical
  rw [Finsetₓ.prod_eq_mul_prod_diff_singleton ha]
...
theorem prod_partition (R : Setoidₓ α) [DecidableRel R.R] :
  ...
  refine' (Finsetₓ.prod_image' f fun x hx => _).symm
  rfl
...
theorem prod_cancels_of_partition_cancels (R : Setoidₓ α) [DecidableRel R.R]
  ...
  rw [prod_partition R, ← Finsetₓ.prod_eq_one]
...
@[to_additive]
theorem prod_update_of_not_mem [DecidableEq α] {s : Finsetₓ α} {i : α} (h : i ∉ s) (f : α → β) (b : β) :
...
@[to_additive]
theorem prod_update_of_mem [DecidableEq α] {s : Finsetₓ α} {i : α} (h : i ∈ s) (f : α → β) (b : β) :
...
theorem eq_of_card_le_one_of_prod_eq {s : Finsetₓ α} (hc : s.card ≤ 1) {f : α → β} {b : β} (h : (∏ x in s, f x) = b) :
    ∀ x ∈ s, f x = b := by
  intro x hx
...
theorem mul_prod_erase [DecidableEq α] (s : Finsetₓ α) (f : α → β) {a : α} (h : a ∈ s) :
    (f a * ∏ x in s.erase a, f x) = ∏ x in s, f x := by rw [← prod_insert (not_mem_erase a s), insert_erase h]

/-- A variant of `finset.mul_prod_erase` with the multiplication swapped. -/
@[to_additive "A variant of `finset.add_sum_erase` with the addition swapped."]
theorem prod_erase_mul [DecidableEq α] (s : Finsetₓ α) (f : α → β) {a : α} (h : a ∈ s) :
...
theorem prod_erase [DecidableEq α] (s : Finsetₓ α) {f : α → β} {a : α} (h : f a = 1) :
...
theorem sum_erase_lt_of_pos {γ : Type _} [DecidableEq α] [OrderedAddCommMonoid γ] [CovariantClass γ γ (· + ·) (· < ·)]
    {s : Finsetₓ α} {d : α} (hd : d ∈ s) {f : α → γ} (hdf : 0 < f d) :
    (∑ m : α in s.erase d, f m) < ∑ m : α in s, f m := by
  nth_rw_rhs 0 [← Finsetₓ.insert_erase hd]
  rw [Finsetₓ.sum_insert (Finsetₓ.not_mem_erase d s)]
  exact lt_add_of_pos_left _ hdf

/-- If a product is 1 and the function is 1 except possibly at one
point, it is 1 everywhere on the `finset`. -/
@[to_additive "If a sum is 0 and the function is 0 except possibly at one\npoint, it is 0 everywhere on the `finset`."]
theorem eq_one_of_prod_eq_one {s : Finsetₓ α} {f : α → β} {a : α} (hp : (∏ x in s, f x) = 1)
...
theorem prod_pow_boole [DecidableEq α] (s : Finsetₓ α) (f : α → β) (a : α) :
    (∏ x in s, f x ^ ite (a = x) 1 0) = ite (a ∈ s) (f a) 1 := by simp

theorem prod_dvd_prod_of_dvd {S : Finsetₓ α} (g1 g2 : α → β) (h : ∀ a ∈ S, g1 a ∣ g2 a) : S.Prod g1 ∣ S.Prod g2 := by
  classical
  apply Finsetₓ.induction_on' S
  · simp
    
  intro a T haS _ haT IH
  repeat' rw [Finsetₓ.prod_insert haT]
  exact mul_dvd_mul (h a haS) IH

theorem prod_dvd_prod_of_subset {ι M : Type _} [CommMonoidₓ M] (s t : Finsetₓ ι) (f : ι → M) (h : s ⊆ t) :
...
theorem prod_add_prod_eq [CommSemiringₓ β] {s : Finsetₓ α} {i : α} {f g h : α → β} (hi : i ∈ s) (h1 : g i + h i = f i)
...
theorem card_eq_sum_ones (s : Finsetₓ α) : s.card = ∑ _ in s, 1 := by simp
...
@[simp]
theorem sum_boole {s : Finsetₓ α} {p : α → Prop} [NonAssocSemiringₓ β] {hp : DecidablePred p} :
    (∑ x in s, if p x then (1 : β) else (0 : β)) = (s.filter p).card := by simp [sum_ite]

theorem _root_.commute.sum_right [NonUnitalNonAssocSemiringₓ β] (s : Finsetₓ α) (f : α → β) (b : β)
    (h : ∀ i ∈ s, Commute b (f i)) : Commute b (∑ i in s, f i) :=
  (Commute.multiset_sum_right _ _) fun b hb => by
    obtain ⟨i, hi, rfl⟩ := multiset.mem_map.mp hb
    exact h _ hi

theorem _root_.commute.sum_left [NonUnitalNonAssocSemiringₓ β] (s : Finsetₓ α) (f : α → β) (b : β)
...
@[simp]
theorem op_sum [AddCommMonoidₓ β] {s : Finsetₓ α} (f : α → β) : op (∑ x in s, f x) = ∑ x in s, op (f x) :=
  (opAddEquiv : β ≃+ βᵐᵒᵖ).map_sum _ _

@[simp]
theorem unop_sum [AddCommMonoidₓ β] {s : Finsetₓ α} (f : α → βᵐᵒᵖ) : unop (∑ x in s, f x) = ∑ x in s, unop (f x) :=
...
@[to_additive]
theorem prod_zpow (f : α → β) (s : Finsetₓ α) (n : ℤ) : (∏ a in s, f a ^ n) = (∏ a in s, f a) ^ n :=
...
theorem prod_sdiff_div_prod_sdiff : ((∏ x in s₂ \ s₁, f x) / ∏ x in s₁ \ s₂, f x) = (∏ x in s₂, f x) / ∏ x in s₁, f x :=
  by simp [← Finsetₓ.prod_sdiff (@inf_le_left _ _ s₁ s₂), ← Finsetₓ.prod_sdiff (@inf_le_right _ _ s₁ s₂)]
...
@[simp]
theorem card_sigma {σ : α → Type _} (s : Finsetₓ α) (t : ∀ a, Finsetₓ (σ a)) :
    card (s.Sigma t) = ∑ a in s, card (t a) :=
  Multiset.card_sigma _ _

theorem card_bUnion [DecidableEq β] {s : Finsetₓ α} {t : α → Finsetₓ β}
    (h : ∀ x ∈ s, ∀ y ∈ s, x ≠ y → Disjoint (t x) (t y)) : (s.bUnion t).card = ∑ u in s, card (t u) :=
  calc
    (s.bUnion t).card = ∑ i in s.bUnion t, 1 := by simp
    _ = ∑ a in s, ∑ i in t a, 1 := Finsetₓ.sum_bUnion h
    _ = ∑ u in s, card (t u) := by simp
    

theorem card_bUnion_le [DecidableEq β] {s : Finsetₓ α} {t : α → Finsetₓ β} : (s.bUnion t).card ≤ ∑ a in s, (t a).card :=
  haveI := Classical.decEq α
  Finsetₓ.induction_on s (by simp) fun a s has ih =>
    calc
      ((insert a s).bUnion t).card ≤ (t a).card + (s.bUnion t).card := by
        rw [bUnion_insert] <;> exact Finsetₓ.card_union_le _ _
      _ ≤ ∑ a in insert a s, card (t a) := by rw [sum_insert has] <;> exact add_le_add_left ih _
      

theorem card_eq_sum_card_fiberwise [DecidableEq β] {f : α → β} {s : Finsetₓ α} {t : Finsetₓ β} (H : ∀ x ∈ s, f x ∈ t) :
    s.card = ∑ a in t, (s.filter fun x => f x = a).card := by simp only [card_eq_sum_ones, sum_fiberwise_of_maps_to H]

theorem card_eq_sum_card_image [DecidableEq β] (f : α → β) (s : Finsetₓ α) :
    s.card = ∑ a in s.Image f, (s.filter fun x => f x = a).card :=
  card_eq_sum_card_fiberwise fun _ => mem_image_of_mem _

theorem mem_sum {f : α → Multiset β} (s : Finsetₓ α) (b : β) : (b ∈ ∑ x in s, f x) ↔ ∃ a ∈ s, b ∈ f a := by
  classical
...
theorem prod_boole {s : Finsetₓ α} {p : α → Prop} [DecidablePred p] :
...
@[to_additive]
theorem prod_unique_nonempty {α β : Type _} [CommMonoidₓ β] [Unique α] (s : Finsetₓ α) (f : α → β) (h : s.Nonempty) :
    (∏ x in s, f x) = f default := by rw [h.eq_singleton_default, Finsetₓ.prod_singleton]
...
theorem prod_bijective {α β M : Type _} [Fintypeₓ α] [Fintypeₓ β] [CommMonoidₓ M] (e : α → β)
    (he : Function.Bijective e) (f : α → M) (g : β → M) (h : ∀ x, f x = g (e x)) : (∏ x : α, f x) = ∏ x : β, g x :=
...
theorem prod_equiv {α β M : Type _} [Fintypeₓ α] [Fintypeₓ β] [CommMonoidₓ M] (e : α ≃ β) (f : α → M) (g : β → M)
...
theorem prod_empty {α β : Type _} [CommMonoidₓ β] [IsEmpty α] (f : α → β) : (∏ x : α, f x) = 1 := by
  rw [eq_empty_of_is_empty (univ : Finsetₓ α), Finsetₓ.prod_empty]

@[to_additive]
theorem prod_subsingleton {α β : Type _} [CommMonoidₓ β] [Subsingleton α] [Fintypeₓ α] (f : α → β) (a : α) :
    (∏ x : α, f x) = f a := by
  haveI : Unique α := uniqueOfSubsingleton a
  convert prod_unique f

@[to_additive]
theorem prod_subtype_mul_prod_subtype {α β : Type _} [Fintypeₓ α] [CommMonoidₓ β] (p : α → Prop) (f : α → β)
    [DecidablePred p] : ((∏ i : { x // p x }, f i) * ∏ i : { x // ¬p x }, f i) = ∏ i, f i := by
  classical
  let s := { x | p x }.toFinset
  rw [← Finsetₓ.prod_subtype s, ← Finsetₓ.prod_subtype (sᶜ)]
  · exact Finsetₓ.prod_mul_prod_compl _ _
...
theorem prod_to_finset {M : Type _} [DecidableEq α] [CommMonoidₓ M] (f : α → M) :
  ...
  | a::l, hl => by
    ...
    simp [Finsetₓ.prod_insert (mt list.mem_to_finset.mp not_mem), prod_to_finset hl]
...
theorem disjoint_finset_sum_left {β : Type _} {i : Finsetₓ β} {f : β → Multiset α} {a : Multiset α} :
    Multiset.Disjoint (i.Sum f) a ↔ ∀ b ∈ i, Multiset.Disjoint (f b) a := by
  convert (@disjoint_sum_left _ a) (map f i.val)
  simp [Finsetₓ.mem_def, And.congr_left_iffₓ, iff_selfₓ]

theorem disjoint_finset_sum_right {β : Type _} {i : Finsetₓ β} {f : β → Multiset α} {a : Multiset α} :
...
theorem finset_sum_eq_sup_iff_disjoint {β : Type _} {i : Finsetₓ β} {f : β → Multiset α} :
    i.Sum f = i.sup f ↔ ∀ (x y) (_ : x ∈ i) (_ : y ∈ i), x ≠ y → Multiset.Disjoint (f x) (f y) := by
  induction' i using Finsetₓ.cons_induction_on with z i hz hr
  · simp only [Finsetₓ.not_mem_empty, IsEmpty.forall_iff, implies_true_iff, Finsetₓ.sum_empty, Finsetₓ.sup_empty,
      bot_eq_zero, eq_self_iff_true]
    
  · simp_rw [Finsetₓ.sum_cons hz, Finsetₓ.sup_cons, Finsetₓ.mem_cons, Multiset.sup_eq_union, forall_eq_or_imp, Ne.def,
    ...
    exact add_eq_union_left_of_le (Finsetₓ.sup_le fun x hx => le_sum_of_mem (mem_map_of_mem f hx))
    

theorem sup_powerset_len {α : Type _} [DecidableEq α] (x : Multiset α) :
    (Finsetₓ.sup (Finsetₓ.range (x.card + 1)) fun k => x.powersetLen k) = x.Powerset := by
  convert bind_powerset_len x
  rw [Multiset.bind, Multiset.join, ← Finsetₓ.range_coe, ← Finsetₓ.sum_eq_multiset_sum]
  ...
  Multiset.induction_on s rfl fun a s ih =>
    calc
      (∑ x in toFinset (a ::ₘ s), count x (a ::ₘ s)) =
        ...
        (Finsetₓ.sum_congr rfl) fun _ _ => by
      ...
      _ = card (a ::ₘ s) := by
        ...
        · have : (∑ x in s.to_finset, ite (x = a) 1 0) = ∑ x in {a}, ite (x = a) 1 0 := by
            rw [Finsetₓ.sum_ite_eq', if_pos h, Finsetₓ.sum_singleton, if_pos rfl]
          rw [to_finset_cons, Finsetₓ.insert_eq_of_mem h, Finsetₓ.sum_add_distrib, ih, this, Finsetₓ.sum_singleton,
            if_pos rfl, add_commₓ, card_cons]
          
        · have ha : a ∉ s := by rwa [mem_to_finset] at h
          have : (∑ x in to_finset s, ite (x = a) 1 0) = ∑ x in to_finset s, 0 :=
            Finsetₓ.sum_congr rfl fun x hx => if_neg <| by rintro rfl <;> cc
          rw [to_finset_cons, Finsetₓ.sum_insert h, if_pos rfl, Finsetₓ.sum_add_distrib, this, Finsetₓ.sum_const_zero,
            ih, count_eq_zero_of_not_mem ha, zero_addₓ, add_commₓ, card_cons]
          
      

theorem count_sum' {s : Finsetₓ β} {a : α} {f : β → Multiset α} : count a (∑ x in s, f x) = ∑ x in s, count a (f x) :=
  by
  dsimp only [Finsetₓ.sum]
  rw [count_sum]
...
theorem to_finset_sum_count_nsmul_eq (s : Multiset α) : (∑ a in s.toFinset, s.count a • {a}) = s := by
  ...
  intro b
  rw [count_sum']
  have h : count b s = count b (count b s • {b}) := by rw [count_nsmul, count_singleton_self, mul_oneₓ]
  rw [h]
  clear h
  apply Finsetₓ.sum_eq_single b
  · intro c h hcb
    rw [count_nsmul]
...
theorem exists_smul_of_dvd_count (s : Multiset α) {k : ℕ} (h : ∀ a : α, a ∈ s → k ∣ Multiset.count a s) :
  ...
  rw [← Finsetₓ.sum_nsmul, h₂, to_finset_sum_count_nsmul_eq]

theorem to_finset_prod_dvd_prod [CommMonoidₓ α] (S : Multiset α) : S.toFinset.Prod id ∣ S.Prod := by
  rw [Finsetₓ.prod_eq_multiset_prod]
  refine' Multiset.prod_dvd_prod_of_le _
  simp [Multiset.dedup_le S]

@[to_additive]
theorem prod_sum {α : Type _} {ι : Type _} [CommMonoidₓ α] (f : ι → Multiset α) (s : Finsetₓ ι) :
    (∑ x in s, f x).Prod = ∏ x in s, (f x).Prod := by
  classical
  induction' s using Finsetₓ.induction_on with a t hat ih
  · rw [Finsetₓ.sum_empty, Finsetₓ.prod_empty, Multiset.prod_zero]
    
  · rw [Finsetₓ.sum_insert hat, Finsetₓ.prod_insert hat, Multiset.prod_add, ih]
...
@[simp, norm_cast]
theorem cast_sum [AddCommMonoidWithOne β] (s : Finsetₓ α) (f : α → ℕ) : ↑(∑ x in s, f x : ℕ) = ∑ x in s, (f x : β) :=
  map_sum (castAddMonoidHom β) _ _

@[simp, norm_cast]
theorem cast_prod [CommSemiringₓ β] (f : α → ℕ) (s : Finsetₓ α) : (↑(∏ i in s, f i) : β) = ∏ i in s, f i :=
...
@[simp, norm_cast]
theorem cast_sum [AddCommGroupWithOne β] (s : Finsetₓ α) (f : α → ℤ) : ↑(∑ x in s, f x : ℤ) = ∑ x in s, (f x : β) :=
  map_sum (castAddHom β) _ _

@[simp, norm_cast]
theorem cast_prod {R : Type _} [CommRingₓ R] (f : α → ℤ) (s : Finsetₓ α) : (↑(∏ i in s, f i) : R) = ∏ i in s, f i :=
  (Int.castRingHom R).map_prod _ _

end Int

@[simp, norm_cast]
theorem Units.coe_prod {M : Type _} [CommMonoidₓ M] (f : α → Mˣ) (s : Finsetₓ α) :
    (↑(∏ i in s, f i) : M) = ∏ i in s, f i :=
  (Units.coeHom M).map_prod _ _

theorem Units.mk0_prod [CommGroupWithZero β] (s : Finsetₓ α) (f : α → β) (h) :
    Units.mk0 (∏ b in s, f b) h = ∏ b in s.attach, Units.mk0 (f b) fun hh => h (Finsetₓ.prod_eq_zero b.2 hh) := by
  classical
  induction s using Finsetₓ.induction_on <;> simp [*]

theorem nat_abs_sum_le {ι : Type _} (s : Finsetₓ ι) (f : ι → ℤ) : (∑ i in s, f i).natAbs ≤ ∑ i in s, (f i).natAbs := by
  classical
  apply Finsetₓ.induction_on s
  · simp only [Finsetₓ.sum_empty, Int.nat_abs_zero]
...
@[simp]
theorem to_mul_list_sum (s : List (Additive α)) : toMul s.Sum = (s.map toMul).Prod := by simpa [to_mul, of_mul]
...
@[simp]
theorem to_add_list_sum (s : List (Multiplicative α)) : toAdd s.Prod = (s.map toAdd).Sum := by simpa [to_add, of_add]
...
@[simp]
theorem of_mul_multiset_prod (s : Multiset α) : ofMul s.Prod = (s.map ofMul).Sum := by simpa [of_mul]

@[simp]
theorem to_mul_multiset_sum (s : Multiset (Additive α)) : toMul s.Sum = (s.map toMul).Prod := by simpa [to_mul, of_mul]

@[simp]
theorem of_mul_prod (s : Finsetₓ ι) (f : ι → α) : ofMul (∏ i in s, f i) = ∑ i in s, ofMul (f i) :=
  rfl

@[simp]
theorem to_mul_sum (s : Finsetₓ ι) (f : ι → Additive α) : toMul (∑ i in s, f i) = ∏ i in s, toMul (f i) :=
  rfl
...
@[simp]
theorem of_add_multiset_prod (s : Multiset α) : ofAdd s.Sum = (s.map ofAdd).Prod := by simpa [of_add]
...
@[simp]
theorem of_add_sum (s : Finsetₓ ι) (f : ι → α) : ofAdd (∑ i in s, f i) = ∏ i in s, ofAdd (f i) :=
  rfl

@[simp]
theorem to_add_prod (s : Finsetₓ ι) (f : ι → Multiplicative α) : toAdd (∏ i in s, f i) = ∑ i in s, toAdd (f i) :=
  rfl
