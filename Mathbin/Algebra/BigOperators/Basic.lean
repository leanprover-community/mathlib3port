...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » «expr \ »(s₂, s₁))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s₁)
@[toAdditive]
...
theorem prod_eq_mul {s : Finset α} {f : α → β} (a b : α) (hn : a ≠ b) (h₀ : ∀ c _ : c ∈ s, c ≠ a ∧ c ≠ b → f c = 1)
  ...
  by 
    have  := Classical.decEq α <;> byCases' h₁ : a ∈ s <;> byCases' h₂ : b ∈ s
    ·
...
@[ toAdditive sum_nsmul ]
  theorem
    prod_pow
    ( s : Finset α ) ( n : ℕ ) ( f : α → β ) : ∏ x in s , f x ^ n = ∏ x in s , f x ^ n
    :=
      by
        have := Classical.decEq α
          <;>
          exact
            Finset.induction_on
              s by simp by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) [ mul_powₓ ]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » (s.erase x).erase (g x hx))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s)
@[toAdditive]
    ...
    :=
      calc
        ∏ a in s , f g a = ∏ x in s.image g . Sigma fun b : γ => s.filter fun a => g a = b , f g x . 2
            :=
            prod_bij fun a ha => ⟨ g a , a ⟩ by simp <;> tauto fun _ _ => rfl by simp by finish
          _ = ∏ b in s.image g , ∏ a in s.filter fun a => g a = b , f g a := prod_sigma _ _ _
          _ = ∏ b in s.image g , ∏ a in s.filter fun a => g a = b , f b
            :=
            prod_congr
              rfl fun b hb => prod_congr rfl by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } )
...
theorem prod_partition (R : Setoidₓ α) [DecidableRel R.r] :
  (∏ x in s, f x) = ∏ xbar in s.image Quotientₓ.mk, ∏ y in s.filter fun y => ⟦y⟧ = xbar, f y :=
  by 
...
@[ toAdditive ]
  theorem
    ...
    { α β : Type _ } [ Fintype α ] [ CommMonoidₓ β ] ( p : α → Prop ) ( f : α → β ) [ DecidablePred p ]
      : ∏ i : { x // p x } , f i * ∏ i : { x // ¬ p x } , f i = ∏ i , f i
    :=
      by
        classical
          let s := { x | p x } . toFinset
          rw [ ← Finset.prod_subtype s , ← Finset.prod_subtype s ᶜ ]
...
theorem exists_smul_of_dvd_count (s : Multiset α) {k : ℕ} (h : ∀ a : α, a ∈ s → k ∣ Multiset.count a s) :
  ...
  by 
    ...
    have h₂ :
      (∑ x : α in s.to_finset, k • (count x s / k) • ({x} : Multiset α)) = ∑ x : α in s.to_finset, count x s • {x}
    ·
