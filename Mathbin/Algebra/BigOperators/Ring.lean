        ...
        refine'
            congr_arg₂ _
              (prod_bij (fun (a : α) (ha : a ∈ t) => ⟨a, mem_powerset.1 ht ha⟩) _ _ _ fun b hb =>
                ⟨b, by
                  cases b <;>
                    simpa only [true_and_iff, exists_prop, mem_filter, and_true_iff, mem_attach, eq_self_iff_true,
                ...
                ⟨b, by
                  cases b <;>
                    · simp only [true_and_iff, mem_filter, mem_attach, Subtype.coe_mk] at hb
                      simpa only [true_and_iff, exists_prop, and_true_iff, mem_sdiff, eq_self_iff_true, Subtype.coe_mk,
                        b_property]
                      ⟩) <;>
...
theorem prod_add_ordered {ι R : Type _} [CommSemiring R] [LinearOrder ι] (s : Finset ι) (f g : ι → R) :
    (∏ i in s, f i + g i) =
  ...
  · rw [mul_sum]
    ...
    rw [filter_insert, if_neg (ha i hi).not_lt, filter_insert, if_pos (ha i hi), prod_insert, mul_left_comm]
...
theorem prod_one_sub_ordered {ι R : Type _} [CommRing R] [LinearOrder ι] (s : Finset ι) (f : ι → R) :
...
theorem prod_powerset [CommMonoid β] (s : Finset α) (f : Finset α → β) :
    (∏ t in powerset s, f t) = ∏ j in range (card s + 1), ∏ t in powersetLen j s, f t := by classical
