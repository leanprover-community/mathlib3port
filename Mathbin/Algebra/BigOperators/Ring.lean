...
variable {s s₁ s₂ : Finsetₓ α} {a : α} {b : β} {f g : α → β}
...
theorem prod_pow_eq_pow_sum {x : β} {f : α → ℕ} : ∀ {s : Finsetₓ α}, (∏ i in s, x ^ f i) = x ^ ∑ x in s, f x := by
  apply Finsetₓ.induction
  · simp
    
  · intro a s has H
    rw [Finsetₓ.prod_insert has, Finsetₓ.sum_insert has, pow_addₓ, H]
...
theorem sum_mul_sum {ι₁ : Type _} {ι₂ : Type _} (s₁ : Finsetₓ ι₁) (s₂ : Finsetₓ ι₂) (f₁ : ι₁ → β) (f₂ : ι₂ → β) :
...
theorem sum_mul_boole [DecidableEq α] (s : Finsetₓ α) (f : α → β) (a : α) :
    (∑ x in s, f x * ite (a = x) 1 0) = ite (a ∈ s) (f a) 0 := by simp

theorem sum_boole_mul [DecidableEq α] (s : Finsetₓ α) (f : α → β) (a : α) :
    (∑ x in s, ite (a = x) 1 0 * f x) = ite (a ∈ s) (f a) 0 := by simp

end Semiringₓ

theorem sum_div [DivisionSemiring β] {s : Finsetₓ α} {f : α → β} {b : β} : (∑ x in s, f x) / b = ∑ x in s, f x / b := by
...
theorem prod_sum {δ : α → Type _} [DecidableEq α] [∀ a, DecidableEq (δ a)] {s : Finsetₓ α} {t : ∀ a, Finsetₓ (δ a)}
    {f : ∀ a, δ a → β} : (∏ a in s, ∑ b in t a, f a b) = ∑ p in s.pi t, ∏ x in s.attach, f x.1 (p x.1 x.2) := by
  induction' s using Finsetₓ.induction with a s ha ih
  · rw [pi_empty, sum_singleton]
    rfl
    
  · have h₁ :
      ∀ x ∈ t a, ∀ y ∈ t a, ∀ h : x ≠ y, Disjoint (image (pi.cons s a x) (pi s t)) (image (pi.cons s a y) (pi s t)) :=
      by
      intro x hx y hy h
    ...
    have h₂ : ∀ p₁ ∈ pi s t, ∀ p₂ ∈ pi s t, pi.cons s a b p₁ = pi.cons s a b p₂ → p₁ = p₂ := fun p₁ h₁ p₂ h₂ eq =>
      pi_cons_injective ha Eq
...
theorem prod_add (f g : α → β) (s : Finsetₓ α) :
    (∏ a in s, f a + g a) = ∑ t in s.Powerset, (∏ a in t, f a) * ∏ a in s \ t, g a :=
  calc
    (∏ a in s, f a + g a) = ∏ a in s, ∑ p in ({True, False} : Finsetₓ Prop), if p then f a else g a := by simp
    _ =
        ∑ p in (s.pi fun _ => {True, False} : Finsetₓ (∀ a ∈ s, Prop)),
...
theorem prod_add_ordered {ι R : Type _} [CommSemiringₓ R] [LinearOrderₓ ι] (s : Finsetₓ ι) (f g : ι → R) :
    (∏ i in s, f i + g i) =
      (∏ i in s, f i) + ∑ i in s, (g i * ∏ j in s.filter (· < i), f j + g j) * ∏ j in s.filter fun j => i < j, f j :=
  by
  refine' Finsetₓ.induction_on_max s (by simp) _
  clear s
  intro a s ha ihs
...
/-- `∏ i, (f i - g i) = (∏ i, f i) - ∑ i, g i * (∏ j < i, f j - g j) * (∏ j > i, f j)`. -/
theorem prod_sub_ordered {ι R : Type _} [CommRingₓ R] [LinearOrderₓ ι] (s : Finsetₓ ι) (f g : ι → R) :
    (∏ i in s, f i - g i) =
      (∏ i in s, f i) - ∑ i in s, (g i * ∏ j in s.filter (· < i), f j - g j) * ∏ j in s.filter fun j => i < j, f j :=
  by
...
theorem prod_one_sub_ordered {ι R : Type _} [CommRingₓ R] [LinearOrderₓ ι] (s : Finsetₓ ι) (f : ι → R) :
...
gives `(a + b)^s.card`.-/
theorem sum_pow_mul_eq_add_pow {α R : Type _} [CommSemiringₓ R] (a b : R) (s : Finsetₓ α) :
    (∑ t in s.Powerset, a ^ t.card * b ^ (s.card - t.card)) = (a + b) ^ s.card := by
  rw [← prod_const, prod_add]
  refine' Finsetₓ.sum_congr rfl fun t ht => _
  rw [prod_const, prod_const, ← card_sdiff (mem_powerset.1 ht)]

theorem dvd_sum {b : β} {s : Finsetₓ α} {f : α → β} (h : ∀ x ∈ s, b ∣ f x) : b ∣ ∑ x in s, f x :=
  Multiset.dvd_sum fun y hy => by rcases Multiset.mem_map.1 hy with ⟨x, hx, rfl⟩ <;> exact h x hx

@[norm_cast]
theorem prod_nat_cast (s : Finsetₓ α) (f : α → ℕ) : ↑(∏ x in s, f x : ℕ) = ∏ x in s, (f x : β) :=
...
theorem prod_powerset_insert [DecidableEq α] [CommMonoidₓ β] {s : Finsetₓ α} {x : α} (h : x ∉ s) (f : Finsetₓ α → β) :
    (∏ a in (insert x s).Powerset, f a) = (∏ a in s.Powerset, f a) * ∏ t in s.Powerset, f (insert x t) := by
  rw [powerset_insert, Finsetₓ.prod_union, Finsetₓ.prod_image]
  · intro t₁ h₁ t₂ h₂ heq
    rw [← Finsetₓ.erase_insert (not_mem_of_mem_powerset_of_not_mem h₁ h), ←
      Finsetₓ.erase_insert (not_mem_of_mem_powerset_of_not_mem h₂ h), HEq]
    
  · rw [Finsetₓ.disjoint_iff_ne]
    intro t₁ h₁ t₂ h₂
    rcases Finsetₓ.mem_image.1 h₂ with ⟨t₃, h₃, H₃₂⟩
    rw [← H₃₂]
...
theorem prod_powerset [CommMonoidₓ β] (s : Finsetₓ α) (f : Finsetₓ α → β) :
...
theorem sum_range_succ_mul_sum_range_succ [NonUnitalNonAssocSemiringₓ β] (n k : ℕ) (f g : ℕ → β) :
    ((∑ i in range (n + 1), f i) * ∑ i in range (k + 1), g i) =
      (((∑ i in range n, f i) * ∑ i in range k, g i) + f n * ∑ i in range k, g i) + (∑ i in range n, f i) * g k +
        f n * g k :=
  by simp only [add_mulₓ, mul_addₓ, add_assocₓ, sum_range_succ]
