...
theorem finprod_eq_finset_prod_of_mul_support_subset (f : α → M) {s : Finset α} (h : mulSupport f ⊆ (s : Set α)) :
    (∏ᶠ i, f i) = ∏ i in s, f i :=
  haveI h' : (s.finite_to_set.subset h).toFinset ⊆ s := by simpa [← Finset.coe_subset, Set.coe_to_finset]
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (i «expr ≠ » a) -/
@[to_additive]
...
theorem finprod_mul_distrib (hf : (mulSupport f).Finite) (hg : (mulSupport g).Finite) :
  ...
  classical rw [finprod_eq_prod_of_mul_support_to_finset_subset _ hf (Finset.subset_union_left _ _),
...
theorem finprod_mem_union_inter (hs : s.Finite) (ht : t.Finite) :
  ...
  classical rw [← Finset.coe_union, ← Finset.coe_inter]
...
theorem finprod_mem_image' {s : Set β} {g : β → α} (hg : (s ∩ mulSupport (f ∘ g)).InjOn g) :
  ...
  classical by_cases hs:(s ∩ mul_support (f ∘ g)).Finite
...
theorem finprod_mem_Union [Finite ι] {t : ι → Set α} (h : Pairwise (Disjoint on t)) (ht : ∀ i, (t i).Finite) :
  ...
  classical rw [← bUnion_univ, ← Finset.coe_univ, ← Finset.coe_bUnion, finprod_mem_coe_finset, Finset.prod_bUnion]
...
theorem mul_finprod_cond_ne (a : α) (hf : (mulSupport f).Finite) : (f a * ∏ᶠ (i) (_ : i ≠ a), f i) = ∏ᶠ i, f i := by
  classical rw [finprod_eq_prod _ hf]
    · intro x hx
     ...
     (Command.theorem
      ...
      (Command.declSig
       [(Term.implicitBinder "{" [`M] [":" (Term.type "Type" [(Level.hole "_")])] "}")
        ...
        (Term.explicitBinder "(" [`hf] [":" (Term.proj (Term.app `mulSupport [`f]) "." `Finite)] [] ")")
        (Term.explicitBinder
         "("
         [`h]
...
@[ to_additive ]
  theorem
    single_le_finprod
    { M : Type _ } [ OrderedCommMonoid M ] ( i : α ) { f : α → M } ( hf : mulSupport f . Finite ) ( h : ∀ j , 1 ≤ f j )
      : f i ≤ ∏ᶠ j , f j
    :=
      by
        skip
          ...
          calc
            ...
            _ = ∏ᶠ j , f j := finprod_eq_prod_of_mul_support_to_finset_subset _ hf Finset.subset_insert _ _ . symm
...
theorem finprod_mem_finset_product (s : Finset (α × β)) (f : α × β → M) :
  ...
  classical rw [finprod_mem_finset_product']
#align finprod_mem_finset_product finprod_mem_finset_product
