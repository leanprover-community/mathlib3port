...
def prod : Multiset α → α :=
  foldr (· * ·) (fun x y z => by simp [mul_left_commₓ]) 1

@[to_additive]
theorem prod_eq_foldr (s : Multiset α) : prod s = foldr (· * ·) (fun x y z => by simp [mul_left_commₓ]) 1 s :=
  rfl

@[to_additive]
theorem prod_eq_foldl (s : Multiset α) : prod s = foldl (· * ·) (fun x y z => by simp [mul_right_commₓ]) 1 s :=
...
theorem prod_singleton (a : α) : prod {a} = a := by
  simp only [mul_oneₓ, prod_cons, ← cons_zero, eq_self_iff_true, prod_zero]
...
theorem prod_add (s t : Multiset α) : prod (s + t) = prod s * prod t :=
  (Quotientₓ.induction_on₂ s t) fun l₁ l₂ => by simp

theorem prod_nsmul (m : Multiset α) : ∀ n : ℕ, (n • m).Prod = m.Prod ^ n
  | 0 => by
    rw [zero_nsmul, pow_zeroₓ]
    rfl
  | n + 1 => by rw [add_nsmul, one_nsmul, pow_addₓ, pow_oneₓ, prod_add, prod_nsmul n]
...
@[to_additive]
theorem pow_count [DecidableEq α] (a : α) : a ^ s.count a = (s.filter (Eq a)).Prod := by rw [filter_eq, prod_repeat]

@[to_additive]
theorem prod_hom [CommMonoidₓ β] (s : Multiset α) {F : Type _} [MonoidHomClass F α β] (f : F) :
    (s.map f).Prod = f s.Prod :=
  (Quotientₓ.induction_on s) fun l => by simp only [l.prod_hom f, quot_mk_to_coe, coe_map, coe_prod]
...
theorem prod_hom₂ [CommMonoidₓ β] [CommMonoidₓ γ] (s : Multiset ι) (f : α → β → γ)
  ...
  (Quotientₓ.induction_on s) fun l => by simp only [l.prod_hom₂ f hf hf', quot_mk_to_coe, coe_map, coe_prod]
...
theorem prod_hom_rel [CommMonoidₓ β] (s : Multiset ι) {r : α → β → Prop} {f : ι → α} {g : ι → β} (h₁ : r 1 1)
  ...
  (Quotientₓ.induction_on s) fun l => by simp only [l.prod_hom_rel h₁ h₂, quot_mk_to_coe, coe_map, coe_prod]
...
@[simp]
theorem prod_map_neg [HasDistribNeg α] (s : Multiset α) : (s.map Neg.neg).Prod = -1 ^ s.card * s.Prod := by
  refine' Quotientₓ.ind _ s
  simp
...
theorem prod_induction (p : α → Prop) (s : Multiset α) (p_mul : ∀ a b, p a → p b → p (a * b)) (p_one : p 1)
  ...
  rw [prod_eq_foldr]
  exact foldr_induction (· * ·) (fun x y z => by simp [mul_left_commₓ]) 1 p s p_mul p_one p_s
...
theorem dvd_prod : a ∈ s → a ∣ s.Prod :=
  Quotientₓ.induction_on s (fun l a h => by simpa using List.dvd_prod h) a
...
theorem sum_map_mul_left : sum (s.map fun i => a * f i) = a * sum (s.map f) :=
  Multiset.induction_on s (by simp) fun i s ih => by simp [ih, mul_addₓ]

theorem sum_map_mul_right : sum (s.map fun i => f i * a) = sum (s.map f) * a :=
  Multiset.induction_on s (by simp) fun a s ih => by simp [ih, add_mulₓ]
...
theorem one_le_prod_of_one_le : (∀ x ∈ s, (1 : α) ≤ x) → 1 ≤ s.Prod :=
  (Quotientₓ.induction_on s) fun l hl => by simpa using List.one_le_prod_of_one_le hl

@[to_additive]
theorem single_le_prod : (∀ x ∈ s, (1 : α) ≤ x) → ∀ x ∈ s, x ≤ s.Prod :=
  (Quotientₓ.induction_on s) fun l hl x hx => by simpa using List.single_le_prod hl x hx
...
theorem all_one_of_le_one_le_of_prod_eq_one : (∀ x ∈ s, (1 : α) ≤ x) → s.Prod = 1 → ∀ x ∈ s, x = (1 : α) := by
...
theorem prod_eq_one_iff [CanonicallyOrderedMonoid α] {m : Multiset α} : m.Prod = 1 ↔ ∀ x ∈ m, x = (1 : α) :=
  (Quotientₓ.induction_on m) fun l => by simpa using List.prod_eq_one_iff l
...
theorem le_prod_of_submultiplicative [CommMonoidₓ α] [OrderedCommMonoid β] (f : α → β) (h_one : f 1 = 1)
  ...
  le_prod_of_submultiplicative_on_pred f (fun i => True) h_one trivialₓ (fun x y _ _ => h_mul x y) (by simp) s (by simp)
