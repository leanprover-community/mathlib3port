...
@[toAdditive]
theorem prod_le_of_forall_le (s : Finset ι) (f : ι → N) (n : N) (h : ∀ x _ : x ∈ s, f x ≤ n) : s.prod f ≤ n ^ s.card :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
@[toAdditive]
theorem le_prod_of_forall_le (s : Finset ι) (f : ι → N) (n : N) (h : ∀ x _ : x ∈ s, n ≤ f x) : n ^ s.card ≤ s.prod f :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s.image f)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (k «expr ∈ » s)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » s)
@[toAdditive sum_pos]
theorem one_lt_prod (h : ∀ i _ : i ∈ s, 1 < f i) (hs : s.nonempty) : 1 < ∏ i in s, f i :=
  lt_of_le_of_ltₓ
      (by 
        rw [prod_const_one])$
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » s)
@[toAdditive]
theorem prod_lt_one (h : ∀ i _ : i ∈ s, f i < 1) (hs : s.nonempty) : (∏ i in s, f i) < 1 :=
...
theorem prod_le_prod (h0 : ∀ i _ : i ∈ s, 0 ≤ f i) (h1 : ∀ i _ : i ∈ s, f i ≤ g i) : (∏ i in s, f i) ≤ ∏ i in s, g i :=
  by 
...
/--
  ...
  theorem
    ...
    { i : ι }
        ...
        ( hi : i ∈ s )
        ( h2i : g i + h i ≤ f i )
        ( hgf : ∀ j _ : j ∈ s , j ≠ i → g j ≤ f j )
        ( hhf : ∀ j _ : j ∈ s , j ≠ i → h j ≤ f j )
        ( hg : ∀ i _ : i ∈ s , 0 ≤ g i )
        ( hh : ∀ i _ : i ∈ s , 0 ≤ h i )
            ...
            rw [ right_distrib ]
              apply add_le_add
                ...
                apply mul_le_mul_of_nonneg_left
                  <;>
                  try applyAssumption <;> assumption
                    <;>
                    apply prod_le_prod <;> simp ( config := { contextual := Bool.true._@._internal._hyg.0 } )
          ·
            apply prod_nonneg
...
theorem prod_mono' [OrderedCommMonoid M] : Monotone fun f : ι → M => ∏ i, f i :=
  fun f g hfg => Finset.prod_le_prod''$ fun x _ => hfg x

attribute [mono] sum_mono

@[toAdditive sum_strict_mono]
