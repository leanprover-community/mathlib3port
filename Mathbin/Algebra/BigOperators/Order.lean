...
@[ to_additive sum_le_sum_of_subset_of_nonneg ]
  theorem
    ...
    ( h : s ⊆ t ) ( hf : ∀ i ∈ t , i ∉ s → 1 ≤ f i ) : ∏ i in s , f i ≤ ∏ i in t , f i
    :=
      by
        skip
          <;>
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (y «expr ∉ » t) -/
...
[PrettyPrinter.parenthesize.input] (Command.declaration
     ...
     (Command.theorem
      "theorem"
      (Command.declId `prod_le_prod_of_ne_one' [])
      (Command.declSig
       [(Term.explicitBinder
         "("
         [`h]
                   ...
                   (Tactic.rwSeq
                    ...
                    (Tactic.rwRuleSeq
                     ...
                     [(Tactic.rwRule [(patternIgnore (token.«← » "←"))] `prod_union)
                      ","
                      (Tactic.rwRule [] `filter_union_filter_neg_eq)]
                     "]")
                    [])
                   "<;>"
                   (Tactic.exact
...
@[ to_additive sum_le_sum_of_ne_zero ]
  theorem
    ...
    ( h : ∀ x ∈ s , f x ≠ 1 → x ∈ t ) : ∏ x in s , f x ≤ ∏ x in t , f x
    :=
      by
        skip
          <;>
...
theorem prod_lt_prod' (Hle : ∀ i ∈ s, f i ≤ g i) (Hlt : ∃ i ∈ s, f i < g i) : (∏ i in s, f i) < ∏ i in s, g i := by
  classical rcases Hlt with ⟨i, hi, hlt⟩
        ...
        skip
          ...
          calc
            ∏ j in s , f j < ∏ j in insert i s , f j
              :=
              by rw [ prod_insert hs ] exact lt_mul_of_one_lt_left' ∏ j in s , f j hlt
            _ ≤ ∏ j in t , f j
              :=
              by
                apply prod_le_prod_of_subset_of_one_le'
                  · simp [ Finset.insert_subset , h , ht ]
                  · intro x hx h'x simp only [ mem_insert , not_or ] at h'x exact hle x hx h'x . 2
