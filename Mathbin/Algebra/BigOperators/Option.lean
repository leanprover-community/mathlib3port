...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `prod_insert_none [])
  (Command.declSig
   ...
   (Term.byTactic
    ...
    (Tactic.tacticSeq
     (Tactic.tacticSeq1Indented
      [(group (Tactic.simp "simp" [] [] ["[" [(Tactic.simpLemma [] [] `insert_none)] "]"] []) [])])))
   [])
  []
  []))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Tactic.simp "simp" [] [] ["[" [(Tactic.simpLemma [] [] `insert_none)] "]"] [])
...
[PrettyPrinter.parenthesize] parenthesized: (Term.paren "(" [(Term.app `some [`x]) []] ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
  `f
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'antiquot'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'ident.antiquot'
...
@[ simp , to_additive ]
  theorem
    prod_insert_none
    ( f : Option α → M ) ( s : Finset α ) : ∏ x in s.insert_none , f x = f none * ∏ x in s , f some x
    := by simp [ insert_none ]
   ...
   (Term.typeSpec
    ...
    («term_=_»
     ...
     (Algebra.BigOperators.Basic.«term∏_in_,_»
      ...
      `s
      ", "
      (Term.app `Option.elim [`x (numLit "1") `f])))))
  (Command.declValSimple
   ":="
   (Term.byTactic
    "by"
            ...
            (Term.proj
             (Term.app
              ...
              [`s.erase_none
               ...
               (Term.fun
                "fun"
                (Term.basicFun [(Term.simpleBinder [`x] [])] "=>" (Term.app `Option.elim [`x (numLit "1") `f])))])
             "."
             `symm))
           (calcStep
            («term_=_»
    ...
    [(calcStep
     ...
     (calcStep
      ...
      (Term.byTactic
       ...
       (Tactic.tacticSeq
        (Tactic.tacticSeq1Indented
         [(group (Tactic.rwSeq "rw" [] (Tactic.rwRuleSeq "[" [(Tactic.rwRule [] `map_some_erase_none)] "]") []) [])]))))
     (calcStep
      («term_=_»
       (Term.hole "_")
       "="
...
@[ to_additive ]
  theorem
    prod_erase_none
    ( f : α → M ) ( s : Finset Option α ) : ∏ x in s.erase_none , f x = ∏ x in s , Option.elim x 1 f
    :=
      by
        classical
          <;>
          calc
            ∏ x in s.erase_none , f x = ∏ x in s.erase_none.map embedding.some , Option.elim x 1 f
                :=
                prod_mapₓ s.erase_none embedding.some fun x => Option.elim x 1 f . symm
              _ = ∏ x in s.erase none , Option.elim x 1 f := by rw [ map_some_erase_none ]
              _ = ∏ x in s , Option.elim x 1 f := prod_erase _ rfl
