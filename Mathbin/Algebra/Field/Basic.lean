...
theorem neg_div_neg_eq (a b : K) : -a / -b = a / b :=
  by 
...
theorem same_add_div {a b : K} (h : b ≠ 0) : (b+a) / b = 1+a / b :=
  by 
...
theorem div_add_same {a b : K} (h : b ≠ 0) : (a+b) / b = (a / b)+1 :=
  by 
...
theorem div_sub_div_same (a b c : K) : a / c - b / c = (a - b) / c :=
  by 
    rw [sub_eq_add_neg, ←neg_div, div_add_div_same, sub_eq_add_neg]

theorem same_sub_div {a b : K} (h : b ≠ 0) : (b - a) / b = 1 - a / b :=
  by 
...
theorem div_sub_same {a b : K} (h : b ≠ 0) : (a - b) / b = a / b - 1 :=
  by 
...
theorem one_div_mul_add_mul_one_div_eq_one_div_add_one_div (ha : a ≠ 0) (hb : b ≠ 0) :
  (((1 / a)*a+b)*1 / b) = (1 / a)+1 / b :=
  by 
    rw [left_distrib (1 / a), one_div_mul_cancel ha, right_distrib, one_mulₓ, mul_assocₓ, mul_one_div_cancel hb,
      mul_oneₓ, add_commₓ]
...
theorem one_div_add_one_div {a b : K} (ha : a ≠ 0) (hb : b ≠ 0) : ((1 / a)+1 / b) = (a+b) / a*b :=
  by 
...
theorem inv_add_inv {a b : K} (ha : a ≠ 0) (hb : b ≠ 0) : (a⁻¹+b⁻¹) = (a+b) / a*b :=
  by 
...
@[field_simps]
theorem add_div' (a b c : K) (hc : c ≠ 0) : (b+a / c) = ((b*c)+a) / c :=
  by 
    simpa using div_add_div b a one_ne_zero hc

@[field_simps]
theorem sub_div' (a b c : K) (hc : c ≠ 0) : b - a / c = ((b*c) - a) / c :=
  by 
    simpa using div_sub_div b a one_ne_zero hc

@[field_simps]
theorem div_add' (a b c : K) (hc : c ≠ 0) : ((a / c)+b) = (a+b*c) / c :=
  by 
    rwa [add_commₓ, add_div', add_commₓ]

@[field_simps]
theorem div_sub' (a b c : K) (hc : c ≠ 0) : a / c - b = (a - c*b) / c :=
  by 
...
theorem uniq_inv_of_is_field (R : Type u) [Ringₓ R] (hf : IsField R) : ∀ (x : R), x ≠ 0 → ∃!y : R, (x*y) = 1 :=
  by 
    ...
    ·
      ...
      calc y = y*x*z :=
        by 
          rw [hxz, mul_oneₓ]_ = (x*y)*z :=
        by 
          rw [←mul_assocₓ, hf.mul_comm y x]_ = z :=
        by 
          rw [hxy, one_mulₓ]
