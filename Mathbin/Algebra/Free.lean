...
@[ to_additive ] instance : Monadₓ FreeMagma where pure _ := of bind _ _ x f := lift f x
    ...
    where
      mul
        ...
        mul_assoc
          ...
          Quot.induction_on x
            $
            fun p => Quot.induction_on y $ fun q => Quot.induction_on z $ fun r => of_mul_assoc p q r
...
@[ to_additive ]
  instance
    : Semigroupₓ ( FreeSemigroup α )
    where
      mul L1 L2 := ( L1 . 1 , L1 . 2 ++ L2 . 1 :: L2 . 2 ) mul_assoc L1 L2 L3 := Prod.extₓ rfl $ List.append_assoc _ _ _
...
-- failed to format: format: uncaught backtrack exception
@[ to_additive ] instance : Monadₓ FreeSemigroup where pure _ := of bind _ _ x f := lift f x
...
@[ to_additive ]
  instance
    : IsLawfulMonad FreeSemigroup .{ u }
    where
      pure_bind _ _ _ _ := rfl
        bind_assoc
          α β γ x f g
          :=
          rec_on_pure x ( fun x => rfl ) fun x y ih1 ih2 => by rw [ mul_bind , mul_bind , mul_bind , ih1 , ih2 ]
        id_map α x := rec_on_pure x ( fun _ => rfl ) fun x y ih1 ih2 => by rw [ map_mul' , ih1 , ih2 ]
