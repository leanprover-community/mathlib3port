...
theorem geom_sum_succ {x : α} {n : ℕ} : geomSum x (n+1) = (x*geomSum x n)+1 :=
  by 
...
@[simp]
theorem geom_sum_one (x : α) : geomSum x 1 = 1 :=
  by 
    simp [geom_sum_succ']

@[simp]
theorem geom_sum_two {x : α} : geomSum x 2 = x+1 :=
  by 
    simp [geom_sum_succ']
...
@[simp]
theorem one_geom_sum (n : ℕ) : geomSum (1 : α) n = n :=
  by 
    simp [geom_sum_def]

@[simp]
theorem op_geom_sum (x : α) (n : ℕ) : op (geomSum x n) = geomSum (op x) n :=
  by 
    simp [geom_sum_def]
...
@[simp]
theorem geom_sum₂_one (x y : α) : geomSum₂ x y 1 = 1 :=
  by 
    have  : 1 - 1 - 0 = 0 := rfl 
    rw [geom_sum₂_def, sum_range_one, this, pow_zeroₓ, pow_zeroₓ, mul_oneₓ]

@[simp]
theorem op_geom_sum₂ (x y : α) (n : ℕ) : op (geomSum₂ x y n) = geomSum₂ (op y) (op x) n :=
  by 
...
@[simp]
theorem neg_one_geom_sum [Ringₓ α] {n : ℕ} : geomSum (-1 : α) n = if Even n then 0 else 1 :=
  by 
...
theorem geom_sum_pos [OrderedSemiring α] (hx : 0 < x) (hn : n ≠ 0) : 0 < geomSum x n :=
  by 
...
theorem geom_sum_alternating_of_lt_neg_one [OrderedRing α] (hx : (x+1) < 0) (hn : 1 < n) :
  if Even n then geomSum x n < 0 else 1 < geomSum x n :=
  by 
    have hx0 : x < 0 
    exact ((le_add_iff_nonneg_right _).2 (@zero_le_one α _)).trans_lt hx 
    refine' Nat.le_induction _ _ n (show 2 ≤ n from hn)
    ·
...
theorem geom_sum_pos_of_odd [LinearOrderedRing α] (h : Odd n) : 0 < geomSum x n :=
  by 
    rcases n with (_ | _ | k)
    ·
      exact
...
theorem geom_sum_pos_iff [LinearOrderedRing α] (hn : 1 < n) : 0 < geomSum x n ↔ Odd n ∨ 0 < x+1 :=
  by 
    refine' ⟨fun h => _, _⟩
    ·
      suffices  : (¬0 < x+1) → Odd n
      ·
        tauto 
      intro hx 
      rw [not_ltₓ] at hx 
...
theorem geom_sum_eq_zero_iff_neg_one [LinearOrderedRing α] (hn : 1 < n) : geomSum x n = 0 ↔ x = -1 ∧ Even n :=
  by 
    refine'
      ⟨fun h => _,
        fun ⟨h, hn⟩ =>
...
theorem geom_sum_neg_iff [LinearOrderedRing α] (hn : 1 < n) : geomSum x n < 0 ↔ Even n ∧ (x+1) < 0 :=
  by 
