...
theorem image_to_kernel_arrow (w : f ≫ g = 0) :
  imageToKernel f g w ≫ (kernel_subobject g).arrow = (image_subobject f).arrow :=
  by 
    simp [imageToKernel]

theorem factor_thru_image_subobject_comp_image_to_kernel (w : f ≫ g = 0) :
  factor_thru_image_subobject f ≫ imageToKernel f g w = factor_thru_kernel_subobject g f w :=
  by 
    ext 
    simp 
...
theorem image_to_kernel_zero_right [has_images V] {w} :
  imageToKernel f (0 : B ⟶ C) w = (image_subobject f).arrow ≫ inv (kernel_subobject (0 : B ⟶ C)).arrow :=
  by 
    ext 
    simp 
...
theorem image_subobject_map_comp_image_to_kernel (p : α.right = β.left) :
  imageToKernel f g w ≫ kernel_subobject_map β = image_subobject_map α ≫ imageToKernel f' g' w' :=
  by 
    ext 
    simp [p]
...
@[simp]
theorem homology.map_id : homology.map w w (𝟙 _) (𝟙 _) rfl = 𝟙 _ :=
  by 
...
theorem homology.comp_right_eq_comp_left {V : Type _} [category V] {A₁ B₁ C₁ A₂ B₂ C₂ A₃ B₃ C₃ : V} {f₁ : A₁ ⟶ B₁}
  ...
  (p₁ : α₁.right = β₁.left) (p₂ : α₂.right = β₂.left) : (α₁ ≫ α₂).right = (β₁ ≫ β₂).left :=
  by 
...
theorem image_subobject_iso_image_to_kernel' (w : f ≫ g = 0) :
  (image_subobject_iso f).Hom ≫ imageToKernel' f g w = imageToKernel f g w ≫ (kernel_subobject_iso g).Hom :=
  by 
    ext 
    simp [imageToKernel']
...
theorem image_to_kernel'_kernel_subobject_iso (w : f ≫ g = 0) :
  imageToKernel' f g w ≫ (kernel_subobject_iso g).inv = (image_subobject_iso f).inv ≫ imageToKernel f g w :=
  by 
    ext 
    simp [imageToKernel']
