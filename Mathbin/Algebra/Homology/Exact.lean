...
theorem exact_iso_comp [IsIso f] : Exact (f ≫ g) h ↔ Exact g h :=
  ⟨fun w => by rw [← is_iso.inv_hom_id_assoc f g]; exact exact_epi_comp w, fun w => exactEpiComp w⟩
...
theorem exactKernelι : Exact (kernel.ι f) f := by rw [← kernel_subobject_arrow', exact_iso_comp];
