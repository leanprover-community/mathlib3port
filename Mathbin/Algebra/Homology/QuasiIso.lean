...
instance
  ( priority := 100 )
  quasi_iso_of_iso
  ( f : C ⟶ D ) [ is_iso f ] : QuasiIso f
  where IsIso i := by change is_iso ( ( homologyFunctor V c i ) . mapIso ( as_iso f ) ) . Hom infer_instance

-- failed to format: format: uncaught backtrack exception
instance
  quasi_iso_comp
  ( f : C ⟶ D ) [ QuasiIso f ] ( g : D ⟶ E ) [ QuasiIso g ] : QuasiIso ( f ≫ g )
  where IsIso i := by rw [ functor.map_comp ] infer_instance
