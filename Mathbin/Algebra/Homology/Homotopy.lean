...
-- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:62:18: unsupported non-interactive tactic obviously'
...
def equivSubZero : Homotopy f g ≃ Homotopy (f - g) 0 where
  toFun := fun h =>
    { Hom := fun i j => h.Hom i j, zero' := fun i j w => h.zero _ _ w, comm := fun i => by simp [h.comm] }
  invFun := fun h =>
...
def ofEq (h : f = g) : Homotopy f g where
  Hom := 0
  zero' := fun _ _ _ => rfl
  comm := fun _ => by simp only [AddMonoidHom.map_zero, zero_addₓ, h]
...
def symm {f g : C ⟶ D} (h : Homotopy f g) : Homotopy g f where
  Hom := -h.Hom
  zero' := fun i j w => by rw [Pi.neg_apply, Pi.neg_apply, h.zero i j w, neg_zero]
  comm := fun i => by
...
def trans {e f g : C ⟶ D} (h : Homotopy e f) (k : Homotopy f g) : Homotopy e g where
  Hom := h.Hom + k.Hom
  zero' := fun i j w => by rw [Pi.add_apply, Pi.add_apply, h.zero i j w, k.zero i j w, zero_addₓ]
  comm := fun i => by
...
def add {f₁ g₁ f₂ g₂ : C ⟶ D} (h₁ : Homotopy f₁ g₁) (h₂ : Homotopy f₂ g₂) : Homotopy (f₁ + f₂) (g₁ + g₂) where
  Hom := h₁.Hom + h₂.Hom
  zero' := fun i j hij => by rw [Pi.add_apply, Pi.add_apply, h₁.zero' i j hij, h₂.zero' i j hij, add_zeroₓ]
  comm := fun i => by
...
def compRight {e f : C ⟶ D} (h : Homotopy e f) (g : D ⟶ E) : Homotopy (e ≫ g) (f ≫ g) where
  ...
  zero' := fun i j w => by rw [h.zero i j w, zero_comp]
  comm := fun i => by simp only [h.comm i, d_next_comp_right, preadditive.add_comp, prev_d_comp_right, comp_f]

/-- homotopy is closed under composition (on the left) -/
@[simps]
def compLeft {f g : D ⟶ E} (h : Homotopy f g) (e : C ⟶ D) : Homotopy (e ≫ f) (e ≫ g) where
  Hom := fun i j => e.f i ≫ h.Hom i j
  zero' := fun i j w => by rw [h.zero i j w, comp_zero]
  comm := fun i => by simp only [h.comm i, d_next_comp_left, preadditive.comp_add, prev_d_comp_left, comp_f]
  ...
  comm := fun i => by
    ...
    · cases i
      ...
      · dsimp [fromNext]
        ...
        rw [mk_inductive_aux₃ e zero comm_zero one comm_one succ ((ComplexShape.down ℕ).next i.succ) (i + 1)
            (by rw [aux])]
        dsimp [X_next_iso]
  ...
  comm := fun i => by
    ...
    · cases i
      ...
      · dsimp [toPrev]
        ...
        rw [mk_coinductive_aux₃ e zero comm_zero one comm_one succ ((ComplexShape.up ℕ).prev i.succ) (i + 1)
            (by rw [aux])]
        dsimp [X_prev_iso]
        erw [category.comp_id]
...
def Functor.mapHomotopy (F : V ⥤ W) [F.Additive] {f g : C ⟶ D} (h : Homotopy f g) :
  ...
  zero' := fun i j w => by rw [h.zero i j w, F.map_zero]
  comm := fun i => by
