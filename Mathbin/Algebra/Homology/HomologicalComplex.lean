...
/-- Identity chain map. -/
def id (A : HomologicalComplex V c) : hom A A where
  f := fun _ => 𝟙 _
...
instance : has_zero_morphisms (HomologicalComplex V c) where
...
instance [has_zero_object V] : has_zero_object (HomologicalComplex V c) where
...
@[simps]
def forget : HomologicalComplex V c ⥤ graded_object ι V where
  obj := fun C => C.X
  map := fun _ _ f => f.f
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (X₀ X₁ X₂)
/-- Flatten to a tuple. -/
def mk_struct.flat (t : mk_struct V) : Σ' (X₀ : V) (X₁ : V) (X₂ : V) (d₀ : X₁ ⟶ X₀) (d₁ : X₂ ⟶ X₁), d₁ ≫ d₀ = 0 :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (X₀ X₁)
