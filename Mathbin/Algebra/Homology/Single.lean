...
-- failed to format: format: uncaught backtrack exception
instance
  ( j : ι ) : full ( single V c j )
  where
    Preimage X Y f := eq_to_hom ( by simp ) ≫ f.f j ≫ eq_to_hom ( by simp )
      witness'
        X Y f
        :=
        by ext i dsimp split_ifs · subst h simp · symm apply zero_of_target_iso_zero dsimp rw [ if_neg h ]
