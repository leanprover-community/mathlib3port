...
-- error in Algebra.MonoidAlgebra.Basic: ././Mathport/Syntax/Translate/Basic.lean:702:9: unsupported derive handler inhabited
/--
            ...
            fun a₁ a₂ =>
              by 
                ...
                ·
                  intros 
                  rw [←add_smul]
                simpRw [Finsupp.mul_sum, Finsupp.sum_mul, smul_mul_smul, ←f.map_mul, mul_def, sum_comm a₂ a₁,
...
protected noncomputable def op_ring_equiv [Monoidₓ G] :
  «expr ᵒᵖ» (MonoidAlgebra k G) ≃+* MonoidAlgebra («expr ᵒᵖ» k) («expr ᵒᵖ» G) :=
  { op_add_equiv.symm.trans$
      (Finsupp.mapRange.addEquiv (op_add_equiv : k ≃+ «expr ᵒᵖ» k)).trans$ Finsupp.domCongr equiv_to_opposite with
    map_mul' :=
      by 
...
@[simp]
theorem op_ring_equiv_symm_single [Monoidₓ G] (r : «expr ᵒᵖ» k) (x : «expr ᵒᵖ» G) :
...
-- error in Algebra.MonoidAlgebra.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
/-- The equivalence between `add_monoid_algebra` and `monoid_algebra` in terms of
`multiplicative` -/
protected
def add_monoid_algebra.to_multiplicative
[semiring k]
[has_add G] : «expr ≃+* »(add_monoid_algebra k G, monoid_algebra k (multiplicative G)) :=
{ to_fun := equiv_map_domain multiplicative.of_add,
  map_mul' := λ x y, begin
    repeat { rw [expr equiv_map_domain_eq_map_domain] [] },
    dsimp [] ["[", expr multiplicative.of_add, "]"] [] [],
    convert [] [expr monoid_algebra.map_domain_mul (mul_hom.id (multiplicative G))] []
  end,
...
protected
def monoid_algebra.to_additive
[semiring k]
[has_mul G] : «expr ≃+* »(monoid_algebra k G, add_monoid_algebra k (additive G)) :=
{ to_fun := equiv_map_domain additive.of_mul,
  map_mul' := λ x y, begin
    repeat { rw [expr equiv_map_domain_eq_map_domain] [] },
    dsimp [] ["[", expr additive.of_mul, "]"] [] [],
    convert [] [expr monoid_algebra.map_domain_mul (mul_hom.id G)] []
  end,
...
protected noncomputable def op_ring_equiv [AddCommMonoidₓ G] :
  «expr ᵒᵖ» (AddMonoidAlgebra k G) ≃+* AddMonoidAlgebra («expr ᵒᵖ» k) G :=
  { Opposite.opAddEquiv.symm.trans (Finsupp.mapRange.addEquiv (Opposite.opAddEquiv : k ≃+ «expr ᵒᵖ» k)) with
    map_mul' :=
      by 
        dsimp only [AddEquiv.to_fun_eq_coe, ←AddEquiv.coe_to_add_monoid_hom]
        rw [AddMonoidHom.map_mul_iff]
...
@[simp]
theorem op_ring_equiv_symm_single [AddCommMonoidₓ G] (r : «expr ᵒᵖ» k) (x : «expr ᵒᵖ» G) :
