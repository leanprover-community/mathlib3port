...
theorem grade_by_id : grade_by R (id : M → M) = grade R := by
  rfl

theorem mem_grade_by_iff (f : M → ι) (i : ι) (a : AddMonoidAlgebra R M) :
    a ∈ grade_by R f i ↔ (a.support : Set M) ⊆ f ⁻¹' {i} := by
  rfl
...
theorem single_mem_grade_by {R} [CommSemiringₓ R] (f : M → ι) (i : ι) (m : M) (h : f m = i) (r : R) :
    Finsupp.single m r ∈ grade_by R f i := by
  intro x hx
...
instance grade_by.graded_monoid [AddMonoidₓ M] [AddMonoidₓ ι] [CommSemiringₓ R] (f : M →+ ι) :
    SetLike.GradedMonoid (grade_by R f : ι → Submodule R (AddMonoidAlgebra R M)) where
  one_mem := fun m h => by
    rw [one_def] at h
    by_cases' H : (1 : R) = (0 : R)
...
@[simp]
theorem of_grades_of (i : ι) (x : grade R i) : of_grades (DirectSum.of (fun i => grade R i) i x) = x := by
...
@[simp]
theorem of_grades_comp_to_grades : of_grades.comp to_grades = AlgHom.id R (AddMonoidAlgebra R ι) := by
...
@[simp]
theorem of_grades_to_grades (x : AddMonoidAlgebra R ι) : of_grades x.to_grades = x := by
  apply of_grades_by_to_grades_by

@[simp]
theorem to_grades_by_comp_of_grades_by :
    (to_grades_by f).comp (of_grades_by f) = AlgHom.id R (⨁ i : ι, grade_by R f i) := by
  ext : 2
  dsimp [DirectSum.lof_eq_of]
  rw [of_grades_by_of, to_grades_by_coe]

@[simp]
theorem to_grades_comp_of_grades : to_grades.comp of_grades = AlgHom.id R (⨁ i : ι, grade R i) := by
...
@[simp]
theorem to_grades_of_grades (g : ⨁ i : ι, grade R i) : (of_grades g).toGrades = g := by
