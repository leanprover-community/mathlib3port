...
def compl₂ [MulOneClassₓ M] [MulOneClassₓ N] [CommMonoidₓ P] [CommMonoidₓ Q] (f : M →* N →* P) (g : Q →* N) :
    M →* Q →* P :=
  (compHom' g).comp f

@[simp, to_additive]
theorem compl₂_apply [MulOneClassₓ M] [MulOneClassₓ N] [CommMonoidₓ P] [CommMonoidₓ Q] (f : M →* N →* P) (g : Q →* N)
