...
theorem unique_has_one {α : Type _} [Unique α] [One α] : default = (1 : α) :=
  Unique.default_eq 1
#align unique_has_one unique_has_one
...
theorem coe_mk (a : α) (b h₁ h₂) : ↑(Units.mk a b h₁ h₂) = a :=
  rfl
#align units.coe_mk Units.coe_mk
...
theorem mk_coe (u : αˣ) (y h₁ h₂) : mk (u : α) y h₁ h₂ = u :=
  ext rfl
#align units.mk_coe Units.mk_coe
...
theorem coe_mul : (↑(a * b) : α) = a * b :=
  rfl
#align units.coe_mul Units.coe_mul

@[simp, norm_cast, to_additive]
theorem coe_one : ((1 : αˣ) : α) = 1 :=
  rfl
#align units.coe_one Units.coe_one

@[simp, norm_cast, to_additive]
theorem coe_eq_one {a : αˣ} : (a : α) = 1 ↔ a = 1 := by rw [← Units.coe_one, eq_iff]
#align units.coe_eq_one Units.coe_eq_one
...
theorem inv_eq_coe_inv : a.inv = ((a⁻¹ : αˣ) : α) :=
  rfl
#align units.inv_eq_coe_inv Units.inv_eq_coe_inv
...
theorem Units.coe_mk_of_mul_eq_one [CommMonoid α] {a b : α} (h : a * b = 1) : (Units.mkOfMulEqOne a b h : α) = a :=
  rfl
#align units.coe_mk_of_mul_eq_one Units.coe_mk_of_mul_eq_one
...
#align coe_div_eq_divp coe_div_eq_divp
...
#align is_unit_of_subsingleton is_unit_of_subsingleton
...
#align is_unit_iff_exists_inv is_unit_iff_exists_inv
...
#align units.is_unit_mul_units Units.is_unit_mul_units
...
theorem Units.is_unit_units_mul {M : Type _} [Monoid M] (u : Mˣ) (a : M) : IsUnit (↑u * a) ↔ IsUnit a :=
  Iff.intro
    ...
    u.IsUnit.mul
#align units.is_unit_units_mul Units.is_unit_units_mul
...
#align is_unit_of_mul_is_unit_left is_unit_of_mul_is_unit_left
...
theorem coe_inv_mul (h : IsUnit a) : ↑h.Unit⁻¹ * a = 1 :=
  Units.mul_inv _
#align is_unit.coe_inv_mul IsUnit.coe_inv_mul

@[simp, to_additive]
theorem mul_coe_inv (h : IsUnit a) : a * ↑h.Unit⁻¹ = 1 := by convert h.unit.mul_inv
#align is_unit.mul_coe_inv IsUnit.mul_coe_inv
