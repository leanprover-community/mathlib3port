...
@[simp, normCast]
theorem cast_eq_zero {n : ℕ} : (n : R) = 0 ↔ n = 0 :=
  by 
...
theorem nat_mul_inj' {n : ℕ} {a b : R} (h : ((n : R)*a) = (n : R)*b) (w : n ≠ 0) : a = b :=
  by 
    simpa [w] using nat_mul_inj h

theorem bit0_injective : Function.Injective (bit0 : R → R) :=
  fun a b h =>
    by 
      dsimp [bit0]  at h 
...
theorem bit1_injective : Function.Injective (bit1 : R → R) :=
  fun a b h =>
    by 
...
@[simp]
theorem bit1_eq_one {a : R} : bit1 a = 1 ↔ a = 0 :=
  by 
    rw
      [show (1 : R) = bit1 0 by 
        simp ,
...
@[simp]
theorem add_halves' (a : R) : ((a / 2)+a / 2) = a :=
  by 
    rw [←add_div, half_add_self]

theorem sub_half (a : R) : a - a / 2 = a / 2 :=
  by 
    rw [sub_eq_iff_eq_add, add_halves']

theorem half_sub (a : R) : a / 2 - a = -(a / 2) :=
  by 
    rw [←neg_sub, sub_half]
