...
/-- The canonical ring isomorphism between `⨁ i, A i` and `R`-/
@[simp]
theorem DirectSum.coe_ring_hom_of [AddMonoidₓ ι] [Semiringₓ R] (A : ι → AddSubmonoid R) [SetLike.GradedMonoid A] (i : ι)
    (x : A i) : DirectSum.coeRingHom A (DirectSum.of (fun i => A i) i x) = x :=
