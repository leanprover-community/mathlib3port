...
open Equivₓ Function Fintypeₓ Finsetₓ
...
instance {α : Type _} [Fintypeₓ α] [DecidableEq α] (i j : α) : DecidableRel (modSwap i j).R := fun σ τ => Or.decidable

theorem perm_inv_on_of_perm_on_finset {s : Finsetₓ α} {f : Perm α} (h : ∀ x ∈ s, f x ∈ s) {y : α} (hy : y ∈ s) :
    f⁻¹ y ∈ s := by
  have h0 : ∀ y ∈ s, ∃ (x : _)(hx : x ∈ s), y = (fun i (hi : i ∈ s) => f i) x hx :=
    Finsetₓ.surj_on_of_inj_on_of_card_le (fun x hx => (fun i hi => f i) x hx) (fun a ha => h a ha)
...
abbrev subtypePermOfFintype (f : Perm α) {p : α → Prop} [Fintypeₓ { x // p x }] (h : ∀ x, p x → p (f x)) :
    Perm { x // p x } :=
  f.subtypePerm fun x => ⟨h x, fun h₂ => f.inv_apply_self x ▸ perm_inv_on_of_perm_on_finite h h₂⟩

@[simp]
theorem subtype_perm_of_fintype_apply (f : Perm α) {p : α → Prop} [Fintypeₓ { x // p x }] (h : ∀ x, p x → p (f x))
    (x : { x // p x }) : subtypePermOfFintype f h x = ⟨f x, h x x.2⟩ :=
  rfl

@[simp]
theorem subtype_perm_of_fintype_one (p : α → Prop) [Fintypeₓ { x // p x }] (h : ∀ x, p x → p ((1 : Perm α) x)) :
...
def swapFactorsAux :
  ...
  | x::l => fun f h =>
    if hfx : x = f x then
      swap_factors_aux l f fun y hy => List.mem_of_ne_of_memₓ (fun h : y = x => by simpa [h, hfx.symm] using hy) (h hy)
    else
      let m :=
...
def swapFactors [Fintypeₓ α] [LinearOrderₓ α] (f : Perm α) : { l : List (Perm α) // l.Prod = f ∧ ∀ g ∈ l, IsSwap g } :=
...
def truncSwapFactors [Fintypeₓ α] (f : Perm α) : Trunc { l : List (Perm α) // l.Prod = f ∧ ∀ g ∈ l, IsSwap g } :=
...
def finPairsLt (n : ℕ) : Finsetₓ (Σa : Finₓ n, Finₓ n) :=
  (univ : Finsetₓ (Finₓ n)).Sigma fun a => (range a).attachFin fun m hm => (mem_range.1 hm).trans a.2
...
theorem sign_aux_one (n : ℕ) : signAux (1 : Perm (Finₓ n)) = 1 := by
  ...
  conv =>
  rhs
  rw [← @Finsetₓ.prod_const_one ℤˣ _ (fin_pairs_lt n)]
  exact Finsetₓ.prod_congr rfl fun a ha => if_neg (mem_fin_pairs_lt.1 ha).not_le
...
theorem sign_bij_aux_surj {n : ℕ} {f : Perm (Finₓ n)} : ∀ a ∈ finPairsLt n, ∃ b ∈ finPairsLt n, a = signBijAux f b :=
  fun ⟨a₁, a₂⟩ ha =>
  if hxa : f⁻¹ a₂ < f⁻¹ a₁ then
    ...
    ⟨⟨f⁻¹ a₂, f⁻¹ a₁⟩,
      mem_fin_pairs_lt.2 <|
        (le_of_not_gtₓ hxa).lt_of_ne fun h => by simpa [mem_fin_pairs_lt, f⁻¹.Injective h, lt_irreflₓ] using ha,
      by
      dsimp [sign_bij_aux]
...
private theorem sign_aux_swap_zero_one' (n : ℕ) : signAux (swap (0 : Finₓ (n + 2)) 1) = -1 :=
  show
    ...
    refine'
      Eq.symm
        (prod_subset (fun ⟨x₁, x₂⟩ => by simp (config := { contextual := true }) [mem_fin_pairs_lt, Finₓ.one_pos])
          fun a ha₁ ha₂ => _)
    rcases a with ⟨a₁, a₂⟩
...
private theorem sign_aux_swap_zero_one {n : ℕ} (hn : 2 ≤ n) :
    signAux (swap (⟨0, lt_of_lt_of_leₓ (by decide) hn⟩ : Finₓ n) ⟨1, lt_of_lt_of_leₓ (by decide) hn⟩) = -1 := by
  rcases n with (_ | _ | n)
  · norm_num at hn
    ...
    · rw [if_pos hfx]
      exact
        sign_aux_eq_sign_aux2 l f _ fun y (hy : f y ≠ y) =>
          List.mem_of_ne_of_memₓ (fun h : y = x => by simpa [h, hfx.symm] using hy) (h y hy)
...
def signAux3 [Fintypeₓ α] (f : Perm α) {s : Multiset α} : (∀ x, x ∈ s) → ℤˣ :=
  Quotientₓ.hrecOn s (fun l h => signAux2 l f)
    (Trunc.induction_on (Fintypeₓ.truncEquivFin α) fun e l₁ l₂ h =>
...
theorem sign_aux3_mul_and_swap [Fintypeₓ α] (f g : Perm α) (s : Multiset α) (hs : ∀ x, x ∈ s) :
  ...
  have hfg : (e.symm.trans (f * g)).trans e = (e.symm.trans f).trans e * (e.symm.trans g).trans e :=
    Equivₓ.ext fun h => by simp [mul_apply]
  constructor
...
theorem sign_aux3_symm_trans_trans [DecidableEq β] [Fintypeₓ β] (f : Perm α) (e : α ≃ β) {s : Multiset α}
  ...
  Quotientₓ.induction_on₂ t s
    (fun l₁ l₂ h₁ h₂ =>
      show signAux2 _ _ = signAux2 _ _ by
        ...
        exact
          congr_arg sign_aux (Equivₓ.ext fun x => by simp only [Equivₓ.coe_trans, apply_eq_iff_eq, symm_trans_apply]))
    ht hs

@[simp]
theorem sign_symm_trans_trans [DecidableEq β] [Fintypeₓ β] (f : Perm α) (e : α ≃ β) :
    sign ((e.symm.trans f).trans e) = sign f :=
  sign_aux3_symm_trans_trans f e mem_univ mem_univ

@[simp]
theorem sign_trans_trans_symm [DecidableEq β] [Fintypeₓ β] (f : Perm β) (e : α ≃ β) :
...
theorem sign_eq_sign_of_equiv [DecidableEq β] [Fintypeₓ β] (f : Perm α) (g : Perm β) (e : α ≃ β)
    (h : ∀ x, e (f x) = g (e x)) : sign f = sign g := by
  have hg : g = (e.symm.trans f).trans e := Equivₓ.ext <| by simp [h]
  rw [hg, sign_symm_trans_trans]

theorem sign_bij [DecidableEq β] [Fintypeₓ β] {f : Perm α} {g : Perm β} (i : ∀ x : α, f x ≠ x → β)
...
variable [DecidableEq β] [Fintypeₓ β]
...
theorem sign_prod_congr_right (σ : α → Perm β) : sign (prodCongrRight σ) = ∏ k, (σ k).sign := by
  ...
  have l_to_finset : l.to_finset = Finsetₓ.univ := by
...
theorem sign_extend_domain (e : Perm α) {p : β → Prop} [DecidablePred p] (f : α ≃ Subtype p) :
  ...
  simp only [Equivₓ.Perm.extendDomain, sign_subtype_congr, sign_perm_congr, sign_refl, mul_oneₓ]

@[simp]
theorem sign_of_subtype {p : α → Prop} [DecidablePred p] (f : Equivₓ.Perm (Subtype p)) :
    Equivₓ.Perm.sign f.ofSubtype = Equivₓ.Perm.sign f :=
  sign_extend_domain f (Equivₓ.refl (Subtype p))
