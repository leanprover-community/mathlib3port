...
theorem disjoint.symmetric : Symmetric (@Disjoint α) :=
  fun _ _ => Disjoint.symm
...
@[simp]
theorem disjoint_one_left (f : perm α) : Disjoint 1 f :=
  fun _ => Or.inl rfl
...
@[simp]
theorem disjoint_inv_right_iff : Disjoint f (g⁻¹) ↔ Disjoint f g :=
  by 
    rw [disjoint_comm, disjoint_inv_left_iff, disjoint_comm]

theorem disjoint.mul_left (H1 : Disjoint f h) (H2 : Disjoint g h) : Disjoint (f*g) h :=
  fun x =>
    by 
...
theorem disjoint.mul_eq_one_iff {σ τ : perm α} (hστ : Disjoint σ τ) : (σ*τ) = 1 ↔ σ = 1 ∧ τ = 1 :=
  by 
    simpRw [ext_iff, one_apply, hστ.mul_apply_eq_iff, forall_and_distrib]

theorem disjoint.zpow_disjoint_zpow {σ τ : perm α} (hστ : Disjoint σ τ) (m n : ℤ) : Disjoint (σ ^ m) (τ ^ n) :=
  fun x =>
...
@[simp]
theorem mem_support {x : α} : x ∈ f.support ↔ f x ≠ x :=
  by 
    rw [support, mem_filter, and_iff_right (mem_univ x)]

theorem not_mem_support {x : α} : x ∉ f.support ↔ f x = x :=
  by 
    simp 
...
theorem support_mul_le (f g : perm α) : (f*g).support ≤ f.support⊔g.support :=
  fun x =>
    by 
...
theorem exists_mem_support_of_mem_support_prod {l : List (perm α)} {x : α} (hx : x ∈ l.prod.support) :
  ...
  by 
    contrapose! hx 
    simpRw [mem_support, not_not]  at hx⊢
...
theorem support_pow_le (σ : perm α) (n : ℕ) : (σ ^ n).support ≤ σ.support :=
  fun x h1 => mem_support.mpr fun h2 => mem_support.mp h1 (pow_apply_eq_self_of_apply_eq_self h2 n)

@[simp]
theorem support_inv (σ : perm α) : support (σ⁻¹) = σ.support :=
  by 
    simpRw [Finset.ext_iff, mem_support, not_iff_not, inv_eq_iff_eq.trans eq_comm, iff_selfₓ, imp_true_iff]

@[simp]
theorem apply_mem_support {x : α} : f x ∈ f.support ↔ x ∈ f.support :=
  by 
...
theorem disjoint_iff_disjoint_support : Disjoint f g ↔ _root_.disjoint f.support g.support :=
  by 
...
theorem support_prod_le (l : List (perm α)) : l.prod.support ≤ (l.map support).foldr (·⊔·) ⊥ :=
  by 
    ...
    ·
      simp 
    ·
      rw [List.prod_cons, List.map_consₓ, List.foldr_cons]
...
theorem support_zpow_le (σ : perm α) (n : ℤ) : (σ ^ n).support ≤ σ.support :=
  fun x h1 => mem_support.mpr fun h2 => mem_support.mp h1 (zpow_apply_eq_self_of_apply_eq_self h2 n)
...
theorem support_swap_mul_swap {x y z : α} (h : List.Nodup [x, y, z]) : support (swap x y*swap y z) = {x, y, z} :=
  by 
    simp only [List.not_mem_nil, and_trueₓ, List.mem_cons_iffₓ, not_false_iff, List.nodup_cons, List.mem_singleton,
...
@[simp]
theorem card_support_le_one {f : perm α} : f.support.card ≤ 1 ↔ f = 1 :=
  by 
    rw [le_iff_lt_or_eqₓ, Nat.lt_succ_iff, Nat.le_zero_iff, card_support_eq_zero, or_iff_not_imp_right,
...
theorem card_support_swap_mul {f : perm α} {x : α} (hx : f x ≠ x) : (swap x (f x)*f).support.card < f.support.card :=
  Finset.card_lt_card
    ⟨fun z hz => (mem_support_swap_mul_imp_mem_support_ne hz).left,
      fun h =>
...
theorem card_support_prod_list_of_pairwise_disjoint {l : List (perm α)} (h : l.pairwise Disjoint) :
  ...
  by 
    ...
    ·
      obtain ⟨ha, ht⟩ := List.pairwise_consₓ.1 h 
      rw [List.prod_cons, List.map_consₓ, List.sum_cons, ←ih ht]
