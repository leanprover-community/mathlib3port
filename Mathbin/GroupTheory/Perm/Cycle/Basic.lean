    ...
    let ⟨i, hi⟩ :=
      hx.2 y
        (by
          simp_all only [inv_eq_iff_eq, forall_prop_of_true, Ne.def]
          cc)
    ⟨-i, by rwa [zpow_neg, inv_zpow, inv_invₓ]⟩⟩
  ...
  Equivₓ.ofBijective
    (fun τ =>
      ⟨τ (Classical.choose hσ), by
        obtain ⟨τ, n, rfl⟩ := τ
        rw [Finsetₓ.mem_coe, coe_fn_coe_base', Subtype.coe_mk, zpow_apply_mem_support, mem_support]
    ...
    (by
      ...
      · rintro ⟨y, hy⟩
        rw [Finsetₓ.mem_coe, mem_support] at hy
...
theorem order_of_is_cycle {σ : Perm α} (hσ : IsCycle σ) : orderOf σ = σ.support.card := by
  rw [order_eq_card_zpowers, ← Fintypeₓ.card_coe]
  convert Fintypeₓ.card_congr (is_cycle.zpowers_equiv_support hσ)
    ...
    calc
      ...
      _ = -(-1 ^ f.support.card) :=
        if h1 : f (f x) = x then by
          ...
          simp only [pow_addₓ, mul_oneₓ, neg_negₓ, one_mulₓ, mul_neg, eq_self_iff_true, pow_oneₓ, neg_mul_neg]
...
theorem SameCycle.nat'' [Finite β] {f : Perm β} {x y : β} (h : SameCycle f x y) :
  ...
  · refine' ⟨orderOf f, order_of_pos f, le_rflₓ, _⟩
    rw [pow_order_of_eq_one, pow_zeroₓ]
    
  · exact ⟨i.succ, i.zero_lt_succ, hi.le, rfl⟩
    

instance [Fintypeₓ α] (f : Perm α) : DecidableRel (SameCycle f) := fun x y =>
  decidableOfIff (∃ n ∈ List.range (Fintypeₓ.card (Perm α)), (f ^ n) x = y)
...
theorem same_cycle_inv (f : Perm β) {x y : β} : SameCycle f⁻¹ x y ↔ SameCycle f x y :=
  ⟨fun ⟨i, hi⟩ => ⟨-i, by rw [zpow_neg, ← inv_zpow, hi]⟩, fun ⟨i, hi⟩ =>
    ⟨-i, by rw [zpow_neg, ← inv_zpow, inv_invₓ, hi]⟩⟩
...
theorem IsCycle.support_congr [Fintypeₓ α] {f g : Perm α} (hf : IsCycle f) (hg : IsCycle g) (h : f.support ⊆ g.support)
...
theorem IsCycle.eq_on_support_inter_nonempty_congr [Fintypeₓ α] {f g : Perm α} (hf : IsCycle f) (hg : IsCycle g)
...
theorem IsCycle.support_pow_eq_iff [Fintypeₓ α] {f : Perm α} (hf : IsCycle f) {n : ℕ} :
...
theorem IsCycle.mem_support_pos_pow_iff_of_lt_order_of [Fintypeₓ α] {f : Perm α} (hf : IsCycle f) {n : ℕ} (npos : 0 < n)
...
def cycleOf [Fintypeₓ α] (f : Perm α) (x : α) : Perm α :=
  ofSubtype (@subtypePerm _ f (SameCycle f x) fun _ => same_cycle_apply.symm)

theorem cycle_of_apply [Fintypeₓ α] (f : Perm α) (x y : α) : cycleOf f x y = if SameCycle f x y then f y else y := by
  dsimp only [cycle_of]
  split_ifs
...
theorem cycle_of_inv [Fintypeₓ α] (f : Perm α) (x : α) : (cycleOf f x)⁻¹ = cycleOf f⁻¹ x :=
  Equivₓ.ext fun y => by
    rw [inv_eq_iff_eq, cycle_of_apply, cycle_of_apply]
    split_ifs <;> simp_all [same_cycle_inv, same_cycle_inv_apply]

@[simp]
theorem cycle_of_pow_apply_self [Fintypeₓ α] (f : Perm α) (x : α) : ∀ n : ℕ, (cycleOf f x ^ n) x = (f ^ n) x
  | 0 => rfl
  | n + 1 => by
...
@[simp]
theorem cycle_of_zpow_apply_self [Fintypeₓ α] (f : Perm α) (x : α) : ∀ n : ℤ, (cycleOf f x ^ n) x = (f ^ n) x
...
theorem SameCycle.cycle_of_apply [Fintypeₓ α] {f : Perm α} {x y : α} (h : SameCycle f x y) : cycleOf f x y = f y := by
  apply of_subtype_apply_of_mem
  exact h

theorem cycle_of_apply_of_not_same_cycle [Fintypeₓ α] {f : Perm α} {x y : α} (h : ¬SameCycle f x y) :
    cycleOf f x y = y := by
  apply of_subtype_apply_of_not_mem
  exact h

theorem SameCycle.cycle_of_eq [Fintypeₓ α] {f : Perm α} {x y : α} (h : SameCycle f x y) : cycleOf f x = cycleOf f y :=
  by
  ext z
  rw [cycle_of_apply]
...
@[simp]
theorem cycle_of_apply_apply_zpow_self [Fintypeₓ α] (f : Perm α) (x : α) (k : ℤ) :
...
@[simp]
theorem cycle_of_apply_apply_pow_self [Fintypeₓ α] (f : Perm α) (x : α) (k : ℕ) :
    cycleOf f x ((f ^ k) x) = (f ^ (k + 1)) x := by convert cycle_of_apply_apply_zpow_self f x k using 1

@[simp]
theorem cycle_of_apply_apply_self [Fintypeₓ α] (f : Perm α) (x : α) : cycleOf f x (f x) = f (f x) := by
  convert cycle_of_apply_apply_pow_self f x 1 using 1

@[simp]
theorem cycle_of_apply_self [Fintypeₓ α] (f : Perm α) (x : α) : cycleOf f x x = f x :=
  (SameCycle.refl _ _).cycle_of_apply

theorem IsCycle.cycle_of_eq [Fintypeₓ α] {f : Perm α} (hf : IsCycle f) {x : α} (hx : f x ≠ x) : cycleOf f x = f :=
  Equivₓ.ext fun y =>
...
@[simp]
theorem cycle_of_eq_one_iff [Fintypeₓ α] (f : Perm α) {x : α} : cycleOf f x = 1 ↔ f x = x := by
...
@[simp]
theorem cycle_of_self_apply [Fintypeₓ α] (f : Perm α) (x : α) : cycleOf f (f x) = cycleOf f x :=
  (same_cycle_apply.mpr (SameCycle.refl _ _)).symm.cycle_of_eq

@[simp]
theorem cycle_of_self_apply_pow [Fintypeₓ α] (f : Perm α) (n : ℕ) (x : α) : cycleOf f ((f ^ n) x) = cycleOf f x :=
  (same_cycle_pow_left_iff.mpr (SameCycle.refl _ _)).cycle_of_eq

@[simp]
theorem cycle_of_self_apply_zpow [Fintypeₓ α] (f : Perm α) (n : ℤ) (x : α) : cycleOf f ((f ^ n) x) = cycleOf f x :=
  (same_cycle_zpow_left_iff.mpr (SameCycle.refl _ _)).cycle_of_eq

theorem IsCycle.cycle_of [Fintypeₓ α] {f : Perm α} (hf : IsCycle f) {x : α} : cycleOf f x = if f x = x then 1 else f :=
  by
  by_cases hx:f x = x
...
theorem cycle_of_one [Fintypeₓ α] (x : α) : cycleOf 1 x = 1 :=
  (cycle_of_eq_one_iff 1).mpr rfl

theorem is_cycle_cycle_of [Fintypeₓ α] (f : Perm α) {x : α} (hx : f x ≠ x) : IsCycle (cycleOf f x) :=
...
@[simp]
theorem two_le_card_support_cycle_of_iff [Fintypeₓ α] {f : Perm α} {x : α} : 2 ≤ card (cycleOf f x).support ↔ f x ≠ x :=
  by
...
@[simp]
theorem card_support_cycle_of_pos_iff [Fintypeₓ α] {f : Perm α} {x : α} : 0 < card (cycleOf f x).support ↔ f x ≠ x := by
...
theorem pow_apply_eq_pow_mod_order_of_cycle_of_apply [Fintypeₓ α] (f : Perm α) (n : ℕ) (x : α) :
    (f ^ n) x = (f ^ (n % orderOf (cycleOf f x))) x := by
  rw [← cycle_of_pow_apply_self f, ← cycle_of_pow_apply_self f, pow_eq_mod_order_of]

theorem cycle_of_mul_of_apply_right_eq_self [Fintypeₓ α] {f g : Perm α} (h : Commute f g) (x : α) (hx : g x = x) :
...
theorem Disjoint.cycle_of_mul_distrib [Fintypeₓ α] {f g : Perm α} (h : f.Disjoint g) (x : α) :
...
theorem support_cycle_of_eq_nil_iff [Fintypeₓ α] {f : Perm α} {x : α} : (f.cycleOf x).support = ∅ ↔ x ∉ f.support := by
  simp

theorem support_cycle_of_le [Fintypeₓ α] (f : Perm α) (x : α) : support (f.cycleOf x) ≤ support f := by
  intro y hy
...
theorem mem_support_cycle_of_iff [Fintypeₓ α] {f : Perm α} {x y : α} :
...
theorem SameCycle.mem_support_iff [Fintypeₓ α] {f : Perm α} {x y : α} (h : SameCycle f x y) :
...
theorem pow_mod_card_support_cycle_of_self_apply [Fintypeₓ α] (f : Perm α) (n : ℕ) (x : α) :
...
theorem is_cycle_cycle_of_iff [Fintypeₓ α] (f : Perm α) {x : α} : IsCycle (cycleOf f x) ↔ f x ≠ x := by
  constructor
  · intro hx
    rw [Ne.def]
...
def cycleFactorsAux [Fintypeₓ α] :
    ∀ (l : List α) (f : Perm α),
  ...
  | [], f, h =>
    ⟨[], by
      ...
      simp [*]⟩
  | x::l, f, h =>
    if hx : f x = x then cycle_factors_aux l f fun y hy => List.mem_of_ne_of_memₓ (fun h => hy (by rwa [h])) (h hy)
    else
      let ⟨m, hm₁, hm₂, hm₃⟩ :=
...
variable [Fintypeₓ α] (f : Perm α)

/-- Factors a permutation `f` into a `finset` of disjoint cyclic permutations that multiply to `f`.
-/
def cycleFactorsFinset : Finsetₓ (Perm α) :=
...
theorem cycle_factors_finset_eq_list_to_finset {σ : Perm α} {l : List (Perm α)} (hn : l.Nodup) :
  ...
  · intro h
    ...
    · rwa [List.Perm.pairwise_iffₓ disjoint.symmetric hperm]
...
theorem cycle_factors_finset_eq_finset {σ : Perm α} {s : Finsetₓ (Perm α)} :
...
theorem cycle_factors_finset_mul_inv_mem_eq_sdiff [Fintypeₓ α] {f g : Perm α} (h : f ∈ cycleFactorsFinset g) :
...
theorem SameCycle.nat_of_mem_support [Fintypeₓ α] (f : Perm α) {x y : α} (h : SameCycle f x y) (hx : x ∈ f.support) :
...
theorem SameCycle.nat [Fintypeₓ α] (f : Perm α) {x y : α} (h : SameCycle f x y) :
  ...
  · obtain ⟨k, hk, hk'⟩ := same_cycle.nat_of_mem_support f h hx
    ...
    · exact ⟨k + 1, by simp, Nat.le_succ_of_leₓ hk.le, hk'⟩
...
theorem closure_is_cycle : closure { σ : Perm β | IsCycle σ } = ⊤ := by
  classical
  cases nonempty_fintype β
  exact top_le_iff.mp (le_transₓ (ge_of_eqₓ closure_is_swap) (closure_mono fun _ => is_swap.is_cycle))
...
theorem closure_cycle_adjacent_swap {σ : Perm α} (h1 : IsCycle σ) (h2 : σ.support = ⊤) (x : α) :
  ...
  have step3 : ∀ y : α, swap x y ∈ H := by
    intro y
    have hx : x ∈ (⊤ : Finsetₓ α) := Finsetₓ.mem_univ x
    rw [← h2, mem_support] at hx
    have hy : y ∈ (⊤ : Finsetₓ α) := Finsetₓ.mem_univ y
...
theorem closure_cycle_coprime_swap {n : ℕ} {σ : Perm α} (h0 : Nat.Coprime n (Fintypeₓ.card α)) (h1 : IsCycle σ)
    (h2 : σ.support = Finsetₓ.univ) (x : α) : closure ({σ, swap x ((σ ^ n) x)} : Set (Perm α)) = ⊤ := by
  rw [← Finsetₓ.card_univ, ← h2, ← order_of_is_cycle h1] at h0
...
theorem closure_prime_cycle_swap {σ τ : Perm α} (h0 : (Fintypeₓ.card α).Prime) (h1 : IsCycle σ)
    (h2 : σ.support = Finsetₓ.univ) (h3 : IsSwap τ) : closure ({σ, τ} : Set (Perm α)) = ⊤ := by
  obtain ⟨x, y, h4, h5⟩ := h3
  obtain ⟨i, hi⟩ :=
    h1.exists_pow_eq (mem_support.mp ((finset.ext_iff.mp h2 x).mpr (Finsetₓ.mem_univ x)))
      (mem_support.mp ((finset.ext_iff.mp h2 y).mpr (Finsetₓ.mem_univ y)))
  rw [h5, ← hi]
  refine' closure_cycle_coprime_swap (Nat.Coprime.symm (h0.coprime_iff_not_dvd.mpr fun h => h4 _)) h1 h2 x
  cases' h with m hm
  rwa [hm, pow_mulₓ, ← Finsetₓ.card_univ, ← h2, ← order_of_is_cycle h1, pow_order_of_eq_one, one_pow, one_apply] at hi

end Generation

section

variable [Fintypeₓ α] {σ τ : Perm α}
...
theorem is_conj_of_support_equiv (f : { x // x ∈ (σ.support : Set α) } ≃ { x // x ∈ (τ.support : Set α) })
  ...
  · rw [Equivₓ.extend_subtype_apply_of_mem, Equivₓ.extend_subtype_apply_of_mem]
    · exact hf x (Finsetₓ.mem_coe.2 hx)
...
theorem IsCycle.is_conj_iff (hσ : IsCycle σ) (hτ : IsCycle τ) : IsConj σ τ ↔ σ.support.card = τ.support.card :=
  ⟨by
    ...
    apply Finsetₓ.card_congr (fun a ha => π a) (fun _ ha => _) (fun _ _ _ _ ab => π.injective ab) fun b hb => _
...
theorem fixed_point_card_lt_of_ne_one [Fintypeₓ α] {σ : Perm α} (h : σ ≠ 1) :
    (filter (fun x => σ x = x) univ).card < Fintypeₓ.card α - 1 := by
  rw [lt_tsub_iff_left, ← lt_tsub_iff_right, ← Finsetₓ.card_compl, Finsetₓ.compl_filter]
