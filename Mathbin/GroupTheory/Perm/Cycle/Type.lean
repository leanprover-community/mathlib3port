...
def cycleType (σ : Perm α) : Multiset ℕ :=
  σ.cycleFactorsFinset.1.map (Finsetₓ.card ∘ support)

theorem cycle_type_def (σ : Perm α) : σ.cycleType = σ.cycleFactorsFinset.1.map (Finsetₓ.card ∘ support) :=
  rfl

theorem cycle_type_eq' {σ : Perm α} (s : Finsetₓ (Perm α)) (h1 : ∀ f : Perm α, f ∈ s → f.IsCycle)
    ...
    (h0 :
      (s.noncommProd id fun a ha b hb =>
        ...
        σ) :
    σ.cycleType = s.1.map (Finsetₓ.card ∘ support) := by
  rw [cycle_type_def]
  congr
...
theorem cycle_type_eq {σ : Perm α} (l : List (Perm α)) (h0 : l.Prod = σ) (h1 : ∀ σ : Perm α, σ ∈ l → σ.IsCycle)
    (h2 : l.Pairwise Disjoint) : σ.cycleType = l.map (Finsetₓ.card ∘ support) := by
...
theorem card_cycle_type_eq_zero {σ : Perm α} : σ.cycleType.card = 0 ↔ σ = 1 := by rw [card_eq_zero, cycle_type_eq_zero]

theorem two_le_of_mem_cycle_type {σ : Perm α} {n : ℕ} (h : n ∈ σ.cycleType) : 2 ≤ n := by
  simp only [cycle_type_def, ← Finsetₓ.mem_def, Function.comp_app, Multiset.mem_map, mem_cycle_factors_finset_iff] at h
  obtain ⟨_, ⟨hc, -⟩, rfl⟩ := h
...
theorem card_cycle_type_eq_one {σ : Perm α} : σ.cycleType.card = 1 ↔ σ.IsCycle := by
  rw [card_eq_one]
  simp_rw [cycle_type_def, Multiset.map_eq_singleton, ← Finsetₓ.singleton_val, Finsetₓ.val_inj,
...
theorem Disjoint.cycle_type {σ τ : Perm α} (h : Disjoint σ τ) : (σ * τ).cycleType = σ.cycleType + τ.cycleType := by
  rw [cycle_type_def, cycle_type_def, cycle_type_def, h.cycle_factors_finset_mul_eq_union, ← Multiset.map_add,
    Finsetₓ.union_val, multiset.add_eq_union_iff_disjoint.mpr _]
  rw [← Finsetₓ.disjoint_val]
...
theorem sum_cycle_type (σ : Perm α) : σ.cycleType.Sum = σ.support.card :=
  cycle_induction_on (fun τ : Perm α => τ.cycleType.Sum = τ.support.card) σ
    (by rw [cycle_type_one, sum_zero, support_one, Finsetₓ.card_empty])
...
theorem sign_of_cycle_type' (σ : Perm α) : sign σ = (σ.cycleType.map fun n => -((-1 : ℤˣ) ^ n)).Prod :=
  cycle_induction_on (fun τ : Perm α => sign τ = (τ.cycleType.map fun n => -((-1 : ℤˣ) ^ n)).Prod) σ
    ...
    (fun σ hσ => by rw [hσ.sign, hσ.cycle_type, coe_map, coe_prod, List.map_singletonₓ, List.prod_singleton])
...
theorem sign_of_cycle_type (f : Perm α) : sign f = (-1 : ℤˣ) ^ (f.cycleType.Sum + f.cycleType.card) :=
  cycle_induction_on (fun f : Perm α => sign f = (-1 : ℤˣ) ^ (f.cycleType.Sum + f.cycleType.card)) f
    (-- base_one
    by rw [Equivₓ.Perm.cycle_type_one, sign_one, Multiset.sum_zero, Multiset.card_zero, pow_zeroₓ])
    (-- base_cycles
    fun f hf => by
...
theorem lcm_cycle_type (σ : Perm α) : σ.cycleType.lcm = orderOf σ :=
  cycle_induction_on (fun τ : Perm α => τ.cycleType.lcm = orderOf τ) σ (by rw [cycle_type_one, lcm_zero, order_of_one])
    (fun σ hσ => by rw [hσ.cycle_type, coe_singleton, lcm_singleton, order_of_is_cycle hσ, normalize_eq])
...
theorem order_of_cycle_of_dvd_order_of (f : Perm α) (x : α) : orderOf (cycleOf f x) ∣ orderOf f := by
  ...
  · refine' dvd_of_mem_cycle_type _
    ...
    · rwa [← Finsetₓ.mem_def, cycle_of_mem_cycle_factors_finset_iff, mem_support]
...
theorem two_dvd_card_support {σ : Perm α} (hσ : σ ^ 2 = 1) : 2 ∣ σ.support.card :=
  (congr_arg (Dvd.Dvd 2) σ.sum_cycle_type).mp
    (Multiset.dvd_sum fun n hn => by
      rw [le_antisymmₓ (Nat.le_of_dvdₓ zero_lt_two <| (dvd_of_mem_cycle_type hn).trans <| order_of_dvd_of_pow_eq_one hσ)
...
theorem cycle_type_le_of_mem_cycle_factors_finset {f g : Perm α} (hf : f ∈ g.cycleFactorsFinset) :
  ...
  refine' map_le_map _
  simpa [← Finsetₓ.mem_def, mem_cycle_factors_finset_iff] using hf
...
theorem is_conj_of_cycle_type_eq {σ τ : Perm α} (h : cycleType σ = cycleType τ) : IsConj σ τ := by
  ...
  · intro σ τ hστ hσ h1 h2 π hπ
    ...
    · have h : σ.support.card ∈ map (Finsetₓ.card ∘ perm.support) π.cycle_factors_finset.val := by
      ...
      have hs : σ.cycle_type = σ'.cycle_type := by
        rw [← Finsetₓ.mem_def, mem_cycle_factors_finset_iff] at hσ'l
...
@[simp]
theorem cycle_type_extend_domain {β : Type _} [Fintypeₓ β] [DecidableEq β] {p : β → Prop} [DecidablePred p]
...
theorem cycle_type_of_subtype {p : α → Prop} [DecidablePred p] {g : Perm (Subtype p)} :
  ...
  cycle_type_extend_domain (Equivₓ.refl (Subtype p))
...
theorem cycle_type_of_card_le_mem_cycle_type_add_two {n : ℕ} {g : Perm α} (hn2 : Fintypeₓ.card α < n + 2)
...
theorem card_compl_support_modeq [DecidableEq α] {p n : ℕ} [hp : Fact p.Prime] {σ : Perm α} (hσ : σ ^ p ^ n = 1) :
    σ.supportᶜ.card ≡ Fintypeₓ.card α [MOD p] := by
  rw [Nat.modeq_iff_dvd' σ.supportᶜ.card_le_univ, ← Finsetₓ.card_compl, compl_compl]
  ...
  exact dvd_pow_self _ fun h => (one_lt_of_mem_cycle_type hk).Ne <| by rw [h, pow_zeroₓ]

theorem exists_fixed_point_of_prime {p n : ℕ} [hp : Fact p.Prime] (hα : ¬p ∣ Fintypeₓ.card α) {σ : Perm α}
...
theorem exists_fixed_point_of_prime' {p n : ℕ} [hp : Fact p.Prime] (hα : p ∣ Fintypeₓ.card α) {σ : Perm α}
  ...
  classical
  have h : ∀ b : α, b ∈ σ.supportᶜ ↔ σ b = b := fun b => by rw [Finsetₓ.mem_compl, mem_support, not_not]
  obtain ⟨b, hb1, hb2⟩ :=
    Finsetₓ.exists_ne_of_one_lt_card
...
theorem is_cycle_of_prime_order' {σ : Perm α} (h1 : (orderOf σ).Prime) (h2 : Fintypeₓ.card α < 2 * orderOf σ) :
    σ.IsCycle := by
  classical
  exact is_cycle_of_prime_order h1 (lt_of_le_of_ltₓ σ.support.card_le_univ h2)

theorem is_cycle_of_prime_order'' {σ : Perm α} (h1 : (Fintypeₓ.card α).Prime) (h2 : orderOf σ = Fintypeₓ.card α) :
    σ.IsCycle :=
  is_cycle_of_prime_order' ((congr_arg Nat.Prime h2).mpr h1)
    (by
      classical
      rw [← one_mulₓ (Fintypeₓ.card α), ← h2, mul_lt_mul_right (order_of_pos σ)]
      exact one_lt_two)
...
def vectorEquiv : Vector G n ≃ VectorsProdEqOne G (n + 1) where
  toFun := fun v => ⟨v.toList.Prod⁻¹ ::ᵥ v, by rw [mem_iff, Vector.to_list_cons, List.prod_cons, inv_mul_selfₓ]⟩
...
def equivVector : VectorsProdEqOne G n ≃ Vector G (n - 1) :=
  ((vectorEquiv G (n - 1)).trans
      ...
      else by rw [tsub_add_cancel_of_le (Nat.pos_of_ne_zeroₓ hn).nat_succ_le])).symm

instance [Fintypeₓ G] : Fintypeₓ (VectorsProdEqOne G n) :=
  Fintypeₓ.ofEquiv (Vector G (n - 1)) (equivVector G n).symm

theorem card [Fintypeₓ G] : Fintypeₓ.card (VectorsProdEqOne G n) = Fintypeₓ.card G ^ (n - 1) :=
  (Fintypeₓ.card_congr (equivVector G n)).trans (card_vector (n - 1))
...
theorem _root_.exists_prime_order_of_dvd_card {G : Type _} [Groupₓ G] [Fintypeₓ G] (p : ℕ) [hp : Fact p.Prime]
    (hdvd : p ∣ Fintypeₓ.card G) : ∃ x : G, orderOf x = p := by
  have hp' : p - 1 ≠ 0 := mt tsub_eq_zero_iff_le.mp (not_le_of_ltₓ hp.out.one_lt)
  have Scard :=
    calc
      p ∣ Fintypeₓ.card G ^ (p - 1) := hdvd.trans (dvd_pow (dvd_refl _) hp')
      _ = Fintypeₓ.card (vectors_prod_eq_one G p) := (vectors_prod_eq_one.card G p).symm
  ...
  let σ :=
    Equivₓ.mk (f 1) (f (p - 1)) (fun s => by rw [hf2, add_tsub_cancel_of_le hp.out.one_lt.le, hf3]) fun s => by
  ...
  replace hσ : σ ^ p ^ 1 = 1 := perm.ext fun v => by rw [pow_oneₓ, hσ, hf3, one_apply]
...
theorem _root_.exists_prime_add_order_of_dvd_card {G : Type _} [AddGroupₓ G] [Fintypeₓ G] (p : ℕ) [hp : Fact p.Prime]
    (hdvd : p ∣ Fintypeₓ.card G) : ∃ x : G, addOrderOf x = p :=
...
theorem subgroup_eq_top_of_swap_mem [DecidableEq α] {H : Subgroup (Perm α)} [d : DecidablePred (· ∈ H)] {τ : Perm α}
    (h0 : (Fintypeₓ.card α).Prime) (h1 : Fintypeₓ.card α ∣ Fintypeₓ.card H) (h2 : τ ∈ H) (h3 : IsSwap τ) : H = ⊤ := by
  haveI : Fact (Fintypeₓ.card α).Prime := ⟨h0⟩
  obtain ⟨σ, hσ⟩ := exists_prime_order_of_dvd_card (Fintypeₓ.card α) h1
  have hσ1 : orderOf (σ : perm α) = Fintypeₓ.card α := (order_of_subgroup σ).trans hσ
  have hσ2 : is_cycle ↑σ := is_cycle_of_prime_order'' h0 hσ1
  have hσ3 : (σ : perm α).support = ⊤ :=
    Finsetₓ.eq_univ_of_card (σ : perm α).support ((order_of_is_cycle hσ2).symm.trans hσ1)
...
def partition (σ : Perm α) : (Fintypeₓ.card α).partition where
  parts := σ.cycleType + repeat 1 (Fintypeₓ.card α - σ.support.card)
...
theorem parts_partition {σ : Perm α} : σ.partition.parts = σ.cycleType + repeat 1 (Fintypeₓ.card α - σ.support.card) :=
  rfl
...
theorem is_three_cycle_swap_mul_swap_same {a b c : α} (ab : a ≠ b) (ac : a ≠ c) (bc : b ≠ c) :
  ...
  · simp only [Finsetₓ.mem_insert, Finsetₓ.mem_singleton] at hx
    rw [mem_support]
