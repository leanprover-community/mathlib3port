...
theorem cycle_type_eq' {σ : perm α} (s : Finset (perm α)) (h1 : ∀ f : perm α, f ∈ s → f.is_cycle)
    ...
    (h0 :
      (s.noncomm_prod id fun a ha b hb =>
        ...
        σ) :
    σ.cycle_type = s.1.map (Finset.card ∘ support) := by
  rw [cycle_type_def]
  congr
...
theorem cycle_type_eq {σ : perm α} (l : List (perm α)) (h0 : l.prod = σ) (h1 : ∀ σ : perm α, σ ∈ l → σ.is_cycle)
    (h2 : l.pairwise Disjoint) : σ.cycle_type = l.map (Finset.card ∘ support) := by
...
theorem card_cycle_type_eq_zero {σ : perm α} : σ.cycle_type.card = 0 ↔ σ = 1 := by
  rw [card_eq_zero, cycle_type_eq_zero]

theorem two_le_of_mem_cycle_type {σ : perm α} {n : ℕ} (h : n ∈ σ.cycle_type) : 2 ≤ n := by
...
theorem card_cycle_type_eq_one {σ : perm α} : σ.cycle_type.card = 1 ↔ σ.is_cycle := by
  rw [card_eq_one]
...
theorem disjoint.cycle_type {σ τ : perm α} (h : Disjoint σ τ) : (σ*τ).cycleType = σ.cycle_type+τ.cycle_type := by
...
theorem cycle_type_conj {σ τ : perm α} : ((τ*σ)*τ⁻¹).cycleType = σ.cycle_type := by
  revert τ
...
theorem dvd_of_mem_cycle_type {σ : perm α} {n : ℕ} (h : n ∈ σ.cycle_type) : n ∣ orderOf σ := by
  rw [← lcm_cycle_type]
  exact dvd_lcm h

theorem order_of_cycle_of_dvd_order_of (f : perm α) (x : α) : orderOf (cycle_of f x) ∣ orderOf f := by
  by_cases' hx : f x = x
  ·
...
theorem cycle_type_prime_order {σ : perm α} (hσ : (orderOf σ).Prime) :
    ∃ n : ℕ, σ.cycle_type = repeat (orderOf σ) (n+1) := by
  rw
...
theorem cycle_type_mul_mem_cycle_factors_finset_eq_sub {f g : perm α} (hf : f ∈ g.cycle_factors_finset) :
    (g*f⁻¹).cycleType = g.cycle_type - f.cycle_type := by
...
theorem is_conj_of_cycle_type_eq {σ τ : perm α} (h : cycle_type σ = cycle_type τ) : IsConj σ τ := by
  revert τ
  apply cycle_induction_on _ σ
  ·
    intro τ h
...
theorem cycle_type_extend_domain {β : Type _} [Fintype β] [DecidableEq β] {p : β → Prop} [DecidablePred p]
    (f : α ≃ Subtype p) {g : perm α} : cycle_type (g.extend_domain f) = cycle_type g := by
...
theorem mem_cycle_type_iff {n : ℕ} {σ : perm α} :
    n ∈ cycle_type σ ↔ ∃ c τ : perm α, (σ = c*τ) ∧ Disjoint c τ ∧ is_cycle c ∧ c.support.card = n := by
  constructor
  ·
    intro h
...
theorem cycle_type_of_card_le_mem_cycle_type_add_two {n : ℕ} {g : perm α} (hn2 : Fintype.card α < n+2)
    (hng : n ∈ g.cycle_type) : g.cycle_type = {n} := by
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers
  [(Command.docComment "/--" " The type of vectors with terms from `G`, length `n`, and product equal to `1:G`. -/")]
  []
  []
  []
  []
 ...
 (Command.def
  "def"
  (Command.declId `vectors_prod_eq_one [])
  (Command.optDeclSig [] [(Term.typeSpec ":" (Term.app `Set [(Term.app `Vector [`G `n])]))])
  (Command.declValSimple ":=" (Set.«term{_|_}» "{" `v "|" («term_=_» `v.to_list.prod "=" (numLit "1")) "}") [])
  []
  []
  []))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Set.«term{_|_}» "{" `v "|" («term_=_» `v.to_list.prod "=" (numLit "1")) "}")
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Set.«term{_|_}»', expected 'antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  («term_=_» `v.to_list.prod "=" (numLit "1"))
...
/-- The type of vectors with terms from `G`, length `n`, and product equal to `1:G`. -/
  def vectors_prod_eq_one : Set Vector G n := { v | v.to_list.prod = 1 }
...
theorem one_eq : vectors_prod_eq_one G 1 = {Vector.nil.cons 1} := by
...
instance zero_unique : Unique (vectors_prod_eq_one G 0) := by
  rw [zero_eq]
  exact Set.uniqueSingleton Vector.nil

instance one_unique : Unique (vectors_prod_eq_one G 1) := by
  rw [one_eq]
...
theorem subgroup_eq_top_of_swap_mem [DecidableEq α] {H : Subgroup (perm α)} [d : DecidablePred (· ∈ H)] {τ : perm α}
    (h0 : (Fintype.card α).Prime) (h1 : Fintype.card α ∣ Fintype.card H) (h2 : τ ∈ H) (h3 : is_swap τ) : H = ⊤ := by
...
theorem filter_parts_partition_eq_cycle_type {σ : perm α} :
    ((partition σ).parts.filter fun n => 2 ≤ n) = σ.cycle_type := by
...
theorem card_support (h : is_three_cycle σ) : σ.support.card = 3 := by
  rw [← sum_cycle_type, h.cycle_type, Multiset.sum_singleton]

theorem _root_.card_support_eq_three_iff : σ.support.card = 3 ↔ σ.is_three_cycle := by
...
theorem is_cycle (h : is_three_cycle σ) : is_cycle σ := by
  rw [← card_cycle_type_eq_one, h.cycle_type, card_singleton]

theorem sign (h : is_three_cycle σ) : sign σ = 1 := by
...
theorem orderOf {g : perm α} (ht : is_three_cycle g) : orderOf g = 3 := by
  rw [← lcm_cycle_type, ht.cycle_type, Multiset.lcm_singleton, normalize_eq]

theorem is_three_cycle_sq {g : perm α} (ht : is_three_cycle g) : is_three_cycle (g*g) := by
...
theorem is_three_cycle_swap_mul_swap_same {a b c : α} (ab : a ≠ b) (ac : a ≠ c) (bc : b ≠ c) :
    is_three_cycle (swap a b*swap a c) := by
...
theorem swap_mul_swap_same_mem_closure_three_cycles {a b c : α} (ab : a ≠ b) (ac : a ≠ c) :
    (swap a b*swap a c) ∈ closure { σ : perm α | is_three_cycle σ } := by
  by_cases' bc : b = c
  ·
    subst bc
    simp [one_mem]
...
theorem is_swap.mul_mem_closure_three_cycles {σ τ : perm α} (hσ : is_swap σ) (hτ : is_swap τ) :
    (σ*τ) ∈ closure { σ : perm α | is_three_cycle σ } := by
