...
theorem sign_of_cycle_type (σ : perm α) : sign σ = (σ.cycle_type.map fun n => -((-1 : (ℤ)ˣ) ^ n)).Prod :=
  cycle_induction_on (fun τ : perm α => sign τ = (τ.cycle_type.map fun n => -((-1 : (ℤ)ˣ) ^ n)).Prod) σ
    (by
...
@[simps]
def vector_equiv : Vector G n ≃ vectors_prod_eq_one G (n + 1) where
  toFun := fun v =>
