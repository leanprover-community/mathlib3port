...
theorem is_cycle.ne_one {f : Perm β} (h : IsCycle f) : f ≠ 1 := fun hf => by
  simpa [hf, is_cycle] using h

@[simp]
theorem not_is_cycle_one : ¬(1 : Perm β).IsCycle := fun H => H.ne_one rfl

theorem is_cycle.two_le_card_support {f : Perm α} (h : IsCycle f) : 2 ≤ f.support.card :=
...
theorem is_swap.is_cycle {α : Type _} [DecidableEq α] {f : Perm α} (hf : IsSwap f) : IsCycle f := by
  obtain ⟨x, y, hxy, rfl⟩ := hf
  exact is_cycle_swap hxy

theorem is_cycle.inv {f : Perm β} (hf : IsCycle f) : IsCycle f⁻¹ :=
  let ⟨x, hx⟩ := hf
  ⟨x, by
...
theorem is_cycle.is_cycle_conj {f g : Perm β} (hf : IsCycle f) : IsCycle (g * f * g⁻¹) := by
  obtain ⟨a, ha1, ha2⟩ := hf
  refine'
    ⟨g a, by
      simp [ha1], fun b hb => _⟩
...
theorem is_cycle.exists_zpow_eq {f : Perm β} (hf : IsCycle f) {x y : β} (hx : f x ≠ x) (hy : f y ≠ y) :
    ∃ i : ℤ, (f ^ i) x = y :=
  let ⟨g, hg⟩ := hf
  let ⟨a, ha⟩ := hg.2 x hx
  let ⟨b, hb⟩ := hg.2 y hy
...
theorem is_cycle.exists_pow_eq [Fintype β] {f : Perm β} (hf : IsCycle f) {x y : β} (hx : f x ≠ x) (hy : f y ≠ y) :
    ∃ i : ℕ, (f ^ i) x = y := by
...
theorem is_cycle.exists_pow_eq_one [Fintype β] {f : Perm β} (hf : IsCycle f) : ∃ (k : ℕ)(hk : 1 < k), f ^ k = 1 := by
  classical
...
noncomputable def is_cycle.zpowers_equiv_support {σ : Perm α} (hσ : IsCycle σ) :
...
@[simp]
theorem is_cycle.zpowers_equiv_support_apply {σ : Perm α} (hσ : IsCycle σ) {n : ℕ} :
    hσ.zpowersEquivSupport ⟨σ ^ n, n, rfl⟩ =
      ⟨(σ ^ n) (Classical.some hσ), pow_apply_mem_support.2 (mem_support.2 (Classical.some_spec hσ).1)⟩ :=
  rfl

@[simp]
theorem is_cycle.zpowers_equiv_support_symm_apply {σ : Perm α} (hσ : IsCycle σ) (n : ℕ) :
...
theorem is_cycle.eq_swap_of_apply_apply_eq_self {α : Type _} [DecidableEq α] {f : Perm α} (hf : IsCycle f) {x : α}
...
theorem is_cycle.swap_mul {α : Type _} [DecidableEq α] {f : Perm α} (hf : IsCycle f) {x : α} (hx : f x ≠ x)
...
theorem is_cycle.sign : ∀ {f : Perm α} hf : IsCycle f, sign f = -(-1 ^ f.support.card)
  | f => fun hf =>
    let ⟨x, hx⟩ := hf
    calc
...
@[refl]
theorem same_cycle.refl (f : Perm β) (x : β) : SameCycle f x x :=
  ⟨0, rfl⟩

@[symm]
theorem same_cycle.symm {f : Perm β} {x y : β} : SameCycle f x y → SameCycle f y x := fun ⟨i, hi⟩ =>
  ⟨-i, by
    rw [zpow_neg, ← hi, inv_apply_self]⟩

@[trans]
theorem same_cycle.trans {f : Perm β} {x y z : β} : SameCycle f x y → SameCycle f y z → SameCycle f x z :=
  fun ⟨i, hi⟩ ⟨j, hj⟩ =>
  ⟨j + i, by
    rw [zpow_add, mul_apply, hi, hj]⟩

theorem same_cycle.apply_eq_self_iff {f : Perm β} {x y : β} : SameCycle f x y → (f x = x ↔ f y = y) := fun ⟨i, hi⟩ => by
  rw [← hi, ← mul_apply, ← zpow_one_add, add_commₓ, zpow_add_one, mul_apply, (f ^ i).Injective.eq_iff]

theorem is_cycle.same_cycle {f : Perm β} (hf : IsCycle f) {x y : β} (hx : f x ≠ x) (hy : f y ≠ y) : SameCycle f x y :=
  hf.exists_zpow_eq hx hy

theorem same_cycle.nat' [Fintype β] {f : Perm β} {x y : β} (h : SameCycle f x y) :
...
theorem is_cycle.support_congr [Fintype α] {f g : Perm α} (hf : IsCycle f) (hg : IsCycle g) (h : f.support ⊆ g.support)
...
theorem is_cycle.eq_on_support_inter_nonempty_congr [Fintype α] {f g : Perm α} (hf : IsCycle f) (hg : IsCycle g)
...
theorem is_cycle.support_pow_eq_iff [Fintype α] {f : Perm α} (hf : IsCycle f) {n : ℕ} :
...
theorem is_cycle.pow_iff [Fintype β] {f : Perm β} (hf : IsCycle f) {n : ℕ} : IsCycle (f ^ n) ↔ n.Coprime (orderOf f) :=
  by
  classical
  constructor
  · intro h
...
theorem is_cycle.pow_eq_one_iff [Fintype α] {f : Perm α} (hf : IsCycle f) {n : ℕ} :
...
theorem is_cycle.mem_support_pos_pow_iff_of_lt_order_of [Fintype α] {f : Perm α} (hf : IsCycle f) {n : ℕ} (npos : 0 < n)
...
theorem is_cycle.is_cycle_pow_pos_of_lt_prime_order [Fintype β] {f : Perm β} (hf : IsCycle f) (hf' : (orderOf f).Prime)
...
theorem same_cycle.cycle_of_apply [Fintype α] {f : Perm α} {x y : α} (h : SameCycle f x y) : cycleOf f x y = f y :=
  dif_pos h
...
theorem same_cycle.cycle_of_eq [Fintype α] {f : Perm α} {x y : α} (h : SameCycle f x y) : cycleOf f x = cycleOf f y :=
  by
  ext z
  rw [cycle_of_apply]
...
theorem is_cycle.cycle_of_eq [Fintype α] {f : Perm α} (hf : IsCycle f) {x : α} (hx : f x ≠ x) : cycleOf f x = f :=
  Equivₓ.ext fun y =>
...
theorem is_cycle.cycle_of [Fintype α] {f : Perm α} (hf : IsCycle f) {x : α} : cycleOf f x = if f x = x then 1 else f :=
  by
  by_cases' hx : f x = x
...
theorem same_cycle.mem_support_iff [Fintype α] {f : Perm α} {x y : α} (h : SameCycle f x y) :
...
theorem is_cycle.cycle_factors_finset_eq_singleton {f : Perm α} (hf : IsCycle f) : f.cycleFactorsFinset = {f} :=
...
theorem same_cycle.nat_of_mem_support [Fintype α] (f : Perm α) {x y : α} (h : SameCycle f x y) (hx : x ∈ f.support) :
...
theorem same_cycle.nat [Fintype α] (f : Perm α) {x y : α} (h : SameCycle f x y) :
...
theorem is_cycle.is_conj (hσ : IsCycle σ) (hτ : IsCycle τ) (h : σ.support.card = τ.support.card) : IsConj σ τ := by
  refine'
...
theorem is_cycle.is_conj_iff (hσ : IsCycle σ) (hτ : IsCycle τ) : IsConj σ τ ↔ σ.support.card = τ.support.card :=
  ⟨by
    intro h
