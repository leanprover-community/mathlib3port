...
instance sumCongrHom.decidableMemRange {α β : Type _} [DecidableEq α] [DecidableEq β] [Fintypeₓ α] [Fintypeₓ β] :
    DecidablePred (· ∈ (sumCongrHom α β).range) := fun x => inferInstance

@[simp]
theorem sumCongrHom.card_range {α β : Type _} [Fintypeₓ (sumCongrHom α β).range] [Fintypeₓ (Perm α × Perm β)] :
    Fintypeₓ.card (sumCongrHom α β).range = Fintypeₓ.card (Perm α × Perm β) :=
  Fintypeₓ.card_eq.mpr ⟨(ofInjective (sumCongrHom α β) sum_congr_hom_injective).symm⟩

instance sigmaCongrRightHom.decidableMemRange {α : Type _} {β : α → Type _} [DecidableEq α] [∀ a, DecidableEq (β a)]
    [Fintypeₓ α] [∀ a, Fintypeₓ (β a)] : DecidablePred (· ∈ (sigmaCongrRightHom β).range) := fun x => inferInstance

@[simp]
theorem sigmaCongrRightHom.card_range {α : Type _} {β : α → Type _} [Fintypeₓ (sigmaCongrRightHom β).range]
    [Fintypeₓ (∀ a, Perm (β a))] : Fintypeₓ.card (sigmaCongrRightHom β).range = Fintypeₓ.card (∀ a, Perm (β a)) :=
  Fintypeₓ.card_eq.mpr ⟨(ofInjective (sigmaCongrRightHom β) sigma_congr_right_hom_injective).symm⟩

instance subtypeCongrHom.decidableMemRange {α : Type _} (p : α → Prop) [DecidablePred p]
    [Fintypeₓ (Perm { a // p a } × Perm { a // ¬p a })] [DecidableEq (Perm α)] :
    DecidablePred (· ∈ (subtypeCongrHom p).range) := fun x => inferInstance

@[simp]
theorem subtypeCongrHom.card_range {α : Type _} (p : α → Prop) [DecidablePred p] [Fintypeₓ (subtypeCongrHom p).range]
    [Fintypeₓ (Perm { a // p a } × Perm { a // ¬p a })] :
    Fintypeₓ.card (subtypeCongrHom p).range = Fintypeₓ.card (Perm { a // p a } × Perm { a // ¬p a }) :=
  Fintypeₓ.card_eq.mpr ⟨(ofInjective (subtypeCongrHom p) (subtype_congr_hom_injective p)).symm⟩
