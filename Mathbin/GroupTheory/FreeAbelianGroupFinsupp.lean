...
theorem FreeAbelianGroup.to_finsupp_comp_to_free_abelian_group :
  to_finsupp.comp to_free_abelian_group = AddMonoidHom.id (X →₀ ℤ) :=
  by 
    ext x y 
...
theorem Finsupp.to_free_abelian_group_comp_to_finsupp :
  to_free_abelian_group.comp to_finsupp = AddMonoidHom.id (FreeAbelianGroup X) :=
  by 
    ext 
...
@[simp]
theorem to_finsupp_to_free_abelian_group (f : X →₀ ℤ) : f.to_free_abelian_group.to_finsupp = f :=
  by 
...
@[simp]
theorem support_zero : support (0 : FreeAbelianGroup X) = ∅ :=
  by 
    simp only [support, Finsupp.support_zero, AddMonoidHom.map_zero]

@[simp]
theorem support_of (x : X) : support (of x) = {x} :=
  by 
    simp only [support, to_finsupp_of, Finsupp.support_single_ne_zero one_ne_zero]

@[simp]
theorem support_neg (a : FreeAbelianGroup X) : support (-a) = support a :=
  by 
