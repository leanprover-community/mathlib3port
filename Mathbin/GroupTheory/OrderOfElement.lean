...
-/
@[toAdditive add_order_of_eq_of_nsmul_and_div_prime_nsmul]
...
@[toAdditive add_order_of_add_dvd_mul_add_order_of]
...
@[toAdditive add_order_of_add_eq_mul_add_order_of_of_coprime]
theorem Commute.order_of_mul_eq_mul_order_of_of_coprime {x y : G} (h : Commute x y)
  (hco : Nat.Coprime (orderOf x) (orderOf y)) : orderOf (x*y) = orderOf x*orderOf y :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:168:9: warning: unsupported option pp.all
...
theorem zpowers_equiv_zpowers_apply (h : orderOf x = orderOf y) (n : ℕ) :
  ...
  by 
    rw [zpowersEquivZpowers, Equivₓ.trans_apply, Equivₓ.trans_apply, fin_equiv_zpowers_symm_apply,
      ←Equivₓ.eq_symm_apply, fin_equiv_zpowers_symm_apply]
    simp [h]
...
@[toAdditive nsmulCoprime "If `gcd(|G|,n)=1` then the smul by `n` is a bijection", simps]
...
@[toAdditive add_inf_eq_bot_of_coprime]
...
@[toAdditive "A nonempty idempotent subset of a finite cancellative add monoid is a submonoid"]
...
@[toAdditive "A nonempty idempotent subset of a finite add group is a subgroup"]
...
@[toAdditive smulCardAddSubgroup
