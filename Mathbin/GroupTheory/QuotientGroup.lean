...
instance {G : Type _} [CommGroup G] (N : Subgroup G) : CommGroup (G ⧸ N) :=
  { @QuotientGroup.Quotient.group _ _ N N.normal_of_comm with
    mul_comm := fun a b => Quotient.induction_on₂' a b fun a b => congr_arg mk (mul_comm a b) }
...
@[simp, to_additive]
theorem map_map {I : Type _} [Group I] (M : Subgroup H) (O : Subgroup I) [M.Normal] [O.Normal] (f : G →* H) (g : H →* I)
    (hf : N ≤ Subgroup.comap f M) (hg : M ≤ Subgroup.comap g O)
...
@[simp, to_additive]
theorem map_comp_map {I : Type _} [Group I] (M : Subgroup H) (O : Subgroup I) [M.Normal] [O.Normal] (f : G →* H)
...
theorem ker_lift_injective : Injective (kerLift φ) := fun a b =>
  (Quotient.induction_on₂' a b) fun a b (h : φ a = φ b) =>
...
theorem range_ker_lift_injective : Injective (rangeKerLift φ) := fun a b =>
  (Quotient.induction_on₂' a b) fun a b (h : φ.range_restrict a = φ.range_restrict b) =>
    Quotient.sound' <| by
...
def quotientMulEquivOfEq {M N : Subgroup G} [M.Normal] [N.Normal] (h : M = N) : G ⧸ M ≃* G ⧸ N :=
  { Subgroup.quotientEquivOfEq h with map_mul' := fun q r => Quotient.induction_on₂' q r fun g h => rfl }
