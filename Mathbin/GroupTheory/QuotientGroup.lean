...
@[toAdditive "The additive congruence relation generated by a normal additive subgroup."]
protected def Con : Con G :=
  { toSetoid := left_rel N,
    mul' :=
      fun a b c d hab : (a⁻¹*b) ∈ N hcd : (c⁻¹*d) ∈ N =>
...
@[simp, toAdditive QuotientAddGroup.coe_sub]
...
def equiv_quotient_of_eq {M N : Subgroup G} [M.normal] [N.normal] (h : M = N) : G ⧸ M ≃* G ⧸ N :=
  { toFun :=
      lift M (mk' N)
        fun m hm =>
