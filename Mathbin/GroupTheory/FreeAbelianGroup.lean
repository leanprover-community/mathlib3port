...
protected theorem induction_on {C : FreeAbelianGroup α → Prop} (z : FreeAbelianGroup α) (C0 : C 0) (C1 : ∀ x, C$ of x)
  ...
  Quotientₓ.induction_on' z$
    fun x =>
      Quot.induction_on x$
        fun L => List.recOn L C0$ fun ⟨x, b⟩ tl ih => Bool.recOn b (Cp _ _ (Cn _ (C1 x)) ih) (Cp _ _ (C1 x) ih)
...
instance  : Monadₓ FreeAbelianGroup.{u} :=
  { pure := fun α => of, bind := fun α β x f => lift f x }
  ...
  { map_add :=
      fun x y =>
        show lift (· <$> x+y) _ = _ by 
          simp only [map_add] <;>
            exact
              @IsAddHom.map_add _ _ _ (@FreeAbelianGroup.is_add_group_hom_lift' (FreeAbelianGroup β) _ _).to_is_add_hom
                _ _ }
...
instance  : IsLawfulMonad FreeAbelianGroup.{u} :=
  { id_map :=
      fun α x =>
...
instance  : IsCommApplicative FreeAbelianGroup.{u} :=
  { commutative_prod :=
      fun α β x y =>
        FreeAbelianGroup.induction_on' x
          (by 
...
instance  : Semigroupₓ (FreeAbelianGroup α) :=
...
instance pempty_unique : Unique (FreeAbelianGroup Pempty) :=
  { default := 0,
    uniq :=
      fun x =>
