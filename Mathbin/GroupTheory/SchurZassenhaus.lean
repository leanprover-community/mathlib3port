...
-- error in GroupTheory.SchurZassenhaus: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
theorem smul_left_injective
[H.normal]
(α : H.quotient_diff)
(hH : nat.coprime (fintype.card H) H.index) : function.injective (λ h : H, «expr • »(h, α)) :=
λ h₁ h₂, begin
  refine [expr quotient.induction_on α (λ α hα, _)],
  replace [ident hα] [":", expr «expr = »(diff «expr • »(h₁, α) «expr • »(h₂, α), 1)] [":=", expr quotient.exact hα],
  rw ["[", expr smul_diff, ",", "<-", expr diff_inv, ",", expr smul_diff, ",", expr diff_self, ",", expr mul_one, ",", expr mul_inv_eq_one, "]"] ["at", ident hα],
  exact [expr (pow_coprime hH).injective hα]
end
...
[fintype G]
[H.normal]
(hH : nat.coprime (fintype.card H) H.index) : «expr∃ , »((K : subgroup G), is_complement' H K) :=
nonempty_of_inhabited.elim (λ α : H.quotient_diff, ⟨mul_action.stabilizer G α, is_complement'_stabilizer_of_coprime hH⟩)
...
variable{G :
    Type
      ...
      G][Fintype
      G]{N :
    Subgroup G}[normal N](h1 : Nat.Coprime (Fintype.card N) N.index)(h2 :
    ∀ (G' : Type u) [Groupₓ G'] [Fintype G'],
      by 
        exact
          ∀ (hG'3 : Fintype.card G' < Fintype.card G) {N' : Subgroup G'} [N'.normal]
            (hN : Nat.Coprime (Fintype.card N') N'.index),
            ∃ H' : Subgroup G', is_complement' N' H')(h3 : ∀ (H : Subgroup G), ¬is_complement' N H)
