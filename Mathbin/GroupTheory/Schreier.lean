...
theorem closure_mul_image_eq (hR : R ∈ RightTransversals (H : Set G)) (hR1 : (1 : G) ∈ R) (hS : closure S = ⊤) :
  ...
  suffices (⟨r, hr⟩ : R) = (⟨1, hR1⟩ : R) by rwa [show r = 1 from subtype.ext_iff.mp this, mul_oneₓ]
...
theorem closure_mul_image_eq_top (hR : R ∈ RightTransversals (H : Set G)) (hR1 : (1 : G) ∈ R) (hS : closure S = ⊤) :
  ...
  exact (map_subtype_le ⊤).trans (ge_of_eqₓ (closure_mul_image_eq hR hR1 hS))
...
/-- **Schreier's Lemma**: If `R : finset G` is a right_transversal of `H : subgroup G`
  ...
  `(R * S).image (λ g, g * (to_fun hR g)⁻¹)`. -/
theorem closure_mul_image_eq_top' [DecidableEq G] {R S : Finsetₓ G} (hR : (R : Set G) ∈ RightTransversals (H : Set G))
    (hR1 : (1 : G) ∈ R) (hS : closure (S : Set G) = ⊤) :
    closure (((R * S).Image fun g => ⟨_, mul_inv_to_fun_mem hR g⟩ : Finsetₓ H) : Set H) = ⊤ := by
  rw [Finsetₓ.coe_image, Finsetₓ.coe_mul]
  exact closure_mul_image_eq_top hR hR1 hS

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem exists_finset_card_le_mul (hH : H.index ≠ 0) {S : Finsetₓ G} (hS : closure (S : Set G) = ⊤) :
    ∃ T : Finsetₓ H, T.card ≤ H.index * S.card ∧ closure (T : Set H) = ⊤ := by
  haveI : DecidableEq G := Classical.decEq G
  obtain ⟨R₀, hR : R₀ ∈ right_transversals (H : Set G), hR1⟩ := exists_right_transversal (1 : G)
  haveI : Fintypeₓ (G ⧸ H) := fintype_of_index_ne_zero hH
  haveI : Fintypeₓ R₀ := Fintypeₓ.ofEquiv _ (mem_right_transversals.to_equiv hR)
  let R : Finsetₓ G := Set.toFinset R₀
  ...
  calc
    _ ≤ (R * S).card := Finsetₓ.card_image_le
    _ ≤ (R ×ˢ S).card := Finsetₓ.card_image_le
    _ = R.card * S.card := R.card_product S
    _ = H.index * S.card := congr_arg (· * S.card) _
    
  calc
    R.card = Fintypeₓ.card R := (Fintypeₓ.card_coe R).symm
    _ = _ := (Fintypeₓ.card_congr (mem_right_transversals.to_equiv hR)).symm
    _ = Fintypeₓ.card (G ⧸ H) := QuotientGroup.card_quotient_right_rel H
...
theorem rank_le_index_mul_rank [hG : Groupₓ.Fg G] {H : Subgroup G} (hH : H.index ≠ 0) :
    @Groupₓ.rank H _ (fg_of_index_ne_zero hH) ≤ H.index * Groupₓ.rank G := by
...
theorem card_commutator_dvd_index_center_pow [Finite { g | ∃ g₁ g₂ : G, ⁅g₁, g₂⁆ = g }] :
  ...
  · simp_rw [hG, zero_mul, zero_addₓ, pow_oneₓ, dvd_zero]
    
  -- Rewrite as `|Z(G) ∩ G'| * [G' : Z(G) ∩ G'] ∣ [G : Z(G)] ^ ([G : Z(G)] * n) * [G : Z(G)]`
  rw [← ((center G).subgroupOf (commutator G)).card_mul_index, pow_succ'ₓ]
  ...
  have h3 := Nat.mul_le_mulₓ (Nat.le_of_dvdₓ (Nat.pos_of_ne_zeroₓ hG) h1) (rank_commutator_le_card G)
