...
inductive QuaternionGroup (n : ℕ) : Type
  ...
  | xa : Zmod (2*n) → QuaternionGroup deriving [anonymous]
...
def quaternion_group_zero_equiv_dihedral_group_zero : QuaternionGroup 0 ≃* DihedralGroup 0 :=
...
-/
theorem card [Fact (0 < n)] : Fintype.card (QuaternionGroup n) = 4*n :=
  by 
...
@[simp]
theorem xa_sq (i : Zmod (2*n)) : xa i ^ 2 = a n :=
  by 
    simp [sq]
...
theorem order_of_xa [hpos : Fact (0 < n)] (i : Zmod (2*n)) : orderOf (xa i) = 4 :=
  by 
    ...
    ·
      ...
      applyFun · / n  at h' 
      simp only [Zmod.val_nat_cast, Zmod.val_zero, Nat.zero_divₓ, Nat.mod_mul_left_div_self, Nat.div_selfₓ hpos.1] at
        h' 
      normNum  at h'
    ·
      normNum
...
-/
theorem order_of_a [Fact (0 < n)] (i : Zmod (2*n)) : orderOf (a i) = (2*n) / Nat.gcdₓ (2*n) i.val :=
  by 
