...
-- ./././Mathport/Syntax/Translate/Command.lean:326:30: infer kinds are unsupported in Lean 4: #[`exists_generator] []
...
theorem is_cyclic_of_order_of_eq_card [Fintypeₓ α] (x : α) (hx : orderOf x = Fintypeₓ.card α) : IsCyclic α := by
  classical
  use x
  simp_rw [← SetLike.mem_coe, ← Set.eq_univ_iff_forall]
  rw [← Fintypeₓ.card_congr (Equivₓ.Set.univ α), order_eq_card_zpowers] at hx
  exact Set.eq_of_subset_of_card_le (Set.subset_univ _) (ge_of_eqₓ hx)

/-- A finite group of prime order is cyclic. -/
@[to_additive is_add_cyclic_of_prime_card "A finite group of prime order is cyclic."]
theorem is_cyclic_of_prime_card {α : Type u} [Groupₓ α] [Fintypeₓ α] {p : ℕ} [hp : Fact p.Prime]
    (h : Fintypeₓ.card α = p) : IsCyclic α :=
  ⟨by
    obtain ⟨g, hg⟩ : ∃ g : α, g ≠ 1 := Fintypeₓ.exists_ne_of_one_lt_card (h.symm ▸ hp.1.one_lt) 1
    classical
    -- for fintype (subgroup.zpowers g)
    have : Fintypeₓ.card (Subgroup.zpowers g) ∣ p := by
      rw [← h]
      apply card_subgroup_dvd_card
    rw [Nat.dvd_prime hp.1] at this
    cases this
    · rw [Fintypeₓ.card_eq_one_iff] at this
...
theorem order_of_eq_card_of_forall_mem_zpowers [Fintypeₓ α] {g : α} (hx : ∀ x, x ∈ zpowers g) :
    orderOf g = Fintypeₓ.card α := by
  classical
  rw [order_eq_card_zpowers]
  apply Fintypeₓ.card_of_finset'
  simpa using hx
...
theorem Infinite.order_of_eq_zero_of_forall_mem_zpowers [Infinite α] {g : α} (h : ∀ x, x ∈ zpowers g) : orderOf g = 0 :=
  ...
  obtain ⟨x, hx⟩ := Infinite.exists_not_mem_finset (Finsetₓ.image (pow g) <| Finsetₓ.range <| orderOf g)
  apply hx
  ...
  have : 0 ≤ t := Int.mod_nonnegₓ (-k) (by exact_mod_cast ho.ne')
  refine' ⟨t.to_nat, _⟩
...
instance Subgroup.is_cyclic {α : Type u} [Groupₓ α] [IsCyclic α] (H : Subgroup α) : IsCyclic H :=
  ...
  if hx : ∃ x : α, x ∈ H ∧ x ≠ (1 : α) then
    ...
    have hex : ∃ n : ℕ, 0 < n ∧ g ^ n ∈ H :=
      ⟨k.natAbs, Nat.pos_of_ne_zeroₓ fun h => hx₂ <| by rw [← hk, Int.eq_zero_of_nat_abs_eq_zero h, zpow_zero],
        match k, hk with
        | (k : ℕ), hk => by rw [Int.nat_abs_of_nat, ← zpow_coe_nat, hk] <;> exact hx₁
        | -[1 + k], hk => by rw [Int.nat_abs_of_neg_succ_of_nat, ← Subgroup.inv_mem_iff H] <;> simp_all⟩
    ⟨⟨⟨g ^ Nat.findₓ hex, (Nat.find_specₓ hex).2⟩, fun ⟨x, hx⟩ =>
        let ⟨k, hk⟩ := hg x
        have hk₁ : g ^ ((Nat.findₓ hex : ℤ) * (k / Nat.findₓ hex)) ∈ zpowers (g ^ Nat.findₓ hex) :=
          ⟨k / Nat.findₓ hex, by rw [← zpow_coe_nat, zpow_mul]⟩
        have hk₂ : g ^ ((Nat.findₓ hex : ℤ) * (k / Nat.findₓ hex)) ∈ H := by
          rw [zpow_mul]
          apply H.zpow_mem
          exact_mod_cast (Nat.find_specₓ hex).2
        have hk₃ : g ^ (k % Nat.findₓ hex) ∈ H :=
          (Subgroup.mul_mem_cancel_right H hk₂).1 <| by rw [← zpow_add, Int.mod_add_divₓ, hk] <;> exact hx
        ...
        have hk₅ : g ^ (k % Nat.findₓ hex).natAbs ∈ H := by rwa [← zpow_coe_nat, ← hk₄]
        ...
        ⟨k / (Nat.findₓ hex : ℤ),
          Subtype.ext_iff_val.2
            (by
              suffices g ^ ((Nat.findₓ hex : ℤ) * (k / Nat.findₓ hex)) = x by simpa [zpow_mul]
...
theorem IsCyclic.card_pow_eq_one_le [DecidableEq α] [Fintypeₓ α] [IsCyclic α] {n : ℕ} (hn0 : 0 < n) :
    (univ.filter fun a : α => a ^ n = 1).card ≤ n :=
  let ⟨g, hg⟩ := IsCyclic.exists_generator α
  calc
    (univ.filter fun a : α => a ^ n = 1).card ≤
        (zpowers (g ^ (Fintypeₓ.card α / Nat.gcdₓ n (Fintypeₓ.card α))) : Set α).toFinset.card :=
      card_le_of_subset fun x hx =>
        let ⟨m, hm⟩ := show x ∈ Submonoid.powers g from mem_powers_iff_mem_zpowers.2 <| hg x
        Set.mem_to_finset.2
          ⟨(m / (Fintypeₓ.card α / Nat.gcdₓ n (Fintypeₓ.card α)) : ℕ), by
            have hgmn : g ^ (m * Nat.gcdₓ n (Fintypeₓ.card α)) = 1 := by
              rw [pow_mulₓ, hm, ← pow_gcd_card_eq_one_iff] <;> exact (mem_filter.1 hx).2
            rw [zpow_coe_nat, ← pow_mulₓ, Nat.mul_div_cancel_left', hm]
            refine' dvd_of_mul_dvd_mul_right (gcd_pos_of_pos_left (Fintypeₓ.card α) hn0) _
    ...
    _ ≤ n := by
      let ⟨m, hm⟩ := Nat.gcd_dvd_rightₓ n (Fintypeₓ.card α)
      have hm0 : 0 < m :=
        Nat.pos_of_ne_zeroₓ fun hm0 => by
          rw [hm0, mul_zero, Fintypeₓ.card_eq_zero_iff] at hm
          exact hm.elim' 1
...
variable [DecidableEq α] [Fintypeₓ α]

@[to_additive]
theorem IsCyclic.image_range_order_of (ha : ∀ x : α, x ∈ zpowers a) :
    Finsetₓ.image (fun i => a ^ i) (range (orderOf a)) = univ := by
...
theorem IsCyclic.image_range_card (ha : ∀ x : α, x ∈ zpowers a) :
    Finsetₓ.image (fun i => a ^ i) (range (Fintypeₓ.card α)) = univ := by
...
variable [DecidableEq α] [Fintypeₓ α] (hn : ∀ n : ℕ, 0 < n → (univ.filter fun a : α => a ^ n = 1).card ≤ n)

include hn

private theorem card_pow_eq_one_eq_order_of_aux (a : α) :
    (Finsetₓ.univ.filter fun b : α => b ^ orderOf a = 1).card = orderOf a :=
  le_antisymmₓ (hn _ (order_of_pos a))
    (calc
      orderOf a = @Fintypeₓ.card (zpowers a) (id _) := order_eq_card_zpowers
      _ ≤
          @Fintypeₓ.card (↑(univ.filter fun b : α => b ^ orderOf a = 1) : Set α)
            (Fintypeₓ.ofFinset _ fun _ => Iff.rfl) :=
        @Fintypeₓ.card_le_of_injective (zpowers a) (↑(univ.filter fun b : α => b ^ orderOf a = 1) : Set α) (id _) (id _)
          (fun b =>
            ⟨b.1,
              mem_filter.2
                ⟨mem_univ _, by
      ...
      _ = (univ.filter fun b : α => b ^ orderOf a = 1).card := Fintypeₓ.card_of_finset _ _
      )
...
private theorem card_order_of_eq_totient_aux₁ :
  ...
  · cases Fintypeₓ.card_ne_zero (eq_zero_of_zero_dvd hd)
  ...
  have h1 : (∑ m in d.proper_divisors, (univ.filter fun a : α => orderOf a = m).card) = ∑ m in d.proper_divisors, φ m :=
    by
    refine' Finsetₓ.sum_congr rfl fun m hm => _
    simp only [mem_filter, mem_range, mem_proper_divisors] at hm
    refine' IH m hm.2 (hm.1.trans hd) (Finsetₓ.card_pos.2 ⟨a ^ (d / m), _⟩)
...
theorem card_order_of_eq_totient_aux₂ {d : ℕ} (hd : d ∣ Fintypeₓ.card α) :
    (univ.filter fun a : α => orderOf a = d).card = φ d := by
  let c := Fintypeₓ.card α
  have hc0 : 0 < c := Fintypeₓ.card_pos_iff.2 ⟨1⟩
...
theorem is_cyclic_of_card_pow_eq_one_le : IsCyclic α :=
  have : (univ.filter fun a : α => orderOf a = Fintypeₓ.card α).Nonempty :=
    card_pos.1 <| by rw [card_order_of_eq_totient_aux₂ hn dvd_rfl] <;> exact totient_pos (Fintypeₓ.card_pos_iff.2 ⟨1⟩)
  let ⟨x, hx⟩ := this
  is_cyclic_of_order_of_eq_card x (Finsetₓ.mem_filter.1 hx).2

theorem is_add_cyclic_of_card_pow_eq_one_le {α} [AddGroupₓ α] [DecidableEq α] [Fintypeₓ α]
...
theorem IsCyclic.card_order_of_eq_totient [IsCyclic α] [Fintypeₓ α] {d : ℕ} (hd : d ∣ Fintypeₓ.card α) :
    (univ.filter fun a : α => orderOf a = d).card = totient d := by
  classical
  apply card_order_of_eq_totient_aux₂ (fun n => IsCyclic.card_pow_eq_one_le) hd

theorem IsAddCyclic.card_order_of_eq_totient {α} [AddGroupₓ α] [IsAddCyclic α] [Fintypeₓ α] {d : ℕ}
    (hd : d ∣ Fintypeₓ.card α) : (univ.filter fun a : α => addOrderOf a = d).card = totient d := by
...
theorem is_simple_group_of_prime_card {α : Type u} [Groupₓ α] [Fintypeₓ α] {p : ℕ} [hp : Fact p.Prime]
    (h : Fintypeₓ.card α = p) : IsSimpleGroup α :=
  ⟨by
    have h' := Nat.Prime.one_lt (Fact.out p.prime)
    rw [← h] at h'
    haveI := Fintypeₓ.one_lt_card_iff_nontrivial.1 h'
    ...
    · haveI := Fintypeₓ.card_le_one_iff_subsingleton.1 (le_of_eqₓ h1)
...
theorem commutative_of_cyclic_center_quotient [IsCyclic H] (f : G →* H) (hf : f.ker ≤ center G) (a b : G) :
  ...
  have ha : y ^ -m * a ∈ center G := hf (by rw [f.mem_ker, f.map_mul, f.map_zpow, hxy, zpow_neg, hm, inv_mul_selfₓ])
  have hb : y ^ -n * b ∈ center G := hf (by rw [f.mem_ker, f.map_mul, f.map_zpow, hxy, zpow_neg, hn, inv_mul_selfₓ])
  calc
...
def commGroupOfCycleCenterQuotient [IsCyclic H] (f : G →* H) (hf : f.ker ≤ center G) : CommGroupₓ G :=
  { show Groupₓ G by infer_instance with mul_comm := commutative_of_cyclic_center_quotient f hf }
...
@[to_additive]
theorem prime_card [Fintypeₓ α] : (Fintypeₓ.card α).Prime := by
  have h0 : 0 < Fintypeₓ.card α := Fintypeₓ.card_pos_iff.2 (by infer_instance)
  obtain ⟨g, hg⟩ := IsCyclic.exists_generator α
  rw [Nat.prime_def_lt'']
  refine' ⟨Fintypeₓ.one_lt_card_iff_nontrivial.2 inferInstance, fun n hn => _⟩
  ...
  · intro h
    ...
    rw [order_of_eq_card_of_forall_mem_zpowers hg, Nat.gcd_eq_right_iff_dvdₓ.1 hn,
      ...
      hgo
    · exact (mul_left_cancel₀ (ne_of_gtₓ h0) ((mul_oneₓ (Fintypeₓ.card α)).trans hgo)).symm
...
theorem CommGroupₓ.is_simple_iff_is_cyclic_and_prime_card [Fintypeₓ α] [CommGroupₓ α] :
    IsSimpleGroup α ↔ IsCyclic α ∧ (Fintypeₓ.card α).Prime := by
  constructor
  · intro h
    exact ⟨IsSimpleGroup.is_cyclic, IsSimpleGroup.prime_card⟩
    
  · rintro ⟨hc, hp⟩
    haveI : Fact (Fintypeₓ.card α).Prime := ⟨hp⟩
...
@[to_additive]
theorem IsCyclic.exponent_eq_card [Groupₓ α] [IsCyclic α] [Fintypeₓ α] : exponent α = Fintypeₓ.card α := by
  obtain ⟨g, hg⟩ := IsCyclic.exists_generator α
  apply Nat.dvd_antisymm
  · rw [← lcm_order_eq_exponent, Finsetₓ.lcm_dvd_iff]
...
@[to_additive]
theorem IsCyclic.of_exponent_eq_card [CommGroupₓ α] [Fintypeₓ α] (h : exponent α = Fintypeₓ.card α) : IsCyclic α :=
  let ⟨g, _, hg⟩ := Finsetₓ.mem_image.mp (Finsetₓ.max'_mem _ _)
  is_cyclic_of_order_of_eq_card g <| hg.trans <| exponent_eq_max'_order_of.symm.trans h

@[to_additive]
theorem IsCyclic.iff_exponent_eq_card [CommGroupₓ α] [Fintypeₓ α] : IsCyclic α ↔ exponent α = Fintypeₓ.card α :=
