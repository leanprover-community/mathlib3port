...
open Equivₓ Equivₓ.Perm Subgroup Fintypeₓ

variable (α : Type _) [Fintypeₓ α] [DecidableEq α]

/-- The alternating group on a finite type, realized as a subgroup of `equiv.perm`.
  For $A_n$, use `alternating_group (fin n)`. -/
def alternatingGroup : Subgroup (Perm α) :=
  sign.ker deriving Fintypeₓ
...
theorem two_mul_card_alternating_group [Nontrivial α] : 2 * card (alternatingGroup α) = card (Perm α) := by
  ...
  rw [← Fintypeₓ.card_units_int, ← Fintypeₓ.card_congr this]
...
theorem is_conj_of {σ τ : alternatingGroup α} (hc : IsConj (σ : Perm α) (τ : Perm α))
    (hσ : (σ : Perm α).support.card + 2 ≤ Fintypeₓ.card α) : IsConj σ τ := by
  obtain ⟨σ, hσ⟩ := σ
  obtain ⟨τ, hτ⟩ := τ
  ...
  · have h2 : 2 ≤ σ.supportᶜ.card := by
      rw [Finsetₓ.card_compl, le_tsub_iff_left σ.support.card_le_univ]
      exact hσ
    obtain ⟨a, ha, b, hb, ab⟩ := Finsetₓ.one_lt_card.1 h2
    ...
    · simp only [← hπ, coe_mk, Subgroup.coe_mul, Subtype.val_eq_coe]
      have hd : Disjoint (swap a b) σ := by
        rw [disjoint_iff_disjoint_support, support_swap ab, Finsetₓ.disjoint_insert_left,
          Finsetₓ.disjoint_singleton_left]
        exact ⟨Finsetₓ.mem_compl.1 ha, Finsetₓ.mem_compl.1 hb⟩
      rw [mul_assoc π _ σ, hd.commute.eq, coe_inv, coe_mk]
      simp [mul_assoc]
      
    

theorem is_three_cycle_is_conj (h5 : 5 ≤ Fintypeₓ.card α) {σ τ : alternatingGroup α} (hσ : IsThreeCycle (σ : Perm α))
    (hτ : IsThreeCycle (τ : Perm α)) : IsConj σ τ :=
  alternatingGroup.is_conj_of (is_conj_iff_cycle_type_eq.2 (hσ.trans hτ.symm)) (by rwa [hσ.card_support])
...
theorem IsThreeCycle.alternating_normal_closure (h5 : 5 ≤ Fintypeₓ.card α) {f : Perm α} (hf : IsThreeCycle f) :
...
theorem nontrivial_of_three_le_card (h3 : 3 ≤ card α) : Nontrivial (alternatingGroup α) := by
  haveI := Fintypeₓ.one_lt_card_iff_nontrivial.1 (lt_transₓ (by decide) h3)
  rw [← Fintypeₓ.one_lt_card_iff_nontrivial]
...
theorem normal_closure_swap_mul_swap_five :
    normalClosure ({⟨swap 0 4 * swap 1 3, mem_alternating_group.2 (by decide)⟩} : Set (alternatingGroup (Finₓ 5))) =
      ⊤ :=
  by
  let g1 := (⟨swap 0 2 * swap 0 1, mem_alternating_group.2 (by decide)⟩ : alternatingGroup (Finₓ 5))
  let g2 := (⟨swap 0 4 * swap 1 3, mem_alternating_group.2 (by decide)⟩ : alternatingGroup (Finₓ 5))
...
theorem is_conj_swap_mul_swap_of_cycle_type_two {g : Perm (Finₓ 5)} (ha : g ∈ alternatingGroup (Finₓ 5)) (h1 : g ≠ 1)
  ...
  have h := le_of_mul_le_mul_right (le_transₓ h h56) (by decide)
  ...
  · have h04 : (0 : Finₓ 5) ≠ 4 := by decide
    have h13 : (1 : Finₓ 5) ≠ 3 := by decide
...
instance is_simple_group_five : IsSimpleGroup (alternatingGroup (Finₓ 5)) :=
  ⟨exists_pair_ne _, fun H => by
    ...
    · -- If `n = 3`, then `g` has a 3-cycle in its decomposition, so `g^2` is a 3-cycle.
      ...
      rw [eq_top_iff, ←
        (is_three_cycle_sq_of_three_mem_cycle_type_five ng).alternating_normal_closure (by rw [card_fin])]
      refine' normal_closure_le_normal _
      rw [Set.singleton_subset_iff, SetLike.mem_coe]
