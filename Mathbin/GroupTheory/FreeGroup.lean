...
@[to_additive]
theorem Step.sublist (H : Red.Step L₁ L₂) : L₂ <+ L₁ := by cases H <;> simp <;> constructor <;> constructor <;> rfl
...
instance : Group (FreeGroup α) where
  ...
  mul_left_inv := by
    rintro ⟨L⟩ <;> exact List.recOn L rfl $ fun ⟨x, b⟩ tl ih => Eq.trans (Quot.sound $ by simp [inv_rev, one_eq_mk]) ih
  ...
  rintro ⟨L⟩ <;>
    exact
      List.recOn L g.map_one fun ⟨x, b⟩ t (ih : g (mk t) = map f (mk t)) =>
        Bool.recOn b (show g ((of x)⁻¹ * mk t) = map f ((of x)⁻¹ * mk t) by simp [g.map_mul, g.map_inv, hg, ih])
...
as `equiv.of_free_group_equiv`
 -/
@[to_additive "Equivalent types give rise to additively equivalent additive free groups.", simps apply]
...
@[to_additive "The bijection between the additive free group on the empty type, and a type with one element."]
...
[PrettyPrinter.parenthesize.input] (Command.declaration
     ...
     (Command.theorem
      ...
      (Command.declSig
       [(Term.implicitBinder "{" [`p] [":" (Init.Core.termProp "Prop")] "}")]
       (Term.typeSpec
        ":"
        (Term.forall
         "∀"
         [(Term.implicitBinder "{" [`L₁ `L₂ `L₃] [":" (Term.app `List [(Init.Core.«term_×_» `α " × " `Bool)])] "}")
          ...
          (Init.Core.«term_=_»
           ...
           (Init.Core.«term_++_»
            ...
            (Sym.Data.Sym.Basic.sym.cons'
             (Term.tuple "(" [`x "," [`b]] ")")
             "::"
             (Sym.Data.Sym.Basic.sym.cons' (Term.tuple "(" [`x "," [(Term.app `not [`b])]] ")") "::" `L₃))))
          "→"
          `p))))
      (Command.declValEqns
       (Term.matchAltsWhereDecls
        (Term.matchAlts
         [(Term.matchAlt
           "|"
           [[(Init.Core.«term[_,» "[" [] "]") "," `L2 "," `L3 "," (Term.hole "_") "," (Term.hole "_")]]
           "=>"
           (Term.fun
            "fun"
            (Term.basicFun
               ...
               («tactic___;_»
                ...
                [(group (Tactic.dsimp "dsimp" [] [] [] [] []) [])
                 ...
                 (group
                  (Tactic.exact
                   "exact"
                   (Term.app `absurd [`this (Tactic.Init.Meta.Tactic.termdec_trivial "dec_trivial")]))
                  [])])
               []
               (Tactic.cases'
                "cases'"
                 ...
                 (group
                  (Tactic.exact
                   ...
                   (Term.app
                    ...
                    [`L1 (Sym.Data.Sym.Basic.sym.cons' (Term.tuple "(" [`y "," [`c]] ")") "::" `L2) `L3 `x' `b' `r]))
                  [])])
               []
               (Std.Tactic.rcases
                "rcases"
          ...
          («tactic___;_»
           ...
           [(group (Tactic.dsimp "dsimp" [] [] [] [] []) [])
            ...
            (group
             ...
             [])
            (group
             (Tactic.exact "exact" (Term.app `absurd [`this (Tactic.Init.Meta.Tactic.termdec_trivial "dec_trivial")]))
             [])])
          []
          (Tactic.cases'
           "cases'"
    ...
    | [ ] , L2 , L3 , _ , _ => fun h => by cases L2 <;> injections
      |
        ...
        by
          dsimp
            cases r : reduce L1
            · dsimp intro h have := congr_arg List.length h simp [ - add_comm ] at this exact absurd this dec_trivial
            cases' hd with y c
            dsimp only
