...
@[simp]
theorem step.bnot_rev {x b} : step (L₁ ++ (x, bnot b) :: (x, b) :: L₂) (L₁ ++ L₂) :=
  by 
...
-- error in GroupTheory.FreeGroup: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
...
| [], _, (x3, b3) :: (x4, b4) :: tl, _, _, _, _, _, H =>
  by 
    injections <;> substVars <;> simp  <;> right <;> exact ⟨_, red.step.bnot, red.step.cons_bnot⟩
| (x3, b3) :: (x4, b4) :: tl, _, [], _, _, _, _, _, H =>
  by 
    injections <;> substVars <;> simp  <;> right <;> exact ⟨_, red.step.cons_bnot, red.step.bnot⟩
...
theorem eqv_gen_step_iff_join_red : EqvGen red.step L₁ L₂ ↔ join red L₁ L₂ :=
  Iff.intro
    ...
    (join_of_equivalence (EqvGen.is_equivalence _)$
      fun a b => refl_trans_gen_of_equivalence (EqvGen.is_equivalence _) EqvGen.rel)
...
def lift.aux : List (α × Bool) → β :=
  fun L => List.prod$ L.map$ fun x => cond x.2 (f x.1) (f x.1⁻¹)

theorem red.step.lift {f : α → β} (H : red.step L₁ L₂) : lift.aux f L₁ = lift.aux f L₂ :=
  by 
...
def lift : (α → β) ≃ (FreeGroup α →* β) :=
  { toFun :=
      fun f =>
        MonoidHom.mk' (Quot.lift (lift.aux f)$ fun L₁ L₂ => red.step.lift)$
          by 
            rintro ⟨L₁⟩ ⟨L₂⟩
            simp [lift.aux],
...
theorem lift.range_subset {s : Subgroup β} (H : Set.Range f ⊆ s) : Set.Range (lift f) ⊆ s :=
  by 
    rintro _ ⟨⟨L⟩, rfl⟩ <;>
      exact
        List.recOn L s.one_mem
          fun ⟨x, b⟩ tl ih =>
            Bool.recOn b
              (by 
...
theorem closure_subset {G : Type _} [Groupₓ G] {s : Set G} {t : Subgroup G} (h : s ⊆ t) : Subgroup.closure s ≤ t :=
  by 
...
def map.to_fun (x : FreeGroup α) : FreeGroup β :=
  (x.lift_on fun L => mk$ map.aux f L)$
    fun L₁ L₂ H =>
      Quot.sound$
        by 
...
theorem lift_eq_prod_map {β : Type v} [Groupₓ β] {f : α → β} {x} : lift f x = Prod (map f x) :=
  by 
...
protected theorem induction_on {C : FreeGroup α → Prop} (z : FreeGroup α) (C1 : C 1) (Cp : ∀ x, C$ pure x)
  ...
  Quot.induction_on z$
    fun L => List.recOn L C1$ fun ⟨x, b⟩ tl ih => Bool.recOn b (Cm _ _ (Ci _$ Cp x) ih) (Cm _ _ (Cp x) ih)
...
theorem reduce.red : red L (reduce L) :=
  by 
    ...
    case list.cons => 
      ...
      case list.cons => 
        dsimp 
        byCases' h : hd1.fst = hd2.fst ∧ hd1.snd = bnot hd2.snd
        ·
          rw [if_pos h]
          trans
          ·
...
theorem to_word.mk : ∀ {x : FreeGroup α}, mk (to_word x) = x :=
  by 
