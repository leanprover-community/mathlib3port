...
theorem coe_Inf (S : Set (Submonoid M)) : ((Inf S : Submonoid M) : Set M) = ⋂(s : _)(_ : s ∈ S), «expr↑ » s :=
  rfl
...
@[simp, normCast, toAdditive]
theorem coe_infi {ι : Sort _} {S : ι → Submonoid M} : («expr↑ » (⨅i, S i) : Set M) = ⋂i, S i :=
  by 
...
@[toAdditive "The `add_submonoid` generated by a set"]
...
-- error in GroupTheory.Submonoid.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
/-- The submonoid of elements `x : M` such that `f x = g x` -/
@[to_additive #[expr "The additive submonoid of elements `x : M` such that `f x = g x`"]]
def eq_mlocus (f g : «expr →* »(M, N)) : submonoid M :=
{ carrier := {x | «expr = »(f x, g x)},
  one_mem' := by rw ["[", expr set.mem_set_of_eq, ",", expr f.map_one, ",", expr g.map_one, "]"] [],
  mul_mem' := λ (x y) (hx : «expr = »(_, _)) (hy : «expr = »(_, _)), by simp [] [] [] ["[", "*", "]"] [] [] }
...
@[simp, normCast, toAdditive]
theorem coe_of_mdense (f : M → N) (hs : closure s = ⊤) h1 hmul : «expr⇑ » (of_mdense f hs h1 hmul) = f :=
  rfl
