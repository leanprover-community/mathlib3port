...
@[toAdditive "the `add_monoid_hom` from the preimage of an additive submonoid to itself.", simps]
...
@[toAdditive]
theorem submonoid_map_surjective (f : M →* N) (M' : Submonoid M) : Function.Surjective (f.submonoid_map M') :=
  by 
    rintro ⟨_, x, hx, rfl⟩
    exact ⟨⟨x, hx⟩, rfl⟩
