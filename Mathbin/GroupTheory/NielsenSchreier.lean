...
-- ././Mathport/Syntax/Translate/Basic.lean:1157:11: unsupported: unusual advanced open style
...
theorem ext_functor {G} [Groupoid.{v} G] [IsFreeGroupoid G] {X : Type v} [Groupₓ X] (f g : G ⥤ SingleObj X)
  ...
  let ⟨_, _, u⟩ := @unique_lift G _ _ X _ fun e : a ⟶ b => g.map (of e)
...
-- ././Mathport/Syntax/Translate/Basic.lean:597:2: warning: expanding binder collection (e «expr ∈ » wide_subquiver_symmetrify T a b)
...
def endIsFree : IsFreeGroup (End (root' T)) :=
  IsFreeGroup.ofUniqueLift (Set.Compl (wide_subquiver_equiv_set_total <| WideSubquiverSymmetrify T))
    ...
    (by
      ...
      · suffices ∀ {x y} q : x ⟶ y, F'.map (loop_of_hom T q) = (F'.map q : X) by
        ...
        suffices ∀ {a} p : path (root' T) a, F'.map (hom_of_path T p) = 1 by
          simp only [this, tree_hom, comp_as_mul, inv_as_inv, loop_of_hom, inv_one, mul_oneₓ, one_mulₓ, functor.map_inv,
            functor.map_comp]
        intro a p
