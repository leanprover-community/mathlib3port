...
/- ./././Mathport/Syntax/Translate/Command.lean:227:11: unsupported: unusual advanced open style -/
...
quiver. -/
@[ext.1]
theorem ext_functor {G} [Groupoid.{v} G] [IsFreeGroupoid G] {X : Type v} [Group X] (f g : G ⥤ SingleObj X)
...
/- ./././Mathport/Syntax/Translate/Basic.lean:611:2: warning: expanding binder collection (e «expr ∈ » wide_subquiver_symmetrify[quiver.wide_subquiver_symmetrify] T a b) -/
    ...
    (by
      ...
      · suffices ∀ {x y} (q : x ⟶ y), F'.map (loop_of_hom T q) = (F'.map q : X) by
        ...
        suffices ∀ {a} (p : path (root' T) a), F'.map (hom_of_path T p) = 1 by
          simp only [this, tree_hom, comp_as_mul, inv_as_inv, loop_of_hom, inv_one, mul_one, one_mul, functor.map_inv,
            functor.map_comp]
        intro a p
...
instance subgroupIsFreeOfIsFree {G : Type u} [Group G] [IsFreeGroup G] (H : Subgroup G) : IsFreeGroup H :=
