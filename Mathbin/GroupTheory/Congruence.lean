...
def mul_ker (f : M → P) (h : ∀ x y, f (x * y) = f x * f y) : Con M where
...
instance Mul : Mul c.quotient :=
  ⟨fun x y =>
    (Quotientₓ.liftOn₂' x y fun w z => ((w * z : M) : c.quotient)) $ fun _ _ _ _ h1 h2 => c.eq.2 $ c.mul h1 h2⟩
...
protected noncomputable def gi : @GaloisInsertion (M → M → Prop) (Con M) _ _ conGen coeFn where
...
def map_of_surjective (f : M → N) (H : ∀ x y, f (x * y) = f x * f y) (h : mul_ker f H ≤ c) (hf : surjective f) :
    Con N :=
...
def of_submonoid (N : Submonoid (M × M)) (H : Equivalenceₓ fun x y => (x, y) ∈ N) : Con M where
  R := fun x y => (x, y) ∈ N
  iseqv := H
