...
theorem Inf_to_setoid (S : Set (Con M)) : (Inf S).toSetoid = Inf (to_setoid '' S) :=
  Setoidₓ.ext' $ fun x y =>
    ⟨fun h r ⟨c, hS, hr⟩ => by
      rw [← hr] <;> exact h c hS, fun h c hS => h c.to_setoid ⟨c, hS, rfl⟩⟩
...
theorem con_gen_le {r : M → M → Prop} {c : Con M} (h : ∀ x y, r x y → @Setoidₓ.R _ c.to_setoid x y) : conGen r ≤ c := by
...
/-- The quotient of a monoid by a congruence relation is a monoid. -/
  ...
  instance
    ...
    where
      one := ( ( 1 : M ) : c.quotient )
        mul := · * ·
        mul_one x := Quotientₓ.induction_on' x $ fun _ => congr_argₓ coeₓ $ mul_oneₓ _
        one_mul x := Quotientₓ.induction_on' x $ fun _ => congr_argₓ coeₓ $ one_mulₓ _
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers
  [(Command.docComment "/--" " The submonoid of `M × M` defined by a congruence relation on a monoid `M`. -/")]
  [(Term.attributes
    "@["
    [(Term.attrInstance
      (Term.attrKind [])
 ...
 (Command.def
  "def"
  (Command.declId `Submonoid [])
  (Command.optDeclSig [] [(Term.typeSpec ":" (Term.app `Submonoid [(«term_×_» `M "×" `M)]))])
    ...
    [(group
      (Term.structInstField
       ...
       (Set.«term{_|_}»
        ...
        `x
        "|"
        (Term.app `c [(Term.proj `x "." (fieldIdx "1")) (Term.proj `x "." (fieldIdx "2"))])
        "}"))
      [","])
     (group
      (Term.structInstField
       ...
       ":="
       (Term.app (Term.proj `c.iseqv "." (fieldIdx "1")) [(numLit "1")]))
      [","])
     (group
      (Term.structInstField
       (Term.structInstLVal `mul_mem' [])
       ":="
       (Term.fun "fun" (Term.basicFun [(Term.simpleBinder [(Term.hole "_") (Term.hole "_")] [])] "=>" `c.mul)))
      [])]
    (Term.optEllipsis [])
    []
    "}")
...
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.structInstField', expected 'Lean.Parser.Term.structInstField.antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Set.«term{_|_}» "{" `x "|" (Term.app `c [(Term.proj `x "." (fieldIdx "1")) (Term.proj `x "." (fieldIdx "2"))]) "}")
...
theorem ker_lift_injective (f : M →* P) : injective (ker_lift f) := fun x y =>
...
-- failed to format: format: uncaught backtrack exception
instance
  _root_.add_con.quotient.has_nsmul
  { M : Type _ } [ AddMonoidₓ M ] ( c : AddCon M ) : HasScalar ℕ c.quotient
  where
    smul n x := ( Quotientₓ.liftOn' x fun w => ( ( n • w : M ) : c.quotient ) ) $ fun x y h => c.eq . 2 $ c.nsmul n h

-- failed to format: format: uncaught backtrack exception
@[ to_additive AddCon.Quotient.hasNsmul ]
  instance
    { M : Type _ } [ Monoidₓ M ] ( c : Con M ) : Pow c.quotient ℕ
    where pow x n := ( Quotientₓ.liftOn' x fun w => ( ( w ^ n : M ) : c.quotient ) ) $ fun x y h => c.eq . 2 $ c.pow n h
...
instance Semigroupₓ {M : Type _} [Semigroupₓ M] (c : Con M) : Semigroupₓ c.quotient :=
...
instance CommSemigroupₓ {M : Type _} [CommSemigroupₓ M] (c : Con M) : CommSemigroupₓ c.quotient :=
...
instance CommMonoidₓ {M : Type _} [CommMonoidₓ M] (c : Con M) : CommMonoidₓ c.quotient :=
  { c.comm_semigroup, c.monoid with }
...
instance Div : Div c.quotient :=
  ⟨fun x y =>
    (Quotientₓ.liftOn₂' x y fun w z => ((w / z : M) : c.quotient)) $ fun _ _ _ _ h1 h2 => c.eq.2 $ c.div h1 h2⟩
...
instance has_zpow : Pow c.quotient ℤ :=
