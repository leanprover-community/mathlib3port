...
variable [∀ i, HasSmul M (α i)] [∀ i, HasSmul N (α i)] (a : M) (i : ι) (b : α i) (x : Σ i, α i)

@[to_additive Sigma.hasVadd]
instance : HasSmul M (Σ i, α i) :=
...
@[to_additive]
instance [HasSmul M N] [∀ i, IsScalarTower M N (α i)] : IsScalarTower M N (Σ i, α i) :=
  ⟨fun a b x => by
    cases x
...
@[to_additive]
instance [∀ i, HasSmul Mᵐᵒᵖ (α i)] [∀ i, IsCentralScalar M (α i)] : IsCentralScalar M (Σ i, α i) :=
  ⟨fun a x => by
    cases x
...
protected theorem has_faithful_smul' [HasFaithfulSmul M (α i)] : HasFaithfulSmul M (Σ i, α i) :=
...
@[to_additive]
instance [Nonempty ι] [∀ i, HasFaithfulSmul M (α i)] : HasFaithfulSmul M (Σ i, α i) :=
  Nonempty.elim ‹_› $ fun i => Sigma.has_faithful_smul' i

end HasSmul

@[to_additive]
instance {m : Monoid M} [∀ i, MulAction M (α i)] : MulAction M (Σ i, α i) where
  mul_smul a b x := by
    cases x
