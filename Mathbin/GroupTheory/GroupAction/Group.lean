...
@[toAdditive " `add_monoid.to_add_action` is faithful on additive cancellative monoids. "]
...
@[simp, toAdditive]
theorem smul_inv_smul (c : α) (x : β) : c • c⁻¹ • x = x :=
  by 
...
theorem smul_inv [Groupₓ β] [SmulCommClass α β β] [IsScalarTower α β β] (c : α) (x : β) : (c • x)⁻¹ = c⁻¹ • x⁻¹ :=
  by 
