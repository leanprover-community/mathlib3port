...
attribute [toAdditive AddSubmonoid.LocalizationMap] Submonoid.LocalizationMap

attribute [toAdditive AddSubmonoid.LocalizationMap.toAddMonoidHom] Submonoid.LocalizationMap.toMonoidHom
...
-- error in GroupTheory.MonoidLocalization: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
...
@[to_additive #[expr "An alternate form of the congruence relation on `M × S`, `M` a `comm_monoid` and\n`S` a submonoid of `M`, whose quotient is the localization of `M` at `S`."]]
def r' : con «expr × »(M, S) :=
begin
  refine [expr { r := λ
     a
     b : «expr × »(M, S), «expr∃ , »((c : S), «expr = »(«expr * »(«expr * »(a.1, b.2), c), «expr * »(«expr * »(b.1, a.2), c))),
     iseqv := ⟨λ a, ⟨1, rfl⟩, λ (a b) ⟨c, hc⟩, ⟨c, hc.symm⟩, _⟩,
     .. }],
  { rintros [ident a, ident b, ident c, "⟨", ident t₁, ",", ident ht₁, "⟩", "⟨", ident t₂, ",", ident ht₂, "⟩"],
    use [expr «expr * »(«expr * »(b.2, t₁), t₂)],
    simp [] [] ["only"] ["[", expr submonoid.coe_mul, "]"] [] [],
    calc
      «expr = »(«expr * »(«expr * »(a.1, c.2), «expr * »(«expr * »(b.2, t₁), t₂)), «expr * »(«expr * »(«expr * »(«expr * »(a.1, b.2), t₁), c.2), t₂)) : by ac_refl
      «expr = »(..., «expr * »(«expr * »(«expr * »(«expr * »(b.1, c.2), t₂), a.2), t₁)) : by { rw [expr ht₁] [],
        ac_refl }
      «expr = »(..., «expr * »(«expr * »(c.1, a.2), «expr * »(«expr * »(b.2, t₁), t₂))) : by { rw [expr ht₂] [],
        ac_refl } },
  { rintros [ident a, ident b, ident c, ident d, "⟨", ident t₁, ",", ident ht₁, "⟩", "⟨", ident t₂, ",", ident ht₂, "⟩"],
    use [expr «expr * »(t₁, t₂)],
    calc
      «expr = »(«expr * »(«expr * »(«expr * »(a.1, c.1), «expr * »(b.2, d.2)), «expr * »(t₁, t₂)), «expr * »(«expr * »(«expr * »(a.1, b.2), t₁), «expr * »(«expr * »(c.1, d.2), t₂))) : by ac_refl
      «expr = »(..., «expr * »(«expr * »(«expr * »(b.1, d.1), «expr * »(a.2, c.2)), «expr * »(t₁, t₂))) : by { rw ["[", expr ht₁, ",", expr ht₂, "]"] [],
        ac_refl } }
end
...
@[toAdditive]
theorem r_iff_exists {x y : M × S} : r S x y ↔ ∃ c : S, ((x.1*y.2)*c) = (y.1*x.2)*c :=
  by 
    rw [r_eq_r' S] <;> rfl
...
@[toAdditive AddLocalization "The localization of an `add_comm_monoid` at one\nof its submonoids (as a quotient type)."]
def Localization :=
...
@[toAdditive]
instance  : CommMonoidₓ (Localization S) :=
  { mul := ·*·, one := 1,
    ...
    one_mul := show ∀ (x : Localization S), (1*x) = x from (r S).CommMonoid.one_mul, npow := Localization.npow S,
    npow_zero' := show ∀ (x : Localization S), Localization.npow S 0 x = 1 from pow_zeroₓ,
    npow_succ' :=
      show ∀ (n : ℕ) (x : Localization S), Localization.npow S n.succ x = x*Localization.npow S n x from
        fun n x => pow_succₓ x n }
...
def lift_on₂ {p : Sort u} (x y : Localization S) (f : M → S → M → S → p)
  ...
  lift_on x (fun a b => lift_on y (f a b) fun c c' d d' hy => H ((r S).refl _) hy)
    fun a a' b b' hx => induction_on y fun ⟨c, d⟩ => H hx ((r S).refl _)
...
@[toAdditive]
theorem mk_self (a : S) : mk (a : M) a = 1 :=
  by 
    symm 
...
@[toAdditive "Makes a localization map from an `add_comm_monoid` hom satisfying the characteristic\npredicate."]
...
@[toAdditive "Short for `to_add_monoid_hom`; used to apply a localization map as a function."]
abbrev to_map (f : localization_map S N) :=
  f.to_monoid_hom

@[ext, toAdditive]
theorem ext {f g : localization_map S N} (h : ∀ x, f.to_map x = g.to_map x) : f = g :=
  by 
    rcases f with ⟨⟨⟩⟩
    rcases g with ⟨⟨⟩⟩
    simp only 
...
@[toAdditive]
theorem to_map_injective : Function.Injective (@localization_map.to_map _ _ S N _) :=
  fun _ _ h => ext$ MonoidHom.ext_iff.1 h
...
@[toAdditive]
theorem sec_spec' {f : localization_map S N} (z : N) : f.to_map (f.sec z).1 = f.to_map (f.sec z).2*z :=
  by 
...
theorem inv_inj {f : M →* N} (hf : ∀ (y : S), IsUnit (f y)) {y z}
  (h : IsUnit.liftRight (f.mrestrict S) hf y⁻¹ = IsUnit.liftRight (f.mrestrict S) hf z⁻¹) : f y = f z :=
  by 
...
@[toAdditive]
theorem mk'_one x : f.mk' x (1 : S) = f.to_map x :=
  by 
...
@[toAdditive]
theorem mk'_spec' x (y : S) : (f.to_map y*f.mk' x y) = f.to_map x :=
  by 
...
@[toAdditive]
theorem mk'_eq_iff_eq_mul {x} {y : S} {z} : f.mk' x y = z ↔ f.to_map x = z*f.to_map y :=
  by 
...
@[toAdditive]
protected theorem eq' {a₁ b₁} {a₂ b₂ : S} : f.mk' a₁ a₂ = f.mk' b₁ b₂ ↔ Localization.r S (a₁, a₂) (b₁, b₂) :=
  by 
...
@[simp, toAdditive]
theorem mk'_self x (H : x ∈ S) : f.mk' x ⟨x, H⟩ = 1 :=
  by 
    convert mk'_self' _ _ <;> rfl

@[toAdditive]
theorem mul_mk'_eq_mk'_of_mul x₁ x₂ (y : S) : (f.to_map x₁*f.mk' x₂ y) = f.mk' (x₁*x₂) y :=
  by 
    rw [←mk'_one, ←mk'_mul, one_mulₓ]

@[toAdditive]
theorem mk'_mul_eq_mk'_of_mul x₁ x₂ (y : S) : (f.mk' x₂ y*f.to_map x₁) = f.mk' (x₁*x₂) y :=
  by 
    rw [mul_commₓ, mul_mk'_eq_mk'_of_mul]

@[toAdditive]
theorem mul_mk'_one_eq_mk' x (y : S) : (f.to_map x*f.mk' 1 y) = f.mk' x y :=
  by 
    rw [mul_mk'_eq_mk'_of_mul, mul_oneₓ]

@[simp, toAdditive]
theorem mk'_mul_cancel_right (x : M) (y : S) : f.mk' (x*y) y = f.to_map x :=
  by 
    rw [←mul_mk'_one_eq_mk', f.to_map.map_mul, mul_assocₓ, mul_mk'_one_eq_mk', mk'_self', mul_oneₓ]

@[toAdditive]
theorem mk'_mul_cancel_left x (y : S) : f.mk' ((y : M)*x) y = f.to_map x :=
  by 
...
`k (g x) = k (g y)`. -/
@[to_additive #[expr "Given `add_comm_monoid`s `M, P`, localization maps `f : M →+ N, k : P →+ Q` for\nsubmonoids `S, T` respectively, and `g : M →+ P` such that `g(S) ⊆ T`, `f x = f y`\nimplies `k (g x) = k (g y)`."]]
...
{x y}
(h : «expr = »(f.to_map x, f.to_map y)) : «expr = »(k.to_map (g x), k.to_map (g y)) :=
...
@[toAdditive]
theorem lift_mk'_spec x v (y : S) : f.lift hg (f.mk' x y) = v ↔ g x = g y*v :=
  by 
...
theorem lift_mul_left z : (g (f.sec z).2*f.lift hg z) = g (f.sec z).1 :=
  by 
    rw [mul_commₓ, lift_mul_right]

@[simp, toAdditive]
theorem lift_eq (x : M) : f.lift hg (f.to_map x) = g x :=
  by 
...
@[simp, toAdditive]
theorem lift_comp : (f.lift hg).comp f.to_map = g :=
  by 
...
@[toAdditive]
theorem epic_of_localization_map {j k : N →* P} (h : ∀ a, j.comp f.to_map a = k.comp f.to_map a) : j = k :=
  by 
...
@[toAdditive]
theorem lift_unique {j : N →* P} (hj : ∀ x, j (f.to_map x) = g x) : f.lift hg = j :=
  by 
    ext 
...
theorem map_mul_left z : (k.to_map (g (f.sec z).2)*f.map hy k z) = k.to_map (g (f.sec z).1) :=
  by 
...
theorem map_map {A : Type _} [CommMonoidₓ A] {U : Submonoid A} {R} [CommMonoidₓ R] (j : localization_map U R)
  ...
  k.map hl j (f.map hy k x) = f.map (fun x => show l.comp g x ∈ U from hl ⟨g x, hy x⟩) j x :=
  by 
...
variable{A :
    Type
      _}[AddCommMonoidₓ
      A](x : A){B : Type _}[AddCommMonoidₓ B](F : away_map x B){C : Type _}[AddCommMonoidₓ C]{g : A →+ C}
...
variable(f : S.localization_map N){g : M →* P}(hg : ∀ (y : S), IsUnit (g y)){T : Submonoid P}{Q : Type _}[CommMonoidₓ Q]
...
@[toAdditive add_equiv_of_localizations_right_inv]
...
@[simp, toAdditive add_equiv_of_localizations_right_inv_apply]
...
@[simp, toAdditive]
theorem of_mul_equiv_of_localizations_id : f.of_mul_equiv_of_localizations (MulEquiv.refl N) = f :=
  by 
    ext <;> rfl
        ...
        erw [f.eq_iff_exists] <;>
          exact
            ⟨fun ⟨c, hc⟩ =>
                ...
                ⟨⟨d, H' ▸ show k d ∈ S from hd.symm ▸ c.2⟩,
                  by 
                    erw [←hd, ←k.map_mul, ←k.map_mul] at hc <;> exact k.to_equiv.injective hc⟩,
              fun ⟨c, hc⟩ =>
                ⟨⟨k c, H ▸ Set.mem_image_of_mem k c.2⟩,
                  by 
...
@[simp, toAdditive "A special case of `f ∘ id = f`, `f` a localization map."]
...
-- error in GroupTheory.MonoidLocalization: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp, to_additive #[]]
theorem mul_equiv_of_mul_equiv_eq_map_apply
{k : localization_map T Q}
{j : «expr ≃* »(M, P)}
(H : «expr = »(S.map j.to_monoid_hom, T))
(x) : «expr = »(f.mul_equiv_of_mul_equiv k H x, f.map (λ
  y : S, show «expr ∈ »(j.to_monoid_hom y, T), from «expr ▸ »(H, set.mem_image_of_mem j y.2)) k x) :=
rfl
...
{k : localization_map T Q}
{j : «expr ≃* »(M, P)}
(H : «expr = »(S.map j.to_monoid_hom, T)) : «expr = »((f.mul_equiv_of_mul_equiv k H).to_monoid_hom, f.map (λ
  y : S, show «expr ∈ »(j.to_monoid_hom y, T), from «expr ▸ »(H, set.mem_image_of_mem j y.2)) k) :=
rfl

-- error in GroupTheory.MonoidLocalization: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp, to_additive #[]]
theorem mul_equiv_of_mul_equiv_eq
{k : localization_map T Q}
{j : «expr ≃* »(M, P)}
(H : «expr = »(S.map j.to_monoid_hom, T))
(x) : «expr = »(f.mul_equiv_of_mul_equiv k H (f.to_map x), k.to_map (j x)) :=
f.map_eq (λ y : S, «expr ▸ »(H, set.mem_image_of_mem j y.2)) _

-- error in GroupTheory.MonoidLocalization: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp, to_additive #[]]
theorem mul_equiv_of_mul_equiv_mk'
{k : localization_map T Q}
{j : «expr ≃* »(M, P)}
(H : «expr = »(S.map j.to_monoid_hom, T))
(x
 y) : «expr = »(f.mul_equiv_of_mul_equiv k H (f.mk' x y), k.mk' (j x) ⟨j y, «expr ▸ »(H, set.mem_image_of_mem j y.2)⟩) :=
f.map_mk' (λ y : S, «expr ▸ »(H, set.mem_image_of_mem j y.2)) _ _
...
theorem mk_eq_monoid_of_mk'_apply x y : mk x y = (monoid_of S).mk' x y :=
  show _ = _*_ from
    (Submonoid.LocalizationMap.mul_inv_right (monoid_of S).map_units _ _ _).2$
      by 
...
@[toAdditive]
theorem mul_equiv_of_quotient_mk x y : mul_equiv_of_quotient f (mk x y) = f.mk' x y :=
  by 
...
@[toAdditive]
theorem mul_equiv_of_quotient_symm_mk x y : (mul_equiv_of_quotient f).symm (f.mk' x y) = mk x y :=
  by 
...
@[reducible, toAdditive "Given `x : M`, the localization of `M` at the submonoid generated\nby `x`, as a quotient."]
def away :=
