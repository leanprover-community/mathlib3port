...
noncomputable def coproductCofan :
    Cocone F where 
  x := coproduct F
  ι :=
...
noncomputable def coproductCofanIsColimit :
    IsColimit
      (coproductCofan
        F) where 
  desc s :=
...
theorem is_local_ring_hom_stalk_map_congr {X Y : RingedSpaceCat} (f g : X ⟶ Y) (H : f = g) (x)
    ...
    IsLocalRingHom (PresheafedSpaceCat.stalkMap g x) := by
