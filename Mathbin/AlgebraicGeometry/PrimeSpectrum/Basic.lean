...
theorem
  ...
  ( t t' : Set PrimeSpectrum R ) : vanishing_ideal t ⊔ vanishing_ideal t' ≤ vanishing_ideal t ∩ t'
  :=
    by
      intro r
        ...
        apply Submodule.add_mem <;> solveByElim

theorem mem_compl_zero_locus_iff_not_mem {f : R} {I : PrimeSpectrum R} :
  I ∈ (zero_locus {f} : Set (PrimeSpectrum R))ᶜ ↔ f ∉ I.as_ideal :=
  by 
...
theorem is_open_iff (U : Set (PrimeSpectrum R)) : IsOpen U ↔ ∃ s, Uᶜ = zero_locus s :=
  by 
...
theorem localization_comap_inducing [Algebra R S] (M : Submonoid R) [IsLocalization M S] :
  ...
  by 
    ...
    intro U 
    simpRw [←is_closed_compl_iff]
    generalize Uᶜ = Z 
    simpRw [is_closed_induced_iff, is_closed_iff_zero_locus]
    constructor
    ·
      rintro ⟨s, rfl⟩
