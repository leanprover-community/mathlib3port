...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (z «expr ∉ » M)
/--In a noetherian ring, every ideal contains a product of prime ideals
([samuel, § 3.3, Lemma 3])-/
theorem exists_prime_spectrum_prod_le (I : Ideal R) :
  ∃ Z : Multiset (PrimeSpectrum R), Multiset.prod (Z.map (coeₓ : Subtype _ → Ideal R)) ≤ I :=
  by 
    refine' IsNoetherian.induction (fun M : Ideal R hgt => _) I 
    byCases' h_prM : M.is_prime
    ·
      use {⟨M, h_prM⟩}
