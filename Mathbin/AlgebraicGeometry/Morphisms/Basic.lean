...
theorem AffineTargetMorphismProperty.respectsIso_mk {P : AffineTargetMorphismProperty}
    ...
    (h₂ :
      ∀ {X Y Z} (e : Y ≅ Z) (f : X ⟶ Y) [h : IsAffine Y],
        P f → @P (f ≫ e.hom) (isAffineOfIso e.inv)) :
    P.toProperty.RespectsIso := by
  constructor
  · rintro X Y Z e f ⟨a, h⟩
    exact ⟨a, h₁ e f h⟩
  · rintro X Y Z e f ⟨a, h⟩
    exact ⟨isAffineOfIso e.inv, h₂ e f h⟩
...
theorem targetAffineLocally_respectsIso {P : AffineTargetMorphismProperty}
  ...
  · introv H U
    ...
    exact H U
  · introv H
    rintro ⟨U, hU : IsAffineOpen U⟩
    dsimp
    haveI : IsAffine _ := hU
    haveI : IsAffine _ := hU.map_is_iso e.hom
...
structure AffineTargetMorphismProperty.IsLocal (P : AffineTargetMorphismProperty) : Prop where
  ...
  toBasicOpen :
    ∀ {X Y : Scheme} [IsAffine Y] (f : X ⟶ Y) (r : Y.presheaf.obj <| op ⊤),
      P f → @P (f ∣_ Y.basic_open r) ((top_isAffineOpen Y).basicOpen_is_affine _)
  ofBasicOpenCover :
    ∀ {X Y : Scheme} [IsAffine Y] (f : X ⟶ Y) (s : Finset (Y.presheaf.obj <| op ⊤))
      (hs : Ideal.span (s : Set (Y.presheaf.obj <| op ⊤)) = ⊤),
      (∀ r : s, @P (f ∣_ Y.basic_open r.1) ((top_isAffineOpen Y).basicOpen_is_affine _)) → P f
  ...
  classical
    ...
    · intro U s hs H
      ...
      · apply_fun Ideal.comap (Y.presheaf.map (eqToHom U.1.openEmbedding_obj_top.symm).op)  at hs
        ...
        have :
          ∀ {R S : CommRingCat} (e : S = R) (s : Set S),
            Ideal.span (eqToHom e '' s) = Ideal.comap (eqToHom e.symm) (Ideal.span s) :=
          by
          intros
          subst e
          simpa
      ...
      · rintro ⟨r, hr⟩
        ...
        exact (hP.1.arrow_mk_iso_iff (morphismRestrictRestrictBasicOpen f _ r)).mpr (H ⟨r, hr'⟩)
    ...
    · rintro ⟨_, i, rfl⟩
      ...
      exact (hP.1.arrow_mk_iso_iff (morphismRestrictOpensRange f _)).mpr (h𝒰 i)
...
theorem AffineTargetMorphismProperty.IsLocal.affine_openCover_tFAE
  ...
  · intro H U g h₁ h₂
    skip
    replace H := H ⟨⟨_, h₂.base_open.open_range⟩, range_isAffineOpen_of_open_immersion g⟩
  ...
  tfae_have 2 → 1
  · rintro ⟨𝒰, h𝒰, H⟩
    exact targetAffineLocallyOfOpenCover hP f 𝒰 H
  tfae_have 5 → 2
  · rintro ⟨ι, U, hU, hU', H⟩
    refine' ⟨Y.open_cover_of_supr_eq_top U hU, hU', _⟩
    intro i
    specialize H i
    rw [← P.to_property_apply, ← hP.1.arrow_mk_iso_iff (morphismRestrictOpensRange f _)]
    rw [← P.to_property_apply] at H
    convert H
  ...
  · intro H
    refine'
      ⟨Y.carrier, fun x => (Y.affine_cover.map x).opensRange, _, fun i =>
        range_isAffineOpen_of_open_immersion _, _⟩
    · rw [eq_top_iff]
      intro x _
      erw [Opens.mem_supᵢ]
      exact ⟨x, Y.affine_cover.covers x⟩
    · intro i
      exact H ⟨_, range_isAffineOpen_of_open_immersion _⟩
  tfae_finish
...
theorem AffineTargetMorphismProperty.isLocalOfOpenCoverImply (P : AffineTargetMorphismProperty)
  ...
  · introv h
    skip
    haveI : IsAffine _ := (top_isAffineOpen Y).basicOpen_is_affine r
    delta morphism_restrict
    rw [affine_cancel_left_isIso hP]
    refine' @H f ⟨Scheme.openCoverOfIsIso (𝟙 Y), _, _⟩ (Y.of_restrict _) _inst _
    · intro i
      dsimp
      infer_instance
    · intro i
  ...
  · introv hs hs'
    skip
    replace hs := ((top_isAffineOpen Y).basicOpen_union_eq_self_iff _).mpr hs
    have := H f ⟨Y.open_cover_of_supr_eq_top _ hs, _, _⟩ (𝟙 _)
    rwa [← Category.comp_id pullback.snd, ← pullback.condition, affine_cancel_left_isIso hP] at this
    · intro i
      exact (top_isAffineOpen Y).basicOpen_is_affine _
    · rintro (i : s)
      specialize hs' i
      haveI : IsAffine _ := (top_isAffineOpen Y).basicOpen_is_affine i.1
...
theorem AffineTargetMorphismProperty.IsLocal.affine_target_iff {P : AffineTargetMorphismProperty}
  ...
  swap; · exact Scheme.openCoverOfIsIso (𝟙 Y)
  swap;
  · intro
    dsimp
    infer_instance
...
theorem AffineTargetMorphismProperty.IsLocal.targetAffineLocallyIsLocal
  ...
  · intro X Y f U H V
    rw [← P.to_property_apply, hP.1.arrow_mk_iso_iff (morphismRestrictRestrict f _ _)]
    convert H ⟨_, IsAffineOpen.image_isOpenImmersion V.2 (Y.of_restrict _)⟩
...
theorem IsLocal.targetAffineLocallyPullbackFstOfRightOfStableUnderBaseChange
  ...
  intro i
  let e := pullbackSymmetry _ _ ≪≫ pullbackRightPullbackFstIso f g (X.affine_cover.map i)
...
theorem diagonalTargetAffineLocallyOfOpenCover (P : AffineTargetMorphismProperty) (hP : P.IsLocal)
  ...
  · rintro ⟨i, j, k⟩
    dsimp
    convert
      (affine_cancel_left_isIso hP.1 (pullbackDiagonalMapIso _ _ ((𝒰' i).map j) ((𝒰' i).map k)).inv
            pullback.snd).mp
        _
    pick_goal 3
    · convert h𝒰' i j k
    ...
    TFAE
      [(targetAffineLocally P).diagonal f,
        ...
        ∃ (𝒰 : Scheme.OpenCover.{u} Y)(_ : ∀ i, IsAffine (𝒰.obj i))(𝒰' :
          ...
          ∀ i j k, P (pullback.mapDesc ((𝒰' i).map j) ((𝒰' i).map k) pullback.snd)] :=
  by
  tfae_have 1 → 4
  · introv H hU hg _ _
    skip
...
theorem AffineTargetMorphismProperty.IsLocal.diagonal {P : AffineTargetMorphismProperty}
  ...
  AffineTargetMorphismProperty.isLocalOfOpenCoverImply P.diagonal
    (congr_arg.diagonal_respectsIso hP.1) fun _ _ f =>
    ((hP.diagonal_affine_openCover_tFAE f).out 1 3).mp
...
theorem universallyIsLocalAtTarget (P : MorphismProperty Scheme)
  ...
  refine'
    ⟨P.universally_respects_iso, fun X Y f U =>
      P.universally_stable_under_base_change (isPullbackMorphismRestrict f U).flip, _⟩
  ...
  refine' (IsPullback.ofRight _ (pullback.lift_snd _ _ _) (IsPullback.ofHasPullback _ _)).flip
  rw [pullback.lift_fst, ← pullback.condition]
  exact (IsPullback.ofHasPullback _ _).pasteHoriz H.flip
