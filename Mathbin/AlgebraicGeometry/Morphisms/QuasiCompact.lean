...
theorem isCompact_open_iff_eq_finset_affine_union {X : Scheme} (U : Set X.carrier) :
  ...
  by
  apply
    Opens.isCompact_open_iff_eq_finite_unionᵢ_of_isBasis (coe : X.affine_opens → Opens X.carrier)
...
theorem isCompact_open_iff_eq_basicOpen_union {X : Scheme} [IsAffine X] (U : Set X.carrier) :
  ...
  by
  apply Opens.isCompact_open_iff_eq_finite_unionᵢ_of_isBasis
  · exact isBasis_basicOpen X
  · intro i
...
theorem isCompact_basicOpen (X : Scheme) {U : Opens X.carrier} (hU : IsCompact (U : Set X.carrier))
  ...
  classical
    ...
    obtain ⟨s, hs, e⟩ := (isCompact_open_iff_eq_finset_affine_union _).mp ⟨hU, U.prop⟩
    let g : s → X.affine_opens := by
      ...
      exact IsAffineOpen.basicOpen_is_affine V.1.prop _
    haveI : Finite s := hs.to_subtype
    refine' ⟨Set.range g, Set.finite_range g, _⟩
    refine' (set.inter_eq_right_iff_subset.mpr (RingedSpace.basicOpen_le _ _)).symm.trans _
...
theorem QuasiCompact.affinePropertyIsLocal : (QuasiCompact.affineProperty : _).IsLocal :=
  by
  constructor
  · apply AffineTargetMorphismProperty.respectsIso_mk <;> rintro X Y Z _ _ _ H
    exacts[@homeomorph.compact_space _ _ H (TopCat.homeoOfIso (asIso e.inv.1.base)), H]
  · introv H
    ...
    exact isCompact_basicOpen X H _
  · rintro X Y H f S hS hS'
    ...
    rw [← hS]
    dsimp [Opens.map]
    simp only [Opens.coe_supᵢ, Set.preimage_unionᵢ, Subtype.val_eq_coe]
    exacts[isCompact_unionᵢ fun i => is_compact_iff_compact_space.mpr (hS' i), top_isAffineOpen _]
...
theorem QuasiCompact.affineProperty_stableUnderBaseChange :
  ...
  skip
  let 𝒰 := Scheme.Pullback.openCoverOfRight Y.affine_cover.finite_subcover f g
  have : Finite 𝒰.J := by
    dsimp [𝒰]
    infer_instance
...
theorem exists_pow_mul_eq_zero_of_res_basicOpen_eq_zero_of_isAffineOpen (X : Scheme)
  ...
  by
  rw [← map_zero (X.presheaf.map (homOfLe <| X.basic_open_le f : X.basic_open f ⟶ U).op)] at H
  have := (is_localization_basicOpen hU f).3
