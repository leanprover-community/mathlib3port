  ...
  · introv H h₁ h₂
    ...
    exact
      @homeomorph.compact_space _ _
        (H ⟨⟨_, h₁.base_open.open_range⟩, range_isAffineOpen_of_open_immersion _⟩
          ⟨⟨_, h₂.base_open.open_range⟩, range_isAffineOpen_of_open_immersion _⟩)
        e.symm
...
theorem quasiSeparated_eq_affineProperty_diagonal :
  ...
  exact diagonal_targetAffineLocally_eq_targetAffineLocally _ QuasiCompact.affinePropertyIsLocal
...
theorem quasiSeparated_over_affine_iff {X Y : Scheme} (f : X ⟶ Y) [IsAffine Y] :
  ...
  rw [quasiSeparated_eq_affineProperty,
    quasi_separated.affine_property_is_local.affine_target_iff f, QuasiSeparated.affineProperty]
...
instance quasiSeparatedSpace_of_isAffine (X : Scheme) [IsAffine X] :
  ...
  · intro i hi
    ...
    exact ((top_isAffineOpen _).basicOpen_is_affine _).isCompact
...
theorem quasiSeparatedOfComp {X Y Z : Scheme} (f : X ⟶ Y) (g : Y ⟶ Z) [H : QuasiSeparated (f ≫ g)] :
    QuasiSeparated f :=
  by
  rw [(QuasiSeparated.affine_openCover_tFAE f).out 0 1]
  rw [(QuasiSeparated.affine_openCover_tFAE (f ≫ g)).out 0 2] at H
  use (Z.affine_cover.pullback_cover g).bind fun x => Scheme.affineCover _
  constructor;
  · intro i
    dsimp
    infer_instance
  ...
  ·
    exact
      pullback.map _ _ _ _ (𝟙 _) _ _ (by simp) (Category.comp_id _) ≫
        (pullbackRightPullbackFstIso g (Z.affine_cover.map i) f).hom
...
theorem exists_eq_pow_mul_of_isAffineOpen (X : Scheme) (U : Opens X.carrier) (hU : IsAffineOpen U)
  ...
  by
  have := (is_localization_basicOpen hU f).2
  ...
  · -- Given `f : 𝒪(S ∪ U), x : 𝒪(X_f)`, we need to show that `f ^ n * x` is the restriction of
    ...
    refine'
      X.sheaf.eq_of_locally_eq₂
        (homOfLe (_ : X.basic_open (X.presheaf.map (homOfLe le_sup_left).op f) ≤ _))
        (homOfLe (_ : X.basic_open (X.presheaf.map (homOfLe le_sup_right).op f) ≤ _)) _ _ _ _ _
    · rw [X.basic_open_res]
      exact inf_le_right
    · rw [X.basic_open_res]
      exact inf_le_right
