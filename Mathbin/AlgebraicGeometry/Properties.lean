...
instance : QuasiSober X.carrier :=
  ...
  · rintro ⟨_, i, rfl⟩
    exact
      @open_embedding.quasi_sober _ _ _
        (Homeomorph.ofEmbedding _
              (X.affine_cover.is_open i).base_open.to_embedding).symm.openEmbedding
...
theorem isReduced_of_open_immersion {X Y : Scheme} (f : X ⟶ Y) [H : IsOpenImmersion f]
  ...
  exact
    isReduced_of_injective (inv <| f.1.c.app (op <| H.base_open.is_open_map.functor.obj U))
      (asIso <| f.1.c.app (op <| H.base_open.is_open_map.functor.obj U) :
...
instance {R : CommRingCat} [H : IsReduced R] : IsReduced (Scheme.spec.obj <| op R) :=
  by
  apply (config := { instances := false }) isReduced_of_stalk_isReduced
  intro x
  dsimp
  have : IsReduced (CommRingCat.of <| Localization.AtPrime (PrimeSpectrum.asIdeal x)) :=
    by
    dsimp
    infer_instance
  exact
    isReduced_of_injective (StructureSheaf.stalkIso R x).hom
      (StructureSheaf.stalkIso R x).commRingIsoToRingEquiv.injective
...
theorem affine_isReduced_iff (R : CommRingCat) :
  ...
  intro h
  skip
  have : IsReduced (LocallyRingedSpace.Γ.obj (op <| Spec.to_LocallyRingedSpace.obj <| op R)) :=
    by
    change IsReduced ((Scheme.Spec.obj <| op R).presheaf.obj <| op ⊤)
    infer_instance
  exact isReduced_of_injective (toSpecΓ R) (asIso <| toSpecΓ R).commRingIsoToRingEquiv.Injective
...
theorem isReduced_of_isAffine_isReduced [IsAffine X] [h : IsReduced (X.presheaf.obj (op ⊤))] :
  ...
  haveI : IsReduced (Scheme.Spec.obj (op (Scheme.Γ.obj (op X)))) :=
    ...
    exact h
  isReduced_of_open_immersion X.iso_Spec.hom
...
theorem eq_zero_of_basicOpen_eq_bot {X : Scheme} [hX : IsReduced X] {U : Opens X.carrier}
  ...
  · rintro X Y f hf
    ...
    haveI := isReduced_of_open_immersion f
    specialize
      H (f.1.c.app _ s) _
        ⟨x, by
...
instance (priority := 900) isReduced_of_isIntegral [IsIntegral X] : IsReduced X :=
  ...
  · have : U = ∅ := Subtype.eq h
    ...
    change IsReduced (X.sheaf.val.obj (op U))
    infer_instance
...
theorem isIntegral_iff_is_irreducible_and_isReduced :
  ...
  ⟨fun _ => ⟨inferInstance, inferInstance⟩, fun ⟨_, _⟩ => isIntegral_of_is_irreducible_isReduced X⟩
...
theorem isIntegral_of_open_immersion {X Y : Scheme} (f : X ⟶ Y) [H : IsOpenImmersion f]
  ...
  have : IsDomain (Y.presheaf.obj (op (H.base_open.is_open_map.functor.obj U))) :=
    by
    apply (config := { instances := false }) IsIntegral.component_integral
    infer_instance
...
instance {R : CommRingCat} [H : IsDomain R] : IsIntegral (Scheme.spec.obj <| op R) :=
  by
  apply (config := { instances := false }) isIntegral_of_is_irreducible_isReduced
  · infer_instance
  · dsimp [Spec.topObj]
    infer_instance
...
theorem affine_isIntegral_iff (R : CommRingCat) :
  ...
  ⟨fun h =>
    RingEquiv.isDomain ((Scheme.Spec.obj <| op R).presheaf.obj _)
      (asIso <| toSpecΓ R).commRingIsoToRingEquiv,
    fun h => inferInstance⟩
...
theorem isIntegral_of_isAffine_isDomain [IsAffine X] [Nonempty X.carrier]
  ...
  haveI : IsIntegral (Scheme.Spec.obj (op (Scheme.Γ.obj (op X)))) :=
    ...
    exact h
  isIntegral_of_open_immersion X.iso_Spec.hom
...
theorem map_injective_of_isIntegral [IsIntegral X] {U V : Opens X.carrier} (i : U ⟶ V)
  ...
  apply nonempty_preirreducible_inter U.prop (RingedSpace.basicOpen _ _).prop
  simpa using H
