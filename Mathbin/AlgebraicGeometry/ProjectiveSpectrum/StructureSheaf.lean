...
theorem add_mem' (U : (Opens (ProjectiveSpectrum.top 𝒜))ᵒᵖ) (a b : ∀ x : unop U, at x.1)
  ...
  · cases' (y : ProjectiveSpectrum.top 𝒜).isPrime.mem_or_mem h with h h
    · obtain ⟨nin, -⟩ := wa ⟨y, (Opens.infLeLeft Va Vb y).2⟩
      exact nin h
    · obtain ⟨nin, -⟩ := wb ⟨y, (Opens.infLeRight Va Vb y).2⟩
      exact nin h
...
theorem mul_mem' (U : (Opens (ProjectiveSpectrum.top 𝒜))ᵒᵖ) (a b : ∀ x : unop U, at x.1)
  ...
  · cases' (y : ProjectiveSpectrum.top 𝒜).isPrime.mem_or_mem h with h h
    · choose nin hy using wa ⟨y, (Opens.infLeLeft Va Vb y).2⟩
      exact nin h
    · choose nin hy using wb ⟨y, (Opens.infLeRight Va Vb y).2⟩
      exact nin h
...
theorem stalkToFiberRingHom_germ (U : Opens (ProjectiveSpectrum.top 𝒜)) (x : U)
  ...
  by
  cases x
  exact stalkToFiberRingHom_germ' 𝒜 U _ _ _
#align algebraic_geometry.stalk_to_fiber_ring_hom_germ AlgebraicGeometry.stalkToFiberRingHom_germ
...
def Proj.stalkIso' (x : ProjectiveSpectrum.top 𝒜) :
  ...
  RingEquiv.ofBijective (stalkToFiberRingHom _ x)
    ⟨fun z1 z2 eq1 =>
          ...
          (Localization.mk a1
                ⟨b1,
                  show b1 ∉ y.asHomogeneousIdeal by
                    rw [← ProjectiveSpectrum.mem_basicOpen] <;>
                      exact le_of_hom (Opens.infLeLeft _ _ ≫ Opens.infLeLeft _ _) hy⟩ :
              Localization.AtPrime y.1.toIdeal) =
            Localization.mk a2
              ⟨b2,
                show b2 ∉ y.asHomogeneousIdeal by
                  rw [← ProjectiveSpectrum.mem_basicOpen] <;>
                    exact le_of_hom (Opens.infLeLeft _ _ ≫ Opens.infLeRight _ _) hy⟩ :=
        by
        intro y hy
        ...
        exact
          ⟨⟨c,
              show c ∉ y.as_homogeneous_ideal by
                rw [← ProjectiveSpectrum.mem_basicOpen] <;>
                  exact le_of_hom (Opens.infLeRight _ _) hy⟩,
            eq3⟩
      refine'
      ...
      simp only [res_apply]
      obtain ⟨b1_nin_y, eq6⟩ :=
        hs1 ⟨_, le_of_hom (Opens.infLeLeft _ _ ≫ opens.inf_le_right _ _) y.2⟩
      obtain ⟨b2_nin_y, eq7⟩ := hs2 ⟨_, le_of_hom (Opens.infLeRight _ _) y.2⟩
      ...
      exact
        eq3' _
          ⟨⟨le_of_hom
                (Opens.infLeLeft _ _ ≫
                  Opens.infLeLeft _ _ ≫ Opens.infLeLeft _ _ ≫ Opens.infLeLeft _ _)
                y.2,
              le_of_hom
                (Opens.infLeLeft _ _ ≫
                  Opens.infLeLeft _ _ ≫ Opens.infLeLeft _ _ ≫ Opens.infLeRight _ _)
                y.2⟩,
            le_of_hom (Opens.infLeLeft _ _ ≫ Opens.infLeLeft _ _ ≫ Opens.infLeRight _ _) y.2⟩,
      Function.surjective_iff_hasRightInverse.mpr
        ⟨homogeneousLocalizationToStalk 𝒜 x, fun f =>
          by
          rw [homogeneousLocalizationToStalk]
          erw [stalkToFiberRingHom_germ 𝒜 (ProjectiveSpectrum.basicOpen 𝒜 f.denom) ⟨x, _⟩
              (sectionInBasicOpen _ x f)]
          simp only [sectionInBasicOpen, Subtype.ext_iff_val, HomogeneousLocalization.ext_iff_val,
