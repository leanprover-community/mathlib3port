...
theorem MemCarrier.clear_denominator' [DecidableEq (Away f)] {z : Localization.Away f}
  ...
  congr 1
  ext i
  rw [map_mul, hacd, (Classical.choose_spec i.1.2).2, smul_eq_mul, smul_mul_assoc]
  rfl
...
theorem carrier_ne_top : carrier 𝒜 x ≠ ⊤ :=
  ...
  classical
    contrapose! eq_top
    obtain ⟨c, N, acd, eq1⟩ := MemCarrier.clear_denominator _ x ((Ideal.eq_top_iff_one _).mp eq_top)
...
theorem preimage_eq (a b : A) (k : ℕ) (a_mem : a ∈ 𝒜 k) (b_mem1 : b ∈ 𝒜 k)
  ...
  classical
    ...
    · change y.1 ∈ _ at hy
      ...
      rw [Set.mem_preimage, toFun, Opens.mem_coe, PrimeSpectrum.mem_basicOpen]
      intro rid
      dsimp at rid
      rcases MemCarrier.clear_denominator 𝒜 _ rid with ⟨c, N, acd, eq1⟩
...
def toSpec {f : A} : (Proj.T| pbo f) ⟶ Spec.T A⁰_ f
  ...
  continuous_toFun :=
    by
    apply IsTopologicalBasis.continuous prime_spectrum.is_topological_basis_basic_opens
...
/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:72:18: unsupported non-interactive tactic _private.3599523199.mem_tac -/
/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:72:18: unsupported non-interactive tactic _private.3599523199.mem_tac -/
