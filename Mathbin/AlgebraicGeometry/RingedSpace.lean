...
theorem mem_basic_open {U : opens X} (f : X.presheaf.obj (op U)) (x : U) :
  ↑x ∈ X.basic_open f ↔ IsUnit (X.presheaf.germ x f) :=
  by 
    constructor
    ·
      rintro ⟨x, hx, a⟩
...
theorem basic_open_subset {U : opens X} (f : X.presheaf.obj (op U)) : X.basic_open f ⊆ U :=
  by 
    rintro _ ⟨x, hx, rfl⟩
    exact x.2
...
theorem is_unit_res_basic_open {U : opens X} (f : X.presheaf.obj (op U)) :
  IsUnit (X.presheaf.map (@hom_of_le (opens X) _ _ _ (X.basic_open_subset f)).op f) :=
  by 
