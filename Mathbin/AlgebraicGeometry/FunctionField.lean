...
noncomputable instance [IsIntegral X] : Field X.functionField :=
  ...
  have hs : genericPoint X.carrier ∈ RingedSpace.basicOpen _ s :=
    ...
    exacts[ha, (RingedSpace.basicOpen _ _).prop]
...
instance {X : Scheme} [IsIntegral X] {U : Opens X.carrier} [hU : Nonempty U] :
  ...
  isIntegral_of_open_immersion (X.of_restrict U.open_embedding)
...
theorem IsAffineOpen.primeIdealOf_genericPoint {X : Scheme} [IsIntegral X] {U : Opens X.carrier}
  ...
  ext1
  exact (genericPoint_eq_of_isOpenImmersion (X.of_restrict U.open_embedding)).symm
...
theorem functionField_isFractionRing_of_isAffineOpen [IsIntegral X] (U : Opens X.carrier)
  ...
  rw [hU.prime_ideal_of_generic_point, genericPoint_eq_bot_of_affine]
  ext
  exact mem_nonZeroDivisors_iff_ne_zero
...
instance [h : IsIntegral X] (x : X.carrier) : IsFractionRing (X.presheaf.stalk x) X.functionField :=
  ...
  have hU : IsAffineOpen U := range_isAffineOpen_of_open_immersion (X.affine_cover.map x)
  exact
    @is_fraction_ring.is_fraction_ring_of_is_domain_of_is_localization _ _ _ _ _ _ _ _ _ _ _
