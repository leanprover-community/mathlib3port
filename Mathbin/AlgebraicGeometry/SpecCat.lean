...
-/
@[simps]
def SpecCat.sheafedSpaceObj (R : CommRingCat) :
...
def SpecCat.sheafedSpaceMap {R S : CommRingCat.{u}} (f : R ⟶ S) :
    SpecCat.sheafedSpaceObj S ⟶
      SpecCat.sheafedSpaceObj R where 
...
-/
@[simps]
def SpecCat.toLocallyRingedSpace :
    CommRingCatᵒᵖ ⥤
