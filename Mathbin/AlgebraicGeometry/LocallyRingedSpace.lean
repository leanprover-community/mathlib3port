...
-/
@[simps]
def hom_of_SheafedSpace_hom_of_is_iso {X Y : LocallyRingedSpace} (f : X.toSheafedSpace ⟶ Y.toSheafedSpace) [IsIso f] :
    X ⟶ Y :=
  (Subtype.mk f) fun x =>
...
-/
def iso_of_SheafedSpace_iso {X Y : LocallyRingedSpace} (f : X.toSheafedSpace ≅ Y.toSheafedSpace) : X ≅ Y where
...
def of_restrict {U : Top} (X : LocallyRingedSpace) {f : U ⟶ X.toTop} (h : OpenEmbedding f) : X.restrict h ⟶ X :=
