...
-/
noncomputable def stalkMap {X Y : LocallyRingedSpaceCat} (f : X ⟶ Y) (x : X) : Y.stalk (f.1.1 x) ⟶ X.stalk x :=
...
instance {X Y : LocallyRingedSpaceCat} (f : X ⟶ Y) (x : X) : IsLocalRingHom (PresheafedSpaceCat.stalkMap f.1 x) :=
  f.2 x
...
@[simp]
theorem comp_val {X Y Z : LocallyRingedSpaceCat} (f : X ⟶ Y) (g : Y ⟶ Z) : (f ≫ g).val = f.val ≫ g.val :=
  rfl
...
def homOfSheafedSpaceHomOfIsIso {X Y : LocallyRingedSpaceCat} (f : X.toSheafedSpace ⟶ Y.toSheafedSpace) [IsIso f] :
  ...
  Hom.mk f $ fun x =>
    ...
    show IsLocalRingHom (PresheafedSpaceCat.stalkMap (SheafedSpaceCat.forgetToPresheafedSpace.map f) x) by
      infer_instance
#align
...
-/
def isoOfSheafedSpaceIso {X Y : LocallyRingedSpaceCat} (f : X.toSheafedSpace ≅ Y.toSheafedSpace) : X ≅ Y where
...
theorem preimage_basic_open {X Y : LocallyRingedSpaceCat} (f : X ⟶ Y) {U : Opens Y} (s : Y.Presheaf.obj (op U)) :
