...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » M)
...
noncomputable def Module.freeOfFiniteTypeTorsionFree [Fintype ι] {s : ι → M} (hs : span R (range s) = ⊤)
  [NoZeroSmulDivisors R M] : Σ n : ℕ, Basis (Finₓ n) R M :=
  by 
    classical 
...
noncomputable def Submodule.smithNormalFormOfLe [Fintype ι] (b : Basis ι R M) (N O : Submodule R M) (N_le_O : N ≤ O) :
  Σ o n : ℕ, Basis.SmithNormalForm (N.comap O.subtype) (Finₓ o) n :=
  by 
