...
theorem rank_lt_omega : Module.rank R M < ω :=
  by 
...
@[simp]
theorem finrank_eq_rank : ↑finrank R M = Module.rank R M :=
  by 
...
theorem finrank_linear_hom : finrank R (M →ₗ[R] N) = finrank R M*finrank R N :=
  by 
    classical 
    let this' := nontrivial_of_invariant_basis_number R 
