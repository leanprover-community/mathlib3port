...
theorem Equivalent.prod {Q₁ : QuadraticForm R M₁} {Q₂ : QuadraticForm R M₂} {Q₁' : QuadraticForm R N₁}
  ...
  Nonempty.map2ₓ Isometry.prod e₁ e₂
...
theorem anisotropic_of_prod {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticForm R M₁}
  ...
  simp_rw [anisotropic, prod_apply, Prod.forallₓ, Prod.mk_eq_zero] at h
  constructor
  · intro x hx
    refine' (h x 0 _).1
    rw [hx, zero_addₓ, map_zero]
    
  · intro x hx
    refine' (h 0 x _).2
    rw [hx, add_zeroₓ, map_zero]
...
theorem nonneg_prod_iff {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticForm R M₁}
  ...
  simp_rw [Prod.forallₓ, prod_apply]
  constructor
  · intro h
    constructor
    · intro x
...
def pi [Fintypeₓ ι] (Q : ∀ i, QuadraticForm R (Mᵢ i)) : QuadraticForm R (∀ i, Mᵢ i) :=
  ∑ i, (Q i).comp (LinearMap.proj i : _ →ₗ[R] Mᵢ i)

@[simp]
theorem pi_apply [Fintypeₓ ι] (Q : ∀ i, QuadraticForm R (Mᵢ i)) (x : ∀ i, Mᵢ i) : pi Q x = ∑ i, Q i (x i) :=
  sum_apply _ _ _

/-- An isometry between quadratic forms generated by `quadratic_form.prod` can be constructed
from a pair of isometries between the left and right parts. -/
@[simps toLinearEquiv]
def Isometry.pi [Fintypeₓ ι] {Q : ∀ i, QuadraticForm R (Mᵢ i)} {Q' : ∀ i, QuadraticForm R (Nᵢ i)}
...
theorem Equivalent.pi [Fintypeₓ ι] {Q : ∀ i, QuadraticForm R (Mᵢ i)} {Q' : ∀ i, QuadraticForm R (Nᵢ i)}
...
theorem anisotropic_of_pi [Fintypeₓ ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)] {Q : ∀ i, QuadraticForm R (Mᵢ i)}
...
theorem nonneg_pi_iff [Fintypeₓ ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)] {Q : ∀ i, QuadraticForm R (Mᵢ i)} :
  ...
  · intro h i x
    ...
    rw [Finsetₓ.sum_eq_single_of_mem i (Finsetₓ.mem_univ _) fun j _ hji => _, Pi.single_eq_same]
    rw [Pi.single_eq_of_ne hji, map_zero]
    
  · rintro h x
    exact Finsetₓ.sum_nonneg fun i hi => h i (x i)
    

theorem pos_def_pi_iff [Fintypeₓ ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)] {Q : ∀ i, QuadraticForm R (Mᵢ i)} :
  ...
  · intro h
    ...
    rw [pi_apply, Finsetₓ.sum_eq_zero_iff_of_nonneg fun j hj => _] at hx
    · exact hx _ (Finsetₓ.mem_univ _)
