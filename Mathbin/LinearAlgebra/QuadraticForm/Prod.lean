...
#align_import linear_algebra.quadratic_form.prod from "leanprover-community/mathlib"@"c20927220ef87bb4962ba08bf6da2ce3cf50a6dd"
...
#print QuadraticMap.prod /-
/-- Construct a quadratic form on a product of two modules from the quadratic form on each module.
-/
@[simps]
def prod (Q₁ : QuadraticMap R M₁) (Q₂ : QuadraticMap R M₂) : QuadraticMap R (M₁ × M₂) :=
  Q₁.comp (LinearMap.fst _ _ _) + Q₂.comp (LinearMap.snd _ _ _)
#align quadratic_form.prod QuadraticMap.prod
-/

#print QuadraticMap.IsometryEquiv.prod /-
...
@[simps toLinearEquiv]
def IsometryEquiv.prod {Q₁ : QuadraticMap R M₁} {Q₂ : QuadraticMap R M₂} {Q₁' : QuadraticMap R N₁}
    {Q₂' : QuadraticMap R N₂} (e₁ : Q₁.IsometryEquivₓ Q₁') (e₂ : Q₂.IsometryEquivₓ Q₂') :
    (Q₁.Prod Q₂).IsometryEquivₓ (Q₁'.Prod Q₂')
    where
  map_app' x := congr_arg₂ (· + ·) (e₁.map_app x.1) (e₂.map_app x.2)
  toLinearEquiv := LinearEquiv.prod e₁.toLinearEquiv e₂.toLinearEquiv
#align quadratic_form.isometry.prod QuadraticMap.IsometryEquiv.prod
-/

#print QuadraticMap.Equivalent.prod /-
theorem Equivalent.prod {Q₁ : QuadraticMap R M₁} {Q₂ : QuadraticMap R M₂} {Q₁' : QuadraticMap R N₁}
    {Q₂' : QuadraticMap R N₂} (e₁ : Q₁.Equivalent Q₁') (e₂ : Q₂.Equivalent Q₂') :
    (Q₁.Prod Q₂).Equivalent (Q₁'.Prod Q₂') :=
  Nonempty.map2 IsometryEquiv.prod e₁ e₂
#align quadratic_form.equivalent.prod QuadraticMap.Equivalent.prod
-/

#print QuadraticMap.anisotropic_of_prod /-
/-- If a product is anisotropic then its components must be. The converse is not true. -/
theorem anisotropic_of_prod {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticMap R M₁}
    {Q₂ : QuadraticMap R M₂} (h : (Q₁.Prod Q₂).Anisotropic) : Q₁.Anisotropic ∧ Q₂.Anisotropic :=
  by
...
#align quadratic_form.anisotropic_of_prod QuadraticMap.anisotropic_of_prod
-/

#print QuadraticMap.nonneg_prod_iff /-
theorem nonneg_prod_iff {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticMap R M₁}
    {Q₂ : QuadraticMap R M₂} : (∀ x, 0 ≤ (Q₁.Prod Q₂) x) ↔ (∀ x, 0 ≤ Q₁ x) ∧ ∀ x, 0 ≤ Q₂ x :=
  by
...
#align quadratic_form.nonneg_prod_iff QuadraticMap.nonneg_prod_iff
-/

#print QuadraticMap.posDef_prod_iff /-
theorem posDef_prod_iff {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticMap R M₁}
    {Q₂ : QuadraticMap R M₂} : (Q₁.Prod Q₂).PosDef ↔ Q₁.PosDef ∧ Q₂.PosDef :=
  by
  ...
  · rintro ⟨⟨hle₁, ha₁⟩, ⟨hle₂, ha₂⟩⟩
    ...
    rwa [Prod.mk_eq_zero]
#align quadratic_form.pos_def_prod_iff QuadraticMap.posDef_prod_iff
-/

#print QuadraticMap.PosDef.prod /-
theorem PosDef.prod {R} [OrderedRing R] [Module R M₁] [Module R M₂] {Q₁ : QuadraticMap R M₁}
    {Q₂ : QuadraticMap R M₂} (h₁ : Q₁.PosDef) (h₂ : Q₂.PosDef) : (Q₁.Prod Q₂).PosDef :=
  posDef_prod_iff.mpr ⟨h₁, h₂⟩
#align quadratic_form.pos_def.prod QuadraticMap.PosDef.prod
-/

open scoped BigOperators

#print QuadraticMap.pi /-
/-- Construct a quadratic form on a family of modules from the quadratic form on each module. -/
def pi [Fintype ι] (Q : ∀ i, QuadraticMap R (Mᵢ i)) : QuadraticMap R (∀ i, Mᵢ i) :=
  ∑ i, (Q i).comp (LinearMap.proj i : _ →ₗ[R] Mᵢ i)
#align quadratic_form.pi QuadraticMap.pi
-/

#print QuadraticMap.pi_apply /-
@[simp]
theorem pi_apply [Fintype ι] (Q : ∀ i, QuadraticMap R (Mᵢ i)) (x : ∀ i, Mᵢ i) :
    pi Q x = ∑ i, Q i (x i) :=
  sum_apply _ _ _
#align quadratic_form.pi_apply QuadraticMap.pi_apply
-/

#print QuadraticMap.IsometryEquiv.pi /-
/-- An isometry between quadratic forms generated by `quadratic_form.prod` can be constructed
from a pair of isometries between the left and right parts. -/
@[simps toLinearEquiv]
def IsometryEquiv.pi [Fintype ι] {Q : ∀ i, QuadraticMap R (Mᵢ i)} {Q' : ∀ i, QuadraticMap R (Nᵢ i)}
    (e : ∀ i, (Q i).IsometryEquivₓ (Q' i)) : (pi Q).IsometryEquivₓ (pi Q')
    where
  map_app' x := by
...
#align quadratic_form.isometry.pi QuadraticMap.IsometryEquiv.pi
-/

#print QuadraticMap.Equivalent.pi /-
theorem Equivalent.pi [Fintype ι] {Q : ∀ i, QuadraticMap R (Mᵢ i)} {Q' : ∀ i, QuadraticMap R (Nᵢ i)}
    (e : ∀ i, (Q i).Equivalent (Q' i)) : (pi Q).Equivalent (pi Q') :=
  ⟨IsometryEquiv.pi fun i => Classical.choice (e i)⟩
#align quadratic_form.equivalent.pi QuadraticMap.Equivalent.pi
-/

#print QuadraticMap.anisotropic_of_pi /-
...
theorem anisotropic_of_pi [Fintype ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)]
    {Q : ∀ i, QuadraticMap R (Mᵢ i)} (h : (pi Q).Anisotropic) : ∀ i, (Q i).Anisotropic :=
  by
...
#align quadratic_form.anisotropic_of_pi QuadraticMap.anisotropic_of_pi
-/

#print QuadraticMap.nonneg_pi_iff /-
theorem nonneg_pi_iff [Fintype ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)]
    {Q : ∀ i, QuadraticMap R (Mᵢ i)} : (∀ x, 0 ≤ pi Q x) ↔ ∀ i x, 0 ≤ Q i x :=
  by
...
#align quadratic_form.nonneg_pi_iff QuadraticMap.nonneg_pi_iff
-/

#print QuadraticMap.posDef_pi_iff /-
theorem posDef_pi_iff [Fintype ι] {R} [OrderedRing R] [∀ i, Module R (Mᵢ i)]
    {Q : ∀ i, QuadraticMap R (Mᵢ i)} : (pi Q).PosDef ↔ ∀ i, (Q i).PosDef :=
  by
  ...
  · intro h
    ...
    exact (h j).1 _
#align quadratic_form.pos_def_pi_iff QuadraticMap.posDef_pi_iff
-/
