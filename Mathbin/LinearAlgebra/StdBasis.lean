...
theorem infᵢ_ker_proj_le_supᵢ_range_stdBasis {I : Finset ι} {J : Set ι} (hu : Set.univ ⊆ ↑I ∪ J) :
  ...
  SetLike.le_def.2
    (by
      ...
      exact sum_mem_bsupr fun i hi => mem_range_self (stdBasis R φ i) (b i))
...
theorem supᵢ_range_stdBasis [Finite ι] : (⨆ i, range (stdBasis R φ i)) = ⊤ :=
  ...
  convert top_unique (infi_emptyset.ge.trans <| infᵢ_ker_proj_le_supᵢ_range_stdBasis R φ _)
  ·
    exact
      funext fun i => ((@supr_pos _ _ _ fun h => range <| stdBasis R φ i) <| Finset.mem_univ i).symm
...
theorem linearIndependent_stdBasis [Ring R] [∀ i, AddCommGroup (Ms i)] [∀ i, Module R (Ms i)]
  ...
  · intro j J _ hiJ
    ...
    have h₀ :
      ∀ j, span R (range fun i : ιs j => stdBasis R Ms j (v j i)) ≤ range (stdBasis R Ms j) :=
      by
      intro j
      rw [span_le, LinearMap.range_coe]
      apply range_comp_subset_range
    have h₁ :
      span R (range fun i : ιs j => stdBasis R Ms j (v j i)) ≤ ⨆ i ∈ {j}, range (stdBasis R Ms i) :=
      by
      rw [@supr_singleton _ _ _ fun i => LinearMap.range (stdBasis R (fun j : η => Ms j) i)]
      apply h₀
    have h₂ :
      (⨆ j ∈ J, span R (range fun i : ιs j => stdBasis R Ms j (v j i))) ≤
        ⨆ j ∈ J, range (stdBasis R (fun j : η => Ms j) j) :=
...
theorem stdBasis_eq_stdBasisMatrix (i : n) (j : m) [DecidableEq n] [DecidableEq m] :
  ...
  · simp [stdBasis, hi, hj]
  · simp [stdBasis, hi, hj, Ne.symm hj, LinearMap.stdBasis_ne]
  · simp [stdBasis, hi, hj, Ne.symm hi, LinearMap.stdBasis_ne]
  · simp [stdBasis, hi, hj, Ne.symm hj, Ne.symm hi, LinearMap.stdBasis_ne]
