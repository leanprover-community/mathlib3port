...
theorem ι_mul_ι_add_swap (a b : M) : ((ι Q a*ι Q b)+ι Q b*ι Q a) = algebraMap R _ (QuadraticForm.polar Q a b) :=
  calc ((ι Q a*ι Q b)+ι Q b*ι Q a) = ((ι Q (a+b)*ι Q (a+b)) - ι Q a*ι Q a) - ι Q b*ι Q b :=
    by 
      rw [(ι Q).map_add, mul_addₓ, add_mulₓ, add_mulₓ]
      abel 
    _ = algebraMap R _ (Q (a+b)) - algebraMap R _ (Q a) - algebraMap R _ (Q b) :=
    by 
      rw [ι_sq_scalar, ι_sq_scalar, ι_sq_scalar]
    _ = algebraMap R _ (Q (a+b) - Q a - Q b) :=
    by 
...
@[simp]
theorem map_id : (map Q₁ Q₁ (LinearMap.id : M₁ →ₗ[R] M₁) fun m => rfl) = AlgHom.id R (CliffordAlgebra Q₁) :=
  by 
    ext m 
...
@[simp]
theorem equiv_of_isometry_refl : (equiv_of_isometry$ QuadraticForm.Isometry.refl Q₁) = AlgEquiv.refl :=
  by 
    ext x 
...
@[simp]
theorem to_clifford_ι (m : M) : (TensorAlgebra.ι R m).toClifford = CliffordAlgebra.ι Q m :=
  by 
    simp [to_clifford]
