...
def reverse : CliffordAlgebra Q →ₗ[R] CliffordAlgebra Q :=
  (op_linear_equiv R).symm.toLinearMap.comp
    (CliffordAlgebra.lift Q
        ⟨(MulOpposite.opLinearEquiv R).toLinearMap.comp (ι Q),
          fun m =>
            unop_injective$
              by 
                simp ⟩).toLinearMap
...
@[simp]
theorem reverse_ι (m : M) : reverse (ι Q m) = ι Q m :=
  by 
    simp [reverse]

@[simp]
theorem reverse.commutes (r : R) : reverse (algebraMap R (CliffordAlgebra Q) r) = algebraMap R _ r :=
  by 
    simp [reverse]
...
@[simp]
theorem reverse.map_mul (a b : CliffordAlgebra Q) : reverse (a*b) = reverse b*reverse a :=
  by 
    simp [reverse]
