...
@[simp]
theorem involute_eq_id : (involute : CliffordAlgebra (0 : QuadraticForm R Unit) →ₐ[R] _) = AlgHom.id R _ :=
  by 
    ext 
    simp 
...
def to_complex : CliffordAlgebra Q →ₐ[ℝ] ℂ :=
  CliffordAlgebra.lift Q
    ⟨LinearMap.toSpanSingleton _ _ Complex.i,
      fun r =>
        by 
...
@[simp]
theorem to_complex_comp_of_complex : to_complex.comp of_complex = AlgHom.id ℝ ℂ :=
  by 
    ext1 
...
@[simp]
theorem of_complex_comp_to_complex : of_complex.comp to_complex = AlgHom.id ℝ (CliffordAlgebra Q) :=
  by 
    ext 
...
instance : CommRingₓ (CliffordAlgebra Q) :=
  { CliffordAlgebra.ring _ with
    mul_comm :=
      fun x y =>
        CliffordAlgebraComplex.equiv.Injective$
          by 
...
theorem to_quaternion_involute_reverse (c : CliffordAlgebra (Q c₁ c₂)) :
  to_quaternion (involute (reverse c)) = QuaternionAlgebra.conj (to_quaternion c) :=
  by 
...
theorem of_quaternion_mk (a₁ a₂ a₃ a₄ : R) :
  of_quaternion (⟨a₁, a₂, a₃, a₄⟩ : ℍ[R,c₁,c₂]) =
    ((algebraMap R _ a₁+a₂ • ι (Q c₁ c₂) (1, 0))+a₃ • ι (Q c₁ c₂) (0, 1))+a₄ • ι (Q c₁ c₂) (1, 0)*ι (Q c₁ c₂) (0, 1) :=
  rfl
...
@[simp]
theorem to_quaternion_comp_of_quaternion : to_quaternion.comp of_quaternion = AlgHom.id R ℍ[R,c₁,c₂] :=
  by 
...
theorem of_quaternion_conj (q : ℍ[R,c₁,c₂]) : of_quaternion q.conj = (of_quaternion q).reverse.involute :=
  CliffordAlgebraQuaternion.equiv.Injective$
    by 
