...
@[simp]
theorem slope_same (f : k → PE) (a : k) : (slope f a a : E) = 0 :=
  by 
    rw [slope, sub_self, inv_zero, zero_smul]

include E

theorem eq_of_slope_eq_zero {f : k → PE} {a b : k} (h : slope f a b = (0 : E)) : f a = f b :=
  by 
    rw [slope, smul_eq_zero, inv_eq_zero, sub_eq_zero, vsub_eq_zero_iff_eq] at h 
    exact h.elim (fun h => h ▸ rfl) Eq.symm

theorem slope_comm (f : k → PE) (a b : k) : slope f a b = slope f b a :=
  by 
...
theorem sub_div_sub_smul_slope_add_sub_div_sub_smul_slope (f : k → PE) (a b c : k) :
  ...
  by 
    ...
    rw [add_commₓ]
    simpRw [slope, div_eq_inv_mul, mul_smul, ←smul_add, smul_inv_smul₀ (sub_ne_zero.2$ Ne.symm hab),
...
theorem line_map_slope_slope_sub_div_sub (f : k → PE) (a b c : k) (h : a ≠ c) :
  ...
  by 
    fieldSimp [sub_ne_zero.2 h.symm, ←sub_div_sub_smul_slope_add_sub_div_sub_smul_slope f a b c, line_map_apply_module]
...
theorem line_map_strict_mono_left (ha : a < a') (hr : r < 1) : line_map a b r < line_map a' b r :=
  by 
    simp only [line_map_apply_module]
    exact add_lt_add_right (smul_lt_smul_of_pos ha (sub_pos.2 hr)) _

theorem line_map_mono_right (hb : b ≤ b') (hr : 0 ≤ r) : line_map a b r ≤ line_map a b' r :=
  by 
    simp only [line_map_apply_module]
    exact add_le_add_left (smul_le_smul_of_nonneg hb hr) _

theorem line_map_strict_mono_right (hb : b < b') (hr : 0 < r) : line_map a b r < line_map a b' r :=
  by 
    simp only [line_map_apply_module]
...
theorem line_map_lt_line_map_iff_of_lt (h : r < r') : line_map a b r < line_map a b r' ↔ a < b :=
  by 
    simp only [line_map_apply_module]
...
theorem left_lt_line_map_iff_lt (h : 0 < r) : a < line_map a b r ↔ a < b :=
  Iff.trans
    (by 
      rw [line_map_apply_zero])
    (line_map_lt_line_map_iff_of_lt h)

theorem line_map_lt_left_iff_lt (h : 0 < r) : line_map a b r < a ↔ b < a :=
  @left_lt_line_map_iff_lt k (OrderDual E) _ _ _ _ _ _ _ h

theorem line_map_lt_right_iff_lt (h : r < 1) : line_map a b r < b ↔ a < b :=
  Iff.trans
    (by 
      rw [line_map_apply_one])
    (line_map_lt_line_map_iff_of_lt h)

theorem right_lt_line_map_iff_lt (h : r < 1) : b < line_map a b r ↔ b < a :=
...
theorem left_le_line_map_iff_le (h : 0 < r) : a ≤ line_map a b r ↔ a ≤ b :=
  Iff.trans
    (by 
      rw [line_map_apply_zero])
...
theorem map_le_line_map_iff_slope_le_slope_left (h : 0 < r*b - a) :
  ...
  by 
    rw [line_map_apply, line_map_apply, slope, slope, vsub_eq_sub, vsub_eq_sub, vsub_eq_sub, vadd_eq_add, vadd_eq_add,
      ...
      ←smul_sub, ←smul_sub, ←smul_add, smul_smul, ←mul_inv_rev₀, smul_le_iff_of_pos (inv_pos.2 h), inv_inv₀, smul_smul,
...
theorem map_le_line_map_iff_slope_le_slope_right (h : 0 < (1 - r)*b - a) :
  ...
  by 
    ...
    clear r 
    intro h 
    simpRw [line_map_apply, slope, vsub_eq_sub, vadd_eq_add, smul_eq_mul]
    rw [sub_add_eq_sub_sub_swap, sub_self, zero_sub, le_smul_iff_of_pos, inv_inv₀, smul_smul, neg_mul_eq_mul_neg,
      neg_sub, mul_inv_cancel_right₀, le_sub, ←neg_sub (f b), smul_neg, neg_add_eq_sub]
    ·
