...
theorem line_map_mono_left (ha : a ≤ a') (hr : r ≤ 1) : line_map a b r ≤ line_map a' b r := by
  simp only [line_map_apply_module]
  exact add_le_add_right (smul_le_smul_of_nonneg ha (sub_nonneg.2 hr)) _

theorem line_map_strict_mono_left (ha : a < a') (hr : r < 1) : line_map a b r < line_map a' b r := by
  simp only [line_map_apply_module]
  exact add_lt_add_right (smul_lt_smul_of_pos ha (sub_pos.2 hr)) _

theorem line_map_mono_right (hb : b ≤ b') (hr : 0 ≤ r) : line_map a b r ≤ line_map a b' r := by
  simp only [line_map_apply_module]
  exact add_le_add_left (smul_le_smul_of_nonneg hb hr) _

theorem line_map_strict_mono_right (hb : b < b') (hr : 0 < r) : line_map a b r < line_map a b' r := by
  simp only [line_map_apply_module]
...
theorem line_map_le_line_map_iff_of_lt (h : r < r') : line_map a b r ≤ line_map a b r' ↔ a ≤ b := by
  simp only [line_map_apply_module]
