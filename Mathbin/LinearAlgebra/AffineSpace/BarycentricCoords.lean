...
structure
  AffineBasis(ι :
    Type u₁)(k : Type u₂){V : Type u₃}(P : Type u₄)[AddCommGroupₓ V][affine_space V P][Ringₓ k][Module k V] where
...
-- error in LinearAlgebra.AffineSpace.BarycentricCoords: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
...
noncomputable
def basis_of (i : ι) : basis {j : ι // «expr ≠ »(j, i)} k V :=
basis.mk ((affine_independent_iff_linear_independent_vsub k b.points i).mp b.ind) (begin
   suffices [] [":", expr «expr = »(submodule.span k (range (λ
       j : {x // «expr ≠ »(x, i)}, «expr -ᵥ »(b.points «expr↑ »(j), b.points i))), vector_span k (range b.points))],
   { rw ["[", expr this, ",", "<-", expr direction_affine_span, ",", expr b.tot, ",", expr affine_subspace.direction_top, "]"] [] },
   conv_rhs [] [] { rw ["<-", expr image_univ] },
   rw [expr vector_span_image_eq_span_vsub_set_right_ne k b.points (mem_univ i)] [],
   congr,
   ext [] [ident v] [],
   simp [] [] [] [] [] []
 end)
