...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i2 «expr ∈ » s)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i j «expr ∈ » s)
...
theorem centroid_eq_centroid_image_of_inj_on {p : ι → P} (hi : ∀ i j _ : i ∈ s _ : j ∈ s, p i = p j → i = j)
  {ps : Set P} [Fintype ps] (hps : ps = p '' ↑s) : s.centroid k p = (univ : Finset ps).centroid k fun x => x :=
  by 
