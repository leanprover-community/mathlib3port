...
protected def pointwiseAddAction : AddAction V (AffineSubspace k P) where
  ...
  zero_vadd := fun p => ((congr_argₓ fun f => p.map f) <| AffineMap.ext <| zero_vadd _).trans p.map_id
  add_vadd := fun x y p => ((congr_argₓ fun f => p.map f) <| AffineMap.ext <| add_vadd _ _).trans (p.map_map _ _).symm
