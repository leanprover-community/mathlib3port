...
noncomputable def coord (i : ι) : P →ᵃ[k] k where
  toFun := fun q => 1 - (b.basis_of i).sumCoords (q -ᵥ b.points i)
  linear := -(b.basis_of i).sumCoords
  map_vadd' := fun q v => by
...
theorem to_matrix_row_sum_one {ι' : Type _} (q : ι' → P) (i : ι') : (∑ j, b.to_matrix q i j) = 1 := by
  simp
...
theorem affine_independent_of_to_matrix_right_inv [DecidableEq ι'] (p : ι' → P) {A : Matrix ι ι' k}
    (hA : b.to_matrix p ⬝ A = 1) : AffineIndependent k p := by
...
theorem affine_span_eq_top_of_to_matrix_left_inv [DecidableEq ι] [Nontrivial k] (p : ι' → P) {A : Matrix ι ι' k}
    (hA : A ⬝ b.to_matrix p = 1) : affineSpan k (range p) = ⊤ := by
