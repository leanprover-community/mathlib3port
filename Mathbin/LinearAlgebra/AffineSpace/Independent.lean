...
theorem affine_independent_iff_linear_independent_vsub (p : ι → P) (i1 : ι) :
  ...
  by 
    ...
    ·
      ...
      simpRw [Finset.mem_subtype]  at h2 
      have h2b : ∀ i _ : i ∈ s, i ≠ i1 → w i = 0 := fun i his hi => h2 ⟨i, hi⟩ (Finset.mem_erase_of_ne_of_mem hi his)
...
theorem affine_independent_set_iff_linear_independent_vsub {s : Set P} {p₁ : P} (hp₁ : p₁ ∈ s) :
  ...
  by 
    ...
    ·
      intro h 
      have hv : ∀ v : (fun p => (p -ᵥ p₁ : V)) '' (s \ {p₁}), (v : V) +ᵥ p₁ ∈ s \ {p₁} :=
        fun v => (vsub_left_injective p₁).mem_set_image.1 ((vadd_vsub (v : V) p₁).symm ▸ v.property)
      let f : (fun p : P => (p -ᵥ p₁ : V)) '' (s \ {p₁}) → { x : s // x ≠ ⟨p₁, hp₁⟩ } :=
        fun x =>
    ...
    ·
      intro h 
      let f : { x : s // x ≠ ⟨p₁, hp₁⟩ } → (fun p : P => (p -ᵥ p₁ : V)) '' (s \ {p₁}) :=
        fun x => ⟨((x : s) : P) -ᵥ p₁, ⟨x, ⟨⟨(x : s).property, fun hx => x.property (Subtype.ext hx)⟩, rfl⟩⟩⟩
...
theorem linear_independent_set_iff_affine_independent_vadd_union_singleton {s : Set V} (hs : ∀ v _ : v ∈ s, v ≠ (0 : V))
  (p₁ : P) :
  ...
  by 
    ...
    ·
      simpRw [Set.union_diff_left, Set.image_diff (vsub_left_injective p₁), Set.image_image, Set.image_singleton,
...
theorem affine_independent_iff_indicator_eq_of_affine_combination_eq (p : ι → P) :
  AffineIndependent k p ↔
    ∀ s1 s2 : Finset ι w1 w2 : ι → k,
      (∑i in s1, w1 i) = 1 →
        (∑i in s2, w2 i) = 1 →
          s1.affine_combination p w1 = s2.affine_combination p w2 →
            Set.indicator («expr↑ » s1) w1 = Set.indicator («expr↑ » s2) w2 :=
  by 
    split 
    ·
      ...
      ·
        ...
        have hws : (∑i in s1 ∪ s2, (Set.indicator («expr↑ » s1) w1 - Set.indicator («expr↑ » s2) w2) i) = 0
        ·
          simp [hw1, hw2]
        ...
        exact ha (s1 ∪ s2) (Set.indicator («expr↑ » s1) w1 - Set.indicator («expr↑ » s2) w2) hws HEq i hi
      ·
        rw [←Finset.mem_coe, Finset.coe_union] at hi 
        simp [mt (Set.mem_union_left («expr↑ » s2)) hi, mt (Set.mem_union_right («expr↑ » s1)) hi]
    ·
...
-- error in LinearAlgebra.AffineSpace.Independent: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
/-- An affinely independent family is injective, if the underlying
ring is nontrivial. -/
protected
theorem affine_independent.injective [nontrivial k] {p : ι → P} (ha : affine_independent k p) : function.injective p :=
begin
  intros [ident i, ident j, ident hij],
  rw [expr affine_independent_iff_linear_independent_vsub _ _ j] ["at", ident ha],
  by_contra [ident hij'],
  exact [expr ha.ne_zero ⟨i, hij'⟩ (vsub_eq_zero_iff_eq.mpr hij)]
end
...
theorem affine_independent_equiv {ι' : Type _} (e : ι ≃ ι') {p : ι' → P} :
  ...
  by 
    ...
    intro h 
    have  : p = (p ∘ e ∘ e.symm.to_embedding)
    ·
      ext 
      simp 
    rw [this]
...
theorem AffineIndependent.of_comp {p : ι → P} (f : P →ᵃ[k] P₂) (hai : AffineIndependent k (f ∘ p)) :
  ...
  by 
    ...
    simpRw [affine_independent_iff_linear_independent_vsub k (f ∘ p) i, Function.comp_app, ←f.linear_map_vsub]  at hai 
...
theorem AffineIndependent.map' {p : ι → P} (hai : AffineIndependent k p) (f : P →ᵃ[k] P₂) (hf : Function.Injective f) :
  ...
  by 
    ...
    simpRw [affine_independent_iff_linear_independent_vsub k (f ∘ p) i, Function.comp_app, ←f.linear_map_vsub]
...
theorem AffineEquiv.affine_independent_set_of_eq_iff {s : Set P} (e : P ≃ᵃ[k] P₂) :
  AffineIndependent k (coeₓ : e '' s → P₂) ↔ AffineIndependent k (coeₓ : s → P) :=
  by 
...
theorem AffineIndependent.exists_mem_inter_of_exists_mem_inter_affine_span [Nontrivial k] {p : ι → P}
  ...
  by 
    ...
    simpRw [←Set.indicator_of_mem (Finset.mem_coe.2 hifs1) w1, ha]  at hinz 
    use i, hfs1 hifs1, hfs2 (Set.mem_of_indicator_ne_zero hinz)

-- error in LinearAlgebra.AffineSpace.Independent: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
ring is nontrivial. -/
theorem affine_independent.affine_span_disjoint_of_disjoint
[nontrivial k]
{p : ι → P}
(ha : affine_independent k p)
{s1 s2 : set ι}
(hd : «expr = »(«expr ∩ »(s1, s2), «expr∅»())) : «expr = »(«expr ∩ »((affine_span k «expr '' »(p, s1) : set P), affine_span k «expr '' »(p, s2)), «expr∅»()) :=
begin
  by_contradiction [ident hne],
  change [expr «expr ≠ »(«expr ∩ »((affine_span k «expr '' »(p, s1) : set P), affine_span k «expr '' »(p, s2)), «expr∅»())] [] ["at", ident hne],
  rw [expr set.ne_empty_iff_nonempty] ["at", ident hne],
  rcases [expr hne, "with", "⟨", ident p0, ",", ident hp0s1, ",", ident hp0s2, "⟩"],
  cases [expr ha.exists_mem_inter_of_exists_mem_inter_affine_span hp0s1 hp0s2] ["with", ident i, ident hi],
  exact [expr set.not_mem_empty i «expr ▸ »(hd, hi)]
end
...
protected theorem AffineIndependent.mem_affine_span_iff [Nontrivial k] {p : ι → P} (ha : AffineIndependent k p) (i : ι)
  (s : Set ι) : p i ∈ affineSpan k (p '' s) ↔ i ∈ s :=
  by 
    split 
    ·
      intro hs 
...
theorem exists_affine_independent (s : Set P) :
  ∃ (t : _)(_ : t ⊆ s), affineSpan k t = affineSpan k s ∧ AffineIndependent k (coeₓ : t → P) :=
  by 
    rcases s.eq_empty_or_nonempty with (rfl | ⟨p, hp⟩)
    ·
    ...
    ·
      ...
      ·
        have  : Submodule.span k b = Submodule.span k (insert 0 b)
        ·
          ·
            simp 
...
-- error in LinearAlgebra.AffineSpace.Independent: ././Mathport/Syntax/Translate/Basic.lean:340:40: in rw: ././Mathport/Syntax/Translate/Basic.lean:557:61: unsupported notation `«expr![ , ]»
...
-- error in LinearAlgebra.AffineSpace.Independent: ././Mathport/Syntax/Translate/Basic.lean:340:40: in all_goals: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
