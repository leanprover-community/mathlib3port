...
theorem affine_independent_def (p : ι → P) :
    AffineIndependent k p ↔
      ∀ (s : Finset ι) (w : ι → k),
        (∑ i in s, w i) = 0 → s.weightedVsub p w = (0 : V) → ∀ i ∈ s, w i = 0 :=
  Iff.rfl
#align affine_independent_def affine_independent_def
...
#align affine_independent_of_subsingleton affine_independent_of_subsingleton
...
theorem affine_independent_iff_of_fintype [Fintype ι] (p : ι → P) :
  ...
  · intro h s w hw hs i hi
    ...
    simpa [hi] using h
#align affine_independent_iff_of_fintype affine_independent_iff_of_fintype
...
#align affine_independent_iff_linear_independent_vsub affine_independent_iff_linear_independent_vsub
...
#align affine_independent_set_iff_linear_independent_vsub affine_independent_set_iff_linear_independent_vsub
...
theorem linear_independent_set_iff_affine_independent_vadd_union_singleton {s : Set V}
    (hs : ∀ v ∈ s, v ≠ (0 : V)) (p₁ : P) :
  ...
  rw [h]
#align linear_independent_set_iff_affine_independent_vadd_union_singleton linear_independent_set_iff_affine_independent_vadd_union_singleton
...
theorem affine_independent_iff_indicator_eq_of_affine_combination_eq (p : ι → P) :
  ...
  classical
    ...
    · intro ha s w hw hs i0 hi0
      ...
      simpa [w2] using hws
#align affine_independent_iff_indicator_eq_of_affine_combination_eq affine_independent_iff_indicator_eq_of_affine_combination_eq
...
#align affine_independent_iff_eq_of_fintype_affine_combination_eq affine_independent_iff_eq_of_fintype_affine_combination_eq
...
theorem AffineIndependent.units_line_map {p : ι → P} (hp : AffineIndependent k p) (j : ι)
...
#align affine_independent.units_line_map AffineIndependent.units_line_map

theorem AffineIndependent.indicator_eq_of_affine_combination_eq {p : ι → P}
...
#align affine_independent.indicator_eq_of_affine_combination_eq AffineIndependent.indicator_eq_of_affine_combination_eq
...
#align affine_independent_equiv affine_independent_equiv
...
theorem AffineIndependent.of_comp {p : ι → P} (f : P →ᵃ[k] P₂) (hai : AffineIndependent k (f ∘ p)) :
  ...
  simp_rw [affine_independent_iff_linear_independent_vsub k (f ∘ p) i, Function.comp_apply, ←
...
#align affine_map.affine_independent_iff AffineMap.affine_independent_iff
...
#align affine_equiv.affine_independent_iff AffineEquiv.affine_independent_iff
...
ring is nontrivial. -/
theorem AffineIndependent.exists_mem_inter_of_exists_mem_inter_affine_span [Nontrivial k]
    {p : ι → P} (ha : AffineIndependent k p) {s1 s2 : Set ι} {p0 : P}
    (hp0s1 : p0 ∈ affineSpan k (p '' s1)) (hp0s2 : p0 ∈ affineSpan k (p '' s2)) :
    ∃ i : ι, i ∈ s1 ∩ s2 := by
...
#align affine_independent.exists_mem_inter_of_exists_mem_inter_affine_span AffineIndependent.exists_mem_inter_of_exists_mem_inter_affine_span
...
#align affine_independent.affine_span_disjoint_of_disjoint AffineIndependent.affine_span_disjoint_of_disjoint
...
ring is nontrivial. -/
@[simp]
protected theorem AffineIndependent.mem_affine_span_iff [Nontrivial k] {p : ι → P}
...
#align affine_independent.mem_affine_span_iff AffineIndependent.mem_affine_span_iff
...
nontrivial. -/
theorem AffineIndependent.not_mem_affine_span_diff [Nontrivial k] {p : ι → P}
    (ha : AffineIndependent k p) (i : ι) (s : Set ι) : p i ∉ affineSpan k (p '' (s \ {i})) := by
  simp [ha]
#align affine_independent.not_mem_affine_span_diff AffineIndependent.not_mem_affine_span_diff
...
#align affine_independent_iff affine_independent_iff
...
theorem weighted_vsub_mem_vector_span_pair {p : ι → P} (h : AffineIndependent k p) {w w₁ w₂ : ι → k}
  ...
  · rcases h with ⟨r, hr⟩
    ...
    congr
#align weighted_vsub_mem_vector_span_pair weighted_vsub_mem_vector_span_pair
...
two points. -/
theorem affine_combination_mem_affine_span_pair {p : ι → P} (h : AffineIndependent k p)
  ...
  rw [← vsub_vadd (s.affine_combination p w) (s.affine_combination p w₁),
    AffineSubspace.vadd_mem_iff_mem_direction _ (left_mem_affine_span_pair _ _ _),
...
#align affine_combination_mem_affine_span_pair affine_combination_mem_affine_span_pair
...
theorem exists_subset_affine_independent_affine_span_eq_top {s : Set P}
...
#align exists_subset_affine_independent_affine_span_eq_top exists_subset_affine_independent_affine_span_eq_top
...
theorem exists_affine_independent (s : Set P) :
  ...
  obtain ⟨b, hb₁, hb₂, hb₃⟩ := exists_linear_independent k ((Equiv.vaddConst p).symm '' s)
...
#align exists_affine_independent exists_affine_independent

variable (k) {V P}

/-- Two different points are affinely independent. -/
theorem affine_independent_of_ne {p₁ p₂ : P} (h : p₁ ≠ p₂) : AffineIndependent k ![p₁, p₂] :=
  by
...
#align affine_independent_of_ne affine_independent_of_ne
...
theorem AffineIndependent.affine_independent_of_not_mem_span {p : ι → P} {i : ι}
...
#align affine_independent.affine_independent_of_not_mem_span AffineIndependent.affine_independent_of_not_mem_span

/-- If distinct points `p₁` and `p₂` lie in `s` but `p₃` does not, the three points are affinely
independent. -/
theorem affine_independent_of_ne_of_mem_of_mem_of_not_mem {s : AffineSubspace k P} {p₁ p₂ p₃ : P}
    (hp₁p₂ : p₁ ≠ p₂) (hp₁ : p₁ ∈ s) (hp₂ : p₂ ∈ s) (hp₃ : p₃ ∉ s) :
    AffineIndependent k ![p₁, p₂, p₃] :=
  by
  have ha : AffineIndependent k fun x : { x : Fin 3 // x ≠ 2 } => ![p₁, p₂, p₃] x :=
    by
    rw [← affine_independent_equiv (finSuccAboveEquiv (2 : Fin 3)).toEquiv]
...
#align affine_independent_of_ne_of_mem_of_mem_of_not_mem affine_independent_of_ne_of_mem_of_mem_of_not_mem

/-- If distinct points `p₁` and `p₃` lie in `s` but `p₂` does not, the three points are affinely
independent. -/
theorem affine_independent_of_ne_of_mem_of_not_mem_of_mem {s : AffineSubspace k P} {p₁ p₂ p₃ : P}
    (hp₁p₃ : p₁ ≠ p₃) (hp₁ : p₁ ∈ s) (hp₂ : p₂ ∉ s) (hp₃ : p₃ ∈ s) :
    AffineIndependent k ![p₁, p₂, p₃] :=
  by
  ...
  ext x
  fin_cases x <;> rfl
#align affine_independent_of_ne_of_mem_of_not_mem_of_mem affine_independent_of_ne_of_mem_of_not_mem_of_mem

/-- If distinct points `p₂` and `p₃` lie in `s` but `p₁` does not, the three points are affinely
independent. -/
theorem affine_independent_of_ne_of_not_mem_of_mem_of_mem {s : AffineSubspace k P} {p₁ p₂ p₃ : P}
    (hp₂p₃ : p₂ ≠ p₃) (hp₁ : p₁ ∉ s) (hp₂ : p₂ ∈ s) (hp₃ : p₃ ∈ s) :
    AffineIndependent k ![p₁, p₂, p₃] :=
  by
  ...
  ext x
  fin_cases x <;> rfl
#align affine_independent_of_ne_of_not_mem_of_mem_of_mem affine_independent_of_ne_of_not_mem_of_mem_of_mem
...
sign. -/
theorem sign_eq_of_affine_combination_mem_affine_span_pair {p : ι → P} (h : AffineIndependent k p)
    {w w₁ w₂ : ι → k} {s : Finset ι} (hw : (∑ i in s, w i) = 1) (hw₁ : (∑ i in s, w₁ i) = 1)
    (hw₂ : (∑ i in s, w₂ i) = 1)
...
#align sign_eq_of_affine_combination_mem_affine_span_pair sign_eq_of_affine_combination_mem_affine_span_pair
...
theorem sign_eq_of_affine_combination_mem_affine_span_single_line_map {p : ι → P}
  ...
  classical
    ...
    refine'
      sign_eq_of_affine_combination_mem_affine_span_pair h hw
...
#align sign_eq_of_affine_combination_mem_affine_span_single_line_map sign_eq_of_affine_combination_mem_affine_span_single_line_map
...
theorem mk_of_point_points (p : P) (i : Fin 1) : (mkOfPoint k p).points i = p :=
  rfl
#align affine.simplex.mk_of_point_points Affine.Simplex.mk_of_point_points
...
theorem face_eq_mk_of_point {n : ℕ} (s : Simplex k P n) (i : Fin (n + 1)) :
  ...
  ext
  simp [face_points]
#align affine.simplex.face_eq_mk_of_point Affine.Simplex.face_eq_mk_of_point
...
theorem centroid_eq_iff [CharZero k] {n : ℕ} (s : Simplex k P n) {fs₁ fs₂ : Finset (Fin (n + 1))}
  ...
  have ha :=
    (affine_independent_iff_indicator_eq_of_affine_combination_eq k s.points).1 s.independent _ _ _
      _ (fs₁.sum_centroid_weights_indicator_eq_one_of_card_eq_add_one k h₁)
  ...
  simp_rw [Finset.coe_univ, Set.indicator_univ, Function.funext_iff,
    Finset.centroid_weights_indicator_def, Finset.centroidWeights, h₁, h₂] at ha
  ext i
  specialize ha i
