  ...
  by 
    rw
      [affine_independent_iff_linear_independent_vsub k
        («expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»") 0]
    let i₁ : { x // x ≠ (0 : Finₓ 2) } :=
      ⟨1,
        by 
          normNum⟩
...
theorem centroid_eq_iff [CharZero k] {n : ℕ} (s : simplex k P n) {fs₁ fs₂ : Finset (Finₓ (n+1))} {m₁ m₂ : ℕ}
  ...
  by 
    ...
    ·
      ...
      simpRw [Finset.coe_univ, Set.indicator_univ, Function.funext_iffₓ, Finset.centroid_weights_indicator_def,
