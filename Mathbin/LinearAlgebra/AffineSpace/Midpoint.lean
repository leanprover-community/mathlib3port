...
@[simp]
theorem AffineEquiv.point_reflection_midpoint_left (x y : P) : point_reflection R (midpoint R x y) x = y :=
  by 
    rw [midpoint, point_reflection_apply, line_map_apply, vadd_vsub, vadd_vadd, ←add_smul, ←two_mul, mul_inv_of_self,
      one_smul, vsub_vadd]

theorem midpoint_comm (x y : P) : midpoint R x y = midpoint R y x :=
  by 
    rw [midpoint, ←line_map_apply_one_sub, one_sub_inv_of_two, midpoint]

@[simp]
theorem AffineEquiv.point_reflection_midpoint_right (x y : P) : point_reflection R (midpoint R x y) y = x :=
  by 
...
@[simp]
theorem midpoint_vsub_right (p₁ p₂ : P) : midpoint R p₁ p₂ -ᵥ p₂ = (⅟ 2 : R) • (p₁ -ᵥ p₂) :=
  by 
...
@[simp]
theorem right_vsub_midpoint (p₁ p₂ : P) : p₂ -ᵥ midpoint R p₁ p₂ = (⅟ 2 : R) • (p₂ -ᵥ p₁) :=
  by 
...
theorem midpoint_add_self (x y : V) : (midpoint R x y+midpoint R x y) = x+y :=
  calc midpoint R x y +ᵥ midpoint R x y = midpoint R x y +ᵥ midpoint R y x :=
    by 
      rw [midpoint_comm]
    _ = x+y :=
    by 
...
theorem midpoint_zero_add (x y : V) : midpoint R 0 (x+y) = midpoint R x y :=
  (midpoint_eq_midpoint_iff_vsub_eq_vsub R).2$
    by 
      simp [sub_add_eq_sub_sub_swap]

theorem midpoint_eq_smul_add (x y : V) : midpoint R x y = (⅟ 2 : R) • x+y :=
  by 
...
theorem line_map_one_half {R : Type _} {V P : Type _} [DivisionRing R] [CharZero R] [AddCommGroupₓ V] [Module R V]
  [AddTorsor V P] (a b : P) : line_map a b (1 / 2 : R) = midpoint R a b :=
  by 
...
theorem homothety_one_half {k : Type _} {V P : Type _} [Field k] [CharZero k] [AddCommGroupₓ V] [Module k V]
  [AddTorsor V P] (a b : P) : homothety a (1 / 2 : k) b = midpoint k a b :=
  by 
