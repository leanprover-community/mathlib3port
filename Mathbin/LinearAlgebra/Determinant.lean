...
@[simp]
theorem det_smul {𝕜 : Type _} [Field 𝕜] {M : Type _} [AddCommGroup M] [Module 𝕜 M] (c : 𝕜) (f : M →ₗ[𝕜] M) :
...
theorem det_eq_one_of_finrank_eq_zero {𝕜 : Type _} [Field 𝕜] {M : Type _} [AddCommGroup M] [Module 𝕜 M]
    (h : FiniteDimensional.finrank 𝕜 M = 0) (f : M →ₗ[𝕜] M) : (f : M →ₗ[𝕜] M).det = 1 := by
  classical
...
theorem is_unit_det {A : Type _} [CommRing A] [Module A M] (f : M →ₗ[A] M) (hf : IsUnit f) : IsUnit f.det := by
...
theorem finiteDimensionalOfDetNeOne {𝕜 : Type _} [Field 𝕜] [Module 𝕜 M] (f : M →ₗ[𝕜] M) (hf : f.det ≠ 1) :
...
@[reducible]
def LinearMap.equivOfDetNeZero {𝕜 : Type _} [Field 𝕜] {M : Type _} [AddCommGroup M] [Module 𝕜 M] [FiniteDimensional 𝕜 M]
...
theorem LinearMap.associated_det_comp_equiv {N : Type _} [AddCommGroup N] [Module R N] (f : N →ₗ[R] M)
...
@[simp]
theorem Basis.det_is_empty [IsEmpty ι] : e.det = AlternatingMap.constOfIsEmpty R M 1 := by
  ext v
  exact Matrix.det_is_empty
