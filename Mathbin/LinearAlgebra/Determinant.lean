...
variable {m n : Type _} [Fintypeₓ m] [Fintypeₓ n]
...
theorem det_comm [DecidableEq n] (M N : Matrix n n A) : det (M ⬝ N) = det (N ⬝ M) := by rw [det_mul, det_mul, mul_comm]
...
theorem det_comm' [DecidableEq m] [DecidableEq n] {M : Matrix n m A} {N : Matrix m n A} {M' : Matrix m n A}
  ...
  rw [← det_submatrix_equiv_self e, ← submatrix_mul_equiv _ _ _ (Equivₓ.refl n) _, det_comm, submatrix_mul_equiv,
    Equivₓ.coe_refl, submatrix_id_id]
...
theorem det_aux_def' {ι' : Type _} [Fintypeₓ ι'] [DecidableEq ι'] (tb : Trunc <| Basis ι A M) (b' : Basis ι' A M)
...
protected irreducible_def det : (M →ₗ[A] M) →* A :=
  if H : ∃ s : Finsetₓ M, Nonempty (Basis s A M) then LinearMap.detAux (Trunc.mk H.some_spec.some) else 1
...
theorem det_eq_det_to_matrix_of_finset [DecidableEq M] {s : Finsetₓ M} (b : Basis s A M) (f : M →ₗ[A] M) :
    f.det = Matrix.det (LinearMap.toMatrix b b f) := by
  have : ∃ s : Finsetₓ M, Nonempty (Basis s A M) := ⟨s, ⟨b⟩⟩
...
@[simp]
theorem det_to_matrix' {ι : Type _} [Fintypeₓ ι] [DecidableEq ι] (f : (ι → A) →ₗ[A] ι → A) : det f.toMatrix' = f.det :=
...
theorem det_cases [DecidableEq M] {P : A → Prop} (f : M →ₗ[A] M)
    (hb : ∀ (s : Finsetₓ M) (b : Basis s A M), P (toMatrix b b f).det) (h1 : P 1) : P f.det := by
  unfold LinearMap.det
  split_ifs with h
...
theorem det_smul {𝕜 : Type _} [Field 𝕜] {M : Type _} [AddCommGroupₓ M] [Module 𝕜 M] (c : 𝕜) (f : M →ₗ[𝕜] M) :
  ...
  by_cases H:∃ s : Finsetₓ M, Nonempty (Basis s 𝕜 M)
  · have : FiniteDimensional 𝕜 M := by
      rcases H with ⟨s, ⟨hs⟩⟩
      exact FiniteDimensional.of_fintype_basis hs
    simp only [← det_to_matrix (FiniteDimensional.finBasis 𝕜 M), LinearEquiv.map_smul, Fintypeₓ.card_fin, det_smul]
...
theorem det_conj {N : Type _} [AddCommGroupₓ N] [Module A N] (f : M →ₗ[A] M) (e : M ≃ₗ[A] N) :
  ...
  classical
  by_cases H:∃ s : Finsetₓ M, Nonempty (Basis s A M)
  · rcases H with ⟨s, ⟨b⟩⟩
  ...
  · have H' : ¬∃ t : Finsetₓ N, Nonempty (Basis t A N) := by
      contrapose! H
      rcases H with ⟨s, ⟨b⟩⟩
...
theorem LinearEquiv.det_coe_symm {𝕜 : Type _} [Field 𝕜] [Module 𝕜 M] (f : M ≃ₗ[𝕜] M) :
    (f.symm : M →ₗ[𝕜] M).det = (f : M →ₗ[𝕜] M).det⁻¹ := by field_simp [IsUnit.ne_zero f.is_unit_det']
...
theorem Basis.det_reindex {ι' : Type _} [Fintypeₓ ι'] [DecidableEq ι'] (b : Basis ι R M) (v : ι' → M) (e : ι ≃ ι') :
    (b.reindex e).det v = b.det (v ∘ e) := by
  rw [Basis.det_apply, Basis.to_matrix_reindex', det_reindex_alg_equiv, Basis.det_apply]

theorem Basis.det_reindex_symm {ι' : Type _} [Fintypeₓ ι'] [DecidableEq ι'] (b : Basis ι R M) (v : ι → M) (e : ι' ≃ ι) :
...
theorem Basis.det_smul_mk_coord_eq_det_update {v : ι → M} (hli : LinearIndependent R v) (hsp : ⊤ ≤ span R (Range v))
  ...
  · rw [Basis.mk_coord_apply_ne hik, mul_zero, eq_comm]
    exact e.det.map_eq_zero_of_eq _ (by simp [hik, Function.update_applyₓ]) hik
...
theorem Basis.det_units_smul (e : Basis ι R M) (w : ι → Rˣ) : (e.units_smul w).det = (↑(∏ i, w i)⁻¹ : R) • e.det := by
  ext f
  change
    (Matrix.det fun i j => (e.units_smul w).repr (f j) i) = (↑(∏ i, w i)⁻¹ : R) • Matrix.det fun i j => e.repr (f j) i
  simp only [e.repr_units_smul]
  convert Matrix.det_mul_column (fun i => (↑(w i)⁻¹ : R)) fun i j => e.repr (f j) i
  simp [← Finsetₓ.prod_inv_distrib]

/-- The determinant of a basis constructed by `units_smul` is the product of the given units. -/
@[simp]
theorem Basis.det_units_smul_self (w : ι → Rˣ) : e.det (e.units_smul w) = ∏ i, w i := by simp [Basis.det_apply]
