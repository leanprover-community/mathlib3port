...
theorem union_support_maximal_linear_independent_eq_range_basis {ι : Type w} (b : Basis ι R M) {κ : Type w'} (v : κ → M)
  (i : LinearIndependent R v) (m : i.maximal) : (⋃ k, ((b.repr (v k)).support : Set ι)) = univ :=
  by 
    byContra h 
    ...
    ·
      ...
      ·
        congr 
        exact i.injective z 
    have i' : LinearIndependent R (coeₓ : range v' → M)
    ·
      ...
      ·
        ...
        applyFun fun x => b.repr x b'  at z 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (v «expr ∈ » V i)
...
theorem dim_zero_iff_forall_zero : Module.rank R M = 0 ↔ ∀ x : M, x = 0 :=
  by 
    ...
    ·
      ...
      suffices  : LinearIndependent R fun y : ({x} : Set M) => ↑y
      ·
...
theorem dim_span {v : ι → M} (hv : LinearIndependent R v) : Module.rank R (↥span R (range v)) = # (range v) :=
  by 
    have  := nontrivial_of_invariant_basis_number R 
...
theorem dim_prod : Module.rank K (V × V₁) = Module.rank K V+Module.rank K V₁ :=
  by 
    ...
    exact
      Cardinal.lift_inj.1 (Cardinal.lift_mk_eq.2 ⟨equiv.ulift.trans (Equivₓ.sumCongr Equivₓ.ulift Equivₓ.ulift).symm⟩)
...
theorem dim_quotient_add_dim (p : Submodule K V) : (Module.rank K (V ⧸ p)+Module.rank K p) = Module.rank K V :=
  by 
    classical <;>
      exact
...
theorem le_dim_iff_exists_linear_independent_finset {n : ℕ} :
  ↑n ≤ Module.rank K V ↔ ∃ s : Finset V, s.card = n ∧ LinearIndependent K (coeₓ : (s : Set V) → V) :=
  by 
