...
instance :
    ...
    { bilin := fun x y => 0, bilin_add_left := fun x y z => (add_zero 0).symm,
      bilin_smul_left := fun a x y => (mul_zero a).symm, bilin_add_right := fun x y z => (zero_add 0).symm,
...
instance :
    Add
      (BilinForm R
        M) where add B D :=
    { bilin := fun x y => B x y + D x y, bilin_add_left := fun x y z => by rw [add_left, add_left, add_add_add_comm],
...
@[simp]
theorem coe_smul {α} [Monoid α] [DistribMulAction α R] [SmulCommClass α R R] (a : α) (B : BilinForm R M) :
    ⇑(a • B) = a • B :=
  rfl
#align bilin_form.coe_smul BilinForm.coe_smul

@[simp]
theorem smul_apply {α} [Monoid α] [DistribMulAction α R] [SmulCommClass α R R] (a : α) (B : BilinForm R M) (x y : M) :
    (a • B) x y = a • B x y :=
  rfl
...
instance {α} [Monoid α] [DistribMulAction α R] [SmulCommClass α R R] : DistribMulAction α (BilinForm R M) :=
...
theorem comp_comp {M'' : Type _} [AddCommMonoid M''] [Module R M''] (B : BilinForm R M'') (l r : M →ₗ[R] M')
...
#align bilin_form.is_skew_adjoint_iff_neg_self_adjoint BilinForm.is_skew_adjoint_iff_neg_self_adjoint
...
#align bilin_form.orthogonal_span_singleton_eq_to_lin_ker BilinForm.orthogonal_span_singleton_eq_to_lin_ker
...
#align bilin_form.nondegenerate_restrict_of_disjoint_orthogonal BilinForm.nondegenerate_restrict_of_disjoint_orthogonal

/-- An orthogonal basis with respect to a nondegenerate bilinear form has no self-orthogonal
elements. -/
theorem IsOrtho.not_is_ortho_basis_self_of_nondegenerate {n : Type w} [Nontrivial R] {B : BilinForm R M}
...
theorem IsOrtho.nondegenerate_iff_not_is_ortho_basis_self {n : Type w} [Nontrivial R] [NoZeroDivisors R]
...
#align bilin_form.to_lin_restrict_ker_eq_inf_orthogonal BilinForm.to_lin_restrict_ker_eq_inf_orthogonal
...
theorem finrank_add_finrank_orthogonal {B : BilinForm K V} {W : Subspace K V} (b₁ : B.IsRefl) :
  ...
  conv_rhs =>
    rw [← @Subspace.finrank_add_finrank_dual_annihilator_comap_eq K V _ _ _ _ (B.to_lin.dom_restrict W).range, add_comm,
      ← add_assoc, add_comm (finrank K ↥(B.to_lin.dom_restrict W).ker), LinearMap.finrank_range_add_finrank_ker]
...
theorem restrict_nondegenerate_of_is_compl_orthogonal {B : BilinForm K V} {W : Subspace K V} (b₁ : B.IsRefl)
  ...
  exact le_self_add
#align bilin_form.restrict_nondegenerate_of_is_compl_orthogonal BilinForm.restrict_nondegenerate_of_is_compl_orthogonal
...
#align bilin_form.is_adjoint_pair_unique_of_nondegenerate BilinForm.is_adjoint_pair_unique_of_nondegenerate
...
#align bilin_form.comp_symm_comp_of_nondegenerate_apply BilinForm.comp_symm_comp_of_nondegenerate_apply
...
theorem symm_comp_of_nondegenerate_left_apply (B₁ : BilinForm K V) {B₂ : BilinForm K V} (b₂ : B₂.Nondegenerate)
  ...
  rfl
#align bilin_form.symm_comp_of_nondegenerate_left_apply BilinForm.symm_comp_of_nondegenerate_left_apply
