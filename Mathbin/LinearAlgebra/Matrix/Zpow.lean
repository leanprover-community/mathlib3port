...
theorem zpow_neg {A : M} (h : IsUnit A.det) : ∀ n : ℤ, A ^ (-n) = (A ^ n)⁻¹
  ...
  | -[n+1] =>
    by
    rw [zpow_negSucc, neg_negSucc, ofNat_eq_coe, zpow_coe_nat, nonsing_inv_nonsing_inv]
    rw [det_pow]
    exact h.pow _
...
theorem SemiconjBy.zpow_right {A X Y : M} (hx : IsUnit X.det) (hy : IsUnit Y.det)
  ...
  | -[n+1] =>
    ...
    refine' (isRegular_of_isLeftRegular_det hy'.is_regular.left).left _
...
theorem zpow_mul (A : M) (h : IsUnit A.det) : ∀ m n : ℤ, A ^ (m * n) = (A ^ m) ^ n
  ...
  | (m : ℕ), -[n+1] => by
    rw [zpow_coe_nat, zpow_negSucc, ← pow_mul, ofNat_mul_negSucc, ← Int.ofNat_mul, zpow_neg_coe_nat]
  | -[m+1], (n : ℕ) => by
    rw [zpow_coe_nat, zpow_negSucc, ← inv_pow', ← pow_mul, negSucc_mul_ofNat, ← Int.ofNat_mul,
...
theorem coe_units_zpow (u : Mˣ) : ∀ n : ℤ, ((u ^ n : Mˣ) : M) = u ^ n
  | (n : ℕ) => by rw [zpow_ofNat, zpow_coe_nat, Units.val_pow_eq_pow_val]
  | -[k+1] => by
