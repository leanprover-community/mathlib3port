...
theorem pow_inv_comm' (A : M) (m n : ℕ) : (A⁻¹^m) ⬝ (A^n) = (A^n) ⬝ (A⁻¹^m) :=
  by 
    ...
    ·
      calc (A⁻¹^m+1) ⬝ (A^n+1) = (A⁻¹^m) ⬝ (A⁻¹ ⬝ A) ⬝ (A^n) :=
        by 
          simp only [pow_succ'ₓ (A⁻¹), pow_succₓ A, mul_eq_mul, Matrix.mul_assoc]_ = (A^n) ⬝ (A⁻¹^m) :=
        by 
          simp only [h, Matrix.mul_one, Matrix.one_mul, IH m]_ = (A^n) ⬝ (A ⬝ A⁻¹) ⬝ (A⁻¹^m) :=
        by 
          simp only [h', Matrix.mul_one, Matrix.one_mul]_ = (A^n+1) ⬝ (A⁻¹^m+1) :=
        by 
...
theorem _root_.is_unit.det_zpow {A : M} (h : IsUnit A.det) (n : ℤ) : IsUnit (A^n).det :=
  by 
    cases n
    ·
      simpa using h.pow n
...
theorem inv_zpow' {A : M} (h : IsUnit A.det) (n : ℤ) : (A⁻¹^n) = (A^-n) :=
  by 
...
theorem zpow_add_of_nonpos {A : M} {m n : ℤ} (hm : m ≤ 0) (hn : n ≤ 0) : (A^m+n) = (A^m)*A^n :=
  by 
    ...
    ·
      ...
      simpRw [←neg_add, ←Int.coe_nat_add, zpow_neg_coe_nat, ←inv_pow', h, pow_addₓ]
...
| -[1+ n] =>
  by 
    ...
    rw [←mul_assocₓ, ←(h.pow_right n.succ).Eq, mul_assocₓ, mul_eq_mul (X^_), mul_smul, mul_adjugate, mul_eq_mul,
      mul_eq_mul, mul_eq_mul, ←Matrix.mul_assoc, mul_smul (Y^_) («expr↑ » (hy'.unit⁻¹) : R), mul_adjugate, smul_smul,
...
| -(n : ℕ)+2, _ =>
  by 
    ...
    ·
      ...
      simpRw [zpow_neg_coe_nat, ←inv_pow', h, zero_pow Nat.succ_pos', zero_mul]
...
theorem zpow_mul' (A : M) (h : IsUnit A.det) (m n : ℤ) : (A^m*n) = ((A^n)^m) :=
  by 
    rw [mul_commₓ, zpow_mul _ h]

@[simp, normCast]
theorem units.coe_inv'' (u : Units M) : ((u⁻¹ : Units M) : M) = u⁻¹ :=
  by 
...
theorem zpow_neg_mul_zpow_self (n : ℤ) {A : M} (h : IsUnit A.det) : ((A^-n)*A^n) = 1 :=
  by 
    rw [zpow_neg h, mul_eq_mul, nonsing_inv_mul _ (h.det_zpow _)]

theorem one_div_pow {A : M} (n : ℕ) : (1 / A^n) = 1 / (A^n) :=
  by 
    simp only [one_div, inv_pow']

theorem one_div_zpow {A : M} (n : ℤ) : (1 / A^n) = 1 / (A^n) :=
  by 
