...
`matrix.dot_product`). -/
def HasOrthogonalRows [Fintypeₓ n] : Prop :=
  ∀ ⦃i₁ i₂⦄, i₁ ≠ i₂ → dotProduct (A i₁) (A i₂) = 0

/-- `A.has_orthogonal_rows` means matrix `A` has orthogonal columns (with respect to
`matrix.dot_product`). -/
def HasOrthogonalCols [Fintypeₓ m] : Prop :=
  HasOrthogonalRows Aᵀ

/-- `Aᵀ` has orthogonal rows iff `A` has orthogonal columns. -/
@[simp]
theorem transpose_has_orthogonal_rows_iff_has_orthogonal_cols [Fintypeₓ m] :
    Aᵀ.HasOrthogonalRows ↔ A.HasOrthogonalCols :=
  Iff.rfl

/-- `Aᵀ` has orthogonal columns iff `A` has orthogonal rows. -/
@[simp]
theorem transpose_has_orthogonal_cols_iff_has_orthogonal_rows [Fintypeₓ n] :
    Aᵀ.HasOrthogonalCols ↔ A.HasOrthogonalRows :=
  Iff.rfl

variable {A}

theorem HasOrthogonalRows.has_orthogonal_cols [Fintypeₓ m] (h : Aᵀ.HasOrthogonalRows) : A.HasOrthogonalCols :=
  h

theorem HasOrthogonalCols.transpose_has_orthogonal_rows [Fintypeₓ m] (h : A.HasOrthogonalCols) : Aᵀ.HasOrthogonalRows :=
  h

theorem HasOrthogonalCols.has_orthogonal_rows [Fintypeₓ n] (h : Aᵀ.HasOrthogonalCols) : A.HasOrthogonalRows :=
  h

theorem HasOrthogonalRows.transpose_has_orthogonal_cols [Fintypeₓ n] (h : A.HasOrthogonalRows) : Aᵀ.HasOrthogonalCols :=
  h
