...
open Finsetₓ LinearMap Matrix

open Matrix

section AuxToLinearMap

variable [CommSemiringₓ R] [CommSemiringₓ R₁] [CommSemiringₓ R₂]

variable [Fintypeₓ n] [Fintypeₓ m]
...
def Matrix.toLinearMap₂'Aux (f : Matrix n m R) : (n → R₁) →ₛₗ[σ₁] (m → R₂) →ₛₗ[σ₂] R :=
  mk₂'ₛₗ σ₁ σ₂ (fun (v : n → R₁) (w : m → R₂) => ∑ (i) (j), σ₁ (v i) * f i j * σ₂ (w j))
    (fun _ _ _ => by simp only [Pi.add_apply, map_add, add_mulₓ, sum_add_distrib])
    (fun _ _ _ => by simp only [Pi.smul_apply, smul_eq_mul, RingHom.map_mul, mul_assoc, mul_sum])
    (fun _ _ _ => by simp only [Pi.add_apply, map_add, mul_addₓ, sum_add_distrib]) fun _ _ _ => by
    simp only [Pi.smul_apply, smul_eq_mul, RingHom.map_mul, mul_assoc, mul_left_commₓ, mul_sum]

variable [DecidableEq n] [DecidableEq m]

-- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (i' j')
theorem Matrix.to_linear_map₂'_aux_std_basis (f : Matrix n m R) (i : n) (j : m) :
    f.toLinearMap₂'Aux σ₁ σ₂ (stdBasis R₁ (fun _ => R₁) i 1) (stdBasis R₂ (fun _ => R₂) j 1) = f i j := by
  rw [Matrix.toLinearMap₂'Aux, mk₂'ₛₗ_apply]
  have : (∑ (i') (j'), (if i = i' then 1 else 0) * f i' j' * if j = j' then 1 else 0) = f i j := by
    simp_rw [mul_assoc, ← Finsetₓ.mul_sum]
    simp only [boole_mul, Finsetₓ.sum_ite_eq, Finsetₓ.mem_univ, if_true, mul_comm (f _ _)]
  rw [← this]
  exact Finsetₓ.sum_congr rfl fun _ _ => Finsetₓ.sum_congr rfl fun _ _ => by simp
...
def LinearMap.toMatrix₂Aux (b₁ : n → M₁) (b₂ : m → M₂) : (M₁ →ₛₗ[σ₁] M₂ →ₛₗ[σ₂] R) →ₗ[R] Matrix n m R where
  toFun := fun f => of fun i j => f (b₁ i) (b₂ j)
  map_add' := fun f g => rfl
  map_smul' := fun f g => rfl

@[simp]
theorem LinearMap.to_matrix₂_aux_apply (f : M₁ →ₛₗ[σ₁] M₂ →ₛₗ[σ₂] R) (b₁ : n → M₁) (b₂ : m → M₂) (i : n) (j : m) :
...
variable [CommRingₓ R] [CommRingₓ R₁] [CommRingₓ R₂]
...
theorem LinearMap.to_linear_map₂'_aux_to_matrix₂_aux (f : (n → R₁) →ₛₗ[σ₁] (m → R₂) →ₛₗ[σ₂] R) :
    Matrix.toLinearMap₂'Aux σ₁ σ₂
        (LinearMap.toMatrix₂Aux (fun i => stdBasis R₁ (fun _ => R₁) i 1) (fun j => stdBasis R₂ (fun _ => R₂) j 1) f) =
      f :=
  by
...
theorem Matrix.to_matrix₂_aux_to_linear_map₂'_aux (f : Matrix n m R) :
    LinearMap.toMatrix₂Aux (fun i => stdBasis R₁ (fun _ => R₁) i 1) (fun j => stdBasis R₂ (fun _ => R₂) j 1)
        (f.toLinearMap₂'Aux σ₁ σ₂) =
      f :=
  by
  ext i j
...
def LinearMap.toMatrixₛₗ₂' : ((n → R₁) →ₛₗ[σ₁] (m → R₂) →ₛₗ[σ₂] R) ≃ₗ[R] Matrix n m R :=
  { LinearMap.toMatrix₂Aux (fun i => stdBasis R₁ (fun _ => R₁) i 1) fun j => stdBasis R₂ (fun _ => R₂) j 1 with
    toFun := LinearMap.toMatrix₂Aux _ _, invFun := Matrix.toLinearMap₂'Aux σ₁ σ₂,
    left_inv := LinearMap.to_linear_map₂'_aux_to_matrix₂_aux, right_inv := Matrix.to_matrix₂_aux_to_linear_map₂'_aux }
...
theorem Matrix.to_linear_map₂'_apply' (M : Matrix n m R) (v : n → R) (w : m → R) :
  ...
  simp_rw [Matrix.to_linear_map₂'_apply, Matrix.dotProduct, Matrix.mulVecₓ, Matrix.dotProduct]
...
theorem Matrix.to_linear_mapₛₗ₂'_std_basis (M : Matrix n m R) (i : n) (j : m) :
    Matrix.toLinearMapₛₗ₂' σ₁ σ₂ M (stdBasis R₁ (fun _ => R₁) i 1) (stdBasis R₂ (fun _ => R₂) j 1) = M i j :=
...
@[simp]
theorem LinearMap.to_matrix'_to_linear_map₂' (M : Matrix n m R) : LinearMap.toMatrix₂' (Matrix.toLinearMap₂' M) = M :=
  LinearMap.toMatrixₛₗ₂'.apply_symm_apply M

@[simp]
theorem LinearMap.to_matrixₛₗ₂'_apply (B : (n → R₁) →ₛₗ[σ₁] (m → R₂) →ₛₗ[σ₂] R) (i : n) (j : m) :
    LinearMap.toMatrixₛₗ₂' B i j = B (stdBasis R₁ (fun _ => R₁) i 1) (stdBasis R₂ (fun _ => R₂) j 1) :=
  rfl
...
theorem LinearMap.to_matrix₂'_compl₁₂ (B : (n → R) →ₗ[R] (m → R) →ₗ[R] R) (l : (n' → R) →ₗ[R] n → R)
    (r : (m' → R) →ₗ[R] m → R) : (B.compl₁₂ l r).toMatrix₂' = l.toMatrix'ᵀ ⬝ B.toMatrix₂' ⬝ r.toMatrix' := by
  ext i j
  simp only [LinearMap.to_matrix₂'_apply, LinearMap.compl₁₂_apply, transpose_apply, Matrix.mul_apply,
...
theorem LinearMap.to_matrix₂'_compl₂ (B : (n → R) →ₗ[R] (m → R) →ₗ[R] R) (f : (m' → R) →ₗ[R] m → R) :
    (B.compl₂ f).toMatrix₂' = B.toMatrix₂' ⬝ f.toMatrix' := by
  rw [← LinearMap.comp_id B, ← LinearMap.compl₁₂]
  simp

theorem LinearMap.mul_to_matrix₂'_mul (B : (n → R) →ₗ[R] (m → R) →ₗ[R] R) (M : Matrix n' n R) (N : Matrix m m' R) :
    M ⬝ B.toMatrix₂' ⬝ N = (B.compl₁₂ Mᵀ.toLin' N.toLin').toMatrix₂' := by simp
...
theorem LinearMap.to_matrix₂'_mul (B : (n → R) →ₗ[R] (m → R) →ₗ[R] R) (M : Matrix m m' R) :
    B.toMatrix₂' ⬝ M = (B.compl₂ M.toLin').toMatrix₂' := by simp only [B.to_matrix₂'_compl₂, to_matrix'_to_lin']
...
variable [DecidableEq m] [Fintypeₓ m]
...
noncomputable def LinearMap.toMatrix₂ : (M₁ →ₗ[R] M₂ →ₗ[R] R) ≃ₗ[R] Matrix n m R :=
  (b₁.equivFun.arrowCongr (b₂.equivFun.arrowCongr (LinearEquiv.refl R R))).trans LinearMap.toMatrix₂'
...
theorem LinearMap.to_matrix₂_apply (B : M₁ →ₗ[R] M₂ →ₗ[R] R) (i : n) (j : m) :
  ...
  simp only [LinearMap.toMatrix₂, LinearEquiv.trans_apply, LinearMap.to_matrix₂'_apply, LinearEquiv.trans_apply,
    LinearMap.to_matrix₂'_apply, LinearEquiv.arrow_congr_apply, Basis.equiv_fun_symm_std_basis, LinearEquiv.refl_apply]
...
theorem Matrix.to_linear_map₂_basis_fun :
  ...
  ext M
  simp only [Matrix.to_linear_map₂_apply, Matrix.to_linear_map₂'_apply, Pi.basis_fun_repr, coe_comp, Function.comp_app]

theorem LinearMap.to_matrix₂_basis_fun :
    LinearMap.toMatrix₂ (Pi.basisFun R n) (Pi.basisFun R m) = LinearMap.toMatrix₂' := by
  ext B
  rw [LinearMap.to_matrix₂_apply, LinearMap.to_matrix₂'_apply, Pi.basis_fun_apply, Pi.basis_fun_apply]
...
theorem LinearMap.to_matrix₂_compl₁₂ (B : M₁ →ₗ[R] M₂ →ₗ[R] R) (l : M₁' →ₗ[R] M₁) (r : M₂' →ₗ[R] M₂) :
    LinearMap.toMatrix₂ b₁' b₂' (B.compl₁₂ l r) =
      (toMatrix b₁' b₁ l)ᵀ ⬝ LinearMap.toMatrix₂ b₁ b₂ B ⬝ toMatrix b₂' b₂ r :=
  by
  ext i j
  simp only [LinearMap.to_matrix₂_apply, compl₁₂_apply, transpose_apply, Matrix.mul_apply, LinearMap.to_matrix_apply,
...
@[simp]
theorem LinearMap.to_matrix₂_mul_basis_to_matrix (c₁ : Basis n' R M₁) (c₂ : Basis m' R M₂) (B : M₁ →ₗ[R] M₂ →ₗ[R] R) :
    (b₁.toMatrix c₁)ᵀ ⬝ LinearMap.toMatrix₂ b₁ b₂ B ⬝ b₂.toMatrix c₂ = LinearMap.toMatrix₂ c₁ c₂ B := by
  simp_rw [← LinearMap.to_matrix_id_eq_basis_to_matrix]
  rw [← LinearMap.to_matrix₂_compl₁₂, LinearMap.compl₁₂_id_id]

theorem LinearMap.mul_to_matrix₂_mul (B : M₁ →ₗ[R] M₂ →ₗ[R] R) (M : Matrix n' n R) (N : Matrix m m' R) :
...
theorem Matrix.to_linear_map₂_compl₁₂ (M : Matrix n m R) (P : Matrix n n' R) (Q : Matrix m m' R) :
  ...
  (LinearMap.toMatrix₂ b₁' b₂').Injective
    (by simp only [LinearMap.to_matrix₂_compl₁₂ b₁ b₂, LinearMap.to_matrix₂_to_linear_map₂, to_matrix_to_lin])
...
variable [Fintypeₓ n] [Fintypeₓ n']
...
theorem is_adjoint_pair_to_linear_map₂' :
    IsAdjointPair (Matrix.toLinearMap₂' J) (Matrix.toLinearMap₂' J') (Matrix.toLin' A) (Matrix.toLin' A') ↔
      Matrix.IsAdjointPair J J' A A' :=
  by
  rw [is_adjoint_pair_iff_comp_eq_compl₂]
  have h : ∀ B B' : (n → R) →ₗ[R] (n' → R) →ₗ[R] R, B = B' ↔ LinearMap.toMatrix₂' B = LinearMap.toMatrix₂' B' := by
    intro B B'
    constructor <;> intro h
    · rw [h]
  ...
  simp_rw [h, LinearMap.to_matrix₂'_comp, LinearMap.to_matrix₂'_compl₂, LinearMap.to_matrix'_to_lin',
    LinearMap.to_matrix'_to_linear_map₂']
  rfl

@[simp]
theorem is_adjoint_pair_to_linear_map₂ :
    IsAdjointPair (Matrix.toLinearMap₂ b₁ b₁ J) (Matrix.toLinearMap₂ b₂ b₂ J') (Matrix.toLin b₁ b₂ A)
        (Matrix.toLin b₂ b₁ A') ↔
      Matrix.IsAdjointPair J J' A A' :=
  by
  rw [is_adjoint_pair_iff_comp_eq_compl₂]
  have h : ∀ B B' : M₁ →ₗ[R] M₂ →ₗ[R] R, B = B' ↔ LinearMap.toMatrix₂ b₁ b₂ B = LinearMap.toMatrix₂ b₁ b₂ B' := by
    intro B B'
    constructor <;> intro h
    · rw [h]
  ...
  simp_rw [h, LinearMap.to_matrix₂_comp b₂ b₂, LinearMap.to_matrix₂_compl₂ b₁ b₁, LinearMap.to_matrix_to_lin,
    LinearMap.to_matrix₂_to_linear_map₂]
  rfl
...
variable [DecidableEq ι] [Fintypeₓ ι]

theorem _root_.matrix.separating_left_to_linear_map₂'_iff_separating_left_to_linear_map₂ {M : Matrix ι ι R₁}
    (b : Basis ι R₁ M₁) : M.toLinearMap₂'.SeparatingLeft ↔ (Matrix.toLinearMap₂ b b M).SeparatingLeft :=
