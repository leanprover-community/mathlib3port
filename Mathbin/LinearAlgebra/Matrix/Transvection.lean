...
theorem exists_is_two_block_diagonal_of_ne_zero (hM : M (inr star) (inr star) ≠ 0) :
  ...
  by 
    ...
    refine' ⟨L, L', _⟩
    have A : L.map to_matrix = list_transvec_col M
    ·
      simp [L, list_transvec_col, · ∘ ·]
    have B : L'.map to_matrix = list_transvec_row M
    ·
...
theorem exists_list_transvec_mul_mul_list_transvec_eq_diagonal_induction
  ...
  by 
    ...
    refine'
      ⟨L₀.map (sum_inl Unit) ++ L₁, L₁' ++ L₀'.map (sum_inl Unit), Sum.elim D₀ fun _ => M' (inr star) (inr star), _⟩
    suffices  :
...
theorem reindex_exists_list_transvec_mul_mul_list_transvec_eq_diagonal (M : Matrix p p 𝕜) (e : p ≃ n)
  ...
  by 
    ...
    ·
      simp only [Equiv.symm_symm, minor_minor, reindex_apply, minor_id_id, Equiv.symm_comp_self,
...
theorem diagonal_transvection_induction (P : Matrix n n 𝕜 → Prop) (M : Matrix n n 𝕜)
  ...
  (htransvec : ∀ t : transvection_struct n 𝕜, P t.to_matrix) (hmul : ∀ A B, P A → P B → P (A ⬝ B)) : P M :=
  by 
