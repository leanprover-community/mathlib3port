...
theorem to_matrix_mk (i j : n) (hij : i ≠ j) (c : R) :
  ...
  rfl
#align matrix.transvection_struct.to_matrix_mk Matrix.TransvectionStruct.to_matrix_mk
...
@[simp]
theorem det_to_matrix_prod [Fintype n] (L : List (TransvectionStruct n 𝕜)) :
    det (L.map toMatrix).Prod = 1 := by
  induction' L with t L IH
  · simp
  · simp [IH]
#align matrix.transvection_struct.det_to_matrix_prod Matrix.TransvectionStruct.det_to_matrix_prod

/-- The inverse of a `transvection_struct`, designed so that `t.inv.to_matrix` is the inverse of
`t.to_matrix`. -/
@[simps]
protected def inv (t : TransvectionStruct n R) :
    TransvectionStruct n R where 
  i := t.i
  j := t.j
  hij := t.hij
  c := -t.c
...
#align
  matrix.transvection_struct.reverse_inv_prod_mul_prod Matrix.TransvectionStruct.reverse_inv_prod_mul_prod
...
#align
  matrix.transvection_struct.prod_mul_reverse_inv_prod Matrix.TransvectionStruct.prod_mul_reverse_inv_prod
...
def sumInl (t : TransvectionStruct n R) :
    TransvectionStruct (Sum n p) R where 
  i := inl t.i
  j := inl t.j
  hij := by simp [t.hij]
  c := t.c
...
theorem to_matrix_sum_inl (t : TransvectionStruct n R) :
...
#align matrix.transvection_struct.to_matrix_sum_inl Matrix.TransvectionStruct.to_matrix_sum_inl

@[simp]
theorem sum_inl_to_matrix_prod_mul [Fintype n] [Fintype p] (M : Matrix n n R)
...
#align
  matrix.transvection_struct.sum_inl_to_matrix_prod_mul Matrix.TransvectionStruct.sum_inl_to_matrix_prod_mul

@[simp]
theorem mul_sum_inl_to_matrix_prod [Fintype n] [Fintype p] (M : Matrix n n R)
    (L : List (TransvectionStruct n R)) (N : Matrix p p R) :
...
#align
  matrix.transvection_struct.mul_sum_inl_to_matrix_prod Matrix.TransvectionStruct.mul_sum_inl_to_matrix_prod

variable {p}

/-- Given a `transvection_struct` on `n` and an equivalence between `n` and `p`, define the
corresponding `transvection_struct` on `p`. -/
def reindexEquiv (e : n ≃ p) (t : TransvectionStruct n R) :
    TransvectionStruct p R where 
  i := e t.i
  j := e t.j
  hij := by simp [t.hij]
  c := t.c
...
#align
  matrix.transvection_struct.to_matrix_reindex_equiv Matrix.TransvectionStruct.to_matrix_reindex_equiv
...
#align
  matrix.transvection_struct.to_matrix_reindex_equiv_prod Matrix.TransvectionStruct.to_matrix_reindex_equiv_prod
...
#align
  matrix.pivot.mul_list_transvec_row_last_col_take Matrix.Pivot.mul_list_transvec_row_last_col_take
...
#align matrix.pivot.mul_list_transvec_row_last_col Matrix.Pivot.mul_list_transvec_row_last_col
  ...
  · have hnr : n < r := hk
    ...
    · have hni : n ≠ i := by 
      ...
      · rw [if_neg, if_neg]
        · simpa only [not_le] using hi
        · simpa only [hni.symm, not_le, or_false_iff] using Nat.lt_succ_iff_lt_or_eq.1 hi
#align matrix.pivot.mul_list_transvec_row_last_row Matrix.Pivot.mul_list_transvec_row_last_row
...
theorem list_transvec_col_mul_mul_list_transvec_row_last_col (hM : M (inr unit) (inr unit) ≠ 0)
    (i : Fin r) :
...
#align
  matrix.pivot.list_transvec_col_mul_mul_list_transvec_row_last_col Matrix.Pivot.list_transvec_col_mul_mul_list_transvec_row_last_col

/-- Multiplying by all the matrices either in `list_transvec_col M` and `list_transvec_row M` kills
all the coefficients in the last column but the last one. -/
theorem list_transvec_col_mul_mul_list_transvec_row_last_row (hM : M (inr unit) (inr unit) ≠ 0)
    (i : Fin r) :
...
#align
  matrix.pivot.list_transvec_col_mul_mul_list_transvec_row_last_row Matrix.Pivot.list_transvec_col_mul_mul_list_transvec_row_last_row
...
#align
  matrix.pivot.exists_is_two_block_diagonal_of_ne_zero Matrix.Pivot.exists_is_two_block_diagonal_of_ne_zero
...
theorem exists_list_transvec_mul_mul_list_transvec_eq_diagonal_induction
  ...
  simp [h₀]
#align
  matrix.pivot.exists_list_transvec_mul_mul_list_transvec_eq_diagonal_induction Matrix.Pivot.exists_list_transvec_mul_mul_list_transvec_eq_diagonal_induction
...
#align
  matrix.pivot.exists_list_transvec_mul_diagonal_mul_list_transvec Matrix.Pivot.exists_list_transvec_mul_diagonal_mul_list_transvec
...
theorem diagonalTransvectionInduction (P : Matrix n n 𝕜 → Prop) (M : Matrix n n 𝕜)
...
#align matrix.diagonal_transvection_induction Matrix.diagonalTransvectionInduction
...
theorem diagonalTransvectionInductionOfDetNeZero (P : Matrix n n 𝕜 → Prop) (M : Matrix n n 𝕜)
  ...
  exact this.2
#align
  matrix.diagonal_transvection_induction_of_det_ne_zero Matrix.diagonalTransvectionInductionOfDetNeZero
