...
theorem updateRow_eq_transvection [Finite n] (c : R) :
  ...
  ·
    simp only [updateRow, transvection, ha, hb, Function.update_same, stdBasisMatrix.apply_same,
      Pi.add_apply, one_apply_eq, Pi.smul_apply, mul_one, Algebra.id.smul_eq_mul]
  ·
    simp only [updateRow, transvection, ha, hb, stdBasisMatrix.apply_of_ne, Function.update_same,
      Pi.add_apply, Ne.def, not_false_iff, Pi.smul_apply, and_false_iff, one_apply_ne,
      Algebra.id.smul_eq_mul, mul_zero]
  ·
    simp only [updateRow, transvection, ha, Ne.symm ha, stdBasisMatrix.apply_of_ne, add_zero,
...
theorem reverse_inv_prod_mul_prod (L : List (TransvectionStruct n R)) :
  ...
  · simp
  · suffices
      (L.reverse.map (toMatrix ∘ TransvectionStruct.inv)).prod ⬝ (t.inv.to_matrix ⬝ t.to_matrix) ⬝
          (L.map toMatrix).prod =
        1
...
theorem listTransvecCol_mul_last_col (hM : M (inr unit) (inr unit) ≠ 0) (i : Fin r) :
  ...
  · intro n hn hk IH
    ...
    · have hni : n = i := by
        ...
        simp [h]
      rw [h, transvection_mul_apply_same, IH, listTransvecCol_mul_last_row_drop _ _ hn, ← hni]
      field_simp [hM]
    · have hni : n ≠ i := by
        rintro rfl
        cases i
