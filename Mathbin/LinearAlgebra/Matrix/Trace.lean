...
@[simp]
theorem diag_one [DecidableEq n] : diag n R R 1 = fun i => 1 :=
  by 
    dunfold diag 
    ext 
    simp [one_apply_eq]
...
@[simp]
theorem diag_col_mul_row (a b : n → R) : diag n R R (col a ⬝ row b) = a*b :=
  by 
    ext 
...
@[simp]
theorem trace_col_mul_row (a b : n → R) : trace n R R (col a ⬝ row b) = dot_product a b :=
  by 
    simp [dot_product]
...
theorem trace_fin_three (A : Matrix (Finₓ 3) (Finₓ 3) R) : trace _ R R A = (A 0 0+A 1 1)+A 2 2 :=
  by 
