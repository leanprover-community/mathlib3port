...
theorem LinearMap.to_matrix'_algebra_map (x : R) :
  LinearMap.toMatrix' (algebraMap R (Module.End R (n → R)) x) = scalar n x :=
  by 
    simp [Module.algebra_map_End_eq_smul_id]

theorem Matrix.ker_to_lin'_eq_bot_iff {M : Matrix n n R} : M.to_lin'.ker = ⊥ ↔ ∀ v, M.mul_vec v = 0 → v = 0 :=
  by 
...
theorem LinearMap.to_matrix_alg_equiv'_apply (f : (n → R) →ₗ[R] n → R) i j :
  LinearMap.toMatrixAlgEquiv' f i j = f (fun j' => if j' = j then 1 else 0) i :=
  by 
...
`pi.basis_fun R n`. -/
theorem Matrix.to_lin_eq_to_lin' : Matrix.toLin (Pi.basisFun R n) (Pi.basisFun R n) = Matrix.toLin' :=
  rfl
...
@[simp]
theorem Matrix.to_lin_to_matrix (f : M₁ →ₗ[R] M₂) : Matrix.toLin v₁ v₂ (LinearMap.toMatrix v₁ v₂ f) = f :=
  by 
    rw [←Matrix.to_lin_symm, LinearEquiv.apply_symm_apply]

@[simp]
theorem LinearMap.to_matrix_to_lin (M : Matrix m n R) : LinearMap.toMatrix v₁ v₂ (Matrix.toLin v₁ v₂ M) = M :=
  by 
...
theorem Matrix.to_lin_apply (M : Matrix m n R) (v : M₁) : Matrix.toLin v₁ v₂ M v = ∑j, M.mul_vec (v₁.repr v) j • v₂ j :=
...
@[simp]
theorem Matrix.to_lin_one : Matrix.toLin v₁ v₁ 1 = id :=
  by 
...
theorem LinearMap.to_matrix_comp [Fintype l] [DecidableEq m] (f : M₂ →ₗ[R] M₃) (g : M₁ →ₗ[R] M₂) :
  ...
  by 
    simpRw [LinearMap.toMatrix, LinearEquiv.trans_apply, LinearEquiv.arrow_congr_comp _ v₂.equiv_fun,
...
theorem LinearMap.to_matrix_algebra_map (x : R) :
  LinearMap.toMatrix v₁ v₁ (algebraMap R (Module.End R M₁) x) = scalar n x :=
  by 
...
[decidable_eq m]
(A : matrix l m R)
(B : matrix m n R) : «expr = »(matrix.to_lin v₁ v₃ «expr ⬝ »(A, B), (matrix.to_lin v₂ v₃ A).comp (matrix.to_lin v₁ v₂ B)) :=
begin
  apply [expr (linear_map.to_matrix v₁ v₃).injective],
  haveI [] [":", expr decidable_eq l] [":=", expr λ _ _, classical.prop_decidable _],
  rw [expr linear_map.to_matrix_comp v₁ v₂ v₃] [],
  repeat { rw [expr linear_map.to_matrix_to_lin] [] }
end
...
theorem Matrix.to_lin_alg_equiv_to_matrix_alg_equiv (f : M₁ →ₗ[R] M₁) :
  Matrix.toLinAlgEquiv v₁ (LinearMap.toMatrixAlgEquiv v₁ f) = f :=
  by 
...
theorem LinearMap.to_matrix_alg_equiv_to_lin_alg_equiv (M : Matrix n n R) :
  LinearMap.toMatrixAlgEquiv v₁ (Matrix.toLinAlgEquiv v₁ M) = M :=
  by 
...
theorem Matrix.to_lin_alg_equiv_apply (M : Matrix n n R) (v : M₁) :
  Matrix.toLinAlgEquiv v₁ M v = ∑j, M.mul_vec (v₁.repr v) j • v₁ j :=
  show v₁.equiv_fun.symm (Matrix.toLinAlgEquiv' M (v₁.repr v)) = _ by 
    rw [Matrix.to_lin_alg_equiv'_apply, v₁.equiv_fun_symm_apply]

@[simp]
theorem Matrix.to_lin_alg_equiv_self (M : Matrix n n R) (i : n) : Matrix.toLinAlgEquiv v₁ M (v₁ i) = ∑j, M j i • v₁ j :=
  Matrix.to_lin_self _ _ _ _

theorem LinearMap.to_matrix_alg_equiv_id : LinearMap.toMatrixAlgEquiv v₁ id = 1 :=
  by 
    simpRw [LinearMap.toMatrixAlgEquiv, AlgEquiv.of_linear_equiv_apply, LinearMap.to_matrix_id]

@[simp]
theorem Matrix.to_lin_alg_equiv_one : Matrix.toLinAlgEquiv v₁ 1 = id :=
  by 
...
theorem LinearMap.to_matrix_alg_equiv_reindex_range [DecidableEq M₁] (f : M₁ →ₗ[R] M₁) (k i : n) :
  ...
  by 
    simpRw [LinearMap.to_matrix_alg_equiv_apply, Basis.reindex_range_self, Basis.reindex_range_repr]
...
theorem Matrix.to_lin_alg_equiv_mul (A B : Matrix n n R) :
  Matrix.toLinAlgEquiv v₁ (A ⬝ B) = (Matrix.toLinAlgEquiv v₁ A).comp (Matrix.toLinAlgEquiv v₁ B) :=
  by 
...
theorem to_matrix_lmul' (x : S) i j : LinearMap.toMatrix b b (lmul R S x) i j = b.repr (x*b j) i :=
  by 
    rw [LinearMap.to_matrix_apply', lmul_apply]

@[simp]
theorem to_matrix_lsmul (x : R) i j : LinearMap.toMatrix b b (Algebra.lsmul R S x) i j = if i = j then x else 0 :=
  by 
...
theorem left_mul_matrix_injective : Function.Injective (left_mul_matrix b) :=
  fun x x' h =>
    calc x = Algebra.lmul R S x 1 := (mul_oneₓ x).symm 
      _ = Algebra.lmul R S x' 1 :=
      by 
...
theorem smul_left_mul_matrix_algebra_map (x : S) :
  left_mul_matrix (b.smul c) (algebraMap _ _ x) = block_diagonal fun k => left_mul_matrix b x :=
  by 
    ext ⟨i, k⟩ ⟨j, k'⟩
...
theorem smul_left_mul_matrix_algebra_map_eq (x : S) i j k :
  left_mul_matrix (b.smul c) (algebraMap _ _ x) (i, k) (j, k) = left_mul_matrix b x i j :=
  by 
    rw [smul_left_mul_matrix_algebra_map, block_diagonal_apply_eq]

theorem smul_left_mul_matrix_algebra_map_ne (x : S) i j {k k'} (h : k ≠ k') :
  left_mul_matrix (b.smul c) (algebraMap _ _ x) (i, k) (j, k') = 0 :=
  by 
...
theorem finrank_linear_map :
  FiniteDimensional.finrank K (V →ₗ[K] W) = FiniteDimensional.finrank K V*FiniteDimensional.finrank K W :=
  by 
