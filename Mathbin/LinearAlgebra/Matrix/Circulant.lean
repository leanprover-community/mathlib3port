...
theorem circulant_injective [AddGroupₓ n] : injective (circulant : (n → α) → Matrix n n α) :=
  by 
    intro v w h 
    ext k 
...
theorem transpose_circulant [AddGroupₓ n] (v : n → α) : (circulant v)ᵀ = circulant fun i => v (-i) :=
  by 
    ext <;> simp 
...
theorem circulant_smul [Sub n] [HasScalar R α] (k : R) (v : n → α) : circulant (k • v) = k • circulant v :=
  by 
    ext <;> simp 
...
theorem circulant_is_symm_iff [AddGroupₓ n] {v : n → α} : (circulant v).IsSymm ↔ ∀ i, v (-i) = v i :=
  by 
