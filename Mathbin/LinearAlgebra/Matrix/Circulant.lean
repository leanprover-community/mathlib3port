...
theorem transpose_circulant [AddGroupₓ n] (v : n → α) : (circulant v)ᵀ = circulant fun i => v (-i) := by ext <;> simp
...
/-- The product of two circulant matrices `circulant v` and `circulant w` is
    the circulant matrix generated by `mul_vec (circulant v) w`. -/
theorem circulant_mul [Semiringₓ α] [Fintypeₓ n] [AddGroupₓ n] (v w : n → α) :
    circulant v ⬝ circulant w = circulant (mulVecₓ (circulant v) w) := by
  ext i j
  simp only [mul_apply, mul_vec, circulant, dot_product]
  refine' Fintypeₓ.sum_equiv (Equivₓ.subRight j) _ _ _
  intro x
...
theorem circulant_mul_comm [CommSemigroupₓ α] [AddCommMonoidₓ α] [Fintypeₓ n] [AddCommGroupₓ n] (v w : n → α) :
    circulant v ⬝ circulant w = circulant w ⬝ circulant v := by
  ext i j
  simp only [mul_apply, circulant, mul_comm]
  refine' Fintypeₓ.sum_equiv ((Equivₓ.subLeft i).trans (Equivₓ.addRight j)) _ _ _
  intro x
  congr 2
  · simp
...
@[simp]
theorem circulant_single (n) [Semiringₓ α] [DecidableEq n] [AddGroupₓ n] [Fintypeₓ n] (a : α) :
