...
def BlockTriangular' {o : Type _} (M : Matrix o o R) {n : ℕ} (b : o → Finₓ n) : Prop :=
  ∀ i j, b j < b i → M i j = 0
...
theorem upper_two_block_triangular' {m n : Type _} (A : Matrix m m R) (B : Matrix m n R) (D : Matrix n n R) :
    (fromBlocks A B 0 D).BlockTriangular' (Sum.elim (fun i => (0 : Finₓ 2)) fun j => 1) := by
  intro k1 k2 hk12
...
/-- Let `b` map rows and columns of a square matrix `M` to blocks indexed by `ℕ`s. Then
  `M.block_triangular b` says the matrix is block triangular. -/
def BlockTriangular {o : Type _} (M : Matrix o o R) (b : o → ℕ) : Prop :=
  ∀ i j, b j < b i → M i j = 0

theorem upper_two_block_triangular {m n : Type _} (A : Matrix m m R) (B : Matrix m n R) (D : Matrix n n R) :
    (fromBlocks A B 0 D).BlockTriangular (Sum.elim (fun i => 0) fun j => 1) := by
  intro k1 k2 hk12
...
theorem det_of_block_triangular (M : Matrix m m R) (b : m → ℕ) (h : M.BlockTriangular b) :
  ...
  · rw [Finset.prod_range_succ_comm]
    ...
    · rw [mul_comm]
      ...
      · let m' := { a // ¬b a = n }
        let b' := fun i : m' => b ↑i
        have h' : (M.to_square_block_prop fun i : m => ¬b i = n).BlockTriangular b' := by
          intro i j
          apply h ↑i ↑j
...
theorem det_of_block_triangular' (M : Matrix m m R) {n : ℕ} (b : m → Finₓ n) (h : M.BlockTriangular' b) :
...
theorem det_of_upper_triangular {n : ℕ} (M : Matrix (Finₓ n) (Finₓ n) R) (h : ∀ i j : Finₓ n, j < i → M i j = 0) :
  ...
  convert det_of_block_triangular' M id h
  ext i
