  ...
  by 
    ...
    ·
      ...
      ·
        ...
        ·
          let m' := { a // ¬b a = n }
          let b' := fun i : m' => b («expr↑ » i)
          have h' : block_triangular_matrix (M.to_square_block_prop fun i : m => ¬b i = n) b'
          ·
            intro i j 
            apply h («expr↑ » i) («expr↑ » j)
          have hni : ∀ i : { a // ¬b a = n }, b' i < n
          ·
            exact fun i => (Ne.le_iff_lt i.property).mp (nat.lt_succ_iff.mp (hn («expr↑ » i)))
          ...
          let he : { a // b' a = «expr↑ » k } ≃ { a // b a = «expr↑ » k }
          ·
            have hc : ∀ i : m, (fun a => b a = «expr↑ » k) i → (fun a => ¬b a = n) i
            ·
              intro i hbi 
              rw [hbi]
              exact ne_of_ltₓ (Finₓ.is_lt k)
            exact Equiv.subtypeSubtypeEquivSubtype hc 
          exact Matrix.det_reindex_self he fun i j : { a // b' a = «expr↑ » k } => M («expr↑ » i) («expr↑ » j)
        ·
