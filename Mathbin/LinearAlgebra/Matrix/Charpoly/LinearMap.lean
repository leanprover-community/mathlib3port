...
def PiToModule.fromMatrix [DecidableEq ι] : Matrix ι ι R →ₗ[R] (ι → R) →ₗ[R] M :=
  (LinearMap.llcomp R _ _ _ (Fintypeₓ.total R R b)).comp algEquivMatrix'.symm.toLinearMap

theorem PiToModule.from_matrix_apply [DecidableEq ι] (A : Matrix ι ι R) (w : ι → R) :
    PiToModule.fromMatrix R b A w = Fintypeₓ.total R R b (A.mulVec w) :=
  rfl
...
theorem PiToModule.from_matrix_apply_single_one [DecidableEq ι] (A : Matrix ι ι R) (j : ι) :
  ...
  rw [PiToModule.from_matrix_apply, Fintypeₓ.total_apply, Matrix.mul_vec_single]
  simp_rw [mul_oneₓ]

/-- The endomorphisms of `M` acts on `(ι → R) →ₗ[R] M`, and takes the projection
to a `(ι → R) →ₗ[R] M`. -/
def PiToModule.fromEnd : Module.End R M →ₗ[R] (ι → R) →ₗ[R] M :=
  LinearMap.lcomp _ _ (Fintypeₓ.total R R b)

theorem PiToModule.from_End_apply (f : Module.End R M) (w : ι → R) :
    PiToModule.fromEnd R b f w = f (Fintypeₓ.total R R b w) :=
  rfl

theorem PiToModule.from_End_apply_single_one [DecidableEq ι] (f : Module.End R M) (i : ι) :
    PiToModule.fromEnd R b f (Pi.single i 1) = f (b i) := by
  rw [PiToModule.from_End_apply]
  congr
  convert Fintypeₓ.total_apply_single R b i 1
  rw [one_smul]
...
theorem PiToModule.from_End_injective (hb : Submodule.span R (Set.Range b) = ⊤) :
  ...
  intro x y e
  ext m
  obtain ⟨m, rfl⟩ : m ∈ (Fintypeₓ.total R R b).range := by
    rw [(Fintypeₓ.range_total R b).trans hb]
    trivial
...
theorem Matrix.Represents.congr_fun {A : Matrix ι ι R} {f : Module.End R M} (h : A.Represents b f) (x) :
    Fintypeₓ.total R R b (A.mulVec x) = f (Fintypeₓ.total R R b x) :=
  LinearMap.congr_fun h x

theorem Matrix.represents_iff {A : Matrix ι ι R} {f : Module.End R M} :
    A.Represents b f ↔ ∀ x, Fintypeₓ.total R R b (A.mulVec x) = f (Fintypeₓ.total R R b x) :=
...
def Matrix.isRepresentation : Subalgebra R (Matrix ι ι R) where
  ...
  algebra_map_mem' := fun r => ⟨r • 1, Matrix.Represents.one.smul r⟩

/-- The map sending a matrix to the endomorphism it represents. This is an `R`-algebra morphism. -/
noncomputable def Matrix.isRepresentation.toEnd : Matrix.isRepresentation R b →ₐ[R] Module.End R M where
  toFun := fun A => A.2.some
  map_one' := (1 : Matrix.isRepresentation R b).2.some_spec.Eq hb Matrix.Represents.one
  map_mul' := fun A₁ A₂ => (A₁ * A₂).2.some_spec.Eq hb (A₁.2.some_spec.mul A₂.2.some_spec)
  map_zero' := (0 : Matrix.isRepresentation R b).2.some_spec.Eq hb Matrix.Represents.zero
  map_add' := fun A₁ A₂ => (A₁ + A₂).2.some_spec.Eq hb (A₁.2.some_spec.add A₂.2.some_spec)
  commutes' := fun r => (r • 1 : Matrix.isRepresentation R b).2.some_spec.Eq hb (Matrix.Represents.one.smul r)
...
theorem Matrix.isRepresentation.eq_to_End_of_represents (A : Matrix.isRepresentation R b) {f : Module.End R M}
    (h : (A : Matrix ι ι R).Represents b f) : Matrix.isRepresentation.toEnd R b hb A = f :=
  A.2.some_spec.Eq hb h

theorem Matrix.isRepresentation.to_End_exists_mem_ideal (f : Module.End R M) (I : Ideal R) (hI : f.range ≤ I • ⊤) :
...
theorem LinearMap.exists_monic_and_coeff_mem_pow_and_aeval_eq_zero_of_range_le_smul [Module.Finite R M]
  ...
  obtain ⟨s : Finsetₓ M, hs : Submodule.span R (s : Set M) = ⊤⟩ := Module.Finite.out
  obtain ⟨A, rfl, h⟩ :=
    Matrix.isRepresentation.to_End_exists_mem_ideal R (coe : s → M)
      (by rw [Subtype.range_coe_subtype, Finsetₓ.set_of_mem, hs]) f I hI
