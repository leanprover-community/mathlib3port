...
def is_diag [Zero α] (A : Matrix n n α) : Prop :=
  ∀ ⦃i j⦄, i ≠ j → A i j = 0
...
theorem is_diag.exists_diagonal [Zero α] [DecidableEq n] {A : Matrix n n α} (h : A.IsDiag) : ∃ d, diagonalₓ d = A := by
...
theorem is_diag.map [Zero α] [Zero β] {A : Matrix n n α} (ha : A.IsDiag) {f : α → β} (hf : f 0 = 0) :
    (A.map f).IsDiag := by
  intro i j h
  simp [ha h, hf]

theorem is_diag.neg [AddGroupₓ α] {A : Matrix n n α} (ha : A.IsDiag) : (-A).IsDiag := by
  intro i j h
  simp [ha h]
...
theorem is_diag.add [AddZeroClass α] {A B : Matrix n n α} (ha : A.IsDiag) (hb : B.IsDiag) : (A + B).IsDiag := by
  intro i j h
  simp [ha h, hb h]

theorem is_diag.sub [AddGroupₓ α] {A B : Matrix n n α} (ha : A.IsDiag) (hb : B.IsDiag) : (A - B).IsDiag := by
  intro i j h
  simp [ha h, hb h]

theorem is_diag.smul [Monoidₓ R] [AddMonoidₓ α] [DistribMulAction R α] (k : R) {A : Matrix n n α} (ha : A.IsDiag) :
    (k • A).IsDiag := by
  intro i j h
  simp [ha h]
...
theorem is_diag.transpose [Zero α] {A : Matrix n n α} (ha : A.IsDiag) : (A)ᵀ.IsDiag := fun i j h => ha h.symm

@[simp]
theorem is_diag_transpose_iff [Zero α] {A : Matrix n n α} : (A)ᵀ.IsDiag ↔ A.IsDiag :=
  ⟨IsDiag.transpose, IsDiag.transpose⟩

theorem is_diag.conj_transpose [Semiringₓ α] [StarRing α] {A : Matrix n n α} (ha : A.IsDiag) : (A)ᴴ.IsDiag :=
...
theorem is_diag.minor [Zero α] {A : Matrix n n α} (ha : A.IsDiag) {f : m → n} (hf : Injective f) :
    (A.minor f f).IsDiag := fun i j h => ha (hf.Ne h)

/-- `(A ⊗ B).is_diag` if both `A` and `B` are diagonal. -/
theorem is_diag.kronecker [MulZeroClassₓ α] {A : Matrix m m α} {B : Matrix n n α} (hA : A.IsDiag) (hB : B.IsDiag) :
    (A ⊗ₖ B).IsDiag := by
  rintro ⟨a, b⟩ ⟨c, d⟩ h
...
theorem is_diag.is_symm [Zero α] {A : Matrix n n α} (h : A.IsDiag) : A.IsSymm := by
  ext i j
  by_cases' g : i = j
  · rw [g]
...
theorem is_diag.from_blocks [Zero α] {A : Matrix m m α} {D : Matrix n n α} (ha : A.IsDiag) (hd : D.IsDiag) :
