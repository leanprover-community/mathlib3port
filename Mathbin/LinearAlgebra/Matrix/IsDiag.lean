...
@[simp]
theorem is_diag_diagonal [HasZero α] [DecidableEq n] (d : n → α) : (diagonal d).IsDiag := fun i j =>
...
theorem is_diag_of_subsingleton [HasZero α] [Subsingleton n] (A : Matrix n n α) : A.is_diag := fun i j h =>
  (h $ Subsingleton.elimₓ i j).elim

/--  Every zero matrix is diagonal. -/
@[simp]
theorem is_diag_zero [HasZero α] : (0 : Matrix n n α).IsDiag := fun i j h => rfl

/--  Every identity matrix is diagonal. -/
@[simp]
theorem is_diag_one [DecidableEq n] [HasZero α] [HasOne α] : (1 : Matrix n n α).IsDiag := fun i j => one_apply_ne
...
theorem is_diag.transpose [HasZero α] {A : Matrix n n α} (ha : A.is_diag) : (A)ᵀ.IsDiag := fun i j h => ha h.symm
...
theorem is_diag.is_symm [HasZero α] {A : Matrix n n α} (h : A.is_diag) : A.is_symm := by
  ext i j
  by_cases' g : i = j
  ·
    rw [g]
...
theorem is_diag_from_blocks_iff [HasZero α] {A : Matrix m m α} {B : Matrix m n α} {C : Matrix n m α}
    {D : Matrix n n α} : (A.from_blocks B C D).IsDiag ↔ A.is_diag ∧ B = 0 ∧ C = 0 ∧ D.is_diag := by
  constructor
  ·
    intro h
