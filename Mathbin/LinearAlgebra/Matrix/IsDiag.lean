...
theorem is_diag.exists_diagonal [Zero α] [DecidableEq n] {A : Matrix n n α} (h : A.is_diag) : ∃ d, diagonal d = A := by
...
theorem is_diag_iff_exists_diagonal [Zero α] [DecidableEq n] (A : Matrix n n α) : A.is_diag ↔ ∃ d, diagonal d = A :=
...
theorem is_diag.neg [AddGroupₓ α] {A : Matrix n n α} (ha : A.is_diag) : (-A).IsDiag := by
  intro i j h
  simp [ha h]
...
theorem is_diag.add [AddZeroClass α] {A B : Matrix n n α} (ha : A.is_diag) (hb : B.is_diag) : (A + B).IsDiag := by
  intro i j h
  simp [ha h, hb h]

theorem is_diag.sub [AddGroupₓ α] {A B : Matrix n n α} (ha : A.is_diag) (hb : B.is_diag) : (A - B).IsDiag := by
  intro i j h
  simp [ha h, hb h]
...
theorem is_diag.transpose [Zero α] {A : Matrix n n α} (ha : A.is_diag) : (A)ᵀ.IsDiag := fun i j h => ha h.symm

@[simp]
theorem is_diag_transpose_iff [Zero α] {A : Matrix n n α} : (A)ᵀ.IsDiag ↔ A.is_diag :=
...
theorem is_diag.minor [Zero α] {A : Matrix n n α} (ha : A.is_diag) {f : m → n} (hf : injective f) :
...
theorem is_diag.is_symm [Zero α] {A : Matrix n n α} (h : A.is_diag) : A.is_symm := by
  ext i j
  by_cases' g : i = j
  · rw [g]
...
theorem is_diag_from_blocks_iff [Zero α] {A : Matrix m m α} {B : Matrix m n α} {C : Matrix n m α} {D : Matrix n n α} :
    (A.from_blocks B C D).IsDiag ↔ A.is_diag ∧ B = 0 ∧ C = 0 ∧ D.is_diag := by
  constructor
  · intro h
...
theorem is_diag.from_blocks_of_is_symm [Zero α] {A : Matrix m m α} {C : Matrix n m α} {D : Matrix n n α}
    (h : (A.from_blocks 0 C D).IsSymm) (ha : A.is_diag) (hd : D.is_diag) : (A.from_blocks 0 C D).IsDiag := by
