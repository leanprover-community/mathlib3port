...
open Equivₓ Equivₓ.Perm Finsetₓ Function

namespace Matrix

open Matrix BigOperators

variable {m n : Type _} [DecidableEq n] [Fintypeₓ n] [DecidableEq m] [Fintypeₓ m]
...
theorem det_diagonal {d : n → R} : det (diagonalₓ d) = ∏ i, d i := by
  rw [det_apply']
  refine' (Finsetₓ.sum_eq_single 1 _ _).trans _
  · intro σ h1 h2
    cases' not_forall.1 (mt Equivₓ.ext h2) with x h3
    convert mul_zero _
    apply Finsetₓ.prod_eq_zero
    · change x ∈ _
      simp
...
theorem det_eq_one_of_card_eq_zero {A : Matrix n n R} (h : Fintypeₓ.card n = 0) : det A = 1 :=
...
@[simp]
theorem det_unique {n : Type _} [Unique n] [DecidableEq n] [Fintypeₓ n] (A : Matrix n n R) :
...
theorem det_eq_elem_of_card_eq_one {A : Matrix n n R} (h : Fintypeₓ.card n = 1) (k : n) : det A = A k k :=
    ...
    sum_involution (fun σ _ => σ * swap i j)
      (fun σ _ => by
        have : (∏ x, M (σ x) (p x)) = ∏ x, M ((σ * swap i j) x) (p x) :=
          Fintypeₓ.prod_equiv (swap i j) _ _ (by simp [apply_swap_eq_self hpij])
...
theorem det_mul (M N : Matrix n n R) : det (M ⬝ N) = det M * det N :=
  calc
    det (M ⬝ N) = ∑ p : n → n, ∑ σ : Perm n, ε σ * ∏ i, M (σ i) (p i) * N (p i) i := by
      simp only [det_apply', mul_apply, prod_univ_sum, mul_sum, Fintypeₓ.pi_finset_univ] <;> rw [Finsetₓ.sum_comm]
    _ = ∑ p in (@univ (n → n) _).filter Bijective, ∑ σ : Perm n, ε σ * ∏ i, M (σ i) (p i) * N (p i) i :=
      Eq.symm <|
        sum_subset (filter_subset _ _) fun f _ hbij =>
          det_mul_aux <| by simpa only [true_andₓ, mem_filter, mem_univ] using hbij
    ...
    _ = ∑ σ : Perm n, ∑ τ : Perm n, (∏ i, N (σ i) i) * (ε σ * ε τ) * ∏ i, M (τ i) i :=
      sum_congr rfl fun σ _ =>
        Fintypeₓ.sum_equiv (Equivₓ.mulRight σ⁻¹) _ _ fun τ => by
          ...
          simp only [this]
    _ = det M * det N := by simp only [det_apply', Finsetₓ.mul_sum, mul_comm, mul_left_commₓ]
...
theorem det_mul_comm (M N : Matrix m m R) : det (M ⬝ N) = det (N ⬝ M) := by rw [det_mul, det_mul, mul_comm]
...
theorem det_transpose (M : Matrix n n R) : Mᵀ.det = M.det := by
  ...
  refine' Fintypeₓ.sum_bijective _ inv_involutive.bijective _ _ _
  intro σ
  rw [sign_inv]
  congr 1
  apply Fintypeₓ.prod_equiv σ
  intros
  simp
...
@[simp]
theorem det_submatrix_equiv_self (e : n ≃ m) (A : Matrix m m R) : det (A.submatrix e e) = det A := by
  rw [det_apply', det_apply']
  apply Fintypeₓ.sum_equiv (Equivₓ.permCongr e)
  intro σ
  rw [Equivₓ.Perm.sign_perm_congr e σ]
  congr 1
  apply Fintypeₓ.prod_equiv e
  intro i
  rw [Equivₓ.perm_congr_apply, Equivₓ.symm_apply_apply, submatrix_apply]
...
theorem det_smul (A : Matrix n n R) (c : R) : det (c • A) = c ^ Fintypeₓ.card n * det A :=
  calc
    det (c • A) = det (Matrix.mul (diagonalₓ fun _ => c) A) := by rw [smul_eq_diagonal_mul]
    _ = det (diagonalₓ fun _ => c) * det A := det_mul _ _
    _ = c ^ Fintypeₓ.card n * det A := by simp [card_univ]
    

@[simp]
theorem det_smul_of_tower {α} [Monoidₓ α] [DistribMulAction α R] [IsScalarTower α R R] [SmulCommClass α R R] (c : α)
    (A : Matrix n n R) : det (c • A) = c ^ Fintypeₓ.card n • det A := by
  rw [← smul_one_smul R c A, det_smul, smul_pow, one_pow, smul_mul_assoc, one_mulₓ]

theorem det_neg (A : Matrix n n R) : det (-A) = -1 ^ Fintypeₓ.card n * det A := by rw [← det_smul, neg_one_smul]

/-- A variant of `matrix.det_neg` with scalar multiplication by `units ℤ` instead of multiplication
by `R`. -/
theorem det_neg_eq_smul (A : Matrix n n R) : det (-A) = (-1 : Units ℤ) ^ Fintypeₓ.card n • det A := by
...
theorem det_update_row_smul' (M : Matrix n n R) (j : n) (s : R) (u : n → R) :
    det (updateRow (s • M) j u) = s ^ (Fintypeₓ.card n - 1) * det (updateRow M j u) :=
  MultilinearMap.map_update_smul _ M j s u

theorem det_update_column_smul' (M : Matrix n n R) (j : n) (s : R) (u : n → R) :
    det (updateColumn (s • M) j u) = s ^ (Fintypeₓ.card n - 1) * det (updateColumn M j u) := by
...
theorem det_eq_of_forall_row_eq_smul_add_const_aux {A B : Matrix n n R} {s : Finsetₓ n} :
  ...
  · intro i s hi ih B c hs k hk A_eq
    ...
    · exact mt (fun h => show k ∈ insert i s from h ▸ Finsetₓ.mem_insert_self _ _) hk
    ...
    · exact fun h => hk (Finsetₓ.mem_insert_of_mem h)
    ...
    · intro i' j'
      ...
      rw [A_eq, update_row_ne fun h : k = i => hk <| h ▸ Finsetₓ.mem_insert_self k s]
  ...
  det_eq_of_forall_row_eq_smul_add_const_aux c
    (fun i =>
      not_imp_comm.mp fun hi =>
        Finsetₓ.mem_erase.mpr ⟨mt (fun h : i = k => show c i = 0 from h.symm ▸ hk) hi, Finsetₓ.mem_univ i⟩)
    k (Finsetₓ.not_mem_erase k Finsetₓ.univ) A_eq
...
@[simp]
theorem det_block_diagonal {o : Type _} [Fintypeₓ o] [DecidableEq o] (M : o → Matrix n n R) :
    (blockDiagonalₓ M).det = ∏ k, (M k).det := by
  -- Rewrite the determinants as a sum over permutations.
  simp_rw [det_apply']
  -- The right hand side is a product of sums, rewrite it as a sum of products.
  rw [Finsetₓ.prod_sum]
  simp_rw [Finsetₓ.mem_univ, Finsetₓ.prod_attach_univ, Finsetₓ.univ_pi_univ]
  -- We claim that the only permutations contributing to the sum are those that
  -- preserve their second component.
  let preserving_snd : Finsetₓ (Equivₓ.Perm (n × o)) := finset.univ.filter fun σ => ∀ x, (σ x).snd = x.snd
  have mem_preserving_snd : ∀ {σ : Equivₓ.Perm (n × o)}, σ ∈ preserving_snd ↔ ∀ x, (σ x).snd = x.snd := fun σ =>
    finset.mem_filter.trans ⟨fun h => h.2, fun h => ⟨Finsetₓ.mem_univ _, h⟩⟩
  rw [← Finsetₓ.sum_subset (Finsetₓ.subset_univ preserving_snd) _]
  -- And that these are in bijection with `o → equiv.perm m`.
  rw [(Finsetₓ.sum_bij
        (fun (σ : ∀ k : o, k ∈ Finsetₓ.univ → Equivₓ.Perm n) _ => prod_congr_left fun k => σ k (Finsetₓ.mem_univ k)) _ _
        _ _).symm]
  · intro σ _
  ...
  · intro σ _
    rw [Finsetₓ.prod_mul_distrib, ← Finsetₓ.univ_product_univ, Finsetₓ.prod_product_right]
  ...
  · intro σ σ' _ _ eq
    ...
    have :
      ∀ k x,
        prod_congr_left (fun k => σ k (Finsetₓ.mem_univ _)) (k, x) =
          prod_congr_left (fun k => σ' k (Finsetₓ.mem_univ _)) (k, x) :=
      fun k x => by rw [Eq]
  ...
  · intro σ _ hσ
    ...
    rw [Finsetₓ.prod_eq_zero (Finsetₓ.mem_univ (k, x)), mul_zero]
...
theorem det_from_blocks_zero₂₁ (A : Matrix m m R) (B : Matrix m n R) (D : Matrix n n R) :
  ...
  · intro σ hσ hσn
    ...
    · rw [Finsetₓ.prod_eq_zero (Finsetₓ.mem_univ (Sum.inl a)), mul_zero]
...
theorem det_succ_column_zero {n : ℕ} (A : Matrix (Finₓ n.succ) (Finₓ n.succ) R) :
    det A = ∑ i : Finₓ n.succ, -1 ^ (i : ℕ) * A i 0 * det (A.submatrix i.succAbove Finₓ.succ) := by
  rw [Matrix.det_apply, Finsetₓ.univ_perm_fin_succ, ← Finsetₓ.univ_product_univ]
  simp only [Finsetₓ.sum_map, Equivₓ.to_embedding_apply, Finsetₓ.sum_product, Matrix.submatrix]
  refine' Finsetₓ.sum_congr rfl fun i _ => Finₓ.cases _ (fun i => _) i
  · simp only [Finₓ.prod_univ_succ, Matrix.det_apply, Finsetₓ.mul_sum, Equivₓ.Perm.decompose_fin_symm_apply_zero,
...
theorem det_succ_row_zero {n : ℕ} (A : Matrix (Finₓ n.succ) (Finₓ n.succ) R) :
    det A = ∑ j : Finₓ n.succ, -1 ^ (j : ℕ) * A 0 j * det (A.submatrix Finₓ.succ j.succAbove) := by
...
theorem det_succ_row {n : ℕ} (A : Matrix (Finₓ n.succ) (Finₓ n.succ) R) (i : Finₓ n.succ) :
    det A = ∑ j : Finₓ n.succ, -1 ^ (i + j : ℕ) * A i j * det (A.submatrix i.succAbove j.succAbove) := by
...
theorem det_succ_column {n : ℕ} (A : Matrix (Finₓ n.succ) (Finₓ n.succ) R) (j : Finₓ n.succ) :
    det A = ∑ i : Finₓ n.succ, -1 ^ (i + j : ℕ) * A i j * det (A.submatrix i.succAbove j.succAbove) := by
  rw [← det_transpose, det_succ_row _ j]
  refine' Finsetₓ.sum_congr rfl fun i _ => _
  rw [add_commₓ, ← det_transpose, transpose_apply, transpose_submatrix, transpose_transpose]
