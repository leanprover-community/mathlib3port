    ...
    :=
      by
        have card_pos : 0 < t.card := lt_transₓ ( Nat.succ_posₓ _ ) h
          ...
          · simp only [ t' , card_map , Finset.card_erase_of_mem m ] exact nat.lt_pred_iff.mpr h
...
theorem finrank_quotient_le [FiniteDimensional K V] (s : Submodule K V) : finrank K (V ⧸ s) ≤ finrank K V :=
  by 
    rw [←s.finrank_quotient_add_finrank]
...
theorem span_eq_top_of_linear_independent_of_card_eq_finrank {ι : Type _} [hι : Nonempty ι] [Fintype ι] {b : ι → V}
  ...
  by 
    byCases' fin : FiniteDimensional K V
    ·
      have  := Finₓ 
      byContra ne_top 
...
theorem exists_ker_pow_eq_ker_pow_succ [FiniteDimensional K V] (f : End K V) :
  ...
  by 
    classical 
    byContra h_contra 
    simpRw [not_exists, not_and]  at h_contra 
    ...
    have h_any_n_lt : ∀ n, n ≤ (finrank K V).succ → n ≤ finrank K V :=
      fun n hn => (h_le_ker_pow n hn).trans (h_le_finrank_V n)
    show False 
