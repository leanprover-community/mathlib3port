...
def GradedAlgebra.liftι : ExteriorAlgebra R M →ₐ[R] ⨁ i : ℕ, ↥((ι R).range ^ i : Submodule R (ExteriorAlgebra R M)) :=
  lift R ⟨GradedAlgebra.ι R M, GradedAlgebra.ι_sq_zero R M⟩

variable {R M}

theorem GradedAlgebra.lift_ι_eq (i : ℕ) (x : ((ι R).range ^ i : Submodule R (ExteriorAlgebra R M))) :
    GradedAlgebra.liftι R M x =
      DirectSum.of (fun i => (((ι R).range ^ i : Submodule R (ExteriorAlgebra R M)) : Type _)) i x :=
  by
  cases' x with x hx
  ...
  · obtain ⟨_, rfl⟩ := hm
    rw [AlgHom.map_mul, ih, graded_algebra.lift_ι, lift_ι_apply, graded_algebra.ι_apply, DirectSum.of_mul_of]
...
instance gradedAlgebra :
  ...
  GradedAlgebra.ofAlgHom _ (GradedAlgebra.liftι R M)
