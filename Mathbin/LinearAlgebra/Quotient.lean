...
instance AddCommGroupₓ : AddCommGroupₓ (M ⧸ p) where
  zero := (0 : M ⧸ p)
  add := · + ·
  neg := Neg.neg
  sub := Sub.sub
...
def comap_mkq.rel_iso : Submodule R (M ⧸ p) ≃o { p' : Submodule R M // p ≤ p' } where
...
def mapq_linear : compatible_maps p q →ₗ[R] M ⧸ p →ₗ[R] M₂ ⧸ q where
