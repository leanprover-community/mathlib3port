...
theorem sum_eapprox_diff (f : α → ℝ≥0∞) (n : ℕ) (a : α) :
  ...
  · rw [Finset.sum_range_succ, Nat.succ_eq_add_one, IH, eapprox_diff, coe_map, Function.comp_app, coe_sub, Pi.sub_apply,
      Ennreal.coe_to_nnreal, add_tsub_cancel_of_le (monotone_eapprox f (Nat.le_succ _) _)]
...
/- ./././Mathport/Syntax/Translate/Basic.lean:572:2: warning: expanding binder collection (y «expr ≠ » 0) -/
...
theorem exists_pos_set_lintegral_lt_of_measure_lt {f : α → ℝ≥0∞} (h : (∫⁻ x, f x ∂μ) ≠ ∞) {ε : ℝ≥0∞} (hε : ε ≠ 0) :
  ...
  calc
    (map coe ψ).lintegral (μ.restrict s) ≤
      ...
      simp only [add_tsub_eq_max, le_max_right, coe_map, Function.comp_app, simple_func.coe_add, simple_func.coe_sub,
