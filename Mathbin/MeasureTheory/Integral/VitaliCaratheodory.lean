...
theorem simple_func.exists_le_lower_semicontinuous_lintegral_ge (f : α →ₛ  ℝ≥0 ) {ε : ℝ≥0∞} (ε0 : ε ≠ 0) :
  ...
  by 
    ...
    ·
      ...
      ·
        refine'
          ⟨fun x => c, fun x => _, lower_semicontinuous_const,
            by 
      ...
      ·
        ...
        simpa only [hs, hc, lt_top_iff_ne_top, true_andₓ, simple_func.coe_const, Function.const_applyₓ, lintegral_const,
          ...
          simple_func.coe_zero, Set.piecewise_eq_indicator, simple_func.coe_piecewise, false_andₓ, restrict_apply] using
          h 
      ...
      ·
        ...
        calc ((c : ℝ≥0∞)*μ u) ≤ c*μ s+ε / c := Ennreal.mul_le_mul (le_reflₓ _) μu.le _ = (c*μ s)+ε :=
          by 
            simpRw [mul_addₓ]
    ...
    ·
      ...
      convLHS => rw [←Ennreal.add_halves ε]
      abel
...
theorem exists_le_lower_semicontinuous_lintegral_ge (f : α → ℝ≥0∞) (hf : Measurable f) {ε : ℝ≥0∞} (εpos : ε ≠ 0) :
  ...
  by 
    ...
    ·
      calc (∫⁻x, ∑'n : ℕ, g n x ∂μ) = ∑'n, ∫⁻x, g n x ∂μ :=
        by 
          rw
            [lintegral_tsum
              fun n => (gcont n).Measurable.coe_nnreal_ennreal]_ ≤ ∑'n, (∫⁻x, eapprox_diff f n x ∂μ)+δ n :=
        Ennreal.tsum_le_tsum hg _ = (∑'n, ∫⁻x, eapprox_diff f n x ∂μ)+∑'n, δ n :=
        Ennreal.tsum_add _ ≤ (∫⁻x : α, f x ∂μ)+ε :=
        by 
          refine' add_le_add _ hδ.le 
          rw [←lintegral_tsum]
          ·
            simpRw [tsum_eapprox_diff f hf, le_reflₓ]
          ·
            intro n 
...
theorem exists_lt_lower_semicontinuous_lintegral_ge [sigma_finite μ] (f : α →  ℝ≥0 ) (fmeas : Measurable f) {ε : ℝ≥0∞}
  (ε0 : ε ≠ 0) : ∃ g : α → ℝ≥0∞, (∀ x, (f x : ℝ≥0∞) < g x) ∧ LowerSemicontinuous g ∧ (∫⁻x, g x ∂μ) ≤ (∫⁻x, f x ∂μ)+ε :=
  by 
    ...
    ·
      calc (f x : ℝ≥0∞) < f' x :=
        by 
          simpa [←Ennreal.coe_lt_coe] using add_lt_add_left (wpos x) (f x)_ ≤ g x :=
        le_g x
    ·
      calc (∫⁻x : α, g x ∂μ) ≤ (∫⁻x : α, f x+w x ∂μ)+ε / 2 := gint _ = ((∫⁻x : α, f x ∂μ)+∫⁻x : α, w x ∂μ)+ε / 2 :=
        by 
          rw [lintegral_add fmeas.coe_nnreal_ennreal wmeas.coe_nnreal_ennreal]_ ≤ ((∫⁻x : α, f x ∂μ)+ε / 2)+ε / 2 :=
        add_le_add_right (add_le_add_left wint.le _) _ _ = (∫⁻x : α, f x ∂μ)+ε :=
        by 
...
theorem exists_lt_lower_semicontinuous_lintegral_ge_of_ae_measurable [sigma_finite μ] (f : α →  ℝ≥0 )
  ...
  by 
    ...
    rcases
      exists_le_lower_semicontinuous_lintegral_ge μ (s.indicator fun x => ∞) (measurable_const.indicator smeas)
        this with
    ...
    ·
      calc (∫⁻x, g0 x+g1 x ∂μ) = (∫⁻x, g0 x ∂μ)+∫⁻x, g1 x ∂μ :=
        lintegral_add g0_cont.measurable g1_cont.measurable _ ≤ ((∫⁻x, f x ∂μ)+ε / 2)+0+ε / 2 :=
        by 
  ...
  ∃ g : α → ℝ≥0∞,
    (∀ x, (f x : ℝ≥0∞) < g x) ∧
      LowerSemicontinuous g ∧
        (∀ᵐx ∂μ, g x < ⊤) ∧ integrable (fun x => (g x).toReal) μ ∧ (∫x, (g x).toReal ∂μ) < (∫x, f x ∂μ)+ε :=
  by 
    ...
    have gint_ne : (∫⁻x : α, g x ∂μ) ≠ ∞ :=
      ne_top_of_le_ne_top
        (by 
          simpa)
        gint 
    have g_lt_top : ∀ᵐx : α ∂μ, g x < ∞ := ae_lt_top gcont.measurable gint_ne 
    have Ig : (∫⁻a : α, Ennreal.ofReal (g a).toReal ∂μ) = ∫⁻a : α, g a ∂μ
    ·
      apply lintegral_congr_ae 
      filterUpwards [g_lt_top]
      intro x hx 
    ...
    ·
      ...
      ·
        calc Ennreal.toReal (∫⁻a : α, Ennreal.ofReal (g a).toReal ∂μ) = Ennreal.toReal (∫⁻a : α, g a ∂μ) :=
          by 
            congr 1_ ≤ Ennreal.toReal ((∫⁻a : α, f a ∂μ)+δ) :=
          by 
            apply Ennreal.to_real_mono _ gint 
            simpa using int_f_ne_top _ = Ennreal.toReal (∫⁻a : α, f a ∂μ)+δ :=
          by 
            rw [Ennreal.to_real_add int_f_ne_top Ennreal.coe_ne_top,
              Ennreal.coe_to_real]_ < Ennreal.toReal (∫⁻a : α, f a ∂μ)+ε :=
          add_lt_add_left hδε _ _ = (∫⁻a : α, Ennreal.ofReal («expr↑ » (f a)) ∂μ).toReal+ε :=
          by 
            simp 
      ·
...
theorem simple_func.exists_upper_semicontinuous_le_lintegral_le (f : α →ₛ  ℝ≥0 ) (int_f : (∫⁻x, f x ∂μ) ≠ ∞) {ε : ℝ≥0∞}
  (ε0 : ε ≠ 0) : ∃ g : α →  ℝ≥0 , (∀ x, g x ≤ f x) ∧ UpperSemicontinuous g ∧ (∫⁻x, f x ∂μ) ≤ (∫⁻x, g x ∂μ)+ε :=
  by 
    induction' f using MeasureTheory.SimpleFunc.induction with c s hs f₁ f₂ H h₁ h₂ generalizing ε
    ·
      let f := simple_func.piecewise s hs (simple_func.const α c) (simple_func.const α 0)
      ...
      ·
        ...
        ·
          simp only [hc, Set.indicator_zero', Pi.zero_apply, simple_func.const_zero, implies_true_iff, eq_self_iff_true,
            simple_func.coe_zero, Set.piecewise_eq_indicator, simple_func.coe_piecewise, le_zero_iff]
        ·
          simp only [hc, Set.indicator_zero', lintegral_const, zero_mul, Pi.zero_apply, simple_func.const_zero,
            zero_addₓ, zero_le', simple_func.coe_zero, Set.piecewise_eq_indicator, Ennreal.coe_zero,
            simple_func.coe_piecewise, zero_le]
      have μs_lt_top : μ s < ∞
      ·
        simpa only [hs, hc, lt_top_iff_ne_top, true_andₓ, simple_func.coe_const, or_falseₓ, lintegral_const,
          ...
          WithTop.mul_eq_top_iff, simple_func.const_zero, Function.const_applyₓ, lintegral_indicator,
          Ennreal.coe_eq_zero, Ne.def, not_false_iff, simple_func.coe_zero, Set.piecewise_eq_indicator,
          simple_func.coe_piecewise, false_andₓ] using int_f 
      have  : (0 : ℝ≥0∞) < ε / c := Ennreal.div_pos_iff.2 ⟨ε0, Ennreal.coe_ne_top⟩
      ...
      ·
        ...
        ·
          simpa only [hs, F_closed.measurable_set, simple_func.coe_const, Function.const_applyₓ, lintegral_const,
            Ennreal.coe_indicator, Set.univ_inter, MeasurableSet.univ, simple_func.const_zero, lintegral_indicator,
            simple_func.coe_zero, Set.piecewise_eq_indicator, simple_func.coe_piecewise, restrict_apply]
        calc ((c : ℝ≥0∞)*μ s) ≤ c*μ F+ε / c := Ennreal.mul_le_mul (le_reflₓ _) μF.le _ = (c*μ F)+ε :=
          by 
            simpRw [mul_addₓ]
            rw [Ennreal.mul_div_cancel' _ Ennreal.coe_ne_top]
            simpa using hc
  ...
  by 
    obtain ⟨fs, fs_le_f, int_fs⟩ : ∃ fs : α →ₛ  ℝ≥0 , (∀ x, fs x ≤ f x) ∧ (∫⁻x, f x ∂μ) ≤ (∫⁻x, fs x ∂μ)+ε / 2 :=
      by 
        ...
        convRHS at this => rw [lintegral_eq_nnreal (fun x => (f x : ℝ≥0∞)) μ]
    ...
    calc (∫⁻x, f x ∂μ) ≤ (∫⁻x, fs x ∂μ)+ε / 2 := int_fs _ ≤ ((∫⁻x, g x ∂μ)+ε / 2)+ε / 2 :=
      add_le_add gint (le_reflₓ _)_ = (∫⁻x, g x ∂μ)+ε :=
      by 
        rw [add_assocₓ, Ennreal.add_halves]
...
theorem exists_lt_lower_semicontinuous_integral_lt [sigma_finite μ] (f : α → ℝ) (hf : integrable f μ) {ε : ℝ}
  ...
  ∃ g : α → Ereal,
    (∀ x, (f x : Ereal) < g x) ∧
      LowerSemicontinuous g ∧
        integrable (fun x => Ereal.toReal (g x)) μ ∧ (∀ᵐx ∂μ, g x < ⊤) ∧ (∫x, Ereal.toReal (g x) ∂μ) < (∫x, f x ∂μ)+ε :=
  by 
    ...
    have ae_g : ∀ᵐx ∂μ, (g x).toReal = (gp x : Ereal).toReal - (gm x : Ereal).toReal
    ·
      filterUpwards [gp_lt_top]
      intro x hx 
    ...
    exact
      calc (∫x : α, (g x).toReal ∂μ) = ∫x : α, Ereal.toReal (gp x) - Ereal.toReal (gm x) ∂μ := integral_congr_ae ae_g 
        ...
        _ < ((∫x : α, «expr↑ » (fp x) ∂μ)+«expr↑ » δ) - ∫x : α, gm x ∂μ :=
        by 
          apply sub_lt_sub_right 
          convert gpint 
          simp only [Ereal.to_real_coe_ennreal]
        _ ≤ ((∫x : α, «expr↑ » (fp x) ∂μ)+«expr↑ » δ) - ((∫x : α, fm x ∂μ) - δ) := sub_le_sub_left gmint _ 
        _ = (∫x : α, f x ∂μ)+2*δ :=
        by 
          simpRw [integral_eq_integral_pos_part_sub_integral_neg_part hf, fp, fm]
          ring 
        _ = (∫x : α, f x ∂μ)+ε :=
        by 
          congr 1
          fieldSimp [δ, mul_commₓ]
    ...
    ·
      ...
      ·
        exact
          continuous_coe_ennreal_ereal.comp_lower_semicontinuous gpcont
            fun x y hxy => Ereal.coe_ennreal_le_coe_ennreal_iff.2 hxy
      ·
        apply ereal.continuous_neg.comp_upper_semicontinuous_antitone _ fun x y hxy => Ereal.neg_le_neg_iff.2 hxy 
        dsimp 
        apply
          continuous_coe_ennreal_ereal.comp_upper_semicontinuous _
            fun x y hxy => Ereal.coe_ennreal_le_coe_ennreal_iff.2 hxy 
