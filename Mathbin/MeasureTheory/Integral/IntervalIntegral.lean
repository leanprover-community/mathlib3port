...
theorem ae_interval_oc_iff' :
  (∀ᵐ x ∂μ, x ∈ Ι a b → P x) ↔ (∀ᵐ x ∂μ.restrict$ Ioc a b, P x) ∧ ∀ᵐ x ∂μ.restrict$ Ioc b a, P x :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » exprΙ() a b)
...
theorem integral_congr_ae' {f g : α → E} (h : ∀ᵐ x ∂μ, x ∈ Ioc a b → f x = g x)
  (h' : ∀ᵐ x ∂μ, x ∈ Ioc b a → f x = g x) : (∫ x : α in a..b, f x ∂μ) = ∫ x : α in a..b, g x ∂μ :=
  by 
...
theorem continuous_at_of_dominated_interval {F : X → α → E} {x₀ : X} {bound : α → ℝ} {a b : α}
  ...
  (h_cont : ∀ᵐ t ∂μ, t ∈ Ι a b → ContinuousAt (fun x => F x t) x₀) : ContinuousAt (fun x => ∫ t in a..b, F x t ∂μ) x₀ :=
  tendsto_integral_filter_of_dominated_convergence bound hF_meas h_bound bound_integrable h_cont
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » Icc b₁ b₂)
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
theorem
  continuous_within_at_primitive
  { f : α → E } { a b₀ b₁ b₂ : α } ( hb₀ : μ { b₀ } = 0 ) ( h_int : IntervalIntegrable f μ min a b₁ max a b₂ )
          ...
          have h₁₂ : b₁ ≤ b₂ := h₀ . 1 . trans h₀ . 2
            ...
            ·
              rintro x ⟨ h₁ , h₂ ⟩
                ...
                ·
                  exact
                    ⟨
                      min_le_of_left_le min_le_rightₓ a b₁ , h₁.trans h₂.trans $ le_max_of_le_right $ le_max_rightₓ _ _
                      ⟩
                ·
                  exact
                    ⟨
                      min_le_of_left_le $ min_le_rightₓ _ _ . trans h₁
                        ,
                        le_max_of_le_right $ h₂.trans $ le_max_rightₓ _ _
                      ⟩
            have : ∀ b _ : b ∈ Icc b₁ b₂ , ∫ x in a .. b , f x ∂ μ = ∫ x in a .. b₁ , f x ∂ μ + ∫ x in b₁ .. b , f x ∂ μ
            ·
              rintro b ⟨ h₁ , h₂ ⟩
                ...
                ·
                  exact
                    ⟨ min_le_of_left_le min_le_rightₓ _ _ , le_max_of_le_right h₁.trans $ h₂.trans le_max_rightₓ a b₂ ⟩
            apply ContinuousWithinAt.congr _ this this _ h₀
            clear this
            refine' continuous_within_at_const.add _
            have
              :
                fun b => ∫ x in b₁ .. b , f x ∂ μ
                  =ᶠ[
                  𝓝[ Icc b₁ b₂ ] b₀
                  ]
                  fun b => ∫ x in b₁ .. b₂ , indicator { x | x ≤ b } f x ∂ μ
            · apply eventually_eq_of_mem self_mem_nhds_within exact fun b b_in => integral_indicator b_in . symm
            apply ContinuousWithinAt.congr_of_eventually_eq _ this integral_indicator h₀ . symm
            have : IntervalIntegrable fun x => ∥ f x ∥ μ b₁ b₂
            exact IntervalIntegrable.norm h_int' $ right_mem_Icc.mpr h₁₂
            ...
            ·
              apply eventually.mono self_mem_nhds_within
                ...
                · rw [ min₁₂ ] exact h_int' hx . 1 . AeMeasurable
            ...
            ·
              have : ∀ᵐ t ∂ μ , t < b₀ ∨ b₀ < t
                · apply eventually.mono compl_mem_ae_iff.mpr hb₀ intro x hx exact Ne.lt_or_lt hx
                apply this.mono
                rintro x₀ ( hx₀ | hx₀ ) -
                ·
                  have : ∀ᶠ x in 𝓝[ Icc b₁ b₂ ] b₀ , { t : α | t ≤ x } . indicator f x₀ = f x₀
                    · apply mem_nhds_within_of_mem_nhds apply eventually.mono Ioi_mem_nhds hx₀ intro x hx simp [ hx.le ]
                    apply continuous_within_at_const.congr_of_eventually_eq this
                    simp [ hx₀.le ]
                ·
                  have : ∀ᶠ x in 𝓝[ Icc b₁ b₂ ] b₀ , { t : α | t ≤ x } . indicator f x₀ = 0
                    · apply mem_nhds_within_of_mem_nhds apply eventually.mono Iio_mem_nhds hx₀ intro x hx simp [ hx ]
                    apply continuous_within_at_const.congr_of_eventually_eq this
                    simp [ hx₀ ]
        · apply continuous_within_at_of_not_mem_closure rwa [ closure_Icc ]
...
theorem continuous_on_primitive_interval' {f : α → E} {a b₁ b₂ : α} [has_no_atoms μ]
  ...
  (ha : a ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
instance nhds_interval {x a b : β}
  [h : Fact (x ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")] :
  FTC_filter x (𝓝["././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"] x)
    (𝓝["././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"] x) :=
  by 
...
theorem integral_has_strict_fderiv_at (hf : IntervalIntegrable f volume a b) (hmeas_a : MeasurableAtFilter f (𝓝 a))
  ...
  HasStrictFderivAt (fun p : ℝ × ℝ => ∫ x in p.1 ..p.2, f x) ((snd ℝ ℝ ℝ).smulRight (f b) - (fst ℝ ℝ ℝ).smulRight (f a))
    (a, b) :=
...
theorem integral_has_fderiv_within_at (hf : IntervalIntegrable f volume a b) (hmeas_a : MeasurableAtFilter f la)
  ...
  HasFderivWithinAt (fun p : ℝ × ℝ => ∫ x in p.1 ..p.2, f x) ((snd ℝ ℝ ℝ).smulRight (f b) - (fst ℝ ℝ ℝ).smulRight (f a))
    (s.prod t) (a, b) :=
...
/--
  ...
  theorem
    ...
    ( hab : a ≤ b )
        ...
        ( hderiv : ∀ x _ : x ∈ Ico a b , HasDerivWithinAt g g' x Ioi x x )
    ...
    :=
      by
        refine' le_of_forall_pos_le_add fun ε εpos => _
          rcases
            exists_lt_lower_semicontinuous_integral_lt g' g'int εpos
            with ⟨ G' , g'_lt_G' , G'cont , G'int , G'lt_top , hG' ⟩
          ...
          ·
            apply
              ...
              have I1 : ∀ᶠ u in 𝓝[ Ioi t ] t , u - t * y ≤ ∫ w in t .. u , G' w . toReal
              ·
                have B : ∀ᶠ u in 𝓝 t , ( y : Ereal ) < G' u := G'cont.lower_semicontinuous_at _ _ y_lt_G'
                  rcases mem_nhds_iff_exists_Ioo_subset . 1 B with ⟨ m , M , ⟨ hm , hM ⟩ , H ⟩
                  have
                    : Ioo t min M b ∈ 𝓝[ Ioi t ] t
                      :=
                      mem_nhds_within_Ioi_iff_exists_Ioo_subset . 2
                        ⟨
                          min M b
                            ,
                            by simp only [ hM , ht.right.right , lt_min_iff , mem_Ioi , and_selfₓ ]
                            ,
                            subset.refl _
                          ⟩
                  filterUpwards [ this ]
                        ...
                        by
                          rw [ intervalIntegral.integral_of_le hu . 1 . le , ← integral_Icc_eq_integral_Ioc ]
                            ...
                            ·
                              have
                                  C1
                                    : ∀ᵐ x : ℝ ∂ volume.restrict Icc t u , G' x < ⊤
                                    :=
                                    ae_mono measure.restrict_mono I le_reflₓ _ G'lt_top
                                have
                                  C2
                                    : ∀ᵐ x : ℝ ∂ volume.restrict Icc t u , x ∈ Icc t u
                                    :=
                                ...
                                ·
                                  simp
                                    ...
                                    [
                                      hm.trans_le hx.left
                                        ,
                                        hx.right.trans_lt hu.right . trans_le min_le_leftₓ M b
                                        ,
                                        mem_Ioo
                                        ,
                                        and_selfₓ
              ...
              have I3 : ∀ᶠ u in 𝓝[ Ioi t ] t , g u - g t ≤ ∫ w in t .. u , G' w . toReal
              · filterUpwards [ I1 , I2 ] intro u hu1 hu2 exact hu2.trans hu1
                      ...
                      apply integral_add_adjacent_intervals
                        ·
                          rw [ interval_integrable_iff_integrable_Ioc_of_le ht . 2 . 1 ]
                            exact
                              integrable_on.mono_set
                                G'int Ioc_subset_Icc_self.trans Icc_subset_Icc le_rfl ht . 2 . 2 . le
                        ·
                          rw [ interval_integrable_iff_integrable_Ioc_of_le h'x . 1 . le ]
                            apply integrable_on.mono_set G'int
                            refine'
                              Ioc_subset_Icc_self.trans Icc_subset_Icc ht . 2 . 1 h'x . 2 . trans min_le_rightₓ _ _
          calc
...
/--
  ...
  theorem
    ...
    ( hab : a ≤ b )
        ( hcont : ContinuousOn g Icc a b )
        ( hderiv : ∀ x _ : x ∈ Ioo a b , HasDerivWithinAt g g' x Ioi x x )
        ( g'int : integrable_on g' Icc a b )
      : ∫ y in a .. b , g' y = g b - g a
    :=
      by
          ...
          ·
            apply s_closed.closure_subset_iff . 2
              ...
              exact
                integral_eq_sub_of_has_deriv_right_of_le_real
                  ht . 2
                    hcont.mono Icc_subset_Icc ht . 1 . le le_reflₓ _
                    fun x hx => hderiv x ⟨ ht . 1 . trans_le hx . 1 , hx . 2 ⟩
                    g'int.mono_set Icc_subset_Icc ht . 1 . le le_reflₓ _
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » Ioo (min a b) (max a b))
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » interval a b)
...
theorem integral_comp_smul_deriv'' {f f' : ℝ → ℝ} {g : ℝ → E}
  ...
  (hff' : ∀ x _ : x ∈ Ioo (min a b) (max a b), HasDerivWithinAt f (f' x) (Ioi x) x)
  (hf' : ContinuousOn f' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")
  (hg : ContinuousOn g (f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")) :
  ...
  by 
    ...
    have h_der :
      ∀ x _ : x ∈ Ioo (min a b) (max a b), HasDerivWithinAt (fun u => ∫ t in f a..f u, g t) (f' x • (g ∘ f) x) (Ioi x) x
    ·
      intro x hx 
      let I := "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"
      have hI : f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)" = I :=
        hf.image_interval 
      have h2x : f x ∈ I
      ·
        rw [←hI]
    ...
    simpRw [integral_eq_sub_of_has_deriv_right h_cont h_der h_int, integral_same, sub_zero]
...
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » Ioo (min (f a) (f b)) (max (f a) (f b)))
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
theorem integral_deriv_comp_smul_deriv' {f f' : ℝ → ℝ} {g g' : ℝ → E}
  (hf : ContinuousOn f "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")
  (hff' : ∀ x _ : x ∈ Ioo (min a b) (max a b), HasDerivWithinAt f (f' x) (Ioi x) x)
  (hf' : ContinuousOn f' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")
  (hg : ContinuousOn g "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")
  (hgg' : ∀ x _ : x ∈ Ioo (min (f a) (f b)) (max (f a) (f b)), HasDerivWithinAt g (g' x) (Ioi x) x)
  (hg' : ContinuousOn g' (f '' "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)")) :
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » interval a b)
theorem integral_deriv_comp_smul_deriv {f f' : ℝ → ℝ} {g g' : ℝ → E}
  (hf : ∀ x _ : x ∈ interval a b, HasDerivAt f (f' x) x) (hg : ∀ x _ : x ∈ interval a b, HasDerivAt g (g' (f x)) (f x))
  (hf' : ContinuousOn f' (interval a b)) (hg' : Continuous g') :
  (∫ x in a..b, f' x • (g' ∘ f) x) = (g ∘ f) b - (g ∘ f) a :=
