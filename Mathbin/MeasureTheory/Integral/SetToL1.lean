...
theorem set_to_simple_func_add_left {m : MeasurableSpace α} (T T' : Set α → F →L[ℝ] F')
    {f : α →ₛ F} : setToSimpleFunc (T + T') f = setToSimpleFunc T f + setToSimpleFunc T' f := by
...
theorem set_to_simple_func_add_left' (T T' T'' : Set α → E →L[ℝ] F)
  ...
  intro x hx
  refine'
    h_add (f ⁻¹' {x}) (measurableSetPreimage _ _) (measure_preimage_lt_top_of_integrable _ hf _)
  rw [mem_filter] at hx
  exact hx.2
#align
  measure_theory.simple_func.set_to_simple_func_add_left' MeasureTheory.SimpleFunc.set_to_simple_func_add_left'
...
theorem set_to_simple_func_smul_left' (T T' : Set α → E →L[ℝ] F') (c : ℝ)
  ...
  intro x hx
  refine'
    h_smul (f ⁻¹' {x}) (measurableSetPreimage _ _) (measure_preimage_lt_top_of_integrable _ hf _)
  rw [mem_filter] at hx
  exact hx.2
#align
  measure_theory.simple_func.set_to_simple_func_smul_left' MeasureTheory.SimpleFunc.set_to_simple_func_smul_left'
...
theorem set_to_L1s_congr_measure {μ' : Measure α} (T : Set α → E →L[ℝ] F)
    ...
    (f : α →₁ₛ[μ] E) (f' : α →₁ₛ[μ'] E) (h : f =ᵐ[μ] f') : setToL1s T f = setToL1s T f' := by
...
theorem norm_set_to_fun_le (hT : DominatedFinMeasAdditive μ T C) (hf : Integrable f μ)
    (hC : 0 ≤ C) : ‖setToFun μ T hT f‖ ≤ C * ‖hf.toL1 f‖ := by
  rw [set_to_fun_eq hT hf]
