...
theorem ofSmulMeasure (c : ℝ≥0∞) (hc_ne_top : c ≠ ∞) (hT : FinMeasAdditive (c • μ) T) :
    FinMeasAdditive μ T :=
  by
  refine' ofEqTopImpEqTop (fun s hs hμs => _) hT
...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (i j «expr ∈ » sι) -/
theorem map_unionᵢ_fin_meas_set_eq_sum (T : Set α → β) (T_empty : T ∅ = 0)
  ...
  ·
    simp only [Finset.not_mem_empty, IsEmpty.forall_iff, unionᵢ_false, unionᵢ_empty, sum_empty,
  ...
  rw [←
    h_add (S a) (⋃ i ∈ s, S i) (hS_meas a) (measurableSet_bunionᵢ _ fun i _ => hS_meas i)
...
theorem zero {m : MeasurableSpace α} (μ : Measure α) (hC : 0 ≤ C) :
  ...
  by
  refine' ⟨FinMeasAdditive.zero, fun s hs hμs => _⟩
...
theorem setToSimpleFunc_zero_apply {m : MeasurableSpace α} (T : Set α → F →L[ℝ] F') :
    setToSimpleFunc T (0 : α →ₛ F) = 0 := by cases isEmpty_or_nonempty α <;> simp [setToSimpleFunc]
...
theorem map_setToSimpleFunc (T : Set α → F →L[ℝ] F') (h_add : FinMeasAdditive μ T) {f : α →ₛ G}
  ...
  · simp only [[anonymous], sum_apply, ContinuousLinearMap.coe_sum']
...
theorem setToSimpleFunc_congr' (T : Set α → E →L[ℝ] F) (h_add : FinMeasAdditive μ T) {f g : α →ₛ E}
  ...
  show ((pair f g).map Prod.fst).setToSimpleFunc T = ((pair f g).map Prod.snd).setToSimpleFunc T
    by
    have h_pair : Integrable (f.pair g) μ := integrablePair hf hg
...
theorem setToSimpleFunc_add (T : Set α → E →L[ℝ] F) (h_add : FinMeasAdditive μ T) {f g : α →ₛ E}
  ...
  calc
    setToSimpleFunc T (f + g) = ∑ x in (pair f g).range, T (pair f g ⁻¹' {x}) (x.fst + x.snd) :=
      by
      rw [add_eq_map₂, map_setToSimpleFunc T h_add hp_pair]
      simp
...
theorem setToSimpleFunc_smul_real (T : Set α → E →L[ℝ] F) (h_add : FinMeasAdditive μ T) (c : ℝ)
  ...
  calc
    setToSimpleFunc T (c • f) = ∑ x in f.range, T (f ⁻¹' {x}) (c • x) :=
      by
      rw [smul_eq_map c f, map_setToSimpleFunc T h_add hf]
      rw [smul_zero]
...
theorem setToL1s_congr_measure {μ' : Measure α} (T : Set α → E →L[ℝ] F)
  ...
  refine' (toSimpleFunc_eq_to_fun f).trans _
  suffices : f' =ᵐ[μ] ⇑(simpleFunc.toSimpleFunc f')
  exact h.trans this
  have goal' : f' =ᵐ[μ'] simpleFunc.toSimpleFunc f' := (toSimpleFunc_eq_to_fun f').symm
  exact hμ.ae_eq goal'
...
theorem setToL1s_add (T : Set α → E →L[ℝ] F) (h_zero : ∀ s, MeasurableSet s → μ s = 0 → T s = 0)
  ...
  exact
    SimpleFunc.setToSimpleFunc_congr T h_zero h_add (SimpleFunc.integrable _) (add_toSimpleFunc f g)
...
theorem setToL1s_nonneg (h_zero : ∀ s, MeasurableSet s → μ s = 0 → T s = 0)
  ...
  simp_rw [setToL1s]
  obtain ⟨f', hf', hff'⟩ : ∃ f' : α →ₛ G'', 0 ≤ f' ∧ simpleFunc.toSimpleFunc f =ᵐ[μ] f' :=
    by
    obtain ⟨f'', hf'', hff''⟩ := exists_simpleFunc_nonneg_ae_eq hf
    exact ⟨f'', hf'', (lp.simpleFunc.toSimpleFunc_eq_to_fun f).trans hff''⟩
...
theorem setToL1_congr_left (T T' : Set α → E →L[ℝ] F) {C C' : ℝ}
  ...
  refine' ContinuousLinearMap.extend_unique (setToL1sClm α E μ hT) _ _ _ _ _
  ext1 f
  suffices setToL1 hT' f = setToL1sClm α E μ hT f
    by
    rw [← this]
    congr 1
...
theorem setToL1_add_left (hT : DominatedFinMeasAdditive μ T C)
  ...
  suffices setToL1 hT f + setToL1 hT' f = setToL1sClm α E μ (hT.add hT') f
    by
    rw [← this]
    congr
...
theorem setToL1_add_left' (hT : DominatedFinMeasAdditive μ T C)
  ...
  simp only [ContinuousLinearMap.add_comp, ContinuousLinearMap.coe_comp', Function.comp_apply,
    ContinuousLinearMap.add_apply]
  suffices setToL1 hT f + setToL1 hT' f = setToL1sClm α E μ hT'' f
    by
    rw [← this]
    congr
...
theorem setToL1_smul_left (hT : DominatedFinMeasAdditive μ T C) (c : ℝ) (f : α →₁[μ] E) :
  ...
  suffices c • setToL1 hT f = setToL1sClm α E μ (hT.smul c) f
    by
    rw [← this]
    congr
...
theorem setToL1_smul_left' (hT : DominatedFinMeasAdditive μ T C)
  ...
  simp only [ContinuousLinearMap.coe_comp', Function.comp_apply, ContinuousLinearMap.smul_comp,
    Pi.smul_apply, ContinuousLinearMap.coe_smul']
  suffices c • setToL1 hT f = setToL1sClm α E μ hT' f
    by
    rw [← this]
    congr
...
theorem setToL1_mono_left' {T T' : Set α → E →L[ℝ] G''} {C C' : ℝ}
  ...
  · exact isClosed_le (setToL1 hT).continuous (setToL1 hT').continuous
...
theorem L1.setToFun_eq_setToL1 (hT : DominatedFinMeasAdditive μ T C) (f : α →₁[μ] E) :
  ...
  rw [setToFun_eq hT (L1.integrableCoeFn f), Integrable.toL1_coeFn]
...
theorem setToFun_zero (hT : DominatedFinMeasAdditive μ T C) : setToFun μ T hT (0 : α → E) = 0 :=
  by
  rw [setToFun_eq hT]
  · simp only [Integrable.toL1_zero, ContinuousLinearMap.map_zero]
...
theorem setToFun_zero_left {hT : DominatedFinMeasAdditive μ (0 : Set α → E →L[ℝ] F) C} :
  ...
  · rw [setToFun_eq hT hf]
    exact L1.setToL1_zero_left hT _
...
theorem setToFun_zero_left' (hT : DominatedFinMeasAdditive μ T C)
  ...
  by_cases hf : Integrable f μ
  · rw [setToFun_eq hT hf]
    exact L1.setToL1_zero_left' hT h_zero _
  · exact setToFun_undef hT hf
#align measure_theory.set_to_fun_zero_left' MeasureTheory.setToFun_zero_left'
...
theorem setToFun_finset_sum' (hT : DominatedFinMeasAdditive μ T C) {ι} (s : Finset ι)
  ...
  · intro i s his ih hf
    ...
    · convert integrableFinsetSum s fun i hi => hf i (Finset.mem_insert_of_mem hi)
      ext1 x
      simp
...
theorem setToFun_finset_sum (hT : DominatedFinMeasAdditive μ T C) {ι} (s : Finset ι) {f : ι → α → E}
  ...
  by
  convert setToFun_finset_sum' hT s hf
  ext1 a
  simp
#align measure_theory.set_to_fun_finset_sum MeasureTheory.setToFun_finset_sum
...
theorem setToFun_smul [NontriviallyNormedField 𝕜] [NormedSpace 𝕜 E] [NormedSpace 𝕜 F]
  ...
  by
  by_cases hf : Integrable f μ
  · rw [setToFun_eq hT hf, setToFun_eq hT, Integrable.toL1_smul', L1.setToL1_smul hT h_smul c _]
  · by_cases hr : c = 0
    · rw [hr]
      simp
...
theorem setToFun_indicator_const (hT : DominatedFinMeasAdditive μ T C) {s : Set α}
  ...
  by
  rw [setToFun_congr_ae hT (@indicator_const_Lp_coe_fn _ _ _ 1 _ _ _ hs hμs x).symm]
...
theorem setToFun_mono_left' {T T' : Set α → E →L[ℝ] G''} {C C' : ℝ}
  ...
  by
  by_cases hf : Integrable f μ
  · simp_rw [setToFun_eq _ hf]
    exact L1.setToL1_mono_left' hT hT' hTT' _
  · simp_rw [setToFun_undef _ hf]
...
theorem continuous_setToFun (hT : DominatedFinMeasAdditive μ T C) :
  ...
  by
  simp_rw [L1.setToFun_eq_setToL1 hT]
...
theorem tendsto_setToFun_of_L1 (hT : DominatedFinMeasAdditive μ T C) {ι} (f : α → E)
  ...
  classical
    ...
    suffices Tendsto (fun i => setToFun μ T hT (F_lp i)) l (𝓝 (setToFun μ T hT f))
      ...
      exact setToFun_congr_ae hT h_ae_eq
      simp_rw [F_lp, dif_pos hi]
      exact hi.coe_fn_to_L1
    rw [setToFun_congr_ae hT hfi.coe_fn_to_L1.symm]
...
theorem tendsto_setToFun_approxOn_of_measurable_of_range_subset
    ...
    Tendsto (fun n => setToFun μ T hT (SimpleFunc.approxOn f fmeas s 0 (hs <| by simp) n)) atTop
      (𝓝 <| setToFun μ T hT f) :=
  by
  refine' tendsto_setToFun_approxOn_of_measurable hT hf fmeas _ _ (integrableZero _ _ _)
...
theorem continuous_L1_toL1 {μ' : Measure α} (c' : ℝ≥0∞) (hc' : c' ≠ ∞) (hμ'_le : μ' ≤ c' • μ) :
  ...
  let h_int := fun f' : α →₁[μ] G => (L1.integrableCoeFn f').ofMeasureLeSmul c' hc' hμ'_le
  have :
    snorm (Integrable.toL1 g (h_int g) - Integrable.toL1 f (h_int f)) 1 μ' = snorm (g - f) 1 μ' :=
    snorm_congr_ae ((Integrable.coeFn_toL1 _).sub (Integrable.coeFn_toL1 _))
  rw [this]
  have h_snorm_ne_top : snorm (g - f) 1 μ ≠ ∞ :=
    by
    rw [← snorm_congr_ae (lp.coeFn_sub _ _)]
  ...
  calc
    (snorm (g - f) 1 μ').toReal ≤ (c' * snorm (g - f) 1 μ).toReal :=
      ...
      rw [snorm_smul_measure_of_ne_zero hc'0, smul_eq_mul]
      simp
    _ = c'.to_real * (snorm (⇑g - ⇑f) 1 μ).toReal := toReal_mul
    _ ≤ c'.to_real * (ε / 2 / c'.to_real) := mul_le_mul le_rfl hfg.le toReal_nonneg toReal_nonneg
    _ = ε / 2 := by
      refine' mul_div_cancel' (ε / 2) _
      rw [Ne.def, toReal_eq_zero_iff]
      simp [hc', hc'0]
...
theorem setToFun_congr_measure_of_integrable {μ' : Measure α} (c' : ℝ≥0∞) (hc' : c' ≠ ∞)
  ...
  have h_int : ∀ g : α → E, Integrable g μ → Integrable g μ' := fun g hg =>
    Integrable.ofMeasureLeSmul c' hc' hμ'_le hg
  ...
  · refine' isClosed_eq (continuous_setToFun hT) _
    have :
      (fun f : α →₁[μ] E => setToFun μ' T hT' f) = fun f : α →₁[μ] E =>
        setToFun μ' T hT' ((h_int f (L1.integrableCoeFn f)).toL1 f) :=
      by
      ext1 f
      exact setToFun_congr_ae hT' (Integrable.coeFn_toL1 _).symm
    rw [this]
    exact (continuous_setToFun hT').comp (continuous_L1_toL1 c' hc' hμ'_le)
  · intro f₂ g₂ hfg hf₂ hf_eq
    have hfg' : f₂ =ᵐ[μ'] g₂ := (Measure.absolutelyContinuousOfLeSmul hμ'_le).ae_eq hfg
...
theorem setToFun_top_smul_measure (hT : DominatedFinMeasAdditive (∞ • μ) T C) (f : α → E) :
    setToFun (∞ • μ) T hT f = 0 :=
  by
  refine' setToFun_measure_zero' hT fun s hs hμs => _
...
theorem tendsto_setToFun_of_dominated_convergence (hT : DominatedFinMeasAdditive μ T C)
  ...
  have f_int : Integrable f μ :=
    ⟨f_measurable,
      hasFiniteIntegralOfDominatedConvergence bound_integrable.has_finite_integral h_bound h_lim⟩
  -- it suffices to prove the result for the corresponding L1 functions
  suffices
    Tendsto (fun n => L1.setToL1 hT ((fs_int n).toL1 (fs n))) atTop
      (𝓝 (L1.setToL1 hT (f_int.to_L1 f)))
    by
    convert this
    · ext1 n
