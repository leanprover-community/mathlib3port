...
theorem integral_finset_bUnion {ι : Type _} (t : Finset ι) {s : ι → Set α} (hs : ∀ i _ : i ∈ t, MeasurableSet (s i))
  (h's : Set.Pairwise (↑t) (Disjoint on s)) (hf : ∀ i _ : i ∈ t, integrable_on f (s i) μ) :
...
theorem tendsto_set_integral_of_monotone {ι : Type _} [Encodable ι] [SemilatticeSup ι] {s : ι → Set α} {f : α → E}
  ...
  by 
    ...
    exact tendsto_measure_Union hsm h_mono (Ennreal.Icc_mem_nhds hfi'.ne (Ennreal.coe_pos.2 ε0).ne')
...
theorem
  ...
  [ LinearOrderₓ E ] [ OrderClosedTopology E ] { f : α → E } ( hf : Measurable f ) ( hfi : integrable f μ )
    : ∫ x in { x | f x < 0 } , f x ∂ μ = ∫ x in { x | f x ≤ 0 } , f x ∂ μ
  :=
    by
      have h_union : { x | f x ≤ 0 } = { x | f x < 0 } ∪ { x | f x = 0 }
        · · ext simpRw [ Set.mem_union_eq , Set.mem_set_of_eq ] exact le_iff_lt_or_eqₓ
        rw [ h_union ]
        exact
          set_integral_union_eq_left
              hf hfi measurable_set_lt hf measurable_const measurable_set_eq_fun hf measurable_const fun x hx => hx
            .
            symm
...
theorem
  ...
  :=
    have
      h_meas : MeasurableSet { x | 0 ≤ f x } := measurable_set_le measurable_const hf
      calc
...
theorem
  ...
  { C : ℝ } ( hs : μ s < ∞ ) ( hC : ∀ᵐ x ∂ μ , x ∈ s → ∥ f x ∥ ≤ C ) ( hfm : AeMeasurable f μ.restrict s )
    : ∥ ∫ x in s , f x ∂ μ ∥ ≤ C * μ s . toReal
  :=
    by
      apply norm_set_integral_le_of_norm_le_const_ae hs
        have A : ∀ᵐ x : α ∂ μ , x ∈ s → ∥ AeMeasurable.mk f hfm x ∥ ≤ C
        · filterUpwards [ hC , hfm.ae_mem_imp_eq_mk ] intro a h1 h2 h3 rw [ ← h2 h3 ] exact h1 h3
...
theorem
  ...
  { s : Set α } ( hs : MeasurableSet s ) ( hf : Measurable f ) ( hfi : integrable f μ )
  ...
  :=
    by
      rw [ ← integral_indicator hs , ← integral_indicator measurable_set_le measurable_const hf ]
        exact
          integral_mono
            hfi.indicator hs hfi.indicator measurable_set_le measurable_const hf indicator_le_indicator_nonneg s f
...
theorem
  ...
  { s : Set α } ( hs : MeasurableSet s ) { f : α → ℝ } ( hf : Measurable f ) ( hfi : integrable f μ )
  ...
  :=
    by
      rw [ ← integral_indicator hs , ← integral_indicator measurable_set_le hf measurable_const ]
        exact
          integral_mono
            hfi.indicator measurable_set_le hf measurable_const hfi.indicator hs indicator_nonpos_le_indicator s f
...
theorem Lp_to_Lp_restrict_add (f g : Lp E p μ) (s : Set α) :
  ((Lp.mem_ℒp (f+g)).restrict s).toLp (⇑f+g) = ((Lp.mem_ℒp f).restrict s).toLp f+((Lp.mem_ℒp g).restrict s).toLp g :=
  by 
    ext1 
...
theorem Lp_to_Lp_restrict_smul (c : 𝕜) (f : Lp F p μ) (s : Set α) :
  ((Lp.mem_ℒp (c • f)).restrict s).toLp (⇑(c • f)) = c • ((Lp.mem_ℒp f).restrict s).toLp f :=
  by 
    ext1 
