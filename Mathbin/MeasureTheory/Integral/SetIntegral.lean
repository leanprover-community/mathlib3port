...
-- error in MeasureTheory.Integral.SetIntegral: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
(ht : measurable_set t)
(hfs : integrable_on f s μ)
(hft : integrable_on f t μ)
(hts : «expr ⊆ »(t, s)) : «expr = »(«expr∫ in , ∂ »((x), «expr \ »(s, t), f x, μ), «expr - »(«expr∫ in , ∂ »((x), s, f x, μ), «expr∫ in , ∂ »((x), t, f x, μ))) :=
begin
  rw ["[", expr eq_sub_iff_add_eq, ",", "<-", expr integral_union, ",", expr diff_union_of_subset hts, "]"] [],
  exacts ["[", expr disjoint_diff.symm, ",", expr hs.diff ht, ",", expr ht, ",", expr hfs.mono_set (diff_subset _ _), ",", expr hft, "]"]
end
...
theorem has_sum_integral_Union {ι : Type _} [Encodable ι] {s : ι → Set α} {f : α → E} (hm : ∀ i, MeasurableSet (s i))
  ...
  by 
    have hfi' : ∀ i, integrable_on f (s i) μ 
    exact fun i => hfi.mono_set (subset_Union _ _)
...
-- error in MeasureTheory.Integral.SetIntegral: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem set_integral_union_eq_left
{f : α → E}
(hf : measurable f)
(hfi : integrable f μ)
(hs : measurable_set s)
(ht : measurable_set t)
(ht_eq : ∀
 x «expr ∈ » t, «expr = »(f x, 0)) : «expr = »(«expr∫ in , ∂ »((x), «expr ∪ »(s, t), f x, μ), «expr∫ in , ∂ »((x), s, f x, μ)) :=
begin
  rw ["[", "<-", expr set.union_diff_self, ",", expr integral_union, ",", expr set_integral_eq_zero_of_forall_eq_zero _ (λ
    x hx, ht_eq x (diff_subset _ _ hx)), ",", expr add_zero, "]"] [],
  exacts ["[", expr hf, ",", expr disjoint_diff, ",", expr hs, ",", expr ht.diff hs, ",", expr hfi.integrable_on, ",", expr hfi.integrable_on, "]"]
end

theorem set_integral_neg_eq_set_integral_nonpos [LinearOrderₓ E] [OrderClosedTopology E] {f : α → E} (hf : Measurable f)
  (hfi : integrable f μ) : (∫x in { x | f x < 0 }, f x ∂μ) = ∫x in { x | f x ≤ 0 }, f x ∂μ :=
  by 
    have h_union : { x | f x ≤ 0 } = { x | f x < 0 } ∪ { x | f x = 0 }
    ·
      ·
        ext 
        simpRw [Set.mem_union_eq, Set.mem_set_of_eq]
...
theorem norm_set_integral_le_of_norm_le_const_ae {C : ℝ} (hs : μ s < ∞) (hC : ∀ᵐx ∂μ.restrict s, ∥f x∥ ≤ C) :
...
theorem norm_set_integral_le_of_norm_le_const_ae' {C : ℝ} (hs : μ s < ∞) (hC : ∀ᵐx ∂μ, x ∈ s → ∥f x∥ ≤ C)
  (hfm : AeMeasurable f (μ.restrict s)) : ∥∫x in s, f x ∂μ∥ ≤ C*(μ s).toReal :=
  by 
    apply norm_set_integral_le_of_norm_le_const_ae hs 
    have A : ∀ᵐx : α ∂μ, x ∈ s → ∥AeMeasurable.mk f hfm x∥ ≤ C
    ·
      filterUpwards [hC, hfm.ae_mem_imp_eq_mk]
      intro a h1 h2 h3 
      rw [←h2 h3]
      exact h1 h3 
    have B : MeasurableSet { x | ∥(hfm.mk f) x∥ ≤ C } := hfm.measurable_mk.norm measurable_set_Iic 
    filterUpwards [hfm.ae_eq_mk, (ae_restrict_iff B).2 A]
    intro a h1 h2 
    rwa [h1]
...
-- error in MeasureTheory.Integral.SetIntegral: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem set_integral_mono_set
(hfi : integrable f μ)
(hf : «expr ≤ᵐ[ ] »(0, μ, f))
(hst : «expr ≤ᵐ[ ] »(s, μ, t)) : «expr ≤ »(«expr∫ in , ∂ »((x), s, f x, μ), «expr∫ in , ∂ »((x), t, f x, μ)) :=
begin
  repeat { rw [expr integral_eq_lintegral_of_nonneg_ae (ae_restrict_of_ae hf) (hfi.1.mono_measure measure.restrict_le_self)] [] },
  rw [expr ennreal.to_real_le_to_real «expr $ »(ne_of_lt, (has_finite_integral_iff_of_real (ae_restrict_of_ae hf)).mp hfi.integrable_on.2) «expr $ »(ne_of_lt, (has_finite_integral_iff_of_real (ae_restrict_of_ae hf)).mp hfi.integrable_on.2)] [],
  exact [expr lintegral_mono_set' hst]
end
...
theorem _root_.antitone.tendsto_set_integral (hsm : ∀ i, MeasurableSet (s i)) (h_anti : Antitone s)
  (hfi : integrable_on f (s 0) μ) : tendsto (fun i => ∫a in s i, f a ∂μ) at_top (𝓝 (∫a in ⋂n, s n, f a ∂μ)) :=
  by 
    let bound : α → ℝ := indicator (s 0) fun a => ∥f a∥
    have h_int_eq : (fun i => ∫a in s i, f a ∂μ) = fun i => ∫a, (s i).indicator f a ∂μ 
    exact funext fun i => (integral_indicator (hsm i)).symm 
    rw [h_int_eq]
...
theorem Filter.Tendsto.integral_sub_linear_is_o_ae [NormedSpace ℝ E] [second_countable_topology E] [CompleteSpace E]
  ...
  is_o (fun i => (∫x in s i, f x ∂μ) - m i • b) m li :=
  by 
    ...
    have  : ∀ᶠs in l.lift' powerset, ∀ᶠx in μ.ae, x ∈ s → f x ∈ closed_ball b ε :=
      eventually_lift'_powerset_eventually.2 (h.eventually$ closed_ball_mem_nhds _ ε₀)
    filterUpwards [hμ.eventually, (hμ.integrable_at_filter_of_tendsto_ae hfm h).Eventually, hfm.eventually, this]
...
theorem ContinuousWithinAt.integral_sub_linear_is_o_ae [TopologicalSpace α] [OpensMeasurableSpace α] [NormedSpace ℝ E]
  ...
  {t : Set α} {f : α → E} (ha : ContinuousWithinAt f t a) (ht : MeasurableSet t) (hfm : MeasurableAtFilter f (𝓝[t] a) μ)
  {s : ι → Set α} {li : Filter ι} (hs : tendsto s li ((𝓝[t] a).lift' powerset)) (m : ι → ℝ := fun i => (μ (s i)).toReal)
  (hsμ : (fun i => (μ (s i)).toReal) =ᶠ[li] m :=  by 
    runTac 
      tactic.interactive.refl) :
  ...
  by 
    haveI  : (𝓝[t] a).IsMeasurablyGenerated := ht.nhds_within_is_measurably_generated _ <;>
      exact (ha.mono_left inf_le_left).integral_sub_linear_is_o_ae hfm (μ.finite_at_nhds_within a t) hs m hsμ
...
theorem integral_apply {H : Type _} [NormedGroup H] [NormedSpace ℝ H] [second_countable_topology$ H →L[ℝ] E]
  {φ : α → H →L[ℝ] E} (φ_int : integrable φ μ) (v : H) : (∫a, φ a ∂μ) v = ∫a, φ a v ∂μ :=
  ((ContinuousLinearMap.apply ℝ E v).integral_comp_comm φ_int).symm

theorem integral_comp_comm' (L : E →L[𝕜] F) {K} (hL : AntilipschitzWith K L) (φ : α → E) :
  (∫a, L (φ a) ∂μ) = L (∫a, φ a ∂μ) :=
  by 
    byCases' h : integrable φ μ
    ·
