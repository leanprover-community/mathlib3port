...
def TorusIntegrable (f : ℂⁿ → E) (c : ℂⁿ) (R : ℝⁿ) : Prop :=
  IntegrableOn (fun θ : ℝⁿ => f (torusMap c R θ)) (IccCat (0 : ℝⁿ) fun _ => 2 * π) volume
...
def torusIntegral (f : ℂⁿ → E) (c : ℂⁿ) (R : ℝⁿ) :=
  ∫ θ : ℝⁿ in IccCat (0 : ℝⁿ) fun _ => 2 * π, (∏ i, R i * exp (θ i * I) * I : ℂ) • f (torusMap c R θ)
...
theorem torus_integral_radius_zero (hn : n ≠ 0) (f : ℂⁿ → E) (c : ℂⁿ) : (∯ x in T(c, 0), f x) = 0 := by
  simp only [torusIntegral, Pi.zero_apply, of_real_zero, mul_zero, zero_mul, Fin.prod_const, zero_pow' n hn, zero_smul,
    integral_zero]
...
theorem torus_integral_dim0 (f : ℂ⁰ → E) (c : ℂ⁰) (R : ℝ⁰) : (∯ x in T(c, R), f x) = f c := by
  simp only [torusIntegral, Fin.prod_univ_zero, one_smul, Subsingleton.elim (fun i : Fin 0 => 2 * π) 0, Icc_self,
...
theorem torus_integral_dim1 (f : ℂ¹ → E) (c : ℂ¹) (R : ℝ¹) : (∯ x in T(c, R), f x) = ∮ z in C(c 0, R 0), f fun _ => z :=
  ...
  simp only [torusIntegral, circleIntegral, intervalIntegral.integral_of_le real.two_pi_pos.le,
    ...
    ((volume_preserving_fun_unique (Fin 1) ℝ).symm _).set_integral_preimage_emb (MeasurableEquiv.measurableEmbedding _),
...
theorem torus_integral_succ_above {f : ℂⁿ⁺¹ → E} {c : ℂⁿ⁺¹} {R : ℝⁿ⁺¹} (hf : TorusIntegrable f c R) (i : Fin (n + 1)) :
  ...
  have hem : measure_preserving e := (volume_preserving_pi_fin_succ_above_equiv (fun j : Fin (n + 1) => ℝ) i).symm _
  ...
  · refine' set_integral_congr measurableSetIcc fun θ hθ => _
    ...
    congr 2
    simp only [funext_iff, i.forall_iff_succ_above, circleMap, Fin.insert_nth_apply_same, eq_self_iff_true,
      Fin.insert_nth_apply_succ_above, imp_true_iff, and_self_iff]
