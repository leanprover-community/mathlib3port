...
theorem weighted_smul_smul [NormedField 𝕜] [NormedSpace 𝕜 F] [SmulCommClass ℝ 𝕜 F] (c : 𝕜) (s : Set α) (x : F) :
    weightedSmul μ s (c • x) = c • weightedSmul μ s x := by simp_rw [weighted_smul_apply, smul_comm]
...
theorem norm_weighted_smul_le (s : Set α) : ∥(weightedSmul μ s : F →L[ℝ] F)∥ ≤ (μ s).toReal :=
  calc
    ...
    _ ≤ ∥(μ s).toReal∥ := (mul_le_mul_of_nonneg_left norm_id_le (norm_nonneg _)).trans (mul_one _).le
...
#align measure_theory.simple_func.pos_part_sub_neg_part MeasureTheory.SimpleFunc.pos_part_sub_neg_part
...
variable [NormedAddCommGroup E] [NormedAddCommGroup F] [NormedSpace ℝ F] {p : ℝ≥0∞} {G F' : Type _}
  [NormedAddCommGroup G] [NormedAddCommGroup F'] [NormedSpace ℝ F'] {m : MeasurableSpace α} {μ : Measure α}
        ...
        (calcStep
         (Init.Core.«term_=_»
          ...
          (BigOperators.Algebra.BigOperators.Basic.finset.sum
           ...
           (Algebra.Group.Defs.«term_•_»
            (Term.proj
             (Term.app `μ [(Set.Data.Set.Basic.«term_⁻¹'_» (Term.app `const [`α `y]) " ⁻¹' " («term{_}» "{" [`z] "}"))])
             "."
             `toReal)
            " • "
            `z)))
...
@[ simp ]
  theorem
    integral_const
    { m : MeasurableSpace α } ( μ : Measure α ) ( y : F ) : const α y . integral μ = μ univ . toReal • y
    :=
      by
        skip
          <;>
...
theorem integral_piecewise_zero {m : MeasurableSpace α} (f : α →ₛ F) (μ : Measure α) {s : Set α}
    (hs : MeasurableSet s) : (piecewise s hs f 0).integral μ = f.integral (μ.restrict s) := by classical
...
theorem norm_eq_integral (f : α →₁ₛ[μ] E) : ∥f∥ = ((toSimpleFunc f).map norm).integral μ := by
  ...
  simp_rw [smul_eq_mul]
#align measure_theory.L1.simple_func.norm_eq_integral MeasureTheory.L1Cat.SimpleFunc.norm_eq_integral
...
variable [NormedField 𝕜] [NormedSpace 𝕜 E] [NormedSpace ℝ E] [SmulCommClass ℝ 𝕜 E] {F' : Type _} [NormedAddCommGroup F']
  [NormedSpace ℝ F']
...
theorem integral_eq_integral (f : α →₁ₛ[μ] E) : integral f = (toSimpleFunc f).integral μ :=
  rfl
#align measure_theory.L1.simple_func.integral_eq_integral MeasureTheory.L1Cat.SimpleFunc.integral_eq_integral
...
#align measure_theory.L1.simple_func.integral_eq_lintegral MeasureTheory.L1Cat.SimpleFunc.integral_eq_lintegral

theorem integral_eq_set_to_L1s (f : α →₁ₛ[μ] E) : integral f = setToL1s (weightedSmul μ) f :=
  rfl
#align measure_theory.L1.simple_func.integral_eq_set_to_L1s MeasureTheory.L1Cat.SimpleFunc.integral_eq_set_to_L1s
...
#align measure_theory.L1.simple_func.norm_integral_le_norm MeasureTheory.L1Cat.SimpleFunc.norm_integral_le_norm
...
#align measure_theory.L1.simple_func.norm_Integral_le_one MeasureTheory.L1Cat.SimpleFunc.norm_Integral_le_one
...
theorem pos_part_to_simple_func (f : α →₁ₛ[μ] ℝ) : toSimpleFunc (posPart f) =ᵐ[μ] (toSimpleFunc f).posPart := by
  ...
  rw [h, Eq]
#align measure_theory.L1.simple_func.pos_part_to_simple_func MeasureTheory.L1Cat.SimpleFunc.pos_part_to_simple_func
...
theorem neg_part_to_simple_func (f : α →₁ₛ[μ] ℝ) : toSimpleFunc (negPart f) =ᵐ[μ] (toSimpleFunc f).negPart := by
  ...
  rw [h₂]
  rfl
#align measure_theory.L1.simple_func.neg_part_to_simple_func MeasureTheory.L1Cat.SimpleFunc.neg_part_to_simple_func
...
theorem integral_eq_set_to_L1 (f : α →₁[μ] E) : integral f = setToL1 (dominatedFinMeasAdditiveWeightedSmul μ) f :=
  rfl
...
#align measure_theory.L1.simple_func.integral_L1_eq_integral MeasureTheory.L1Cat.SimpleFunc.integral_L1_eq_integral
...
#align measure_theory.L1.integral_eq_norm_pos_part_sub MeasureTheory.L1Cat.integral_eq_norm_pos_part_sub
...
variable [NormedAddCommGroup E] [NormedSpace ℝ E] [CompleteSpace E] [NontriviallyNormedField 𝕜] [NormedSpace 𝕜 E]
  [SmulCommClass ℝ 𝕜 E] [NormedAddCommGroup F] [NormedSpace ℝ F] [CompleteSpace F]
...
theorem integral_eq (f : α → E) (hf : Integrable f μ) : (∫ a, f a ∂μ) = L1Cat.integral (hf.toL1 f) :=
...
#align measure_theory.integral_non_ae_strongly_measurable MeasureTheory.integral_non_ae_strongly_measurable
...
@[simp]
theorem L1Cat.integral_of_fun_eq_integral {f : α → E} (hf : Integrable f μ) : (∫ a, (hf.toL1 f) a ∂μ) = ∫ a, f a ∂μ :=
...
theorem HasFiniteIntegral.tendsto_set_integral_nhds_zero {ι} {f : α → E} (hf : HasFiniteIntegral f μ) {l : Filter ι}
  ...
  exact
    tendsto_of_tendsto_of_tendsto_of_le_of_le tendsto_const_nhds (tendsto_set_lintegral_zero (ne_of_lt hf) hs)
...
theorem tendsto_integral_of_dominated_convergence {F : ℕ → α → E} {f : α → E} (bound : α → ℝ)
  ...
  tendsto_set_to_fun_of_dominated_convergence (dominatedFinMeasAdditiveWeightedSmul μ) bound F_measurable
...
theorem tendsto_integral_filter_of_dominated_convergence {ι} {l : Filter ι} [l.IsCountablyGenerated] {F : ι → α → E}
  ...
  tendsto_set_to_fun_filter_of_dominated_convergence (dominatedFinMeasAdditiveWeightedSmul μ) bound hF_meas h_bound
...
#align measure_theory.has_sum_integral_of_dominated_convergence MeasureTheory.has_sum_integral_of_dominated_convergence
...
theorem continuous_at_of_dominated {F : X → α → E} {x₀ : X} {bound : α → ℝ}
  ...
  continuous_at_set_to_fun_of_dominated (dominatedFinMeasAdditiveWeightedSmul μ) hF_meas h_bound bound_integrable h_cont
...
theorem continuous_of_dominated {F : X → α → E} {bound : α → ℝ} (hF_meas : ∀ x, AeStronglyMeasurable (F x) μ)
  ...
  continuous_set_to_fun_of_dominated (dominatedFinMeasAdditiveWeightedSmul μ) hF_meas h_bound bound_integrable h_cont
...
theorem integral_eq_lintegral_of_nonneg_ae {f : α → ℝ} (hf : 0 ≤ᵐ[μ] f) (hfm : AeStronglyMeasurable f μ) :
  ...
  · rw [integral_undef hfi]
    simp_rw [integrable, hfm, has_finite_integral_iff_norm, lt_top_iff_ne_top, Ne.def, true_and_iff, not_not] at hfi
...
theorem integral_norm_eq_lintegral_nnnorm {G} [NormedAddCommGroup G] {f : α → G} (hf : AeStronglyMeasurable f μ) :
...
theorem of_real_integral_norm_eq_lintegral_nnnorm {G} [NormedAddCommGroup G] {f : α → G} (hf : Integrable f μ) :
...
#align measure_theory.of_real_integral_norm_eq_lintegral_nnnorm MeasureTheory.of_real_integral_norm_eq_lintegral_nnnorm
...
theorem integral_nonneg_of_ae {f : α → ℝ} (hf : 0 ≤ᵐ[μ] f) : 0 ≤ ∫ a, f a ∂μ :=
  set_to_fun_nonneg (dominatedFinMeasAdditiveWeightedSmul μ) (fun s _ _ => weighted_smul_nonneg s) hf
...
#align measure_theory.of_real_integral_eq_lintegral_of_real MeasureTheory.of_real_integral_eq_lintegral_of_real
...
theorem integral_eq_zero_iff_of_nonneg_ae {f : α → ℝ} (hf : 0 ≤ᵐ[μ] f) (hfi : Integrable f μ) :
  ...
  simp_rw [integral_eq_lintegral_of_nonneg_ae hf hfi.1, Ennreal.to_real_eq_zero_iff,
    ...
    has_finite_integral_iff_of_real hf, hfi.2, not_true, or_false_iff, ← hf.le_iff_eq, Filter.EventuallyEq,
...
theorem L1Cat.norm_of_fun_eq_integral_norm {f : α → H} (hf : Integrable f μ) : ∥hf.toL1 f∥ = ∫ a, ∥f a∥ ∂μ := by
  rw [L1.norm_eq_integral_norm]
  refine' integral_congr_ae _
  apply hf.coe_fn_to_L1.mono
  intro a ha
  simp [ha]
#align measure_theory.L1.norm_of_fun_eq_integral_norm MeasureTheory.L1Cat.norm_of_fun_eq_integral_norm
...
theorem integral_mono_ae {f g : α → ℝ} (hf : Integrable f μ) (hg : Integrable g μ) (h : f ≤ᵐ[μ] g) :
    (∫ a, f a ∂μ) ≤ ∫ a, g a ∂μ :=
  set_to_fun_mono (dominatedFinMeasAdditiveWeightedSmul μ) (fun s _ _ => weighted_smul_nonneg s) hf hg h
...
theorem integral_finset_sum_measure {ι} {m : MeasurableSpace α} {f : α → E} {μ : ι → Measure α} {s : Finset ι}
    (hf : ∀ i ∈ s, Integrable f (μ i)) : (∫ a, f a ∂∑ i in s, μ i) = ∑ i in s, ∫ a, f a ∂μ i := by classical
...
theorem integral_map_of_strongly_measurable {β} [MeasurableSpace β] {φ : α → β} (hφ : Measurable φ) {f : β → E}
  ...
  rw [hy]
  simp
#align measure_theory.integral_map_of_strongly_measurable MeasureTheory.integral_map_of_strongly_measurable
...
theorem integral_map {β} [MeasurableSpace β] {φ : α → β} (hφ : AeMeasurable φ μ) {f : β → E}
  ...
  calc
    ...
    _ = ∫ x, g (hφ.mk φ x) ∂μ := integral_map_of_strongly_measurable hφ.measurableMk hfm.stronglyMeasurableMk
...
#align measure_theory._root_.measurable_embedding.integral_map measure_theory._root_.measurable_embedding.integral_map
...
#align measure_theory._root_.closed_embedding.integral_map measure_theory._root_.closed_embedding.integral_map
...
theorem integral_dirac' [MeasurableSpace α] (f : α → E) (a : α) (hfm : StronglyMeasurable f) :
  ...
  borelize E
  calc
    (∫ x, f x ∂measure.dirac a) = ∫ x, f a ∂measure.dirac a := integral_congr_ae $ ae_eq_dirac' hfm.measurable
...
theorem mul_meas_ge_le_integral_of_nonneg [IsFiniteMeasure μ] {f : α → ℝ} (hf_nonneg : 0 ≤ f) (hf_int : Integrable f μ)
  ...
  · simp only [Ne.def, WithTop.mul_eq_top_iff, Ennreal.of_real_eq_zero, not_le, Ennreal.of_real_ne_top, false_and_iff,
      or_false_iff, not_and]
...
theorem SimpleFunc.coe_to_larger_space_eq (hm : m ≤ m0) (f : @SimpleFunc β m γ) : ⇑(f.toLargerSpace hm) = f :=
  rfl
#align measure_theory.simple_func.coe_to_larger_space_eq MeasureTheory.SimpleFunc.coe_to_larger_space_eq
...
theorem integral_simple_func_larger_space (hm : m ≤ m0) (f : @SimpleFunc β m F) (hf_int : Integrable f μ) :
  ...
  congr
#align measure_theory.integral_simple_func_larger_space MeasureTheory.integral_simple_func_larger_space
...
theorem integral_trim_simple_func (hm : m ≤ m0) (f : @SimpleFunc β m F) (hf_int : Integrable f μ) :
  ...
  rw [integral_simple_func_larger_space (le_refl m) f hf_int_m, integral_simple_func_larger_space hm f hf_int]
  congr with x
  congr
...
#align measure_theory.ae_eq_trim_of_strongly_measurable MeasureTheory.ae_eq_trim_of_strongly_measurable
...
theorem ae_le_trim_of_strongly_measurable [LinearOrder γ] [TopologicalSpace γ] [OrderClosedTopology γ]
...
#align measure_theory.ae_le_trim_of_strongly_measurable MeasureTheory.ae_le_trim_of_strongly_measurable

theorem ae_le_trim_iff [LinearOrder γ] [TopologicalSpace γ] [OrderClosedTopology γ] [PseudoMetrizableSpace γ]
...
theorem snorm_one_le_of_le {r : ℝ≥0} {f : α → ℝ} (hfint : Integrable f μ) (hfint' : 0 ≤ ∫ x, f x ∂μ)
  ...
  rw [mem_ℒp.snorm_eq_integral_rpow_norm one_ne_zero Ennreal.one_ne_top (mem_ℒp_one_iff_integrable.2 hfint),
    Ennreal.of_real_le_iff_le_to_real
      (Ennreal.mul_ne_top (Ennreal.mul_ne_top Ennreal.two_ne_top $ @measure_ne_top _ _ _ hμ _) Ennreal.coe_ne_top)]
