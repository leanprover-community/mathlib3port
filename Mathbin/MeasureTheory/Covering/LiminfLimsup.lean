...
theorem blimsup_cthickening_ae_le_of_eventually_mul_le_aux (p : ℕ → Prop) {s : ℕ → Set α}
  ...
  suffices ∀ i, μ (at_top.blimsup Y₁ p \ Z i) = 0 by
    rwa [ae_le_set, @blimsup_eq_infi_bsupr_of_nat _ _ _ Y₂, infᵢ_eq_interᵢ, diff_interᵢ,
      measure_unionᵢ_null_iff]
  intros
    ...
    ∃ d,
      d ∈ W ∧
        ∀ {ι : Type _} {l : Filter ι} (w : ι → α) (δ : ι → ℝ),
          Tendsto δ l (𝓝[>] 0) →
            (∀ᶠ j in l, d ∈ closedBall (w j) (2 * δ j)) →
              Tendsto (fun j => μ (W ∩ closedBall (w j) (δ j)) / μ (closedBall (w j) (δ j))) l
                (𝓝 1) :=
  ...
  · intro j
    ...
    ·
      exact
        mem_Union₂.mp
          (cthickening_subset_unionᵢ_closedBall_of_lt (s (f j)) (by positivity)
  ...
  suffices
    ∃ η < (1 : ℝ≥0),
      ∀ᶠ j in atTop, μ (W ∩ closedBall (w j) (r₁ (f j))) / μ (closedBall (w j) (r₁ (f j))) ≤ η
    by
    obtain ⟨η, hη, hη'⟩ := this
    replace hη' : 1 ≤ η := by
  ...
  have h₃ : ∀ᶠ j in atTop, Disjoint (b j) (W ∩ B j) :=
    by
    apply hMr.mp
    rw [eventually_atTop]
    refine'
    ...
    refine'
      (closedBall_subset_cthickening (hw j) (M * r₁ (f j))).trans
...
theorem blimsup_cthickening_mul_ae_eq (p : ℕ → Prop) (s : ℕ → Set α) {M : ℝ} (hM : 0 < M)
  ...
  by
  have :
    ∀ (p : ℕ → Prop) {r : ℕ → ℝ} (hr : Tendsto r atTop (𝓝[>] 0)),
      (blimsup (fun i => cthickening (M * r i) (s i)) atTop p : Set α) =ᵐ[μ]
        (blimsup (fun i => cthickening (r i) (s i)) atTop p : Set α) :=
    by
    clear p hr r
...
theorem blimsup_thickening_mul_ae_eq_aux (p : ℕ → Prop) (s : ℕ → Set α) {M : ℝ} (hM : 0 < M)
  ...
  replace hr : Tendsto (fun i => M * r i) atTop (𝓝 0);
...
theorem blimsup_thickening_mul_ae_eq (p : ℕ → Prop) (s : ℕ → Set α) {M : ℝ} (hM : 0 < M) (r : ℕ → ℝ)
  ...
  have h₁ :
    blimsup (fun i => thickening (r i) (s i)) atTop p =
      blimsup (fun i => thickening (r i) (s i)) atTop q :=
    by
  ...
  have h₂ :
    blimsup (fun i => thickening (M * r i) (s i)) atTop p =
      blimsup (fun i => thickening (M * r i) (s i)) atTop q :=
    by
    refine' blimsup_congr' (eventually_of_forall fun i h => _)
    replace h : 0 < r i
