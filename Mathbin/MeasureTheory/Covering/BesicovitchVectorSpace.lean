      ...
      Sup
        {
          ...
          ∃
            s : Finset E
            ,
            s.card = N ∧ ∀ c _ : c ∈ s , ∥ c ∥ ≤ 2 ∧ ∀ c _ : c ∈ s , ∀ d _ : d ∈ s , c ≠ d → 1 ≤ ∥ c - d ∥
          }
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (f «expr ∈ » closed_ball (0 : fin N → E) 2)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (d «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∈ » s)
...
theorem exists_good_δ :
  ∃ δ : ℝ,
    0 < δ ∧
      δ < 1 ∧
        ∀ s : Finset E,
          (∀ c _ : c ∈ s, ∥c∥ ≤ 2) → (∀ c _ : c ∈ s d _ : d ∈ s, c ≠ d → 1 - δ ≤ ∥c - d∥) → s.card ≤ multiplicity E :=
  by 
    classical 
    byContra h 
    pushNeg  at h 
    set N := multiplicity E+1 with hN 
    have  : ∀ δ : ℝ, 0 < δ → ∃ f : Finₓ N → E, (∀ i : Finₓ N, ∥f i∥ ≤ 2) ∧ ∀ i j, i ≠ j → 1 - δ ≤ ∥f i - f j∥
    ·
      intro δ hδ 
      ...
      ·
        ...
        ·
          have  : Fintype.card (Finₓ N) ≤ s.card
          ·
    ...
    have h's : ∀ c _ : c ∈ s d _ : d ∈ s, c ≠ d → 1 ≤ ∥c - d∥
    ·
...
theorem le_multiplicity_of_δ_of_fin {n : ℕ} (f : Finₓ n → E) (h : ∀ i, ∥f i∥ ≤ 2)
  (h' : ∀ i j, i ≠ j → 1 - good_δ E ≤ ∥f i - f j∥) : n ≤ multiplicity E :=
  by 
    classical 
    have finj : Function.Injective f
    ·
    ...
    ·
      simp only [h, forall_apply_eq_imp_iff', forall_const, forall_exists_index, Finset.mem_univ, Finset.mem_image,
        implies_true_iff]
    have h's : ∀ c _ : c ∈ s d _ : d ∈ s, c ≠ d → 1 - good_δ E ≤ ∥c - d∥
    ·
      simp only [s, forall_apply_eq_imp_iff', forall_exists_index, Finset.mem_univ, Finset.mem_image, Ne.def,
        exists_true_left, forall_apply_eq_imp_iff', forall_true_left]
      intro i j hij 
