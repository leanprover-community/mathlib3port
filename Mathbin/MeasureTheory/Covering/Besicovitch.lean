...
-- error in MeasureTheory.Covering.Besicovitch: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
theorem exist_finset_disjoint_balls_large_measure [second_countable_topology α] [MeasurableSpace α]
  ...
  by 
    ...
    ·
      ...
      intro x hx 
      obtain ⟨i, y, hxy, h'⟩ :
        ∃ (i : Finₓ N)(i_1 : «expr↥ » s)(i : i_1 ∈ u i), x ∈ ball («expr↑ » i_1) (r («expr↑ » i_1))
      ·
        have  : x ∈ range a.c
        ·
    ...
    ·
      suffices H : μ (o \ ⋃(x : _)(_ : x ∈ w), closed_ball («expr↑ » x) (r («expr↑ » x))) ≤ (N / N+1)*μ s
      ·
      ...
      calc μ o = ((1 / N+1)*μ s)+(N / N+1)*μ s :=
        by 
          rw [μo, ←add_mulₓ, Ennreal.div_add_div_same, add_commₓ, Ennreal.div_self, one_mulₓ] <;>
            simp _ ≤ μ ((⋃(x : _)(_ : x ∈ w), closed_ball («expr↑ » x) (r («expr↑ » x))) ∩ o)+(N / N+1)*μ s :=
        by 
          ...
          ·
            ...
            ·
              ·
                intro k hk l hl hkl 
                exact hu i k k.2 l l.2 (subtype.coe_injective.ne hkl)
    ...
    ·
      intro k hk l hl hkl 
      obtain ⟨k', k'w, rfl⟩ : ∃ k' : u i, k' ∈ w ∧ «expr↑ » («expr↑ » k') = k
      ·
        simpa only [mem_image, Finset.mem_coe, coe_coe, Finset.coe_image] using hk 
      obtain ⟨l', l'w, rfl⟩ : ∃ l' : u i, l' ∈ w ∧ «expr↑ » («expr↑ » l') = l
      ·
    ...
    ·
      ...
      ·
        ...
        ·
          ...
          refine' hv p' p'v q' q'v fun hp'q' => _ 
          rw [hp'q'] at hpq 
          exact hpq rfl
        ·
          intro p hp q hq hpq 
...
protected def VitaliFamily [second_countable_topology α] [HasBesicovitchCovering α] [MeasurableSpace α]
  [OpensMeasurableSpace α] (μ : Measureₓ α) [sigma_finite μ] : VitaliFamily μ :=
  { SetsAt := fun x => (fun r : ℝ => closed_ball x r) '' Ioi (0 : ℝ),
    ...
    covering :=
      by 
        ...
        ·
          intro x xs 
          obtain ⟨t, tf, ht⟩ : ∃ (t : Set α)(H : t ∈ f x), t ⊆ closed_ball x 1 := ffine x xs 1 zero_lt_one 
        ...
        ·
          intro x xs r hr 
          replace hr : 0 < r ∧ closed_ball x r ∈ f x
          ·
            simpa only using hr 
          exact hr.1
