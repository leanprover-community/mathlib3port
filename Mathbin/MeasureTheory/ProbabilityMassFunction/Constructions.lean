...
@[simp]
theorem map_apply : (map f p) b = ∑' a, if b = f a then p a else 0 :=
  by 
    simp [map]
...
theorem mem_support_map_iff : b ∈ (map f p).Support ↔ ∃ (a : _)(_ : a ∈ p.support), f a = b :=
  by 
    simp 
...
theorem map_comp (g : β → γ) : (p.map f).map g = p.map (g ∘ f) :=
  by 
    simp [map]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (f «expr ∈ » q.support)
@[simp]
theorem support_seq : (seq q p).Support = ⋃ (f : _)(_ : f ∈ q.support), f '' p.support :=
  Set.ext
    fun b =>
      by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (f «expr ∈ » q.support)
theorem mem_support_seq_iff : b ∈ (seq q p).Support ↔ ∃ (f : _)(_ : f ∈ q.support), b ∈ f '' p.support :=
  by 
    simp 
...
theorem mem_support_of_finset_iff (a : α) : a ∈ (of_finset f s h h').Support ↔ a ∈ s ∧ f a ≠ 0 :=
  by 
    simp 
...
theorem mem_support_of_multiset_iff (a : α) : a ∈ (of_multiset s hs).Support ↔ a ∈ s.to_finset :=
  by 
    simp 
...
theorem uniform_of_finset_apply_of_mem (ha : a ∈ s) : uniform_of_finset s hs a = s.card⁻¹ :=
  by 
    simp [ha]

theorem uniform_of_finset_apply_of_not_mem (ha : a ∉ s) : uniform_of_finset s hs a = 0 :=
  by 
    simp [ha]
...
theorem mem_support_uniform_of_finset_iff (a : α) : a ∈ (uniform_of_finset s hs).Support ↔ a ∈ s :=
  by 
    simp 
...
@[simp]
theorem uniform_of_fintype_apply (a : α) : uniform_of_fintype α a = Fintype.card α⁻¹ :=
  by 
...
theorem mem_support_uniform_of_fintype_iff (a : α) : a ∈ (uniform_of_fintype α).Support :=
  by 
    simp 
...
theorem mem_support_normalize_iff (a : α) : a ∈ (normalize f hf0).Support ↔ f a ≠ 0 :=
  by 
    simp 
...
def Filter (p : Pmf α) (s : Set α) (h : ∃ (a : _)(_ : a ∈ s), a ∈ p.support) : Pmf α :=
  Pmf.normalize (s.indicator p)$ Nnreal.tsum_indicator_ne_zero p.2.Summable h

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s)
variable {p : Pmf α} {s : Set α} (h : ∃ (a : _)(_ : a ∈ s), a ∈ p.support)
...
theorem filter_apply_eq_zero_of_not_mem {a : α} (ha : a ∉ s) : (p.filter s h) a = 0 :=
  by 
...
theorem filter_apply_eq_zero_iff (a : α) : (p.filter s h) a = 0 ↔ a ∉ s ∨ a ∉ p.support :=
  by 
    erw [apply_eq_zero_iff, support_filter, Set.mem_inter_iff, not_and_distrib]

theorem filter_apply_ne_zero_iff (a : α) : (p.filter s h) a ≠ 0 ↔ a ∈ s ∧ a ∈ p.support :=
  by 
    ...
    :=
      by
        refine' Set.ext fun b => _
          induction b
          ·
            simpRw [ mem_support_iff , bernoulli_apply , Bool.cond_ff , Ne.def , tsub_eq_zero_iff_le , not_leₓ ]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » p.support)
variable {p : Pmf α} (f : ∀ a _ : a ∈ p.support, Pmf β)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » p.support)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » (p.bind_on_support f).support)
@[simp]
theorem bind_on_support_bind_on_support (p : Pmf α) (f : ∀ a _ : a ∈ p.support, Pmf β)
  ...
  (p.bind_on_support f).bindOnSupport g =
    p.bind_on_support
      fun a ha => (f a ha).bindOnSupport fun b hb => g b ((mem_support_bind_on_support_iff f b).mpr ⟨a, ha, hb⟩) :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » p.support)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » q.support)
