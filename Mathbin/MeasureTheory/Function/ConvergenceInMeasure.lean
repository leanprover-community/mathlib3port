...
theorem congrLeft (h : ∀ i, f i =ᵐ[μ] f' i) (h_tendsto : TendstoInMeasure μ f l g) : TendstoInMeasure μ f' l g :=
...
theorem congrRight (h : g =ᵐ[μ] g') (h_tendsto : TendstoInMeasure μ f l g) : TendstoInMeasure μ f l g' :=
...
theorem tendstoInMeasureOfTendstoAeOfStronglyMeasurable [IsFiniteMeasure μ] (hf : ∀ n, StronglyMeasurable (f n))
...
theorem tendstoInMeasureOfTendstoAe [IsFiniteMeasure μ] (hf : ∀ n, AeStronglyMeasurable (f n) μ)
  ...
  refine'
    tendsto_in_measure_of_tendsto_ae_of_strongly_measurable (fun i => (hf i).stronglyMeasurableMk)
...
theorem TendstoInMeasure.exists_seq_tendsto_ae (hfg : TendstoInMeasure μ f atTop g) :
  ...
  have h_tendsto : ∀ x ∈ sᶜ, tendsto (fun i => f (ns i) x) at_top (𝓝 (g x)) := by
    ...
    simp only [Set.supr_eq_Union, Set.infi_eq_Inter, Set.compl_Inter, Set.compl_Union, Set.mem_Union, Set.mem_Inter,
      Set.mem_compl_iff, Set.mem_set_of_eq, not_le] at hx
    obtain ⟨N, hNx⟩ := hx
...
theorem TendstoInMeasure.aeMeasurable {u : Filter ι} [NeBot u] [IsCountablyGenerated u] {f : ι → α → E} {g : α → E}
...
theorem tendstoInMeasureOfTendstoSnormOfNeTop (hp_ne_zero : p ≠ 0) (hp_ne_top : p ≠ ∞)
  ...
  refine'
    tendsto_in_measure_of_tendsto_snorm_of_strongly_measurable hp_ne_zero hp_ne_top
      (fun i => (hf i).stronglyMeasurableMk) hg.strongly_measurable_mk _
...
theorem tendstoInMeasureOfTendstoSnormTop {E} [NormedAddCommGroup E] {f : ι → α → E} {g : α → E} {l : Filter ι}
  ...
  congr with x
  simp only [Ennreal.of_real_le_iff_le_to_real ennreal.coe_lt_top.ne, Ennreal.coe_to_real, not_lt, coe_nnnorm,
...
/-- Convergence in Lp implies convergence in measure. -/
theorem tendstoInMeasureOfTendstoLp [hp : Fact (1 ≤ p)] {f : ι → lp E p μ} {g : lp E p μ} {l : Filter ι}
    (hfg : Tendsto f l (𝓝 g)) : TendstoInMeasure μ (fun n => f n) l g :=
  tendstoInMeasureOfTendstoSnorm (Ennreal.zero_lt_one.trans_le hp.elim).Ne.symm (fun n => lp.aeStronglyMeasurable _)
    (lp.aeStronglyMeasurable _) ((lp.tendsto_Lp_iff_tendsto_ℒp' _ _).mp hfg)
