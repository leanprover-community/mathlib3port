...
theorem bounded_continuous_function_dense [μ.weakly_regular] :
  ...
  by 
    ...
    intro ε hε 
    obtain ⟨η, hη_pos, hη_le⟩ : ∃ η, 0 < η ∧ («expr↑ » (∥bit0 ∥c∥∥₊*(2*η)^1 / p.to_real) : ℝ) ≤ ε
    ·
    ...
    have g_norm : ∀ x, ∥g x∥ = g x :=
      fun x =>
        by 
          rw [Real.norm_eq_abs, abs_of_nonneg (hg_range x).1]
    have gc_bd : ∀ x, ∥g x • c - s.indicator (fun x => c) x∥ ≤ ∥(u \ F).indicator (fun x => bit0 ∥c∥) x∥
    ·
      intro x 
      byCases' hu : x ∈ u
      ·
      ...
      ·
        ...
        simp [hgu hu, this]
    have gc_snorm :
      snorm ((fun x => g x • c) - s.indicator fun x => c) p μ ≤ («expr↑ » (∥bit0 ∥c∥∥₊*(2*η)^1 / p.to_real) : ℝ≥0∞)
    ·
      ...
      pushCast [←Ennreal.coe_rpow_of_nonneg _ hp₀']
    ...
    ·
      have  : mem_ℒp ((fun x => g x • c) - s.indicator fun x => c) p μ :=
        ⟨(gc_cont.ae_measurable μ).sub (measurable_const.indicator hs).AeMeasurable,
...
theorem to_Lp_dense_range [μ.weakly_regular] [is_finite_measure μ] :
  DenseRange («expr⇑ » (to_Lp p μ 𝕜 : (α →ᵇ E) →L[𝕜] Lp E p μ)) :=
  by 
    haveI  : NormedSpace ℝ E := RestrictScalars.normedSpace ℝ 𝕜 E 
    rw [dense_range_iff_closure_range]
    suffices  : (to_Lp p μ 𝕜 : _ →L[𝕜] Lp E p μ).range.toAddSubgroup.topologicalClosure = ⊤
    ·
...
theorem to_Lp_dense_range [CompactSpace α] [μ.weakly_regular] [is_finite_measure μ] :
  DenseRange («expr⇑ » (to_Lp p μ 𝕜 : C(α, E) →L[𝕜] Lp E p μ)) :=
  by 
    haveI  : NormedSpace ℝ E := RestrictScalars.normedSpace ℝ 𝕜 E 
    rw [dense_range_iff_closure_range]
    suffices  : (to_Lp p μ 𝕜 : _ →L[𝕜] Lp E p μ).range.toAddSubgroup.topologicalClosure = ⊤
