...
protected theorem aeStronglyMeasurable {f : ι → α → β} {p : ℝ≥0∞} (hf : UniformIntegrable f p μ)
  ...
  hf.1 i
#align
  measure_theory.uniform_integrable.ae_strongly_measurable MeasureTheory.UniformIntegrable.aeStronglyMeasurable

protected theorem unifIntegrable {f : ι → α → β} {p : ℝ≥0∞} (hf : UniformIntegrable f p μ) :
    UnifIntegrable f p μ :=
  hf.2.1
#align
  measure_theory.uniform_integrable.unif_integrable MeasureTheory.UniformIntegrable.unifIntegrable
...
theorem tendsto_indicator_ge (f : α → β) (x : α) :
  ...
  refine'
    @tendsto_at_top_of_eventually_const _ _ _ _ _ _ _ (Nat.ceil (‖f x‖₊ : ℝ) + 1) fun n hn => _
...
theorem Memℒp.integral_indicator_norm_ge_le (hf : Memℒp f 1 μ) (hmeas : StronglyMeasurable f)
  ...
  · refine' fun n => univ_mem' (id fun x => _)
    ...
    · dsimp
      ...
      · assumption
#align
  measure_theory.mem_ℒp.integral_indicator_norm_ge_le MeasureTheory.Memℒp.integral_indicator_norm_ge_le
...
theorem Memℒp.integral_indicator_norm_ge_nonneg_le (hf : Memℒp f 1 μ) {ε : ℝ} (hε : 0 < ε) :
  ...
  obtain ⟨M, hM_pos, hfM⟩ :=
    hf_mk.integral_indicator_norm_ge_nonneg_le_of_meas μ hf.1.stronglyMeasurableMk hε
...
theorem Memℒp.snorm_ess_sup_indicator_norm_ge_eq_zero (hf : Memℒp f ∞ μ)
...
theorem Memℒp.snorm_indicator_norm_ge_le (hf : Memℒp f p μ) (hmeas : StronglyMeasurable f) {ε : ℝ}
  ...
  · rw [Set.indicator_of_not_mem hx, Set.indicator_of_not_mem]
    ...
    · change ¬_ ≤ _
      rwa [← hiff]
#align
  measure_theory.mem_ℒp.snorm_indicator_norm_ge_le MeasureTheory.Memℒp.snorm_indicator_norm_ge_le
...
#align
  measure_theory.mem_ℒp.snorm_indicator_norm_ge_pos_le MeasureTheory.Memℒp.snorm_indicator_norm_ge_pos_le

end

/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:64:38: in filter_upwards #[[], [], []]: ./././Mathport/Syntax/Translate/Basic.lean:349:22: unsupported: parse error -/
...
theorem Memℒp.snorm_indicator_le_of_meas (hp_one : 1 ≤ p) (hp_top : p ≠ ∞) (hf : Memℒp f p μ)
  ...
  rw [Ennreal.of_real_div_of_pos zero_lt_two, (by norm_num : Ennreal.ofReal 2 = 2),
      Ennreal.mul_div_cancel'] <;>
    norm_num
#align
  measure_theory.mem_ℒp.snorm_indicator_le_of_meas MeasureTheory.Memℒp.snorm_indicator_le_of_meas
...
theorem tendsto_Lp_of_tendsto_ae_of_meas [IsFiniteMeasure μ] (hp : 1 ≤ p) (hp' : p ≠ ∞)
  ...
  have hlt : snorm (tᶜ.indicator (f n - g)) p μ ≤ Ennreal.ofReal (ε.to_real / 3) := by
    ...
    · refine' Ennreal.of_real_le_of_real (mul_le_of_le_one_right hε'.le _)
      ...
      · refine'
          Real.rpow_le_rpow Ennreal.to_real_nonneg
...
#align
  measure_theory.tendsto_Lp_of_tendsto_ae_of_meas MeasureTheory.tendsto_Lp_of_tendsto_ae_of_meas
...
theorem tendsto_Lp_of_tendsto_ae [IsFiniteMeasure μ] (hp : 1 ≤ p) (hp' : p ≠ ∞) {f : ℕ → α → β}
  ...
  refine'
    tendsto_Lp_of_tendsto_ae_of_meas μ hp hp' (fun n => (hf n).stronglyMeasurableMk)
      hg.1.stronglyMeasurableMk (hg.ae_eq hg.1.ae_eq_mk) (hui.ae_eq fun n => (hf n).ae_eq_mk) _
  ...
  exact
    ⟨ms,
      tendsto_Lp_of_tendsto_ae μ hp hp' (fun _ => hf _) hg
        ...
        hms'⟩
#align
  measure_theory.tendsto_Lp_of_tendsto_in_measure MeasureTheory.tendsto_Lp_of_tendsto_in_measure
...
theorem tendsto_in_measure_iff_tendsto_Lp [IsFiniteMeasure μ] (hp : 1 ≤ p) (hp' : p ≠ ∞)
...
#align
  measure_theory.tendsto_in_measure_iff_tendsto_Lp MeasureTheory.tendsto_in_measure_iff_tendsto_Lp
      ...
      refine'
        le_trans (Eq.le _)
          (snorm_add_le
            ...
            (strongly_measurable.ae_strongly_measurable
              ((hf i).indicator
                (hs.inter ((hf i).nnnorm.measurableSetLt strongly_measurable_const))))
            hp)
      congr
      change
        _ = fun x =>
...
theorem UniformIntegrable.spec' (hp : p ≠ 0) (hp' : p ≠ ∞) (hf : ∀ i, StronglyMeasurable (f i))
  ...
  obtain ⟨C, hC⟩ : ∃ C : ℝ≥0, ∀ i, μ { x | C ≤ ‖f i x‖₊ } ≤ Ennreal.ofReal δ := by
        ...
        C • (δ : ℝ≥0∞) ^ (1 / p.to_real) ≤ C • μ { x | C ≤ ‖f (ℐ C) x‖₊ } ^ (1 / p.to_real) := by
          ...
          refine'
            Ennreal.mul_le_mul le_rfl
              (Ennreal.rpow_le_rpow (hℐ C).le (one_div_nonneg.2 Ennreal.to_real_nonneg))
        _ ≤ snorm ({ x | C ≤ ‖f (ℐ C) x‖₊ }.indicator (f (ℐ C))) p μ := by
          refine'
            snorm_indicator_ge_of_bdd_below hp hp' _
              (measurableSetLe measurableConst (hf _).nnnorm.Measurable)
    ...
    rw [Ennreal.coe_rpow_of_nonneg _ (one_div_nonneg.2 Ennreal.to_real_nonneg), ← Ennreal.coe_smul,
