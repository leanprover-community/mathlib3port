...
theorem congr (hf : ae_measurable' m f μ) (hfg : f =ᵐ[μ] g) : ae_measurable' m g μ := by
...
theorem ae_measurable'_of_ae_measurable'_trim {α β} {m m0 m0' : MeasurableSpace α} [MeasurableSpace β] (hm0 : m0 ≤ m0')
    {μ : Measureₓ α} {f : α → β} (hf : ae_measurable' m f (μ.trim hm0)) : ae_measurable' m f μ := by
...
def Lp_meas_subgroup (m : MeasurableSpace α) [MeasurableSpace α] (p : ℝ≥0∞) (μ : Measureₓ α) :
...
theorem mem_Lp_meas_subgroup_iff_ae_measurable' {m m0 : MeasurableSpace α} {μ : Measureₓ α} {f : Lp F p μ} :
    f ∈ Lp_meas_subgroup F m p μ ↔ ae_measurable' m f μ := by
...
theorem mem_Lp_meas_iff_ae_measurable' {m m0 : MeasurableSpace α} {μ : Measureₓ α} {f : Lp F p μ} :
    f ∈ Lp_meas F 𝕜 m p μ ↔ ae_measurable' m f μ := by
...
theorem Lp_meas_coe {m m0 : MeasurableSpace α} {μ : Measureₓ α} {f : Lp_meas F 𝕜 m p μ} : ⇑f = (f : Lp F p μ) :=
  coe_fn_coe_base f

theorem mem_Lp_meas_indicator_const_Lp {m m0 : MeasurableSpace α} (hm : m ≤ m0) {μ : Measureₓ α} {s : Set α}
    (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) {c : F} : indicator_const_Lp p (hm s hs) hμs c ∈ Lp_meas F 𝕜 m p μ :=
...
theorem mem_ℒp_trim_of_mem_Lp_meas_subgroup (hm : m ≤ m0) (f : Lp F p μ) (hf_meas : f ∈ Lp_meas_subgroup F m p μ) :
    mem_ℒp (mem_Lp_meas_subgroup_iff_ae_measurable'.mp hf_meas).some p (μ.trim hm) := by
...
theorem mem_Lp_meas_subgroup_to_Lp_of_trim (hm : m ≤ m0) (f : Lp F p (μ.trim hm)) :
    (mem_ℒp_of_mem_ℒp_trim hm (Lp.mem_ℒp f)).toLp f ∈ Lp_meas_subgroup F m p μ := by
...
theorem Lp_meas_subgroup_to_Lp_trim_right_inv (hm : m ≤ m0) :
    Function.RightInverse (Lp_trim_to_Lp_meas_subgroup F p μ hm) (Lp_meas_subgroup_to_Lp_trim F p μ hm) := by
  intro f
  ext1
...
theorem Lp_meas_subgroup_to_Lp_trim_left_inv (hm : m ≤ m0) :
    Function.LeftInverse (Lp_trim_to_Lp_meas_subgroup F p μ hm) (Lp_meas_subgroup_to_Lp_trim F p μ hm) := by
  intro f
  ext1
  ext1
...
theorem Lp_meas_subgroup_to_Lp_trim_neg (hm : m ≤ m0) (f : Lp_meas_subgroup F m p μ) :
    Lp_meas_subgroup_to_Lp_trim F p μ hm (-f) = -Lp_meas_subgroup_to_Lp_trim F p μ hm f := by
  ext1
...
theorem Lp_meas_to_Lp_trim_smul (hm : m ≤ m0) (c : 𝕜) (f : Lp_meas F 𝕜 m p μ) :
    Lp_meas_to_Lp_trim F 𝕜 p μ hm (c • f) = c • Lp_meas_to_Lp_trim F 𝕜 p μ hm f := by
  ext1
...
theorem Lp_meas_subgroup_to_Lp_trim_norm_map [hp : Fact (1 ≤ p)] (hm : m ≤ m0) (f : Lp_meas_subgroup F m p μ) :
    ∥Lp_meas_subgroup_to_Lp_trim F p μ hm f∥ = ∥f∥ := by
...
theorem isometry_Lp_meas_subgroup_to_Lp_trim [hp : Fact (1 ≤ p)] (hm : m ≤ m0) :
    Isometry (Lp_meas_subgroup_to_Lp_trim F p μ hm) := by
...
def Lp_meas_subgroup_to_Lp_trim_iso [hp : Fact (1 ≤ p)] (hm : m ≤ m0) :
    Lp_meas_subgroup F m p μ ≃ᵢ Lp F p (μ.trim hm) where
  toFun := Lp_meas_subgroup_to_Lp_trim F p μ hm
  invFun := Lp_trim_to_Lp_meas_subgroup F p μ hm
...
def Lp_meas_to_Lp_trim_lie [hp : Fact (1 ≤ p)] (hm : m ≤ m0) : Lp_meas F 𝕜 m p μ ≃ₗᵢ[𝕜] Lp F p (μ.trim hm) where
  toFun := Lp_meas_to_Lp_trim F 𝕜 p μ hm
  invFun := Lp_trim_to_Lp_meas F 𝕜 p μ hm
  left_inv := Lp_meas_subgroup_to_Lp_trim_left_inv hm
  right_inv := Lp_meas_subgroup_to_Lp_trim_right_inv hm
...
instance [hm : Fact (m ≤ m0)] [CompleteSpace F] [hp : Fact (1 ≤ p)] : CompleteSpace (Lp_meas_subgroup F m p μ) := by
  rw [(Lp_meas_subgroup_to_Lp_trim_iso F p μ hm.elim).complete_space_iff]
  infer_instance

instance [hm : Fact (m ≤ m0)] [CompleteSpace F] [hp : Fact (1 ≤ p)] : CompleteSpace (Lp_meas F 𝕜 m p μ) := by
...
theorem is_complete_ae_measurable' [hp : Fact (1 ≤ p)] [CompleteSpace F] (hm : m ≤ m0) :
    IsComplete { f : Lp F p μ | ae_measurable' m f μ } := by
...
theorem Lp_meas.ae_eq_zero_of_forall_set_integral_eq_zero (hm : m ≤ m0) (f : Lp_meas E' 𝕜 m p μ) (hp_ne_zero : p ≠ 0)
    (hp_ne_top : p ≠ ∞) (hf_int_finite : ∀ s, measurable_set[m] s → μ s < ∞ → integrable_on f s μ)
...
theorem Lp.ae_eq_zero_of_forall_set_integral_eq_zero' (hm : m ≤ m0) (f : Lp E' p μ) (hp_ne_zero : p ≠ 0)
    (hp_ne_top : p ≠ ∞) (hf_int_finite : ∀ s, measurable_set[m] s → μ s < ∞ → integrable_on f s μ)
    (hf_zero : ∀ s : Set α, measurable_set[m] s → μ s < ∞ → (∫ x in s, f x ∂μ) = 0) (hf_meas : ae_measurable' m f μ) :
    f =ᵐ[μ] 0 := by
...
theorem Lp.ae_eq_of_forall_set_integral_eq' (hm : m ≤ m0) (f g : Lp E' p μ) (hp_ne_zero : p ≠ 0) (hp_ne_top : p ≠ ∞)
    ...
    (hf_meas : ae_measurable' m f μ) (hg_meas : ae_measurable' m g μ) : f =ᵐ[μ] g := by
...
theorem ae_eq_of_forall_set_integral_eq_of_sigma_finite' (hm : m ≤ m0) [sigma_finite (μ.trim hm)] {f g : α → F'}
    (hf_int_finite : ∀ s, measurable_set[m] s → μ s < ∞ → integrable_on f s μ)
    (hg_int_finite : ∀ s, measurable_set[m] s → μ s < ∞ → integrable_on g s μ)
    (hfg_eq : ∀ s : Set α, measurable_set[m] s → μ s < ∞ → (∫ x in s, f x ∂μ) = ∫ x in s, g x ∂μ)
    (hfm : ae_measurable' m f μ) (hgm : ae_measurable' m g μ) : f =ᵐ[μ] g := by
...
def condexp_L2 (hm : m ≤ m0) : (α →₂[μ] E) →L[𝕜] Lp_meas E 𝕜 m 2 μ :=
  @orthogonalProjection 𝕜 (α →₂[μ] E) _ _ (Lp_meas E 𝕜 m 2 μ)
    (have : Fact (m ≤ m0) := ⟨hm⟩
    inferInstance)
...
theorem snorm_condexp_L2_le (hm : m ≤ m0) (f : α →₂[μ] E) : snorm (condexp_L2 𝕜 hm f) 2 μ ≤ snorm f 2 μ := by
...
theorem norm_condexp_L2_coe_le (hm : m ≤ m0) (f : α →₂[μ] E) : ∥(condexp_L2 𝕜 hm f : α →₂[μ] E)∥ ≤ ∥f∥ := by
...
theorem condexp_L2_indicator_of_measurable (hm : m ≤ m0) (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) (c : E) :
    (condexp_L2 𝕜 hm (indicator_const_Lp 2 (hm s hs) hμs c) : α →₂[μ] E) = indicator_const_Lp 2 (hm s hs) hμs c := by
  rw [condexp_L2]
  have : Fact (m ≤ m0) := ⟨hm⟩
...
theorem inner_condexp_L2_eq_inner_fun (hm : m ≤ m0) (f g : α →₂[μ] E) (hg : ae_measurable' m g μ) :
    ⟪(condexp_L2 𝕜 hm f : α →₂[μ] E), g⟫₂ = ⟪f, g⟫₂ := by
  symm
...
theorem integral_condexp_L2_eq_of_fin_meas_real (f : Lp 𝕜 2 μ) (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) :
    (∫ x in s, condexp_L2 𝕜 hm f x ∂μ) = ∫ x in s, f x ∂μ := by
...
theorem lintegral_nnnorm_condexp_L2_le (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) (f : Lp ℝ 2 μ) :
    (∫⁻ x in s, ∥condexp_L2 ℝ hm f x∥₊ ∂μ) ≤ ∫⁻ x in s, ∥f x∥₊ ∂μ := by
...
theorem condexp_L2_ae_eq_zero_of_ae_eq_zero (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) {f : Lp ℝ 2 μ}
    (hf : f =ᵐ[μ.restrict s] 0) : condexp_L2 ℝ hm f =ᵐ[μ.restrict s] 0 := by
...
theorem lintegral_nnnorm_condexp_L2_indicator_le_real (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (ht : measurable_set[m] t)
    (hμt : μ t ≠ ∞) : (∫⁻ a in t, ∥condexp_L2 ℝ hm (indicator_const_Lp 2 hs hμs (1 : ℝ)) a∥₊ ∂μ) ≤ μ (s ∩ t) := by
...
theorem condexp_L2_const_inner (hm : m ≤ m0) (f : Lp E 2 μ) (c : E) :
    condexp_L2 𝕜 hm (((Lp.mem_ℒp f).const_inner c).toLp fun a => ⟪c, f a⟫) =ᵐ[μ] fun a => ⟪c, condexp_L2 𝕜 hm f a⟫ := by
  rw [Lp_meas_coe]
...
variable {E'' 𝕜' : Type _} [IsROrC 𝕜'] [MeasurableSpace E''] [InnerProductSpace 𝕜' E''] [BorelSpace E'']
  [second_countable_topology E''] [CompleteSpace E''] [NormedSpace ℝ E'']

variable (𝕜 𝕜')

theorem condexp_L2_comp_continuous_linear_map (hm : m ≤ m0) (T : E' →L[ℝ] E'') (f : α →₂[μ] E') :
    (condexp_L2 𝕜' hm (T.comp_Lp f) : α →₂[μ] E'') =ᵐ[μ] T.comp_Lp (condexp_L2 𝕜 hm f : α →₂[μ] E') := by
  refine'
    Lp.ae_eq_of_forall_set_integral_eq' hm _ _ ennreal.zero_lt_two.ne.symm Ennreal.coe_ne_top
...
theorem condexp_L2_indicator_ae_eq_smul (hm : m ≤ m0) (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : E') :
    condexp_L2 𝕜 hm (indicator_const_Lp 2 hs hμs x) =ᵐ[μ] fun a =>
      condexp_L2 ℝ hm (indicator_const_Lp 2 hs hμs (1 : ℝ)) a • x :=
  by
...
theorem condexp_L2_indicator_eq_to_span_singleton_comp (hm : m ≤ m0) (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : E') :
    (condexp_L2 𝕜 hm (indicator_const_Lp 2 hs hμs x) : α →₂[μ] E') =
      (to_span_singleton ℝ x).compLp (condexp_L2 ℝ hm (indicator_const_Lp 2 hs hμs (1 : ℝ))) :=
  by
  ext1
  rw [← Lp_meas_coe]
    ...
    (∫⁻ a in t, ∥condexp_L2 𝕜 hm (indicator_const_Lp 2 hs hμs x) a∥₊ ∂μ) =
      ...
      set_lintegral_congr_fun (hm t ht)
        ((condexp_L2_indicator_ae_eq_smul 𝕜 hm hs hμs x).mono fun a ha hat => by
          rw [ha])
    _ = (∫⁻ a in t, ∥condexp_L2 ℝ hm (indicator_const_Lp 2 hs hμs (1 : ℝ)) a∥₊ ∂μ) * ∥x∥₊ := by
...
theorem lintegral_nnnorm_condexp_L2_indicator_le (hm : m ≤ m0) (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : E')
    [sigma_finite (μ.trim hm)] : (∫⁻ a, ∥condexp_L2 𝕜 hm (indicator_const_Lp 2 hs hμs x) a∥₊ ∂μ) ≤ μ s * ∥x∥₊ := by
...
theorem integrable_condexp_L2_indicator (hm : m ≤ m0) [sigma_finite (μ.trim hm)] (hs : MeasurableSet s) (hμs : μ s ≠ ∞)
    (x : E') : integrable (condexp_L2 𝕜 hm (indicator_const_Lp 2 hs hμs x)) μ := by
...
theorem ae_measurable'_condexp_ind_smul (hm : m ≤ m0) (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : G) :
    ae_measurable' m (condexp_ind_smul hm hs hμs x) μ := by
...
theorem condexp_ind_smul_add (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x y : G) :
    condexp_ind_smul hm hs hμs (x + y) = condexp_ind_smul hm hs hμs x + condexp_ind_smul hm hs hμs y := by
...
theorem condexp_ind_smul_smul (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (c : ℝ) (x : G) :
    condexp_ind_smul hm hs hμs (c • x) = c • condexp_ind_smul hm hs hμs x := by
  simp_rw [condexp_ind_smul]
...
theorem lintegral_nnnorm_condexp_ind_smul_le (hm : m ≤ m0) (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : G)
    [sigma_finite (μ.trim hm)] : (∫⁻ a, ∥condexp_ind_smul hm hs hμs x a∥₊ ∂μ) ≤ μ s * ∥x∥₊ := by
  refine' lintegral_le_of_forall_fin_meas_le' hm (μ s * ∥x∥₊) _ fun t ht hμt => _
  · exact (Lp.ae_measurable _).nnnorm.coe_nnreal_ennreal
...
theorem integrable_condexp_ind_smul (hm : m ≤ m0) [sigma_finite (μ.trim hm)] (hs : MeasurableSet s) (hμs : μ s ≠ ∞)
    (x : G) : integrable (condexp_ind_smul hm hs hμs x) μ := by
  refine' integrable_of_forall_fin_meas_le' hm (μ s * ∥x∥₊) (Ennreal.mul_lt_top hμs Ennreal.coe_ne_top) _ _
  · exact Lp.ae_measurable _
...
theorem condexp_ind_smul_empty {x : G} :
    condexp_ind_smul hm MeasurableSet.empty ((@measure_empty _ _ μ).le.trans_lt Ennreal.coe_lt_top).Ne x = 0 := by
...
theorem set_integral_condexp_ind_smul (hs : measurable_set[m] s) (ht : MeasurableSet t) (hμs : μ s ≠ ∞) (hμt : μ t ≠ ∞)
  ...
  calc
    ...
    _ = (∫ a in s, indicator_const_Lp 2 ht hμt (1 : ℝ) a ∂μ) • x := by
...
variable {m m0 : MeasurableSpace α} {μ : Measureₓ α} {s t : Set α} [NormedSpace ℝ G]
...
theorem condexp_ind_L1_fin_add (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x y : G) :
    condexp_ind_L1_fin hm hs hμs (x + y) = condexp_ind_L1_fin hm hs hμs x + condexp_ind_L1_fin hm hs hμs y := by
  ext1
...
theorem condexp_ind_L1_fin_smul (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (c : ℝ) (x : G) :
    condexp_ind_L1_fin hm hs hμs (c • x) = c • condexp_ind_L1_fin hm hs hμs x := by
  ext1
  refine' (mem_ℒp.coe_fn_to_Lp _).trans _
  refine' eventually_eq.trans _ (Lp.coe_fn_smul _ _).symm
...
theorem condexp_ind_L1_fin_smul' [NormedSpace ℝ F] [SmulCommClass ℝ 𝕜 F] (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (c : 𝕜)
    (x : F) : condexp_ind_L1_fin hm hs hμs (c • x) = c • condexp_ind_L1_fin hm hs hμs x := by
  ext1
  refine' (mem_ℒp.coe_fn_to_Lp _).trans _
  refine' eventually_eq.trans _ (Lp.coe_fn_smul _ _).symm
  rw [condexp_ind_smul_smul' hs hμs c x]
...
theorem norm_condexp_ind_L1_fin_le (hs : MeasurableSet s) (hμs : μ s ≠ ∞) (x : G) :
    ∥condexp_ind_L1_fin hm hs hμs x∥ ≤ (μ s).toReal * ∥x∥ := by
...
theorem condexp_ind_L1_of_measure_eq_top (hμs : μ s = ∞) (x : G) : condexp_ind_L1 hm μ s x = 0 := by
...
theorem condexp_ind_L1_add (x y : G) :
    condexp_ind_L1 hm μ s (x + y) = condexp_ind_L1 hm μ s x + condexp_ind_L1 hm μ s y := by
...
theorem condexp_ind_L1_smul (c : ℝ) (x : G) : condexp_ind_L1 hm μ s (c • x) = c • condexp_ind_L1 hm μ s x := by
  by_cases' hs : MeasurableSet s
  swap
  · simp_rw [condexp_ind_L1_of_not_measurable_set hs]
    rw [smul_zero]
...
theorem condexp_ind_L1_smul' [NormedSpace ℝ F] [SmulCommClass ℝ 𝕜 F] (c : 𝕜) (x : F) :
    condexp_ind_L1 hm μ s (c • x) = c • condexp_ind_L1 hm μ s x := by
  by_cases' hs : MeasurableSet s
  swap
  · simp_rw [condexp_ind_L1_of_not_measurable_set hs]
    rw [smul_zero]
...
theorem norm_condexp_ind_L1_le (x : G) : ∥condexp_ind_L1 hm μ s x∥ ≤ (μ s).toReal * ∥x∥ := by
  by_cases' hs : MeasurableSet s
  swap
  · simp_rw [condexp_ind_L1_of_not_measurable_set hs]
    rw [Lp.norm_zero]
...
theorem condexp_ind_L1_disjoint_union (hs : MeasurableSet s) (ht : MeasurableSet t) (hμs : μ s ≠ ∞) (hμt : μ t ≠ ∞)
    (hst : s ∩ t = ∅) (x : G) : condexp_ind_L1 hm μ (s ∪ t) x = condexp_ind_L1 hm μ s x + condexp_ind_L1 hm μ t x := by
  have hμst : μ (s ∪ t) ≠ ∞ :=
...
theorem condexp_ind_ae_eq_condexp_ind_smul (hm : m ≤ m0) [sigma_finite (μ.trim hm)] (hs : MeasurableSet s)
    (hμs : μ s ≠ ∞) (x : G) : condexp_ind hm μ s x =ᵐ[μ] condexp_ind_smul hm hs hμs x := by
...
@[simp]
theorem condexp_ind_empty : condexp_ind hm μ ∅ = (0 : G →L[ℝ] α →₁[μ] G) := by
  ext1
  ext1
  refine'
...
theorem condexp_ind_disjoint_union (hs : MeasurableSet s) (ht : MeasurableSet t) (hμs : μ s ≠ ∞) (hμt : μ t ≠ ∞)
    (hst : s ∩ t = ∅) : (condexp_ind hm μ (s ∪ t) : G →L[ℝ] α →₁[μ] G) = condexp_ind hm μ s + condexp_ind hm μ t := by
  ext1
  push_cast
...
theorem condexp_ind_of_measurable (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) (c : G) :
    condexp_ind hm μ s c = indicator_const_Lp 1 (hm s hs) hμs c := by
  ext1
...
variable {m m0 : MeasurableSpace α} {μ : Measureₓ α} {hm : m ≤ m0} [sigma_finite (μ.trim hm)] {f g : α → F'} {s : Set α}
...
theorem set_integral_condexp_L1_clm_of_measure_ne_top (f : α →₁[μ] F') (hs : measurable_set[m] s) (hμs : μ s ≠ ∞) :
    (∫ x in s, condexp_L1_clm hm μ f x ∂μ) = ∫ x in s, f x ∂μ := by
  refine'
...
theorem ae_measurable'_condexp_L1_clm (f : α →₁[μ] F') : ae_measurable' m (condexp_L1_clm hm μ f) μ := by
...
theorem condexp_L1_clm_Lp_meas (f : Lp_meas F' ℝ m 1 μ) : condexp_L1_clm hm μ (f : α →₁[μ] F') = ↑f := by
...
theorem ae_measurable'_condexp_L1 {f : α → F'} : ae_measurable' m (condexp_L1 hm μ f) μ := by
...
theorem set_integral_condexp_L1 (hf : integrable f μ) (hs : measurable_set[m] s) :
    (∫ x in s, condexp_L1 hm μ f x ∂μ) = ∫ x in s, f x ∂μ := by
...
variable {𝕜} {m m0 : MeasurableSpace α} {μ : Measureₓ α} {hm : m ≤ m0} [sigma_finite (μ.trim hm)] {f g : α → F'}
  {s : Set α}
...
theorem condexp_const (c : F') [is_finite_measure μ] : μ[fun x : α => c|m,hm] = fun _ => c :=
  condexp_of_measurable (@measurable_const _ _ _ m _) (integrable_const c)

theorem condexp_ae_eq_condexp_L1 (f : α → F') : μ[f|m,hm] =ᵐ[μ] condexp_L1 hm μ f := by
  unfold condexp
...
theorem condexp_ae_eq_condexp_L1_clm (hf : integrable f μ) : μ[f|m,hm] =ᵐ[μ] condexp_L1_clm hm μ (hf.to_L1 f) := by
...
theorem ae_eq_condexp_of_forall_set_integral_eq (hm : m ≤ m0) [sigma_finite (μ.trim hm)] {f g : α → F'}
    ...
    (hgm : ae_measurable' m g μ) : g =ᵐ[μ] μ[f|m,hm] := by
  refine'
...
theorem condexp_condexp_of_le {m₁ m₂ m0 : MeasurableSpace α} {μ : Measureₓ α} (hm₁₂ : m₁ ≤ m₂) (hm₂ : m₂ ≤ m0)
    [sigma_finite (μ.trim (hm₁₂.trans hm₂))] [sigma_finite (μ.trim hm₂)] :
...
theorem rn_deriv_ae_eq_condexp {f : α → ℝ} (hf : integrable f μ) :
    signed_measure.rn_deriv ((μ.with_densityᵥ f).trim hm) (μ.trim hm) =ᵐ[μ] μ[f|m,hm] := by
