...
theorem snorm'_congr_norm_ae {f g : α → F} (hfg : ∀ᵐ x ∂μ, ∥f x∥ = ∥g x∥) : snorm' f q μ = snorm' g q μ :=
  by 
...
theorem snorm_mono_ae {f : α → F} {g : α → G} (h : ∀ᵐ x ∂μ, ∥f x∥ ≤ ∥g x∥) : snorm f p μ ≤ snorm g p μ :=
  by 
    simp only [snorm]
    splitIfs
    ·
...
theorem snorm_norm_rpow (f : α → F) (hq_pos : 0 < q) :
  ...
  by 
    ...
    ·
      ...
      have h_rpow : essSup (fun x : α => (nnnorm (∥f x∥^q) : ℝ≥0∞)) μ = essSup (fun x : α => ↑nnnorm (f x)^q) μ
      ·
        congr 
        ext1 x 
        nthRw 1[←nnnorm_norm]
...
theorem snorm'_le_snorm'_mul_rpow_measure_univ {p q : ℝ} (hp0_lt : 0 < p) (hpq : p ≤ q) {f : α → E}
  (hf : AeMeasurable f μ) : snorm' f p μ ≤ snorm' f q μ*μ Set.Univ^1 / p - 1 / q :=
  by 
    have hq0_lt : 0 < q 
    ...
    ·
      fieldSimp [(ne_of_ltₓ hp0_lt).symm, (ne_of_ltₓ hq0_lt).symm]
      ring 
    calc
      ...
      Ennreal.lintegral_Lp_mul_le_Lq_mul_Lr hp0_lt hpq hpqr μ hf.ennnorm
        ae_measurable_const _ = ((∫⁻ a : α, ↑nnnorm (f a)^q ∂μ)^1 / q)*μ Set.Univ^1 / p - 1 / q :=
      by 
        simp [hpqr]
...
theorem snorm'_const_smul {f : α → F} (c : 𝕜) (hq_pos : 0 < q) : snorm' (c • f) q μ = (nnnorm c : ℝ≥0∞)*snorm' f q μ :=
  by 
    ...
    suffices h_integral : (∫⁻ a, (↑nnnorm c^q)*↑nnnorm (f a)^q ∂μ) = ((nnnorm c : ℝ≥0∞)^q)*∫⁻ a, nnnorm (f a)^q ∂μ
    ·
...
theorem mem_ℒp.of_le_mul [MeasurableSpace F] {f : α → E} {g : α → F} {c : ℝ} (hg : mem_ℒp g p μ) (hf : AeMeasurable f μ)
  (hfg : ∀ᵐ x ∂μ, ∥f x∥ ≤ c*∥g x∥) : mem_ℒp f p μ :=
  by 
...
theorem norm_le_mul_norm_of_ae_le_mul [second_countable_topology F] [MeasurableSpace F] [BorelSpace F] {c : ℝ}
  {f : Lp E p μ} {g : Lp F p μ} (h : ∀ᵐ x ∂μ, ∥f x∥ ≤ c*∥g x∥) : ∥f∥ ≤ c*∥g∥ :=
  by 
    byCases' pzero : p = 0
    ·
      simp [pzero, norm_def]
...
theorem norm_le_norm_of_ae_le [second_countable_topology F] [MeasurableSpace F] [BorelSpace F] {f : Lp E p μ}
  {g : Lp F p μ} (h : ∀ᵐ x ∂μ, ∥f x∥ ≤ ∥g x∥) : ∥f∥ ≤ ∥g∥ :=
  by 
...
theorem snorm_ess_sup_indicator_const_eq (s : Set α) (c : G) (hμs : μ s ≠ 0) :
  snorm_ess_sup (s.indicator fun x : α => c) μ = ∥c∥₊ :=
  by 
...
theorem snorm_indicator_const {c : G} (hs : MeasurableSet s) (hp : p ≠ 0) (hp_top : p ≠ ∞) :
  ...
  by 
    ...
    have h_indicator_pow :
      (fun a : α => s.indicator (fun x : α => (∥c∥₊ : ℝ≥0∞)) a^p.to_real) = s.indicator fun x : α => ↑∥c∥₊^p.to_real
    ·
...
theorem coe_fn_neg_part_eq_max (f : Lp ℝ p μ) : ∀ᵐ a ∂μ, neg_part f a = max (-f a) 0 :=
  by 
    rw [neg_part]
...
private theorem lintegral_rpow_sum_coe_nnnorm_sub_le_rpow_tsum {f : ℕ → α → E} (hf : ∀ n, AeMeasurable (f n) μ) {p : ℝ}
  ...
  by 
    ...
    change ((∫⁻ a, (fun x => ↑nnnorm (∑ i in Finset.range (n+1), ∥f (i+1) x - f i x∥)^p) a ∂μ)^1 / p) ≤ ∑' i, B i at hn 
...
theorem ae_tendsto_of_cauchy_snorm' [CompleteSpace E] {f : ℕ → α → E} {p : ℝ} (hf : ∀ n, AeMeasurable (f n) μ)
  ...
  by 
    ...
    have h : ∀ᵐ x ∂μ, ∃ l : E, at_top.tendsto (fun n => ∑ i in Finset.range n, f (i+1) x - f i x) (𝓝 l)
    ·
