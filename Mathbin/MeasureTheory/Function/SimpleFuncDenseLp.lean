...
/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:75:38: in by_cases #[[expr hp_zero[has_mem.mem], ":", expr «expr = »(p, 0)]]: ./././Mathport/Syntax/Translate/Basic.lean:350:22: unsupported: too many args -/
theorem tendsto_approxOn_Lp_snorm [OpensMeasurableSpace E] {f : β → E} (hf : Measurable f)
  ...
  have hp : 0 < p.to_real := toReal_pos hp_zero hp_ne_top
  suffices Tendsto (fun n => ∫⁻ x, ‖approxOn f hf s y₀ h₀ n x - f x‖₊ ^ p.to_real ∂μ) atTop (𝓝 0)
    by
  ...
  have hF_meas : ∀ n, Measurable fun x => (‖approxOn f hf s y₀ h₀ n x - f x‖₊ : ℝ≥0∞) ^ p.to_real :=
    by
    simpa only [← edist_eq_coe_nnnorm_sub] using fun n =>
      (approxOn f hf s y₀ h₀ n).measurable_bind (fun y x => edist y (f x) ^ p.to_real) fun y =>
        (measurable_edist_right.comp hf).pow_const p.to_real
  -- (2) The functions "`p`-th power of distance between `f` and the approximation" are uniformly
  -- bounded, at any given point, by `λ x, ‖f x - y₀‖ ^ p.to_real`
  have h_bound :
    ∀ n,
      (fun x => (‖approxOn f hf s y₀ h₀ n x - f x‖₊ : ℝ≥0∞) ^ p.to_real) ≤ᵐ[μ] fun x =>
        ‖f x - y₀‖₊ ^ p.to_real :=
    fun n =>
  ...
  have h_lim :
    ∀ᵐ a : β ∂μ,
      Tendsto (fun n => (‖approxOn f hf s y₀ h₀ n a - f a‖₊ : ℝ≥0∞) ^ p.to_real) atTop (𝓝 0) :=
    by
...
theorem memℒpApproxOn [BorelSpace E] {f : β → E} {μ : Measure β} (fmeas : Measurable f)
  ...
  have : ∀ᵐ x ∂μ, ‖approxOn f fmeas s y₀ h₀ n x - y₀‖ ≤ ‖‖f x - y₀‖ + ‖f x - y₀‖‖ :=
    ...
    intro x
    convert norm_approxOn_y₀_le fmeas h₀ x n
...
theorem integrableApproxOn [BorelSpace E] {f : β → E} {μ : Measure β} (fmeas : Measurable f)
  ...
  exact memℒpApproxOn fmeas hf h₀ hi₀ n
...
theorem measure_lt_top_of_memℒp_indicator (hp_pos : p ≠ 0) (hp_ne_top : p ≠ ∞) {c : E} (hc : c ≠ 0)
  ...
  simpa only [memℒp_iff_finMeasSupp hp_pos hp_ne_top, finMeasSupp_iff_support, support_indicator,
...
protected def hasSmul : SMul 𝕜 (lp.simpleFunc E p μ) :=
  ⟨fun k f =>
    ⟨k • f, by
      ...
      use k • s
      apply Eq.trans (AeEqFun.smul_mk k s s.ae_strongly_measurable).symm _
      rw [hs]
      rfl⟩⟩
...
theorem toLp_smul (f : α →ₛ E) (hf : Memℒp f p μ) (c : 𝕜) :
    toLp (c • f) (hf.constSmul c) = c • toLp f hf :=
  rfl
...
theorem zero_toSimpleFunc : toSimpleFunc (0 : lp.simpleFunc E p μ) =ᵐ[μ] 0 :=
  by
  filter_upwards [toSimpleFunc_eq_to_fun (0 : lp.simpleFunc E p μ), lp.coeFn_zero E 1 μ]with _ h₁ _
  rwa [h₁]
...
theorem add_toSimpleFunc (f g : lp.simpleFunc E p μ) :
  ...
  by
  filter_upwards [toSimpleFunc_eq_to_fun (f + g), toSimpleFunc_eq_to_fun f,
    toSimpleFunc_eq_to_fun g, lp.coeFn_add (f : lp E p μ) g]with _
...
theorem neg_toSimpleFunc (f : lp.simpleFunc E p μ) : toSimpleFunc (-f) =ᵐ[μ] -toSimpleFunc f :=
  by
  filter_upwards [toSimpleFunc_eq_to_fun (-f), toSimpleFunc_eq_to_fun f,
...
theorem sub_toSimpleFunc (f g : lp.simpleFunc E p μ) :
  ...
  by
  filter_upwards [toSimpleFunc_eq_to_fun (f - g), toSimpleFunc_eq_to_fun f,
    toSimpleFunc_eq_to_fun g, lp.coeFn_sub (f : lp E p μ) g]with _
...
theorem smul_toSimpleFunc (k : 𝕜) (f : lp.simpleFunc E p μ) :
  ...
  by
  filter_upwards [toSimpleFunc_eq_to_fun (k • f), toSimpleFunc_eq_to_fun f,
...
protected theorem induction (hp_pos : p ≠ 0) (hp_ne_top : p ≠ ∞) {P : lp.simpleFunc E p μ → Prop}
  ...
  · intro f g hfg hf hg hfg'
    obtain ⟨hf', hg'⟩ : Memℒp f p μ ∧ Memℒp g p μ :=
      (memℒp_add_of_disjoint hfg f.strongly_measurable g.strongly_measurable).mp hfg'
...
protected theorem denseEmbedding (hp_ne_top : p ≠ ∞) :
  ...
  refine'
    ⟨fun n =>
      ↑(toLp
          (SimpleFunc.approxOn f (lp.stronglyMeasurable f).measurable (range f ∪ {0}) 0 (by simp) n)
          (SimpleFunc.memℒpApproxOnRange (lp.stronglyMeasurable f).Measurable hfi' n)),
...
theorem coeFn_le (f g : lp.simpleFunc G p μ) : f ≤ᵐ[μ] g ↔ f ≤ g := by
  rw [← Subtype.coe_le_coe, ← lp.coeFn_le, coeFn_coe_base', coeFn_coe_base' g]
...
instance : CovariantClass (lp.simpleFunc G p μ) (lp.simpleFunc G p μ) (· + ·) (· ≤ ·) :=
  ...
  have h_add_1 : ⇑(f + g₁) =ᵐ[μ] f + g₁ := lp.coeFn_add _ _
  have h_add_2 : ⇑(f + g₂) =ᵐ[μ] f + g₂ := lp.coeFn_add _ _
...
theorem exists_simpleFunc_nonneg_ae_eq {f : lp.simpleFunc G p μ} (hf : 0 ≤ f) :
  ...
  have hf_ae : 0 ≤ᵐ[μ] simpleFunc.toSimpleFunc f :=
    by
    filter_upwards [toSimpleFunc_eq_to_fun f, hf]with _ h1 _
    rwa [h1]
  let s := toMeasurable μ { x | ¬0 ≤ simpleFunc.toSimpleFunc f x }ᶜ
  have hs_zero : μ (sᶜ) = 0 := by
    rw [compl_compl, measure_toMeasurable]
    rwa [EventuallyLe, ae_iff] at hf_ae
  have hfs_nonneg : ∀ x ∈ s, 0 ≤ simpleFunc.toSimpleFunc f x :=
    by
    intro x hxs
...
theorem denseRange_coeSimpleFuncNonnegToLpNonneg [hp : Fact (1 ≤ p)] (hp_ne_top : p ≠ ∞) :
  ...
  intro g
  rw [mem_closure_iff_seq_limit]
  have hg_mem_ℒp : Memℒp g p μ := lp.memℒp g
  ...
  have : SeparableSpace ((range g ∪ {0}) ∩ { y | 0 ≤ y } : Set G) :=
    by
  ...
  have g_meas : Measurable g := (lp.stronglyMeasurable (g : lp G p μ)).measurable
  let x n := SimpleFunc.approxOn g g_meas ((range g ∪ {0}) ∩ { y | 0 ≤ y }) 0 zero_mem n
  ...
  have hx_mem_ℒp : ∀ n, Memℒp (x n) p μ :=
    SimpleFunc.memℒpApproxOn _ hg_mem_ℒp _ ⟨aeStronglyMeasurableConst, by simp⟩
  have h_to_Lp := fun n => Memℒp.coeFn_toLp (hx_mem_ℒp n)
  have hx_nonneg_Lp : ∀ n, 0 ≤ toLp (x n) (hx_mem_ℒp n) :=
    by
    intro n
    rw [← lp.simpleFunc.coeFn_le, coeFn_coe_base' (simpleFunc.toLp (x n) _),
      lp.simpleFunc.toLp_eq_toLp]
    have h0 := lp.simpleFunc.coeFn_zero p μ G
    filter_upwards [lp.simpleFunc.coeFn_zero p μ G, h_to_Lp n]with a ha0 ha_to_Lp
    rw [ha0, ha_to_Lp]
    exact hx_nonneg n a
  have hx_tendsto : Tendsto (fun n : ℕ => snorm (x n - g) p μ) atTop (𝓝 0) :=
    by
    apply SimpleFunc.tendsto_approxOn_Lp_snorm g_meas zero_mem hp_ne_top
    · have hg_nonneg : 0 ≤ᵐ[μ] g := (lp.coeFn_nonneg _).mpr g.2
      refine' hg_nonneg.mono fun a ha => subset_closure _
      simpa using ha
    · simp_rw [sub_zero]
      exact hg_mem_ℒp.snorm_lt_top
  refine'
    ⟨fun n => (coeSimpleFuncNonnegToLpNonneg p μ G) ⟨toLp (x n) (hx_mem_ℒp n), hx_nonneg_Lp n⟩,
      fun n => mem_range_self _, _⟩
  suffices Tendsto (fun n : ℕ => ↑(toLp (x n) (hx_mem_ℒp n))) atTop (𝓝 (g : lp G p μ))
    by
  ...
  convert hx_tendsto
  refine' funext fun n => snorm_congr_ae (EventuallyEq.sub _ _)
...
theorem lp.induction [_i : Fact (1 ≤ p)] (hp_ne_top : p ≠ ∞) (P : lp E p μ → Prop)
  ...
  refine' lp.simpleFunc.induction (lt_of_lt_of_le Ennreal.zero_lt_one _i.elim).ne' hp_ne_top _ _
  ...
  have : ∀ f : SimpleFunc α E, Memℒp f p μ → P f :=
    ...
    · intro f g hfg hf hg int_fg
      rw [SimpleFunc.coe_add,
        memℒp_add_of_disjoint hfg f.strongly_measurable g.strongly_measurable] at int_fg
      refine' h_add hfg int_fg.1 int_fg.2 (hf int_fg.1) (hg int_fg.2)
  have : ∀ f : lp.simpleFunc E p μ, P f := by
    intro f
    exact
      h_ae (lp.simpleFunc.toSimpleFunc_eq_to_fun f) (lp.simpleFunc.memℒp f)
...
theorem Integrable.induction (P : (α → E) → Prop)
  ...
  exact Memℒp.induction one_ne_top P h_ind h_add h_closed h_ae
