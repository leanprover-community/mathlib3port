...
theorem ae_seq_set_measurable_set {hf : ∀ i, AeMeasurable (f i) μ} :
    MeasurableSet (aeSeqSet hf p) :=
  (measurable_set_to_measurable _ _).compl
#align ae_seq.ae_seq_set_measurable_set aeSeq.ae_seq_set_measurable_set
