...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ∈ » «expr ∩ »(s, Ioi p))
    ...
    :=
      by
        have : Encodable s := s_count.to_encodable
          ...
          · intro i exact MeasurableSet.bInter s_count.mono inter_subset_left _ _ fun b hb => huv i b . 1
          let f' : α → β := fun x => ⨅ i : s , piecewise u' i fun x => ( i : β ) fun x => ( ⊤ : β ) x
          have f'_meas : Measurable f'
          · apply measurable_infi exact fun i => Measurable.piecewise u'_meas i measurable_const measurable_const
    ...
    :=
      by
        obtain
          ...
          have I : ∀ x _ : x ∈ s , x ≠ ∞ := fun x xs hx => s_top hx ▸ xs
