...
@[protectProj]
structure
  is_add_fundamental_domain(G :
    Type _){α : Type _}[HasZero G][HasVadd G α][MeasurableSpace α](s : Set α)(μ : Measureₓ α :=  by 
    runTac 
      volume_tac) :
  Prop where 
...
structure
  is_fundamental_domain(G :
    Type _){α : Type _}[HasOne G][HasScalar G α][MeasurableSpace α](s : Set α)(μ : Measureₓ α :=  by 
    runTac 
      volume_tac) :
  Prop where 
  MeasurableSet : MeasurableSet s 
...
-- error in MeasureTheory.Group.FundamentalDomain: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[to_additive #[]]
theorem pairwise_ae_disjoint
(h : is_fundamental_domain G s μ) : pairwise (λ
 g₁ g₂ : G, «expr = »(μ «expr ∩ »(«expr • »(g₁, s), «expr • »(g₂, s)), 0)) :=
λ g₁ g₂ hne, calc
  «expr = »(μ «expr ∩ »(«expr • »(g₁, s), «expr • »(g₂, s)), μ «expr • »(g₂, «expr ∩ »(«expr • »(«expr * »(«expr ⁻¹»(g₂), g₁), s), s))) : by rw ["[", expr smul_set_inter, ",", "<-", expr mul_smul, ",", expr mul_inv_cancel_left, "]"] []
  «expr = »(..., μ «expr ∩ »(«expr • »(«expr * »(«expr ⁻¹»(g₂), g₁), s), s)) : measure_smul_set _ _ _
  «expr = »(..., 0) : «expr $ »(h.ae_disjoint _, mt inv_mul_eq_one.1 hne.symm)
...
(hν : «expr ≪ »(ν, μ)) : «expr = »(sum (λ g : G, ν.restrict «expr • »(g, s)), ν) :=
by rw ["[", "<-", expr restrict_Union_ae «expr $ »(h.pairwise_ae_disjoint.mono, λ
  i
  j
  h, hν h) h.measurable_set_smul, ",", expr restrict_congr_set (hν h.Union_smul_ae_eq), ",", expr restrict_univ, "]"] []
