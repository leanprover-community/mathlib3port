...
theorem closedBall_ae_eq_ball {x : AddCircle T} {ε : ℝ} : closedBall x ε =ᵐ[volume] ball x ε :=
  ...
  · suffices volume (closedBall x ε) ≤ volume (ball x ε) by
      exact
        (ae_eq_of_subset_of_measure_ge ball_subset_closedBall this measurableSet_ball
...
theorem isAddFundamentalDomainOfAeBall (I : Set <| AddCircle T) (u x : AddCircle T)
  ...
  · -- `∀ (g : G), g ≠ 0 → ae_disjoint volume (g +ᵥ I) I`
    ...
    refine'
      AeDisjoint.congr (Disjoint.aeDisjoint _)
  ...
  · -- `volume univ ≤ ∑' (g : G), volume (g +ᵥ I)`
    replace hI : I =ᵐ[volume] closedBall x (T / (2 * ↑n)) := hI.trans closed_ball_ae_eq_ball.symm
    ...
    rw [AddCircle.measure_univ, tsum_fintype, Finset.sum_const, measure_congr hI, volume_closedBall,
      ← Ennreal.ofReal_nsmul, mul_div, mul_div_mul_comm, div_self (@two_ne_zero ℝ _ _ _ _), one_mul,
...
theorem volume_of_add_preimage_eq (s I : Set <| AddCircle T) (u x : AddCircle T)
  ...
  rw [(isAddFundamentalDomainOfAeBall I u x hu hI).measure_eq_card_smul_of_vadd_ae_eq_self s hsG,
