...
@[to_additive]
theorem MeasurableSet.constSmul {G α : Type _} [Group G] [MulAction G α] [MeasurableSpace G] [MeasurableSpace α]
    [HasMeasurableSmul G α] {s : Set α} (hs : MeasurableSet s) (a : G) : MeasurableSet (a • s) := by
  rw [← preimage_smul_inv]
  exact measurable_const_smul _ hs
#align measurable_set.const_smul MeasurableSet.constSmul

theorem MeasurableSet.constSmulOfNeZero {G₀ α : Type _} [GroupWithZero G₀] [MulAction G₀ α] [MeasurableSpace G₀]
    [MeasurableSpace α] [HasMeasurableSmul G₀ α] {s : Set α} (hs : MeasurableSet s) {a : G₀} (ha : a ≠ 0) :
    MeasurableSet (a • s) := by
  rw [← preimage_smul_inv₀ ha]
  exact measurable_const_smul _ hs
#align measurable_set.const_smul_of_ne_zero MeasurableSet.constSmulOfNeZero

theorem MeasurableSet.constSmul₀ {G₀ α : Type _} [GroupWithZero G₀] [Zero α] [MulActionWithZero G₀ α]
