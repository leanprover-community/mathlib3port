...
instance add [smul_invariant_measure M α μ] [smul_invariant_measure M α ν] : smul_invariant_measure M α (μ+ν) :=
  ⟨fun c s hs => show (_+_) = _+_ from congr_arg2 (·+·) (measure_preimage_smul μ c hs) (measure_preimage_smul ν c hs)⟩
...
variable(G){m :
    MeasurableSpace α}[Groupₓ G][MulAction G α][MeasurableSpace G][HasMeasurableSmul G α](c : G)(μ : Measureₓ α)
...
variable(G){μ}[TopologicalSpace G][TopologicalSpace α][HasContinuousSmul G α][MulAction.IsMinimal G α]{K U : Set α}
