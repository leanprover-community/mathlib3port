...
theorem measurable_set_integrable [SigmaFinite ν] ⦃f : α → β → E⦄
...
#align
  measure_theory.strongly_measurable.integral_prod_right MeasureTheory.StronglyMeasurable.integral_prod_right
...
#align
  measure_theory.strongly_measurable.integral_prod_left MeasureTheory.StronglyMeasurable.integral_prod_left
...
theorem integrableMeasureProdMkLeft {s : Set (α × β)} (hs : MeasurableSet s)
  ...
  by
  refine'
    ⟨(measurable_measure_prod_mk_left hs).ennreal_to_real.AeMeasurable.AeStronglyMeasurable, _⟩
...
theorem AeMeasurable.prodSwap [SigmaFinite μ] [SigmaFinite ν] {f : β × α → γ}
    (hf : AeMeasurable f (ν.Prod μ)) : AeMeasurable (fun z : α × β => f z.swap) (μ.Prod ν) :=
  by
  rw [← prod_swap] at hf
