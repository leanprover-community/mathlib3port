...
theorem IsCountablySpanning.prod {C : Set (Set α)} {D : Set (Set β)} (hC : IsCountablySpanning C)
  ...
  rw [unionᵢ_unpair_prod, h2s, h2t, univ_prod_univ]
...
theorem measurable_measure_prod_mk_left_finite [IsFiniteMeasure ν] {s : Set (α × β)}
  ...
  · intro f h1f h2f h3f
    simp_rw [preimage_unionᵢ]
    have : ∀ b, ν (⋃ i, Prod.mk b ⁻¹' f i) = ∑' i, ν (Prod.mk b ⁻¹' f i) := fun b =>
      measure_unionᵢ (fun i j hij => Disjoint.preimage _ (h1f hij)) fun i =>
...
theorem Measurable.lintegral_prod_right' [SigmaFinite ν] :
  ...
  · intro f hf h2f h3f
    ...
    simpa [lintegral_supᵢ fun n => (hf n).comp m, this]
...
noncomputable def FiniteSpanningSetsIn.prod {ν : Measure β} {C : Set (Set α)} {D : Set (Set β)}
  ...
  · simp_rw [unionᵢ_unpair_prod, hμ.spanning, hν.spanning, univ_prod_univ]
...
theorem skewProduct [SigmaFinite μb] [SigmaFinite μd] {f : α → β} (hf : MeasurePreserving f μa μb)
  ...
  classical
    ...
    have : SigmaFinite μc :=
      ...
      exact SigmaFinite.ofMap _ hgm.of_uncurry_left.ae_measurable (by rwa [hx])
...
theorem prodOfLeft {α β γ} [MeasurableSpace α] [MeasurableSpace β] [MeasurableSpace γ]
  ...
  rw [← prod_swap]
  convert
    (QuasiMeasurePreserving.prodOfRight (hf.comp measurable_swap) h2f).comp
...
theorem lintegral_prod_of_measurable :
  ...
  · intro f hf h2f h3f
    ...
    simp only [lintegral_supᵢ hf h2f, lintegral_supᵢ (kf _), k2f, lintegral_supᵢ lf l2f, h3f]
...
theorem lintegral_lintegral_symm [SigmaFinite μ] ⦃f : α → β → ℝ≥0∞⦄
  ...
  (lintegral_prod_symm _ hf.prodSwap).symm
...
theorem Integrable.swap [SigmaFinite μ] ⦃f : α × β → E⦄ (hf : Integrable f (μ.prod ν)) :
  ...
  ⟨hf.aeStronglyMeasurable.prodSwap,
...
theorem hasFiniteIntegral_prod_iff' ⦃f : α × β → E⦄ (h1f : AeStronglyMeasurable f (μ.prod ν)) :
    HasFiniteIntegral f (μ.prod ν) ↔
      (∀ᵐ x ∂μ, HasFiniteIntegral (fun y => f (x, y)) ν) ∧
        HasFiniteIntegral (fun x => ∫ y, ‖f (x, y)‖ ∂ν) μ :=
  by
  rw [hasFiniteIntegral_congr h1f.ae_eq_mk, hasFiniteIntegral_prod_iff h1f.strongly_measurable_mk]
  apply and_congr
  ...
  · apply hasFiniteIntegral_congr
    filter_upwards [ae_ae_of_ae_prod
        h1f.ae_eq_mk.symm]with _ hx using integral_congr_ae (EventuallyEq.fun_comp hx _)
  · infer_instance
#align measure_theory.has_finite_integral_prod_iff' MeasureTheory.hasFiniteIntegral_prod_iff'
...
theorem continuous_integral_integral :
  ...
  refine'
    tendsto_integral_of_L1 _ (L1.integrableCoeFn g).integralProdLeft
      (eventually_of_forall fun h => (L1.integrableCoeFn h).integralProdLeft) _
  simp_rw [←
    lintegral_fn_integral_sub (fun x => (‖x‖₊ : ℝ≥0∞)) (L1.integrableCoeFn _)
      (L1.integrableCoeFn g)]
...
theorem integral_prod_mul {L : Type _} [IsROrC L] (f : α → L) (g : β → L) :
  ...
  have H : ¬Integrable f μ ∨ ¬Integrable g ν :=
    by
    contrapose! h
    exact integrableProdMul h.1 h.2
