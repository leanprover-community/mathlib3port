...
theorem
  ...
  [ TopologicalSpace α ] : borel α = generate_from { s | IsClosed s }
  :=
    le_antisymmₓ
      generate_from_le
          $
          fun
            t ht
              ...
              @ MeasurableSet.of_compl
                α _ generate_from { s | IsClosed s } generate_measurable.basic _ $ is_closed_compl_iff . 2 ht
        generate_from_le
          $
      ...
      refine' le_antisymmₓ _ generate_from_le _
        ·
          rw [ borel_eq_generate_from_of_subbasis @ OrderTopology.topology_eq_generate_intervals α _ _ _ ]
            let this' : MeasurableSpace α := MeasurableSpace.generateFrom range Iio
            ...
            byCases' h : ∃ a' , ∀ b , a < b ↔ a' ≤ b
            · rcases h with ⟨ a' , ha' ⟩ rw [ ( _ : Ioi a = Iio a' ᶜ ) ] · exact H _ . Compl simp [ Set.ext_iff , ha' ]
            ·
              rcases
                  is_open_Union_countable fun a' : { a' : α // a < a' } => { b | a' . 1 < b } fun a' => is_open_lt' _
                  with ⟨ v , ⟨ hv ⟩ , vu ⟩
        ...
        · rw [ forall_range_iff ] intro a exact generate_measurable.basic _ is_open_Iio
...
instance (priority := 100) OrderDual.opens_measurable_space {α : Type _} [TopologicalSpace α] [MeasurableSpace α]
  [h : OpensMeasurableSpace α] : OpensMeasurableSpace (OrderDual α) :=
  { borel_le := h.borel_le }

instance (priority := 100) OrderDual.borel_space {α : Type _} [TopologicalSpace α] [MeasurableSpace α]
  [h : BorelSpace α] : BorelSpace (OrderDual α) :=
...
theorem
  ...
  { β } [ EmetricSpace β ] ( f : α → β ) : MeasurableSet { x | ContinuousAt f x }
...
theorem MeasurableSet.nhds_within_is_measurably_generated {s : Set α} (hs : MeasurableSet s) (a : α) :
  ...
  by 
    have  := hs.principal_is_measurably_generated <;> exact Filter.inf_is_measurably_generated _ _
...
-- failed to format: format: uncaught backtrack exception
instance
  Pi.opens_measurable_space_encodable
  { ι : Type _ }
      { π : ι → Type _ }
      [ Encodable ι ]
      [ t' : ∀ i , TopologicalSpace π i ]
      [ ∀ i , MeasurableSpace π i ]
      [ ∀ i , second_countable_topology π i ]
      [ ∀ i , OpensMeasurableSpace π i ]
    : OpensMeasurableSpace ∀ i , π i
  :=
    by
      constructor
        have
              ...
              generate_from
                {
                  t
                  |
                  ∃ ( s : ∀ a , Set π a ) ( i : Finset ι ) , ∀ a _ : a ∈ i , s a ∈ countable_basis π a ∧ t = pi ↑ i s
                  }
...
instance Pi.opens_measurable_space_fintype {ι : Type _} {π : ι → Type _} [Fintype ι] [t' : ∀ i, TopologicalSpace (π i)]
  [∀ i, MeasurableSpace (π i)] [∀ i, second_countable_topology (π i)] [∀ i, OpensMeasurableSpace (π i)] :
  OpensMeasurableSpace (∀ i, π i) :=
  by 
...
@[ measurability ]
  theorem
    measurable_set_le
    { f g : δ → α } ( hf : Measurable f ) ( hg : Measurable g ) : MeasurableSet { a | f a ≤ g a }
...
instance nhds_within_interval_is_measurably_generated :
  is_measurably_generated (𝓝["././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"] x) :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » s)
theorem Set.OrdConnected.measurable_set (h : ord_connected s) : MeasurableSet s :=
  by 
    let u := ⋃ (x : _)(_ : x ∈ s)(y : _)(_ : y ∈ s), Ioo x y 
...
theorem
  ...
  { α : Type _ } [ TopologicalSpace α ] [ LinearOrderₓ α ] [ OrderClosedTopology α ] ( s t : Set α )
    :
      MeasurableSpace.generateFrom { S | ∃ ( l : _ ) ( _ : l ∈ s ) ( u : _ ) ( _ : u ∈ t ) ( h : l < u ) , Ico l u = S }
        ≤
        borel α
  :=
    by
      apply generate_from_le
        let this' : MeasurableSpace α := borel α
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (l «expr ∈ » t)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (u «expr ∈ » t)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (l «expr ∈ » t)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (l «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (u «expr ∈ » s)
...
-- failed to format: format: uncaught backtrack exception
theorem
  Dense.borel_eq_generate_from_Ico_mem_aux
  { α : Type _ }
      [ TopologicalSpace α ]
      [ LinearOrderₓ α ]
      [ OrderTopology α ]
    ...
    :
      borel α
        =
        generate_from { S : Set α | ∃ ( l : _ ) ( _ : l ∈ s ) ( u : _ ) ( _ : u ∈ s ) ( h : l < u ) , Ico l u = S }
  :=
    by
      set S : Set Set α := { S | ∃ ( l : _ ) ( _ : l ∈ s ) ( u : _ ) ( _ : u ∈ s ) ( h : l < u ) , Ico l u = S }
        refine' le_antisymmₓ _ generate_from_Ico_mem_le_borel _ _
        let this' : MeasurableSpace α := generate_from S
        ...
        byCases' ha : ∀ b _ : b < a , Ioo b a . Nonempty
        ·
          convertTo
              MeasurableSet ⋃ ( l : _ ) ( _ : l ∈ t ) ( u : _ ) ( _ : u ∈ t ) ( hlu : l < u ) ( hu : u ≤ a ) , Ico l u
            ·
              ext y
                ...
                · rintro ⟨ l , - , u , - , - , hua , - , hyu ⟩ exact hyu.trans_le hua
            ·
        ...
        ·
          simp only [ not_forall , not_nonempty_iff_eq_empty ] at ha
            ...
            convertTo MeasurableSet ⋃ ( l : _ ) ( _ : l ∈ t ) ( hl : l < a ) , Ico l a
            ·
              symm
                simp
                  only
...
theorem ext_of_Ico' {α : Type _} [TopologicalSpace α] {m : MeasurableSpace α} [second_countable_topology α]
  ...
  (hμ : ∀ ⦃a b⦄, a < b → μ (Ico a b) ≠ ∞) (h : ∀ ⦃a b⦄, a < b → μ (Ico a b) = ν (Ico a b)) : μ = ν :=
  by 
    rcases exists_countable_dense_bot_top α with ⟨s, hsc, hsd, hsb, hst⟩
    have  : countable (⋃ (l : _)(_ : l ∈ s)(u : _)(_ : u ∈ s)(h : l < u), {Ico l u} : Set (Set α))
...
intervals. -/
theorem ext_of_Iic {α : Type _} [TopologicalSpace α] {m : MeasurableSpace α} [second_countable_topology α]
...
instance (priority := 100) HasContinuousSup.has_measurable_sup [HasSup γ] [HasContinuousSup γ] : HasMeasurableSup γ :=
...
instance (priority := 100) HasContinuousSup.has_measurable_sup₂ [second_countable_topology γ] [HasSup γ]
  [HasContinuousSup γ] : HasMeasurableSup₂ γ :=
  ⟨continuous_sup.Measurable⟩

instance (priority := 100) HasContinuousInf.has_measurable_inf [HasInf γ] [HasContinuousInf γ] : HasMeasurableInf γ :=
...
instance (priority := 100) HasContinuousInf.has_measurable_inf₂ [second_countable_topology γ] [HasInf γ]
...
theorem pi_le_borel_pi {ι : Type _} {π : ι → Type _} [∀ i, TopologicalSpace (π i)] [∀ i, MeasurableSpace (π i)]
  [∀ i, BorelSpace (π i)] : MeasurableSpace.pi ≤ borel (∀ i, π i) :=
  by 
...
private
  theorem
    ae_measurable.is_lub_of_nonempty
    { ι }
        ( hι : Nonempty ι )
        { μ : Measureₓ δ }
        [ Encodable ι ]
        ...
        ( hg : ∀ᵐ b ∂ μ , IsLub { a | ∃ i , f i b = a } g b )
      : AeMeasurable g μ
    :=
      by
...
private
  theorem
    ae_measurable.is_glb_of_nonempty
    { ι }
        ( hι : Nonempty ι )
        { μ : Measureₓ δ }
        [ Encodable ι ]
        ...
        { g : δ → α }
        ( hf : ∀ i , AeMeasurable f i μ )
        ( hg : ∀ᵐ b ∂ μ , IsGlb { a | ∃ i , f i b = a } g b )
      : AeMeasurable g μ
    :=
      by
...
theorem measurable_bsupr {ι} (s : Set ι) {f : ι → δ → α} (hs : countable s) (hf : ∀ i, Measurable (f i)) :
  Measurable fun b => ⨆ (i : _)(_ : i ∈ s), f i b :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » s)
theorem ae_measurable_bsupr {ι} {μ : Measureₓ δ} (s : Set ι) {f : ι → δ → α} (hs : countable s)
  (hf : ∀ i, AeMeasurable (f i) μ) : AeMeasurable (fun b => ⨆ (i : _)(_ : i ∈ s), f i b) μ :=
  by 
    have  : Encodable s := hs.to_encodable 
    simp only [supr_subtype']
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » s)
theorem measurable_binfi {ι} (s : Set ι) {f : ι → δ → α} (hs : countable s) (hf : ∀ i, Measurable (f i)) :
  Measurable fun b => ⨅ (i : _)(_ : i ∈ s), f i b :=
  by 
    have  : Encodable s := hs.to_encodable 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » s)
theorem ae_measurable_binfi {ι} {μ : Measureₓ δ} (s : Set ι) {f : ι → δ → α} (hs : countable s)
  (hf : ∀ i, AeMeasurable (f i) μ) : AeMeasurable (fun b => ⨅ (i : _)(_ : i ∈ s), f i b) μ :=
  by 
    have  : Encodable s := hs.to_encodable 
    simp only [infi_subtype']
    ...
    ·
      ...
      ·
        ...
        ·
          ...
          calc f x < (t^i+1) := hx.2.2_ ≤ (t^j) := Ennreal.zpow_le_of_le (Ennreal.one_le_coe_iff.2 ht.le) hij _ ≤ f x :=
            h'x.2.1
        ·
          intro hij 
          rw [Disjoint.comm]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr > » a)
theorem borel_eq_generate_from_Iio_rat : borel ℝ = generate_from (⋃ a : ℚ, {Iio a}) :=
  by 
    ...
    ·
      ...
      simp only [mem_Union, mem_singleton_iff]
      rintro ⟨a, b, h, rfl⟩
      rw [(Set.ext fun x => _ : Ioo (a : ℝ) b = (⋃ (c : _)(_ : c > a), Iio cᶜ) ∩ Iio b)]
      ·
...
theorem measurable_of_tendsto_nnreal' {ι} {f : ι → α →  ℝ≥0 } {g : α →  ℝ≥0 } (u : Filter ι) [ne_bot u]
  [is_countably_generated u] (hf : ∀ i, Measurable (f i)) (lim : tendsto f u (𝓝 g)) : Measurable g :=
  by 
    rcases u.exists_seq_tendsto with ⟨x, hx⟩
    rw [tendsto_pi_nhds] at lim 
    rw [←measurable_coe_nnreal_ennreal_iff]
    have  : ∀ y, (liminf at_top fun n => (f (x n) y : ℝ≥0∞)) = (g y : ℝ≥0∞) :=
      fun y => ((ennreal.continuous_coe.tendsto (g y)).comp$ (limₓ y).comp hx).liminf_eq 
    simp only [←this]
...
theorem measurable_of_tendsto_metric' {ι} {f : ι → α → β} {g : α → β} (u : Filter ι) [ne_bot u]
  [is_countably_generated u] (hf : ∀ i, Measurable (f i)) (lim : tendsto f u (𝓝 g)) : Measurable g :=
  by 
    apply measurable_of_is_closed' 
    intro s h1s h2s h3s 
...
theorem ae_measurable_of_tendsto_metric_ae {μ : Measureₓ α} {f : ℕ → α → β} {g : α → β} (hf : ∀ n, AeMeasurable (f n) μ)
  (h_ae_tendsto : ∀ᵐ x ∂μ, tendsto (fun n => f n x) at_top (𝓝 (g x))) : AeMeasurable g μ :=
  by 
    let p : α → (ℕ → β) → Prop := fun x f' => filter.at_top.tendsto (fun n => f' n) (𝓝 (g x))
    let hp : ∀ᵐ x ∂μ, p x fun n => f n x 
    exact h_ae_tendsto 
...
theorem measurable_of_tendsto_metric_ae {μ : Measureₓ α} [μ.is_complete] {f : ℕ → α → β} {g : α → β}
  (hf : ∀ n, Measurable (f n)) (h_ae_tendsto : ∀ᵐ x ∂μ, Filter.atTop.Tendsto (fun n => f n x) (𝓝 (g x))) :
  Measurable g :=
  ae_measurable_iff_measurable.mp (ae_measurable_of_tendsto_metric_ae (fun i => (hf i).AeMeasurable) h_ae_tendsto)

theorem measurable_limit_of_tendsto_metric_ae {μ : Measureₓ α} {f : ℕ → α → β} (hf : ∀ n, AeMeasurable (f n) μ)
  (h_ae_tendsto : ∀ᵐ x ∂μ, ∃ l : β, Filter.atTop.Tendsto (fun n => f n x) (𝓝 l)) :
  ∃ (f_lim : α → β)(hf_lim_meas : Measurable f_lim), ∀ᵐ x ∂μ, Filter.atTop.Tendsto (fun n => f n x) (𝓝 (f_lim x)) :=
  by 
    let p : α → (ℕ → β) → Prop := fun x f' => ∃ l : β, filter.at_top.tendsto (fun n => f' n) (𝓝 l)
    have hp_mem : ∀ x, x ∈ AeSeqSet hf p → p x fun n => f n x 
    exact fun x hx => aeSeq.fun_prop_of_mem_ae_seq_set hf hx 
    have hμ_compl : μ (AeSeqSet hf pᶜ) = 0 
    exact aeSeq.measure_compl_ae_seq_set_eq_zero hf h_ae_tendsto 
    let f_lim : α → β :=
      fun x => dite (x ∈ AeSeqSet hf p) (fun h => (hp_mem x h).some) fun h => (⟨f 0 x⟩ : Nonempty β).some 
    have hf_lim_conv : ∀ x, x ∈ AeSeqSet hf p → filter.at_top.tendsto (fun n => f n x) (𝓝 (f_lim x))
    ·
      intro x hx_conv 
      simp only [f_lim, hx_conv, dif_pos]
      exact (hp_mem x hx_conv).some_spec 
    have hf_lim : ∀ x, filter.at_top.tendsto (fun n => aeSeq hf p n x) (𝓝 (f_lim x))
    ·
      intro x 
      ...
      ·
        exact tendsto_const_nhds 
    have h_ae_tendsto_f_lim : ∀ᵐ x ∂μ, filter.at_top.tendsto (fun n => f n x) (𝓝 (f_lim x))
    ·
      refine' le_antisymmₓ (le_of_eqₓ (measure_mono_null _ hμ_compl)) (zero_le _)
      exact set.compl_subset_compl.mpr fun x hx => hf_lim_conv x hx 
    have h_f_lim_meas : Measurable f_lim 
    exact measurable_of_tendsto_metric (aeSeq.measurable hf p) (tendsto_pi_nhds.mpr fun x => hf_lim x)
