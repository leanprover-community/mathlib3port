...
theorem borel_eq_generate_from_is_closed [TopologicalSpace α] : borel α = generate_from { s | IsClosed s } :=
  le_antisymmₓ
    ...
    (generate_from_le $ fun t ht =>
      @MeasurableSet.of_compl α _ (borel α) (generate_measurable.basic _ $ is_open_compl_iff.2 ht))
...
theorem borel_eq_generate_from_Iio : borel α = generate_from (range Iio) := by
...
theorem borel_eq_generate_from_Ioi : borel α = generate_from (range Ioi) :=
  @borel_eq_generate_from_Iio (OrderDual α) _
    (by
      infer_instance : second_countable_topology α)
    _ _
...
instance Pi.opens_measurable_space_encodable {ι : Type _} {π : ι → Type _} [Encodable ι]
    [t' : ∀ i, TopologicalSpace (π i)] [∀ i, MeasurableSpace (π i)] [∀ i, second_countable_topology (π i)]
...
instance nhds_within_Icc_is_measurably_generated : is_measurably_generated (𝓝[Icc a b] x) := by
...
theorem bsupr_measure_Iic {μ : Measureₓ α} {s : Set α} (hsc : countable s) (hst : ∀ x : α, ∃ y ∈ s, x ≤ y)
...
instance nhds_within_interval_is_measurably_generated :
    is_measurably_generated (𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)"] x) :=
...
theorem Set.OrdConnected.measurable_set (h : ord_connected s) : MeasurableSet s := by
...
theorem Dense.borel_eq_generate_from_Ico_mem_aux {α : Type _} [TopologicalSpace α] [LinearOrderₓ α] [OrderTopology α]
    [second_countable_topology α] {s : Set α} (hd : Dense s) (hbot : ∀ x, IsBot x → x ∈ s)
    (hIoo : ∀ x y : α, x < y → Ioo x y = ∅ → y ∈ s) :
    borel α = generate_from { S : Set α | ∃ l ∈ s, ∃ u ∈ s, ∃ h : l < u, Ico l u = S } := by
...
theorem borel_eq_generate_from_Ico (α : Type _) [TopologicalSpace α] [second_countable_topology α] [LinearOrderₓ α]
    [OrderTopology α] : borel α = generate_from { S : Set α | ∃ (l u : _)(h : l < u), Ico l u = S } := by
...
theorem Dense.borel_eq_generate_from_Ioc_mem_aux {α : Type _} [TopologicalSpace α] [LinearOrderₓ α] [OrderTopology α]
    ...
    borel α = generate_from { S : Set α | ∃ l ∈ s, ∃ u ∈ s, ∃ h : l < u, Ioc l u = S } := by
...
theorem borel_eq_generate_from_Ioc (α : Type _) [TopologicalSpace α] [second_countable_topology α] [LinearOrderₓ α]
    [OrderTopology α] : borel α = generate_from { S : Set α | ∃ (l u : _)(h : l < u), Ioc l u = S } := by
  simpa only [exists_prop, mem_univ, true_andₓ] using
...
instance Pi.borel_space_fintype_encodable {ι : Type _} {π : ι → Type _} [Encodable ι] [t' : ∀ i, TopologicalSpace (π i)]
    [∀ i, MeasurableSpace (π i)] [∀ i, second_countable_topology (π i)] [∀ i, BorelSpace (π i)] :
    BorelSpace (∀ i, π i) :=
...
theorem measurable_liminf' {ι ι'} {f : ι → δ → α} {u : Filter ι} (hf : ∀ i, Measurable (f i)) {p : ι' → Prop}
    ...
    Measurable fun x => liminf u fun i => f i x := by
...
theorem measurable_limsup' {ι ι'} {f : ι → δ → α} {u : Filter ι} (hf : ∀ i, Measurable (f i)) {p : ι' → Prop}
    {s : ι' → Set ι} (hu : u.has_countable_basis p s) (hs : ∀ i, (s i).Countable) :
    Measurable fun x => limsup u fun i => f i x := by
...
protected theorem IsFiniteMeasureOnCompacts.map {α : Type _} {m0 : MeasurableSpace α} [TopologicalSpace α]
    ...
    (μ : Measureₓ α) [is_finite_measure_on_compacts μ] (f : α ≃ₜ β) : is_finite_measure_on_compacts (measure.map f μ) :=
  ⟨by
    intro K hK
  ...
  have C : μ (s ∩ f ⁻¹' Ioo 0 ∞) = ∑' n : ℤ, μ (s ∩ f ⁻¹' Ico (t ^ n) (t ^ (n + 1))) := by
    ...
    · intro i j
      ...
      · intro hij
        ...
        calc f x < t ^ (i + 1) := hx.2.2_ ≤ t ^ j :=
            Ennreal.zpow_le_of_le (Ennreal.one_le_coe_iff.2 ht.le) hij _ ≤ f x := h'x.2.1
...
theorem tendsto_measure_cthickening_of_is_compact [ProperSpace α] {μ : Measureₓ α} [is_finite_measure_on_compacts μ]
    {s : Set α} (hs : IsCompact s) : tendsto (fun r => μ (cthickening r s)) (𝓝 0) (𝓝 (μ s)) :=
  tendsto_measure_cthickening_of_is_closed ⟨1, zero_lt_one, (bounded.measure_lt_top hs.bounded.cthickening).Ne⟩
    hs.is_closed
...
theorem borel_eq_generate_from_Ioo_rat : borel ℝ = generate_from (⋃ (a : ℚ) (b : ℚ) (h : a < b), {Ioo a b}) :=
...
def finite_spanning_sets_in_Ioo_rat (μ : Measureₓ ℝ) [is_locally_finite_measure μ] :
    μ.finite_spanning_sets_in (⋃ (a : ℚ) (b : ℚ) (h : a < b), {Ioo a b}) where
...
theorem measure_ext_Ioo_rat {μ ν : Measureₓ ℝ} [is_locally_finite_measure μ]
  ...
  (finite_spanning_sets_in_Ioo_rat μ).ext borel_eq_generate_from_Ioo_rat is_pi_system_Ioo_rat $ by
    simp only [mem_Union, mem_singleton_iff]
    rintro _ ⟨a, b, -, rfl⟩
    apply h

theorem borel_eq_generate_from_Iio_rat : borel ℝ = generate_from (⋃ a : ℚ, {Iio a}) := by
...
theorem measurable_of_tendsto_nnreal' {ι} {f : ι → α → ℝ≥0 } {g : α → ℝ≥0 } (u : Filter ι) [ne_bot u]
    [is_countably_generated u] (hf : ∀ i, Measurable (f i)) (lim : tendsto f u (𝓝 g)) : Measurable g := by
  rcases u.exists_seq_tendsto with ⟨x, hx⟩
  rw [tendsto_pi_nhds] at lim
  rw [← measurable_coe_nnreal_ennreal_iff]
  have : ∀ y, (liminf at_top fun n => (f (x n) y : ℝ≥0∞)) = (g y : ℝ≥0∞) := fun y =>
    ((ennreal.continuous_coe.tendsto (g y)).comp $ (limₓ y).comp hx).liminf_eq
  simp only [← this]
...
theorem ae_measurable_of_tendsto_metric_ae {μ : Measureₓ α} {f : ℕ → α → β} {g : α → β} (hf : ∀ n, AeMeasurable (f n) μ)
    (h_ae_tendsto : ∀ᵐ x ∂μ, tendsto (fun n => f n x) at_top (𝓝 (g x))) : AeMeasurable g μ := by
  ...
  split_ifs with hx
  · simp_rw [aeSeq.mk_eq_fun_of_mem_ae_seq_set hf hx]
...
theorem measurable_limit_of_tendsto_metric_ae {μ : Measureₓ α} {f : ℕ → α → β} (hf : ∀ n, AeMeasurable (f n) μ)
  ...
  have hp_mem : ∀ x, x ∈ AeSeqSet hf p → p x fun n => f n x := fun x hx => aeSeq.fun_prop_of_mem_ae_seq_set hf hx
  have hμ_compl : μ (AeSeqSet hf pᶜ) = 0 := aeSeq.measure_compl_ae_seq_set_eq_zero hf h_ae_tendsto
  let f_lim : α → β := fun x =>
    dite (x ∈ AeSeqSet hf p) (fun h => (hp_mem x h).some) fun h => (⟨f 0 x⟩ : Nonempty β).some
  have hf_lim_conv : ∀ x, x ∈ AeSeqSet hf p → filter.at_top.tendsto (fun n => f n x) (𝓝 (f_lim x)) := by
    intro x hx_conv
    simp only [f_lim, hx_conv, dif_pos]
    exact (hp_mem x hx_conv).some_spec
  have hf_lim : ∀ x, filter.at_top.tendsto (fun n => aeSeq hf p n x) (𝓝 (f_lim x)) := by
    intro x
  ...
  have h_ae_tendsto_f_lim : ∀ᵐ x ∂μ, filter.at_top.tendsto (fun n => f n x) (𝓝 (f_lim x)) := by
  ...
  have h_f_lim_meas : Measurable f_lim :=
    measurable_of_tendsto_metric (aeSeq.measurable hf p) (tendsto_pi_nhds.mpr fun x => hf_lim x)
