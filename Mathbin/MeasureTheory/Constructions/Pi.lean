...
theorem measure_preserving_fun_unique {β : Type u} {m : MeasurableSpace β} (μ : Measureₓ β) (α : Type v) [Unique α] :
  measure_preserving (MeasurableEquiv.funUnique α β) (measure.pi fun a : α => μ) μ :=
  by 
...
theorem measure_preserving_fin_two_arrow_vec {α : Type u} {m : MeasurableSpace α} (μ ν : Measureₓ α) [sigma_finite μ]
  ...
  measure_preserving MeasurableEquiv.finTwoArrow
    (measure.pi («expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"))
    (μ.prod ν) :=
  by 
    have  :
      ∀ i,
