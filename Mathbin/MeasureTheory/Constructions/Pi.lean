...
theorem generate_from_pi_eq {C : ∀ i, Set (Set (α i))} (hC : ∀ i, IsCountablySpanning (C i)) :
    (@MeasurableSpace.pi _ _ fun i => generate_from (C i)) = generate_from (pi univ '' pi univ C) := by
  have := Fintype.encodable ι
  apply le_antisymmₓ
  · refine' supr_le _
    intro i
...
theorem pi_premeasure_pi {s : ∀ i, Set (α i)} (hs : (pi univ s).Nonempty) :
    pi_premeasure m (pi univ s) = ∏ i, m i (s i) := by
  simp [hs]

theorem pi_premeasure_pi' [Nonempty ι] {s : ∀ i, Set (α i)} : pi_premeasure m (pi univ s) = ∏ i, m i (s i) := by
...
theorem pi_pi_le (m : ∀ i, outer_measure (α i)) (s : ∀ i, Set (α i)) :
    outer_measure.pi m (pi univ s) ≤ ∏ i, m i (s i) := by
  cases' (pi univ s).eq_empty_or_nonempty with h h
  simp [h]
  exact (bounded_by_le _).trans_eq (pi_premeasure_pi h)

theorem le_pi {m : ∀ i, outer_measure (α i)} {n : outer_measure (∀ i, α i)} :
    n ≤ outer_measure.pi m ↔ ∀ s : ∀ i, Set (α i), (pi univ s).Nonempty → n (pi univ s) ≤ ∏ i, m i (s i) := by
...
theorem pi'_pi [∀ i, sigma_finite (μ i)] (s : ∀ i, Set (α i)) : pi' μ (pi univ s) = ∏ i, μ i (s i) := by
...
theorem ae_eval_ne (i : ι) [has_no_atoms (μ i)] (x : α i) : ∀ᵐ y : ∀ i, α i ∂measure.pi μ, y i ≠ x :=
...
theorem measure_preserving_fin_two_arrow_vec {α : Type u} {m : MeasurableSpace α} (μ ν : Measureₓ α) [sigma_finite μ]
    ...
    measure_preserving MeasurableEquiv.finTwoArrow
      (measure.pi («expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:706:61: unsupported notation `«expr![ , ]»"))
      (μ.prod ν) :=
  have :
    ∀ i,
      sigma_finite
