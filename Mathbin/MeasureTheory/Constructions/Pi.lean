...
theorem generate_from_pi [∀ i, MeasurableSpace (α i)] :
    generateFrom (pi univ '' pi univ fun i => { s : Set (α i) | MeasurableSet s }) =
      MeasurableSpace.pi :=
  generate_from_eq_pi (fun i => generate_from_measurable_set) fun i =>
...
theorem pi_premeasure_pi_mono {s t : Set (∀ i, α i)} (h : s ⊆ t) :
...
instance pi.isInvInvariant [∀ i, Group (α i)] [∀ i, HasMeasurableInv (α i)]
  ...
  have A : Inv.inv ⁻¹' pi univ s = Set.pi univ fun i => Inv.inv ⁻¹' s i := by
    ext
    simp
  simp_rw [measure.inv, measure.map_apply measurable_inv (MeasurableSet.univPi hs), A, pi_pi,
...
theorem measurePreservingFinTwoArrow {α : Type u} {m : MeasurableSpace α} (μ : Measure α)
  ...
  simpa only [Matrix.vec_single_eq_const, Matrix.vec_cons_const] using
