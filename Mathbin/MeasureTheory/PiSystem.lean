...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (s t «expr ∈ » C) -/
...
theorem piUnionInter_singleton (π : ι → Set (Set α)) (i : ι) : piUnionInter π {i} = π i ∪ {univ} :=
  ...
  · rintro ⟨t, hti, f, hfπ, rfl⟩
    ...
    · have ht_empty : t = ∅ := by
      ...
      simp only [ht_empty, interᵢ_false, interᵢ_univ, Set.mem_singleton univ, or_true_iff]
  · cases' h with hs hs
...
theorem generateFrom_piUnionInter_measurableSet (m : ι → MeasurableSpace α) (S : Set ι) :
  ...
  · rw [← @generate_from_measurable_set α (⨆ i ∈ S, m i)]
    exact generateFrom_mono (measurableSet_supᵢ_of_mem_piUnionInter m S)
  · refine' supᵢ₂_le fun i hi => _
    rw [← @generate_from_measurable_set α (m i)]
    exact generateFrom_mono (mem_piUnionInter_of_measurableSet m hi)
