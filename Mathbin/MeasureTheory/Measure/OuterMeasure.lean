...
theorem
  of_function_eq_Sup
  : outer_measure.of_function m m_empty = Sup { μ | ∀ s , μ s ≤ m s }
  := @ is_greatest_of_function α m m_empty . IsLub . Sup_eq . symm

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (u «expr ⊆ » «expr ∪ »(s, t))
...
theorem is_caratheodory_compl : is_caratheodory s₁ → is_caratheodory (s₁ᶜ) :=
  by 
  ...
  :=
    by
      refine' extend_Union_le_tsum_nat' MeasurableSet.Union _
        intro f h
        simp ( config := { singlePass := Bool.true._@._internal._hyg.0 } ) [ Union_disjointed.symm ]
...
theorem
  trim_eq_infi
  ( s : Set α ) : m.trim s = ⨅ ( t : _ ) ( st : s ⊆ t ) ( ht : MeasurableSet t ) , m t
  :=
    by
      simp ( config := { singlePass := Bool.true._@._internal._hyg.0 } ) only [ infi_comm ]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r «expr > » ms)
  ...
  by 
    rcases
      exists_measurable_superset_forall_eq_trim
        («expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»") s with
      ⟨t, hst, ht, htm⟩
