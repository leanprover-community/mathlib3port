...
@[simp]
theorem Union_null_iff' (m : OuterMeasure α) {ι : Prop} {s : ι → Set α} : m (⋃ i, s i) = 0 ↔ ∀ i, m (s i) = 0 := by
  by_cases i:ι <;> simp [i]
#align measure_theory.outer_measure.Union_null_iff' MeasureTheory.OuterMeasure.Union_null_iff'
...
theorem sUnion_null_iff (m : OuterMeasure α) {S : Set (Set α)} (hS : S.Countable) : m (⋃₀ S) = 0 ↔ ∀ s ∈ S, m s = 0 :=
...
theorem null_of_locally_null [TopologicalSpace α] [SecondCountableTopology α] (m : OuterMeasure α) (s : Set α)
  ...
  obtain ⟨t, ts, t_count, ht⟩ : ∃ (t) (_ : t ⊆ s), t.Countable ∧ s ⊆ ⋃ x ∈ t, u x :=
...
#align measure_theory.outer_measure.Union_of_tendsto_zero MeasureTheory.OuterMeasure.Union_of_tendsto_zero
...
instance : Zero (OuterMeasure α) :=
  ⟨{ measureOf := fun _ => 0, Empty := rfl, mono := fun _ _ _ => le_refl 0, Union_nat := fun s => zero_le _ }⟩
...
instance : Add (OuterMeasure α) :=
  ⟨fun m₁ m₂ =>
    { measureOf := fun s => m₁ s + m₂ s, Empty := show m₁ ∅ + m₂ ∅ = 0 by simp [outer_measure.empty],
...
instance [Monoid R] [DistribMulAction R ℝ≥0∞] [IsScalarTower R ℝ≥0∞ ℝ≥0∞] : DistribMulAction R (OuterMeasure α) :=
...
def comap {β} (f : α → β) : OuterMeasure β →ₗ[ℝ≥0∞] OuterMeasure α where
  toFun m :=
    { measureOf := fun s => m (f '' s), Empty := by simp, mono := fun s t h => m.mono $ image_subset f h,
      Union_nat := fun s => by
        rw [image_Union]
        apply m.Union_nat }
  map_add' m₁ m₂ := rfl
...
@[simp]
theorem restrict_apply (s t : Set α) (m : OuterMeasure α) : restrict s m t = m (t ∩ s) := by simp [restrict]
...
protected def ofFunction : OuterMeasure α :=
  ...
  { measureOf := μ,
    Empty := le_antisymm ((infi_le_of_le fun _ => ∅) $ infi_le_of_le (empty_subset _) $ by simp [m_empty]) (zero_le _),
    ...
    Union_nat := fun s =>
      Ennreal.le_of_forall_pos_le_add $ by
        ...
        exact
          Union_subset fun i =>
            subset.trans (hf i).1 $
              Union_subset $ fun j => subset.trans (by simp) $ subset_Union _ $ Nat.mkpairEquiv (i, j) }
...
theorem of_function_le (s : Set α) : OuterMeasure.ofFunction m m_empty s ≤ m s :=
  ...
  infi_le_of_le f $
    infi_le_of_le (subset_Union f 0) $ le_of_eq $ tsum_eq_single 0 $ by rintro (_ | i) <;> simp [f, m_empty]
...
theorem of_function_eq (s : Set α) (m_mono : ∀ ⦃t : Set α⦄, s ⊆ t → m s ≤ m t)
  ...
  le_antisymm (of_function_le s) $ le_infi $ fun f => le_infi $ fun hf => le_trans (m_mono hf) (m_subadd f)
...
#align measure_theory.outer_measure.is_greatest_of_function MeasureTheory.OuterMeasure.is_greatest_of_function
...
/- ./././Mathport/Syntax/Translate/Basic.lean:611:2: warning: expanding binder collection (u «expr ⊆ » «expr ∪ »(s, t)) -/
...
#align measure_theory.outer_measure.restrict_of_function MeasureTheory.OuterMeasure.restrict_of_function

theorem smul_of_function {c : ℝ≥0∞} (hc : c ≠ ∞) :
    c • OuterMeasure.ofFunction m m_empty = OuterMeasure.ofFunction (c • m) (by simp [m_empty]) := by
  ext1 s
...
#align measure_theory.outer_measure.is_caratheodory_empty MeasureTheory.OuterMeasure.isCaratheodoryEmpty

theorem isCaratheodoryCompl : is_caratheodory s₁ → is_caratheodory (s₁ᶜ) := by simp [is_caratheodory, diff_eq, add_comm]
#align measure_theory.outer_measure.is_caratheodory_compl MeasureTheory.OuterMeasure.isCaratheodoryCompl
...
#align measure_theory.outer_measure.is_caratheodory_compl_iff MeasureTheory.OuterMeasure.is_caratheodory_compl_iff

theorem isCaratheodoryUnion (h₁ : is_caratheodory s₁) (h₂ : is_caratheodory s₂) : is_caratheodory (s₁ ∪ s₂) := fun t =>
  by
...
#align measure_theory.outer_measure.is_caratheodory_union MeasureTheory.OuterMeasure.isCaratheodoryUnion
    ...
    rw [bUnion_lt_succ] <;>
      exact
        is_caratheodory_union m (is_caratheodory_Union_lt $ fun i hi => h i $ lt_of_lt_of_le hi $ Nat.le_succ _)
          (h n (le_refl (n + 1)))
#align measure_theory.outer_measure.is_caratheodory_Union_lt MeasureTheory.OuterMeasure.isCaratheodoryUnionLt

theorem isCaratheodoryInter (h₁ : is_caratheodory s₁) (h₂ : is_caratheodory s₂) : is_caratheodory (s₁ ∩ s₂) := by
...
#align measure_theory.outer_measure.is_caratheodory_inter MeasureTheory.OuterMeasure.isCaratheodoryInter
...
#align measure_theory.outer_measure.is_caratheodory_sum MeasureTheory.OuterMeasure.is_caratheodory_sum
...
theorem isCaratheodoryUnionNat {s : ℕ → Set α} (h : ∀ i, is_caratheodory (s i)) (hd : Pairwise (Disjoint on s)) :
  ...
  is_caratheodory_iff_le'.2 $ fun t => by
    ...
    rw [Ennreal.supr_add]
    refine' supr_le fun n => le_trans (add_le_add_left _ _) (ge_of_eq (is_caratheodory_Union_lt m (fun i _ => h i) _))
    refine' m.mono (diff_subset_diff_right _)
    exact Union₂_subset fun i _ => subset_Union _ i
#align measure_theory.outer_measure.is_caratheodory_Union_nat MeasureTheory.OuterMeasure.isCaratheodoryUnionNat
...
protected theorem Union_eq_of_caratheodory {s : ℕ → Set α} (h : ∀ i, measurable_set[caratheodory] (s i))
  ...
  f_Union h hd
#align measure_theory.outer_measure.Union_eq_of_caratheodory MeasureTheory.OuterMeasure.Union_eq_of_caratheodory

end CaratheodoryMeasurable

variable {α : Type _}

theorem ofFunctionCaratheodory {m : Set α → ℝ≥0∞} {s : Set α} {h₀ : m ∅ = 0} (hs : ∀ t, m (t ∩ s) + m (t \ s) ≤ m t) :
...
#align measure_theory.outer_measure.of_function_caratheodory MeasureTheory.OuterMeasure.ofFunctionCaratheodory

theorem boundedByCaratheodory {m : Set α → ℝ≥0∞} {s : Set α} (hs : ∀ t, m (t ∩ s) + m (t \ s) ≤ m t) :
...
#align measure_theory.outer_measure.bounded_by_caratheodory MeasureTheory.OuterMeasure.boundedByCaratheodory
...
theorem le_sum_caratheodory {ι} (m : ι → OuterMeasure α) : (⨅ i, (m i).caratheodory) ≤ (sum m).caratheodory :=
  fun s h t => by simp [fun i => MeasurableSpace.measurable_set_infi.1 h i t, Ennreal.tsum_add]
...
#align measure_theory.outer_measure.Inf_eq_bounded_by_Inf_gen MeasureTheory.OuterMeasure.Inf_eq_bounded_by_Inf_gen
...
#align measure_theory.outer_measure.supr_Inf_gen_nonempty MeasureTheory.OuterMeasure.supr_Inf_gen_nonempty
...
#align measure_theory.outer_measure.restrict_infi_restrict MeasureTheory.OuterMeasure.restrict_infi_restrict
...
theorem restrict_Inf_eq_Inf_restrict (m : Set (OuterMeasure α)) {s : Set α} (hm : m.Nonempty) :
    restrict s (inf m) = inf (restrict s '' m) := by simp only [Inf_eq_infi, restrict_binfi, hm, infi_image]
#align measure_theory.outer_measure.restrict_Inf_eq_Inf_restrict MeasureTheory.OuterMeasure.restrict_Inf_eq_Inf_restrict
...
#align measure_theory.induced_outer_measure_exists_set MeasureTheory.induced_outer_measure_exists_set
...
#align measure_theory.induced_outer_measure_caratheodory MeasureTheory.induced_outer_measure_caratheodory
...
theorem trim_zero : (0 : OuterMeasure α).trim = 0 :=
  ext $ fun s =>
    le_antisymm (le_trans ((trim 0).mono (subset_univ s)) $ le_of_eq $ trim_eq _ MeasurableSet.univ) (zero_le _)
