...
theorem le_inter_add_diff {m : outer_measure α} {t : Set α} (s : Set α) : m t ≤ m (t ∩ s) + m (t \ s) := by
  convert m.union _ _
  rw [inter_union_diff t s]

theorem diff_null (m : outer_measure α) (s : Set α) {t : Set α} (ht : m t = 0) : m (s \ t) = m s := by
...
theorem coe_fn_injective : injective fun μ : outer_measure α s : Set α => μ s := fun μ₁ μ₂ h => by
  cases μ₁
  cases μ₂
  congr
  exact h
...
@[simp]
theorem coe_zero : ⇑(0 : outer_measure α) = 0 :=
  rfl
...
@[simp]
theorem coe_add (m₁ m₂ : outer_measure α) : ⇑(m₁ + m₂) = m₁ + m₂ :=
  rfl
...
@[simp]
theorem coe_smul (c : ℝ≥0∞) (m : outer_measure α) : ⇑(c • m) = c • m :=
  rfl
...
instance outer_measure.partial_order : PartialOrderₓ (outer_measure α) where
  le := fun m₁ m₂ => ∀ s, m₁ s ≤ m₂ s
  le_refl := fun a s => le_reflₓ _
...
instance : HasSupₓ (outer_measure α) :=
  ⟨fun ms =>
    { measureOf := fun s => ⨆ m ∈ ms, (m : outer_measure α) s,
      ...
      Union_nat := fun f =>
        bsupr_le fun m hm =>
          calc
            ...
            _ ≤ ∑' i, ⨆ m ∈ ms, (m : outer_measure α) (f i) := Ennreal.tsum_le_tsum fun i => le_bsupr m hm
             }⟩

instance : CompleteLattice (outer_measure α) :=
  { outer_measure.order_bot,
    completeLatticeOfSup (outer_measure α) fun ms =>
      ⟨fun m hm s => le_bsupr m hm, fun m hm s => bsupr_le fun m' hm' => hm hm' s⟩ with }

@[simp]
theorem Sup_apply (ms : Set (outer_measure α)) (s : Set α) : (Sup ms) s = ⨆ m ∈ ms, (m : outer_measure α) s :=
  rfl

@[simp]
theorem supr_apply {ι} (f : ι → outer_measure α) (s : Set α) : (⨆ i : ι, f i) s = ⨆ i, f i s := by
  rw [supr, Sup_apply, supr_range, supr]

@[norm_cast]
theorem coe_supr {ι} (f : ι → outer_measure α) : (⇑⨆ i, f i) = ⨆ i, f i :=
  funext fun s => by
    rw [supr_apply, _root_.supr_apply]

@[simp]
theorem sup_apply (m₁ m₂ : outer_measure α) (s : Set α) : (m₁⊔m₂) s = m₁ s⊔m₂ s := by
...
def map {β} (f : α → β) : outer_measure α →ₗ[ℝ≥0∞] outer_measure β where
  toFun := fun m =>
...
def dirac (a : α) : outer_measure α where
...
def Sum {ι} (f : ι → outer_measure α) : outer_measure α where
...
def comap {β} (f : α → β) : outer_measure β →ₗ[ℝ≥0∞] outer_measure α where
  toFun := fun m =>
...
@[simp]
theorem restrict_supr {ι} (s : Set α) (m : ι → outer_measure α) : restrict s (⨆ i, m i) = ⨆ i, restrict s (m i) := by
  simp [restrict]
...
theorem map_of_function {β} {f : α → β} (hf : injective f) :
    map f (outer_measure.of_function m m_empty) = outer_measure.of_function (fun s => m (f ⁻¹' s)) m_empty := by
...
theorem bounded_by_apply (s : Set α) :
    bounded_by m s = ⨅ (t : ℕ → Set α) (h : s ⊆ Union t), ∑' n, ⨆ h : (t n).Nonempty, m (t n) := by
  simp [bounded_by, of_function_apply]

theorem bounded_by_eq (s : Set α) (m_empty : m ∅ = 0) (m_mono : ∀ ⦃t : Set α⦄, s ⊆ t → m s ≤ m t)
    (m_subadd : ∀ s : ℕ → Set α, m (⋃ i, s i) ≤ ∑' i, m (s i)) : bounded_by m s = m s := by
...
theorem le_bounded_by {μ : outer_measure α} : μ ≤ bounded_by m ↔ ∀ s, μ s ≤ m s := by
  rw [bounded_by, le_of_function, forall_congrₓ]
  intro s
  cases' s.eq_empty_or_nonempty with h h <;> simp [h, empty_not_nonempty]

theorem le_bounded_by' {μ : outer_measure α} : μ ≤ bounded_by m ↔ ∀ s : Set α, s.nonempty → μ s ≤ m s := by
  rw [le_bounded_by, forall_congrₓ]
  intro s
  cases' s.eq_empty_or_nonempty with h h <;> simp [h]

theorem smul_bounded_by {c : ℝ≥0∞} (hc : c ≠ ∞) : c • bounded_by m = bounded_by (c • m) := by
...
theorem of_function_caratheodory {m : Set α → ℝ≥0∞} {s : Set α} {h₀ : m ∅ = 0} (hs : ∀ t, m (t ∩ s) + m (t \ s) ≤ m t) :
    (outer_measure.of_function m h₀).caratheodory.MeasurableSet' s := by
...
theorem le_add_caratheodory (m₁ m₂ : outer_measure α) :
    m₁.caratheodory⊓m₂.caratheodory ≤ (m₁ + m₂ : outer_measure α).caratheodory := fun s ⟨hs₁, hs₂⟩ t => by
...
theorem le_smul_caratheodory (a : ℝ≥0∞) (m : outer_measure α) : m.caratheodory ≤ (a • m).caratheodory := fun s h t => by
  simp [h t, mul_addₓ]
...
theorem Inf_eq_bounded_by_Inf_gen (m : Set (outer_measure α)) : Inf m = outer_measure.bounded_by (Inf_gen m) := by
...
theorem supr_Inf_gen_nonempty {m : Set (outer_measure α)} (h : m.nonempty) (t : Set α) :
    (⨆ h : t.nonempty, Inf_gen m t) = ⨅ (μ : outer_measure α) (h : μ ∈ m), μ t := by
...
theorem Inf_apply {m : Set (outer_measure α)} {s : Set α} (h : m.nonempty) :
    Inf m s = ⨅ (t : ℕ → Set α) (h2 : s ⊆ Union t), ∑' n, ⨅ (μ : outer_measure α) (h3 : μ ∈ m), μ (t n) := by
...
theorem comap_infi {ι β} (f : α → β) (m : ι → outer_measure β) : comap f (⨅ i, m i) = ⨅ i, comap f (m i) := by
...
theorem induced_outer_measure_preimage (f : α ≃ α) (Pm : ∀ s : Set α, P (f ⁻¹' s) ↔ P s)
    ...
    induced_outer_measure m P0 m0 (f ⁻¹' A) = induced_outer_measure m P0 m0 A := by
...
theorem induced_outer_measure_exists_set {s : Set α} (hs : induced_outer_measure m P0 m0 s ≠ ∞) {ε : ℝ≥0∞}
    (hε : ε ≠ 0) :
    ∃ (t : Set α)(ht : P t), s ⊆ t ∧ induced_outer_measure m P0 m0 t ≤ induced_outer_measure m P0 m0 s + ε := by
...
and `m₃ s`. -/
theorem trim_binop {m₁ m₂ m₃ : outer_measure α} {op : ℝ≥0∞ → ℝ≥0∞ → ℝ≥0∞} (h : ∀ s, m₁ s = op (m₂ s) (m₃ s))
    (s : Set α) : m₁.trim s = op (m₂.trim s) (m₃.trim s) := by
  rcases exists_measurable_superset_forall_eq_trim
      («expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:706:61: unsupported notation `«expr![ , ]»") s with
    ⟨t, hst, ht, htm⟩
...
theorem trim_supr {ι} [Encodable ι] (μ : ι → outer_measure α) : trim (⨆ i, μ i) = ⨆ i, trim (μ i) := by
  ext1 s
