...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r «expr < » μ U)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (K «expr ⊆ » U)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r «expr > » μ A)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (U «expr ⊇ » A)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (U «expr ⊇ » A n)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (k «expr ∈ » t)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (k «expr ∈ » t)
...
theorem weakly_regular_of_finite [BorelSpace α] (μ : Measureₓ α) [is_finite_measure μ]
  ...
  by 
    ...
    ·
      ...
      refine'
        ⟨⋃ (k : _)(_ : k ∈ t), F k, Union_subset_Union$ fun k => Union_subset$ fun _ => hFs _, ⋃ n, U n,
          Union_subset_Union hsU, is_closed_bUnion t.finite_to_set$ fun k _ => hFc k, is_open_Union hUo, ht.le.trans _,
          _⟩
      ·
        calc ((∑ k in t, μ (s k))+ε / 2) ≤ ((∑ k in t, μ (F k))+∑ k in t, δ k)+ε / 2 :=
          ...
          add_le_add_right (add_le_add_left ((Ennreal.sum_le_tsum _).trans hδε.le) _)
            _ _ = μ (⋃ (k : _)(_ : k ∈ t), F k)+ε :=
          _ 
      ...
      ·
        calc μ (⋃ n, U n) ≤ ∑' n, μ (U n) := measure_Union_le _ _ ≤ ∑' n, μ (s n)+δ n :=
          Ennreal.tsum_le_tsum hU _ = μ (⋃ n, s n)+∑' n, δ n :=
          by 
...
theorem is_compact_is_closed {X : Type _} [TopologicalSpace X] [T2Space X] [SigmaCompactSpace X] [MeasurableSpace X]
  [OpensMeasurableSpace X] (μ : Measureₓ X) : inner_regular μ IsCompact IsClosed :=
  by 
    intro F hF r hr 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (K «expr ⊆ » A)
/-- If `μ` is a regular measure, then any measurable set of finite measure can be approximated by a
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (F «expr ⊆ » U)
