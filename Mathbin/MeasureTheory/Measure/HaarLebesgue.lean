...
theorem is_add_left_invariant_real_volume : is_add_left_invariant (⇑(volume : Measureₓ ℝ)) :=
  by 
...
theorem is_add_left_invariant_real_volume_pi (ι : Type _) [Fintype ι] :
  is_add_left_invariant (⇑(volume : Measureₓ (ι → ℝ))) :=
  by 
