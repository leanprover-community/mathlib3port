...
theorem measure_of_cont_bdd_of_tendsto_filter_indicator {ι : Type _} {L : Filter ι} [L.IsCountablyGenerated]
    [TopologicalSpace Ω] [OpensMeasurableSpace Ω] (μ : Measure Ω) [IsFiniteMeasure μ] {c : ℝ≥0} {E : Set Ω}
    (E_mble : MeasurableSet E) (fs : ι → Ω →ᵇ ℝ≥0) (fs_bdd : ∀ᶠ i in L, ∀ᵐ ω : Ω ∂μ, fs i ω ≤ c)
    (fs_lim :
      ∀ᵐ ω : Ω ∂μ,
        Tendsto (fun i : ι => (coeFn : (Ω →ᵇ ℝ≥0) → Ω → ℝ≥0) (fs i) ω) L (𝓝 (indicator E (fun x => (1 : ℝ≥0)) ω))) :
  ...
  have aux : ∀ ω, indicator E (fun ω => (1 : ℝ≥0∞)) ω = ↑(indicator E (fun ω => (1 : ℝ≥0)) ω) := fun ω => by
...
-/
theorem measure_of_cont_bdd_of_tendsto_indicator [TopologicalSpace Ω] [OpensMeasurableSpace Ω] (μ : Measure Ω)
    [IsFiniteMeasure μ] {c : ℝ≥0} {E : Set Ω} (E_mble : MeasurableSet E) (fs : ℕ → Ω →ᵇ ℝ≥0)
    (fs_bdd : ∀ n ω, fs n ω ≤ c)
    (fs_lim : Tendsto (fun n : ℕ => (coeFn : (Ω →ᵇ ℝ≥0) → Ω → ℝ≥0) (fs n)) atTop (𝓝 (indicator E fun x => (1 : ℝ≥0)))) :
    Tendsto (fun n => lintegral μ fun ω => fs n ω) atTop (𝓝 (μ E)) := by
  have fs_lim' : ∀ ω, tendsto (fun n : ℕ => (fs n ω : ℝ≥0)) at_top (𝓝 (indicator E (fun x => (1 : ℝ≥0)) ω)) := by
...
-/
theorem tendsto_lintegral_thickened_indicator_of_is_closed {Ω : Type _} [MeasurableSpace Ω] [PseudoEmetricSpace Ω]
    ...
    Tendsto (fun n => lintegral μ fun ω => (thickenedIndicator (δs_pos n) F ω : ℝ≥0∞)) atTop (𝓝 (μ F)) := by
  apply
...
-/
theorem FiniteMeasure.limsup_measure_closed_le_of_tendsto {Ω ι : Type _} {L : Filter ι} [MeasurableSpace Ω]
    [PseudoEmetricSpace Ω] [OpensMeasurableSpace Ω] {μ : FiniteMeasure Ω} {μs : ι → FiniteMeasure Ω}
  ...
  have key₁ := tendsto_lintegral_thickened_indicator_of_is_closed (μ : Measure Ω) F_closed δs_pos δs_lim
  ...
  have aux := fun n =>
    le_trans (measure_le_lintegral_thickened_indicator (μs n : Measure Ω) F_closed.measurable_set (δs_pos M))
...
-/
theorem ProbabilityMeasure.limsup_measure_closed_le_of_tendsto {Ω ι : Type _} {L : Filter ι} [MeasurableSpace Ω]
...
-/
theorem ProbabilityMeasure.le_liminf_measure_open_of_tendsto {Ω ι : Type _} {L : Filter ι} [MeasurableSpace Ω]
    [PseudoEmetricSpace Ω] [OpensMeasurableSpace Ω] {μ : ProbabilityMeasure Ω} {μs : ι → ProbabilityMeasure Ω}
    (μs_lim : Tendsto μs L (𝓝 μ)) {G : Set Ω} (G_open : IsOpen G) :
    (μ : Measure Ω) G ≤ L.liminf fun i => (μs i : Measure Ω) G :=
  haveI h_closeds : ∀ F, IsClosed F → (L.limsup fun i => (μs i : Measure Ω) F) ≤ (μ : Measure Ω) F := fun F F_closed =>
