...
noncomputable def Monotone.stieltjesFunction {f : ℝ → ℝ} (hf : Monotone f) : StieltjesFunction
  ...
  right_continuous' := by
    ...
    calc
      rightLim f z ≤ f a := hf.right_lim_le za
...
theorem length_subadditive_Icc_Ioo {a b : ℝ} {c d : ℕ → ℝ} (ss : Icc a b ⊆ ⋃ i, Ioo (c i) (d i)) :
  ...
  suffices
    ∀ (s : Finset ℕ) (b) (cv : Icc a b ⊆ ⋃ i ∈ (↑s : Set ℕ), Ioo (c i) (d i)),
      (ofReal (f b - f a) : ℝ≥0∞) ≤ ∑ i in s, ofReal (f (d i) - f (c i))
    by
    ...
    have e : (⋃ i ∈ (↑hf.to_finset : Set ℕ), Ioo (c i) (d i)) = ⋃ i ∈ s, Ioo (c i) (d i) := by
      simp only [ext_iff, exists_prop, Finset.set_bunionᵢ_coe, mem_unionᵢ, forall_const,
  ...
  rw [Finset.coe_insert, bunionᵢ_insert] at cv
...
theorem outer_Ioc (a b : ℝ) : f.outer (Ioc a b) = ofReal (f b - f a) :=
  ...
  have :
    ∀ i, ∃ p : ℝ × ℝ, s i ⊆ Ioo p.1 p.2 ∧ (ofReal (f p.2 - f p.1) : ℝ≥0∞) < f.length (s i) + ε' i :=
    by
    intro i
    have :=
  ...
  have I_subset : Icc a' b ⊆ ⋃ i, Ioo (g i).1 (g i).2 :=
    calc
      ...
      _ ⊆ ⋃ i, s i := hs
      _ ⊆ ⋃ i, Ioo (g i).1 (g i).2 := unionᵢ_mono fun i => (hg i).1
...
theorem measurableSet_Ioi {c : ℝ} : measurable_set[f.outer.caratheodory] (Ioi c) :=
  by
  apply OuterMeasure.ofFunction_caratheodory fun t => _
...
theorem outer_trim : f.outer.trim = f.outer :=
  ...
  simp at hg
  apply infᵢ_le_of_le (unionᵢ g) _
  apply infᵢ_le_of_le (ht.trans <| unionᵢ_mono fun i => (hg i).1) _
...
theorem measure_singleton (a : ℝ) : f.measure {a} = ofReal (f a - leftLim f a) :=
  ...
  have L1 : Tendsto (fun n => f.measure (Ioc (u n) a)) atTop (𝓝 (f.measure {a})) :=
    by
    rw [A]
    refine' tendsto_measure_interᵢ (fun n => measurableSet_Ioc) (fun m n hmn => _) _
