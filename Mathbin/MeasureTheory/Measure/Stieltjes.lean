...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » («expr↑ »(s) : set exprℕ()))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » («expr↑ »(hf.to_finset) : set exprℕ()))
      ...
      rcases
        is_compact_Icc.elim_finite_subcover_image (fun i : ℕ _ : i ∈ univ => @is_open_Ioo _ _ _ _ (c i) (d i))
          (by 
            simpa using ss) with
        ⟨s, su, hf, hs⟩
      have e : (⋃ (i : _)(_ : i ∈ (↑hf.to_finset : Set ℕ)), Ioo (c i) (d i)) = ⋃ (i : _)(_ : i ∈ s), Ioo (c i) (d i)
      ·
