...
theorem inner_content_exists_compact {U : opens G} (hU : μ.inner_content U ≠ ∞) {ε :  ℝ≥0 } (hε : ε ≠ 0) :
  ...
  by 
    ...
    conv  at this => toRHS rw [inner_content]
...
theorem inner_content_pos_of_is_mul_left_invariant [T2Space G] [Groupₓ G] [TopologicalGroup G]
  ...
  by 
    ...
    have  : K.1 ⊆ «expr↑ » (⨆(g : _)(_ : g ∈ s), U.comap$ continuous_mul_left g)
    ·
...
theorem borel_le_caratheodory : S ≤ μ.outer_measure.caratheodory :=
  by 
    ...
    have  : «expr↑ » U' \ U ⊆ U' \ L.1 := diff_subset_diff_right hL.2
    refine' le_transₓ (add_le_add_left (μ.outer_measure.mono' this) _) _ 
    rw [μ.outer_measure_of_is_open («expr↑ » U' \ L.1) (IsOpen.sdiff U'.2 L.2.IsClosed)]
    simp only [inner_content, supr_subtype']
    rw [opens.coe_mk]
    haveI  : Nonempty { M : compacts G // M.1 ⊆ «expr↑ » U' \ L.1 } := ⟨⟨⊥, empty_subset _⟩⟩
    rw [Ennreal.add_supr]
    refine' supr_le _ 
    rintro ⟨M, hM⟩
    ...
    rw [μ.outer_measure_of_is_open («expr↑ » U') U'.2]
