...
@[toAdditive]
theorem nonempty_Inter_cl_prehaar (K₀ : positive_compacts G) :
  (haar_product K₀.1 ∩ ⋂V : open_nhds_of (1 : G), cl_prehaar K₀.1 V).Nonempty :=
  by 
    have  : IsCompact (haar_product K₀.1)
    ·
...
@[toAdditive add_chaar_mem_cl_add_prehaar]
theorem chaar_mem_cl_prehaar (K₀ : positive_compacts G) (V : open_nhds_of (1 : G)) : chaar K₀ ∈ cl_prehaar K₀.1 V :=
  by 
    have  := (Classical.some_spec (nonempty_Inter_cl_prehaar K₀)).2
    rw [mem_Inter] at this 
    exact this V

@[toAdditive add_chaar_nonneg]
theorem chaar_nonneg (K₀ : positive_compacts G) (K : compacts G) : 0 ≤ chaar K₀ K :=
  by 
    have  := chaar_mem_haar_product K₀ K (mem_univ _)
    rw [mem_Icc] at this 
    exact this.1

@[toAdditive add_chaar_empty]
theorem chaar_empty (K₀ : positive_compacts G) : chaar K₀ ⊥ = 0 :=
  by 
...
@[toAdditive add_chaar_self]
theorem chaar_self (K₀ : positive_compacts G) : chaar K₀ ⟨K₀.1, K₀.2.1⟩ = 1 :=
  by 
...
@[toAdditive add_chaar_mono]
theorem chaar_mono {K₀ : positive_compacts G} {K₁ K₂ : compacts G} (h : K₁.1 ⊆ K₂.1) : chaar K₀ K₁ ≤ chaar K₀ K₂ :=
  by 
...
@[toAdditive add_chaar_sup_le]
theorem chaar_sup_le {K₀ : positive_compacts G} (K₁ K₂ : compacts G) : chaar K₀ (K₁⊔K₂) ≤ chaar K₀ K₁+chaar K₀ K₂ :=
  by 
    ...
    apply
      mem_of_subset_of_mem _
        (chaar_mem_cl_prehaar K₀
          ⟨V⁻¹, (IsOpen.inter h1V₁ h1V₂).Preimage continuous_inv,
            by 
...
@[toAdditive is_left_invariant_add_chaar]
...
@[toAdditive]
theorem haar_measure_self {K₀ : positive_compacts G} : haar_measure K₀ K₀.1 = 1 :=
  by 
...
theorem is_haar_measure_eq_smul_is_haar_measure [LocallyCompactSpace G] [second_countable_topology G] (μ ν : Measureₓ G)
  ...
  by 
    ...
    have νlt : ν K.1 < ∞ := IsCompact.haar_lt_top _ K.2.1
    ...
    ·
      simp only [div_eq_mul_inv, νpos.ne', (IsCompact.haar_lt_top μ K.property.left).Ne, or_selfₓ, Ennreal.inv_eq_top,
        WithTop.mul_eq_top_iff, Ne.def, not_false_iff, and_falseₓ, false_andₓ]
    ·
      calc μ = μ K.1 • haar_measure K :=
        haar_measure_unique (is_mul_left_invariant_haar μ) K _ = (μ K.1 / ν K.1) • ν K.1 • haar_measure K :=
        by 
...
theorem map_haar_inv {G : Type _} [CommGroupₓ G] [TopologicalSpace G] [TopologicalGroup G] [T2Space G]
  ...
  [is_haar_measure μ] : measure.map HasInv.inv μ = μ :=
  by 
    haveI  : is_haar_measure (measure.map HasInv.inv μ) :=
    ...
    have  : map HasInv.inv (map HasInv.inv μ) = (c^2) • μ
    ·
    ...
    ·
      ·
        ...
        rw [one_pow, one_mulₓ]
    have  : (c^2) = (1^2) :=
      (Ennreal.mul_eq_mul_right (haar_pos_of_nonempty_interior _ K.2.2).ne' (IsCompact.haar_lt_top _ K.2.1).Ne).1 this 
