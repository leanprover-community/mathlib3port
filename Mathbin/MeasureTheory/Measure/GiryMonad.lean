...
theorem measurable_lintegral {f : α → ℝ≥0∞} (hf : Measurable f) : Measurable fun μ : Measureₓ α => ∫⁻ x, f x ∂μ := by
...
@[simp]
theorem join_zero : (0 : Measureₓ (Measureₓ α)).join = 0 := by
  ext1 s hs
  simp [hs]
...
@[simp]
theorem bind_zero_left (f : α → Measureₓ β) : bind 0 f = 0 := by
  simp [bind]

@[simp]
theorem bind_zero_right (m : Measureₓ α) : bind m (0 : α → Measureₓ β) = 0 := by
  ext1 s hs
...
theorem join_map_join (μ : Measureₓ (Measureₓ (Measureₓ α))) : join (map join μ) = join (join μ) := by
