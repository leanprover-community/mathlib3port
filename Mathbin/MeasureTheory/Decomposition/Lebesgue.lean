...
theorem mutuallySingularSingularPart (μ ν : Measure α) : μ.singularPart ν ⊥ₘ ν :=
  ...
  · rw [singularPart, dif_neg h]
    exact MutuallySingular.zeroLeft
...
theorem lintegral_rnDeriv_lt_top_of_measure_ne_top {μ : Measure α} (ν : Measure α) {s : Set α}
  ...
  · haveI := hl
    ...
    suffices : (∫⁻ x in toMeasurable μ s, μ.rn_deriv ν x ∂ν) < ∞
    ...
    refine'
      lt_of_le_of_lt
        (le_add_left le_rfl :
          _ ≤
            μ.singular_part ν (toMeasurable μ s) + ν.with_density (μ.rn_deriv ν) (toMeasurable μ s))
        _
...
theorem rnDeriv_lt_top (μ ν : Measure α) [SigmaFinite μ] : ∀ᵐ x ∂ν, μ.rnDeriv ν x < ∞ :=
  by
  suffices ∀ n, ∀ᵐ x ∂ν, x ∈ spanningSets μ n → μ.rn_deriv ν x < ∞ by
...
theorem eq_singularPart {s : Measure α} {f : α → ℝ≥0∞} (hf : Measurable f) (hs : s ⊥ₘ ν)
  ...
  have heq : s.restrict ((S ∩ T)ᶜ) = (μ.singular_part ν).restrict ((S ∩ T)ᶜ) :=
    ...
    have hf : ν.with_density f (A ∩ (S ∩ T)ᶜ) = 0 :=
      by
      refine' withDensityAbsolutelyContinuous ν _ _
...
theorem singularPart_zero (ν : Measure α) : (0 : Measure α).singularPart ν = 0 :=
  by
  refine' (eq_singularPart measurable_zero MutuallySingular.zeroLeft _).symm
...
theorem singularPart_add (μ₁ μ₂ ν : Measure α) [HaveLebesgueDecomposition μ₁ ν]
  ...
  refine'
    (eq_singularPart ((measurable_rnDeriv μ₁ ν).add (measurable_rnDeriv μ₂ ν))
        ((haveLebesgueDecomposition_spec _ _).2.1.addLeft (haveLebesgueDecomposition_spec _ _).2.1)
        _).symm
  erw [withDensity_add_left (measurable_rnDeriv μ₁ ν)]
  conv_rhs => rw [add_assoc, add_comm (μ₂.singular_part ν), ← add_assoc, ← add_assoc]
  rw [← haveLebesgueDecomposition_add μ₁ ν, add_assoc, add_comm (ν.with_density (μ₂.rn_deriv ν)), ←
...
theorem singularPart_withDensity (ν : Measure α) {f : α → ℝ≥0∞} (hf : Measurable f) :
  ...
  (eq_singularPart hf MutuallySingular.zeroLeft this).symm
...
theorem eq_withDensity_rnDeriv {s : Measure α} {f : α → ℝ≥0∞} (hf : Measurable f) (hs : s ⊥ₘ ν)
  ...
  have heq' :
    ...
    have hνfinter : ν.with_density f (A ∩ (S ∩ T)ᶜ) = 0 :=
      by
      rw [← nonpos_iff_eq_zero]
      exact withDensityAbsolutelyContinuous ν f hνinter ▸ measure_mono (inter_subset_right _ _)
  ...
  have hνrn : ν.with_density (μ.rn_deriv ν) (A ∩ (S ∩ T)ᶜ) = 0 :=
    ...
    rw [← nonpos_iff_eq_zero]
    exact
      withDensityAbsolutelyContinuous ν (μ.rn_deriv ν) hνinter ▸
...
theorem eq_rnDeriv [SigmaFinite ν] {s : Measure α} {f : α → ℝ≥0∞} (hf : Measurable f) (hs : s ⊥ₘ ν)
  ...
  intro a ha h'a
  calc
    (∫⁻ x : α in a, f x ∂ν) = ν.with_density f a := (withDensity_apply f ha).symm
    _ = ν.with_density (μ.rn_deriv ν) a := by rw [eq_withDensity_rnDeriv hf hs hadd]
    _ = ∫⁻ x : α in a, μ.rn_deriv ν x ∂ν := withDensity_apply _ ha
...
theorem rnDeriv_withDensity (ν : Measure α) [SigmaFinite ν] {f : α → ℝ≥0∞} (hf : Measurable f) :
  ...
  haveI : ν.with_density f = 0 + ν.with_density f := by rw [zero_add]
  (eq_rnDeriv hf MutuallySingular.zeroLeft this).symm
...
theorem exists_positive_of_not_mutuallySingular (μ ν : Measure α) [IsFiniteMeasure μ]
  ...
  have hA₂ : ∀ n : ℕ, μ.to_signed_measure - ((1 / (n + 1) : ℝ≥0) • ν).toSignedMeasure ≤[A] 0 :=
    by
    intro n
    exact restrict_le_restrict_subset _ _ (hf₁ n).compl (hf₃ n) (interᵢ_subset _ _)
  ...
  rw [MutuallySingular] at h
  push_neg  at h
  have := h _ hAmeas hμ
  simp_rw [hA₁, compl_interᵢ, compl_compl] at this
  -- as `Aᶜ = ⋃ n, f n`, `ν Aᶜ > 0` implies there exists some `n` such that `ν (f n) > 0`
  obtain ⟨n, hn⟩ := exists_measure_pos_of_not_measure_unionᵢ_null this
...
theorem supᵢ_mem_measurableLe (f : ℕ → α → ℝ≥0∞) (hf : ∀ n, f n ∈ measurableLe μ ν) (n : ℕ) :
  ...
  · have :
      ...
      funext fun _ => supᵢ_succ_eq_sup _ _ _
    refine' ⟨measurable_supᵢ fun n => Measurable.supᵢ_Prop _ (hf n).1, fun A hA => _⟩
    rw [this]
    exact (sup_mem_measurableLe (hf m.succ) hm).2 A hA
...
theorem supᵢ_mem_measurable_le' (f : ℕ → α → ℝ≥0∞) (hf : ∀ n, f n ∈ measurableLe μ ν) (n : ℕ) :
  ...
  by
  convert supᵢ_mem_measurableLe f hf n
  ext; simp
...
theorem haveLebesgueDecompositionOfFiniteMeasure [IsFiniteMeasure μ] [IsFiniteMeasure ν] :
  ...
  ⟨by
    have h :=
      @exists_seq_tendsto_Sup _ _ _ _ _ (measurableLeEval ν μ)
    ...
    have hξ₁ : supₛ (measurableLeEval ν μ) = ∫⁻ a, ξ a ∂ν :=
      by
      have :=
      ...
      · refine' tendsto_nhds_unique _ this
        ...
        · intro n
          ...
          simp only [supᵢ_apply, supᵢ_le_le f n n le_rfl]
        · intro n
          exact le_supₛ ⟨⨆ (k : ℕ) (hk : k ≤ n), f k, supᵢ_mem_measurable_le' _ hf₁ _, rfl⟩
      · intro n
        refine' Measurable.aeMeasurable _
        convert (supᵢ_mem_measurableLe _ hf₁ n).1
        ext
        simp
      · refine' Filter.eventually_of_forall fun a => _
        simp [supᵢ_monotone' f _]
      · refine' Filter.eventually_of_forall fun a => _
        simp [tendsto_atTop_supᵢ (supᵢ_monotone' f a)]
    have hξm : Measurable ξ :=
      by
      convert measurable_supᵢ fun n => (supᵢ_mem_measurableLe _ hf₁ n).1
      ext
      simp [hξ]
    ...
    have hle : ν.with_density ξ ≤ μ := by
      ...
      simp_rw [supᵢ_apply]
      rw [lintegral_supᵢ (fun i => (supᵢ_mem_measurableLe _ hf₁ i).1) (supᵢ_monotone _)]
      exact supᵢ_le fun i => (supᵢ_mem_measurableLe _ hf₁ i).2 B hB
    have : IsFiniteMeasure (ν.with_density ξ) :=
      by
      refine' isFiniteMeasureWithDensity _
    ...
    · by_contra
      ...
      have hξle : ∀ A, MeasurableSet A → (∫⁻ a in A, ξ a ∂ν) ≤ μ A :=
        ...
        rw [hξ]
        simp_rw [supᵢ_apply]
        rw [lintegral_supᵢ (fun n => (supᵢ_mem_measurableLe _ hf₁ n).1) (supᵢ_monotone _)]
        exact supᵢ_le fun n => (supᵢ_mem_measurableLe _ hf₁ n).2 A hA
      ...
      have hξε : (ξ + E.indicator fun _ => ε) ∈ measurableLe ν μ :=
        by
      ...
      have : (∫⁻ a, ξ a + E.indicator (fun _ => ε) a ∂ν) ≤ supₛ (measurableLeEval ν μ) :=
...
instance (priority := 100) haveLebesgueDecompositionOfSigmaFinite (μ ν : Measure α) [SigmaFinite μ]
  ...
  ⟨by
    ...
    have hμ : μ = sum μn := by rw [hμn, ← restrict_unionᵢ h₂ S.set_mem, S.spanning, restrict_univ]
    set νn : ℕ → Measure α := fun n => ν.restrict (T.set n) with hνn
    have hν : ν = sum νn := by rw [hνn, ← restrict_unionᵢ h₃ T.set_mem, T.spanning, restrict_univ]
    ...
    set f := ∑' n, (S.set n).indicator (rnDeriv (μn n) (νn n)) with hf
    ...
    · choose A hA₁ hA₂ hA₃ using fun n => mutuallySingularSingularPart (μn n) (νn n)
      simp only [hξ]
      ...
      · have hcompl : IsCompl (⋃ n, S.set n ∩ A n) (⋃ n, S.set n ∩ A nᶜ) :=
          ...
          · rw [codisjoint_iff_le_sup]
            intro x hx
            simp only [mem_unionᵢ, sup_eq_union, mem_inter_iff, mem_union, mem_compl_iff,
              or_iff_not_imp_left]
            intro h
            push_neg  at h
            rw [top_eq_univ, ← S.spanning, mem_unionᵢ] at hx
            obtain ⟨i, hi⟩ := hx
            exact ⟨i, hi, h i hi⟩
        rw [hcompl.compl_eq, measure_unionᵢ, tsum_eq_zero_iff Ennreal.summable]
        · intro n
    ...
    · simp only [hξ, hf, hμ]
      ...
      · refine' sum_congr fun n => _
        conv_lhs => rw [haveLebesgueDecomposition_add (μn n) (νn n)]
        suffices heq :
...
theorem singularPartMutuallySingular (s : SignedMeasure α) (μ : Measure α) :
  ...
  · rw [not_haveLebesgueDecomposition_iff] at hl
    ...
    · rw [Measure.singularPart, Measure.singularPart, dif_neg hn]
      exact MutuallySingular.zeroRight
  ...
  have :
    (s.singular_part μ).toJordanDecomposition =
      ⟨s.to_jordan_decomposition.pos_part.singular_part μ,
        s.to_jordan_decomposition.neg_part.singular_part μ, singularPartMutuallySingular s μ⟩ :=
    by
...
theorem mutuallySingularSingularPart (s : SignedMeasure α) (μ : Measure α) :
  ...
  exact (mutuallySingularSingularPart _ _).addLeft (mutuallySingularSingularPart _ _)
  ...
  refine' Integrable.sub _ _ <;>
    · constructor
      · apply Measurable.aeStronglyMeasurable
        measurability
      exact hasFiniteIntegralToRealOfLintegralNeTop (lintegral_rnDeriv_lt_top _ μ).ne
...
theorem singularPart_add_with_density_rnDeriv_eq [s.HaveLebesgueDecomposition μ] :
  ...
  rw [singularPart, rnDeriv,
    withDensityᵥ_sub' (integrableToRealOfLintegralNeTop _ _) (integrableToRealOfLintegralNeTop _ _),
  ...
  all_goals first |exact (lintegral_rnDeriv_lt_top _ _).ne|measurability
...
theorem jordanDecompositionAddWithDensityMutuallySingular {f : α → ℝ} (hf : Measurable f)
  ...
  exact
    ((JordanDecomposition.mutuallySingular _).addRight
          (htμ.1.monoAc (refl _) (withDensityAbsolutelyContinuous _ _))).addLeft
      ((htμ.2.symm.monoAc (withDensityAbsolutelyContinuous _ _) (refl _)).addRight
        (withDensityOfRealMutuallySingular hf))
...
theorem toJordanDecomposition_eq_of_eq_add_withDensity {f : α → ℝ} (hf : Measurable f)
    ...
    s.toJordanDecomposition =
      @JordanDecomposition.mk α _
        ...
        (by
          haveI := isFiniteMeasureWithDensityOfReal hfi.2
          infer_instance)
        (by
          haveI := isFiniteMeasureWithDensityOfReal hfi.neg.2
          infer_instance)
...
private theorem have_lebesgue_decomposition_mk' (μ : Measure α) {f : α → ℝ} (hf : Measurable f)
  ...
  rw [vector_measure.equiv_measure.right_inv, totalVariation_mutuallySingular_iff] at htμ
  refine'
    { posPart := by
...
theorem haveLebesgueDecompositionMk (μ : Measure α) {f : α → ℝ} (hf : Measurable f)
  ...
  · exact haveLebesgueDecompositionMk' μ hf hfi htμ hadd
  · rw [withDensityᵥ, dif_neg hfi, add_zero] at hadd
    refine' haveLebesgueDecompositionMk' μ measurable_zero (integrableZero _ _ μ) htμ _
...
private theorem eq_singular_part' (t : SignedMeasure α) {f : α → ℝ} (hf : Measurable f)
  ...
  change
    ...
    htμ
  rw [vector_measure.equiv_measure.right_inv] at htμ
  · rw [singularPart, ← t.to_signed_measure_to_jordan_decomposition,
...
theorem eq_singularPart (t : SignedMeasure α) (f : α → ℝ) (htμ : t ⊥ᵥ μ.toEnnrealVectorMeasure)
  ...
  · rw [withDensityᵥ, dif_neg hfi, add_zero] at hadd
    refine' eq_singular_part' t measurable_zero (integrableZero _ _ μ) htμ _
    rwa [withDensityᵥ_zero, add_zero]
...
theorem singularPart_zero (μ : Measure α) : (0 : SignedMeasure α).singularPart μ = 0 :=
  by
  refine' (eq_singularPart 0 0 VectorMeasure.MutuallySingular.zeroLeft _).symm
...
theorem singularPart_add (s t : SignedMeasure α) (μ : Measure α) [s.HaveLebesgueDecomposition μ]
  ...
  by
  refine'
    (eq_singularPart _ (s.rn_deriv μ + t.rn_deriv μ)
        ((mutuallySingularSingularPart s μ).addLeft (mutuallySingularSingularPart t μ)) _).symm
  erw [withDensityᵥ_add (integrableRnDeriv s μ) (integrableRnDeriv t μ)]
  rw [add_assoc, add_comm (t.singular_part μ), add_assoc, add_comm _ (t.singular_part μ),
    singularPart_add_with_density_rnDeriv_eq, ← add_assoc, singularPart_add_with_density_rnDeriv_eq]
...
theorem eq_rnDeriv (t : SignedMeasure α) (f : α → ℝ) (hfi : Integrable f μ)
  ...
  have hadd' : s = t + μ.with_densityᵥ f' :=
    ...
    convert hadd using 2
    exact WithDensityᵥEq.congr_ae hfi.1.ae_eq_mk.symm
  haveI := haveLebesgueDecompositionMk μ hfi.1.measurable_mk htμ hadd'
  refine' (Integrable.ae_eq_of_withDensityᵥ_eq (integrableRnDeriv _ _) hfi _).symm
  rw [← add_right_inj t, ← hadd, eq_singularPart _ f htμ hadd,
    singularPart_add_with_density_rnDeriv_eq]
...
theorem rnDeriv_neg (s : SignedMeasure α) (μ : Measure α) [s.HaveLebesgueDecomposition μ] :
  ...
  by
  refine' Integrable.ae_eq_of_withDensityᵥ_eq (integrableRnDeriv _ _) (integrableRnDeriv _ _).neg _
  rw [withDensityᵥ_neg, ← add_right_inj ((-s).singularPart μ),
    singularPart_add_with_density_rnDeriv_eq, singularPart_neg, ← neg_add,
    singularPart_add_with_density_rnDeriv_eq]
...
theorem rnDeriv_smul (s : SignedMeasure α) (μ : Measure α) [s.HaveLebesgueDecomposition μ] (r : ℝ) :
  ...
  by
  refine'
    Integrable.ae_eq_of_withDensityᵥ_eq (integrableRnDeriv _ _) ((integrableRnDeriv _ _).smul r) _
  change _ = μ.with_densityᵥ ((r : ℝ) • s.rn_deriv μ)
  rw [withDensityᵥ_smul (rnDeriv s μ) (r : ℝ), ← add_right_inj ((r • s).singularPart μ),
    singularPart_add_with_density_rnDeriv_eq, singularPart_smul]
  change _ = _ + r • _
  rw [← smul_add, singularPart_add_with_density_rnDeriv_eq]
...
theorem rnDeriv_add (s t : SignedMeasure α) (μ : Measure α) [s.HaveLebesgueDecomposition μ]
  ...
  by
  refine'
    Integrable.ae_eq_of_withDensityᵥ_eq (integrableRnDeriv _ _)
      ((integrableRnDeriv _ _).add (integrableRnDeriv _ _)) _
  rw [← add_right_inj ((s + t).singularPart μ), singularPart_add_with_density_rnDeriv_eq,
    withDensityᵥ_add (integrableRnDeriv _ _) (integrableRnDeriv _ _), singularPart_add, add_assoc,
    add_comm (t.singular_part μ), add_assoc, add_comm _ (t.singular_part μ),
    singularPart_add_with_density_rnDeriv_eq, ← add_assoc, singularPart_add_with_density_rnDeriv_eq]
...
theorem integrableRnDeriv (c : ComplexMeasure α) (μ : Measure α) : Integrable (c.rnDeriv μ) μ :=
  ...
  exact
    ⟨memℒp_one_iff_integrable.2 (SignedMeasure.integrableRnDeriv _ _),
      memℒp_one_iff_integrable.2 (SignedMeasure.integrableRnDeriv _ _)⟩
...
theorem singularPart_add_with_density_rnDeriv_eq [c.HaveLebesgueDecomposition μ] :
  ...
  ext
  · rw [Complex.add_re, withDensityᵥ_apply (c.integrable_rn_deriv μ) hi, ← IsROrC.re_eq_complex_re,
    ...
    · exact SignedMeasure.integrableRnDeriv _ _
  · rw [Complex.add_im, withDensityᵥ_apply (c.integrable_rn_deriv μ) hi, ← IsROrC.im_eq_complex_im,
