...
theorem lintegral_rn_deriv_lt_top (μ ν : Measureₓ α) [is_finite_measure μ] : (∫⁻ x, μ.rn_deriv ν x ∂ν) < ∞ := by
...
theorem eq_rn_deriv [sigma_finite ν] {s : Measureₓ α} {f : α → ℝ≥0∞} (hf : Measurable f) (hs : s ⊥ₘ ν)
    (hadd : μ = s + ν.with_density f) : f =ᵐ[ν] μ.rn_deriv ν := by
...
theorem supr_mem_measurable_le (f : ℕ → α → ℝ≥0∞) (hf : ∀ n, f n ∈ measurable_le μ ν) (n : ℕ) :
    (fun x => ⨆ (k) (hk : k ≤ n), f k x) ∈ measurable_le μ ν := by
...
theorem supr_mem_measurable_le' (f : ℕ → α → ℝ≥0∞) (hf : ∀ n, f n ∈ measurable_le μ ν) (n : ℕ) :
    (⨆ (k) (hk : k ≤ n), f k) ∈ measurable_le μ ν := by
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (n k)
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (n k)
...
theorem singular_part_add_with_density_rn_deriv_eq [c.have_lebesgue_decomposition μ] :
    c.singular_part μ + μ.with_densityᵥ (c.rn_deriv μ) = c := by
