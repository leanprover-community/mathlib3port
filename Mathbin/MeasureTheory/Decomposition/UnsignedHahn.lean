...
theorem hahn_decomposition [IsFiniteMeasure μ] [IsFiniteMeasure ν] :
  ...
  have d_split : ∀ s t, MeasurableSet s → MeasurableSet t → d s = d (s \ t) + d (s ∩ t) :=
    ...
    ac_rfl
  have d_Union :
    ∀ s : ℕ → Set α, Monotone s → Tendsto (fun n => d (s n)) atTop (𝓝 (d (⋃ n, s n))) :=
    by
    intro s hm
  ...
  have d_Inter :
    ∀ s : ℕ → Set α,
      (∀ n, MeasurableSet (s n)) →
        (∀ n m, n ≤ m → s m ⊆ s n) → Tendsto (fun n => d (s n)) atTop (𝓝 (d (⋂ n, s n))) :=
    by
    intro s hs hm
    refine' Tendsto.sub _ _ <;>
      refine'
  ...
  have γ_le_d_s : γ ≤ d s :=
    ...
    have hγ : Tendsto (fun m : ℕ => γ - 2 * (1 / 2) ^ m) atTop (𝓝 γ) :=
      by
      suffices Tendsto (fun m : ℕ => γ - 2 * (1 / 2) ^ m) atTop (𝓝 (γ - 2 * 0)) by simpa
      exact
