...
theorem left_of_comp (hfg : is_fixed_pt (f ∘ g) x) (hg : is_fixed_pt g x) : is_fixed_pt f x :=
  calc
    f x = f (g x) := congr_argₓ f hg.symm
    _ = x := hfg
...
theorem to_left_inverse (hf : is_fixed_pt f x) (h : left_inverse g f) : is_fixed_pt g x :=
  calc
    g x = g (f x) := congr_argₓ g hf.symm
    _ = x := h x
...
protected theorem apply {x : α} (hx : is_fixed_pt f x) : is_fixed_pt f (f x) := by
  convert hx
...
theorem mem_fixed_points_iff {α : Type _} {f : α → α} {x : α} : x ∈ fixed_points f ↔ f x = x := by
  rfl
...
theorem semiconj.maps_to_fixed_pts {g : α → β} (h : semiconj g fa fb) :
    Set.MapsTo g (fixed_points fa) (fixed_points fb) := fun x hx => hx.map h
