...
theorem ae_empty_or_univ_of_forall_vadd_ae_eq_self {s : Set <| AddCircle T}
        ...
        ∀ {ι'} {l : Filter ι'} (w : ι' → AddCircle T) (δ : ι' → ℝ),
          Tendsto δ l (𝓝[>] 0) →
            (∀ᶠ j in l, d ∈ closedBall (w j) (1 * δ j)) →
              Tendsto (fun j => μ (s ∩ closedBall (w j) (δ j)) / μ (closedBall (w j) (δ j))) l
                (𝓝 1) :=
  ...
  · let δ : ι → ℝ := fun j => T / (2 * ↑(n j))
    ...
    have hw : ∀ᶠ j in l, d ∈ closedBall d (1 * δ j) := hδ₀.mono fun j hj => by simp [hj.le]
    exact hd _ δ hδ₁ hw
  ...
  { measure_preserving_zsmul volume (abs_pos.mp <| lt_trans zero_lt_one hn) with
    ae_empty_or_univ := fun s hs hs' =>
      ...
      have hu₀ : ∀ j, addOrderOf (u j) = n.nat_abs ^ j := fun j =>
        addOrderOf_div_of_gcd_eq_one (pow_pos (pos_of_gt hn) j) (gcd_one_left _)
      ...
      have hu₂ : Tendsto (fun j => addOrderOf <| u j) atTop atTop :=
        by
        simp_rw [hu₀]
...
theorem ergodicZsmulAdd (x : AddCircle T) {n : ℤ} (h : 1 < |n|) : Ergodic fun y => n • y + x :=
  ...
  have he : MeasurePreserving e volume volume := measure_preserving_add_left volume _
  suffices e ∘ f ∘ e.symm = fun y => n • y
    by
    rw [← he.ergodic_conjugate_iff, this]
    exact ergodicZsmul h
  replace h : n - 1 ≠ 0
  · rw [← abs_one] at h
    rw [sub_ne_zero]
  ...
  ext y
  simp only [f, hnx, MeasurableEquiv.coe_add_left, MeasurableEquiv.symm_add_left, comp_apply,
