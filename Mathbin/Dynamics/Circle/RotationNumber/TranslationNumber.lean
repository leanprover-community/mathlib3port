...
instance
  : Monoidₓ CircleDeg1Lift
  where
    mul
        f g
        :=
        { toFun := f ∘ g , monotone' := f.monotone.comp g.monotone , map_add_one' := fun x => by simp [ map_add_one ] }
...
@[simp, norm_cast]
theorem units_coe (f : Units CircleDeg1Lift) : ⇑(f : CircleDeg1Lift) = f :=
  rfl

@[simp]
theorem units_inv_apply_apply (f : Units CircleDeg1Lift) (x : ℝ) : (f⁻¹ : Units CircleDeg1Lift) (f x) = x := by
  simp only [← units_coe, ← mul_apply, f.inv_mul, coe_one, id]

@[simp]
theorem units_apply_inv_apply (f : Units CircleDeg1Lift) (x : ℝ) : f ((f⁻¹ : Units CircleDeg1Lift) x) = x := by
...
@[simp]
theorem coe_to_order_iso (f : Units CircleDeg1Lift) : ⇑to_order_iso f = f :=
  rfl

@[simp]
theorem coe_to_order_iso_symm (f : Units CircleDeg1Lift) : ⇑(to_order_iso f).symm = (f⁻¹ : Units CircleDeg1Lift) :=
  rfl

@[simp]
theorem coe_to_order_iso_inv (f : Units CircleDeg1Lift) : ⇑to_order_iso f⁻¹ = (f⁻¹ : Units CircleDeg1Lift) :=
  rfl

theorem is_unit_iff_bijective {f : CircleDeg1Lift} : IsUnit f ↔ bijective f :=
  ⟨fun ⟨u, h⟩ => h ▸ (to_order_iso u).Bijective, fun h =>
    Units.is_unit
      { val := f,
        inv :=
...
def translate : Multiplicative ℝ →* Units CircleDeg1Lift := by
...
-- failed to format: format: uncaught backtrack exception
/-- Monotone circle maps form a lattice with respect to the pointwise order -/ noncomputable
  instance
    : Lattice CircleDeg1Lift
    where
      sup
          f g
          :=
        ...
        le f g := ∀ x , f x ≤ g x
        le_refl f x := le_reflₓ ( f x )
        le_trans f₁ f₂ f₃ h₁₂ h₂₃ x := le_transₓ ( h₁₂ x ) ( h₂₃ x )
        le_antisymm f₁ f₂ h₁₂ h₂₁ := ext $ fun x => le_antisymmₓ ( h₁₂ x ) ( h₂₁ x )
        le_sup_left f g x := le_max_leftₓ ( f x ) ( g x )
        le_sup_right f g x := le_max_rightₓ ( f x ) ( g x )
        sup_le f₁ f₂ f₃ h₁ h₂ x := max_leₓ ( h₁ x ) ( h₂ x )
        inf
          f g
          :=
          {
        ...
        inf_le_left f g x := min_le_leftₓ ( f x ) ( g x )
        inf_le_right f g x := min_le_rightₓ ( f x ) ( g x )
        le_inf f₁ f₂ f₃ h₂ h₃ x := le_minₓ ( h₂ x ) ( h₃ x )
...
theorem transnum_aux_seq_zero : f.transnum_aux_seq 0 = f 0 := by
  simp [transnum_aux_seq]

theorem transnum_aux_seq_dist_lt (n : ℕ) : dist (f.transnum_aux_seq n) (f.transnum_aux_seq (n+1)) < 1 / 2 / 2 ^ n := by
...
theorem tendsto_translation_number_of_dist_bounded_aux (x : ℕ → ℝ) (C : ℝ) (H : ∀ n : ℕ, dist ((f ^ n) 0) (x n) ≤ C) :
    tendsto (fun n : ℕ => x (2 ^ n) / 2 ^ n) at_top (𝓝 $ τ f) := by
...
@[simp]
theorem translation_number_units_inv (f : Units CircleDeg1Lift) : τ (↑f⁻¹) = -τ f :=
...
@[simp]
theorem translation_number_conj_eq (f : Units CircleDeg1Lift) (g : CircleDeg1Lift) : τ (((↑f)*g)*↑f⁻¹) = τ g :=
...
theorem tendsto_translation_number₀' : tendsto (fun n : ℕ => (f ^ n+1) 0 / n+1) at_top (𝓝 $ τ f) := by
  refine'
...
theorem tendsto_translation_number (x : ℝ) : tendsto (fun n : ℕ => ((f ^ n) x - x) / n) at_top (𝓝 $ τ f) := by
