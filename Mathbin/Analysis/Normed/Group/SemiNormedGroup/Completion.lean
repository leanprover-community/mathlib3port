...
instance
  : preadditive SemiNormedGroupₓ .{ u }
  where
    homGroup P Q := inferInstance
      add_comp'
        :=
        by intros ext simp only [ NormedGroupHom.add_apply , CategoryTheory.comp_apply , NormedGroupHom.map_add ]
      comp_add'
        :=
        by intros ext simp only [ NormedGroupHom.add_apply , CategoryTheory.comp_apply , NormedGroupHom.map_add ]

-- failed to format: format: uncaught backtrack exception
instance : functor.additive Completion where map_add' X Y := ( Completion.map_hom _ _ ) . map_add
