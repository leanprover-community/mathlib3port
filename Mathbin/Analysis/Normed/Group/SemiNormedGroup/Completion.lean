...
@[simps]
def completion : SemiNormedGroup.{u} ⥤ SemiNormedGroup.{u} where
  obj := fun V => SemiNormedGroup.of (completion V)
  map := fun V W f => f.Completion
  map_id' := fun V => completion_id
  map_comp' := fun U V W f g => (completion_comp f g).symm

instance Completion_complete_space {V : SemiNormedGroup} : CompleteSpace (completion.obj V) :=
...
def completion.mapHom (V W : SemiNormedGroup.{u}) : (V ⟶ W) →+ (completion.obj V ⟶ completion.obj W) :=
...
instance : Preadditive SemiNormedGroup.{u} where
...
theorem completion.lift_comp_incl {V W : SemiNormedGroup} [CompleteSpace W] [SeparatedSpace W] (f : V ⟶ W) :
    Completion.incl ≫ completion.lift f = f := by
  ext
  apply NormedAddGroupHom.extension_coe

theorem completion.lift_unique {V W : SemiNormedGroup} [CompleteSpace W] [SeparatedSpace W] (f : V ⟶ W)
