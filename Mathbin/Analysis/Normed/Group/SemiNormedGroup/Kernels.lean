              ...
              (fun c =>
                NormedGroupHom.ker.lift (fork.ι c) _$
                  show NormedGroupHom.compHom (f - g) c.ι = 0 by 
                    ...
                    exact c.condition)
              (fun c => NormedGroupHom.ker.incl_comp_lift _ _ _)
              fun c g h =>
                by 
                  ext x 
                  dsimp 
...
@[simp, reassoc]
theorem comp_explicit_cokernel_π {X Y : SemiNormedGroupₓ.{u}} (f : X ⟶ Y) : f ≫ explicit_cokernel_π f = 0 :=
  by 
...
theorem explicit_cokernel_desc_unique {X Y Z : SemiNormedGroupₓ.{u}} {f : X ⟶ Y} {g : Y ⟶ Z} (w : f ≫ g = 0)
  (e : explicit_cokernel f ⟶ Z) (he : explicit_cokernel_π f ≫ e = g) : e = explicit_cokernel_desc w :=
  by 
    apply
...
theorem explicit_cokernel_desc_norm_noninc {X Y Z : SemiNormedGroupₓ.{u}} {f : X ⟶ Y} {g : Y ⟶ Z} {cond : f ≫ g = 0}
  (hg : g.norm_noninc) : (explicit_cokernel_desc cond).NormNoninc :=
  by 
...
theorem explicit_cokernel_desc_comp_eq_zero {X Y Z W : SemiNormedGroupₓ.{u}} {f : X ⟶ Y} {g : Y ⟶ Z} {h : Z ⟶ W}
  (cond : f ≫ g = 0) (cond2 : g ≫ h = 0) : explicit_cokernel_desc cond ≫ h = 0 :=
  by 
...
noncomputable def explicit_cokernel.map {A B C D : SemiNormedGroupₓ.{u}} {fab : A ⟶ B} {fbd : B ⟶ D} {fac : A ⟶ C}
  ...
  @explicit_cokernel_desc _ _ _ fab (fbd ≫ explicit_cokernel_π _)$
    by 
      simp [reassoc_of h]
