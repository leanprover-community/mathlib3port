...
instance
  has_zero_object
  : limits.has_zero_object SemiNormedGroupₓ .{ u }
  where
    zero := 0
      uniqueTo X := { default := 0 , uniq := fun a => by ext ⟨ ⟩ exact a.map_zero }
      uniqueFrom X := { default := 0 , uniq := fun f => by ext }
...
-- failed to format: format: uncaught backtrack exception
instance
  : large_category .{ u } SemiNormedGroup₁
  where
    hom X Y := { f : NormedGroupHom X Y // f.norm_noninc }
      id X := ⟨ NormedGroupHom.id X , NormedGroupHom.NormNoninc.id ⟩
      comp X Y Z f g := ⟨ ( g : NormedGroupHom Y Z ) . comp ( f : NormedGroupHom X Y ) , g . 2 . comp f . 2 ⟩
...
def mk_iso {M N : SemiNormedGroupₓ} (f : M ≅ N) (i : f.hom.norm_noninc) (i' : f.inv.norm_noninc) :
  ...
  { hom := mk_hom f.hom i, inv := mk_hom f.inv i',
    hom_inv_id' := by
      apply Subtype.eq
      exact f.hom_inv_id,
    inv_hom_id' := by
...
-- failed to format: format: uncaught backtrack exception
instance
  : limits.has_zero_morphisms .{ u , u + 1 } SemiNormedGroup₁
  where
    HasZero X Y := { zero := ⟨ 0 , NormedGroupHom.NormNoninc.zero ⟩ }
      comp_zero' X Y f Z := by ext rfl
      zero_comp' X Y Z f := by ext simp [ coe_fn_coe_base' ]
...
instance
  has_zero_object
  : limits.has_zero_object SemiNormedGroup₁ .{ u }
  where
    zero := 0
      uniqueTo X := { default := 0 , uniq := fun a => by ext ⟨ ⟩ exact a . 1 . map_zero }
      uniqueFrom X := { default := 0 , uniq := fun f => by ext }
