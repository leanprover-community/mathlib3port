...
theorem is_conformal_map_iff (f' : E →L[ℝ] F) :
  IsConformalMap f' ↔ ∃ c : ℝ, 0 < c ∧ ∀ u v : E, ⟪f' u, f' v⟫ = (c : ℝ)*⟪u, v⟫ :=
  by 
    constructor
    ·
      rintro ⟨c₁, hc₁, li, h⟩
      refine' ⟨c₁*c₁, mul_self_pos hc₁, fun u v => _⟩
    ...
    ·
      ...
      have minor' : (f' : E → F) = c⁻¹ • f₁ :=
        by 
          ext <;> simpRw [minor, Pi.smul_apply] <;> rw [smul_smul, inv_mul_cancel hc, one_smul]
      refine' ⟨c⁻¹, inv_ne_zero hc, f₁.to_linear_map.isometry_of_inner fun u v => _, minor'⟩
      simpRw [to_linear_map_eq_coe, ContinuousLinearMap.coe_coe, minor, Pi.smul_apply]
