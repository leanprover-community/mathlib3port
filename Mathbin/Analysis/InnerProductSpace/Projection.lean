...
-- error in Analysis.InnerProductSpace.Projection: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
...
theorem eq_orthogonal_projection_of_mem_of_inner_eq_zero {u v : E} (hvm : v ∈ K)
  (hvo : ∀ w (_ : w ∈ K), ⟪u - v, w⟫ = 0) : (orthogonalProjection K u : E) = v :=
...
theorem linear_isometry_equiv.reflections_generate
[finite_dimensional exprℝ() F] : «expr = »(subgroup.closure (set.range (λ
   v : F, reflection «expr ᗮ»(«expr ∙ »(exprℝ(), v)))), «expr⊤»()) :=
begin
  rw [expr subgroup.eq_top_iff'] [],
  intros [ident φ],
  rcases [expr φ.reflections_generate_dim, "with", "⟨", ident l, ",", "_", ",", ident rfl, "⟩"],
  apply [expr (subgroup.closure _).list_prod_mem],
  intros [ident x, ident hx],
  rcases [expr list.mem_map.mp hx, "with", "⟨", ident a, ",", "_", ",", ident hax, "⟩"],
  exact [expr subgroup.subset_closure ⟨a, hax⟩]
end
