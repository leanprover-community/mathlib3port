...
theorem TimesContDiffOn.norm_sq (hf : TimesContDiffOn ℝ n f s) : TimesContDiffOn ℝ n (fun y => ∥f y∥^2) s := fun x hx =>
  (hf x hx).normSq
...
theorem DifferentiableOn.norm_sq (hf : DifferentiableOn ℝ f s) : DifferentiableOn ℝ (fun y => ∥f y∥^2) s := fun x hx =>
  (hf x hx).normSq
