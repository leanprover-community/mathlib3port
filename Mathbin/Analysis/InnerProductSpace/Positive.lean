...
def IsPositive (T : E →L[𝕜] E) : Prop :=
  IsSelfAdjoint (T : E →ₗ[𝕜] E) ∧ ∀ x, 0 ≤ T.reApplyInnerSelf x

theorem IsPositive.is_self_adjoint {T : E →L[𝕜] E} (hT : IsPositive T) : IsSelfAdjoint (T : E →ₗ[𝕜] E) :=
  hT.1
...
theorem is_positive_zero : IsPositive (0 : E →L[𝕜] E) := by
  refine' ⟨is_self_adjoint_zero, fun x => _⟩
  change 0 ≤ re ⟪_, _⟫
  rw [zero_apply, inner_zero_left, ZeroHomClass.map_zero]

theorem is_positive_id : IsPositive (1 : E →L[𝕜] E) :=
  ⟨fun x y => rfl, fun x => inner_self_nonneg⟩
...
theorem IsPositive.conj_adjoint [CompleteSpace E] [CompleteSpace F] {T : E →L[𝕜] E} (hT : T.IsPositive)
    (S : E →L[𝕜] F) : (S ∘L T ∘L S†).IsPositive := by
  refine' ⟨hT.is_self_adjoint.conj_adjoint S, fun x => _⟩
  rw [re_apply_inner_self, comp_apply, ← adjoint_inner_right]
  exact hT.inner_nonneg_left _

theorem IsPositive.adjoint_conj [CompleteSpace E] [CompleteSpace F] {T : E →L[𝕜] E} (hT : T.IsPositive)
    (S : F →L[𝕜] E) : (S† ∘L T ∘L S).IsPositive := by
  convert hT.conj_adjoint (S†)
  rw [adjoint_adjoint]

theorem IsPositive.conj_orthogonal_projection [CompleteSpace E] (U : Submodule 𝕜 E) {T : E →L[𝕜] E} (hT : T.IsPositive)
    [CompleteSpace U] :
...
theorem IsPositive.orthogonal_projection_comp [CompleteSpace E] {T : E →L[𝕜] E} (hT : T.IsPositive) (U : Submodule 𝕜 E)
...
variable {E' : Type _} [InnerProductSpace ℂ E']

theorem is_positive_iff_complex (T : E' →L[ℂ] E') :
    IsPositive T ↔ ∀ x, (re ⟪T x, x⟫_ℂ : ℂ) = ⟪T x, x⟫_ℂ ∧ 0 ≤ re ⟪T x, x⟫_ℂ := by
  simp_rw [is_positive, forall_and_distrib, is_self_adjoint_iff_inner_map_self_real, eq_conj_iff_re]
  rfl
