...
theorem tendsto_exp_atTop : Tendsto exp atTop atTop :=
  ...
  have B : ∀ᶠ x in atTop, x + 1 ≤ exp x := eventually_atTop.2 ⟨0, fun x hx => add_one_le_exp x⟩
...
theorem tendsto_exp_div_pow_atTop (n : ℕ) : Tendsto (fun x => exp x / x ^ n) atTop atTop :=
  by
  refine' (at_top_basis_Ioi.tendsto_iff (atTop_basis' 1)).2 fun C hC₁ => _
...
theorem tendsto_mul_exp_add_div_pow_atTop (b c : ℝ) (n : ℕ) (hb : 0 < b) :
  ...
  exact
    ((tendsto_exp_div_pow_atTop n).const_mul_atTop hb).atTop_add
      (tendsto_const_nhds.div_at_top (tendsto_pow_atTop hn))
...
theorem tendsto_div_pow_mul_exp_add_atTop (b c : ℝ) (n : ℕ) (hb : 0 ≠ b) :
  ...
  have H : ∀ d e, 0 < d → Tendsto (fun x : ℝ => x ^ n / (d * exp x + e)) atTop (𝓝 0) :=
    by
    intro b' c' h
    convert (tendsto_mul_exp_add_div_pow_atTop b' c' n h).inv_tendsto_atTop
    ext x
...
theorem range_exp : range exp = Ioi 0 := by
  rw [← coe_comp_expOrderIso, range_comp, exp_order_iso.range_eq, image_univ, Subtype.range_coe]
...
theorem tendsto_exp_comp_atTop {f : α → ℝ} :
  ...
  rw [← tendsto_comap_iff, comap_exp_atTop]
...
theorem tendsto_comp_exp_atTop {f : ℝ → α} :
  ...
  rw [← tendsto_map'_iff, map_exp_atTop]
#align real.tendsto_comp_exp_at_top Real.tendsto_comp_exp_atTop

@[simp]
theorem map_exp_atBot : map exp atBot = 𝓝[>] 0 := by
  rw [← coe_comp_expOrderIso, ← Filter.map_map, exp_order_iso.map_at_bot, ← map_coe_Ioi_atBot]
...
theorem isOCat_pow_exp_atTop {n : ℕ} : (fun x => x ^ n) =o[atTop] Real.exp := by
  simpa [isOCat_iff_tendsto fun x hx => ((exp_pos x).ne' hx).elim] using
...
theorem isTheta_exp_comp_exp_comp {f g : α → ℝ} :
  ...
  simp only [isBoundedUnder_le_abs, ← isBoundedUnder_le_neg, neg_sub, IsTheta,
    isO_exp_comp_exp_comp, Pi.sub_def]
...
theorem isOCat_exp_comp_exp_comp {f g : α → ℝ} :
  ...
  simp only [isOCat_iff_tendsto, exp_ne_zero, ← exp_sub, ← tendsto_neg_atTop_iff, false_imp_iff,
...
theorem isOCat_one_exp_comp {f : α → ℝ} :
  ...
  simp only [← exp_zero, isOCat_exp_comp_exp_comp, sub_zero]
...
theorem isO_one_exp_comp {f : α → ℝ} :
  ...
  simp only [← exp_zero, isO_exp_comp_exp_comp, Pi.sub_def, zero_sub, isBoundedUnder_le_neg]
...
theorem isO_exp_comp_one {f : α → ℝ} :
  ...
  simp only [isO_one_iff, norm_eq_abs, abs_exp, isBoundedUnder_le_exp_comp]
