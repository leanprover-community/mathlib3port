...
theorem interval_integrable_zpow {n : ℤ}
  (h : 0 ≤ n ∨ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
theorem interval_integrable_rpow {r : ℝ}
  (h : 0 ≤ r ∨ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
theorem interval_integrable_one_div
  (h : ∀ x : ℝ, x ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)" → f x ≠ 0)
...
theorem interval_integrable_log
  (h : (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
theorem integral_rpow {r : ℝ}
  (h : 0 ≤ r ∨ r ≠ -1 ∧ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
  (∫ x in a..b, x^r) = ((b^r+1) - (a^r+1)) / r+1 :=
  by 
    suffices  :
      ∀ x _ : x ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)",
...
theorem integral_zpow {n : ℤ}
  (h : 0 ≤ n ∨ n ≠ -1 ∧ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
  (∫ x in a..b, x^n) = ((b^n+1) - (a^n+1)) / n+1 :=
  by 
    replace h :
      0 ≤ (n : ℝ) ∨ (n : ℝ) ≠ -1 ∧ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)"
    ·
      exactModCast h 
    exactModCast integral_rpow h
...
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
@[simp]
theorem integral_inv (h : (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
  (∫ x in a..b, x⁻¹) = log (b / a) :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
theorem integral_one_div (h : (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
@[simp]
theorem integral_log (h : (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)") :
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » _)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » «expr[ , ]»(a, b))
-- ././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)
theorem integral_sin_pow_aux :
  (∫ x in a..b, sin x^n+2) =
    ((((sin a^n+1)*cos a) - (sin b^n+1)*cos b)+(n+1)*∫ x in a..b, sin x^n) - (n+1)*∫ x in a..b, sin x^n+2 :=
  by 
    let C := ((sin a^n+1)*cos a) - (sin b^n+1)*cos b 
    have h : ∀ α β γ : ℝ, (α*(β*α)*γ) = β*(α*α)*γ :=
      fun α β γ =>
        by 
          ring 
    have hu : ∀ x _ : x ∈ _, HasDerivAt (fun y => sin y^n+1) (((n+1)*cos x)*sin x^n) x :=
      fun x hx =>
        by 
          simpa only [mul_right_commₓ] using (has_deriv_at_sin x).pow 
    have hv :
      ∀ x _ : x ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)",
    ...
    calc (∫ x in a..b, sin x^n+2) = ∫ x in a..b, (sin x^n+1)*sin x :=
      by 
        simp only [pow_succ'ₓ]_ = C+(n+1)*∫ x in a..b, (cos x^2)*sin x^n :=
      by 
        simp [H, h, sq]_ = C+(n+1)*∫ x in a..b, (sin x^n) - (sin x^n+2) :=
      by 
...
theorem integral_cos_pow_aux :
  (∫ x in a..b, cos x^n+2) =
    ((((cos b^n+1)*sin b) - (cos a^n+1)*sin a)+(n+1)*∫ x in a..b, cos x^n) - (n+1)*∫ x in a..b, cos x^n+2 :=
  by 
    ...
    have h : ∀ α β γ : ℝ, (α*(β*α)*γ) = β*(α*α)*γ :=
      fun α β γ =>
        by 
          ring 
    have hu : ∀ x _ : x ∈ _, HasDerivAt (fun y => cos y^n+1) (((-n+1)*sin x)*cos x^n) x :=
      fun x hx =>
        by 
          simpa only [mul_right_commₓ, neg_mul_eq_neg_mul_symm, mul_neg_eq_neg_mul_symm] using (has_deriv_at_cos x).pow 
    have hv :
      ∀ x _ : x ∈ "././Mathport/Syntax/Translate/Basic.lean:589:47: unsupported (impossible)",
        HasDerivAt sin (cos x) x :=
      fun x hx => has_deriv_at_sin x 
    have H := integral_mul_deriv_eq_deriv_mul hu hv _ _ 
    calc (∫ x in a..b, cos x^n+2) = ∫ x in a..b, (cos x^n+1)*cos x :=
      by 
        simp only [pow_succ'ₓ]_ = C+(n+1)*∫ x in a..b, (sin x^2)*cos x^n :=
      by 
        simp [H, h, sq, -neg_add_rev]_ = C+(n+1)*∫ x in a..b, (cos x^n) - (cos x^n+2) :=
      by 
        simp [sin_sq, sub_mul, ←pow_addₓ, add_commₓ]_ = (C+(n+1)*∫ x in a..b, cos x^n) - (n+1)*∫ x in a..b, cos x^n+2 :=
      by 
        rw [integral_sub, mul_sub, add_sub_assoc] <;> apply Continuous.interval_integrable <;> continuity 
    all_goals 
      apply Continuous.interval_integrable 
