...
theorem exp_map_circle_eq_exp_map_circle {x y : ℝ} : expMapCircle x = expMapCircle y ↔ ∃ m : ℤ, x = y+m*2*π :=
  by 
    ...
    rw [←mul_assocₓ, ←add_mulₓ, mul_left_inj' I_ne_zero, ←of_real_one, ←of_real_bit0, ←of_real_mul, ←of_real_int_cast,
      ←of_real_mul, ←of_real_add, of_real_inj]

theorem periodic_exp_map_circle : periodic expMapCircle (2*π) :=
  fun z =>
