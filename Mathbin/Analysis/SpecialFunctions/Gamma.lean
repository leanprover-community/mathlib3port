...
theorem gammaIntegralConvergent {s : ℂ} (hs : 0 < s.re) :
  ...
  by
  constructor
  · refine' ContinuousOn.aeStronglyMeasurable _ measurable_set_Ioi
...
theorem Gamma_integral_of_real (s : ℝ) : gammaIntegral ↑s = ↑s.gammaIntegral :=
  by
...
theorem Gamma_integral_one : gammaIntegral 1 = 1 :=
  by
...
theorem Gamma_eq_Gamma_aux (s : ℂ) (n : ℕ) (h1 : -s.re < ↑n) : gamma s = gammaAux n s :=
  by
...
theorem dGammaIntegralAbsConvergent (s : ℝ) (hs : 1 < s) :
  ...
  rw [← Ioc_union_Ioi_eq_Ioi (@zero_le_one ℝ _ _ _ _), integrable_on_union]
  refine' ⟨⟨_, _⟩, _⟩
  · refine' ContinuousOn.aeStronglyMeasurable (ContinuousOn.mul _ _).norm measurable_set_Ioc
