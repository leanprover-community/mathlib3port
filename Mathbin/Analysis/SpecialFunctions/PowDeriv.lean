...
theorem times_cont_diff_rpow_const_of_le {p : ℝ} {n : ℕ} (h : ↑n ≤ p) : TimesContDiff ℝ n fun x : ℝ => x^p :=
  by 
...
theorem times_cont_diff_at_rpow_const_of_le {x p : ℝ} {n : ℕ} (h : ↑n ≤ p) : TimesContDiffAt ℝ n (fun x : ℝ => x^p) x :=
