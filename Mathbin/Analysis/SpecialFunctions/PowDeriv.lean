...
theorem hasStrictDerivAtConstCpow {x y : ℂ} (h : x ≠ 0 ∨ y ≠ 0) : HasStrictDerivAt (fun y => x ^ y) (x ^ y * log x) y :=
  by
  ...
  · simpa only [cpow_def_of_ne_zero hx, mul_one] using ((hasStrictDerivAtId y).const_mul (log x)).cexp
...
theorem HasFderivAt.cpow (hf : HasFderivAt f f' x) (hg : HasFderivAt g g' x) (h0 : 0 < (f x).re ∨ (f x).im ≠ 0) :
  ...
  convert (@Complex.hasFderivAtCpow ((fun x => (f x, g x)) x) h0).comp x (hf.prod hg)
...
theorem HasFderivWithinAt.cpow (hf : HasFderivWithinAt f f' s x) (hg : HasFderivWithinAt g g' s x)
  ...
  convert (@Complex.hasFderivAtCpow ((fun x => (f x, g x)) x) h0).compHasFderivWithinAt x (hf.prod hg)
...
theorem DifferentiableWithinAt.constCpow (hf : DifferentiableWithinAt ℂ f s x) (h0 : c ≠ 0 ∨ f x ≠ 0) :
...
private theorem aux :
  ...
  by
  simp only [Algebra.id.smul_eq_mul, one_mul, ContinuousLinearMap.one_apply, ContinuousLinearMap.smul_right_apply,
...
theorem Complex.hasStrictDerivAtCpowConst (h : 0 < x.re ∨ x.im ≠ 0) :
  ...
  simpa only [mul_zero, add_zero, mul_one] using (hasStrictDerivAtId x).cpow (hasStrictDerivAtConst x c) h
#align complex.has_strict_deriv_at_cpow_const Complex.hasStrictDerivAtCpowConst

theorem HasStrictDerivAt.cpowConst (hf : HasStrictDerivAt f f' x) (h0 : 0 < (f x).re ∨ (f x).im ≠ 0) :
...
theorem HasDerivWithinAt.cpowConst (hf : HasDerivWithinAt f f' s x) (h0 : 0 < (f x).re ∨ (f x).im ≠ 0) :
...
theorem contDiffAtRpowOfNe (p : ℝ × ℝ) (hp : p.1 ≠ 0) {n : ℕ∞} : ContDiffAt ℝ n (fun p : ℝ × ℝ => p.1 ^ p.2) p := by
...
theorem differentiableAtRpowOfNe (p : ℝ × ℝ) (hp : p.1 ≠ 0) : DifferentiableAt ℝ (fun p : ℝ × ℝ => p.1 ^ p.2) p :=
...
theorem _root_.has_strict_deriv_at.rpow {f g : ℝ → ℝ} {f' g' : ℝ} (hf : HasStrictDerivAt f f' x)
  ...
  convert (has_strict_fderiv_at_rpow_of_pos ((fun x => (f x, g x)) x) h).compHasStrictDerivAt _ (hf.prod hg) using 1
  simp [mul_assoc, mul_comm, mul_left_comm]
#align real._root_.has_strict_deriv_at.rpow real._root_.has_strict_deriv_at.rpow
...
theorem hasStrictDerivAtConstRpow {a : ℝ} (ha : 0 < a) (x : ℝ) : HasStrictDerivAt (fun x => a ^ x) (a ^ x * log a) x :=
  by simpa using (hasStrictDerivAtConst _ _).rpow (hasStrictDerivAtId x) ha
...
theorem hasDerivAtRpowConst {x p : ℝ} (h : x ≠ 0 ∨ 1 ≤ p) : HasDerivAt (fun x => x ^ p) (p * x ^ (p - 1)) x := by
  rcases ne_or_eq x 0 with (hx | rfl)
  · exact (has_strict_deriv_at_rpow_const_of_ne hx _).HasDerivAt
    
  replace h : 1 ≤ p := h.neg_resolve_left rfl
  apply hasDerivAtOfHasDerivAtOfNe fun x hx => (has_strict_deriv_at_rpow_const_of_ne hx p).HasDerivAt
...
theorem differentiableRpowConst {p : ℝ} (hp : 1 ≤ p) : Differentiable ℝ fun x : ℝ => x ^ p := fun x =>
  (hasDerivAtRpowConst (Or.inr hp)).DifferentiableAt
...
theorem contDiffAtRpowConstOfLe {x p : ℝ} {n : ℕ} (h : ↑n ≤ p) : ContDiffAt ℝ n (fun x : ℝ => x ^ p) x :=
  (contDiffRpowConstOfLe h).ContDiffAt
#align real.cont_diff_at_rpow_const_of_le Real.contDiffAtRpowConstOfLe

theorem contDiffAtRpowConst {x p : ℝ} {n : ℕ} (h : x ≠ 0 ∨ ↑n ≤ p) : ContDiffAt ℝ n (fun x : ℝ => x ^ p) x :=
...
theorem hasStrictDerivAtRpowConst {x p : ℝ} (hx : x ≠ 0 ∨ 1 ≤ p) :
  ...
  ContDiffAt.hasStrictDerivAt' (contDiffAtRpowConst (by rwa [Nat.cast_one])) (hasDerivAtRpowConst hx) le_rfl
...
theorem DifferentiableWithinAt.rpowConst (hf : DifferentiableWithinAt ℝ f s x) (h : f x ≠ 0 ∨ 1 ≤ p) :
...
theorem ContDiffAt.rpowConstOfNe (hf : ContDiffAt ℝ n f x) (h : f x ≠ 0) : ContDiffAt ℝ n (fun x => f x ^ p) x :=
...
theorem ContDiff.rpowConstOfNe (hf : ContDiff ℝ n f) (h : ∀ x, f x ≠ 0) : ContDiff ℝ n fun x => f x ^ p :=
...
theorem ContDiffAt.rpowConstOfLe (hf : ContDiffAt ℝ m f x) (h : ↑m ≤ p) : ContDiffAt ℝ m (fun x => f x ^ p) x := by
  rw [← cont_diff_within_at_univ] at *
  exact hf.rpow_const_of_le h
#align cont_diff_at.rpow_const_of_le ContDiffAt.rpowConstOfLe

theorem ContDiffOn.rpowConstOfLe (hf : ContDiffOn ℝ m f s) (h : ↑m ≤ p) : ContDiffOn ℝ m (fun x => f x ^ p) s :=
  fun x hx => (hf x hx).rpowConstOfLe h
