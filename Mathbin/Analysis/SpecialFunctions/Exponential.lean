...
theorem hasStrictFDerivAt_exp_zero_of_radius_pos (h : 0 < (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasStrictFDerivAt (NormedSpace.exp 𝕂) (1 : 𝔸 →L[𝕂] 𝔸) 0 :=
  by
...
theorem hasFDerivAt_exp_zero_of_radius_pos (h : 0 < (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasFDerivAt (NormedSpace.exp 𝕂) (1 : 𝔸 →L[𝕂] 𝔸) 0 :=
...
theorem hasFDerivAt_exp_of_mem_ball [CharZero 𝕂] {x : 𝔸}
    ...
    HasFDerivAt (NormedSpace.exp 𝕂) (NormedSpace.exp 𝕂 x • 1 : 𝔸 →L[𝕂] 𝔸) x :=
  by
  ...
  suffices
    (fun h =>
        NormedSpace.exp 𝕂 x *
          (NormedSpace.exp 𝕂 (0 + h) - NormedSpace.exp 𝕂 0 - ContinuousLinearMap.id 𝕂 𝔸 h)) =ᶠ[𝓝 0]
      fun h =>
      NormedSpace.exp 𝕂 (x + h) - NormedSpace.exp 𝕂 x -
        NormedSpace.exp 𝕂 x • ContinuousLinearMap.id 𝕂 𝔸 h
    by
    refine' (is_o.const_mul_left _ _).congr' this (eventually_eq.refl _ _)
    rw [← hasFDerivAt_iff_isLittleO_nhds_zero]
    exact hasFDerivAt_exp_zero_of_radius_pos hpos
  have : ∀ᶠ h in 𝓝 (0 : 𝔸), h ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius :=
...
theorem hasStrictFDerivAt_exp_of_mem_ball [CharZero 𝕂] {x : 𝔸}
    (hx : x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasStrictFDerivAt (NormedSpace.exp 𝕂) (NormedSpace.exp 𝕂 x • 1 : 𝔸 →L[𝕂] 𝔸) x :=
...
`1 : 𝔸 →L[𝕂] 𝔸` at zero. -/
theorem hasStrictFDerivAt_exp_zero : HasStrictFDerivAt (NormedSpace.exp 𝕂) (1 : 𝔸 →L[𝕂] 𝔸) 0 :=
...
`1 : 𝔸 →L[𝕂] 𝔸` at zero. -/
theorem hasFDerivAt_exp_zero : HasFDerivAt (NormedSpace.exp 𝕂) (1 : 𝔸 →L[𝕂] 𝔸) 0 :=
...
theorem hasFDerivAt_exp_smul_const_of_mem_ball (x : 𝔸) (t : 𝕊)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasFDerivAt (fun u : 𝕊 => NormedSpace.exp 𝕂 (u • x))
      (NormedSpace.exp 𝕂 (t • x) • (1 : 𝕊 →L[𝕂] 𝕊).smul_right x) t :=
  by
  ...
  suffices
    (fun h =>
        NormedSpace.exp 𝕂 (t • x) *
          (NormedSpace.exp 𝕂 ((0 + h) • x) - NormedSpace.exp 𝕂 ((0 : 𝕊) • x) -
            ((1 : 𝕊 →L[𝕂] 𝕊).smul_right x) h)) =ᶠ[𝓝 0]
      fun h =>
      NormedSpace.exp 𝕂 ((t + h) • x) - NormedSpace.exp 𝕂 (t • x) -
        (NormedSpace.exp 𝕂 (t • x) • (1 : 𝕊 →L[𝕂] 𝕊).smul_right x) h
    by
    refine' (is_o.const_mul_left _ _).congr' this (eventually_eq.refl _ _)
    rw [←
      @hasFDerivAt_iff_isLittleO_nhds_zero _ _ _ _ _ _ _ _ (fun u => NormedSpace.exp 𝕂 (u • x))
        ((1 : 𝕊 →L[𝕂] 𝕊).smul_right x) 0]
    have : HasFDerivAt (NormedSpace.exp 𝕂) (1 : 𝔸 →L[𝕂] 𝔸) ((1 : 𝕊 →L[𝕂] 𝕊).smul_right x 0) :=
      by
  ...
  have : ∀ᶠ h in 𝓝 (0 : 𝕊), h • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius :=
...
theorem hasFDerivAt_exp_smul_const_of_mem_ball' (x : 𝔸) (t : 𝕊)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasFDerivAt (fun u : 𝕊 => NormedSpace.exp 𝕂 (u • x))
      (((1 : 𝕊 →L[𝕂] 𝕊).smul_right x).smul_right (NormedSpace.exp 𝕂 (t • x))) t :=
  by
  convert hasFDerivAt_exp_smul_const_of_mem_ball 𝕂 _ _ htx using 1
  ext t'
  show Commute (t' • x) (NormedSpace.exp 𝕂 (t • x))
...
theorem hasStrictFDerivAt_exp_smul_const_of_mem_ball (x : 𝔸) (t : 𝕊)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasStrictFDerivAt (fun u : 𝕊 => NormedSpace.exp 𝕂 (u • x))
      (NormedSpace.exp 𝕂 (t • x) • (1 : 𝕊 →L[𝕂] 𝕊).smul_right x) t :=
  let ⟨p, hp⟩ := NormedSpace.analyticAt_exp_of_mem_ball (t • x) htx
  have deriv₁ : HasStrictFDerivAt (fun u : 𝕊 => NormedSpace.exp 𝕂 (u • x)) _ t :=
    hp.HasStrictFDerivAt.comp t ((ContinuousLinearMap.id 𝕂 𝕊).smul_right x).HasStrictFDerivAt
  have deriv₂ : HasFDerivAt (fun u : 𝕊 => NormedSpace.exp 𝕂 (u • x)) _ t :=
...
theorem hasStrictDerivAt_exp_smul_const_of_mem_ball (x : 𝔸) (t : 𝕂)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasStrictDerivAt (fun u : 𝕂 => NormedSpace.exp 𝕂 (u • x)) (NormedSpace.exp 𝕂 (t • x) * x) t :=
...
theorem hasStrictDerivAt_exp_smul_const_of_mem_ball' (x : 𝔸) (t : 𝕂)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasStrictDerivAt (fun u : 𝕂 => NormedSpace.exp 𝕂 (u • x)) (x * NormedSpace.exp 𝕂 (t • x)) t :=
  by simpa using (hasStrictFDerivAt_exp_smul_const_of_mem_ball' 𝕂 x t htx).HasStrictDerivAt
#align has_strict_deriv_at_exp_smul_const_of_mem_ball' hasStrictDerivAt_exp_smul_const_of_mem_ball'
-/
...
theorem hasDerivAt_exp_smul_const_of_mem_ball (x : 𝔸) (t : 𝕂)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasDerivAt (fun u : 𝕂 => NormedSpace.exp 𝕂 (u • x)) (NormedSpace.exp 𝕂 (t • x) * x) t :=
...
theorem hasDerivAt_exp_smul_const_of_mem_ball' (x : 𝔸) (t : 𝕂)
    (htx : t • x ∈ EMetric.ball (0 : 𝔸) (NormedSpace.expSeries 𝕂 𝔸).radius) :
    HasDerivAt (fun u : 𝕂 => NormedSpace.exp 𝕂 (u • x)) (x * NormedSpace.exp 𝕂 (t • x)) t :=
  (hasStrictDerivAt_exp_smul_const_of_mem_ball' x t htx).HasDerivAt
#align has_deriv_at_exp_smul_const_of_mem_ball' hasDerivAt_exp_smul_const_of_mem_ball'
-/
