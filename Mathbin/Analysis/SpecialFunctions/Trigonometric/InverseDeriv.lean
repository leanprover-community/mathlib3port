...
theorem deriv_arcsin_aux {x : ℝ} (h₁ : x ≠ -1) (h₂ : x ≠ 1) :
  ...
  · have : 0 < sqrt (1 - x ^ 2) := sqrt_pos.2 (by nlinarith [h₁, h₂])
    ...
    exact
      ⟨sin_local_homeomorph.has_strict_deriv_at_symm ⟨h₁, h₂⟩ this.ne' (hasStrictDerivAt_sin _),
        sin_local_homeomorph.cont_diff_at_symm_deriv this.ne' ⟨h₁, h₂⟩ (hasDerivAt_sin _)
...
theorem differentiableWithinAt_arcsin_Iic {x : ℝ} :
  ...
  have := (h.comp (-x) differentiable_within_at_id.neg (mapsTo_image _ _)).neg
