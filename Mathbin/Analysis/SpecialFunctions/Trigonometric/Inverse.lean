...
theorem arcsin_proj_Icc (x : ℝ) : arcsin (proj_Icc (-1) 1 (neg_le_self$ @zero_le_one ℝ _) x) = arcsin x :=
  by 
    rw [arcsin, Function.comp_app, Icc_extend_coe, Function.comp_app, Icc_extend]

theorem sin_arcsin' {x : ℝ} (hx : x ∈ Icc (-1 : ℝ) 1) : sin (arcsin x) = x :=
  by 
...
theorem arcsin_sin' {x : ℝ} (hx : x ∈ Icc (-(π / 2)) (π / 2)) : arcsin (sin x) = x :=
  inj_on_sin (arcsin_mem_Icc _) hx$
    by 
...
theorem arcsin_eq_of_sin_eq {x y : ℝ} (h₁ : sin x = y) (h₂ : x ∈ Icc (-(π / 2)) (π / 2)) : arcsin y = x :=
  by 
    subst y 
...
theorem arcsin_of_one_le {x : ℝ} (hx : 1 ≤ x) : arcsin x = π / 2 :=
  by 
...
theorem arcsin_of_le_neg_one {x : ℝ} (hx : x ≤ -1) : arcsin x = -(π / 2) :=
  by 
...
theorem le_arcsin_iff_sin_le' {x y : ℝ} (hx : x ∈ Ioc (-(π / 2)) (π / 2)) : x ≤ arcsin y ↔ sin x ≤ y :=
  by 
...
theorem arcsin_eq_iff_eq_sin {x y : ℝ} (hy : y ∈ Ioo (-(π / 2)) (π / 2)) : arcsin x = y ↔ x = sin y :=
  by 
...
theorem arcsin_nonneg {x : ℝ} : 0 ≤ arcsin x ↔ 0 ≤ x :=
  (le_arcsin_iff_sin_le' ⟨neg_lt_zero.2 pi_div_two_pos, pi_div_two_pos.le⟩).trans$
    by 
      rw [sin_zero]
...
@[simp]
theorem arcsin_eq_zero_iff {x : ℝ} : arcsin x = 0 ↔ x = 0 :=
  by 
...
theorem maps_to_sin_Ioo : maps_to sin (Ioo (-(π / 2)) (π / 2)) (Ioo (-1) 1) :=
  fun x h =>
    by 
...
theorem arcsin_eq_pi_div_two_sub_arccos (x : ℝ) : arcsin x = π / 2 - arccos x :=
  by 
    simp [arccos]

theorem arccos_le_pi (x : ℝ) : arccos x ≤ π :=
  by 
    unfold arccos <;> linarith [neg_pi_div_two_le_arcsin x]

theorem arccos_nonneg (x : ℝ) : 0 ≤ arccos x :=
  by 
...
theorem arccos_cos {x : ℝ} (hx₁ : 0 ≤ x) (hx₂ : x ≤ π) : arccos (cos x) = x :=
  by 
    rw [arccos, ←sin_pi_div_two_sub, arcsin_sin] <;> simp [sub_eq_add_neg] <;> linarith

theorem strict_anti_on_arccos : StrictAntiOn arccos (Icc (-1) 1) :=
  fun x hx y hy h => sub_lt_sub_left (strict_mono_on_arcsin hx hy h) _
...
@[simp]
theorem arccos_zero : arccos 0 = π / 2 :=
  by 
    simp [arccos]

@[simp]
theorem arccos_one : arccos 1 = 0 :=
  by 
    simp [arccos]

@[simp]
theorem arccos_neg_one : arccos (-1) = π :=
  by 
    simp [arccos, add_halves]

@[simp]
theorem arccos_eq_zero {x} : arccos x = 0 ↔ 1 ≤ x :=
  by 
    simp [arccos, sub_eq_zero]

@[simp]
theorem arccos_eq_pi_div_two {x} : arccos x = π / 2 ↔ x = 0 :=
  by 
    simp [arccos, sub_eq_iff_eq_add]

@[simp]
theorem arccos_eq_pi {x} : arccos x = π ↔ x ≤ -1 :=
  by 
    rw [arccos, sub_eq_iff_eq_add, ←sub_eq_iff_eq_add', div_two_sub_self, neg_pi_div_two_eq_arcsin]

theorem arccos_neg (x : ℝ) : arccos (-x) = π - arccos x :=
  by 
    rw [←add_halves π, arccos, arcsin_neg, arccos, add_sub_assoc, sub_sub_self, sub_neg_eq_add]

theorem sin_arccos {x : ℝ} (hx₁ : -1 ≤ x) (hx₂ : x ≤ 1) : sin (arccos x) = sqrt (1 - (x^2)) :=
  by 
