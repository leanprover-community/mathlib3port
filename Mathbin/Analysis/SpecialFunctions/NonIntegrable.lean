...
variable {E F : Type _} [NormedGroup E] [NormedSpace ℝ E] [MeasurableSpace E] [BorelSpace E]
  [second_countable_topology E] [CompleteSpace E] [NormedGroup F] [MeasurableSpace F] [BorelSpace F]
...
theorem not_interval_integrable_of_tendsto_norm_at_top_of_deriv_is_O_filter {f : ℝ → E} {g : ℝ → F} {a b : ℝ}
    (l : Filter ℝ) [ne_bot l] [tendsto_Ixx_class Icc l l]
    (hl : "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" ∈ l)
    (hd : ∀ᶠ x in l, DifferentiableAt ℝ f x) (hf : tendsto (fun x => ∥f x∥) l at_top) (hfg : is_O (deriv f) g l) :
  ...
  obtain ⟨C, hC₀, s, hsl, hsub, hfd, hg⟩ :
          ...
          (∀,
              ∀ x ∈ s,
                ∀,
                  ∀ y ∈ s,
                    ∀,
                      ∀ z ∈ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)",
    ...
    have h :
      ∀ᶠ x : ℝ × ℝ in l.prod l,
        ∀,
          ∀ y ∈ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)",
            ∀,
...
theorem not_interval_integrable_of_tendsto_norm_at_top_of_deriv_is_O_within_diff_singleton {f : ℝ → E} {g : ℝ → F}
    {a b c : ℝ} (hne : a ≠ b) (hc : c ∈ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)")
    (h_deriv :
      ∀ᶠ x in 𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c}] c,
        DifferentiableAt ℝ f x)
    (h_infty :
      tendsto (fun x => ∥f x∥) (𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c}] c)
        at_top)
    (hg : is_O (deriv f) g (𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c}] c)) :
    ¬IntervalIntegrable g volume a b := by
  obtain ⟨l, hl, hl', hle, hmem⟩ :
    ∃ l : Filter ℝ,
      tendsto_Ixx_class Icc l l ∧
        l.ne_bot ∧ l ≤ 𝓝 c ∧ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c} ∈ l :=
    by
  ...
  skip
  have : l ≤ 𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c}] c :=
...
theorem not_interval_integrable_of_tendsto_norm_at_top_of_deriv_is_O_punctured {f : ℝ → E} {g : ℝ → F} {a b c : ℝ}
    (h_deriv : ∀ᶠ x in 𝓝[≠] c, DifferentiableAt ℝ f x) (h_infty : tendsto (fun x => ∥f x∥) (𝓝[≠] c) at_top)
    (hg : is_O (deriv f) g (𝓝[≠] c)) (hne : a ≠ b)
    (hc : c ∈ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)") :
    ¬IntervalIntegrable g volume a b :=
  have : 𝓝["././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" \ {c}] c ≤ 𝓝[≠] c :=
...
theorem not_interval_integrable_of_sub_inv_is_O_punctured {f : ℝ → F} {a b c : ℝ}
    (hf : is_O (fun x => (x - c)⁻¹) f (𝓝[≠] c)) (hne : a ≠ b)
    (hc : c ∈ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)") :
  ...
  exact
    not_interval_integrable_of_tendsto_norm_at_top_of_deriv_is_O_punctured (A.mono fun x hx => hx.differentiable_at) B
...
theorem interval_integrable_sub_inv_iff {a b c : ℝ} :
    IntervalIntegrable (fun x => (x - c)⁻¹) volume a b ↔
      a = b ∨ c ∉ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" :=
  by
  constructor
...
theorem interval_integrable_inv_iff {a b : ℝ} :
    IntervalIntegrable (fun x => x⁻¹) volume a b ↔
      a = b ∨ (0 : ℝ) ∉ "././Mathport/Syntax/Translate/Basic.lean:694:47: unsupported (impossible)" :=
  by
