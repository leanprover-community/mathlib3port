...
theorem isEquivalent_atTop_lead :
  ...
  · simp only [Polynomial.eval_eq_sum_range, sum_range_succ]
    exact
      IsOCat.add_isEquivalent
        (IsOCat.sum fun i hi =>
          IsOCat.const_mul_left
            ((IsOCat.const_mul_right fun hz => h <| leading_coeff_eq_zero.mp hz) <|
              isOCat_pow_pow_atTop_of_lt (mem_range.mp hi))
            _)
        IsEquivalent.refl
...
theorem tendsto_atTop_iff_leadingCoeff_nonneg :
  ...
  have : Tendsto (fun x => P.leading_coeff * x ^ P.nat_degree) atTop atTop :=
...
theorem tendsto_atBot_iff_leadingCoeff_nonpos :
  ...
  simp only [← tendsto_neg_atTop_iff, ← eval_neg, tendsto_atTop_iff_leadingCoeff_nonneg, degree_neg,
    leadingCoeff_neg, neg_nonneg]
...
theorem tendsto_nhds_iff {c : 𝕜} :
  ...
  · refine' P.is_equivalent_at_top_lead.symm.tendsto_nhds _
    have : P.nat_degree = 0 := natDegree_eq_zero_iff_degree_le_zero.2 h.2
...
theorem div_tendsto_leadingCoeff_div_of_degree_eq (hdeg : P.degree = Q.degree) :
  ...
  by
  refine' (isEquivalent_atTop_div P Q).symm.tendsto_nhds _
  rw [show (P.nat_degree : ℤ) = Q.nat_degree by simp [hdeg, natDegree]]
...
theorem isO_of_degree_le (h : P.degree ≤ Q.degree) :
  ...
  by
  by_cases hp : P = 0
  · simpa [hp] using isO_zero (fun x => eval x Q) atTop
  · have hq : Q ≠ 0 := ne_zero_of_degree_ge_degree h hp
    ...
    · exact isO_of_div_tendsto_nhds hPQ 0 (div_tendsto_zero_of_degree_lt P Q h)
    · exact isO_of_div_tendsto_nhds hPQ _ (div_tendsto_leadingCoeff_div_of_degree_eq P Q h)
