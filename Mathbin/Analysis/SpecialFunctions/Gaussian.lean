...
theorem integrableExpNegMulSq {b : ℝ} (hb : 0 < b) : Integrable fun x : ℝ => exp (-b * x ^ 2) :=
  by
...
theorem integrableMulExpNegMulSq {b : ℝ} (hb : 0 < b) :
    Integrable fun x : ℝ => x * exp (-b * x ^ 2) :=
  by
...
theorem integral_gaussian (b : ℝ) : (∫ x, exp (-b * x ^ 2)) = sqrt (π / b) :=
  ...
  calc
    (∫ x, Real.exp (-b * x ^ 2)) ^ 2 = ∫ p : ℝ × ℝ, exp (-b * p.1 ^ 2) * exp (-b * p.2 ^ 2) :=
      by
      rw [pow_two, ← integral_prod_mul]
      rfl
    _ = ∫ p : ℝ × ℝ, Real.exp (-b * (p.1 ^ 2 + p.2 ^ 2)) :=
      by
      congr
      ext p
    ...
    _ = (∫ r in Ioi (0 : ℝ), r * exp (-b * r ^ 2)) * ∫ θ in Ioo (-π) π, 1 :=
      by
