...
theorem exp_neg_mul_sq_isOCat_exp_neg {b : ℝ} (hb : 0 < b) :
  ...
  have A : (fun x : ℝ => -x - -b * x ^ 2) = fun x => x * (b * x + -1) :=
    ...
    ext x
    ring
  rw [isOCat_exp_comp_exp_comp, A]
  apply Tendsto.atTop_mul_atTop tendsto_id
  apply tendsto_atTop_add_const_right atTop (-1 : ℝ)
  exact Tendsto.const_mul_atTop hb tendsto_id
...
theorem rpow_mul_exp_neg_mul_sq_isOCat_exp_neg {b : ℝ} (hb : 0 < b) (s : ℝ) :
  ...
  by
  apply ((isO_refl (fun x : ℝ => x ^ s) atTop).mul_isOCat (exp_neg_mul_sq_isOCat_exp_neg hb)).trans
  convert Gamma_integrand_isOCat s
  simp_rw [mul_comm]
...
theorem integrableOnRpowMulExpNegMulSq {b : ℝ} (hb : 0 < b) {s : ℝ} (hs : -1 < s) :
  ...
  · rw [← integrableOn_Icc_iff_integrableOn_Ioc]
    refine' IntegrableOn.mulContinuousOn _ _ isCompact_Icc
  ...
  · have B : (0 : ℝ) < 1 / 2 := by norm_num
    apply integrableOfIsOExpNeg B _ (IsOCat.isO (rpow_mul_exp_neg_mul_sq_isOCat_exp_neg hb _))
    intro x hx
    have N : x ≠ 0 := by
...
theorem integral_mul_cexp_neg_mul_sq {b : ℂ} (hb : 0 < b.re) :
  ...
  refine'
    tendsto_nhds_unique
      (intervalIntegral_tendsto_integral_Ioi _ (integrableMulCexpNegMulSq hb).integrableOn
        Filter.tendsto_id)
      _
  have A :
  ...
  simp_rw [this]
  have L :
    Tendsto (fun x : ℝ => (2 * b)⁻¹ - (2 * b)⁻¹ * cexp (-b * x ^ 2)) atTop
      (𝓝 ((2 * b)⁻¹ - (2 * b)⁻¹ * 0)) :=
    by
    ...
    exact
      tendsto_exp_at_bot.comp
        (Tendsto.neg_const_mul_atTop (neg_lt_zero.2 hb) (tendsto_pow_atTop two_ne_zero))
  simpa using L
...
theorem continuousAt_gaussian_integral (b : ℂ) (hb : 0 < re b) :
  ...
  exact
    continuousAt_of_dominated (eventually_of_forall f_meas) f_le_bd (integrableExpNegMulSq hd)
      (ae_of_all _ f_cts)
...
theorem integral_gaussian_complex_Ioi {b : ℂ} (hb : 0 < re b) :
  ...
  have : ∀ c : ℝ, (∫ x in 0 ..c, cexp (-b * x ^ 2)) = ∫ x in -c..0, cexp (-b * x ^ 2) :=
    by
    intro c
    have := @interval_integral.integral_comp_sub_left _ _ _ _ 0 c (fun x => cexp (-b * x ^ 2)) 0
    simpa [zero_sub, neg_sq, neg_zero] using this
  have t1 :=
    intervalIntegral_tendsto_integral_Ioi _ (integrableCexpNegMulSq hb).IntegrableOn tendsto_id
  have t2 :
    Tendsto (fun c : ℝ => ∫ x : ℝ in 0 ..c, cexp (-b * x ^ 2)) atTop
...
theorem integrableCexpNegMulSqAddRealMulI (hb : 0 < b.re) (c : ℝ) :
  ...
  suffices Integrable fun x : ℝ => exp (-(b.re * x ^ 2)) by
    exact (Integrable.comp_sub_right this (b.im * c / b.re)).hasFiniteIntegral.constMul _
  simp_rw [← neg_mul]
...
theorem integral_cexp_neg_mul_sq_add_real_mul_i (hb : 0 < b.re) (c : ℝ) :
  ...
  refine'
    tendsto_nhds_unique
      (intervalIntegral_tendsto_integral (integrableCexpNegMulSqAddRealMulI hb c)
  ...
  exact
    intervalIntegral_tendsto_integral (integrableCexpNegMulSq hb) tendsto_neg_atTop_atBot tendsto_id
...
theorem fourier_transform_gaussian_pi (hb : 0 < b.re) (t : ℂ) :
  ...
  have h2 : b ≠ 0 := by
    contrapose! hb
    rw [hb, zero_re]
  convert fourier_transform_gaussian h1 (2 * π * t) using 1
  · congr 1
    ext1 x
    congr 2
    all_goals ring
