...
theorem integral_mul_cexp_neg_mul_sq {b : ℂ} (hb : 0 < b.re) :
  ...
  have :
    ...
    fun y =>
    intervalIntegral.integral_eq_sub_of_has_deriv_at (fun x hx => (A x).comp_of_real)
...
theorem continuous_at_gaussian_integral (b : ℂ) (hb : 0 < re b) :
  ...
  exact
    continuous_at_of_dominated (eventually_of_forall f_meas) f_le_bd (integrableExpNegMulSq hd)
      (ae_of_all _ f_cts)
#align continuous_at_gaussian_integral continuous_at_gaussian_integral
...
theorem integral_gaussian_complex {b : ℂ} (hb : 0 < re b) :
  ...
  ·-- integral is continuous
    exact ContinuousAt.continuous_on continuous_at_gaussian_integral
  · -- `(π / b) ^ (1 / 2 : ℂ)` is continuous
    refine'
      ContinuousAt.continuous_on fun b hb =>
        (continuous_at_cpow_const (Or.inl _)).comp
