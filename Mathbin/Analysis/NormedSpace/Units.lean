...
theorem inverse_one_sub (t : R) (h : ∥t∥ < 1) : inverse (1 - t) = ↑Units.oneSub t h⁻¹ :=
  by 
    rw [←inverse_unit (Units.oneSub t h), Units.coe_one_sub]

/-- The formula `inverse (x + t) = inverse (1 + x⁻¹ * t) * x⁻¹` holds for `t` sufficiently small. -/
theorem inverse_add (x : Units R) : ∀ᶠ t in 𝓝 0, inverse ((x : R)+t) = inverse (1+(↑x⁻¹)*t)*↑x⁻¹ :=
  by 
    nontriviality R 
...
theorem inverse_add_norm (x : Units R) : is_O (fun t => inverse ((↑x)+t)) (fun t => (1 : ℝ)) (𝓝 (0 : R)) :=
  by 
...
theorem inverse_add_norm_diff_nth_order (x : Units R) (n : ℕ) :
  is_O (fun t : R => inverse ((↑x)+t) - (∑ i in range n, ((-↑x⁻¹)*t) ^ i)*↑x⁻¹) (fun t => ∥t∥ ^ n) (𝓝 (0 : R)) :=
  by 
    byCases' h : n = 0
    ·
    ...
    ·
      calc ∥(-(↑x⁻¹)*t) ^ n∥ ≤ ∥-(↑x⁻¹)*t∥ ^ n := norm_pow_le' _ hn _ = ∥(↑x⁻¹)*t∥ ^ n :=
        by 
          rw [norm_neg]_ ≤ (∥(↑x⁻¹ : R)∥*∥t∥) ^ n :=
        _ _ = (∥(↑x⁻¹ : R)∥ ^ n)*∥t∥ ^ n := mul_powₓ _ _ n 
...
theorem inverse_add_norm_diff_first_order (x : Units R) :
  is_O (fun t => inverse ((↑x)+t) - ↑x⁻¹) (fun t => ∥t∥) (𝓝 (0 : R)) :=
  by 
...
theorem inverse_add_norm_diff_second_order (x : Units R) :
  is_O (fun t => (inverse ((↑x)+t) - ↑x⁻¹)+((↑x⁻¹)*t)*↑x⁻¹) (fun t => ∥t∥ ^ 2) (𝓝 (0 : R)) :=
  by 
...
theorem inverse_continuous_at (x : Units R) : ContinuousAt inverse (x : R) :=
  by 
    have h_is_o : is_o (fun t : R => ∥inverse ((↑x)+t) - ↑x⁻¹∥) (fun t : R => (1 : ℝ)) (𝓝 0)
    ·
...
theorem is_open_map_coe : IsOpenMap (coeₓ : Units R → R) :=
  by 
    ...
    rintro ⟨t, ht, hts⟩
    obtain ⟨u, hu, v, hv, huvt⟩ : ∃ u : Set R, u ∈ 𝓝 (↑x) ∧ ∃ v : Set (Rᵐᵒᵖ), v ∈ 𝓝 (op (↑x⁻¹)) ∧ u.prod v ⊆ t
    ·
      simpa [embedProduct, mem_nhds_prod_iff] using ht 
    have  : u ∩ (op ∘ Ring.inverse) ⁻¹' v ∩ Set.Range (coeₓ : Units R → R) ∈ 𝓝 (↑x)
    ·
