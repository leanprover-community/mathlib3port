...
theorem norm_image_of_norm_zero {f : E →ₛₗ[σ₁₂] F} (hf : Continuous f) {x : E} (hx : ∥x∥ = 0) : ∥f x∥ = 0 :=
  by 
...
theorem op_norm_neg (f : E →SL[σ₁₂] F) : ∥-f∥ = ∥f∥ :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » ball (0 : E) ε)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » linear_map.ker f)
...
theorem homothety_norm [Nontrivial E] (f : E →SL[σ₁₂] F) {a : ℝ} (hf : ∀ x, ∥f x∥ = a*∥x∥) : ∥f∥ = a :=
  by 
...
theorem antilipschitz_of_uniform_embedding (f : E →L[𝕜] Fₗ) (hf : UniformEmbedding f) : ∃ K, AntilipschitzWith K f :=
  by 
...
instance {E : Type _} [SemiNormedGroup E] [SemiNormedSpace 𝕜 E] [CompleteSpace F] : CompleteSpace (E →SL[σ₁₂] F) :=
  by 
...
@[simp]
theorem norm_smul_right_apply (c : E →L[𝕜] 𝕜) (f : Fₗ) : ∥smul_right c f∥ = ∥c∥*∥f∥ :=
  by 
    refine' le_antisymmₓ _ _
    ·
