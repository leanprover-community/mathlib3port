...
protected theorem edist_comm (f g : PiLp p β) : edist f g = edist g f :=
  ...
  · simp only [edist_eq_supᵢ, edist_comm]
...
def pseudoMetricAux : PseudoMetricSpace (PiLp p α) :=
  PseudoEmetricSpace.toPseudoMetricSpaceOfDist dist
    (fun f g => by
      ...
      · exact supᵢ_edist_ne_top_aux f g
...
theorem lipschitzWith_equiv_aux : LipschitzWith 1 (PiLp.equiv p β) :=
  ...
  rcases p.dichotomy with (rfl | h)
  ·
    simpa only [Ennreal.coe_one, one_mul, edist_eq_supᵢ, edist, Finset.sup_le_iff, Finset.mem_univ,
...
theorem antilipschitzWith_equiv_aux :
  ...
  intro x y
  rcases p.dichotomy with (rfl | h)
  · simp only [edist_eq_supᵢ, Ennreal.div_top, Ennreal.zero_toReal, Nnreal.rpow_zero,
...
def LinearIsometryEquiv.piLpCongrLeft (e : ι ≃ ι') :
  ...
  norm_map' x := by
    rcases p.dichotomy with (rfl | h)
    · simp_rw [norm_eq_csupr, LinearEquiv.piCongrLeft'_apply 𝕜 (fun i : ι => E) e x _]
      exact e.symm.supr_congr fun i => rfl
    · simp only [norm_eq_sum (zero_lt_one.trans_le h)]
      simp_rw [LinearEquiv.piCongrLeft'_apply 𝕜 (fun i : ι => E) e x _]
      congr
