...
theorem exp_mem_exp [RCLike 𝕜] [NormedRing A] [NormedAlgebra 𝕜 A] [CompleteSpace A] (a : A) {z : 𝕜}
    (hz : z ∈ spectrum 𝕜 a) : NormedSpace.exp 𝕜 z ∈ spectrum 𝕜 (NormedSpace.exp 𝕜 a) :=
  by
  have hexpmul : NormedSpace.exp 𝕜 a = NormedSpace.exp 𝕜 (a - ↑ₐ z) * ↑ₐ (NormedSpace.exp 𝕜 z) := by
  ...
  have hb : Summable fun n : ℕ => ((n + 1).factorial⁻¹ : 𝕜) • (a - ↑ₐ z) ^ n :=
    ...
    exact
      div_le_div (Nonneg.pow_nonneg (norm_nonneg _) n)
        (norm_pow_le' (a - ↑ₐ z) (zero_lt_iff.mpr hn)) (by exact_mod_cast Nat.factorial_pos n)
  ...
  have h₃ : NormedSpace.exp 𝕜 (a - ↑ₐ z) = 1 + (a - ↑ₐ z) * b :=
    by
    rw [NormedSpace.exp_eq_tsum]
    convert tsum_eq_zero_add (NormedSpace.expSeries_summable' (a - ↑ₐ z))
    simp only [Nat.factorial_zero, Nat.cast_one, inv_one, pow_zero, one_smul]
    exact h₀.symm
  rw [spectrum.mem_iff, IsUnit.sub_iff, ← one_mul (↑ₐ (NormedSpace.exp 𝕜 z)), hexpmul, ←
    _root_.sub_mul,
    Commute.isUnit_mul_iff
      (Algebra.commutes (NormedSpace.exp 𝕜 z) (NormedSpace.exp 𝕜 (a - ↑ₐ z) - 1)).symm,
