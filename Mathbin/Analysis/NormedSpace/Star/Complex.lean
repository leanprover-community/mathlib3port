...
def mulNegILin : skewAdjoint E ≃ₗ[ℝ] selfAdjoint E where
  toFun := fun x => ⟨-I • x, by simp [selfAdjoint.mem_iff]⟩
  invFun := fun x => ⟨I • x, by simp [skewAdjoint.mem_iff]⟩
  map_add' := fun x y => by
    ext
  ...
  right_inv := fun x => by simp only [← mul_smul, I_mul_I, AddSubgroup.coe_mk, neg_mul, neg_negₓ, one_smul, SetLike.eta]
...
"imaginary" parts -/
theorem re_add_im (x : E) : (re x : E) + I • im x = x := by simp [← mul_smul, I_mul_I, ← smul_add, ← two_smul ℝ]
