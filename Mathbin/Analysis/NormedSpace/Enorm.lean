...
theorem map_smul (c : 𝕜) (x : V) : e (c • x) = nnnorm c*e x :=
  le_antisymmₓ (e.map_smul_le' c x)$
    by 
      byCases' hc : c = 0
      ·
        simp [hc]
      calc ((nnnorm c : ℝ≥0∞)*e x) = nnnorm c*e (c⁻¹ • c • x) :=
        by 
          rw [inv_smul_smul₀ hc]_ ≤ nnnorm c*nnnorm (c⁻¹)*e (c • x) :=
        _ _ = e (c • x) := _
      ·
...
theorem map_sub_rev (x y : V) : e (x - y) = e (y - x) :=
  by 
