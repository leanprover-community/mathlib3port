...
attribute [local instance 1001] AddCommGroup.toAddCommMonoid NormedAddCommGroup.toAddCommGroup
...
#print MultilinearMap.bound_of_shell /-
...
#print MultilinearMap.exists_bound_of_continuous /-
...
theorem exists_bound_of_continuous (hf : Continuous f) :
  ...
  · refine' ⟨‖f 0‖ + 1, add_pos_of_nonneg_of_pos (norm_nonneg _) zero_lt_one, fun m => _⟩
    obtain rfl : m = 0; exact funext (IsEmpty.elim ‹_›)
...
#print MultilinearMap.norm_image_sub_le_of_bound' /-
  ...
  have A :
    ...
    calc
      ...
      _ = ‖m₁ - m₂‖ * max ‖m₁‖ ‖m₂‖ ^ (Fintype.card ι - 1) := by
        rw [prod_update_of_mem (Finset.mem_univ _)]; simp [card_univ_diff]
  calc
    ‖f m₁ - f m₂‖ ≤ C * ∑ i, ∏ j, if j = i then ‖m₁ i - m₂ i‖ else max ‖m₁ j‖ ‖m₂ j‖ :=
    ...
    _ = C * Fintype.card ι * max ‖m₁‖ ‖m₂‖ ^ (Fintype.card ι - 1) * ‖m₁ - m₂‖ := by
      rw [sum_const, card_univ, nsmul_eq_mul]; ring
#align multilinear_map.norm_image_sub_le_of_bound MultilinearMap.norm_image_sub_le_of_bound
-/

#print MultilinearMap.continuous_of_bound /-
...
#print MultilinearMap.coe_mkContinuous /-
@[simp]
...
#print MultilinearMap.restr_norm_le /-
...
#print ContinuousMultilinearMap.bound /-
...
#print ContinuousMultilinearMap.hasOpNorm /-
...
#print ContinuousMultilinearMap.bounds_bddBelow /-
...
#print ContinuousMultilinearMap.opNorm_nonneg /-
...
#print ContinuousMultilinearMap.le_opNorm /-
/-- The fundamental property of the operator norm of a continuous multilinear map:
`‖f m‖` is bounded by `‖f‖` times the product of the `‖m i‖`. -/
theorem le_opNorm : ‖f m‖ ≤ ‖f‖ * ∏ i, ‖m i‖ :=
  by
...
#print ContinuousMultilinearMap.le_of_opNorm_le /-
theorem le_of_opNorm_le {C : ℝ} (h : ‖f‖ ≤ C) : ‖f m‖ ≤ C * ∏ i, ‖m i‖ :=
  (f.le_opNorm m).trans <| mul_le_mul_of_nonneg_right h (prod_nonneg fun i _ => norm_nonneg (m i))
#align continuous_multilinear_map.le_of_op_norm_le ContinuousMultilinearMap.le_of_opNorm_le
-/

#print ContinuousMultilinearMap.ratio_le_opNorm /-
theorem ratio_le_opNorm : ‖f m‖ / ∏ i, ‖m i‖ ≤ ‖f‖ :=
  div_le_of_nonneg_of_le_mul (prod_nonneg fun i _ => norm_nonneg _) (opNorm_nonneg _)
    (f.le_opNorm m)
#align continuous_multilinear_map.ratio_le_op_norm ContinuousMultilinearMap.ratio_le_opNorm
-/

#print ContinuousMultilinearMap.unit_le_opNorm /-
/-- The image of the unit ball under a continuous multilinear map is bounded. -/
theorem unit_le_opNorm (h : ‖m‖ ≤ 1) : ‖f m‖ ≤ ‖f‖ :=
  calc
    ‖f m‖ ≤ ‖f‖ * ∏ i, ‖m i‖ := f.le_opNorm m
    _ ≤ ‖f‖ * ∏ i : ι, 1 :=
...
#print ContinuousMultilinearMap.opNorm_le_bound /-
...
#print ContinuousMultilinearMap.opNorm_add_le /-
/-- The operator norm satisfies the triangle inequality. -/
theorem opNorm_add_le : ‖f + g‖ ≤ ‖f‖ + ‖g‖ :=
  csInf_le bounds_bddBelow
    ⟨add_nonneg (opNorm_nonneg _) (opNorm_nonneg _), fun x =>
      by
      rw [add_mul]
...
#print ContinuousMultilinearMap.opNorm_zero /-
...
#print ContinuousMultilinearMap.opNorm_zero_iff /-
/-- A continuous linear map is zero iff its norm vanishes. -/
theorem opNorm_zero_iff : ‖f‖ = 0 ↔ f = 0 :=
  ⟨fun h => by ext m; simpa [h] using f.le_op_norm m, by rintro rfl; exact op_norm_zero⟩
...
#print ContinuousMultilinearMap.opNorm_smul_le /-
theorem opNorm_smul_le (c : 𝕜') : ‖c • f‖ ≤ ‖c‖ * ‖f‖ :=
  (c • f).opNorm_le_bound (mul_nonneg (norm_nonneg _) (opNorm_nonneg _))
    (by
      intro m
...
#print ContinuousMultilinearMap.opNorm_neg /-
theorem opNorm_neg : ‖-f‖ = ‖f‖ := by rw [norm_def]; apply congr_arg; ext; simp
#align continuous_multilinear_map.op_norm_neg ContinuousMultilinearMap.opNorm_neg
-/

#print ContinuousMultilinearMap.normedAddCommGroup /-
...
instance normedAddCommGroup : NormedAddCommGroup (ContinuousMultilinearMap 𝕜 E G) :=
  AddGroupNorm.toNormedAddCommGroup
    { toFun := norm
      ...
      eq_zero_of_map_eq_zero' := fun f => f.opNorm_zero_iff.1 }
...
#print ContinuousMultilinearMap.normedSpace /-
...
#print ContinuousMultilinearMap.le_opNorm_mul_prod_of_le /-
...
#print ContinuousMultilinearMap.le_opNorm_mul_pow_card_of_le /-
theorem le_opNorm_mul_pow_card_of_le {b : ℝ} (hm : ∀ i, ‖m i‖ ≤ b) :
...
#print ContinuousMultilinearMap.le_opNorm_mul_pow_of_le /-
theorem le_opNorm_mul_pow_of_le {Ei : Fin n → Type _} [∀ i, NormedAddCommGroup (Ei i)]
...
#print ContinuousMultilinearMap.le_opNNNorm /-
/-- The fundamental property of the operator norm of a continuous multilinear map:
...
#print ContinuousMultilinearMap.le_of_opNNNorm_le /-
...
theorem opNorm_prod (f : ContinuousMultilinearMap 𝕜 E G) (g : ContinuousMultilinearMap 𝕜 E G') :
  ...
  le_antisymm
    ...
    max_le
      (f.opNorm_le_bound (norm_nonneg _) fun m => (le_max_left _ _).trans ((f.Prod g).le_opNorm _))
      (g.opNorm_le_bound (norm_nonneg _) fun m => (le_max_right _ _).trans ((f.Prod g).le_opNorm _))
#align continuous_multilinear_map.op_norm_prod ContinuousMultilinearMap.opNorm_prod
-/

#print ContinuousMultilinearMap.opNorm_pi /-
theorem opNorm_pi {ι' : Type v'} [Fintype ι'] {E' : ι' → Type wE'}
  ...
  · refine' op_norm_le_bound _ (norm_nonneg f) fun m => _
    ...
    exacts [fun i => (f i).le_of_opNorm_le m (norm_le_pi_norm f i),
  ...
  · refine' (pi_norm_le_iff_of_nonneg (norm_nonneg _)).2 fun i => _
    ...
    refine' le_trans _ ((pi f).le_opNorm m)
...
theorem norm_ofSubsingleton_id_le [Subsingleton ι] (i' : ι) : ‖ofSubsingleton 𝕜 G i'‖ ≤ 1 :=
  opNorm_le_bound _ zero_le_one fun m => by
...
#print ContinuousMultilinearMap.norm_constOfIsEmpty /-
@[simp]
...
#print ContinuousMultilinearMap.prodL /-
...
#print ContinuousMultilinearMap.piₗᵢ /-
...
#print ContinuousMultilinearMap.norm_restrictScalars /-
@[simp]
theorem norm_restrictScalars : ‖f.restrictScalars 𝕜'‖ = ‖f‖ :=
  rfl
...
#print ContinuousMultilinearMap.continuous_restrictScalars /-
...
#print ContinuousMultilinearMap.norm_image_sub_le' /-
...
#print ContinuousMultilinearMap.continuous_eval /-
...
#print ContinuousMultilinearMap.tsum_eval /-
...
#print MultilinearMap.mkContinuous_norm_le /-
...
#print ContinuousMultilinearMap.restr /-
...
#print ContinuousMultilinearMap.norm_restr /-
...
#print ContinuousMultilinearMap.norm_mkPiAlgebra_le /-
@[simp]
...
#print ContinuousMultilinearMap.norm_mkPiAlgebra_of_empty /-
...
#print ContinuousMultilinearMap.norm_mkPiAlgebra /-
@[simp]
...
#print ContinuousMultilinearMap.norm_mkPiAlgebraFin_succ_le /-
...
#print ContinuousMultilinearMap.norm_mkPiAlgebraFin_le_of_pos /-
...
#print ContinuousMultilinearMap.norm_mkPiAlgebraFin_zero /-
...
#print ContinuousMultilinearMap.norm_mkPiAlgebraFin /-
@[simp]
...
#print ContinuousMultilinearMap.mkPiRing /-
/-- The canonical continuous multilinear map on `𝕜^ι`, associating to `m` the product of all the
`m i` (multiplied by a fixed reference element `z` in the target module) -/
protected def mkPiRing (z : G) : ContinuousMultilinearMap 𝕜 (fun i : ι => 𝕜) G :=
...
#print ContinuousMultilinearMap.mkPiRing_apply /-
@[simp]
theorem mkPiRing_apply (z : G) (m : ι → 𝕜) :
    (ContinuousMultilinearMap.mkPiRing 𝕜 ι z : (ι → 𝕜) → G) m = (∏ i, m i) • z :=
  rfl
#align continuous_multilinear_map.mk_pi_field_apply ContinuousMultilinearMap.mkPiRing_apply
-/

#print ContinuousMultilinearMap.mkPiRing_apply_one_eq_self /-
theorem mkPiRing_apply_one_eq_self (f : ContinuousMultilinearMap 𝕜 (fun i : ι => 𝕜) G) :
    ContinuousMultilinearMap.mkPiRing 𝕜 ι (f fun i => 1) = f :=
...
#print ContinuousMultilinearMap.norm_mkPiRing /-
@[simp]
theorem norm_mkPiRing (z : G) : ‖ContinuousMultilinearMap.mkPiRing 𝕜 ι z‖ = ‖z‖ :=
  (MultilinearMap.mkContinuous_norm_le _ (norm_nonneg z) _).antisymm <| by
    simpa using (ContinuousMultilinearMap.mkPiRing 𝕜 ι z).le_opNorm fun _ => 1
#align continuous_multilinear_map.norm_mk_pi_field ContinuousMultilinearMap.norm_mkPiRing
-/

#print ContinuousMultilinearMap.mkPiRing_eq_iff /-
theorem mkPiRing_eq_iff {z₁ z₂ : G} :
    ContinuousMultilinearMap.mkPiRing 𝕜 ι z₁ = ContinuousMultilinearMap.mkPiRing 𝕜 ι z₂ ↔ z₁ = z₂ :=
  by
...
#print ContinuousMultilinearMap.mkPiRing_zero /-
theorem mkPiRing_zero : ContinuousMultilinearMap.mkPiRing 𝕜 ι (0 : G) = 0 := by
...
#print ContinuousMultilinearMap.mkPiRing_eq_zero_iff /-
theorem mkPiRing_eq_zero_iff (z : G) : ContinuousMultilinearMap.mkPiRing 𝕜 ι z = 0 ↔ z = 0 := by
...
#print ContinuousMultilinearMap.piFieldEquiv /-
...
#print ContinuousLinearMap.norm_compContinuousMultilinearMap_le /-
...
#print ContinuousLinearMap.compContinuousMultilinearMapL /-
...
def compContinuousMultilinearMapL :
  ...
  LinearMap.mkContinuous₂
    ...
    1 fun f g => by rw [one_mul]; exact f.norm_comp_continuous_multilinear_map_le g
#align continuous_linear_map.comp_continuous_multilinear_mapL ContinuousLinearMap.compContinuousMultilinearMapL
-/

variable {𝕜 G G'}

#print ContinuousLinearEquiv.compContinuousMultilinearMapL /-
...
#print ContinuousLinearEquiv.compContinuousMultilinearMapL_symm /-
@[simp]
...
#print ContinuousLinearEquiv.compContinuousMultilinearMapL_apply /-
@[simp]
...
#print ContinuousLinearMap.flipMultilinear /-
  ...
  MultilinearMap.mkContinuous
    { toFun := fun m =>
        LinearMap.mkContinuous
          ...
          (‖f‖ * ∏ i, ‖m i‖) fun x => by rw [mul_right_comm];
          exact (f x).le_of_opNorm_le _ (f.le_op_norm x)
      map_add' := fun _ m i x y => by ext1;
        simp only [add_apply, ContinuousMultilinearMap.map_add, LinearMap.coe_mk,
          LinearMap.mkContinuous_apply]
      map_smul' := fun _ m i c x => by ext1;
...
#print MultilinearMap.mkContinuousLinear /-
...
def mkContinuousLinear (f : G →ₗ[𝕜] MultilinearMap 𝕜 E G') (C : ℝ)
  ...
  LinearMap.mkContinuous
    { toFun := fun x => (f x).mkContinuous (C * ‖x‖) <| H x
      map_add' := fun x y => by ext1; simp only [_root_.map_add]; rfl
      map_smul' := fun c x => by ext1; simp only [MulActionSemiHomClass.map_smul]; rfl } (max C 0)
    fun x =>
...
#print MultilinearMap.mkContinuousLinear_norm_le' /-
...
#print MultilinearMap.mkContinuousMultilinear /-
...
def mkContinuousMultilinear (f : MultilinearMap 𝕜 E (MultilinearMap 𝕜 E' G)) (C : ℝ)
  ...
  mkContinuous
    { toFun := fun m => mkContinuous (f m) (C * ∏ i, ‖m i‖) <| H m
      map_add' := fun _ m i x y => by ext1; simp
      map_smul' := fun _ m i c x => by ext1; simp } (max C 0) fun m =>
    ((f m).mkContinuous_norm_le' _).trans_eq <| by rw [max_mul_of_nonneg, MulZeroClass.zero_mul];
...
#print MultilinearMap.mkContinuousMultilinear_apply /-
@[simp]
...
#print MultilinearMap.mkContinuousMultilinear_norm_le' /-
...
#print ContinuousMultilinearMap.norm_compContinuousLinearMap_le /-
theorem norm_compContinuousLinearMap_le (g : ContinuousMultilinearMap 𝕜 E₁ G)
  ...
  opNorm_le_bound _ (mul_nonneg (norm_nonneg _) <| prod_nonneg fun i hi => norm_nonneg _) fun m =>
    calc
      ‖g fun i => f i (m i)‖ ≤ ‖g‖ * ∏ i, ‖f i (m i)‖ := g.le_opNorm _
      _ ≤ ‖g‖ * ∏ i, ‖f i‖ * ‖m i‖ :=
        (mul_le_mul_of_nonneg_left
          (prod_le_prod (fun _ _ => norm_nonneg _) fun i hi => (f i).le_opNorm (m i))
          (norm_nonneg g))
...
#print ContinuousMultilinearMap.norm_compContinuous_linearIsometry_le /-
...
#print ContinuousMultilinearMap.norm_compContinuous_linearIsometryEquiv /-
...
#print ContinuousMultilinearMap.compContinuousLinearMapL /-
...
#print ContinuousMultilinearMap.compContinuousLinearMapL_apply /-
@[simp]
...
#print ContinuousMultilinearMap.norm_compContinuousLinearMapL_le /-
...
#print ContinuousMultilinearMap.compContinuousLinearMapEquivL /-
...
#print ContinuousMultilinearMap.compContinuousLinearMapEquivL_symm /-
@[simp]
...
#print ContinuousMultilinearMap.compContinuousLinearMapEquivL_apply /-
@[simp]
...
#print ContinuousLinearMap.norm_map_tail_le /-
theorem ContinuousLinearMap.norm_map_tail_le
  ...
  calc
    ...
    _ ≤ ‖f‖ * ‖m 0‖ * ∏ i, ‖(tail m) i‖ :=
      (mul_le_mul_of_nonneg_right (f.le_opNorm _) (prod_nonneg fun i hi => norm_nonneg _))
...
#print ContinuousMultilinearMap.norm_map_init_le /-
...
#print ContinuousMultilinearMap.norm_map_cons_le /-
theorem ContinuousMultilinearMap.norm_map_cons_le (f : ContinuousMultilinearMap 𝕜 Ei G) (x : Ei 0)
  ...
  calc
    ...
    _ = ‖f‖ * ‖x‖ * ∏ i, ‖m i‖ := by rw [prod_univ_succ]; simp [mul_assoc]
#align continuous_multilinear_map.norm_map_cons_le ContinuousMultilinearMap.norm_map_cons_le
-/

#print ContinuousMultilinearMap.norm_map_snoc_le /-
theorem ContinuousMultilinearMap.norm_map_snoc_le (f : ContinuousMultilinearMap 𝕜 Ei G)
  ...
  calc
    ...
    _ = (‖f‖ * ∏ i, ‖m i‖) * ‖x‖ := by rw [prod_univ_cast_succ]; simp [mul_assoc]
...
#print ContinuousLinearMap.uncurryLeft /-
...
#print ContinuousLinearMap.uncurryLeft_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curryLeft /-
...
def ContinuousMultilinearMap.curryLeft (f : ContinuousMultilinearMap 𝕜 Ei G) :
  ...
  LinearMap.mkContinuous
    { -- define a linear map into `n` continuous multilinear maps from an `n+1` continuous multilinear
      ...
      map_add' := fun x y => by ext m; exact f.cons_add m x y
      map_smul' := fun c x => by ext m;
        exact
...
#print ContinuousMultilinearMap.curryLeft_apply /-
@[simp]
...
#print ContinuousLinearMap.curry_uncurryLeft /-
@[simp]
...
#print ContinuousMultilinearMap.uncurry_curryLeft /-
@[simp]
...
#print continuousMultilinearCurryLeftEquiv /-
...
def continuousMultilinearCurryLeftEquiv :
  ...
  LinearIsometryEquiv.ofBounds
    { toFun := ContinuousLinearMap.uncurryLeft
      map_add' := fun f₁ f₂ => by ext m; rfl
      map_smul' := fun c f => by ext m; rfl
...
#print continuousMultilinearCurryLeftEquiv_apply /-
@[simp]
...
#print continuousMultilinearCurryLeftEquiv_symm_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curryLeft_norm /-
@[simp]
...
#print ContinuousLinearMap.uncurryLeft_norm /-
@[simp]
...
#print ContinuousMultilinearMap.uncurryRight /-
...
#print ContinuousMultilinearMap.uncurryRight_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curryRight /-
...
def ContinuousMultilinearMap.curryRight (f : ContinuousMultilinearMap 𝕜 Ei G) :
  ...
  let f' : MultilinearMap 𝕜 (fun i : Fin n => Ei i.cast_succ) (Ei (last n) →L[𝕜] G) :=
    { toFun := fun m =>
      ...
      map_add' := fun _ m i x y => by simp; rfl
      map_smul' := fun _ m i c x => by simp; rfl }
...
#print ContinuousMultilinearMap.curryRight_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curry_uncurryRight /-
@[simp]
...
#print ContinuousMultilinearMap.uncurry_curryRight /-
@[simp]
...
#print continuousMultilinearCurryRightEquiv /-
/--
...
#print continuousMultilinearCurryRightEquiv_apply /-
@[simp]
...
#print continuousMultilinearCurryRightEquiv_symm_apply /-
@[simp]
...
#print continuousMultilinearCurryRightEquiv_symm_apply' /-
@[simp]
theorem continuousMultilinearCurryRightEquiv_symm_apply' (f : G[×n.succ]→L[𝕜] G') (v : Fin n → G)
    (x : G) : (continuousMultilinearCurryRightEquiv' 𝕜 n G G').symm f v x = f (snoc v x) :=
  rfl
#align continuous_multilinear_curry_right_equiv_symm_apply' continuousMultilinearCurryRightEquiv_symm_apply'
-/

#print ContinuousMultilinearMap.curryRight_norm /-
@[simp]
...
#print ContinuousMultilinearMap.uncurryRight_norm /-
@[simp]
...
#print ContinuousMultilinearMap.uncurry0 /-
...
#print ContinuousMultilinearMap.curry0 /-
...
#print ContinuousMultilinearMap.curry0_apply /-
@[simp]
...
#print ContinuousMultilinearMap.uncurry0_apply /-
@[simp]
...
#print ContinuousMultilinearMap.apply_zero_curry0 /-
@[simp]
theorem ContinuousMultilinearMap.apply_zero_curry0 (f : G[×0]→L[𝕜] G') {x : Fin 0 → G} :
    ContinuousMultilinearMap.curry0 𝕜 G (f x) = f := by ext m;
...
#print ContinuousMultilinearMap.uncurry0_curry0 /-
...
#print ContinuousMultilinearMap.curry0_uncurry0 /-
@[simp]
...
#print ContinuousMultilinearMap.curry0_norm /-
@[simp]
...
#print ContinuousMultilinearMap.fin0_apply_norm /-
@[simp]
...
#print ContinuousMultilinearMap.uncurry0_norm /-
...
#print continuousMultilinearCurryFin0 /-
...
#print continuousMultilinearCurryFin0_apply /-
@[simp]
...
#print continuousMultilinearCurryFin0_symm_apply /-
@[simp]
...
#print continuousMultilinearCurryFin1 /-
...
#print continuousMultilinearCurryFin1_apply /-
@[simp]
...
#print continuousMultilinearCurryFin1_symm_apply /-
@[simp]
...
#print ContinuousMultilinearMap.currySum /-
...
#print ContinuousMultilinearMap.currySum_apply /-
@[simp]
...
#print ContinuousMultilinearMap.uncurrySum /-
...
def uncurrySum
  ...
  MultilinearMap.mkContinuous
    ...
    simpa [Fintype.prod_sum_type, mul_assoc] using
      (f (m ∘ Sum.inl)).le_of_opNorm_le (m ∘ Sum.inr) (f.le_op_norm _)
#align continuous_multilinear_map.uncurry_sum ContinuousMultilinearMap.uncurrySum
-/

#print ContinuousMultilinearMap.uncurrySum_apply /-
@[simp]
...
#print ContinuousMultilinearMap.currySumEquiv /-
...
#print ContinuousMultilinearMap.curryFinFinset /-
...
#print ContinuousMultilinearMap.curryFinFinset_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curryFinFinset_symm_apply /-
@[simp]
...
#print ContinuousMultilinearMap.curryFinFinset_symm_apply_piecewise_const /-
@[simp]
...
#print ContinuousMultilinearMap.curryFinFinset_symm_apply_const /-
@[simp]
...
#print ContinuousMultilinearMap.curryFinFinset_apply_const /-
@[simp]
