...
theorem snd : IsBoundedLinearMap 𝕜 fun x : E × F => x.2 :=
  by
  refine' (LinearMap.snd 𝕜 E F).isLinear.withBound 1 fun x => _
  rw [one_mul]
...
theorem isBoundedLinearMapProdMultilinear {E : ι → Type _} [∀ i, NormedAddCommGroup (E i)]
    [∀ i, NormedSpace 𝕜 (E i)] :
