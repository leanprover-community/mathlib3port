...
theorem exp_series_apply_eq_field [TopologicalSpace 𝕂] [TopologicalRing 𝕂] (x : 𝕂) (n : ℕ) :
    (expSeries 𝕂 𝕂 n fun _ => x) = x ^ n / n ! := by
  rw [div_eq_inv_mul, ← smul_eq_mul, inv_eq_one_div]
  exact exp_series_apply_eq x n

theorem exp_series_apply_eq_field' [TopologicalSpace 𝕂] [TopologicalRing 𝕂] (x : 𝕂) :
    (fun n => expSeries 𝕂 𝕂 n fun _ => x) = fun n => x ^ n / n ! :=
...
theorem exp_series_sum_eq_field [TopologicalSpace 𝕂] [TopologicalRing 𝕂] (x : 𝕂) :
    (expSeries 𝕂 𝕂).Sum x = ∑' n : ℕ, x ^ n / n ! :=
  tsum_congr fun n => exp_series_apply_eq_field x n

theorem exp_eq_tsum : exp 𝕂 𝔸 = fun x : 𝔸 => ∑' n : ℕ, (1 / n ! : 𝕂) • x ^ n :=
  funext exp_series_sum_eq

theorem exp_eq_tsum_field [TopologicalSpace 𝕂] [TopologicalRing 𝕂] : exp 𝕂 𝕂 = fun x : 𝕂 => ∑' n : ℕ, x ^ n / n ! :=
...
@[simp]
theorem exp_zero [T2Space 𝔸] : exp 𝕂 𝔸 0 = 1 := by
...
theorem inv_of_exp_of_mem_ball [CharZero 𝕂] {x : 𝔸} (hx : x ∈ Emetric.Ball (0 : 𝔸) (expSeries 𝕂 𝔸).radius)
    [Invertible (exp 𝕂 𝔸 x)] : ⅟ (exp 𝕂 𝔸 x) = exp 𝕂 𝔸 (-x) := by
  let this := invertibleExpOfMemBall hx
  convert (rfl : ⅟ (exp 𝕂 𝔸 x) = _)
...
theorem algebra_map_exp_comm_of_mem_ball [CompleteSpace 𝕂] (x : 𝕂)
    (hx : x ∈ Emetric.Ball (0 : 𝕂) (expSeries 𝕂 𝕂).radius) : algebraMap 𝕂 𝔸 (exp 𝕂 𝕂 x) = exp 𝕂 𝔸 (algebraMap 𝕂 𝔸 x) :=
...
theorem exp_neg_of_mem_ball [CharZero 𝕂] [CompleteSpace 𝔸] {x : 𝔸}
    (hx : x ∈ Emetric.Ball (0 : 𝔸) (expSeries 𝕂 𝔸).radius) : exp 𝕂 𝔸 (-x) = (exp 𝕂 𝔸 x)⁻¹ := by
  let this := invertibleExpOfMemBall hx
...
`exp 𝕂 𝔸 (x+y) = (exp 𝕂 𝔸 x) * (exp 𝕂 𝔸 y)`. -/
theorem exp_add_of_commute {x y : 𝔸} (hxy : Commute x y) : exp 𝕂 𝔸 (x + y) = exp 𝕂 𝔸 x * exp 𝕂 𝔸 y :=
...
/-- `exp 𝕂 𝔸 x` has explicit two-sided inverse `exp 𝕂 𝔸 (-x)`. -/
noncomputable def invertibleExp (x : 𝔸) : Invertible (exp 𝕂 𝔸 x) :=
  invertibleExpOfMemBall <| (exp_series_radius_eq_top 𝕂 𝔸).symm ▸ edist_lt_top _ _

theorem is_unit_exp (x : 𝔸) : IsUnit (exp 𝕂 𝔸 x) :=
  is_unit_exp_of_mem_ball <| (exp_series_radius_eq_top 𝕂 𝔸).symm ▸ edist_lt_top _ _

theorem inv_of_exp (x : 𝔸) [Invertible (exp 𝕂 𝔸 x)] : ⅟ (exp 𝕂 𝔸 x) = exp 𝕂 𝔸 (-x) :=
...
theorem exp_sum_of_commute {ι} (s : Finset ι) (f : ι → 𝔸) (h : ∀, ∀ i ∈ s, ∀, ∀ j ∈ s, ∀, Commute (f i) (f j)) :
    exp 𝕂 𝔸 (∑ i in s, f i) = s.noncommProd (fun i => exp 𝕂 𝔸 (f i)) fun i hi j hj => (h i hi j hj).exp 𝕂 := by
  classical
...
theorem exp_nsmul (n : ℕ) (x : 𝔸) : exp 𝕂 𝔸 (n • x) = exp 𝕂 𝔸 x ^ n := by
...
/-- Any continuous ring homomorphism commutes with `exp`. -/
theorem map_exp {F} [RingHomClass F 𝔸 𝔹] (f : F) (hf : Continuous f) (x : 𝔸) : f (exp 𝕂 𝔸 x) = exp 𝕂 𝔹 (f x) :=
...
theorem exp_units_conj (y : 𝔸ˣ) (x : 𝔸) : exp 𝕂 𝔸 (y * x * ↑y⁻¹) = y * exp 𝕂 𝔸 x * ↑y⁻¹ :=
...
@[simp]
theorem Prod.fst_exp [CompleteSpace 𝔹] (x : 𝔸 × 𝔹) : (exp 𝕂 (𝔸 × 𝔹) x).fst = exp 𝕂 𝔸 x.fst :=
  map_exp _ (RingHom.fst 𝔸 𝔹) continuous_fst x

@[simp]
theorem Prod.snd_exp [CompleteSpace 𝔹] (x : 𝔸 × 𝔹) : (exp 𝕂 (𝔸 × 𝔹) x).snd = exp 𝕂 𝔹 x.snd :=
...
theorem Pi.exp_apply {ι : Type _} {𝔸 : ι → Type _} [Fintype ι] [∀ i, NormedRing (𝔸 i)] [∀ i, NormedAlgebra 𝕂 (𝔸 i)]
    [∀ i, CompleteSpace (𝔸 i)] (x : ∀ i, 𝔸 i) (i : ι) : exp 𝕂 (∀ i, 𝔸 i) x i = exp 𝕂 (𝔸 i) (x i) := by
...
theorem Pi.exp_def {ι : Type _} {𝔸 : ι → Type _} [Fintype ι] [∀ i, NormedRing (𝔸 i)] [∀ i, NormedAlgebra 𝕂 (𝔸 i)]
    [∀ i, CompleteSpace (𝔸 i)] (x : ∀ i, 𝔸 i) : exp 𝕂 (∀ i, 𝔸 i) x = fun i => exp 𝕂 (𝔸 i) (x i) :=
...
theorem Function.update_exp {ι : Type _} {𝔸 : ι → Type _} [Fintype ι] [DecidableEq ι] [∀ i, NormedRing (𝔸 i)]
    ...
    Function.update (exp 𝕂 (∀ i, 𝔸 i) x) j (exp 𝕂 (𝔸 j) xj) = exp 𝕂 _ (Function.update x j xj) := by
  ext i
  simp_rw [Pi.exp_def]
  exact (Function.apply_update (fun i => exp 𝕂 (𝔸 i)) x j xj i).symm

end CompleteAlgebra

theorem algebra_map_exp_comm (x : 𝕂) : algebraMap 𝕂 𝔸 (exp 𝕂 𝕂 x) = exp 𝕂 𝔸 (algebraMap 𝕂 𝔸 x) :=
...
theorem exp_neg (x : 𝔸) : exp 𝕂 𝔸 (-x) = (exp 𝕂 𝔸 x)⁻¹ :=
  exp_neg_of_mem_ball <| (exp_series_radius_eq_top 𝕂 𝔸).symm ▸ edist_lt_top _ _

theorem exp_zsmul (z : ℤ) (x : 𝔸) : exp 𝕂 𝔸 (z • x) = exp 𝕂 𝔸 x ^ z := by
...
theorem exp_conj (y : 𝔸) (x : 𝔸) (hy : y ≠ 0) : exp 𝕂 𝔸 (y * x * y⁻¹) = y * exp 𝕂 𝔸 x * y⁻¹ :=
...
theorem exp_sum {ι} (s : Finset ι) (f : ι → 𝔸) : exp 𝕂 𝔸 (∑ i in s, f i) = ∏ i in s, exp 𝕂 𝔸 (f i) := by
...
variable (𝕂 𝕂' 𝔸 : Type _) [Field 𝕂] [Field 𝕂'] [Ringₓ 𝔸] [Algebra 𝕂 𝔸] [Algebra 𝕂' 𝔸] [TopologicalSpace 𝔸]
  [TopologicalRing 𝔸] [HasContinuousConstSmul 𝕂 𝔸] [HasContinuousConstSmul 𝕂' 𝔸]
...
theorem star_exp {𝕜 A : Type _} [IsROrC 𝕜] [NormedRing A] [NormedAlgebra 𝕜 A] [StarRing A] [NormedStarGroup A]
    [CompleteSpace A] [StarModule 𝕜 A] (a : A) : star (exp 𝕜 A a) = exp 𝕜 A (star a) := by
  rw [exp_eq_tsum]
  have :=
