...
theorem geom_mean_le_arith_mean_weighted (w z : ι → ℝ) (hw : ∀ i ∈ s, 0 ≤ w i) (hw' : (∑ i in s, w i) = 1)
  ...
  · simp only [not_exists, not_and, Ne.def, not_not] at A
    ...
    · cases' eq_or_lt_of_leₓ (hz i hi) with hz hz
      · simp [A i hi hz.symm]
...
theorem geom_mean_le_arith_mean2_weighted (w₁ w₂ p₁ p₂ : ℝ≥0) :
  ...
  simpa only [Finₓ.prod_univ_succ, Finₓ.sum_univ_succ, Finsetₓ.prod_empty, Finsetₓ.sum_empty, Fintypeₓ.univ_of_is_empty,
...
private theorem inner_le_Lp_mul_Lp_of_norm_le_one (f g : ι → ℝ≥0) {p q : ℝ} (hpq : p.IsConjugateExponent q)
  ...
  calc
    (∑ i in s, f i * g i) ≤ ∑ i in s, f i ^ p / Real.toNnreal p + g i ^ q / Real.toNnreal q :=
      Finsetₓ.sum_le_sum fun i his => young_inequality_real (f i) (g i) hpq
...
theorem inner_le_Lp_mul_Lq_tsum {f g : ι → ℝ≥0} {p q : ℝ} (hpq : p.IsConjugateExponent q)
  ...
  have H₁ : ∀ s : Finsetₓ ι, (∑ i in s, f i * g i) ≤ (∑' i, f i ^ p) ^ (1 / p) * (∑' i, g i ^ q) ^ (1 / q) := by
    intro s
...
theorem summable_mul_of_Lp_Lq {f g : ι → ℝ≥0} {p q : ℝ} (hpq : p.IsConjugateExponent q) (hf : Summable fun i => f i ^ p)
...
theorem rpow_sum_le_const_mul_sum_rpow (f : ι → ℝ≥0) {p : ℝ} (hp : 1 ≤ p) :
  ...
  cases' eq_or_lt_of_leₓ hp with hp hp
  · simp [← hp]
    
  let q : ℝ := p / (p - 1)
  have hpq : p.is_conjugate_exponent q := by rw [Real.is_conjugate_exponent_iff hp]
...
theorem Lp_add_le (f g : ι → ℝ≥0) {p : ℝ} (hp : 1 ≤ p) :
  ...
  rcases eq_or_lt_of_leₓ hp with (rfl | hp)
...
theorem Lp_add_le_tsum {f g : ι → ℝ≥0} {p : ℝ} (hp : 1 ≤ p) (hf : Summable fun i => f i ^ p)
  ...
  have H₁ :
    ∀ s : Finsetₓ ι, (∑ i in s, (f i + g i) ^ p) ≤ ((∑' i, f i ^ p) ^ (1 / p) + (∑' i, g i ^ p) ^ (1 / p)) ^ p := by
    intro s
...
theorem inner_le_Lp_mul_Lq_of_nonneg (hpq : IsConjugateExponent p q) (hf : ∀ i ∈ s, 0 ≤ f i) (hg : ∀ i ∈ s, 0 ≤ g i) :
    (∑ i in s, f i * g i) ≤ (∑ i in s, f i ^ p) ^ (1 / p) * (∑ i in s, g i ^ q) ^ (1 / q) := by
...
theorem inner_le_Lp_mul_Lq (hpq : p.IsConjugateExponent q) :
  ...
  convert this using 1 <;> [skip, congr 2] <;> [skip, skip, simp, skip, simp] <;>
    · apply Finsetₓ.sum_congr rfl fun i hi => _
