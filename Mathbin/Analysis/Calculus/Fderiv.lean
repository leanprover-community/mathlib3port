...
theorem HasFderivWithinAt.lim (h : HasFderivWithinAt f f' s x) {α : Type _} (l : Filter α) {c : α → 𝕜} {d : α → E}
  {v : E} (dtop : ∀ᶠn in l, (x+d n) ∈ s) (clim : tendsto (fun n => ∥c n∥) l at_top)
...
theorem HasFderivAt.le_of_lip {f : E → F} {f' : E →L[𝕜] F} {x₀ : E} (hf : HasFderivAt f f' x₀) {s : Set E}
  ...
  by 
    ...
    filterUpwards [is_o_iff.1 (has_fderiv_at_iff_is_o_nhds_zero.1 hf) ε0, hs']
    intro y hy hys 
    have  := hlip.norm_sub_le hys (mem_of_mem_nhds hs)
    rw [add_sub_cancel'] at this 
    calc ∥f' y∥ ≤ ∥f (x₀+y) - f x₀∥+∥f (x₀+y) - f x₀ - f' y∥ := norm_le_insert _ _ _ ≤ (C*∥y∥)+ε*∥y∥ :=
      add_le_add this hy _ = (C+ε)*∥y∥ := (add_mulₓ _ _ _).symm
...
-- error in Analysis.Calculus.Fderiv: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
protected
theorem has_fderiv_within_at.iterate
{f : E → E}
{f' : «expr →L[ ] »(E, 𝕜, E)}
(hf : has_fderiv_within_at f f' s x)
(hx : «expr = »(f x, x))
(hs : maps_to f s s)
(n : exprℕ()) : has_fderiv_within_at «expr ^[ ]»(f, n) «expr ^ »(f', n) s x :=
begin
  refine [expr hf.iterate _ hx n],
  convert [] [expr tendsto_inf.2 ⟨hf.continuous_within_at, _⟩] [],
  exacts ["[", expr hx.symm, ",", expr (tendsto_principal_principal.2 hs).mono_left inf_le_right, "]"]
end
...
theorem has_fderiv_at_ring_inverse (x : Units R) :
  HasFderivAt Ring.inverse (-lmul_left_right 𝕜 R («expr↑ » (x⁻¹)) («expr↑ » (x⁻¹))) x :=
  by 
    have h_is_o :
      is_o (fun t : R => (inverse («expr↑ » x+t) - «expr↑ » (x⁻¹))+(«expr↑ » (x⁻¹)*t)*«expr↑ » (x⁻¹)) (fun t : R => t)
        (𝓝 0)
    ·
...
theorem HasStrictFderivAt.of_local_left_inverse {f : E → F} {f' : E ≃L[𝕜] F} {g : F → E} {a : F} (hg : ContinuousAt g a)
  (hf : HasStrictFderivAt f (f' : E →L[𝕜] F) (g a)) (hfg : ∀ᶠy in 𝓝 a, f (g y) = y) :
...
theorem HasFderivAt.of_local_left_inverse {f : E → F} {f' : E ≃L[𝕜] F} {g : F → E} {a : F} (hg : ContinuousAt g a)
  (hf : HasFderivAt f (f' : E →L[𝕜] F) (g a)) (hfg : ∀ᶠy in 𝓝 a, f (g y) = y) : HasFderivAt g (f'.symm : F →L[𝕜] E) a :=
  by 
...
-- error in Analysis.Calculus.Fderiv: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
