...
theorem image_le_of_liminf_slope_right_le_deriv_boundary {f : ℝ → ℝ} {a b : ℝ} (hf : ContinuousOn f (Icc a b))
  ...
  (bound : ∀ x _ : x ∈ Ico a b, ∀ r, B' x < r → ∃ᶠz in 𝓝[Ioi x] x, ((z - x)⁻¹*f z - f x) < r) :
  ∀ ⦃x⦄, x ∈ Icc a b → f x ≤ B x :=
  by 
...
variable{𝕜 G :
    Type
      ...
      𝕜][NormedSpace 𝕜
      E][IsScalarTower ℝ 𝕜
      E][NormedGroup G][NormedSpace 𝕜 G]{f : E → G}{C : ℝ}{s : Set E}{x y : E}{f' : E → E →L[𝕜] G}{φ : E →L[𝕜] G}

/-- The mean value theorem on a convex set: if the derivative of a function is bounded by `C`, then
the function is `C`-Lipschitz. Version with `has_fderiv_within`. -/
theorem Convex.norm_image_sub_le_of_norm_has_fderiv_within_le (hf : ∀ x _ : x ∈ s, HasFderivWithinAt f (f' x) s x)
  (bound : ∀ x _ : x ∈ s, ∥f' x∥ ≤ C) (hs : Convex ℝ s) (xs : x ∈ s) (ys : y ∈ s) : ∥f y - f x∥ ≤ C*∥y - x∥ :=
  by 
    letI this : NormedSpace ℝ G := RestrictScalars.normedSpace ℝ 𝕜 G 
    letI this : IsScalarTower ℝ 𝕜 G := RestrictScalars.is_scalar_tower _ _ _ 
...
theorem Convex.exists_nhds_within_lipschitz_on_with_of_has_fderiv_within_at_of_nnnorm_lt (hs : Convex ℝ s) {f : E → G}
...
theorem Convex.exists_nhds_within_lipschitz_on_with_of_has_fderiv_within_at (hs : Convex ℝ s) {f : E → G}
...
theorem Convex.norm_image_sub_le_of_norm_fderiv_within_le (hf : DifferentiableOn 𝕜 f s)
...
`lipschitz_on_with`. -/
theorem Convex.lipschitz_on_with_of_nnnorm_fderiv_within_le {C :  ℝ≥0 } (hf : DifferentiableOn 𝕜 f s)
...
theorem Convex.norm_image_sub_le_of_norm_fderiv_le (hf : ∀ x _ : x ∈ s, DifferentiableAt 𝕜 f x)
...
theorem Convex.lipschitz_on_with_of_nnnorm_fderiv_le {C :  ℝ≥0 } (hf : ∀ x _ : x ∈ s, DifferentiableAt 𝕜 f x)
...
theorem Convex.is_const_of_fderiv_within_eq_zero (hs : Convex ℝ s) (hf : DifferentiableOn 𝕜 f s)
...
theorem is_const_of_fderiv_eq_zero (hf : Differentiable 𝕜 f) (hf' : ∀ x, fderiv 𝕜 f x = 0) (x y : E) : f x = f y :=
...
theorem Convex.norm_image_sub_le_of_norm_has_deriv_within_le {f f' : ℝ → F} {C : ℝ} {s : Set ℝ} {x y : ℝ}
  (hf : ∀ x _ : x ∈ s, HasDerivWithinAt f (f' x) s x) (bound : ∀ x _ : x ∈ s, ∥f' x∥ ≤ C) (hs : Convex ℝ s) (xs : x ∈ s)
...
theorem Convex.lipschitz_on_with_of_nnnorm_has_deriv_within_le {f f' : ℝ → F} {C :  ℝ≥0 } {s : Set ℝ} (hs : Convex ℝ s)
...
theorem Convex.norm_image_sub_le_of_norm_deriv_within_le {f : ℝ → F} {C : ℝ} {s : Set ℝ} {x y : ℝ}
  (hf : DifferentiableOn ℝ f s) (bound : ∀ x _ : x ∈ s, ∥derivWithin f s x∥ ≤ C) (hs : Convex ℝ s) (xs : x ∈ s)
  (ys : y ∈ s) : ∥f y - f x∥ ≤ C*∥y - x∥ :=
...
theorem Convex.lipschitz_on_with_of_nnnorm_deriv_within_le {f : ℝ → F} {C :  ℝ≥0 } {s : Set ℝ} (hs : Convex ℝ s)
  (hf : DifferentiableOn ℝ f s) (bound : ∀ x _ : x ∈ s, ∥derivWithin f s x∥₊ ≤ C) : LipschitzOnWith C f s :=
...
theorem Convex.norm_image_sub_le_of_norm_deriv_le {f : ℝ → F} {C : ℝ} {s : Set ℝ} {x y : ℝ}
  (hf : ∀ x _ : x ∈ s, DifferentiableAt ℝ f x) (bound : ∀ x _ : x ∈ s, ∥deriv f x∥ ≤ C) (hs : Convex ℝ s) (xs : x ∈ s)
  (ys : y ∈ s) : ∥f y - f x∥ ≤ C*∥y - x∥ :=
...
theorem Convex.lipschitz_on_with_of_nnnorm_deriv_le {f : ℝ → F} {C :  ℝ≥0 } {s : Set ℝ}
  (hf : ∀ x _ : x ∈ s, DifferentiableAt ℝ f x) (bound : ∀ x _ : x ∈ s, ∥deriv f x∥₊ ≤ C) (hs : Convex ℝ s) :
  LipschitzOnWith C f s :=
...
theorem domain_mvt {f : E → ℝ} {s : Set E} {x y : E} {f' : E → E →L[ℝ] ℝ}
  ...
  by 
    ...
    ·
      ...
      ·
        refine' fun t Ht => (hfg t$ hIccIoo Ht).HasDerivAt _ 
...
differentiable. -/
theorem has_strict_fderiv_at_of_has_fderiv_at_of_continuous_at (hder : ∀ᶠy in 𝓝 x, HasFderivAt f (f' y) y)
