...
variable {α β 𝕜 E F : Type _} [IsROrC 𝕜] [NormedAddCommGroup E] [NormedSpace 𝕜 E] [NormedAddCommGroup F]
...
theorem tendsto_uniformly_on_tsum {f : α → β → F} (hu : Summable u) {s : Set β} (hfu : ∀ n x, x ∈ s → ‖f n x‖ ≤ u n) :
    TendstoUniformlyOn (fun t : Finset α => fun x => ∑ n in t, f n x) (fun x => ∑' n, f n x) atTop s := by
  ...
  have A : Summable fun n => ‖f n x‖ := summable_of_nonneg_of_le (fun n => norm_nonneg _) (fun n => hfu n x hx) hu
...
theorem tendsto_uniformly_on_tsum_nat {f : ℕ → β → F} {u : ℕ → ℝ} (hu : Summable u) {s : Set β}
    ...
    TendstoUniformlyOn (fun N => fun x => ∑ n in Finset.range N, f n x) (fun x => ∑' n, f n x) atTop s := fun v hv =>
  tendsto_finset_range.Eventually (tendsto_uniformly_on_tsum hu hfu v hv)
...
theorem tendsto_uniformly_tsum_nat {f : ℕ → β → F} {u : ℕ → ℝ} (hu : Summable u) (hfu : ∀ n x, ‖f n x‖ ≤ u n) :
    TendstoUniformly (fun N => fun x => ∑ n in Finset.range N, f n x) (fun x => ∑' n, f n x) atTop := fun v hv =>
  tendsto_finset_range.Eventually (tendsto_uniformly_tsum hu hfu v hv)
...
theorem continuous_on_tsum [TopologicalSpace β] {f : α → β → F} {s : Set β} (hf : ∀ i, ContinuousOn (f i) s)
    (hu : Summable u) (hfu : ∀ n x, x ∈ s → ‖f n x‖ ≤ u n) : ContinuousOn (fun x => ∑' n, f n x) s := by classical
...
theorem continuous_tsum [TopologicalSpace β] {f : α → β → F} (hf : ∀ i, Continuous (f i)) (hu : Summable u)
    (hfu : ∀ n x, ‖f n x‖ ≤ u n) : Continuous fun x => ∑' n, f n x := by
...
theorem summable_of_summable_has_fderiv_at_of_is_preconnected (hu : Summable u) (hs : IsOpen s) (h's : IsPreconnected s)
    ...
    (hf0 : Summable fun n => f n x₀) {x : E} (hx : x ∈ s) : Summable fun n => f n x := by
...
theorem hasFderivAtTsumOfIsPreconnected (hu : Summable u) (hs : IsOpen s) (h's : IsPreconnected s)
    (hf : ∀ n x, x ∈ s → HasFderivAt (f n) (f' n x) x) (hf' : ∀ n x, x ∈ s → ‖f' n x‖ ≤ u n) (hx₀ : x₀ ∈ s)
    (hf0 : Summable fun n => f n x₀) (hx : x ∈ s) : HasFderivAt (fun y => ∑' n, f n y) (∑' n, f' n x) x := by classical
  have A : ∀ x : E, x ∈ s → tendsto (fun t : Finset α => ∑ n in t, f n x) at_top (𝓝 (∑' n, f n x)) := by
    intro y hy
    apply Summable.has_sum
    exact summable_of_summable_has_fderiv_at_of_is_preconnected hu hs h's hf hf' hx₀ hf0 hy
  apply hasFderivAtOfTendstoUniformlyOn hs (tendsto_uniformly_on_tsum hu hf') (fun t y hy => _) A _ hx
...
theorem summable_of_summable_has_fderiv_at (hu : Summable u) (hf : ∀ n x, HasFderivAt (f n) (f' n x) x)
    (hf' : ∀ n x, ‖f' n x‖ ≤ u n) (hf0 : Summable fun n => f n x₀) (x : E) : Summable fun n => f n x := by
  let : NormedSpace ℝ E
  exact NormedSpace.restrictScalars ℝ 𝕜 _
  apply
    summable_of_summable_has_fderiv_at_of_is_preconnected hu is_open_univ is_connected_univ.is_preconnected
...
theorem hasFderivAtTsum (hu : Summable u) (hf : ∀ n x, HasFderivAt (f n) (f' n x) x) (hf' : ∀ n x, ‖f' n x‖ ≤ u n)
    (hf0 : Summable fun n => f n x₀) (x : E) : HasFderivAt (fun y => ∑' n, f n y) (∑' n, f' n x) x := by
  let : NormedSpace ℝ E
  exact NormedSpace.restrictScalars ℝ 𝕜 _
  exact
    hasFderivAtTsumOfIsPreconnected hu is_open_univ is_connected_univ.is_preconnected (fun n x hx => hf n x)
...
theorem differentiableTsum (hu : Summable u) (hf : ∀ n x, HasFderivAt (f n) (f' n x) x) (hf' : ∀ n x, ‖f' n x‖ ≤ u n) :
    Differentiable 𝕜 fun y => ∑' n, f n y := by
...
theorem fderiv_tsum (hu : Summable u) (hf : ∀ n, Differentiable 𝕜 (f n)) (hf' : ∀ n x, ‖fderiv 𝕜 (f n) x‖ ≤ u n)
    {x₀ : E} (hf0 : Summable fun n => f n x₀) : (fderiv 𝕜 fun y => ∑' n, f n y) = fun x => ∑' n, fderiv 𝕜 (f n) x := by
  ext1 x
...
theorem iterated_fderiv_tsum (hf : ∀ i, ContDiff 𝕜 N (f i)) (hv : ∀ k : ℕ, (k : ℕ∞) ≤ N → Summable (v k))
  ...
  · have h'k : (k : ℕ∞) < N := lt_of_lt_of_le (WithTop.coe_lt_coe.2 (Nat.lt_succ_self _)) hk
    ...
    · ext1 x
      exact (continuousMultilinearCurryLeftEquiv 𝕜 (fun i : Fin (k + 1) => E) F).toContinuousLinearEquiv.map_tsum
...
/-- Consider a series of smooth functions, with summable uniform bounds on the successive
derivatives. Then the iterated derivative of the sum is the sum of the iterated derivative. -/
theorem iterated_fderiv_tsum_apply (hf : ∀ i, ContDiff 𝕜 N (f i)) (hv : ∀ k : ℕ, (k : ℕ∞) ≤ N → Summable (v k))
    (h'f : ∀ (k : ℕ) (i : α) (x : E), (k : ℕ∞) ≤ N → ‖iteratedFderiv 𝕜 k (f i) x‖ ≤ v k i) {k : ℕ} (hk : (k : ℕ∞) ≤ N)
...
theorem contDiffTsum (hf : ∀ i, ContDiff 𝕜 N (f i)) (hv : ∀ k : ℕ, (k : ℕ∞) ≤ N → Summable (v k))
  ...
  · intro m hm
    have h'm : ((m + 1 : ℕ) : ℕ∞) ≤ N := by
      simpa only [Enat.coe_add, Nat.cast_with_bot, Enat.coe_one] using Enat.add_one_le_of_lt hm
    rw [iterated_fderiv_tsum hf hv h'f hm.le]
    have A : ∀ n x, HasFderivAt (iteratedFderiv 𝕜 m (f n)) (fderiv 𝕜 (iteratedFderiv 𝕜 m (f n)) x) x := fun n x =>
      (ContDiff.differentiableIteratedFderiv hm (hf n)).DifferentiableAt.HasFderivAt
...
theorem contDiffTsumOfEventually (hf : ∀ i, ContDiff 𝕜 N (f i)) (hv : ∀ k : ℕ, (k : ℕ∞) ≤ N → Summable (v k))
    (h'f :
      ∀ k : ℕ, (k : ℕ∞) ≤ N → ∀ᶠ i in (Filter.cofinite : Filter α), ∀ x : E, ‖iteratedFderiv 𝕜 k (f i) x‖ ≤ v k i) :
    ContDiff 𝕜 N fun x => ∑' i, f i x := by classical
  ...
  have : (fun x => ∑' i, f i x) = (fun x => ∑ i in T, f i x) + fun x => ∑' i : { i // i ∉ T }, f i x := by
    ext1 x
