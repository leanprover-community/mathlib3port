...
/--
  ...
  theorem
    IsLocalExtrOn.range_ne_top_of_has_strict_fderiv_at
...
/--
  ...
  theorem
    IsLocalExtrOn.exists_linear_map_of_has_strict_fderiv_at
    ( hextr : IsLocalExtrOn φ { x | f x = f x₀ } x₀ )
        ( hf' : HasStrictFderivAt f f' x₀ )
        ( hφ' : HasStrictFderivAt φ φ' x₀ )
      ...
      by
        rcases
            Submodule.exists_le_ker_of_lt_top
              _ lt_top_iff_ne_top . 2 $ hextr.range_ne_top_of_has_strict_fderiv_at hf' hφ'
            with ⟨ Λ' , h0 , hΛ' ⟩
          set
            e
            : F →ₗ[ ℝ ] ℝ × ℝ ≃ₗ[ ℝ ] F × ℝ →ₗ[ ℝ ] ℝ
            :=
          ...
          simp
            only
            [
              LinearMap.coprod_equiv_apply
                ,
                LinearEquiv.refl_apply
                ,
                LinearMap.ring_lmap_equiv_self_symm_apply
                ,
...
/--
  ...
  theorem
    IsLocalExtrOn.exists_multipliers_of_has_strict_fderiv_at_1d
    { f : E → ℝ }
        { f' : E →L[ ℝ ] ℝ }
        ( hextr : IsLocalExtrOn φ { x | f x = f x₀ } x₀ )
        ( hf' : HasStrictFderivAt f f' x₀ )
...
/--
  ...
  theorem
    IsLocalExtrOn.exists_multipliers_of_has_strict_fderiv_at
    { ι : Type _ }
        [ Fintype ι ]
        { f : ι → E → ℝ }
        { f' : ι → E →L[ ℝ ] ℝ }
    ...
    :=
      by
        let this' := Classical.decEq ι
          ...
          rcases
            hextr.exists_linear_map_of_has_strict_fderiv_at has_strict_fderiv_at_pi . 2 fun i => hf' i hφ'
            with ⟨ Λ , Λ₀ , h0 , hsum ⟩
          ...
          · ext x simpa [ mul_commₓ ] using hsum x
...
/--
  ...
  theorem
    IsLocalExtrOn.linear_dependent_of_has_strict_fderiv_at
    { ι : Type _ }
        [ Fintype ι ]
        { f : ι → E → ℝ }
        { f' : ι → E →L[ ℝ ] ℝ }
