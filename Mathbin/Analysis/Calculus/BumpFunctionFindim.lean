...
theorem IsOpen.exists_smooth_support_eq {s : Set E} (hs : IsOpen s) :
  ...
  let ι :=
    { f : E → ℝ // f.support ⊆ s ∧ HasCompactSupport f ∧ cont_diff ℝ ⊤ f ∧ range f ⊆ Icc 0 1 }
  obtain ⟨T, T_count, hT⟩ : ∃ T : Set ι, T.Countable ∧ (⋃ f ∈ T, support (f : E → ℝ)) = s :=
    by
    have : (⋃ f : ι, (f : E → ℝ).support) = s :=
      by
      refine' Subset.antisymm (unionᵢ_subset fun f => f.2.1) _
      intro x hx
      ...
      exact mem_unionᵢ_of_mem _ this
    simp_rw [← this]
    apply isOpen_unionᵢ_countable
    rintro ⟨f, hf⟩
  ...
  obtain ⟨g0, hg⟩ : ∃ g0 : ℕ → ι, T = range g0 :=
    ...
    · simp only [unionᵢ_false, unionᵢ_empty] at hT
  ...
  have s_g : ∀ x ∈ s, ∃ n, x ∈ support (g n) :=
    ...
    obtain ⟨i, iT, hi⟩ : ∃ (i : ι)(hi : i ∈ T), x ∈ support (i : E → ℝ) := by
      simpa only [mem_unionᵢ] using hx
    rw [hg, mem_range] at iT
    rcases iT with ⟨n, hn⟩
    rw [← hn] at hi
    exact ⟨n, hi⟩
  have g_smooth : ∀ n, cont_diff ℝ ⊤ (g n) := fun n => (g0 n).2.2.2.1
  ...
  have A : (w D ⋆[lsmul ℝ ℝ, μ] φ) x ≤ (w D ⋆[lsmul ℝ ℝ, μ] 1) x :=
    ...
    refine'
      (HasCompactSupport.convolutionExistsLeft _ (w_compact_support E Dpos) _
          (locallyIntegrableConst (1 : ℝ)) x).integrable
...
theorem y_pos_of_mem_ball {D : ℝ} {x : E} (Dpos : 0 < D) (D_lt_one : D < 1)
  ...
  · have F_comp : HasCompactSupport (w D) := w_compact_support E Dpos
    have B : LocallyIntegrable (φ : E → ℝ) μ :=
      (locallyIntegrableConst _).indicator measurableSet_closedBall
  ...
  · set z := (D / (1 + D)) • x with hz
    ...
    have C : ball z (D * (1 + D - ‖x‖) / (1 + D)) ⊆ support fun y : E => w D y * φ (x - y) :=
      ...
      · have ID : ‖D / (1 + D) - 1‖ = 1 / (1 + D) :=
        ...
        apply closedBall_subset_closed_ball' _ (ball_subset_closedBall hy)
...
theorem y_smooth : ContDiffOn ℝ ⊤ (uncurry y) (Ioo (0 : ℝ) 1 ×ˢ (univ : Set E)) :=
  ...
  · exact (locallyIntegrableConst _).indicator measurableSet_closedBall
    ...
    { toFun := fun R x => if 1 < R then y ((R - 1) / (R + 1)) (((R + 1) / 2)⁻¹ • x) else 0
      ...
      smooth :=
        ...
        suffices
          ...
          simp only [hR, uncurry_apply_pair, if_true, comp_apply]
