...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r' «expr ∈ » Ioc «expr / »(r, 2) r)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y z «expr ∈ » ball x r')
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (p «expr ≥ » n)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ≥ » n)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r «expr ∈ » Ioo (0 : exprℝ()) R)
    ...
    :=
      by
        intro x hx
          rw [ D , mem_Inter ]
          intro e
          have : ( 0 : ℝ ) < 1 / 2 ^ e := pow_pos by normNum _
          rcases mem_A_of_differentiable this hx . 1 with ⟨ R , R_pos , hR ⟩
          obtain
            ⟨ n , hn ⟩
            : ∃ n : ℕ , 1 / 2 ^ n < R
            := exists_pow_lt_of_lt_one R_pos ( by normNum : ( 1 : ℝ ) / 2 < 1 )
          simp only [ mem_Union , mem_Inter , B , mem_inter_eq ]
          refine' ⟨ n , fun p hp q hq => ⟨ fderiv 𝕜 f x , hx . 2 , ⟨ _ , _ ⟩ ⟩ ⟩
            <;>
            ·
              refine' hR _ ⟨ pow_pos by normNum _ , lt_of_le_of_ltₓ _ hn ⟩
                exact pow_le_pow_of_le_one by normNum by normNum by assumption

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (L «expr ∈ » K)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (f' «expr ∈ » K)
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Harder inclusion: at a point in `D f K`, the function `f` has a derivative, in `K`. -/
  theorem
    D_subset_differentiable_set
    { K : Set E →L[ 𝕜 ] F } ( hK : IsComplete K ) : D f K ⊆ { x | DifferentiableAt 𝕜 f x ∧ fderiv 𝕜 f x ∈ K }
    :=
      by
        have P : ∀ { n : ℕ } , ( 0 : ℝ ) < 1 / 2 ^ n := pow_pos by normNum
              ...
              ∀
                ...
                ∃
                  ...
                  ∀
                    ...
                    n ≤ p
                      →
                      n ≤ q → ∃ ( L : _ ) ( _ : L ∈ K ) , x ∈ A f L 1 / 2 ^ p 1 / 2 ^ e ∩ A f L 1 / 2 ^ q 1 / 2 ^ e
          ·
            intro e
          ...
          ·
            intro e p q e' p' q' hp hq hp' hq' he'
              let r := max n e n e'
              have I : ( 1 : ℝ ) / 2 ^ e' ≤ 1 / 2 ^ e := pow_le_pow_of_le_one by normNum by normNum he'
              ...
              calc
                ∥ L e p q - L e' p' q' ∥ = ∥ L e p q - L e p r + L e p r - L e' p' r + L e' p' r - L e' p' q' ∥
                  ...
                  _ ≤ ∥ L e p q - L e p r ∥ + ∥ L e p r - L e' p' r ∥ + ∥ L e' p' r - L e' p' q' ∥
                    :=
                    le_transₓ norm_add_le _ _ add_le_add_right norm_add_le _ _ _
                  _ ≤ 4 * ∥ c ∥ * 1 / 2 ^ e + 4 * ∥ c ∥ * 1 / 2 ^ e + 4 * ∥ c ∥ * 1 / 2 ^ e
                    :=
                    by applyRules [ add_le_add ]
          ...
          ·
            rw [ Metric.cauchy_seq_iff' ]
              ...
              obtain
                ⟨ e , he ⟩
                : ∃ e : ℕ , 1 / 2 ^ e < ε / 12 * ∥ c ∥
                := exists_pow_lt_of_lt_one div_pos εpos mul_pos by normNum cpos by normNum
              refine' ⟨ e , fun e' he' => _ ⟩
              rw [ dist_comm , dist_eq_norm ]
              calc
                ∥ L0 e - L0 e' ∥ ≤ 12 * ∥ c ∥ * 1 / 2 ^ e
                    :=
                    M _ _ _ _ _ _ le_reflₓ _ le_reflₓ _ le_reflₓ _ le_reflₓ _ he'
                  _ < 12 * ∥ c ∥ * ε / 12 * ∥ c ∥ := mul_lt_mul' le_reflₓ _ he le_of_ltₓ P mul_pos by normNum cpos
                  _ = ε := by fieldSimp [ ( by normNum : ( 12 : ℝ ) ≠ 0 ) , ne_of_gtₓ cpos ] ring
          obtain
            ⟨ f' , f'K , hf' ⟩
            : ∃ ( f' : _ ) ( _ : f' ∈ K ) , tendsto L0 at_top 𝓝 f'
          ...
          ·
            simp only [ has_fderiv_at_iff_is_o_nhds_zero , is_o_iff ]
              intro ε εpos
              have pos : 0 < 4 + 12 * ∥ c ∥ := add_pos_of_pos_of_nonneg by normNum mul_nonneg by normNum norm_nonneg _
              obtain
                ⟨ e , he ⟩
                : ∃ e : ℕ , 1 / 2 ^ e < ε / 4 + 12 * ∥ c ∥
                := exists_pow_lt_of_lt_one div_pos εpos Pos by normNum
              ...
              have yone : ∥ y ∥ ≤ 1 := le_transₓ y_lt.le pow_le_one _ by normNum by normNum
              obtain
                ⟨ k , hk , h'k ⟩
                : ∃ k : ℕ , 1 / 2 ^ k + 1 < ∥ y ∥ ∧ ∥ y ∥ ≤ 1 / 2 ^ k
                :=
                  exists_nat_pow_near_of_lt_one
                    yzero yone ( by normNum : ( 0 : ℝ ) < 1 / 2 ) ( by normNum : ( 1 : ℝ ) / 2 < 1 )
              have k_gt : n e < k
              ·
                have : ( 1 : ℝ ) / 2 ^ k + 1 < 1 / 2 ^ n e + 1 := lt_transₓ hk y_lt
                  rw [ pow_lt_pow_iff_of_lt_one ( by normNum : ( 0 : ℝ ) < 1 / 2 ) by normNum ] at this
                  linarith
              set m := k - 1 with hl
              ...
              ·
                apply le_of_mem_A hn e n e m le_reflₓ _ m_ge . 2 . 2
                  · simp only [ mem_closed_ball , dist_self ] exact div_nonneg le_of_ltₓ P zero_le_two
                  · simpa only [ dist_eq_norm , add_sub_cancel' , mem_closed_ball , pow_succ'ₓ , mul_one_div ] using h'k
              have
                J2
                  : ∥ f x + y - f x - L e n e m y ∥ ≤ 4 * 1 / 2 ^ e * ∥ y ∥
                  :=
                  calc
                    ∥ f x + y - f x - L e n e m y ∥ ≤ 1 / 2 ^ e * 1 / 2 ^ m
                        :=
                        by simpa only [ add_sub_cancel' ] using J1
                      _ = 4 * 1 / 2 ^ e * 1 / 2 ^ m + 2 := by fieldSimp ringExp
                      _ ≤ 4 * 1 / 2 ^ e * ∥ y ∥
                        :=
                        mul_le_mul_of_nonneg_left le_of_ltₓ hk mul_nonneg by normNum le_of_ltₓ P
              calc
                ∥ f x + y - f x - f' y ∥ = ∥ f x + y - f x - L e n e m y + L e n e m - f' y ∥ := congr_argₓ _ by simp
                  ...
                  _ ≤ 4 + 12 * ∥ c ∥ * ∥ y ∥ * ε / 4 + 12 * ∥ c ∥
                    ...
                    mul_le_mul_of_nonneg_left
                      he.le mul_nonneg add_nonneg by normNum mul_nonneg by normNum norm_nonneg _ norm_nonneg _
                  _ = ε * ∥ y ∥ := by fieldSimp [ ne_of_gtₓ Pos ] ring
...
theorem
  ...
  ( hK : IsComplete K ) : { x | DifferentiableAt 𝕜 f x ∧ fderiv 𝕜 f x ∈ K } = D f K
...
/--
  ...
  theorem
    ...
    { K : Set E →L[ 𝕜 ] F } ( hK : IsComplete K ) : MeasurableSet { x | DifferentiableAt 𝕜 f x ∧ fderiv 𝕜 f x ∈ K }
    :=
      by
        simp
          [
            differentiable_set_eq_D K hK
              ,
              D
              ,
              is_open_B.measurable_set
              ,
