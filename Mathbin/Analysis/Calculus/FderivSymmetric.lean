  ...
  by 
    ...
    ·
      ...
      ·
        ...
        ·
          exact xt_mem t ht 
        apply HasDerivAt.has_deriv_within_at 
        suffices  : HasDerivAt (fun u => (x+h • v)+(u*h) • w) ((0+0)+(1*h) • w) t
        ·
          simpa only [one_mulₓ, zero_addₓ]
        applyRules [HasDerivAt.add, has_deriv_at_const, HasDerivAt.smul_const, has_deriv_at_id']
      ·
        suffices  : HasDerivWithinAt (fun u => (u*h) • f' x w) ((1*h) • f' x w) (Icc 0 1) t
        ·
          simpa only [one_mulₓ]
        applyRules [HasDerivAt.has_deriv_within_at, HasDerivAt.smul_const, has_deriv_at_id']
      ·
        suffices  : HasDerivWithinAt (fun u => (u*h^2) • f'' v w) ((1*h^2) • f'' v w) (Icc 0 1) t
        ·
          simpa only [one_mulₓ]
        applyRules [HasDerivAt.has_deriv_within_at, HasDerivAt.smul_const, has_deriv_at_id']
      ·
...
theorem Convex.second_derivative_within_at_symmetric {s : Set E} (s_conv : Convex ℝ s) (hne : (Interior s).Nonempty)
  ...
  by 
    ...
    have B : ∀ m : E, ∀ᶠt in 𝓝[Ioi (0 : ℝ)] (0 : ℝ), (x+(4 : ℝ) • z+t • m) ∈ Interior s
    ·
      intro m 
...
theorem second_derivative_symmetric_of_eventually {f : E → F} {f' : E → E →L[ℝ] F} {f'' : E →L[ℝ] E →L[ℝ] F}
  (hf : ∀ᶠy in 𝓝 x, HasFderivAt f (f' y) y) (hx : HasFderivAt f' f'' x) (v w : E) : f'' v w = f'' w v :=
  by 
