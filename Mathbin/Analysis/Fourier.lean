...
@[simp]
theorem fourier_neg {n : ℤ} {z : circle} : fourier (-n) z = conj (fourier n z) :=
  by 
    simp [←coe_inv_circle_eq_conj z]

@[simp]
theorem fourier_add {m n : ℤ} {z : circle} : fourier (m+n) z = fourier m z*fourier n z :=
  by 
...
these functions. -/
theorem fourier_subalgebra_coe : fourierSubalgebra.toSubmodule = span ℂ (range fourier) :=
  by 
...
theorem fourier_subalgebra_separates_points : fourierSubalgebra.SeparatesPoints :=
  by 
    intro x y hxy 
...
conjugation. -/
theorem fourier_subalgebra_conj_invariant : conj_invariant_subalgebra (fourierSubalgebra.restrictScalars ℝ) :=
  by 
    rintro _ ⟨f, hf, rfl⟩
    ...
    ·
      rintro _ ⟨n, rfl⟩
      suffices  : fourier (-n) ∈ fourierSubalgebra
      ·
        convert this 
        ext1 
        simp 
...
theorem span_fourier_closure_eq_top : (span ℂ (range fourier)).topologicalClosure = ⊤ :=
  by 
