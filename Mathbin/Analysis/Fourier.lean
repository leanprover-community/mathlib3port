...
def haarCircle : Measure circle :=
  haarMeasure ⊤deriving IsHaarMeasure
#align haar_circle haarCircle
...
theorem fourierSubalgebraSeparatesPoints : fourierSubalgebra.SeparatesPoints := by
  intro x y hxy
...
#align fourier_subalgebra_separates_points fourierSubalgebraSeparatesPoints

/-- The subalgebra of `C(circle, ℂ)` generated by `z ^ n` for `n ∈ ℤ` is invariant under complex
conjugation. -/
theorem fourierSubalgebraConjInvariant : ConjInvariantSubalgebra (fourierSubalgebra.restrictScalars ℝ) := by
  rintro _ ⟨f, hf, rfl⟩
...
theorem fourier_subalgebra_closure_eq_top : fourierSubalgebra.topologicalClosure = ⊤ :=
  ContinuousMap.subalgebra_is_R_or_C_topological_closure_eq_top_of_separates_points fourierSubalgebra
    fourierSubalgebraSeparatesPoints fourierSubalgebraConjInvariant
...
theorem orthonormalFourier : Orthonormal ℂ (fourierLp 2) := by
