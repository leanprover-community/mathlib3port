...
private theorem convex.smul_integral_mem_of_measurable [is_finite_measure μ] {s : Set E} (hs : Convex ℝ s)
  (hsc : IsClosed s) (hμ : μ ≠ 0) {f : α → E} (hfs : ∀ᵐx ∂μ, f x ∈ s) (hfi : integrable f μ) (hfm : Measurable f) :
  ((μ univ).toReal⁻¹ • ∫x, f x ∂μ) ∈ s :=
  by 
    unfreezingI 
    ...
    have  : tendsto (fun n => (F n).integral μ) at_top (𝓝$ ∫x, f x ∂μ)
    ·
    ...
    have  : (∑y in (F n).range, (μ (F n ⁻¹' {y})).toReal) = (μ univ).toReal
    ·
...
theorem Convex.smul_integral_mem [is_finite_measure μ] {s : Set E} (hs : Convex ℝ s) (hsc : IsClosed s) (hμ : μ ≠ 0)
  {f : α → E} (hfs : ∀ᵐx ∂μ, f x ∈ s) (hfi : integrable f μ) : ((μ univ).toReal⁻¹ • ∫x, f x ∂μ) ∈ s :=
  by 
    have  : ∀ᵐx : α ∂μ, hfi.ae_measurable.mk f x ∈ s
    ·
      filterUpwards [hfs, hfi.ae_measurable.ae_eq_mk]
      intro a ha h 
      rwa [←h]
    convert
...
theorem ConvexOn.map_smul_integral_le [is_finite_measure μ] {s : Set E} {g : E → ℝ} (hg : ConvexOn ℝ s g)
  (hgc : ContinuousOn g s) (hsc : IsClosed s) (hμ : μ ≠ 0) {f : α → E} (hfs : ∀ᵐx ∂μ, f x ∈ s) (hfi : integrable f μ)
  (hgi : integrable (g ∘ f) μ) : g ((μ univ).toReal⁻¹ • ∫x, f x ∂μ) ≤ (μ univ).toReal⁻¹ • ∫x, g (f x) ∂μ :=
  by 
    ...
    have ht_mem : ∀ᵐx ∂μ, (f x, g (f x)) ∈ t := hfs.mono fun x hx => ⟨hx, le_rfl⟩
