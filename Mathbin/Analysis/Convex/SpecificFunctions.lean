...
theorem int_prod_range_nonneg (m : ℤ) (n : ℕ) (hn : Even n) : 0 ≤ ∏k in Finset.range n, m - k :=
  by 
    ...
    ·
      have  : m ≤ k+1 
      exact hmk.trans (lt_add_one («expr↑ » k)).le 
...
theorem convex_on_zpow (m : ℤ) : ConvexOn ℝ (Ioi 0) fun x : ℝ => x^m :=
  by 
    have  : ∀ n : ℤ, DifferentiableOn ℝ (fun x => x^n) (Ioi (0 : ℝ))
    exact fun n => differentiable_on_zpow _ _ (Or.inl$ lt_irreflₓ _)
...
theorem strict_convex_on_zpow {m : ℤ} (hm₀ : m ≠ 0) (hm₁ : m ≠ 1) : StrictConvexOn ℝ (Ioi 0) fun x : ℝ => x^m :=
  by 
    have  : ∀ n : ℤ, DifferentiableOn ℝ (fun x => x^n) (Ioi (0 : ℝ))
    exact fun n => differentiable_on_zpow _ _ (Or.inl$ lt_irreflₓ _)
...
theorem strict_concave_on_log_Ioi : StrictConcaveOn ℝ (Ioi 0) log :=
  by 
    have h₁ : Ioi 0 ⊆ «expr ᶜ» ({0} : Set ℝ)
    ·
      exact fun x hx : 0 < x hx' : x = 0 => hx.ne' hx' 
    refine'
      strict_concave_on_open_of_deriv2_neg (convex_Ioi 0) is_open_Ioi (differentiable_on_log.mono h₁)
        fun x hx : 0 < x => _ 
...
theorem strict_concave_on_log_Iio : StrictConcaveOn ℝ (Iio 0) log :=
  by 
    have h₁ : Iio 0 ⊆ «expr ᶜ» ({0} : Set ℝ)
    ·
      exact fun x hx : x < 0 hx' : x = 0 => hx.ne hx' 
    refine'
      strict_concave_on_open_of_deriv2_neg (convex_Iio 0) is_open_Iio (differentiable_on_log.mono h₁)
        fun x hx : x < 0 => _ 
    rw [Function.iterate_succ, Function.iterate_one]
    change (deriv (deriv log)) x < 0
    rw [deriv_log', deriv_inv]
