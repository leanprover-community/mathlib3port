...
theorem ConvexOn.slope_mono_adjacent (hf : ConvexOn 𝕜 s f) {x y z : 𝕜} (hx : x ∈ s) (hz : z ∈ s) (hxy : x < y)
  (hyz : y < z) : (f y - f x) / (y - x) ≤ (f z - f y) / (z - y) :=
  by 
    ...
    fieldSimp [hxy.ne', hyz.ne', hxz.ne', mul_commₓ (z - x) _]  at key⊢
    rw [div_le_div_right]
    ·
      linarith
    ·
...
theorem convex_on_of_slope_mono_adjacent (hs : Convex 𝕜 s)
  ...
  LinearOrderₓ.convex_on_of_lt hs
    (by 
      ...
      exact (div_le_div_iff (sub_pos.2 hxy) (sub_pos.2 hyz)).1 (hf hx hz hxy hyz)
      have hxz : 0 < z - x 
      exact sub_pos.2 (hxy.trans hyz)
      have ha : (z - y) / (z - x) = a
      ·
        rw [eq_comm, ←sub_eq_iff_eq_add'] at hab 
        simpRw [div_eq_iff hxz.ne', y, ←hab]
        ring 
      have hb : (y - x) / (z - x) = b
      ·
        rw [eq_comm, ←sub_eq_iff_eq_add] at hab 
  ...
  LinearOrderₓ.strict_convex_on_of_lt hs
    (by 
      ...
      ·
        ...
        exact add_lt_add_right ((mul_lt_mul_left ha).2 hxz) _ 
      have  : ((f y - f x)*z - y) < (f z - f y)*y - x 
      exact (div_lt_div_iff (sub_pos.2 hxy) (sub_pos.2 hyz)).1 (hf hx hz hxy hyz)
      have hxz : 0 < z - x 
      exact sub_pos.2 (hxy.trans hyz)
      have ha : (z - y) / (z - x) = a
      ·
