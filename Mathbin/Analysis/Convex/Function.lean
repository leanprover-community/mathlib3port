...
theorem ConvexOn.translate_right (hf : ConvexOn 𝕜 s f) (c : E) : ConvexOn 𝕜 ((fun z => c+z) ⁻¹' s) (f ∘ fun z => c+z) :=
  ⟨hf.1.translate_preimage_right _,
    fun x y hx hy a b ha hb hab =>
      calc f (c+(a • x)+b • y) = f ((a • c+x)+b • c+y) :=
        by 
...
theorem convex_on_iff_forall_pos {s : Set E} {f : E → β} :
  ...
  by 
    ...
    ·
      rw [zero_addₓ] at hab 
      subst b 
      simpRw [zero_smul, zero_addₓ, one_smul]
    obtain rfl | hb' := hb.eq_or_lt
    ·
      rw [add_zeroₓ] at hab 
      subst a 
      simpRw [zero_smul, add_zeroₓ, one_smul]
  ...
  convex_iff_pairwise_pos.2$
    fun x hx y hy hxy a b ha hb hab =>
      ⟨hf.1 hx.1 hy.1 ha.le hb.le hab,
        calc f ((a • x)+b • y) < (a • f x)+b • f y := hf.2 hx.1 hy.1 hxy ha hb hab 
...
theorem ConvexOn.comp_linear_map {f : F → β} {s : Set F} (hf : ConvexOn 𝕜 s f) (g : E →ₗ[𝕜] F) :
  ConvexOn 𝕜 (g ⁻¹' s) (f ∘ g) :=
  ⟨hf.1.linear_preimage _,
    fun x y hx hy a b ha hb hab =>
      calc f (g ((a • x)+b • y)) = f ((a • g x)+b • g y) :=
        by 
...
theorem StrictConvexOn.add (hf : StrictConvexOn 𝕜 s f) (hg : StrictConvexOn 𝕜 s g) : StrictConvexOn 𝕜 s (f+g) :=
  ⟨hf.1,
    fun x y hx hy hxy a b ha hb hab =>
      calc (f ((a • x)+b • y)+g ((a • x)+b • y)) < ((a • f x)+b • f y)+(a • g x)+b • g y :=
...
theorem ConvexOn.convex_lt (hf : ConvexOn 𝕜 s f) (r : β) : Convex 𝕜 { x ∈ s | f x < r } :=
  convex_iff_forall_pos.2$
    fun x y hx hy a b ha hb hab =>
      ⟨hf.1 hx.1 hy.1 ha.le hb.le hab,
        calc f ((a • x)+b • y) ≤ (a • f x)+b • f y := hf.2 hx.1 hy.1 ha.le hb.le hab 
          _ < (a • r)+b • r :=
...
theorem StrictConvexOn.lt_right_of_left_lt' (hf : StrictConvexOn 𝕜 s f) {x y : E} {a b : 𝕜} (hx : x ∈ s) (hy : y ∈ s)
  (ha : 0 < a) (hb : 0 < b) (hab : (a+b) = 1) (hfx : f x < f ((a • x)+b • y)) : f ((a • x)+b • y) < f y :=
  by 
    rw [add_commₓ] at hab hfx⊢
...
theorem neg_convex_on_iff : ConvexOn 𝕜 s (-f) ↔ ConcaveOn 𝕜 s f :=
  by 
    ...
    ·
      ...
      refine' ⟨hconv, fun x y hx hy a b ha hb hab => _⟩
      rw [←neg_le_neg_iff]
      simpRw [neg_add, Pi.neg_apply, smul_neg, neg_negₓ]
      exact h hx hy ha hb hab

/-- A function `-f` is concave iff `f` is convex. -/
@[simp]
theorem neg_concave_on_iff : ConcaveOn 𝕜 s (-f) ↔ ConvexOn 𝕜 s f :=
  by 
...
@[simp]
theorem neg_strict_concave_on_iff : StrictConcaveOn 𝕜 s (-f) ↔ StrictConvexOn 𝕜 s f :=
  by 
