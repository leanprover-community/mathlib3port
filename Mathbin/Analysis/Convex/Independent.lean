...
-- error in Analysis.Convex.Independent: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
theorem convex_independent_iff_not_mem_convex_hull_diff
{p : ι → E} : «expr ↔ »(convex_independent 𝕜 p, ∀
 i s, «expr ∉ »(p i, convex_hull 𝕜 «expr '' »(p, «expr \ »(s, {i})))) :=
begin
  refine [expr ⟨λ hc i s h, _, λ h s i hi, _⟩],
  { rw [expr hc.mem_convex_hull_iff] ["at", ident h],
    exact [expr h.2 (set.mem_singleton _)] },
  { by_contra [ident H],
    refine [expr h i s _],
    rw [expr set.diff_singleton_eq_self H] [],
    exact [expr hi] }
end
...
theorem convex_independent_set_iff_not_mem_convex_hull_diff
{s : set E} : «expr ↔ »(convex_independent 𝕜 (λ
 x, x : s → E), ∀ x «expr ∈ » s, «expr ∉ »(x, convex_hull 𝕜 «expr \ »(s, {x}))) :=
begin
  rw [expr convex_independent_set_iff_inter_convex_hull_subset] [],
  split,
  { rintro [ident hs, ident x, ident hxs, ident hx],
    exact [expr (hs _ (set.diff_subset _ _) ⟨hxs, hx⟩).2 (set.mem_singleton _)] },
  { rintro [ident hs, ident t, ident ht, ident x, "⟨", ident hxs, ",", ident hxt, "⟩"],
    by_contra [ident h],
    exact [expr hs _ hxs (convex_hull_mono (set.subset_diff_singleton ht h) hxt)] }
end
