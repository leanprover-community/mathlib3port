    ...
    :
      convexHull R range v
        =
        {
          ...
          |
          ∃
            ( s : Finset ι ) ( w : ι → R ) ( hw₀ : ∀ i _ : i ∈ s , 0 ≤ w i ) ( hw₁ : s.sum w = 1 )
            ,
            s.affine_combination v w = x
          }
  :=
    by
        ...
        ·
          rw [ Convex ]
            ...
            ·
              rw
                  [
                    sum_add_distrib
                      ,
                      ← sum_subset subset_union_left s s'
                      ,
                      ← sum_subset subset_union_right s s'
                      ,
                      sum_ite_of_true _ _ fun i hi => hi
                      ,
                      sum_ite_of_true _ _ fun i hi => hi
                      ,
                      ← mul_sum
            ...
            ·
              simpRw
                  [
                    affine_combination_eq_linear_combination s ∪ s' v _ hW₁
                      ,
                ...
                · intro i hi hi' simp [ hi' ]
                · intro i hi hi' simp [ hi' ]
        · rintro x ⟨ s , w , hw₀ , hw₁ , rfl ⟩ exact affine_combination_mem_convex_hull hw₀ hw₁
...
theorem Set.Finite.convex_hull_eq_image {s : Set E} (hs : finite s) :
  convexHull R s =
    by 
      have  := hs.fintype <;>
        exact (⇑∑ x : s, (@LinearMap.proj R s _ (fun i => R) _ _ x).smulRight x.1) '' StdSimplex R s :=
  by 
...
/--
  ...
  theorem
    ...
    { ι : Type _ } ( b : AffineBasis ι R E ) : convexHull R range b.points = { x | ∀ i , 0 ≤ b.coord i x }
    :=
      by
        rw [ convex_hull_range_eq_exists_affine_combination ]
          ...
          ·
            rintro ⟨ s , w , hw₀ , hw₁ , rfl ⟩ i
              byCases' hi : i ∈ s
              · rw [ b.coord_apply_combination_of_mem hi hw₁ ] exact hw₀ i hi
              · rw [ b.coord_apply_combination_of_not_mem hi hw₁ ]
          ·
            intro hx
              have hx' : x ∈ affineSpan R range b.points
              · rw [ b.tot ] exact AffineSubspace.mem_top R E x
