...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (z «expr > » a)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » Icc a z)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » Ioc a x)
...
theorem IsMinOn.of_is_local_min_on_of_convex_on_Icc {f : ℝ → β} {a b : ℝ} (a_lt_b : a < b)
  (h_local_min : IsLocalMinOn f (Icc a b) a) (h_conv : ConvexOn ℝ (Icc a b) f) : ∀ x _ : x ∈ Icc a b, f a ≤ f x :=
  by 
    byContra H_cont 
    pushNeg  at H_cont 
    rcases H_cont with ⟨x, ⟨h_ax, h_xb⟩, fx_lt_fa⟩
    obtain ⟨z, hz, ge_on_nhd⟩ : ∃ (z : _)(_ : z > a), ∀ y _ : y ∈ Icc a z, f y ≥ f a
    ·
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » (Icc 0 1 : set exprℝ()))
...
theorem IsMinOn.of_is_local_min_on_of_convex_on {f : E → β} {a : E} (a_in_s : a ∈ s) (h_localmin : IsLocalMinOn f s a)
  (h_conv : ConvexOn ℝ s f) : ∀ x _ : x ∈ s, f a ≤ f x :=
  by 
    byContra H_cont 
    pushNeg  at H_cont 
    ...
    have fg_local_min_on : IsLocalMinOn (f ∘ g) (g ⁻¹' s) 0
    ·
      rw [←hg0] at h_localmin 
      refine'
        IsLocalMinOn.comp_continuous_on h_localmin subset.rfl (Continuous.continuous_on AffineMap.line_map_continuous)
          _ 
      simp [mem_preimage, hg0, a_in_s]
    have fg_min_on : ∀ x _ : x ∈ (Icc 0 1 : Set ℝ), (f ∘ g) 0 ≤ (f ∘ g) x
    ·
