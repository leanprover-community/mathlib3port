...
theorem summable_nnnorm_mul_pow (p : FormalMultilinearSeries 𝕜 E F) {r : ℝ≥0} (h : ↑r < p.radius) :
    Summable fun n : ℕ => ‖p n‖₊ * r ^ n := by rw [← NNReal.summable_coe]; push_cast;
...
/- ././././Mathport/Syntax/Translate/Basic.lean:642:2: warning: expanding binder collection (i «expr ≠ » 0) -/
...
theorem HasFPowerSeriesOnBall.isBigO_image_sub_image_sub_deriv_principal
  ...
  have hL : ∀ y ∈ EMetric.ball (x, x) r', ‖f y.1 - f y.2 - p 1 fun _ => y.1 - y.2‖ ≤ L y :=
    by
    intro y hy'
    have hy : y ∈ EMetric.ball x r ×ˢ EMetric.ball x r := by rw [EMetric.ball_prod_same];
...
theorem HasFPowerSeriesOnBall.tendstoLocallyUniformlyOn' (hf : HasFPowerSeriesOnBall f p x r) :
  ...
  · ext z; simp
  · intro z; simp [edist_eq_coe_nnnorm, edist_eq_coe_nnnorm_sub]
#align has_fpower_series_on_ball.tendsto_locally_uniformly_on' HasFPowerSeriesOnBall.tendstoLocallyUniformlyOn'
-/
    ...
    suffices
      ∀ k' l',
        k' = k →
          l' = l →
            ∀ (hkl : k + l = k' + l') (hs'),
              (⟨k', l', ⟨Finset.map (Fin.castOrderIso hkl).toEquiv.toEmbedding s, hs'⟩⟩ :
                  Σ k l : ℕ, { s : Finset (Fin (k + l)) // s.card = l }) =
                ⟨k, l, ⟨s, hs⟩⟩
      by apply this <;> simp only [hs, add_tsub_cancel_right]
    rintro _ _ rfl rfl hkl hs'
    simp only [Equiv.refl_toEmbedding, Fin.castOrderIso_refl, Finset.map_refl, eq_self_iff_true,
      OrderIso.refl_toEquiv, and_self_iff, heq_iff_eq]
  right_inv := by
    rintro ⟨n, s⟩
    simp [tsub_add_cancel_of_le (card_finset_fin_le s), Fin.castOrderIso_toEquiv]
  ...
  have :
    ∀ (m) (hm : n = m),
      p n (s.piecewise (fun _ => x) fun _ => y) =
        p m ((s.map (Fin.castOrderIso hm).toEquiv.toEmbedding).piecewise (fun _ => x) fun _ => y) :=
