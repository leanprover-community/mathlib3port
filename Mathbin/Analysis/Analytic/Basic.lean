...
theorem is_o_of_lt_radius (h : «expr↑ » r < p.radius) :
...
theorem lt_radius_of_is_O (h₀ : r ≠ 0) {a : ℝ} (ha : a ∈ Ioo (-1 : ℝ) 1)
  (hp : is_O (fun n => ∥p n∥*r ^ n) (pow a) at_top) : «expr↑ » r < p.radius :=
  by 
...
-- error in Analysis.Analytic.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
/-- If a function admits a power series expansion, then it is exponentially close to the partial
sums of this power series on strict subdisks of the disk of convergence. -/
...
theorem HasFpowerSeriesOnBall.is_O_image_sub_image_sub_deriv_principal (hf : HasFpowerSeriesOnBall f p x r)
  ...
  by 
    ...
    obtain ⟨a, ha, C, hC : 0 < C, hp⟩ :
      ∃ (a : _)(_ : a ∈ Ioo (0 : ℝ) 1)(C : _)(_ : C > 0), ∀ n : ℕ, (∥p n∥*«expr↑ » r' ^ n) ≤ C*a ^ n 
    ...
    ·
      ...
      have hAB : ∀ n, ∥A (n+2)∥ ≤ B n :=
        fun n =>
          calc ∥A (n+2)∥ ≤ ((∥p (n+2)∥*«expr↑ » (n+2))*∥y - (x, x)∥ ^ n+1)*∥y.1 - y.2∥ :=
            by 
            ...
            _ = (∥p (n+2)∥*∥y - (x, x)∥ ^ n)*(«expr↑ » (n+2)*∥y - (x, x)∥)*∥y.1 - y.2∥ :=
            by 
              rw [pow_succₓ ∥y - (x, x)∥]
              acRfl 
            _ ≤ (((C*a ^ n+2) / r' ^ n+2)*r' ^ n)*(«expr↑ » (n+2)*∥y - (x, x)∥)*∥y.1 - y.2∥ :=
            by 
...
theorem change_origin_series_summable_aux₁ {r r' :  ℝ≥0 } (hr : (r+r' : ℝ≥0∞) < p.radius) :
  Summable fun s : Σk l : ℕ, { s : Finset (Finₓ (k+l)) // s.card = l } => (∥p (s.1+s.2.1)∥₊*r ^ s.2.1)*r' ^ s.1 :=
  by 
...
theorem nnnorm_change_origin_le (k : ℕ) (h : (∥x∥₊ : ℝ≥0∞) < p.radius) :
  ∥p.change_origin x k∥₊ ≤ ∑'s : Σl : ℕ, { s : Finset (Finₓ (k+l)) // s.card = l }, ∥p (k+s.1)∥₊*∥x∥₊ ^ s.1 :=
  by 
...
theorem change_origin_eval (h : (∥x∥₊+∥y∥₊ : ℝ≥0∞) < p.radius) : (p.change_origin x).Sum y = p.sum (x+y) :=
  by 
    ...
    set f : (Σk l : ℕ, { s : Finset (Finₓ (k+l)) // s.card = l }) → F :=
