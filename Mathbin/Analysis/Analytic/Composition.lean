  ...
  by 
    ...
    ·
      ...
      ·
        ...
        simp 
      show
        ∀ b : Composition n, b ∈ Finset.univ → b ≠ Composition.single n n_pos → comp_along_composition (id 𝕜 F) p b = 0
      ·
        intro b _ hb 
        have A : b.length ≠ 1
        ·
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (Cq «expr > » 0)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (Cp «expr ≥ » 1)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (r «expr > » (0 : «exprℝ≥0»()))
...
theorem comp_summable_nnreal (q : FormalMultilinearSeries 𝕜 F G) (p : FormalMultilinearSeries 𝕜 E F) (hq : 0 < q.radius)
  (hp : 0 < p.radius) :
  ∃ (r : _)(_ : r > (0 :  ℝ≥0 )),
    Summable fun i : Σ n, Composition n => nnnorm (q.comp_along_composition p i.2)*r ^ i.1 :=
  by 
    ...
    obtain ⟨Cq, hCq0, hCq⟩ : ∃ (Cq : _)(_ : Cq > 0), ∀ n, (nnnorm (q n)*rq ^ n) ≤ Cq :=
      q.nnnorm_mul_pow_le_of_lt_radius hrq.2
    obtain ⟨Cp, hCp1, hCp⟩ : ∃ (Cp : _)(_ : Cp ≥ 1), ∀ n, (nnnorm (p n)*rp ^ n) ≤ Cp
    ·
    ...
    have I : ∀ i : Σ n : ℕ, Composition n, (nnnorm (q.comp_along_composition p i.2)*r ^ i.1) ≤ Cq / 4 ^ i.1
    ·
      rintro ⟨n, c⟩
      have A 
      ...
      have B 
      calc ((∏ i, nnnorm (p (c.blocks_fun i)))*rp ^ n) = ∏ i, nnnorm (p (c.blocks_fun i))*rp ^ c.blocks_fun i :=
        by 
          simp only [Finset.prod_mul_distrib, Finset.prod_pow_eq_pow_sum,
            c.sum_blocks_fun]_ ≤ ∏ i : Finₓ c.length, Cp :=
        Finset.prod_le_prod' fun i _ => hCp _ _ = Cp ^ c.length :=
        by 
          simp _ ≤ Cp ^ n :=
        pow_le_pow hCp1 c.length_le 
      calc
        (nnnorm (q.comp_along_composition p c)*r ^ n) ≤ (nnnorm (q c.length)*∏ i, nnnorm (p (c.blocks_fun i)))*r ^ n :=
        ...
        by 
          ...
          acRfl _ ≤ (Cq*Cp ^ n)*r0 ^ n :=
        mul_le_mul' (mul_le_mul' A B) le_rfl _ = Cq / 4 ^ n :=
        by 
          simp only [r0]
          fieldSimp [mul_powₓ, (zero_lt_one.trans_le hCp1).ne']
          acRfl 
    ...
    fieldSimp [←mul_assocₓ, pow_succ'ₓ, mul_powₓ, show (4 :  ℝ≥0 ) = 2*2 from (two_mul 2).symm, mul_right_commₓ]
...
theorem le_comp_radius_of_summable (q : FormalMultilinearSeries 𝕜 F G) (p : FormalMultilinearSeries 𝕜 E F) (r :  ℝ≥0 )
  (hr : Summable fun i : Σ n, Composition n => nnnorm (q.comp_along_composition p i.2)*r ^ i.1) :
...
power series -/
def comp_change_of_variables (m M N : ℕ) (i : Σ n, Finₓ n → ℕ) (hi : i ∈ comp_partial_sum_source m M N) :
  Σ n, Composition n :=
  by 
    rcases i with ⟨n, f⟩
...
@[simp]
theorem comp_change_of_variables_length (m M N : ℕ) {i : Σ n, Finₓ n → ℕ} (hi : i ∈ comp_partial_sum_source m M N) :
...
theorem comp_change_of_variables_blocks_fun (m M N : ℕ) {i : Σ n, Finₓ n → ℕ} (hi : i ∈ comp_partial_sum_source m M N)
  (j : Finₓ i.1) :
...
theorem comp_change_of_variables_sum {α : Type _} [AddCommMonoidₓ α] (m M N : ℕ) (f : (Σ n : ℕ, Finₓ n → ℕ) → α)
  ...
  by 
    ...
    ·
      ...
      simp only [mem_comp_partial_sum_target_iff, Composition.length, Composition.blocks, H.left, map_of_fn,
        length_of_fn, true_andₓ, comp_change_of_variables]
      intro j 
        ...
        by 
          apply Composition.blocks_fun_congr <;>
            try 
              rw [HEq]
          rfl _ = blocks_fun' i :=
        comp_change_of_variables_blocks_fun m M N H' i
    ·
      intro i hi 
...
theorem comp_partial_sum_target_tendsto_at_top : tendsto (fun N => comp_partial_sum_target 0 N N) at_top at_top :=
  by 
    ...
    ·
      ...
      refine'
        ⟨max n c.length+1, bot_le, lt_of_le_of_ltₓ (le_max_rightₓ n c.length) (lt_add_one _),
          fun j => lt_of_le_of_ltₓ (le_transₓ _ (le_max_leftₓ _ _)) (lt_add_one _)⟩
      apply hn 
...
theorem HasFpowerSeriesAt.comp {g : F → G} {f : E → F} {q : FormalMultilinearSeries 𝕜 F G}
  ...
  by 
    ...
    obtain ⟨δ, δpos, hδ⟩ : ∃ (δ : ℝ≥0∞)(H : 0 < δ), ∀ {z : E}, z ∈ Emetric.Ball x δ → f z ∈ Emetric.Ball (f x) rg
    ·
    ...
    refine' ⟨min rf' r, _⟩
    refine'
      ⟨le_transₓ (min_le_rightₓ rf' r) (FormalMultilinearSeries.le_comp_radius_of_summable q p r hr), min_pos,
        fun y hy => _⟩
    ...
    ·
      have L : ∀ᶠ n in at_top, (∑ a in Finset.range n, p a fun b => y) - f x = ∑ a in Finset.ico 1 n, p a fun b => y
      ·
    ...
    have B : tendsto (fun n => q.partial_sum n (∑ a in Finset.ico 1 n, p a fun b => y)) at_top (𝓝 (g (f (x+y))))
    ·
    ...
    ·
      ...
      ·
        ...
        calc ∥(comp_along_composition q p c) fun j : Finₓ n => y∥ ≤ ∥comp_along_composition q p c∥*∏ j : Finₓ n, ∥y∥ :=
          by 
            apply ContinuousMultilinearMap.le_op_norm _ ≤ ∥comp_along_composition q p c∥*(r : ℝ) ^ n :=
          by 
            apply mul_le_mul_of_nonneg_left _ (norm_nonneg _)
...
theorem sigma_pi_composition_eq_iff (u v : Σ c : Composition n, ∀ i : Finₓ c.length, Composition (c.blocks_fun i)) :
  u = v ↔ (of_fn fun i => (u.2 i).blocks) = of_fn fun i => (v.2 i).blocks :=
  by 
    refine'
      ⟨fun H =>
          by 
...
theorem size_up_to_size_up_to_add (a : Composition n) (b : Composition a.length) {i j : ℕ} (hi : i < b.length)
  ...
  size_up_to a (size_up_to b i+j) =
    size_up_to (a.gather b) i+size_up_to (sigma_composition_aux a b ⟨i, (length_gather a b).symm ▸ hi⟩) j :=
  by 
...
def sigma_equiv_sigma_pi (n : ℕ) :
  (Σ a : Composition n, Composition a.length) ≃
    Σ c : Composition n, ∀ i : Finₓ c.length, Composition (c.blocks_fun i) :=
  { toFun := fun i => ⟨i.1.gather i.2, i.1.sigmaCompositionAux i.2⟩,
    invFun :=
      fun i =>
        ⟨{ blocks := (of_fn fun j => (i.2 j).blocks).join,
            blocks_pos :=
              by 
          ...
          { blocks := of_fn fun j => (i.2 j).length,
            blocks_pos :=
              forall_mem_of_fn_iff.2 fun j => Composition.length_pos_of_pos _ (Composition.blocks_pos' _ _ _),
            blocks_sum :=
              by 
    ...
    left_inv :=
      by 
        ...
        ·
          ...
          convRHS => rw [←join_split_wrt_composition a.blocks b, ←of_fn_nth_le (split_wrt_composition a.blocks b)]
          congr
          ·
            exact A
          ·
        ...
        ·
          ...
          convRHS => rw [←of_fn_nth_le b.blocks]
          congr 1
    ...
    right_inv :=
      by 
        rintro ⟨c, d⟩
        have  : map List.sum (of_fn fun i : Finₓ (Composition.length c) => (d i).blocks) = c.blocks
        ·
...
theorem comp_assoc (r : FormalMultilinearSeries 𝕜 G H) (q : FormalMultilinearSeries 𝕜 F G)
  ...
  by 
    ...
    let g : (Σ c : Composition n, ∀ i : Finₓ c.length, Composition (c.blocks_fun i)) → H :=
      fun c =>
