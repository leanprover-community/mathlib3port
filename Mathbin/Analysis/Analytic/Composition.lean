...
theorem comp_summable_nnreal (q : FormalMultilinearSeries 𝕜 F G) (p : FormalMultilinearSeries 𝕜 E F) (hq : 0 < q.radius)
  (hp : 0 < p.radius) :
  ∃ (r : _)(_ : r > (0 :  ℝ≥0 )),
    Summable fun i : Σn, Composition n => nnnorm (q.comp_along_composition p i.2)*r ^ i.1 :=
  by 
    ...
    have I : ∀ i : Σn : ℕ, Composition n, (nnnorm (q.comp_along_composition p i.2)*r ^ i.1) ≤ Cq / 4 ^ i.1
    ·
      rintro ⟨n, c⟩
      have A 
...
theorem comp_partial_sum_target_tendsto_at_top : tendsto (fun N => comp_partial_sum_target 0 N N) at_top at_top :=
  by 
    ...
    ·
      ...
      obtain ⟨n, hn⟩ : BddAbove («expr↑ » (finset.univ.image fun i : Finₓ c.length => c.blocks_fun i)) :=
        Finset.bdd_above _ 
      refine'
...
theorem HasFpowerSeriesAt.comp {g : F → G} {f : E → F} {q : FormalMultilinearSeries 𝕜 F G}
  ...
  by 
    ...
    ·
      have L : ∀ᶠn in at_top, (∑a in Finset.range n, p a fun b => y) - f x = ∑a in Finset.ico 1 n, p a fun b => y
      ·
    ...
    ·
      have cau : CauchySeq fun s : Finset (Σn, Composition n) => ∑i in s, q.comp_along_composition p i.2 fun j => y
      ·
...
theorem sigma_pi_composition_eq_iff (u v : Σc : Composition n, ∀ i : Finₓ c.length, Composition (c.blocks_fun i)) :
...
def sigma_equiv_sigma_pi (n : ℕ) :
  (Σa : Composition n, Composition a.length) ≃ Σc : Composition n, ∀ i : Finₓ c.length, Composition (c.blocks_fun i) :=
