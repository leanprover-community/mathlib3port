...
theorem removeZero_applyComposition (p : FormalMultilinearSeries 𝕜 E F) {n : ℕ}
  ...
  by
  ext (v i)
  simp [applyComposition, zero_lt_one.trans_le (c.one_le_blocks_fun i), removeZero_of_pos]
...
def compAlongComposition {n : ℕ} (p : FormalMultilinearSeries 𝕜 E F) (c : Composition n)
  ...
  map_add' v i x y := by simp only [applyComposition_update, ContinuousMultilinearMap.map_add]
  map_smul' v i c x := by simp only [applyComposition_update, ContinuousMultilinearMap.map_smul]
  cont :=
    f.cont.comp <|
...
theorem le_comp_radius_of_summable (q : FormalMultilinearSeries 𝕜 F G)
  ...
  refine'
    le_radius_of_bound_nnreal _
      (∑' i : Σn, Composition n, ‖compAlongComposition q p i.snd‖₊ * r ^ i.fst) fun n => _
  calc
    ‖FormalMultilinearSeries.comp q p n‖₊ * r ^ n ≤
        ∑' c : Composition n, ‖compAlongComposition q p c‖₊ * r ^ n :=
      by
      rw [tsum_fintype, ← Finset.sum_mul]
      exact mul_le_mul' (nnnorm_sum_le _ _) le_rfl
    _ ≤ ∑' i : Σn : ℕ, Composition n, ‖compAlongComposition q p i.snd‖₊ * r ^ i.fst :=
...
theorem compChangeOfVariables_blocksFun (m M N : ℕ) {i : Σn, Fin n → ℕ}
  ...
  simp only [map_ofFn, nthLe_ofFn', Function.comp_apply]
  apply congr_arg
  exact Fin.eta _ _
...
theorem comp_partial_sum_target_subset_image_compPartialSumSource (m M N : ℕ)
  ...
  · dsimp [compChangeOfVariables]
    ...
    simp only [Composition.blocksFun, composition.blocks, Subtype.coe_eta, nthLe_map']
    conv_lhs => rw [← ofFn_nthLe c.blocks]
...
theorem compChangeOfVariables_sum {α : Type _} [AddCommMonoid α] (m M N : ℕ)
  ...
  · rintro ⟨k, blocks_fun⟩ H
    ...
    intro j
    simp only [Composition.blocksFun, (H.right _).right, nthLe_ofFn']
  ...
  · rintro ⟨k, blocks_fun⟩ ⟨k', blocks_fun'⟩ H H' heq
    ...
    calc
      ...
      _ = (compChangeOfVariables m M N _ H').2.blocksFun _ :=
        by
  ...
  · intro i hi
    apply comp_partial_sum_target_subset_image_compPartialSumSource m M N i
...
theorem comp_partialSum (q : FormalMultilinearSeries 𝕜 F G) (p : FormalMultilinearSeries 𝕜 E F)
  ...
  suffices H :
    (∑ n in Finset.range N,
      ...
      ∑ i in compPartialSumTarget 0 N N, q.comp_along_composition p i.2 fun j => z
...
theorem HasFpowerSeriesAt.comp {g : F → G} {f : E → F} {q : FormalMultilinearSeries 𝕜 F G}
  ...
  have A : Tendsto (fun n => ∑ a in Finset.Ico 1 n, p a fun b => y) atTop (𝓝 (f (x + y) - f x)) :=
    by
    have L :
      ∀ᶠ n in atTop,
    ...
    have :
      Tendsto (fun n => (∑ a in Finset.range n, p a fun b => y) - f x) atTop
        (𝓝 (f (x + y) - f x)) :=
  ...
  have B :
    Tendsto (fun n => q.partial_sum n (∑ a in Finset.Ico 1 n, p a fun b => y)) atTop
      (𝓝 (g (f (x + y)))) :=
    by
  ...
  have C :
    Tendsto (fun n => ∑ i in compPartialSumTarget 0 n n, q.comp_along_composition p i.2 fun j => y)
      atTop (𝓝 (g (f (x + y)))) :=
  ...
  have D :
    ...
    haveI cau :
      ...
      rintro ⟨n, c⟩
      calc
        ‖(compAlongComposition q p c) fun j : Fin n => y‖ ≤
            ‖compAlongComposition q p c‖ * ∏ j : Fin n, ‖y‖ :=
...
theorem sigma_pi_composition_eq_iff
  ...
  ext i : 2
  have :
    nthLe (ofFn fun i : Fin (Composition.length a) => (b i).blocks) i (by simp [i.is_lt]) =
      nthLe (ofFn fun i : Fin (Composition.length a) => (b' i).blocks) i (by simp [i.is_lt]) :=
    nthLe_of_eq H _
  rwa [nthLe_ofFn, nthLe_ofFn] at this
...
def sigmaCompositionAux (a : Composition n) (b : Composition a.length)
  ...
  blocks_pos i hi :=
    a.blocks_pos
      (by
        rw [← a.blocks.join_split_wrt_composition b]
        exact mem_join_of_mem (nthLe_mem _ _ _) hi)
  blocks_sum := by simp only [Composition.blocksFun, nthLe_map', Composition.gather]
...
theorem sizeUpTo_sizeUpTo_add (a : Composition n) (b : Composition a.length) {i j : ℕ}
  ...
  · have A : j < blocksFun b ⟨i, hi⟩ := lt_trans (lt_add_one j) hj
    have B : j < length (sigmaCompositionAux a b ⟨i, (length_gather a b).symm ▸ hi⟩) :=
      by
      convert A
    ...
    have C : sizeUpTo b i + j < sizeUpTo b (i + 1) :=
      ...
      exact A
    have D : sizeUpTo b i + j < length a := lt_of_lt_of_le C (b.size_up_to_le _)
...
def sigmaEquivSigmaPi (n : ℕ) :
  ...
  left_inv :=
    ...
    dsimp
    constructor
    · have A := length_map List.sum (splitWrtComposition a.blocks b)
      conv_rhs =>
    ...
    · have B : Composition.length (Composition.gather a b) = List.length b.blocks :=
      ...
      rw [sigmaCompositionAux, Composition.length, nthLe_map_rev List.length,
        nthLe_of_eq (map_length_splitWrtComposition _ _)]
      rfl
  right_inv :=
    by
    ...
    · rw [Fin.heq_fun_iff]
      · intro i
        ...
        rw [nthLe_of_eq (splitWrtComposition_join _ _ _)]
        · simp only [nthLe_ofFn']
        · simp only [map_ofFn]
      · congr
        ext1
        dsimp [Composition.gather]
