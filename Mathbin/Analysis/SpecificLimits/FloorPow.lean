...
theorem tendsto_div_of_monotone_of_exists_subseq_tendsto_div (u : ℕ → ℝ) (l : ℝ) (hmono : Monotoneₓ u)
    (hlim :
      ∀ a : ℝ,
        1 < a →
          ∃ c : ℕ → ℕ,
  ...
  have A : ∀ ε : ℝ, 0 < ε → ∀ᶠ n in at_top, u n - n * l ≤ ε * (1 + ε + l) * n := by
    ...
    let M := ((Finsetₓ.range (a + 1)).Image fun i => c i).max' (by simp)
    ...
    calc
      ...
      _ ≤ ε * c N + ε * c (N - 1) * l := by
        apply add_le_add
        · apply (ha _ _).2
          exact le_transₓ (by simp only [le_add_iff_nonneg_right, zero_le']) aN
          
        · apply mul_le_mul_of_nonneg_right _ lnonneg
          linarith only [IcN]
          
      _ ≤ ε * ((1 + ε) * c (N - 1)) + ε * c (N - 1) * l := add_le_add (mul_le_mul_of_nonneg_left IcN εpos.le) le_rflₓ
  ...
  have B : ∀ ε : ℝ, 0 < ε → ∀ᶠ n : ℕ in at_top, (n : ℝ) * l - u n ≤ ε * (1 + l) * n := by
    ...
    calc
      ...
      _ ≤ (1 + ε) * c (N - 1) * l - u (c (N - 1)) := by
        refine' add_le_add (mul_le_mul_of_nonneg_right _ lnonneg) le_rflₓ
        have B : N - 1 + 1 = N := Nat.succ_pred_eq_of_posₓ Npos
        have := (ha _ aN').1
        rwa [B] at this
      _ = c (N - 1) * l - u (c (N - 1)) + ε * c (N - 1) * l := by ring
      _ ≤ ε * c (N - 1) + ε * c (N - 1) * l := add_le_add (ha _ aN').2 le_rflₓ
...
`c`s converging to `1`. -/
theorem tendsto_div_of_monotone_of_tendsto_div_floor_pow (u : ℕ → ℝ) (l : ℝ) (hmono : Monotoneₓ u) (c : ℕ → ℝ)
...
theorem sum_div_pow_sq_le_div_sq (N : ℕ) {j : ℝ} (hj : 0 < j) {c : ℝ} (hc : 1 < c) :
  ...
  calc
    ...
    _ ≤ (c⁻¹ ^ 2) ^ (Real.log j / Real.log c - 1) / (1 - c⁻¹ ^ 2) := by
      apply div_le_div _ _ _ le_rflₓ
    ...
    _ = c ^ 2 * (1 - c⁻¹ ^ 2)⁻¹ / j ^ 2 := by
      ...
      ring
    _ ≤ c ^ 3 * (c - 1)⁻¹ / j ^ 2 := by
      apply div_le_div _ B (sq_pos_of_pos hj) le_rflₓ
...
theorem mul_pow_le_nat_floor_pow {c : ℝ} (hc : 1 < c) (i : ℕ) : (1 - c⁻¹) * c ^ i ≤ ⌊c ^ i⌋₊ := by
  ...
  calc
    ...
    _ ≤ c ^ i - 1 := by
      simpa only [← div_eq_mul_inv, sub_le_sub_iff_left, one_le_div cpos, pow_oneₓ] using pow_le_pow hc.le hident
...
theorem sum_div_nat_floor_pow_sq_le_div_sq (N : ℕ) {j : ℝ} (hj : 0 < j) {c : ℝ} (hc : 1 < c) :
  ...
  calc
    (∑ i in (range N).filter fun i => j < ⌊c ^ i⌋₊, (1 : ℝ) / ⌊c ^ i⌋₊ ^ 2) ≤
      ...
      · intro i hi
        simp only [mem_filter, mem_range] at hi
        simpa only [hi.1, mem_filter, mem_range, true_andₓ] using hi.2.trans_le (Nat.floor_le (pow_nonneg cpos.le _))
