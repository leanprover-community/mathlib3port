...
def SuperpolynomialDecay {α β : Type _} [TopologicalSpace β] [CommSemiring β] (l : Filter α) (k : α → β) (f : α → β) :=
...
theorem superpolynomial_decay_iff_zpow_tendsto_zero (hk : Tendsto k l atTop) :
  ...
  · have : tendsto (fun a => k a ^ z) l (𝓝 0) := tendsto.comp (tendsto_zpow_at_top_zero (not_le.1 hz)) hk
...
theorem SuperpolynomialDecay.inv_param_mul (hk : Tendsto k l atTop) (hf : SuperpolynomialDecay l k f) :
    SuperpolynomialDecay l k (k⁻¹ * f) := by simpa using hf.param_zpow_mul hk (-1)
