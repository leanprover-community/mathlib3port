...
theorem Filter.IsBoundedUnder.isOCat_sub_self_inv {𝕜 E : Type _} [NormedField 𝕜] [HasNorm E] {a : 𝕜}
    ...
    f =o[𝓝[≠] a] fun x => (x - a)⁻¹ :=
  by
  refine' (h.is_O_const (one_ne_zero' ℝ)).trans_isOCat (isOCat_const_left.2 <| Or.inr _)
  simp only [(· ∘ ·), norm_inv]
...
theorem Asymptotics.isOCat_pow_pow_atTop_of_lt [OrderTopology 𝕜] {p q : ℕ} (hpq : p < q) :
  ...
  by
  refine' (isOCat_iff_tendsto' _).mpr (tendsto_pow_div_pow_atTop_zero hpq)
...
theorem Asymptotics.IsOCat.sum_range {α : Type _} [NormedAddCommGroup α] {f : ℕ → α} {g : ℕ → ℝ}
  ...
  apply isOCat_iff.2 fun ε εpos => _
  obtain ⟨N, hN⟩ : ∃ N : ℕ, ∀ b : ℕ, N ≤ b → ‖f b‖ ≤ ε / 2 * g b := by
    simpa only [A, eventually_atTop] using is_o_iff.mp h (half_pos εpos)
  have : (fun n : ℕ => ∑ i in range N, f i) =o[atTop] fun n : ℕ => ∑ i in range n, g i :=
    by
    apply isOCat_const_left.2
    exact Or.inr (h'g.congr fun n => (B n).symm)
  filter_upwards [isOCat_iff.1 this (half_pos εpos), Ici_mem_atTop N]with n hn Nn
  calc
...
theorem Asymptotics.isOCat_sum_range_of_tendsto_zero {α : Type _} [NormedAddCommGroup α] {f : ℕ → α}
  ...
  by
  have := ((isOCat_one_iff ℝ).2 h).sum_range fun i => zero_le_one
...
theorem Filter.Tendsto.cesaro_smul {E : Type _} [NormedAddCommGroup E] [NormedSpace ℝ E] {u : ℕ → E}
  ...
  by
  rw [← tendsto_sub_nhds_zero_iff, ← isOCat_one_iff ℝ]
  have := Asymptotics.isOCat_sum_range_of_tendsto_zero (tendsto_sub_nhds_zero_iff.2 h)
  apply ((isO_refl (fun n : ℕ => (n : ℝ)⁻¹) atTop).smul_isOCat this).congr' _ _
