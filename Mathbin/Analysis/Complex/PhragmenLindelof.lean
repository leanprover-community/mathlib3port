...
theorem horizontal_strip (hfd : DiffContOnCl ℂ f (im ⁻¹' Ioo a b))
  ...
  rw [le_iff_eq_or_ltₓ] at hza hzb
  ...
  suffices ∀ C' : ℝ, 0 < C' → (∀ w : ℂ, im w = a → ∥f w∥ ≤ C') → (∀ w : ℂ, im w = b → ∥f w∥ ≤ C') → ∥f z∥ ≤ C' by
    refine' le_of_forall_le_of_denseₓ fun C' hC' => this C' _ (fun w hw => _) fun w hw => _
  ...
  obtain ⟨R, hzR, hR⟩ : ∃ R : ℝ, abs z.re < R ∧ ∀ w, abs (re w) = R → im w ∈ Ioo (a - b) (a + b) → ∥g ε w • f w∥ ≤ C :=
    ...
    · rw [mul_zero, add_zeroₓ] at H
      ...
      simpa only [id, (· ∘ ·), add_mulₓ, mul_assoc, ← div_eq_inv_mul, ← Real.exp_sub, ← sub_mul, sub_sub_cancel] using
