...
-- ././Mathport/Syntax/Translate/Basic.lean:978:9: unsupported derive handler λ α, has_coe α exprℂ()
...
instance sLOnGLPos : HasScalar SL(2,ℤ) GL(2, ℝ)⁺ :=
  ⟨fun s g => s * g⟩
...
instance subgroupGLPos : HasScalar Γ GL(2, ℝ)⁺ :=
  ⟨fun s g => s * g⟩

theorem subgroup_on_GL_pos_smul_apply (s : Γ) (g : GL(2, ℝ)⁺) (z : ℍ) : (s • g) • z = ((s : GL(2, ℝ)⁺) * g) • z :=
  rfl

instance subgroup_on_GL_pos : IsScalarTower Γ GL(2, ℝ)⁺ ℍ where
  smul_assoc := by
    intro s g z
    simp only [subgroup_on_GL_pos_smul_apply, coe_coe]
    apply mul_smul'

instance subgroupSL : HasScalar Γ SL(2,ℤ) :=
  ⟨fun s g => s * g⟩
...
theorem neg_smul (g : GL(2, ℝ)⁺) (z : ℍ) : -g • z = g • z := by
  ...
  change _ / _ = _ / _
  field_simp [denom_ne_zero, -denom, -Num]
  simp only [Num, denom, coe_coe, Complex.of_real_neg, neg_mul, GL_pos.coe_neg_GL, Units.coe_neg, Pi.neg_apply]
  ring_nf
