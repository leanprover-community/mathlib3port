...
theorem norm_cderiv_le (hr : 0 < r) (hf : ∀ w ∈ sphere z r, ‖f w‖ ≤ M) : ‖cderiv r f z‖ ≤ M / r :=
  ...
  field_simp [abs_of_nonneg real.pi_pos.le, real.pi_pos.ne.symm, hr.ne.symm]
  ring
...
theorem TendstoUniformlyOn.cderiv (hF : TendstoUniformlyOn F f φ (cthickening δ K)) (hδ : 0 < δ)
  ...
  have e3 := sphere_subset_closed_ball.trans (closedBall_subset_cthickening hz δ)
  have hf : ContinuousOn f (sphere z δ) :=
    e1.mono (sphere_subset_closed_ball.trans (closedBall_subset_cthickening hz δ))
...
theorem TendstoLocallyUniformlyOn.differentiableOn [φ.NeBot]
  ...
  obtain ⟨δ, hδ, -, h1⟩ := exists_cthickening_tendstoUniformlyOn hf hF hK hU hKU
