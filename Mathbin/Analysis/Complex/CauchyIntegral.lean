...
theorem integral_boundary_rect_eq_zero_of_differentiable_on_off_countable (f : ℂ → E) (z w : ℂ)
  ...
  by
  refine'
      (integral_boundary_rect_of_hasFderivAt_real_off_countable f
...
theorem circleIntegral_sub_center_inv_smul_of_differentiable_on_off_countable_of_tendsto {c : ℂ}
  ...
  calc
    ‖(∮ z in C(c, R), (z - c)⁻¹ • f z) - (2 * ↑π * i) • y‖ =
      ...
      ·
        exact
          circleIntegral_sub_center_inv_smul_eq_of_differentiable_on_annulus_off_countable hr0 hrR
    ...
    _ = ‖∮ z in C(c, r), (z - c)⁻¹ • (f z - y)‖ :=
      ...
      ·
        exact
          hc.mono
            (subset_inter (sphere_subset_closed_ball.trans <| closedBall_subset_closedBall hrR)
              hzne)
...
theorem circleIntegral_sub_inv_smul_of_differentiable_on_off_countable_aux {R : ℝ} {c w : ℂ}
  ...
  have hnhds : closedBall c R ∈ 𝓝 w := closedBall_mem_nhds_of_mem hw.1
  ...
  have hFeq : EqOn F (fun z => (z - w)⁻¹ • f z - (z - w)⁻¹ • f w) (sphere c R) :=
    by
    intro z hz
    calc
...
theorem two_pi_i_inv_smul_circleIntegral_sub_inv_smul_of_differentiable_on_off_countable {R : ℝ}
  ...
  suffices w ∈ closure (ball c R \ s) by
    ...
    have B : ContinuousAt f w := hc.continuous_at (closedBall_mem_nhds_of_mem hw)
...
theorem circleIntegral_sub_inv_smul_of_differentiable_on_off_countable {R : ℝ} {c w : ℂ} {f : ℂ → E}
  ...
  by
  rw [←
    two_pi_i_inv_smul_circleIntegral_sub_inv_smul_of_differentiable_on_off_countable hs hw hc hd,
    smul_inv_smul₀]
  simp [Real.pi_ne_zero, i_ne_zero]
...
theorem DiffContOnCl.two_pi_i_inv_smul_circleIntegral_sub_inv_smul {R : ℝ} {c w : ℂ} {f : ℂ → E}
  ...
  refine'
    two_pi_i_inv_smul_circleIntegral_sub_inv_smul_of_differentiable_on_off_countable countable_empty
      hw _ _
  · simpa only [closure_ball c hR.ne.symm] using hf.continuous_on
  · simpa only [diff_empty] using fun z hz => hf.differentiable_at isOpen_ball hz
...
theorem hasFpowerSeriesOnBallOfDifferentiableOffCountable {R : ℝ≥0} {c : ℂ} {f : ℂ → E} {s : Set ℂ}
  ...
  { r_le := le_radius_cauchyPowerSeries _ _ _
    ...
    HasSum := fun w hw =>
      ...
      rw [←
        two_pi_i_inv_smul_circleIntegral_sub_inv_smul_of_differentiable_on_off_countable hs hw' hc
          hd]
      exact
        (hasFpowerSeriesOnCauchyIntegral ((hc.mono sphere_subset_closedBall).circleIntegrable R.2)
              hR).hasSum
          hw }
