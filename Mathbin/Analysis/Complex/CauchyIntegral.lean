...
theorem integral_boundary_rect_of_has_fderiv_within_at_real_off_countable (f : ℂ → E) (f' : ℂ → ℂ →L[ℝ] E) (z w : ℂ)
  ...
  have htd :
    ∀,
      ∀ p ∈ (Ioo (min z.re w.re) (max z.re w.re)).Prod (Ioo (min w.im z.im) (max w.im z.im)) \ t,
        ∀, HasFderivAt F (F' p) p :=
...
theorem integral_boundary_rect_eq_zero_of_differentiable_on_off_countable (f : ℂ → E) (z w : ℂ) (s : Set ℂ)
  ...
  by
  refine'
      (integral_boundary_rect_of_has_fderiv_within_at_real_off_countable f (fun z => (fderiv ℂ f z).restrictScalars ℝ) z
            w s hs Hc (fun x hx => (Hd x hx).HasFderivAt.restrictScalars ℝ) _).trans
        _ <;>
...
protected theorem _root_.differentiable_on.has_fpower_series_on_ball {R : ℝ≥0 } {c : ℂ} {f : ℂ → E}
  ...
  has_fpower_series_on_ball_of_differentiable_off_countable countable_empty hd.continuous_on
    (fun z hz => hd.differentiable_at $ closed_ball_mem_nhds_of_mem hz.1) hR
...
protected theorem differentiable.analytic_at {f : ℂ → E} (hf : Differentiable ℂ f) (z : ℂ) : AnalyticAt ℂ f z :=
