...
theorem arith_mean_le_rpow_mean (w z : ι → ℝ) (hw : ∀ i _ : i ∈ s, 0 ≤ w i) (hw' : (∑i in s, w i) = 1)
  (hz : ∀ i _ : i ∈ s, 0 ≤ z i) {p : ℝ} (hp : 1 ≤ p) : (∑i in s, w i*z i) ≤ ((∑i in s, w i*z i^p)^1 / p) :=
  by 
    ...
    all_goals 
      applyRules [sum_nonneg, rpow_nonneg_of_nonneg]
      intro i hi 
      applyRules [mul_nonneg, rpow_nonneg_of_nonneg, hw i hi, hz i hi]
...
theorem rpow_arith_mean_le_arith_mean2_rpow (w₁ w₂ z₁ z₂ :  ℝ≥0 ) (hw' : (w₁+w₂) = 1) {p : ℝ} (hp : 1 ≤ p) :
  ...
  by 
    have h :=
      rpow_arith_mean_le_arith_mean_rpow (univ : Finset (Finₓ 2)) (Finₓ.cons w₁$ Finₓ.cons w₂ finZeroElim)
        (Finₓ.cons z₁$ Finₓ.cons z₂$ finZeroElim) _ hp
    ·
      simpa [Finₓ.sum_univ_succ, Finₓ.sum_univ_zero, Finₓ.cons_succ, Finₓ.cons_zero] using h
    ·
      simp [hw', Finₓ.sum_univ_succ, Finₓ.sum_univ_zero, Finₓ.cons_succ, Finₓ.cons_zero]
...
theorem rpow_arith_mean_le_arith_mean_rpow (w z : ι → ℝ≥0∞) (hw' : (∑i in s, w i) = 1) {p : ℝ} (hp : 1 ≤ p) :
  ...
  by 
    have hp_pos : 0 < p 
    exact lt_of_lt_of_leₓ zero_lt_one hp 
    have hp_nonneg : 0 ≤ p 
    exact le_of_ltₓ hp_pos 
    ...
    ·
      ...
      simpRw [to_nnreal_sum h_top_rpow, ←to_nnreal_rpow, to_nnreal_sum h_top, to_nnreal_mul, ←to_nnreal_rpow]
      refine' Nnreal.rpow_arith_mean_le_arith_mean_rpow s (fun i => (w i).toNnreal) (fun i => (z i).toNnreal) _ hp 
      have h_sum_nnreal : (∑i in s, w i) = «expr↑ » (∑i in s, (w i).toNnreal)
      ·
        rw [coe_finset_sum]
...
private theorem add_rpow_le_one_of_add_le_one {p : ℝ} (a b : ℝ≥0∞) (hab : (a+b) ≤ 1) (hp1 : 1 ≤ p) : ((a^p)+b^p) ≤ 1 :=
  by 
    have h_le_one : ∀ x : ℝ≥0∞, x ≤ 1 → (x^p) ≤ x 
    exact fun x hx => rpow_le_self_of_le_one hx hp1 
    have ha : a ≤ 1 
    exact (self_le_add_right a b).trans hab 
    have hb : b ≤ 1 
    exact (self_le_add_left b a).trans hab 
...
theorem add_rpow_le_rpow_add {p : ℝ} (a b : ℝ≥0∞) (hp1 : 1 ≤ p) : ((a^p)+b^p) ≤ ((a+b)^p) :=
  by 
    ...
    rwa [div_eq_mul_inv, div_eq_mul_inv, mul_addₓ, mul_commₓ (a^p), mul_commₓ (b^p), ←mul_assocₓ, ←mul_assocₓ,
      mul_inv_cancel hab_0 hab_top, one_mulₓ, one_mulₓ] at h_mul
...
-- error in Analysis.MeanInequalitiesPow: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
