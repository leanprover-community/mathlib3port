...
def of_map_split_add [Fintype ι] (f : box ι → M) (I₀ : WithTop (box ι))
  ...
  by 
    refine' ⟨f, _⟩
    replace hf : ∀ I : box ι, «expr↑ » I ≤ I₀ → ∀ s, (∑J in (split_many I s).boxes, f J) = f I
    ·
      intro I hI s 
...
-- error in Analysis.BoxIntegral.Partition.Additive: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
(hI : «expr ≤ »(«expr↑ »(I), I₀))
{π₁ π₂ : prepartition I}
(h : «expr = »(π₁.Union, π₂.Union)) : «expr = »(«expr∑ in , »((J), π₁.boxes, f J), «expr∑ in , »((J), π₂.boxes, f J)) :=
begin
  rcases [expr exists_split_many_inf_eq_filter_of_finite {π₁, π₂} ((finite_singleton _).insert _), "with", "⟨", ident s, ",", ident hs, "⟩"],
  simp [] [] ["only"] ["[", expr inf_split_many, "]"] [] ["at", ident hs],
  rcases ["⟨", expr hs _ (or.inl rfl), ",", expr hs _ (or.inr rfl), "⟩", "with", "⟨", ident h₁, ",", ident h₂, "⟩"],
  clear [ident hs],
  rw [expr h] ["at", ident h₁],
  calc
    «expr = »(«expr∑ in , »((J), π₁.boxes, f J), «expr∑ in , »((J), π₁.boxes, «expr∑ in , »((J'), (split_many J s).boxes, f J'))) : finset.sum_congr rfl (λ
     J hJ, (f.sum_partition_boxes _ (is_partition_split_many _ _)).symm)
    «expr = »(..., «expr∑ in , »((J), (π₁.bUnion (λ J, split_many J s)).boxes, f J)) : (sum_bUnion_boxes _ _ _).symm
    «expr = »(..., «expr∑ in , »((J), (π₂.bUnion (λ
        J, split_many J s)).boxes, f J)) : by rw ["[", expr h₁, ",", expr h₂, "]"] []
    «expr = »(..., «expr∑ in , »((J), π₂.boxes, «expr∑ in , »((J'), (split_many J s).boxes, f J'))) : sum_bUnion_boxes _ _ _
    «expr = »(..., «expr∑ in , »((J), π₂.boxes, f J)) : finset.sum_congr rfl (λ
     J hJ, f.sum_partition_boxes _ (is_partition_split_many _ _)),
  exacts ["[", expr «expr $ »(with_top.coe_le_coe.2, π₁.le_of_mem hJ).trans hI, ",", expr «expr $ »(with_top.coe_le_coe.2, π₂.le_of_mem hJ).trans hI, "]"]
end
