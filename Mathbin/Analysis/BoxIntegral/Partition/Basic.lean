...
theorem inj_on_set_of_mem_Icc_set_of_lower_eq (x : ι → ℝ) :
    inj_on (fun J : box ι => { i | J.lower i = x i }) { J | J ∈ π ∧ x ∈ J.Icc } := by
...
theorem bUnion_index_mem (hJ : J ∈ π.bUnion πi) : π.bUnion_index πi J ∈ π := by
  rw [bUnion_index, dif_pos hJ]
  exact (π.mem_bUnion.1 hJ).some_spec.fst

theorem bUnion_index_le (πi : ∀ J, prepartition J) (J : box ι) : π.bUnion_index πi J ≤ I := by
...
theorem bUnion_assoc (πi : ∀ J, prepartition J) (πi' : box ι → ∀ J : box ι, prepartition J) :
    (π.bUnion fun J => (πi J).bUnion (πi' J)) = (π.bUnion πi).bUnion fun J => πi' (π.bUnion_index πi J) J := by
  ext J
...
theorem Union_of_with_bot (boxes : Finset (WithBot (box ι))) (le_of_mem : ∀, ∀ J ∈ boxes, ∀, (J : WithBot (box ι)) ≤ I)
    ...
    (of_with_bot boxes le_of_mem pairwise_disjoint).Union = ⋃ J ∈ boxes, ↑J := by
...
theorem of_with_bot_le {boxes : Finset (WithBot (box ι))} {le_of_mem : ∀, ∀ J ∈ boxes, ∀, (J : WithBot (box ι)) ≤ I}
    {pairwise_disjoint : Set.Pairwise (boxes : Set (WithBot (box ι))) Disjoint}
    (H : ∀, ∀ J ∈ boxes, ∀, J ≠ ⊥ → ∃ J' ∈ π, J ≤ ↑J') : of_with_bot boxes le_of_mem pairwise_disjoint ≤ π := by
...
theorem le_of_with_bot {boxes : Finset (WithBot (box ι))} {le_of_mem : ∀, ∀ J ∈ boxes, ∀, (J : WithBot (box ι)) ≤ I}
    {pairwise_disjoint : Set.Pairwise (boxes : Set (WithBot (box ι))) Disjoint}
    (H : ∀, ∀ J ∈ π, ∀, ∃ J' ∈ boxes, ↑J ≤ J') : π ≤ of_with_bot boxes le_of_mem pairwise_disjoint := by
  intro J hJ
...
@[simp]
theorem is_partition_single_iff (h : J ≤ I) : is_partition (single I J h) ↔ J = I := by
  simp [is_partition_iff_Union_eq]

theorem is_partition_top (I : box ι) : is_partition (⊤ : prepartition I) := fun x hx => ⟨I, mem_top.2 rfl, hx⟩
...
protected theorem ExistsUnique (h : π.is_partition) (hx : x ∈ I) : ∃! J ∈ π, x ∈ J := by
