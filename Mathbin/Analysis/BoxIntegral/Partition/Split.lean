...
@[simp]
theorem coe_split_lower : (split_lower I i x : Set (ι → ℝ)) = I ∩ { y | y i ≤ x } := by
  rw [split_lower, coe_mk']
  ext y
  simp only [mem_univ_pi, mem_Ioc, mem_inter_eq, mem_coe, mem_set_of_eq, forall_and_distrib, ← Pi.le_def, le_update_iff,
...
@[simp]
theorem split_lower_eq_bot {i x} : I.split_lower i x = ⊥ ↔ x ≤ I.lower i := by
  rw [split_lower, mk'_eq_bot, exists_update_iff I.upper fun j y => y ≤ I.lower j]
  simp [(I.lower_lt_upper _).not_le]

@[simp]
theorem split_lower_eq_self : I.split_lower i x = I ↔ I.upper i ≤ x := by
...
theorem split_lower_def [DecidableEq ι] {i x} (h : x ∈ Ioo (I.lower i) (I.upper i))
    ...
    I.split_lower i x = (⟨I.lower, update I.upper i x, h'⟩ : box ι) := by
...
@[simp]
theorem coe_split_upper : (split_upper I i x : Set (ι → ℝ)) = I ∩ { y | x < y i } := by
...
@[simp]
theorem split_upper_eq_bot {i x} : I.split_upper i x = ⊥ ↔ I.upper i ≤ x := by
  rw [split_upper, mk'_eq_bot, exists_update_iff I.lower fun j y => I.upper j ≤ y]
  simp [(I.lower_lt_upper _).not_le]

@[simp]
theorem split_upper_eq_self : I.split_upper i x = I ↔ x ≤ I.lower i := by
...
@[simp]
theorem mem_split_iff : J ∈ split I i x ↔ ↑J = I.split_lower i x ∨ ↑J = I.split_upper i x := by
  simp [split]
...
theorem sum_split_boxes {M : Type _} [AddCommMonoidₓ M] (I : box ι) (i : ι) (x : ℝ) (f : box ι → M) :
    (∑ J in (split I i x).boxes, f J) = (I.split_lower i x).elim 0 f + (I.split_upper i x).elim 0 f := by
...
theorem split_many_insert (I : box ι) (s : Finset (ι × ℝ)) (p : ι × ℝ) :
    split_many I (insert p s) = split_many I s⊓split I p.1 p.2 := by
...
theorem inf_split_many {I : box ι} (π : prepartition I) (s : Finset (ι × ℝ)) :
    π⊓split_many I s = π.bUnion fun J => split_many J s := by
...
theorem eventually_not_disjoint_imp_le_of_mem_split_many (s : Finset (box ι)) :
  ...
  refine'
    eventually_at_top.2
      ⟨s.bUnion fun J => finset.univ.bUnion fun i => {(i, J.lower i), (i, J.upper i)}, fun t ht I J hJ J' hJ' =>
...
theorem eventually_split_many_inf_eq_filter (π : prepartition I) :
    ∀ᶠ t : Finset (ι × ℝ) in at_top, π⊓split_many I t = (split_many I t).filter fun J => ↑J ⊆ π.Union := by
...
theorem exists_split_many_inf_eq_filter_of_finite (s : Set (prepartition I)) (hs : s.finite) :
    ∃ t : Finset (ι × ℝ), ∀, ∀ π ∈ s, ∀, π⊓split_many I t = (split_many I t).filter fun J => ↑J ⊆ π.Union :=
...
theorem is_partition.compl_eq_bot {π : prepartition I} (h : is_partition π) : π.compl = ⊥ := by
