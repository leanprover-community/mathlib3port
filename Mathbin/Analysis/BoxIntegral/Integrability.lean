...
theorem has_integral_indicator_const (l : integration_params) (hl : l.bRiemann = ff) {s : Set (ι → ℝ)}
  ...
  by 
    ...
    choose! rs hrsU 
    have  : ∀ x _ : x ∈ I.Icc \ s, ∃ r : Ioi (0 : ℝ), closed_ball x r ⊆ «expr ᶜ» F 
    exact
      fun x hx =>
...
theorem has_integral_zero_of_ae_eq_zero {l : integration_params} {I : box ι} {f : (ι → ℝ) → E} {μ : Measureₓ (ι → ℝ)}
  ...
  by 
    ...
    rintro n -
    dsimp [integral_sum]
    have  : ∀ J _ : J ∈ π.filter fun J => N (π.tag J) = n, ∥(μ («expr↑ » J)).toReal • f (π.tag J)∥ ≤ (μ J).toReal*n
    ·
      intro J hJ 
...
-- error in Analysis.BoxIntegral.Integrability: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
theorem integrable_on.has_box_integral [second_countable_topology E] [MeasurableSpace E] [BorelSpace E]
  ...
  by 
    ...
    ·
      ...
      have hfi : ∀ n J _ : J ∈ π, integrable_on (f n) («expr↑ » J) μ 
      exact fun n J hJ => (hfi n).mono_set (π.le_of_mem' J hJ)
      have hgi : ∀ J _ : J ∈ π, integrable_on g («expr↑ » J) μ 
