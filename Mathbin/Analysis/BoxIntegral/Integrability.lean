...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (F «expr ⊆ » «expr ∩ »(s, I.Icc))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (U «expr ⊇ » «expr ∩ »(s, I.Icc))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » «expr ∩ »(s, I.Icc))
...
theorem has_integral_indicator_const (l : integration_params) (hl : l.bRiemann = ff) {s : Set (ι → ℝ)}
  ...
  has_integral.{u, v, v} I l (s.indicator fun _ => y) μ.to_box_additive.to_smul ((μ (s ∩ I)).toReal • y) :=
  by 
    ...
    have  : ∀ x _ : x ∈ s ∩ I.Icc, ∃ r : Ioi (0 : ℝ), closed_ball x r ⊆ U 
    exact fun x hx => Subtype.exists'.1 (nhds_basis_closed_ball.mem_iff.1 (hUo.mem_nhds$ hsU hx))
    choose! rs hrsU 
    have  : ∀ x _ : x ∈ I.Icc \ s, ∃ r : Ioi (0 : ℝ), closed_ball x r ⊆ Fᶜ
    exact
      fun x hx =>
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (U «expr ⊇ » «expr ⁻¹' »(N, {n}))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (J «expr ∈ » π.filter (λ J, «expr = »(N (π.tag J), n)))
    ...
    :=
      by
        refine' has_integral_iff . 2 fun ε ε0 => _
          ...
          rintro n -
          dsimp [ integral_sum ]
          have : ∀ J _ : J ∈ π.filter fun J => N π.tag J = n , ∥ μ ↑ J . toReal • f π.tag J ∥ ≤ μ J . toReal * n
          ·
            intro J hJ
...
theorem has_integral.congr_ae {l : integration_params} {I : box ι} {y : E} {f g : (ι → ℝ) → E} {μ : Measureₓ (ι → ℝ)}
  ...
  (hfg : f =ᵐ[μ.restrict I] g) (hl : l.bRiemann = ff) : has_integral.{u, v, v} I l g μ.to_box_additive.to_smul y :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (J «expr ∈ » π.filter (λ J, «expr = »(Nx (π.tag J), n)))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (J «expr ∈ » π.filter (λ J, «expr = »(Nx (π.tag J), n)))
...
theorem integrable_on.has_box_integral [second_countable_topology E] [MeasurableSpace E] [BorelSpace E]
  ...
  by 
    ...
    ·
      ...
      have hNxn : ∀ J _ : J ∈ π.filter fun J => Nx (π.tag J) = n, Nx (π.tag J) = n 
      exact fun J hJ => (π.mem_filter.1 hJ).2
      have hrn : ∀ J _ : J ∈ π.filter fun J => Nx (π.tag J) = n, r c (π.tag J) = (hfi' n).convergenceR (δ n) c (π.tag J)
      ·
        intro J hJ 
        have  := hNxn J hJ 
        clear hJ 
    ...
    ·
      ...
      have hfi : ∀ n J _ : J ∈ π, integrable_on (f n) (↑J) μ 
      exact fun n J hJ => (hfi n).mono_set (π.le_of_mem' J hJ)
      have hgi : ∀ J _ : J ∈ π, integrable_on g (↑J) μ 
      exact fun J hJ => hgi.mono_set (π.le_of_mem' J hJ)
      have hfgi : ∀ n J _ : J ∈ π, integrable_on (fun x => ∥f n x - g x∥) J μ 
