...
nonnegative. -/
theorem integral_nonneg {g : ℝⁿ → ℝ} (hg : ∀ x ∈ I.icc, 0 ≤ g x) (μ : Measure ℝⁿ) [IsLocallyFiniteMeasure μ] :
...
theorem norm_integral_le_of_le_const {c : ℝ} (hc : ∀ x ∈ I.icc, ∥f x∥ ≤ c) (μ : Measure ℝⁿ) [IsLocallyFiniteMeasure μ] :
...
theorem integrableOfContinuousOn [CompleteSpace E] {I : Box ι} {f : ℝⁿ → E} (hc : ContinuousOn f I.icc) (μ : Measure ℝⁿ)
    [IsLocallyFiniteMeasure μ] : Integrable.{u, v, v} I l f μ.toBoxAdditive.toSmul := by
...
theorem hasIntegralOfLeHenstockOfForallIsO (hl : l ≤ Henstock) (B : ι →ᵇᵃ[I] ℝ) (hB0 : ∀ J, 0 ≤ B J) (g : ι →ᵇᵃ[I] F)
  ...
  have A : l.bHenstock := hl.2.1.resolve_left dec_trivial
  hasIntegralOfBRiemannEqFfOfForallIsO (hl.1.resolve_right dec_trivial) B hB0 _ s hs (fun _ => A) H₁ $ by
