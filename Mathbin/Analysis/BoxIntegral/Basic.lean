...
theorem integralSum_bUnion_partition (f : ℝⁿ → E) (vol : ι →ᵇᵃ E →L[ℝ] F) (π : TaggedPrepartition I)
  ...
  calc
    (∑ J' in (πi J).boxes, vol J' (f (π.tag <| π.to_prepartition.bUnion_index πi J'))) =
      ...
      sum_congr rfl fun J' hJ' => by rw [Prepartition.bUnionIndex_of_mem _ hJ hJ']
    _ = vol J (f (π.tag J)) :=
...
theorem integralSum_inf_partition (f : ℝⁿ → E) (vol : ι →ᵇᵃ E →L[ℝ] F) (π : TaggedPrepartition I)
  ...
  integralSum_bUnion_partition f vol π _ fun J hJ => h.restrict (Prepartition.le_of_mem _ Mem)
    ...
    integralSum f vol π₁ - integralSum f vol π₂ =
      ∑ J in (π₁.toPrepartition ⊓ π₂.toPrepartition).boxes,
        vol J (f <| (π₁.infPrepartition π₂.toPrepartition).tag J) -
          vol J (f <| (π₂.infPrepartition π₁.toPrepartition).tag J) :=
  by
  rw [← integralSum_inf_partition f vol π₁ h₂, ← integralSum_inf_partition f vol π₂ h₁, integralSum,
    integralSum, Finset.sum_sub_distrib]
...
theorem hasIntegral_iff :
  ...
  ((l.hasBasis_toFilterUnion_top I).tendsto_iff nhds_basis_closedBall).trans <| by
...
theorem integral_neg : integral I l (-f) vol = -integral I l f vol :=
  ...
  else by rw [integral, integral, dif_neg h, dif_neg (mt Integrable.ofNeg h), neg_zero]
...
theorem hasIntegralZero : HasIntegral I l (fun _ => (0 : E)) vol 0 := by
  simpa only [← (vol I).map_zero] using hasIntegralConst (0 : E)
...
theorem hasIntegralSum {α : Type _} {s : Finset α} {f : α → ℝⁿ → E} {g : α → F}
  ...
  by
  induction' s using Finset.induction_on with a s ha ihs; · simp [hasIntegralZero]
...
theorem integral_smul (c : ℝ) : integral I l (fun x => c • f x) vol = c • integral I l f vol :=
  ...
  · have : ¬Integrable I l (fun x => c • f x) vol := mt (fun h => h.ofSmul hc) hf
...
theorem norm_integral_le_of_le_const {c : ℝ} (hc : ∀ x ∈ I.icc, ‖f x‖ ≤ c) (μ : Measure ℝⁿ)
  ...
  by simpa only [integral_const] using norm_integral_le_of_norm_le hc μ (integrableConst c)
...
theorem convergenceR_cond (h : Integrable I l f vol) (ε : ℝ) (c : ℝ≥0) :
  ...
  exacts[(hasIntegral_iff.1 h.has_integral ε h₀).choose_spec.1 _, fun _ x => rfl]
...
theorem dist_integralSum_integral_le_of_memBaseSet (h : Integrable I l f vol) (h₀ : 0 < ε)
  ...
  exact (hasIntegral_iff.1 h.has_integral ε h₀).choose_spec.2 c _ hπ hπp
...
theorem dist_integralSum_le_of_memBaseSet (h : Integrable I l f vol) (hpos₁ : 0 < ε₁)
  ...
  have H₁ :
    ...
    h.dist_integral_sum_integral_le_of_mem_base_set hpos₁
      ...
      (isPartitionUnionComplToSubordinate _ _ _ _)
  rw [HU] at hπU
  have H₂ :
    dist (integralSum f vol (π₂.union_compl_to_subordinate π hπU r)) (integral I l f vol) ≤ ε₂ :=
    h.dist_integral_sum_integral_le_of_mem_base_set hpos₂
...
theorem cauchy_map_integralSum_toFilterUnion (h : Integrable I l f vol) (π₀ : Prepartition I) :
  ...
  rw [prod_map_map_eq, ← toFilter_inf_union_eq, ← prod_inf_prod, prod_principal_principal]
  exact
...
theorem dist_integralSum_sum_integral_le_of_memBaseSet_of_union_eq (h : Integrable I l f vol)
  ...
  have : l.mem_base_set I C (h.convergence_r δ' C) (π₀.bUnion_tagged πi) :=
    bUnionTaggedMemBaseSet hπiC hπip fun _ => le_max_right _ _
  ...
  rw [integralSum_bUnionTagged] at this
  calc
...
theorem tendsto_integralSum_sum_integral (h : Integrable I l f vol) (π₀ : Prepartition I) :
  ...
  by
  refine' ((l.has_basis_to_filter_Union I π₀).tendsto_iff nhds_basis_closedBall).2 fun ε ε0 => _
  refine' ⟨h.convergence_r ε, h.convergence_r_cond ε, _⟩
  simp only [mem_inter_iff, Set.mem_unionᵢ, mem_setOf_eq]
  rintro π ⟨c, hc, hU⟩
...
theorem integrableOfContinuousOn [CompleteSpace E] {I : Box ι} {f : ℝⁿ → E}
  ...
  have :
    ...
    · exact Prepartition.bUnionIndex_mem _ hJ
    · exact Box.le_iff_icc.1 (Prepartition.le_bUnionIndex _ hJ) J.upper_mem_Icc
    · rw [inf_comm] at hJ
      exact Prepartition.bUnionIndex_mem _ hJ
    · rw [inf_comm] at hJ
      exact Box.le_iff_icc.1 (Prepartition.le_bUnionIndex _ hJ) J.upper_mem_Icc
...
theorem hasIntegralOfBRiemannEqFfOfForallIsO (hl : l.bRiemann = false) (B : ι →ᵇᵃ[I] ℝ)
  ...
  simp only [Set.mem_unionᵢ, mem_inter_iff, mem_setOf_eq]
  rintro π ⟨c, hπδ, hπp⟩
