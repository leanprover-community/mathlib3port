...
theorem to_circle_add (x : AddCircle T) (y : AddCircle T) : toCircle (x + y) = toCircle x * toCircle y := by
  induction x using QuotientAddGroup.induction_on'
  induction y using QuotientAddGroup.induction_on'
  simp_rw [← QuotientAddGroup.coe_add, to_circle, Function.Periodic.lift_coe, mul_add, exp_map_circle_add]
...
theorem injective_to_circle (hT : T ≠ 0) : Function.Injective (@toCircle T) := by
  intro a b h
...
theorem volume_eq_smul_haar_add_circle : (volume : Measure (AddCircle T)) = Ennreal.ofReal T • haar_add_circle :=
  rfl
...
theorem fourier_zero {x : AddCircle T} : fourier 0 x = 1 := by
  induction x using QuotientAddGroup.induction_on'
  rw [fourier_apply, to_circle, zero_zsmul, ← QuotientAddGroup.coe_zero, Function.Periodic.lift_coe, mul_zero,
    exp_map_circle_zero, coe_one_unit_sphere]
...
theorem fourier_neg {n : ℤ} {x : AddCircle T} : fourier (-n) x = conj (fourier n x) := by
  induction x using QuotientAddGroup.induction_on'
  simp_rw [fourier_apply, to_circle, ← QuotientAddGroup.coe_zsmul, Function.Periodic.lift_coe, ← coe_inv_circle_eq_conj,
    ← exp_map_circle_neg, neg_smul, mul_neg]
...
theorem fourier_subalgebra_coe : (@fourierSubalgebra T).toSubmodule = span ℂ (range fourier) := by
...
complex conjugation. -/
theorem fourierSubalgebraConjInvariant : ConjInvariantSubalgebra ((@fourierSubalgebra T).restrictScalars ℝ) := by
...
/-- The subalgebra of `C(add_circle T, ℂ)` generated by `fourier n` for `n ∈ ℤ`
separates points. -/
theorem fourier_subalgebra_separates_points : (@fourierSubalgebra T).SeparatesPoints := by
  intro x y hxy
...
theorem fourier_subalgebra_closure_eq_top : (@fourierSubalgebra T).topologicalClosure = ⊤ :=
  ContinuousMap.subalgebra_is_R_or_C_topological_closure_eq_top_of_separates_points fourierSubalgebra
    fourier_subalgebra_separates_points fourierSubalgebraConjInvariant
...
theorem coe_fn_fourier_Lp (p : ℝ≥0∞) [Fact (1 ≤ p)] (n : ℤ) : @fourierLp T hT p _ n =ᵐ[haar_add_circle] fourier n :=
...
theorem span_fourier_Lp_closure_eq_top {p : ℝ≥0∞} [Fact (1 ≤ p)] (hp : p ≠ ∞) :
  ...
  convert
    (ContinuousMap.to_Lp_dense_range ℂ hp (@haar_add_circle T hT) ℂ).topological_closure_map_submodule
...
theorem orthonormalFourier : Orthonormal ℂ (@fourierLp T _ 2 _) := by
  ...
  · simp_rw [h, neg_add_self]
    have : ⇑(@fourier T 0) = (fun x => 1 : AddCircle T → ℂ) := by
      ext1
      exact fourier_zero
...
theorem fourier_series_repr (f : lp ℂ 2 <| @haarAddCircle T hT) (i : ℤ) :
    fourierSeries.repr f i = ∫ t : AddCircle T, fourier (-i) t * f t ∂haar_add_circle := by
...
theorem tsum_sq_fourier_series_repr (f : lp ℂ 2 <| @haarAddCircle T hT) :
    (∑' i : ℤ, ‖fourierSeries.repr f i‖ ^ 2) = ∫ t : AddCircle T, ‖f t‖ ^ 2 ∂haar_add_circle := by
...
theorem fourier_series_repr' (f : lp ℂ 2 <| @haarAddCircle T hT) (n : ℤ) (a : ℝ) :
    fourierSeries.repr f n = 1 / T * ∫ x in a..a + T, @fourier T (-n) x * f x := by
  ...
  rw [fourier_series_repr, AddCircle.interval_integral_preimage T a (ha.smul_measure _), volume_eq_smul_haar_add_circle,
