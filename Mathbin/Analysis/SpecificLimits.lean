...
theorem tendsto_norm_zero' {𝕜 : Type _} [NormedGroup 𝕜] : tendsto (norm : 𝕜 → ℝ) (𝓝[{0}ᶜ] 0) (𝓝[Set.Ioi 0] 0) :=
...
theorem tendsto_norm_inverse_nhds_within_0_at_top {𝕜 : Type _} [NormedField 𝕜] :
  tendsto (fun x : 𝕜 => ∥x⁻¹∥) (𝓝[{0}ᶜ] 0) at_top :=
...
theorem tendsto_norm_zpow_nhds_within_0_at_top {𝕜 : Type _} [NormedField 𝕜] {m : ℤ} (hm : m < 0) :
  tendsto (fun x : 𝕜 => ∥x ^ m∥) (𝓝[{0}ᶜ] 0) at_top :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » Ioo «expr- »(R) R)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » Ioo 0 R)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » Ioo «expr- »(R) R)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » Ioo 0 R)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr < » R)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » Ioo 0 R)
...
theorem tfae_exists_lt_is_o_pow (f : ℕ → ℝ) (R : ℝ) :
  tfae
    [∃ (a : _)(_ : a ∈ Ioo (-R) R), is_o f (pow a) at_top, ∃ (a : _)(_ : a ∈ Ioo 0 R), is_o f (pow a) at_top,
      ...
      ∃ (a : _)(_ : a < R), ∀ᶠ n in at_top, |f n| ≤ a ^ n, ∃ (a : _)(_ : a ∈ Ioo 0 R), ∀ᶠ n in at_top, |f n| ≤ a ^ n] :=
  by 
...
theorem summable_of_ratio_norm_eventually_le {α : Type _} [SemiNormedGroup α] [CompleteSpace α] {f : ℕ → α} {r : ℝ}
  (hr₁ : r < 1) (h : ∀ᶠ n in at_top, ∥f (n+1)∥ ≤ r*∥f n∥) : Summable f :=
  by 
    byCases' hr₀ : 0 ≤ r
    ·
...
theorem summable_of_ratio_test_tendsto_lt_one {α : Type _} [NormedGroup α] [CompleteSpace α] {f : ℕ → α} {l : ℝ}
  (hl₁ : l < 1) (hf : ∀ᶠ n in at_top, f n ≠ 0) (h : tendsto (fun n => ∥f (n+1)∥ / ∥f n∥) at_top (𝓝 l)) : Summable f :=
  by 
...
theorem not_summable_of_ratio_norm_eventually_ge {α : Type _} [SemiNormedGroup α] {f : ℕ → α} {r : ℝ} (hr : 1 < r)
  (hf : ∃ᶠ n in at_top, ∥f n∥ ≠ 0) (h : ∀ᶠ n in at_top, (r*∥f n∥) ≤ ∥f (n+1)∥) : ¬Summable f :=
  by 
    rw [eventually_at_top] at h 
    rcases h with ⟨N₀, hN₀⟩
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (n «expr ≥ » «expr⌊ ⌋₊»(«expr∥ ∥»(x)))
...
theorem Real.summable_pow_div_factorial (x : ℝ) : Summable (fun n => x ^ n / n ! : ℕ → ℝ) :=
  by 
    ...
    suffices  : ∀ n _ : n ≥ ⌊∥x∥⌋₊, ∥(x ^ n+1) / (n+1)!∥ ≤ (∥x∥ / ⌊∥x∥⌋₊+1)*∥x ^ n / ↑n !∥
