...
theorem AntitoneOn.integral_le_sum (hf : AntitoneOn f (IccCat x₀ (x₀ + a))) :
...
theorem AntitoneOn.integral_le_sum_Ico (hab : a ≤ b) (hf : AntitoneOn f (Set.IccCat a b)) :
...
theorem AntitoneOn.sum_le_integral (hf : AntitoneOn f (IccCat x₀ (x₀ + a))) :
...
theorem AntitoneOn.sum_le_integral_Ico (hab : a ≤ b) (hf : AntitoneOn f (Set.IccCat a b)) :
...
theorem MonotoneOn.sum_le_integral (hf : MonotoneOn f (IccCat x₀ (x₀ + a))) :
...
theorem MonotoneOn.sum_le_integral_Ico (hab : a ≤ b) (hf : MonotoneOn f (Set.IccCat a b)) :
    (∑ x in Finset.ico a b, f x) ≤ ∫ x in a..b, f x := by
  rw [← neg_le_neg_iff, ← Finset.sum_neg_distrib, ← intervalIntegral.integral_neg]
  exact hf.neg.integral_le_sum_Ico hab

theorem MonotoneOn.integral_le_sum (hf : MonotoneOn f (IccCat x₀ (x₀ + a))) :
    (∫ x in x₀..x₀ + a, f x) ≤ ∑ i in Finset.range a, f (x₀ + (i + 1 : ℕ)) := by
  rw [← neg_le_neg_iff, ← Finset.sum_neg_distrib, ← intervalIntegral.integral_neg]
  exact hf.neg.sum_le_integral

theorem MonotoneOn.integral_le_sum_Ico (hab : a ≤ b) (hf : MonotoneOn f (Set.IccCat a b)) :
