...
theorem AntitoneOnₓ.integral_le_sum (hf : AntitoneOnₓ f (Icc x₀ (x₀ + a))) :
    (∫ x in x₀..x₀ + a, f x) ≤ ∑ i in Finsetₓ.range a, f (x₀ + i) := by
  ...
  calc
    (∫ x in x₀..x₀ + a, f x) = ∑ i in Finsetₓ.range a, ∫ x in x₀ + i..x₀ + (i + 1 : ℕ), f x := by
      convert (intervalIntegral.sum_integral_adjacent_intervals hint).symm
      simp only [Nat.cast_zeroₓ, add_zeroₓ]
    _ ≤ ∑ i in Finsetₓ.range a, ∫ x in x₀ + i..x₀ + (i + 1 : ℕ), f (x₀ + i) := by
      apply Finsetₓ.sum_le_sum fun i hi => _
      have ia : i < a := Finsetₓ.mem_range.1 hi
      ...
      · refine' mem_Icc.2 ⟨le_transₓ (by simp) hx.1, le_transₓ hx.2 _⟩
        simp only [add_le_add_iff_left, Nat.cast_le, Nat.succ_le_of_ltₓ ia]
        
    _ = ∑ i in Finsetₓ.range a, f (x₀ + i) := by simp
    

theorem AntitoneOnₓ.integral_le_sum_Ico (hab : a ≤ b) (hf : AntitoneOnₓ f (Set.Icc a b)) :
    (∫ x in a..b, f x) ≤ ∑ x in Finsetₓ.ico a b, f x := by
  ...
  congr
  rw [← zero_addₓ a]
  rw [← Finsetₓ.sum_Ico_add, Nat.Ico_zero_eq_range]
  conv =>
  rhs
  congr
  skip
  ext
  rw [Nat.cast_addₓ]
  apply AntitoneOnₓ.integral_le_sum
  simp only [hf, hab, Nat.cast_sub, add_sub_cancel'_right]

theorem AntitoneOnₓ.sum_le_integral (hf : AntitoneOnₓ f (Icc x₀ (x₀ + a))) :
    (∑ i in Finsetₓ.range a, f (x₀ + (i + 1 : ℕ))) ≤ ∫ x in x₀..x₀ + a, f x := by
  have hint : ∀ k : ℕ, k < a → IntervalIntegrable f volume (x₀ + k) (x₀ + (k + 1 : ℕ)) := by
    intro k hk
    refine' (hf.mono _).IntervalIntegrable
    rw [interval_of_le]
  ...
  calc
    (∑ i in Finsetₓ.range a, f (x₀ + (i + 1 : ℕ))) =
        ∑ i in Finsetₓ.range a, ∫ x in x₀ + i..x₀ + (i + 1 : ℕ), f (x₀ + (i + 1 : ℕ)) :=
      by simp
    _ ≤ ∑ i in Finsetₓ.range a, ∫ x in x₀ + i..x₀ + (i + 1 : ℕ), f x := by
      apply Finsetₓ.sum_le_sum fun i hi => _
      have ia : i + 1 ≤ a := Finsetₓ.mem_range.1 hi
...
theorem AntitoneOnₓ.sum_le_integral_Ico (hab : a ≤ b) (hf : AntitoneOnₓ f (Set.Icc a b)) :
    (∑ i in Finsetₓ.ico a b, f (i + 1 : ℕ)) ≤ ∫ x in a..b, f x := by
  rw [(Nat.sub_add_cancelₓ hab).symm, Nat.cast_addₓ]
  conv =>
  congr
  congr
  ...
  skip
  ext
  rw [add_assocₓ, Nat.cast_addₓ]
  apply AntitoneOnₓ.sum_le_integral
  simp only [hf, hab, Nat.cast_sub, add_sub_cancel'_right]

theorem MonotoneOnₓ.sum_le_integral (hf : MonotoneOnₓ f (Icc x₀ (x₀ + a))) :
    (∑ i in Finsetₓ.range a, f (x₀ + i)) ≤ ∫ x in x₀..x₀ + a, f x := by
  rw [← neg_le_neg_iff, ← Finsetₓ.sum_neg_distrib, ← intervalIntegral.integral_neg]
  exact hf.neg.integral_le_sum

theorem MonotoneOnₓ.sum_le_integral_Ico (hab : a ≤ b) (hf : MonotoneOnₓ f (Set.Icc a b)) :
    (∑ x in Finsetₓ.ico a b, f x) ≤ ∫ x in a..b, f x := by
  rw [← neg_le_neg_iff, ← Finsetₓ.sum_neg_distrib, ← intervalIntegral.integral_neg]
  exact hf.neg.integral_le_sum_Ico hab

theorem MonotoneOnₓ.integral_le_sum (hf : MonotoneOnₓ f (Icc x₀ (x₀ + a))) :
    (∫ x in x₀..x₀ + a, f x) ≤ ∑ i in Finsetₓ.range a, f (x₀ + (i + 1 : ℕ)) := by
  rw [← neg_le_neg_iff, ← Finsetₓ.sum_neg_distrib, ← intervalIntegral.integral_neg]
  exact hf.neg.sum_le_integral

theorem MonotoneOnₓ.integral_le_sum_Ico (hab : a ≤ b) (hf : MonotoneOnₓ f (Set.Icc a b)) :
    (∫ x in a..b, f x) ≤ ∑ i in Finsetₓ.ico a b, f (i + 1 : ℕ) := by
  rw [← neg_le_neg_iff, ← Finsetₓ.sum_neg_distrib, ← intervalIntegral.integral_neg]
