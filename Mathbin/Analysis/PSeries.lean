...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (k «expr ∈ » Ico «expr ^ »(2, n) «expr ^ »(2, «expr + »(n, 1)))
theorem le_sum_condensed' (hf : ∀ ⦃m n⦄, 0 < m → m ≤ n → f n ≤ f m) (n : ℕ) :
  ...
  by 
    ...
    have  : ∀ k _ : k ∈ Ico (2^n) (2^n+1), f k ≤ f (2^n) := fun k hk => hf (pow_pos zero_lt_two _) (mem_Ico.mp hk).1
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (k «expr ∈ » Ico «expr + »(«expr ^ »(2, n), 1) «expr + »(«expr ^ »(2, «expr + »(n, 1)), 1))
  ...
  by 
    ...
    ·
      ...
      exacts[add_le_add_right n.one_le_two_pow _,
        add_le_add_right (Nat.pow_le_pow_of_le_rightₓ zero_lt_two n.le_succ) _]
    have  : ∀ k _ : k ∈ Ico ((2^n)+1) ((2^n+1)+1), f (2^n+1) ≤ f k :=
      fun k hk =>
