...
instance category_of_differential_objects : category (differential_object C) :=
  { Hom := hom, id := hom.id, comp := fun X Y Z f g => hom.comp f g }
        ...
        { x := F.obj X.X, d := F.map X.d ≫ η.app X.X,
          d_squared' :=
            by 
              ...
              sliceLHS 2 3 => rw [←η.naturality X.d]
              rw [functor.comp_map]
              sliceLHS 1 2 => rw [←F.map_comp, X.d_squared, hF]
      ...
      fun X Y f =>
        { f := F.map f.f,
          comm' :=
            by 
              dsimp 
              sliceLHS 2 3 => rw [←functor.comp_map F (shift D).Functor, ←η.naturality f.f]
              sliceLHS 1 2 => rw [functor.comp_map, ←F.map_comp, f.comm, F.map_comp]
              rw [category.assoc] },
    map_id' :=
      by 
        intros 
    ...
    { x := X.X⟦-1⟧, d := X.d⟦-1⟧' ≫ (shift C).unitInv.app X.X ≫ (shift C).counitInv.app X.X,
      d_squared' :=
        by 
          dsimp 
          rw [functor.map_comp]
          sliceLHS 3 4 => erw [←(shift C).counitInv.naturality]
          sliceLHS 2 3 => erw [←(shift C).unitInv.naturality]
          sliceLHS 1 2 => erw [←functor.map_comp, X.d_squared]
          simp  }
        ...
        { f := f.f⟦-1⟧',
          comm' :=
            by 
              ...
              sliceLHS 3 4 => erw [←(shift C).counitInv.naturality]
              sliceLHS 2 3 => erw [←(shift C).unitInv.naturality]
              sliceLHS 1 2 => erw [←functor.map_comp, f.comm, functor.map_comp]
      ...
      fun X =>
        { f := (shift C).Unit.app X.X,
          comm' :=
            by 
              dsimp 
              sliceRHS 1 2 => erw [←(shift C).Unit.naturality]
      ...
      fun X =>
        { f := (shift C).unitInv.app X.X,
          comm' :=
            by 
              dsimp 
              sliceRHS 1 2 => erw [←(shift C).unitInv.naturality]
              rw [equivalence.counit_inv_app_functor]
              sliceLHS 3 4 => rw [←functor.map_comp]
      ...
      fun X =>
        { f := (shift C).counit.app X.X,
          comm' :=
            by 
              dsimp 
              sliceRHS 1 2 => erw [←(shift C).counit.naturality]
              rw [(shift C).Functor.map_comp, (shift C).Functor.map_comp]
              sliceLHS 3 4 => erw [←functor.map_comp, iso.inv_hom_id_app, Functor.map_id]
        ...
        { f := (shift C).counitInv.app X.X,
          comm' :=
            by 
              dsimp 
              rw [(shift C).Functor.map_comp, (shift C).Functor.map_comp]
              sliceRHS 1 2 => erw [←(shift C).counitInv.naturality]
              rw [←equivalence.counit_app_functor]
              sliceRHS 2 3 => rw [iso.inv_hom_id_app]
              rw [category.id_comp]
              rfl } }
