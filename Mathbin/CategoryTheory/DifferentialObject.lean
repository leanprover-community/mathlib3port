...
def mapDifferentialObject (F : C ⥤ D)
    ...
    (hF : ∀ c c', F.map (0 : c ⟶ c') = 0) : differential_object C ⥤ DifferentialObject D
    where
  obj X :=
    { x := F.obj X.x
...
def shiftFunctor (n : ℤ) : DifferentialObject C ⥤ DifferentialObject C
  ...
  map X Y f :=
    { f := f.f⟦n⟧'
      comm' := by
        dsimp
        rw [Category.assoc, shiftComm_hom_comp, ← Functor.map_comp_assoc, f.comm,
...
def shiftFunctorAdd (m n : ℤ) : shiftFunctor C (m + n) ≅ shiftFunctor C m ⋙ shiftFunctor C n :=
  by
  refine' NatIso.ofComponents (fun X => mkIso (shiftAdd X.x _ _) _) _
  · dsimp
    ...
    simp only [eqToHom_map, eqToHom_app, eqToIso.hom, eqToHom_trans_assoc, eqToIso.inv]
  · intro X Y f
    ext
    dsimp
...
def shiftε : 𝟭 (DifferentialObject C) ≅ shiftFunctor C 0 :=
  by
  refine' NatIso.ofComponents (fun X => mkIso ((shiftMonoidalFunctor C ℤ).εIso.app X.x) _) _
  · dsimp
    simp
    dsimp
    simp
