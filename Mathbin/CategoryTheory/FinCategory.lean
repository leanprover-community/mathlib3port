...
noncomputable instance category_as_type : small_category (as_type α) where
  hom := fun i j => Finₓ (Fintype.card (@Quiver.Hom (obj_as_type α) _ i j))
...
noncomputable def obj_as_type_equiv_as_type : as_type α ≌ obj_as_type α where
  Functor :=
  ...
  inverse :=
    { obj := id, map := fun i j f => Fintype.equivFin _ f,
      map_comp' := fun _ _ _ _ _ => by
        ...
        simp }
  unitIso :=
    nat_iso.of_components iso.refl fun _ _ _ => by
      dsimp
      simp
  counitIso :=
    nat_iso.of_components iso.refl fun _ _ _ => by
...
-/
def fin_category_opposite {J : Type v} [small_category J] [fin_category J] : fin_category (Jᵒᵖ) where
  decidableEqObj := Equivₓ.decidableEq equiv_to_opposite.symm
  fintypeObj := Fintype.ofEquiv _ equiv_to_opposite
