...
@[simp, reassoc]
theorem underlying_iso_arrow {X Y : C} (f : X ⟶ Y) [mono f] : (underlying_iso f).inv ≫ (subobject.mk f).arrow = f :=
  over.w _
...
theorem eq_mk_of_comm {B A : C} {X : subobject B} (f : A ⟶ B) [mono f] (i : (X : C) ≅ A) (w : i.hom ≫ f = X.arrow) :
  X = mk f :=
  eq_of_comm (i.trans (underlying_iso f).symm)$
    by 
      simp [w]
...
theorem mk_eq_mk_of_comm {B A₁ A₂ : C} (f : A₁ ⟶ B) (g : A₂ ⟶ B) [mono f] [mono g] (i : A₁ ≅ A₂) (w : i.hom ≫ g = f) :
  mk f = mk g :=
  eq_mk_of_comm _ ((underlying_iso f).trans i)$
    by 
      simp [w]
...
theorem of_le_mk_le_mk_of_comm {B A₁ A₂ : C} {f₁ : A₁ ⟶ B} {f₂ : A₂ ⟶ B} [mono f₁] [mono f₂] (g : A₁ ⟶ A₂)
  (w : g ≫ f₂ = f₁) : of_le _ _ (mk_le_mk_of_comm g w) = (underlying_iso _).Hom ≫ g ≫ (underlying_iso _).inv :=
  by 
    ext 
    simp [w]
...
theorem of_le_comp_of_le_mk {B A : C} (X Y : subobject B) (f : A ⟶ B) [mono f] (h₁ : X ≤ Y) (h₂ : Y ≤ mk f) :
  of_le X Y h₁ ≫ of_le_mk Y f h₂ = of_le_mk X f (h₁.trans h₂) :=
  by 
...
theorem of_le_mk_comp_of_mk_le {B A : C} (X : subobject B) (f : A ⟶ B) [mono f] (Y : subobject B) (h₁ : X ≤ mk f)
  (h₂ : mk f ≤ Y) : of_le_mk X f h₁ ≫ of_mk_le f Y h₂ = of_le X Y (h₁.trans h₂) :=
  by 
...
theorem of_le_mk_comp_of_mk_le_mk {B A₁ A₂ : C} (X : subobject B) (f : A₁ ⟶ B) [mono f] (g : A₂ ⟶ B) [mono g]
  (h₁ : X ≤ mk f) (h₂ : mk f ≤ mk g) : of_le_mk X f h₁ ≫ of_mk_le_mk f g h₂ = of_le_mk X g (h₁.trans h₂) :=
  by 
...
theorem of_mk_le_comp_of_le_mk {B A₁ A₂ : C} (f : A₁ ⟶ B) [mono f] (X : subobject B) (g : A₂ ⟶ B) [mono g]
  (h₁ : mk f ≤ X) (h₂ : X ≤ mk g) : of_mk_le f X h₁ ≫ of_le_mk X g h₂ = of_mk_le_mk f g (h₁.trans h₂) :=
  by 
    simp [of_mk_le, of_le_mk, of_le, of_mk_le_mk, ←functor.map_comp_assoc underlying]

@[simp, reassoc]
theorem of_mk_le_mk_comp_of_mk_le {B A₁ A₂ : C} (f : A₁ ⟶ B) [mono f] (g : A₂ ⟶ B) [mono g] (X : subobject B)
  (h₁ : mk f ≤ mk g) (h₂ : mk g ≤ X) : of_mk_le_mk f g h₁ ≫ of_mk_le g X h₂ = of_mk_le f X (h₁.trans h₂) :=
  by 
    simp [of_mk_le, of_le_mk, of_le, of_mk_le_mk, ←functor.map_comp underlying]

@[simp, reassoc]
theorem of_mk_le_mk_comp_of_mk_le_mk {B A₁ A₂ A₃ : C} (f : A₁ ⟶ B) [mono f] (g : A₂ ⟶ B) [mono g] (h : A₃ ⟶ B) [mono h]
  (h₁ : mk f ≤ mk g) (h₂ : mk g ≤ mk h) : of_mk_le_mk f g h₁ ≫ of_mk_le_mk g h h₂ = of_mk_le_mk f h (h₁.trans h₂) :=
  by 
    simp [of_mk_le, of_le_mk, of_le, of_mk_le_mk, ←functor.map_comp_assoc underlying]

@[simp]
theorem of_le_refl {B : C} (X : subobject B) : of_le X X (le_reflₓ _) = 𝟙 _ :=
  by 
    apply (cancel_mono X.arrow).mp 
    simp 

@[simp]
theorem of_mk_le_mk_refl {B A₁ : C} (f : A₁ ⟶ B) [mono f] : of_mk_le_mk f f (le_reflₓ _) = 𝟙 _ :=
  by 
...
def map_iso_to_order_iso (e : X ≅ Y) : subobject X ≃o subobject Y :=
  { toFun := (map e.hom).obj, invFun := (map e.inv).obj,
    left_inv :=
      fun g =>
        by 
          simpRw [←map_comp, e.hom_inv_id, map_id],
    right_inv :=
      fun g =>
        by 
          simpRw [←map_comp, e.inv_hom_id, map_id],
    map_rel_iff' :=
      fun A B =>
        by 
          dsimp 
          ...
          ·
            ...
            simpRw [←map_comp, e.hom_inv_id, map_id]  at h 
            exact h
          ·
            intro h 
            applyFun (map e.hom).obj  at h 
            exact h }
...
@[simp]
theorem pullback_map_self [has_pullbacks C] (f : X ⟶ Y) [mono f] (g : subobject X) :
...
theorem map_pullback [has_pullbacks C] {X Y Z W : C} {f : X ⟶ Y} {g : X ⟶ Z} {h : Y ⟶ W} {k : Z ⟶ W} [mono h] [mono g]
