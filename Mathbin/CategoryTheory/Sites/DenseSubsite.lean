...
#align
  category_theory.presieve.cover_by_image_structure CategoryTheory.Presieve.CoverByImageStructure
...
theorem Presieve.in_cover_by_image (G : C ⥤ D) {X : D} {Y : C} (f : G.obj Y ⟶ X) :
...
theorem functor_pullback_pushforward_covering [Full G] (H : CoverDense K G) {X : C}
...
#align
  category_theory.cover_dense.sheaf_eq_amalgamation CategoryTheory.CoverDense.sheaf_eq_amalgamation
...
#align
  category_theory.cover_dense.types.pushforward_family CategoryTheory.CoverDense.Types.pushforwardFamily

/-- (Implementation). The `pushforward_family` defined is compatible. -/
theorem pushforward_family_compatible {X} (x : ℱ.obj (op X)) :
...
theorem pushforward_family_apply {X} (x : ℱ.obj (op X)) {Y : C} (f : G.obj Y ⟶ X) :
    pushforwardFamily H α x f (Presieve.in_cover_by_image G f) = α.app (op Y) (ℱ.map f.op x) :=
  by
...
#align
  category_theory.cover_dense.types.app_hom_restrict CategoryTheory.CoverDense.Types.app_hom_restrict
...
#align
  category_theory.cover_dense.types.app_hom_valid_glue CategoryTheory.CoverDense.Types.app_hom_valid_glue
...
theorem sheaf_hom_restrict_eq (α : G.op ⋙ ℱ ⟶ G.op ⋙ ℱ'.val) :
  ...
  infer_instance
#align
  category_theory.cover_dense.sheaf_hom_restrict_eq CategoryTheory.CoverDense.sheaf_hom_restrict_eq

/-- If the pullback map is obtained via whiskering,
then the result `sheaf_hom (whisker_left G.op α)` is equal to `α`.
-/
theorem sheaf_hom_eq (α : ℱ ⟶ ℱ'.val) : sheafHom H (whiskerLeft G.op α) = α :=
  by
  ext X
  apply yoneda.map_injective
  swap; · infer_instance
...
#align
  category_theory.cover_dense.restrict_hom_equiv_hom CategoryTheory.CoverDense.restrictHomEquivHom
...
#align
  category_theory.cover_dense.sites.pullback.faithful CategoryTheory.CoverDense.Sites.Pullback.faithful
