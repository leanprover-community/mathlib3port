...
noncomputable
  instance
    sites.pullback.full
    [ faithful G ] ( Hp : cover_preserving J K G ) : full ( sites.pullback A H.compatible_preserving Hp )
    where Preimage ℱ ℱ' α := ⟨ H.sheaf_hom α.val ⟩ witness' ℱ ℱ' α := Sheaf.hom.ext _ _ $ H.sheaf_hom_restrict_eq α.val
