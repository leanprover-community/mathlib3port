...
def meqOfSep (P : Cᵒᵖ ⥤ D)
  ...
  property := by
    ...
    rw [← comp_apply, ← comp_apply, (J.to_plus P).naturality, (J.to_plus P).naturality, comp_apply,
      comp_apply]
    erw [toPlus_apply (T II.fst.from_middle) (t II.fst.from_middle) II.fst.to_middle,
      toPlus_apply (T II.snd.from_middle) (t II.snd.from_middle) II.snd.to_middle, ← ht, ← ht, ←
...
theorem isIso_toSheafify {P : Cᵒᵖ ⥤ D} (hP : Presheaf.IsSheaf J P) : IsIso (J.toSheafify P) :=
  by
  dsimp [toSheafify]
  haveI : IsIso (J.to_plus P) := by apply isIso_toPlus_of_isSheaf J P hP
  haveI : IsIso ((J.plus_functor D).map (J.to_plus P)) := by apply Functor.map_isIso
...
theorem sheafifyMap_sheafifyLift {P Q R : Cᵒᵖ ⥤ D} (η : P ⟶ Q) (γ : Q ⟶ R)
  ...
  by
  apply J.sheafify_lift_unique
  rw [← Category.assoc, ← J.to_sheafify_naturality, Category.assoc, toSheafify_sheafifyLift]
