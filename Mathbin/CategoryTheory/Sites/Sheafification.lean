...
theorem to_plus_eq_mk {X : C} {P : Cᵒᵖ ⥤ D} (x : P.obj (op X)) : (J.to_plus P).app _ x = mk (meq.mk ⊤ x) :=
  by 
    dsimp [mk, to_plus]
    delta' cover.to_multiequalizer 
    simp only [comp_apply]
...
/-- `P⁺⁺` is always a sheaf. -/
theorem is_sheaf_plus_plus (P : Cᵒᵖ ⥤ D) : presheaf.is_sheaf J (J.plus_obj (J.plus_obj P)) :=
  by 
...
@[simp]
theorem sheafify_map_id (P : Cᵒᵖ ⥤ D) : J.sheafify_map (𝟙 P) = 𝟙 (J.sheafify P) :=
  by 
...
theorem is_iso_to_sheafify {P : Cᵒᵖ ⥤ D} (hP : presheaf.is_sheaf J P) : is_iso (J.to_sheafify P) :=
  by 
    dsimp [to_sheafify]
    have  : is_iso (J.to_plus P) :=
      by 
        apply is_iso_to_plus_of_is_sheaf J P hP 
    have  : is_iso ((J.plus_functor D).map (J.to_plus P)) :=
      by 
...
def iso_sheafify {P : Cᵒᵖ ⥤ D} (hP : presheaf.is_sheaf J P) : P ≅ J.sheafify P :=
  by 
...
def presheaf_to_Sheaf : (Cᵒᵖ ⥤ D) ⥤ Sheaf J D :=
  { obj := fun P => ⟨J.sheafify P, J.sheafify_is_sheaf P⟩, map := fun P Q η => J.sheafify_map η,
    map_id' := J.sheafify_map_id, map_comp' := fun P Q R => J.sheafify_map_comp }
...
@[simp]
theorem sheafification_adjunction_unit_app P : (sheafification_adjunction J D).Unit.app P = J.to_sheafify P :=
  by 
    ext 
...
def sheafification_iso (P : Sheaf J D) : P ≅ (presheaf_to_Sheaf J D).obj P :=
...
@[simp]
theorem sheafification_iso_hom (P : Sheaf J D) : (sheafification_iso P).Hom = J.to_sheafify P :=
  rfl

@[simp]
theorem sheafification_iso_inv (P : Sheaf J D) : (sheafification_iso P).inv = J.sheafify_lift (𝟙 P) P.property :=
  by 
    dsimp [sheafification_iso]
    simp 
