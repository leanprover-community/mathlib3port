...
theorem imageSieve_eq_sieveOfSection {F G : Cᵒᵖ ⥤ A} (f : F ⟶ G) {U : C} (s : G.obj (op U)) :
    imageSieve f s = (imagePresheaf (whiskerRight ConcreteCategory (forget A))).sieveOfSection s :=
  rfl
...
theorem imageSieve_app {F G : Cᵒᵖ ⥤ A} (f : F ⟶ G) {U : C} (s : F.obj (op U)) :
    imageSieve coeSort (f.app _ s) = ⊤ := by
  ext (V i)
...
theorem isLocallySurjective_iff_isIso {F G : Sheaf J (Type w)} (f : F ⟶ G) :
  ...
  exact
    ⟨fun h => @is_iso_of_reflects_iso _ _ (imageSheafι f) (sheafToPresheaf J _) h _, fun h =>
      @functor.map_is_iso _ _ (sheafToPresheaf J _) _ h⟩
...
theorem isLocallySurjective_iff_whisker_forget {F G : Cᵒᵖ ⥤ A} (f : F ⟶ G) :
  ...
  simpa only [isLocallySurjective_iff_imagePresheaf_sheafify_eq_top]
...
noncomputable def sheafificationIsoImagePresheaf :
  ...
  inv := Subpresheaf.ι _
  hom_inv_id' := J.sheafify_hom_ext _ _ (J.sheafify_isSheaf _) (by simp [toImagePresheafSheafify])
  inv_hom_id' :=
    by
    rw [← cancel_mono (Subpresheaf.ι _), Category.id_comp, Category.assoc]
    refine' Eq.trans _ (Category.comp_id _)
    congr 1
    exact J.sheafify_hom_ext _ _ (J.sheafify_is_sheaf _) (by simp [toImagePresheafSheafify])
    infer_instance
