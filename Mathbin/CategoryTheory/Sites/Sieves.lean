...
@[simp]
theorem functor_pullback_id (R : presieve X) : R.functor_pullback (𝟭 _) = R :=
  rfl
...
theorem le_functor_pushforward_pullback (R : sieve X) : R ≤ (R.functor_pushforward F).FunctorPullback F :=
  (functor_galois_connection F X).le_u_l _

theorem functor_pullback_pushforward_le (R : sieve (F.obj X)) : (R.functor_pullback F).FunctorPushforward F ≤ R :=
