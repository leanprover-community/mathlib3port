...
-/
inductive pullback_arrows [HasPullbacks C] (R : Presieve X) : Presieve Y
...
def gi_generate : GaloisInsertion (generate : Presieve X → Sieve X) Arrows where
...
def galois_insertion_of_split_epi (f : Y ⟶ X) [SplitEpi f] : GaloisInsertion (Sieve.pushforward f) (Sieve.pullback f) :=
  by
...
def ess_surj_full_functor_galois_insertion [EssSurj F] [Full F] (X : C) :
...
-/
@[simps]
def nat_trans_of_le {S T : Sieve X} (h : S ≤ T) : S.Functor ⟶ T.Functor where
  app := fun Y f => ⟨f.1, h _ f.2⟩

/-- The natural inclusion from the functor induced by a sieve to the yoneda embedding. -/
@[simps]
def functor_inclusion (S : Sieve X) : S.Functor ⟶ yoneda.obj X where
  app := fun Y f => f.1
...
-/
@[simps]
def sieve_of_subfunctor {R} (f : R ⟶ yoneda.obj X) : Sieve X where
