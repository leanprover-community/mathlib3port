...
-/
@[ext]
theorem ext {J₁ J₂ : grothendieck_topology C} (h : (J₁ : ∀ (X : C), Set (sieve X)) = J₂) : J₁ = J₂ :=
  by 
    cases J₁ 
    cases J₂ 
    congr 
...
theorem covering_of_eq_top : S = ⊤ → S ∈ J X :=
  fun h => h.symm ▸ J.top_mem X
...
theorem intersection_covering_iff : R⊓S ∈ J X ↔ R ∈ J X ∧ S ∈ J X :=
  ⟨fun h => ⟨J.superset_covering inf_le_left h, J.superset_covering inf_le_right h⟩,
    fun t => intersection_covering _ t.1 t.2⟩
...
theorem covering_iff_covers_id (S : sieve X) : S ∈ J X ↔ J.covers S (𝟙 X) :=
  by 
    simp [covers_iff]
...
theorem arrow_stable (f : Y ⟶ X) (S : sieve X) (h : J.covers S f) {Z : C} (g : Z ⟶ Y) : J.covers S (g ≫ f) :=
  by 
    rw [covers_iff] at h⊢
...
theorem arrow_intersect (f : Y ⟶ X) (S R : sieve X) (hS : J.covers S f) (hR : J.covers R f) : J.covers (S⊓R) f :=
  by 
...
/-- See https://stacks.math.columbia.edu/tag/00Z7 -/
theorem is_glb_Inf (s : Set (grothendieck_topology C)) : IsGlb s (Inf s) :=
  by 
...
theorem bot_covers (S : sieve X) (f : Y ⟶ X) : (⊥ : grothendieck_topology C).Covers S f ↔ S f :=
  by 
    rw [covers_iff, bot_covering, ←sieve.pullback_eq_top_iff_mem]

@[simp]
theorem top_covers (S : sieve X) (f : Y ⟶ X) : (⊤ : grothendieck_topology C).Covers S f :=
  by 
    simp [covers_iff]
...
theorem right_ore_of_pullbacks [limits.has_pullbacks C] : right_ore_condition C :=
  fun X Y Z yx zx => ⟨_, _, _, limits.pullback.condition⟩
...
def relation.base {f : Y ⟶ X} {S : J.cover X} (I : (S.pullback f).Relation) : S.relation :=
  ⟨_, _, _, I.g₁, I.g₂, I.f₁ ≫ f, I.f₂ ≫ f, I.h₁, I.h₂,
    by 
      simp [reassoc_of I.w]⟩
...
noncomputable def arrow.to_middle_hom {X : C} {S : J.cover X} {T : ∀ (I : S.arrow), J.cover I.Y}
...
def pullback_id (X : C) : J.pullback (𝟙 X) ≅ 𝟭 _ :=
  (nat_iso.of_components fun S => S.pullback_id)$
    by 
      tidy
...
def pullback_comp {X Y Z : C} (f : X ⟶ Y) (g : Y ⟶ Z) : J.pullback (f ≫ g) ≅ J.pullback g ⋙ J.pullback f :=
  (nat_iso.of_components fun S => S.pullback_comp f g)$
    by 
      tidy
