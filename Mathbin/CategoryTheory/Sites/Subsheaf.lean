...
def Subpresheaf.toPresheaf : Cᵒᵖ ⥤ Type w
    where
  obj U := G.obj U
  map U V i x := ⟨F.map i x, G.map i ConcreteCategory.prop⟩
  map_id' X := by
    ext ⟨x, _⟩
    dsimp
    rw [F.map_id]
...
instance {U} : Coe (G.toPresheaf.obj coeFn) (F.obj U) :=
  coeSubtype
...
instance : IsIso (Subpresheaf.ι (⊤ : Subpresheaf F)) :=
  ...
  · intro X
    ...
    exact ⟨Subtype.coe_injective, fun x => ⟨⟨x, trivial⟩, rfl⟩⟩
...
noncomputable def Subpresheaf.sheafifyLift (f : G.toPresheaf ⟶ F') (h : Presieve.IsSheaf J F') :
  ...
  naturality' := by
    ...
    intro W j hj
    refine' (Presieve.IsSheafFor.valid_glue _ _ _ hj).trans _
    dsimp
    conv_rhs => rw [← FunctorToTypes.map_comp_apply]
    change _ = F'.map (j ≫ i.unop).op _
    refine' Eq.trans _ (Presieve.IsSheafFor.valid_glue _ _ _ _).symm
    · dsimp at hj⊢
      rwa [FunctorToTypes.map_comp_apply]
    · dsimp [Presieve.FamilyOfElements.compPresheafMap]
      congr 1
      ext1
      exact (FunctorToTypes.map_comp_apply _ _ _ _).symm
...
theorem Subpresheaf.to_sheafifyLift (f : G.toPresheaf ⟶ F') (h : Presieve.IsSheaf J F') :
  ...
  intro V i hi
  have := elementwise_of f.naturality
  exact (Presieve.IsSheafFor.valid_glue _ _ _ hi).trans (this _ _)
...
theorem Subpresheaf.sheafify_le (h : G ≤ G') (hF : Presieve.IsSheaf J F)
  ...
  by
  intro U x hx
  convert ((G.sheafify_lift (Subpresheaf.homOfLe h) hG').app U ⟨x, hx⟩).2
  apply (hF _ hx).isSeparatedFor.ext
  intro V i hi
  have :=
    congr_arg (fun f : G.to_presheaf ⟶ G'.to_presheaf => (NatTrans.app f (op V) ⟨_, hi⟩).1)
      (G.to_sheafify_lift (Subpresheaf.homOfLe h) hG')
  convert this.symm
...
theorem imagePresheaf_comp_le (f₁ : F ⟶ F') (f₂ : F' ⟶ F'') :
    imagePresheaf (f₁ ≫ f₂) ≤ imagePresheaf f₂ := fun U x hx => ⟨f₁.app U hx.choose, hx.choose_spec⟩
