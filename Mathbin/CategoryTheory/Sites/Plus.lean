...
theorem plusMap_id (P : Cᵒᵖ ⥤ D) : J.plusMap (𝟙 P) = 𝟙 _ :=
  ...
  rw [J.diagram_nat_trans_id, NatTrans.id_app]
  ext
  dsimp
  simp
...
theorem plusMap_toPlus : J.plusMap (J.toPlus P) = J.toPlus (J.plusObj P) :=
  ...
  ext II
  dsimp
  simp only [limit.lift_π, Multifork.ofι_π_app, multiequalizer.lift_ι, Category.assoc]
  dsimp [Multifork.ofι]
  convert
...
theorem isIso_toPlus_of_isSheaf (hP : Presheaf.IsSheaf J P) : IsIso (J.toPlus P) :=
  ...
  rsuffices : ∀ X, IsIso ((J.to_plus P).app X)
  · apply NatIso.isIso_of_isIso_app
  intro X
  dsimp
  rsuffices : IsIso (colimit.ι (J.diagram P X.unop) (op ⊤))
  · apply IsIso.comp_isIso
  rsuffices : ∀ (S T : (J.cover X.unop)ᵒᵖ) (f : S ⟶ T), IsIso ((J.diagram P X.unop).map f)
...
instance plusFunctor_preservesZeroMorphisms [Preadditive D] :
    ...
    ext
    dsimp
    rw [J.plus_map_zero, NatTrans.app_zero]
