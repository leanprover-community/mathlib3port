...
`R₁`.
-/
def family_of_elements.restrict {R₁ R₂ : Presieve X} (h : R₁ ≤ R₂) : FamilyOfElements P R₂ → FamilyOfElements P R₁ :=
...
-/
def family_of_elements.compatible (x : FamilyOfElements P R) : Prop :=
...
-/
def family_of_elements.pullback_compatible (x : FamilyOfElements P R) [HasPullbacks C] : Prop :=
...
theorem family_of_elements.compatible.restrict {R₁ R₂ : Presieve X} (h : R₁ ≤ R₂) {x : FamilyOfElements P R₂} :
...
-/
noncomputable def family_of_elements.sieve_extend (x : FamilyOfElements P R) : FamilyOfElements P (generate R) :=
...
theorem family_of_elements.compatible.sieve_extend {x : FamilyOfElements P R} (hx : x.Compatible) :
...
-/
def family_of_elements.sieve_compatible (x : FamilyOfElements P S) : Prop :=
...
theorem family_of_elements.compatible.to_sieve_compatible {x : FamilyOfElements P S} (t : x.Compatible) :
    x.SieveCompatible :=
...
@[simps]
noncomputable def compatible_equiv_generate_sieve_compatible :
...
theorem family_of_elements.comp_of_compatible (S : Sieve X) {x : FamilyOfElements P S} (t : x.Compatible) {f : Y ⟶ X}
...
-/
def family_of_elements.functor_pullback (x : FamilyOfElements P T) :
    FamilyOfElements (F.op ⋙ P) (T.FunctorPullback F) := fun Y f hf => x (F.map f) hf

theorem family_of_elements.compatible.functor_pullback (h : x.Compatible) : (x.FunctorPullback F).Compatible := by
...
-/
def family_of_elements.pullback (f : Y ⟶ X) (x : FamilyOfElements P S) : FamilyOfElements P (S.pullback f) :=
  fun _ g hg => x (g ≫ f) hg

theorem family_of_elements.compatible.pullback (f : Y ⟶ X) {x : FamilyOfElements P S} (h : x.Compatible) :
...
-/
def family_of_elements.comp_presheaf_map (f : P ⟶ Q) (x : FamilyOfElements P R) : FamilyOfElements Q R := fun Y g hg =>
  f.app (op Y) (x g hg)

@[simp]
theorem family_of_elements.comp_presheaf_map_id (x : FamilyOfElements P R) : x.compPresheafMap (𝟙 P) = x :=
  rfl

@[simp]
theorem family_of_elements.comp_prersheaf_map_comp (x : FamilyOfElements P R) (f : P ⟶ Q) (g : Q ⟶ U) :
    (x.compPresheafMap f).compPresheafMap g = x.compPresheafMap (f ≫ g) :=
  rfl

theorem family_of_elements.compatible.comp_presheaf_map (f : P ⟶ Q) {x : FamilyOfElements P R} (h : x.Compatible) :
...
equation (2).
-/
def family_of_elements.is_amalgamation (x : FamilyOfElements P R) (t : P.obj (op X)) : Prop :=
  ∀ ⦃Y : C⦄ f : Y ⟶ X h : R f, P.map f.op t = x f h

theorem family_of_elements.is_amalgamation.comp_presheaf_map {x : FamilyOfElements P R} {t} (f : P ⟶ Q)
...
theorem is_separated_for.ext {R : Presieve X} (hR : IsSeparatedFor P R) {t₁ t₂ : P.obj (op X)}
...
-/
noncomputable def is_sheaf_for.extend {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S) (f : S.Functor ⟶ P) : yoneda.obj X ⟶ P :=
...
-/
@[simp, reassoc]
theorem is_sheaf_for.functor_inclusion_comp_extend {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S) (f : S.Functor ⟶ P) :
...
theorem is_sheaf_for.unique_extend {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S) {f : S.Functor ⟶ P} (t : yoneda.obj X ⟶ P)
...
-/
theorem is_sheaf_for.hom_ext {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S) (t₁ t₂ : yoneda.obj X ⟶ P)
...
-/
theorem is_separated_for.is_sheaf_for (t : IsSeparatedFor P R) :
...
theorem is_sheaf_for.is_separated_for : IsSheafFor P R → IsSeparatedFor P R := fun q =>
...
noncomputable def is_sheaf_for.amalgamate (t : IsSheafFor P R) (x : FamilyOfElements P R) (hx : x.Compatible) :
    P.obj (op X) :=
  (t x hx).exists.some

theorem is_sheaf_for.is_amalgamation (t : IsSheafFor P R) {x : FamilyOfElements P R} (hx : x.Compatible) :
    x.IsAmalgamation (t.amalgamate x hx) :=
  (t x hx).exists.some_spec

@[simp]
theorem is_sheaf_for.valid_glue (t : IsSheafFor P R) {x : FamilyOfElements P R} (hx : x.Compatible) (f : Y ⟶ X)
...
theorem is_sheaf.is_sheaf_for {P : Cᵒᵖ ⥤ Type w} (hp : IsSheaf J P) (R : Presieve X) (hr : generate R ∈ J X) :
    IsSheafFor P R :=
...
@[simps]
def first_obj_eq_family : FirstObj P R ≅ R.FamilyOfElements P where
...
of https://stacks.math.columbia.edu/tag/00VM.
-/
def fork_map : P.obj (op X) ⟶ FirstObj P R :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:627:6: warning: expanding binder group (Y Z)
...
def first_map : FirstObj P S ⟶ SecondObj P S :=
...
def second_map : FirstObj P S ⟶ SecondObj P S :=
...
def first_map : FirstObj P R ⟶ SecondObj P R :=
...
/-- The map `pr₁*` of https://stacks.math.columbia.edu/tag/00VL. -/
def second_map : FirstObj P R ⟶ SecondObj P R :=
...
-/
@[simps]
def SheafOfTypes_bot_equiv : SheafOfTypes (⊥ : GrothendieckTopology C) ≌ Cᵒᵖ ⥤ Type w where
