...
theorem isSheafOfLe (P : Cᵒᵖ ⥤ Type w) {J₁ J₂ : GrothendieckTopology C} :
    J₁ ≤ J₂ → IsSheaf J₂ P → IsSheaf J₁ P := fun h t X S hS => t S (h _ hS)
#align category_theory.presieve.is_sheaf_of_le CategoryTheory.Presieve.isSheafOfLe

theorem isSeparatedOfIsSheaf (P : Cᵒᵖ ⥤ Type w) (h : IsSheaf J P) : IsSeparated J P := fun X S hS =>
  (h S hS).IsSeparatedFor
#align
  category_theory.presieve.is_separated_of_is_sheaf CategoryTheory.Presieve.isSeparatedOfIsSheaf

/-- The property of being a sheaf is preserved by isomorphism. -/
theorem isSheafIso {P' : Cᵒᵖ ⥤ Type w} (i : P ≅ P') (h : IsSheaf J P) : IsSheaf J P' :=
  fun X S hS => is_sheaf_for_iso i (h S hS)
#align category_theory.presieve.is_sheaf_iso CategoryTheory.Presieve.isSheafIso

theorem isSheafOfYoneda {P : Cᵒᵖ ⥤ Type v₁}
    (h : ∀ {X} (S : Sieve X), S ∈ J X → YonedaSheafCondition P S) : IsSheaf J P := fun X S hS =>
  is_sheaf_for_iff_yoneda_sheaf_condition.2 (h _ hS)
#align category_theory.presieve.is_sheaf_of_yoneda CategoryTheory.Presieve.isSheafOfYoneda
...
theorem isSheafBot : IsSheaf (⊥ : GrothendieckTopology C) P := fun X => by
  simp [is_sheaf_for_top_sieve]
#align category_theory.presieve.is_sheaf_bot CategoryTheory.Presieve.isSheafBot
