...
def nat_trans_equiv_compatible_family {P : Cᵒᵖ ⥤ Type v₁} :
    (S.functor ⟶ P) ≃ { x : family_of_elements P S // x.compatible } where
  toFun := fun α => by
...
@[simps]
def first_obj_eq_family : first_obj P R ≅ R.family_of_elements P where
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (Y Z)
...
@[simps]
def SheafOfTypes_to_presheaf : SheafOfTypes J ⥤ Cᵒᵖ ⥤ Type w where
  obj := SheafOfTypes.val
  map := fun X Y f => f.val
  map_id' := fun X => rfl
  map_comp' := fun X Y Z f g => rfl

instance : full (SheafOfTypes_to_presheaf J) where
...
-/
@[simps]
def SheafOfTypes_bot_equiv : SheafOfTypes (⊥ : grothendieck_topology C) ≌ Cᵒᵖ ⥤ Type w where
