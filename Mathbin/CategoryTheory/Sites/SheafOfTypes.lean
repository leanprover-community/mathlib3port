...
def family_of_elements.restrict {R₁ R₂ : presieve X} (h : R₁ ≤ R₂) :
    family_of_elements P R₂ → family_of_elements P R₁ := fun x Y f hf => x f (h _ hf)
...
theorem extend_agrees {x : family_of_elements P R} (t : x.compatible) {f : Y ⟶ X} (hf : R f) :
    x.sieve_extend f ⟨_, 𝟙 _, f, hf, id_comp _⟩ = x f hf := by
...
theorem compatible_iff_sieve_compatible (x : family_of_elements P S) : x.compatible ↔ x.sieve_compatible := by
  constructor
  ·
    intro h Y Z f g hf
...
theorem restrict_inj {x₁ x₂ : family_of_elements P (generate R)} (t₁ : x₁.compatible) (t₂ : x₂.compatible) :
    x₁.restrict (le_generate R) = x₂.restrict (le_generate R) → x₁ = x₂ := by
  intro h
...
def family_of_elements.functor_pullback (x : family_of_elements P T) :
    family_of_elements (F.op ⋙ P) (T.functor_pullback F) := fun Y f hf => x (F.map f) hf

theorem family_of_elements.compatible.functor_pullback (h : x.compatible) : (x.functor_pullback F).Compatible := by
...
theorem family_of_elements.compatible.comp_presheaf_map (f : P ⟶ Q) {x : family_of_elements P R} (h : x.compatible) :
    (x.comp_presheaf_map f).Compatible := by
  intro Z₁ Z₂ W g₁ g₂ f₁ f₂ h₁ h₂ eq
...
theorem family_of_elements.is_amalgamation.comp_presheaf_map {x : family_of_elements P R} {t} (f : P ⟶ Q)
    (h : x.is_amalgamation t) : (x.comp_presheaf_map f).IsAmalgamation (f.app (op X) t) := by
  intro Y g hg
...
theorem is_amalgamation_restrict {R₁ R₂ : presieve X} (h : R₁ ≤ R₂) (x : family_of_elements P R₂) (t : P.obj (op X))
    (ht : x.is_amalgamation t) : (x.restrict h).IsAmalgamation t := fun Y f hf => ht f (h Y hf)

theorem is_amalgamation_sieve_extend {R : presieve X} (x : family_of_elements P R) (t : P.obj (op X))
    (ht : x.is_amalgamation t) : x.sieve_extend.is_amalgamation t := by
  intro Y f hf
  dsimp [family_of_elements.sieve_extend]
...
theorem is_separated_for_iff_generate : is_separated_for P R ↔ is_separated_for P (generate R) := by
  constructor
  ·
    intro h x t₁ t₂ ht₁ ht₂
...
theorem is_separated_for_top (P : Cᵒᵖ ⥤ Type w) : is_separated_for P (⊤ : presieve X) := fun x t₁ t₂ h₁ h₂ => by
  have q₁ :=
    h₁ (𝟙 X)
      (by
        simp )
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.def
  "def"
  (Command.declId `is_sheaf_for [])
  (Command.optDeclSig
   [(Term.explicitBinder
     "("
     [`P]
     [":"
      (CategoryTheory.CategoryTheory.Functor.«term_⥤_» (Data.Opposite.«term_ᵒᵖ» `C "ᵒᵖ") " ⥤ " (Term.type "Type" [`w]))]
     []
     ")")
    (Term.explicitBinder "(" [`R] [":" (Term.app `presieve [`X])] [] ")")]
   [(Term.typeSpec ":" (Term.prop "Prop"))])
  (Command.declValSimple
   ":="
   (Term.forall
    "∀"
    [(Term.simpleBinder [`x] [(Term.typeSpec ":" (Term.app `family_of_elements [`P `R]))])]
    ","
    (Term.arrow
     `x.compatible
     "→"
...
/--
  ...
  def
    is_sheaf_for
    ( P : C ᵒᵖ ⥤ Type w ) ( R : presieve X ) : Prop
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.def
  "def"
  (Command.declId `yoneda_sheaf_condition [])
  (Command.optDeclSig
   [(Term.explicitBinder
     "("
     [`P]
     [":"
      (CategoryTheory.CategoryTheory.Functor.«term_⥤_»
       (Data.Opposite.«term_ᵒᵖ» `C "ᵒᵖ")
       " ⥤ "
       (Term.type "Type" [`v₁]))]
     []
     ")")
    (Term.explicitBinder "(" [`S] [":" (Term.app `sieve [`X])] [] ")")]
   [(Term.typeSpec ":" (Term.prop "Prop"))])
  (Command.declValSimple
   ":="
   (Term.forall
    "∀"
    [(Term.simpleBinder [`f] [(Term.typeSpec ":" (Combinatorics.Quiver.«term_⟶_» `S.functor " ⟶ " `P))])]
    ","
    (Init.Logic.«term∃!_,_»
     "∃!"
     (Lean.explicitBinders (Lean.unbracketedExplicitBinders [(Lean.binderIdent `g)] []))
     ", "
     («term_=_» (CategoryTheory.CategoryTheory.Category.Basic.«term_≫_» `S.functor_inclusion " ≫ " `g) "=" `f)))
   [])
  []
  []
  []))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 50, term))
  (CategoryTheory.CategoryTheory.Category.Basic.«term_≫_» `S.functor_inclusion " ≫ " `g)
...
theorem extension_iff_amalgamation {P : Cᵒᵖ ⥤ Type v₁} (x : S.functor ⟶ P) (g : yoneda.obj X ⟶ P) :
    S.functor_inclusion ≫ g = x ↔ (nat_trans_equiv_compatible_family x).1.IsAmalgamation (yoneda_equiv g) := by
...
theorem is_separated_for.is_sheaf_for (t : is_separated_for P R) :
    (∀ x : family_of_elements P R, x.compatible → ∃ t, x.is_amalgamation t) → is_sheaf_for P R := by
...
/--  C2.1.3 in [Elephant] -/
theorem is_sheaf_for_iff_generate (R : presieve X) : is_sheaf_for P R ↔ is_sheaf_for P (generate R) := by
  rw [← is_separated_for_and_exists_is_amalgamation_iff_sheaf_for]
  rw [← is_separated_for_and_exists_is_amalgamation_iff_sheaf_for]
...
[Elephant] C2.1.5(i)
-/
theorem is_sheaf_for_singleton_iso (P : Cᵒᵖ ⥤ Type w) : is_sheaf_for P (presieve.singleton (𝟙 X)) := by
  intro x hx
...
[Elephant] C2.1.5(ii)
-/
theorem is_sheaf_for_top_sieve (P : Cᵒᵖ ⥤ Type w) : is_sheaf_for P ((⊤ : sieve X) : presieve X) := by
...
-/
theorem is_sheaf_for_iso {P' : Cᵒᵖ ⥤ Type w} (i : P ≅ P') : is_sheaf_for P R → is_sheaf_for P' R := by
  intro h x hx
...
theorem is_separated_of_is_sheaf (P : Cᵒᵖ ⥤ Type w) (h : is_sheaf J P) : is_separated J P := fun X S hS =>
  (h S hS).IsSeparatedFor

/--  The property of being a sheaf is preserved by isomorphism. -/
theorem is_sheaf_iso {P' : Cᵒᵖ ⥤ Type w} (i : P ≅ P') (h : is_sheaf J P) : is_sheaf J P' := fun X S hS =>
  is_sheaf_for_iso i (h S hS)

theorem is_sheaf_of_yoneda {P : Cᵒᵖ ⥤ Type v₁} (h : ∀ {X} S : sieve X, S ∈ J X → yoneda_sheaf_condition P S) :
    is_sheaf J P := fun X S hS => is_sheaf_for_iff_yoneda_sheaf_condition.2 (h _ hS)
...
theorem is_sheaf_pretopology [has_pullbacks C] (K : pretopology C) :
    is_sheaf (K.to_grothendieck C) P ↔ ∀ {X : C} R : presieve X, R ∈ K X → is_sheaf_for P R := by
  constructor
  ·
    intro PJ X R hR
    rw [is_sheaf_for_iff_generate]
...
theorem is_sheaf_bot : is_sheaf (⊥ : grothendieck_topology C) P := fun X => by
...
def first_obj_eq_family : first_obj P R ≅ R.family_of_elements P :=
  { Hom := fun t Y f hf => pi.π (fun f => P.obj (op f.1)) ⟨_, _, hf⟩ t, inv := pi.lift fun f x => x _ f.2.2,
    hom_inv_id' := by
      ext ⟨Y, f, hf⟩ p
      simpa,
    inv_hom_id' := by
...
def second_obj : Type max v₁ u₁ :=
  ∏ fun f => P.obj (op f.2.1)

/- failed to parenthesize: parenthesize: uncaught backtrack exception
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers [(Command.docComment "/--" " The map `p` of Equations (3,4) [MM92]. -/")] [] [] [] [] [])
 (Command.def
  "def"
  (Command.declId `first_map [])
  (Command.optDeclSig
   []
   [(Term.typeSpec
     ":"
     (Combinatorics.Quiver.«term_⟶_» (Term.app `first_obj [`P `S]) " ⟶ " (Term.app `second_obj [`P `S])))])
  (Command.declValSimple
   ":="
   (Term.app
    `pi.lift
          ...
          [(Term.anonymousCtor
            ...
            [(Term.hole "_")
             ...
             (Term.app
              `S.downward_closed
              [(Term.proj
                (Term.proj (Term.proj (Term.proj `fg "." (fieldIdx "2")) "." (fieldIdx "2")) "." (fieldIdx "2"))
                "."
                (fieldIdx "2"))
               (Term.proj (Term.proj (Term.proj `fg "." (fieldIdx "2")) "." (fieldIdx "2")) "." (fieldIdx "1"))])]
            "⟩")
           [(Term.typeAscription
             ":"
             (Init.Data.Sigma.Basic.«termΣ_,_»
              ...
              («term{__:_//_}»
               "{"
               `f
               [":" (Combinatorics.Quiver.«term_⟶_» `Y " ⟶ " `X)]
               "//"
               (Term.app `S [`f])
               "}")))]]
          ")")])))])
         ...
         [(Term.anonymousCtor
          ...
          [(Term.typeAscription
            ...
            (Init.Data.Sigma.Basic.«termΣ_,_»
             ...
             (Lean.explicitBinders (Lean.unbracketedExplicitBinders [(Lean.binderIdent `Y)] []))
             ", "
             («term{__:_//_}» "{" `f [":" (Combinatorics.Quiver.«term_⟶_» `Y " ⟶ " `X)] "//" (Term.app `S [`f]) "}")))]]
         ")")])))])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Term.app', expected 'antiquot'
...
/-- The map `p` of Equations (3,4) [MM92]. -/
  def
    ...
    :=
      pi.lift
        fun
          fg => pi.π _ ( ⟨ _ , _ , S.downward_closed fg . 2 . 2 . 2 . 2 fg . 2 . 2 . 1 ⟩ : Σ Y , { f : Y ⟶ X // S f } )
...
theorem w : fork_map P S ≫ first_map P S = fork_map P S ≫ second_map P S := by
  apply limit.hom_ext
  rintro ⟨Y, Z, g, f, hf⟩
...
theorem compatible_iff (x : first_obj P S) :
    ((first_obj_eq_family P S).Hom x).Compatible ↔ first_map P S x = second_map P S x := by
...
theorem equalizer_sheaf_condition : presieve.is_sheaf_for P S ↔ Nonempty (is_limit (fork.of_ι _ (w P S))) := by
...
theorem w : fork_map P R ≫ first_map P R = fork_map P R ≫ second_map P R := by
  apply limit.hom_ext
...
theorem compatible_iff (x : first_obj P R) :
    ((first_obj_eq_family P R).Hom x).Compatible ↔ first_map P R x = second_map P R x := by
...
-/
theorem sheaf_condition : R.is_sheaf_for P ↔ Nonempty (is_limit (fork.of_ι _ (w P R))) := by
...
structure SheafOfTypes (J : grothendieck_topology C) : Type max u₁ v₁ (w + 1) where
  val : Cᵒᵖ ⥤ Type w
...
def SheafOfTypes_to_presheaf : SheafOfTypes J ⥤ Cᵒᵖ ⥤ Type w :=
  { obj := SheafOfTypes.val, map := fun X Y f => f.val, map_id' := fun X => rfl, map_comp' := fun X Y Z f g => rfl }

-- failed to format: format: uncaught backtrack exception
instance : full ( SheafOfTypes_to_presheaf J ) where Preimage X Y f := ⟨ f ⟩
