...
instance proj_reflects_iso : ReflectsIsomorphisms (proj S T)
    where reflects Y Z f t := ⟨⟨StructuredArrow.homMk (inv ((proj S T).map f)) (by simp), by tidy⟩⟩
...
instance proj_reflects_iso : ReflectsIsomorphisms (proj S T)
    ...
    ⟨⟨CostructuredArrow.homMk (inv ((proj S T).map f)) (by simp), by tidy⟩⟩
