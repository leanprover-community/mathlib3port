...
instance
  proj_reflects_iso
  : reflects_isomorphisms ( proj S T )
  where
    reflects Y Z f t := by exact ⟨ ⟨ structured_arrow.hom_mk ( inv ( ( proj S T ) . map f ) ) ( by simp ) , by tidy ⟩ ⟩
...
instance
  ...
  where
    reflects
      Y Z f t
      :=
      by exact ⟨ ⟨ costructured_arrow.hom_mk ( inv ( ( proj S T ) . map f ) ) ( by simp ) , by tidy ⟩ ⟩
