  ...
  naturality_id' :
    ∀ a : B,
      naturality (𝟙 a) ≫ app a ◁ G.mapId a =
        F.mapId a ▷ Hom a ≫ (λ_ (app a)).hom ≫ (ρ_ (app a)).inv := by
    obviously
  naturality_comp' :
    ∀ {a b c : B} (f : a ⟶ b) (g : b ⟶ comp),
...
theorem whiskerLeft_naturality_comp (f : a' ⟶ G.obj a) (g : a ⟶ b) (h : b ⟶ c) :
  ...
  by simp_rw [← Bicategory.whiskerLeft_comp, naturality_comp]
...
theorem whiskerLeft_naturality_id (f : a' ⟶ G.obj a) :
  ...
  by simp_rw [← Bicategory.whiskerLeft_comp, naturality_id]
...
theorem whiskerRight_naturality_id (f : G.obj a ⟶ a') :
  ...
  by
  rw [← associator_naturality_middle, ← comp_whiskerRight_assoc, naturality_id]
  simp
