...
`[nolint]`). Currently that is not a problem, as there are almost no instances of `bundled_hom`. -/
@[nolint dangerous_instance]
instance concrete_category : ConcreteCategory.{u} (Bundled c) where
  forget :=
...
def mk_has_forget₂ {d : Type u → Type u} {hom_d : ∀ ⦃α β : Type u⦄ Iα : d α Iβ : d β, Type u} [BundledHom hom_d]
...
-/
@[reducible]
def map_hom (F : ∀ {α}, d α → c α) : ∀ ⦃α β : Type u⦄ Iα : d α Iβ : d β, Type u := fun α β iα iβ => hom (F iα) (F iβ)
...
instance bundled_hom_of_parent_projection (F : ∀ {α}, d α → c α) [ParentProjection @F] : BundledHom (MapHom hom @F) :=
  map hom @F
...
instance forget₂_full (F : ∀ {α}, d α → c α) [ParentProjection @F] : Full (forget₂ (Bundled d) (Bundled c)) where
