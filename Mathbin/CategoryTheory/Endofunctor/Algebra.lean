...
-/
@[simps]
def isoMk (h : A₀.1 ≅ A₁.1) (w : F.map h.Hom ≫ A₁.str = A₀.str ≫ h.Hom) :
    A₀ ≅ A₁ where 
  Hom := { f := h.Hom }
  inv :=
    { f := h.inv,
...
-/
@[simps]
def equivOfNatIso {F G : C ⥤ C} (α : F ≅ G) :
    Algebra F ≌ Algebra G where 
...
-/
@[simps]
def isoMk (h : V₀.1 ≅ V₁.1) (w : V₀.str ≫ F.map h.Hom = h.Hom ≫ V₁.str) :
    V₀ ≅ V₁ where 
  Hom := { f := h.Hom }
  inv :=
    { f := h.inv,
...
def Algebra.toCoalgebraOf (adj : F ⊣ G) :
    Algebra F ⥤
      Coalgebra
        G where 
...
def Coalgebra.toAlgebraOf (adj : F ⊣ G) :
    Coalgebra G ⥤
      Algebra
        F where 
...
def AlgCoalgEquiv.counitIso (adj : F ⊣ G) :
    Coalgebra.toAlgebraOf adj ⋙ Algebra.toCoalgebraOf adj ≅
      𝟭
        (Coalgebra
          G) where 
  Hom :=
...
def algebraCoalgebraEquiv (adj : F ⊣ G) :
    Algebra F ≌ Coalgebra G where 
