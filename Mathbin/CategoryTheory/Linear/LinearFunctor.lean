...
class
  functor.linear{C D :
    Type _}[category C][category D][preadditive C][preadditive D][linear R C][linear R D](F : C ⥤ D)[F.additive] :
  Prop where 
...
variable{R}{C D :
    Type
      ...
      C][category
      D][preadditive
      C][preadditive D][CategoryTheory.Linear R C][CategoryTheory.Linear R D](F : C ⥤ D)[Additive F][linear R F]
