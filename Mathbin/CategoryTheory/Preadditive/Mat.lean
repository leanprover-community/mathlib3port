...
def id (M : Mat_ C) : hom M M :=
  fun i j => if h : i = j then eq_to_hom (congr_argₓ M.X h) else 0
  ...
  { hom := hom, id := hom.id, comp := fun M N K f g => f.comp g,
    ...
    assoc' :=
      fun M N K L f g h =>
        by 
          ext i k 
          simpRw [hom.comp, sum_comp, comp_sum, category.assoc]
          rw [Finset.sum_comm] }
