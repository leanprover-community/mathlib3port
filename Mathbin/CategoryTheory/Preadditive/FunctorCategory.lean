    ...
    homGroup
        ...
        :=
        {
          add
              ...
              ,
            zero := { app := fun X => 0 , naturality' := by intros rw [ zero_comp , comp_zero ] } ,
            neg
                :=
                fun
                  α
                    =>
                    { app := fun X => - α.app X , naturality' := by intros rw [ comp_neg , neg_comp , α.naturality ] }
              ,
            sub
                :=
                fun
              ...
              ,
            add_assoc := by intros ext apply add_assocₓ ,
            zero_add := by intros ext apply zero_addₓ ,
            add_zero := by intros ext apply add_zeroₓ ,
            sub_eq_add_neg := by intros ext apply sub_eq_add_neg ,
            add_left_neg := by intros ext apply add_left_negₓ ,
            add_comm := by intros ext apply add_commₓ
          }
   ...
   (Term.typeSpec
    ...
    («term_=_»
     ...
     (Algebra.BigOperators.Basic.«term∑_in_,_»
      ...
      `s
      ", "
      (Term.app (Term.proj (Term.app `α [`i]) "." `app) [`X])))))
     ...
     (Tactic.tacticSeq1Indented
      [(group
        (Tactic.rwSeq
         "rw"
         []
         (Tactic.rwRuleSeq "[" [(Tactic.rwRule ["←"] `app_hom_apply) "," (Tactic.rwRule [] `AddMonoidHom.map_sum)] "]")
         [])
        [])
...
@[ simp ]
  theorem
    app_sum
    { ι : Type _ } ( s : Finset ι ) ( X : C ) ( α : ι → F ⟶ G ) : ∑ i in s , α i . app X = ∑ i in s , α i . app X
    := by rw [ ← app_hom_apply , AddMonoidHom.map_sum ] rfl
