...
instance
  ...
  where
    Hom := monad_hom
      ...
      comp _ _ _ f g := { toNatTrans := { app := fun X => f.app X ≫ g.app X } }
...
instance
  ...
  where
    Hom := comonad_hom
      id M := { toNatTrans := 𝟙 ( M : C ⥤ C ) }
      comp M N L f g := { toNatTrans := { app := fun X => f.app X ≫ g.app X } }
...
-- failed to format: format: uncaught backtrack exception
instance
  : reflects_isomorphisms ( monad_to_functor C )
  where
    reflects
      M N f i
      :=
...
-- failed to format: format: uncaught backtrack exception
instance
  : reflects_isomorphisms ( comonad_to_functor C )
  where
    reflects
      M N f i
      :=
