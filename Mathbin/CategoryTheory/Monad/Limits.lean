  ...
  lift s :=
    { f := t.lift ((forget T).mapCone s)
      h' :=
        t.hom_ext fun j => by
          dsimp
          rw [Category.assoc, Category.assoc, t.fac, newCone_π_app, ← Functor.map_comp_assoc, t.fac,
...
noncomputable def monadicCreatesColimitsOfShapeOfPreservesColimitsOfShape (R : D ⥤ C)
  ...
  haveI : PreservesColimitsOfShape J (leftAdjoint R ⋙ R) :=
    by
    ...
    infer_instance
  ⟨fun K => monadicCreatesColimitOfPreservesColimit _ _⟩
...
noncomputable def monadicCreatesColimitsOfPreservesColimits (R : D ⥤ C) [MonadicRightAdjoint R]
    ...
    where CreatesColimitsOfShape J 𝒥₁ := monadicCreatesColimitsOfShapeOfPreservesColimitsOfShape _
...
theorem hasLimitsOfReflective (R : D ⥤ C) [HasLimitsOfSize.{v, u} C] [Reflective R] :
  ...
  { HasLimitsOfShape := fun J 𝒥₁ => hasLimitsOfShapeOfReflective R }
...
theorem hasColimitsOfShapeOfReflective (R : D ⥤ C) [Reflective R] [HasColimitsOfShape J C] :
    HasColimitsOfShape J D :=
  {
    HasColimit := fun F =>
      ...
      apply (isoWhiskerLeft F (asIso (Adjunction.ofRightAdjoint R).counit) : _) ≪≫ F.right_unitor }
...
theorem hasColimitsOfReflective (R : D ⥤ C) [Reflective R] [HasColimitsOfSize.{v, u} C] :
  ...
  { HasColimitsOfShape := fun J 𝒥 => hasColimitsOfShapeOfReflective R }
...
noncomputable def leftAdjointPreservesTerminalOfReflective (R : D ⥤ C) [Reflective R] :
    ...
    fconstructor; intro c h; haveI : HasLimit (F ⋙ R) := ⟨⟨⟨c, h⟩⟩⟩
    haveI : HasLimit F := hasLimitOfReflective F R
    ...
    · apply (leftAdjoint R).mapIso
      ...
      apply (this (limit.isLimit F)).conePointUniqueUpToIso h
    infer_instance
