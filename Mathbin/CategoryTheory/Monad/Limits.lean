...
/-- The forgetful functor from the Eilenberg-Moore category creates limits. -/ noncomputable
  instance
    forget_creates_limits
...
noncomputable
  instance
    forget_creates_colimits_of_shape
    [ preserves_colimits_of_shape J ( T : C ⥤ C ) ] : creates_colimits_of_shape J ( forget T )
    where CreatesColimit K := by infer_instance

-- failed to format: format: uncaught backtrack exception
noncomputable
  instance
    forget_creates_colimits
    [ preserves_colimits ( T : C ⥤ C ) ] : creates_colimits ( forget T )
    where CreatesColimitsOfShape J 𝒥₁ := by infer_instance
...
noncomputable def monadic_creates_colimit_of_preserves_colimit (R : D ⥤ C) (K : J ⥤ D) [monadic_right_adjoint R]
    ...
    [preserves_colimit ((K ⋙ R) ⋙ left_adjoint R ⋙ R) (left_adjoint R ⋙ R)] : creates_colimit K R := by
...
noncomputable def monadic_creates_colimits_of_shape_of_preserves_colimits_of_shape (R : D ⥤ C) [monadic_right_adjoint R]
    [preserves_colimits_of_shape J R] : creates_colimits_of_shape J R := by
...
theorem has_limit_of_reflective (F : J ⥤ D) (R : D ⥤ C) [has_limit (F ⋙ R)] [reflective R] : has_limit F := by
