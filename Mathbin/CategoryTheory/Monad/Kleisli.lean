...
/--
  ...
  instance
    ...
    : category ( kleisli T )
    where
      Hom X Y : C := X ⟶ ( T : C ⥤ C ) . obj Y
        id X := T.η.app X
        comp X Y Z f g := f ≫ ( T : C ⥤ C ) . map g ≫ T.μ.app Z
        id_comp' X Y f := by rw [ ← T.η.naturality_assoc f , T.left_unit ] apply category.comp_id
        assoc'
          W X Y Z f g h
          :=
          by simp only [ functor.map_comp , category.assoc , monad.assoc ] erw [ T.μ.naturality_assoc ]
