...
instance
  ( F : lax_monoidal_functor V W ) : enriched_category W ( transport_enrichment F C )
  where
    Hom X Y : C := F.obj ( X ⟶[ V ] Y )
      id X : C := F.ε ≫ F.map ( e_id V X )
      comp X Y Z : C := F.μ _ _ ≫ F.map ( e_comp V X Y Z )
      id_comp
        X Y
        :=
        by
