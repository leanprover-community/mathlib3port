...
instance exact_pairing_unit : exact_pairing (𝟙_ C) (𝟙_ C) where
...
instance has_right_dual_unit : has_right_dual (𝟙_ C) where
  rightDual := 𝟙_ C

instance has_left_dual_unit : has_left_dual (𝟙_ C) where
  leftDual := 𝟙_ C

instance has_right_dual_left_dual {X : C} [has_left_dual X] : has_right_dual ᘁ(X) where
  rightDual := X

instance has_left_dual_right_dual {X : C} [has_right_dual X] : has_left_dual (X)ᘁ where
  leftDual := X
...
@[simp]
theorem right_adjoint_mate_id {X : C} [has_right_dual X] : 𝟙 Xᘁ = 𝟙 (Xᘁ) := by
...
@[simp]
theorem left_adjoint_mate_id {X : C} [has_left_dual X] : (ᘁ𝟙 X) = 𝟙 (ᘁX) := by
...
@[simp]
theorem left_dual_iso_id {X Y : C} (p : exact_pairing X Y) : left_dual_iso p p = iso.refl X := by
  ext
