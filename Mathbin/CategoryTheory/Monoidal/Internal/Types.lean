...
instance monMonoid (A : Mon_ (Type u)) : Monoidₓ A.x where
  ...
  one_mul := fun x => by convert congr_fun A.one_mul (PUnit.unit, x)
  mul_one := fun x => by convert congr_fun A.mul_one (x, PUnit.unit)
  mul_assoc := fun x y z => by convert congr_fun A.mul_assoc ((x, y), z)
...
def monTypeEquivalenceMon : Mon_ (Type u) ≌ Mon.{u} where
  Functor := Functor
  inverse := inverse
  unitIso := NatIso.ofComponents (fun A => { Hom := { Hom := 𝟙 _ }, inv := { Hom := 𝟙 _ } }) (by tidy)
  counitIso :=
    NatIso.ofComponents
      (fun A =>
...
instance commMonCommMonoid (A : CommMon_ (Type u)) : CommMonoidₓ A.x :=
  { MonTypeEquivalenceMon.monMonoid A.toMon_ with mul_comm := fun x y => by convert congr_fun A.mul_comm (y, x) }
