...
instance
  Mon_monoid
  ( A : Mon_ ( Type u ) ) : Monoidₓ A.X
  where
    one := A.one PUnit.unit
      mul x y := A.mul ( x , y )
      one_mul x := by convert congr_funₓ A.one_mul ( PUnit.unit , x )
      mul_one x := by convert congr_funₓ A.mul_one ( x , PUnit.unit )
      mul_assoc x y z := by convert congr_funₓ A.mul_assoc ( ( x , y ) , z )
