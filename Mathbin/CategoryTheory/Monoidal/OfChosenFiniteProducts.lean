...
-/
@[simps]
def is_limit.swap_binary_fan {P Q : C} {t : binary_fan P Q} (I : is_limit t) : is_limit t.swap where
...
def is_limit.assoc {X Y Z : C} {sXY : binary_fan X Y} (P : is_limit sXY) {sYZ : binary_fan Y Z} (Q : is_limit sYZ)
    {s : binary_fan sXY.X Z} (R : is_limit s) : is_limit (s.assoc Q) where
...
-/
@[simps]
def binary_fan.left_unitor {X : C} {s : cone (functor.empty.{v} C)} (P : is_limit s) {t : binary_fan s.X X}
    (Q : is_limit t) : t.X ≅ X where
  Hom := t.snd
  inv := Q.lift (binary_fan.mk (P.lift { x, π := { app := Pempty.rec _ } }) (𝟙 X))
  hom_inv_id' := by
    apply Q.hom_ext
    rintro ⟨⟩
    · apply P.hom_ext
...
-/
@[simps]
def binary_fan.right_unitor {X : C} {s : cone (functor.empty.{v} C)} (P : is_limit s) {t : binary_fan X s.X}
    (Q : is_limit t) : t.X ≅ X where
  Hom := t.fst
  inv := Q.lift (binary_fan.mk (𝟙 X) (P.lift { x, π := { app := Pempty.rec _ } }))
  hom_inv_id' := by
    apply Q.hom_ext
    rintro ⟨⟩
    · simp
...
variable (𝒯 : limit_cone (functor.empty.{v} C))
...
def monoidal_of_chosen_finite_products : monoidal_category C where
  tensorUnit := 𝒯.cone.X
  tensorObj := fun X Y => tensor_obj ℬ X Y
  tensorHom := fun _ _ _ _ f g => tensor_hom ℬ f g
  ...
  rightUnitor := fun X => binary_fan.right_unitor 𝒯.is_limit (ℬ X 𝒯.cone.X).IsLimit
  pentagon' := pentagon ℬ
...
def monoidal_of_chosen_finite_products_synonym (𝒯 : limit_cone (functor.empty.{v} C))
...
-/
def symmetric_of_chosen_finite_products : symmetric_category (monoidal_of_chosen_finite_products_synonym 𝒯 ℬ) where
