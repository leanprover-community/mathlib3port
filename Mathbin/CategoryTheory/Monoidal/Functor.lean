...
structure LaxMonoidalFunctor extends C ⥤ D where
  ...
  associativity' :
    ∀ X Y Z : C,
      (μ X Y ⊗ 𝟙 (assoc Z)) ≫ μ (X ⊗ Y) Z ≫ map (α_ X Y Z).hom =
        (α_ (obj X) (obj Y) (assoc Z)).hom ≫ (𝟙 (obj X) ⊗ μ Y Z) ≫ μ X (Y ⊗ Z) := by
    obviously
  -- unitality
...
noncomputable def monoidalAdjoint (F : MonoidalFunctor C D) {G : D ⥤ C} (h : F.toFunctor ⊣ G) :
  ...
  μ_natural' X Y X' Y' f g := by
    rw [← h.hom_equiv_naturality_left, ← h.hom_equiv_naturality_right, Equiv.apply_eq_iff_eq, assoc,
      IsIso.eq_inv_comp, ← F.to_lax_monoidal_functor.μ_natural_assoc, IsIso.hom_inv_id_assoc, ←
  ...
  associativity' X Y Z :=
    ...
    rw [← h.hom_equiv_naturality_right, ← h.hom_equiv_naturality_left, ←
      ...
      F.to_lax_monoidal_functor.μ_natural_assoc, assoc, IsIso.hom_inv_id_assoc, ←
      F.to_lax_monoidal_functor.μ_natural_assoc, IsIso.hom_inv_id_assoc, ← tensor_comp, ←
      ...
      assoc, assoc, h.counit_naturality, h.left_triangle_components_assoc, IsIso.hom_inv_id_assoc,
  ...
  right_unitality' X := by
    rw [← h.hom_equiv_naturality_right, ← h.hom_equiv_naturality_left, ← Equiv.symm_apply_eq,
      h.hom_equiv_counit, F.map_right_unitor, assoc, assoc, ← rightUnitor_naturality, ←
