...
instance : category ( Mon_ C ) where Hom M N := hom M N id := id comp M N O f g := comp f g
...
-- failed to format: format: uncaught backtrack exception
instance
  unique_hom_from_trivial
  ( A : Mon_ C ) : Unique ( trivialₓ C ⟶ A )
  where
    default := { Hom := A.one , one_hom' := by dsimp simp , mul_hom' := by dsimp simp [ A.one_mul , unitors_equal ] }
      uniq f := by ext simp rw [ ← category.id_comp f.hom ] erw [ f.one_hom ]
