...
instance uniqueHomFromTrivial (A : Mon_ C) :
    Unique
      (trivial C ⟶
        A) where 
  default :=
...
@[simps]
def laxMonoidalToMon :
    LaxMonoidalFunctor (Discrete PUnit.{u + 1}) C ⥤
      Mon_
        C where 
...
-/
@[simps]
def equivLaxMonoidalFunctorPunit :
    LaxMonoidalFunctor (Discrete PUnit.{u + 1}) C ≌
      Mon_ C where 
