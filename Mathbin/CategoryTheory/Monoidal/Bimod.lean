...
theorem id_tensor_π_preserves_coequalizer_inv_colimMap_desc {X Y Z X' Y' Z' : C} (f g : X ⟶ Y)
    (f' g' : assoc ⟶ Y') (p : Z ⊗ X ⟶ X') (q : Z ⊗ Y ⟶ Y') (wf : (𝟙 Z ⊗ f) ≫ q = p ≫ f')
...
@[simps]
def regular : Bimod comp_whiskerRight id_whiskerRight
    where
  x := A.x
  actLeft := A.mul
  actRight := A.mul
...
theorem right_assoc' :
  ...
  slice_rhs 1 2 => rw [← MonoidalCategory.tensor_id, associator_inv_naturality]
...
theorem tensor_id {X Y Z : Mon_ C} {M : Bimod X Y} {N : Bimod Y Z} :
  ...
  by
  ext
  simp only [id_hom', tensor_id, tensorHom_hom, ι_colimMap, parallelPairHom_app_one]
...
theorem tensor_comp {X Y Z : Mon_ C} {M₁ M₂ M₃ : Bimod X Y} {N₁ N₂ N₃ : Bimod Y Z} (f₁ : M₁ ⟶ M₂)
  ...
  by
  ext
  simp only [comp_hom', tensor_comp, tensorHom_hom, ι_colimMap, parallelPairHom_app_one,
    Category.assoc, ι_colimMap_assoc]
...
theorem hom_left_act_hom' :
  ...
  slice_rhs 3 4 => erw [TensorBimod.id_tensor_π_actLeft P (Q.tensor_Bimod L)]
...
theorem hom_right_act_hom' :
  ...
  slice_rhs 3 4 => erw [TensorBimod.π_tensor_id_actRight P (Q.tensor_Bimod L)]
  ...
  (PreservesCoequalizer.iso (tensorLeft P.x) _ _).inv ≫
    coequalizer.desc ((α_ _ _ _).inv ≫ (coequalizer.π _ _ ⊗ 𝟙 L.x) ≫ coequalizer.π _ _)
      (by
        ...
        slice_lhs 3 4 =>
          rw [← comp_tensor_id, Category.assoc, ← TensorBimod.π_tensor_id_actRight, comp_tensor_id]
...
theorem pentagon_bimod {V W X Y Z : Mon_ C} (M : Bimod V W) (N : Bimod W X) (P : Bimod X Y)
  ...
  by
  dsimp [tensorHom, associatorBimod]; ext; dsimp
