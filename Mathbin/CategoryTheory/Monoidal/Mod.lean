...
-/
@[simps]
def comap {A B : Mon_ C} (f : A ⟶ B) : Modₓ B ⥤ Modₓ A :=
  { obj :=
      fun M =>
        { x := M.X, act := (f.hom ⊗ 𝟙 M.X) ≫ M.act,
          one_act' :=
            by 
              sliceLHS 1 2 => rw [←comp_tensor_id]
              rw [f.one_hom, one_act],
          assoc' :=
            by 
              sliceRHS 2 3 => rw [id_tensor_comp_tensor_id, ←tensor_id_comp_id_tensor]
              rw [id_tensor_comp]
              sliceRHS 4 5 => rw [Modₓ.assoc_flip]
              sliceRHS 3 4 => rw [associator_inv_naturality]
              sliceRHS 2 3 => rw [←tensor_id, associator_inv_naturality]
              sliceRHS 1 3 => rw [iso.hom_inv_id_assoc]
              sliceRHS 1 2 => rw [←comp_tensor_id, tensor_id_comp_id_tensor]
              sliceRHS 1 2 => rw [←comp_tensor_id, ←f.mul_hom]
      ...
      fun M N g =>
        { Hom := g.hom,
          act_hom' :=
            by 
              dsimp 
              sliceRHS 1 2 => rw [id_tensor_comp_tensor_id, ←tensor_id_comp_id_tensor]
              sliceRHS 2 3 => rw [←g.act_hom]
              rw [category.assoc] } }
