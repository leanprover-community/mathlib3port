...
instance functorHasRightDual [RightRigidCategory D] (F : C ⥤ D) : HasRightDual F
  ...
  rightDual :=
    { obj := fun X => F.obj Xᘁ
      ...
      map_comp' := fun X Y Z f g => by simp [comp_rightAdjointMate] }
  exact :=
    { evaluation :=
        { app := fun X => ε_ _ _
...
instance functorHasLeftDual [LeftRigidCategory D] (F : C ⥤ D) : HasLeftDual F
  ...
  leftDual :=
    { obj := fun X => ᘁF.obj X
      ...
      map_comp' := fun X Y Z f g => by simp [comp_leftAdjointMate] }
  exact :=
    { evaluation :=
        { app := fun X => ε_ _ _
          naturality' := fun X Y f => by
            ...
            rw [Category.comp_id, Functor.map_inv, ← tensor_id_comp_id_tensor, Category.assoc,
              leftAdjointMate_comp_evaluation, ← Category.assoc, ← comp_tensor_id, IsIso.hom_inv_id,
        ...
        { app := fun X => η_ _ _
          naturality' := fun X Y f => by
            ...
            rw [Functor.map_inv, Category.id_comp, ← tensor_id_comp_id_tensor, ← Category.assoc,
              coevaluation_comp_leftAdjointMate, Category.assoc, ← id_tensor_comp, IsIso.inv_hom_id,
              tensor_id, Category.comp_id] } }
