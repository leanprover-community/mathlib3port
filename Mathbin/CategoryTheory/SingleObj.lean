...
-/
instance groupoid [Groupₓ α] : groupoid (single_obj α) where
  inv := fun _ _ x => x⁻¹
...
-/
def map_hom (α : Type u) (β : Type v) [Monoidₓ α] [Monoidₓ β] : (α →* β) ≃ single_obj α ⥤ single_obj β where
  toFun := fun f =>
...
@[simp]
theorem to_Aut_inv (x : (α)ˣ) : (to_Aut α x).inv = single_obj.to_End α (x⁻¹ : (α)ˣ) :=
  rfl
