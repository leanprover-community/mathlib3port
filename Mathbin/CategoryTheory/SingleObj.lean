...
/-- One and `flip (*)` become `id` and `comp` for morphisms of the single object category. -/
  instance
    category_struct
    [ HasOne α ] [ Mul α ] : category_struct ( single_obj α )
    where Hom _ _ := α comp _ _ _ x y := y * x id _ := 1

-- failed to format: format: uncaught backtrack exception
/-- Monoid laws become category laws for the single object category. -/
  instance
    category
    [ Monoidₓ α ] : category ( single_obj α )
    where comp_id' _ _ := one_mulₓ id_comp' _ _ := mul_oneₓ assoc' _ _ _ _ x y z := ( mul_assocₓ z y x ) . symm
...
/--
  ...
  instance
    groupoid
    [ Groupₓ α ] : groupoid ( single_obj α )
    where inv _ _ x := x ⁻¹ inv_comp' _ _ := mul_right_invₓ comp_inv' _ _ := mul_left_invₓ

theorem inv_as_inv [Groupₓ α] {x y : single_obj α} (f : x ⟶ y) : inv f = f⁻¹ := by
  ext
...
instance
  to_Cat_full
  : full to_Cat
  where Preimage x y := ( single_obj.map_hom x y ) . invFun witness' x y := by apply Equivₓ.right_inv

-- failed to format: format: uncaught backtrack exception
instance to_Cat_faithful : faithful to_Cat where map_injective' x y := by apply Equivₓ.injective
