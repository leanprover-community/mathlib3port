...
def adjoint_of_op_adjoint_op (F : C ⥤ D) (G : D ⥤ C) (h : G.op ⊣ F.op) : F ⊣ G :=
  adjunction.mk_of_hom_equiv
    { homEquiv :=
        fun X Y => ((h.hom_equiv (Opposite.op Y) (Opposite.op X)).trans (op_equiv _ _)).symm.trans (op_equiv _ _) }

/-- If `G` is adjoint to `F.op` then `F` is adjoint to `G.unop`. -/
def adjoint_unop_of_adjoint_op (F : C ⥤ D) (G : «expr ᵒᵖ» D ⥤ «expr ᵒᵖ» C) (h : G ⊣ F.op) : F ⊣ G.unop :=
...
def unop_adjoint_of_op_adjoint (F : «expr ᵒᵖ» C ⥤ «expr ᵒᵖ» D) (G : D ⥤ C) (h : G.op ⊣ F) : F.unop ⊣ G :=
  adjoint_of_op_adjoint_op _ _ (h.of_nat_iso_right F.op_unop_iso.symm)

/-- If `G` is adjoint to `F` then `F.unop` is adjoint to `G.unop`. -/
def unop_adjoint_unop_of_adjoint (F : «expr ᵒᵖ» C ⥤ «expr ᵒᵖ» D) (G : «expr ᵒᵖ» D ⥤ «expr ᵒᵖ» C) (h : G ⊣ F) :
  F.unop ⊣ G.unop :=
...
def adjoint_op_of_adjoint_unop (F : «expr ᵒᵖ» C ⥤ «expr ᵒᵖ» D) (G : D ⥤ C) (h : G ⊣ F.unop) : F ⊣ G.op :=
  (op_adjoint_op_of_adjoint F.unop _ h).ofNatIsoLeft F.op_unop_iso

/-- If `G.unop` is adjoint to `F` then `F.op` is adjoint to `G`. -/
def op_adjoint_of_unop_adjoint (F : C ⥤ D) (G : «expr ᵒᵖ» D ⥤ «expr ᵒᵖ» C) (h : G.unop ⊣ F) : F.op ⊣ G :=
  (op_adjoint_op_of_adjoint _ G.unop h).ofNatIsoRight G.op_unop_iso

/-- If `G.unop` is adjoint to `F.unop` then `F` is adjoint to `G`. -/
def adjoint_of_unop_adjoint_unop (F : «expr ᵒᵖ» C ⥤ «expr ᵒᵖ» D) (G : «expr ᵒᵖ» D ⥤ «expr ᵒᵖ» C) (h : G.unop ⊣ F.unop) :
  F ⊣ G :=
