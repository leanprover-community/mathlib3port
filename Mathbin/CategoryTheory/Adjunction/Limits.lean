...
instance
  ( priority := 100 )
  is_equivalence_reflects_colimits
                    ...
                    reflects
                      ...
                      fun
                        c t
                          ...
                          by
                            have
                                l := ( is_colimit_of_preserves E.inv t ) . mapCoconeEquiv E.as_equivalence.unit_iso.symm
...
instance
  ( priority := 100 )
  is_equivalence_creates_colimits
  ( H : D ⥤ C ) [ is_equivalence H ] : creates_colimits H
  where
              ...
              fun
                F
                  =>
                  {
                    lifts
                      :=
                      fun c t => { liftedCocone := H.map_cocone_inv c , validLift := H.map_cocone_map_cocone_inv c }
                    }
            }
...
instance
  ( priority := 100 )
  is_equivalence_reflects_limits
  ( E : D ⥤ C ) [ is_equivalence E ] : reflects_limits E
  where
        ...
        exact
          {
            CreatesLimit
              :=
              fun
                F => { lifts := fun c t => { liftedCone := H.map_cone_inv c , validLift := H.map_cone_map_cone_inv c } }
            }
