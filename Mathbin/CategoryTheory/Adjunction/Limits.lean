...
-/
@[simps]
def functoriality_unit : 𝟭 (cocone K) ⟶ cocones.functoriality _ F ⋙ functoriality_right_adjoint adj K where
...
-/
@[simps]
def functoriality_counit : functoriality_right_adjoint adj K ⋙ cocones.functoriality _ F ⟶ 𝟭 (cocone (K ⋙ F)) where
  app := fun c => { Hom := adj.counit.app c.X }

/-- The functor `cocones.functoriality K F : cocone K ⥤ cocone (K ⋙ F)` is a left adjoint. -/
def functoriality_is_left_adjoint : is_left_adjoint (cocones.functoriality K F) where
...
-/
def left_adjoint_preserves_colimits : preserves_colimits_of_size.{v, u} F where
...
instance (priority := 100) is_equivalence_preserves_colimits (E : C ⥤ D) [is_equivalence E] :
    preserves_colimits_of_size.{v, u} E :=
  left_adjoint_preserves_colimits E.adjunction

instance (priority := 100) is_equivalence_reflects_colimits (E : D ⥤ C) [is_equivalence E] :
    reflects_colimits_of_size.{v, u} E where
...
instance (priority := 100) is_equivalence_creates_colimits (H : D ⥤ C) [is_equivalence H] :
    creates_colimits_of_size.{v, u} H where
...
theorem has_colimits_of_equivalence (E : C ⥤ D) [is_equivalence E] [has_colimits_of_size.{v, u} D] :
...
-/
@[simps]
def functoriality_unit' : 𝟭 (cone (K ⋙ G)) ⟶ functoriality_left_adjoint adj K ⋙ cones.functoriality _ G where
  app := fun c => { Hom := adj.unit.app c.X }
...
-/
@[simps]
def functoriality_counit' : cones.functoriality _ G ⋙ functoriality_left_adjoint adj K ⟶ 𝟭 (cone K) where
  app := fun c => { Hom := adj.counit.app c.X }

/-- The functor `cones.functoriality K G : cone K ⥤ cone (K ⋙ G)` is a right adjoint. -/
def functoriality_is_right_adjoint : is_right_adjoint (cones.functoriality K G) where
...
See https://stacks.math.columbia.edu/tag/0038.
-/
def right_adjoint_preserves_limits : preserves_limits_of_size.{v, u} G where
...
instance (priority := 100) is_equivalence_preserves_limits (E : D ⥤ C) [is_equivalence E] :
    preserves_limits_of_size.{v, u} E :=
  right_adjoint_preserves_limits E.inv.adjunction

instance (priority := 100) is_equivalence_reflects_limits (E : D ⥤ C) [is_equivalence E] :
    reflects_limits_of_size.{v, u} E where
...
instance (priority := 100) is_equivalence_creates_limits (H : D ⥤ C) [is_equivalence H] :
    creates_limits_of_size.{v, u} H where
...
theorem has_limits_of_equivalence (E : D ⥤ C) [is_equivalence E] [has_limits_of_size.{v, u} C] :
...
@[simps]
def cocones_iso_component_hom {J : Type u} [category.{v} J] {K : J ⥤ C} (Y : D)
...
/-- auxiliary construction for `cocones_iso` -/
@[simps]
def cocones_iso_component_inv {J : Type u} [category.{v} J] {K : J ⥤ C} (Y : D)
...
@[simps]
def cones_iso_component_hom {J : Type u} [category.{v} J] {K : J ⥤ D} (X : Cᵒᵖ)
...
/-- auxiliary construction for `cones_iso` -/
@[simps]
def cones_iso_component_inv {J : Type u} [category.{v} J] {K : J ⥤ D} (X : Cᵒᵖ) (t : ((cones J C).obj (K ⋙ G)).obj X) :
...
-/
def cones_iso {J : Type u} [category.{v} J] {K : J ⥤ D} : F.op ⋙ (cones J D).obj K ≅ (cones J C).obj (K ⋙ G) :=
