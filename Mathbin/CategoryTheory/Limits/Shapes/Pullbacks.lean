...
instance (X Y : walking_cospan) : Subsingleton (X ⟶ Y) := by
  tidy
...
instance (X Y : walking_span) : Subsingleton (X ⟶ Y) := by
  tidy
...
@[simp]
theorem walking_cospan_functor_one : walking_cospan_functor.obj one = one :=
  rfl

@[simp]
theorem walking_cospan_functor_left : walking_cospan_functor.obj left = left :=
  rfl

@[simp]
theorem walking_cospan_functor_right : walking_cospan_functor.obj right = right :=
  rfl

@[simp]
theorem walking_cospan_functor_id X : walking_cospan_functor.map (𝟙 X) = 𝟙 _ :=
  rfl

@[simp]
theorem walking_cospan_functor_inl : walking_cospan_functor.map hom.inl = hom.inl :=
  rfl

@[simp]
theorem walking_cospan_functor_inr : walking_cospan_functor.map hom.inr = hom.inr :=
  rfl
...
@[simp]
theorem walking_span_functor_zero : walking_span_functor.obj zero = zero :=
  rfl

@[simp]
theorem walking_span_functor_left : walking_span_functor.obj left = left :=
  rfl

@[simp]
theorem walking_span_functor_right : walking_span_functor.obj right = right :=
  rfl

@[simp]
theorem walking_span_functor_id X : walking_span_functor.map (𝟙 X) = 𝟙 _ :=
  rfl

@[simp]
theorem walking_span_functor_fst : walking_span_functor.map hom.fst = hom.fst :=
  rfl

@[simp]
theorem walking_span_functor_snd : walking_span_functor.map hom.snd = hom.snd :=
  rfl
...
@[simps]
def mk {W : C} (fst : W ⟶ X) (snd : W ⟶ Y) (eq : fst ≫ f = snd ≫ g) : pullback_cone f g where
  x := W
  π := { app := fun j => Option.casesOn j (fst ≫ f) fun j' => walking_pair.cases_on j' fst snd }
...
def is_colimit_aux' (t : pushout_cocone f g)
    ...
    is_colimit t :=
  is_colimit_aux t (fun s => (create s).1) (fun s => (create s).2.1) (fun s => (create s).2.2.1) fun s m w =>
    (create s).2.2.2 (w walking_cospan.left) (w walking_cospan.right)

/-- A pushout cocone on `f` and `g` is determined by morphisms `inl : Y ⟶ W` and `inr : Z ⟶ W` such
    that `f ≫ inl = g ↠ inr`. -/
@[simps]
def mk {W : C} (inl : Y ⟶ W) (inr : Z ⟶ W) (eq : f ≫ inl = g ≫ inr) : pushout_cocone f g where
  x := W
  ι := { app := fun j => Option.casesOn j (f ≫ inl) fun j' => walking_pair.cases_on j' inl inr }
...
@[simps]
def cone.of_pullback_cone {F : walking_cospan ⥤ C} (t : pullback_cone (F.map inl) (F.map inr)) : cone F where
  x := t.X
...
@[simps]
def cocone.of_pushout_cocone {F : walking_span ⥤ C} (t : pushout_cocone (F.map fst) (F.map snd)) : cocone F where
  x := t.X
...
@[simps]
def pullback_cone.of_cone {F : walking_cospan ⥤ C} (t : cone F) : pullback_cone (F.map inl) (F.map inr) where
  x := t.X
...
@[simps]
def pushout_cocone.of_cocone {F : walking_span ⥤ C} (t : cocone F) : pushout_cocone (F.map fst) (F.map snd) where
  x := t.X
...
instance pullback.map_is_iso {W X Y Z S T : C} (f₁ : W ⟶ S) (f₂ : X ⟶ S) [has_pullback f₁ f₂] (g₁ : Y ⟶ T) (g₂ : Z ⟶ T)
    [has_pullback g₁ g₂] (i₁ : W ⟶ Y) (i₂ : X ⟶ Z) (i₃ : S ⟶ T) (eq₁ : f₁ ≫ i₃ = i₁ ≫ g₁) (eq₂ : f₂ ≫ i₃ = i₂ ≫ g₂)
    [is_iso i₁] [is_iso i₂] [is_iso i₃] : is_iso (pullback.map f₁ f₂ g₁ g₂ i₁ i₂ i₃ eq₁ eq₂) := by
...
instance pushout.map_is_iso {W X Y Z S T : C} (f₁ : S ⟶ W) (f₂ : S ⟶ X) [has_pushout f₁ f₂] (g₁ : T ⟶ Y) (g₂ : T ⟶ Z)
    [has_pushout g₁ g₂] (i₁ : W ⟶ Y) (i₂ : X ⟶ Z) (i₃ : S ⟶ T) (eq₁ : f₁ ≫ i₁ = i₃ ≫ g₁) (eq₂ : f₂ ≫ i₂ = i₃ ≫ g₂)
    [is_iso i₁] [is_iso i₂] [is_iso i₃] : is_iso (pushout.map f₁ f₂ g₁ g₂ i₁ i₂ i₃ eq₁ eq₂) := by
...
@[simp, reassoc]
theorem inl_comp_pushout_symmetry_inv [has_pushout f g] : pushout.inl ≫ (pushout_symmetry f g).inv = pushout.inr := by
  simp [iso.comp_inv_eq]

@[simp, reassoc]
theorem inr_comp_pushout_symmetry_inv [has_pushout f g] : pushout.inr ≫ (pushout_symmetry f g).inv = pushout.inl := by
  simp [iso.comp_inv_eq]
...
@[simp]
theorem pullback_cone_of_left_iso_π_app_none : (pullback_cone_of_left_iso f g).π.app none = g := by
...
instance pullback_snd_iso_of_left_iso : is_iso (pullback.snd : pullback f g ⟶ _) := by
  refine'
...
instance has_pullback_of_right_factors_mono (f : X ⟶ Z) : has_pullback i (f ≫ i) := by
  nth_rw 0[← category.id_comp i]
  infer_instance

instance pullback_snd_iso_of_right_factors_mono (f : X ⟶ Z) : is_iso (pullback.snd : pullback i (f ≫ i) ⟶ _) := by
  convert
      (congr_argₓ is_iso
...
instance pullback_snd_iso_of_right_iso : is_iso (pullback.fst : pullback f g ⟶ _) := by
  refine'
...
instance has_pullback_of_left_factors_mono (f : X ⟶ Z) : has_pullback (f ≫ i) i := by
  nth_rw 1[← category.id_comp i]
  infer_instance

instance pullback_snd_iso_of_left_factors_mono (f : X ⟶ Z) : is_iso (pullback.fst : pullback (f ≫ i) i ⟶ _) := by
  convert
      (congr_argₓ is_iso
...
@[simp]
theorem pushout_cocone_of_left_iso_ι_app_none : (pushout_cocone_of_left_iso f g).ι.app none = g := by
...
def pushout_cocone_of_left_iso_is_limit : is_colimit (pushout_cocone_of_left_iso f g) :=
  pushout_cocone.is_colimit_aux' _ fun s =>
    ⟨s.inr, by
      simp [← s.condition]⟩
...
instance pushout_inr_iso_of_left_iso : is_iso (pushout.inr : _ ⟶ pushout f g) := by
  refine'
...
instance has_pushout_of_right_factors_epi (f : X ⟶ Y) : has_pushout h (h ≫ f) := by
  nth_rw 0[← category.comp_id h]
  infer_instance

instance pushout_inr_iso_of_right_factors_epi (f : X ⟶ Y) : is_iso (pushout.inr : _ ⟶ pushout h (h ≫ f)) := by
  convert
      (congr_argₓ is_iso
...
@[simp]
theorem pushout_cocone_of_right_iso_ι_app_none : (pushout_cocone_of_right_iso f g).ι.app none = f := by
...
instance pushout_inl_iso_of_right_iso : is_iso (pushout.inl : _ ⟶ pushout f g) := by
  refine'
...
instance has_pushout_of_left_factors_epi (f : X ⟶ Y) : has_pushout (h ≫ f) h := by
  nth_rw 1[← category.comp_id h]
  infer_instance

instance pushout_inl_iso_of_left_factors_epi (f : X ⟶ Y) : is_iso (pushout.inl : _ ⟶ pushout (h ≫ f) h) := by
  convert
      (congr_argₓ is_iso
...
@[simp]
theorem pullback_symmetry_hom_of_mono_eq [mono f] : (pullback_symmetry f f).Hom = 𝟙 _ := by
  ext <;> simp [fst_eq_snd_of_mono_eq]

instance fst_iso_of_mono_eq [mono f] : is_iso (pullback.fst : pullback f f ⟶ _) := by
  refine'
...
instance snd_iso_of_mono_eq [mono f] : is_iso (pullback.snd : pullback f f ⟶ _) := by
...
@[simp]
theorem pullback_symmetry_hom_of_epi_eq [epi f] : (pushout_symmetry f f).Hom = 𝟙 _ := by
  ext <;> simp [inl_eq_inr_of_epi_eq]

instance inl_iso_of_epi_eq [epi f] : is_iso (pushout.inl : _ ⟶ pushout f f) := by
  refine'
...
instance inr_iso_of_epi_eq [epi f] : is_iso (pushout.inr : _ ⟶ pushout f f) := by
    ...
    (H' :
      is_limit
        (pullback_cone.mk _ _
          (show i₁ ≫ g₁ ≫ g₂ = (f₁ ≫ f₂) ≫ i₃ by
            rw [← category.assoc, h₁, category.assoc, h₂, category.assoc]))) :
    is_limit (pullback_cone.mk _ _ h₁) := by
  fapply pullback_cone.is_limit_aux'
  intro s
    ...
    (H' :
      is_colimit
        (pushout_cocone.mk _ _
          (show i₁ ≫ g₁ ≫ g₂ = (f₁ ≫ f₂) ≫ i₃ by
            rw [← category.assoc, h₁, category.assoc, h₂, category.assoc]))) :
    is_colimit (pushout_cocone.mk _ _ h₂) := by
  fapply pushout_cocone.is_colimit_aux'
  intro s
...
theorem pullback_right_pullback_fst_iso_inv_snd_fst :
    (pullback_right_pullback_fst_iso f g f').inv ≫ pullback.snd ≫ pullback.fst = pullback.fst ≫ f' := by
...
theorem inl_inl_pushout_left_pushout_inr_iso_hom :
    pushout.inl ≫ pushout.inl ≫ (pushout_left_pushout_inr_iso f g g').Hom = pushout.inl := by
  rw [← category.assoc, ← iso.eq_comp_inv, inl_pushout_left_pushout_inr_iso_inv]

@[simp, reassoc]
theorem inr_inl_pushout_left_pushout_inr_iso_hom :
    pushout.inr ≫ pushout.inl ≫ (pushout_left_pushout_inr_iso f g g').Hom = g' ≫ pushout.inr := by
...
def pullback_pullback_left_is_pullback [has_pullback (g₂ ≫ f₃) f₄] :
    is_limit (pullback_cone.mk l₁ l₂ (show l₁ ≫ g₂ = l₂ ≫ g₃ from (pullback.lift_fst _ _ _).symm)) := by
...
def pullback_pullback_right_is_pullback [has_pullback f₁ (g₃ ≫ f₂)] :
    is_limit (pullback_cone.mk l₁' l₂' (show l₁' ≫ g₂ = l₂' ≫ g₃ from pullback.lift_snd _ _ _)) := by
  apply pullback_cone.flip_is_limit
  apply left_square_is_pullback
  · apply pullback_cone.flip_is_limit
    exact pullback_is_pullback f₁ f₂
...
@[simp, reassoc]
theorem pullback_assoc_inv_fst_fst : (pullback_assoc f₁ f₂ f₃ f₄).inv ≫ pullback.fst ≫ pullback.fst = pullback.fst := by
...
@[simp, reassoc]
theorem pullback_assoc_hom_fst : (pullback_assoc f₁ f₂ f₃ f₄).Hom ≫ pullback.fst = pullback.fst ≫ pullback.fst := by
  rw [← iso.eq_inv_comp, pullback_assoc_inv_fst_fst]

@[simp, reassoc]
theorem pullback_assoc_hom_snd_fst :
    (pullback_assoc f₁ f₂ f₃ f₄).Hom ≫ pullback.snd ≫ pullback.fst = pullback.fst ≫ pullback.snd := by
  trans l₂ ≫ pullback.fst
  rw [← category.assoc]
  congr 1
...
@[simp, reassoc]
theorem pullback_assoc_hom_snd_snd : (pullback_assoc f₁ f₂ f₃ f₄).Hom ≫ pullback.snd ≫ pullback.snd = pullback.snd := by
...
theorem pullback_assoc_inv_fst_snd :
    (pullback_assoc f₁ f₂ f₃ f₄).inv ≫ pullback.fst ≫ pullback.snd = pullback.snd ≫ pullback.fst := by
  rw [iso.inv_comp_eq, pullback_assoc_hom_snd_fst]

@[simp, reassoc]
theorem pullback_assoc_inv_snd : (pullback_assoc f₁ f₂ f₃ f₄).inv ≫ pullback.snd = pullback.snd ≫ pullback.snd := by
...
def pushout_pushout_left_is_pushout [has_pushout (g₃ ≫ f₂) g₄] :
    is_colimit (pushout_cocone.mk l₁' l₂' (show f₂ ≫ l₁' = f₃ ≫ l₂' from (pushout.inl_desc _ _ _).symm)) := by
...
def pushout_pushout_right_is_pushout [has_pushout g₁ (g₂ ≫ f₃)] :
    is_colimit (pushout_cocone.mk l₁ l₂ (show f₂ ≫ l₁ = f₃ ≫ l₂ from pushout.inr_desc _ _ _)) := by
  apply right_square_is_pushout
  · exact pushout_is_pushout _ _
...
@[simp, reassoc]
theorem inl_inl_pushout_assoc_hom : pushout.inl ≫ pushout.inl ≫ (pushout_assoc g₁ g₂ g₃ g₄).Hom = pushout.inl := by
  trans f₁ ≫ l₁
  · congr 1
...
theorem inr_inl_pushout_assoc_hom :
    pushout.inr ≫ pushout.inl ≫ (pushout_assoc g₁ g₂ g₃ g₄).Hom = pushout.inl ≫ pushout.inr := by
  trans f₂ ≫ l₁
  · congr 1
    exact (pushout_pushout_left_is_pushout g₁ g₂ g₃ g₄).comp_cocone_point_unique_up_to_iso_hom _ walking_cospan.left
...
@[simp, reassoc]
theorem inr_inr_pushout_assoc_inv : pushout.inr ≫ pushout.inr ≫ (pushout_assoc g₁ g₂ g₃ g₄).inv = pushout.inr := by
  trans f₄ ≫ l₂'
  · congr 1
    exact
...
@[simp, reassoc]
theorem inl_pushout_assoc_inv : pushout.inl ≫ (pushout_assoc g₁ g₂ g₃ g₄).inv = pushout.inl ≫ pushout.inl := by
  rw [iso.comp_inv_eq, category.assoc, inl_inl_pushout_assoc_hom]

@[simp, reassoc]
theorem inl_inr_pushout_assoc_inv :
    pushout.inl ≫ pushout.inr ≫ (pushout_assoc g₁ g₂ g₃ g₄).inv = pushout.inr ≫ pushout.inl := by
  rw [← category.assoc, iso.comp_inv_eq, category.assoc, inr_inl_pushout_assoc_hom]

@[simp, reassoc]
theorem inr_pushout_assoc_hom : pushout.inr ≫ (pushout_assoc g₁ g₂ g₃ g₄).Hom = pushout.inr ≫ pushout.inr := by
