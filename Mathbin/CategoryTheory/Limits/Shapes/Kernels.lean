...
def isKernelOfComp {W : C} (g : Y ⟶ W) (h : X ⟶ W) {c : KernelFork h} (i : IsLimit c)
  ...
  Fork.IsLimit.mk _ (fun s => i.lift (KernelFork.ofι s.ι (by simp [← hfg])))
    (fun s => by simp only [KernelFork.ι_ofι, Fork.IsLimit.lift_ι]) fun s m h =>
    by
    ...
    cokernel.desc _ (cokernel.π f)
      (by
        ...
        conv at w =>
          ...
          congr
          rw [← image.fac f]
        rw [← HasZeroMorphisms.comp_zero (Limits.factorThruImage f), Category.assoc, cancel_epi] at
          w
        exact w)
  inv :=
...
def zeroCokernelOfZeroCancel {X Y : C} (f : X ⟶ Y)
  ...
  Cofork.IsColimit.mk _ (fun s => 0)
    (fun s => by rw [hf _ _ (CokernelCofork.condition s), comp_zero]) fun s m h => by ext
