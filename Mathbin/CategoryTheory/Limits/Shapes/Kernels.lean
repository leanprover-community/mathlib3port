...
@[simp]
theorem kernel_fork.app_one (s : kernel_fork f) : s.π.app one = 0 :=
  by 
...
theorem kernel.lift_map {X Y Z X' Y' Z' : C} (f : X ⟶ Y) (g : Y ⟶ Z) [has_kernel g] (w : f ≫ g = 0) (f' : X' ⟶ Y')
  ...
  (h₂ : g ≫ r = q ≫ g') : kernel.lift g f w ≫ kernel.map g g' q r h₂ = p ≫ kernel.lift g' f' w' :=
  by 
    ext 
    simp [h₁]
...
@[simp]
theorem kernel_iso_of_eq_refl {h : f = f} : kernel_iso_of_eq h = iso.refl (kernel f) :=
  by 
    ext 
...
instance has_kernel_iso_comp {X Y Z : C} (f : X ⟶ Y) (g : Y ⟶ Z) [is_iso f] [has_kernel g] : has_kernel (f ≫ g) :=
  { exists_limit :=
      ⟨{ Cone :=
...
@[simp]
theorem cokernel_cofork.app_zero (s : cokernel_cofork f) : s.ι.app zero = 0 :=
  by 
...
theorem cokernel.map_desc {X Y Z X' Y' Z' : C} (f : X ⟶ Y) [has_cokernel f] (g : Y ⟶ Z) (w : f ≫ g = 0) (f' : X' ⟶ Y')
  ...
  (h₂ : g ≫ r = q ≫ g') : cokernel.map f f' p q h₁ ≫ cokernel.desc f' g' w' = cokernel.desc f g w ≫ r :=
  by 
    ext 
    simp [h₂]
...
@[simp]
theorem cokernel_iso_of_eq_refl {h : f = f} : cokernel_iso_of_eq h = iso.refl (cokernel f) :=
  by 
    ext 
...
instance has_cokernel_epi_comp {X Y Z : C} (f : X ⟶ Y) [epi f] (g : Y ⟶ Z) [has_cokernel g] : has_cokernel (f ≫ g) :=
  { exists_colimit :=
      ⟨{ Cocone :=
