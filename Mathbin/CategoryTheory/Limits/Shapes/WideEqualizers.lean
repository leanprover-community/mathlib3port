...
/-- A wide equalizer morphism is a monomorphism -/
  instance
    ...
    [ Nonempty J ] : mono ( wide_equalizer.ι f )
    where right_cancellation Z h k w := wide_equalizer.hom_ext w
...
/-- A wide coequalizer morphism is an epimorphism -/
  instance
    wide_coequalizer.π_epi
    [ Nonempty J ] : epi ( wide_coequalizer.π f )
    where left_cancellation Z h k w := wide_coequalizer.hom_ext w
