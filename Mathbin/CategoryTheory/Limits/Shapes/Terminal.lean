...
abbrev is_terminal (X : C) :=
  is_limit (as_empty_cone.{v₁} X)

/-- `X` is initial if the cocone it induces on the empty diagram is colimiting. -/
abbrev is_initial (X : C) :=
  is_colimit (as_empty_cocone.{v₁} X)

/-- An object `Y` is terminal if for every `X` there is a unique morphism `X ⟶ Y`. -/
def is_terminal.of_unique (Y : C) [h : ∀ X : C, Unique (X ⟶ Y)] : is_terminal Y where
...
def is_initial.of_unique (X : C) [h : ∀ Y : C, Unique (X ⟶ Y)] : is_initial X where
...
theorem is_terminal.mono_from {X Y : C} (t : is_terminal X) (f : X ⟶ Y) : mono f := by
  have := t.split_mono_from f <;> infer_instance

/-- Any morphism to an initial object is epi. -/
theorem is_initial.epi_to {X Y : C} (t : is_initial X) (f : Y ⟶ X) : epi f := by
  have := t.split_epi_to f <;> infer_instance

/-- If `T` and `T'` are terminal, they are isomorphic. -/
@[simps]
def is_terminal.unique_up_to_iso {T T' : C} (hT : is_terminal T) (hT' : is_terminal T') : T ≅ T' where
  Hom := hT'.from _
  inv := hT.from _

/-- If `I` and `I'` are initial, they are isomorphic. -/
@[simps]
def is_initial.unique_up_to_iso {I I' : C} (hI : is_initial I) (hI' : is_initial I') : I ≅ I' where
  Hom := hI.to _
  inv := hI'.to _
...
-/
abbrev has_terminal :=
  has_limits_of_shape (discrete.{v₁} Pempty) C

/-- A category has an initial object if it has a colimit over the empty diagram.
Use `has_initial_of_unique` to construct instances.
-/
abbrev has_initial :=
  has_colimits_of_shape (discrete.{v₁} Pempty) C
...
def is_limit_change_empty_cone {c₁ : cone F₁} (hl : is_limit c₁) (c₂ : cone F₂) (hi : c₁.X ≅ c₂.X) : is_limit c₂ where
  lift := fun c =>
    hl.lift
        ⟨c.X, by
          tidy⟩ ≫
...
/-- Replacing an empty cone in `is_limit` by another with the same cone point
    is an equivalence. -/
def is_limit_empty_cone_equiv (c₁ : cone F₁) (c₂ : cone F₂) (h : c₁.X ≅ c₂.X) : is_limit c₁ ≃ is_limit c₂ where
  toFun := fun hl => is_limit_change_empty_cone C hl c₂ h
  invFun := fun hl => is_limit_change_empty_cone C hl c₁ h.symm
  left_inv := by
    tidy
  right_inv := by
    tidy

theorem has_terminal_change_diagram (h : has_limit F₁) : has_limit F₂ :=
  ⟨⟨⟨⟨limit F₁, by
          tidy⟩,
        is_limit_change_empty_cone C (limit.is_limit F₁) _ (eq_to_iso rfl)⟩⟩⟩
...
def is_colimit_change_empty_cocone {c₁ : cocone F₁} (hl : is_colimit c₁) (c₂ : cocone F₂) (hi : c₁.X ≅ c₂.X) :
    is_colimit c₂ where
  desc := fun c =>
    hi.inv ≫
      hl.desc
...
def is_colimit_empty_cocone_equiv (c₁ : cocone F₁) (c₂ : cocone F₂) (h : c₁.X ≅ c₂.X) :
    is_colimit c₁ ≃ is_colimit c₂ where
  toFun := fun hl => is_colimit_change_empty_cocone C hl c₂ h
  invFun := fun hl => is_colimit_change_empty_cocone C hl c₁ h.symm
  left_inv := by
    tidy
  right_inv := by
    tidy

theorem has_initial_change_diagram (h : has_colimit F₁) : has_colimit F₂ :=
  ⟨⟨⟨⟨colimit F₁, by
          tidy⟩,
...
instance unique_to_terminal [has_terminal C] (P : C) : Unique (P ⟶ ⊤_ C) where
  default := terminal.from P
  uniq := fun m => by
    apply limit.hom_ext
    rintro ⟨⟩

instance unique_from_initial [has_initial C] (P : C) : Unique (⊥_ C ⟶ P) where
...
@[simp]
theorem terminal.comp_from [has_terminal C] {P Q : C} (f : P ⟶ Q) : f ≫ terminal.from Q = terminal.from P := by
  tidy

@[simp]
theorem initial.to_comp [has_initial C] {P Q : C} (f : P ⟶ Q) : initial.to P ≫ f = initial.to Q := by
  tidy

/-- A terminal object is terminal. -/
def terminal_is_terminal [has_terminal C] : is_terminal (⊤_ C) where
  lift := fun s => terminal.from _

/-- An initial object is initial. -/
def initial_is_initial [has_initial C] : is_initial (⊥_ C) where
...
def terminal_op_of_initial {X : C} (t : is_initial X) : is_terminal (Opposite.op X) where
  lift := fun s => (t.to s.X.unop).op
  uniq' := fun s m w => Quiver.Hom.unop_inj (t.hom_ext _ _)

/-- An initial object in the opposite category is terminal in the original category. -/
def terminal_unop_of_initial {X : Cᵒᵖ} (t : is_initial X) : is_terminal X.unop where
  lift := fun s => (t.to (Opposite.op s.X)).unop
  uniq' := fun s m w => Quiver.Hom.op_inj (t.hom_ext _ _)

/-- A terminal object is initial in the opposite category. -/
def initial_op_of_terminal {X : C} (t : is_terminal X) : is_initial (Opposite.op X) where
  desc := fun s => (t.from s.X.unop).op
  uniq' := fun s m w => Quiver.Hom.unop_inj (t.hom_ext _ _)

/-- A terminal object in the opposite category is initial in the original category. -/
def initial_unop_of_terminal {X : Cᵒᵖ} (t : is_terminal X) : is_initial X.unop where
...
theorem is_initial.mono_from [initial_mono_class C] {I} {X : C} (hI : is_initial I) (f : I ⟶ X) : mono f := by
...
@[simps]
def cone_of_diagram_initial {X : J} (tX : is_initial X) (F : J ⥤ C) : cone F where
  x := F.obj X
  π :=
...
def limit_of_diagram_initial {X : J} (tX : is_initial X) (F : J ⥤ C) : is_limit (cone_of_diagram_initial tX F) where
...
def limit_of_diagram_terminal {X : J} (hX : is_terminal X) (F : J ⥤ C) [∀ i j : J f : i ⟶ j, is_iso (F.map f)] :
    is_limit (cone_of_diagram_terminal hX F) where
...
@[simps]
def cocone_of_diagram_terminal {X : J} (tX : is_terminal X) (F : J ⥤ C) : cocone F where
  x := F.obj X
  ι :=
...
def colimit_of_diagram_terminal {X : J} (tX : is_terminal X) (F : J ⥤ C) :
    is_colimit (cocone_of_diagram_terminal tX F) where
...
def colimit_of_diagram_initial {X : J} (hX : is_initial X) (F : J ⥤ C) [∀ i j : J f : i ⟶ j, is_iso (F.map f)] :
    is_colimit (cocone_of_diagram_initial hX F) where
...
theorem is_iso_π_of_is_terminal {j : J} (I : is_terminal j) (F : J ⥤ C) [has_limit F]
  ...
  ⟨⟨limit.lift _ (cone_of_diagram_terminal I F), by
      ext
      simp , by
      simp ⟩⟩
