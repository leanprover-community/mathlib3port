...
instance
  unique_to_terminal
  [ has_terminal C ] ( P : C ) : Unique ( P ⟶ ⊤_ C )
  where default := terminal.from P uniq m := by apply limit.hom_ext rintro ⟨ ⟩

-- failed to format: format: uncaught backtrack exception
instance
  unique_from_initial
  [ has_initial C ] ( P : C ) : Unique ( ⊥_ C ⟶ P )
  where default := initial.to P uniq m := by apply colimit.hom_ext rintro ⟨ ⟩
