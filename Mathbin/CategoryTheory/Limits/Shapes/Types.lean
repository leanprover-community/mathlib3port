...
def terminal_limit_cone : limits.limit_cone (functor.empty (Type u)) where
  Cone :=
    { x := PUnit,
      π := by
        tidy }
...
def initial_limit_cone : limits.colimit_cocone (functor.empty (Type u)) where
  Cocone :=
    { x := Pempty,
      ι := by
        tidy }
...
-/
def coproduct_colimit_cocone {J : Type u} (F : J → Type u) : limits.colimit_cocone (discrete.functor F) where
...
theorem unique_of_type_equalizer (t : is_limit (fork.of_ι _ w)) (y : Y) (hy : g y = h y) : ∃! x : X, f x = y := by
...
-/
@[simps]
def pullback_limit_cone (f : X ⟶ Z) (g : Y ⟶ Z) : limits.limit_cone (cospan f g) where
