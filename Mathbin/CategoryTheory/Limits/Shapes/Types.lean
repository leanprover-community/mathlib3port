...
-- error in CategoryTheory.Limits.Shapes.Types: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
...
{ lift := λ (s : binary_fan X Y) (x), (s.fst x, s.snd x),
  ...
  uniq' := λ s m w, «expr $ »(funext, λ x, prod.ext (congr_fun (w left) x) (congr_fun (w right) x)) }
...
{ desc := λ s : binary_cofan X Y, sum.elim s.inl s.inr,
  fac' := λ s j, walking_pair.cases_on j rfl rfl,
  uniq' := λ s m w, «expr $ »(funext, λ x, sum.cases_on x (congr_fun (w left)) (congr_fun (w right))) }
...
{ cocone := cofork.of_π (quot.mk (coequalizer_rel f g)) (funext (λ x, quot.sound (coequalizer_rel.rel x))),
  is_colimit := «expr $ »(cofork.is_colimit.mk' _, λ
   s, ⟨quot.lift s.π (λ (a b) (h : coequalizer_rel f g a b), by { cases [expr h] [],
       exact [expr congr_fun s.condition h_1] }), rfl, λ
    m hm, «expr $ »(funext, λ x, quot.induction_on x (congr_fun hm : _))⟩) }
