...
def terminal_limit_cone : Limits.LimitCone (Functor.empty (Type u)) where
  Cone :=
    { x := PUnit,
      π := by
        tidy }
...
def initial_limit_cone : Limits.ColimitCocone (Functor.empty (Type u)) where
  Cocone :=
    { x := Pempty,
      ι := by
        tidy }
...
@[simps x]
def binary_product_cone (X Y : Type u) : BinaryFan X Y :=
...
@[simps]
def binary_product_limit (X Y : Type u) : IsLimit (binaryProductCone X Y) where
...
-/
@[simps]
def binary_product_limit_cone (X Y : Type u) : Limits.LimitCone (pair X Y) :=
  ⟨_, binaryProductLimit X Y⟩

/-- The functor which sends `X, Y` to the product type `X × Y`. -/
@[simps (config := { typeMd := reducible })]
def binary_product_functor : Type u ⥤ Type u ⥤ Type u where
  obj := fun X =>
    { obj := fun Y => X × Y,
...
@[simps]
def binary_coproduct_colimit (X Y : Type u) : IsColimit (binaryCoproductCocone X Y) where
...
-/
def binary_coproduct_colimit_cocone (X Y : Type u) : Limits.ColimitCocone (pair X Y) :=
  ⟨_, binaryCoproductColimit X Y⟩

/-- The category of types has `Π j, f j` as the product of a type family `f : J → Type`.
-/
def product_limit_cone {J : Type u} (F : J → Type u) : Limits.LimitCone (Discrete.functor F) where
...
-/
def coproduct_colimit_cocone {J : Type u} (F : J → Type u) : Limits.ColimitCocone (Discrete.functor F) where
...
-/
noncomputable def type_equalizer_of_unique (t : ∀ y : Y, g y = h y → ∃! x : X, f x = y) : IsLimit (Fork.ofι _ w) :=
...
def equalizer_limit : Limits.LimitCone (parallelPair g h) where
...
-/
def coequalizer_colimit : Limits.ColimitCocone (parallelPair f g) where
...
-/
abbrev pullback_cone : Limits.PullbackCone f g :=
...
-/
@[simps]
def pullback_limit_cone (f : X ⟶ Z) (g : Y ⟶ Z) : Limits.LimitCone (cospan f g) where
...
-/
noncomputable def pullback_cone_iso_pullback : Limit.cone (cospan f g) ≅ pullbackCone f g :=
...
-/
noncomputable def pullback_iso_pullback : pullback f g ≅ PullbackObj f g :=
