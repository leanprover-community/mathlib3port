...
def imageMapComp : ImageMap (sq ≫ sq') where
...
def imageMapId : ImageMap (𝟙 f) where
  map := 𝟙 (image f.Hom)
...
instance (priority := 100) hasImageMapsOfHasStrongEpiImages [HasStrongEpiImages C] : HasImageMaps C where
