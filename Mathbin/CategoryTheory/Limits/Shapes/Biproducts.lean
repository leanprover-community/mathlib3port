...
@[simps]
def biproduct.map_iso [Fintype J] {f g : J → C} [has_finite_biproducts C] (p : ∀ b, f b ≅ g b) : ⨁ f ≅ ⨁ g where
...
@[simps]
def to_binary_bicone {X Y : C} (b : bicone (pair X Y).obj) : binary_bicone X Y where
  x := b.X
...
def to_binary_bicone_is_limit {X Y : C} {b : bicone (pair X Y).obj} (c : is_limit b.to_cone) :
    is_limit b.to_binary_bicone.to_cone where
  lift := fun s => c.lift s
  fac' := fun s j => by
    cases j <;> erw [c.fac]
  uniq' := fun s m w => by
...
def to_binary_bicone_is_colimit {X Y : C} {b : bicone (pair X Y).obj} (c : is_colimit b.to_cocone) :
    is_colimit b.to_binary_bicone.to_cocone where
  desc := fun s => c.desc s
  fac' := fun s j => by
    cases j <;> erw [c.fac]
  uniq' := fun s m w => by
