...
instance equalizer_regular (g h : X ⟶ Y) [has_limit (parallel_pair g h)] : regular_mono (equalizer.ι g h) where
  z := Y
  left := g
  right := h
...
instance (priority := 100) regular_mono.of_split_mono (f : X ⟶ Y) [split_mono f] : regular_mono f where
  z := Y
  left := 𝟙 Y
...
def regular_of_is_pullback_snd_of_regular {P Q R S : C} {f : P ⟶ Q} {g : P ⟶ R} {h : Q ⟶ S} {k : R ⟶ S}
    [hr : regular_mono h] (comm : f ≫ h = g ≫ k) (t : is_limit (pullback_cone.mk _ _ comm)) : regular_mono g where
  z := hr.Z
  left := k ≫ hr.left
  right := k ≫ hr.right
  w := by
...
instance coequalizer_regular (g h : X ⟶ Y) [has_colimit (parallel_pair g h)] : regular_epi (coequalizer.π g h) where
  w := X
  left := g
  right := h
...
instance (priority := 100) regular_epi.of_split_epi (f : X ⟶ Y) [split_epi f] : regular_epi f where
  w := X
  left := 𝟙 X
  right := f ≫ section_ f
  w := by
...
def regular_of_is_pushout_snd_of_regular {P Q R S : C} {f : P ⟶ Q} {g : P ⟶ R} {h : Q ⟶ S} {k : R ⟶ S}
    [gr : regular_epi g] (comm : f ≫ h = g ≫ k) (t : is_colimit (pushout_cocone.mk _ _ comm)) : regular_epi h where
  w := gr.W
  left := gr.left ≫ f
  right := gr.right ≫ f
  w := by
...
instance (priority := 100) strong_epi_of_regular_epi (f : X ⟶ Y) [regular_epi f] : strong_epi f where
  Epi := by
    infer_instance
  HasLift := by
    intros
