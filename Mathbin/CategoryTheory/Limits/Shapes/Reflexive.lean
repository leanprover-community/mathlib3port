...
theorem has_coequalizer_of_common_section [has_reflexive_coequalizers C] {A B : C} {f g : A ⟶ B} (r : B ⟶ A)
    (rf : r ≫ f = 𝟙 _) (rg : r ≫ g = 𝟙 _) : has_coequalizer f g := by
...
theorem has_equalizer_of_common_retraction [has_coreflexive_equalizers C] {A B : C} {f g : A ⟶ B} (r : B ⟶ A)
    (fr : f ≫ r = 𝟙 _) (gr : g ≫ r = 𝟙 _) : has_equalizer f g := by
...
/-- If `C` has coequalizers, then it has reflexive coequalizers. -/
  instance
    ( priority := 100 )
    has_reflexive_coequalizers_of_has_coequalizers
    [ has_coequalizers C ] : has_reflexive_coequalizers C
    where has_coeq A B f g i := by infer_instance

-- failed to format: format: uncaught backtrack exception
/-- If `C` has equalizers, then it has coreflexive equalizers. -/
  instance
    ( priority := 100 )
    has_coreflexive_equalizers_of_has_equalizers
    [ has_equalizers C ] : has_coreflexive_equalizers C
    where has_eq A B f g i := by infer_instance
