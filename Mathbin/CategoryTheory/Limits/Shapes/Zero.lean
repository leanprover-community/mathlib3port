...
instance
  has_zero_morphisms_opposite
  [ has_zero_morphisms C ] : has_zero_morphisms ( C ᵒᵖ )
  where
    HasZero X Y := ⟨ ( 0 : unop Y ⟶ unop X ) . op ⟩
      comp_zero' X Y f Z := congr_argₓ Quiver.Hom.op ( has_zero_morphisms.zero_comp ( unop Z ) f.unop )
      zero_comp' X Y Z f := congr_argₓ Quiver.Hom.op ( has_zero_morphisms.comp_zero f.unop ( unop X ) )
...
-- failed to format: format: uncaught backtrack exception
instance : has_zero_morphisms ( C ⥤ D ) where HasZero F G := ⟨ { app := fun X => 0 } ⟩
...
theorem equivalence_preserves_zero_morphisms (F : C ≌ D) (X Y : C) :
    F.functor.map (0 : X ⟶ Y) = (0 : F.functor.obj X ⟶ F.functor.obj Y) := by
...
-- failed to format: format: uncaught backtrack exception
instance
  { B : Type _ } [ category B ] [ has_zero_morphisms C ] : has_zero_object ( B ⥤ C )
  where
    zero := { obj := fun X => 0 , map := fun X Y f => 0 }
      uniqueTo F := ⟨ ⟨ { app := fun X => 0 } ⟩ , by tidy ⟩
      uniqueFrom F := ⟨ ⟨ { app := fun X => 0 } ⟩ , by tidy ⟩
