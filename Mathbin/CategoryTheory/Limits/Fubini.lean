...
def DiagramOfCones.conePoints (D : DiagramOfCones F) : J ⥤ C
  ...
  map_id' j := D.id j
  map_comp' j₁ j₂ j₃ f g := comp.comp f g
            ...
            π :=
              { app := fun k => c.π.app (j, k)
                naturality' := fun k k' f => by
                  ...
                  dsimp at this
                  simp only [Category.id_comp, CategoryTheory.Functor.map_id, NatTrans.id_app] at
                    this
                  exact this } }
...
theorem limitCurrySwapCompLimIsoLimitCurryCompLim_hom_π_π {j} {k} :
  ...
  simp only [Iso.refl_hom, braiding_counitIso_hom_app, Limits.HasLimit.isoOfEquivalence_hom_π,
...
theorem limitCurrySwapCompLimIsoLimitCurryCompLim_inv_π_π {j} {k} :
  ...
  by
  dsimp [limitCurrySwapCompLimIsoLimitCurryCompLim]
  simp only [Iso.refl_hom, braiding_counitIso_hom_app, Limits.HasLimit.isoOfEquivalence_inv_π,
    Iso.refl_inv, limitIsoLimitCurryCompLim_hom_π_π, eqToIso_refl, Category.assoc]
  erw [NatTrans.id_app]
  -- Why can't `simp` do this`?
  dsimp;
