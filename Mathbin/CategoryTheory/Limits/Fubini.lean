...
-/
noncomputable def limit_uncurry_iso_limit_comp_lim : limit (uncurry.obj F) ≅ limit (F ⋙ lim) := by
...
-/
noncomputable def limit_iso_limit_curry_comp_lim : limit G ≅ limit (curry.obj G ⋙ lim) := by
