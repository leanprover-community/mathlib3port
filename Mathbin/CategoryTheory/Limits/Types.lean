...
theorem is_limit_equiv_sections_symm_apply {F : J ⥤ Type u} {c : cone F} (t : is_limit c) (x : F.sections) (j : J) :
  c.π.app j ((is_limit_equiv_sections t).symm x) = (x : ∀ j, F.obj j) j :=
  by 
...
@[ext]
theorem limit_ext (F : J ⥤ Type u) (x y : limit F) (w : ∀ j, limit.π F j x = limit.π F j y) : x = y :=
  by 
...
theorem colimit_sound' {F : J ⥤ Type u} {j j' : J} {x : F.obj j} {x' : F.obj j'} {j'' : J} (f : j ⟶ j'') (f' : j' ⟶ j'')
  (w : F.map f x = F.map f' x') : colimit.ι F j x = colimit.ι F j' x' :=
  by 
...
protected theorem rel_equiv : Equivalenceₓ (filtered_colimit.rel F) :=
  ⟨fun x => ⟨x.1, 𝟙 x.1, 𝟙 x.1, rfl⟩, fun x y ⟨k, f, g, h⟩ => ⟨k, g, f, h.symm⟩,
    fun x y z ⟨k, f, g, h⟩ ⟨k', f', g', h'⟩ =>
      ...
      ⟨m, f ≫ fl ≫ n, g' ≫ gl ≫ n,
        calc F.map (f ≫ fl ≫ n) x.2 = F.map (fl ≫ n) (F.map f x.2) :=
          by 
            simp 
          _ = F.map (fl ≫ n) (F.map g y.2) :=
          by 
            rw [h]
          _ = F.map ((g ≫ fl) ≫ n) y.2 :=
          by 
          ...
          by 
            rw [hn]
          _ = F.map (gl ≫ n) (F.map f' y.2) :=
          by 
            simp 
          _ = F.map (gl ≫ n) (F.map g' z.2) :=
          by 
            rw [h']
          _ = F.map (g' ≫ gl ≫ n) z.2 :=
          by 
...
theorem colimit_eq_iff_aux {i j : J} {xi : F.obj i} {xj : F.obj j} :
  (colimit_cocone F).ι.app i xi = (colimit_cocone F).ι.app j xj ↔ filtered_colimit.rel F ⟨i, xi⟩ ⟨j, xj⟩ :=
  by 
