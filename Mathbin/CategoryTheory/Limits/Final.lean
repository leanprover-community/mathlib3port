...
@[simps]
def colimit_cocone_comp (t : colimit_cocone G) : colimit_cocone (F ⋙ G) where
  Cocone := _
  IsColimit := (is_colimit_whisker_equiv F _).symm t.is_colimit
...
@[simps]
def colimit_cocone_of_comp (t : colimit_cocone (F ⋙ G)) : colimit_cocone G where
  Cocone := extend_cocone.obj t.cocone
  IsColimit := (is_colimit_extend_cocone_equiv F _).symm t.is_colimit
...
theorem zigzag_of_eqv_gen_quot_rel {F : C ⥤ D} {d : D} {f₁ f₂ : Σ X, d ⟶ F.obj X}
    ...
    zigzag (structured_arrow.mk f₁.2) (structured_arrow.mk f₂.2) := by
  induction t
...
@[simps]
def limit_cone_comp (t : limit_cone G) : limit_cone (F ⋙ G) where
  Cone := _
  IsLimit := (is_limit_whisker_equiv F _).symm t.is_limit
...
@[simps]
def limit_cone_of_comp (t : limit_cone (F ⋙ G)) : limit_cone G where
  Cone := extend_cone.obj t.cone
  IsLimit := (is_limit_extend_cone_equiv F _).symm t.is_limit
