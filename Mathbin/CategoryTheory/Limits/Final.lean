...
def induction {d : D} (Z : ∀ (X : C) (k : d ⟶ F.obj X), Sort _)
  ...
  apply Nonempty.some
  apply
    @is_preconnected_induction _ _ _ (fun Y : StructuredArrow d F => Z Y.right Y.hom) _ _
      { right := X₀
        Hom := k₀ } z
  · intro j₁ j₂ f a
...
def induction {d : D} (Z : ∀ (X : C) (k : F.obj X ⟶ d), Sort _)
  ...
  apply Nonempty.some
  apply
    @is_preconnected_induction _ _ _ (fun Y : CostructuredArrow F d => Z Y.left Y.hom) _ _
      { left := X₀
        Hom := k₀ } z
  · intro j₁ j₂ f a
