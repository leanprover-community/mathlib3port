...
-/
def finite_limit_cone [SemilatticeInfTop α] (F : J ⥤ α) : limit_cone F :=
...
-/
def finite_colimit_cocone [SemilatticeSupBot α] (F : J ⥤ α) : colimit_cocone F :=
...
-/
theorem finite_limit_eq_finset_univ_inf [SemilatticeInfTop α] (F : J ⥤ α) : limit F = Finset.univ.inf F.obj :=
...
objects in the image.
-/
theorem finite_colimit_eq_finset_univ_sup [SemilatticeSupBot α] (F : J ⥤ α) : colimit F = Finset.univ.sup F.obj :=
...
-/
theorem finite_product_eq_finset_inf [SemilatticeInfTop α] {ι : Type u} [DecidableEq ι] [Fintype ι] (f : ι → α) :
...
-/
theorem finite_coproduct_eq_finset_sup [SemilatticeSupBot α] {ι : Type u} [DecidableEq ι] [Fintype ι] (f : ι → α) :
...
-/
@[simp]
theorem prod_eq_inf [SemilatticeInfTop α] (x y : α) : limits.prod x y = x⊓y :=
...
-/
@[simp]
theorem coprod_eq_sup [SemilatticeSupBot α] (x y : α) : limits.coprod x y = x⊔y :=
...
-/
@[simp]
theorem pullback_eq_inf [SemilatticeInfTop α] {x y z : α} (f : x ⟶ z) (g : y ⟶ z) : pullback f g = x⊓y :=
...
-/
@[simp]
theorem pushout_eq_sup [SemilatticeSupBot α] (x y z : α) (f : z ⟶ x) (g : z ⟶ y) : pushout f g = x⊔y :=
