...
def preservesLimitRightOp (K : J ⥤ C) (F : Cᵒᵖ ⥤ D) [PreservesColimit K.op F] :
    PreservesLimit K
      F.rightOp where preserves c hc := isLimitConeRightOpOfCocone _ (isColimitOfPreserves F (isColimitConeOp _ hc))
...
def preservesLimitUnop (K : J ⥤ C) (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesColimit K.op F] :
    PreservesLimit K
      F.unop where preserves c hc := isLimitConeUnopOfCocone _ (isColimitOfPreserves F (isColimitConeOp _ hc))
...
def preservesColimitRightOp (K : J ⥤ C) (F : Cᵒᵖ ⥤ D) [PreservesLimit K.op F] :
    PreservesColimit K
      F.rightOp where preserves c hc := isColimitCoconeRightOpOfCone _ (isLimitOfPreserves F (isLimitCoconeOp _ hc))
...
def preservesColimitUnop (K : J ⥤ C) (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesLimit K.op F] :
    PreservesColimit K
      F.unop where preserves c hc := isColimitCoconeUnopOfCone _ (isLimitOfPreserves F (isLimitCoconeOp _ hc))
...
def preservesLimitsOp (F : C ⥤ D) [PreservesColimits F] :
    PreservesLimits F.op where PreservesLimitsOfShape J _ := preserves_limits_of_shape_op J F

/-- If `F : C ⥤ Dᵒᵖ` preserves colimits, then `F.left_op : Cᵒᵖ ⥤ D` preserves limits. -/
def preservesLimitsLeftOp (F : C ⥤ Dᵒᵖ) [PreservesColimits F] :
    PreservesLimits F.leftOp where PreservesLimitsOfShape J _ := preserves_limits_of_shape_left_op J F
...
def preservesLimitsRightOp (F : Cᵒᵖ ⥤ D) [PreservesColimits F] :
    PreservesLimits F.rightOp where PreservesLimitsOfShape J _ := preserves_limits_of_shape_right_op J F

/-- If `F : Cᵒᵖ ⥤ Dᵒᵖ` preserves colimits, then `F.unop : C ⥤ D` preserves limits. -/
def preservesLimitsUnop (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesColimits F] :
    PreservesLimits F.unop where PreservesLimitsOfShape J _ := preserves_limits_of_shape_unop J F

/-- If `F : C ⥤ D` preserves limits, then `F.op : Cᵒᵖ ⥤ Dᵒᵖ` preserves colimits. -/
def perservesColimitsOp (F : C ⥤ D) [PreservesLimits F] :
    PreservesColimits F.op where PreservesColimitsOfShape J _ := preserves_colimits_of_shape_op J F

/-- If `F : C ⥤ Dᵒᵖ` preserves limits, then `F.left_op : Cᵒᵖ ⥤ D` preserves colimits. -/
def preservesColimitsLeftOp (F : C ⥤ Dᵒᵖ) [PreservesLimits F] :
    PreservesColimits F.leftOp where PreservesColimitsOfShape J _ := preserves_colimits_of_shape_left_op J F
...
def preservesColimitsRightOp (F : Cᵒᵖ ⥤ D) [PreservesLimits F] :
    PreservesColimits F.rightOp where PreservesColimitsOfShape J _ := preserves_colimits_of_shape_right_op J F

/-- If `F : Cᵒᵖ ⥤ Dᵒᵖ` preserves limits, then `F.unop : C ⥤ D` preserves colimits. -/
def preservesColimitsUnop (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesLimits F] :
    PreservesColimits F.unop where PreservesColimitsOfShape J _ := preserves_colimits_of_shape_unop J F
...
def preservesFiniteLimitsOp (F : C ⥤ D) [PreservesFiniteColimits F] :
    PreservesFiniteLimits F.op where PreservesFiniteLimits J _ _ := preserves_limits_of_shape_op J F
...
def preservesFiniteLimitsLeftOp (F : C ⥤ Dᵒᵖ) [PreservesFiniteColimits F] :
    PreservesFiniteLimits F.leftOp where PreservesFiniteLimits J _ _ := preserves_limits_of_shape_left_op J F
...
def preservesFiniteLimitsRightOp (F : Cᵒᵖ ⥤ D) [PreservesFiniteColimits F] :
    PreservesFiniteLimits F.rightOp where PreservesFiniteLimits J _ _ := preserves_limits_of_shape_right_op J F
...
def preservesFiniteLimitsUnop (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesFiniteColimits F] :
    PreservesFiniteLimits F.unop where PreservesFiniteLimits J _ _ := preserves_limits_of_shape_unop J F
...
def preservesFiniteColimitsOp (F : C ⥤ D) [PreservesFiniteLimits F] :
    PreservesFiniteColimits F.op where PreservesFiniteColimits J _ _ := preserves_colimits_of_shape_op J F
...
def preservesFiniteColimitsLeftOp (F : C ⥤ Dᵒᵖ) [PreservesFiniteLimits F] :
    PreservesFiniteColimits F.leftOp where PreservesFiniteColimits J _ _ := preserves_colimits_of_shape_left_op J F
...
def preservesFiniteColimitsRightOp (F : Cᵒᵖ ⥤ D) [PreservesFiniteLimits F] :
    PreservesFiniteColimits F.rightOp where PreservesFiniteColimits J _ _ := preserves_colimits_of_shape_right_op J F
...
def preservesFiniteColimitsUnop (F : Cᵒᵖ ⥤ Dᵒᵖ) [PreservesFiniteLimits F] :
    PreservesFiniteColimits F.unop where PreservesFiniteColimits J _ _ := preserves_colimits_of_shape_unop J F
