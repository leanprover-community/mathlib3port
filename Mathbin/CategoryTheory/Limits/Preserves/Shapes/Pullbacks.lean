...
variable (f g) [preserves_limit (cospan f g) G]
...
def preserves_pullback_symmetry : preserves_limit (cospan g f) G where
  preserves := fun c hc => by
    apply (is_limit.whisker_equivalence_equiv walking_cospan_equiv.{v₂, v₁}).symm.toFun
...
variable [has_pullback f g] [has_pullback (G.map f) (G.map g)]
...
@[reassoc]
theorem preserves_pullback.iso_hom_fst : (preserves_pullback.iso G f g).Hom ≫ pullback.fst = G.map pullback.fst := by
  simp [preserves_pullback.iso]

@[reassoc]
theorem preserves_pullback.iso_hom_snd : (preserves_pullback.iso G f g).Hom ≫ pullback.snd = G.map pullback.snd := by
  simp [preserves_pullback.iso]

@[simp, reassoc]
theorem preserves_pullback.iso_inv_fst : (preserves_pullback.iso G f g).inv ≫ G.map pullback.fst = pullback.fst := by
  simp [preserves_pullback.iso, iso.inv_comp_eq]

@[simp, reassoc]
theorem preserves_pullback.iso_inv_snd : (preserves_pullback.iso G f g).inv ≫ G.map pullback.snd = pullback.snd := by
  simp [preserves_pullback.iso, iso.inv_comp_eq]
...
variable (f g) [preserves_colimit (span f g) G]
...
def preserves_pushout_symmetry : preserves_colimit (span g f) G where
  preserves := fun c hc => by
    apply (is_colimit.whisker_equivalence_equiv walking_span_equiv.{v₂, v₁}).symm.toFun
...
variable [has_pushout f g] [has_pushout (G.map f) (G.map g)]
...
@[reassoc]
theorem preserves_pushout.inl_iso_hom : pushout.inl ≫ (preserves_pushout.iso G f g).Hom = G.map pushout.inl := by
  delta' preserves_pushout.iso
  simp

@[reassoc]
theorem preserves_pushout.inr_iso_hom : pushout.inr ≫ (preserves_pushout.iso G f g).Hom = G.map pushout.inr := by
  delta' preserves_pushout.iso
  simp

@[simp, reassoc]
theorem preserves_pushout.inl_iso_inv : G.map pushout.inl ≫ (preserves_pushout.iso G f g).inv = pushout.inl := by
  simp [preserves_pushout.iso, iso.comp_inv_eq]

@[simp, reassoc]
theorem preserves_pushout.inr_iso_inv : G.map pushout.inr ≫ (preserves_pushout.iso G f g).inv = pushout.inr := by
  simp [preserves_pushout.iso, iso.comp_inv_eq]
...
pullback of `(f,g)`. -/
def preserves_pullback.of_iso_comparison [i : is_iso (pullback_comparison G f g)] : preserves_limit (cospan f g) G := by
...
instance : is_iso (pullback_comparison G f g) := by
...
pushout of `(f,g)`. -/
def preserves_pushout.of_iso_comparison [i : is_iso (pushout_comparison G f g)] : preserves_colimit (span f g) G := by
...
instance : is_iso (pushout_comparison G f g) := by
