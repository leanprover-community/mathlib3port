...
class PreservesFiniteBiproducts (F : C ⥤ D) [PreservesZeroMorphisms F] where
  preserves : ∀ {J : Type v} [DecidableEq J] [Fintype J], PreservesBiproductsOfShape J F
...
class PreservesBiproducts (F : C ⥤ D) [PreservesZeroMorphisms F] where
  preserves : ∀ {J : Type v} [DecidableEq J], PreservesBiproductsOfShape J F
...
instance (priority := 100) preservesFiniteBiproductsOfPreservesBiproducts (F : C ⥤ D) [PreservesZeroMorphisms F]
    [PreservesBiproducts F] : PreservesFiniteBiproducts F where
  preserves := fun J _ _ => inferInstance
...
def preservesBinaryBiproductOfPreservesBiproduct (F : C ⥤ D) [PreservesZeroMorphisms F] (X Y : C)
    [PreservesBiproduct (pair X Y).obj F] : PreservesBinaryBiproduct X Y F where
  preserves := fun b hb =>
    { IsLimit :=
        IsLimit.ofIsoLimit
...
def preservesBinaryBiproductsOfPreservesBiproducts (F : C ⥤ D) [PreservesZeroMorphisms F]
    [PreservesBiproductsOfShape (Discrete WalkingPair.{v}) F] : PreservesBinaryBiproducts F where
  preserves := fun X Y => preservesBinaryBiproductOfPreservesBiproduct F X Y
...
variable {J : Type v} [DecidableEq J] (F : C ⥤ D) [PreservesZeroMorphisms F] (f : J → C) [HasBiproduct f]
...
variable {J : Type v} [DecidableEq J] (f : J → C) [HasBiproduct f] [PreservesBiproduct f F] {W : C}
...
def preservesProductsOfShapeOfPreservesBiproductsOfShape [PreservesBiproductsOfShape J F] :
  ...
  PreservesLimit := fun f => preservesLimitOfIsoDiagram _ Discrete.natIsoFunctor.symm
...
def preservesBiproductsOfShapeOfPreservesProductsOfShape [PreservesLimitsOfShape (Discrete J) F] :
  ...
  preserves := fun f => preservesBiproductOfPreservesProduct F
...
def preservesCoproductsOfShapeOfPreservesBiproductsOfShape [PreservesBiproductsOfShape J F] :
  ...
  PreservesColimit := fun f => preservesColimitOfIsoDiagram _ Discrete.natIsoFunctor.symm
...
def preservesBiproductsOfShapeOfPreservesCoproductsOfShape [PreservesColimitsOfShape (Discrete J) F] :
    PreservesBiproductsOfShape J F where
  preserves := fun f => preservesBiproductOfPreservesCoproduct F
...
def preservesBinaryProductsOfPreservesBinaryBiproducts [PreservesBinaryBiproducts F] :
    PreservesLimitsOfShape (Discrete WalkingPair.{v}) F where
  PreservesLimit := fun K => preservesLimitOfIsoDiagram _ (diagramIsoPair _).symm
...
/-- A functor between preadditive categories that preserves (zero morphisms and) binary products
    preserves binary biproducts. -/
def preservesBinaryBiproductsOfPreservesBinaryProducts [PreservesLimitsOfShape (Discrete WalkingPair.{v}) F] :
    PreservesBinaryBiproducts F where
  preserves := fun X Y => preservesBinaryBiproductOfPreservesBinaryProduct F
...
def preservesBinaryCoproductsOfPreservesBinaryBiproducts [PreservesBinaryBiproducts F] :
    PreservesColimitsOfShape (Discrete WalkingPair.{v}) F where
  PreservesColimit := fun K => preservesColimitOfIsoDiagram _ (diagramIsoPair _).symm
...
/-- A functor between preadditive categories that preserves (zero morphisms and) binary coproducts
    preserves binary biproducts. -/
def preservesBinaryBiproductsOfPreservesBinaryCoproducts [PreservesColimitsOfShape (Discrete WalkingPair.{v}) F] :
    PreservesBinaryBiproducts F where
  preserves := fun X Y => preservesBinaryBiproductOfPreservesBinaryCoproduct F
