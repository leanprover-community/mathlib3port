...
instance
  id_preserves_limits
  : preserves_limits_of_size .{ w' , w } ( 𝟭 C )
  where
                      ...
                      c h
                        ...
                        ⟨
                          fun s => h.lift ⟨ s.X , fun j => s.π.app j , fun j j' f => s.π.naturality f ⟩
                            ,
                            by cases K <;> rcases c with ⟨ _ , _ , _ ⟩ <;> intro s j <;> cases s <;> exact h.fac _ j
                            ,
                            by
                              cases K
                                <;>
...
-- failed to format: format: uncaught backtrack exception
instance
  id_preserves_colimits
  : preserves_colimits_of_size .{ w' , w } ( 𝟭 C )
  where
...
instance
  id_reflects_limits
  : reflects_limits_of_size .{ w , w' } ( 𝟭 C )
  where
...
-- failed to format: format: uncaught backtrack exception
instance
  id_reflects_colimits
  : reflects_colimits_of_size .{ w , w' } ( 𝟭 C )
  where
...
def reflects_limits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_limits_of_size.{w', w} F] :
  ...
  { ReflectsLimitsOfShape := fun J 𝒥₁ => by
...
def reflects_colimits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_colimits_of_size.{w, w'} F] :
  ...
  { ReflectsColimitsOfShape := fun J 𝒥₁ => by
        ...
        { ReflectsColimit := fun K =>
            { reflects := fun c t =>
                (is_colimit.mk_cocone_morphism fun s =>
                    (cocones.functoriality K F).Preimage (t.desc_cocone_morphism _)) $
                  by
