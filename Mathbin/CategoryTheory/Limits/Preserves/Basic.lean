...
instance preserves_limit_subsingleton (K : J ⥤ C) (F : C ⥤ D) : Subsingleton (preserves_limit K F) := by
  constructor <;> rintro ⟨a⟩ ⟨b⟩ <;> congr

instance preserves_colimit_subsingleton (K : J ⥤ C) (F : C ⥤ D) : Subsingleton (preserves_colimit K F) := by
  constructor <;> rintro ⟨a⟩ ⟨b⟩ <;> congr

instance preserves_limits_of_shape_subsingleton (J : Type w) [category.{w'} J] (F : C ⥤ D) :
    Subsingleton (preserves_limits_of_shape J F) := by
  constructor
  intros
  cases a
  cases b
...
instance preserves_colimits_of_shape_subsingleton (J : Type w) [category.{w'} J] (F : C ⥤ D) :
    Subsingleton (preserves_colimits_of_shape J F) := by
  constructor
  intros
  cases a
  cases b
...
instance preserves_limits_subsingleton (F : C ⥤ D) : Subsingleton (preserves_limits_of_size.{w', w} F) := by
  constructor
  intros
  cases a
  cases b
...
instance preserves_colimits_subsingleton (F : C ⥤ D) : Subsingleton (preserves_colimits_of_size.{w', w} F) := by
  constructor
  intros
  cases a
  cases b
...
instance id_preserves_limits : preserves_limits_of_size.{w', w} (𝟭 C) where
...
instance id_preserves_colimits : preserves_colimits_of_size.{w', w} (𝟭 C) where
...
def preserves_limit_of_nat_iso (K : J ⥤ C) {F G : C ⥤ D} (h : F ≅ G) [preserves_limit K F] : preserves_limit K G where
...
def preserves_limits_of_shape_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [preserves_limits_of_shape J F] :
    preserves_limits_of_shape J G where
...
def preserves_limits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [preserves_limits_of_size.{w, w'} F] :
    preserves_limits_of_size.{w, w'} G where
...
def preserves_limits_of_shape_of_equiv {J' : Type w₂} [category.{w₂'} J'] (e : J ≌ J') (F : C ⥤ D)
    [preserves_limits_of_shape J F] : preserves_limits_of_shape J' F where
  PreservesLimit := fun K =>
    { preserves := fun c t => by
...
def preserves_colimits_of_shape_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [preserves_colimits_of_shape J F] :
    preserves_colimits_of_shape J G where
...
def preserves_colimits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [preserves_colimits_of_size.{w, w'} F] :
    preserves_colimits_of_size.{w, w'} G where
...
def preserves_colimits_of_shape_of_equiv {J' : Type w₂} [category.{w₂'} J'] (e : J ≌ J') (F : C ⥤ D)
    [preserves_colimits_of_shape J F] : preserves_colimits_of_shape J' F where
  PreservesColimit := fun K =>
    { preserves := fun c t => by
        let equ := e.inv_fun_id_assoc (K ⋙ F)
...
instance reflects_limit_subsingleton (K : J ⥤ C) (F : C ⥤ D) : Subsingleton (reflects_limit K F) := by
  constructor <;> rintro ⟨a⟩ ⟨b⟩ <;> congr

instance reflects_colimit_subsingleton (K : J ⥤ C) (F : C ⥤ D) : Subsingleton (reflects_colimit K F) := by
  constructor <;> rintro ⟨a⟩ ⟨b⟩ <;> congr

instance reflects_limits_of_shape_subsingleton (J : Type w) [category.{w'} J] (F : C ⥤ D) :
    Subsingleton (reflects_limits_of_shape J F) := by
  constructor
  intros
  cases a
  cases b
...
instance reflects_colimits_of_shape_subsingleton (J : Type w) [category.{w'} J] (F : C ⥤ D) :
    Subsingleton (reflects_colimits_of_shape J F) := by
  constructor
  intros
  cases a
  cases b
...
instance reflects_limits_subsingleton (F : C ⥤ D) : Subsingleton (reflects_limits_of_size.{w', w} F) := by
  constructor
  intros
  cases a
  cases b
...
instance reflects_colimits_subsingleton (F : C ⥤ D) : Subsingleton (reflects_colimits_of_size.{w', w} F) := by
  constructor
  intros
  cases a
  cases b
...
instance id_reflects_limits : reflects_limits_of_size.{w, w'} (𝟭 C) where
...
instance id_reflects_colimits : reflects_colimits_of_size.{w, w'} (𝟭 C) where
...
def preserves_limits_of_shape_of_reflects_of_preserves [preserves_limits_of_shape J (F ⋙ G)]
    [reflects_limits_of_shape J G] : preserves_limits_of_shape J F where
...
def preserves_limits_of_reflects_of_preserves [preserves_limits_of_size.{w', w} (F ⋙ G)]
    [reflects_limits_of_size.{w', w} G] : preserves_limits_of_size.{w', w} F where
...
def reflects_limit_of_nat_iso (K : J ⥤ C) {F G : C ⥤ D} (h : F ≅ G) [reflects_limit K F] : reflects_limit K G where
...
def reflects_limits_of_shape_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_limits_of_shape J F] :
    reflects_limits_of_shape J G where
...
def reflects_limits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_limits_of_size.{w', w} F] :
    reflects_limits_of_size.{w', w} G where
...
def reflects_limit_of_reflects_isomorphisms (F : J ⥤ C) (G : C ⥤ D) [reflects_isomorphisms G] [has_limit F]
    [preserves_limit F G] : reflects_limit F G where
  reflects := fun c t => by
...
def reflects_limits_of_shape_of_reflects_isomorphisms {G : C ⥤ D} [reflects_isomorphisms G] [has_limits_of_shape J C]
    [preserves_limits_of_shape J G] : reflects_limits_of_shape J G where
...
def reflects_limits_of_reflects_isomorphisms {G : C ⥤ D} [reflects_isomorphisms G] [has_limits_of_size.{w', w} C]
    [preserves_limits_of_size.{w', w} G] : reflects_limits_of_size.{w', w} G where
...
def preserves_colimits_of_shape_of_reflects_of_preserves [preserves_colimits_of_shape J (F ⋙ G)]
    [reflects_colimits_of_shape J G] : preserves_colimits_of_shape J F where
...
def preserves_colimits_of_reflects_of_preserves [preserves_colimits_of_size.{w', w} (F ⋙ G)]
    [reflects_colimits_of_size.{w', w} G] : preserves_colimits_of_size.{w', w} F where
...
def reflects_colimits_of_shape_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_colimits_of_shape J F] :
    reflects_colimits_of_shape J G where
...
def reflects_colimits_of_nat_iso {F G : C ⥤ D} (h : F ≅ G) [reflects_colimits_of_size.{w, w'} F] :
    reflects_colimits_of_size.{w, w'} G where
...
def reflects_colimit_of_reflects_isomorphisms (F : J ⥤ C) (G : C ⥤ D) [reflects_isomorphisms G] [has_colimit F]
    [preserves_colimit F G] : reflects_colimit F G where
  reflects := fun c t => by
...
def reflects_colimits_of_shape_of_reflects_isomorphisms {G : C ⥤ D} [reflects_isomorphisms G]
    [has_colimits_of_shape J C] [preserves_colimits_of_shape J G] : reflects_colimits_of_shape J G where
...
def reflects_colimits_of_reflects_isomorphisms {G : C ⥤ D} [reflects_isomorphisms G] [has_colimits_of_size.{w', w} C]
    [preserves_colimits_of_size.{w', w} G] : reflects_colimits_of_size.{w', w} G where
...
def fully_faithful_reflects_limits [full F] [faithful F] : reflects_limits_of_size.{w, w'} F where
  ReflectsLimitsOfShape := fun J 𝒥₁ =>
    { ReflectsLimit := fun K =>
        { reflects := fun c t =>
            (is_limit.mk_cone_morphism fun s => (cones.functoriality K F).Preimage (t.lift_cone_morphism _)) <| by
              apply fun s m => (cones.functoriality K F).map_injective _
              rw [functor.image_preimage]
              apply t.uniq_cone_morphism } }

/-- A fully faithful functor reflects colimits. -/
def fully_faithful_reflects_colimits [full F] [faithful F] : reflects_colimits_of_size.{w, w'} F where
  ReflectsColimitsOfShape := fun J 𝒥₁ =>
    { ReflectsColimit := fun K =>
        { reflects := fun c t =>
            (is_colimit.mk_cocone_morphism fun s => (cocones.functoriality K F).Preimage (t.desc_cocone_morphism _)) <|
              by
