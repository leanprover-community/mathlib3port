...
#align_import category_theory.limits.constructions.filtered from "leanprover-community/mathlib"@"10bf4f825ad729c5653adc039dafa3622e7f93c9"
  ...
  IsColimit :=
    { desc := fun s =>
      ...
      uniq := fun s m h => by
        ext t ⟨⟨j, hj⟩⟩
        convert h j using 1
        · simp [← colimit.w (lift_to_finset F) ⟨⟨Finset.singleton_subset_iff.2 hj⟩⟩]; rfl
        · tidy }
