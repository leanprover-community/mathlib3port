...
instance preserves_mono {X Y : C} (f : X ⟶ Y) [preserves_limit (cospan f f) F] [mono f] : mono (F.map f) :=
  by 
    have  := is_limit_pullback_cone_map_of_is_limit F _ (pullback_cone.is_limit_mk_id_id f)
    simpRw [F.map_id]  at this 
    apply pullback_cone.mono_of_is_limit_mk_id_id _ this

/-- If `F` reflects pullbacks, then it reflects monomorphisms. -/
theorem reflects_mono {X Y : C} (f : X ⟶ Y) [reflects_limit (cospan f f) F] [mono (F.map f)] : mono f :=
  by 
