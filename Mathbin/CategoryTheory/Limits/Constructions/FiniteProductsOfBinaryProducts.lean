...
than this.
-/
private theorem has_product_ulift_fin : ∀ n : ℕ f : Ulift (Finₓ n) → C, has_product f
| 0 =>
  fun f =>
    by 
      letI this : has_limits_of_shape (discrete (Ulift (Finₓ 0))) C :=
        has_limits_of_shape_of_equivalence (discrete.equivalence (equiv.ulift.trans finZeroEquiv').symm)
      infer_instance
| n+1 =>
  fun f =>
    by 
...
than this.
-/
private theorem has_limits_of_shape_ulift_fin (n : ℕ) : has_limits_of_shape (discrete (Ulift (Finₓ n))) C :=
  { HasLimit :=
      fun K =>
        by 
          letI this := has_product_ulift_fin n K.obj 
...
noncomputable def preserves_fin_of_preserves_binary_and_terminal :
  ∀ n : ℕ f : Ulift (Finₓ n) → C, preserves_limit (discrete.functor f) F
| 0 =>
  fun f =>
    by 
      letI this : preserves_limits_of_shape (discrete (Ulift (Finₓ 0))) F :=
        preserves_limits_of_shape_of_equiv (discrete.equivalence (equiv.ulift.trans finZeroEquiv').symm) _ 
      infer_instance
| n+1 =>
  by 
...
def preserves_ulift_fin_of_preserves_binary_and_terminal (n : ℕ) :
  preserves_limits_of_shape (discrete (Ulift (Finₓ n))) F :=
  { PreservesLimit :=
      fun K =>
        by 
          let this : discrete.functor K.obj ≅ K := discrete.nat_iso fun i => iso.refl _ 
...
than this.
-/
private theorem has_coproduct_ulift_fin : ∀ n : ℕ f : Ulift (Finₓ n) → C, has_coproduct f
| 0 =>
  fun f =>
    by 
      letI this : has_colimits_of_shape (discrete (Ulift (Finₓ 0))) C :=
        has_colimits_of_shape_of_equivalence (discrete.equivalence (equiv.ulift.trans finZeroEquiv').symm)
      infer_instance
| n+1 =>
  fun f =>
    by 
...
than this.
-/
private theorem has_colimits_of_shape_ulift_fin (n : ℕ) : has_colimits_of_shape (discrete (Ulift (Finₓ n))) C :=
  { HasColimit :=
      fun K =>
        by 
          letI this := has_coproduct_ulift_fin n K.obj 
...
noncomputable def preserves_fin_of_preserves_binary_and_initial :
  ∀ n : ℕ f : Ulift (Finₓ n) → C, preserves_colimit (discrete.functor f) F
| 0 =>
  fun f =>
    by 
      letI this : preserves_colimits_of_shape (discrete (Ulift (Finₓ 0))) F :=
        preserves_colimits_of_shape_of_equiv (discrete.equivalence (equiv.ulift.trans finZeroEquiv').symm) _ 
      infer_instance
| n+1 =>
  by 
...
def preserves_ulift_fin_of_preserves_binary_and_initial (n : ℕ) :
  preserves_colimits_of_shape (discrete (Ulift (Finₓ n))) F :=
  { PreservesColimit :=
      fun K =>
        by 
          let this : discrete.functor K.obj ≅ K := discrete.nat_iso fun i => iso.refl _ 
