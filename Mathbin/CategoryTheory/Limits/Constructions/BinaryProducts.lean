...
def isProductOfIsTerminalIsPullback {W X Y Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) (h : W ⟶ X) (k : W ⟶ Y) (H₁ : is_terminal Z)
  ...
  { lift :=
      fun c => H₂.lift (pullback_cone.mk (c.π.app walking_pair.left) (c.π.app walking_pair.right) (H₁.hom_ext _ _)),
    fac' :=
      fun c j =>
        by 
          convert
...
def isPullbackOfIsTerminalIsProduct {W X Y Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) (h : W ⟶ X) (k : W ⟶ Y) (H₁ : is_terminal Z)
  (H₂ : is_limit (binary_fan.mk h k)) : is_limit (pullback_cone.mk _ _ (show h ≫ f = k ≫ g from H₁.hom_ext _ _)) :=
  by 
...
def isCoproductOfIsInitialIsPushout {W X Y Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) (h : W ⟶ X) (k : W ⟶ Y) (H₁ : is_initial W)
  ...
  { desc :=
      fun c => H₂.desc (pushout_cocone.mk (c.ι.app walking_pair.left) (c.ι.app walking_pair.right) (H₁.hom_ext _ _)),
    fac' :=
      fun c j =>
        by 
          convert
