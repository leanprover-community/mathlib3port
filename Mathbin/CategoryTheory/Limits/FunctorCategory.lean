...
instance
  ...
  where
    HasLimit F := has_limit.mk { Cone := combine_cones F fun k => get_limit_cone _ , IsLimit := combined_is_limit _ _ }
...
-- failed to format: format: uncaught backtrack exception
instance
  evaluation_preserves_limits_of_shape
  [ has_limits_of_shape J C ] ( k : K ) : preserves_limits_of_shape J ( ( evaluation K C ) . obj k )
  where
    PreservesLimit
      F
      :=
...
-- failed to format: format: uncaught backtrack exception
instance
  evaluation_preserves_colimits_of_shape
  [ has_colimits_of_shape J C ] ( k : K ) : preserves_colimits_of_shape J ( ( evaluation K C ) . obj k )
  where
    PreservesColimit
      F
      :=
...
-- failed to format: format: uncaught backtrack exception
instance
  evaluation_preserves_limits
  [ has_limits C ] ( k : K ) : preserves_limits ( ( evaluation K C ) . obj k )
...
def preserves_limits_of_evaluation (F : D ⥤ K ⥤ C) (H : ∀ k : K, preserves_limits (F ⋙ (evaluation K C).obj k)) :
    preserves_limits F :=
  ⟨fun L hL => by
    exact preserves_limits_of_shape_of_evaluation F L fun k => preserves_limits_of_size.preserves_limits_of_shape⟩

-- failed to format: format: uncaught backtrack exception
instance
  evaluation_preserves_colimits
  [ has_colimits C ] ( k : K ) : preserves_colimits ( ( evaluation K C ) . obj k )
...
def preserves_colimits_of_evaluation (F : D ⥤ K ⥤ C) (H : ∀ k : K, preserves_colimits (F ⋙ (evaluation K C).obj k)) :
  ...
  ⟨fun L hL => by
    exact preserves_colimits_of_shape_of_evaluation F L fun k => preserves_colimits_of_size.preserves_colimits_of_shape⟩
