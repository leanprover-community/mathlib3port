...
theorem bowtie {j₁ j₂ k₁ k₂ : C} (f₁ : j₁ ⟶ k₁) (g₁ : j₁ ⟶ k₂) (f₂ : j₂ ⟶ k₁) (g₂ : j₂ ⟶ k₂) :
  ...
  let sd := max sb sc
  let s :=
    coeq ((coeq_hom _ _ : sa ⟶ sb) ≫ left_to_max _ _) ((coeq_hom _ _ : sa ⟶ sc) ≫ right_to_max _ _)
  use s
  fconstructor
...
instance is_cofiltered_op_of_is_filtered [IsFiltered C] :
    IsCofiltered
      Cᵒᵖ where 
  cocone_objs X Y :=
...
instance is_filtered_op_of_is_cofiltered [IsCofiltered C] :
    IsFiltered
      Cᵒᵖ where 
  cocone_objs X Y :=
