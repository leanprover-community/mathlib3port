...
noncomputable def IsVanKampenColimit.isColimit {F : J ⥤ C} {c : Cocone F}
  ...
  refine'
    ((h c (𝟙 F) (𝟙 c.X : _) (by rw [Functor.map_id, Category.comp_id, Category.id_comp])
        ...
        fun j => _).some
  haveI : IsIso (𝟙 c.X) := inferInstance
  exact IsPullback.ofVertIsIso ⟨by erw [NatTrans.id_app, Category.comp_id, Category.id_comp]⟩
...
theorem IsInitial.isVanKampenColimit [HasStrictInitialObjects C] {X : C} (h : IsInitial X) :
  ...
  refine'
    ⟨by rintro _ ⟨⟨⟩⟩, fun _ =>
      ⟨IsColimit.ofIsoColimit h (Cocones.ext (asIso f).symm <| by rintro ⟨⟨⟩⟩)⟩⟩
...
theorem map_pair_equifibered {F F' : Discrete WalkingPair ⥤ C} (α : F ⟶ F') : α.Equifibered :=
  ...
  all_goals
    ...
    exact IsPullback.ofHorizIsIso ⟨by simp only [Category.comp_id, Category.id_comp]⟩
...
theorem BinaryCofan.is_van_kampen_iff (c : BinaryCofan X Y) :
  ...
  constructor
  · introv H hαX hαY
    rw [H c' (mapPair αX αY) f (by ext ⟨⟨⟩⟩ <;> dsimp <;> assumption) (map_pair_equifibered _)]
    constructor
    · intro H
      exact ⟨H _, H _⟩
    · rintro H ⟨⟨⟩⟩
...
theorem BinaryCofan.is_van_kampen_mk {X Y : C} (c : BinaryCofan X Y)
  ...
  rw [BinaryCofan.is_van_kampen_iff]
  introv hX hY
  constructor
  · rintro ⟨h⟩
    ...
    · rw [← Category.id_comp αX, ← Iso.hom_inv_id_assoc e f]
      ...
      exact (IsPullback.ofVertIsIso ⟨this⟩).pasteVert hl
    · rw [← Category.id_comp αY, ← Iso.hom_inv_id_assoc e f]
      ...
      exact (IsPullback.ofVertIsIso ⟨this⟩).pasteVert hr
  · rintro ⟨H₁, H₂⟩
...
theorem BinaryCofan.mono_inr_of_is_van_kampen [HasInitial C] {X Y : C} {c : BinaryCofan X Y}
  ...
  by
  refine' PullbackCone.mono_of_isLimit_mk_id_id _ (IsPullback.isLimit _)
  refine'
    (h (BinaryCofan.mk (initial.to Y) (𝟙 Y)) (mapPair (initial.to X) (𝟙 Y)) c.inr _
...
theorem has_strict_initial_of_is_universal [HasInitial C]
  ...
  hasStrictInitialObjects_of_initial_is_strict
    (by
      ...
      refine'
        (H (BinaryCofan.mk (𝟙 _) (𝟙 _)) (mapPair f f) f (by ext ⟨⟨⟩⟩ <;> dsimp <;> simp)
            (map_pair_equifibered _) _).some
      rintro ⟨⟨⟩⟩ <;> dsimp <;>
        exact IsPullback.ofHorizIsIso ⟨(Category.id_comp _).trans (Category.comp_id _).symm⟩)
...
theorem finitaryExtensive_iff_of_isTerminal (C : Type u) [Category.{v} C] [HasFiniteCoproducts C]
  ...
  constructor
  simp_rw [BinaryCofan.is_van_kampen_iff] at H⊢
...
instance types.finitaryExtensive : FinitaryExtensive (Type u) :=
  ...
  apply
    BinaryCofan.is_van_kampen_mk _ _ (fun X Y => Types.binaryCoproductColimit X Y) _
      fun X Y Z f g => (Limits.Types.pullbackLimitCone f g).2
  · intros
    constructor
...
instance : FinitaryExtensive TopCat.{u} :=
  ...
  apply
    BinaryCofan.is_van_kampen_mk _ _ (fun X Y => TopCat.binaryCofanIsColimit X Y) _ fun X Y Z f g =>
  ...
  · intros
    ...
    · refine' ⟨⟨hαX.symm⟩, ⟨PullbackCone.isLimitAux' _ _⟩⟩
      ...
      have : ∀ x, ∃! y, s.fst x = Sum.inl y := by
        ...
        · apply_fun f  at h
          cases
            ((ConcreteCategory.congr_hom s.condition x).symm.trans h).trans
    ...
    · refine' ⟨⟨hαY.symm⟩, ⟨PullbackCone.isLimitAux' _ _⟩⟩
      ...
      exact (funext hl).symm
  · intro Z f
    exact finitaryExtensiveTopAux Z f
...
theorem IsVanKampenColimit.of_iso {F : J ⥤ C} {c c' : Cocone F} (H : IsVanKampenColimit c)
  ...
  exact (IsPullback.ofVertIsIso ⟨by simp⟩).paste_vert_iff (NatTrans.congr_app h j).symm
...
theorem IsVanKampenColimit.of_map {D : Type _} [Category D] (G : C ⥤ D) {F : J ⥤ C} {c : Cocone F}
  ...
  refine'
    (Iff.trans _
          (H (G.map_cocone c') (whiskerRight α G) (G.map f)
            (by
              ext j
...
instance [HasPullbacks C] [FinitaryExtensive C] : FinitaryExtensive (D ⥤ C) :=
  haveI : HasFiniteCoproducts (D ⥤ C) := ⟨fun n => Limits.functorCategoryHasColimitsOfShape⟩
  ⟨fun X Y c hc =>
    isVanKampenColimit_of_evaluation _ c fun x =>
      FinitaryExtensive.van_kampen _ <| PreservesColimit.preserves hc⟩
...
theorem finitaryExtensiveOfPreservesAndReflectsIsomorphism (F : C ⥤ D) [FinitaryExtensive D]
  ...
  exact finitaryExtensiveOfPreservesAndReflects F
