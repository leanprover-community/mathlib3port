...
instance category : category ( Quotientₓ r ) where Hom := hom r id a := Quot.mk _ ( 𝟙 a.as ) comp := comp r
...
-- failed to format: format: uncaught backtrack exception
instance : ess_surj ( Functor r ) where mem_ess_image Y := ⟨ Y.as , ⟨ eq_to_iso ( by ext rfl ) ⟩ ⟩
