    ...
    where
      Hom
          ...
          match
            X , Y
            with
            | inl X , inl Y => X ⟶ Y | inl X , inr Y => Pempty | inr X , inl Y => Pempty | inr X , inr Y => X ⟶ Y
        id X := match X with | inl X => 𝟙 X | inr X => 𝟙 X
        comp
          X Y Z f g
          :=
          match X , Y , Z , f , g with | inl X , inl Y , inl Z , f , g => f ≫ g | inr X , inr Y , inr Z , f , g => f ≫ g
...
@[simps]
def inl_ : C ⥤ Sum C D :=
  { obj := fun X => inl X, map := fun X Y f => f }

/--  `inr_` is the functor `X ↦ inr X`. -/
@[simps]
def inr_ : D ⥤ Sum C D :=
  { obj := fun X => inr X, map := fun X Y f => f }
