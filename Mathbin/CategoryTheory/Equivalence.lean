...
theorem equivalence_mk'_counit (functor inverse unit_iso counit_iso f) :
    (⟨functor, assoc, unit_iso, counit_iso, f⟩ : C ≌ D).counit = counit_iso.hom :=
  rfl
...
#align category_theory.equivalence.adjointify_η_ε CategoryTheory.Equivalence.adjointifyη_ε
...
protected def mk (F : C ⥤ D) (G : D ⥤ C) (η : 𝟭 C ≅ F ⋙ G) (ε : G ⋙ F ≅ 𝟭 D) : C ≌ D :=
  ⟨F, G, adjointifyη η ε, ε, adjointifyη_ε η ε⟩
...
def trans (e : C ≌ D) (f : D ≌ E) : C ≌ E
  ...
  unitIso := by
    ...
    exact isoWhiskerLeft e.functor (isoWhiskerRight f.unit_iso e.inverse)
  counitIso := by
    refine' Iso.trans _ f.counit_iso
    exact isoWhiskerLeft f.inverse (isoWhiskerRight e.counit_iso f.functor)
...
def funInvIdAssoc (e : C ≌ D) (F : C ⥤ E) : e.functor ⋙ e.inverse ⋙ F ≅ F :=
  (Functor.associator _ _ _).map_injective ≪≫ isoWhiskerRight e.unitIso.symm F ≪≫ F.leftUnitor
...
def pow (e : C ≌ C) : ℤ → (C ≌ C)
  | int.of_nat n => e.powNat n
  | int.neg_succ_of_nat n => e.symm.powNat (n + 1)
...
protected def mk {F : C ⥤ D} (G : D ⥤ C) (η : 𝟭 C ≅ F ⋙ G) (ε : G ⋙ F ≅ 𝟭 D) : IsEquivalence F :=
  ⟨G, adjointifyη η ε, ε, adjointifyη_ε η ε⟩
...
instance essSurj_inducedFunctor {C' : Type _} (e : C' ≃ D) : EssSurj (inducedFunctor e)
    where mem_essImage Y := ⟨e.symm Y, by simp⟩
#align category_theory.equivalence.ess_surj_induced_functor CategoryTheory.Equivalence.essSurj_inducedFunctor
