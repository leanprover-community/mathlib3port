...
theorem counit_inv_functor_comp (e : C ≌ D) (X : C) :
    e.counit_inv.app (e.functor.obj X) ≫ e.functor.map (e.unit_inv.app X) = 𝟙 (e.functor.obj X) := by
...
theorem counit_inv_app_functor (e : C ≌ D) (X : C) :
    e.counit_inv.app (e.functor.obj X) = e.functor.map (e.unit.app X) := by
  symm
  erw [← iso.comp_hom_eq_id (e.counit_iso.app _), functor_unit_comp]
  rfl

theorem counit_app_functor (e : C ≌ D) (X : C) : e.counit.app (e.functor.obj X) = e.functor.map (e.unit_inv.app X) := by
...
theorem inverse_counit_inv_comp (e : C ≌ D) (Y : D) :
    e.inverse.map (e.counit_inv.app Y) ≫ e.unit_inv.app (e.inverse.obj Y) = 𝟙 (e.inverse.obj Y) := by
  erw [iso.inv_eq_inv (e.unit_iso.app (e.inverse.obj Y) ≪≫ e.inverse.map_iso (e.counit_iso.app Y)) (iso.refl _)]
  exact e.unit_inverse_comp Y

theorem unit_app_inverse (e : C ≌ D) (Y : D) : e.unit.app (e.inverse.obj Y) = e.inverse.map (e.counit_inv.app Y) := by
...
theorem adjointify_η_ε (X : C) : F.map ((adjointify_η η ε).Hom.app X) ≫ ε.hom.app (F.obj X) = 𝟙 (F.obj X) := by
  dsimp [adjointify_η]
  simp
...
theorem fun_inv_id_assoc_hom_app (e : C ≌ D) (F : C ⥤ E) (X : C) :
    (fun_inv_id_assoc e F).Hom.app X = F.map (e.unit_inv.app X) := by
  dsimp [fun_inv_id_assoc]
  tidy

@[simp]
theorem fun_inv_id_assoc_inv_app (e : C ≌ D) (F : C ⥤ E) (X : C) :
    (fun_inv_id_assoc e F).inv.app X = F.map (e.unit.app X) := by
  dsimp [fun_inv_id_assoc]
  tidy
...
theorem inv_fun_id_assoc_hom_app (e : C ≌ D) (F : D ⥤ E) (X : D) :
    (inv_fun_id_assoc e F).Hom.app X = F.map (e.counit.app X) := by
  dsimp [inv_fun_id_assoc]
  tidy

@[simp]
theorem inv_fun_id_assoc_inv_app (e : C ≌ D) (F : D ⥤ E) (X : D) :
    (inv_fun_id_assoc e F).inv.app X = F.map (e.counit_inv.app X) := by
  dsimp [inv_fun_id_assoc]
  tidy
...
theorem cancel_unit_inv_right {X Y : C} (f f' : X ⟶ e.inverse.obj (e.functor.obj Y)) :
    f ≫ e.unit_inv.app Y = f' ≫ e.unit_inv.app Y ↔ f = f' := by
  simp only [cancel_mono]
...
theorem cancel_counit_inv_right_assoc {W X X' Y : D} (f : W ⟶ X) (g : X ⟶ Y) (f' : W ⟶ X') (g' : X' ⟶ Y) :
    f ≫ g ≫ e.counit_inv.app Y = f' ≫ g' ≫ e.counit_inv.app Y ↔ f ≫ g = f' ≫ g' := by
  simp only [← category.assoc, cancel_mono]
...
theorem cancel_counit_inv_right_assoc' {W X X' Y Y' Z : D} (f : W ⟶ X) (g : X ⟶ Y) (h : Y ⟶ Z) (f' : W ⟶ X')
    (g' : X' ⟶ Y') (h' : Y' ⟶ Z) :
    f ≫ g ≫ h ≫ e.counit_inv.app Z = f' ≫ g' ≫ h' ≫ e.counit_inv.app Z ↔ f ≫ g ≫ h = f' ≫ g' ≫ h' := by
  simp only [← category.assoc, cancel_mono]
...
@[simp]
theorem as_equivalence_unit {F : C ⥤ D} [h : is_equivalence F] :
    F.as_equivalence.unit_iso = @is_equivalence.unit_iso _ _ h :=
  rfl

@[simp]
theorem as_equivalence_counit {F : C ⥤ D} [is_equivalence F] :
...
@[simp]
theorem functor_as_equivalence (E : C ≌ D) : E.functor.as_equivalence = E := by
  cases E
  congr

@[simp]
theorem inverse_as_equivalence (E : C ≌ D) : E.inverse.as_equivalence = E.symm := by
  cases E
  congr
...
theorem fun_inv_map (F : C ⥤ D) [is_equivalence F] (X Y : D) (f : X ⟶ Y) :
    F.map (F.inv.map f) = F.as_equivalence.counit.app X ≫ f ≫ F.as_equivalence.counit_inv.app Y := by
  erw [nat_iso.naturality_2]
  rfl

@[simp]
theorem inv_fun_map (F : C ⥤ D) [is_equivalence F] (X Y : C) (f : X ⟶ Y) :
    F.inv.map (F.map f) = F.as_equivalence.unit_inv.app X ≫ f ≫ F.as_equivalence.unit.app Y := by
...
See https://stacks.math.columbia.edu/tag/02C3.
-/
instance (priority := 100) faithful_of_equivalence (F : C ⥤ D) [is_equivalence F] : faithful F where
...
@[simps]
private noncomputable def equivalence_inverse (F : C ⥤ D) [full F] [faithful F] [ess_surj F] : D ⥤ C where
...
instance ess_surj_induced_functor {C' : Type _} (e : C' ≃ D) : ess_surj (induced_functor e) where
  mem_ess_image := fun Y =>
    ⟨e.symm Y, by
      simp ⟩
