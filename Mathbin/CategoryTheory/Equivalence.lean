...
instance of_equivalence (F : C ≌ D) : IsEquivalence F.Functor :=
  { F with }

instance of_equivalence_inverse (F : C ≌ D) : IsEquivalence F.inverse :=
...
instance is_equivalence_inv (F : C ⥤ D) [IsEquivalence F] : IsEquivalence F.inv :=
...
@[simp]
theorem inv_invₓ (F : C ⥤ D) [IsEquivalence F] : inv (inv F) = F :=
  rfl

variable {E : Type u₃} [Category.{v₃} E]

instance is_equivalence_trans (F : C ⥤ D) (G : D ⥤ E) [IsEquivalence F] [IsEquivalence G] : IsEquivalence (F ⋙ G) :=
...
See https://stacks.math.columbia.edu/tag/02C3.
-/
instance (priority := 100) full_of_equivalence (F : C ⥤ D) [IsEquivalence F] : Full F where
...
See https://stacks.math.columbia.edu/tag/02C3.
-/
noncomputable def of_fully_faithfully_ess_surj (F : C ⥤ D) [Full F] [Faithful F] [EssSurj F] : IsEquivalence F :=
...
noncomputable instance induced_functor_of_equiv {C' : Type _} (e : C' ≃ D) : IsEquivalence (inducedFunctor e) :=
  Equivalence.ofFullyFaithfullyEssSurj _

noncomputable instance fully_faithful_to_ess_image (F : C ⥤ D) [Full F] [Faithful F] : IsEquivalence F.toEssImage :=
