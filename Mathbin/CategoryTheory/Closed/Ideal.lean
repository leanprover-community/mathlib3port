...
def exponentialIdealReflective (A : C) [Reflective i] [ExponentialIdeal i] :
  ...
  · intro X
    haveI := (ExponentialIdeal.exp_closed (i.obj_mem_ess_image X) A).unit_isIso
...
instance (priority := 10) exponentialIdealOfPreservesBinaryProducts
  ...
  apply CartesianClosed.curry (ir.hom_equiv _ _ _)
  ...
  have : η.app (A ⟹ i.obj B) ≫ q = 𝟙 (A ⟹ i.obj B) :=
    ...
    rw [← curry_natural_left, curry_eq_iff, uncurry_id_eq_ev, ← ir.hom_equiv_naturality_left,
      ir.hom_equiv_apply_eq, assoc, assoc, prodComparison_natural_assoc, L.map_id, ←
...
theorem bijection_symm_apply_id (A B : C) : (bijection i A B _).symm (𝟙 _) = prodComparison _ _ _ :=
  ...
  rw [comp_id, comp_id, comp_id, i.map_id, comp_id, unitCompPartialBijective_symm_apply,
    unitCompPartialBijective_symm_apply, uncurry_natural_left, uncurry_curry, uncurry_natural_left,
    uncurry_curry, prod.lift_map_assoc, comp_id, prod.lift_map_assoc, comp_id, prod.comp_lift_assoc,
    prod.lift_snd, prod.lift_fst_assoc, prod.lift_fst_comp_snd_comp, ← Adjunction.eq_homEquiv_apply,
    Adjunction.homEquiv_unit, Iso.comp_inv_eq, assoc, PreservesLimitPair.iso_hom]
  apply prod.hom_ext
