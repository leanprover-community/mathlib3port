...
-/
def unique_homset_of_initial_iso_terminal [HasInitial C] (i : ⊥_ C ≅ ⊤_ C) (X Y : C) : Unique (X ⟶ Y) :=
  Equivₓ.unique <|
    calc
...
def unique_homset_of_zero [HasZeroObject C] (X Y : C) : Unique (X ⟶ Y) := by
...
one morphism.
-/
def equiv_punit [HasZeroObject C] : C ≌ Discrete PUnit :=
