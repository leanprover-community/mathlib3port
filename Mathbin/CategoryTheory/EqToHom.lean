...
@[simp]
theorem eq_to_hom_unop {X Y : Cᵒᵖ} (h : X = Y) :
    (eqToHom h).unop = eqToHom (congr_arg unop h.symm) :=
  by
  cases h
  rfl
...
-/
theorem eq_to_hom_map (F : C ⥤ D) {X Y : C} (p : X = Y) :
...
-/
theorem eq_to_iso_map (F : C ⥤ D) {X Y : C} (p : X = Y) :
