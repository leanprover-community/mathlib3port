...
theorem exact_iff_image_eq_kernel : Exact f g ↔ imageSubobject f = kernelSubobject g :=
  ...
  · intro h
    ...
    · suffices IsIso (imageToKernel _ _ h.w) by exact asIso (imageToKernel _ _ h.w)
  ...
  · refine' fun h => ⟨h.1, _⟩
    ...
    · have :
        imageToKernel f g h.1 =
          (IsLimit.conePointUniqueUpToIso hl (limit.isLimit _)).hom ≫ (kernelSubobjectIso _).inv :=
        by
        ext
        simp
      rw [this]
    ...
    · refine' fun W u hu =>
        kernel.lift (cokernel.π f) u _ ≫ (imageIsoImage f).hom ≫ (imageSubobjectIso _).inv
      rw [← kernel.lift_ι g u hu, Category.assoc, h.2, HasZeroMorphisms.comp_zero]
    · tidy
    · intros
      rw [← cancel_mono (imageSubobject f).arrow, w]
      simp
...
def isColimitCoimage (h : Exact f g) :
  ...
  · refine' fun W u hu => cokernel.desc (kernel.ι g) u _
    rw [← cokernel.π_desc f u hu, ← Category.assoc, h.2, HasZeroMorphisms.zero_comp]
  tidy
...
theorem map_exact {X Y Z : A} (f : X ⟶ Y) (g : Y ⟶ Z) (e1 : Exact f g) :
  ...
  rw [Fork.ι_ofι, Cofork.π_ofπ, ← L.map_comp, kernel_comp_cokernel _ _ e1, L.map_zero]
    ...
    have hf :=
      (isLimitMapConeForkEquiv' L (KernelFork.condition c)).symm
        (isLimitOfExactOfMono (L.map (Fork.ι c)) (L.map f)
          (h
            (exact_of_is_kernel (Fork.ι c) f (KernelFork.condition c)
              (ic.of_iso_limit (isoOfι _)))))
    exact hf.of_iso_limit ((Cones.functoriality _ L).mapIso (isoOfι _).symm)
    ...
    have hf :=
      (isColimitMapCoconeCoforkEquiv' L (CokernelCofork.condition c)).symm
        (isColimitOfExactOfEpi (L.map f) (L.map (Cofork.π c))
          (h
            (exact_of_is_cokernel f (Cofork.π c) (CokernelCofork.condition c)
              (ic.of_iso_colimit (isoOfπ _)))))
    exact hf.of_iso_colimit ((Cocones.functoriality _ L).mapIso (isoOfπ _).symm)
