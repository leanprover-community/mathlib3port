...
theorem exact_tfae :
  tfae [exact f g, f ≫ g = 0 ∧ kernel.ι g ≫ cokernel.π f = 0, image_subobject f = kernel_subobject g] :=
  by 
    tfaeHave 1 ↔ 2
    ·
      apply exact_iff 
...
def is_colimit_coimage [h : exact f g] :
  is_colimit (cokernel_cofork.of_π (coimages.coimage.π g) (coimages.comp_coimage_π_eq_zero h.1) : cokernel_cofork f) :=
  by 
    rw [exact_iff] at h 
...
instance  [exact f g] :
  ...
  suffices h :
    cokernel.desc f g
        (by 
          simp ) =
      (is_colimit.cocone_point_unique_up_to_iso (colimit.is_colimit _) (is_colimit_image f g)).Hom ≫ image.ι g by
