...
instance
  : abelian ( C ᵒᵖ )
  where
    NormalMono X Y f m := by exact normal_mono_of_normal_epi_unop _ ( abelian.normal_epi f.unop )
      NormalEpi X Y f m := by exact normal_epi_of_normal_mono_unop _ ( abelian.normal_mono f.unop )
