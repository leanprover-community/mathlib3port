...
theorem is_connected.of_induct [Nonempty J] {j₀ : J}
  ...
  is_connected.of_constant_of_preserves_morphisms fun α F a => by
    have w :=
...
theorem is_preconnected_of_equivalent {K : Type u₁} [category.{v₂} K] [is_preconnected J] (e : J ≌ K) :
  ...
  { iso_constant := fun α F k =>
      ⟨calc
          F ≅ e.inverse ⋙ e.functor ⋙ F := (e.inv_fun_id_assoc F).symm
...
instance is_preconnected_op [is_preconnected J] : is_preconnected (Jᵒᵖ) where
...
-/
theorem zigzag_is_connected [Nonempty J] (h : ∀ j₁ j₂ : J, zigzag j₁ j₂) : is_connected J := by
...
def discrete_is_connected_equiv_punit {α : Type u₁} [is_connected (discrete α)] : α ≃ PUnit :=
  discrete.equiv_of_equivalence.{u₁, u₁}
    { Functor := functor.star α, inverse := discrete.functor fun _ => Classical.arbitrary _,
      unitIso := iso_constant _ (Classical.arbitrary _), counitIso := functor.punit_ext _ _ }
...
instance [is_connected J] : full (Functor.Const J : C ⥤ J ⥤ C) where
