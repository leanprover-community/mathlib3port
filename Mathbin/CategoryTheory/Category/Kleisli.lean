...
instance Kleisli.category_struct {m} [Monadₓ.{u, v} m] : category_struct (Kleisli m) :=
  { Hom := fun α β => α → m β, id := fun α x => pure x, comp := fun X Y Z f g => f >=> g }

instance Kleisli.category {m} [Monadₓ.{u, v} m] [IsLawfulMonad m] : category (Kleisli m) :=
  by 
    refine' { id_comp' := _, comp_id' := _, assoc' := _ } <;>
      intros  <;> ext <;> unfoldProjs <;> simp' only [· >=> ·] with functor_norm
