...
-/
def skeleton : Type u₁ :=
  induced_category C Quotientₓ.out deriving [anonymous]
...
instance : ess_surj ( from_skeleton C ) where mem_ess_image X := ⟨ Quotientₓ.mk X , Quotientₓ.mk_out X ⟩
...
-- failed to format: format: uncaught backtrack exception
instance
  thin_skeleton.preorder
  : Preorderₓ ( thin_skeleton C )
  where
    le
        :=
        Quotientₓ.lift₂
          ( fun X Y => Nonempty ( X ⟶ Y ) )
            (
              by
                rintro _ _ _ _ ⟨ i₁ ⟩ ⟨ i₂ ⟩
                  exact
                    propext ⟨ Nonempty.map fun f => i₁.inv ≫ f ≫ i₂.hom , Nonempty.map fun f => i₁.hom ≫ f ≫ i₂.inv ⟩
              )
      le_refl := by refine' Quotientₓ.ind fun a => _ exact ⟨ 𝟙 _ ⟩
      le_trans a b c := Quotientₓ.induction_on₃ a b c $ fun A B C => Nonempty.map2 ( · ≫ · )
...
noncomputable instance from_thin_skeleton_equivalence : is_equivalence (from_thin_skeleton C) where
  ...
  unitIso :=
    nat_iso.of_components
      (fun x =>
        Quotientₓ.recOnSubsingleton x fun X => eq_to_iso (Quotientₓ.sound ⟨(Nonempty.some (Quotientₓ.mk_out X)).symm⟩))
      (by
        tidy)
