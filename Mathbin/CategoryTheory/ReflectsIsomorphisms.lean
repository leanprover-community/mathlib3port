...
instance (priority := 100) of_full_and_faithful (F : C ⥤ D) [full F] [faithful F] : reflects_isomorphisms F where
...
instance (F : C ⥤ D) (G : D ⥤ E) [reflects_isomorphisms F] [reflects_isomorphisms G] : reflects_isomorphisms (F ⋙ G) :=
  ⟨fun _ _ f hf : is_iso (G.map _) => by
    skip
