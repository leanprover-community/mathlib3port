...
theorem mk_injective (A B : T) : Function.Injective (arrow.mk : (A ⟶ B) → arrow T) :=
  fun f g h =>
    by 
      cases h 
      rfl
...
@[simp]
theorem left_hom_inv_right [is_iso sq] : sq.left ≫ g.hom ≫ inv sq.right = f.hom :=
  by 
    simp only [←category.assoc, is_iso.comp_inv_eq, w]

theorem inv_left_hom_right [is_iso sq] : inv sq.left ≫ f.hom ≫ sq.right = g.hom :=
  by 
...
theorem lift.fac_left {f g : arrow T} (sq : f ⟶ g) [HasLift sq] : f.hom ≫ lift sq = sq.left :=
  by 
    simp 

theorem lift.fac_right {f g : arrow T} (sq : f ⟶ g) [HasLift sq] : lift sq ≫ g.hom = sq.right :=
  by 
    simp 
...
theorem lift_mk'_left {X Y P Q : T} {f : X ⟶ Y} {g : P ⟶ Q} {u : X ⟶ P} {v : Y ⟶ Q} (h : u ≫ g = f ≫ v)
  [HasLift$ arrow.hom_mk' h] : f ≫ lift (arrow.hom_mk' h) = u :=
  by 
...
theorem lift_mk'_right {X Y P Q : T} {f : X ⟶ Y} {g : P ⟶ Q} {u : X ⟶ P} {v : Y ⟶ Q} (h : u ≫ g = f ≫ v)
  [HasLift$ arrow.hom_mk' h] : lift (arrow.hom_mk' h) ≫ g = v :=
  by 
