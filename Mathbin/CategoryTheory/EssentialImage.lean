...
def to_ess_image (F : C ⥤ D) : C ⥤ F.ess_image :=
  { obj := fun X => ⟨_, obj_mem_ess_image _ X⟩, map := fun X Y f => (ess_image_inclusion F).Preimage (F.map f) }
...
instance
  : ess_surj F.to_ess_image
  where mem_ess_image ⟨ Y , hY ⟩ := ⟨ _ , ⟨ ⟨ _ , _ , hY.get_iso.hom_inv_id , hY.get_iso.inv_hom_id ⟩ ⟩ ⟩
...
instance full.to_ess_image (F : C ⥤ D) [full F] : full F.to_ess_image := by
