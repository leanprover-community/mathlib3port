...
/--
  ...
  instance
    category.opposite
    : category .{ v₁ } ( C ᵒᵖ )
    where comp _ _ _ f g := ( g.unop ≫ f.unop ) . op id X := ( 𝟙 ( unop X ) ) . op
...
def op_op_equivalence : Cᵒᵖᵒᵖ ≌ C :=
  { Functor := op_op C, inverse := unop_unop C, unitIso := iso.refl (𝟭 (Cᵒᵖᵒᵖ)),
...
instance
  ...
  where map_injective' X Y f g h := Quiver.Hom.unop_inj $ by simpa using map_injective F ( Quiver.Hom.op_inj h )

-- failed to format: format: uncaught backtrack exception
/-- If F is faithful then the right_op of F is also faithful. -/
  instance
    right_op_faithful
    { F : C ᵒᵖ ⥤ D } [ faithful F ] : faithful F.right_op
...
/-- If F is faithful then the left_op of F is also faithful. -/
  instance
    left_op_faithful
    { F : C ⥤ D ᵒᵖ } [ faithful F ] : faithful F.left_op
