...
-/
@[simps]
def from_costructured_arrow (F : «expr ᵒᵖ» C ⥤ Type v) : «expr ᵒᵖ» (costructured_arrow yoneda F) ⥤ F.elements :=
...
theorem from_to_costructured_arrow_eq (F : «expr ᵒᵖ» C ⥤ Type v) :
...
-/
theorem costructured_arrow_yoneda_equivalence_naturality {F₁ F₂ : «expr ᵒᵖ» C ⥤ Type v} (α : F₁ ⟶ F₂) :
  ...
  by 
    ...
    ·
      ...
      ext 
      have  :
        ∀ {F : «expr ᵒᵖ» C ⥤ Type v} {a b : costructured_arrow yoneda F} H : a = b,
