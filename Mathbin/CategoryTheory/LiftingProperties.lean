...
theorem right_lifting_property_initial_iff (i p : arrow C) (h : is_initial i.left) :
    has_lifting_property i p ↔ ∀ {e : i.right ⟶ p.right}, ∃ l : i.right ⟶ p.left, l ≫ p.hom = e := by
  fconstructor
  · intros hlift e
...
instance right_lifting_subcat.inhabited (R : Type u) [Inhabited R] : Inhabited (right_lifting_subcat R) where
  default := (default : R)
...
theorem id_has_right_lifting_property' {F : D → arrow C} (X : C) :
    ∀ i : D, has_lifting_property (F i) (arrow.mk (𝟙 X)) := fun i => id_has_right_lifting_property (F i)
...
def right_lifting_subcategory (F : D → arrow C) : category (right_lifting_subcat C) where
