...
theorem comap_eq_symm {β γ : Type w} {f g : β → γ} (h : f = g) : comap_eq C h.symm = (comap_eq C h).symm :=
  by 
    tidy
...
instance has_shift {β : Type _} [AddCommGroupₓ β] (s : β) : has_shift (graded_object_with_shift s C) :=
  { shift :=
      comap_equiv C
...
instance has_zero_morphisms [has_zero_morphisms C] (β : Type w) : has_zero_morphisms.{max w v} (graded_object β C) :=
