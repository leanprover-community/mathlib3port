...
instance : category .{ v } ( with_terminal C ) where Hom X Y := hom X Y id X := id _ comp X Y Z f g := comp f g
...
-- failed to format: format: uncaught backtrack exception
instance : category .{ v } ( with_initial C ) where Hom X Y := hom X Y id X := id _ comp X Y Z f g := comp f g
