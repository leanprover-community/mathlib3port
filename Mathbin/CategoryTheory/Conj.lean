...
@[simp]
theorem hom_congr_refl {X Y : C} (f : X ⟶ Y) : (iso.refl X).homCongr (iso.refl Y) f = f :=
  by 
    simp 

@[simp]
theorem hom_congr_trans {X₁ Y₁ X₂ Y₂ X₃ Y₃ : C} (α₁ : X₁ ≅ X₂) (β₁ : Y₁ ≅ Y₂) (α₂ : X₂ ≅ X₃) (β₂ : Y₂ ≅ Y₃)
  (f : X₁ ⟶ Y₁) : (α₁ ≪≫ α₂).homCongr (β₁ ≪≫ β₂) f = (α₁.hom_congr β₁).trans (α₂.hom_congr β₂) f :=
  by 
    simp 
...
@[simp]
theorem symm_self_conj (f : End X) : α.symm.conj (α.conj f) = f :=
  by 
...
theorem conj_Aut_apply (f : Aut X) : α.conj_Aut f = α.symm ≪≫ f ≪≫ α :=
  by 
...
@[simp]
theorem trans_conj_Aut {Z : C} (β : Y ≅ Z) (f : Aut X) : (α ≪≫ β).conjAut f = β.conj_Aut (α.conj_Aut f) :=
  by 
...
theorem map_conj_Aut (F : C ⥤ D) {X Y : C} (α : X ≅ Y) (f : Aut X) :
  F.map_iso (α.conj_Aut f) = (F.map_iso α).conjAut (F.map_iso f) :=
  by 
