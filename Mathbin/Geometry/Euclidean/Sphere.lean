...
-- error in Geometry.Euclidean.Sphere: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
(hab : «expr ≠ »(a, b))
(hcd : «expr ≠ »(c, d))
(hapb : «expr = »(«expr∠»() a p b, 0))
(hcpd : «expr = »(«expr∠»() c p d, 0)) : «expr = »(«expr * »(dist a p, dist b p), «expr * »(dist c p, dist d p)) :=
begin
  obtain ["⟨", "-", ",", ident k₁, ",", "-", ",", ident hab₁, "⟩", ":=", expr angle_eq_zero_iff.mp hapb],
  obtain ["⟨", "-", ",", ident k₂, ",", "-", ",", ident hcd₁, "⟩", ":=", expr angle_eq_zero_iff.mp hcpd],
  refine [expr mul_dist_eq_mul_dist_of_cospherical h ⟨k₁, _, hab₁⟩ ⟨k₂, _, hcd₁⟩]; by_contra [ident hnot]; simp [] [] ["only"] ["[", expr not_not, ",", "*", ",", expr one_smul, "]"] [] ["at", "*"],
  exacts ["[", expr hab (vsub_left_cancel hab₁).symm, ",", expr hcd (vsub_left_cancel hcd₁).symm, "]"]
end
