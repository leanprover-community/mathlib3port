...
theorem monge_point_vsub_face_centroid_weights_with_circumcenter_eq_sub {n : ℕ} {i₁ i₂ : Finₓ (n+3)} (h : i₁ ≠ i₂) :
  monge_point_vsub_face_centroid_weights_with_circumcenter i₁ i₂ =
    monge_point_weights_with_circumcenter n - centroid_weights_with_circumcenter ({i₁, i₂}ᶜ) :=
  by 
    ext i 
    cases i
    ·
      ...
      have hu : card ({i₁, i₂}ᶜ : Finset (Finₓ (n+3))) = n+1
      ·
...
theorem monge_point_vsub_face_centroid_eq_weighted_vsub_of_points_with_circumcenter {n : ℕ} (s : simplex ℝ P (n+2))
  {i₁ i₂ : Finₓ (n+3)} (h : i₁ ≠ i₂) :
  s.monge_point -ᵥ ({i₁, i₂}ᶜ : Finset (Finₓ (n+3))).centroid ℝ s.points =
...
theorem inner_monge_point_vsub_face_centroid_vsub {n : ℕ} (s : simplex ℝ P (n+2)) {i₁ i₂ : Finₓ (n+3)} (h : i₁ ≠ i₂) :
  ⟪s.monge_point -ᵥ ({i₁, i₂}ᶜ : Finset (Finₓ (n+3))).centroid ℝ s.points, s.points i₁ -ᵥ s.points i₂⟫ = 0 :=
  by 
    simpRw [monge_point_vsub_face_centroid_eq_weighted_vsub_of_points_with_circumcenter s h,
    ...
    have hfs : ∀ i : Finₓ (n+3), i ∉ fs → i ≠ i₁ ∧ i ≠ i₂
    ·
      intro i hi 
      constructor <;>
        ·
    ...
    ·
      simpRw [sum_points_with_circumcenter, points_with_circumcenter_eq_circumcenter, points_with_circumcenter_point,
        Pi.sub_apply, point_weights_with_circumcenter]
      rw [←sum_subset fs.subset_univ _]
      ·
        simpRw [sum_insert (not_mem_singleton.2 h), sum_singleton]
        repeat' 
          rw [←sum_subset fs.subset_univ _]
        ·
          simpRw [sum_insert (not_mem_singleton.2 h), sum_singleton]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∈ » affine_span exprℝ() (set.range t.points))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (p₁ «expr ∈ » set.range p)
