...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » e.source)
          ...
          locality'
              ...
              by
                intro e h
                  ...
                  · rw [ e.restr_source ] refine' ⟨ hx , _ ⟩ rw [ hs.interior_eq ] exact hxs
                  simpa only with mfld_simps using LocalHomeomorph.EqOnSource.eq_on hes' hes
            ,
          eq_on_source' := by rintro e e' ⟨ s , hs , hse ⟩ hee' exact ⟨ s , hs , Setoidₓ.trans hee' hse ⟩
        }
...
theorem ChartedSpace.second_countable_of_countable_cover [second_countable_topology H] {s : Set M}
  ...
  by 
    have  : ∀ x : M, second_countable_topology (chart_at H x).Source :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (e' «expr ∈ » atlas H M)
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/--
