...
theorem lift_prop_on_univ : lift_prop_on P g univ ↔ lift_prop P g := by
...
theorem lift_prop_within_at_indep_chart_aux (he : e ∈ G.maximal_atlas M) (xe : x ∈ e.source)
  ...
  obtain ⟨o, o_open, xo, oe, oe', of, of'⟩ :
    ∃ o : Set M,
      IsOpen o ∧ x ∈ o ∧ o ⊆ e.source ∧ o ⊆ e'.source ∧ o ∩ s ⊆ g ⁻¹' f.source ∧ o ∩ s ⊆ g ⁻¹' f'.to_local_equiv.source
  ·
...
theorem lift_prop_on_indep_chart [HasGroupoid M G] [HasGroupoid M' G'] (he : e ∈ G.maximal_atlas M)
    (hf : f ∈ G'.maximal_atlas M') (h : lift_prop_on P g s) :
    ∀,
...
theorem lift_prop_within_at_inter' (ht : t ∈ 𝓝[s] x) :
    lift_prop_within_at P g (s ∩ t) x ↔ lift_prop_within_at P g s x := by
...
theorem lift_prop_at_of_lift_prop_within_at (h : lift_prop_within_at P g s x) (hs : s ∈ 𝓝 x) : lift_prop_at P g x := by
  have : s = univ ∩ s := by
    rw [univ_inter]
  rwa [this, hG.lift_prop_within_at_inter hs] at h

theorem lift_prop_within_at_of_lift_prop_at_of_mem_nhds (h : lift_prop_at P g x) (hs : s ∈ 𝓝 x) :
    lift_prop_within_at P g s x := by
  have : s = univ ∩ s := by
    rw [univ_inter]
  rwa [this, hG.lift_prop_within_at_inter hs]

theorem lift_prop_on_of_locally_lift_prop_on (h : ∀, ∀ x ∈ s, ∀, ∃ u, IsOpen u ∧ x ∈ u ∧ lift_prop_on P g (s ∩ u)) :
...
theorem lift_prop_of_locally_lift_prop_on (h : ∀ x, ∃ u, IsOpen u ∧ x ∈ u ∧ lift_prop_on P g u) : lift_prop P g := by
...
theorem lift_prop_within_at_congr_of_eventually_eq (h : lift_prop_within_at P g s x) (h₁ : g' =ᶠ[𝓝[s] x] g)
    (hx : g' x = g x) : lift_prop_within_at P g' s x := by
...
theorem lift_prop_at_congr_of_eventually_eq (h : lift_prop_at P g x) (h₁ : g' =ᶠ[𝓝 x] g) : lift_prop_at P g' x := by
...
theorem lift_prop_on_congr (h : lift_prop_on P g s) (h₁ : ∀, ∀ y ∈ s, ∀, g' y = g y) : lift_prop_on P g' s :=
...
theorem lift_prop_within_at_mono (mono : ∀ ⦃s x t⦄ ⦃f : H → H'⦄, t ⊆ s → P f s x → P f t x)
    (h : lift_prop_within_at P g t x) (hst : s ⊆ t) : lift_prop_within_at P g s x := by
...
theorem lift_prop_within_at_of_lift_prop_at (mono : ∀ ⦃s x t⦄ ⦃f : H → H'⦄, t ⊆ s → P f s x → P f t x)
    (h : lift_prop_at P g x) : lift_prop_within_at P g s x := by
...
theorem lift_prop_at_of_mem_maximal_atlas [HasGroupoid M G] (hG : G.local_invariant_prop G Q) (hQ : ∀ y, Q id univ y)
    (he : e ∈ maximal_atlas M G) (hx : x ∈ e.source) : lift_prop_at Q e x := by
  ...
  apply hG.congr _ _ A <;> simp' (config := { contextual := Bool.true.0 }) only [hx] with mfld_simps

theorem lift_prop_on_of_mem_maximal_atlas [HasGroupoid M G] (hG : G.local_invariant_prop G Q) (hQ : ∀ y, Q id univ y)
    (he : e ∈ maximal_atlas M G) : lift_prop_on Q e e.source := by
  intro x hx
...
theorem lift_prop_at_symm_of_mem_maximal_atlas [HasGroupoid M G] {x : H} (hG : G.local_invariant_prop G Q)
    (hQ : ∀ y, Q id univ y) (he : e ∈ maximal_atlas M G) (hx : x ∈ e.target) : lift_prop_at Q e.symm x := by
...
theorem lift_prop_on_symm_of_mem_maximal_atlas [HasGroupoid M G] (hG : G.local_invariant_prop G Q)
    (hQ : ∀ y, Q id univ y) (he : e ∈ maximal_atlas M G) : lift_prop_on Q e.symm e.target := by
  intro x hx
...
theorem lift_prop_id (hG : G.local_invariant_prop G Q) (hQ : ∀ y, Q id univ y) : lift_prop Q (id : M → M) := by
  intro x
