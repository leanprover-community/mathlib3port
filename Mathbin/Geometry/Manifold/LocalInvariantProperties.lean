...
theorem lift_prop_within_at_indep_chart_aux (he : e ∈ G.maximal_atlas M) (xe : x ∈ e.source)
  ...
  by 
    ...
    have A : P (f ∘ g ∘ e.symm) (e.target ∩ e.symm ⁻¹' (s ∩ g ⁻¹' f.source) ∩ (e.target ∩ e.symm ⁻¹' o)) (e x)
    ·
    ...
    have C : P (f' ∘ g ∘ e.symm) (e.target ∩ e.symm ⁻¹' (s ∩ g ⁻¹' f.source) ∩ (e.target ∩ e.symm ⁻¹' o)) (e x)
    ·
      refine'
        hG.congr (fun y hy => _)
          (by 
    ...
    rw [this] at E 
    have  : ow = e'.target ∩ e'.symm ⁻¹' (s ∩ g ⁻¹' f'.source) ∩ (w.target ∩ (e'.target ∩ e'.symm ⁻¹' o))
    ·
      ext y 
      constructor
      ·
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » «expr ∩ »(e.target, «expr ⁻¹' »(e.symm, «expr ∩ »(s, «expr ⁻¹' »(g, f.source)))))
theorem lift_prop_on_indep_chart [HasGroupoid M G] [HasGroupoid M' G'] (he : e ∈ G.maximal_atlas M)
  ...
  ∀ y _ : y ∈ e.target ∩ e.symm ⁻¹' (s ∩ g ⁻¹' f.source),
...
theorem lift_prop_within_at_inter' (ht : t ∈ 𝓝[s] x) :
  ...
  by 
    byCases' hcont : ¬ContinuousWithinAt g s x
    ·
      have  : ¬ContinuousWithinAt g (s ∩ t) x
      ·
        rwa [continuous_within_at_inter' ht]
      simp only [lift_prop_within_at, hcont, this, false_andₓ]
    pushNeg  at hcont 
    have A : ContinuousWithinAt g (s ∩ t) x
    ·
      rwa [continuous_within_at_inter' ht]
    ...
    convLHS => rw [hG.is_local B C]
    convRHS => rw [hG.is_local B C]
    congr 2
...
theorem lift_prop_at_of_lift_prop_within_at (h : lift_prop_within_at P g s x) (hs : s ∈ 𝓝 x) : lift_prop_at P g x :=
  by 
    have  : s = univ ∩ s
    ·
      rw [univ_inter]
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
theorem lift_prop_on_of_locally_lift_prop_on (h : ∀ x _ : x ∈ s, ∃ u, IsOpen u ∧ x ∈ u ∧ lift_prop_on P g (s ∩ u)) :
  lift_prop_on P g s :=
  by 
    intro x hx 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » s)
theorem lift_prop_within_at_congr (h : lift_prop_within_at P g s x) (h₁ : ∀ y _ : y ∈ s, g' y = g y) (hx : g' x = g x) :
  lift_prop_within_at P g' s x :=
  by 
...
theorem lift_prop_id (hG : G.local_invariant_prop G Q) (hQ : ∀ y, Q id univ y) : lift_prop Q (id : M → M) :=
  by 
    ...
    ·
      ...
      simp' only with mfld_simps  at hy 
      simp' only [hy] with mfld_simps 
    have  : t = univ ∩ (chart_at H x).Target
    ·
      mfldSetTac 
    rw [this]
    exact
  ...
  { is_local :=
      by 
        ...
        ·
          ...
          ·
            have  : s ∩ u ∩ e.source = s ∩ (e.source ∩ u) :=
              by 
                mfldSetTac 
