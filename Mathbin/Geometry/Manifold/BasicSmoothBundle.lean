...
def trivialBasicSmoothBundleCore {𝕜 : Type _} [NondiscreteNormedField 𝕜] {E : Type _} [NormedGroup E] [NormedSpace 𝕜 E]
    {H : Type _} [TopologicalSpace H] (I : ModelWithCorners 𝕜 E H) (M : Type _) [TopologicalSpace M] [ChartedSpace H M]
    [SmoothManifoldWithCorners I M] (F : Type _) [NormedGroup F] [NormedSpace 𝕜 F] : BasicSmoothBundleCore I M F where
...
of the point `p`. -/
def TangentBundle.proj : TangentBundle I M → M := fun p => p.1
...
instance : HasContinuousSmul 𝕜 (TangentSpace I x) := by
  infer_instance
