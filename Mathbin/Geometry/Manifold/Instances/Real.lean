...
theorem
  range_half_space
  ( n : ℕ ) [ HasZero Finₓ n ] : range fun x : EuclideanHalfSpace n => x.val = { y | 0 ≤ y 0 }
  := by simp
...
theorem
  range_quadrant
  ( n : ℕ ) : range fun x : EuclideanQuadrant n => x.val = { y | ∀ i : Finₓ n , 0 ≤ y i }
  := by simp

end 

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » ({0} : set (fin n)))
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/--
  ...
  def
    ...
    ( n : ℕ ) [ HasZero Finₓ n ] : ModelWithCorners ℝ EuclideanSpace ℝ Finₓ n EuclideanHalfSpace n
    :=
      {
        toFun := Subtype.val ,
          invFun := fun x => ⟨ update x 0 max x 0 0 , by simp [ le_reflₓ ] ⟩ ,
          Source := univ ,
          Target := { x | 0 ≤ x 0 } ,
          ...
          left_inv'
              ...
              fun
                ⟨ xval , xprop ⟩ hx
                  =>
                  by rw [ Subtype.mk_eq_mk , update_eq_iff ] exact ⟨ max_eq_leftₓ xprop , fun i _ => rfl ⟩
            ,
          ...
          continuous_inv_fun
            :=
            continuous_subtype_mk _ $ continuous_id . update 0 $ continuous_apply 0 . max continuous_const
        }
...
/--
  ...
  def
    ...
    ( n : ℕ ) : ModelWithCorners ℝ EuclideanSpace ℝ Finₓ n EuclideanQuadrant n
    :=
      {
        toFun := Subtype.val ,
          invFun := fun x => ⟨ fun i => max x i 0 , fun i => by simp only [ le_reflₓ , or_trueₓ , le_max_iff ] ⟩ ,
          Source := univ ,
          Target := { x | ∀ i , 0 ≤ x i } ,
          map_source' := fun x hx => by simpa only [ Subtype.range_val ] using x.property ,
          map_target' := fun x hx => mem_univ _ ,
          left_inv' := fun ⟨ xval , xprop ⟩ hx => by ext i simp only [ Subtype.coe_mk , xprop i , max_eq_leftₓ ] ,
          right_inv' := fun x hx => by ext1 i simp only [ hx i , max_eq_leftₓ ] ,
          source_eq := rfl ,
          unique_diff'
              :=
              have
                this : UniqueDiffOn ℝ _ := UniqueDiffOn.univ_pi Finₓ n fun _ => ℝ _ fun i => unique_diff_on_Ici 0
          ...
          continuous_inv_fun
            ...
            continuous_subtype_mk _
              $
              continuous_pi $ fun i => continuous_id . max continuous_const . comp continuous_apply i
        }
...
def iccLeftChart (x y : ℝ) [Fact (x < y)] : LocalHomeomorph (Icc x y) (EuclideanHalfSpace 1) :=
  { Source := { z : Icc x y | z.val < y }, Target := { z : EuclideanHalfSpace 1 | z.val 0 < y - x },
...
def iccRightChart (x y : ℝ) [Fact (x < y)] : LocalHomeomorph (Icc x y) (EuclideanHalfSpace 1) :=
  { Source := { z : Icc x y | x < z.val }, Target := { z : EuclideanHalfSpace 1 | z.val 0 < y - x },
...
-/
instance Icc_smooth_manifold (x y : ℝ) [Fact (x < y)] : SmoothManifoldWithCorners (𝓡∂ 1) (Icc x y) :=
  by 
    have M : TimesContDiffOn ℝ ∞ (fun z : EuclideanSpace ℝ (Finₓ 1) => (-z)+fun i => y - x) univ
    ·
