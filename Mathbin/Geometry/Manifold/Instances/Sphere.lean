...
theorem stereo_left_inv (hv : ∥v∥ = 1) {x : sphere (0 : E) 1} (hx : (x : E) ≠ v) :
  ...
  by 
    ...
    set a : ℝ := innerRight v x 
    set y := orthogonalProjection (ℝ∙v)ᗮ x 
    have split : «expr↑ » x = (a • v)+«expr↑ » y
    ·
    ...
    ·
      ...
      ringNF 
      ring 
    convert congr_arg2 Add.add (congr_argₓ (fun t => t • (y : E)) h₁) (congr_argₓ (fun t => t • v) h₂) using 1
    ·
...
theorem stereo_right_inv (hv : ∥v∥ = 1) (w : (ℝ∙v)ᗮ) : stereoToFun v (stereoInvFun hv w) = w :=
  by 
    ...
    ·
      ...
      have h₃ : innerRight v w = (0 : ℝ) := inner_right_of_mem_orthogonal_singleton v w.2
      have h₄ : innerRight v v = (1 : ℝ) :=
        by 
...
theorem TimesContMdiff.cod_restrict_sphere {n : ℕ} [Fact (finrank ℝ E = n+1)] {m : WithTop ℕ} {f : M → E}
  ...
  by 
    ...
    ext x 
    have hfxv : f x = -«expr↑ » v ↔ ⟪f x, -«expr↑ » v⟫_ℝ = 1
    ·
      have hfx : ∥f x∥ = 1 :=
        by 
          simpa using hf' x 
...
instance  : LieGroup (𝓡 1) circle :=
  { Metric.Sphere.smooth_manifold_with_corners with
    smooth_mul :=
      by 
        let c : circle → ℂ := coeₓ 
        have h₁ : TimesContMdiff _ _ _ (Prod.mapₓ c c) :=
