...
variable {M : Type _} [TopologicalSpace M] [ChartedSpace E M] [SmoothManifoldWithCorners 𝓘(ℂ, E) M]

/-- A holomorphic function on a complex manifold is constant on every compact, preconnected, clopen
subset. -/
theorem apply_eq_of_is_compact {s : Set M} (hs₁ : IsCompact s) (hs₂ : IsPreconnected s) (hs₃ : IsClopen s) {f : M → F}
    (hf : Mdifferentiable 𝓘(ℂ, E) 𝓘(ℂ, F) f) {a b : M} (ha : a ∈ s) (hb : b ∈ s) : f a = f b := by
  ...
  suffices s ⊆ { r : M | f r = f p₀ } ∩ s by
    exact (this ha).1.trans (this hb).1.symm
  clear ha hb a b
...
protected theorem is_locally_constant [CompactSpace M] {f : M → F} (hf : Mdifferentiable 𝓘(ℂ, E) 𝓘(ℂ, F) f) :
  ...
  intro s hs hs' x hx y hy
  exact hf.apply_eq_of_is_compact hs'.2.IsCompact hs hs' hy hx

/-- A holomorphic function on a compact connected complex manifold is constant. -/
theorem apply_eq_of_compact_space [CompactSpace M] [PreconnectedSpace M] {f : M → F}
    (hf : Mdifferentiable 𝓘(ℂ, E) 𝓘(ℂ, F) f) (a b : M) : f a = f b :=
  hf.apply_eq_of_is_compact compact_univ is_preconnected_univ is_clopen_univ (Set.mem_univ _) (Set.mem_univ _)

/-- A holomorphic function on a compact connected complex manifold is the constant function `f ≡ v`,
for some value `v`. -/
theorem exists_eq_const_of_compact_space [CompactSpace M] [PreconnectedSpace M] {f : M → F}
    (hf : Mdifferentiable 𝓘(ℂ, E) 𝓘(ℂ, F) f) : ∃ v : F, f = Function.const M v := by
  cases is_empty_or_nonempty M
  · exact ⟨0, funext fun a => h.elim a⟩
    
  · inhabit M
    exact ⟨f default, funext fun a => hf.apply_eq_of_compact_space a default⟩
