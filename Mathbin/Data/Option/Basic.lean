...
protected theorem exists {p : Option α → Prop} : (∃ x, p x) ↔ p none ∨ ∃ x, p (some x) :=
  ⟨fun ⟨x, hx⟩ => (Option.casesOn x Or.inl$ fun x hx => Or.inr ⟨x, hx⟩) hx,
...
@[simp]
theorem not_mem_none (a : α) : a ∉ (none : Option α) :=
  fun h => Option.noConfusion h
...
theorem join_eq_some {x : Option (Option α)} {a : α} : x.join = some a ↔ x = some (some a) :=
  by 
    simp 

theorem join_ne_none {x : Option (Option α)} : x.join ≠ none ↔ ∃ z, x = some (some z) :=
  by 
    simp 
...
theorem join_eq_none {o : Option (Option α)} : o.join = none ↔ o = none ∨ o = some none :=
  by 
    rcases o with (_ | _ | _) <;> simp 

theorem bind_id_eq_join {x : Option (Option α)} : x >>= id = x.join :=
  by 
    simp 
...
theorem map_eq_some {α β} {x : Option α} {f : α → β} {b : β} : f <$> x = some b ↔ ∃ a, x = some a ∧ f a = b :=
  by 
    cases x <;> simp 

@[simp]
theorem map_eq_some' {x : Option α} {f : α → β} {b : β} : x.map f = some b ↔ ∃ a, x = some a ∧ f a = b :=
  by 
    cases x <;> simp 

theorem map_eq_none {α β} {x : Option α} {f : α → β} : f <$> x = none ↔ x = none :=
  by 
    cases x <;> simp only [map_none, map_some, eq_self_iff_true]

@[simp]
theorem map_eq_none' {x : Option α} {f : α → β} : x.map f = none ↔ x = none :=
  by 
    cases x <;> simp only [map_none', map_some', eq_self_iff_true]
...
@[simp]
theorem map_map (h : β → γ) (g : α → β) (x : Option α) : Option.map h (Option.map g x) = Option.map (h ∘ g) x :=
  by 
...
theorem bind_map_comm {α β} {x : Option (Option α)} {f : α → β} : x >>= Option.map f = x.map (Option.map f) >>= id :=
  by 
    cases x <;> simp 

theorem join_map_eq_map_join {f : α → β} {x : Option (Option α)} : (x.map (Option.map f)).join = x.join.map f :=
  by 
    rcases x with (_ | _ | x) <;> simp 

theorem join_join {x : Option (Option (Option α))} : x.join.join = (x.map join).join :=
  by 
...
theorem map_bind {α β γ} (f : β → γ) (x : Option α) (g : α → Option β) :
  Option.map f (x >>= g) = x >>= fun a => Option.map f (g a) :=
  by 
    simpRw [←map_eq_map, ←bind_pure_comp_eq_map, IsLawfulMonad.bind_assoc]
...
theorem map_pbind (f : β → γ) (x : Option α) (g : ∀ a, a ∈ x → Option β) :
  Option.map f (x.pbind g) = x.pbind fun a H => Option.map f (g a H) :=
  by 
...
theorem pmap_map (g : γ → α) (x : Option γ) H :
  pmap f (x.map g) H = pmap (fun a h => f (g a) h) x fun a h => H _ (mem_map_of_mem _ h) :=
  by 
...
@[simp]
theorem pmap_eq_map (p : α → Prop) (f : α → β) x H : @pmap _ _ p (fun a _ => f a) x H = Option.map f x :=
  by 
    cases x <;> simp only [map_none', map_some', pmap]
...
@[simp]
theorem pmap_eq_none_iff {h} : pmap f x h = none ↔ x = none :=
  by 
    cases x <;> simp 
...
@[simp]
theorem none_orelse' (x : Option α) : none.orelse x = x :=
  by 
    cases x <;> rfl
...
@[simp]
theorem orelse_none' (x : Option α) : x.orelse none = x :=
  by 
    cases x <;> rfl
...
@[simp]
theorem not_is_some {a : Option α} : is_some a = ff ↔ a.is_none = tt :=
  by 
    cases a <;> simp 

theorem eq_some_iff_get_eq {o : Option α} {a : α} : o = some a ↔ ∃ h : o.is_some, Option.get h = a :=
  by 
    cases o <;> simp 

theorem not_is_some_iff_eq_none {o : Option α} : ¬o.is_some ↔ o = none :=
  by 
    cases o <;> simp 

theorem ne_none_iff_is_some {o : Option α} : o ≠ none ↔ o.is_some :=
  by 
    cases o <;> simp 

theorem ne_none_iff_exists {o : Option α} : o ≠ none ↔ ∃ x : α, some x = o :=
  by 
    cases o <;> simp 
...
@[simp]
theorem guard_eq_some {p : α → Prop} [DecidablePred p] {a b : α} : guardₓ p a = some b ↔ a = b ∧ p a :=
  by 
    byCases' p a <;> simp [Option.guard, h] <;> intro  <;> contradiction

@[simp]
theorem guard_eq_some' {p : Prop} [Decidable p] : ∀ u, _root_.guard p = some u ↔ p
| () =>
  by 
    byCases' p <;> simp [guardₓ, h, pure] <;> intro  <;> contradiction
...
@[simp]
theorem lift_or_get_none_left {f} {b : Option α} : lift_or_get f none b = b :=
  by 
    cases b <;> rfl

@[simp]
theorem lift_or_get_none_right {f} {a : Option α} : lift_or_get f a none = a :=
  by 
    cases a <;> rfl
...
@[simp]
theorem cases_on'_none_coe (f : Option α → β) (o : Option α) : cases_on' o (f none) (f ∘ coeₓ) = f o :=
  by 
    cases o <;> rfl

@[simp]
theorem get_or_else_map (f : α → β) (x : α) (o : Option α) : get_or_else (o.map f) (f x) = f (get_or_else o x) :=
  by 
    cases o <;> rfl
