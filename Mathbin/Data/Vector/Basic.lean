...
@[simp]
theorem to_list_map {β : Type _} (v : Vector α n) (f : α → β) : (v.map f).toList = v.to_list.map f :=
  by 
    cases v <;> rfl
...
@[simp]
theorem nth_repeat (a : α) (i : Finₓ n) : (Vector.repeat a n).nth i = a :=
  by 
    apply List.nth_le_repeat

@[simp]
theorem nth_map {β : Type _} (v : Vector α n) (f : α → β) (i : Finₓ n) : (v.map f).nth i = f (v.nth i) :=
  by 
    simp [nth_eq_nth_le]

@[simp]
theorem nth_of_fn {n} (f : Finₓ n → α) i : nth (of_fn f) i = f i :=
  by 
    rw [nth_eq_nth_le, ←List.nth_le_of_fn f] <;> congr <;> apply to_list_of_fn

@[simp]
theorem of_fn_nth (v : Vector α n) : of_fn (nth v) = v :=
  by 
    rcases v with ⟨l, rfl⟩
...
theorem mem_iff_nth {a : α} {v : Vector α n} : a ∈ v.to_list ↔ ∃ i, v.nth i = a :=
  by 
...
@[simp]
theorem head_of_fn {n : ℕ} (f : Finₓ n.succ → α) : head (of_fn f) = f 0 :=
  by 
    rw [←nth_zero, nth_of_fn]

@[simp]
theorem nth_cons_zero (a : α) (v : Vector α n) : nth (a::ᵥv) 0 = a :=
  by 
    simp [nth_zero]
...
@[simp]
theorem nth_cons_nil {ix : Finₓ 1} (x : α) : nth (x::ᵥnil) ix = x :=
  by 
    convert nth_cons_zero x nil

@[simp]
theorem nth_cons_succ (a : α) (v : Vector α n) (i : Finₓ n) : nth (a::ᵥv) i.succ = nth v i :=
  by 
...
-/
@[simp]
theorem scanl_cons (x : α) : scanl f b (x::ᵥv) = b::ᵥscanl f (f b x) v :=
  by 
...
-/
@[simp]
theorem scanl_singleton (v : Vector α 1) : scanl f b v = b::ᵥf b v.head::ᵥnil :=
  by 
    rw [←cons_head_tail v]
...
@[simp]
theorem mmap_nil {m} [Monadₓ m] {α β} (f : α → m β) : mmap f nil = pure nil :=
  rfl
...
def induction_on {C : ∀ {n : ℕ}, Vector α n → Sort _} (v : Vector α n) (h_nil : C nil)
  (h_cons : ∀ {n : ℕ} {x : α} {w : Vector α n}, C w → C (x::ᵥw)) : C v :=
  by 
...
def induction_on₂ {C : ∀ {n}, Vector α n → Vector β n → Sort _} (v : Vector α n) (w : Vector β n) (h_nil : C nil nil)
  (h_cons : ∀ {n a b} {x : Vector α n} {y}, C x y → C (a::ᵥx) (b::ᵥy)) : C v w :=
  by 
...
def induction_on₃ {C : ∀ {n}, Vector α n → Vector β n → Vector γ n → Sort _} (u : Vector α n) (v : Vector β n)
  ...
  (h_cons : ∀ {n a b c} {x : Vector α n} {y z}, C x y z → C (a::ᵥx) (b::ᵥy) (c::ᵥz)) : C u v w :=
  by 
...
@[simp]
theorem to_list_update_nth (v : Vector α n) (i : Finₓ n) (a : α) :
  (v.update_nth i a).toList = v.to_list.update_nth i a :=
  rfl

@[simp]
theorem nth_update_nth_same (v : Vector α n) (i : Finₓ n) (a : α) : (v.update_nth i a).nth i = a :=
  by 
    cases v <;> cases i <;> simp [Vector.updateNth, Vector.nth_eq_nth_le]

theorem nth_update_nth_of_ne {v : Vector α n} {i j : Finₓ n} (h : i ≠ j) (a : α) : (v.update_nth i a).nth j = v.nth j :=
  by 
    cases v <;>
      cases i <;>
...
theorem prod_update_nth [Monoidₓ α] (v : Vector α n) (i : Finₓ n) (a : α) :
  ...
  by 
    ...
    have  : «expr↑ » i < v.to_list.length := lt_of_lt_of_leₓ i.2 (le_of_eqₓ v.2.symm)
    simp [this]
