...
theorem antidiagonal_filter_fst_eq (f g : α →₀ ℕ) [D : ∀ p : (α →₀ ℕ) × (α →₀ ℕ), Decidable (p.1 = g)] :
  ...
  suffices a = g → (a + b = f ↔ g ≤ f ∧ b = f - g) by
    simpa [apply_iteₓ ((· ∈ ·) (a, b)), ← And.assoc, @And.right_comm _ (a = _), And.congr_left_iffₓ]
  rintro rfl
  constructor
  · rintro rfl
    exact ⟨le_add_right le_rflₓ, (add_tsub_cancel_left _ _).symm⟩
...
theorem antidiagonal_filter_snd_eq (f g : α →₀ ℕ) [D : ∀ p : (α →₀ ℕ) × (α →₀ ℕ), Decidable (p.2 = g)] :
  ...
  suffices b = g → (a + b = f ↔ g ≤ f ∧ a = f - g) by
    simpa [apply_iteₓ ((· ∈ ·) (a, b)), ← And.assoc, And.congr_left_iffₓ]
  rintro rfl
  constructor
  · rintro rfl
    exact ⟨le_add_left le_rflₓ, (add_tsub_cancel_right _ _).symm⟩
