...
@[simp]
theorem to_dfinsupp_support (f : ι →₀ M) : f.to_dfinsupp.support = f.support := by
  ext
  simp
...
@[simp]
theorem Dfinsupp.to_finsupp_support (f : Π₀ i : ι, M) : f.to_finsupp.support = f.support := by
  ext
  simp
