...
theorem mem_of_mem_of_eqv [IsStrictWeakOrder α lt] {m : Rbmap α β lt} {k₁ k₂ : α} : (k₁∈m) → k₁ ≈[lt]k₂ → (k₂∈m) :=
  by 
    intro h₁ h₂ 
...
theorem not_mem_of_find_none [IsStrictWeakOrder α lt] {k : α} {m : Rbmap α β lt} : m.find k = none → k ∉ m :=
  by 
    simp [find]
    intro h 
...
theorem mem_of_find_some [IsStrictWeakOrder α lt] {k : α} {v : β} {m : Rbmap α β lt} : m.find k = some v → (k∈m) :=
  by 
    simp [find]
    intro h 
...
theorem find_correct [IsStrictWeakOrder α lt] (k : α) (m : Rbmap α β lt) : (k∈m) ↔ ∃ v, m.find k = some v :=
  by 
    apply Iff.intro
    ·
      intro h 
...
theorem constains_correct [IsStrictWeakOrder α lt] (k : α) (m : Rbmap α β lt) : (k∈m) ↔ m.contains k = tt :=
  by 
    apply Iff.intro
    ·
      intro h 
...
theorem
  ...
  [ IsStrictWeakOrder α lt ] { k₁ k₂ : α } ( m : Rbmap α β lt ) ( v : β )
    : lt k₁ k₂ ∨ lt k₂ k₁ → ( m.insert k₁ v ) . findEntry k₂ = m.find_entry k₂
  :=
    by
      intro h
        have
          h'
            : ∀ { v₁ v₂ : β } , ( RbmapLt lt ) ( k₁ , v₁ ) ( k₂ , v₂ ) ∨ ( RbmapLt lt ) ( k₂ , v₂ ) ( k₁ , v₁ )
            :=
            fun _ _ => h
        generalize h₁ : m = m₁
        ...
        iterate
          2
          rw [ h₂ ] conv => lhs simp [ find_entry ] rw [ ← h₂ , insert , Rbtree.find_insert_of_disj _ h' , h₁ ] rfl
        any_goals simp [ insert ] at h₂ exact absurd h₂ ( Rbtree.insert_ne_mk_rbtree m ( k₁ , v ) )
        any_goals
          rw [ h₂ , h₁ ]
            simp [ find_entry ]
...
theorem find_entry_insert_of_not_eqv [IsStrictWeakOrder α lt] {k₁ k₂ : α} (m : Rbmap α β lt) (v : β) :
  ¬k₁ ≈[lt]k₂ → (m.insert k₁ v).findEntry k₂ = m.find_entry k₂ :=
  by 
    intro hn 
...
theorem find_entry_insert_of_ne [IsStrictTotalOrder α lt] {k₁ k₂ : α} (m : Rbmap α β lt) (v : β) :
  k₁ ≠ k₂ → (m.insert k₁ v).findEntry k₂ = m.find_entry k₂ :=
  by 
    intro h 
