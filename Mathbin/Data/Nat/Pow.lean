...
theorem one_lt_two_pow' (n : ℕ) : 1 < 2 ^ (n + 1) :=
  one_lt_pow (n + 1) 2 (succ_posₓ n) (by decide)

theorem pow_right_strict_mono {x : ℕ} (k : 2 ≤ x) : StrictMonoₓ fun n : ℕ => x ^ n := fun _ _ =>
  pow_lt_pow_of_lt_right k

theorem pow_le_iff_le_right {x m n : ℕ} (k : 2 ≤ x) : x ^ m ≤ x ^ n ↔ m ≤ n :=
  StrictMonoₓ.le_iff_le (pow_right_strict_mono k)

theorem pow_lt_iff_lt_right {x m n : ℕ} (k : 2 ≤ x) : x ^ m < x ^ n ↔ m < n :=
  StrictMonoₓ.lt_iff_lt (pow_right_strict_mono k)

theorem pow_right_injective {x : ℕ} (k : 2 ≤ x) : Function.Injective fun n : ℕ => x ^ n :=
  StrictMonoₓ.injective (pow_right_strict_mono k)

theorem pow_left_strict_mono {m : ℕ} (k : 1 ≤ m) : StrictMonoₓ fun x : ℕ => x ^ m := fun _ _ h =>
...
theorem StrictMonoₓ.nat_pow {n : ℕ} (hn : 1 ≤ n) {f : ℕ → ℕ} (hf : StrictMonoₓ f) : StrictMonoₓ fun m => f m ^ n :=
  (Nat.pow_left_strict_mono hn).comp hf

namespace Nat

theorem pow_le_iff_le_left {m x y : ℕ} (k : 1 ≤ m) : x ^ m ≤ y ^ m ↔ x ≤ y :=
  StrictMonoₓ.le_iff_le (pow_left_strict_mono k)

theorem pow_lt_iff_lt_left {m x y : ℕ} (k : 1 ≤ m) : x ^ m < y ^ m ↔ x < y :=
  StrictMonoₓ.lt_iff_lt (pow_left_strict_mono k)

theorem pow_left_injective {m : ℕ} (k : 1 ≤ m) : Function.Injective fun x : ℕ => x ^ m :=
  StrictMonoₓ.injective (pow_left_strict_mono k)
...
theorem not_pos_pow_dvd : ∀ {p k : ℕ} (hp : 1 < p) (hk : 1 < k), ¬p ^ k ∣ p
  | succ p, succ k, hp, hk, h =>
    have : succ p * succ p ^ k ∣ succ p * 1 := by simpa [pow_succₓ] using h
...
theorem shiftl_eq_mul_pow (m) : ∀ n, shiftl m n = m * 2 ^ n
  ...
  | k + 1 => show bit0 (shiftl m k) = m * (2 * 2 ^ k) by rw [bit0_val, shiftl_eq_mul_pow, mul_left_commₓ, mul_comm 2]

theorem shiftl'_tt_eq_mul_pow (m) : ∀ n, shiftl' true m n + 1 = (m + 1) * 2 ^ n
  | 0 => by simp [shiftl, shiftl', pow_zeroₓ, Nat.one_mul]
  | k + 1 => by
...
theorem shiftr_eq_div_pow (m) : ∀ n, shiftr m n = m / 2 ^ n
  ...
  | k + 1 => (congr_arg div2 (shiftr_eq_div_pow k)).trans <| by rw [div2_val, Nat.div_div_eq_div_mulₓ, mul_comm] <;> rfl
...
theorem size_shiftl' {b m n} (h : shiftl' b m n ≠ 0) : size (shiftl' b m n) = size m + n := by
  ...
  rw [one_mulₓ] at this
  obtain rfl : n = 0 :=
    Nat.eq_zero_of_le_zeroₓ (le_of_not_gtₓ fun hn => ne_of_gtₓ (pow_lt_pow_of_lt_right (by decide) hn) this)
  rfl
...
theorem size_le {m n : ℕ} : size m ≤ n ↔ m < 2 ^ n :=
  ⟨fun h => lt_of_lt_of_leₓ (lt_size_self _) (pow_le_pow_of_le_rightₓ (by decide) h), by
    rw [← one_shiftl]
    revert n
...
theorem lt_size {m n : ℕ} : m < size n ↔ 2 ^ m ≤ n := by rw [← not_ltₓ, Decidable.iff_not_comm, not_ltₓ, size_le]
...
theorem size_pow {n : ℕ} : size (2 ^ n) = n + 1 :=
  le_antisymmₓ (size_le.2 <| pow_lt_pow_of_lt_right (by decide) (lt_succ_selfₓ _)) (lt_size.2 <| le_rflₓ)
