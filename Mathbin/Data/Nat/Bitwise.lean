...
@[simp]
theorem zero_testBit (i : ℕ) : testBit 0 i = false := by simp [testBit]
...
theorem testBit_two_pow (n m : ℕ) : testBit (2 ^ n) m = (bit0 = m) :=
  by
  by_cases bit1 = m
  · cases h
    simp
...
theorem bitwise'_comm {f : Bool → Bool → Bool} (hf : ∀ b b', f b b' = f b' b)
  ...
  suffices bitwise f = swap (bitwise f) by conv_lhs => rw [not_eq]
  calc
