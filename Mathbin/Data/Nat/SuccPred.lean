...
@[ reducible ]
  instance
    ...
    where
      pred := pred
        pred_le := pred_le
        minimal_of_le_pred a ha b h := by cases a · exact b.not_lt_zero h · exact Nat.lt_irreflₓ a ha
        le_pred_of_lt a b h := by cases b · exact ( a.not_lt_zero h ) . elim · exact le_of_succ_le_succ h
        le_of_pred_lt a b h := by cases a · exact b.zero_le · exact h
