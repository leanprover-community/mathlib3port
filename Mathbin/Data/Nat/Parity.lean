 ...
 (Command.theorem
  ...
  (Command.declSig
   ...
   (Term.typeSpec
    ...
    (Term.app
     `IsCompl
     [(Set.«term{_|_}» "{" (Mathlib.ExtendedBinder.extBinder `n [":" (termℕ "ℕ")]) "|" (Term.app `Even [`n]) "}")
...
instance : DecidablePred (Even : ℕ → Prop) := fun n =>
  decidableOfDecidableOfIff
    (by
      infer_instance)
    even_iff.symm

instance decidable_pred_odd : DecidablePred (Odd : ℕ → Prop) := fun n =>
  decidableOfDecidableOfIff
    (by
      infer_instance)
...
@[parity_simps]
theorem even_succ : Even (succ n) ↔ ¬Even n := by
...
theorem neg_one_pow_of_even : Even n → (-1 : R) ^ n = 1 := by
  rintro ⟨c, rfl⟩
  simp [pow_mulₓ]
