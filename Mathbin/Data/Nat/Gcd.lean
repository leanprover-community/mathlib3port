...
theorem gcd_dvd (m n : ℕ) : gcdₓ m n ∣ m ∧ gcdₓ m n ∣ n :=
  gcdₓ.induction m n
    (fun n => by
...
theorem gcd_dvd_left (m n : ℕ) : gcdₓ m n ∣ m :=
  (gcd_dvdₓ m n).left

theorem gcd_dvd_right (m n : ℕ) : gcdₓ m n ∣ n :=
  (gcd_dvdₓ m n).right

theorem gcd_le_left {m} n (h : 0 < m) : gcdₓ m n ≤ m :=
  le_of_dvdₓ h <| gcd_dvd_leftₓ m n

theorem gcd_le_right m {n} (h : 0 < n) : gcdₓ m n ≤ n :=
  le_of_dvdₓ h <| gcd_dvd_rightₓ m n

theorem dvd_gcd {m n k : ℕ} : k ∣ m → k ∣ n → k ∣ gcdₓ m n :=
  gcdₓ.induction m n
    (fun n _ kn => by
...
@[simp]
theorem gcd_one_right (n : ℕ) : gcdₓ n 1 = 1 :=
  Eq.trans (gcd_commₓ n 1) <| gcd_one_leftₓ n

theorem gcd_mul_left (m n k : ℕ) : gcdₓ (m * n) (m * k) = m * gcdₓ n k :=
  gcdₓ.induction n k
    (fun k => by
      repeat'
        first |
...
theorem gcd_mul_right (m n k : ℕ) : gcdₓ (m * n) (k * n) = gcdₓ m k * n := by
  rw [mul_comm m n, mul_comm k n, mul_comm (gcd m k) n, gcd_mul_left]

theorem gcd_pos_of_pos_left {m : ℕ} (n : ℕ) (mpos : 0 < m) : 0 < gcdₓ m n :=
  pos_of_dvd_of_posₓ (gcd_dvd_leftₓ m n) mpos

theorem gcd_pos_of_pos_right (m : ℕ) {n : ℕ} (npos : 0 < n) : 0 < gcdₓ m n :=
  pos_of_dvd_of_posₓ (gcd_dvd_rightₓ m n) npos

theorem eq_zero_of_gcd_eq_zero_left {m n : ℕ} (H : gcdₓ m n = 0) : m = 0 :=
  Or.elim (Nat.eq_zero_or_posₓ m) id fun H1 : 0 < m => absurd (Eq.symm H) (ne_of_ltₓ (gcd_pos_of_pos_leftₓ _ H1))

theorem eq_zero_of_gcd_eq_zero_right {m n : ℕ} (H : gcdₓ m n = 0) : n = 0 := by
  rw [gcd_comm] at H <;> exact eq_zero_of_gcd_eq_zero_left H

@[simp]
theorem gcd_eq_zero_iff {i j : ℕ} : gcdₓ i j = 0 ↔ i = 0 ∧ j = 0 := by
  constructor
  · intro h
...
theorem gcd_div {m n k : ℕ} (H1 : k ∣ m) (H2 : k ∣ n) : gcdₓ (m / k) (n / k) = gcdₓ m n / k :=
...
theorem gcd_dvd_gcd_of_dvd_left {m k : ℕ} (n : ℕ) (H : m ∣ k) : gcdₓ m n ∣ gcdₓ k n :=
  dvd_gcdₓ ((gcd_dvd_leftₓ m n).trans H) (gcd_dvd_rightₓ m n)

theorem gcd_dvd_gcd_of_dvd_right {m k : ℕ} (n : ℕ) (H : m ∣ k) : gcdₓ n m ∣ gcdₓ n k :=
  dvd_gcdₓ (gcd_dvd_leftₓ n m) ((gcd_dvd_rightₓ n m).trans H)

theorem gcd_dvd_gcd_mul_left (m n k : ℕ) : gcdₓ m n ∣ gcdₓ (k * m) n :=
  gcd_dvd_gcd_of_dvd_leftₓ _ (dvd_mul_left _ _)

theorem gcd_dvd_gcd_mul_right (m n k : ℕ) : gcdₓ m n ∣ gcdₓ (m * k) n :=
  gcd_dvd_gcd_of_dvd_leftₓ _ (dvd_mul_right _ _)

theorem gcd_dvd_gcd_mul_left_right (m n k : ℕ) : gcdₓ m n ∣ gcdₓ m (k * n) :=
  gcd_dvd_gcd_of_dvd_rightₓ _ (dvd_mul_left _ _)

theorem gcd_dvd_gcd_mul_right_right (m n k : ℕ) : gcdₓ m n ∣ gcdₓ m (n * k) :=
  gcd_dvd_gcd_of_dvd_rightₓ _ (dvd_mul_right _ _)

theorem gcd_eq_left {m n : ℕ} (H : m ∣ n) : gcdₓ m n = m :=
  dvd_antisymm (gcd_dvd_leftₓ _ _) (dvd_gcdₓ dvd_rfl H)

theorem gcd_eq_right {m n : ℕ} (H : n ∣ m) : gcdₓ m n = n := by
  rw [gcd_comm, gcd_eq_left H]

@[simp]
theorem gcd_mul_left_left (m n : ℕ) : gcdₓ (m * n) n = n :=
  dvd_antisymm (gcd_dvd_rightₓ _ _) (dvd_gcdₓ (dvd_mul_left _ _) dvd_rfl)

@[simp]
theorem gcd_mul_left_right (m n : ℕ) : gcdₓ n (m * n) = n := by
  rw [gcd_comm, gcd_mul_left_left]

@[simp]
theorem gcd_mul_right_left (m n : ℕ) : gcdₓ (n * m) n = n := by
  rw [mul_comm, gcd_mul_left_left]

@[simp]
theorem gcd_mul_right_right (m n : ℕ) : gcdₓ n (n * m) = n := by
  rw [gcd_comm, gcd_mul_right_left]

@[simp]
theorem gcd_gcd_self_right_left (m n : ℕ) : gcdₓ m (gcdₓ m n) = gcdₓ m n :=
  dvd_antisymm (gcd_dvd_rightₓ _ _) (dvd_gcdₓ (gcd_dvd_leftₓ _ _) dvd_rfl)

@[simp]
theorem gcd_gcd_self_right_right (m n : ℕ) : gcdₓ m (gcdₓ n m) = gcdₓ n m := by
  rw [gcd_comm n m, gcd_gcd_self_right_left]

@[simp]
theorem gcd_gcd_self_left_right (m n : ℕ) : gcdₓ (gcdₓ n m) m = gcdₓ n m := by
  rw [gcd_comm, gcd_gcd_self_right_right]

@[simp]
theorem gcd_gcd_self_left_left (m n : ℕ) : gcdₓ (gcdₓ m n) m = gcdₓ m n := by
...
theorem lcm_comm (m n : ℕ) : lcmₓ m n = lcmₓ n m := by
  delta' lcm <;> rw [mul_comm, gcd_comm]

@[simp]
theorem lcm_zero_left (m : ℕ) : lcmₓ 0 m = 0 := by
  delta' lcm <;> rw [zero_mul, Nat.zero_divₓ]

@[simp]
theorem lcm_zero_right (m : ℕ) : lcmₓ m 0 = 0 :=
  lcm_commₓ 0 m ▸ lcm_zero_leftₓ m

@[simp]
theorem lcm_one_left (m : ℕ) : lcmₓ 1 m = m := by
  delta' lcm <;> rw [one_mulₓ, gcd_one_left, Nat.div_oneₓ]

@[simp]
theorem lcm_one_right (m : ℕ) : lcmₓ m 1 = m :=
  lcm_commₓ 1 m ▸ lcm_one_leftₓ m

@[simp]
theorem lcm_self (m : ℕ) : lcmₓ m m = m :=
...
theorem dvd_lcm_left (m n : ℕ) : m ∣ lcmₓ m n :=
  Dvd.intro (n / gcdₓ m n) (Nat.mul_div_assocₓ _ <| gcd_dvd_rightₓ m n).symm

theorem dvd_lcm_right (m n : ℕ) : n ∣ lcmₓ m n :=
  lcm_commₓ n m ▸ dvd_lcm_leftₓ n m

theorem gcd_mul_lcm (m n : ℕ) : gcdₓ m n * lcmₓ m n = m * n := by
  delta' lcm <;> rw [Nat.mul_div_cancel'ₓ ((gcd_dvd_left m n).trans (dvd_mul_right m n))]

theorem lcm_dvd {m n k : ℕ} (H1 : m ∣ k) (H2 : n ∣ k) : lcmₓ m n ∣ k :=
  Or.elim (Nat.eq_zero_or_posₓ k)
    (fun h => by
...
theorem lcm_assoc (m n k : ℕ) : lcmₓ (lcmₓ m n) k = lcmₓ m (lcmₓ n k) :=
  dvd_antisymm
...
theorem lcm_ne_zero {m n : ℕ} (hm : m ≠ 0) (hn : n ≠ 0) : lcmₓ m n ≠ 0 := by
  intro h
...
theorem coprime_iff_gcd_eq_one {m n : ℕ} : Coprime m n ↔ gcdₓ m n = 1 :=
  Iff.rfl
...
theorem coprime_div_gcd_div_gcd {m n : ℕ} (H : 0 < gcdₓ m n) : Coprime (m / gcdₓ m n) (n / gcdₓ m n) := by
...
theorem exists_coprime {m n : ℕ} (H : 0 < gcdₓ m n) : ∃ m' n', Coprime m' n' ∧ m = m' * gcdₓ m n ∧ n = n' * gcdₓ m n :=
  ⟨_, _, coprime_div_gcd_div_gcdₓ H, (Nat.div_mul_cancelₓ (gcd_dvd_leftₓ m n)).symm,
    (Nat.div_mul_cancelₓ (gcd_dvd_rightₓ m n)).symm⟩

theorem exists_coprime' {m n : ℕ} (H : 0 < gcdₓ m n) : ∃ g m' n', 0 < g ∧ Coprime m' n' ∧ m = m' * g ∧ n = n' * g :=
...
theorem gcd_mul_dvd_mul_gcd (k m n : ℕ) : gcdₓ k (m * n) ∣ gcdₓ k m * gcdₓ k n := by
