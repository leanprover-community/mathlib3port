...
| m, n+1 =>
  by 
    rw [←add_assocₓ, Nat.factorial_succ, mul_commₓ (Nat.succ _), pow_succ'ₓ, ←mul_assocₓ] <;>
      exact
...
theorem factorial_inj (hn : 1 < n !) : n ! = m ! ↔ n = m :=
  by 
    ...
    ·
      ...
      ·
        exfalso 
        rw [←factorial_lt, h] at hnm 
        exact lt_irreflₓ _ hnm 
        rw [one_lt_factorial] at hn 
        exact lt_transₓ one_pos hn
      ·
        exact hnm 
      exfalso 
...
theorem add_factorial_succ_le_factorial_add_succ (i : ℕ) (n : ℕ) : (i+(n+1)!) ≤ (i+n+1)! :=
  by 
    ...
    ·
      ...
      rw [add_mulₓ, one_mulₓ, add_commₓ 1 n]
...
theorem zero_asc_factorial (k : ℕ) : (0 : ℕ).ascFactorial k = k ! :=
  by 
    ...
    rw [ht, zero_addₓ, Nat.factorial_succ]
...
theorem asc_factorial_eq_div (n k : ℕ) : n.asc_factorial k = (n+k)! / n ! :=
  by 
...
theorem asc_factorial_of_sub {n k : ℕ} (h : k < n) : ((n - k)*(n - k).ascFactorial k) = (n - k+1).ascFactorial (k+1) :=
  by 
...
theorem zero_desc_factorial_succ (k : ℕ) : (0 : ℕ).descFactorial k.succ = 0 :=
  by 
    rw [desc_factorial_succ, zero_tsub, zero_mul]

@[simp]
theorem desc_factorial_one (n : ℕ) : n.desc_factorial 1 = n :=
  by 
...
theorem succ_desc_factorial (n : ℕ) : ∀ k, (((n+1) - k)*(n+1).descFactorial k) = (n+1)*n.desc_factorial k
| 0 =>
  by 
...
| succ n, succ k =>
  fun h =>
    by 
      rw [succ_desc_factorial_succ, succ_sub_succ, ←mul_assocₓ, mul_commₓ (n - k)!, mul_assocₓ,
...
theorem desc_factorial_eq_div {n k : ℕ} (h : k ≤ n) : n.desc_factorial k = n ! / (n - k)! :=
  by 
...
| k+2 =>
  fun _ =>
    by 
      rw [desc_factorial_succ, pow_succ'ₓ, mul_commₓ]
