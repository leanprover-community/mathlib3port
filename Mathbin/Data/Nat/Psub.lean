...
theorem pred_eq_ppred (n : ℕ) : pred n = (ppred n).getOrElse 0 :=
  by 
    cases n <;> rfl
...
theorem psub'_eq_psub m n : psub' m n = psub m n :=
  by 
    rw [psub'] <;> splitIfs <;> [exact (psub_eq_sub h).symm, exact (psub_eq_none.2 (not_leₓ.1 h)).symm]
