...
theorem succ_mul_central_binom_succ (n : ℕ) : ((n+1)*central_binom (n+1)) = (2*(2*n)+1)*central_binom n :=
  calc ((n+1)*(2*n+1).choose (n+1)) = ((2*n)+2).choose (n+1)*n+1 := mul_commₓ _ _ 
    _ = ((2*n)+1).choose n*(2*n)+2 :=
    by 
      rw [choose_succ_right_eq, choose_mul_succ_eq]
    _ = 2*((2*n)+1).choose n*n+1 :=
    by 
      ring 
    _ = 2*((2*n)+1).choose n*((2*n)+1) - n :=
    by 
      rw [two_mul n, add_assocₓ, Nat.add_sub_cancel_left]
    _ = 2*(2*n).choose n*(2*n)+1 :=
    by 
      rw [choose_mul_succ_eq]
    _ = (2*(2*n)+1)*(2*n).choose n :=
    by 
...
-/
theorem four_pow_lt_mul_central_binom (n : ℕ) (n_big : 4 ≤ n) : 4 ^ n < n*central_binom n :=
  by 
    ...
    ·
      ...
      decide!
    ·
      normNum [central_binom, choose]
    obtain ⟨n, rfl⟩ : ∃ m, n = m+1 := Nat.exists_eq_succ_of_ne_zero (zero_lt_four.trans hn).ne' 
    calc (4 ^ n+1) < 4*n*central_binom n :=
      (mul_lt_mul_left zero_lt_four).mpr (IH n n.lt_succ_self (Nat.le_of_lt_succₓ hn))_ ≤ (2*(2*n)+1)*central_binom n :=
      by 
        rw [←mul_assocₓ]
        linarith _ = (n+1)*central_binom (n+1) :=
      (succ_mul_central_binom_succ n).symm
