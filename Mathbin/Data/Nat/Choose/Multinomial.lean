...
theorem multinomial_singleton : multinomial {a} f = 1 := by
  simp [multinomial, Nat.div_selfₓ (factorial_pos (f a))]
...
theorem multinomial_insert_one [DecidableEq α] (h : a ∉ s) (h₁ : f a = 1) :
  ...
  rw [Finset.sum_insert h, Finset.prod_insert h, h₁, add_commₓ, ← succ_eq_add_one, factorial_succ]
...
theorem multinomial_insert [DecidableEq α] (h : a ∉ s) :
  ...
  rw
    [div_mul_div_comm ((f a).factorial_mul_factorial_dvd_factorial_add (s.sum f))
...
theorem multinomial_univ_three (a b c : ℕ) : multinomial Finset.univ ![a, b, c] = (a + b + c)! / (a ! * b ! * c !) := by
  simp [multinomial, Finₓ.sum_univ_three, Finₓ.prod_univ_three]
