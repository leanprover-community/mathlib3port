...
theorem choose_mul {n k s : ℕ} (hkn : k ≤ n) (hsk : s ≤ k) :
  ...
  calc ((n.choose k*k.choose s)*((n - k)!*(k - s)!)*s !) = (n.choose k*(k.choose s*s !)*(k - s)!)*(n - k)! := by
    rw [mul_assocₓ, mul_assocₓ, mul_assocₓ, mul_assocₓ _ s !, mul_assocₓ, mul_commₓ (n - k)!, mul_commₓ s !]_ = n ! :=
    by
    ...
    by
    rw [tsub_tsub_tsub_cancel_right hsk, mul_assocₓ, mul_left_commₓ s !, mul_assocₓ, mul_commₓ (k - s)!, mul_commₓ s !,
      mul_right_commₓ, ← mul_assocₓ]
