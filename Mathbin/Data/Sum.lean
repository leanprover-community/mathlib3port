...
theorem inl_injective : Function.Injective (Sum.inl : α → Sum α β) := fun x y => Sum.inl.injₓ

theorem inr_injective : Function.Injective (Sum.inr : β → Sum α β) := fun x y => Sum.inr.injₓ
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `update_elim_inl [])
  (Command.declSig
   [(Term.implicitBinder "{" [`α `β `γ] [] "}")
    (Term.instBinder "[" [] (Term.app `DecidableEq [`α]) "]")
    (Term.instBinder "[" [] (Term.app `DecidableEq [(Term.app `Sum [`α `β])]) "]")
    (Term.implicitBinder "{" [`f] [":" (Term.arrow `α "→" `γ)] "}")
    (Term.implicitBinder "{" [`g] [":" (Term.arrow `β "→" `γ)] "}")
    (Term.implicitBinder "{" [`i] [":" `α] "}")
    (Term.implicitBinder "{" [`x] [":" `γ] "}")]
   (Term.typeSpec
    ":"
    («term_=_»
     (Term.app `update [(Term.app `Sum.elim [`f `g]) (Term.app `inl [`i]) `x])
     "="
     (Term.app `Sum.elim [(Term.app `update [`f `i `x]) `g]))))
  (Command.declValSimple
   ":="
   (Term.app
    (Term.proj `update_eq_iff "." (fieldIdx "2"))
    [(Term.anonymousCtor
      "⟨"
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `update_elim_inr [])
  (Command.declSig
   [(Term.implicitBinder "{" [`α `β `γ] [] "}")
    (Term.instBinder "[" [] (Term.app `DecidableEq [`β]) "]")
    (Term.instBinder "[" [] (Term.app `DecidableEq [(Term.app `Sum [`α `β])]) "]")
    (Term.implicitBinder "{" [`f] [":" (Term.arrow `α "→" `γ)] "}")
    (Term.implicitBinder "{" [`g] [":" (Term.arrow `β "→" `γ)] "}")
    (Term.implicitBinder "{" [`i] [":" `β] "}")
    (Term.implicitBinder "{" [`x] [":" `γ] "}")]
   (Term.typeSpec
    ":"
    («term_=_»
     (Term.app `update [(Term.app `Sum.elim [`f `g]) (Term.app `inr [`i]) `x])
     "="
     (Term.app `Sum.elim [`f (Term.app `update [`g `i `x])]))))
  (Command.declValSimple
   ":="
   (Term.app
    (Term.proj `update_eq_iff "." (fieldIdx "2"))
...
theorem lex_inl_inl {a₁ a₂} : lex ra rb (inl a₁) (inl a₂) ↔ ra a₁ a₂ :=
  ⟨fun h => by
    cases h <;> assumption, lex.inl⟩

@[simp]
theorem lex_inr_inr {b₁ b₂} : lex ra rb (inr b₁) (inr b₂) ↔ rb b₁ b₂ :=
  ⟨fun h => by
    cases h <;> assumption, lex.inr⟩

@[simp]
theorem lex_inr_inl {b a} : ¬lex ra rb (inr b) (inl a) := fun h => by
  cases h

attribute [simp] lex.sep

theorem lex_acc_inl {a} (aca : Acc ra a) : Acc (lex ra rb) (inl a) := by
...
theorem lex_acc_inr (aca : ∀ a, Acc (lex ra rb) (inl a)) {b} (acb : Acc rb b) : Acc (lex ra rb) (inr b) := by
