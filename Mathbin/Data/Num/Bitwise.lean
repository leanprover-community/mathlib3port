...
theorem bit_zero b : b :: zero b = zero b :=
  by 
    cases b <;> rfl

theorem bit_one b : b :: zero (bnot b) = msb b :=
  by 
    cases b <;> rfl
...
def cadd : Snum → Snum → Bool → Snum :=
  (rec' fun a p c => czadd c a p)$
    fun a p IH => (rec' fun b c => czadd c b (a :: p))$ fun b q _ c => Bitvec.xor3 a b c :: IH q (Bitvec.carry a b c)
