...
@[simp]
theorem sum_add_hom_zero [∀ i, AddZeroClass (β i)] [AddCommMonoidₓ γ] : (sumAddHom fun i => (0 : β i →+ γ)) = 0 :=
...
@[simp]
theorem map_dfinsupp_sum_add_hom [NonAssocSemiringₓ R] [NonAssocSemiringₓ S] [∀ i, AddZeroClass (β i)] (h : R →+* S)
