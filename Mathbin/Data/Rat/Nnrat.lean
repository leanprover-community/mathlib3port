...
@[simp]
theorem toNnrat_zero : toNnrat 0 = 0 := by simp [toNnrat] <;> rfl
#align rat.to_nnrat_zero Rat.toNnrat_zero

@[simp]
theorem toNnrat_one : toNnrat 1 = 1 := by simp [toNnrat, max_eq_left zero_le_one]
...
@[simp]
theorem den_coe : (q : ℚ).den = q.denom :=
  rfl
