...
protected theorem nonneg_add {a b} : Ratₓ.Nonneg a → Ratₓ.Nonneg b → Ratₓ.Nonneg (a + b) :=
...
protected theorem add_le_add_left {a b c : ℚ} : c + a ≤ c + b ↔ a ≤ b := by
  unfold LE.le Ratₓ.Le <;> rw [add_sub_add_left_eq_sub]
...
theorem div_lt_div_iff_mul_lt_mul {a b c d : ℤ} (b_pos : 0 < b) (d_pos : 0 < d) : (a : ℚ) / b < c / d ↔ a * d < c * b :=
  ...
  apply and_congrₓ
  · simp [div_num_denom, Ratₓ.le_def b_pos d_pos]
...
theorem abs_def (q : ℚ) : abs q = q.num.natAbs /. q.denom := by
  ...
  · rw [abs_of_nonpos hq]
    rw [← @num_denom q, ← mk_zero_one, Ratₓ.le_def (Int.coe_nat_pos.2 q.pos) zero_lt_one, mul_oneₓ, zero_mul] at hq
    rw [Int.of_nat_nat_abs_of_nonpos hq, ← neg_def, num_denom]
    
  · rw [abs_of_nonneg hq]
    rw [← @num_denom q, ← mk_zero_one, Ratₓ.le_def zero_lt_one (Int.coe_nat_pos.2 q.pos), mul_oneₓ, zero_mul] at hq
