...
-- error in Data.Rat.Order: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
...
theorem lt_one_iff_num_lt_denom {q : ℚ} : q < 1 ↔ q.num < q.denom :=
  by 
    ...
    ·
      ...
      exact not_lt.elim_left q_nonpos 
      have  : q.num < q.denom
      ·
        ·
          have  : ¬0 < q.num ↔ ¬0 < q 
          exact not_iff_not.elim_right num_pos_iff_pos 
          simp only [not_ltₓ] at this 
          exact
            lt_of_le_of_ltₓ (this.elim_right q_nonpos)
              (by 
                exactModCast q.pos)
      simp only [this, lt_of_le_of_ltₓ q_nonpos zero_lt_one]
