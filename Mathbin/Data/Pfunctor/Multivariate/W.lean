...
theorem W_path_cases_on_eta {α : Typevec n} {a : P.A} {f : P.last.B a → P.last.W} (h : P.W_path ⟨a, f⟩ ⟹ α) :
    P.W_path_cases_on (P.W_path_dest_left h) (P.W_path_dest_right h) = h := by
  ext i x <;> cases x <;> rfl
...
theorem W_map_W_mk {α β : Typevec n} (g : α ⟹ β) (a : P.A) (f' : P.drop.B a ⟹ α) (f : P.last.B a → P.W α) :
    g <$$> P.W_mk a f' f = P.W_mk a (g ⊚ f') fun i => g <$$> f i := by
...
theorem map_obj_append1 {α γ : Typevec n} (g : α ⟹ γ) (a : P.A) (f' : P.drop.B a ⟹ α) (f : P.last.B a → P.W α) :
    append_fun g (P.W_map g) <$$> P.obj_append1 a f' f = P.obj_append1 a (g ⊚ f') fun x => P.W_map g (f x) := by
...
theorem W_dest'_W_mk {α : Typevec n} (a : P.A) (f' : P.drop.B a ⟹ α) (f : P.last.B a → P.W α) :
    P.W_dest' (P.W_mk a f' f) = ⟨a, split_fun f' f⟩ := by
  rw [W_dest', W_rec_eq]
