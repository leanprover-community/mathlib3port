...
theorem Finset.prod_attach_univ [Fintype α] [CommMonoidₓ β] (f : { a : α // a ∈ @univ α _ } → β) :
  ...
  Fintype.prod_equiv (Equivₓ.subtypeUnivEquiv fun x => mem_univ _) _ _
    fun x =>
      by 
        simp 
...
/--
  ...
  theorem
    ...
    :=
      prod_bij
        fun x _ a => x a mem_univ _
          by simp
          by simp
          by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) [ Function.funext_iffₓ ]
          fun x hx => ⟨ fun a _ => x a , by simp_all ⟩
...
@[ toAdditive ]
  theorem
    ...
    { M : Type _ } [ CommMonoidₓ M ] [ Fintype α ] ( p : α → Prop ) [ DecidablePred p ] ( f : α → M )
      : ∏ a in { x | p x } . toFinset , f a = ∏ a : Subtype p , f a
