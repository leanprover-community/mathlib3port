...
-- ./././Mathport/Syntax/Translate/Command.lean:324:30: infer kinds are unsupported in Lean 4: #[`elems] []
...
theorem fold_inf_univ [SemilatticeInf α] [OrderBot α] (a : α) : (Finset.univ.fold (·⊓·) a fun x => x) = ⊥ :=
  eq_bot_iff.2 <| ((Finset.fold_op_rel_iff_and <| @le_inf_iff α _).1 le_rflₓ).2 ⊥ <| Finset.mem_univ _
...
theorem Finset.exists_minimal {α : Type _} [Preorderₓ α] (s : Finset α) (h : s.Nonempty) : ∃ m ∈ s, ∀ x ∈ s, ¬x < m :=
  by
  obtain ⟨c, hcs : c ∈ s⟩ := h
...
theorem Finset.exists_maximal {α : Type _} [Preorderₓ α] (s : Finset α) (h : s.Nonempty) : ∃ m ∈ s, ∀ x ∈ s, ¬m < x :=
