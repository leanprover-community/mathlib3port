...
@[simp]
theorem cantor_function_aux_ff (h : f n = ff) : cantor_function_aux c f n = 0 := by
  simp [cantor_function_aux, h]

theorem cantor_function_aux_nonneg (h : 0 ≤ c) : 0 ≤ cantor_function_aux c f n := by
  cases h' : f n <;> simp [h']
  apply pow_nonneg h
...
theorem increasing_cantor_function (h1 : 0 < c) (h2 : c < 1 / 2) {n : ℕ} {f g : ℕ → Bool}
    (hn : ∀, ∀ k < n, ∀, f k = g k) (fn : f n = ff) (gn : g n = tt) : cantor_function c f < cantor_function c g := by
  have h3 : c < 1 := by
    apply h2.trans
    norm_num
