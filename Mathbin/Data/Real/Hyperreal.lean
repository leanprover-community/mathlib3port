...
theorem is_st_refl_real (r : ℝ) : is_st r r := fun δ hδ =>
...
theorem is_st_symm_real {r s : ℝ} : is_st r s ↔ is_st s r := by
  rw [is_st_real_iff_eq, is_st_real_iff_eq, eq_comm]

theorem is_st_trans_real {r s t : ℝ} : is_st r s → is_st s t → is_st r t := by
...
theorem is_st_iff_abs_sub_lt_delta {x : ℝ*} {r : ℝ} : is_st x r ↔ ∀ δ : ℝ, 0 < δ → abs (x - r) < δ := by
...
theorem is_st_le_of_le {x y : ℝ*} {r s : ℝ} (hrx : is_st x r) (hsy : is_st y s) : x ≤ y → r ≤ s := by
...
theorem infinite_pos_def {x : ℝ*} : infinite_pos x ↔ ∀ r : ℝ, ↑r < x := by
  rw [iff_eq_eq] <;> rfl

theorem infinite_neg_def {x : ℝ*} : infinite_neg x ↔ ∀ r : ℝ, x < r := by
  rw [iff_eq_eq] <;> rfl
...
theorem ne_zero_of_infinite {x : ℝ*} : Infinite x → x ≠ 0 := fun hI h0 =>
  Or.cases_on hI
    (fun hip =>
      lt_irreflₓ (0 : ℝ*)
        ((by
            rwa [← h0] : infinite_pos 0)
          0))
    fun hin =>
    lt_irreflₓ (0 : ℝ*)
      ((by
          rwa [← h0] : infinite_neg 0)
        0)

theorem not_infinite_zero : ¬Infinite 0 := fun hI => ne_zero_of_infinite hI rfl

theorem pos_of_infinite_pos {x : ℝ*} : infinite_pos x → 0 < x := fun hip => hip 0

theorem neg_of_infinite_neg {x : ℝ*} : infinite_neg x → x < 0 := fun hin => hin 0

theorem not_infinite_pos_of_infinite_neg {x : ℝ*} : infinite_neg x → ¬infinite_pos x := fun hn hp =>
...
theorem infinite_neg_neg_of_infinite_pos {x : ℝ*} : infinite_pos x → infinite_neg (-x) := fun hp r =>
  neg_lt.mp (hp (-r))

theorem infinite_pos_neg_of_infinite_neg {x : ℝ*} : infinite_neg x → infinite_pos (-x) := fun hp r =>
  lt_neg.mp (hp (-r))
...
theorem not_infinitesimal_of_infinite_pos {x : ℝ*} : infinite_pos x → ¬infinitesimal x := fun hp =>
  not_infinitesimal_of_infinite (Or.inl hp)

theorem not_infinitesimal_of_infinite_neg {x : ℝ*} : infinite_neg x → ¬infinitesimal x := fun hn =>
...
theorem infinite_pos_iff_infinite_of_pos {x : ℝ*} (hp : 0 < x) : infinite_pos x ↔ Infinite x := by
...
theorem infinite_neg_iff_infinite_of_neg {x : ℝ*} (hn : x < 0) : infinite_neg x ↔ Infinite x := by
...
theorem infinite_iff_infinite_pos_abs {x : ℝ*} : Infinite x ↔ infinite_pos (abs x) :=
  ⟨fun hi d =>
    Or.cases_on hi
      (fun hip => by
...
theorem infinite_neg_of_tendsto_bot {f : ℕ → ℝ} (hf : tendsto f at_top at_bot) : infinite_neg (of_seq f) := fun r =>
...
theorem not_real_of_infinitesimal_ne_zero (x : ℝ*) : infinitesimal x → x ≠ 0 → ∀ r : ℝ, x ≠ r := fun hi hx r hr =>
  hx <| hr.trans <| coe_eq_zero.2 <| is_st_unique (hr.symm ▸ is_st_refl_real r : is_st x r) hi
...
theorem infinitesimal_pos_iff_infinite_pos_inv {x : ℝ*} : infinite_pos x⁻¹ ↔ infinitesimal x ∧ 0 < x := by
  convert infinite_pos_iff_infinitesimal_inv_pos <;> simp only [inv_inv₀]

theorem infinitesimal_neg_iff_infinite_neg_inv {x : ℝ*} : infinite_neg x⁻¹ ↔ infinitesimal x ∧ x < 0 := by
...
theorem is_st_inv {x : ℝ*} {r : ℝ} (hi : ¬infinitesimal x) : is_st x r → is_st x⁻¹ r⁻¹ := fun hxr =>
