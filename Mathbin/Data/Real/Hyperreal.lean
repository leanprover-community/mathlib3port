...
theorem is_st_st_of_is_st {x : ℝ*} {r : ℝ} (hxr : IsSt x r) : IsSt x (st x) := by rwa [st_of_is_st hxr]
...
theorem is_st_symm_real {r s : ℝ} : IsSt r s ↔ IsSt s r := by rw [is_st_real_iff_eq, is_st_real_iff_eq, eq_comm]
...
theorem not_infinite_iff_exist_lt_gt {x : ℝ*} : ¬Infinite x ↔ ∃ (r : ℝ) (s : ℝ), (r : ℝ*) < x ∧ x < s :=
  ⟨fun hni =>
          ...
          _ ≤ |x| * (d / t / 2 : ℝ) + (d / |s| / 2 : ℝ) * |s| :=
            add_le_add
              (mul_le_mul_of_nonneg_left
                ...
                abs_nonneg _)
              (mul_le_mul_of_nonneg_right (le_of_lt $ hxr' _ $ half_pos $ div_pos hd $ abs_pos.2 hs) $ abs_nonneg _)
          _ = (d / 2 * (|x| / t) + d / 2 : ℝ*) := by
