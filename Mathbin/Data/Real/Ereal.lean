...
theorem to_real_le_to_real {x y : Ereal} (h : x ≤ y) (hx : x ≠ ⊥) (hy : y ≠ ⊤) : x.to_real ≤ y.to_real :=
  by 
    lift x to ℝ 
    lift y to ℝ
    ·
...
theorem coe_to_real {x : Ereal} (hx : x ≠ ⊤) (h'x : x ≠ ⊥) : (x.to_real : Ereal) = x :=
  by 
    induction x using Ereal.rec
    ·
      simpa using h'x
...
theorem le_coe_to_real {x : Ereal} (h : x ≠ ⊤) : x ≤ x.to_real :=
  by 
    byCases' h' : x = ⊥
    ·
      simp only [h', bot_le]
...
theorem add_lt_add_left_coe {x y : Ereal} (h : x < y) (z : ℝ) : ((z : Ereal)+x) < z+y :=
  by 
...
@[simp]
theorem add_eq_top_iff {x y : Ereal} : (x+y) = ⊤ ↔ x = ⊤ ∨ y = ⊤ :=
  by 
    induction x using Ereal.rec <;> induction y using Ereal.rec <;> simp [←Ereal.coe_add]

@[simp]
theorem add_lt_top_iff {x y : Ereal} : (x+y) < ⊤ ↔ x < ⊤ ∧ y < ⊤ :=
  by 
...
theorem neg_inj {a b : Ereal} (h : -a = -b) : a = b :=
  by 
...
@[simp]
theorem neg_eg_top_iff {x : Ereal} : -x = ⊤ ↔ x = ⊥ :=
  by 
    rw [neg_eq_iff_neg_eq]
    simp [eq_comm]

@[simp]
theorem neg_eg_bot_iff {x : Ereal} : -x = ⊥ ↔ x = ⊤ :=
  by 
    rw [neg_eq_iff_neg_eq]
    simp [eq_comm]

@[simp]
theorem neg_eg_zero_iff {x : Ereal} : -x = 0 ↔ x = 0 :=
  by 
    rw [neg_eq_iff_neg_eq]
    simp [eq_comm]
...
@[simp]
theorem neg_le_neg_iff {a b : Ereal} : -a ≤ -b ↔ b ≤ a :=
  by 
    convLHS => rw [Ereal.neg_le, Ereal.neg_neg]
...
@[simp]
theorem sub_zero (x : Ereal) : x - 0 = x :=
  by 
    change (x+-0) = x 
    simp 

@[simp]
theorem zero_sub (x : Ereal) : 0 - x = -x :=
  by 
    change (0+-x) = -x 
    simp 
...
| ⊤, y =>
  by 
    byCases' hy : y = 0 <;> simp [hy]
| x, ⊤ =>
  by 
    byCases' hx : x = 0 <;> simp [hx]
| (x : ℝ), (y : ℝ) =>
  by 
    simp [←Ereal.coe_mul]
| ⊥, (y : ℝ) =>
