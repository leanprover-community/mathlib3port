...
def reprₓ {α} [HasRepr α] : Ordnode α → Stringₓ
| nil => "∅"
        ...
        ·
          ...
          ·
            ...
            ·
              exact nil 
            exact
              if lrs < ratio*lls then node ((ls+rs)+1) ll lx (node ((rs+lrs)+1) lr x r) else
        ...
        ·
          ...
          ·
            ...
            ·
              exact nil 
            exact
              if rls < ratio*rrs then node ((ls+rs)+1) (node ((ls+rls)+1) l x rl) rx rr else
          ...
          ·
            ...
            ·
              ...
              ·
                exact nil 
              exact
                if lrs < ratio*lls then node ((ls+rs)+1) ll lx (node ((lrs+rs)+1) lr x r) else
                  node ((ls+rs)+1) (node ((lls+size lrl)+1) ll lx lrl) lrx (node ((size lrr+rs)+1) lrr x r)
...
| l@(node sl ll lx lr), r@(node sr rl rx rr) =>
  if sl > sr then
    let (l', m) := split_max' ll lx lr 
    balance_r l' m r
  else
    let (m, r') := split_min' rl rx rr 
    balance_l l m r'
...
def merge (l : Ordnode α) : Ordnode α → Ordnode α :=
  (Ordnode.recOn l fun r => r)$
    fun ls ll lx lr IHll IHlr r =>
      Ordnode.recOn r (node ls ll lx lr)$
        fun rs rl rx rr IHrl IHrr =>
          if (delta*ls) < rs then balance_l IHrl rx rr else
            if (delta*rs) < ls then balance_r ll lx (IHlr$ node rs rl rx rr) else
...
def link (l : Ordnode α) (x : α) : Ordnode α → Ordnode α :=
  Ordnode.recOn l (insert_min x)$
    fun ls ll lx lr IHll IHlr r =>
      Ordnode.recOn r (insert_max l x)$
        fun rs rl rx rr IHrl IHrr =>
          if (delta*ls) < rs then balance_l IHrl rx rr else
            if (delta*rs) < ls then balance_r ll lx (IHlr r) else node' l x r
...
def to_list (t : Ordnode α) : List α :=
  foldr List.cons t []
...
def take_while (p : α → Prop) [DecidablePred p] : Ordnode α → Ordnode α
| nil => nil
...
def drop_while (p : α → Prop) [DecidablePred p] : Ordnode α → Ordnode α
| nil => nil
...
| nil => none
| node _ l y r => if x ≤ y then find_lt l else some (find_lt_aux x r y)
...
| nil => none
| node _ l y r => if y ≤ x then find_gt r else some (find_gt_aux x l y)
...
def find_index (x : α) (t : Ordnode α) : Option ℕ :=
...
def of_list (l : List α) : Ordnode α :=
  l.foldr insert nil
...
| [] => nil
| x :: xs => if List.Chain (fun a b => ¬b ≤ a) x xs then of_asc_list (x :: xs) else of_list (x :: xs)
