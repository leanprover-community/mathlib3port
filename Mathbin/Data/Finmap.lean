...
@[simp]
theorem lookup_list_to_finmap (a : α) (s : List (Sigma β)) : lookup a s.to_finmap = s.lookup a :=
  by 
...
theorem disjoint_union_right (x y z : Finmap β) : Disjoint x (y ∪ z) ↔ Disjoint x y ∧ Disjoint x z :=
  by 
