...
theorem eq_of_drop_last_eq {α β : Typevec (n + 1)} {f g : α ⟹ β} (h₀ : drop_fun f = drop_fun g)
    (h₁ : last_fun f = last_fun g) : f = g := by
...
theorem split_fun_inj {α α' : Typevec (n + 1)} {f f' : drop α ⟹ drop α'} {g g' : last α → last α'}
    (H : split_fun f g = split_fun f' g') : f = f' ∧ g = g' := by
...
def typevec_cases_nil₂ {β : Fin2.elim0 ⟹ Fin2.elim0 → Sort _} (f : β nil_fun) : ∀ f, β f := by
  intro g
  have : g = nil_fun
  ext ⟨⟩
  rw [this]
...
theorem const_append1 {β γ} (x : γ) {n} (α : Typevec n) :
    Typevec.constₓ x (α ::: β) = append_fun (Typevec.constₓ x α) fun _ => x := by
  ext i : 1 <;> cases i <;> rfl
...
@[typevec]
theorem repeat_eq_append1 {β} {n} (α : Typevec n) : repeat_eq (α ::: β) = split_fun (repeat_eq α) (uncurry Eq) := by
  induction n <;> rfl

@[typevec]
theorem repeat_eq_nil (α : Typevec 0) : repeat_eq α = nil_fun := by
  ext i : 1 <;> cases i <;> rfl
...
theorem const_iff_true {α : Typevec n} {i x p} : of_repeat (Typevec.constₓ p α i x) ↔ p := by
...
theorem repeat_eq_iff_eq {α : Typevec n} {i x y} : of_repeat (repeat_eq α i (Prod.mk _ x y)) ↔ x = y := by
...
theorem append_prod_append_fun {n} {α α' β β' : Typevec.{u} n} {φ φ' ψ ψ' : Type u} {f₀ : α ⟹ α'} {g₀ : β ⟹ β'}
    {f₁ : φ → φ'} {g₁ : ψ → ψ'} : (f₀ ⊗' g₀ ::: _root_.prod.map f₁ g₁) = ((f₀ ::: f₁) ⊗' (g₀ ::: g₁)) := by
  ext i a <;> cases i <;> [cases a, skip] <;> rfl

end Liftp'

@[simp]
theorem drop_fun_diag {α} : drop_fun (@prod.diag (n + 1) α) = prod.diag := by
  ext i : 2
...
@[simp]
theorem drop_fun_to_subtype {α} (p : α ⟹ repeat (n + 1) Prop) : drop_fun (to_subtype p) = to_subtype _ := by
  ext i : 2
  induction i <;> simp [drop_fun, *] <;> rfl

@[simp]
theorem last_fun_to_subtype {α} (p : α ⟹ repeat (n + 1) Prop) : last_fun (to_subtype p) = _root_.id := by
  ext i : 2
  induction i <;> simp [drop_fun, *] <;> rfl

@[simp]
theorem drop_fun_of_subtype {α} (p : α ⟹ repeat (n + 1) Prop) : drop_fun (of_subtype p) = of_subtype _ := by
  ext i : 2
  induction i <;> simp [drop_fun, *] <;> rfl

@[simp]
theorem last_fun_of_subtype {α} (p : α ⟹ repeat (n + 1) Prop) : last_fun (of_subtype p) = _root_.id := by
  ext i : 2
  induction i <;> simp [drop_fun, *] <;> rfl
...
theorem drop_fun_prod {α α' β β' : Typevec (n + 1)} (f : α ⟹ β) (f' : α' ⟹ β') :
    drop_fun (f ⊗' f') = (drop_fun f ⊗' drop_fun f') := by
  ext i : 2
  induction i <;> simp [drop_fun, *] <;> rfl

@[simp]
theorem last_fun_prod {α α' β β' : Typevec (n + 1)} (f : α ⟹ β) (f' : α' ⟹ β') :
    last_fun (f ⊗' f') = _root_.prod.map (last_fun f) (last_fun f') := by
  ext i : 1
...
@[simp]
theorem subtype_val_diag_sub {α : Typevec n} : subtype_val (repeat_eq α) ⊚ diag_sub = prod.diag := by
  clear * -
  ext i
  induction i <;> [rfl, apply i_ih]

@[simp]
theorem to_subtype_of_subtype {α : Typevec n} (p : α ⟹ repeat n Prop) : to_subtype p ⊚ of_subtype p = id := by
...
theorem subtype_val_to_subtype {α : Typevec n} (p : α ⟹ repeat n Prop) :
    subtype_val p ⊚ to_subtype p = fun _ => Subtype.val := by
...
theorem to_subtype_of_subtype_assoc {α β : Typevec n} (p : α ⟹ repeat n Prop) (f : β ⟹ subtype_ p) :
    @to_subtype n _ p ⊚ of_subtype _ ⊚ f = f := by
  rw [← comp_assoc, to_subtype_of_subtype] <;> simp

@[simp]
theorem to_subtype'_of_subtype' {α : Typevec n} (r : α ⊗ α ⟹ repeat n Prop) : to_subtype' r ⊚ of_subtype' r = id := by
...
theorem subtype_val_to_subtype' {α : Typevec n} (r : α ⊗ α ⟹ repeat n Prop) :
    subtype_val r ⊚ to_subtype' r = fun i x => Prod.mk i x.1.fst x.1.snd := by
