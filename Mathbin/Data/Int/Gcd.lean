...
@[simp]
theorem xgcd_zero_left {s t r' s' t'} : xgcd_aux 0 s t r' s' t' = (r', s', t') := by
  simp [xgcd_aux]
...
@[simp]
theorem gcd_a_zero_left {s : ℕ} : gcd_a 0 s = 0 := by
  unfold gcd_a
  rw [xgcd, xgcd_zero_left]

@[simp]
theorem gcd_b_zero_left {s : ℕ} : gcd_b 0 s = 1 := by
  unfold gcd_b
  rw [xgcd, xgcd_zero_left]

@[simp]
theorem gcd_a_zero_right {s : ℕ} (h : s ≠ 0) : gcd_a s 0 = 1 := by
  unfold gcd_a xgcd
  induction s
  · exact absurd rfl h
...
@[simp]
theorem gcd_b_zero_right {s : ℕ} (h : s ≠ 0) : gcd_b s 0 = 0 := by
  unfold gcd_b xgcd
  induction s
  · exact absurd rfl h
...
theorem xgcd_aux_val x y : xgcd_aux x 1 0 y 0 1 = (gcd x y, xgcd x y) := by
  rw [xgcd, ← xgcd_aux_fst x y 1 0 0 1] <;> cases xgcd_aux x 1 0 y 0 1 <;> rfl

theorem xgcd_val x y : xgcd x y = (gcd_a x y, gcd_b x y) := by
...
-/
theorem gcd_eq_gcd_ab : (gcd x y : ℤ) = x * gcd_a x y + y * gcd_b x y := by
  have :=
...
theorem nat_abs_div (a b : ℤ) (H : b ∣ a) : nat_abs (a / b) = nat_abs a / nat_abs b := by
...
theorem gcd_mul_lcm (i j : ℤ) : gcd i j * lcm i j = nat_abs (i * j) := by
...
@[simp]
theorem gcd_self (i : ℤ) : gcd i i = nat_abs i := by
  simp [gcd]

@[simp]
theorem gcd_zero_left (i : ℤ) : gcd 0 i = nat_abs i := by
  simp [gcd]

@[simp]
theorem gcd_zero_right (i : ℤ) : gcd i 0 = nat_abs i := by
  simp [gcd]
...
theorem gcd_mul_left (i j k : ℤ) : gcd (i * j) (i * k) = nat_abs i * gcd j k := by
  rw [Int.gcdₓ, Int.gcdₓ, nat_abs_mul, nat_abs_mul]
  apply Nat.gcd_mul_leftₓ

theorem gcd_mul_right (i j k : ℤ) : gcd (i * j) (k * j) = gcd i k * nat_abs j := by
  rw [Int.gcdₓ, Int.gcdₓ, nat_abs_mul, nat_abs_mul]
...
theorem gcd_div {i j k : ℤ} (H1 : k ∣ i) (H2 : k ∣ j) : gcd (i / k) (j / k) = gcd i j / nat_abs k := by
...
theorem gcd_eq_right {i j : ℤ} (H : j ∣ i) : gcd i j = nat_abs j := by
...
@[simp]
theorem lcm_one_left (i : ℤ) : lcm 1 i = nat_abs i := by
  rw [Int.lcm]
  apply Nat.lcm_one_leftₓ

@[simp]
theorem lcm_one_right (i : ℤ) : lcm i 1 = nat_abs i := by
  rw [Int.lcm]
  apply Nat.lcm_one_rightₓ

@[simp]
theorem lcm_self (i : ℤ) : lcm i i = nat_abs i := by
  rw [Int.lcm]
  apply Nat.lcm_selfₓ
