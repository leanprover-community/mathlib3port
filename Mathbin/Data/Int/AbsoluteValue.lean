...
@[simp]
theorem AbsoluteValue.map_units_int (abv : AbsoluteValue ℤ S) (x : Units ℤ) : abv x = 1 := by
...
@[simp]
theorem AbsoluteValue.map_units_int_smul (abv : AbsoluteValue R S) (x : Units ℤ) (y : R) : abv (x • y) = abv y := by
  rcases Int.units_eq_one_or x with (rfl | rfl) <;> simp
