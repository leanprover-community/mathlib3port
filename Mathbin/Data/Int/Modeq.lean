...
theorem coe_nat_modeq_iff {a b n : ℕ} : a ≡ b [ZMOD n] ↔ a ≡ b [MOD n] :=
  by 
...
protected theorem add_right_cancelₓ (h₁ : c ≡ d [ZMOD n]) (h₂ : (a+c) ≡ b+d [ZMOD n]) : a ≡ b [ZMOD n] :=
  by 
...
protected theorem neg (h : a ≡ b [ZMOD n]) : -a ≡ -b [ZMOD n] :=
  h.add_left_cancel
    (by 
      simpRw [←sub_eq_add_neg, sub_self])
...
protected theorem mul_left (c : ℤ) (h : a ≡ b [ZMOD n]) : (c*a) ≡ c*b [ZMOD n] :=
  Or.cases_on (le_totalₓ 0 c) (fun hc => (h.mul_left' hc).modeq_of_dvd (dvd_mul_left _ _))
    fun hc =>
      by 
...
theorem gcd_a_modeq (a b : ℕ) : ((a : ℤ)*Nat.gcdA a b) ≡ Nat.gcdₓ a b [ZMOD b] :=
  by 
...
theorem exists_unique_equiv (a : ℤ) {b : ℤ} (hb : 0 < b) : ∃ z : ℤ, 0 ≤ z ∧ z < b ∧ z ≡ a [ZMOD b] :=
  ⟨a % b, mod_nonneg _ (ne_of_gtₓ hb),
    have  : a % b < |b| := mod_lt _ (ne_of_gtₓ hb)
    by 
