...
@[symm]
theorem symm' (r : Setoidₓ α) : ∀ {x y}, r.rel x y → r.rel y x :=
  fun _ _ h => r.2.2.1 h

@[trans]
theorem trans' (r : Setoidₓ α) : ∀ {x y z}, r.rel x y → r.rel y z → r.rel x z :=
  fun _ _ _ hx => r.2.2.2 hx
...
protected def Prod (r : Setoidₓ α) (s : Setoidₓ β) : Setoidₓ (α × β) :=
...
theorem Inf_def {s : Set (Setoidₓ α)} : (Inf s).Rel = Inf (rel '' s) :=
  by 
    ext 
...
theorem sup_eq_eqv_gen (r s : Setoidₓ α) : r⊔s = EqvGen.setoid fun x y => r.rel x y ∨ s.rel x y :=
  by 
    rw [eqv_gen_eq]
...
theorem sup_def {r s : Setoidₓ α} : r⊔s = EqvGen.setoid (r.rel⊔s.rel) :=
  by 
...
theorem Sup_eq_eqv_gen (S : Set (Setoidₓ α)) : Sup S = EqvGen.setoid fun x y => ∃ r : Setoidₓ α, r ∈ S ∧ r.rel x y :=
  by 
    rw [eqv_gen_eq]
    apply congr_argₓ Inf 
...
theorem Sup_def {s : Set (Setoidₓ α)} : Sup s = EqvGen.setoid (Sup (rel '' s)) :=
  by 
...
theorem eqv_gen_le {r : α → α → Prop} {s : Setoidₓ α} (h : ∀ x y, r x y → s.rel x y) : EqvGen.setoid r ≤ s :=
  by 
...
def gi : @GaloisInsertion (α → α → Prop) (Setoidₓ α) _ _ EqvGen.setoid rel :=
  { choice := fun r h => EqvGen.setoid r,
    gc := fun r s => ⟨fun H _ _ h => H$ EqvGen.rel _ _ h, fun H => eqv_gen_of_setoid s ▸ eqv_gen_mono H⟩,
    le_l_u := fun x => (eqv_gen_of_setoid x).symm ▸ le_reflₓ x, choice_eq := fun _ _ => rfl }
...
`quotient r → β`. -/
def lift_equiv (r : Setoidₓ α) : { f : α → β // r ≤ ker f } ≃ (Quotientₓ r → β) :=
...
/-- Given a map f from α to β, the natural map from the quotient of α by the kernel of f is
    injective. -/
theorem ker_lift_injective (f : α → β) : injective (@Quotientₓ.lift _ _ (ker f) f fun _ _ h => h) :=
  fun x y => Quotientₓ.induction_on₂' x y$ fun a b h => Quotientₓ.sound' h
...
domain. -/
@[simps]
def quotient_ker_equiv_of_right_inverse (g : β → α) (hf : Function.RightInverse g f) : Quotientₓ (ker f) ≃ β :=
  { toFun := fun a => Quotientₓ.liftOn' a f$ fun _ _ => id, invFun := fun b => Quotientₓ.mk' (g b),
    left_inv :=
      fun a =>
...
theorem map_of_surjective_eq_map (h : ker f ≤ r) (hf : surjective f) : map r f = map_of_surjective r f h hf :=
  by 
...
def quotient_quotient_equiv_quotient (s : Setoidₓ α) (h : r ≤ s) : Quotientₓ (ker (Quot.mapRight h)) ≃ Quotientₓ s :=
  { toFun :=
    ...
    invFun :=
      fun x =>
        (Quotientₓ.liftOn' x fun w => @Quotientₓ.mk _ (ker$ Quot.mapRight h)$ @Quotientₓ.mk _ r w)$
          fun x y H => Quotientₓ.sound'$ show @Quot.mk _ _ _ = @Quot.mk _ _ _ from Quotientₓ.sound H,
    left_inv :=
      fun x =>
...
def correspondence (r : Setoidₓ α) : { s // r ≤ s } ≃o Setoidₓ (Quotientₓ r) :=
  { toFun := fun s => map_of_surjective s.1 Quotientₓ.mk ((ker_mk_eq r).symm ▸ s.2) exists_rep,
    invFun :=
      fun s =>
        ⟨comap Quotientₓ.mk' s,
          fun x y h =>
            by 
    ...
    right_inv :=
      fun s =>
        let Hm : ker Quotientₓ.mk' ≤ comap Quotientₓ.mk' s :=
          fun x y h =>
            by 
...
@[simp]
theorem Quotientₓ.subsingleton_iff {s : Setoidₓ α} : Subsingleton (Quotientₓ s) ↔ s = ⊤ :=
  by 
