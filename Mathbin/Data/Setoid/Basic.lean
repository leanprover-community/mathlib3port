...
instance
  ...
  where
    le := · ≤ ·
      ...
      le_antisymm r s h1 h2 := Setoidₓ.ext' $ fun x y => ⟨ fun h => h1 h , fun h => h2 h ⟩
...
theorem map_of_surjective_eq_map (h : ker f ≤ r) (hf : surjective f) : map r f = map_of_surjective r f h hf := by
