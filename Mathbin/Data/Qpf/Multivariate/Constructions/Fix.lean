...
theorem recF_eq {α : Typevec n} {β : Type _} (g : F (α.append1 β) → β) (a : q.P.A) (f' : q.P.drop.B a ⟹ α)
    (f : q.P.last.B a → q.P.W α) : recF g (q.P.W_mk a f' f) = g (abs ⟨a, split_fun f' (recF g ∘ f)⟩) := by
  rw [recF, Mvpfunctor.W_rec_eq] <;> rfl

theorem recF_eq' {α : Typevec n} {β : Type _} (g : F (α.append1 β) → β) (x : q.P.W α) :
    recF g x = g (abs (append_fun id (recF g) <$$> q.P.W_dest' x)) := by
...
theorem Wequiv.abs' {α : Typevec n} (x y : q.P.W α) (h : abs (q.P.W_dest' x) = abs (q.P.W_dest' y)) : Wequiv x y := by
  revert h
  apply q.P.W_cases _ x
  intro a₀ f'₀ f₀
  apply q.P.W_cases _ y
...
theorem Wrepr_W_mk {α : Typevec n} (a : q.P.A) (f' : q.P.drop.B a ⟹ α) (f : q.P.last.B a → q.P.W α) :
    Wrepr (q.P.W_mk a f' f) = q.P.W_mk' (reprₓ (abs (append_fun id Wrepr <$$> ⟨a, q.P.append_contents f' f⟩))) := by
...
def fix.map {α β : Typevec n} (g : α ⟹ β) : fix F α → fix F β :=
  Quotientₓ.lift (fun x : q.P.W α => ⟦q.P.W_map g x⟧) fun a b h => Quot.sound (Wequiv_map _ _ _ h)
...
theorem fix.ind_aux (a : q.P.A) (f' : q.P.drop.B a ⟹ α) (f : q.P.last.B a → q.P.W α) :
    fix.mk (abs ⟨a, q.P.append_contents f' fun x => ⟦f x⟧⟩) = ⟦q.P.W_mk a f' f⟧ :=
  have : fix.mk (abs ⟨a, q.P.append_contents f' fun x => ⟦f x⟧⟩) = ⟦Wrepr (q.P.W_mk a f' f)⟧ := by
    apply Quot.sound
    apply Wequiv.abs'
...
theorem fix.rec_unique {β : Type _} (g : F (append1 α β) → β) (h : fix F α → β)
    (hyp : ∀ x, h (fix.mk x) = g (append_fun id h <$$> x)) : fix.rec g = h := by
  ext x
  apply fix.ind_rec
  intro x hyp'
...
instance
  ...
  where
    p := q.P.Wp
      ...
      abs_map := by intro α β g x conv => rhs dsimp [ Mvfunctor.map ] rw [ fix.map ] apply Quot.sound apply Wequiv.refl
