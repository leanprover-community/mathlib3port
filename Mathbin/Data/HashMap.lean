...
theorem mem_as_list {a : Σ a, β a} : a ∈ data.as_list ↔ ∃ i, a ∈ Arrayₓ.read data i :=
  have  :
  ...
  :=
    by
      unfold BucketArray.asList
        ...
        · conv => lhs rw [ ← List.take_append_drop bidx bkts.to_list , List.drop_eq_nth_le_cons h ] simp [ hl ] simp
        · conv => lhs rw [ bkts' , Arrayₓ.write_to_list , List.update_nth_eq_take_cons_drop _ h ] simp [ hfl ] simp
