...
-- error in Data.Seq.Wseq: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exact: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
@[simp]
theorem destruct_flatten (c : computation (wseq α)) : «expr = »(destruct (flatten c), «expr >>= »(c, destruct)) :=
begin
  refine [expr computation.eq_of_bisim (λ
    c1
    c2, «expr ∨ »(«expr = »(c1, c2), «expr∃ , »((c), «expr ∧ »(«expr = »(c1, destruct (flatten c)), «expr = »(c2, computation.bind c destruct))))) _ (or.inr ⟨c, rfl, rfl⟩)],
  intros [ident c1, ident c2, ident h],
  exact [expr match c1, c2, h with
   | _, _, «expr $ »(or.inl, eq.refl c) := by cases [expr c.destruct] []; simp [] [] [] [] [] []
   | _, _, or.inr ⟨c, rfl, rfl⟩ := begin
     apply [expr c.cases_on (λ a, _) (λ c', _)]; repeat { simp [] [] [] [] [] [] },
     { cases [expr (destruct a).destruct] []; simp [] [] [] [] [] [] },
     { exact [expr or.inr ⟨c', rfl, rfl⟩] }
   end
   end]
end
...
-- error in Data.Seq.Wseq: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem head_some_of_nth_some
{s : wseq α}
{a n}
(h : «expr ∈ »(some a, nth s n)) : «expr∃ , »((a'), «expr ∈ »(some a', head s)) :=
begin
  revert [ident a],
  induction [expr n] [] ["with", ident n, ident IH] []; intros [],
  exacts ["[", expr ⟨_, h⟩, ",", expr let ⟨a', h'⟩ := head_some_of_head_tail_some h in IH h', "]"]
end
...
-- error in Data.Seq.Wseq: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
-- error in Data.Seq.Wseq: ././Mathport/Syntax/Translate/Basic.lean:340:40: in repeat: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem length_eq_map (s : wseq α) : «expr = »(length s, computation.map list.length (to_list s)) :=
begin
  refine [expr eq_of_bisim (λ
    c1
    c2, «expr∃ , »((l : list α)
     (s : wseq α), «expr ∧ »(«expr = »(c1, corec length._match_2 (l.length, s)), «expr = »(c2, computation.map list.length (corec to_list._match_2 (l, s)))))) _ ⟨«expr[ , ]»([]), s, rfl, rfl⟩],
  intros [ident s1, ident s2, ident h],
  rcases [expr h, "with", "⟨", ident l, ",", ident s, ",", ident h, "⟩"],
  rw ["[", expr h.left, ",", expr h.right, "]"] [],
  apply [expr s.cases_on _ (λ
    a
    s, _) (λ
    s, _)]; repeat { simp [] [] [] ["[", expr to_list, ",", expr nil, ",", expr cons, ",", expr think, ",", expr length, "]"] [] [] },
  { refine [expr ⟨[«expr :: »/«expr :: »](a, l), s, _, _⟩]; simp [] [] [] [] [] [] },
  { refine [expr ⟨l, s, _, _⟩]; simp [] [] [] [] [] [] }
end
...
(l : list α)
(s : wseq α) : «expr = »(corec to_list._match_2 (l, s), «expr <$> »(((«expr ++ »)) l.reverse, to_list s)) :=
begin
  refine [expr eq_of_bisim (λ
    c1
    c2, «expr∃ , »((l' : list α)
     (s : wseq α), «expr ∧ »(«expr = »(c1, corec to_list._match_2 («expr ++ »(l', l), s)), «expr = »(c2, computation.map (((«expr ++ »)) l.reverse) (corec to_list._match_2 (l', s)))))) _ ⟨«expr[ , ]»([]), s, rfl, rfl⟩],
  intros [ident s1, ident s2, ident h],
  rcases [expr h, "with", "⟨", ident l', ",", ident s, ",", ident h, "⟩"],
  rw ["[", expr h.left, ",", expr h.right, "]"] [],
  apply [expr s.cases_on _ (λ
