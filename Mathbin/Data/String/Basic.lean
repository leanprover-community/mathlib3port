...
instance decidable_lt : @DecidableRel Stringₓ (· < ·) :=
  by 
    infer_instance
...
instance decidable_le : @DecidableRel Stringₓ (· ≤ ·) :=
  by 
    infer_instance
...
theorem as_string_inv_to_list (s : Stringₓ) : s.to_list.as_string = s :=
  by 
    cases s 
    rfl
...
theorem List.to_list_inv_as_string (l : List Charₓ) : l.as_string.to_list = l :=
  by 
...
@[simp]
theorem Stringₓ.length_to_list (s : Stringₓ) : s.to_list.length = s.length :=
  by 
    rw [←Stringₓ.as_string_inv_to_list s, List.to_list_inv_as_string, List.length_as_string]

theorem List.as_string_eq {l : List Charₓ} {s : Stringₓ} : l.as_string = s ↔ l = s.to_list :=
  by 
