  ...
  { toFun := fun p => p.sum fun A B => monomial (A - single i 1) (B*A i),
    map_smul' :=
      by 
        ...
        ·
          dsimp 
          simpRw [←(monomial _).map_smul, smul_eq_mul, mul_assocₓ]
        ·
          intro s 
...
theorem pderiv_X [DecidableEq σ] {i j : σ} : pderiv i (X j : MvPolynomial σ R) = if i = j then 1 else 0 :=
  by 
...
@[simp]
theorem pderiv_X_self {i : σ} : pderiv i (X i : MvPolynomial σ R) = 1 :=
  by 
    simp [pderiv_X]
...
private theorem monomial_sub_single_one_add {i : σ} {s' : σ →₀ ℕ} :
  monomial ((s - single i 1)+s') ((a*s i)*a') = monomial ((s+s') - single i 1) ((a*s i)*a') :=
  by 
    byCases' h : s i = 0 <;> simp [h, sub_single_one_add]

private theorem monomial_add_sub_single_one {i : σ} {s' : σ →₀ ℕ} :
  monomial (s+s' - single i 1) (a*a'*s' i) = monomial ((s+s') - single i 1) (a*a'*s' i) :=
  by 
    byCases' h : s' i = 0 <;> simp [h, add_sub_single_one]
...
@[simp]
theorem pderiv_mul {i : σ} {f g : MvPolynomial σ R} : pderiv i (f*g) = (pderiv i f*g)+f*pderiv i g :=
  by 
...
@[simp]
theorem pderiv_pow {i : σ} {f : MvPolynomial σ R} {n : ℕ} : pderiv i (f ^ n) = (n*pderiv i f)*f ^ (n - 1) :=
  by 
...
@[simp]
theorem pderiv_nat_cast {i : σ} {n : ℕ} : pderiv i (n : MvPolynomial σ R) = 0 :=
  by 
    induction' n with n ih
    ·
      simp 
