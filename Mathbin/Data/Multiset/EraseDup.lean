...
theorem le_erase_dup {s t : Multiset α} : s ≤ erase_dup t ↔ s ≤ t ∧ nodup s :=
  ⟨fun h => ⟨le_transₓ h (erase_dup_le _), nodup_of_le h (nodup_erase_dup _)⟩,
    fun ⟨l, d⟩ => (le_iff_subset d).2$ subset.trans (subset_of_le l) (subset_erase_dup _)⟩
...
theorem erase_dup_map_erase_dup_eq [DecidableEq β] (f : α → β) (s : Multiset α) :
  erase_dup (map f (erase_dup s)) = erase_dup (map f s) :=
  by 
    simp [erase_dup_ext]

@[simp]
theorem erase_dup_nsmul {s : Multiset α} {n : ℕ} (h0 : n ≠ 0) : (n • s).eraseDup = s.erase_dup :=
  by 
    ext a 
    byCases' h : a ∈ s <;> simp [h, h0]

theorem nodup.le_erase_dup_iff_le {s t : Multiset α} (hno : s.nodup) : s ≤ t.erase_dup ↔ s ≤ t :=
  by 
