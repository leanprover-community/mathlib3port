...
theorem disj_sum_mono_right (s : Multiset α) : Monotone (s.disjSum : Multiset β → Multiset (α ⊕ β)) := fun t₁ t₂ ht =>
...
theorem disj_sum_strict_mono_right (s : Multiset α) : StrictMono (s.disjSum : Multiset β → Multiset (α ⊕ β)) :=
