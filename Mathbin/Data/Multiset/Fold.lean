...
theorem
  ...
  :=
    Multiset.induction_on
      s₂
        by rw [ add_zeroₓ , fold_zero , ← fold_cons'_right , ← fold_cons_right op ]
        by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) <;> cc
...
theorem
  ...
  := Multiset.induction_on s by simp by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) <;> cc
...
theorem
  ...
  := Multiset.induction_on s by simp by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) [ hm ]
