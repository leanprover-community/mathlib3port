...
-/
def graph (f : Finₓ n → α) : Finsetₓ (α ×ₗ Finₓ n) :=
  Finsetₓ.univ.Image fun i => (f i, i)
...
theorem graph.card (f : Finₓ n → α) : (graph f).card = n := by
  rw [graph, Finsetₓ.card_image_of_injective]
  · exact Finsetₓ.card_fin _
...
def graphEquiv₁ (f : Finₓ n → α) : Finₓ n ≃ graph f where
  toFun := fun i => ⟨(f i, i), by simp [graph]⟩
  invFun := fun p => p.1.2
...
def graphEquiv₂ (f : Finₓ n → α) : Finₓ n ≃o graph f :=
  Finsetₓ.orderIsoOfFin _ (by simp)
...
theorem monotone_proj (f : Finₓ n → α) : Monotoneₓ (graph.proj : graph f → α) := by
...
theorem monotone_sort (f : Finₓ n → α) : Monotoneₓ (f ∘ sort f) := by
  rw [self_comp_sort]
