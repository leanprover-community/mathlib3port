...
antitone values. -/
theorem bubble_sort_induction' {n : ℕ} {α : Type _} [LinearOrder α] {f : Fin n → α} {P : (Fin n → α) → Prop} (hf : P f)
    (h :
  ...
  refine'
    @WellFounded.induction_bot' _ _ _ (@Finite.Preorder.well_founded_lt (Equiv.Perm (Fin n)) _ _) (Equiv.refl _)
