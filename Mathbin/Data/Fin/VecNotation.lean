...
@[simp]
theorem cons_val_succ (x : α) (u : Finₓ m → α) (i : Finₓ m) : vec_cons x u i.succ = u i := by
  simp [vec_cons]

@[simp]
theorem cons_val_succ' {i : ℕ} (h : i.succ < m.succ) (x : α) (u : Finₓ m → α) :
    vec_cons x u ⟨i.succ, h⟩ = u ⟨i, Nat.lt_of_succ_lt_succₓ h⟩ := by
...
@[simp]
theorem tail_cons (x : α) (u : Finₓ m → α) : vec_tail (vec_cons x u) = u := by
  ext
  simp [vec_tail]
...
-/
@[simp]
theorem cons_val_one (x : α) (u : Finₓ m.succ → α) : vec_cons x u 1 = vec_head u := by
  rw [← Finₓ.succ_zero_eq_one, cons_val_succ]
  rfl

@[simp]
theorem cons_val_fin_one (x : α) (u : Finₓ 0 → α) (i : Finₓ 1) : vec_cons x u i = x := by
...
@[simp]
theorem neg_cons (x : α) (v : Finₓ n → α) : -vec_cons x v = vec_cons (-x) (-v) := by
  ext i
  refine' Finₓ.cases _ _ i <;> simp
