...
@[simp]
protected theorem eta (a : Finₓ n) (h : (a : ℕ) < n) : (⟨(a : ℕ), h⟩ : Finₓ n) = a :=
  by 
    cases a <;> rfl
...
protected theorem heq_ext_iff {k l : ℕ} (h : k = l) {i : Finₓ k} {j : Finₓ l} : HEq i j ↔ (i : ℕ) = (j : ℕ) :=
  by 
    induction h 
    simp [ext_iff]
...
theorem pos_iff_ne_zero (a : Finₓ (n+1)) : 0 < a ↔ a ≠ 0 :=
  by 
    rw [←coe_fin_lt, coe_zero, pos_iff_ne_zero, Ne.def, Ne.def, ext_iff, coe_zero]

theorem eq_zero_or_eq_succ {n : ℕ} (i : Finₓ (n+1)) : i = 0 ∨ ∃ j : Finₓ n, i = j.succ :=
  by 
    rcases i with ⟨_ | j, h⟩
    ·
      left 
...
@[simp]
protected theorem add_zeroₓ (k : Finₓ (n+1)) : (k+0) = k :=
  by 
    simp [eq_iff_veq, add_def, mod_eq_of_lt (is_lt k)]

@[simp]
protected theorem zero_addₓ (k : Finₓ (n+1)) : ((0 : Finₓ (n+1))+k) = k :=
  by 
    simp [eq_iff_veq, add_def, mod_eq_of_lt (is_lt k)]

instance AddCommMonoidₓ (n : ℕ) : AddCommMonoidₓ (Finₓ (n+1)) :=
  { add := ·+·,
    add_assoc :=
      by 
        simp [eq_iff_veq, add_def, add_assocₓ],
    zero := 0, zero_add := Finₓ.zero_add, add_zero := Finₓ.add_zero,
    add_comm :=
      by 
...
theorem coe_bit1 {n : ℕ} (k : Finₓ (n+1)) : ((bit1 k : Finₓ (n+1)) : ℕ) = bit1 (k : ℕ) % n+1 :=
  by 
    cases n
    ·
      cases' k with k h 
...
theorem coe_add_one {n : ℕ} (i : Finₓ (n+1)) : ((i+1 : Finₓ (n+1)) : ℕ) = if i = last _ then 0 else i+1 :=
  by 
...
@[simp]
theorem of_nat_eq_coe (n : ℕ) (a : ℕ) : (of_nat a : Finₓ (n+1)) = a :=
  by 
...
theorem coe_val_of_lt {n : ℕ} {a : ℕ} (h : a < n+1) : (a : Finₓ (n+1)).val = a :=
  by 
    rw [←of_nat_eq_coe]
...
in the same value.  -/
theorem coe_val_eq_self {n : ℕ} (a : Finₓ (n+1)) : (a.val : Finₓ (n+1)) = a :=
  by 
    rw [Finₓ.eq_iff_veq]
...
theorem add_one_pos (i : Finₓ (n+1)) (h : i < Finₓ.last n) : (0 : Finₓ (n+1)) < i+1 :=
  by 
    cases n
    ·
...
@[simp]
theorem coe_succ (j : Finₓ n) : (j.succ : ℕ) = j+1 :=
  by 
    cases j <;> simp [Finₓ.succ]

theorem succ_pos (a : Finₓ n) : (0 : Finₓ (n+1)) < a.succ :=
  by 
    simp [lt_iff_coe_lt_coe]
...
theorem mk_succ_pos (i : ℕ) (h : i < n) : (0 : Finₓ (n+1)) < ⟨i.succ, add_lt_add_right h 1⟩ :=
  by 
    rw [lt_iff_coe_lt_coe, coe_zero]
    exact Nat.succ_posₓ i

theorem one_lt_succ_succ (a : Finₓ n) : (1 : Finₓ (n+2)) < a.succ.succ :=
  by 
    cases n
    ·
      exact finZeroElim a
...
@[simp]
theorem coe_cast_lt (i : Finₓ m) (h : i.1 < n) : (cast_lt i h : ℕ) = i :=
  rfl
...
@[simp]
theorem cast_le_mk (i n m : ℕ) (hn : i < n) (h : n ≤ m) : cast_le h ⟨i, hn⟩ = ⟨i, lt_of_lt_of_leₓ hn h⟩ :=
  rfl

@[simp]
theorem cast_le_zero {n m : ℕ} (h : n.succ ≤ m.succ) : cast_le h 0 = 0 :=
  by 
    simp [eq_iff_veq]
...
@[simp]
theorem cast_refl (h : n = n := rfl) : cast h = OrderIso.refl (Finₓ n) :=
  by 
    ext 
    rfl
...
/-- While in many cases `fin.cast` is better than `equiv.cast`/`cast`, sometimes we want to apply
a generic theorem about `cast`. -/
theorem cast_eq_cast (h : n = m) : (cast h : Finₓ n → Finₓ m) = _root_.cast (h ▸ rfl) :=
  by 
    subst h 
    ext 
    simp 
...
theorem cast_add_lt {m : ℕ} (n : ℕ) (i : Finₓ m) : (cast_add n i : ℕ) < m :=
  i.2

@[simp]
theorem cast_add_mk (m : ℕ) (i : ℕ) (h : i < n) : cast_add m ⟨i, h⟩ = ⟨i, lt_add_right i n m h⟩ :=
  rfl
...
@[simp]
theorem cast_succ_mk (n i : ℕ) (h : i < n) : cast_succ ⟨i, h⟩ = ⟨i, Nat.Lt.step h⟩ :=
  rfl
...
theorem le_cast_succ_iff {i : Finₓ (n+1)} {j : Finₓ n} : i ≤ j.cast_succ ↔ i < j.succ :=
  by 
...
@[simp]
theorem succ_eq_last_succ {n : ℕ} (i : Finₓ n.succ) : i.succ = last (n+1) ↔ i = last n :=
  by 
...
@[simp]
theorem cast_lt_cast_succ {n : ℕ} (a : Finₓ n) (h : (a : ℕ) < n) : cast_lt (cast_succ a) h = a :=
  by 
    cases a <;> rfl
...
theorem cast_succ_pos {i : Finₓ (n+1)} (h : 0 < i) : 0 < cast_succ i :=
  by 
...
theorem cast_succ_fin_succ (n : ℕ) (j : Finₓ n) : cast_succ (Finₓ.succ j) = Finₓ.succ (cast_succ j) :=
  by 
    simp [Finₓ.ext_iff]

@[normCast, simp]
theorem coe_eq_cast_succ : (a : Finₓ (n+1)) = a.cast_succ :=
  by 
    ext 
    exact coe_val_of_lt (Nat.Lt.step a.is_lt)

@[simp]
theorem coe_succ_eq_succ : (a.cast_succ+1) = a.succ :=
  by 
    cases n
    ·
      exact finZeroElim a
...
def add_nat m : Finₓ n ↪o Finₓ (n+m) :=
  (OrderEmbedding.ofStrictMono fun i => ⟨(i : ℕ)+m, add_lt_add_right i.2 _⟩)$
    fun i j h => lt_iff_coe_lt_coe.2$ add_lt_add_right h _
...
def nat_add n {m} : Finₓ m ↪o Finₓ (n+m) :=
  (OrderEmbedding.ofStrictMono fun i => ⟨n+(i : ℕ), add_lt_add_left i.2 _⟩)$
    fun i j h => lt_iff_coe_lt_coe.2$ add_lt_add_left h _
...
theorem nat_add_zero {n : ℕ} : Finₓ.natAdd 0 = (Finₓ.cast (zero_addₓ n).symm).toRelEmbedding :=
  by 
    ext 
    apply zero_addₓ
...
@[simp]
theorem coe_pred (j : Finₓ (n+1)) (h : j ≠ 0) : (j.pred h : ℕ) = j - 1 :=
  by 
    cases j 
    rfl
...
@[simp]
theorem pred_succ (i : Finₓ n) {h : i.succ ≠ 0} : i.succ.pred h = i :=
  by 
    cases i 
    rfl
...
theorem pred_mk_succ (i : ℕ) (h : i < n+1) :
  Finₓ.pred ⟨i+1, add_lt_add_right h 1⟩ (ne_of_vne (ne_of_gtₓ (mk_succ_pos i h))) = ⟨i, h⟩ :=
  by 
...
@[simp]
theorem pred_le_pred_iff {n : ℕ} {a b : Finₓ n.succ} {ha : a ≠ 0} {hb : b ≠ 0} : a.pred ha ≤ b.pred hb ↔ a ≤ b :=
  by 
    rw [←succ_le_succ_iff, succ_pred, succ_pred]

@[simp]
theorem pred_lt_pred_iff {n : ℕ} {a b : Finₓ n.succ} {ha : a ≠ 0} {hb : b ≠ 0} : a.pred ha < b.pred hb ↔ a < b :=
  by 
...
theorem pred_add_one (i : Finₓ (n+2)) (h : (i : ℕ) < n+1) :
  pred (i+1) (ne_of_gtₓ (add_one_pos _ (lt_iff_coe_lt_coe.mpr h))) = cast_lt i h :=
  by 
...
theorem pred_cast_succ_succ (i : Finₓ n) :
  pred (cast_succ i.succ) (ne_of_gtₓ (cast_succ_pos i.succ_pos)) = i.cast_succ :=
  by 
    simp [eq_iff_veq]
...
@[simp]
theorem nat_add_sub_nat_cast {i : Finₓ (n+m)} (h : n ≤ i) : nat_add n (sub_nat n (cast (add_commₓ _ _) i) h) = i :=
  by 
    simp [←cast_add_nat]
...
@[elab_as_eliminator]
def succ_rec_on {n : ℕ} (i : Finₓ n) {C : ∀ n, Finₓ n → Sort _} (H0 : ∀ n, C (succ n) 0)
...
theorem succ_rec_on_succ {C : ∀ n, Finₓ n → Sort _} {H0 Hs} {n} (i : Finₓ n) :
  @Finₓ.succRecOn (succ n) i.succ C H0 Hs = Hs n i (Finₓ.succRecOn i H0 Hs) :=
  by 
    cases i <;> rfl
...
@[simp]
theorem cases_succ {n} {C : Finₓ (succ n) → Sort _} {H0 Hs} (i : Finₓ n) : @Finₓ.cases n C H0 Hs i.succ = Hs i :=
  by 
    cases i <;> rfl

@[simp]
theorem cases_succ' {n} {C : Finₓ (succ n) → Sort _} {H0 Hs} {i : ℕ} (h : (i+1) < n+1) :
  @Finₓ.cases n C H0 Hs ⟨i.succ, h⟩ = Hs ⟨i, lt_of_succ_lt_succ h⟩ :=
  by 
    cases i <;> rfl
...
theorem exists_fin_succ {P : Finₓ (n+1) → Prop} : (∃ i, P i) ↔ P 0 ∨ ∃ i : Finₓ n, P i.succ :=
  ⟨fun ⟨i, h⟩ => Finₓ.cases Or.inl (fun i hi => Or.inr ⟨i, hi⟩) i h,
    fun h => (Or.elim h fun h => ⟨0, h⟩)$ fun ⟨i, hi⟩ => ⟨i.succ, hi⟩⟩
...
def add_cases {m n : ℕ} {C : Finₓ (m+n) → Sort u} (hleft : ∀ i, C (cast_add n i)) (hright : ∀ i, C (nat_add m i))
  (i : Finₓ (m+n)) : C i :=
  if hi : (i : ℕ) < m then Eq.recOnₓ (cast_add_cast_lt n i hi) (hleft (cast_lt i hi)) else
    Eq.recOnₓ (nat_add_sub_nat_cast (le_of_not_ltₓ hi)) (hright _)

@[simp]
theorem add_cases_left {m n : ℕ} {C : Finₓ (m+n) → Sort _} (hleft : ∀ i, C (cast_add n i))
  (hright : ∀ i, C (nat_add m i)) (i : Finₓ m) : add_cases hleft hright (Finₓ.castAdd n i) = hleft i :=
  by 
    cases' i with i hi 
...
protected theorem coe_sub (a b : Finₓ n) : ((a - b : Finₓ n) : ℕ) = (a+n - b) % n :=
  by 
    cases a <;> cases b <;> rfl
...
theorem succ_above_below (p : Finₓ (n+1)) (i : Finₓ n) (h : i.cast_succ < p) : p.succ_above i = i.cast_succ :=
  by 
    rw [succ_above]
    exact if_pos h

@[simp]
theorem succ_above_ne_zero_zero {a : Finₓ (n+2)} (ha : a ≠ 0) : a.succ_above 0 = 0 :=
  by 
...
theorem succ_above_eq_zero_iff {a : Finₓ (n+2)} {b : Finₓ (n+1)} (ha : a ≠ 0) : a.succ_above b = 0 ↔ b = 0 :=
  by 
...
@[simp]
theorem succ_above_last : succ_above (Finₓ.last n) = cast_succ :=
  by 
    ext 
    simp only [succ_above_below, cast_succ_lt_last]

theorem succ_above_last_apply (i : Finₓ n) : succ_above (Finₓ.last n) i = i.cast_succ :=
  by 
    rw [succ_above_last]

/-- Embedding `i : fin n` into `fin (n + 1)` with a hole around `p : fin (n + 1)`
embeds `i` by `succ` when the resulting `p < i.succ`. -/
theorem succ_above_above (p : Finₓ (n+1)) (i : Finₓ n) (h : p ≤ i.cast_succ) : p.succ_above i = i.succ :=
  by 
...
@[simp]
theorem succ_above_lt_iff (p : Finₓ (n+1)) (i : Finₓ n) : p.succ_above i < p ↔ i.cast_succ < p :=
  by 
...
theorem lt_succ_above_iff (p : Finₓ (n+1)) (i : Finₓ n) : p < p.succ_above i ↔ p ≤ i.cast_succ :=
  by 
    refine' Iff.intro _ _
    ·
      intro h 
...
theorem succ_above_ne (p : Finₓ (n+1)) (i : Finₓ n) : p.succ_above i ≠ p :=
  by 
    intro eq 
    byCases' H : i.cast_succ < p
    ·
...
theorem succ_above_pos (p : Finₓ (n+2)) (i : Finₓ (n+1)) (h : 0 < i) : 0 < p.succ_above i :=
  by 
    byCases' H : i.cast_succ < p
    ·
...
theorem pred_succ_above {x : Finₓ n} {y : Finₓ (n+1)} (h : y ≤ cast_succ x)
  (h' : y.succ_above x ≠ 0 := (y.zero_le.trans_lt$ (lt_succ_above_iff _ _).2 h).ne') : (y.succ_above x).pred h' = x :=
  by 
...
theorem succ_succ_above_succ {n : ℕ} (i : Finₓ (n+1)) (j : Finₓ n) : i.succ.succAbove j.succ = (i.succ_above j).succ :=
  (lt_or_geₓ j.cast_succ i).elim
    (fun h =>
      have h' : j.succ.cast_succ < i.succ :=
        by 
          simpa [lt_iff_coe_lt_coe] using h 
      by 
        ext 
    ...
    fun h =>
      have h' : i.succ ≤ j.succ.cast_succ :=
        by 
...
def pred_above (p : Finₓ n) (i : Finₓ (n+1)) : Finₓ n :=
  if h : p.cast_succ < i then i.pred (ne_of_ltₓ (lt_of_le_of_ltₓ (zero_le p.cast_succ) h)).symm else
    i.cast_lt (lt_of_le_of_ltₓ (le_of_not_ltₓ h) p.2)

theorem pred_above_right_monotone (p : Finₓ n) : Monotone p.pred_above :=
  fun a b H =>
    by 
      dsimp [pred_above]
...
@[simp]
theorem cast_pred_one : cast_pred (1 : Finₓ (n+2)) = 1 :=
  by 
    cases n 
...
@[simp]
theorem pred_above_zero {i : Finₓ (n+2)} (hi : i ≠ 0) : pred_above 0 i = i.pred hi :=
  by 
    dsimp [pred_above]
    rw [dif_pos]
...
@[simp]
theorem cast_pred_last : cast_pred (last (n+1)) = last n :=
  by 
...
theorem pred_above_last_apply (i : Finₓ n) : pred_above (Finₓ.last n) i = i.cast_pred :=
  by 
    rw [pred_above_last]
...
@[simp]
theorem pred_above_succ_above (p : Finₓ n) (i : Finₓ n) : p.pred_above (p.cast_succ.succ_above i) = i :=
  by 
    dsimp [pred_above, succ_above]
    rcases p with ⟨p, _⟩
    rcases i with ⟨i, _⟩
...
theorem cast_succ_pred_eq_pred_cast_succ {a : Finₓ (n+1)} (ha : a ≠ 0) (ha' := a.cast_succ_ne_zero_iff.mpr ha) :
  (a.pred ha).cast_succ = a.cast_succ.pred ha' :=
  by 
    cases a 
    rfl
...
theorem cast_succ_cast_pred {i : Finₓ (n+2)} (h : i < last _) : cast_succ i.cast_pred = i :=
  by 
...
theorem coe_cast_pred_le_self (i : Finₓ (n+2)) : (i.cast_pred : ℕ) ≤ i :=
  by 
...
theorem coe_cast_pred_lt_iff {i : Finₓ (n+2)} : (i.cast_pred : ℕ) < i ↔ i = Finₓ.last _ :=
  by 
    rcases i.le_last.eq_or_lt with (rfl | H)
    ·
      simp 
...
theorem lt_last_iff_coe_cast_pred {i : Finₓ (n+2)} : i < Finₓ.last _ ↔ (i.cast_pred : ℕ) = i :=
  by 
    rcases i.le_last.eq_or_lt with (rfl | H)
    ·
      simp 
...
theorem coe_clamp (n m : ℕ) : (clamp n m : ℕ) = min n m :=
  Nat.mod_eq_of_ltₓ$ Nat.lt_succ_iff.mpr$ min_le_rightₓ _ _

@[simp]
theorem coe_of_nat_eq_mod (m n : ℕ) : ((n : Finₓ (succ m)) : ℕ) = n % succ m :=
  by 
...
@[simp]
protected theorem mul_oneₓ (k : Finₓ (n+1)) : (k*1) = k :=
  by 
    cases n 
    simp 
...
@[simp]
protected theorem one_mulₓ (k : Finₓ (n+1)) : ((1 : Finₓ (n+1))*k) = k :=
  by 
    cases n 
    simp 
    simp [eq_iff_veq, mul_def, mod_eq_of_lt (is_lt k)]
...
@[simp]
protected theorem mul_zero (k : Finₓ (n+1)) : (k*0) = 0 :=
  by 
    simp [eq_iff_veq, mul_def]

@[simp]
protected theorem zero_mul (k : Finₓ (n+1)) : ((0 : Finₓ (n+1))*k) = 0 :=
  by 
    simp [eq_iff_veq, mul_def]
