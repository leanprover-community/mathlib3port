...
@[simp]
theorem zip_with_nil_right (f : α → β → γ) l : zip_with f l [] = [] := by
  cases l <;> rfl
...
theorem lt_length_left_of_zip_with {f : α → β → γ} {i : ℕ} {l : List α} {l' : List β}
    (h : i < (zip_with f l l').length) : i < l.length := by
...
theorem zip_with_map {μ} (f : γ → δ → μ) (g : α → γ) (h : β → δ) (as : List α) (bs : List β) :
    zip_with f (as.map g) (bs.map h) = zip_with (fun a b => f (g a) (h b)) as bs := by
...
theorem map_zip_with {δ : Type _} (f : α → β) (g : γ → δ → α) (l : List γ) (l' : List δ) :
    map f (zip_with g l l') = zip_with (fun x y => f (g x y)) l l' := by
...
theorem revzip_swap (l : List α) : (revzip l).map Prod.swap = revzip l.reverse := by
  simp [revzip]

theorem nth_zip_with (f : α → β → γ) (l₁ : List α) (l₂ : List β) (i : ℕ) :
    (zip_with f l₁ l₂).nth i = ((l₁.nth i).map f).bind fun g => (l₂.nth i).map g := by
...
theorem nth_zip_with_eq_some {α β γ} (f : α → β → γ) (l₁ : List α) (l₂ : List β) (z : γ) (i : ℕ) :
    (zip_with f l₁ l₂).nth i = some z ↔ ∃ x y, l₁.nth i = some x ∧ l₂.nth i = some y ∧ f x y = z := by
  induction l₁ generalizing l₂ i
  · simp [zip_with]
...
theorem nth_zip_eq_some (l₁ : List α) (l₂ : List β) (z : α × β) (i : ℕ) :
    (zip l₁ l₂).nth i = some z ↔ l₁.nth i = some z.1 ∧ l₂.nth i = some z.2 := by
  cases z
...
theorem map_uncurry_zip_eq_zip_with (f : α → β → γ) (l : List α) (l' : List β) :
    map (Function.uncurry f) (l.zip l') = zip_with f l l' := by
  induction' l with hd tl hl generalizing l'
  · simp
...
theorem sum_zip_with_distrib_left {γ : Type _} [Semiringₓ γ] (f : α → β → γ) (n : γ) (l : List α) (l' : List β) :
    (l.zip_with (fun x y => n * f x y) l').Sum = n * (l.zip_with f l').Sum := by
...
theorem zip_with_distrib_take : (zip_with f l l').take n = zip_with f (l.take n) (l'.take n) := by
...
theorem zip_with_distrib_drop : (zip_with f l l').drop n = zip_with f (l.drop n) (l'.drop n) := by
  induction' l with hd tl hl generalizing l' n
  · simp
...
theorem zip_with_append (f : α → β → γ) (l la : List α) (l' lb : List β) (h : l.length = l'.length) :
    zip_with f (l ++ la) (l' ++ lb) = zip_with f l l' ++ zip_with f la lb := by
  induction' l with hd tl hl generalizing l'
  · have : l' = [] :=
...
theorem zip_with_distrib_reverse (h : l.length = l'.length) :
    (zip_with f l l').reverse = zip_with f l.reverse l'.reverse := by
  induction' l with hd tl hl generalizing l'
  · simp
