...
theorem duplicate.ne_nil (h : x ∈+ l) : l ≠ [] :=
  fun H => (mem_nil_iff x).mp (H ▸ h.mem)

@[simp]
theorem not_duplicate_nil (x : α) : ¬x ∈+ [] :=
  fun H => H.ne_nil rfl
...
@[simp]
theorem not_duplicate_singleton (x y : α) : ¬x ∈+ [y] :=
  fun H => H.ne_singleton _ rfl
...
theorem duplicate.of_duplicate_cons {y : α} (h : x ∈+ y :: l) (hx : x ≠ y) : x ∈+ l :=
  by 
    simpa [duplicate_cons_iff, hx.symm] using h

theorem duplicate_cons_iff_of_ne {y : α} (hne : x ≠ y) : x ∈+ y :: l ↔ x ∈+ l :=
  by 
...
theorem exists_duplicate_iff_not_nodup : (∃ x : α, x ∈+ l) ↔ ¬nodup l :=
  by 
    simp [nodup_iff_forall_not_duplicate]

theorem duplicate.not_nodup (h : x ∈+ l) : ¬nodup l :=
  fun H => nodup_iff_forall_not_duplicate.mp H _ h

theorem duplicate_iff_two_le_count [DecidableEq α] : x ∈+ l ↔ 2 ≤ count x l :=
  by 
...
| y :: l =>
  ...
  | is_false h =>
    if hx : y = x ∧ x ∈ l then is_true (hx.left.symm ▸ hx.right.duplicate_cons_self) else
      is_false
        (by 
