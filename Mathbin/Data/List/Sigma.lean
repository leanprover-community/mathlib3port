...
@[simp]
theorem nodupkeys_cons {s : Sigma β} {l : List (Sigma β)} : nodupkeys (s :: l) ↔ s.1 ∉ l.keys ∧ nodupkeys l :=
  by 
    simp [keys, nodupkeys]
...
theorem mem_ext {l₀ l₁ : List (Sigma β)} (nd₀ : l₀.nodup) (nd₁ : l₁.nodup) (h : ∀ x, x ∈ l₀ ↔ x ∈ l₁) : l₀ ~ l₁ :=
  by 
...
theorem lookup_eq_none {a : α} {l : List (Sigma β)} : lookup a l = none ↔ a ∉ l.keys :=
  by 
...
theorem mem_lookup {a} {b : β a} {l : List (Sigma β)} (nd : l.nodupkeys) (h : Sigma.mk a b ∈ l) : b ∈ lookup a l :=
  by 
...
-- error in Data.List.Sigma: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
...
theorem perm.kreplace {a : α} {b : β a} {l₁ l₂ : List (Sigma β)} (nd : l₁.nodupkeys) :
  ...
  perm_lookmap _$
    by 
      ...
      intro x y h z h₁ w h₂ 
      splitIfs  at h₁ h₂ <;> cases h₁ <;> cases h₂ 
...
@[simp]
theorem kerase_cons_eq {a} {s : Sigma β} {l : List (Sigma β)} (h : a = s.1) : kerase a (s :: l) = l :=
  by 
    simp [kerase, h]

@[simp]
theorem kerase_cons_ne {a} {s : Sigma β} {l : List (Sigma β)} (h : a ≠ s.1) : kerase a (s :: l) = s :: kerase a l :=
  by 
    simp [kerase, h]

@[simp]
theorem kerase_of_not_mem_keys {a} {l : List (Sigma β)} (h : a ∉ l.keys) : kerase a l = l :=
  by 
    ...
    case list.cons hd tl ih => 
      ...
      ·
        ...
        case or.inr h => 
          ...
          exact
            ⟨b, hd :: tl₁, tl₂, not_mem_cons_of_ne_of_not_mem e h₁,
              by 
                rw [h₂] <;> rfl,
              by 
                simp [e, h₃]⟩
...
theorem mem_keys_kerase_of_ne {a₁ a₂} {l : List (Sigma β)} (h : a₁ ≠ a₂) : a₁ ∈ (kerase a₂ l).keys ↔ a₁ ∈ l.keys :=
  Iff.intro mem_keys_of_mem_keys_kerase$
    fun p =>
      if q : a₂ ∈ l.keys then
...
theorem perm.kerase {a : α} {l₁ l₂ : List (Sigma β)} (nd : l₁.nodupkeys) : l₁ ~ l₂ → kerase a l₁ ~ kerase a l₂ :=
  perm.erasep _$
    (nodupkeys_iff_pairwise.1 nd).imp$
      by 
...
@[simp]
theorem lookup_kerase_ne {a a'} {l : List (Sigma β)} (h : a ≠ a') : lookup a (kerase a' l) = lookup a l :=
  by 
    induction l 
    case list.nil => 
      rfl 
...
theorem sizeof_kerase {α} {β : α → Type _} [DecidableEq α] [SizeOf (Sigma β)] (x : α) (xs : List (Sigma β)) :
  ...
  by 
    ...
    ·
      simp 
    ·
      byCases' x = y.1 <;> simp [List.sizeof]
...
theorem lookup_kinsert {a} {b : β a} (l : List (Sigma β)) : lookup a (kinsert a b l) = some b :=
  by 
...
| [] => (none, [])
| s :: l =>
  if h : s.1 = a then (some (Eq.recOnₓ h s.2), l) else
    let (b', l') := kextract l
    (b', s :: l')
...
theorem sizeof_erase_dupkeys {α} {β : α → Type _} [DecidableEq α] [SizeOf (Sigma β)] (xs : List (Sigma β)) :
  sizeof (List.eraseDupkeys xs) ≤ sizeof xs :=
  by 
    unfoldWf 
...
theorem mem_keys_kunion {a} {l₁ l₂ : List (Sigma β)} : a ∈ (kunion l₁ l₂).keys ↔ a ∈ l₁.keys ∨ a ∈ l₂.keys :=
  by 
    ...
    case list.cons s l₁ ih => 
      byCases' h : a = s.1 <;> [simp [h], simp [h, ih]]
...
| s :: _, l =>
  by 
    byCases' h : a = s.1 <;> simp [h, kerase_comm a s.1 l, kunion_kerase]

theorem kunion_nodupkeys {l₁ l₂ : List (Sigma β)} (nd₁ : l₁.nodupkeys) (nd₂ : l₂.nodupkeys) :
  (kunion l₁ l₂).Nodupkeys :=
  by 
    induction l₁ generalizing l₂ 
    case list.nil => 
...
@[simp]
theorem lookup_kunion_left {a} {l₁ l₂ : List (Sigma β)} (h : a ∈ l₁.keys) : lookup a (kunion l₁ l₂) = lookup a l₁ :=
  by 
...
@[simp]
theorem lookup_kunion_right {a} {l₁ l₂ : List (Sigma β)} (h : a ∉ l₁.keys) : lookup a (kunion l₁ l₂) = lookup a l₂ :=
  by 
    induction l₁ generalizing l₂ 
    case list.nil => 
      simp 
