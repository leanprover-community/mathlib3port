...
@[simp]
theorem countp_cons_of_pos {a : α} (l) (pa : p a) : countpₓ p (a :: l) = countpₓ p l + 1 :=
  if_pos pa

@[simp]
theorem countp_cons_of_neg {a : α} (l) (pa : ¬p a) : countpₓ p (a :: l) = countpₓ p l :=
  if_neg pa

theorem countp_cons (a : α) (l) : countpₓ p (a :: l) = countpₓ p l + ite (p a) 1 0 := by
  by_cases' h : p a <;> simp [h]

theorem length_eq_countp_add_countp (l) : length l = countpₓ p l + countpₓ (fun a => ¬p a) l := by
...
theorem countp_eq_length_filter (l) : countpₓ p l = length (filterₓ p l) := by
...
theorem countp_le_length : countpₓ p l ≤ l.length := by
...
theorem Sublist.countp_le (s : l₁ <+ l₂) : countpₓ p l₁ ≤ countpₓ p l₂ := by
...
@[simp]
theorem count_nil (a : α) : countₓ a [] = 0 :=
  rfl

theorem count_cons (a b : α) (l : List α) : countₓ a (b :: l) = if a = b then succ (countₓ a l) else countₓ a l :=
  rfl

theorem count_cons' (a b : α) (l : List α) : countₓ a (b :: l) = countₓ a l + if a = b then 1 else 0 := by
  rw [count_cons]
  split_ifs <;> rfl

@[simp]
theorem count_cons_self (a : α) (l : List α) : countₓ a (a :: l) = succ (countₓ a l) :=
  if_pos rfl

@[simp]
theorem count_cons_of_ne {a b : α} (h : a ≠ b) (l : List α) : countₓ a (b :: l) = countₓ a l :=
  if_neg h
...
theorem count_le_length (a : α) (l : List α) : countₓ a l ≤ l.length :=
  countp_le_length _

theorem Sublist.count_le (h : l₁ <+ l₂) (a : α) : countₓ a l₁ ≤ countₓ a l₂ :=
  h.countp_le _

theorem count_le_count_cons (a b : α) (l : List α) : countₓ a l ≤ countₓ a (b :: l) :=
  (sublist_cons _ _).count_le _

theorem count_singleton (a : α) : countₓ a [a] = 1 :=
  if_pos rfl

theorem count_singleton' (a b : α) : countₓ a [b] = ite (a = b) 1 0 :=
  rfl
...
theorem count_concat (a : α) (l : List α) : countₓ a (concat l a) = succ (countₓ a l) := by
  simp [-add_commₓ]

@[simp]
theorem count_pos {a : α} {l : List α} : 0 < countₓ a l ↔ a ∈ l := by
  simp only [count, countp_pos, exists_propₓ, exists_eq_right']

@[simp]
theorem one_le_count_iff_mem {a : α} {l : List α} : 1 ≤ countₓ a l ↔ a ∈ l :=
  count_pos

@[simp]
theorem count_eq_zero_of_not_mem {a : α} {l : List α} (h : a ∉ l) : countₓ a l = 0 :=
...
theorem count_eq_length {a : α} {l} : countₓ a l = l.length ↔ ∀ b ∈ l, a = b := by
...
theorem le_count_iff_repeat_sublist {a : α} {l : List α} {n : ℕ} : n ≤ countₓ a l ↔ repeat a n <+ l :=
  ⟨fun h =>
    ((repeat_sublist_repeat a).2 h).trans <| by
      have : filterₓ (Eq a) l = repeat a (countₓ a l) :=
        eq_repeat.2
          ⟨by
...
theorem repeat_count_eq_of_count_eq_length {a : α} {l : List α} (h : countₓ a l = length l) :
    repeat a (countₓ a l) = l :=
  eq_of_sublist_of_length_eq (le_count_iff_repeat_sublist.mp (le_reflₓ (countₓ a l)))
    (Eq.trans (length_repeat a (countₓ a l)) h)

@[simp]
theorem count_filter {p} [DecidablePred p] {a} {l : List α} (h : p a) : countₓ a (filterₓ p l) = countₓ a l := by
...
@[simp]
theorem count_erase_of_ne {a b : α} (ab : a ≠ b) : ∀ s : List α, countₓ a (List.eraseₓ s b) = countₓ a s
  | [] => by
    simp
  | x :: xs => by
    rw [erase_cons]
...
-- ./././Mathport/Syntax/Translate/Basic.lean:556:2: warning: expanding binder collection (a' «expr ≠ » a)
@[to_additive]
