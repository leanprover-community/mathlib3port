...
theorem Perm.filterMap (f : α → Option β) {l₁ l₂ : List α} (p : l₁ ~ l₂) :
  ...
  · simp
  · simp only [filterMap]
    cases' f x with a <;> simp [filterMap, IH, Perm.cons]
  · simp only [filterMap]
    cases' f x with a <;> cases' f y with b <;> simp [filterMap, swap]
  · exact IH₁.trans IH₂
...
theorem Perm.filter (p : α → Prop) [DecidablePred p] {l₁ l₂ : List α} (s : l₁ ~ l₂) :
    filter p l₁ ~ filter p l₂ := by rw [← filterMap_eq_filter] <;> apply s.filter_map _
...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (l₁' list.perm l₁) -/
...
theorem perm_replicate_append_replicate {l : List α} {a b : α} {m n : ℕ} (h : a ≠ b) :
  ...
  simp_rw [Ne.def, ← and_imp, ← not_or, Decidable.not_imp_not, subset_def, mem_cons, not_mem_nil,
...
theorem Perm.take_inter {α} [DecidableEq α] {xs ys : List α} (n : ℕ) (h : xs ~ ys) (h' : ys.Nodup) :
  ...
  case swap h_x h_y h_l n =>
    ...
    cases n <;>
      simp only [mem_cons, false_or_iff, true_or_iff, filter, *, Nat.zero_eq, if_true, not_mem_nil,
    ...
    · convert Perm.swap _ _ _
      ...
      · clear h₁
        ...
        · simp
        cases h_l <;>
          simp only [mem_cons, true_or_iff, eq_self_iff_true, filter_cons_of_pos, true_and_iff,
...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (ts' list.perm «expr[ ,]»([])) -/
...
/- ./././Mathport/Syntax/Translate/Basic.lean:629:2: warning: expanding binder collection (is' list.perm is) -/
...
theorem permutations_perm_permutations' (ts : List α) : ts.permutations ~ ts.permutations' :=
  ...
  simp only [permutations_append, foldr_permutationsAux2, permutationsAux_nil, permutationsAux_cons,
    append_nil]
  refine'
...
theorem count_permutations'Aux_self [DecidableEq α] (l : List α) (x : α) :
  ...
  · rw [permutations'Aux, count_cons_self]
    by_cases hx : x = y
    · subst hx
      simpa [takeWhile, Nat.succ_inj'] using IH _
    · rw [takeWhile]
      rw [if_neg hx]
      cases' permutations'Aux x l with a as
      · simp
...
theorem nodup_permutations'Aux_of_not_mem (s : List α) (x : α) (hx : x ∉ s) :
  ...
  · simp only [not_or, mem_cons] at hx
    simp only [not_and, exists_eq_right_right, mem_map', permutations'Aux, nodup_cons]
...
theorem nodup_permutations'Aux_iff {s : List α} {x : α} : Nodup (permutations'Aux x s) ↔ x ∉ s :=
  ...
  · rcases(Nat.succ_le_of_lt H).eq_or_lt with (rfl | H')
    · rw [nthLe_insertNth_self _ _ _ (Nat.succ_le_of_lt hk)]
      convert hk' using 1
      convert nthLe_insertNth_add_succ _ _ _ 0 _
      simpa using hk
    · obtain ⟨m, rfl⟩ := Nat.exists_eq_add_of_lt H'
      ...
      convert nthLe_insertNth_add_succ s x k m.succ _ using 2
...
theorem nodup_permutations (s : List α) (hs : Nodup s) : Nodup s.permutations :=
  ...
  · rw [permutations']
    ...
    · refine' IH.pairwise_of_forall_ne fun as ha bs hb H => _
      ...
      obtain ⟨n, hn, hn'⟩ := nthLe_of_mem ha'
      obtain ⟨m, hm, hm'⟩ := nthLe_of_mem hb'
      ...
      rw [nthLe_permutations'Aux] at hn' hm'
      have hx :
        nthLe (insertNth n x as) m (by rwa [length_insertNth _ _ hn, Nat.lt_succ_iff, hl]) = x := by
        simp [hn', ← hm', hm]
      have hx' :
        nthLe (insertNth m x bs) n (by rwa [length_insertNth _ _ hm, Nat.lt_succ_iff, ← hl]) = x :=
        by simp [hm', ← hn', hn]
