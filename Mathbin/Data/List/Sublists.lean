...
theorem sublists'_aux_eq_sublists' l f r : @sublists'_aux α β l f r = map f (sublists' l) ++ r :=
  by 
    rw [sublists', map_sublists'_aux, ←sublists'_aux_append] <;> rfl

@[simp]
theorem sublists'_cons (a : α) (l : List α) : sublists' (a :: l) = sublists' l ++ map (cons a) (sublists' l) :=
  by 
...
theorem sublists_aux_cons_eq_sublists_aux₁ (l : List α) : sublists_aux l cons = sublists_aux₁ l fun x => [x] :=
  by 
...
theorem sublists_aux_cons_cons (l : List α) (a : α) :
  sublists_aux (a :: l) cons = [a] :: foldr (fun ys r => ys :: (a :: ys) :: r) [] (sublists_aux l cons) :=
  by 
...
theorem sublists_aux₁_concat (l : List α) (a : α) (f : List α → List β) :
  sublists_aux₁ (l ++ [a]) f = sublists_aux₁ l f ++ f [a] ++ sublists_aux₁ l fun x => f (x ++ [a]) :=
  by 
...
theorem sublists_len_aux_zero {α : Type _} (l : List α) (f : List α → β) r : sublists_len_aux 0 l f r = f [] :: r :=
  by 
    cases l <;> rfl

@[simp]
theorem sublists_len_zero {α : Type _} (l : List α) : sublists_len 0 l = [[]] :=
  sublists_len_aux_zero _ _ _

@[simp]
theorem sublists_len_succ_nil {α : Type _} n : sublists_len (n+1) (@nil α) = [] :=
  rfl

@[simp]
theorem sublists_len_succ_cons {α : Type _} n (a : α) l :
  sublists_len (n+1) (a :: l) = sublists_len (n+1) l ++ (sublists_len n l).map (cons a) :=
  by 
...
theorem sublists_len_sublist_of_sublist {α : Type _} n {l₁ l₂ : List α} (h : l₁ <+ l₂) :
  sublists_len n l₁ <+ sublists_len n l₂ :=
  by 
...
theorem length_of_sublists_len {α : Type _} : ∀ {n} {l l' : List α}, l' ∈ sublists_len n l → length l' = n
...
theorem mem_sublists_len_self {α : Type _} {l l' : List α} (h : l' <+ l) : l' ∈ sublists_len (length l') l :=
  by 
    induction' h with l₁ l₂ a s IH l₁ l₂ a s IH
    ·
      exact Or.inl rfl
...
@[simp]
theorem mem_sublists_len {α : Type _} {n} {l l' : List α} : l' ∈ sublists_len n l ↔ l' <+ l ∧ length l' = n :=
  ⟨fun h => ⟨mem_sublists'.1 ((sublists_len_sublist_sublists' _ _).Subset h), length_of_sublists_len h⟩,
