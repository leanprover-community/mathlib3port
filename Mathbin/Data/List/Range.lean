...
@[simp]
theorem range'_eq_nil {s n : ℕ} : range' s n = [] ↔ n = 0 :=
  by 
...
theorem range'_sublist_right {s m n : ℕ} : range' s m <+ range' s n ↔ m ≤ n :=
  ⟨fun h =>
      by 
        simpa only [length_range'] using length_le_of_sublist h,
    fun h =>
      by 
...
| s, 0, n+1, _ => rfl
| s, m+1, n+1, h =>
  (nth_range' (s+1) (lt_of_add_lt_add_right h)).trans$
    by 
...
@[simp]
theorem length_range (n : ℕ) : length (range n) = n :=
  by 
    simp only [range_eq_range', length_range']

@[simp]
theorem range_eq_nil {n : ℕ} : range n = [] ↔ n = 0 :=
  by 
    rw [←length_eq_zero, length_range]
...
@[simp]
theorem length_iota (n : ℕ) : length (iota n) = n :=
  by 
...
| s, n+1 =>
  by 
    rw [range'_concat, reverse_append, range_succ_eq_map] <;>
      simpa only [show (s+n+1) - 1 = s+n from rfl, · ∘ ·, fun a i => show a - 1 - i = a - succ i from pred_sub _ _,
        reverse_singleton, map_cons, tsub_zero, cons_append, nil_append, eq_self_iff_true, true_andₓ, map_map] using
        reverse_range' s n
...
@[simp]
theorem length_fin_range (n : ℕ) : (fin_range n).length = n :=
  by 
...
@[toAdditive "A variant of `sum_range_succ` which pulls off the first term in the sum\n  rather than the last."]
theorem prod_range_succ' {α : Type u} [Monoidₓ α] (f : ℕ → α) (n : ℕ) :
...
@[simp]
theorem enum_map_fst (l : List α) : map Prod.fst (enum l) = range l.length :=
  by 
...
@[simp]
theorem unzip_enum_eq_prod (l : List α) : l.enum.unzip = (range l.length, l) :=
  by 
...
@[simp]
theorem unzip_enum_from_eq_prod (l : List α) {n : ℕ} : (l.enum_from n).unzip = (range' n l.length, l) :=
  by 
...
@[simp]
theorem nth_le_fin_range {n : ℕ} {i : ℕ} h : (fin_range n).nthLe i h = ⟨i, length_fin_range n ▸ h⟩ :=
  by 
...
theorem of_fn_eq_map {α n} {f : Finₓ n → α} : of_fn f = (fin_range n).map f :=
  by 
    rw [←of_fn_id, map_of_fn, Function.right_id]

theorem nodup_of_fn {α n} {f : Finₓ n → α} (hf : Function.Injective f) : nodup (of_fn f) :=
  by 
