...
theorem kronecker_map_add_left [Add α] [Add γ] (f : α → β → γ) (hf : ∀ a₁ a₂ b, f (a₁+a₂) b = f a₁ b+f a₂ b)
  (A₁ A₂ : Matrix l m α) (B : Matrix n p β) : kronecker_map f (A₁+A₂) B = kronecker_map f A₁ B+kronecker_map f A₂ B :=
  ext$ fun i j => hf _ _ _

theorem kronecker_map_add_right [Add β] [Add γ] (f : α → β → γ) (hf : ∀ a b₁ b₂, f a (b₁+b₂) = f a b₁+f a b₂)
  (A : Matrix l m α) (B₁ B₂ : Matrix n p β) : kronecker_map f A (B₁+B₂) = kronecker_map f A B₁+kronecker_map f A B₂ :=
  ext$ fun i j => hf _ _ _
...
theorem kronecker_map_diagonal_diagonal [HasZero α] [HasZero β] [HasZero γ] [DecidableEq m] [DecidableEq n]
  ...
  kronecker_map f (diagonal a) (diagonal b) = diagonal fun mn => f (a mn.1) (b mn.2) :=
  by 
    ext ⟨i₁, i₂⟩ ⟨j₁, j₂⟩
...
theorem kronecker_map_one_one [HasZero α] [HasZero β] [HasZero γ] [HasOne α] [HasOne β] [HasOne γ] [DecidableEq m]
  ...
  (kronecker_map_diagonal_diagonal _ hf₁ hf₂ _ _).trans$
    by 
...
def kronecker_map_bilinear [CommSemiringₓ R] [AddCommMonoidₓ α] [AddCommMonoidₓ β] [AddCommMonoidₓ γ] [Module R α]
  [Module R β] [Module R γ] (f : α →ₗ[R] β →ₗ[R] γ) : Matrix l m α →ₗ[R] Matrix n p β →ₗ[R] Matrix (l × n) (m × p) γ :=
...
theorem kronecker_map_bilinear_mul_mul [CommSemiringₓ R] [Fintype m] [Fintype m'] [NonUnitalNonAssocSemiring α]
  ...
  (f : α →ₗ[R] β →ₗ[R] γ) (h_comm : ∀ a b a' b', f (a*b) (a'*b') = f a a'*f b b') (A : Matrix l m α) (B : Matrix m n α)
  (A' : Matrix l' m' β) (B' : Matrix m' n' β) :
  kronecker_map_bilinear f (A ⬝ B) (A' ⬝ B') = kronecker_map_bilinear f A A' ⬝ kronecker_map_bilinear f B B' :=
  by 
    ext ⟨i, i'⟩ ⟨j, j'⟩
    simp only [kronecker_map_bilinear_apply_apply, mul_apply, ←Finset.univ_product_univ, Finset.sum_product,
      kronecker_map]
    simpRw [f.map_sum, LinearMap.sum_apply, LinearMap.map_sum, h_comm]
