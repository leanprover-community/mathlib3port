...
"top left" submatrix. -/
def to_blocks₁₁ (M : Matrix (Sum n o) (Sum l m) α) : Matrix n l α :=
  fun i j => M (Sum.inl i) (Sum.inl j)

/-- Given a matrix whose row and column indexes are sum types, we can extract the corresponding
"top right" submatrix. -/
def to_blocks₁₂ (M : Matrix (Sum n o) (Sum l m) α) : Matrix n m α :=
  fun i j => M (Sum.inl i) (Sum.inr j)

/-- Given a matrix whose row and column indexes are sum types, we can extract the corresponding
"bottom left" submatrix. -/
def to_blocks₂₁ (M : Matrix (Sum n o) (Sum l m) α) : Matrix o l α :=
  fun i j => M (Sum.inr i) (Sum.inl j)

/-- Given a matrix whose row and column indexes are sum types, we can extract the corresponding
"bottom right" submatrix. -/
def to_blocks₂₂ (M : Matrix (Sum n o) (Sum l m) α) : Matrix o m α :=
...
theorem from_blocks_transpose (A : Matrix n l α) (B : Matrix n m α) (C : Matrix o l α) (D : Matrix o m α) :
  (from_blocks A B C D)ᵀ = from_blocks (A)ᵀ (C)ᵀ (B)ᵀ (D)ᵀ :=
  by 
    ext i j 
    rcases i with ⟨⟩ <;> rcases j with ⟨⟩ <;> simp [from_blocks]

theorem from_blocks_conj_transpose [HasStar α] (A : Matrix n l α) (B : Matrix n m α) (C : Matrix o l α)
  (D : Matrix o m α) : (from_blocks A B C D)ᴴ = from_blocks (A)ᴴ (C)ᴴ (B)ᴴ (D)ᴴ :=
  by 
...
theorem from_blocks_add (A : Matrix n l α) (B : Matrix n m α) (C : Matrix o l α) (D : Matrix o m α) (A' : Matrix n l α)
  ...
  (from_blocks A B C D+from_blocks A' B' C' D') = from_blocks (A+A') (B+B') (C+C') (D+D') :=
  by 
    ext i j 
    rcases i with ⟨⟩ <;> rcases j with ⟨⟩ <;> rfl
...
theorem block_diagonal_apply ik jk : block_diagonal M ik jk = if ik.2 = jk.2 then M ik.2 ik.1 jk.1 else 0 :=
  by 
    cases ik 
    cases jk 
    rfl
...
@[simp]
theorem block_diagonal_transpose : (block_diagonal M)ᵀ = block_diagonal fun k => (M k)ᵀ :=
  by 
    ext 
...
theorem block_diagonal_conj_transpose {α : Type _} [Semiringₓ α] [StarRing α] (M : o → Matrix m n α) :
  (block_diagonal M)ᴴ = block_diagonal fun k => (M k)ᴴ :=
  by 
...
@[simp]
theorem block_diagonal_add [AddMonoidₓ α] : block_diagonal (M+N) = block_diagonal M+block_diagonal N :=
  by 
    ext 
...
@[simp]
theorem block_diagonal_neg [AddGroupₓ α] : block_diagonal (-M) = -block_diagonal M :=
  by 
    ext 
...
@[simp]
theorem block_diagonal_sub [AddGroupₓ α] : block_diagonal (M - N) = block_diagonal M - block_diagonal N :=
  by 
    simp [sub_eq_add_neg]

@[simp]
theorem block_diagonal_mul {p : Type _} [Fintype n] [Fintype o] [Semiringₓ α] (N : o → Matrix n p α) :
  (block_diagonal fun k => M k ⬝ N k) = block_diagonal M ⬝ block_diagonal N :=
  by 
    ext ⟨i, k⟩ ⟨j, k'⟩
...
theorem block_diagonal'_apply ik jk :
  block_diagonal' M ik jk = if h : ik.1 = jk.1 then M ik.1 ik.2 (cast (congr_argₓ n' h.symm) jk.2) else 0 :=
  by 
    cases ik 
    cases jk 
    rfl
...
@[simp]
theorem block_diagonal'_transpose : (block_diagonal' M)ᵀ = block_diagonal' fun k => (M k)ᵀ :=
  by 
    ext ⟨ii, ix⟩ ⟨ji, jx⟩
...
@[simp]
theorem block_diagonal'_add [AddMonoidₓ α] : block_diagonal' (M+N) = block_diagonal' M+block_diagonal' N :=
  by 
    ext 
...
@[simp]
theorem block_diagonal'_neg [AddGroupₓ α] : block_diagonal' (-M) = -block_diagonal' M :=
  by 
    ext 
...
@[simp]
theorem block_diagonal'_sub [AddGroupₓ α] : block_diagonal' (M - N) = block_diagonal' M - block_diagonal' N :=
  by 
    simp [sub_eq_add_neg]
