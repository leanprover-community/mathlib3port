...
@[simp]
theorem tail_val' (B : Matrix (Finₓ m.succ) n' α) (j : n') : (vec_tail fun i => B i j) = fun i => vec_tail B i j :=
  by 
    ext 
    simp [vec_tail]
...
@[simp]
theorem col_cons (x : α) (u : Finₓ m → α) : col (vec_cons x u) = vec_cons (fun _ => x) (col u) :=
  by 
    ext i j 
    refine' Finₓ.cases _ _ i <;> simp [vec_head, vec_tail]

@[simp]
theorem row_empty : row (vec_empty : Finₓ 0 → α) = fun _ => vec_empty :=
  by 
    ext 
    rfl

@[simp]
theorem row_cons (x : α) (u : Finₓ m → α) : row (vec_cons x u) = fun _ => vec_cons x u :=
  by 
    ext 
    rfl
...
@[simp]
theorem tail_transpose (A : Matrix m' (Finₓ n.succ) α) : vec_tail (A)ᵀ = (vec_tail ∘ A)ᵀ :=
  by 
    ext i j 
    rfl
...
theorem vec_mul_vec_cons (v : m' → α) (x : α) (w : Finₓ n → α) :
  vec_mul_vec v (vec_cons x w) = fun i => v i • vec_cons x w :=
  by 
    ext i j 
...
theorem minor_cons_row (A : Matrix m' n' α) (i : m') (row : Finₓ m → m') (col : o' → n') :
  minor A (vec_cons i row) col = vec_cons (fun j => A i (col j)) (minor A row col) :=
  by 
    ext i j 
