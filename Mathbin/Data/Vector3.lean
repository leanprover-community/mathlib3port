...
@[matchPattern]
def cons {α} {n} (a : α) (v : Vector3 α n) : Vector3 α (succ n) := fun i => by
  refine' i.cases' _ _
  exact a
  exact v

-- ././Mathport/Syntax/Translate/Basic.lean:1233:9: unsupported: advanced notation (l:(foldr `, ` (h t, vector3.cons h t) vector3.nil `]`))
...
def tail {α} {n} (v : Vector3 α (succ n)) : Vector3 α n := fun i => v (fs i)
...
def nil_elim {α} {C : Vector3 α 0 → Sort u} (H : C []) (v : Vector3 α 0) : C v := by
  rw [eq_nil v] <;> apply H

def cons_elim {α n} {C : Vector3 α (succ n) → Sort u} (H : ∀ a : α t : Vector3 α n, C (a :: t))
    (v : Vector3 α (succ n)) : C v := by
...
def insert {α} (a : α) {n} (v : Vector3 α n) (i : Fin2 (succ n)) : Vector3 α (succ n) := fun j =>
  (a :: v) (insert_perm i j)

@[simp]
theorem insert_fz {α} (a : α) {n} (v : Vector3 α n) : insert a v fz = a :: v := by
...
theorem append_insert {α} (a : α) {k} (t : Vector3 α k) {n} (v : Vector3 α n) (i : Fin2 (succ n))
    (e : (succ n+k) = succ (n+k)) : insert a (t +-+ v) (Eq.recOnₓ e (i.add k)) = Eq.recOnₓ e (t +-+ insert a v i) := by
...
theorem vector_allp_iff_forall {α} (p : α → Prop) {n} (v : Vector3 α n) : VectorAllp p v ↔ ∀ i, p (v i) := by
  refine' v.rec_on _ _
  ·
