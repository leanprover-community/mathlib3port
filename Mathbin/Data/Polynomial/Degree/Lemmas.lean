...
theorem nat_degree_C_mul_eq_of_mul_eq_one {ai : R} (au : (ai*a) = 1) : (C a*p).natDegree = p.nat_degree :=
  le_antisymmₓ (nat_degree_C_mul_le a p)
    (calc p.nat_degree = (1*p).natDegree :=
      by 
        nthRw 0[←one_mulₓ p]
      _ = (C ai*C a*p).natDegree :=
      by 
...
theorem nat_degree_mul_C_eq_of_mul_eq_one {ai : R} (au : (a*ai) = 1) : (p*C a).natDegree = p.nat_degree :=
  le_antisymmₓ (nat_degree_mul_C_le p a)
    (calc p.nat_degree = (p*1).natDegree :=
      by 
        nthRw 0[←mul_oneₓ p]
      _ = ((p*C a)*C ai).natDegree :=
      by 
        rw [←C_1, ←au, RingHom.map_mul, ←mul_assocₓ]
...
hurdle.
-/
theorem nat_degree_mul_C_eq_of_mul_ne_zero (h : (p.leading_coeff*a) ≠ 0) : (p*C a).natDegree = p.nat_degree :=
  by 
...
hurdle.
-/
theorem nat_degree_C_mul_eq_of_mul_ne_zero (h : (a*p.leading_coeff) ≠ 0) : (C a*p).natDegree = p.nat_degree :=
  by 
...
theorem nat_degree_C_mul_eq_of_no_zero_divisors (a0 : a ≠ 0) : (C a*p).natDegree = p.nat_degree :=
  by 
    byCases' p0 : p = 0
    ·
      rw [p0, mul_zero]
