          ...
          Finset.sup_le fun n hn =>
            calc
              ...
              _ = (n * natDegree q : ℕ) := by
                rw [natDegree_c, WithBot.coe_zero, zero_add, ← WithBot.coe_nsmul, nsmul_eq_mul] <;>
                  simp
...
theorem coe_lt_degree {p : R[X]} {n : ℕ} : (n : WithBot ℕ) < degree p ↔ n < natDegree p :=
  ...
  by_cases h : p = 0
  · simp [h]
  rw [degree_eq_natDegree h, WithBot.coe_lt_coe]
...
theorem natDegree_comp : natDegree (p.comp q) = natDegree p * natDegree q :=
  ...
  ·
    rw [degree_le_zero_iff.mp (nat_degree_eq_zero_iff_degree_le_zero.mp q0), comp_c, natDegree_c,
