...
instance trailing_monic.decidable [DecidableEq R] : Decidable (trailing_monic p) :=
  by 
...
theorem trailing_degree_eq_top : trailing_degree p = ⊤ ↔ p = 0 :=
  ⟨fun h =>
      by 
        rw [trailing_degree, ←min_eq_inf_with_top] at h <;> exact support_eq_empty.1 (min_eq_none.1 h),
    fun h =>
      by 
        simp [h]⟩
...
theorem nat_trailing_degree_eq_of_trailing_degree_eq [Semiringₓ S] {q : Polynomial S}
  (h : trailing_degree p = trailing_degree q) : nat_trailing_degree p = nat_trailing_degree q :=
  by 
...
theorem nat_trailing_degree_le_of_ne_zero (h : coeff p n ≠ 0) : nat_trailing_degree p ≤ n :=
  by 
...
theorem nat_trailing_degree_le_nat_trailing_degree {hq : q ≠ 0} (hpq : p.trailing_degree ≤ q.trailing_degree) :
  ...
  by 
    ...
    ·
      ...
      exact zero_le _ 
    rwa [trailing_degree_eq_nat_trailing_degree hp, trailing_degree_eq_nat_trailing_degree hq, WithTop.coe_le_coe] at
      hpq

@[simp]
theorem trailing_degree_monomial (ha : a ≠ 0) : trailing_degree (monomial n a) = n :=
  by 
    rw [trailing_degree, support_monomial _ _ ha, inf_singleton, WithTop.some_eq_coe]

theorem nat_trailing_degree_monomial (ha : a ≠ 0) : nat_trailing_degree (monomial n a) = n :=
  by 
...
theorem trailing_degree_one_le : (0 : WithTop ℕ) ≤ trailing_degree (1 : Polynomial R) :=
  by 
...
@[simp]
theorem nat_trailing_degree_nat_cast (n : ℕ) : nat_trailing_degree (n : Polynomial R) = 0 :=
  by 
    simp only [←C_eq_nat_cast, nat_trailing_degree_C]

@[simp]
theorem trailing_degree_C_mul_X_pow (n : ℕ) (ha : a ≠ 0) : trailing_degree (C a*X ^ n) = n :=
  by 
    rw [C_mul_X_pow_eq_monomial, trailing_degree_monomial ha]

theorem le_trailing_degree_C_mul_X_pow (n : ℕ) (a : R) : (n : WithTop ℕ) ≤ trailing_degree (C a*X ^ n) :=
  by 
...
theorem le_trailing_degree_X_pow (n : ℕ) : (n : WithTop ℕ) ≤ trailing_degree (X ^ n : Polynomial R) :=
  by 
...
theorem trailing_coeff_eq_zero : trailing_coeff p = 0 ↔ p = 0 :=
  ⟨fun h =>
      by_contradiction$
        fun hp => mt mem_support_iff.1 (not_not.2 h) (mem_of_min (trailing_degree_eq_nat_trailing_degree hp)),
...
theorem nat_trailing_degree_le_nat_degree (p : Polynomial R) : p.nat_trailing_degree ≤ p.nat_degree :=
  by 
    byCases' hp : p = 0
    ·
...
@[simp]
theorem nat_trailing_degree_neg (p : Polynomial R) : nat_trailing_degree (-p) = nat_trailing_degree p :=
  by 
    simp [nat_trailing_degree]

@[simp]
theorem nat_trailing_degree_int_cast (n : ℤ) : nat_trailing_degree (n : Polynomial R) = 0 :=
  by 
...
@[simp]
theorem next_coeff_up_C_eq_zero (c : R) : next_coeff_up (C c) = 0 :=
  by 
    rw [next_coeff_up]
    simp 
...
theorem ne_zero_of_trailing_degree_lt {n : WithTop ℕ} (h : trailing_degree p < n) : p ≠ 0 :=
  fun h₀ =>
    h.not_le
      (by 
        simp [h₀])
