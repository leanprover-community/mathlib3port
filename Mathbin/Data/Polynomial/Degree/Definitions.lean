...
theorem degree_lt_wf : WellFounded fun p q : R[X] => degree p < degree q :=
  InvImage.wf degree (WithBot.well_founded_lt Nat.lt_wf)
...
@[nontriviality]
theorem degree_of_subsingleton [Subsingleton R] : degree p = ⊥ := by rw [Subsingleton.elim p 0, degree_zero]
...
theorem nat_degree_eq_of_degree_eq [Semiring S] {q : S[X]} (h : degree p = degree q) : natDegree p = natDegree q := by
...
theorem nat_degree_eq_of_le_of_coeff_ne_zero (pn : p.natDegree ≤ n) (p1 : p.coeff n ≠ 0) : p.natDegree = n :=
...
theorem degree_mono [Semiring S] {f : R[X]} {g : S[X]} (h : f.support ⊆ g.support) : f.degree ≤ g.degree :=
  Finset.sup_mono h
...
theorem nat_degree_le_nat_degree [Semiring S] {q : S[X]} (hpq : p.degree ≤ q.degree) : p.natDegree ≤ q.natDegree :=
...
@[simp]
theorem nat_degree_nat_cast (n : ℕ) : natDegree (n : R[X]) = 0 := by simp only [← C_eq_nat_cast, nat_degree_C]
  ...
  ext fun n =>
    Nat.casesOn n (by simp) fun n =>
      Nat.casesOn n (by simp [coeff_C]) fun m => by
        have : degree p < m.succ.succ := lt_of_le_of_lt h dec_trivial
...
@[simp]
theorem nat_degree_int_cast (n : ℤ) : natDegree (n : R[X]) = 0 := by rw [← C_eq_int_cast, nat_degree_C]
...
theorem ne_zero_of_degree_ge_degree (hpq : p.degree ≤ q.degree) (hp : p ≠ 0) : q ≠ 0 :=
  Polynomial.ne_zero_of_degree_gt
    (lt_of_lt_of_le (bot_lt_iff_ne_bot.mpr (by rwa [Ne.def, Polynomial.degree_eq_bot])) hpq : q.degree > ⊥)
...
@[simp]
theorem leading_coeff_X : leadingCoeff (x : R[X]) = 1 := by simpa only [pow_one] using @leading_coeff_X_pow R _ 1
...
theorem Monic.ne_zero {R : Type _} [Semiring R] [Nontrivial R] {p : R[X]} (hp : p.Monic) : p ≠ 0 := by
  rintro rfl
  ...
  calc
    ...
    _ = coeff p (natDegree p) * coeff q (natDegree q) := by
      ...
      · rintro ⟨i, j⟩ h₁ h₂
        ...
        · rw [coeff_eq_zero_of_degree_lt (lt_of_le_of_lt degree_le_nat_degree (WithBot.coe_lt_coe.2 H)), zero_mul]
...
theorem Monic.degree_mul (hq : Monic q) : degree (p * q) = degree p + degree q :=
  if hp : p = 0 then by simp [hp] else degree_mul' $ by rwa [hq.leading_coeff, mul_one, Ne.def, leading_coeff_eq_zero]
...
theorem leading_coeff_mul_monic {p q : R[X]} (hq : Monic q) : leadingCoeff (p * q) = leadingCoeff p :=
  Decidable.byCases (fun H : leadingCoeff p = 0 => by rw [H, leading_coeff_eq_zero.1 H, zero_mul, leading_coeff_zero])
...
theorem zero_le_degree_iff {p : R[X]} : 0 ≤ degree p ↔ p ≠ 0 := by
  rw [Ne.def, ← degree_eq_bot] <;> cases degree p <;> exact dec_trivial
#align polynomial.zero_le_degree_iff Polynomial.zero_le_degree_iff

theorem degree_nonneg_iff_ne_zero : 0 ≤ degree p ↔ p ≠ 0 := by simp [degree_eq_bot, ← not_lt]
#align polynomial.degree_nonneg_iff_ne_zero Polynomial.degree_nonneg_iff_ne_zero
...
theorem ne_zero_of_coe_le_degree (hdeg : ↑n ≤ p.degree) : p ≠ 0 :=
  degree_nonneg_iff_ne_zero.mp $ (WithBot.coe_le_coe.mpr n.zero_le).trans hdeg
...
theorem X_pow_add_C_ne_zero {n : ℕ} (hn : 0 < n) (a : R) : (x : R[X]) ^ n + c a ≠ 0 :=
  mt degree_eq_bot.2 (show degree ((x : R[X]) ^ n + c a) ≠ ⊥ by rw [degree_X_pow_add_C hn a] <;> exact dec_trivial)
