...
@[nontriviality]
theorem nat_degree_of_subsingleton [Subsingleton R] : nat_degree p = 0 := by
  rw [Subsingleton.elimₓ p 0, nat_degree_zero]

theorem degree_eq_nat_degree (hp : p ≠ 0) : degree p = (nat_degree p : WithBot ℕ) := by
...
theorem le_nat_degree_of_ne_zero (h : coeff p n ≠ 0) : n ≤ nat_degree p := by
...
theorem degree_mono [Semiringₓ S] {f : Polynomial R} {g : Polynomial S} (h : f.support ⊆ g.support) :
    f.degree ≤ g.degree :=
  Finset.sup_mono h
...
theorem degree_le_degree (h : coeff q (nat_degree p) ≠ 0) : degree p ≤ degree q := by
  by_cases' hp : p = 0
  · rw [hp]
    exact bot_le
...
@[simp]
theorem nat_degree_C (a : R) : nat_degree (C a) = 0 := by
  by_cases' ha : a = 0
  · have : C a = 0 := by
      rw [ha, C_0]
...
@[simp]
theorem nat_degree_nat_cast (n : ℕ) : nat_degree (n : Polynomial R) = 0 := by
...
@[simp]
theorem nat_degree_C_mul_X (a : R) (ha : a ≠ 0) : nat_degree (C a * X) = 1 := by
  simpa only [pow_oneₓ] using nat_degree_C_mul_X_pow 1 a ha

@[simp]
theorem nat_degree_monomial [DecidableEq R] (i : ℕ) (r : R) : nat_degree (monomial i r) = if r = 0 then 0 else i := by
  split_ifs with hr
  · simp [hr]
...
theorem coeff_eq_zero_of_nat_degree_lt {p : Polynomial R} {n : ℕ} (h : p.nat_degree < n) : p.coeff n = 0 := by
...
theorem ite_le_nat_degree_coeff (p : Polynomial R) (n : ℕ) (I : Decidable (n < 1 + nat_degree p)) :
    @ite _ (n < 1 + nat_degree p) I (coeff p n) 0 = coeff p n := by
  split_ifs
  · rfl
...
theorem sum_over_range' [AddCommMonoidₓ S] (p : Polynomial R) {f : ℕ → R → S} (h : ∀ n, f n 0 = 0) (n : ℕ)
    (w : p.nat_degree < n) : p.sum f = ∑ a : ℕ in range n, f a (coeff p a) := by
  rcases p with ⟨⟩
...
theorem card_supp_le_succ_nat_degree (p : Polynomial R) : p.support.card ≤ p.nat_degree + 1 := by
...
@[simp]
theorem degree_neg (p : Polynomial R) : degree (-p) = degree p := by
  unfold degree <;> rw [support_neg]

@[simp]
theorem nat_degree_neg (p : Polynomial R) : nat_degree (-p) = nat_degree p := by
  simp [nat_degree]

@[simp]
theorem nat_degree_int_cast (n : ℤ) : nat_degree (n : Polynomial R) = 0 := by
...
@[simp]
theorem next_coeff_C_eq_zero (c : R) : next_coeff (C c) = 0 := by
  rw [next_coeff]
  simp

theorem next_coeff_of_pos_nat_degree (p : Polynomial R) (hp : 0 < p.nat_degree) :
    next_coeff p = p.coeff (p.nat_degree - 1) := by
  rw [next_coeff, if_neg]
  contrapose! hp
  simpa

variable {p q : Polynomial R} {ι : Type _}
...
theorem ne_zero_of_nat_degree_gt {n : ℕ} (h : n < nat_degree p) : p ≠ 0 := fun H => by
  simpa [H, Nat.not_lt_zeroₓ] using h

theorem degree_lt_degree (h : nat_degree p < nat_degree q) : degree p < degree q := by
  by_cases' hp : p = 0
  · simp [hp]
...
theorem nat_degree_add_le (p q : Polynomial R) : nat_degree (p + q) ≤ max (nat_degree p) (nat_degree q) := by
...
theorem leading_coeff_eq_zero_iff_deg_eq_bot : leading_coeff p = 0 ↔ degree p = ⊥ := by
  rw [leading_coeff_eq_zero, degree_eq_bot]

theorem nat_degree_mem_support_of_nonzero (H : p ≠ 0) : p.nat_degree ∈ p.support := by
  rw [mem_support_iff]
...
theorem nat_degree_add_eq_left_of_nat_degree_lt (h : nat_degree q < nat_degree p) : nat_degree (p + q) = nat_degree p :=
  nat_degree_eq_of_degree_eq (degree_add_eq_left_of_degree_lt (degree_lt_degree h))

theorem nat_degree_add_eq_right_of_nat_degree_lt (h : nat_degree p < nat_degree q) :
...
theorem degree_erase_le (p : Polynomial R) (n : ℕ) : degree (p.erase n) ≤ degree p := by
  rcases p with ⟨⟩
  simp only [erase, degree, coeff, support]
  convert sup_mono (erase_subset _ _)

theorem degree_erase_lt (hp : p ≠ 0) : degree (p.erase (nat_degree p)) < degree p := by
...
theorem degree_update_le (p : Polynomial R) (n : ℕ) (a : R) : degree (p.update n a) ≤ max (degree p) n := by
...
@[simp]
theorem leading_coeff_monomial (a : R) (n : ℕ) : leading_coeff (monomial n a) = a := by
  by_cases' ha : a = 0
...
theorem leading_coeff_C_mul_X_pow (a : R) (n : ℕ) : leading_coeff (C a * X ^ n) = a := by
...
@[simp]
theorem leading_coeff_X_pow (n : ℕ) : leading_coeff ((X : Polynomial R) ^ n) = 1 := by
  simpa only [C_1, one_mulₓ] using leading_coeff_C_mul_X_pow (1 : R) n

@[simp]
theorem leading_coeff_X : leading_coeff (X : Polynomial R) = 1 := by
...
theorem monic.ne_zero {R : Type _} [Semiringₓ R] [Nontrivial R] {p : Polynomial R} (hp : p.monic) : p ≠ 0 := by
  rintro rfl
  simpa [monic] using hp

theorem monic.ne_zero_of_ne (h : (0 : R) ≠ 1) {p : Polynomial R} (hp : p.monic) : p ≠ 0 := by
  nontriviality R
  exact hp.ne_zero
...
theorem leading_coeff_add_of_degree_lt (h : degree p < degree q) : leading_coeff (p + q) = leading_coeff q := by
...
theorem leading_coeff_add_of_degree_eq (h : degree p = degree q) (hlc : leading_coeff p + leading_coeff q ≠ 0) :
  ...
  have : nat_degree (p + q) = nat_degree p := by
...
theorem coeff_mul_degree_add_degree (p q : Polynomial R) :
  ...
  calc
    coeff (p * q) (nat_degree p + nat_degree q) =
      ...
      coeff_mul _ _ _
    _ = coeff p (nat_degree p) * coeff q (nat_degree q) := by
...
theorem monomial_nat_degree_leading_coeff_eq_self (h : p.support.card ≤ 1) :
    monomial p.nat_degree p.leading_coeff = p := by
...
theorem leading_coeff_monic_mul {p q : Polynomial R} (hp : monic p) : leading_coeff (p * q) = leading_coeff q := by
...
theorem nat_degree_mul_le {p q : Polynomial R} : nat_degree (p * q) ≤ nat_degree p + nat_degree q := by
  apply nat_degree_le_of_degree_le
  apply le_transₓ (degree_mul_le p q)
  rw [WithBot.coe_add]
  refine' add_le_add _ _ <;> apply degree_le_nat_degree

theorem nat_degree_pow_le {p : Polynomial R} {n : ℕ} : (p ^ n).natDegree ≤ n * p.nat_degree := by
...
theorem zero_le_degree_iff {p : Polynomial R} : 0 ≤ degree p ↔ p ≠ 0 := by
...
theorem degree_smul_le (a : R) (p : Polynomial R) : degree (a • p) ≤ degree p := by
...
theorem degree_sub_lt (hd : degree p = degree q) (hp0 : p ≠ 0) (hlc : leading_coeff p = leading_coeff q) :
  ...
  have hd' : nat_degree p = nat_degree q := by
    unfold nat_degree <;> rw [hd]
  have hq0 : q ≠ 0 := mt degree_eq_bot.2 (hd ▸ mt degree_eq_bot.1 hp0)
  calc
    degree (p - q) = degree (erase (nat_degree q) p + -erase (nat_degree q) q) := by
...
theorem degree_X_pow_add_C {n : ℕ} (hn : 0 < n) (a : R) : degree ((X : Polynomial R) ^ n + C a) = n := by
  have : degree (C a) < degree ((X : Polynomial R) ^ n) :=
    calc
      degree (C a) ≤ 0 := degree_C_le
      _ < degree ((X : Polynomial R) ^ n) := by
...
theorem X_add_C_ne_zero (r : R) : X + C r ≠ 0 :=
  pow_oneₓ (X : Polynomial R) ▸ X_pow_add_C_ne_zero zero_lt_one r
...
theorem leading_coeff_X_add_C [Semiringₓ S] (r : S) : (X + C r).leadingCoeff = 1 := by
  nontriviality
  rw [← pow_oneₓ (X : Polynomial S), leading_coeff_X_pow_add_C zero_lt_one]
  infer_instance
...
theorem X_pow_sub_C_ne_zero {n : ℕ} (hn : 0 < n) (a : R) : (X : Polynomial R) ^ n - C a ≠ 0 := by
...
@[simp]
theorem leading_coeff_mul (p q : Polynomial R) : leading_coeff (p * q) = leading_coeff p * leading_coeff q := by
  by_cases' hp : p = 0
...
@[simp]
theorem leading_coeff_C_mul_X_add_C [Nontrivial R] (a b : R) (ha : a ≠ 0) : leading_coeff (C a * X + C b) = a := by
...
def leading_coeff_hom : Polynomial R →* R where
  toFun := leading_coeff
  map_one' := by
    simp
