...
variable {Fq : Type _} [Field Fq] [Fintypeₓ Fq]
...
noncomputable def cardPowDegree : AbsoluteValue Fq[X] ℤ :=
  have card_pos : 0 < Fintypeₓ.card Fq := Fintypeₓ.card_pos_iff.mpr inferInstance
  have pow_pos : ∀ n, 0 < (Fintypeₓ.card Fq : ℤ) ^ n := fun n => pow_pos (Int.coe_nat_pos.mpr card_pos) n
  { toFun := fun p => if p = 0 then 0 else Fintypeₓ.card Fq ^ p.natDegree,
    nonneg' := fun p => by
      dsimp
      split_ifs
      · rfl
    ...
    add_le' := fun p q => by
      ...
      refine' le_transₓ (pow_le_pow (by linarith) (Polynomial.nat_degree_add_le _ _)) _
      refine'
        le_transₓ (le_max_iff.mpr _) (max_le_add_of_nonneg (pow_nonneg (by linarith) _) (pow_nonneg (by linarith) _))
...
theorem card_pow_degree_apply (p : Fq[X]) : cardPowDegree p = if p = 0 then 0 else Fintypeₓ.card Fq ^ natDegree p :=
  rfl
...
@[simp]
theorem card_pow_degree_nonzero (p : Fq[X]) (hp : p ≠ 0) : cardPowDegree p = Fintypeₓ.card Fq ^ p.natDegree :=
  if_neg hp
...
theorem card_pow_degree_is_euclidean : IsEuclidean (cardPowDegree : AbsoluteValue Fq[X] ℤ) :=
  ...
  { map_lt_map_iff' := fun p q => by
      ...
      · simp only [hp, hq, lt_self_iff_falseₓ]
      ...
      · rw [degree_eq_nat_degree hp, degree_eq_nat_degree hq, WithBot.coe_lt_coe, pow_lt_pow_iff]
        exact_mod_cast @Fintypeₓ.one_lt_card Fq _ _
         }
