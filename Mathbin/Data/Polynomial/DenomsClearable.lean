...
-/
def DenomsClearable (a b : R) (N : ℕ) (f : Polynomial R) (i : R →+* K) : Prop :=
...
theorem denoms_clearable_zero (N : ℕ) (a : R) (bu : (bi*i b) = 1) : DenomsClearable a b N 0 i :=
  ⟨0, bi, bu,
    by 
      simp only [eval_zero, RingHom.map_zero, mul_zero, map_zero]⟩
...
theorem denoms_clearable_nat_degree (i : R →+* K) (f : Polynomial R) (a : R) (bu : (bi*i b) = 1) :
  DenomsClearable a b f.nat_degree f i :=
...
theorem one_le_pow_mul_abs_eval_div {K : Type _} [LinearOrderedField K] {f : Polynomial ℤ} {a b : ℤ} (b0 : 0 < b)
  ...
  (1 : K) ≤ (b ^ f.nat_degree)*|eval ((a : K) / b) (f.map (algebraMap ℤ K))| :=
  by 
    obtain ⟨ev, bi, bu, hF⟩ :=
