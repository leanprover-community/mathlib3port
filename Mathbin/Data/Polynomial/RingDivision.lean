...
instance :
    NoZeroDivisors
      R[X] where eq_zero_or_eq_zero_of_mul_eq_zero a b h := by
...
theorem nth_roots_two_eq_zero_iff {r : R} : nthRoots 2 r = 0 ↔ ¬IsSquare r := by
  simp_rw [is_square_iff_exists_sq, eq_zero_iff_forall_not_mem, mem_nth_roots (by norm_num : 0 < 2),
    ← not_exists, eq_comm]
        ...
        (Term.explicitBinder
         ...
         [":"
          (Term.forall
           ...
           []
           ","
           («term_=_» (Term.app (Term.proj `p "." `eval) [`x]) "=" (num "0")))]
         []
         ")")]
