...
#align polynomial.degree_mod_by_monic_lt Polynomial.degree_mod_by_monic_lt
...
#align polynomial.mod_by_monic_zero Polynomial.mod_by_monic_zero
...
#align polynomial.div_by_monic_zero Polynomial.div_by_monic_zero

theorem div_by_monic_eq_of_not_monic (p : R[X]) (hq : ¬Monic q) : p /ₘ q = 0 :=
  dif_neg hq
#align polynomial.div_by_monic_eq_of_not_monic Polynomial.div_by_monic_eq_of_not_monic

theorem mod_by_monic_eq_of_not_monic (p : R[X]) (hq : ¬Monic q) : p %ₘ q = p :=
  dif_neg hq
#align polynomial.mod_by_monic_eq_of_not_monic Polynomial.mod_by_monic_eq_of_not_monic
...
#align polynomial.mod_by_monic_add_div Polynomial.mod_by_monic_add_div

theorem div_by_monic_eq_zero_iff [Nontrivial R] (hq : Monic q) : p /ₘ q = 0 ↔ degree p < degree q :=
  ⟨fun h => by have := mod_by_monic_add_div p hq <;> rwa [h, mul_zero, add_zero, mod_by_monic_eq_self_iff hq] at this,
    fun h => by
    have : ¬degree q ≤ degree p := not_le_of_gt h
...
#align polynomial.degree_add_div_by_monic Polynomial.degree_add_div_by_monic
...
#align polynomial.degree_div_by_monic_le Polynomial.degree_div_by_monic_le
...
#align polynomial.nat_degree_div_by_monic Polynomial.nat_degree_div_by_monic
...
theorem map_div_by_monic [CommRing S] (f : R →+* S) (hq : Monic q) : (p /ₘ q).map f = p.map f /ₘ q.map f :=
  (map_mod_div_by_monic f hq).1
#align polynomial.map_div_by_monic Polynomial.map_div_by_monic

theorem map_mod_by_monic [CommRing S] (f : R →+* S) (hq : Monic q) : (p %ₘ q).map f = p.map f %ₘ q.map f :=
  (map_mod_div_by_monic f hq).2
#align polynomial.map_mod_by_monic Polynomial.map_mod_by_monic

theorem dvd_iff_mod_by_monic_eq_zero (hq : Monic q) : p %ₘ q = 0 ↔ q ∣ p :=
  ⟨fun h => by rw [← mod_by_monic_add_div p hq, h, zero_add] <;> exact dvd_mul_right _ _, fun h => by
    nontriviality R
...
#align polynomial.dvd_iff_mod_by_monic_eq_zero Polynomial.dvd_iff_mod_by_monic_eq_zero
...
#align polynomial.mod_by_monic_one Polynomial.mod_by_monic_one

@[simp]
theorem div_by_monic_one (p : R[X]) : p /ₘ 1 = p := by conv_rhs => rw [← mod_by_monic_add_div p monic_one] <;> simp
#align polynomial.div_by_monic_one Polynomial.div_by_monic_one
...
theorem mul_div_by_monic_eq_iff_is_root : (X - c a) * (p /ₘ (X - c a)) = p ↔ IsRoot p a :=
  ⟨fun h => by rw [← h, is_root.def, eval_mul, eval_sub, eval_X, eval_C, sub_self, zero_mul], fun h : p.eval a = 0 => by
    conv =>
      rhs
      rw [← mod_by_monic_add_div p (monic_X_sub_C a)] <;> rw [mod_by_monic_X_sub_C_eq_C_eval, h, C_0, zero_add]⟩
...
theorem dvd_iff_is_root : X - c a ∣ p ↔ IsRoot p a :=
  ⟨fun h => by
    ...
    ⟨p /ₘ (X - c a), by rw [mul_div_by_monic_eq_iff_is_root.2 h]⟩⟩
#align polynomial.dvd_iff_is_root Polynomial.dvd_iff_is_root

theorem mod_by_monic_X (p : R[X]) : p %ₘ X = c (p.eval 0) := by rw [← mod_by_monic_X_sub_C_eq_C_eval, C_0, sub_zero]
#align polynomial.mod_by_monic_X Polynomial.mod_by_monic_X

theorem eval₂_mod_by_monic_eq_self_of_root [CommRing S] {f : R →+* S} {p q : R[X]} (hq : q.Monic) {x : S}
    (hx : q.eval₂ f x = 0) : (p %ₘ q).eval₂ f x = p.eval₂ f x := by
  rw [mod_by_monic_eq_sub_mul_div p hq, eval₂_sub, eval₂_mul, hx, zero_mul, sub_zero]
#align polynomial.eval₂_mod_by_monic_eq_self_of_root Polynomial.eval₂_mod_by_monic_eq_self_of_root
...
#align polynomial.sum_mod_by_monic_coeff Polynomial.sum_mod_by_monic_coeff
...
#align polynomial.root_multiplicity_eq_multiplicity Polynomial.root_multiplicity_eq_multiplicity
...
theorem root_multiplicity_eq_zero {p : R[X]} {x : R} (h : ¬IsRoot p x) : rootMultiplicity x p = 0 := by
  ...
  rw [← PartEnat.coe_inj, PartEnat.coe_get, multiplicity.multiplicity_eq_zero_of_not_dvd, Nat.cast_zero]
  intro hdvd
...
#align polynomial.pow_root_multiplicity_dvd Polynomial.pow_root_multiplicity_dvd
