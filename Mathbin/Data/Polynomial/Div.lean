...
theorem x_dvd_iff {α : Type u} [CommSemiring α] {f : α[X]} : x ∣ f ↔ f.coeff 0 = 0 :=
  ⟨fun ⟨g, hfg⟩ => by rw [hfg, mul_comm, coeff_mul_x_zero], fun hf =>
    ⟨f.divX, by rw [mul_comm, ← add_zero (f.div_X * x), ← c_0, ← hf, divX_mul_x_add]⟩⟩
...
theorem multiplicity_finite_of_degree_pos_of_monic (hp : (0 : WithBot ℕ) < degree p) (hmp : Monic p)
  ...
  ⟨natDegree q, fun ⟨r, hr⟩ =>
    ...
    have hnp : 0 < natDegree p := by
      rw [← WithBot.coe_lt_coe, ← degree_eq_natDegree hp0] <;> exact hp
...
theorem div_wf_lemma (h : degree q ≤ degree p ∧ p ≠ 0) (hq : Monic q) :
  ...
  degree_sub_lt
    (by
      rw [hq.degree_mul, degree_c_mul_x_pow _ hp, degree_eq_natDegree h.2, degree_eq_natDegree hq0,
...
theorem degree_add_divByMonic (hq : Monic q) (h : degree q ≤ degree p) :
  ...
  by
  nontriviality R
  have hdiv0 : p /ₘ q ≠ 0 := by rwa [(· ≠ ·), divByMonic_eq_zero_iff hq, not_lt]
  have hlc : leadingCoeff q * leadingCoeff (p /ₘ q) ≠ 0 := by
    rwa [Monic.def.1 hq, one_mul, (· ≠ ·), leadingCoeff_eq_zero]
  ...
  if hpq : degree p < degree q then
    ...
    exact
      WithBot.coe_lt_coe.2
        (Nat.lt_add_of_pos_left (WithBot.coe_lt_coe.1 <| degree_eq_natDegree hq.ne_zero ▸ h0q))
...
theorem dvd_iff_modByMonic_eq_zero (hq : Monic q) : p %ₘ q = 0 ↔ q ∣ p :=
  ⟨fun h => by rw [← modByMonic_add_div p hq, h, zero_add] <;> exact dvd_mul_right _ _, fun h =>
    ...
    have hrpq0 : leadingCoeff (r - p /ₘ q) ≠ 0 := fun h =>
      hpq0 <|
        leadingCoeff_eq_zero.1 (by rw [hmod, leadingCoeff_eq_zero.1 h, mul_zero, leadingCoeff_zero])
...
theorem modByMonic_x_sub_c_eq_c_eval (p : R[X]) (a : R) : p %ₘ (x - c a) = c (p.eval a) :=
  ...
  have h : (p %ₘ (x - c a)).eval a = p.eval a := by
    rw [modByMonic_eq_sub_mul_div _ (monic_x_sub_c a), eval_sub, eval_mul, eval_sub, eval_x, eval_c,
...
theorem mul_divByMonic_eq_iff_isRoot : (x - c a) * (p /ₘ (x - c a)) = p ↔ IsRoot p a :=
  ⟨fun h => by rw [← h, IsRoot.def, eval_mul, eval_sub, eval_x, eval_c, sub_self, zero_mul],
    fun h : p.eval a = 0 => by
    conv =>
        rhs
