...
theorem coeff_derivative (p : Polynomial R) (n : ℕ) : coeff (derivative p) n = coeff p (n+1)*n+1 := by
  rw [derivative_apply]
...
@[simp]
theorem iterate_derivative_zero {k : ℕ} : (derivative^[k]) (0 : Polynomial R) = 0 := by
...
@[simp]
theorem derivative_X_pow (n : ℕ) : derivative (X ^ n : Polynomial R) = (n : Polynomial R)*X ^ (n - 1) := by
...
@[simp]
theorem derivative_bit0 {a : Polynomial R} : derivative (bit0 a) = bit0 (derivative a) := by
  simp [bit0]

@[simp]
theorem derivative_bit1 {a : Polynomial R} : derivative (bit1 a) = bit0 (derivative a) := by
  simp [bit1]
   ...
   (Term.typeSpec
    ...
    («term_=_»
     ...
     (Algebra.BigOperators.Basic.«term∑_in_,_»
      ...
      `s
      ", "
      (Term.app `derivative [(Term.app `f [`b])])))))
  (Command.declValSimple ":=" (Term.proj `derivative "." `map_sum) [])
  []
  []))
...
@[ simp ]
  theorem
    derivative_sum
    { s : Finset ι } { f : ι → Polynomial R } : derivative ∑ b in s , f b = ∑ b in s , derivative f b
    := derivative . map_sum
...
theorem iterate_derivative_smul (r : R) (p : Polynomial R) (k : ℕ) :
    (derivative^[k]) (r • p) = r • (derivative^[k]) p := by
...
@[simp]
theorem derivative_C_mul (a : R) (p : Polynomial R) : derivative (C a*p) = C a*derivative p := by
...
theorem derivative_eval (p : Polynomial R) (x : R) : p.derivative.eval x = p.sum fun n a => (a*n)*x ^ (n - 1) := by
...
theorem derivative_mul {f g : Polynomial R} : derivative (f*g) = (derivative f*g)+f*derivative g :=
  calc derivative (f*g) = f.sum fun n a => g.sum fun m b => C ((a*b)*(n+m : ℕ))*X ^ ((n+m) - 1) := by
    rw [mul_eq_sum_sum]
    trans
...
theorem derivative_comp (p q : Polynomial R) : (p.comp q).derivative = q.derivative*p.derivative.comp q := by
...
theorem derivative_comp_one_sub_X (p : Polynomial R) : (p.comp (1 - X)).derivative = -p.derivative.comp (1 - X) := by
...
theorem degree_derivative_eq [CharZero R] (p : Polynomial R) (hp : 0 < nat_degree p) :
    degree (derivative p) = (nat_degree p - 1 : ℕ) := by
  have h0 : p ≠ 0 := by
    contrapose! hp
    simp [hp]
  apply le_antisymmₓ
