...
@[simp]
theorem coeff_add (p q : Polynomial R) (n : ℕ) : coeff (p + q) n = coeff p n + coeff q n := by
  rcases p with ⟨⟩
  rcases q with ⟨⟩
...
theorem support_smul [Monoidₓ S] [DistribMulAction S R] (r : S) (p : Polynomial R) : support (r • p) ⊆ support p := by
  intro i hi
...
def lcoeff (n : ℕ) : Polynomial R →ₗ[R] R where
...
@[simp]
theorem mul_coeff_zero (p q : Polynomial R) : coeff (p * q) 0 = coeff p 0 * coeff q 0 := by
  simp [coeff_mul]

theorem coeff_mul_X_zero (p : Polynomial R) : coeff (p * X) 0 = 0 := by
  simp

theorem coeff_X_mul_zero (p : Polynomial R) : coeff (X * p) 0 = 0 := by
  simp

theorem coeff_C_mul_X (x : R) (k n : ℕ) : coeff (C x * X ^ k : Polynomial R) n = if n = k then x else 0 := by
  rw [← monomial_eq_C_mul_X, coeff_monomial]
  congr 1
  simp [eq_comm]

@[simp]
theorem coeff_C_mul (p : Polynomial R) : coeff (C a * p) n = a * coeff p n := by
  rcases p with ⟨⟩
  simp only [C, monomial, monomial_fun, mul_to_finsupp, RingHom.coe_mk, coeff,
    AddMonoidAlgebra.single_zero_mul_apply p a n]

theorem C_mul' (a : R) (f : Polynomial R) : C a * f = a • f := by
  ext
  rw [coeff_C_mul, coeff_smul, smul_eq_mul]

@[simp]
theorem coeff_mul_C (p : Polynomial R) (n : ℕ) (a : R) : coeff (p * C a) n = coeff p n * a := by
  rcases p with ⟨⟩
  simp only [C, monomial, monomial_fun, mul_to_finsupp, RingHom.coe_mk, coeff,
...
@[simp]
theorem coeff_X_pow_self (n : ℕ) : coeff (X ^ n : Polynomial R) n = 1 := by
  simp [coeff_X_pow]

@[simp]
theorem coeff_mul_X_pow (p : Polynomial R) (n d : ℕ) : coeff (p * Polynomial.x ^ n) (d + n) = coeff p d := by
...
theorem coeff_mul_X_pow' (p : Polynomial R) (n d : ℕ) : (p * X ^ n).coeff d = ite (n ≤ d) (p.coeff (d - n)) 0 := by
  split_ifs
...
theorem coeff_X_add_C_pow (r : R) (n k : ℕ) : ((X + C r) ^ n).coeff k = r ^ (n - k) * (n.choose k : R) := by
  rw [(commute_X (C r : Polynomial R)).add_pow, ← lcoeff_apply, LinearMap.map_sum]
...
theorem C_dvd_iff_dvd_coeff (r : R) (φ : Polynomial R) : C r ∣ φ ↔ ∀ i, r ∣ φ.coeff i := by
  constructor
  · rintro ⟨φ, rfl⟩ c
    rw [coeff_C_mul]
    apply dvd_mul_right
...
@[simp]
theorem nat_cast_coeff_zero {n : ℕ} {R : Type _} [Semiringₓ R] : (n : Polynomial R).coeff 0 = n := by
...
@[simp, norm_cast]
theorem nat_cast_inj {m n : ℕ} {R : Type _} [Semiringₓ R] [CharZero R] : (↑m : Polynomial R) = ↑n ↔ m = n := by
  fconstructor
  · intro h
...
@[simp]
theorem int_cast_coeff_zero {i : ℤ} {R : Type _} [Ringₓ R] : (i : Polynomial R).coeff 0 = i := by
  cases i <;> simp

@[simp, norm_cast]
theorem int_cast_inj {m n : ℤ} {R : Type _} [Ringₓ R] [CharZero R] : (↑m : Polynomial R) = ↑n ↔ m = n := by
  fconstructor
  · intro h
    apply_fun fun p => p.coeff 0  at h
    simpa using h
