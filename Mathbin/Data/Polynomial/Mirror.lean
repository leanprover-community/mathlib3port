...
theorem mirror_natTrailingDegree : p.mirror.natTrailingDegree = p.natTrailingDegree :=
  ...
  · rw [hp, mirror_zero]
  ·
    rw [mirror, natTrailingDegree_mul_x_pow ((mt reverse_eq_zero.mp) hp), reverse_natTrailingDegree,
      zero_add]
...
theorem mirror_eval_one : p.mirror.eval 1 = p.eval 1 :=
  ...
  · exact fun n hn hp => revAt (p.nat_degree + p.nat_trailing_degree) n
  · intro n hn hp
  ...
  · exact fun n₁ n₂ hn₁ hp₁ hn₂ hp₂ h => by rw [← @rev_at_invol _ n₁, h, revAt_invol]
  · intro n hn hp
    use revAt (p.nat_degree + p.nat_trailing_degree) n
