...
theorem rev_at_fun_invol {N i : ℕ} : rev_at_fun N (rev_at_fun N i) = i := by
  unfold rev_at_fun
  split_ifs with h j
...
theorem rev_at_fun_inj {N : ℕ} : Function.Injective (rev_at_fun N) := by
  intro a b hab
...
theorem rev_at_add {N O n o : ℕ} (hn : n ≤ N) (ho : o ≤ O) : rev_at (N + O) (n + o) = rev_at N n + rev_at O o := by
...
theorem reflect_support (N : ℕ) (f : Polynomial R) : (reflect N f).Support = image (rev_at N) f.support := by
  rcases f with ⟨⟩
  ext1
  rw [reflect, mem_image, support, support, support_emb_domain, mem_map]

@[simp]
theorem coeff_reflect (N : ℕ) (f : Polynomial R) (i : ℕ) : coeff (reflect N f) i = f.coeff (rev_at N i) := by
  rcases f with ⟨⟩
...
@[simp]
theorem reflect_eq_zero_iff {N : ℕ} {f : Polynomial R} : reflect N (f : Polynomial R) = 0 ↔ f = 0 := by
  rcases f with ⟨⟩
  simp [reflect, ← zero_to_finsupp]

@[simp]
theorem reflect_add (f g : Polynomial R) (N : ℕ) : reflect N (f + g) = reflect N f + reflect N g := by
  ext
  simp only [coeff_add, coeff_reflect]

@[simp]
theorem reflect_C_mul (f : Polynomial R) (r : R) (N : ℕ) : reflect N (C r * f) = C r * reflect N f := by
  ext
  simp only [coeff_reflect, coeff_C_mul]

@[simp]
theorem reflect_C_mul_X_pow (N n : ℕ) {c : R} : reflect N (C c * X ^ n) = C c * X ^ rev_at N n := by
  ext
...
@[simp]
theorem reflect_monomial (N n : ℕ) : reflect N ((X : Polynomial R) ^ n) = X ^ rev_at N n := by
...
theorem coeff_reverse (f : Polynomial R) (n : ℕ) : f.reverse.coeff n = f.coeff (rev_at f.nat_degree n) := by
  rw [reverse, coeff_reflect]

@[simp]
theorem coeff_zero_reverse (f : Polynomial R) : coeff (reverse f) 0 = leading_coeff f := by
...
theorem reverse_nat_degree_le (f : Polynomial R) : f.reverse.nat_degree ≤ f.nat_degree := by
...
theorem nat_degree_eq_reverse_nat_degree_add_nat_trailing_degree (f : Polynomial R) :
    f.nat_degree = f.reverse.nat_degree + f.nat_trailing_degree := by
  by_cases' hf : f = 0
...
theorem reverse_nat_trailing_degree (f : Polynomial R) : f.reverse.nat_trailing_degree = 0 := by
  by_cases' hf : f = 0
...
theorem trailing_coeff_mul {R : Type _} [Ringₓ R] [IsDomain R] (p q : Polynomial R) :
    (p * q).trailingCoeff = p.trailing_coeff * q.trailing_coeff := by
  rw [← reverse_leading_coeff, reverse_mul_of_domain, leading_coeff_mul, reverse_leading_coeff, reverse_leading_coeff]

@[simp]
theorem coeff_one_reverse (f : Polynomial R) : coeff (reverse f) 1 = next_coeff f := by
