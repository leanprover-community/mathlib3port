...
theorem natDegree_hasseDeriv_le (p : R[X]) (n : ℕ) : natDegree (hasseDeriv n p) ≤ natDegree p - n :=
  ...
  classical
    ...
    · simp only [if_t_t, max_eq_right, zero_le', Finset.fold_max_le, true_and_iff, and_imp,
      ...
      intro x hx hx'
      have hxp : x ≤ p.nat_degree := le_natDegree_of_ne_zero hx
      have hxn : n ≤ x := by
        contrapose! hx'
...
theorem hasseDeriv_mul (f g : R[X]) :
  ...
  show
    (compHom (D k)).comp Φ f g =
      ∑ ij : ℕ × ℕ in antidiagonal k, ((comp_hom.comp ((compHom Φ) (D ij.1))).flip (D ij.2) f) g
  simp only [← finset_sum_apply]
  congr 2
  clear f g
  ext (m r n s) : 4
  simp only [finset_sum_apply, coe_mul_left, coe_comp, flip_apply, comp_apply, hasseDeriv_monomial,
