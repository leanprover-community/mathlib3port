...
theorem taylor_zero' : taylor (0 : R) = LinearMap.id := by
  ext
  simp only [taylor_apply, add_zero, comp_X, _root_.map_zero, LinearMap.id_comp, Function.comp_app, LinearMap.coe_comp]
...
theorem eq_zero_of_hasse_deriv_eq_zero {R} [CommRing R] (f : R[X]) (r : R) (h : ∀ k, (hasseDeriv k f).eval r = 0) :
    f = 0 := by
...
/-- Taylor's formula. -/
theorem sum_taylor_eq {R} [CommRing R] (f : R[X]) (r : R) : ((taylor r f).Sum fun i a => c a * (X - c r) ^ i) = f := by
