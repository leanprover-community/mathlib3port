...
theorem Monic.map [Semiring S] (f : R →+* S) (hp : Monic p) : Monic (p.map f) :=
  by
  nontriviality
  have : f (leadingCoeff p) ≠ 0 :=
    by
    rw [show _ = _ from hp, f.map_one]
    exact one_ne_zero
  rw [Monic, leadingCoeff, coeff_map]
...
theorem monic_c_mul_of_mul_leadingCoeff_eq_one {b : R} (hp : b * p.leadingCoeff = 1) :
    Monic (c b * p) := by
  nontriviality
  rw [Monic, leadingCoeff_mul' _] <;> simp [leadingCoeff_c b, hp]
...
theorem monic_of_degree_le (n : ℕ) (H1 : degree p ≤ n) (H2 : coeff p n = 1) : Monic p :=
  Decidable.byCases
    ...
    fun H : ¬degree p < n => by
    rwa [Monic, leadingCoeff, natDegree, (lt_or_eq_of_le H1).resolve_left H]
...
theorem degree_le_zero_iff_eq_one (hp : p.Monic) : p.degree ≤ 0 ↔ p = 1 := by
  rw [← hp.nat_degree_eq_zero_iff_eq_one, natDegree_eq_zero_iff_degree_le_zero]
...
theorem degree_mul_comm (hp : p.Monic) (q : R[X]) : (p * q).degree = (q * p).degree :=
  ...
  · exact add_comm _ _
  · rwa [hp.leading_coeff, one_mul, leadingCoeff_ne_zero]
...
theorem natDegree_mul' (hp : p.Monic) (hq : q ≠ 0) :
  ...
  simpa [hp.leading_coeff, leadingCoeff_ne_zero]
#align polynomial.monic.nat_degree_mul' Polynomial.Monic.natDegree_mul'
...
theorem eq_one_of_map_eq_one {S : Type _} [Semiring S] [Nontrivial S] (f : R →+* S) (hp : p.Monic)
  ...
  have hndeg : p.nat_degree = 0 :=
    with_bot.coe_eq_coe.mp ((degree_eq_natDegree hp.ne_zero).symm.trans hdeg)
...
theorem monic_of_injective {p : R[X]} (hp : (p.map f).Monic) : p.Monic :=
  by
  apply hf
  rw [← leadingCoeff_of_injective hf, hp.leading_coeff, f.map_one]
...
theorem not_isUnit_x_pow_sub_one (R : Type _) [CommRing R] [Nontrivial R] (n : ℕ) :
  ...
  · simpa using h
  apply hn
  rw [← @nat_degree_one R, ← (monic_x_pow_sub_c _ hn).eq_one_of_isUnit h, natDegree_x_pow_sub_c]
...
theorem Monic.sub_of_right {p q : R[X]} (hq : q.leadingCoeff = -1) (hpq : degree p < degree q) :
  ...
  have : (-q).coeff (-q).natDegree = 1 := by
    rw [natDegree_neg, coeff_neg, show q.coeff q.nat_degree = -1 from hq, neg_neg]
  rw [sub_eq_add_neg]
...
theorem natDegree_smul_of_smul_regular {S : Type _} [Monoid S] [DistribMulAction S R] {k : S}
  ...
  by_cases hp : p = 0
  · simp [hp]
  rw [← WithBot.coe_eq_coe, ← degree_eq_natDegree hp, ← degree_eq_natDegree,
...
theorem leadingCoeff_smul_of_smul_regular {S : Type _} [Monoid S] [DistribMulAction S R] {k : S}
  ...
  rw [leadingCoeff, leadingCoeff, coeff_smul, natDegree_smul_of_smul_regular p h]
