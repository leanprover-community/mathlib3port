  ...
  { C.comp (algebraMap R A) with
    smul_def' :=
      fun r p =>
        by 
          rcases p with ⟨⟩
          simp only [C, monomial, monomial_fun, RingHom.coe_mk, RingHom.to_fun_eq_coe, Function.comp_app,
            RingHom.coe_comp, smul_to_finsupp, mul_to_finsupp]
    ...
    commutes' :=
      fun r p =>
        by 
          rcases p with ⟨⟩
          simp only [C, monomial, monomial_fun, RingHom.coe_mk, RingHom.to_fun_eq_coe, Function.comp_app,
            RingHom.coe_comp, mul_to_finsupp]
...
theorem alg_hom_eval₂_algebra_map {R A B : Type _} [CommRingₓ R] [Ringₓ A] [Ringₓ B] [Algebra R A] [Algebra R B]
  (p : Polynomial R) (f : A →ₐ[R] B) (a : A) : f (eval₂ (algebraMap R A) a p) = eval₂ (algebraMap R B) (f a) p :=
  by 
    dsimp [eval₂, Sum]
...
theorem eval₂_algebra_map_X {R A : Type _} [CommRingₓ R] [Ringₓ A] [Algebra R A] (p : Polynomial R)
  (f : Polynomial R →ₐ[R] A) : eval₂ (algebraMap R A) (f X) p = f p :=
  by 
    convRHS => rw [←Polynomial.sum_C_mul_X_eq p]
    dsimp [eval₂, Sum]
...
@[simp]
theorem adjoin_X : Algebra.adjoin R ({X} : Set (Polynomial R)) = ⊤ :=
  by 
...
theorem aeval_map {A : Type _} [CommSemiringₓ A] [Algebra R A] [Algebra A B] [IsScalarTower R A B] (b : B)
  (p : Polynomial R) : aeval b (p.map (algebraMap R A)) = aeval b p :=
  by 
...
theorem eval_unique (φ : Polynomial R →ₐ[R] A) p : φ p = eval₂ (algebraMap R A) (φ X) p :=
  by 
...
theorem coeff_zero_eq_aeval_zero (p : Polynomial R) : p.coeff 0 = aeval 0 p :=
  by 
    simp [coeff_zero_eq_eval_zero]

theorem coeff_zero_eq_aeval_zero' (p : Polynomial R) : algebraMap R A (p.coeff 0) = aeval (0 : A) p :=
  by 
    simp [aeval_def]

variable (R)

theorem _root_.algebra.adjoin_singleton_eq_range_aeval (x : A) : Algebra.adjoin R {x} = (Polynomial.aeval x).range :=
  by 
...
theorem aeval_sum {ι : Type _} [Algebra R S] (s : Finset ι) (f : ι → Polynomial R) (g : S) :
  aeval g (∑ i in s, f i) = ∑ i in s, aeval g (f i) :=
  (Polynomial.aeval g : Polynomial R →ₐ[_] _).map_sum f s

@[toAdditive]
theorem aeval_prod {ι : Type _} [Algebra R S] (s : Finset ι) (f : ι → Polynomial R) (g : S) :
  aeval g (∏ i in s, f i) = ∏ i in s, aeval g (f i) :=
...
@[simp]
theorem aeval_tower_id : aeval_tower (AlgHom.id S S) = aeval :=
  by 
    ext 
    simp only [eval_X, aeval_tower_X, coe_aeval_eq_eval]

@[simp]
theorem aeval_tower_of_id : aeval_tower (Algebra.ofId S A') = aeval :=
  by 
    ext 
...
theorem aeval_endomorphism {M : Type _} [CommRingₓ R] [AddCommGroupₓ M] [Module R M] (f : M →ₗ[R] M) (v : M)
  (p : Polynomial R) : aeval f p v = p.sum fun n b => b • (f ^ n) v :=
  by 
    rw [aeval_def, eval₂]
