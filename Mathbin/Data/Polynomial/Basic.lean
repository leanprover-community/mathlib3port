...
@[simps]
def to_finsupp_iso : Polynomial R ≃+* AddMonoidAlgebra R ℕ where
  toFun := fun p => p.to_finsupp
  invFun := fun p => ⟨p⟩
  left_inv := fun ⟨p⟩ => rfl
  right_inv := fun p => rfl
  map_mul' := by
...
def monomial (n : ℕ) : R →ₗ[R] Polynomial R where
...
theorem add_submonoid_closure_set_of_eq_monomial :
    AddSubmonoid.closure { p : Polynomial R | ∃ n a, p = monomial n a } = ⊤ := by
  apply top_unique
...
theorem sum_eq_of_subset {S : Type _} [AddCommMonoidₓ S] (p : Polynomial R) (f : ℕ → R → S) (hf : ∀ i, f i 0 = 0)
    (s : Finset ℕ) (hs : p.support ⊆ s) : p.sum f = ∑ n in s, f n (p.coeff n) := by
