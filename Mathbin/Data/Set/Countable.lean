...
theorem subset_range_enumerate {s : Set α} (h : s.Countable) (default : α) :
  ...
  ⟨@Encodable.encode s h.toEncodable ⟨x, hx⟩, by simp [enumerateCountable, Encodable.encodek]⟩
...
theorem Countable.unionₛ_iff {s : Set (Set α)} (hs : s.Countable) :
    (⋃₀ s).Countable ↔ ∀ a ∈ s, (a : _).Countable := by rw [unionₛ_eq_bunionᵢ, hs.bUnion_iff]
...
theorem countable_setOf_finite_subset {s : Set α} :
  ...
  | ⟨h⟩ => by
    ...
    rintro t ⟨⟨ht⟩, ts⟩; skip
    refine' ⟨finset.univ.map (embeddingOfSubset _ _ ts), Set.ext fun a => _⟩
    simpa using @ts a
