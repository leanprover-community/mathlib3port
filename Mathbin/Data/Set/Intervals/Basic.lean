...
/-- Left-open right-open interval -/ def Ioo ( a b : α ) := { x | a < x ∧ x < b }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-closed right-open interval -/ def Ico ( a b : α ) := { x | a ≤ x ∧ x < b }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-infinite right-open interval -/ def Iio ( a : α ) := { x | x < a }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-closed right-closed interval -/ def Icc ( a b : α ) := { x | a ≤ x ∧ x ≤ b }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-infinite right-closed interval -/ def Iic ( b : α ) := { x | x ≤ b }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-open right-closed interval -/ def Ioc ( a b : α ) := { x | a < x ∧ x ≤ b }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-closed right-infinite interval -/ def Ici ( a : α ) := { x | a ≤ x }

-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
/-- Left-open right-infinite interval -/ def Ioi ( a : α ) := { x | a < x }
...
@[simp]
theorem left_mem_Ioo : a ∈ Ioo a b ↔ False :=
  by 
    simp [lt_irreflₓ]
...
@[simp]
theorem left_mem_Ioc : a ∈ Ioc a b ↔ False :=
  by 
    simp [lt_irreflₓ]

theorem left_mem_Ici : a ∈ Ici a :=
  by 
    simp 

@[simp]
theorem right_mem_Ioo : b ∈ Ioo a b ↔ False :=
  by 
    simp [lt_irreflₓ]

@[simp]
theorem right_mem_Ico : b ∈ Ico a b ↔ False :=
  by 
    simp [lt_irreflₓ]
...
theorem Ioo_subset_Ioo (h₁ : a₂ ≤ a₁) (h₂ : b₁ ≤ b₂) : Ioo a₁ b₁ ⊆ Ioo a₂ b₂ :=
  fun x ⟨hx₁, hx₂⟩ => ⟨h₁.trans_lt hx₁, hx₂.trans_le h₂⟩
...
theorem Ico_subset_Ico (h₁ : a₂ ≤ a₁) (h₂ : b₁ ≤ b₂) : Ico a₁ b₁ ⊆ Ico a₂ b₂ :=
  fun x ⟨hx₁, hx₂⟩ => ⟨h₁.trans hx₁, hx₂.trans_le h₂⟩
...
theorem Icc_subset_Icc (h₁ : a₂ ≤ a₁) (h₂ : b₁ ≤ b₂) : Icc a₁ b₁ ⊆ Icc a₂ b₂ :=
  fun x ⟨hx₁, hx₂⟩ => ⟨h₁.trans hx₁, le_transₓ hx₂ h₂⟩
...
theorem Ioc_subset_Ioc (h₁ : a₂ ≤ a₁) (h₂ : b₁ ≤ b₂) : Ioc a₁ b₁ ⊆ Ioc a₂ b₂ :=
  fun x ⟨hx₁, hx₂⟩ => ⟨h₁.trans_lt hx₁, hx₂.trans h₂⟩
...
theorem Ioi_subset_Ici_self : Ioi a ⊆ Ici a :=
  fun x hx => le_of_ltₓ hx

theorem Iio_subset_Iic_self : Iio a ⊆ Iic a :=
  fun x hx => le_of_ltₓ hx
...
theorem Ioi_subset_Ioi (h : a ≤ b) : Ioi b ⊆ Ioi a :=
  fun x hx => h.trans_lt hx
...
theorem Iio_subset_Iio (h : a ≤ b) : Iio a ⊆ Iio b :=
  fun x hx => lt_of_lt_of_leₓ hx h
...
theorem Icc_eq_empty_iff : Icc a b = ∅ ↔ ¬a ≤ b :=
  by 
    rw [←not_nonempty_iff_eq_empty, not_iff_not, nonempty_Icc]

theorem Ico_eq_empty_iff : Ico a b = ∅ ↔ ¬a < b :=
  by 
    rw [←not_nonempty_iff_eq_empty, not_iff_not, nonempty_Ico]

theorem Ioc_eq_empty_iff : Ioc a b = ∅ ↔ ¬a < b :=
  by 
    rw [←not_nonempty_iff_eq_empty, not_iff_not, nonempty_Ioc]

theorem Ioo_eq_empty_iff [DenselyOrdered α] : Ioo a b = ∅ ↔ ¬a < b :=
  by 
...
@[simp]
theorem Icc_diff_both : Icc a b \ {a, b} = Ioo a b :=
  by 
...
@[simp]
theorem Ici_diff_Ioi_same : Ici a \ Ioi a = {a} :=
  by 
    rw [←Ici_diff_left, diff_diff_cancel_left (singleton_subset_iff.2 left_mem_Ici)]

@[simp]
theorem Iic_diff_Iio_same : Iic a \ Iio a = {a} :=
  by 
...
theorem mem_Icc_Ico_Ioc_Ioo_of_subset_of_subset {s : Set α} (ho : Ioo a b ⊆ s) (hc : s ⊆ Icc a b) :
  ...
  by 
    ...
    ·
      ...
      applyRules [subset_diff_singleton]

theorem mem_Ioo_or_eq_endpoints_of_mem_Icc {x : α} (hmem : x ∈ Icc a b) : x = a ∨ x = b ∨ x ∈ Ioo a b :=
  by 
...
theorem mem_Ioo_or_eq_left_of_mem_Ico {x : α} (hmem : x ∈ Ico a b) : x = a ∨ x ∈ Ioo a b :=
  by 
...
@[simp]
theorem Icc_top : Icc a ⊤ = Ici a :=
  by 
    simp [←Ici_inter_Iic]

@[simp]
theorem Ioc_top : Ioc a ⊤ = Ioi a :=
  by 
    simp [←Ioi_inter_Iic]
...
@[simp]
theorem Icc_bot : Icc ⊥ a = Iic a :=
  by 
    simp [←Ici_inter_Iic]

@[simp]
theorem Ico_bot : Ico ⊥ a = Iio a :=
  by 
    simp [←Ici_inter_Iio]
...
@[simp]
theorem Ici_diff_Ici : Ici a \ Ici b = Ico a b :=
  by 
    rw [diff_eq, compl_Ici, Ici_inter_Iio]

@[simp]
theorem Ici_diff_Ioi : Ici a \ Ioi b = Icc a b :=
  by 
    rw [diff_eq, compl_Ioi, Ici_inter_Iic]

@[simp]
theorem Ioi_diff_Ioi : Ioi a \ Ioi b = Ioc a b :=
  by 
    rw [diff_eq, compl_Ioi, Ioi_inter_Iic]

@[simp]
theorem Ioi_diff_Ici : Ioi a \ Ici b = Ioo a b :=
  by 
    rw [diff_eq, compl_Ici, Ioi_inter_Iio]

@[simp]
theorem Iic_diff_Iic : Iic b \ Iic a = Ioc a b :=
  by 
...
@[simp]
theorem Iic_diff_Iio : Iic b \ Iio a = Icc a b :=
  by 
    rw [diff_eq, compl_Iio, inter_comm, Ici_inter_Iic]

@[simp]
theorem Iio_diff_Iio : Iio b \ Iio a = Ico a b :=
  by 
...
theorem Ioi_subset_Ioo_union_Ici : Ioi a ⊆ Ioo a b ∪ Ici b :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx, hxb⟩) fun hxb => Or.inr hxb
...
theorem Ici_subset_Ico_union_Ici : Ici a ⊆ Ico a b ∪ Ici b :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx, hxb⟩) fun hxb => Or.inr hxb
...
theorem Ioi_subset_Ioc_union_Ioi : Ioi a ⊆ Ioc a b ∪ Ioi b :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx, hxb⟩) fun hxb => Or.inr hxb
...
theorem Ici_subset_Icc_union_Ioi : Ici a ⊆ Icc a b ∪ Ioi b :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx, hxb⟩) fun hxb => Or.inr hxb
...
theorem Iic_subset_Iio_union_Icc : Iic b ⊆ Iio a ∪ Icc a b :=
  fun x hx => (lt_or_leₓ x a).elim (fun hxa => Or.inl hxa) fun hxa => Or.inr ⟨hxa, hx⟩
...
theorem Iio_subset_Iio_union_Ico : Iio b ⊆ Iio a ∪ Ico a b :=
  fun x hx => (lt_or_leₓ x a).elim (fun hxa => Or.inl hxa) fun hxa => Or.inr ⟨hxa, hx⟩
...
theorem Iic_subset_Iic_union_Ioc : Iic b ⊆ Iic a ∪ Ioc a b :=
  fun x hx => (le_or_ltₓ x a).elim (fun hxa => Or.inl hxa) fun hxa => Or.inr ⟨hxa, hx⟩
...
theorem Iio_subset_Iic_union_Ioo : Iio b ⊆ Iic a ∪ Ioo a b :=
  fun x hx => (le_or_ltₓ x a).elim (fun hxa => Or.inl hxa) fun hxa => Or.inr ⟨hxa, hx⟩
...
theorem Ioo_subset_Ioo_union_Ico : Ioo a c ⊆ Ioo a b ∪ Ico b c :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Ico_subset_Ico_union_Ico : Ico a c ⊆ Ico a b ∪ Ico b c :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Icc_subset_Ico_union_Icc : Icc a c ⊆ Ico a b ∪ Icc b c :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Ioc_subset_Ioo_union_Icc : Ioc a c ⊆ Ioo a b ∪ Icc b c :=
  fun x hx => (lt_or_leₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Ioo_subset_Ioc_union_Ioo : Ioo a c ⊆ Ioc a b ∪ Ioo b c :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Ico_subset_Icc_union_Ioo : Ico a c ⊆ Icc a b ∪ Ioo b c :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Icc_subset_Icc_union_Ioc : Icc a c ⊆ Icc a b ∪ Ioc b c :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
theorem Ioc_subset_Ioc_union_Ioc : Ioc a c ⊆ Ioc a b ∪ Ioc b c :=
  fun x hx => (le_or_ltₓ x b).elim (fun hxb => Or.inl ⟨hx.1, hxb⟩) fun hxb => Or.inr ⟨hxb, hx.2⟩
...
@[simp]
theorem Iio_inter_Iio [IsTotal α (· ≤ ·)] {a b : α} : Iio a ∩ Iio b = Iio (a⊓b) :=
  by 
    ext x 
    simp [Iio]

@[simp]
theorem Ioc_inter_Iic (a b c : α) : Ioc a b ∩ Iic c = Ioc a (b⊓c) :=
  by 
...
@[simp]
theorem Ico_inter_Ici (a b c : α) : Ico a b ∩ Ici c = Ico (a⊔c) b :=
  by 
    rw [←Ici_inter_Iio, ←Ici_inter_Iio, ←Ici_inter_Ici, inter_right_comm]

@[simp]
theorem Ioi_inter_Ioi [IsTotal α (· ≤ ·)] {a b : α} : Ioi a ∩ Ioi b = Ioi (a⊔b) :=
  by 
    ext x 
    simp [Ioi]

@[simp]
theorem Ioc_inter_Ioi [IsTotal α (· ≤ ·)] {a b c : α} : Ioc a b ∩ Ioi c = Ioc (a⊔c) b :=
  by 
...
theorem Icc_inter_Icc : Icc a₁ b₁ ∩ Icc a₂ b₂ = Icc (a₁⊔a₂) (b₁⊓b₂) :=
  by 
    simp only [Ici_inter_Iic.symm, Ici_inter_Ici.symm, Iic_inter_Iic.symm] <;> acRfl

@[simp]
theorem Icc_inter_Icc_eq_singleton (hab : a ≤ b) (hbc : b ≤ c) : Icc a b ∩ Icc b c = {b} :=
  by 
    rw [Icc_inter_Icc, sup_of_le_right hab, inf_of_le_left hbc, Icc_self]

include ht

theorem Ico_inter_Ico : Ico a₁ b₁ ∩ Ico a₂ b₂ = Ico (a₁⊔a₂) (b₁⊓b₂) :=
  by 
    simp only [Ici_inter_Iio.symm, Ici_inter_Ici.symm, Iio_inter_Iio.symm] <;> acRfl

theorem Ioc_inter_Ioc : Ioc a₁ b₁ ∩ Ioc a₂ b₂ = Ioc (a₁⊔a₂) (b₁⊓b₂) :=
  by 
    simp only [Ioi_inter_Iic.symm, Ioi_inter_Ioi.symm, Iic_inter_Iic.symm] <;> acRfl

theorem Ioo_inter_Ioo : Ioo a₁ b₁ ∩ Ioo a₂ b₂ = Ioo (a₁⊔a₂) (b₁⊓b₂) :=
  by 
    simp only [Ioi_inter_Iio.symm, Ioi_inter_Ioi.symm, Iio_inter_Iio.symm] <;> acRfl
...
theorem Ioo_inter_Ioc_of_left_le (h : b₁ ≤ b₂) : Ioo a₁ b₁ ∩ Ioc a₂ b₂ = Ioo (max a₁ a₂) b₁ :=
  by 
    rw [inter_comm, Ioc_inter_Ioo_of_right_le h, max_commₓ]

theorem Ioo_inter_Ioc_of_right_lt (h : b₂ < b₁) : Ioo a₁ b₁ ∩ Ioc a₂ b₂ = Ioc (max a₁ a₂) b₂ :=
  by 
    rw [inter_comm, Ioc_inter_Ioo_of_left_lt h, max_commₓ]

@[simp]
theorem Ico_diff_Iio : Ico a b \ Iio c = Ico (max a c) b :=
  by 
...
@[ simp ]
  theorem
    ...
    := ext $ by simp ( config := { contextual := Bool.true._@._internal._hyg.0 } ) [ iff_def ]
...
@[simp]
theorem Ioc_diff_Iic : Ioc a b \ Iic c = Ioc (max a c) b :=
  by 
    rw [diff_eq, compl_Iic, Ioc_inter_Ioi, sup_eq_max]

@[simp]
theorem Ioc_union_Ioc_right : Ioc a b ∪ Ioc a c = Ioc a (max b c) :=
  by 
    rw [Ioc_union_Ioc, min_selfₓ] <;> exact (min_le_leftₓ _ _).trans (le_max_leftₓ _ _)

@[simp]
theorem Ioc_union_Ioc_left : Ioc a c ∪ Ioc b c = Ioc (min a b) c :=
  by 
      ...
      by
        rw [ Ioc_union_Ioc , Ioc_union_Ioc ]
          ...
          all_goals
            solveByElim
              ( config := { max_depth := 5 } )
              [ min_le_of_left_le , min_le_of_right_le , le_max_of_le_left , le_max_of_le_right , le_reflₓ ]
...
@[simp]
theorem Icc_prod_Icc (a₁ a₂ : α) (b₁ b₂ : β) : (Icc a₁ a₂).Prod (Icc b₁ b₂) = Icc (a₁, b₁) (a₂, b₂) :=
  by 
    ext ⟨x, y⟩
    simp [And.assoc, and_comm, And.left_comm]

theorem Icc_prod_eq (a b : α × β) : Icc a b = (Icc a.1 b.1).Prod (Icc a.2 b.2) :=
  by 
    simp 
...
@[simp]
theorem preimage_Icc (e : α ≃o β) (a b : β) : e ⁻¹' Icc a b = Icc (e.symm a) (e.symm b) :=
  by 
    simp [←Ici_inter_Iic]

@[simp]
theorem preimage_Ico (e : α ≃o β) (a b : β) : e ⁻¹' Ico a b = Ico (e.symm a) (e.symm b) :=
  by 
    simp [←Ici_inter_Iio]
...
@[simp]
theorem preimage_Ioo (e : α ≃o β) (a b : β) : e ⁻¹' Ioo a b = Ioo (e.symm a) (e.symm b) :=
  by 
    simp [←Ioi_inter_Iio]

@[simp]
theorem image_Iic (e : α ≃o β) (a : α) : e '' Iic a = Iic (e a) :=
  by 
...
@[simp]
theorem image_Iio (e : α ≃o β) (a : α) : e '' Iio a = Iio (e a) :=
  by 
...
@[simp]
theorem image_Ioo (e : α ≃o β) (a b : α) : e '' Ioo a b = Ioo (e a) (e b) :=
  by 
    rw [e.image_eq_preimage, e.symm.preimage_Ioo, e.symm_symm]

@[simp]
theorem image_Ioc (e : α ≃o β) (a b : α) : e '' Ioc a b = Ioc (e a) (e b) :=
  by 
    rw [e.image_eq_preimage, e.symm.preimage_Ioc, e.symm_symm]

@[simp]
theorem image_Ico (e : α ≃o β) (a b : α) : e '' Ico a b = Ico (e a) (e b) :=
  by 
    rw [e.image_eq_preimage, e.symm.preimage_Ico, e.symm_symm]

@[simp]
theorem image_Icc (e : α ≃o β) (a b : α) : e '' Icc a b = Icc (e a) (e b) :=
  by 
