...
def IooCat (a b : α) :=
  { x | a < x ∧ x < b }
#align set.Ioo Set.IooCat

/-- Left-closed right-open interval -/
def IcoCat (a b : α) :=
  { x | a ≤ x ∧ x < b }
#align set.Ico Set.IcoCat

/-- Left-infinite right-open interval -/
def IioCat (a : α) :=
  { x | x < a }
#align set.Iio Set.IioCat

/-- Left-closed right-closed interval -/
def IccCat (a b : α) :=
  { x | a ≤ x ∧ x ≤ b }
#align set.Icc Set.IccCat

/-- Left-infinite right-closed interval -/
def IicCat (b : α) :=
  { x | x ≤ b }
#align set.Iic Set.IicCat

/-- Left-open right-closed interval -/
def IocCat (a b : α) :=
  { x | a < x ∧ x ≤ b }
#align set.Ioc Set.IocCat

/-- Left-closed right-infinite interval -/
def IciCat (a : α) :=
  { x | a ≤ x }
#align set.Ici Set.IciCat

/-- Left-open right-infinite interval -/
def IoiCat (a : α) :=
  { x | a < x }
#align set.Ioi Set.IoiCat
...
theorem _root_.is_top.Iic_eq (h : IsTop a) : IicCat a = univ :=
  eq_univ_of_forall h
#align set._root_.is_top.Iic_eq set._root_.is_top.Iic_eq

theorem _root_.is_bot.Ici_eq (h : IsBot a) : IciCat a = univ :=
  eq_univ_of_forall h
#align set._root_.is_bot.Ici_eq set._root_.is_bot.Ici_eq

theorem _root_.is_max.Ioi_eq (h : IsMax a) : IoiCat a = ∅ :=
  eq_empty_of_subset_empty fun b => h.not_lt
#align set._root_.is_max.Ioi_eq set._root_.is_max.Ioi_eq

theorem _root_.is_min.Iio_eq (h : IsMin a) : IioCat a = ∅ :=
  eq_empty_of_subset_empty fun b => h.not_lt
#align set._root_.is_min.Iio_eq set._root_.is_min.Iio_eq
...
/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:65:38: in apply_rules #[["[", expr subset_diff_singleton, "]"], []]: ./././Mathport/Syntax/Translate/Basic.lean:349:22: unsupported: parse error -/
theorem mem_Icc_Ico_Ioc_Ioo_of_subset_of_subset {s : Set α} (ho : IooCat a b ⊆ s) (hc : s ⊆ IccCat a b) :
  ...
  classical by_cases ha:a ∈ s <;> by_cases hb:b ∈ s
...
#align set._root_.is_max.Ici_eq set._root_.is_max.Ici_eq

theorem _root_.is_min.Iic_eq (h : IsMin a) : IicCat a = {a} :=
  h.toDual.Ici_eq
#align set._root_.is_min.Iic_eq set._root_.is_min.Iic_eq
