...
theorem proj_Icc_of_le_left (hx : x ≤ a) : proj_Icc a b h x = ⟨a, left_mem_Icc.2 h⟩ := by
...
theorem proj_Icc_of_right_le (hx : b ≤ x) : proj_Icc a b h x = ⟨b, right_mem_Icc.2 h⟩ := by
  simp [proj_Icc, hx, h]
...
theorem proj_Icc_eq_left (h : a < b) : proj_Icc a b h.le x = ⟨a, left_mem_Icc.mpr h.le⟩ ↔ x ≤ a := by
...
theorem proj_Icc_eq_right (h : a < b) : proj_Icc a b h.le x = ⟨b, right_mem_Icc.mpr h.le⟩ ↔ b ≤ x := by
...
theorem proj_Icc_of_mem (hx : x ∈ Icc a b) : proj_Icc a b h x = ⟨x, hx⟩ := by
  simp [proj_Icc, hx.1, hx.2]

@[simp]
theorem proj_Icc_coe (x : Icc a b) : proj_Icc a b h x = x := by
  cases x
  apply proj_Icc_of_mem

theorem proj_Icc_surj_on : surj_on (proj_Icc a b h) (Icc a b) univ := fun x _ => ⟨x, x.2, proj_Icc_coe h x⟩

theorem proj_Icc_surjective : surjective (proj_Icc a b h) := fun x => ⟨x, proj_Icc_coe h x⟩

@[simp]
theorem range_proj_Icc : range (proj_Icc a b h) = univ :=
  (proj_Icc_surjective h).range_eq

theorem monotone_proj_Icc : Monotone (proj_Icc a b h) := fun x y hxy => max_le_max le_rfl $ min_le_min le_rfl hxy
...
@[simp]
theorem Icc_extend_range (f : Icc a b → β) : range (Icc_extend h f) = range f := by
