...
@[simp]
theorem SetCoe.exists {s : Set α} {p : s → Prop} : (∃ x : s, p x) ↔ ∃ (x) (h : x ∈ s), p ⟨x, h⟩ :=
  Subtype.exists
#align set_coe.exists SetCoe.exists

theorem SetCoe.exists' {s : Set α} {p : ∀ x, x ∈ s → Prop} : (∃ (x) (h : x ∈ s), p x h) ↔ ∃ x : s, p x x.2 :=
...
theorem _root_.has_mem.mem.out {p : α → Prop} {a : α} (h : a ∈ { x | p x }) : p a :=
  h
#align set._root_.has_mem.mem.out set._root_.has_mem.mem.out
...
theorem ssubset_iff_insert {s t : Set α} : s ⊂ t ↔ ∃ (a) (_ : a ∉ s), insert a s ⊆ t := by
...
theorem _root_.disjoint.inter_eq : Disjoint s t → s ∩ t = ∅ :=
  Disjoint.eq_bot
#align set._root_.disjoint.inter_eq set._root_.disjoint.inter_eq
...
theorem mem_symm_diff : a ∈ s ∆ t ↔ a ∈ s ∧ a ∉ t ∨ a ∈ t ∧ a ∉ s :=
  Iff.rfl
#align set.mem_symm_diff Set.mem_symm_diff

protected theorem symm_diff_def (s t : Set α) : s ∆ t = s \ t ∪ t \ s :=
  rfl
#align set.symm_diff_def Set.symm_diff_def

theorem symm_diff_subset_union : s ∆ t ⊆ s ∪ t :=
  @symm_diff_le_sup (Set α) _ _ _
#align set.symm_diff_subset_union Set.symm_diff_subset_union

@[simp]
theorem symm_diff_eq_empty : s ∆ t = ∅ ↔ s = t :=
  symm_diff_eq_bot
#align set.symm_diff_eq_empty Set.symm_diff_eq_empty
...
#align set.inter_symm_diff_distrib_left Set.inter_symm_diff_distrib_left

theorem inter_symm_diff_distrib_right (s t u : Set α) : s ∆ t ∩ u = (s ∩ u) ∆ (t ∩ u) :=
  inf_symm_diff_distrib_right _ _ _
#align set.inter_symm_diff_distrib_right Set.inter_symm_diff_distrib_right

theorem subset_symm_diff_union_symm_diff_left (h : Disjoint s t) : u ⊆ s ∆ u ∪ t ∆ u :=
  h.le_symm_diff_sup_symm_diff_left
#align set.subset_symm_diff_union_symm_diff_left Set.subset_symm_diff_union_symm_diff_left

theorem subset_symm_diff_union_symm_diff_right (h : Disjoint t u) : s ⊆ s ∆ t ∪ s ∆ u :=
  h.le_symm_diff_sup_symm_diff_right
#align set.subset_symm_diff_union_symm_diff_right Set.subset_symm_diff_union_symm_diff_right
...
theorem mem_image_iff_bex {f : α → β} {s : Set α} {y : β} : y ∈ f '' s ↔ ∃ (x) (_ : x ∈ s), f x = y :=
  bex_def.symm
...
#align set._root_.function.injective.mem_set_image set._root_.function.injective.mem_set_image
...
#align set._root_.function.semiconj.set_image set._root_.function.semiconj.set_image

theorem _root_.function.commute.set_image {f g : α → α} (h : Function.Commute f g) :
    Function.Commute (image f) (image g) :=
  h.set_image
#align set._root_.function.commute.set_image set._root_.function.commute.set_image
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (x y) -/
theorem Nontrivial.pair_subset (hs : s.Nontrivial) : ∃ (x) (y) (hab : x ≠ y), {x, y} ⊆ s :=
  let ⟨x, hx, y, hy, hxy⟩ := hs
  ⟨x, y, hxy, insert_subset.2 ⟨hx, singleton_subset_iff.2 hy⟩⟩
#align set.nontrivial.pair_subset Set.Nontrivial.pair_subset

/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (x y) -/
theorem nontrivial_iff_pair_subset : s.Nontrivial ↔ ∃ (x) (y) (hxy : x ≠ y), {x, y} ⊆ s :=
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (x y) -/
theorem nontrivial_of_exists_lt [Preorder α] (H : ∃ (x ∈ s) (y ∈ s), x < y) : s.Nontrivial :=
  let ⟨x, hx, y, hy, hxy⟩ := H
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (x y) -/
theorem nontrivial_iff_exists_lt [LinearOrder α] : s.Nontrivial ↔ ∃ (x ∈ s) (y ∈ s), x < y :=
...
#align set._root_.nat.mem_range_succ set._root_.nat.mem_range_succ
...
/- ./././Mathport/Syntax/Translate/Basic.lean:611:2: warning: expanding binder collection (s «expr ⊆ » range[set.range] f) -/
theorem exists_subset_range_iff {f : α → β} {p : Set β → Prop} : (∃ (s) (_ : s ⊆ range f), p s) ↔ ∃ s, p (f '' s) := by
...
#align set._root_.prod.range_fst set._root_.prod.range_fst

@[simp]
theorem _root_.prod.range_snd [Nonempty α] : range (Prod.snd : α × β → β) = univ :=
  Prod.snd_surjective.range_eq
#align set._root_.prod.range_snd set._root_.prod.range_snd
...
#align set._root_.sum.range_eq set._root_.sum.range_eq
...
theorem WithBot.range_eq {α β} (f : WithBot α → β) : range f = insert (f ⊥) (range (f ∘ coe : α → β)) :=
  Option.range_eq f
#align with_bot.range_eq WithBot.range_eq

theorem WithTop.range_eq {α β} (f : WithTop α → β) : range f = insert (f ⊤) (range (f ∘ coe : α → β)) :=
  Option.range_eq f
