...
theorem mem_Inter {x : β} {s : ι → Set β} : x ∈ Inter s ↔ ∀ i, x ∈ s i :=
  ⟨fun h : ∀, ∀ a ∈ { a : Set β | ∃ i, s i = a }, ∀, x ∈ a a => h (s a) ⟨a, rfl⟩, fun h t ⟨a, (Eq : s a = t)⟩ =>
    Eq ▸ h a⟩
...
protected theorem preimage_kern_image : GaloisConnection (preimage f) (kern_image f) := fun a b =>
  ⟨fun h x hx y hy =>
...
theorem Union_Inter_subset {ι ι' α} {s : ι → ι' → Set α} : (⋃ j, ⋂ i, s i j) ⊆ ⋂ i, ⋃ j, s i j := by
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (hp hq)
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (i i')
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (i' i)
...
-- ././Mathport/Syntax/Translate/Basic.lean:626:6: warning: expanding binder group (x h)
@[simp]
...
theorem mem_bUnion_iff {s : Set α} {t : α → Set β} {y : β} : (y ∈ ⋃ x ∈ s, t x) ↔ ∃ x ∈ s, y ∈ t x := by
  simp

theorem mem_bUnion_iff' {p : α → Prop} {t : α → Set β} {y : β} :
    (y ∈ ⋃ (i) (h : p i), t i) ↔ ∃ (i : _)(h : p i), y ∈ t i :=
  mem_bUnion_iff

theorem mem_bInter_iff {s : Set α} {t : α → Set β} {y : β} : (y ∈ ⋂ x ∈ s, t x) ↔ ∀, ∀ x ∈ s, ∀, y ∈ t x := by
  simp
...
theorem bUnion_subset {s : Set α} {t : Set β} {u : α → Set β} (h : ∀, ∀ x ∈ s, ∀, u x ⊆ t) : (⋃ x ∈ s, u x) ⊆ t :=
  Union_subset $ fun x => Union_subset (h x)

theorem subset_bInter {s : Set α} {t : Set β} {u : α → Set β} (h : ∀, ∀ x ∈ s, ∀, t ⊆ u x) : t ⊆ ⋂ x ∈ s, u x :=
  subset_Inter $ fun x => subset_Inter $ h x

theorem subset_bUnion_of_mem {s : Set α} {u : α → Set β} {x : α} (xs : x ∈ s) : u x ⊆ ⋃ x ∈ s, u x :=
  show u x ≤ ⨆ x ∈ s, u x from le_supr_of_le x $ le_supr _ xs

theorem bInter_subset_of_mem {s : Set α} {t : α → Set β} {x : α} (xs : x ∈ s) : (⋂ x ∈ s, t x) ⊆ t x :=
  show (⨅ x ∈ s, t x) ≤ t x from infi_le_of_le x $ infi_le _ xs
...
theorem bUnion_subset_bUnion {γ : Type _} {s : Set α} {t : α → Set β} {s' : Set γ} {t' : γ → Set β}
    (h : ∀, ∀ x ∈ s, ∀, ∃ y ∈ s', t x ⊆ t' y) : (⋃ x ∈ s, t x) ⊆ ⋃ y ∈ s', t' y := by
...
theorem bInter_mono {s : Set α} {t t' : α → Set β} (h : ∀, ∀ x ∈ s, ∀, t x ⊆ t' x) : (⋂ x ∈ s, t x) ⊆ ⋂ x ∈ s, t' x :=
  bInter_mono' (subset.refl s) h

theorem bInter_congr {s : Set α} {t1 t2 : α → Set β} (h : ∀, ∀ x ∈ s, ∀, t1 x = t2 x) :
    (⋂ x ∈ s, t1 x) = ⋂ x ∈ s, t2 x :=
  subset.antisymm
    (bInter_mono fun x hx => by
      rw [h x hx])
    (bInter_mono fun x hx => by
      rw [h x hx])

theorem bUnion_mono {s : Set α} {t t' : α → Set β} (h : ∀, ∀ x ∈ s, ∀, t x ⊆ t' x) : (⋃ x ∈ s, t x) ⊆ ⋃ x ∈ s, t' x :=
  bUnion_subset_bUnion fun x x_in => ⟨x, x_in, h x x_in⟩

theorem bUnion_congr {s : Set α} {t1 t2 : α → Set β} (h : ∀, ∀ x ∈ s, ∀, t1 x = t2 x) :
    (⋃ x ∈ s, t1 x) = ⋃ x ∈ s, t2 x :=
  subset.antisymm
    (bUnion_mono fun x hx => by
      rw [h x hx])
    (bUnion_mono fun x hx => by
      rw [h x hx])
...
@[simp]
theorem Union_coe_set {α β : Type _} (s : Set α) (f : α → Set β) : (⋃ i : s, f i) = ⋃ i ∈ s, f i :=
  Union_subtype _ _

@[simp]
theorem Inter_coe_set {α β : Type _} (s : Set α) (f : α → Set β) : (⋂ i : s, f i) = ⋂ i ∈ s, f i :=
  Inter_subtype _ _
...
theorem compl_bUnion (s : Set α) (t : α → Set β) : (⋃ i ∈ s, t i)ᶜ = ⋂ i ∈ s, t iᶜ := by
  simp

theorem compl_bInter (s : Set α) (t : α → Set β) : (⋂ i ∈ s, t i)ᶜ = ⋃ i ∈ s, t iᶜ := by
  simp

theorem inter_bUnion (s : Set α) (t : α → Set β) (u : Set β) : (u ∩ ⋃ i ∈ s, t i) = ⋃ i ∈ s, u ∩ t i := by
  simp only [inter_Union]

theorem bUnion_inter (s : Set α) (t : α → Set β) (u : Set β) : (⋃ i ∈ s, t i) ∩ u = ⋃ i ∈ s, t i ∩ u := by
...
theorem bUnion_eq_univ_iff {f : α → Set β} {s : Set α} : (⋃ x ∈ s, f x) = univ ↔ ∀ y, ∃ x ∈ s, y ∈ f x := by
...
theorem bInter_eq_empty_iff {f : α → Set β} {s : Set α} : (⋂ x ∈ s, f x) = ∅ ↔ ∀ y, ∃ x ∈ s, y ∉ f x := by
  simp [Set.eq_empty_iff_forall_not_mem]
...
@[simp]
theorem nonempty_bInter {f : α → Set β} {s : Set α} : (⋂ x ∈ s, f x).Nonempty ↔ ∃ y, ∀, ∀ x ∈ s, ∀, y ∈ f x := by
  simp [← ne_empty_iff_nonempty, Inter_eq_empty_iff]
...
theorem bUnion_subset_Union (s : Set α) (t : α → Set β) : (⋃ x ∈ s, t x) ⊆ ⋃ x, t x :=
  Union_subset_Union $ fun i =>
...
theorem sInter_union_sInter {S T : Set (Set α)} : ⋂₀S ∪ ⋂₀T = ⋂ p ∈ S.prod T, (p : Set α × Set α).1 ∪ p.2 :=
  Inf_sup_Inf

theorem sUnion_inter_sUnion {s t : Set (Set α)} : ⋃₀s ∩ ⋃₀t = ⋃ p ∈ s.prod t, (p : Set α × Set α).1 ∩ p.2 :=
  Sup_inf_Sup
...
theorem sInter_bUnion {S : Set (Set α)} {T : ∀, ∀ s ∈ S, ∀, Set (Set α)} (hT : ∀, ∀ s ∈ S, ∀, s = ⋂₀T s ‹s ∈ S›) :
    (⋂₀⋃ s ∈ S, T s ‹_›) = ⋂₀S := by
  ext
...
theorem sUnion_bUnion {S : Set (Set α)} {T : ∀, ∀ s ∈ S, ∀, Set (Set α)} (hT : ∀, ∀ s ∈ S, ∀, s = ⋃₀T s ‹_›) :
    (⋃₀⋃ s ∈ S, T s ‹_›) = ⋃₀S := by
  ext
...
theorem maps_to_sUnion {S : Set (Set α)} {t : Set β} {f : α → β} (H : ∀, ∀ s ∈ S, ∀, maps_to f s t) :
    maps_to f (⋃₀S) t := fun x ⟨s, hs, hx⟩ => H s hs hx
...
theorem maps_to_sInter {s : Set α} {T : Set (Set β)} {f : α → β} (H : ∀, ∀ t ∈ T, ∀, maps_to f s t) :
    maps_to f s (⋂₀T) := fun x hx t ht => H t ht hx
...
theorem inj_on.image_bInter_eq {p : ι → Prop} {s : ∀ i hi : p i, Set α} (hp : ∃ i, p i) {f : α → β}
    (h : inj_on f (⋃ (i) (hi), s i hi)) : (f '' ⋂ (i) (hi), s i hi) = ⋂ (i) (hi), f '' s i hi := by
...
theorem surj_on_sUnion {s : Set α} {T : Set (Set β)} {f : α → β} (H : ∀, ∀ t ∈ T, ∀, surj_on f s t) :
    surj_on f s (⋃₀T) := fun x ⟨t, ht, hx⟩ => H t ht hx
...
theorem surj_on_Inter [hi : Nonempty ι] {s : ι → Set α} {t : Set β} {f : α → β} (H : ∀ i, surj_on f (s i) t)
    (Hinj : inj_on f (⋃ i, s i)) : surj_on f (⋂ i, s i) t := by
  intro y hy
...
theorem image_bUnion {f : α → β} {s : ι → Set α} {p : ι → Prop} :
    (f '' ⋃ (i) (hi : p i), s i) = ⋃ (i) (hi : p i), f '' s i := by
...
theorem preimage_bUnion {ι} {f : α → β} {s : Set ι} {t : ι → Set β} : (f ⁻¹' ⋃ i ∈ s, t i) = ⋃ i ∈ s, f ⁻¹' t i := by
  simp
...
theorem preimage_Inter {ι : Sort _} {s : ι → Set β} {f : α → β} : (f ⁻¹' ⋂ i, s i) = ⋂ i, f ⁻¹' s i := by
  ext <;> simp

theorem preimage_bInter {s : γ → Set β} {t : Set γ} {f : α → β} : (f ⁻¹' ⋂ i ∈ t, s i) = ⋂ i ∈ t, f ⁻¹' s i := by
  ext <;> simp
...
theorem monotone_prod [Preorderₓ α] {f : α → Set β} {g : α → Set γ} (hf : Monotone f) (hg : Monotone g) :
    Monotone fun x => (f x).Prod (g x) := fun a b h => prod_mono (hf h) (hg h)

alias monotone_prod ← Monotone.set_prod

theorem prod_Union {ι} {s : Set α} {t : ι → Set β} : s.prod (⋃ i, t i) = ⋃ i, s.prod (t i) := by
  ext
  simp

theorem prod_bUnion {ι} {u : Set ι} {s : Set α} {t : ι → Set β} : s.prod (⋃ i ∈ u, t i) = ⋃ i ∈ u, s.prod (t i) := by
  simp_rw [prod_Union]

theorem prod_sUnion {s : Set α} {C : Set (Set β)} : s.prod (⋃₀C) = ⋃₀((fun t => s.prod t) '' C) := by
  simp only [sUnion_eq_bUnion, prod_bUnion, bUnion_image]

theorem Union_prod_const {ι} {s : ι → Set α} {t : Set β} : (⋃ i, s i).Prod t = ⋃ i, (s i).Prod t := by
  ext
  simp

theorem bUnion_prod_const {ι} {u : Set ι} {s : ι → Set α} {t : Set β} : (⋃ i ∈ u, s i).Prod t = ⋃ i ∈ u, (s i).Prod t :=
  by
  simp_rw [Union_prod_const]

theorem sUnion_prod_const {C : Set (Set α)} {t : Set β} : (⋃₀C).Prod t = ⋃₀((fun s : Set α => s.prod t) '' C) := by
  simp only [sUnion_eq_bUnion, bUnion_prod_const, bUnion_image]

theorem Union_prod {ι α β} (s : ι → Set α) (t : ι → Set β) :
    (⋃ x : ι × ι, (s x.1).Prod (t x.2)) = (⋃ i : ι, s i).Prod (⋃ i : ι, t i) := by
  ext
  simp

theorem Union_prod_of_monotone [SemilatticeSup α] {s : α → Set β} {t : α → Set γ} (hs : Monotone s) (ht : Monotone t) :
    (⋃ x, (s x).Prod (t x)) = (⋃ x, s x).Prod (⋃ x, t x) := by
  ext ⟨z, w⟩
...
theorem prod_eq_seq {s : Set α} {t : Set β} : s.prod t = (Prod.mk '' s).seq t := by
  ext ⟨a, b⟩
  constructor
  · rintro ⟨ha, hb⟩
...
instance : Monadₓ Set where
  pure := fun α : Type u a => {a}
  bind := fun α β : Type u s f => ⋃ i ∈ s, f i
  seq := fun α β : Type u => Set.Seq
  map := fun α β : Type u => Set.Image
...
theorem not_disjoint_iff_nonempty_inter {α : Type _} {s t : Set α} : ¬Disjoint s t ↔ (s ∩ t).Nonempty := by
...
sending `⟨i, x⟩` to `x`. -/
def sigma_to_Union (x : Σ i, t i) : ⋃ i, t i :=
