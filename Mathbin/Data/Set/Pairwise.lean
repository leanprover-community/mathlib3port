...
theorem Pairwise.mono (hr : Pairwise r) (h : ∀ ⦃i j⦄, r i j → p i j) : Pairwise p :=
  fun i j hij => h$ hr i j hij

theorem pairwise_on_bool (hr : Symmetric r) {a b : α} : Pairwise (r on fun c => cond c a b) ↔ r a b :=
  by 
...
theorem pairwise_pair : Set.Pairwise {a, b} r ↔ a ≠ b → r a b ∧ r b a :=
  by 
    simp [pairwise_insert]

theorem pairwise_pair_of_symmetric (hr : Symmetric r) : Set.Pairwise {a, b} r ↔ a ≠ b → r a b :=
  by 
    simp [pairwise_insert_of_symmetric hr]

theorem pairwise_univ : (univ : Set α).Pairwise r ↔ Pairwise r :=
  by 
...
theorem Pairwise.set_pairwise (h : Pairwise r) (s : Set α) : s.pairwise r :=
  fun x hx y hy => h x y
...
theorem pairwise_disjoint_range_singleton : (Set.Range (singleton : ι → Set ι)).PairwiseDisjoint id :=
  by 
