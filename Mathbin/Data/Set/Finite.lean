...
theorem subset_to_finset_iff {s : Finset α} {t : Set α} (ht : finite t) : s ⊆ ht.to_finset ↔ ↑s ⊆ t :=
  by 
...
theorem infinite_of_finite_compl [_root_.infinite α] {s : Set α} (hs : sᶜ.Finite) : s.infinite :=
  fun h =>
    Set.infinite_univ
      (by 
        simpa using hs.union h)

theorem finite.infinite_compl [_root_.infinite α] {s : Set α} (hs : s.finite) : sᶜ.Infinite :=
  fun h =>
    Set.infinite_univ
      (by 
        simpa using hs.union h)
...
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
instance fintype_le_nat ( n : ℕ ) : Fintype { i | i ≤ n } := by simpa [ Nat.lt_succ_iff ] using Set.fintypeLtNat n + 1
...
@[simp]
theorem coe_bUnion {f : α → Finset β} : ↑s.bUnion f = (⋃ (x : _)(_ : x ∈ (↑s : Set α)), ↑f x : Set β) :=
  by 
    simp [Set.ext_iff]
...
theorem range_find_greatest_subset {P : α → ℕ → Prop} [∀ x, DecidablePred (P x)] {b : ℕ} :
  (range fun x => Nat.findGreatest (P x) b) ⊆ ↑Finset.range (b+1) :=
  by 
    rw [range_subset_iff]
    intro x 
...
theorem Set.finite_of_forall_between_eq_endpoints {α : Type _} [LinearOrderₓ α] (s : Set α)
  (h : ∀ x _ : x ∈ s y _ : y ∈ s z _ : z ∈ s, x ≤ y → y ≤ z → x = y ∨ y = z) : Set.Finite s :=
  by 
    byContra hinf 
