...
@[simp]
theorem finite_univ_to_finset [Fintype α] (h : (Set.Univ : Set α).Finite) : h.toFinset = Finset.univ :=
  Finset.ext <| by simp
...
lean 3 declaration is
  forall {α : Type.{u}} {ι : Sort.{w}} [_inst_1 : DecidableEq.{succ u} α] [_inst_2 : Fintype.{w} (PLift.{w} ι)] (f : ι -> (Set.{u} α)) [_inst_3 : forall (i : ι), Fintype.{u} (coeSort.{(max (succ u) 1) succ (succ u)} (Set.{u} α) Type.{u} (Set.hasCoeToSort.{u} α) (f i))], Fintype.{u} (coeSort.{(max (succ u) 1) succ (succ u)} (Set.{u} α) Type.{u} (Set.hasCoeToSort.{u} α) (Set.UnionCat.{u w} α ι (fun (i : ι) => f i)))
...
instance fintypeUnion [DecidableEq α] [Fintype (PLift ι)] (f : ι → Set α) [∀ i, Fintype (f i)] : Fintype (⋃ i, f i) :=
...
structure. -/
def fintypeBUnion [DecidableEq α] {ι : Type _} (s : Set ι) [Fintype s] (t : ι → Set α) (H : ∀ i ∈ s, Fintype (t i)) :
    Fintype (⋃ x ∈ s, t x) :=
  Fintype.ofFinset
...
instance fintypeLeNat (n : ℕ) : Fintype { i | i ≤ n } := by simpa [Nat.lt_succ_iff] using Set.fintypeLtNat (n + 1)
...
def Nat.fintypeIio (n : ℕ) : Fintype (IioCat n) :=
  Set.fintypeLtNat n
...
theorem finite_of_finite_preimage {f : α → β} {s : Set β} (h : (f ⁻¹' s).Finite) (hs : s ⊆ Range f) : s.Finite := by
...
theorem Finite.off_diag {s : Set α} (hs : s.Finite) : s.OffDiag.Finite := by classical cases hs
...
theorem Finite.finite_subsets {α : Type u} {a : Set α} (h : a.Finite) : { b | b ⊆ a }.Finite :=
  ⟨(Fintype.ofFinset ((Finset.powerset h.toFinset).map Finset.coeEmb.1)) fun s => by
      simpa [← @exists_finite_iff_finset α fun t => t ⊆ a ∧ t = s, subset_to_finset_iff, ← and_assoc] using h.subset⟩
...
theorem Finite.pi {δ : Type _} [Finite δ] {κ : δ → Type _} {t : ∀ d, Set (κ d)} (ht : ∀ d, (t d).Finite) :
  ...
  classical rw [← Fintype.coe_pi_finset]
...
theorem seq_of_forall_finite_exists {γ : Type _} {P : γ → Set γ → Prop} (h : ∀ t : Set γ, t.Finite → ∃ c, P c t) :
    ∃ u : ℕ → γ, ∀ n, P (u n) (u '' IioCat n) :=
  ⟨fun n =>
    (@Nat.strongRecOn' (fun _ => γ) n) fun n ih =>
      Classical.choose <| h (range fun m : IioCat n => ih m.1 m.2) (finite_range _),
    fun n => by
    classical refine' Nat.strongRecOn' n fun n ih => _
...
theorem Infinite.exists_lt_map_eq_of_maps_to [LinearOrder α] {s : Set α} {t : Set β} {f : α → β} (hs : s.Infinite)
...
theorem Finite.exists_lt_map_eq_of_forall_mem [LinearOrder α] [Infinite α] {t : Set β} {f : α → β} (hf : ∀ a, f a ∈ t)
...
theorem exists_lower_bound_image [hα : Nonempty α] [LinearOrder β] (s : Set α) (f : α → β) (h : s.Finite) :
    ∃ a : α, ∀ b ∈ s, f a ≤ f b := by
...
theorem exists_upper_bound_image [hα : Nonempty α] [LinearOrder β] (s : Set α) (f : α → β) (h : s.Finite) :
    ∃ a : α, ∀ b ∈ s, f b ≤ f a := by
  by_cases hs:Set.Nonempty s
  · exact
...
#align set._root_.supr_infi_of_monotone set._root_.supr_infi_of_monotone
...
#align set._root_.supr_infi_of_antitone set._root_.supr_infi_of_antitone
...
#align set._root_.infi_supr_of_monotone set._root_.infi_supr_of_monotone
...
#align set._root_.infi_supr_of_antitone set._root_.infi_supr_of_antitone
...
theorem Union_pi_of_monotone {ι ι' : Type _} [LinearOrder ι'] [Nonempty ι'] {α : ι → Type _} {I : Set ι}
...
theorem Union_univ_pi_of_monotone {ι ι' : Type _} [LinearOrder ι'] [Nonempty ι'] [Finite ι] {α : ι → Type _}
