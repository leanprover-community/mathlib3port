...
@[simp]
theorem fin_succ_equiv_zero {n : ℕ} : (finSuccEquiv n) 0 = none := by
  cases n <;> rfl
...
theorem coe_fin_rotate_of_ne_last {n : ℕ} {i : Finₓ n.succ} (h : i ≠ Finₓ.last n) : (finRotate n.succ i : ℕ) = i+1 := by
...
def finProdFinEquiv : Finₓ m × Finₓ n ≃ Finₓ (m*n) :=
  { toFun := fun x =>
      ⟨x.2+n*x.1,
        calc ((x.2.1+n*x.1.1)+1) = ((x.1.1*n)+x.2.1)+1 := by
          ac_rfl
