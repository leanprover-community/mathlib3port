...
@[ext]
protected theorem ext {s₁ s₂ : Streamₓ α} : (∀ n, nth s₁ n = nth s₂ n) → s₁ = s₂ :=
  fun h => funext h
...
theorem mem_of_nth_eq {n : Nat} {s : Streamₓ α} {a : α} : a = nth s n → a ∈ s :=
...
theorem nth_succ_iterate (n : Nat) (f : α → α) (a : α) : nth (iterate f a) (succ n) = nth (iterate f (f a)) n :=
  by 
...
theorem nth_unfolds_head_tail : ∀ n : Nat s : Streamₓ α, nth (unfolds head tail s) n = nth s n :=
  by 
    intro n 
    induction' n with n' ih
    ·
...
theorem nth_odd : ∀ n : Nat s : Streamₓ α, nth (odd s) n = nth s ((2*n)+1) :=
  fun n s =>
    by 
      rw [odd_eq, nth_even]
      rfl
...
@[simp]
theorem length_take (n : ℕ) (s : Streamₓ α) : (take n s).length = n :=
  by 
...
theorem append_take_drop : ∀ n : Nat s : Streamₓ α, append_stream (take n s) (drop n s) = s :=
  by 
    intro n 
    induction' n with n' ih
    ·
...
theorem take_theorem (s₁ s₂ : Streamₓ α) : (∀ n : Nat, take n s₁ = take n s₂) → s₁ = s₂ :=
  by 
    intro h 
    apply Streamₓ.ext 
    intro n 
...
theorem nth_tails : ∀ n : Nat s : Streamₓ α, nth (tails s) n = drop n (tail s) :=
  by 
    intro n 
    induction' n with n' ih
    ·
...
theorem cons_nth_inits_core :
  ∀ a : α n : Nat l : List α s : Streamₓ α, a :: nth (inits_core l s) n = nth (inits_core (a :: l) s) n :=
  by 
    intro a n 
    induction' n with n' ih
    ·
...
theorem nth_inits : ∀ n : Nat s : Streamₓ α, nth (inits s) n = take (succ n) s :=
  by 
    intro n 
    induction' n with n' ih
    ·
