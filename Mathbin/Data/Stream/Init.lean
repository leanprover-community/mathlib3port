...
[PrettyPrinter.parenthesize.input] (Command.declaration
     ...
     (Command.def
      "def"
      (Command.declId `IsBisimulation [])
      (Command.optDeclSig [] [])
      (Command.declValSimple
       ":="
       (Term.forall
        "∀"
        [(Term.strictImplicitBinder "⦃" [`s₁ `s₂] [] "⦄")]
        []
        ","
        (Term.arrow
         (Stream'.Data.Stream.Init.«term_~_» `s₁ " ~ " `s₂)
         "→"
         («term_∧_»
          («term_=_» (Term.app `head [`s₁]) "=" (Term.app `head [`s₂]))
          "∧"
          (Stream'.Data.Stream.Init.«term_~_»
           (Term.app `tail [`s₁])
           " ~ "
           (Term.app `tail [`s₂])))))
       [])
      ...
      (Term.forall
       ...
       (Term.arrow
        ...
        («term_∧_»
         («term_=_» (Term.app `head [`s₁]) "=" (Term.app `head [`s₂]))
         "∧"
         (Stream'.Data.Stream.Init.«term_~_» (Term.app `tail [`s₁]) " ~ " (Term.app `tail [`s₂])))))
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.arrow
       (Stream'.Data.Stream.Init.«term_~_» `s₁ " ~ " `s₂)
       "→"
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Stream'.Data.Stream.Init.«term_~_» (Term.app `tail [`s₁]) " ~ " (Term.app `tail [`s₂]))
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Stream'.Data.Stream.Init.«term_~_»', expected 'Stream'.Data.Stream.Init.term_~_._@.Data.Stream.Init._hyg.14'
...
def IsBisimulation := ∀ ⦃ s₁ s₂ ⦄ , s₁ ~ s₂ → head s₁ = head s₂ ∧ tail s₁ ~ tail s₂
...
[PrettyPrinter.parenthesize.input] (Command.declaration
     ...
     (Command.theorem
      "theorem"
      (Command.declId `nth_of_bisim [])
      (Command.declSig
       [(Term.explicitBinder "(" [`bisim] [":" (Term.app `IsBisimulation [`R])] [] ")")]
       (Term.typeSpec
        ":"
        (Term.forall
         "∀"
         [(Term.implicitBinder "{" [`s₁ `s₂] [] "}") (Term.explicitBinder "(" [`n] [] [] ")")]
         []
         ","
         (Term.arrow
          (Stream'.Data.Stream.Init.«term_~_» `s₁ " ~ " `s₂)
          "→"
          («term_∧_»
           («term_=_» (Term.app `nth [`s₁ `n]) "=" (Term.app `nth [`s₂ `n]))
           "∧"
           (Stream'.Data.Stream.Init.«term_~_»
            (Term.app `drop [(«term_+_» `n "+" (num "1")) `s₁])
            " ~ "
            (Term.app `drop [(«term_+_» `n "+" (num "1")) `s₂])))))))
      (Command.declValEqns
       (Term.matchAltsWhereDecls
        (Term.matchAlts
         [(Term.matchAlt "|" [[`s₁ "," `s₂ "," (num "0") "," `h]] "=>" (Term.app `bisim [`h]))
          (Term.matchAlt
           "|"
           [[`s₁ "," `s₂ "," («term_+_» `n "+" (num "1")) "," `h]]
           "=>"
           (Term.match
            "match"
            []
            []
            [(Term.matchDiscr [] (Term.app `bisim [`h]))]
            "with"
            (Term.matchAlts
             [(Term.matchAlt
               "|"
               [[(Term.anonymousCtor "⟨" [`h₁ "," `trel] "⟩")]]
               "=>"
               (Term.app `nth_of_bisim [`n `trel]))])))])
        []))
      []
      []))
...
theorem
  ...
  | s₁ , s₂ , 0 , h => bisim h
    | s₁ , s₂ , n + 1 , h => match bisim h with | ⟨ h₁ , trel ⟩ => nth_of_bisim n trel
         ...
         (Term.app
          ...
          [(Term.fun
            ...
            (Term.basicFun
             ...
             []
             "=>"
             (Term.app `And.left [(Term.app `nth_of_bisim [`R `bisim `n `r])])))])))
       [])
      []
      []))
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Command.theorem', expected 'Lean.Parser.Command.abbrev'
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.app `nth_of_bisim [`R `bisim `n `r])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'Lean.Parser.Term.namedArgument'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'ident', expected 'Lean.Parser.Term.ellipsis'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      `r
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1023, [anonymous]))
      (Term.forall
       ...
       []
       ","
       (Term.arrow (Stream'.Data.Stream.Init.«term_~_» `s₁ " ~ " `s₂) "→" («term_=_» `s₁ "=" `s₂)))
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      (Term.arrow (Stream'.Data.Stream.Init.«term_~_» `s₁ " ~ " `s₂) "→" («term_=_» `s₁ "=" `s₂))
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
      («term_=_» `s₁ "=" `s₂)
...
theorem
  eq_of_bisim
  ( bisim : IsBisimulation R ) : ∀ { s₁ s₂ } , s₁ ~ s₂ → s₁ = s₂
  := fun s₁ s₂ r => Stream'.ext fun n => And.left nth_of_bisim R bisim n r
