...
/- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation -/
theorem eq_or_mem_of_mem_cons {a b : α} {s : Stream α} : (a ∈ b::s) → a = b ∨ a ∈ s := fun ⟨n, h⟩ =>
  by 
  cases' n with n'
  · left
    exact h
...
theorem coinduction {s₁ s₂ : Stream α} :
    head s₁ = head s₂ →
      (∀ (β : Type u) (fr : Stream α → β), fr s₁ = fr s₂ → fr (tail s₁) = fr (tail s₂)) → s₁ = s₂ :=
  fun hh ht =>
  eq_of_bisim
    (fun s₁ s₂ =>
      head s₁ = head s₂ ∧
...
/- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation -/
/- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation -/
theorem even_cons_cons (a₁ a₂ : α) (s : Stream α) : even (a₁::a₂::s) = a₁::even s := by unfold even;
  rw [corec_eq]; rfl
