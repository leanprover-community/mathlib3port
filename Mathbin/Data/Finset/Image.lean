...
theorem image_bunionᵢ [DecidableEq γ] {f : α → β} {s : Finset α} {t : β → Finset γ} :
  ...
  Finset.induction_on s rfl fun a s has ih => by simp only [image_insert, bunionᵢ_insert, ih]
...
theorem bunionᵢ_image [DecidableEq γ] {s : Finset α} {t : α → Finset β} {f : β → γ} :
  ...
  haveI := Classical.decEq α
  Finset.induction_on s rfl fun a s has ih => by simp only [bunionᵢ_insert, image_union, ih]
