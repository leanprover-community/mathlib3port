...
but is expected to have type
  forall {α : Type.{u2}} {n : Nat} {p : (Finset.{u2} α) -> Sort.{u1}} (s : Finset.{u2} α) (H : forall (t₁ : Finset.{u2} α), (forall {t₂ : Finset.{u2} α}, (LE.le.{0} Nat instLENat (Finset.card.{u2} α t₂) n) -> (HasSSubset.SSubset.{u2} (Finset.{u2} α) (Finset.instHasSSubsetFinset.{u2} α) t₁ t₂) -> (p t₂)) -> (LE.le.{0} Nat instLENat (Finset.card.{u2} α t₁) n) -> (p t₁)), Eq.{u1} ((LE.le.{0} Nat instLENat (Finset.card.{u2} α s) n) -> (p s)) (fun (a._@.Mathlib.Data.Finset.Card._hyg.8651 : LE.le.{0} Nat instLENat (Finset.card.{u2} α s) n) => Finset.strongDownwardInductionOn.{u2, u1} α n (fun (x._@.Mathlib.Data.Finset.Card._hyg.8752 : Finset.{u2} α) => p x._@.Mathlib.Data.Finset.Card._hyg.8752) s H a._@.Mathlib.Data.Finset.Card._hyg.8651) (H s (fun {t : Finset.{u2} α} (ht : LE.le.{0} Nat instLENat (Finset.card.{u2} α t) n) (h : HasSSubset.SSubset.{u2} (Finset.{u2} α) (Finset.instHasSSubsetFinset.{u2} α) s t) => Finset.strongDownwardInductionOn.{u2, u1} α n (fun (t₂ : Finset.{u2} α) => p t₂) t H ht))
