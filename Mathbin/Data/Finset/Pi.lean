...
satisfied. -/
def Pi.empty (β : α → Sort _) (a : α) (h : a ∈ (∅ : Finsetₓ α)) : β a :=
...
def pi (s : Finsetₓ α) (t : ∀ a, Finsetₓ (δ a)) : Finsetₓ (∀ a ∈ s, δ a) :=
  ⟨s.1.pi fun a => (t a).1, s.Nodup.pi fun a _ => (t a).Nodup⟩

@[simp]
theorem pi_val (s : Finsetₓ α) (t : ∀ a, Finsetₓ (δ a)) : (s.pi t).1 = s.1.pi fun a => (t a).1 :=
  rfl

@[simp]
theorem mem_pi {s : Finsetₓ α} {t : ∀ a, Finsetₓ (δ a)} {f : ∀ a ∈ s, δ a} :
...
anyway. -/
def pi.cons (s : Finsetₓ α) (a : α) (b : δ a) (f : ∀ a, a ∈ s → δ a) (a' : α) (h : a' ∈ insert a s) : δ a' :=
  Multiset.Pi.cons s.1 a b f _ (Multiset.mem_cons.2 <| mem_insert.symm.2 h)

@[simp]
theorem pi.cons_same (s : Finsetₓ α) (a : α) (b : δ a) (f : ∀ a, a ∈ s → δ a) (h : a ∈ insert a s) :
    pi.cons s a b f a h = b :=
  Multiset.Pi.cons_same _

theorem pi.cons_ne {s : Finsetₓ α} {a a' : α} {b : δ a} {f : ∀ a, a ∈ s → δ a} {h : a' ∈ insert a s} (ha : a ≠ a') :
    pi.cons s a b f a' h = f a' ((mem_insert.1 h).resolve_left ha.symm) :=
  Multiset.Pi.cons_ne _ _

theorem pi_cons_injective {a : α} {b : δ a} {s : Finsetₓ α} (hs : a ∉ s) : Function.Injective (pi.cons s a b) :=
  fun e₁ e₂ eq =>
...
@[simp]
theorem pi_empty {t : ∀ a : α, Finsetₓ (δ a)} : pi (∅ : Finsetₓ α) t = singleton (Pi.empty δ) :=
  rfl

@[simp]
theorem pi_insert [∀ a, DecidableEq (δ a)] {s : Finsetₓ α} {t : ∀ a : α, Finsetₓ (δ a)} {a : α} (ha : a ∉ s) :
...
theorem pi_singletons {β : Type _} (s : Finsetₓ α) (f : α → β) :
    (s.pi fun a => ({f a} : Finsetₓ β)) = {fun a _ => f a} := by
...
theorem pi_const_singleton {β : Type _} (s : Finsetₓ α) (i : β) : (s.pi fun _ => ({i} : Finsetₓ β)) = {fun _ _ => i} :=
  pi_singletons s fun _ => i

theorem pi_subset {s : Finsetₓ α} (t₁ t₂ : ∀ a, Finsetₓ (δ a)) (h : ∀ a ∈ s, t₁ a ⊆ t₂ a) : s.pi t₁ ⊆ s.pi t₂ :=
  fun g hg => mem_pi.2 fun a ha => h a ha (mem_pi.mp hg a ha)

theorem pi_disjoint_of_disjoint {δ : α → Type _} [∀ a, DecidableEq (δ a)] {s : Finsetₓ α} [DecidableEq (∀ a ∈ s, δ a)]
    (t₁ t₂ : ∀ a, Finsetₓ (δ a)) {a : α} (ha : a ∈ s) (h : Disjoint (t₁ a) (t₂ a)) : Disjoint (s.pi t₁) (s.pi t₂) :=
