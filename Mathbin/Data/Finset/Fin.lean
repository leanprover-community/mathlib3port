...
def attachFin (s : Finsetₓ ℕ) {n : ℕ} (h : ∀ m ∈ s, m < n) : Finsetₓ (Finₓ n) :=
  ⟨s.1.pmap (fun a ha => ⟨a, ha⟩) h, s.Nodup.pmap fun _ _ _ _ => Finₓ.veq_of_eq⟩

@[simp]
theorem mem_attach_fin {n : ℕ} {s : Finsetₓ ℕ} (h : ∀ m ∈ s, m < n) {a : Finₓ n} : a ∈ s.attachFin h ↔ (a : ℕ) ∈ s :=
  ⟨fun h =>
...
@[simp]
theorem card_attach_fin {n : ℕ} (s : Finsetₓ ℕ) (h : ∀ m ∈ s, m < n) : (s.attachFin h).card = s.card :=
