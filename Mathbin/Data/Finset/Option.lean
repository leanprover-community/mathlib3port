...
@[simp]
theorem mem_to_finset {a : α} {o : Option α} : a ∈ o.to_finset ↔ a ∈ o :=
  by 
...
def insert_none : Finset α ↪o Finset (Option α) :=
  (OrderEmbedding.ofMapLeIff
      fun s =>
        cons none (s.map embedding.some)$
          by 
            simp )$
  ...
  iff_of_true (Multiset.mem_cons_self _ _)
    fun a h =>
      by 
        cases h
| some a =>
  Multiset.mem_cons.trans$
    by 
      simp  <;> rfl

theorem some_mem_insert_none {s : Finset α} {a : α} : some a ∈ s.insert_none ↔ a ∈ s :=
  by 
    simp 

@[simp]
theorem card_insert_none (s : Finset α) : s.insert_none.card = s.card+1 :=
  by 
    simp [insert_none]
...
@[simp]
theorem erase_none_image_some [DecidableEq (Option α)] (s : Finset α) : (s.image some).eraseNone = s :=
  by 
...
theorem erase_none_union [DecidableEq (Option α)] [DecidableEq α] (s t : Finset (Option α)) :
  (s ∪ t).eraseNone = s.erase_none ∪ t.erase_none :=
  by 
    ext 
    simp 
...
@[simp]
theorem erase_none_insert_none (s : Finset α) : erase_none (insert_none s) = s :=
  by 
    ext 
    simp 
