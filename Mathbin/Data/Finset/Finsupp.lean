...
variable {ι α : Type _} [Zero α] {s : Finsetₓ ι} {f : ι →₀ α}

namespace Finsetₓ

/-- Finitely supported product of finsets. -/
protected def finsupp (s : Finsetₓ ι) (t : ι → Finsetₓ α) : Finsetₓ (ι →₀ α) :=
  (s.pi t).map ⟨indicator s, indicator_injective s⟩

theorem mem_finsupp_iff {t : ι → Finsetₓ α} : f ∈ s.Finsupp t ↔ f.Support ⊆ s ∧ ∀ i ∈ s, f i ∈ t i := by
...
@[simp]
theorem mem_finsupp_iff_of_support_subset {t : ι →₀ Finsetₓ α} (ht : t.Support ⊆ s) :
...
@[simp]
theorem card_finsupp (s : Finsetₓ ι) (t : ι → Finsetₓ α) : (s.Finsupp t).card = ∏ i in s, (t i).card :=
...
def pi (f : ι →₀ Finsetₓ α) : Finsetₓ (ι →₀ α) :=
  f.Support.Finsupp f

@[simp]
theorem mem_pi {f : ι →₀ Finsetₓ α} {g : ι →₀ α} : g ∈ f.pi ↔ ∀ i, g i ∈ f i :=
  mem_finsupp_iff_of_support_subset <| Subset.refl _

@[simp]
theorem card_pi (f : ι →₀ Finsetₓ α) : f.pi.card = f.Prod fun i => (f i).card := by
  rw [pi, card_finsupp]
