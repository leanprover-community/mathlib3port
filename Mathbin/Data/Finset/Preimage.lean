...
noncomputable def preimage (s : Finsetₓ β) (f : α → β) (hf : Set.InjOn f (f ⁻¹' ↑s)) : Finsetₓ α :=
  (s.finite_to_set.Preimage hf).toFinset

@[simp]
theorem mem_preimage {f : α → β} {s : Finsetₓ β} {hf : Set.InjOn f (f ⁻¹' ↑s)} {x : α} :
    x ∈ preimage s f hf ↔ f x ∈ s :=
  Set.Finite.mem_to_finset _

@[simp, norm_cast]
theorem coe_preimage {f : α → β} (s : Finsetₓ β) (hf : Set.InjOn f (f ⁻¹' ↑s)) :
    (↑(preimage s f hf) : Set α) = f ⁻¹' ↑s :=
  Set.Finite.coe_to_finset _

@[simp]
theorem preimage_empty {f : α → β} : preimage ∅ f (by simp [inj_on]) = ∅ :=
  Finsetₓ.coe_injective (by simp)

@[simp]
theorem preimage_univ {f : α → β} [Fintypeₓ α] [Fintypeₓ β] (hf) : preimage univ f hf = univ :=
  Finsetₓ.coe_injective (by simp)

@[simp]
theorem preimage_inter [DecidableEq α] [DecidableEq β] {f : α → β} {s t : Finsetₓ β} (hs : Set.InjOn f (f ⁻¹' ↑s))
    (ht : Set.InjOn f (f ⁻¹' ↑t)) :
...
@[simp]
theorem preimage_union [DecidableEq α] [DecidableEq β] {f : α → β} {s t : Finsetₓ β} (hst) :
    preimage (s ∪ t) f hst =
...
@[simp]
theorem preimage_compl [DecidableEq α] [DecidableEq β] [Fintypeₓ α] [Fintypeₓ β] {f : α → β} (s : Finsetₓ β)
    (hf : Function.Injective f) : preimage (sᶜ) f (hf.InjOn _) = preimage s f (hf.InjOn _)ᶜ :=
  Finsetₓ.coe_injective (by simp)

theorem monotone_preimage {f : α → β} (h : Injective f) : Monotoneₓ fun s => preimage s f (h.InjOn _) :=
  fun s t hst x hx => mem_preimage.2 (hst <| mem_preimage.1 hx)

theorem image_subset_iff_subset_preimage [DecidableEq β] {f : α → β} {s : Finsetₓ α} {t : Finsetₓ β}
    (hf : Set.InjOn f (f ⁻¹' ↑t)) : s.Image f ⊆ t ↔ s ⊆ t.Preimage f hf :=
  image_subset_iff.trans <| by simp only [subset_iff, mem_preimage]

theorem map_subset_iff_subset_preimage {f : α ↪ β} {s : Finsetₓ α} {t : Finsetₓ β} :
    s.map f ⊆ t ↔ s ⊆ t.Preimage f (f.Injective.InjOn _) := by
  classical <;> rw [map_eq_image, image_subset_iff_subset_preimage]

theorem image_preimage [DecidableEq β] (f : α → β) (s : Finsetₓ β) [∀ x, Decidable (x ∈ Set.Range f)]
    (hf : Set.InjOn f (f ⁻¹' ↑s)) : image f (preimage s f hf) = s.filter fun x => x ∈ Set.Range f :=
  Finsetₓ.coe_inj.1 <| by
    simp only [coe_image, coe_preimage, coe_filter, Set.image_preimage_eq_inter_range, Set.sep_mem_eq]

theorem image_preimage_of_bij [DecidableEq β] (f : α → β) (s : Finsetₓ β) (hf : Set.BijOn f (f ⁻¹' ↑s) ↑s) :
    image f (preimage s f hf.InjOn) = s :=
  Finsetₓ.coe_inj.1 <| by simpa using hf.image_eq

theorem preimage_subset {f : α ↪ β} {s : Finsetₓ β} {t : Finsetₓ α} (hs : s ⊆ t.map f) :
    s.Preimage f (f.Injective.InjOn _) ⊆ t := fun x hx => (mem_map' f).1 (hs (mem_preimage.1 hx))

-- ./././Mathport/Syntax/Translate/Basic.lean:555:2: warning: expanding binder collection (u «expr ⊆ » t)
theorem subset_map_iff {f : α ↪ β} {s : Finsetₓ β} {t : Finsetₓ α} : s ⊆ t.map f ↔ ∃ (u : _)(_ : u ⊆ t), s = u.map f :=
  by
  classical
...
theorem sigma_preimage_mk {β : α → Type _} [DecidableEq α] (s : Finsetₓ (Σa, β a)) (t : Finsetₓ α) :
    (t.Sigma fun a => s.Preimage (Sigma.mk a) <| sigma_mk_injective.InjOn _) = s.filter fun a => a.1 ∈ t := by
  ext x
  simp [and_comm]

theorem sigma_preimage_mk_of_subset {β : α → Type _} [DecidableEq α] (s : Finsetₓ (Σa, β a)) {t : Finsetₓ α}
...
theorem sigma_image_fst_preimage_mk {β : α → Type _} [DecidableEq α] (s : Finsetₓ (Σa, β a)) :
...
@[to_additive]
theorem prod_preimage' [CommMonoidₓ β] (f : α → γ) [DecidablePred fun x => x ∈ Set.Range f] (s : Finsetₓ γ)
...
@[to_additive]
theorem prod_preimage [CommMonoidₓ β] (f : α → γ) (s : Finsetₓ γ) (hf : Set.InjOn f (f ⁻¹' ↑s)) (g : γ → β)
...
@[to_additive]
theorem prod_preimage_of_bij [CommMonoidₓ β] (f : α → γ) (s : Finsetₓ γ) (hf : Set.BijOn f (f ⁻¹' ↑s) ↑s) (g : γ → β) :
