...
[PrettyPrinter.parenthesize.input] (Command.declaration
     ...
     (Command.theorem
      ...
      (Command.declSig
       [(Term.implicitBinder "{" [`f] [":" (Function.Logic.Embedding.Basic.«term_↪_» `α " ↪ " `β)] "}")
...
theorem
  ...
  { f : α ↪ β } { s : Finset α } { t : Finset β } : s . map f ⊆ t ↔ s ⊆ t . Preimage f f . Injective . InjOn _
...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (u «expr ⊆ » t) -/
theorem subset_map_iff {f : α ↪ β} {s : Finset β} {t : Finset α} : s ⊆ t.map f ↔ ∃ (u : _)(_ : u ⊆ t), s = u.map f := by
  classical refine' ⟨fun h => ⟨_, preimage_subset h, _⟩, _⟩
    · rintro ⟨u, hut, rfl⟩
...
@[to_additive]
theorem prod_preimage [CommMonoid β] (f : α → γ) (s : Finset γ) (hf : Set.InjOn f (f ⁻¹' ↑s)) (g : γ → β)
    (hg : ∀ x ∈ s, x ∉ Set.range f → g x = 1) : (∏ x in s.Preimage f hf, g (f x)) = ∏ x in s, g x := by
  classical rw [prod_preimage', prod_filter_of_ne]
#align finset.prod_preimage Finset.prod_preimage
