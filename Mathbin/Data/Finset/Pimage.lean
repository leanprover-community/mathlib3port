...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `coe_to_finset [])
  (Command.declSig
   [(Term.explicitBinder "(" [`o] [":" (Term.app `Part [`α])] [] ")")
    (Term.instBinder "[" [] (Term.app `Decidable [`o.dom]) "]")]
   (Term.typeSpec
    ":"
    («term_=_»
     (Term.paren "(" [`o.to_finset [(Term.typeAscription ":" (Term.app `Set [`α]))]] ")")
     "="
     (Set.«term{_|_}» "{" `x "|" (Init.Core.«term_∈_» `x " ∈ " `o) "}"))))
  (Command.declValSimple
   ":="
   («term_$__» `Set.ext "$" (Term.fun "fun" (Term.basicFun [(Term.simpleBinder [`x] [])] "=>" `mem_to_finset)))
   [])
  []
  []))
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Term.fun "fun" (Term.basicFun [(Term.simpleBinder [`x] [])] "=>" `mem_to_finset))
  ...
  (Command.declSig
   [(Term.explicitBinder "(" [`h₁] [":" («term_=_» `s "=" `t)] [] ")")
    (Term.explicitBinder
     ...
     []
     ")")]
   (Term.typeSpec ":" («term_=_» (Term.app `s.pimage [`f]) "=" (Term.app `t.pimage [`g]))))
  (Command.declValSimple
   ":="
   (Term.byTactic
    "by"
...
theorem
  pimage_congr
  ( h₁ : s = t ) ( h₂ : ∀ , ∀ x ∈ t , ∀ , f x = g x ) : s.pimage f = t.pimage g
