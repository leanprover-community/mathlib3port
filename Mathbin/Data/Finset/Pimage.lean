...
def toFinset (o : Part α) [Decidable o.Dom] : Finsetₓ α :=
  o.toOption.toFinset
...
@[simp]
theorem to_finset_none [Decidable (none : Part α).Dom] : none.toFinset = (∅ : Finsetₓ α) := by simp [to_finset]
...
variable [DecidableEq β] {f g : α →. β} [∀ x, Decidable (f x).Dom] [∀ x, Decidable (g x).Dom] {s t : Finsetₓ α} {b : β}

/-- Image of `s : finset α` under a partially defined function `f : α →. β`. -/
def pimage (f : α →. β) [∀ x, Decidable (f x).Dom] (s : Finsetₓ α) : Finsetₓ β :=
...
@[simp]
theorem pimage_some (s : Finsetₓ α) (f : α → β) [∀ x, Decidable (Part.some <| f x).Dom] :
...
theorem pimage_eq_image_filter :
  ...
  ext x
  simp [Part.mem_eq, And.exists, -exists_propₓ]
...
theorem pimage_subset {t : Finsetₓ β} : s.pimage f ⊆ t ↔ ∀ x ∈ s, ∀ y ∈ f x, y ∈ t := by
