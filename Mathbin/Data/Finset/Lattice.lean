...
def sup (s : Finsetₓ β) (f : β → α) : α :=
  s.fold (· ⊔ ·) ⊥ f

variable {s s₁ s₂ : Finsetₓ β} {f g : β → α}

theorem sup_def : s.sup f = (s.1.map f).sup :=
  rfl

@[simp]
theorem sup_empty : (∅ : Finsetₓ β).sup f = ⊥ :=
  fold_empty
...
@[simp]
theorem sup_insert [DecidableEq β] {b : β} : (insert b s : Finsetₓ β).sup f = f b ⊔ s.sup f :=
  fold_insert_idem

theorem sup_image [DecidableEq β] (s : Finsetₓ γ) (f : γ → β) (g : β → α) : (s.Image f).sup g = s.sup (g ∘ f) :=
  fold_image_idem

@[simp]
theorem sup_map (s : Finsetₓ γ) (f : γ ↪ β) (g : β → α) : (s.map f).sup g = s.sup (g ∘ f) :=
  fold_map

@[simp]
theorem sup_singleton {b : β} : ({b} : Finsetₓ β).sup f = f b :=
  sup_singleton

theorem sup_union [DecidableEq β] : (s₁ ∪ s₂).sup f = s₁.sup f ⊔ s₂.sup f :=
  (Finsetₓ.induction_on s₁ (by rw [empty_union, sup_empty, bot_sup_eq])) fun a s has ih => by
    rw [insert_union, sup_insert, sup_insert, ih, sup_assoc]

theorem sup_sup : s.sup (f ⊔ g) = s.sup f ⊔ s.sup g := by
  refine' Finsetₓ.cons_induction_on s _ fun b t _ h => _
...
theorem sup_congr {f g : β → α} (hs : s₁ = s₂) (hfg : ∀ a ∈ s₂, f a = g a) : s₁.sup f = s₂.sup g := by
  subst hs <;> exact Finsetₓ.fold_congr hfg
...
@[simp]
theorem sup_bUnion [DecidableEq β] (s : Finsetₓ γ) (t : γ → Finsetₓ β) :
    (s.bUnion t).sup f = s.sup fun x => (t x).sup f :=
  eq_of_forall_ge_iffₓ fun c => by simp [@forall_swap _ β]

theorem sup_const {s : Finsetₓ β} (h : s.Nonempty) (c : α) : (s.sup fun _ => c) = c :=
  eq_of_forall_ge_iffₓ fun b => Finsetₓ.sup_le_iff.trans h.forall_const

@[simp]
theorem sup_bot (s : Finsetₓ β) : (s.sup fun _ => ⊥) = (⊥ : α) := by
...
theorem sup_le {a : α} : (∀ b ∈ s, f b ≤ a) → s.sup f ≤ a :=
  Finsetₓ.sup_le_iff.2

theorem le_sup {b : β} (hb : b ∈ s) : f b ≤ s.sup f :=
  Finsetₓ.sup_le_iff.1 le_rflₓ _ hb
...
theorem sup_comm (s : Finsetₓ β) (t : Finsetₓ γ) (f : β → γ → α) :
    (s.sup fun b => t.sup (f b)) = t.sup fun c => s.sup fun b => f b c := by
  refine' eq_of_forall_ge_iffₓ fun a => _
  simp_rw [Finsetₓ.sup_le_iff]
  exact ⟨fun h c hc b hb => h b hb c hc, fun h b hb c hc => h c hc b hb⟩

@[simp]
theorem sup_attach (s : Finsetₓ β) (f : β → α) : (s.attach.sup fun x => f x) = s.sup f :=
  (s.attach.sup_map (Function.Embedding.subtype _) f).symm.trans <| congr_arg _ attach_map_val

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
/-- See also `finset.product_bUnion`. -/
theorem sup_product_left (s : Finsetₓ β) (t : Finsetₓ γ) (f : β × γ → α) :
...
-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem sup_product_right (s : Finsetₓ β) (t : Finsetₓ γ) (f : β × γ → α) :
    (s ×ˢ t).sup f = t.sup fun i' => s.sup fun i => f ⟨i, i'⟩ := by rw [sup_product_left, sup_comm]

@[simp]
theorem sup_erase_bot [DecidableEq α] (s : Finsetₓ α) : (s.erase ⊥).sup id = s.sup id := by
  refine' (sup_mono (s.erase_subset _)).antisymm (Finsetₓ.sup_le_iff.2 fun a ha => _)
...
theorem sup_sdiff_right {α β : Type _} [GeneralizedBooleanAlgebra α] (s : Finsetₓ β) (f : β → α) (a : α) :
...
theorem comp_sup_eq_sup_comp [SemilatticeSup γ] [OrderBot γ] {s : Finsetₓ β} {f : β → α} (g : α → γ)
    (g_sup : ∀ x y, g (x ⊔ y) = g x ⊔ g y) (bot : g ⊥ = ⊥) : g (s.sup f) = s.sup (g ∘ f) :=
  Finsetₓ.cons_induction_on s bot fun c t hc ih => by rw [sup_cons, sup_cons, g_sup, ih]

/-- Computing `sup` in a subtype (closed under `sup`) is the same as computing it in `α`. -/
theorem sup_coe {P : α → Prop} {Pbot : P ⊥} {Psup : ∀ ⦃x y⦄, P x → P y → P (x ⊔ y)} (t : Finsetₓ β)
    (f : β → { x : α // P x }) :
    (@sup _ _ (Subtype.semilatticeSup Psup) (Subtype.orderBot Pbot) t f : α) = t.sup fun x => f x := by
  rw [comp_sup_eq_sup_comp coe] <;> intros <;> rfl

@[simp]
theorem sup_to_finset {α β} [DecidableEq β] (s : Finsetₓ α) (f : α → Multiset β) :
    (s.sup f).toFinset = s.sup fun x => (f x).toFinset :=
  comp_sup_eq_sup_comp Multiset.toFinset to_finset_union rfl

theorem _root_.list.foldr_sup_eq_sup_to_finset [DecidableEq α] (l : List α) : l.foldr (· ⊔ ·) ⊥ = l.toFinset.sup id :=
  by
  rw [← coe_fold_r, ← Multiset.fold_dedup_idem, sup_def, ← List.to_finset_coe, to_finset_val, Multiset.map_id]
  rfl

theorem subset_range_sup_succ (s : Finsetₓ ℕ) : s ⊆ range (s.sup id).succ := fun n hn =>
  mem_range.2 <| Nat.lt_succ_of_leₓ <| le_sup hn

theorem exists_nat_subset_range (s : Finsetₓ ℕ) : ∃ n : ℕ, s ⊆ range n :=
  ⟨_, s.subset_range_sup_succ⟩

theorem sup_induction {p : α → Prop} (hb : p ⊥) (hp : ∀ a₁, p a₁ → ∀ a₂, p a₂ → p (a₁ ⊔ a₂)) (hs : ∀ b ∈ s, p (f b)) :
    p (s.sup f) := by
  induction' s using Finsetₓ.cons_induction with c s hc ih
  · exact hb
...
theorem sup_le_of_le_directed {α : Type _} [SemilatticeSup α] [OrderBot α] (s : Set α) (hs : s.Nonempty)
    (hdir : DirectedOn (· ≤ ·) s) (t : Finsetₓ α) : (∀ x ∈ t, ∃ y ∈ s, x ≤ y) → ∃ x, x ∈ s ∧ t.sup id ≤ x := by
  classical
  apply Finsetₓ.induction_on t
  · simpa only [forall_prop_of_true, and_trueₓ, forall_prop_of_false, bot_le, not_false_iff, sup_empty, forall_true_iff,
      not_mem_empty]
    
  · intro a r har ih h
    have incs : ↑r ⊆ ↑(insert a r) := by
      rw [Finsetₓ.coe_subset]
      apply Finsetₓ.subset_insert
    -- x ∈ s is above the sup of r
    obtain ⟨x, ⟨hxs, hsx_sup⟩⟩ := ih fun x hx => h x <| incs hx
    -- y ∈ s is above a
    obtain ⟨y, hys, hay⟩ := h a (Finsetₓ.mem_insert_self a r)
...
theorem sup_mem (s : Set α) (w₁ : ⊥ ∈ s) (w₂ : ∀ (x y) (_ : x ∈ s) (_ : y ∈ s), x ⊔ y ∈ s) {ι : Type _} (t : Finsetₓ ι)
    (p : ι → α) (h : ∀ i ∈ t, p i ∈ s) : t.sup p ∈ s :=
  @sup_induction _ _ _ _ _ _ (· ∈ s) w₁ w₂ h

@[simp]
theorem sup_eq_bot_iff (f : β → α) (S : Finsetₓ β) : S.sup f = ⊥ ↔ ∀ s ∈ S, f s = ⊥ := by
  classical
  induction' S using Finsetₓ.induction with a S haS hi <;> simp [*]

end Sup

theorem sup_eq_supr [CompleteLattice β] (s : Finsetₓ α) (f : α → β) : s.sup f = ⨆ a ∈ s, f a :=
  le_antisymmₓ (Finsetₓ.sup_le fun a ha => le_supr_of_le a <| le_supr _ ha)
    (supr_le fun a => supr_le fun ha => le_sup ha)

theorem sup_id_eq_Sup [CompleteLattice α] (s : Finsetₓ α) : s.sup id = sup s := by simp [Sup_eq_supr, sup_eq_supr]

theorem sup_id_set_eq_sUnion (s : Finsetₓ (Set α)) : s.sup id = ⋃₀↑s :=
  sup_id_eq_Sup _

@[simp]
theorem sup_set_eq_bUnion (s : Finsetₓ α) (f : α → Set β) : s.sup f = ⋃ x ∈ s, f x :=
  sup_eq_supr _ _

theorem sup_eq_Sup_image [CompleteLattice β] (s : Finsetₓ α) (f : α → β) : s.sup f = sup (f '' s) := by
  classical
  rw [← Finsetₓ.coe_image, ← sup_id_eq_Sup, sup_image, Function.comp.left_id]
...
@[simp]
theorem inf_empty : (∅ : Finsetₓ β).inf f = ⊤ :=
  fold_empty
...
@[simp]
theorem inf_insert [DecidableEq β] {b : β} : (insert b s : Finsetₓ β).inf f = f b ⊓ s.inf f :=
  fold_insert_idem

theorem inf_image [DecidableEq β] (s : Finsetₓ γ) (f : γ → β) (g : β → α) : (s.Image f).inf g = s.inf (g ∘ f) :=
  fold_image_idem

@[simp]
theorem inf_map (s : Finsetₓ γ) (f : γ ↪ β) (g : β → α) : (s.map f).inf g = s.inf (g ∘ f) :=
  fold_map

@[simp]
theorem inf_singleton {b : β} : ({b} : Finsetₓ β).inf f = f b :=
  inf_singleton
...
@[simp]
theorem inf_bUnion [DecidableEq β] (s : Finsetₓ γ) (t : γ → Finsetₓ β) :
    (s.bUnion t).inf f = s.inf fun x => (t x).inf f :=
  @sup_bUnion αᵒᵈ _ _ _ _ _ _ _ _

theorem inf_const {s : Finsetₓ β} (h : s.Nonempty) (c : α) : (s.inf fun _ => c) = c :=
  @sup_const αᵒᵈ _ _ _ _ h _

@[simp]
theorem inf_top (s : Finsetₓ β) : (s.inf fun _ => ⊤) = (⊤ : α) :=
  @sup_bot αᵒᵈ _ _ _ _

theorem le_inf_iff {a : α} : a ≤ s.inf f ↔ ∀ b ∈ s, a ≤ f b :=
  @Finsetₓ.sup_le_iff αᵒᵈ _ _ _ _ _ _

theorem inf_le {b : β} (hb : b ∈ s) : s.inf f ≤ f b :=
  le_inf_iff.1 le_rflₓ _ hb
...
theorem inf_attach (s : Finsetₓ β) (f : β → α) : (s.attach.inf fun x => f x) = s.inf f :=
  @sup_attach αᵒᵈ _ _ _ _ _

theorem inf_comm (s : Finsetₓ β) (t : Finsetₓ γ) (f : β → γ → α) :
    (s.inf fun b => t.inf (f b)) = t.inf fun c => s.inf fun b => f b c :=
  @sup_comm αᵒᵈ _ _ _ _ _ _ _

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem inf_product_left (s : Finsetₓ β) (t : Finsetₓ γ) (f : β × γ → α) :
    (s ×ˢ t).inf f = s.inf fun i => t.inf fun i' => f ⟨i, i'⟩ :=
  @sup_product_left αᵒᵈ _ _ _ _ _ _ _

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem inf_product_right (s : Finsetₓ β) (t : Finsetₓ γ) (f : β × γ → α) :
    (s ×ˢ t).inf f = t.inf fun i' => s.inf fun i => f ⟨i, i'⟩ :=
  @sup_product_right αᵒᵈ _ _ _ _ _ _ _

@[simp]
theorem inf_erase_top [DecidableEq α] (s : Finsetₓ α) : (s.erase ⊤).inf id = s.inf id :=
  @sup_erase_bot αᵒᵈ _ _ _ _

theorem sup_sdiff_left {α β : Type _} [BooleanAlgebra α] (s : Finsetₓ β) (f : β → α) (a : α) :
...
theorem inf_sdiff_left {α β : Type _} [BooleanAlgebra α] {s : Finsetₓ β} (hs : s.Nonempty) (f : β → α) (a : α) :
    (s.inf fun b => a \ f b) = a \ s.sup f := by
  induction' hs using Finsetₓ.Nonempty.cons_induction with b b t _ _ h
  · rw [sup_singleton, inf_singleton]
    
  · rw [sup_cons, inf_cons, h, sdiff_sup]
    

theorem inf_sdiff_right {α β : Type _} [BooleanAlgebra α] {s : Finsetₓ β} (hs : s.Nonempty) (f : β → α) (a : α) :
...
theorem comp_inf_eq_inf_comp [SemilatticeInf γ] [OrderTop γ] {s : Finsetₓ β} {f : β → α} (g : α → γ)
...
theorem inf_coe {P : α → Prop} {Ptop : P ⊤} {Pinf : ∀ ⦃x y⦄, P x → P y → P (x ⊓ y)} (t : Finsetₓ β)
    (f : β → { x : α // P x }) :
    (@inf _ _ (Subtype.semilatticeInf Pinf) (Subtype.orderTop Ptop) t f : α) = t.inf fun x => f x :=
  @sup_coe αᵒᵈ _ _ _ _ Ptop Pinf t f

theorem _root_.list.foldr_inf_eq_inf_to_finset [DecidableEq α] (l : List α) : l.foldr (· ⊓ ·) ⊤ = l.toFinset.inf id :=
  by
  rw [← coe_fold_r, ← Multiset.fold_dedup_idem, inf_def, ← List.to_finset_coe, to_finset_val, Multiset.map_id]
  rfl
...
-- ./././Mathport/Syntax/Translate/Basic.lean:555:2: warning: expanding binder collection (x y «expr ∈ » s)
theorem inf_mem (s : Set α) (w₁ : ⊤ ∈ s) (w₂ : ∀ (x y) (_ : x ∈ s) (_ : y ∈ s), x ⊓ y ∈ s) {ι : Type _} (t : Finsetₓ ι)
    (p : ι → α) (h : ∀ i ∈ t, p i ∈ s) : t.inf p ∈ s :=
  @inf_induction _ _ _ _ _ _ (· ∈ s) w₁ w₂ h

@[simp]
theorem inf_eq_top_iff (f : β → α) (S : Finsetₓ β) : S.inf f = ⊤ ↔ ∀ s ∈ S, f s = ⊤ :=
  @Finsetₓ.sup_eq_bot_iff αᵒᵈ _ _ _ _ _

end Inf

@[simp]
theorem to_dual_sup [SemilatticeSup α] [OrderBot α] (s : Finsetₓ β) (f : β → α) :
    toDual (s.sup f) = s.inf (to_dual ∘ f) :=
  rfl

@[simp]
theorem to_dual_inf [SemilatticeInf α] [OrderTop α] (s : Finsetₓ β) (f : β → α) :
    toDual (s.inf f) = s.sup (to_dual ∘ f) :=
  rfl

@[simp]
theorem of_dual_sup [SemilatticeInf α] [OrderTop α] (s : Finsetₓ β) (f : β → αᵒᵈ) :
    ofDual (s.sup f) = s.inf (of_dual ∘ f) :=
  rfl

@[simp]
theorem of_dual_inf [SemilatticeSup α] [OrderBot α] (s : Finsetₓ β) (f : β → αᵒᵈ) :
...
variable [OrderBot α] {s : Finsetₓ β} {f : β → α} {a : α}

theorem sup_inf_distrib_left (s : Finsetₓ ι) (f : ι → α) (a : α) : a ⊓ s.sup f = s.sup fun i => a ⊓ f i := by
  induction' s using Finsetₓ.cons_induction with i s hi h
  · simp_rw [Finsetₓ.sup_empty, inf_bot_eq]
    
  · rw [sup_cons, sup_cons, inf_sup_left, h]
    

theorem sup_inf_distrib_right (s : Finsetₓ ι) (f : ι → α) (a : α) : s.sup f ⊓ a = s.sup fun i => f i ⊓ a := by
...
theorem inf_sup_distrib_left (s : Finsetₓ ι) (f : ι → α) (a : α) : a ⊔ s.inf f = s.inf fun i => a ⊔ f i :=
  @sup_inf_distrib_left αᵒᵈ _ _ _ _ _ _

theorem inf_sup_distrib_right (s : Finsetₓ ι) (f : ι → α) (a : α) : s.inf f ⊔ a = s.inf fun i => f i ⊔ a :=
...
variable [OrderBot α] {s : Finsetₓ ι} {f : ι → α} {a : α}

theorem comp_sup_eq_sup_comp_of_is_total [SemilatticeSup β] [OrderBot β] (g : α → β) (mono_g : Monotoneₓ g)
    (bot : g ⊥ = ⊥) : g (s.sup f) = s.sup (g ∘ f) :=
  comp_sup_eq_sup_comp g mono_g.map_sup bot

@[simp]
protected theorem le_sup_iff (ha : ⊥ < a) : a ≤ s.sup f ↔ ∃ b ∈ s, a ≤ f b :=
  ⟨Finsetₓ.cons_induction_on s (fun h => absurd h (not_le_of_ltₓ ha)) fun c t hc ih => by
      simpa using
...
protected theorem lt_sup_iff : a < s.sup f ↔ ∃ b ∈ s, a < f b :=
  ⟨Finsetₓ.cons_induction_on s (fun h => absurd h not_lt_bot) fun c t hc ih => by
      simpa using
        @Or.ndrec _ _ (∃ b, (b = c ∨ b ∈ t) ∧ a < f b) (fun h => ⟨c, Or.inl rfl, h⟩) fun h =>
          let ⟨b, hb, hlt⟩ := ih h
          ⟨b, Or.inr hb, hlt⟩,
...
protected theorem sup_lt_iff (ha : ⊥ < a) : s.sup f < a ↔ ∀ b ∈ s, f b < a :=
  ⟨fun hs b hb => lt_of_le_of_ltₓ (le_sup hb) hs,
    Finsetₓ.cons_induction_on s (fun _ => ha) fun c t hc => by
...
variable [OrderTop α] {s : Finsetₓ ι} {f : ι → α} {a : α}

theorem comp_inf_eq_inf_comp_of_is_total [SemilatticeInf β] [OrderTop β] (g : α → β) (mono_g : Monotoneₓ g)
    (top : g ⊤ = ⊤) : g (s.inf f) = s.inf (g ∘ f) :=
  comp_inf_eq_inf_comp g mono_g.map_inf top

@[simp]
protected theorem inf_le_iff (ha : a < ⊤) : s.inf f ≤ a ↔ ∃ b ∈ s, f b ≤ a :=
  @Finsetₓ.le_sup_iff αᵒᵈ _ _ _ _ _ _ ha

@[simp]
protected theorem inf_lt_iff : s.inf f < a ↔ ∃ b ∈ s, f b < a :=
  @Finsetₓ.lt_sup_iff αᵒᵈ _ _ _ _ _ _

@[simp]
protected theorem lt_inf_iff (ha : a < ⊤) : a < s.inf f ↔ ∀ b ∈ s, a < f b :=
  @Finsetₓ.sup_lt_iff αᵒᵈ _ _ _ _ _ _ ha

end OrderTop

end LinearOrderₓ

theorem inf_eq_infi [CompleteLattice β] (s : Finsetₓ α) (f : α → β) : s.inf f = ⨅ a ∈ s, f a :=
  @sup_eq_supr _ βᵒᵈ _ _ _

theorem inf_id_eq_Inf [CompleteLattice α] (s : Finsetₓ α) : s.inf id = inf s :=
  @sup_id_eq_Sup αᵒᵈ _ _

theorem inf_id_set_eq_sInter (s : Finsetₓ (Set α)) : s.inf id = ⋂₀ ↑s :=
  inf_id_eq_Inf _

@[simp]
theorem inf_set_eq_bInter (s : Finsetₓ α) (f : α → Set β) : s.inf f = ⋂ x ∈ s, f x :=
  inf_eq_infi _ _

theorem inf_eq_Inf_image [CompleteLattice β] (s : Finsetₓ α) (f : α → β) : s.inf f = inf (f '' s) :=
...
theorem sup_of_mem {s : Finsetₓ β} (f : β → α) {b : β} (h : b ∈ s) : ∃ a : α, s.sup (coe ∘ f : β → WithBot α) = ↑a :=
...
def sup' (s : Finsetₓ β) (H : s.Nonempty) (f : β → α) : α :=
  WithBot.unbot (s.sup (coe ∘ f)) (by simpa using H)

variable {s : Finsetₓ β} (H : s.Nonempty) (f : β → α)
...
@[simp]
theorem sup'_singleton {b : β} {h : ({b} : Finsetₓ β).Nonempty} : ({b} : Finsetₓ β).sup' h f = f b :=
  rfl
...
theorem sup'_bUnion [DecidableEq β] {s : Finsetₓ γ} (Hs : s.Nonempty) {t : γ → Finsetₓ β} (Ht : ∀ b, (t b).Nonempty) :
    (s.bUnion t).sup' (Hs.bUnion fun b _ => Ht b) f = s.sup' Hs fun b => (t b).sup' (Ht b) f :=
  eq_of_forall_ge_iffₓ fun c => by simp [@forall_swap _ β]

theorem comp_sup'_eq_sup'_comp [SemilatticeSup γ] {s : Finsetₓ β} (H : s.Nonempty) {f : β → α} (g : α → γ)
...
-- ./././Mathport/Syntax/Translate/Basic.lean:555:2: warning: expanding binder collection (x y «expr ∈ » s)
theorem sup'_mem (s : Set α) (w : ∀ (x y) (_ : x ∈ s) (_ : y ∈ s), x ⊔ y ∈ s) {ι : Type _} (t : Finsetₓ ι)
    (H : t.Nonempty) (p : ι → α) (h : ∀ i ∈ t, p i ∈ s) : t.sup' H p ∈ s :=
  sup'_induction H p w h

@[congr]
theorem sup'_congr {t : Finsetₓ β} {f g : β → α} (h₁ : s = t) (h₂ : ∀ x ∈ s, f x = g x) :
...
@[simp]
theorem sup'_map {s : Finsetₓ γ} {f : γ ↪ β} (g : β → α) (hs : (s.map f).Nonempty)
    (hs' : s.Nonempty := Finsetₓ.map_nonempty.mp hs) : (s.map f).sup' hs g = s.sup' hs' (g ∘ f) := by
...
theorem inf_of_mem {s : Finsetₓ β} (f : β → α) {b : β} (h : b ∈ s) : ∃ a : α, s.inf (coe ∘ f : β → WithTop α) = ↑a :=
...
def inf' (s : Finsetₓ β) (H : s.Nonempty) (f : β → α) : α :=
  WithTop.untop (s.inf (coe ∘ f)) (by simpa using H)

variable {s : Finsetₓ β} (H : s.Nonempty) (f : β → α) {a : α} {b : β}
...
@[simp]
theorem inf'_singleton {b : β} {h : ({b} : Finsetₓ β).Nonempty} : ({b} : Finsetₓ β).inf' h f = f b :=
  rfl
...
theorem inf'_bUnion [DecidableEq β] {s : Finsetₓ γ} (Hs : s.Nonempty) {t : γ → Finsetₓ β} (Ht : ∀ b, (t b).Nonempty) :
    (s.bUnion t).inf' (Hs.bUnion fun b _ => Ht b) f = s.inf' Hs fun b => (t b).inf' (Ht b) f :=
  @sup'_bUnion αᵒᵈ _ _ _ _ _ _ Hs _ Ht

theorem comp_inf'_eq_inf'_comp [SemilatticeInf γ] {s : Finsetₓ β} (H : s.Nonempty) {f : β → α} (g : α → γ)
...
-- ./././Mathport/Syntax/Translate/Basic.lean:555:2: warning: expanding binder collection (x y «expr ∈ » s)
theorem inf'_mem (s : Set α) (w : ∀ (x y) (_ : x ∈ s) (_ : y ∈ s), x ⊓ y ∈ s) {ι : Type _} (t : Finsetₓ ι)
    (H : t.Nonempty) (p : ι → α) (h : ∀ i ∈ t, p i ∈ s) : t.inf' H p ∈ s :=
  inf'_induction H p w h

@[congr]
theorem inf'_congr {t : Finsetₓ β} {f g : β → α} (h₁ : s = t) (h₂ : ∀ x ∈ s, f x = g x) :
    s.inf' H f = t.inf' (h₁ ▸ H) g :=
  @sup'_congr αᵒᵈ _ _ _ H _ _ _ h₁ h₂

@[simp]
theorem inf'_map {s : Finsetₓ γ} {f : γ ↪ β} (g : β → α) (hs : (s.map f).Nonempty)
    (hs' : s.Nonempty := Finsetₓ.map_nonempty.mp hs) : (s.map f).inf' hs g = s.inf' hs' (g ∘ f) :=
...
theorem sup'_eq_sup {s : Finsetₓ β} (H : s.Nonempty) (f : β → α) : s.sup' H f = s.sup f :=
...
theorem sup_closed_of_sup_closed {s : Set α} (t : Finsetₓ α) (htne : t.Nonempty) (h_subset : ↑t ⊆ s)
    (h : ∀ (a b) (_ : a ∈ s) (_ : b ∈ s), a ⊔ b ∈ s) : t.sup id ∈ s :=
  sup'_eq_sup htne id ▸ sup'_induction _ _ h h_subset

theorem coe_sup_of_nonempty {s : Finsetₓ β} (h : s.Nonempty) (f : β → α) : (↑(s.sup f) : WithBot α) = s.sup (coe ∘ f) :=
...
theorem inf'_eq_inf {s : Finsetₓ β} (H : s.Nonempty) (f : β → α) : s.inf' H f = s.inf f :=
  @sup'_eq_sup αᵒᵈ _ _ _ _ H f

-- ./././Mathport/Syntax/Translate/Basic.lean:555:2: warning: expanding binder collection (a b «expr ∈ » s)
theorem inf_closed_of_inf_closed {s : Set α} (t : Finsetₓ α) (htne : t.Nonempty) (h_subset : ↑t ⊆ s)
    (h : ∀ (a b) (_ : a ∈ s) (_ : b ∈ s), a ⊓ b ∈ s) : t.inf id ∈ s :=
  @sup_closed_of_sup_closed αᵒᵈ _ _ _ t htne h_subset h

theorem coe_inf_of_nonempty {s : Finsetₓ β} (h : s.Nonempty) (f : β → α) :
...
@[simp]
protected theorem sup_apply (s : Finsetₓ α) (f : α → ∀ b : β, C b) (b : β) : s.sup f b = s.sup fun a => f a b :=
...
@[simp]
protected theorem inf_apply (s : Finsetₓ α) (f : α → ∀ b : β, C b) (b : β) : s.inf f b = s.inf fun a => f a b :=
  @Finsetₓ.sup_apply _ _ (fun b => (C b)ᵒᵈ) _ _ s f b
...
@[simp]
protected theorem sup'_apply {s : Finsetₓ α} (H : s.Nonempty) (f : α → ∀ b : β, C b) (b : β) :
...
@[simp]
protected theorem inf'_apply {s : Finsetₓ α} (H : s.Nonempty) (f : α → ∀ b : β, C b) (b : β) :
    s.inf' H f b = s.inf' H fun a => f a b :=
  @Finsetₓ.sup'_apply _ _ (fun b => (C b)ᵒᵈ) _ _ H f b

end Inf'

@[simp]
theorem to_dual_sup' [SemilatticeSup α] {s : Finsetₓ ι} (hs : s.Nonempty) (f : ι → α) :
    toDual (s.sup' hs f) = s.inf' hs (to_dual ∘ f) :=
  rfl

@[simp]
theorem to_dual_inf' [SemilatticeInf α] {s : Finsetₓ ι} (hs : s.Nonempty) (f : ι → α) :
    toDual (s.inf' hs f) = s.sup' hs (to_dual ∘ f) :=
  rfl

@[simp]
theorem of_dual_sup' [SemilatticeInf α] {s : Finsetₓ ι} (hs : s.Nonempty) (f : ι → αᵒᵈ) :
    ofDual (s.sup' hs f) = s.inf' hs (of_dual ∘ f) :=
  rfl

@[simp]
theorem of_dual_inf' [SemilatticeSup α] {s : Finsetₓ ι} (hs : s.Nonempty) (f : ι → αᵒᵈ) :
    ofDual (s.inf' hs f) = s.sup' hs (of_dual ∘ f) :=
  rfl

section LinearOrderₓ

variable [LinearOrderₓ α] {s : Finsetₓ ι} (H : s.Nonempty) {f : ι → α} {a : α}

@[simp]
theorem le_sup'_iff : a ≤ s.sup' H f ↔ ∃ b ∈ s, a ≤ f b := by
  rw [← WithBot.coe_le_coe, coe_sup', Finsetₓ.le_sup_iff (WithBot.bot_lt_coe a)]
  exact bex_congr fun b hb => WithBot.coe_le_coe

@[simp]
theorem lt_sup'_iff : a < s.sup' H f ↔ ∃ b ∈ s, a < f b := by
  rw [← WithBot.coe_lt_coe, coe_sup', Finsetₓ.lt_sup_iff]
  exact bex_congr fun b hb => WithBot.coe_lt_coe

@[simp]
theorem sup'_lt_iff : s.sup' H f < a ↔ ∀ i ∈ s, f i < a := by
  rw [← WithBot.coe_lt_coe, coe_sup', Finsetₓ.sup_lt_iff (WithBot.bot_lt_coe a)]
...
theorem exists_mem_eq_sup [OrderBot α] (s : Finsetₓ ι) (h : s.Nonempty) (f : ι → α) : ∃ i, i ∈ s ∧ s.sup f = f i :=
  sup'_eq_sup h f ▸ exists_mem_eq_sup' h f

theorem exists_mem_eq_inf [OrderTop α] (s : Finsetₓ ι) (h : s.Nonempty) (f : ι → α) : ∃ i, i ∈ s ∧ s.inf f = f i :=
...
`s.max'`. -/
protected def max (s : Finsetₓ α) : WithBot α :=
  sup s coe

theorem max_eq_sup_coe {s : Finsetₓ α} : s.max = s.sup coe :=
  rfl

theorem max_eq_sup_with_bot (s : Finsetₓ α) : s.max = sup s coe :=
  rfl

@[simp]
theorem max_empty : (∅ : Finsetₓ α).max = ⊥ :=
  rfl

@[simp]
theorem max_insert {a : α} {s : Finsetₓ α} : (insert a s).max = max a s.max :=
  fold_insert_idem

@[simp]
theorem max_singleton {a : α} : Finsetₓ.max {a} = (a : WithBot α) := by
  rw [← insert_emptyc_eq]
  exact max_insert

theorem max_of_mem {s : Finsetₓ α} {a : α} (h : a ∈ s) : ∃ b : α, s.max = b :=
  (@le_sup (WithBot α) _ _ _ _ _ _ h _ rfl).imp fun b => Exists.fst

theorem max_of_nonempty {s : Finsetₓ α} (h : s.Nonempty) : ∃ a : α, s.max = a :=
  let ⟨a, ha⟩ := h
  max_of_mem ha

theorem max_eq_bot {s : Finsetₓ α} : s.max = ⊥ ↔ s = ∅ :=
  ⟨fun h =>
...
theorem mem_of_max {s : Finsetₓ α} : ∀ {a : α}, s.max = a → a ∈ s :=
  Finsetₓ.induction_on s (fun _ H => by cases H)
...
theorem le_max {a : α} {s : Finsetₓ α} (as : a ∈ s) : ↑a ≤ s.max :=
  le_sup as

theorem not_mem_of_max_lt_coe {a : α} {s : Finsetₓ α} (h : s.max < a) : a ∉ s :=
  mt le_max h.not_le

theorem le_max_of_eq {s : Finsetₓ α} {a b : α} (h₁ : a ∈ s) (h₂ : s.max = b) : a ≤ b :=
  WithBot.coe_le_coe.mp <| (le_max h₁).trans h₂.le

theorem not_mem_of_max_lt {s : Finsetₓ α} {a b : α} (h₁ : b < a) (h₂ : s.max = ↑b) : a ∉ s :=
  Finsetₓ.not_mem_of_max_lt_coe <| h₂.trans_lt <| WithBot.coe_lt_coe.mpr h₁

theorem max_mono {s t : Finsetₓ α} (st : s ⊆ t) : s.max ≤ t.max :=
  sup_mono st

theorem max_le {M : WithBot α} {s : Finsetₓ α} (st : ∀ a : α, a ∈ s → (a : WithBot α) ≤ M) : s.max ≤ M :=
  sup_le st

/-- Let `s` be a finset in a linear order. Then `s.min` is the minimum of `s` if `s` is not empty,
and `⊤` otherwise. It belongs to `with_top α`. If you want to get an element of `α`, see
`s.min'`. -/
protected def min (s : Finsetₓ α) : WithTop α :=
  inf s coe

theorem min_eq_inf_with_top (s : Finsetₓ α) : s.min = inf s coe :=
  rfl

@[simp]
theorem min_empty : (∅ : Finsetₓ α).min = ⊤ :=
  rfl

@[simp]
theorem min_insert {a : α} {s : Finsetₓ α} : (insert a s).min = min (↑a) s.min :=
  fold_insert_idem

@[simp]
theorem min_singleton {a : α} : Finsetₓ.min {a} = (a : WithTop α) := by
  rw [← insert_emptyc_eq]
  exact min_insert

theorem min_of_mem {s : Finsetₓ α} {a : α} (h : a ∈ s) : ∃ b : α, s.min = b :=
  (@inf_le (WithTop α) _ _ _ _ _ _ h _ rfl).imp fun b => Exists.fst

theorem min_of_nonempty {s : Finsetₓ α} (h : s.Nonempty) : ∃ a : α, s.min = a :=
  let ⟨a, ha⟩ := h
  min_of_mem ha

theorem min_eq_top {s : Finsetₓ α} : s.min = ⊤ ↔ s = ∅ :=
  ⟨fun h =>
    s.eq_empty_or_nonempty.elim id fun H => by
...
theorem mem_of_min {s : Finsetₓ α} : ∀ {a : α}, s.min = a → a ∈ s :=
  @mem_of_max αᵒᵈ _ s

theorem min_le {a : α} {s : Finsetₓ α} (as : a ∈ s) : s.min ≤ a :=
  inf_le as

theorem not_mem_of_coe_lt_min {a : α} {s : Finsetₓ α} (h : ↑a < s.min) : a ∉ s :=
  mt min_le h.not_le

theorem min_le_of_eq {s : Finsetₓ α} {a b : α} (h₁ : b ∈ s) (h₂ : s.min = a) : a ≤ b :=
  WithTop.coe_le_coe.mp <| h₂.Ge.trans (min_le h₁)

theorem not_mem_of_lt_min {s : Finsetₓ α} {a b : α} (h₁ : a < b) (h₂ : s.min = ↑b) : a ∉ s :=
  Finsetₓ.not_mem_of_coe_lt_min <| (WithTop.coe_lt_coe.mpr h₁).trans_eq h₂.symm

theorem min_mono {s t : Finsetₓ α} (st : s ⊆ t) : t.min ≤ s.min :=
  inf_mono st

theorem le_min {m : WithTop α} {s : Finsetₓ α} (st : ∀ a : α, a ∈ s → m ≤ a) : m ≤ s.min :=
  le_inf st

/-- Given a nonempty finset `s` in a linear order `α`, then `s.min' h` is its minimum, as an
element of `α`, where `h` is a proof of nonemptiness. Without this assumption, use instead `s.min`,
taking values in `with_top α`. -/
def min' (s : Finsetₓ α) (H : s.Nonempty) : α :=
  inf' s H id
...
def max' (s : Finsetₓ α) (H : s.Nonempty) : α :=
  sup' s H id

variable (s : Finsetₓ α) (H : s.Nonempty) {x : α}

theorem min'_mem : s.min' H ∈ s :=
  mem_of_min <| by simp [min', Finsetₓ.min]
...
/-- `{a}.min' _` is `a`. -/
@[simp]
theorem min'_singleton (a : α) : ({a} : Finsetₓ α).min' (singleton_nonempty _) = a := by simp [min']

theorem max'_mem : s.max' H ∈ s :=
  mem_of_max <| by simp [max', Finsetₓ.max]
...
theorem max'_eq_sup' : s.max' H = s.sup' H id :=
  eq_of_forall_ge_iffₓ fun a => (max'_le_iff _ _).trans (sup'_le_iff _ _).symm
...
/-- `{a}.max' _` is `a`. -/
@[simp]
theorem max'_singleton (a : α) : ({a} : Finsetₓ α).max' (singleton_nonempty _) = a := by simp [max']
...
theorem min'_lt_max'_of_card (h₂ : 1 < card s) :
    s.min' (Finsetₓ.card_pos.mp <| lt_transₓ zero_lt_one h₂) <
      s.max' (Finsetₓ.card_pos.mp <| lt_transₓ zero_lt_one h₂) :=
  by
  rcases one_lt_card.1 h₂ with ⟨a, ha, b, hb, hab⟩
  exact s.min'_lt_max' ha hb hab

theorem map_of_dual_min (s : Finsetₓ αᵒᵈ) : s.min.map ofDual = (s.Image ofDual).max := by
  rw [max_eq_sup_with_bot, sup_image]
  exact congr_fun Option.map_id _

theorem map_of_dual_max (s : Finsetₓ αᵒᵈ) : s.max.map ofDual = (s.Image ofDual).min := by
  rw [min_eq_inf_with_top, inf_image]
  exact congr_fun Option.map_id _

theorem map_to_dual_min (s : Finsetₓ α) : s.min.map toDual = (s.Image toDual).max := by
  rw [max_eq_sup_with_bot, sup_image]
  exact congr_fun Option.map_id _

theorem map_to_dual_max (s : Finsetₓ α) : s.max.map toDual = (s.Image toDual).min := by
  rw [min_eq_inf_with_top, inf_image]
  exact congr_fun Option.map_id _

theorem of_dual_min' {s : Finsetₓ αᵒᵈ} (hs : s.Nonempty) : ofDual (min' s hs) = max' (s.Image ofDual) (hs.Image _) := by
  convert rfl
  exact image_id

theorem of_dual_max' {s : Finsetₓ αᵒᵈ} (hs : s.Nonempty) : ofDual (max' s hs) = min' (s.Image ofDual) (hs.Image _) := by
  convert rfl
  exact image_id

theorem to_dual_min' {s : Finsetₓ α} (hs : s.Nonempty) : toDual (min' s hs) = max' (s.Image toDual) (hs.Image _) := by
  convert rfl
  exact image_id

theorem to_dual_max' {s : Finsetₓ α} (hs : s.Nonempty) : toDual (max' s hs) = min' (s.Image toDual) (hs.Image _) := by
  convert rfl
  exact image_id

theorem max'_subset {s t : Finsetₓ α} (H : s.Nonempty) (hst : s ⊆ t) : s.max' H ≤ t.max' (H.mono hst) :=
  le_max' _ _ (hst (s.max'_mem H))

theorem min'_subset {s t : Finsetₓ α} (H : s.Nonempty) (hst : s ⊆ t) : t.min' (H.mono hst) ≤ s.min' H :=
  min'_le _ _ (hst (s.min'_mem H))

theorem max'_insert (a : α) (s : Finsetₓ α) (H : s.Nonempty) :
...
theorem min'_insert (a : α) (s : Finsetₓ α) (H : s.Nonempty) :
...
theorem lt_max'_of_mem_erase_max' [DecidableEq α] {a : α} (ha : a ∈ s.erase (s.max' H)) : a < s.max' H :=
  lt_of_le_of_neₓ (le_max' _ _ (mem_of_mem_erase ha)) <| ne_of_mem_of_not_memₓ ha <| not_mem_erase _ _
...
@[simp]
theorem max'_image [LinearOrderₓ β] {f : α → β} (hf : Monotoneₓ f) (s : Finsetₓ α) (h : (s.Image f).Nonempty) :
...
@[simp]
theorem min'_image [LinearOrderₓ β] {f : α → β} (hf : Monotoneₓ f) (s : Finsetₓ α) (h : (s.Image f).Nonempty) :
...
theorem coe_max' {s : Finsetₓ α} (hs : s.Nonempty) : ↑(s.max' hs) = s.max :=
  coe_sup' hs id

theorem coe_min' {s : Finsetₓ α} (hs : s.Nonempty) : ↑(s.min' hs) = s.min :=
  coe_inf' hs id

theorem max'_erase_ne_self {s : Finsetₓ α} (s0 : (s.erase x).Nonempty) : (s.erase x).max' s0 ≠ x :=
  ne_of_mem_erase (max'_mem _ s0)

theorem min'_erase_ne_self {s : Finsetₓ α} (s0 : (s.erase x).Nonempty) : (s.erase x).min' s0 ≠ x :=
  ne_of_mem_erase (min'_mem _ s0)

theorem max_erase_ne_self {s : Finsetₓ α} : (s.erase x).max ≠ x := by
...
theorem min_erase_ne_self {s : Finsetₓ α} : (s.erase x).min ≠ x := by convert @max_erase_ne_self αᵒᵈ _ _ _
...
@[elabAsElim]
theorem induction_on_max [DecidableEq α] {p : Finsetₓ α → Prop} (s : Finsetₓ α) (h0 : p ∅)
    (step : ∀ a s, (∀ x ∈ s, x < a) → p s → p (insert a s)) : p s := by
  induction' s using Finsetₓ.strongInductionOn with s ihs
...
* for every `s : finset α` and an element `a` strictly less than all elements of `s`, `p s`
  implies `p (insert a s)`. -/
@[elabAsElim]
theorem induction_on_min [DecidableEq α] {p : Finsetₓ α → Prop} (s : Finsetₓ α) (h0 : p ∅)
...
@[elabAsElim]
theorem induction_on_max_value [DecidableEq ι] (f : ι → α) {p : Finsetₓ ι → Prop} (s : Finsetₓ ι) (h0 : p ∅)
  ...
  · have H : (s.image f).max' hne ∈ s.image f := max'_mem (s.image f) hne
    simp only [mem_image, exists_propₓ] at H
...
@[elabAsElim]
theorem induction_on_min_value [DecidableEq ι] (f : ι → α) {p : Finsetₓ ι → Prop} (s : Finsetₓ ι) (h0 : p ∅)
...
theorem exists_max_image (s : Finsetₓ β) (f : β → α) (h : s.Nonempty) : ∃ x ∈ s, ∀ x' ∈ s, f x' ≤ f x := by
  cases' max_of_nonempty (h.image f) with y hy
  rcases mem_image.mp (mem_of_max hy) with ⟨x, hx, rfl⟩
  exact ⟨x, hx, fun x' hx' => le_max_of_eq (mem_image_of_mem f hx') hy⟩

theorem exists_min_image (s : Finsetₓ β) (f : β → α) (h : s.Nonempty) : ∃ x ∈ s, ∀ x' ∈ s, f x ≤ f x' :=
...
theorem map_finset_sup [DecidableEq α] [DecidableEq β] (s : Finsetₓ γ) (f : γ → Multiset β) (g : β → α)
    (hg : Function.Injective g) : map g (s.sup f) = s.sup (map g ∘ f) :=
  Finsetₓ.comp_sup_eq_sup_comp _ (fun _ _ => map_union hg) (map_zero _)

theorem count_finset_sup [DecidableEq β] (s : Finsetₓ α) (f : α → Multiset β) (b : β) :
  ...
  · intro i s his ih
    rw [Finsetₓ.sup_insert, sup_eq_union, count_union, Finsetₓ.sup_insert, ih]
    rfl
    

theorem mem_sup {α β} [DecidableEq β] {s : Finsetₓ α} {f : α → Multiset β} {x : β} : x ∈ s.sup f ↔ ∃ v ∈ s, x ∈ f v :=
  by
  classical
  ...
  · intro a s has hxs
    rw [Finsetₓ.sup_insert, Multiset.sup_eq_union, Multiset.mem_union]
    constructor
    · intro hxi
      cases' hxi with hf hf
      · refine' ⟨a, _, hf⟩
        simp only [true_orₓ, eq_self_iff_true, Finsetₓ.mem_insert]
        
      · rcases hxs.mp hf with ⟨v, hv, hfv⟩
        refine' ⟨v, _, hfv⟩
        simp only [hv, or_trueₓ, Finsetₓ.mem_insert]
        
      
    · rintro ⟨v, hv, hfv⟩
      rw [Finsetₓ.mem_insert] at hv
...
theorem mem_sup {α β} [DecidableEq β] {s : Finsetₓ α} {f : α → Finsetₓ β} {x : β} : x ∈ s.sup f ↔ ∃ v ∈ s, x ∈ f v := by
  change _ ↔ ∃ v ∈ s, x ∈ (f v).val
  rw [← Multiset.mem_sup, ← Multiset.mem_to_finset, sup_to_finset]
  simp_rw [val_to_finset]

theorem sup_eq_bUnion {α β} [DecidableEq β] (s : Finsetₓ α) (t : α → Finsetₓ β) : s.sup t = s.bUnion t := by
  ext
  rw [mem_sup, mem_bUnion]

@[simp]
theorem sup_singleton'' [DecidableEq α] (s : Finsetₓ β) (f : β → α) : (s.sup fun b => {f b}) = s.Image f := by
  ext a
  rw [mem_sup, mem_image]
  simp only [mem_singleton, eq_comm]

@[simp]
theorem sup_singleton' [DecidableEq α] (s : Finsetₓ α) : s.sup singleton = s :=
...
theorem supr_eq_supr_finset (s : ι → α) : (⨆ i, s i) = ⨆ t : Finsetₓ ι, ⨆ i ∈ t, s i := by
  classical
  exact
    le_antisymmₓ (supr_le fun b => le_supr_of_le {b} <| le_supr_of_le b <| le_supr_of_le (by simp) <| le_rflₓ)
...
theorem supr_eq_supr_finset' (s : ι' → α) : (⨆ i, s i) = ⨆ t : Finsetₓ (Plift ι'), ⨆ i ∈ t, s (Plift.down i) := by
...
that works for `ι : Sort*`. -/
theorem infi_eq_infi_finset (s : ι → α) : (⨅ i, s i) = ⨅ (t : Finsetₓ ι) (i ∈ t), s i :=
  @supr_eq_supr_finset αᵒᵈ _ _ _

/-- Infimum of `s i`, `i : ι`, is equal to the infimum over `t : finset ι` of infima
`⨅ i ∈ t, s i`. This version works for `ι : Sort*`. See `infi_eq_infi_finset` for a version
that assumes `ι : Type*` but has no `plift`s. -/
theorem infi_eq_infi_finset' (s : ι' → α) : (⨅ i, s i) = ⨅ t : Finsetₓ (Plift ι'), ⨅ i ∈ t, s (Plift.down i) :=
  @supr_eq_supr_finset' αᵒᵈ _ _ _
...
theorem Union_eq_Union_finset (s : ι → Set α) : (⋃ i, s i) = ⋃ t : Finsetₓ ι, ⋃ i ∈ t, s i :=
...
theorem Union_eq_Union_finset' (s : ι' → Set α) : (⋃ i, s i) = ⋃ t : Finsetₓ (Plift ι'), ⋃ i ∈ t, s (Plift.down i) :=
  supr_eq_supr_finset' s
...
theorem Inter_eq_Inter_finset (s : ι → Set α) : (⋂ i, s i) = ⋂ t : Finsetₓ ι, ⋂ i ∈ t, s i :=
...
hand side. -/
theorem Inter_eq_Inter_finset' (s : ι' → Set α) : (⋂ i, s i) = ⋂ t : Finsetₓ (Plift ι'), ⋂ i ∈ t, s (Plift.down i) :=
  infi_eq_infi_finset' s
...
theorem sup_mul_le_mul_sup_of_nonneg [LinearOrderedSemiring α] [OrderBot α] {a b : ι → α} (s : Finsetₓ ι)
    (ha : ∀ i ∈ s, 0 ≤ a i) (hb : ∀ i ∈ s, 0 ≤ b i) : s.sup (a * b) ≤ s.sup a * s.sup b :=
  Finsetₓ.sup_le fun i hi => mul_le_mul (le_sup hi) (le_sup hi) (hb _ hi) ((ha _ hi).trans <| le_sup hi)

theorem mul_inf_le_inf_mul_of_nonneg [LinearOrderedSemiring α] [OrderTop α] {a b : ι → α} (s : Finsetₓ ι)
    (ha : ∀ i ∈ s, 0 ≤ a i) (hb : ∀ i ∈ s, 0 ≤ b i) : s.inf a * s.inf b ≤ s.inf (a * b) :=
  Finsetₓ.le_inf fun i hi => mul_le_mul (inf_le hi) (inf_le hi) (Finsetₓ.le_inf hb) (ha i hi)

theorem sup'_mul_le_mul_sup'_of_nonneg [LinearOrderedSemiring α] {a b : ι → α} (s : Finsetₓ ι) (H : s.Nonempty)
...
theorem inf'_mul_le_mul_inf'_of_nonneg [LinearOrderedSemiring α] {a b : ι → α} (s : Finsetₓ ι) (H : s.Nonempty)
...
theorem supr_coe [HasSupₓ β] (f : α → β) (s : Finsetₓ α) : (⨆ x ∈ (↑s : Set α), f x) = ⨆ x ∈ s, f x :=
  rfl

theorem infi_coe [HasInfₓ β] (f : α → β) (s : Finsetₓ α) : (⨅ x ∈ (↑s : Set α), f x) = ⨅ x ∈ s, f x :=
  rfl

variable [CompleteLattice β]

theorem supr_singleton (a : α) (s : α → β) : (⨆ x ∈ ({a} : Finsetₓ α), s x) = s a := by simp

theorem infi_singleton (a : α) (s : α → β) : (⨅ x ∈ ({a} : Finsetₓ α), s x) = s a := by simp

theorem supr_option_to_finset (o : Option α) (f : α → β) : (⨆ x ∈ o.toFinset, f x) = ⨆ x ∈ o, f x := by simp
...
theorem supr_union {f : α → β} {s t : Finsetₓ α} : (⨆ x ∈ s ∪ t, f x) = (⨆ x ∈ s, f x) ⊔ ⨆ x ∈ t, f x := by
  simp [supr_or, supr_sup_eq]

theorem infi_union {f : α → β} {s t : Finsetₓ α} : (⨅ x ∈ s ∪ t, f x) = (⨅ x ∈ s, f x) ⊓ ⨅ x ∈ t, f x :=
  @supr_union α βᵒᵈ _ _ _ _ _

theorem supr_insert (a : α) (s : Finsetₓ α) (t : α → β) : (⨆ x ∈ insert a s, t x) = t a ⊔ ⨆ x ∈ s, t x := by
  rw [insert_eq]
  simp only [supr_union, Finsetₓ.supr_singleton]

theorem infi_insert (a : α) (s : Finsetₓ α) (t : α → β) : (⨅ x ∈ insert a s, t x) = t a ⊓ ⨅ x ∈ s, t x :=
  @supr_insert α βᵒᵈ _ _ _ _ _

theorem supr_finset_image {f : γ → α} {g : α → β} {s : Finsetₓ γ} : (⨆ x ∈ s.Image f, g x) = ⨆ y ∈ s, g (f y) := by
  rw [← supr_coe, coe_image, supr_image, supr_coe]

theorem sup_finset_image {β γ : Type _} [SemilatticeSup β] [OrderBot β] (f : γ → α) (g : α → β) (s : Finsetₓ γ) :
    (s.Image f).sup g = s.sup (g ∘ f) := by
  classical
  induction' s using Finsetₓ.induction_on with a s' ha ih <;> simp [*]

theorem infi_finset_image {f : γ → α} {g : α → β} {s : Finsetₓ γ} : (⨅ x ∈ s.Image f, g x) = ⨅ y ∈ s, g (f y) := by
  rw [← infi_coe, coe_image, infi_image, infi_coe]

theorem supr_insert_update {x : α} {t : Finsetₓ α} (f : α → β) {s : β} (hx : x ∉ t) :
    (⨆ i ∈ insert x t, Function.update f x s i) = s ⊔ ⨆ i ∈ t, f i := by
  simp only [Finsetₓ.supr_insert, update_same]
  rcongr i hi
  apply update_noteq
  rintro rfl
  exact hx hi

theorem infi_insert_update {x : α} {t : Finsetₓ α} (f : α → β) {s : β} (hx : x ∉ t) :
    (⨅ i ∈ insert x t, update f x s i) = s ⊓ ⨅ i ∈ t, f i :=
  @supr_insert_update α βᵒᵈ _ _ _ _ f _ hx

theorem supr_bUnion (s : Finsetₓ γ) (t : γ → Finsetₓ α) (f : α → β) :
    (⨆ y ∈ s.bUnion t, f y) = ⨆ (x ∈ s) (y ∈ t x), f y := by simp [@supr_comm _ α, supr_and]

theorem infi_bUnion (s : Finsetₓ γ) (t : γ → Finsetₓ α) (f : α → β) :
    (⨅ y ∈ s.bUnion t, f y) = ⨅ (x ∈ s) (y ∈ t x), f y :=
  @supr_bUnion _ βᵒᵈ _ _ _ _ _ _

end Lattice

theorem set_bUnion_coe (s : Finsetₓ α) (t : α → Set β) : (⋃ x ∈ (↑s : Set α), t x) = ⋃ x ∈ s, t x :=
  rfl

theorem set_bInter_coe (s : Finsetₓ α) (t : α → Set β) : (⋂ x ∈ (↑s : Set α), t x) = ⋂ x ∈ s, t x :=
  rfl

theorem set_bUnion_singleton (a : α) (s : α → Set β) : (⋃ x ∈ ({a} : Finsetₓ α), s x) = s a :=
  supr_singleton a s

theorem set_bInter_singleton (a : α) (s : α → Set β) : (⋂ x ∈ ({a} : Finsetₓ α), s x) = s a :=
  infi_singleton a s

@[simp]
theorem set_bUnion_preimage_singleton (f : α → β) (s : Finsetₓ β) : (⋃ y ∈ s, f ⁻¹' {y}) = f ⁻¹' s :=
...
theorem subset_set_bUnion_of_mem {s : Finsetₓ α} {f : α → Set β} {x : α} (h : x ∈ s) : f x ⊆ ⋃ y ∈ s, f y :=
  show f x ≤ ⨆ y ∈ s, f y from le_supr_of_le x <| le_supr _ h

variable [DecidableEq α]

theorem set_bUnion_union (s t : Finsetₓ α) (u : α → Set β) : (⋃ x ∈ s ∪ t, u x) = (⋃ x ∈ s, u x) ∪ ⋃ x ∈ t, u x :=
  supr_union

theorem set_bInter_inter (s t : Finsetₓ α) (u : α → Set β) : (⋂ x ∈ s ∪ t, u x) = (⋂ x ∈ s, u x) ∩ ⋂ x ∈ t, u x :=
  infi_union

theorem set_bUnion_insert (a : α) (s : Finsetₓ α) (t : α → Set β) : (⋃ x ∈ insert a s, t x) = t a ∪ ⋃ x ∈ s, t x :=
  supr_insert a s t

theorem set_bInter_insert (a : α) (s : Finsetₓ α) (t : α → Set β) : (⋂ x ∈ insert a s, t x) = t a ∩ ⋂ x ∈ s, t x :=
  infi_insert a s t

theorem set_bUnion_finset_image {f : γ → α} {g : α → Set β} {s : Finsetₓ γ} :
    (⋃ x ∈ s.Image f, g x) = ⋃ y ∈ s, g (f y) :=
  supr_finset_image

theorem set_bInter_finset_image {f : γ → α} {g : α → Set β} {s : Finsetₓ γ} :
    (⋂ x ∈ s.Image f, g x) = ⋂ y ∈ s, g (f y) :=
  infi_finset_image

theorem set_bUnion_insert_update {x : α} {t : Finsetₓ α} (f : α → Set β) {s : Set β} (hx : x ∉ t) :
    (⋃ i ∈ insert x t, @update _ _ _ f x s i) = s ∪ ⋃ i ∈ t, f i :=
  supr_insert_update f hx

theorem set_bInter_insert_update {x : α} {t : Finsetₓ α} (f : α → Set β) {s : Set β} (hx : x ∉ t) :
    (⋂ i ∈ insert x t, @update _ _ _ f x s i) = s ∩ ⋂ i ∈ t, f i :=
  infi_insert_update f hx

theorem set_bUnion_bUnion (s : Finsetₓ γ) (t : γ → Finsetₓ α) (f : α → Set β) :
    (⋃ y ∈ s.bUnion t, f y) = ⋃ (x ∈ s) (y ∈ t x), f y :=
  supr_bUnion s t f

theorem set_bInter_bUnion (s : Finsetₓ γ) (t : γ → Finsetₓ α) (f : α → Set β) :
    (⋂ y ∈ s.bUnion t, f y) = ⋂ (x ∈ s) (y ∈ t x), f y :=
  infi_bUnion s t f
