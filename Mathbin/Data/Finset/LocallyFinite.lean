...
theorem left_not_mem_Ioc : a ∉ Ioc a b :=
  fun h => lt_irreflₓ _ (mem_Ioc.1 h).1

@[simp]
theorem left_not_mem_Ioo : a ∉ Ioo a b :=
  fun h => lt_irreflₓ _ (mem_Ioo.1 h).1

@[simp]
theorem right_not_mem_Ico : b ∉ Ico a b :=
  fun h => lt_irreflₓ _ (mem_Ico.1 h).2

@[simp]
theorem right_not_mem_Ioo : b ∉ Ioo a b :=
  fun h => lt_irreflₓ _ (mem_Ioo.1 h).2
...
theorem Icc_erase_right : (Icc a b).erase b = Ico a b :=
  by 
    rw [←coe_inj, coe_erase, coe_Icc, coe_Ico, Set.Icc_diff_right]

theorem Ico_insert_right (h : a ≤ b) : insert b (Ico a b) = Icc a b :=
  by 
    rw [←coe_inj, coe_insert, coe_Icc, coe_Ico, Set.insert_eq, Set.union_comm, Set.Ico_union_right h]

theorem Ioo_insert_left (h : a < b) : insert a (Ioo a b) = Ico a b :=
  by 
...
theorem card_Ico_eq_card_Icc_sub_one (h : a ≤ b) : (Ico a b).card = (Icc a b).card - 1 :=
  by 
    classical 
...
theorem card_Ioo_eq_card_Ico_sub_one (h : a ≤ b) : (Ioo a b).card = (Ico a b).card - 1 :=
  by 
...
theorem card_Ioo_eq_card_Icc_sub_two (h : a ≤ b) : (Ioo a b).card = (Icc a b).card - 2 :=
  by 
...
theorem Ico_union_Ico_eq_Ico {a b c : α} (hab : a ≤ b) (hbc : b ≤ c) : Ico a b ∪ Ico b c = Ico a c :=
  by 
    rw [←coe_inj, coe_union, coe_Ico, coe_Ico, coe_Ico, Set.Ico_union_Ico_eq_Ico hab hbc]

theorem Ico_union_Ico' {a b c d : α} (hcb : c ≤ b) (had : a ≤ d) : Ico a b ∪ Ico c d = Ico (min a c) (max b d) :=
  by 
...
theorem Ico_inter_Ico {a b c d : α} : Ico a b ∩ Ico c d = Ico (max a c) (min b d) :=
  by 
...
theorem image_add_right_Icc (a b c : α) : ((Icc a b).Image fun x => x+c) = Icc (a+c) (b+c) :=
  by 
    simpRw [add_commₓ _ c]
    exact image_add_left_Icc a b c

theorem image_add_right_Ico (a b c : α) : ((Ico a b).Image fun x => x+c) = Ico (a+c) (b+c) :=
  by 
    simpRw [add_commₓ _ c]
    exact image_add_left_Ico a b c

theorem image_add_right_Ioc (a b c : α) : ((Ioc a b).Image fun x => x+c) = Ioc (a+c) (b+c) :=
  by 
    simpRw [add_commₓ _ c]
    exact image_add_left_Ioc a b c

theorem image_add_right_Ioo (a b c : α) : ((Ioo a b).Image fun x => x+c) = Ioo (a+c) (b+c) :=
  by 
    simpRw [add_commₓ _ c]
