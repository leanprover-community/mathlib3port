...
theorem mem_powersetLen {n} {s t : Finset α} : s ∈ powersetLen n t ↔ s ⊆ t ∧ card s = n := by
  cases s <;> simp [powersetLen, val_le_iff.symm] <;> rfl
...
theorem powerset_len_sup [DecidableEq α] (u : Finset α) (n : ℕ) (hn : n < u.card) :
  ...
  · rw [sup_eq_bunionᵢ, le_iff_subset, subset_iff]
    ...
    · intro x hx
      ...
      obtain ⟨t, ht⟩ : ∃ t, t ∈ powersetLen n (u.erase x) := powersetLen_nonempty _
...
theorem powersetLen_map {β : Type _} (f : α ↪ β) (n : ℕ) (s : Finset α) :
  ...
  eq_of_veq <|
    Multiset.map_injective (@eq_of_veq _) <| by
      simp_rw [map_val_val_powersetLen, map_val, Multiset.map_map, Function.comp,
        RelEmbedding.coeFn_toEmbedding, mapEmbedding_apply, map_val, ← Multiset.map_map _ val,
        map_val_val_powersetLen, Multiset.powersetLen_map]
