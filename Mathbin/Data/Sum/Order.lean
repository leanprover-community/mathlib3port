...
@[refl]
theorem lift_rel.refl [IsRefl α r] [IsRefl β s] : ∀ x, lift_rel r s x x
  | inl a => lift_rel.inl (refl _)
  | inr a => lift_rel.inr (refl _)

instance [IsRefl α r] [IsRefl β s] : IsRefl (Sum α β) (lift_rel r s) :=
  ⟨lift_rel.refl _ _⟩

instance [IsIrrefl α r] [IsIrrefl β s] : IsIrrefl (Sum α β) (lift_rel r s) :=
  ⟨by
...
instance [IsAntisymm α r] [IsAntisymm β s] : IsAntisymm (Sum α β) (lift_rel r s) :=
  ⟨by
...
theorem le_def [LE α] [LE β] {a b : Sum α β} : a ≤ b ↔ lift_rel (· ≤ ·) (· ≤ ·) a b :=
  Iff.rfl

theorem lt_def [LT α] [LT β] {a b : Sum α β} : a < b ↔ lift_rel (· < ·) (· < ·) a b :=
  Iff.rfl
...
theorem inl_mono : Monotone (inl : α → Sum α β) := fun a b => lift_rel.inl

theorem inr_mono : Monotone (inr : β → Sum α β) := fun a b => lift_rel.inr

theorem inl_strict_mono : StrictMono (inl : α → Sum α β) := fun a b => lift_rel.inl

theorem inr_strict_mono : StrictMono (inr : β → Sum α β) := fun a b => lift_rel.inr
...
instance DenselyOrdered [LT α] [LT β] [DenselyOrdered α] [DenselyOrdered β] : DenselyOrdered (Sum α β) :=
  ⟨fun a b h =>
    ...
    | inl a, inl b, lift_rel.inl h =>
      ...
      ⟨toLex (inl c), lift_rel.inl ha, lift_rel.inl hb⟩
    | inr a, inr b, lift_rel.inr h =>
      let ⟨c, ha, hb⟩ := exists_between h
      ⟨toLex (inr c), lift_rel.inr ha, lift_rel.inr hb⟩⟩
...
instance Preorderₓ : Preorderₓ (α ⊕ₗ β) :=
  { lex.has_le, lex.has_lt with le_refl := refl_of (Lex (· ≤ ·) (· ≤ ·)),
...
instance LinearOrderₓ [LinearOrderₓ α] [LinearOrderₓ β] : LinearOrderₓ (α ⊕ₗ β) :=
  { lex.partial_order with le_total := total_of (Lex (· ≤ ·) (· ≤ ·)), decidableLe := lex.decidable_rel,
...
@[simp]
theorem sum_assoc_apply_inl_inl : sum_assoc α β γ (inl (inl a)) = inl a :=
  rfl

@[simp]
theorem sum_assoc_apply_inl_inr : sum_assoc α β γ (inl (inr b)) = inr (inl b) :=
  rfl

@[simp]
theorem sum_assoc_apply_inr : sum_assoc α β γ (inr c) = inr (inr c) :=
  rfl

@[simp]
theorem sum_assoc_symm_apply_inl : (sum_assoc α β γ).symm (inl a) = inl (inl a) :=
  rfl

@[simp]
theorem sum_assoc_symm_apply_inr_inl : (sum_assoc α β γ).symm (inr (inl b)) = inl (inr b) :=
  rfl

@[simp]
theorem sum_assoc_symm_apply_inr_inr : (sum_assoc α β γ).symm (inr (inr c)) = inr c :=
  rfl
...
@[simp]
theorem sum_dual_distrib_inl : sum_dual_distrib α β (to_dual (inl a)) = inl (to_dual a) :=
  rfl

@[simp]
theorem sum_dual_distrib_inr : sum_dual_distrib α β (to_dual (inr b)) = inr (to_dual b) :=
  rfl

@[simp]
theorem sum_dual_distrib_symm_inl : (sum_dual_distrib α β).symm (inl (to_dual a)) = to_dual (inl a) :=
  rfl

@[simp]
theorem sum_dual_distrib_symm_inr : (sum_dual_distrib α β).symm (inr (to_dual b)) = to_dual (inr b) :=
  rfl
...
@[simp]
theorem sum_lex_assoc_symm_apply_inl : (sum_lex_assoc α β γ).symm (inl a) = inl (inl a) :=
  rfl

@[simp]
theorem sum_lex_assoc_symm_apply_inr_inl : (sum_lex_assoc α β γ).symm (inr (inl b)) = inl (inr b) :=
  rfl

@[simp]
theorem sum_lex_assoc_symm_apply_inr_inr : (sum_lex_assoc α β γ).symm (inr (inr c)) = inr c :=
  rfl
...
@[simp]
theorem sum_lex_dual_antidistrib_inl : sum_lex_dual_antidistrib α β (to_dual (inl a)) = inr (to_dual a) :=
  rfl

@[simp]
theorem sum_lex_dual_antidistrib_inr : sum_lex_dual_antidistrib α β (to_dual (inr b)) = inl (to_dual b) :=
  rfl

@[simp]
theorem sum_lex_dual_antidistrib_symm_inl : (sum_lex_dual_antidistrib α β).symm (inl (to_dual b)) = to_dual (inr b) :=
  rfl

@[simp]
theorem sum_lex_dual_antidistrib_symm_inr : (sum_lex_dual_antidistrib α β).symm (inr (to_dual a)) = to_dual (inl a) :=
  rfl
