...
theorem ack_succ_right_le_ack_succ_left (m n : ℕ) : ack m (n + 1) ≤ ack (m + 1) n := by
  ...
  · rw [ack_succ_succ, succ_eq_add_one]
    apply ack_mono_right m (le_transₓ _ <| add_add_one_le_ack _ n)
    linarith
...
private theorem sq_le_two_pow_add_one_minus_three (n : ℕ) : n ^ 2 ≤ 2 ^ (n + 1) - 3 := by
  ...
  · cases k
    ...
    · rw [succ_eq_add_one, add_sq, pow_succₓ 2, two_mul (2 ^ _), add_tsub_assoc_of_le, add_commₓ (2 ^ _), add_assocₓ]
      · apply add_le_add hk
        norm_num
        apply succ_le_of_lt
        rw [pow_succₓ, mul_lt_mul_left (@zero_lt_two ℕ _ _)]
        apply lt_two_pow
...
theorem ack_ack_lt_ack_max_add_two (m n k : ℕ) : ack m (ack n k) < ack (max m n + 2) k :=
  calc
    ack m (ack n k) ≤ ack (max m n) (ack n k) := ack_mono_left _ (le_max_leftₓ _ _)
    _ < ack (max m n) (ack (max m n + 1) k) :=
      ack_strict_mono_right _ <| ack_strict_mono_left k <| lt_succ_of_le <| le_max_rightₓ m n
...
theorem exists_lt_ack_of_nat_primrec {f : ℕ → ℕ} (hf : Nat.Primrec f) : ∃ m, ∀ n, f n < ack m n := by
  ...
  · refine'
      ⟨max a b + 3, fun n =>
        (mkpair_lt_max_add_one_sq _ _).trans_le <|
          (Nat.pow_le_pow_of_le_leftₓ (add_le_add_right _ _) 2).trans <| ack_add_one_sq_lt_ack_add_three _ _⟩
    rw [max_ack_left]
  ...
  · -- We prove this simpler inequality first.
    have : ∀ {m n}, elim (f m) (fun y IH => g <| mkpair m <| mkpair y IH) n < ack (max a b + 9) (m + n) := by
      ...
      · apply (ha m).trans (ack_strict_mono_left m <| (le_max_leftₓ a b).trans_lt _)
        linarith
