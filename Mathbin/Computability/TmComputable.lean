...
def init_list (tm : fin_tm2) (s : List (tm.Γ tm.k₀)) : tm.cfg :=
  { l := Option.some tm.main, var := tm.initial_state,
    stk :=
      fun k =>
...
def halt_list (tm : fin_tm2) (s : List (tm.Γ tm.k₁)) : tm.cfg :=
  { l := Option.none, var := tm.initial_state,
    stk :=
      fun k =>
        @dite (List (tm.Γ k)) (k = tm.k₁) (tm.K_decidable_eq k tm.k₁)
          (fun h =>
...
def id_computer {α : Type} (ea : fin_encoding α) : fin_tm2 :=
  { k := Unit, k₀ := ⟨⟩, k₁ := ⟨⟩, Γ := fun _ => ea.Γ, Λ := Unit, main := ⟨⟩, σ := Unit, initialState := ⟨⟩,
    Γk₀Fin := ea.Γ_fin, m := fun _ => halt }
