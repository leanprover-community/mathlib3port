...
/- ./././Mathport/Syntax/Translate/Tactic/Lean3.lean:334:16: warning: unsupported simp config option: constructor_eq -/
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (k' v') -/
...
def trNormal : Code → Cont' → Λ'
  ...
  | code.cons f fs, k =>
    (Λ'.push stack fun _ => some Γ'.Cons) $ Λ'.move (fun _ => false) main rev $ Λ'.copy $ tr_normal f (Cont'.cons₁ fs k)
...
theorem head_main_ok {q s L} {c d : List Γ'} :
  ...
  refine'
    (move_ok dec_trivial (split_at_pred_eq _ _ (tr_nat L.head) o (tr_list L.tail) (tr_nat_nat_end _) _)).trans
...
theorem head_stack_ok {q s L₁ L₂ L₃} :
  ...
  cases' L₂ with a L₂
  · refine'
      trans_gen.trans (move_ok dec_trivial (split_at_pred_eq _ _ [] (some Γ'.Cons) L₃ (by rintro _ ⟨⟩) ⟨rfl, rfl⟩))
        (trans_gen.head rfl (trans_gen.head rfl _))
    convert unrev_ok
    simp
    rfl
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (l₁' l₂' s') -/
/- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation -/
/- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation -/
...
theorem pred_ok (q₁ q₂ s v) (c d : List Γ') :
  ...
  · obtain ⟨a, l, e, h⟩ : ∃ (a) (l), (tr_pos_num m = a::l) ∧ nat_end a = ff := by cases m <;> refine' ⟨_, _, rfl, rfl⟩
    refine' ⟨Γ'.bit0::l₁, _, some a, rfl, trans_gen.single _⟩
    simp [tr_pos_num, PosNum.succ, e, h, nat_end, show some Γ'.bit1 ≠ some Γ'.bit0 from dec_trivial]
...
theorem tr_ret_respects (k v s) :
  ...
  simp
  refine' (move₂_ok dec_trivial _ (split_at_pred_ff _)).trans _
  · rfl
    
  simp
  refine' (move₂_ok dec_trivial _ _).trans _
  pick_goal 4
  · rfl
