...
inductive Primrec : (ℕ → ℕ) → Prop
  ...
  | prec {f g} : Primrec f → Primrec g → Primrec (unpaired fun z n => n.elim (f z) fun y IH => g$ mkpair z$ mkpair y IH)
    ...
    prim :=
      (Primcodable.prim α).of_eq$
        fun n =>
          show
            encode (decode α n) =
              (Option.casesOn (Option.map e.symm (decode α n)) 0 fun a => Nat.succ (encode (e a)) : ℕ)by
...
theorem dom_denumerable {α β} [Denumerable α] [Primcodable β] {f : α → β} :
  Primrec f ↔ Nat.Primrec fun n => encode (f (of_nat α n)) :=
  ⟨fun h =>
      (pred.comp h).of_eq$
        fun n =>
          by 
...
theorem of_nat_iff {α β} [Denumerable α] [Primcodable β] {f : α → β} : Primrec f ↔ Primrec fun n => f (of_nat α n) :=
...
theorem of_equiv {β} {e : β ≃ α} :
  by 
    haveI  := Primcodable.ofEquiv α e <;> exact Primrec e :=
  by 
    letI this : Primcodable β := Primcodable.ofEquiv α e <;> exact encode_iff.1 Primrec.encode

theorem of_equiv_symm {β} {e : β ≃ α} :
  by 
    haveI  := Primcodable.ofEquiv α e <;> exact Primrec e.symm :=
  by 
    letI this := Primcodable.ofEquiv α e <;>
      exact
        encode_iff.1
...
theorem of_equiv_iff {β} (e : β ≃ α) {f : σ → β} :
  by 
    haveI  := Primcodable.ofEquiv α e <;> exact (Primrec fun a => e (f a)) ↔ Primrec f :=
  by 
    letI this := Primcodable.ofEquiv α e <;>
      exact
        ⟨fun h =>
...
theorem of_equiv_symm_iff {β} (e : β ≃ α) {f : σ → α} :
  by 
    haveI  := Primcodable.ofEquiv α e <;> exact (Primrec fun a => e.symm (f a)) ↔ Primrec f :=
  by 
    letI this := Primcodable.ofEquiv α e <;>
      exact
        ⟨fun h =>
...
theorem unpaired {f : ℕ → ℕ → α} : Primrec (Nat.unpaired f) ↔ Primrec₂ f :=
  ⟨fun h =>
      by 
        simpa using h.comp mkpair,
    fun h => h.comp Primrec.unpair⟩
...
theorem bind_decode_iff {f : α → β → Option σ} : (Primrec₂ fun a n => (decode β n).bind (f a)) ↔ Primrec₂ f :=
  ⟨fun h =>
    ...
    fun h => option_bind (Primrec.decode.comp snd)$ h.comp (fst.comp fst) snd⟩
...
theorem ite {c : α → Prop} [DecidablePred c] {f : α → σ} {g : α → σ} (hc : PrimrecPred c) (hf : Primrec f)
  (hg : Primrec g) : Primrec fun a => if c a then f a else g a :=
  by 
...
theorem dom_bool₂ (f : Bool → Bool → α) : Primrec₂ f :=
  (cond fst ((dom_bool (f tt)).comp snd) ((dom_bool (f ff)).comp snd)).of_eq$
    fun ⟨a, b⟩ =>
      by 
        cases a <;> rfl
...
theorem nat_bodd_div2 : Primrec Nat.boddDiv2 :=
  (nat_elim' Primrec.id (const (ff, 0))
        (((cond fst (pair (const ff) (succ.comp snd)) (pair (const tt) snd)).comp snd).comp snd).to₂).of_eq$
    fun n =>
      by 
...
theorem nat_bit : Primrec₂ Nat.bit :=
  (cond Primrec.fst (nat_bit1.comp Primrec.snd) (nat_bit0.comp Primrec.snd)).of_eq$
    fun n =>
      by 
        cases n.1 <;> rfl
...
private theorem list_cases' {f : α → List β} {g : α → σ} {h : α → β × List β → σ}
  (hf :
    by 
      haveI  := prim H <;> exact Primrec f)
  (hg : Primrec g)
  (hh :
    by 
      haveI  := prim H <;> exact Primrec₂ h) :
...
private theorem list_cons' :
  by 
    haveI  := prim H <;> exact Primrec₂ (@List.cons β) :=
  by 
    letI this := prim H <;> exact encode_iff.1 (succ.comp$ primrec₂.mkpair.comp (encode_iff.2 fst) (encode_iff.2 snd))

private theorem list_reverse' :
  by 
    haveI  := prim H <;> exact Primrec (@List.reverse β) :=
  by 
    letI this := prim H <;>
      exact
...
instance Sum : Primcodable (Sum α β) :=
  ⟨Primrec.nat_iff.1$
      (encode_iff.2
            (cond nat_bodd
              (((@Primrec.decode β _).comp nat_div2).option_map$ to₂$ nat_bit.comp (const tt) (Primrec.encode.comp snd))
              (((@Primrec.decode α _).comp nat_div2).option_map$
                to₂$ nat_bit.comp (const ff) (Primrec.encode.comp snd)))).of_eq$
        fun n =>
          show _ = encode (decode_sum n)by 
            simp [decode_sum]
...
theorem list_head' : Primrec (@List.head' α) :=
  (list_cases Primrec.id (const none) (option_some_iff.2$ fst.comp snd).to₂).of_eq$
    fun l =>
      by 
        cases l <;> rfl
...
theorem list_map {f : α → List β} {g : α → β → σ} (hf : Primrec f) (hg : Primrec₂ g) :
  Primrec fun a => (f a).map (g a) :=
  (list_foldr hf (const [])$ to₂$ list_cons.comp (hg.comp fst (fst.comp snd)) (snd.comp snd)).of_eq$
    fun a =>
      by 
        induction f a <;> simp 

theorem list_range : Primrec List.range :=
  (nat_elim' Primrec.id (const []) ((list_concat.comp snd fst).comp snd).to₂).of_eq$
    fun n =>
      by 
        simp  <;> induction n <;> simp [List.range_succ] <;> rfl

theorem list_join : Primrec (@List.join α) :=
  (list_foldr Primrec.id (const [])$ to₂$ comp (@list_append α _) snd).of_eq$
    fun l =>
      by 
...
theorem subtype_val {p : α → Prop} [DecidablePred p] {hp : PrimrecPred p} :
  by 
    haveI  := Primcodable.subtype hp <;> exact Primrec (@Subtype.val α p) :=
  by 
    letI this := Primcodable.subtype hp 
...
theorem subtype_val_iff {p : β → Prop} [DecidablePred p] {hp : PrimrecPred p} {f : α → Subtype p} :
  by 
    haveI  := Primcodable.subtype hp <;> exact (Primrec fun a => (f a).1) ↔ Primrec f :=
  by 
    letI this := Primcodable.subtype hp 
...
theorem subtype_mk {p : β → Prop} [DecidablePred p] {hp : PrimrecPred p} {f : α → β} {h : ∀ a, p (f a)}
  (hf : Primrec f) :
  by 
    haveI  := Primcodable.subtype hp <;> exact Primrec fun a => @Subtype.mk β p (f a) (h a) :=
...
theorem ulower_down : Primrec (Ulower.down : α → Ulower α) :=
  by 
    letI this : ∀ a, Decidable (a ∈ Set.Range (encode : α → ℕ)) := decidable_range_encode _ <;>
...
theorem vector_tail {n} : Primrec (@Vector.tail α n) :=
  vector_to_list_iff.1$
    (list_tail.comp vector_to_list).of_eq$
      fun ⟨l, h⟩ =>
        by 
          cases l <;> rfl
...
theorem list_of_fn : ∀ {n} {f : Finₓ n → α → σ}, (∀ i, Primrec (f i)) → Primrec fun a => List.ofFn fun i => f i a
| 0, f, hf => const []
| n+1, f, hf =>
  by 
...
theorem fin_curry₁ {n} {f : Finₓ n → α → σ} : Primrec₂ f ↔ ∀ i, Primrec (f i) :=
  ⟨fun h i => h.comp (const i) Primrec.id,
    fun h =>
      (vector_nth.comp ((vector_of_fn h).comp snd) fst).of_eq$
        fun a =>
          by 
            simp ⟩

theorem fin_curry {n} {f : α → Finₓ n → σ} : Primrec f ↔ Primrec₂ f :=
  ⟨fun h => fin_app.comp (h.comp fst) snd,
    fun h =>
...
-- error in Computability.Primrec: ././Mathport/Syntax/Translate/Basic.lean:1228:6: unsupported: hide command
hide ite
    ...
    case nat.primrec'.prec n f g _ _ hf hg => 
      exact
        nat_elim' vector_head (hf.comp vector_tail)
          (hg.comp$
              vector_cons.comp (fst.comp snd)$ vector_cons.comp (snd.comp snd)$ (@vector_tail _ _ (n+1)).comp fst).to₂
...
theorem tail {n f} (hf : @primrec' n f) : @primrec' n.succ fun v => f v.tail :=
  (hf.comp _ fun i => @nth _ i.succ).of_eq$
    fun v =>
      by 
...
protected theorem cons {n m f g} (hf : @primrec' n f) (hg : @vec n m g) : vec fun v => f v::ᵥg v :=
  fun i =>
    Finₓ.cases
      (by 
        simp )
...
theorem comp₂ (f : ℕ → ℕ → ℕ) (hf : @primrec' 2 fun v => f v.head v.tail.head) {n g h} (hg : @primrec' n g)
  (hh : @primrec' n h) : primrec' fun v => f (g v) (h v) :=
  by 
...
theorem of_prim : ∀ {n f}, Primrec f → @primrec' n f :=
  suffices ∀ f, Nat.Primrec f → @primrec' 1 fun v => f v.head from
    fun n f hf =>
      (pred.comp₁ _$ (this _ hf).comp₁ (fun m => Encodable.encode$ (decode (Vector ℕ n) m).map f) primrec'.encode).of_eq
        fun i =>
          by 
            simp [encodek]
  fun f hf =>
...
theorem prim_iff₁ {f : ℕ → ℕ} : (@primrec' 1 fun v => f v.head) ↔ Primrec f :=
  prim_iff.trans
    ⟨fun h =>
        (h.comp$ vector_of_fn$ fun i => Primrec.id).of_eq
          fun v =>
            by 
              simp ,
...
theorem prim_iff₂ {f : ℕ → ℕ → ℕ} : (@primrec' 2 fun v => f v.head v.tail.head) ↔ Primrec₂ f :=
  prim_iff.trans
    ⟨fun h =>
        (h.comp$ vector_cons.comp fst$ vector_cons.comp snd (Primrec.const nil)).of_eq
          fun v =>
            by 
              simp ,
...
theorem vec_iff {m n f} : @vec m n f ↔ Primrec f :=
  ⟨fun h =>
      by 
        simpa using vector_of_fn fun i => to_prim (h i),
    fun h i => of_prim$ vector_nth.comp h (Primrec.const i)⟩
