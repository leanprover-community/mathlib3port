...
theorem const_prim : Primrec code.const :=
  (Primrec.id.nat_iterate (Primrec.const zero) (comp_prim.comp (Primrec.const succ) Primrec.snd).to₂).of_eq$
    fun n =>
      by 
...
| k+1, c, n, x, h =>
  by 
    suffices  : ∀ {o : Option ℕ}, x ∈ guardₓ (n ≤ k) >> o → n < k+1
    ·
      cases c <;> rw [evaln] at h <;> exact this h 
    simpa [· >> ·] using Nat.lt_succ_of_leₓ

-- error in Computability.PartrecCode: ././Mathport/Syntax/Translate/Basic.lean:340:40: in introv: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
  ...
  ⟨fun h =>
      by 
        ...
        case nat.partrec.code.rfind' cf hf => 
          ...
          suffices  : ∃ k, (y+n.unpair.2) ∈ evaln (k+1) (rfind' cf) (mkpair n.unpair.1 n.unpair.2)
          ·
            simpa [evaln, · >> ·]
          revert hy₁ hy₂ 
          ...
          ·
            ...
            exact
              ⟨_, Nat.le_of_lt_succₓ$ evaln_bound hk, _, hk,
                by 
                  simp  <;> rfl⟩
          ·
  ...
  Option.some$
    ...
    (List.range k).map
      fun n =>
        k.cases none$
          fun k' =>
            Nat.Partrec.Code.recOn c (some 0) (some (Nat.succ n)) (some n.unpair.1) (some n.unpair.2)
              (fun cf cg _ _ =>
                do 
                  let x ← lup L (k, cf) n 
                  let y ← lup L (k, cg) n 
...
theorem eval_part : Partrec₂ eval :=
  (rfind_opt (evaln_prim.to_comp.comp ((snd.pair (fst.comp fst)).pair (snd.comp fst))).to₂).of_eq$
    fun a =>
      by 
        simp [eval_eq_rfind_opt]

theorem fixed_point {f : code → code} (hf : Computable f) : ∃ c : code, eval (f c) = eval c :=
  let g (x y : ℕ) : Part ℕ := eval (of_nat code x) x >>= fun b => eval (of_nat code b) y 
  have  : Partrec₂ g :=
...
theorem fixed_point₂ {f : code → ℕ →. ℕ} (hf : Partrec₂ f) : ∃ c : code, eval c = f c :=
  ...
  (fixed_point (curry_prim.comp (Primrec.const cf) Primrec.encode).to_comp).imp$
    fun c e =>
      funext$
        fun n =>
          by 
