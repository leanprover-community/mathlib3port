...
theorem ord_cof_eq (r : α → α → Prop) [IsWellOrder α r] :
  ...
  · intro a
    ...
    exact is_well_founded.wf.not_lt_min _ this (IsOrderConnected.neg_trans h ba)
...
theorem supᵢ_lt_lift {ι} {f : ι → Cardinal} {c : Cardinal} (hι : Cardinal.lift (#ι) < c.ord.cof)
    (hf : ∀ i, f i < c) : supᵢ f < c :=
  by
  rw [← ord_lt_ord, supᵢ_ord (Cardinal.bddAbove_range _)]
...
theorem blsub_lt_ord_lift {o : Ordinal} {f : ∀ a < o, Ordinal} {c : Ordinal}
  ...
  lt_of_le_of_ne (blsub_le hf) fun h =>
    ho.not_le (by simpa [← supᵢ_ord, hf, h] using cof_blsub_le_lift.{u} f)
...
theorem exists_fundamental_sequence (a : Ordinal.{u}) : ∃ f, IsFundamentalSequence a a.cof.ord f :=
  by
  rsuffices ⟨o, f, hf⟩ : ∃ o f, IsFundamentalSequence a o f
  · exact ⟨_, hf.ord_cof⟩
  ...
  · rw [← hf, lsub_le_iff]
    intro i
    rsuffices ⟨i', hi', hfg⟩ : ∃ i' hi', f i ≤ bfamilyOfFamily' r' (fun i => f i) i' hi'
...
theorem aleph0_le_cof {o} : ℵ₀ ≤ cof o ↔ IsLimit o :=
  ...
  · simp [l]
    ...
    cases n
    · simp at e
      simpa [e, not_zero_isLimit] using l
...
theorem isStrongLimit_aleph0 : IsStrongLimit ℵ₀ :=
  ⟨aleph0_ne_zero, fun x hx => by
    ...
    exact_mod_cast nat_lt_aleph0 (pow 2 n)⟩
...
theorem mk_bounded_subset {α : Type _} (h : ∀ x < #α, (2^x) < (#α)) {r : α → α → Prop}
  ...
  · have : { s : Set α | Bounded r s } = ⋃ i, 𝒫{ j | r j i } := setOf_exists _
    rw [← coe_setOf, this]
    convert mk_Union_le_sum_mk.trans ((sum_le_supᵢ _).trans (mul_le_max_of_aleph0_le_left ha))
...
theorem mk_subset_mk_lt_cof {α : Type _} (h : ∀ x < #α, (2^x) < (#α)) :
  ...
  · refine' @mk_le_of_injective α _ (fun x => Subtype.mk {x} _) _
    · rw [mk_singleton]
      exact one_lt_aleph_0.trans_le (aleph0_le_cof.2 (ord_isLimit h'.is_limit.aleph_0_le))
    · intro a b hab
      simpa only [singleton_eq_singleton_iff] using hab
...
theorem nfpFamily_lt_ord_lift_of_isRegular {ι} {f : ι → Ordinal → Ordinal} {c} (hc : IsRegular c)
  ...
  by
  apply nfpFamily_lt_ord_lift _ _ hf ha <;> rwa [hc.cof_eq]
