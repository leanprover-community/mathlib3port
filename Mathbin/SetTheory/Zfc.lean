...
theorem Equivₓ.refl x : Equiv x x :=
...
theorem Equivₓ.symm {x y} : Equiv x y → Equiv y x :=
  (Equiv.refl y).euc

theorem Equivₓ.trans {x y z} (h1 : Equiv x y) (h2 : Equiv y z) : Equiv x z :=
  h1.euc h2.symm
...
theorem Equivₓ.ext : ∀ x y : PSet, Equiv x y ↔ x ⊆ y ∧ y ⊆ x
  | ⟨α, A⟩, ⟨β, B⟩ =>
    ⟨fun ⟨αβ, βα⟩ =>
      ⟨αβ, fun b =>
        let ⟨a, h⟩ := βα b
