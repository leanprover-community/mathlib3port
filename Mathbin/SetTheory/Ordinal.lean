...
theorem lift_umax : lift.{max u v, u} = lift.{v, u} :=
  funext$
    fun a =>
      induction_on a$
        fun α r _ => Quotientₓ.sound ⟨(RelIso.preimage Equivₓ.ulift r).trans (RelIso.preimage Equivₓ.ulift r).symm⟩
...
theorem lift_type_eq {α : Type u} {β : Type v} {r s} [IsWellOrder α r] [IsWellOrder β s] :
  ...
  Quotientₓ.eq.trans
    ⟨fun ⟨f⟩ => ⟨(RelIso.preimage Equivₓ.ulift r).symm.trans$ f.trans (RelIso.preimage Equivₓ.ulift s)⟩,
      fun ⟨f⟩ => ⟨(RelIso.preimage Equivₓ.ulift r).trans$ f.trans (RelIso.preimage Equivₓ.ulift s).symm⟩⟩
  ...
  by 
    have  :=
        @RelEmbedding.is_well_order _ _ (@Equivₓ.ulift.{max v w} α ⁻¹'o r) r (RelIso.preimage Equivₓ.ulift.{max v w} r)
          _ <;>
      have  :=
          @RelEmbedding.is_well_order _ _ (@Equivₓ.ulift.{max u w} β ⁻¹'o s) s
            (RelIso.preimage Equivₓ.ulift.{max u w} s) _ <;>
        exact
          ⟨fun ⟨f⟩ =>
              ⟨(f.equiv_lt (RelIso.preimage Equivₓ.ulift r).symm).ltLe
                  (InitialSeg.ofIso (RelIso.preimage Equivₓ.ulift s))⟩,
            fun ⟨f⟩ =>
              ⟨(f.equiv_lt (RelIso.preimage Equivₓ.ulift r)).ltLe
                  (InitialSeg.ofIso (RelIso.preimage Equivₓ.ulift s).symm)⟩⟩
