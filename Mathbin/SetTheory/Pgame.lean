...
theorem le_zero {x : Pgame} :
  x ≤ 0 ↔ ∀ i : x.left_moves, ∃ j : (x.move_left i).RightMoves, (x.move_left i).moveRight j ≤ 0 :=
  by 
    rw [le_def]
    dsimp 
...
theorem zero_le {x : Pgame} :
  0 ≤ x ↔ ∀ j : x.right_moves, ∃ i : (x.move_right j).LeftMoves, 0 ≤ (x.move_right j).moveLeft i :=
  by 
    rw [le_def]
    dsimp 
    simp [forall_pempty, exists_pempty]
...
theorem lt_zero {x : Pgame} :
  x < 0 ↔ ∃ j : x.right_moves, ∀ i : (x.move_right j).LeftMoves, (x.move_right j).moveLeft i < 0 :=
  by 
    rw [lt_def]
    dsimp 
    simp [forall_pempty, exists_pempty]
...
theorem zero_lt {x : Pgame} :
  0 < x ↔ ∃ i : x.left_moves, ∀ j : (x.move_left i).RightMoves, 0 < (x.move_left i).moveRight j :=
  by 
    rw [lt_def]
    dsimp 
    simp [forall_pempty, exists_pempty]
...
theorem lt_of_le_mk {xl xr xL xR y i} : (⟨xl, xr, xL, xR⟩ : Pgame) ≤ y → xL i < y :=
  by 
    cases y 
    rw [mk_le_mk]
    tauto

theorem lt_of_mk_le {x : Pgame} {yl yr yL yR i} : x ≤ ⟨yl, yr, yL, yR⟩ → x < yR i :=
  by 
    cases x 
    rw [mk_le_mk]
    tauto

theorem mk_lt_of_le {xl xr xL xR y i} : (xR : xr → Pgame) i ≤ y → (⟨xl, xr, xL, xR⟩ : Pgame) < y :=
  by 
    cases y 
    rw [mk_lt_mk]
    tauto

theorem lt_mk_of_le {x : Pgame} {yl yr : Type _} {yL : yl → Pgame} {yR i} : x ≤ yL i → x < ⟨yl, yr, yL, yR⟩ :=
  by 
    cases x 
    rw [mk_lt_mk]
    exact fun h => Or.inl ⟨_, h⟩

theorem not_le_lt {x y : Pgame} : (¬x ≤ y ↔ y < x) ∧ (¬x < y ↔ y ≤ x) :=
  by 
    induction' x with xl xr xL xR IHxl IHxr generalizing y 
    induction' y with yl yr yL yR IHyl IHyr 
    classical 
    simp only [mk_le_mk, mk_lt_mk, not_and_distrib, not_or_distrib, not_forall, not_exists, and_comm, or_comm, IHxl,
      IHxr, IHyl, IHyr, iff_selfₓ, and_selfₓ]
...
theorem le_trans_aux {xl xr} {xL : xl → Pgame} {xR : xr → Pgame} {yl yr} {yL : yl → Pgame} {yR : yr → Pgame} {zl zr}
  ...
  by 
    simp only [mk_le_mk] at * <;>
      exact
        fun ⟨xLy, xyR⟩ ⟨yLz, yzR⟩ =>
          ⟨fun i => not_leₓ.1 fun h => not_ltₓ.2 (h₁ _ ⟨yLz, yzR⟩ h) (xLy _),
            fun i => not_leₓ.1 fun h => not_ltₓ.2 (h₂ _ h ⟨xLy, xyR⟩) (yzR _)⟩
...
@[trans]
theorem lt_of_le_of_ltₓ {x y z : Pgame} (hxy : x ≤ y) (hyz : y < z) : x < z :=
  by 
    rw [←not_leₓ] at hyz⊢
    exact mt (fun H => le_transₓ H hxy) hyz

@[trans]
theorem lt_of_lt_of_leₓ {x y z : Pgame} (hxy : x < y) (hyz : y ≤ z) : x < z :=
  by 
    rw [←not_leₓ] at hxy⊢
...
| mk xl xr xL xR, mk yl yr yL yR, restricted.mk L_embedding R_embedding L_restriction R_restriction =>
  by 
    rw [le_def]
...
| mk xl xr xL xR, mk yl yr yL yR, relabelling.mk L_equiv R_equiv L_relabelling R_relabelling =>
  restricted.mk L_equiv.to_embedding R_equiv.symm.to_embedding (fun i => (L_relabelling i).Restricted)
    fun j => (R_relabelling j).Restricted
...
theorem relabel_move_left {x : Pgame} {xl' xr'} (el : x.left_moves ≃ xl') (er : x.right_moves ≃ xr')
  (i : x.left_moves) : move_left (relabel el er) (el i) = x.move_left i :=
  by 
    simp 
...
theorem relabel_move_right {x : Pgame} {xl' xr'} (el : x.left_moves ≃ xl') (er : x.right_moves ≃ xr')
  (j : x.right_moves) : move_right (relabel el er) (er j) = x.move_right j :=
  by 
    simp 
...
def left_moves_neg (x : Pgame) : (-x).LeftMoves ≃ x.right_moves :=
  by 
    cases x 
    rfl

/-- An explicit equivalence between the moves for Right in `-x` and the moves for Left in `x`. -/
def right_moves_neg (x : Pgame) : (-x).RightMoves ≃ x.left_moves :=
  by 
    cases x 
    rfl
...
def left_moves_add (x y : Pgame) : (x+y).LeftMoves ≃ Sum x.left_moves y.left_moves :=
  by 
    cases x 
    cases y 
    rfl
...
def right_moves_add (x y : Pgame) : (x+y).RightMoves ≃ Sum x.right_moves y.right_moves :=
  by 
    cases x 
    cases y 
    rfl
...
theorem add_move_left_inl {x y : Pgame} {i} :
  (x+y).moveLeft ((@left_moves_add x y).symm (Sum.inl i)) = x.move_left i+y :=
  by 
    cases x 
    cases y 
    rfl
...
theorem add_move_right_inl {x y : Pgame} {i} :
  (x+y).moveRight ((@right_moves_add x y).symm (Sum.inl i)) = x.move_right i+y :=
  by 
    cases x 
    cases y 
    rfl
...
theorem add_move_left_inr {x y : Pgame} {i : y.left_moves} :
  (x+y).moveLeft ((@left_moves_add x y).symm (Sum.inr i)) = x+y.move_left i :=
  by 
    cases x 
    cases y 
    rfl
...
theorem add_move_right_inr {x y : Pgame} {i} :
  (x+y).moveRight ((@right_moves_add x y).symm (Sum.inr i)) = x+y.move_right i :=
  by 
    cases x 
    cases y 
    rfl
...
| mk xl xr xL xR, mk yl yr yL yR =>
  ⟨Equiv.refl _, Equiv.refl _,
    ...
    fun i =>
      Sum.casesOn i (fun i => neg_add_relabelling (xL i) (mk yl yr yL yR))
        fun i => neg_add_relabelling (mk xl xr xL xR) (yL i)⟩
...
theorem le_iff_sub_nonneg {x y : Pgame} : x ≤ y ↔ 0 ≤ y - x :=
  ⟨fun h => le_transₓ zero_le_add_right_neg (add_le_add_right h),
    fun h =>
...
theorem lt_iff_sub_pos {x y : Pgame} : x < y ↔ 0 < y - x :=
  ⟨fun h => lt_of_le_of_ltₓ zero_le_add_right_neg (add_lt_add_right h),
    fun h =>
      calc x ≤ 0+x := (zero_add_relabelling x).symm.le 
