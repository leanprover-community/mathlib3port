...
theorem rel_iso_enum' {α β : Type u} {r : α → α → Prop} {s : β → β → Prop} [IsWellOrder α r]
...
instance add_swap_covariant_class_le :
...
instance uniqueOutOne :
    Unique (1 : Ordinal).out.α where 
...
@[simps]
def enumIso (r : α → α → Prop) [IsWellOrder α r] :
    Subrel (· < ·) (· < type r) ≃r
      r where 
