...
theorem turn_bound_ne_zero_of_left_move {s t : S} (m : t ∈ L s) : turn_bound s ≠ 0 := by
  intro h
  have t := state.left_bound m
  rw [h] at t
  exact Nat.not_succ_le_zeroₓ _ t

theorem turn_bound_ne_zero_of_right_move {s t : S} (m : t ∈ R s) : turn_bound s ≠ 0 := by
  intro h
...
def relabelling_move_left (s : S) (t : left_moves (of s)) :
    relabelling (move_left (of s) t) (of ((left_moves_of s).toFun t : S)) := by
  trans
...
def relabelling_move_right (s : S) (t : right_moves (of s)) :
    relabelling (move_right (of s) t) (of ((right_moves_of s).toFun t : S)) := by
  trans
  apply relabelling_move_right_aux
  apply of_aux_relabelling

instance fintype_left_moves_of_aux (n : ℕ) (s : S) (h : turn_bound s ≤ n) : Fintype (left_moves (of_aux n s h)) := by
  apply Fintype.ofEquiv _ (left_moves_of_aux _ _).symm
  infer_instance

instance fintype_right_moves_of_aux (n : ℕ) (s : S) (h : turn_bound s ≤ n) : Fintype (right_moves (of_aux n s h)) := by
