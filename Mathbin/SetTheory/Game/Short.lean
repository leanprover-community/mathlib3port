...
def short.mk' {x : Pgame} [Fintype x.left_moves] [Fintype x.right_moves]
    (sL : ∀ i : x.left_moves, short (x.move_left i)) (sR : ∀ j : x.right_moves, short (x.move_right j)) : short x := by
  (
      cases x
      dsimp  at *) <;>
...
instance fintype_left_moves (x : Pgame) [S : short x] : Fintype x.left_moves := by
  cases' x
  dsimp
  infer_instance
...
instance fintype_right_moves (x : Pgame) [S : short x] : Fintype x.right_moves := by
  cases' x
  dsimp
  infer_instance

instance move_left_short (x : Pgame) [S : short x] (i : x.left_moves) : short (x.move_left i) := by
...
instance move_right_short (x : Pgame) [S : short x] (j : x.right_moves) : short (x.move_right j) := by
...
example : short (of_lists [0] [1]) := by
  infer_instance

example : short (of_lists [-2, -1] [1]) := by
  infer_instance
