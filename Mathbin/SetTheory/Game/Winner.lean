...
theorem winner_cases (G : Pgame) : G.left_wins ∨ G.right_wins ∨ G.first_loses ∨ G.first_wins :=
  by 
    classical 
    byCases' hpos : 0 < G <;>
      byCases' hneg : G < 0 <;>
...
theorem first_loses_is_zero {G : Pgame} : G.first_loses ↔ (G ≈ 0) :=
  by 
    rfl

theorem first_loses_of_equiv {G H : Pgame} (h : G ≈ H) : G.first_loses → H.first_loses :=
  fun hGp => ⟨le_of_equiv_of_le h.symm hGp.1, le_of_le_of_equiv hGp.2 h⟩

theorem first_wins_of_equiv {G H : Pgame} (h : G ≈ H) : G.first_wins → H.first_wins :=
  fun hGn => ⟨lt_of_lt_of_equiv hGn.1 h, lt_of_equiv_of_lt h.symm hGn.2⟩

theorem left_wins_of_equiv {G H : Pgame} (h : G ≈ H) : G.left_wins → H.left_wins :=
  fun hGl => ⟨lt_of_lt_of_equiv hGl.1 h, le_of_le_of_equiv hGl.2 h⟩

theorem right_wins_of_equiv {G H : Pgame} (h : G ≈ H) : G.right_wins → H.right_wins :=
  fun hGr => ⟨le_of_equiv_of_le h.symm hGr.1, lt_of_equiv_of_lt h.symm hGr.2⟩
...
theorem not_first_wins_of_first_loses {G : Pgame} : G.first_loses → ¬G.first_wins :=
  by 
