...
| G, H =>
  by 
    ...
    all_goals 
      intro i 
      first |
        equivRw Pgame.leftMovesAdd G H  at i|
        equivRw Pgame.rightMovesAdd G H  at i 
      cases i 
    all_goals 
...
| G =>
  by 
    ...
    all_goals 
      intro i 
      first |
        equivRw G.left_moves_neg  at i|
        equivRw G.right_moves_neg  at i 
...
theorem le_zero_iff {G : Pgame} [G.impartial] : G ≤ 0 ↔ 0 ≤ G :=
  by 
    rw [le_zero_iff_zero_le_neg, le_congr (equiv_refl 0) (neg_equiv_self G)]

theorem lt_zero_iff {G : Pgame} [G.impartial] : G < 0 ↔ 0 < G :=
  by 
...
theorem good_left_move_iff_first_wins (G : Pgame) [G.impartial] :
  (∃ i : G.left_moves, (G.move_left i).FirstLoses) ↔ G.first_wins :=
  by 
...
theorem good_right_move_iff_first_wins (G : Pgame) [G.impartial] :
  (∃ j : G.right_moves, (G.move_right j).FirstLoses) ↔ G.first_wins :=
  by 
