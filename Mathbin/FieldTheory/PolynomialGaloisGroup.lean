...
theorem ext {σ τ : p.Gal} (h : ∀ x ∈ p.rootSet p.SplittingField, σ x = τ x) : σ = τ :=
  ...
  rwa [eq_top_iff, ← SplittingField.adjoin_roots, Algebra.adjoin_le_iff]
...
def uniqueGalOfSplits (h : p.Splits (RingHom.id F)) : Unique p.Gal
  ...
  uniq f :=
    AlgEquiv.ext fun x =>
      ...
      obtain ⟨y, rfl⟩ :=
        algebra.mem_bot.mp
          ((set_like.ext_iff.mp ((IsSplittingField.splits_iff _ p).mp h) x).mp Algebra.mem_top)
  ...
  · intro y
    ...
    have key :=
      roots_map (IsScalarTower.toAlgHom F p.splitting_field E : p.splitting_field →+* E)
        ((splits_id_iff_splits _).mpr (IsSplittingField.splits p.splitting_field p))
...
theorem restrictDvd_surjective (hpq : p ∣ q) (hq : q ≠ 0) : Function.Surjective (restrictDvd hpq) :=
  by simp only [restrictDvd, dif_neg hq, restrict_surjective]
...
theorem restrictProd_injective : Function.Injective (restrictProd p q) :=
  ...
  intro f g hfg
  dsimp only [restrictProd, restrictDvd] at hfg
  simp only [dif_neg hpq, MonoidHom.prod_apply, Prod.mk.inj_iff] at hfg
  ext (x hx)
  rw [rootSet, Polynomial.map_mul, Polynomial.roots_mul] at hx
    ...
    have key :
      x =
        algebraMap p.splitting_field (p * q).SplittingField
          ((rootsEquivRoots p _).invFun ⟨x, multiset.mem_to_finset.mpr h⟩) :=
    ...
    have key :
      x =
        algebraMap q.splitting_field (p * q).SplittingField
          ((rootsEquivRoots q _).invFun ⟨x, multiset.mem_to_finset.mpr h⟩) :=
...
theorem mul_splits_in_splittingField_of_mul {p₁ q₁ p₂ q₂ : F[X]} (hq₁ : q₁ ≠ 0) (hq₂ : q₂ ≠ 0)
  ...
  · rw [←
      (SplittingField.lift q₁
          (splits_of_splits_of_dvd _ (mul_ne_zero hq₁ hq₂) (SplittingField.splits _)
  ...
  have key1 : ∀ {r : F[X]}, Irreducible r → P r :=
    ...
    obtain ⟨x, hx⟩ :=
      exists_root_of_splits _ (SplittingField.splits (r.comp q)) fun h =>
        hr'
          ((mul_eq_zero.mp
                (nat_degree_comp.symm.trans (natDegree_eq_of_degree_eq_some h))).resolve_right
            hq)
...
theorem restrictComp_surjective (hq : q.natDegree ≠ 0) :
    Function.Surjective (restrictComp p q hq) := by simp only [restrictComp, restrict_surjective]
...
theorem prime_degree_dvd_card [CharZero F] (p_irr : Irreducible p) (p_deg : p.natDegree.Prime) :
  ...
  let α : p.splitting_field :=
    rootOfSplits (algebraMap F p.splitting_field) (SplittingField.splits p) hp
...
theorem card_complex_roots_eq_card_real_add_card_not_gal_inv (p : ℚ[X]) :
  ...
  · haveI : IsEmpty (p.root_set ℂ) := by
      rw [hp, rootSet_zero]
      infer_instance
    simp_rw [(galActionHom p ℂ _).Support.eq_empty_of_isEmpty, hp, rootSet_zero, Set.toFinset_empty,
      Finset.card_empty]
  ...
  have hc0 :
    ∀ w : p.root_set ℂ,
      galActionHom p ℂ (restrict p ℂ (complex.conj_ae.restrict_scalars ℚ)) w = w ↔ w.val.im = 0 :=
    by
    intro w
...
theorem galActionHom_bijective_of_prime_degree' {p : ℚ[X]} (p_irr : Irreducible p)
  ...
  let n := (galActionHom p ℂ (restrict p ℂ (complex.conj_ae.restrict_scalars ℚ))).Support.card
  have hn : 2 ∣ n :=
