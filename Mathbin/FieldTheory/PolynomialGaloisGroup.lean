...
-- ././Mathport/Syntax/Translate/Basic.lean:833:9: unsupported derive handler group
...
@[ext]
theorem ext {σ τ : p.gal} (h : ∀, ∀ x ∈ p.root_set p.splitting_field, ∀, σ x = τ x) : σ = τ := by
  refine'
    AlgEquiv.ext fun x =>
...
def map_roots [Fact (p.splits (algebraMap F E))] : root_set p p.splitting_field → root_set p E := fun x =>
  ⟨IsScalarTower.toAlgHom F p.splitting_field E x, by
...
theorem map_roots_bijective [h : Fact (p.splits (algebraMap F E))] : Function.Bijective (map_roots p E) := by
  constructor
  ·
...
instance
  gal_action_aux
  : MulAction p.gal ( root_set p p.splitting_field )
  where
    smul
        ϕ x
        :=
        ⟨
          ϕ x
            ...
            by
              have key := Subtype.mem x
                by_cases' p = 0
                · simp only [ h , root_set_zero ] at key exact False.ndrec _ key
                ·
                  rw [ mem_root_set h ]
...
/-- The action of `gal p` on the roots of `p` in `E`. -/
  instance
    gal_action
    [ Fact ( p.splits ( algebraMap F E ) ) ] : MulAction p.gal ( root_set p E )
    where
      smul ϕ x := roots_equiv_roots p E ( ϕ • ( roots_equiv_roots p E ) . symm x )
        one_smul _ := by simp only [ Equivₓ.apply_symm_apply , one_smul ]
        mul_smul _ _ _ := by simp only [ Equivₓ.apply_symm_apply , Equivₓ.symm_apply_apply , mul_smul ]
...
theorem gal_action_hom_injective [Fact (p.splits (algebraMap F E))] : Function.Injective (gal_action_hom p E) := by
...
theorem restrict_prod_injective : Function.Injective (restrict_prod p q) := by
  by_cases' hpq : (p*q) = 0
  ·
...
theorem splits_in_splitting_field_of_comp (hq : q.nat_degree ≠ 0) : p.splits (algebraMap F (p.comp q).SplittingField) :=
  ...
  have key1 : ∀ {r : Polynomial F}, Irreducible r → P r := by
    intro r hr
  ...
  have key2 : ∀ {p₁ p₂ : Polynomial F}, P p₁ → P p₂ → P (p₁*p₂) := by
    intro p₁ p₂ hp₁ hp₂
...
equal to the dimension of its splitting field over `F`. -/
theorem card_of_separable (hp : p.separable) : Fintype.card p.gal = finrank F p.splitting_field := by
...
theorem prime_degree_dvd_card [CharZero F] (p_irr : Irreducible p) (p_deg : p.nat_degree.prime) :
    p.nat_degree ∣ Fintype.card p.gal := by
...
theorem card_complex_roots_eq_card_real_add_card_not_gal_inv (p : Polynomial ℚ) :
    (p.root_set ℂ).toFinset.card =
      (p.root_set
              ℝ).toFinset.card+(gal_action_hom p ℂ (restrict p ℂ (Complex.conjAe.restrictScalars ℚ))).support.card :=
  by
  by_cases' hp : p = 0
  ·
...
theorem gal_action_hom_bijective_of_prime_degree' {p : Polynomial ℚ} (p_irr : Irreducible p)
    ...
    Function.Bijective (gal_action_hom p ℂ) := by
  ...
  suffices ∀ m : ℕ, 2 ∣ m → 1 ≤ m → m ≤ 3 → m = 2by
