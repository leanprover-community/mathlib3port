...
def gal :=
  p.splitting_field ≃ₐ[F] p.splitting_field deriving Groupₓ, Fintype
...
def unique_gal_of_splits (h : p.splits (RingHom.id F)) : Unique p.gal where
  default := 1
  uniq := fun f =>
...
instance gal_action_aux : MulAction p.gal (root_set p p.splitting_field) where
  smul := fun ϕ x =>
    ⟨ϕ x, by
      have key := Subtype.mem x
      by_cases' p = 0
...
instance gal_action [Fact (p.splits (algebraMap F E))] : MulAction p.gal (root_set p E) where
...
def gal_action_hom [Fact (p.splits (algebraMap F E))] : p.gal →* Equivₓ.Perm (root_set p E) where
  toFun := fun ϕ =>
...
theorem card_complex_roots_eq_card_real_add_card_not_gal_inv (p : Polynomial ℚ) :
    (p.root_set ℂ).toFinset.card =
      (p.root_set ℝ).toFinset.card +
        (gal_action_hom p ℂ (restrict p ℂ (Complex.conjAe.restrictScalars ℚ))).support.card :=
  by
  by_cases' hp : p = 0
