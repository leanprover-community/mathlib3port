...
theorem AlgHom.normal_bijective [h : Normal F E] (ϕ : E →ₐ[F] K) : Function.Bijective ϕ :=
  ⟨ϕ.toRingHom.injective, fun x =>
    by
    letI : Algebra E Mem := ϕ.to_ring_hom.to_algebra
...
theorem Normal.of_isSplittingField (p : F[X]) [hFEp : IsSplittingField F E p] : Normal F E :=
  by
  by_cases hp : p = 0
  · have : IsSplittingField F F p := by
      rw [hp]
      exact ⟨splits_zero _, Subsingleton.elim _ _⟩
    exact
      (AlgEquiv.transfer_normal
            ((IsSplittingField.algEquiv F p).trans (IsSplittingField.algEquiv E p).symm)).mp
        (normal_self F)
  refine' normal_iff.2 fun x => _
  haveI hFE : FiniteDimensional F E := IsSplittingField.finiteDimensional E p
  ...
  haveI : IsScalarTower F E D := of_algebraMap_eq fun _ => rfl
  haveI : FiniteDimensional F D := FiniteDimensional.trans F E D
  suffices Nonempty (D →ₐ[F] E) by
    cases' this with ϕ
    rw [← WithBot.coe_one, degree_eq_iff_natDegree_eq q_irred.ne_zero, ← finrankED]
  ...
  haveI : IsScalarTower F C D := of_algebraMap_eq fun x => (AdjoinRoot.lift_of _).symm
  haveI : IsScalarTower F C E := of_algebraMap_eq fun x => (AdjoinRoot.lift_of _).symm
...
instance normal_supᵢ {ι : Type _} (t : ι → IntermediateField F K) [h : ∀ i, Normal F (t i)] :
  ...
  by
  refine' ⟨isAlgebraic_supᵢ fun i => (h i).1, fun x => _⟩
  ...
  have hF : Normal F E :=
    ...
    refine' isSplittingField_supᵢ _ fun i hi => adjoin_rootSet_isSplittingField _
...
def AlgHom.restrictNormalAux [h : Normal F E] :
  ...
  toFun x :=
    ⟨ϕ x, by
      ...
      exact map_isIntegral ϕ (map_isIntegral (toAlgHom F E K₁) (h.is_integral z))⟩
...
theorem AlgHom.liftNormal_commutes [Normal F E] (x : K₁) :
  ...
  apply @alg_hom.commutes K₁ E E _ _ _ _
...
theorem restrictScalars_eq_supᵢ_adjoin [h : Normal F L] :
  ...
  · rintro f _ ⟨x, rfl⟩
    ...
    rw [mem_rootSet_of_ne, AlgHom.toRingHom_eq_coe, AlgHom.coe_to_ringHom,
  ...
  · rw [Polynomial.rootSet, Finset.mem_coe, Multiset.mem_toFinset] at hy
    ...
    refine'
      le_supᵢ (fun f : K →ₐ[F] L => f.fieldRange)
        ...
        ⟨x, (g.lift_normal_commutes L (AdjoinSimple.gen F x)).trans _⟩
...
instance normal [h : Normal F L] : Normal F (normalClosure F K L) :=
  ...
  rw [← IntermediateField.restrictScalars_normal, restrictScalars_eq_supᵢ_adjoin]
