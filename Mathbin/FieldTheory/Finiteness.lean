...
-/
theorem iff_dim_lt_omega : IsNoetherian K V ↔ Module.rank K V < ω :=
  by 
...
@[simp]
theorem range_finset_basis [IsNoetherian K V] : Set.Range (finset_basis K V) = Basis.OfVectorSpaceIndex K V :=
  by 
