...
theorem primitive_element_inf_aux [Algebra.IsSeparable F E] : ∃ γ : E, F⟮⟯ = F⟮⟯ :=
  by
...
theorem AlgHom.card (F E K : Type _) [Field F] [Field E] [Field K] [IsAlgClosed K] [Algebra F E]
    [FiniteDimensional F E] [Algebra.IsSeparable F E] [Algebra F K] :
    Fintype.card (E →ₐ[F] K) = finrank F E :=
  by
  convert
    (AlgHom.card_of_powerBasis (Field.powerBasisOfFiniteOfSeparable F E)
