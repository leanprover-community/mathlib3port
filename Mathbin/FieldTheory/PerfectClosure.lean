...
instance
  : PerfectRing ( PerfectClosure K p ) p
  where
    pthRoot'
        e
        :=
        lift_on
          e
            ( fun x => mk K p ( x . 1 + 1 , x . 2 ) )
            fun x y H => match x , y , H with | _ , _ , r.intro n x => Quot.sound ( r.intro _ _ )
      frobenius_pth_root'
        e
        :=
        induction_on
          e fun ⟨ n , x ⟩ => by simp only [ lift_on_mk , frobenius_mk ] exact ( Quot.sound $ r.intro _ _ ) . symm
      pth_root_frobenius'
        e
        :=
        induction_on
