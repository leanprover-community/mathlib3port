...
theorem card_aut_eq_finrank [FiniteDimensional F E] [IsGalois F E] : Fintype.card (E ≃ₐ[F] E) = finrank F E := by
  ...
  let iso :
    «expr ⟮ , ⟯» F "././Mathport/Syntax/Translate/Basic.lean:681:61: unsupported notation `«expr ⟮ , ⟯»" ≃ₐ[F] E :=
    { toFun := fun e => e.val,
      invFun := fun e =>
        ⟨e, by
          rw [hα]
...
theorem AlgEquiv.transfer_galois (f : E ≃ₐ[F] E') : IsGalois F E ↔ IsGalois F E' :=
  ⟨fun h => by
    exact IsGalois.of_alg_equiv f, fun h => by
...
instance
  fixed_field.algebra
  : Algebra K ( fixed_field ( fixing_subgroup K ) )
  where
    smul
        x y
        :=
                ...
                by
                  rw
                    [
                      smul_mul'
                        ,
                        show ϕ • ↑ x = ↑ x by exact Subtype.mem ϕ x
                        ,
 ...
 (Command.theorem
  ...
  (Command.declSig
   [(Term.instBinder "[" [`hFE ":"] (Term.app `FiniteDimensional [`F `E]) "]")
    (Term.instBinder "[" [`sp ":"] (Term.app `p.is_splitting_field [`F `E]) "]")
    (Term.explicitBinder "(" [`hp] [":" `p.separable] [] ")")
    (Term.explicitBinder "(" [`K] [":" (Term.app `IntermediateField [`F `E])] [] ")")
    (Term.implicitBinder "{" [`x] [":" `E] "}")
    (Term.explicitBinder
     "("
     [`hx]
     [":" (Init.Core.«term_∈_» `x " ∈ " (Term.proj (Term.app `p.map [(Term.app `algebraMap [`F `E])]) "." `roots))]
     []
     ")")]
   (Term.typeSpec
    ":"
      ...
      [(group
          ...
          (Term.haveIdDecl
           ...
           [(Term.typeSpec
             ...
             (Init.Core.«term_∣_»
              ...
              " ∣ "
              (Term.app `p.map [(Term.app `algebraMap [`F `K])])))]
           ":="
           (Term.byTactic
            "by"
            (Tactic.tacticSeq
       ...
       (group
        (Tactic.«tactic·._»
         ...
         (Tactic.tacticSeq
          (Tactic.tacticSeq1Indented
           [(group (Tactic.apply "apply" `Fintype.card_congr) []) (group (Tactic.tacticRfl "rfl") [])])))
        [])
       (group
        (Tactic.«tactic·._»
         "·"
       ...
       (Tactic.tacticHave_
        ...
        (Term.haveDecl
         (Term.haveIdDecl
          ...
          [(Term.typeSpec
            ":"
            (Init.Core.«term_∣_» (Term.app `minpoly [`K `x]) " ∣ " (Term.app `p.map [(Term.app `algebraMap [`F `K])])))]
          ":="
          (Term.byTactic
           "by"
           (Tactic.tacticSeq
...
[PrettyPrinter.parenthesize] parenthesized: (Term.paren
 ...
 [(Term.app
   ...
   [`K
    ...
    (Term.hole "_")
    (Term.hole "_")
    (Term.paren "(" [(Term.app `RingHom.toAlgebra [`f]) []] ")")])
  []]
 ")")
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, term))
  `Fintype
    ...
    :
      Fintype.card
        ...
        Fintype.card K →ₐ[ F ] E
          *
          finrank K «expr ⟮ , ⟯» K "././Mathport/Syntax/Translate/Basic.lean:681:61: unsupported notation `«expr ⟮ , ⟯»"
  :=
    by
      have
          ...
          key_equiv
            :
              (
                    ↑
                        «expr ⟮ , ⟯»
                          K "././Mathport/Syntax/Translate/Basic.lean:681:61: unsupported notation `«expr ⟮ , ⟯»"
                      : IntermediateField F E
                    )
                  →ₐ[
                  F
...
theorem of_separable_splitting_field [sp : p.is_splitting_field F E] (hp : p.separable) : IsGalois F E := by
  ...
  exact
    (LinearEquiv.finrank_eq
        (IntermediateField.lift2AlgEquiv
            («expr ⟮ , ⟯» K
              "././Mathport/Syntax/Translate/Basic.lean:681:61: unsupported notation `«expr ⟮ , ⟯»")).toLinearEquiv).symm
