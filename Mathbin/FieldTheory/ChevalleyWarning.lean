...
theorem MvPolynomial.sum_mv_polynomial_eq_zero [DecidableEq σ] (f : MvPolynomial σ K)
  ...
  by 
    ...
    intro x₀ 
    let e : K ≃ { x // (x ∘ coeₓ) = x₀ } := (Equiv.subtypeEquivCodomain _).symm 
    ...
    letI this : Unique { j // j = i } := { default := ⟨i, rfl⟩, uniq := fun ⟨j, h⟩ => Subtype.val_injective h }
    calc (∏j : σ, (e a : σ → K) j^d j) = ((e a : σ → K) i^d i)*∏j : { j // j ≠ i }, (e a : σ → K) j^d j :=
      ...
      by 
        rw [Equiv.subtype_equiv_codomain_symm_apply_eq]_ = (a^d i)*∏j, x₀ j^d j :=
...
theorem char_dvd_card_solutions_family (p : ℕ) [CharP K p] {ι : Type _} {s : Finset ι} {f : ι → MvPolynomial σ K}
  ...
  by 
    ...
    calc (1 - (f i^q - 1)).totalDegree ≤ max (1 : MvPolynomial σ K).totalDegree (f i^q - 1).totalDegree :=
      total_degree_sub _ _ _ ≤ (f i^q - 1).totalDegree :=
      by 
