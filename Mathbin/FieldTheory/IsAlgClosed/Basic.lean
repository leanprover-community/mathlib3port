...
theorem roots_eq_zero_iff [IsAlgClosed k] {p : Polynomial k} : p.roots = 0 ↔ p = Polynomial.c (p.coeff 0) := by
  refine'
    ⟨fun h => _, fun hp => by
      rw [hp, roots_C]⟩
...
instance : LE (subfield_with_hom K L M hL) where
...
instance : Preorderₓ (subfield_with_hom K L M hL) where
  le := · ≤ ·
  le_refl := fun E =>
    ⟨le_reflₓ _, by
      simp ⟩
      ...
      { Carrier := ⨆ i : c, (i : subfield_with_hom K L M hL).Carrier,
        emb :=
          Subalgebra.suprLift (fun i : c => (i : subfield_with_hom K L M hL).Carrier)
            ...
            _ rfl }
    ⟨ub, fun N hN =>
      ⟨(le_supr (fun i : c => (i : subfield_with_hom K L M hL).Carrier) ⟨N, hN⟩ : _), by
        intro x
        simp [ub]
        rfl⟩⟩
  else by
...
noncomputable def equiv_of_equiv_aux (hJK : J ≃+* K) :
  ...
  have : Algebra.IsAlgebraic K J := fun x => by
