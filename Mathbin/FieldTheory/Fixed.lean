...
theorem linear_independent_smul_of_linear_independent {s : Finset F} :
  ...
  by 
    ...
    refine' (@sum_attach _ _ s _ fun i => (g • l i - l i) • (to_fun G F) i).trans _ 
    ext g' 
    dsimp only 
    ...
    show (∑x in s, g • (fun y => l y • to_fun G F y) x (g⁻¹*g')) = ∑x in s, (fun y => l y • to_fun G F y) x g' 
...
def minpoly : Polynomial (FixedPoints.subfield G F) :=
  (prodXSubSmul G F x).toSubring (FixedPoints.subfield G F).toSubring$
    fun c hc g =>
...
theorem ne_one : minpoly G F x ≠ (1 : Polynomial (FixedPoints.subfield G F)) :=
  fun H =>
    have  := eval₂ G F x
    (one_ne_zero : (1 : F) ≠ 0)$
      by 
...
theorem of_eval₂ (f : Polynomial (FixedPoints.subfield G F))
  (hf : Polynomial.eval₂ (Subfield.subtype$ FixedPoints.subfield G F) x f = 0) : minpoly G F x ∣ f :=
  by 
    ...
    rw [Polynomial.dvd_iff_is_root, Polynomial.IsRoot.def, MulAction.of_quotient_stabilizer_mk, Polynomial.eval_smul',
      ...
      ←IsInvariantSubring.coe_subtype_hom' G (FixedPoints.subfield G F).toSubring, ←MulSemiringActionHom.coe_polynomial,
...
theorem dim_le_card : Module.rank (FixedPoints.subfield G F) F ≤ Fintype.card G :=
  dim_le$
    fun s hs =>
      by 
        simpa only [dim_fun', Cardinal.mk_finset, Finset.coe_sort_coe, Cardinal.lift_nat_cast,
...
theorem linear_independent_to_linear_map (R : Type u) (A : Type v) (B : Type w) [CommSemiringₓ R] [Ringₓ A]
  ...
  have  : LinearIndependent B (LinearMap.ltoFun R A B ∘ AlgHom.toLinearMap) :=
    ((linear_independent_monoid_hom A B).comp (coeₓ : (A →ₐ[R] B) → A →* B)
      fun f g hfg => AlgHom.ext$ MonoidHom.ext_iff.1 hfg :
    _)
  this.of_comp _
