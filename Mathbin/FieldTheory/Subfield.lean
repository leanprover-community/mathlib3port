...
theorem gc_map_comap (f : K →+* L) : GaloisConnection (map f) (comap f) :=
...
theorem field_range_eq_map : f.field_range = Subfield.map f ⊤ :=
  by 
    ext 
    simp 

theorem map_field_range : f.field_range.map g = (g.comp f).fieldRange :=
  by 
...
instance : HasInf (Subfield K) :=
  ⟨fun s t =>
      { s.to_subring⊓t.to_subring with
        inv_mem' :=
          fun x hx => Subring.mem_inf.mpr ⟨s.inv_mem (Subring.mem_inf.mp hx).1, t.inv_mem (Subring.mem_inf.mp hx).2⟩ }⟩
...
theorem Inf_to_subring (s : Set (Subfield K)) : (Inf s).toSubring = ⨅ (t : _)(_ : t ∈ s), Subfield.toSubring t :=
  by 
    ...
    erw [Subring.mem_Inf]
    exact
      ⟨fun h p ⟨p', hp⟩ => hp ▸ subring.mem_Inf.mpr fun p ⟨hp', hp⟩ => hp ▸ h _ hp',
        fun h p hp =>
          h p.to_subring
            ⟨p,
              Subring.ext
                fun x =>
                  ⟨fun hx => subring.mem_Inf.mp hx _ ⟨hp, rfl⟩,
                    fun hx => subring.mem_Inf.mpr fun p' ⟨hp, p'_eq⟩ => p'_eq ▸ hx⟩⟩⟩

theorem is_glb_Inf (S : Set (Subfield K)) : IsGlb S (Inf S) :=
  by 
...
theorem subring_closure_le (s : Set K) : Subring.closure s ≤ (closure s).toSubring :=
  fun x hx => ⟨x, hx, 1, Subring.one_mem _, div_one x⟩
...
def cod_restrict_field (f : K →+* L) (s : Subfield L) (h : ∀ x, f x ∈ s) : K →+* s :=
