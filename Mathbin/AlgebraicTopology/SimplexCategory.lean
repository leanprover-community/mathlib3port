...
-- ././Mathport/Syntax/Translate/Basic.lean:1057:31: unsupported: @[derive, irreducible] def
/--  The simplex category:
...
@[simp]
theorem mk_to_order_hom {a b : SimplexCategory.{u}} (f : SimplexCategory.Hom a b) : mk f.to_order_hom = f := by
  cases f
  rfl

@[simp]
theorem to_order_hom_mk {a b : SimplexCategory.{u}} (f : Finₓ (a.len+1) →ₘ Finₓ (b.len+1)) : (mk f).toOrderHom = f := by
...
@[ simps ]
  instance
    small_category
    : small_category .{ u } SimplexCategory
    where
      Hom n m := SimplexCategory.Hom n m id m := SimplexCategory.Hom.id _ comp _ _ _ f g := SimplexCategory.Hom.comp g f
        ...
        ((if h : (j : ℕ) < k then
              ...
              k.cast_lt
                (by
                  ...
                  linarith)).cast_succ <
          i)
        (ite (j.cast_succ < k) (k - 1) k) (ite (j.cast_succ < k) (k - 1) k+1)by
...
theorem δ_comp_σ_self {n} {i : Finₓ (n+1)} : δ i.cast_succ ≫ σ i = 𝟙 [n] := by
  ext j
  suffices
    ite (Finₓ.castSucc i < ite (j < i) (Finₓ.castSucc j) j.succ) (ite (j < i) (j : ℕ) (j+1) - 1) (ite (j < i) j (j+1)) =
      j by
    dsimp [δ, σ, Finₓ.succAbove, Finₓ.predAbove]
    simpa [Finₓ.predAbove] with push_cast
  rcases i with ⟨i, _⟩
        ...
        mk ( Fintype.card X - 1 : ℕ )
          ...
          ⟨
            by
              have
                  aux
                    : Fintype.card X = ( Fintype.card X - 1 ) + 1
                    :=
                    by exact ( Nat.succ_pred_eq_of_posₓ $ fintype.card_pos_iff.mpr ⟨ ⊥ ⟩ ) . symm
...
-- ././Mathport/Syntax/Translate/Basic.lean:833:9: unsupported derive handler small_category
...
instance : concrete_category.{0} SimplexCategory.{u} where
  forget := { obj := fun i => Finₓ (i.len+1), map := fun i j f => f.to_order_hom }
  forget_faithful := {  }
...
-/
theorem mono_iff_injective {n m : SimplexCategory.{u}} {f : n ⟶ m} : mono f ↔ Function.Injective f.to_order_hom := by
  constructor
  ·
    intros m x y h
                    ...
                    fun u => if u ≤ x then 0 else 1
                      ...
                      by
                        intro a b h
                          ...
                          any_goals exact le_reflₓ _ any_goals { }
                          · exact bot_le
              ...
              set
                ...
                hom.mk
                  ⟨
                    ...
                    ⟩
              have f_comp_chi_i : f ≫ chi_1 = f ≫ chi_2 := by dsimp ext simp [ le_iff_lt_or_eqₓ , h_ab x_1 ]
              rw [ CategoryTheory.cancel_epi f ] at f_comp_chi_i
              rename' f_comp_chi_i
              apply_fun fun e => e.to_order_hom x at eq_chi_i
              suffices ( 0 : Finₓ 2 ) = 1 by exact bot_ne_top this
...
theorem len_le_of_mono {x y : SimplexCategory.{u}} {f : x ⟶ y} : mono f → x.len ≤ y.len := by
  intro hyp_f_mono
  have f_inj : Function.Injective f.to_order_hom.to_fun := by
...
theorem len_le_of_epi {x y : SimplexCategory.{u}} {f : x ⟶ y} : epi f → y.len ≤ x.len := by
  intro hyp_f_epi
  have f_surj : Function.Surjective f.to_order_hom.to_fun := by
