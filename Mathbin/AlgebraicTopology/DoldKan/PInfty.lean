...
def pInfty : K[X] ⟶ K[X] :=
  ChainComplex.ofHom _ _ _ _ _ _ (fun n => ((p n).f n : X _[n] ⟶ _)) fun n => by
    simpa only [← p_is_eventually_constant (show n ≤ n by rfl),
      AlternatingFaceMapComplex.obj_d_eq] using (p (n + 1)).comm (n + 1) n
...
theorem karoubi_pInfty_f {Y : Karoubi (SimplicialObject C)} (n : ℕ) :
  ...
  have h₃₂ : (P₃.f n).f = P₂.f n := karoubi.hom_ext.mp (map_pInfty_f (toKaroubi C) Y₂ n)
  have h₄₃ : P₄.f n = P₃.f n :=
    by
    have h := Functor.congr_obj (toKaroubi_comp_karoubiFunctorCategoryEmbedding _ _) Y₂
    simp only [← natTransPInftyF_app]
    congr
  let τ₁ := 𝟙 (karoubiFunctorCategoryEmbedding SimplexCategoryᵒᵖ C)
  let τ₂ := natTransPInftyF (Karoubi C) n
  let τ := τ₁ ◫ τ₂
  have h₁₄ := Idempotents.nat_trans_eq τ Y
  dsimp [τ, τ₁, τ₂, natTransPInftyF] at h₁₄
  rw [id_comp, id_comp, comp_id, comp_id] at h₁₄
  -- We use the three equalities h₃₂, h₄₃, h₁₄.
  rw [← h₃₂, ← h₄₃, h₁₄]
  simp only [KaroubiFunctorCategoryEmbedding.map_app_f, Karoubi.decompIdP_f, Karoubi.decompIdI_f,
    Karoubi.comp_f]
  let π : Y₄ ⟶ Y₄ := (toKaroubi _ ⋙ karoubiFunctorCategoryEmbedding _ _).map Y.p
  have eq := karoubi.hom_ext.mp (pInfty_f_naturality n π)
