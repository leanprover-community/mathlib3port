...
theorem of_succ {Y : C} {n q : ℕ} {φ : Y ⟶ X _[n + 1]} (v : HigherFacesVanish (q + 1) φ) : HigherFacesVanish q φ :=
  fun j hj => v j (by simpa only [← add_assocₓ] using le_add_right hj)
...
theorem comp_Hσ_eq {Y : C} {n a q : ℕ} {φ : Y ⟶ X _[n + 1]} (v : HigherFacesVanish q φ) (hnaq : n = a + q) :
    φ ≫ (hσₓ q).f (n + 1) =
      -(φ ≫
          X.δ ⟨a + 1, Nat.succ_lt_succₓ (Nat.lt_succ_iff.mpr (Nat.Le.intro hnaq.symm))⟩ ≫
  ...
  swap
  · rintro ⟨k, hk⟩
    suffices φ ≫ X.δ (⟨a + 2 + k, by linarith⟩ : Finₓ (n + 2)) = 0 by
  ...
  · rintro ⟨k, hk⟩
    ...
    let i : Finₓ (n + 1) := ⟨a + 1 + k, by linarith⟩
    have h : Finₓ.castSucc (⟨a + 1, by linarith⟩ : Finₓ (n + 1)) < i.succ := by
      simp only [Finₓ.lt_iff_coe_lt_coe, Finₓ.cast_succ_mk, Finₓ.coe_mk, Finₓ.succ_mk]
      linarith
    have δσ_rel := δ_comp_σ_of_gt X h
    conv_lhs at δσ_rel => simp only [Finₓ.cast_succ_mk, Finₓ.succ_mk, show a + 1 + k + 1 + 1 = a + 3 + k by linarith]
...
conv_lhs =>
  congr
  skip
  rw [Finₓ.sum_univ_cast_succ, Finₓ.sum_univ_cast_succ]
  rw [Finₓ.sum_univ_cast_succ]
  simp only [Finₓ.last, Finₓ.cast_le_mk, Finₓ.coe_cast, Finₓ.cast_mk, Finₓ.coe_cast_le, Finₓ.coe_mk, Finₓ.cast_succ_mk,
    Finₓ.coe_cast_succ]
  ...
  · -- d+e = 0
    let b : Finₓ (n + 2) := ⟨a + 1, by linarith⟩
  ...
  · -- c+a = 0
    rw [← Finsetₓ.sum_add_distrib]
    apply Finsetₓ.sum_eq_zero
    rintro ⟨i, hi⟩ h₀
    have hia : (⟨i, by linarith⟩ : Finₓ (n + 2)) ≤ Finₓ.castSucc (⟨a, by linarith⟩ : Finₓ (n + 1)) := by
...
theorem comp_Hσ_eq_zero {Y : C} {n q : ℕ} {φ : Y ⟶ X _[n + 1]} (v : HigherFacesVanish q φ) (hqn : n < q) :
  ...
  · simp only [hσ'_eq (show n + 1 = 0 + q by linarith) (c_mk (n + 2) (n + 1) rfl), pow_zeroₓ, Finₓ.mk_zero, one_zsmul,
      eq_to_hom_refl, comp_id, comp_sum, alternating_face_map_complex.obj_d_eq]
    rw [← Finₓ.sum_congr' _ (show 2 + (n + 1) = n + 1 + 2 by linarith), Finₓ.sum_trunc]
    · simp only [Finₓ.sum_univ_cast_succ, Finₓ.sum_univ_zero, zero_addₓ, Finₓ.last, Finₓ.cast_le_mk, Finₓ.cast_mk,
        Finₓ.cast_succ_mk]
      simp only [Finₓ.mk_zero, Finₓ.coe_zero, pow_zeroₓ, one_zsmul, Finₓ.mk_one, Finₓ.coe_one, pow_oneₓ, neg_smul,
        comp_neg]
    ...
    · intro j
      ...
      have h : Finₓ.cast (by rw [add_commₓ 2]) (Finₓ.natAdd 2 j) = j.succ.succ := by
        ext
...
theorem induction {Y : C} {n q : ℕ} {φ : Y ⟶ X _[n + 1]} (v : HigherFacesVanish q φ) :
  ...
  by_cases hqn:n < q
  · rw [v.comp_Hσ_eq_zero hqn, zero_comp, add_zeroₓ, v j (by linarith)]
  ...
  · simpa only [Nat.eq_zero_of_add_eq_zero_left ha, Finₓ.eq_zero j, Finₓ.mk_zero, Finₓ.mk_one, δ_comp_σ_succ, comp_id]
  ...
  · -- in the last case, a=m, q=1 and j=a+1
    have hq : q = 1 := by rw [← add_left_injₓ a, ha, ham'', add_commₓ]
    have hj₄ : (⟨a + 1, by linarith⟩ : Finₓ (m + 3)) = Finₓ.castSucc j := by
      ext
