...
theorem closure_induction' (s : Set M) {p : ∀ x, x ∈ closure L s → Prop} (Hs : ∀ x h : x ∈ s, p x (subset_closure h))
    (Hfun : ∀ {n : ℕ} f : L.functions n, closed_under f { x | ∃ hx, p x hx }) {x} (hx : x ∈ closure L s) : p x hx := by
...
def realize_term {α : Type} (v : α → M) : ∀ t : L.term α, M
  | var k => v k
  | func f ts => fun_map f fun i => realize_term (ts i)
...
@[simps]
instance : HasBot (L.bounded_formula α n) :=
  ⟨bd_falsum⟩

/-- The negation of a bounded formula is also a bounded formula. -/
@[reducible]
def bd_not (φ : L.bounded_formula α n) : L.bounded_formula α n :=
  bd_imp φ ⊥

@[simps]
instance : HasTop (L.bounded_formula α n) :=
  ⟨bd_not bd_falsum⟩

@[simps]
instance : HasInf (L.bounded_formula α n) :=
  ⟨fun f g => bd_not (bd_imp f (bd_not g))⟩

@[simps]
instance : HasSup (L.bounded_formula α n) :=
  ⟨fun f g => bd_imp (bd_not f) g⟩
...
@[simp]
def realize_bounded_formula : ∀ {l} f : L.bounded_formula α l v : α → M xs : Finₓ l → M, Prop
  | _, bd_falsum, v, xs => False
  | _, bd_equal t₁ t₂, v, xs => realize_term (Sum.elim v xs) t₁ = realize_term (Sum.elim v xs) t₂
  | _, bd_rel R ts, v, xs => rel_map R fun i => realize_term (Sum.elim v xs) (ts i)
...
@[simp]
theorem realize_not {l} (f : L.bounded_formula α l) (v : α → M) (xs : Finₓ l → M) :
...
def realize_formula (f : L.formula α) (v : α → M) : Prop :=
  realize_bounded_formula M f v finZeroElim

/-- A sentence can be evaluated as true or false in a structure. -/
@[reducible]
def realize_sentence (φ : L.sentence) : Prop :=
  realize_formula M φ Pempty.elimₓ

variable {M}

section Definability

variable (L) [Fintype α]
...
structure is_definable (s : Set (α → M)) : Prop where
  exists_formula : ∃ φ : L.formula α, s = SetOf (realize_formula M φ)

variable {L}

@[simp]
theorem is_definable_empty : L.is_definable (∅ : Set (α → M)) :=
  ⟨⟨⊥, by
      ext
      simp ⟩⟩

@[simp]
theorem is_definable_univ : L.is_definable (Set.Univ : Set (α → M)) :=
  ⟨⟨⊤, by
      ext
      simp ⟩⟩

@[simp]
theorem is_definable.inter {f g : Set (α → M)} (hf : L.is_definable f) (hg : L.is_definable g) :
    L.is_definable (f ∩ g) :=
  ⟨by
...
@[simp]
theorem is_definable.union {f g : Set (α → M)} (hf : L.is_definable f) (hg : L.is_definable g) :
    L.is_definable (f ∪ g) :=
  ⟨by
    rcases hf.exists_formula with ⟨φ, hφ⟩
    rcases hg.exists_formula with ⟨θ, hθ⟩
    refine' ⟨φ⊔θ, _⟩
    ext
    simp only [hφ, hθ, Set.sup_eq_union, realize_not, realize_bounded_formula, bounded_formula.has_sup_sup,
      Set.mem_union_eq, Set.mem_set_of_eq]
    tauto⟩

@[simp]
theorem is_definable.compl {s : Set (α → M)} (hf : L.is_definable s) : L.is_definable (sᶜ) :=
  ⟨by
    rcases hf.exists_formula with ⟨φ, hφ⟩
    refine' ⟨bd_not φ, _⟩
    rw [hφ]
...
@[simp]
theorem is_definable.sdiff {s t : Set (α → M)} (hs : L.is_definable s) (ht : L.is_definable t) :
    L.is_definable (s \ t) :=
  hs.inter ht.compl
...
def definable_set :=
  Subtype fun s : Set (α → M) => is_definable L s

namespace DefinableSet

variable {M} {α}

instance : HasTop (L.definable_set M α) :=
  ⟨⟨⊤, is_definable_univ⟩⟩

instance : HasBot (L.definable_set M α) :=
  ⟨⟨⊥, is_definable_empty⟩⟩

instance : Inhabited (L.definable_set M α) :=
  ⟨⊥⟩

instance : SetLike (L.definable_set M α) (α → M) where
  coe := Subtype.val
  coe_injective' := Subtype.val_injective

@[simp]
theorem mem_top {x : α → M} : x ∈ (⊤ : L.definable_set M α) :=
  Set.mem_univ x

@[simp]
theorem coe_top : ((⊤ : L.definable_set M α) : Set (α → M)) = ⊤ :=
  rfl

@[simp]
theorem not_mem_bot {x : α → M} : ¬x ∈ (⊥ : L.definable_set M α) :=
  Set.not_mem_empty x

@[simp]
theorem coe_bot : ((⊥ : L.definable_set M α) : Set (α → M)) = ⊥ :=
  rfl

instance : Lattice (L.definable_set M α) :=
  Subtype.lattice (fun _ _ => is_definable.union) fun _ _ => is_definable.inter

theorem le_iff {s t : L.definable_set M α} : s ≤ t ↔ (s : Set (α → M)) ≤ (t : Set (α → M)) :=
  Iff.rfl

@[simp]
theorem coe_sup {s t : L.definable_set M α} : ((s⊔t : L.definable_set M α) : Set (α → M)) = s ∪ t :=
  rfl

@[simp]
theorem mem_sup {s t : L.definable_set M α} {x : α → M} : x ∈ s⊔t ↔ x ∈ s ∨ x ∈ t :=
  Iff.rfl

@[simp]
theorem coe_inf {s t : L.definable_set M α} : ((s⊓t : L.definable_set M α) : Set (α → M)) = s ∩ t :=
  rfl

@[simp]
theorem mem_inf {s t : L.definable_set M α} {x : α → M} : x ∈ s⊓t ↔ x ∈ s ∧ x ∈ t :=
  Iff.rfl

instance : BoundedOrder (L.definable_set M α) :=
  { definable_set.has_top L, definable_set.has_bot L with bot_le := fun s x hx => False.elim hx,
    le_top := fun s x hx => Set.mem_univ x }

instance : DistribLattice (L.definable_set M α) :=
  { definable_set.lattice L with
    le_sup_inf := by
      intro s t u x
...
@[reducible]
instance : HasCompl (L.definable_set M α) :=
  ⟨fun ⟨s, hs⟩ => ⟨sᶜ, hs.compl⟩⟩

@[simp]
theorem mem_compl {s : L.definable_set M α} {x : α → M} : x ∈ sᶜ ↔ ¬x ∈ s := by
  cases' s with s hs
  rfl

@[simp]
theorem coe_compl {s : L.definable_set M α} : ((sᶜ : L.definable_set M α) : Set (α → M)) = sᶜ := by
  ext
  simp

instance : BooleanAlgebra (L.definable_set M α) :=
  { definable_set.has_compl L, definable_set.bounded_order L, definable_set.distrib_lattice L with
    sdiff := fun s t => s⊓tᶜ, sdiff_eq := fun s t => rfl,
