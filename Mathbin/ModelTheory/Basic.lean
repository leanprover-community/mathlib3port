...
@[simp, normCast]
theorem coe_Inf (S : Set (L.substructure M)) : ((Inf S : L.substructure M) : Set M) = ⋂ (s : _)(_ : s ∈ S), ↑s :=
  rfl
...
@[simp, normCast]
theorem coe_infi {ι : Sort _} {S : ι → L.substructure M} : (↑⨅ i, S i : Set M) = ⋂ i, S i :=
  by 
...
/-- The `L.substructure` generated by a set. -/
  def
    closure
    : LowerAdjoint ( coeₓ : L.substructure M → Set M )
    :=
      ⟨
        fun s => Inf { S | s ⊆ S }
          ,
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » closure L s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
  ...
  Subtype.recOn x$
    fun x hx =>
      by 
        ...
        exact
          closure_induction hx (fun x hx => ⟨subset_closure hx, Hs x hx⟩)
            fun n f x hx =>
