  ...
  refine'
    ⟨fun h =>
      ⟨⟨L[[α]].completeTheory h.some, (subset_union_left _ S).trans CompleteTheory.subset,
          completeTheory.isMaximal _ _⟩,
        (subset_union_right ((L.Lhom_with_constants α).onTheory T) _).trans CompleteTheory.subset⟩,
      _⟩
  rintro ⟨p, hp⟩
...
theorem setOf_subset_eq_univ_iff (S : L[[α]].Theory) :
  ...
  have h : { p : T.complete_type α | S ⊆ ↑p } = ⋂₀ ((fun φ => { p | φ ∈ p }) '' S) :=
    ...
    ext
    simp [subset_def]
  simp_rw [h, interₛ_eq_univ, ← setOf_mem_eq_univ_iff]
...
theorem nonempty_iff : Nonempty (T.CompleteType α) ↔ T.IsSatisfiable :=
  ...
  rw [nonempty_iff_univ_nonempty, nonempty_iff_ne_empty, Ne.def, not_iff_comm, ←
    union_empty ((L.Lhom_with_constants α).onTheory T), ← setOf_subset_eq_empty_iff]
  simp
...
theorem interᵢ_setOf_subset {ι : Type _} (S : ι → L[[α]].Theory) :
  ...
  by
  ext
  simp only [mem_interᵢ, mem_setOf_eq, unionᵢ_subset_iff]
