...
instance setoidPrestructure : L.Prestructure ((u : Filter α).productSetoid M) :=
  {
    ...
    fun_equiv := fun n f x y xy =>
      by
      refine' mem_of_superset (interᵢ_mem.2 xy) fun a ha => _
    ...
    rel_equiv := fun n r x y xy => by
      ...
      · refine' mem_of_superset (inter_mem hx (interᵢ_mem.2 xy)) _
        rintro a ⟨ha1, ha2⟩
        simp only [Set.mem_interᵢ, Set.mem_setOf_eq] at *
        rw [← funext ha2]
        exact ha1
      · refine' mem_of_superset (inter_mem hy (interᵢ_mem.2 xy)) _
        rintro a ⟨ha1, ha2⟩
        simp only [Set.mem_interᵢ, Set.mem_setOf_eq] at *
        rw [funext ha2]
        exact ha1 }
...
theorem term_realize_cast {β : Type _} (x : β → ∀ a, M a) (t : L.Term β) :
  ...
  convert
    @term.realize_quotient_mk L _ ((u : Filter α).productSetoid M)
      (Ultraproduct.setoidPrestructure M u) _ t x
  ext a
  induction t
  · rfl
...
theorem boundedFormula_realize_cast {β : Type _} {n : ℕ} (φ : L.BoundedFormula β n)
  ...
  · have h2 : ∀ a : α, (Sum.elim (fun i : β => x i a) fun i => v i a) = fun i => Sum.elim x v i a :=
    ...
    simp only [BoundedFormula.Realize, (Sum.comp_elim coe x v).symm, h2, term_realize_cast]
    exact Quotient.eq''
  · have h2 : ∀ a : α, (Sum.elim (fun i : β => x i a) fun i => v i a) = fun i => Sum.elim x v i a :=
      fun a => funext fun i => Sum.casesOn i (fun i => rfl) fun i => rfl
    simp only [BoundedFormula.Realize, (Sum.comp_elim coe x v).symm, term_realize_cast, h2]
    exact relMap_quotient_mk' _ _
