...
@[simp]
theorem realize_top : (⊤ : L.BoundedFormula α l).Realize v xs ↔ True := by simp [has_top.top]
...
@[simp]
theorem realize_iff : (φ.iff ψ).Realize v Exists ↔ (φ.Realize v xs ↔ ψ.Realize v xs) := by
...
theorem realize_liftAt {n n' m : ℕ} {φ : L.BoundedFormula α n} {v : α → M} {xs : Fin (n + n') → M}
  ...
  · simp [Realize, mapTermRel]
  · simp [Realize, mapTermRel, realize_rel, realize_liftAt, Sum.elim_comp_map]
  · simp [Realize, mapTermRel, realize_rel, realize_liftAt, Sum.elim_comp_map]
  ...
  · have h : k + 1 + n' = k + n' + 1 := by rw [add_assoc, add_comm 1 n', ← add_assoc]
    ...
    · simp only [Function.comp_apply, Fin.snoc_cast_succ]
      ...
      simp only [cast_refl, coe_castSucc, OrderIso.coe_refl, id.def]
      split_ifs <;> simp
...
theorem realize_liftAt_one {n m : ℕ} {φ : L.BoundedFormula α n} {v : α → M} {xs : Fin (n + 1) → M}
  ...
  by simp_rw [realize_liftAt (add_le_add_right hmn 1), castSucc, addNat_one]
...
theorem realize_constantsVarsEquiv [L[[α]].Structure M] [(lhomWithConstants L α).IsExpansionOn M]
  ...
  rw [←
    (lhomWithConstants L α).map_onRelation
...
theorem realize_toPrenexImpRight {φ ψ : L.BoundedFormula α n} (hφ : IsQf φ) (hψ : IsPrenex ψ)
  ...
  · rw [toPrenexImpRight, realize_ex]
...
theorem realize_toPrenex (φ : L.BoundedFormula α n) {v : α → M} :
  ...
  by
  refine'
    BoundedFormula.recOn φ (fun _ _ => Iff.rfl) (fun _ _ _ _ => Iff.rfl) (fun _ _ _ _ _ => Iff.rfl)
      (fun _ f1 f2 h1 h2 _ => _) fun _ f h xs => _
  · rw [toPrenex, realize_toPrenexImp f1.to_prenex_is_prenex f2.to_prenex_is_prenex, realize_imp,
      realize_imp, h1, h2]
    infer_instance
...
theorem realize_onBoundedFormula [L'.Structure M] (φ : L →ᴸ L') [φ.IsExpansionOn M] {n : ℕ}
  ...
  · simp only [onBoundedFormula, realize_bdEqual, realize_onTerm]
    rfl
  · simp only [onBoundedFormula, realize_rel, realize_onTerm, Lhom.map_onRelation]
    rfl
...
theorem realize_equivSentence_symm_con [L[[α]].Structure M]
  ...
  simp only [equivSentence, Equiv.symm_symm, Equiv.coe_trans, Realize,
    BoundedFormula.realize_relabelEquiv]
...
theorem realize_equivSentence [L[[α]].Structure M] [(L.lhomWithConstants α).IsExpansionOn M]
  ...
  rw [← realize_equivSentence_symm_con M (equivSentence φ), Equiv.symm_apply_apply]
...
theorem realize_iff_of_model_completeTheory [N ⊨ L.completeTheory M] (φ : L.Sentence) :
  ...
  exact (L.complete_theory M).realize_sentence_of_mem (mem_completeTheory.2 h)
  ...
  · rw [toFormula, Formula.Realize, realize_all, realize_all]
    ...
    · cases x
      ...
      · refine' Fin.lastCases _ (fun i => _) x
        ...
        · simp only [castSucc, Function.comp_apply, Sum.elim_inr, finSumFinEquiv_symm_apply_castAdd,
            Sum.map_inl, Sum.elim_inl]
          rw [← castSucc, snoc_cast_succ]
...
theorem realize_boundedFormula (g : M ≃[L] N) (φ : L.BoundedFormula α n) {v : α → M}
  ...
  · simp only [BoundedFormula.Realize, ← Sum.comp_elim, Equiv.realize_term, g.map_rel]
...
theorem model_distinctConstantsTheory {M : Type w} [L[[α]].Structure M] (s : Set α) :
  ...
  by
  simp only [distinctConstantsTheory, Theory.model_iff, Set.mem_image, Set.mem_inter, Set.mem_prod,
    Set.mem_compl, Prod.exists, forall_exists_index, and_imp]
