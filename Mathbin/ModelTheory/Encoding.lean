...
theorem card_sigma : (#Σn, L.term (Sum α (Finₓ n))) = max ℵ₀ (#Sum α (Σi, L.Functions i)) := by
  ...
  · rw [mk_sigma]
    ...
    rw [mk_nat, lift_aleph_0, mul_eq_max_of_aleph_0_le_left le_rflₓ, max_le_iff, csupr_le_iff' (bdd_above_range _)]
    · refine' ⟨le_max_leftₓ _ _, fun i => card_le.trans _⟩
      refine' max_leₓ (le_max_leftₓ _ _) _
      rw [← add_eq_max le_rflₓ, mk_sum, mk_sum, mk_sum, add_commₓ (Cardinal.lift (#α)), lift_add, add_assocₓ, lift_lift,
  ...
  · rintro ⟨n, φ⟩
    ...
    · rw [list_encode, cons_append, cons_append, singleton_append, cons_append, list_decode]
      · have h :
          ...
          by
          intro i
          simp only [Option.join, map_append, map_map, Option.bind_eq_someₓ, id.def, exists_eq_right, nth_eq_some,
            length_append, length_map, length_fin_range]
          refine' ⟨lt_of_lt_of_leₓ i.2 le_self_add, _⟩
          rw [nth_le_append, nth_le_map]
          · simp only [Sum.getLeft, nth_le_fin_range, Finₓ.eta, Function.comp_app, eq_self_iff_true, heq_iff_eq,
              and_selfₓ]
            
          · exact lt_of_lt_of_leₓ i.is_lt (ge_of_eqₓ (length_fin_range _))
...
theorem card_le : (#Σn, L.BoundedFormula α n) ≤ max ℵ₀ (Cardinal.lift.{max u v} (#α) + Cardinal.lift.{u'} L.card) := by
  ...
  rw [mk_sum, term.card_sigma, mk_sum, ← add_eq_max le_rflₓ, mk_sum, mk_nat]
  simp only [lift_add, lift_lift, lift_aleph_0]
  rw [← add_assocₓ, add_commₓ, ← add_assocₓ, ← add_assocₓ, aleph_0_add_aleph_0, add_assocₓ, add_eq_max le_rflₓ,
