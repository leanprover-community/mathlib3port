...
theorem listDecode_encode_list (l : List (L.Term α)) :
  ...
  · intro t
    ...
    · rw [listEncode, cons_append, listDecode]
      have h :
        listDecode (((finRange n).bind fun i : Fin n => (ts i).listEncode) ++ l) =
          (finRange n).map (Option.some ∘ ts) ++ listDecode l :=
        by
...
theorem card_sigma : (#Σn, L.Term (Sum α (Fin n))) = max ℵ₀ (#Sum α (Σi, L.Functions i)) :=
  ...
  · rw [mk_sigma]
    ...
    · rw [← one_le_iff_ne_zero]
      refine' trans _ (le_csupᵢ (bddAbove_range _) 1)
...
theorem listDecode_encode_list (l : List (Σn, L.BoundedFormula α n)) :
  ...
  · induction' l with φ l lih
    · rw [List.nil_bind]
      simp [listDecode]
    · rw [cons_bind, (h φ _).1, head!_cons]
  · rintro ⟨n, φ⟩
    ...
    · rw [listEncode, cons_append, cons_append, singleton_append, cons_append, listDecode]
      · have h :
          ...
          rw [nthLe_append, nthLe_map]
          ·
            simp only [Sum.getLeft, nthLe_finRange, Fin.eta, Function.comp_apply, eq_self_iff_true,
              heq_iff_eq, and_self_iff]
          · exact lt_of_lt_of_le i.is_lt (ge_of_eq (length_finRange _))
          · rw [length_map, length_finRange]
            exact i.2
        rw [dif_pos]
        swap
    ...
    · rw [listEncode, List.append_assoc, cons_append, listDecode]
      ...
      rw [(ih1 _).1, (ih1 _).2, (ih2 _).1, (ih2 _).2, sigmaImp, dif_pos rfl]
      exact ⟨rfl, rfl⟩
    · rw [listEncode, cons_append, listDecode]
      simp only
      simp only [Subtype.val_eq_coe] at *
