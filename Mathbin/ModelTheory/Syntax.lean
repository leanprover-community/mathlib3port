...
/- ./././Mathport/Syntax/Translate/Command.lean:330:30: infer kinds are unsupported in Lean 4: var {} -/
/- ./././Mathport/Syntax/Translate/Command.lean:330:30: infer kinds are unsupported in Lean 4: func {} -/
...
def restrictVarLeft [DecidableEq α] {γ : Type _} : ∀ (t : L.term (α ⊕ γ)) (f : t.varFinsetLeft → β), L.term (β ⊕ γ)
...
@[simp]
theorem constants_vars_equiv_left_apply (t : L[[γ]].term (α ⊕ β)) :
...
@[simp]
theorem constants_vars_equiv_left_symm_apply (t : L.term ((γ ⊕ α) ⊕ β)) :
...
#align first_order.language.Lhom.on_term FirstOrder.Language.LhomCat.onTerm
...
#align first_order.language.Lhom.id_on_term FirstOrder.Language.LhomCat.id_on_term
...
#align first_order.language.Lhom.comp_on_term FirstOrder.Language.LhomCat.comp_on_term
...
def LequivCat.onTerm (φ : L ≃ᴸ L') : L.term α ≃ L'.term α where
  ...
  left_inv := by rw [Function.leftInverse_iff_comp, ← Lhom.comp_on_term, φ.left_inv, Lhom.id_on_term]
  right_inv := by rw [Function.rightInverse_iff_comp, ← Lhom.comp_on_term, φ.right_inv, Lhom.id_on_term]
#align first_order.language.Lequiv.on_term FirstOrder.Language.LequivCat.onTerm

variable (L) (α)

/- ./././Mathport/Syntax/Translate/Command.lean:330:30: infer kinds are unsupported in Lean 4: falsum {} -/
...
def TheoryCat :=
  Set L.Sentence
#align first_order.language.Theory FirstOrder.Language.TheoryCat

variable {L} {α} {n : ℕ}

/-- Applies a relation to terms as a bounded formula. -/
def Relations.boundedFormula {l : ℕ} (R : L.Relations n) (ts : Fin n → L.term (α ⊕ Fin l)) : L.BoundedFormula α l :=
...
def Relations.boundedFormula₁ (r : L.Relations 1) (t : L.term (α ⊕ Fin n)) : L.BoundedFormula α n :=
  r.BoundedFormula ![t]
#align first_order.language.relations.bounded_formula₁ FirstOrder.Language.Relations.boundedFormula₁

/-- Applies a binary relation to two terms as a bounded formula. -/
def Relations.boundedFormula₂ (r : L.Relations 2) (t₁ t₂ : L.term (α ⊕ Fin n)) : L.BoundedFormula α n :=
...
@[simp]
theorem map_term_rel_map_term_rel {L'' : Language} (ft : ∀ n, L.term (α ⊕ Fin n) → L'.term (β ⊕ Fin n))
...
@[simp]
theorem sum_elim_comp_relabel_aux {m : ℕ} {g : α → β ⊕ Fin n} {v : β → M} {xs : Fin (n + m) → M} :
...
@[simp]
theorem relabel_aux_sum_inl (k : ℕ) : relabelAux (Sum.inl : α → α ⊕ Fin n) k = Sum.map id (natAdd n) := by
  ext x
  cases x <;>
    · simp [relabel_aux]
...
@[simp]
theorem relabel_falsum (g : α → β ⊕ Fin n) {k} : (falsum : L.BoundedFormula α k).relabel g = falsum :=
  rfl
...
@[simp]
theorem relabel_not (g : α → β ⊕ Fin n) {k} (φ : L.BoundedFormula α k) : φ.Not.relabel g = (φ.relabel g).Not := by
...
@[simp]
theorem relabel_all (g : α → β ⊕ Fin n) {k} (φ : L.BoundedFormula α (k + 1)) : φ.all.relabel g = (φ.relabel g).all := by
...
inductive IsAtomic : L.BoundedFormula α n → Prop
  | equal (t₁ t₂ : L.term (α ⊕ Fin n)) : IsAtomic (bdEqual t₁ t₂)
  | rel {l : ℕ} (R : L.Relations l) (ts : Fin l → L.term (α ⊕ Fin n)) : IsAtomic (R.BoundedFormula ts)
...
theorem IsAtomic.relabel {m : ℕ} {φ : L.BoundedFormula α m} (h : φ.IsAtomic) (f : α → β ⊕ Fin n) :
...
theorem is_qf_bot : IsQf (⊥ : L.BoundedFormula α n) :=
  is_qf.falsum
#align first_order.language.bounded_formula.is_qf_bot FirstOrder.Language.BoundedFormula.is_qf_bot
...
theorem IsQf.relabel {m : ℕ} {φ : L.BoundedFormula α m} (h : φ.IsQf) (f : α → β ⊕ Fin n) : (φ.relabel f).IsQf :=
...
theorem IsPrenex.relabel {m : ℕ} {φ : L.BoundedFormula α m} (h : φ.IsPrenex) (f : α → β ⊕ Fin n) :
...
#align first_order.language.Lhom.on_bounded_formula FirstOrder.Language.LhomCat.onBoundedFormula
...
def onFormula (g : L →ᴸ L') : L.Formula α → L'.Formula α :=
  g.onBoundedFormula
#align first_order.language.Lhom.on_formula FirstOrder.Language.LhomCat.onFormula
...
def onBoundedFormula (φ : L ≃ᴸ L') : L.BoundedFormula α n ≃ L'.BoundedFormula α n where
  ...
  left_inv := by
    rw [Function.leftInverse_iff_comp, ← Lhom.comp_on_bounded_formula, φ.left_inv, Lhom.id_on_bounded_formula]
  right_inv := by
    rw [Function.rightInverse_iff_comp, ← Lhom.comp_on_bounded_formula, φ.right_inv, Lhom.id_on_bounded_formula]
#align first_order.language.Lequiv.on_bounded_formula FirstOrder.Language.LequivCat.onBoundedFormula
...
def onFormula (φ : L ≃ᴸ L') : L.Formula α ≃ L'.Formula α :=
  φ.onBoundedFormula
#align first_order.language.Lequiv.on_formula FirstOrder.Language.LequivCat.onFormula

@[simp]
theorem on_formula_apply (φ : L ≃ᴸ L') : (φ.onFormula : L.Formula α → L'.Formula α) = φ.toLhom.onFormula :=
  rfl
#align first_order.language.Lequiv.on_formula_apply FirstOrder.Language.LequivCat.on_formula_apply

@[simp]
theorem on_formula_symm (φ : L ≃ᴸ L') : (φ.onFormula.symm : L'.Formula α ≃ L.Formula α) = φ.symm.onFormula :=
  rfl
#align first_order.language.Lequiv.on_formula_symm FirstOrder.Language.LequivCat.on_formula_symm

/-- Maps a sentence's symbols along a language equivalence. -/
@[simps]
def onSentence (φ : L ≃ᴸ L') : L.Sentence ≃ L'.Sentence :=
  φ.onFormula
#align first_order.language.Lequiv.on_sentence FirstOrder.Language.LequivCat.onSentence
