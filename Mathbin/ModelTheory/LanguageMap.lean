...
instance [L.IsAlgebraic] [L.IsRelational] : Unique (L →ᴸ L') :=
  ⟨⟨LhomCat.ofIsEmpty L L'⟩, fun _ => LhomCat.funext (Subsingleton.elim _ _) (Subsingleton.elim _ _)⟩
...
@[simp]
theorem id_comp (F : L →ᴸ L') : LhomCat.id L' ∘ F = F := by
  cases F
  rfl
#align first_order.language.Lhom.id_comp FirstOrder.Language.LhomCat.id_comp

@[simp]
theorem comp_id (F : L →ᴸ L') : F ∘ LhomCat.id L = F := by
  cases F
  rfl
...
class IsExpansionOn (M : Type _) [L.StructureCat M] [L'.StructureCat M] : Prop where
  map_on_function : ∀ {n} (f : L.Functions n) (x : Fin n → M), funMap (ϕ.onFunction f) x = funMap f x
  map_on_relation : ∀ {n} (R : L.Relations n) (x : Fin n → M), RelMap (ϕ.onRelation R) x = RelMap R x
#align first_order.language.Lhom.is_expansion_on FirstOrder.Language.LhomCat.IsExpansionOn

@[simp]
theorem map_on_function {M : Type _} [L.StructureCat M] [L'.StructureCat M] [ϕ.IsExpansionOn M] {n} (f : L.Functions n)
    (x : Fin n → M) : funMap (ϕ.onFunction f) x = funMap f x :=
...
@[simp]
theorem map_on_relation {M : Type _} [L.StructureCat M] [L'.StructureCat M] [ϕ.IsExpansionOn M] {n} (R : L.Relations n)
    (x : Fin n → M) : RelMap (ϕ.onRelation R) x = RelMap R x :=
...
instance of_is_empty_is_expansion_on (M : Type _) [L.StructureCat M] [L'.StructureCat M] [L.IsAlgebraic]
...
instance sum_elim_is_expansion_on {L'' : Language} (ψ : L'' →ᴸ L') (M : Type _) [L.StructureCat M] [L'.StructureCat M]
    [L''.StructureCat M] [ϕ.IsExpansionOn M] [ψ.IsExpansionOn M] : (ϕ.sum_elim ψ).IsExpansionOn M :=
...
instance sum_map_is_expansion_on {L₁ L₂ : Language} (ψ : L₁ →ᴸ L₂) (M : Type _) [L.StructureCat M] [L'.StructureCat M]
    [L₁.StructureCat M] [L₂.StructureCat M] [ϕ.IsExpansionOn M] [ψ.IsExpansionOn M] : (ϕ.sum_map ψ).IsExpansionOn M :=
  ⟨fun _ f _ => Sum.casesOn f (by simp) (by simp), fun _ R _ => Sum.casesOn R (by simp) (by simp)⟩
...
@[simp]
theorem fun_map_sum_inl [(L.Sum L').StructureCat M] [(LhomCat.sumInl : L →ᴸ L.Sum L').IsExpansionOn M] {n}
...
@[simp]
theorem fun_map_sum_inr [(L'.Sum L).StructureCat M] [(LhomCat.sumInr : L →ᴸ L'.Sum L).IsExpansionOn M] {n}
...
#align first_order.language.Lequiv.add_empty_constants FirstOrder.Language.LequivCat.addEmptyConstants

variable {α} {β : Type _}

@[simp]
theorem with_constants_fun_map_sum_inl [L[[α]].StructureCat M] [(lhomWithConstants L α).IsExpansionOn M] {n}
    {f : L.Functions n} {x : Fin n → M} : @funMap (L[[α]]) M _ n (Sum.inl f) x = funMap f x :=
...
@[simp]
theorem with_constants_rel_map_sum_inl [L[[α]].StructureCat M] [(lhomWithConstants L α).IsExpansionOn M] {n}
    {R : L.Relations n} {x : Fin n → M} : @RelMap (L[[α]]) M _ n (Sum.inl R) x = RelMap R x :=
...
instance add_empty_constants_is_expansion_on' : (LequivCat.addEmptyConstants L (∅ : Set M)).toLhom.IsExpansionOn M :=
...
instance add_constants_expansion {L' : Language} [L'.StructureCat M] (φ : L →ᴸ L') [φ.IsExpansionOn M] :
...
instance constants_on_map_inclusion_is_expansion_on : (LhomCat.constantsOnMap (Set.inclusion h)).IsExpansionOn M :=
