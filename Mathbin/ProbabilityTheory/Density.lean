...
theorem has_pdf_of_pdf_ne_zero {m : MeasurableSpace α} {ℙ : Measureₓ α} {μ : Measureₓ E} {X : α → E}
    (h : pdf X ℙ μ ≠ 0) : has_pdf X ℙ μ := by
  by_contra hpdf
...
theorem map_eq_with_density_pdf {m : MeasurableSpace α} (X : α → E) (ℙ : Measureₓ α)
    (μ : Measureₓ E := by
      run_tac
        volume_tac)
    [hX : has_pdf X ℙ μ] : measure.map X ℙ = μ.with_density (pdf X ℙ μ) := by
  rw [pdf, dif_pos hX]
...
theorem map_eq_set_lintegral_pdf {m : MeasurableSpace α} (X : α → E) (ℙ : Measureₓ α)
    (μ : Measureₓ E := by
      run_tac
        volume_tac)
    [hX : has_pdf X ℙ μ] {s : Set E} (hs : MeasurableSet s) : measure.map X ℙ s = ∫⁻ x in s, pdf X ℙ μ x ∂μ := by
...
theorem lintegral_eq_measure_univ {X : α → E} [has_pdf X ℙ μ] : (∫⁻ x, pdf X ℙ μ x ∂μ) = ℙ Set.Univ := by
...
theorem ae_lt_top [is_finite_measure ℙ] {μ : Measureₓ E} {X : α → E} : ∀ᵐ x ∂μ, pdf X ℙ μ x < ∞ := by
...
theorem map_absolutely_continuous {X : α → E} [has_pdf X ℙ μ] : map X ℙ ≪ μ := by
...
theorem has_pdf_iff_of_measurable {X : α → E} (hX : Measurable X) :
    has_pdf X ℙ μ ↔ (map X ℙ).HaveLebesgueDecomposition μ ∧ map X ℙ ≪ μ := by
  rw [has_pdf_iff]
...
theorem quasi_measure_preserving_has_pdf {X : α → E} [has_pdf X ℙ μ] {g : E → F} (hg : quasi_measure_preserving g μ ν)
    (hmap : (map g (map X ℙ)).HaveLebesgueDecomposition ν) : has_pdf (g ∘ X) ℙ ν := by
...
theorem real.has_pdf_iff_of_measurable (hX : Measurable X) : has_pdf X ℙ ↔ map X ℙ ≪ volume := by
  rw [has_pdf_iff_of_measurable hX, and_iff_right_iff_imp]
  exact fun h => inferInstance

theorem real.has_pdf_iff : has_pdf X ℙ ↔ Measurable X ∧ map X ℙ ≪ volume := by
...
theorem has_finite_integral_mul {f : ℝ → ℝ} {g : ℝ → ℝ≥0∞} (hg : pdf X ℙ =ᵐ[volume] g)
    (hgi : (∫⁻ x, ∥f x∥₊ * g x) ≠ ∞) : has_finite_integral fun x => f x * (pdf X ℙ volume x).toReal := by
  rw [has_finite_integral]
