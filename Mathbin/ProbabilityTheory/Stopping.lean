...
class sigma_finite_filtration (μ : Measureₓ α) (f : filtration ι m) : Prop where
  SigmaFinite : ∀ i : ι, sigma_finite (μ.trim (f.le i))

instance sigma_finite_of_sigma_finite_filtration (μ : Measureₓ α) (f : filtration ι m)
    [hf : sigma_finite_filtration μ f] (i : ι) : sigma_finite (μ.trim (f.le i)) := by
...
theorem adapted_natural {u : ι → α → β} (hum : ∀ i, measurable[m] (u i)) : adapted (natural u hum) u := fun i =>
  Measurable.le (le_bsupr_of_le i (le_reflₓ i) (le_reflₓ _)) fun s hs => ⟨s, hs, rfl⟩
...
theorem is_stopping_time.measurable_set_le (hτ : is_stopping_time f τ) (i : ℕ) : measurable_set[f i] { x | τ x ≤ i } :=
  hτ i

theorem is_stopping_time.measurable_set_eq (hτ : is_stopping_time f τ) (i : ℕ) : measurable_set[f i] { x | τ x = i } :=
  by
  cases i
  · convert hτ 0
...
theorem is_stopping_time.measurable_set_ge (hτ : is_stopping_time f τ) (i : ℕ) : measurable_set[f i] { x | i ≤ τ x } :=
  by
...
theorem is_stopping_time_const {f : filtration ι m} (i : ι) : is_stopping_time f fun x => i := fun j => by
  simp
...
theorem max [LinearOrderₓ ι] {f : filtration ι m} {τ π : α → ι} (hτ : is_stopping_time f τ)
    (hπ : is_stopping_time f π) : is_stopping_time f fun x => max (τ x) (π x) := by
  intro i
  simp_rw [max_le_iff, Set.set_of_and]
  exact (hτ i).inter (hπ i)

theorem min [LinearOrderₓ ι] {f : filtration ι m} {τ π : α → ι} (hτ : is_stopping_time f τ)
    (hπ : is_stopping_time f π) : is_stopping_time f fun x => min (τ x) (π x) := by
  intro i
  simp_rw [min_le_iff, Set.set_of_or]
  exact (hτ i).union (hπ i)

theorem add_const [AddGroupₓ ι] [Preorderₓ ι] [CovariantClass ι ι (Function.swap (· + ·)) (· ≤ ·)]
    [CovariantClass ι ι (· + ·) (· ≤ ·)] {f : filtration ι m} {τ : α → ι} (hτ : is_stopping_time f τ) {i : ι}
    (hi : 0 ≤ i) : is_stopping_time f fun x => τ x + i := by
  intro j
...
protected def MeasurableSpace {τ : α → ι} (hτ : is_stopping_time f τ) : MeasurableSpace α where
...
@[protected]
theorem MeasurableSet {τ : α → ι} (hτ : is_stopping_time f τ) (s : Set α) :
    measurable_set[hτ.measurable_space] s ↔ ∀ i : ι, measurable_set[f i] (s ∩ { x | τ x ≤ i }) :=
  Iff.rfl

theorem measurable_space_mono {τ π : α → ι} (hτ : is_stopping_time f τ) (hπ : is_stopping_time f π) (hle : τ ≤ π) :
...
theorem measurable_space_le [Encodable ι] {τ : α → ι} (hτ : is_stopping_time f τ) : hτ.measurable_space ≤ m := by
  intro s hs
  ...
  · ext x
    ...
    · exact fun hx => ⟨τ x, hx, le_reflₓ _⟩
...
theorem measurable_set_eq_const {f : filtration ℕ m} {τ : α → ℕ} (hτ : is_stopping_time f τ) (i : ℕ) :
    measurable_set[hτ.measurable_space] { x | τ x = i } := by
...
theorem Measurable [TopologicalSpace ι] [MeasurableSpace ι] [BorelSpace ι] [OrderTopology ι]
    [second_countable_topology ι] {f : filtration ι m} {τ : α → ι} (hτ : is_stopping_time f τ) :
    measurable[hτ.measurable_space] τ := by
...
theorem adapted.stopped_process [MeasurableSpace β] [HasMeasurableAdd₂ β] (hu : adapted f u)
    (hτ : is_stopping_time f τ) : adapted f (stopped_process u τ) := by
  intro i
...
theorem measurable_stopped_process (hτ : is_stopping_time f τ) (hu : adapted f u) (n : ℕ) :
    Measurable (stopped_process u τ n) :=
  (hu.stopped_process hτ n).le (f.le _)

theorem mem_ℒp_stopped_process {p : ℝ≥0∞} [BorelSpace β] {μ : Measureₓ α} (hτ : is_stopping_time f τ)
    (hu : ∀ n, mem_ℒp (u n) p μ) (n : ℕ) : mem_ℒp (stopped_process u τ n) p μ := by
  rw [stopped_process_eq]
...
theorem integrable_stopped_process [BorelSpace β] {μ : Measureₓ α} (hτ : is_stopping_time f τ)
