...
theorem lintegral_mul_indicator_eq_lintegral_mul_lintegral_indicator {Mf : MeasurableSpace α} [M : MeasurableSpace α]
  ...
  (h_ind : indep_sets Mf.measurable_set' {T} μ) {f : α → ℝ≥0∞} (h_meas_f : @Measurable α ℝ≥0∞ Mf _ f) :
  (∫⁻ a, f a*T.indicator (fun _ => c) a ∂μ) = (∫⁻ a, f a ∂μ)*∫⁻ a, T.indicator (fun _ => c) a ∂μ :=
  by 
    revert f 
    have h_mul_indicator : ∀ g, Measurable g → Measurable fun a => g a*T.indicator (fun x => c) a 
    exact fun g h_mg => h_mg.mul (measurable_const.indicator h_meas_T)
    apply Measurable.ennreal_induction
    ·
      intro c' s' h_meas_s' 
      simpRw [←inter_indicator_mul]
    ...
    ·
      ...
      exact h_meas_f'.mono hMf le_rfl 
      have h_measM_g : Measurable g 
      exact h_meas_g.mono hMf le_rfl 
      simpRw [Pi.add_apply, right_distrib]
    ...
    ·
      ...
      exact fun n => (h_meas_f n).mono hMf le_rfl 
      simpRw [Ennreal.supr_mul]
...
theorem lintegral_mul_eq_lintegral_mul_lintegral_of_independent_measurable_space {Mf Mg : MeasurableSpace α}
  [M : MeasurableSpace α] {μ : Measureₓ α} (hMf : Mf ≤ M) (hMg : Mg ≤ M) (h_ind : indep Mf Mg μ) {f g : α → ℝ≥0∞}
  ...
  by 
    ...
    ·
      ...
      have h_measM_f' : Measurable f' 
      exact h_measMg_f'.mono hMg le_rfl 
      have h_measM_g : Measurable g 
      exact h_measMg_g.mono hMg le_rfl 
      simpRw [Pi.add_apply, left_distrib]
    ...
    ·
      intro f' h_meas_f' h_mono_f' h_ind_f' 
      have h_measM_f' : ∀ n, Measurable (f' n)
      exact fun n => (h_meas_f' n).mono hMg le_rfl 
      simpRw [Ennreal.mul_supr]
