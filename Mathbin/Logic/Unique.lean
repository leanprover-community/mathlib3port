...
instance PUnit.unique : Unique PUnit .{ u } where default := PUnit.unit uniq x := punit_eq x _
...
/-- There is a unique function on an empty domain. -/
  instance
    Pi.uniqueOfIsEmpty
    [ IsEmpty α ] ( β : ∀ a : α , Sort v ) : Unique ( ∀ a , β a )
    where default := isEmptyElim uniq f := funext isEmptyElim
...
instance
  Unique.subtypeEq
  ( y : α ) : Unique { x // x = y }
  where default := ⟨ y , rfl ⟩ uniq ⟨ x , hx ⟩ := by simpa using hx

-- failed to format: format: uncaught backtrack exception
instance
  Unique.subtypeEq'
  ( y : α ) : Unique { x // y = x }
  where default := ⟨ y , rfl ⟩ uniq ⟨ x , hx ⟩ := by simpa using hx.symm
