...
theorem bi_total.rel_forall (h : bi_total R) : ((R⇒Iff)⇒Iff) (fun p => ∀ i, p i) fun q => ∀ i, q i :=
  fun p q Hrel =>
    ⟨fun H b => Exists.elim (h.right b) fun a Rab => (Hrel Rab).mp (H _),
      fun H a => Exists.elim (h.left a) fun b Rab => (Hrel Rab).mpr (H _)⟩

theorem bi_total.rel_exists (h : bi_total R) : ((R⇒Iff)⇒Iff) (fun p => ∃ i, p i) fun q => ∃ i, q i :=
  fun p q Hrel =>
...
theorem rel_imp : (Iff⇒Iff⇒Iff) Implies Implies :=
  fun p q h r s l => imp_congr h l

theorem rel_not : (Iff⇒Iff) Not Not :=
  fun p q h => not_congr h
...
theorem rel_and : ((· ↔ ·)⇒(· ↔ ·)⇒(· ↔ ·)) (· ∧ ·) (· ∧ ·) :=
  fun a b h₁ c d h₂ => and_congr h₁ h₂

theorem rel_or : ((· ↔ ·)⇒(· ↔ ·)⇒(· ↔ ·)) (· ∨ ·) (· ∨ ·) :=
  fun a b h₁ c d h₂ => or_congr h₁ h₂

theorem rel_iff : ((· ↔ ·)⇒(· ↔ ·)⇒(· ↔ ·)) (· ↔ ·) (· ↔ ·) :=
  fun a b h₁ c d h₂ => iff_congr h₁ h₂

theorem rel_eq {r : α → β → Prop} (hr : bi_unique r) : (r⇒r⇒(· ↔ ·)) (· = ·) (· = ·) :=
  fun a b h₁ c d h₂ =>
    Iff.intro
      (by 
        intro h 
