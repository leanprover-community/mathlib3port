...
@[toAdditive "The composition of addition preserving maps also preserves addition"]
...
@[toAdditive "Interpret a map `f : M → N` as a homomorphism `M →+ N`."]
...
@[toAdditive Neg.is_add_group_hom "Negation is an `add_group` homomorphism if the `add_group` is commutative."]
...
theorem map_sub a b : f (a - b) = f a - f b :=
  calc f (a - b) = f (a+-b) := congr_argₓ f (sub_eq_add_neg a b)
...
theorem IsAddGroupHom.sub {α β} [AddGroupₓ α] [AddCommGroupₓ β] {f g : α → β} (hf : IsAddGroupHom f)
  (hg : IsAddGroupHom g) : IsAddGroupHom fun a => f a - g a :=
  by 
...
@[reducible]
def map' {f : M → N} (hf : IsMonoidHom f) : Units M →* Units N :=
  map (MonoidHom.of hf)
