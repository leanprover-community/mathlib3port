...
instance  [CommRingₓ S] [Algebra S R] : Algebra S (AdjoinRoot f) :=
  Ideal.Quotient.algebra S

instance  [CommRingₓ S] [CommRingₓ K] [HasScalar S K] [Algebra S R] [Algebra K R] [IsScalarTower S K R] :
  IsScalarTower S K (AdjoinRoot f) :=
  Submodule.Quotient.is_scalar_tower _ _

instance  [CommRingₓ S] [CommRingₓ K] [Algebra S R] [Algebra K R] [SmulCommClass S K R] :
...
theorem algebra_map_eq' [CommRingₓ S] [Algebra S R] : algebraMap S (AdjoinRoot f) = (of f).comp (algebraMap S R) :=
  rfl
...
def power_basis_aux (hf : f ≠ 0) : Basis (Finₓ f.nat_degree) K (AdjoinRoot f) :=
  by 
