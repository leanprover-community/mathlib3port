...
variable{R :
    Type u}{S : Type v}{T : Type w}[NonAssocSemiring R][NonAssocSemiring S][NonAssocSemiring T](M : Submonoid R)
...
@[mono]
theorem to_submonoid_strict_mono : StrictMono (to_submonoid : Subsemiring R → Submonoid R) :=
  fun _ _ => id
...
@[mono]
theorem to_add_submonoid_strict_mono : StrictMono (to_add_submonoid : Subsemiring R → AddSubmonoid R) :=
  fun _ _ => id
...
instance NoZeroDivisors [NoZeroDivisors R] : NoZeroDivisors s :=
  { eq_zero_or_eq_zero_of_mul_eq_zero :=
      fun x y h =>
        Or.cases_on (eq_zero_or_eq_zero_of_mul_eq_zero$ Subtype.ext_iff.mp h) (fun h => Or.inl$ Subtype.eq h)
...
@[simp, normCast]
theorem coe_pow {R} [Semiringₓ R] (s : Subsemiring R) (x : s) (n : ℕ) : ((x ^ n : s) : R) = (x ^ n : R) :=
  by 
...
theorem map_srange : f.srange.map g = (g.comp f).srange :=
  by 
...
instance  : CompleteLattice (Subsemiring R) :=
  { completeLatticeOfInf (Subsemiring R)
...
instance decidable_mem_center {R} [Semiringₓ R] [DecidableEq R] [Fintype R] : DecidablePred (· ∈ center R) :=
...
theorem coe_closure_eq (s : Set R) : (closure s : Set R) = AddSubmonoid.closure (Submonoid.closure s : Set R) :=
  by 
...
theorem mem_closure_iff_exists_list {R} [Semiringₓ R] {s : Set R} {x} :
  ...
  ⟨fun hx =>
      AddSubmonoid.closure_induction (mem_closure_iff.1 hx)
        (fun x hx =>
          suffices ∃ t : List R, (∀ y (_ : y ∈ t), y ∈ s) ∧ t.prod = x from
            let ⟨t, ht1, ht2⟩ := this
            ⟨[t], List.forall_mem_singleton.2 ht1,
              by 
          ...
          Submonoid.closure_induction hx (fun x hx => ⟨[x], List.forall_mem_singleton.2 hx, one_mulₓ x⟩)
            ...
            fun x y ⟨t, ht1, ht2⟩ ⟨u, hu1, hu2⟩ =>
              ⟨t ++ u, List.forall_mem_appendₓ.2 ⟨ht1, hu1⟩,
                by 
                  rw [List.prod_append, ht2, hu2]⟩)
        ⟨[], List.forall_mem_nil _, rfl⟩
        fun x y ⟨L, HL1, HL2⟩ ⟨M, HM1, HM2⟩ =>
          ⟨L ++ M, List.forall_mem_appendₓ.2 ⟨HL1, HM1⟩,
            by 
...
-- error in RingTheory.Subsemiring.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
theorem prod_mono_right (s : subsemiring R) : monotone (λ t : subsemiring S, s.prod t) := prod_mono (le_refl s)

-- error in RingTheory.Subsemiring.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
theorem prod_mono_left (t : subsemiring S) : monotone (λ s : subsemiring R, s.prod t) :=
...
theorem srange_restrict_surjective (f : R →+* S) : Function.Surjective f.srange_restrict :=
  fun ⟨y, hy⟩ =>
...
{ carrier := {x | «expr < »(0, x)},
  one_mem' := show «expr < »((0 : R), 1), from zero_lt_one,
  mul_mem' := λ (x y) (hx : «expr < »(0, x)) (hy : «expr < »(0, y)), mul_pos hx hy }
