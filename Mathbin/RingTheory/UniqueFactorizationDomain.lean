...
theorem prime_factors_irreducible [CancelCommMonoidWithZero α] {a : α} {f : Multiset α} (ha : Irreducible a)
  (pfa : (∀ b _ : b ∈ f, Prime b) ∧ f.prod ~ᵤ a) : ∃ p, a ~ᵤ p ∧ f = {p} :=
  by 
    have  := Classical.decEq α 
    refine'
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ∈ » factors a)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (q «expr ∈ » normalized_factors a)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ≠ » (0 : R))
...
-/
@[reducible]
def factor_set.{u} (α : Type u) [CancelCommMonoidWithZero α] : Type u :=
...
@[nontriviality]
theorem factors_subsingleton [Subsingleton α] {a : Associates α} : a.factors = Option.none :=
  by 
...
theorem factors_eq_none_iff_zero {a : Associates α} : a.factors = Option.none ↔ a = 0 :=
  by 
    ...
    exact
      ⟨fun h =>
          by 
            rwa [←factors_prod a, factor_set.prod_eq_zero_iff],
        fun h => h.symm ▸ factors_0⟩
...
noncomputable def fintype_subtype_dvd {M : Type _} [CancelCommMonoidWithZero M] [UniqueFactorizationMonoid M]
