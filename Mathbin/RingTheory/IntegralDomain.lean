...
-- error in RingTheory.IntegralDomain: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
...
theorem sum_hom_units_eq_zero (f : G →* R) (hf : f ≠ 1) : (∑g : G, f g) = 0 :=
  by 
    ...
    calc (∑g : G, f g) = ∑g : G, f.to_hom_units g :=
      rfl _ = ∑u : Units R in univ.image f.to_hom_units, (univ.filter fun g => f.to_hom_units g = u).card • u :=
      sum_comp (coeₓ : Units R → R) f.to_hom_units _ = ∑u : Units R in univ.image f.to_hom_units, c • u :=
      sum_congr rfl fun u hu => congr_arg2 _ _ rfl _ = ∑b : MonoidHom.range f.to_hom_units, c • «expr↑ » b :=
      Finset.sum_subtype _
        (by 
          simp )
        _ _ = c • ∑b : MonoidHom.range f.to_hom_units, (b : R) :=
      smul_sum.symm _ = c • 0 := congr_arg2 _ rfl _ _ = 0 := smul_zero _
    ·
    ...
    calc (∑b : MonoidHom.range f.to_hom_units, (b : R)) = ∑n in range (orderOf x), x ^ n :=
      Eq.symm$
        sum_bij (fun n _ => x ^ n)
          ...
          fun b hb =>
            let ⟨n, hn⟩ := hx b
            ⟨n % orderOf x, mem_range.2 (Nat.mod_ltₓ _ (order_of_pos _)),
              by 
