...
natural properties. -/
@[elab_as_elim]
theorem adjoinInduction₂ {p : A → A → Prop} {a b : A} (ha : a ∈ adjoin R s) (hb : b ∈ adjoin R s)
...
theorem adjoinInduction' {p : adjoin R s → Prop} (Hs : ∀ (x) (h : x ∈ s), p ⟨x, subset_adjoin h⟩)
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (z r) -/
theorem adjoin_eq_span : (adjoin R s).toSubmodule = span R (Submonoid.closure s) := by
  ...
  · intro r hr
    ...
    replace HL := HL.1
    clear ih tl
    suffices ∃ (z) (r) (hr : r ∈ Submonoid.closure s), HasSmul.smul z r = List.prod hd by
...
theorem adjoin_inl_union_inr_eq_prod (s) (t) :
  ...
  · simp only [adjoin_le_iff, Set.insert_subset, Subalgebra.zero_mem, Subalgebra.one_mem,
      ...
      LinearMap.coe_inr, and_self_iff, Set.union_singleton, Subalgebra.coe_prod]
...
semiring.  -/
def adjoinCommSemiringOfComm {s : Set A} (hcomm : ∀ a ∈ s, ∀ b ∈ s, a * b = b * a) : CommSemiring (adjoin R s) :=
    ...
    mul_comm := fun x y => by
      ...
      exact
        adjoin_induction₂ x.prop y.prop hcomm (fun _ _ => by rw [commutes]) (fun r x hx => commutes r x)
          (fun r x hx => (commutes r x).symm) (fun _ _ _ h₁ h₂ => by simp only [add_mul, mul_add, h₁, h₂])
...
theorem adjoin_adjoin_of_tower [Semiring B] [Algebra R B] [Algebra A B] [IsScalarTower R A B] (s : Set B) :
    adjoin A (adjoin R s : Set B) = adjoin A s := by
...
theorem pow_smul_mem_of_smul_subset_of_mem_adjoin [CommSemiring B] [Algebra R B] [Algebra A B] [IsScalarTower R A B]
  ...
  dsimp only
  rw [← tsub_add_cancel_of_le this, pow_add, ← smul_smul, ← IsScalarTower.algebra_map_smul A (l a) (a : B),
...
/-- If all elements of `s : set A` commute pairwise, then `adjoin R s` is a commutative
ring.  -/
def adjoinCommRingOfComm {s : Set A} (hcomm : ∀ a ∈ s, ∀ b ∈ s, a * b = b * a) : CommRing (adjoin R s) :=
