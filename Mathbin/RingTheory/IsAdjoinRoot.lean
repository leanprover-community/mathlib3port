...
protected def isAdjoinRoot : IsAdjoinRoot (AdjoinRoot f) f
  ...
  ker_map := by
    ext
    rw [RingHom.mem_ker, ← @adjoin_root.mk_self _ _ f, AdjoinRoot.mk_eq_mk,
...
def modByMonicHom (h : IsAdjoinRootMonic S f) : S →ₗ[R] R[X]
  ...
  map_add' x y := by
    conv_lhs =>
      rw [← h.map_repr x, ← h.map_repr y, ← map_add, h.mod_by_monic_repr_map, add_modByMonic]
  map_smul' c x := by
    rw [RingHom.id_apply, ← h.map_repr x, Algebra.smul_def, h.algebra_map_apply, ← map_mul,
      h.mod_by_monic_repr_map, ← smul_eq_c_mul, smul_modByMonic, h.map_repr]
...
theorem map_modByMonicHom (h : IsAdjoinRootMonic S f) (x : S) : h.map (h.modByMonicHom x) = x := by
  rw [modByMonicHom, LinearMap.coe_mk, map_modByMonic, map_repr]
...
theorem modByMonicHom_root_pow (h : IsAdjoinRootMonic S f) {n : ℕ} (hdeg : n < natDegree f) :
  ...
  by
  nontriviality R
  rwa [← h.map_X, ← map_pow, modByMonicHom_map, modByMonic_eq_self_iff h.monic, degree_x_pow]
  contrapose! hdeg
...
@[simp]
theorem lift_self_apply (h : IsAdjoinRoot Nontrivial f) (x : S) :
...
@[simp]
theorem ofEquiv_root (h : IsAdjoinRoot S f) (e : S ≃ₐ[R] T) : (h.ofEquiv e).root = e h.root :=
  rfl
...
theorem ofEquiv_aequiv {U : Type _} [CommRing U] [Algebra R U] (h : IsAdjoinRoot S f)
  ...
  ext a <;>
    rw [← (h.of_equiv e).map_repr a, aequiv_map, AlgEquiv.trans_apply, ofEquiv_map_apply,
