...
theorem disjoint_powers_iff_not_mem (hI : I.radical = I) : Disjoint (Submonoid.powers y : Set R) (↑I) ↔ y ∉ I.1 :=
  by 
...
theorem is_maximal_comap_C_of_is_maximal [Nontrivial R] (hP' : ∀ x : R, C x ∈ P → x = 0) :
  ...
  by 
    ...
    have hM' : (0 : Polynomial R ⧸ P) ∉ M' :=
      fun ⟨z, hz⟩ => hM (quotient_map_injective (trans hz.2 φ.map_zero.symm) ▸ hz.1)
...
private theorem quotient_mk_comp_C_is_integral_of_jacobson' [Nontrivial R] (hR : is_jacobson R)
  (hP' : ∀ x : R, C x ∈ P → x = 0) : ((Quotientₓ.mk P).comp C : R →+* Polynomial R ⧸ P).IsIntegral :=
  by 
    ...
    ·
      ...
      refine'
        RingHom.is_integral_trans (algebraMap (R ⧸ P') (Localization M)) (IsLocalization.map _ _ M.le_comap_map) _ _
      ·
        exact
...
theorem quotient_mk_comp_C_is_integral_of_jacobson : ((Quotientₓ.mk P).comp C : R →+* Polynomial R ⧸ P).IsIntegral :=
  by 
    let P' : Ideal R := P.comap C 
...
theorem quotient_mk_comp_C_is_integral_of_jacobson {R : Type _} [CommRingₓ R] [is_jacobson R]
  ...
  ((Quotientₓ.mk P).comp MvPolynomial.c : R →+* MvPolynomial _ R ⧸ P).IsIntegral :=
  by 
    (
