...
theorem isJacobson_iff_prime_eq : IsJacobson R ↔ ∀ P : Ideal R, IsPrime P → P.jacobson = P :=
  ...
  rw [← hI.radical, radical_eq_infₛ I, mem_infₛ]
  intro P hP
  rw [Set.mem_setOf_eq] at hP
  erw [mem_infₛ] at hx
  erw [← h P hP.right, mem_infₛ]
...
theorem isJacobson_of_surjective [H : IsJacobson R] :
  ...
  by
  rintro ⟨f, hf⟩
  rw [isJacobson_iff_infₛ_maximal]
  intro p hp
  ...
  have : p = map f (comap f p).jacobson :=
    (IsJacobson.out' _ <| hp.is_radical.comap f).symm ▸ (map_comap_of_surjective f hf p).symm
  exact this.trans (map_infₛ hf fun J ⟨hJ, _⟩ => le_trans (Ideal.ker_le_comap f) hJ)
...
theorem isMaximal_iff_isMaximal_disjoint [H : IsJacobson R] (J : Ideal S) :
  ...
  · refine' fun h =>
    ...
    have hJ : J.is_prime := IsMaximal.isPrime h
    ...
    erw [← H.out hJ.left.is_radical, mem_infₛ] at this
    push_neg  at this
    ...
    · have hI_p : (map (algebraMap R S) I).IsPrime :=
        by
        refine' isPrime_of_isPrime_disjoint (powers y) _ I hI.right.is_prime _
        rwa [disjoint_powers_iff_not_mem y hI.right.is_prime.is_radical]
...
theorem isJacobson_localization [H : IsJacobson R] : IsJacobson S :=
  ...
  have :
    infₛ { I : Ideal R | comap (algebraMap R S) P' ≤ I ∧ I.IsMaximal ∧ y ∉ I } ≤
      comap (algebraMap R S) P' :=
    by
    intro x hx
    have hxy : x * y ∈ (comap (algebraMap R S) P').jacobson :=
      by
      rw [Ideal.jacobson, mem_infₛ]
      intro J hJ
      by_cases y ∈ J
      · exact J.mul_mem_left x h
      · exact J.mul_mem_right y ((mem_infₛ.1 hx) ⟨hJ.left, ⟨hJ.right, h⟩⟩)
    rw [hP] at hxy
...
theorem jacobson_bot_of_integral_localization {R : Type _} [CommRing R] [IsDomain R] [IsJacobson R]
  ...
  suffices ∀ I : Ideal Sₘ, I.IsMaximal → (I.comap (algebraMap S Sₘ)).IsMaximal
    ...
    have hSₘ : IsJacobson Sₘ := isJacobson_of_is_integral' φ' hφ' (isJacobson_localization x)
  ...
  have := isMaximal_comap_of_isIntegral_of_is_maximal' φ' hφ' I hI
  ...
  exact
    f.is_integral_tower_bot_of_is_integral g quotientMap_injective
...
private theorem is_jacobson_polynomial_of_domain (R : Type _) [CommRing R] [IsDomain R]
  ...
  · rw [jacobson_eq_iff_jacobson_quotient_eq_bot]
    ...
    convert isIntegral_isLocalization_polynomial_quotient P p pP
...
theorem isJacobson_polynomial_of_isJacobson (hR : IsJacobson R) : IsJacobson R[X] :=
  ...
  haveI :=
    map_isPrime_of_surjective (show Function.Surjective (mapRingHom i) from map_surjective i hi) hi'
  ...
  exact
    @is_jacobson_polynomial_of_domain R' _ _ (isJacobson_of_surjective ⟨i, hi⟩)
...
theorem isMaximal_comap_c_of_isMaximal [Nontrivial R] (hP' : ∀ x : R, c x ∈ P → x = 0) :
  ...
  have hp0 : ((m : R[X]).map (Quotient.mk (P.comap (c : R →+* R[X]) : Ideal R))).leadingCoeff ≠ 0 :=
    fun hp0' =>
    this <|
  ...
  suffices (⊥ : Ideal (Localization M')).IsMaximal
    ...
    refine' isMaximal_comap_of_isIntegral_of_is_maximal' _ _ ⊥ this
...
private theorem quotient_mk_comp_C_is_integral_of_jacobson' [Nontrivial R] (hR : IsJacobson R)
  ...
  let φ : R ⧸ P' →+* R[X] ⧸ P := quotientMap P c le_rfl
  haveI hp'_prime : P'.is_prime := comap_isPrime c P
...
theorem quotient_mk_comp_c_isIntegral_of_jacobson :
  ...
  by
  let P' : Ideal R := P.comap c
  haveI : P'.is_prime := comap_isPrime c P
  ...
  ·
    exact
      Or.rec_on (map_eq_top_or_isMaximal_of_surjective f hf hP)
...
theorem isMaximal_comap_c_of_isJacobson : (P.comap (c : R →+* R[X])).IsMaximal :=
  ...
  exact
    isMaximal_comap_of_isIntegral_of_is_maximal' _ (quotient_mk_comp_c_isIntegral_of_jacobson P) ⊥
...
theorem comp_c_integral_of_surjective_of_jacobson {S : Type _} [Field S] (f : R[X] →+* S)
  ...
  have hfg : g.comp (Quotient.mk f.ker) = f := ringHom_ext' rfl rfl
  rw [← hfg, RingHom.comp_assoc]
  refine'
    RingHom.isIntegral_trans _ g (quotient_mk_comp_c_isIntegral_of_jacobson f.ker)
...
theorem quotient_mk_comp_c_isIntegral_of_jacobson {R : Type _} [CommRing R] [IsJacobson R]
  ...
  · rw [← finSuccEquiv_comp_c_eq_c, ← RingHom.comp_assoc, ← RingHom.comp_assoc, ←
    ...
    · refine' RingHom.isIntegral_trans _ _ (is_integral_of_surjective _ Quotient.mk_surjective) _
      refine' RingHom.isIntegral_trans _ _ _ _
      · apply (isIntegral_quotientMap_iff _).mpr (IH _)
        apply Polynomial.isMaximal_comap_c_of_isJacobson _
        · exact MvPolynomial.isJacobson_mvPolynomial_fin n
      ...
      · refine' (isIntegral_quotientMap_iff _).mpr _
        ...
        · apply Polynomial.quotient_mk_comp_c_isIntegral_of_jacobson _
          · exact MvPolynomial.isJacobson_mvPolynomial_fin n
          · exact comap_isMaximal_of_surjective _ (fin_succ_equiv R n).symm.Surjective
    · refine' (isIntegral_quotientMap_iff _).mpr _
      refine' RingHom.isIntegral_trans _ _ _ (is_integral_of_surjective _ Quotient.mk_surjective)
      exact RingHom.isIntegral_of_surjective _ (fin_succ_equiv R n).symm.Surjective
...
theorem comp_c_integral_of_surjective_of_jacobson {R : Type _} [CommRing R] [IsJacobson R]
  ...
  have hf' : Function.Surjective f' := Function.Surjective.comp hf (renameEquiv R e).surjective
