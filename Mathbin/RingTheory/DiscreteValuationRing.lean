...
theorem add_val_def' (u : (R)ˣ) {ϖ : R} (hϖ : Irreducible ϖ) (n : ℕ) : addVal R ((u : R) * ϖ ^ n) = n :=
...
instance (R : Type _) [CommRingₓ R] [IsDomain R] [DiscreteValuationRing R] : IsHausdorff (maximalIdeal R) R where
  haus' := fun x hx => by
