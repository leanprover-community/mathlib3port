...
theorem dimension_le_one : dimension_le_one A :=
  by 
    ...
    ·
      ...
      ·
        rw [mul_assocₓ, ←mul_assocₓ («expr↑ » P), h.mul_inv_eq_one P'_ne, one_mulₓ, h.inv_mul_eq_one M'_ne]
      ·
        rw [←mul_assocₓ («expr↑ » P), h.mul_inv_eq_one P'_ne, one_mulₓ]
      ·
        infer_instance 
    intro x hx 
...
theorem exists_multiset_prod_cons_le_and_prod_not_le [IsDedekindDomain A] (hNF : ¬IsField A) {I M : Ideal A}
  ...
  by 
    ...
    letI this := Classical.decEq (Ideal A)
...
theorem is_dedekind_domain_iff_is_dedekind_domain_inv : IsDedekindDomain A ↔ IsDedekindDomainInv A :=
  ⟨fun h I hI =>
      by 
        exactI FractionalIdeal.mul_inv_cancel hI,
    ...
    fun h =>
      by 
        ...
        have  : ((I : FractionalIdeal A⁰ (FractionRing A))⁻¹*J) ≤ 1 :=
          le_transₓ (FractionalIdeal.mul_left_mono («expr↑ » I⁻¹) ((coe_ideal_le_coe_ideal _).mpr h))
...
theorem FiniteDimensional.exists_is_basis_integral : ∃ (s : Finset L)(b : Basis s K L), ∀ x, IsIntegral A (b x) :=
  by 
    ...
    letI this : IsNoetherian K L := IsNoetherian.iff_fg.2 inferInstance 
...
theorem IsIntegralClosure.is_noetherian_ring [IsIntegrallyClosed A] [IsNoetherianRing A] : IsNoetherianRing C :=
  by 
    ...
    letI this := is_noetherian_span_of_finite A (Set.finite_range b')
