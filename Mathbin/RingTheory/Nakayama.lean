...
theorem smul_sup_eq_smul_sup_of_le_smul_of_le_jacobson {I J : Ideal R} {N N' : Submodule R M} (hN' : N'.fg)
  ...
  by 
    ...
    ·
      rw [←(Submodule.comap_injective_of_surjective (LinearMap.range_eq_top.1 (Submodule.range_mkq N))).eq_iff]
      simpa [comap_map_eq, sup_comm, eq_comm] using hNN' 
    have  :=
      @Submodule.eq_smul_of_le_smul_of_le_jacobson _ _ _ _ _ I J (N'.map N.mkq) (fg_map hN')
        (by 
          rw [←map_smul'', this] <;> exact le_reflₓ _)
        hIJ 
    rw [←map_smul'',
      ←(Submodule.comap_injective_of_surjective (LinearMap.range_eq_top.1 (Submodule.range_mkq N))).eq_iff,
      comap_map_eq, comap_map_eq, Submodule.ker_mkq, sup_comm, hNN'] at this 
    rw [this, sup_comm]
