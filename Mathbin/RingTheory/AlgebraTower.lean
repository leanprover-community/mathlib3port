...
theorem Basis.algebra_map_injective {ι : Type _} [NoZeroDivisors R] [Nontrivial S] (b : Basis ι R S) :
  ...
  by 
    exactI NoZeroSmulDivisors.algebra_map_injective R S
...
theorem exists_subalgebra_of_fg (hAC : (⊤ : Subalgebra A C).Fg) (hBC : (⊤ : Submodule B C).Fg) :
  ...
  by 
    ...
    have hxy :
      ∀ xi _ : xi ∈ x, xi ∈ span (Algebra.adjoin A («expr↑ » s : Set B)) («expr↑ » (insert 1 y : Finset C) : Set C) :=
      fun xi hxi =>
        hf xi ▸
          sum_mem _
            fun yj hyj =>
              smul_mem (span (Algebra.adjoin A («expr↑ » s : Set B)) («expr↑ » (insert 1 y : Finset C) : Set C))
                ⟨f xi yj, Algebra.subset_adjoin$ hsx xi hxi yj hyj⟩ (subset_span$ mem_insert_of_mem hyj)
    have hyy :
      (span (Algebra.adjoin A («expr↑ » s : Set B))
            («expr↑ » (insert 1 y : Finset C) :
            Set C)*span (Algebra.adjoin A («expr↑ » s : Set B)) («expr↑ » (insert 1 y : Finset C) : Set C)) ≤
        span (Algebra.adjoin A («expr↑ » s : Set B)) («expr↑ » (insert 1 y : Finset C) : Set C)
    ·
      ...
      ·
        ...
        exact
          SetLike.mem_coe.2
            (sum_mem _$
              fun yk hyk =>
                smul_mem (span (Algebra.adjoin A («expr↑ » s : Set B)) (insert 1 («expr↑ » y) : Set C))
                  ⟨f (yi*yj) yk, Algebra.subset_adjoin$ hsy yi yj yk hyi hyj hyk⟩
                  (subset_span$ Set.mem_insert_of_mem _ hyk : yk ∈ _))
    refine' ⟨Algebra.adjoin A («expr↑ » s : Set B), Subalgebra.fg_adjoin_finset _, insert 1 y, _⟩
  ...
  Algebra.fg_trans' (B₀.fg_top.2 hAB₀)$
    Subalgebra.fg_of_submodule_fg$
      ...
      have  : IsNoetherian B₀ C :=
        by 
          exactI is_noetherian_of_fg_of_noetherian' hB₀C 
      by 
        exactI fg_of_injective (IsScalarTower.toAlgHom B₀ B C).toLinearMap hBCi
...
def algHomEquivSigma : (C →ₐ[A] D) ≃ Σf : B →ₐ[A] D, @AlgHom B C D _ _ _ _ f.to_ring_hom.to_algebra :=
  { toFun := fun f => ⟨f.restrict_domain B, f.extend_scalars B⟩,
    invFun :=
      fun fg =>
        let alg := fg.1.toRingHom.toAlgebra 
        by 
          exactI fg.2.restrictScalars A,
    left_inv :=
      fun f =>
        by 
          dsimp only 
