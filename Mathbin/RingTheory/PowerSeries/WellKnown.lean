...
@[simp]
theorem constant_coeff_inv_units_sub (u : Units R) : constant_coeff R (inv_units_sub u) = 1 /ₚ u := by
  rw [← coeff_zero_eq_constant_coeff_apply, coeff_inv_units_sub, zero_addₓ, pow_oneₓ]

@[simp]
theorem inv_units_sub_mul_X (u : Units R) : (inv_units_sub u*X) = (inv_units_sub u*C R u) - 1 := by
  ext (_ | n)
  ·
    simp
  ·
...
@[simp]
theorem inv_units_sub_mul_sub (u : Units R) : (inv_units_sub u*C R u - X) = 1 := by
...
theorem exp_mul_exp_eq_exp_add [Algebra ℚ A] (a b : A) :
  ...
  suffices
    (((a^x)*b^n - x)*algebraMap ℚ A (1 / ↑x.factorial)*algebraMap ℚ A (1 / ↑(n - x).factorial)) =
      ((a^x)*b^n - x)*(↑n.choose x)*(algebraMap ℚ A) (1 / ↑n.factorial)by
