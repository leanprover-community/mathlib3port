...
theorem to_fun_linear_mul_tmul_mul_aux_2 (k : ℕ) (a₁ a₂ : A) (p₁ p₂ : Polynomial R) :
  ((a₁*a₂)*(algebraMap R A) ((p₁*p₂).coeff k)) =
    (Finset.Nat.antidiagonal k).Sum fun x => (a₁*(algebraMap R A) (coeff p₁ x.1))*a₂*(algebraMap R A) (coeff p₂ x.2) :=
  by 
    simpRw [mul_assocₓ, Algebra.commutes, ←Finset.mul_sum, mul_assocₓ, ←Finset.mul_sum]
    congr 
    simpRw [Algebra.commutes (coeff p₂ _), coeff_mul, RingHom.map_sum, RingHom.map_mul]
...
theorem to_fun_linear_mul_tmul_mul (a₁ a₂ : A) (p₁ p₂ : Polynomial R) :
  ...
  by 
    ...
    ext k 
    simpRw [coeff_sum, coeff_monomial, sum_def, Finset.sum_ite_eq', mem_support_iff, Ne.def]
    convRHS => rw [coeff_mul]
    simpRw [finset_sum_coeff, coeff_monomial, Finset.sum_ite_eq', mem_support_iff, Ne.def, mul_ite, mul_zero, ite_mul,
      zero_mul]
    simpRw [ite_mul_zero_left (¬coeff p₁ _ = 0) (a₁*(algebraMap R A) (coeff p₁ _))]
    simpRw [ite_mul_zero_right (¬coeff p₂ _ = 0) _ (_*_)]
    simpRw [to_fun_linear_mul_tmul_mul_aux_1, to_fun_linear_mul_tmul_mul_aux_2]

theorem to_fun_linear_algebra_map_tmul_one (r : R) :
  (to_fun_linear R A) ((algebraMap R A) r ⊗ₜ[R] 1) = (algebraMap R (Polynomial A)) r :=
  by 
...
theorem to_fun_alg_hom_apply_tmul (a : A) (p : Polynomial R) :
  to_fun_alg_hom R A (a ⊗ₜ[R] p) = p.sum fun n r => monomial n (a*(algebraMap R A) r) :=
  by 
...
@[simp]
theorem inv_fun_add {p q} : inv_fun R A (p+q) = inv_fun R A p+inv_fun R A q :=
  by 
...
theorem left_inv (x : A ⊗ Polynomial R) : inv_fun R A ((to_fun_alg_hom R A) x) = x :=
  by 
    ...
    ·
      ...
      simpRw [eval₂_monomial, AlgHom.coe_to_ring_hom, Algebra.TensorProduct.tmul_pow, one_pow,
...
theorem mat_poly_equiv_coeff_apply_aux_1 (i j : n) (k : ℕ) (x : R) :
  matPolyEquiv (std_basis_matrix i j$ monomial k x) = monomial k (std_basis_matrix i j x) :=
  by 
...
theorem mat_poly_equiv_coeff_apply_aux_2 (i j : n) (p : Polynomial R) (k : ℕ) :
  coeff (matPolyEquiv (std_basis_matrix i j p)) k = std_basis_matrix i j (coeff p k) :=
  by 
