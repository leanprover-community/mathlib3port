...
| ⟨⟨s, hs⟩⟩, ⟨⟨t, ht⟩⟩ =>
  ⟨Submodule.fg_def.2
      ⟨Set.Image2 (· • ·) (↑s : Set A) (↑t : Set B), Set.Finite.image2 _ s.finite_to_set t.finite_to_set,
        by 
...
theorem Equivₓ (hfp : finite_presentation R A) (e : A ≃ₐ[R] B) : finite_presentation R B :=
  by 
    obtain ⟨n, f, hf⟩ := hfp 
...
theorem Iff : finite_presentation R A ↔ ∃ (n : _)(I : Ideal (MvPolynomial (Finₓ n) R))(e : (_ ⧸ I) ≃ₐ[R] A), I.fg :=
  by 
    constructor
    ·
      rintro ⟨n, f, hf⟩
...
theorem mv_polynomial_aeval_of_surjective_of_closure [CommSemiringₓ R] {S : Set M} (hS : closure S = ⊤) :
  Function.Surjective (MvPolynomial.aeval fun s : S => of' R M (↑s) : MvPolynomial S R → AddMonoidAlgebra R M) :=
  by 
...
theorem finite_type_iff_fg [CommRingₓ R] [Nontrivial R] : finite_type R (AddMonoidAlgebra R M) ↔ AddMonoidₓ.Fg M :=
  by 
    ...
    have hm : of' R M m ∈ (adjoin R (of' R M '' ↑S)).toSubmodule
    ·
