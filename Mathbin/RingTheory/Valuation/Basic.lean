...
theorem unit_map_eq (u : Units R) : (Units.map (v : R →* Γ₀) u : Γ₀) = v u :=
  rfl
...
theorem map_add_of_distinct_val (h : v x ≠ v y) : v (x+y) = max (v x) (v y) :=
  by 
    ...
    ·
      ...
      calc v x = v ((x+y) - y) :=
        by 
          simp _ ≤ max (v$ x+y) (v y) :=
        map_sub _ _ _ _ < v x := max_ltₓ h' vyx
    ·
      apply this h.symm 
...
theorem symm (h : v₁.is_equiv v₂) : v₂.is_equiv v₁ :=
  fun _ _ => Iff.symm (h _ _)

@[trans]
theorem trans (h₁₂ : v₁.is_equiv v₂) (h₂₃ : v₂.is_equiv v₃) : v₁.is_equiv v₃ :=
  fun _ _ => Iff.trans (h₁₂ _ _) (h₂₃ _ _)

theorem of_eq {v' : Valuation R Γ₀} (h : v = v') : v.is_equiv v' :=
  by 
    subst h
