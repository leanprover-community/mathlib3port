...
-- error in RingTheory.Ideal.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
[comm_semiring R]
{P : ideal R}
(hmax : ∀ m : ideal R, «expr < »(P, m) → «expr¬ »(is_maximal m))
(J : ideal R)
(hPJ : «expr < »(P, J)) : «expr = »(J, «expr⊤»()) :=
begin
  by_contradiction [ident hnonmax],
  rcases [expr exists_le_maximal J hnonmax, "with", "⟨", ident M, ",", ident hM1, ",", ident hM2, "⟩"],
  exact [expr hmax M (lt_of_lt_of_le hPJ hM2) hM1]
end
  ...
  ⟨H.1.1,
    fun x y hxy =>
      or_iff_not_imp_left.2$
        fun hx =>
          by 
            let J : Ideal α := Submodule.span α (insert x («expr↑ » I))
...
-- error in RingTheory.Ideal.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem is_prime.mul_mem_iff_mem_or_mem
{I : ideal α}
(hI : I.is_prime) : ∀
{x y : α}, «expr ↔ »(«expr ∈ »(«expr * »(x, y), I), «expr ∨ »(«expr ∈ »(x, I), «expr ∈ »(y, I))) :=
λ
x
y, ⟨hI.mem_or_mem, by { rintro ["(", ident h, "|", ident h, ")"],
   exacts ["[", expr I.mul_mem_right y h, ",", expr I.mul_mem_left x h, "]"] }⟩
...
theorem bot_lt_of_maximal (M : Ideal R) [hm : M.is_maximal] (non_field : ¬IsField R) : ⊥ < M :=
  by 
