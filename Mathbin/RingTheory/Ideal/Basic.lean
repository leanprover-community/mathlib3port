...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (z «expr ∈ » span s)
...
theorem pow_multiset_sum_mem_span_pow (s : Multiset α) (n : ℕ) :
  ...
  by 
    ...
    ·
      refine'
        ⟨((a ^ (c - n+1))*s.sum ^ ((((s.card+1)*n)+1) - c))*(((s.card+1)*n)+1).choose c, 0, Submodule.zero_mem _, _⟩
      rw [mul_commₓ _ (a ^ n+1)]
      simpRw [←mul_assocₓ]
      rw [←pow_addₓ, add_zeroₓ, add_tsub_cancel_of_le h]
    ·
      use 0
      simpRw [zero_mul, zero_addₓ]
      refine' ⟨_, _, rfl⟩
      ...
      rw [this, pow_addₓ]
      simpRw [mul_assocₓ, mul_commₓ (s.sum ^ (s.card*n)+1), ←mul_assocₓ]
...
theorem span_pow_eq_top (s : Set α) (hs : span s = ⊤) (n : ℕ) : span ((fun x => x ^ n) '' s) = ⊤ :=
  by 
    ...
    rintro _ hx 
    simpRw [Finset.mem_coe, Set.mem_image]  at hx 
