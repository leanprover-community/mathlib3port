...
theorem unit_mul_mem_iff_mem {x y : α} (hy : IsUnit y) : (y*x) ∈ I ↔ x ∈ I := by
  ...
  rwa [← mul_assocₓ, hy', one_mulₓ] at this
      ...
      («term∃_,_»
       ...
       (Lean.explicitBinders
        [(Lean.bracketedExplicitBinders "(" [(Lean.binderIdent `a) (Lean.binderIdent `b)] ":" (Term.hole "_") ")")
         (Lean.bracketedExplicitBinders "(" [(Lean.binderIdent `h)] ":" (Term.app `r [`a `b]) ")")])
       ","
       («term_=_» (Init.Logic.«term_+_» `x "+" `b) "=" `a))
      "}")])
   [])
  []
  []
...
[PrettyPrinter.parenthesize] parenthesized: (Term.paren "(" [(Init.Logic.«term_+_» `x "+" `b) []] ")")
...
/--
    The ideal generated by an arbitrary binary relation.
    -/
  def of_rel ( r : α → α → Prop ) : Ideal α := Submodule.span α { x | ∃ ( a b : _ ) ( h : r a b ) , x + b = a }
...
/- failed to parenthesize: parenthesize: uncaught backtrack exception
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers [(Command.docComment "/--" " `I^n` as an ideal of `R^n`. -/")] [] [] [] [] [])
 (Command.def
  "def"
  (Command.declId `pi [])
  (Command.optDeclSig [] [(Term.typeSpec ":" (Term.app `Ideal [(Term.arrow `ι "→" `α)]))])
  (Command.declValSimple
   ":="
   (Term.structInst
    "{"
    ...
    [(group
      (Term.structInstField
       ...
       (Set.«term{_|_}»
        ...
        `x
        "|"
        (Term.forall "∀" [(Term.simpleBinder [`i] [])] "," (Init.Core.«term_∈_» (Term.app `x [`i]) " ∈ " `I))
        "}"))
      [","])
     (group
      (Term.structInstField
       ...
       ":="
       (Term.fun "fun" (Term.basicFun [(Term.simpleBinder [`i] [])] "=>" `I.zero_mem)))
      [","])
     (group
      (Term.structInstField
       ...
       (Term.fun
        "fun"
        (Term.basicFun
         [(Term.simpleBinder [`a `b `ha `hb `i] [])]
         "=>"
         (Term.app `I.add_mem [(Term.app `ha [`i]) (Term.app `hb [`i])]))))
      [","])
     (group
      (Term.structInstField
       ...
       (Term.fun
        "fun"
        (Term.basicFun
         [(Term.simpleBinder [`a `b `hb `i] [])]
         "=>"
         (Term.app `I.mul_mem_left [(Term.app `a [`i]) (Term.app `hb [`i])]))))
      [])]
    (Term.optEllipsis [])
    []
    "}")
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Init.Core.«term_∈_» (Term.app `x [`i]) " ∈ " `I)
...
theorem is_maximal.is_prime {I : Ideal α} (H : I.is_maximal) : I.is_prime :=
  ⟨H.1.1, fun x y hxy =>
    or_iff_not_imp_left.2 $ fun hx => by
      ...
      rw [← mul_oneₓ y, F, mul_addₓ, mul_commₓ, smul_eq_mul, mul_assocₓ]
...
theorem pow_multiset_sum_mem_span_pow (s : Multiset α) (n : ℕ) :
  ...
  by_cases' h : (n+1) ≤ c
  ·
    refine' ⟨((a ^ (c - n+1))*s.sum ^ ((((s.card+1)*n)+1) - c))*(((s.card+1)*n)+1).choose c, 0, Submodule.zero_mem _, _⟩
    rw [mul_commₓ _ (a ^ n+1)]
    simp_rw [← mul_assocₓ]
  ...
  ·
    ...
    rw [this, pow_addₓ]
    simp_rw [mul_assocₓ, mul_commₓ (s.sum ^ (s.card*n)+1), ← mul_assocₓ]
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `sum_pow_mem_span_pow [])
  (Command.declSig
    ...
    (Init.Core.«term_∈_»
     («term_^_»
      (Algebra.BigOperators.Basic.«term∑_in_,_»
       ...
       (Term.app `f [`i]))
      "^"
      (Init.Logic.«term_+_» (Finset.Data.Finset.Fold.«term_*_» `s.card "*" `n) "+" (numLit "1")))
     " ∈ "
     (Term.app
      `span
      [(Set.Data.Set.Basic.term_''_
        (Term.fun
         ...
         (Term.basicFun
          [(Term.simpleBinder [`i] [])]
          "=>"
          («term_^_» (Term.app `f [`i]) "^" (Init.Logic.«term_+_» `n "+" (numLit "1")))))
        " '' "
        `s)]))))
  (Command.declValSimple
   ":="
      ...
      [(group
        (Tactic.convert
         ...
         (Term.app
          ...
          [(Term.app (Term.proj (Term.proj `s "." (fieldIdx "1")) "." `map) [`f]) `n])
         [])
        [])
       (group
        (Tactic.«tactic·._»
         ...
         (Tactic.tacticSeq
          (Tactic.tacticSeq1Indented
           [(group (Tactic.rwSeq "rw" [] (Tactic.rwRuleSeq "[" [(Tactic.rwRule [] `Multiset.card_map)] "]") []) [])
        ...
        (Tactic.rwSeq
         ...
         (Tactic.rwRuleSeq
          ...
          [(Tactic.rwRule [] `Multiset.map_map)
           ","
           (Tactic.rwRule [] `Multiset.to_finset_map)
           ","
           (Tactic.rwRule [] `Finset.val_to_finset)
           ","
           (Tactic.rwRule [] `Finset.coe_image)]
          "]")
         [])
        [])])))
   [])
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1023, tactic))
  (Tactic.rwSeq "rw" [] (Tactic.rwRuleSeq "[" [(Tactic.rwRule [] `Multiset.card_map)] "]") [])
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Tactic.rwSeq', expected 'antiquot'
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Tactic.rwRule', expected 'sepBy.antiquot_scope'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  `Multiset.card_map
...
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 1022, tactic))
  (Tactic.convert
   "convert"
   []
   (Term.app `pow_multiset_sum_mem_span_pow [(Term.app (Term.proj (Term.proj `s "." (fieldIdx "1")) "." `map) [`f]) `n])
   [])
...
[PrettyPrinter.parenthesize] parenthesized: (Term.paren "(" [(Finset.Data.Finset.Fold.«term_*_» `s.card "*" `n) []] ")")
[PrettyPrinter.parenthesize] ...precedences are 80 >? 1022, (some 0, term) <=? (none, [anonymous])
[PrettyPrinter.parenthesize] parenthesizing (cont := (some 80, term))
  (Algebra.BigOperators.Basic.«term∑_in_,_»
   "∑"
...
theorem
  ...
  { ι } ( s : Finset ι ) ( f : ι → α ) ( n : ℕ ) : ∑ i in s , f i ^ s.card * n + 1 ∈ span fun i => f i ^ n + 1 '' s
  :=
    by
...
theorem exists_not_is_unit_of_not_is_field [Nontrivial R] (hf : ¬IsField R) : ∃ (x : _)(_ : x ≠ (0 : R)), ¬IsUnit x :=
  by
  have : ¬_ := fun h => hf ⟨exists_pair_ne R, mul_commₓ, h⟩
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers
  [(Command.docComment "/--" " The set of non-invertible elements of a monoid. -/")]
  []
  []
  []
  []
...
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Lean.Parser.Command.declValSimple', expected 'Lean.Parser.Command.declValSimple.antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
  (Set.«term{_|_}» "{" `a "|" («term¬_» "¬" (Term.app `IsUnit [`a])) "}")
[PrettyPrinter.parenthesize.backtrack] unexpected node kind 'Set.«term{_|_}»', expected 'antiquot'
[PrettyPrinter.parenthesize] parenthesizing (cont := (none, [anonymous]))
...
/-- The set of non-invertible elements of a monoid. -/
  def Nonunits ( α : Type u ) [ Monoidₓ α ] : Set α := { a | ¬ IsUnit a }
