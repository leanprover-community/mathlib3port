...
theorem is_coprime_self : IsCoprime x x ↔ IsUnit x :=
  ⟨fun ⟨a, b, h⟩ =>
      is_unit_of_mul_eq_one x (a+b)$
        by 
          rwa [mul_commₓ, add_mulₓ],
    fun h =>
...
theorem is_coprime_zero_left : IsCoprime 0 x ↔ IsUnit x :=
  ⟨fun ⟨a, b, H⟩ =>
      is_unit_of_mul_eq_one x b$
        by 
          rwa [mul_zero, zero_addₓ, mul_commₓ] at H,
    fun H =>
      let ⟨b, hb⟩ := is_unit_iff_exists_inv'.1 H
      ⟨1, b,
        by 
...
theorem IsCoprime.ne_zero [Nontrivial R] {p : Finₓ 2 → R} (h : IsCoprime (p 0) (p 1)) : p ≠ 0 :=
  by 
    rintro rfl 
...
theorem IsCoprime.mul_left (H1 : IsCoprime x z) (H2 : IsCoprime y z) : IsCoprime (x*y) z :=
  ...
  ⟨a*c, (((a*x)*d)+(b*c)*y)+(b*d)*z,
    calc (((a*c)*x*y)+((((a*x)*d)+(b*c)*y)+(b*d)*z)*z) = ((a*x)+b*z)*(c*y)+d*z :=
      by 
        ring 
      _ = 1 :=
      by 
...
theorem IsCoprime.mul_dvd (H : IsCoprime x y) (H1 : x ∣ z) (H2 : y ∣ z) : (x*y) ∣ z :=
  by 
    ...
    apply dvd_add
    ·
      rw [mul_commₓ z, mul_assocₓ]
      exact (mul_dvd_mul_left _ H2).mul_left _
    ·
      rw [mul_commₓ b, ←mul_assocₓ]
...
theorem IsCoprime.of_mul_left_left (H : IsCoprime (x*y) z) : IsCoprime x z :=
  ...
  ⟨a*y, b,
    by 
      rwa [mul_right_commₓ, mul_assocₓ]⟩
...
theorem IsCoprime.of_coprime_of_dvd_left (h : IsCoprime y z) (hdvd : x ∣ y) : IsCoprime x z :=
  by 
    obtain ⟨d, rfl⟩ := hdvd 
...
theorem IsCoprime.of_add_mul_left_left (h : IsCoprime (x+y*z) y) : IsCoprime x y :=
  ...
  ⟨a, (a*z)+b,
    by 
      simpa only [add_mulₓ, mul_addₓ, add_assocₓ, add_commₓ, add_left_commₓ, mul_assocₓ, mul_commₓ,
