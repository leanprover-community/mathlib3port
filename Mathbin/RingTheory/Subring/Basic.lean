...
theorem prod_mono_left (t : Subring S) : Monotone fun s : Subring R => s.prod t :=
  fun s₁ s₂ hs => prod_mono hs (le_reflₓ t)
...
def Units.posSubgroup (R : Type _) [LinearOrderedSemiring R] : Subgroup (Units R) :=
