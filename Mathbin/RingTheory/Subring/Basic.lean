...
theorem coe_Sup_of_directed_on {S : Set (Subring R)} (Sne : S.Nonempty)
    (hS : DirectedOn (· ≤ ·) S) : (↑(sup S) : Set R) = ⋃ s ∈ S, ↑s :=
