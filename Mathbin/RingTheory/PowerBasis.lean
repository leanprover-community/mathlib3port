...
theorem exists_eq_aeval' (pb : PowerBasis R S) (y : S) : ∃ f : Polynomial R, y = aeval pb.gen f := by
  nontriviality S
...
@[simp]
theorem degree_minpoly_gen (pb : PowerBasis A S) : degree (minpoly_gen pb) = pb.dim := by
  unfold minpoly_gen
...
theorem minpoly_gen_monic (pb : PowerBasis A S) : monic (minpoly_gen pb) := by
...
@[simp]
theorem minpoly_gen_map (pb : PowerBasis A S) (e : S ≃ₐ[A] S') : (pb.map e).minpolyGen = pb.minpoly_gen := by
...
@[simp]
theorem equiv_of_root_map (pb : PowerBasis A S) (e : S ≃ₐ[A] S') h₁ h₂ : pb.equiv_of_root (pb.map e) h₁ h₂ = e := by
  ext x
  obtain ⟨f, rfl⟩ := pb.exists_eq_aeval' x
