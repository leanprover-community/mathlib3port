...
theorem finset_integer_multiple_image [DecidableEq R] (s : Finset S) :
  algebraMap R S '' finset_integer_multiple M s = common_denom_of_finset M s • s :=
  by 
    ...
    (by 
      ...
      exact
        fun r : R s : I.prime_compl =>
          not_imp_comm.1
            fun nr => is_unit_iff_exists_inv.2 ⟨mk' S (↑s) (⟨r, nr⟩ : I.prime_compl), mk'_mul_mk'_eq_one' _ _ nr⟩
...
theorem is_prime_iff_is_prime_disjoint (J : Ideal S) :
  J.is_prime ↔ (Ideal.comap (algebraMap R S) J).IsPrime ∧ Disjoint (M : Set R) (↑Ideal.comap (algebraMap R S) J) :=
  by 
    constructor
    ·
...
theorem localization_localization_eq_iff_exists [IsLocalization N T] (x y : R) :
  algebraMap R T x = algebraMap R T y ↔ ∃ c : localization_localization_submodule M N, (x*c) = y*c :=
  by 
...
theorem localization_localization_is_localization_of_has_all_units [IsLocalization N T]
  (H : ∀ x : S, IsUnit x → x ∈ N) : IsLocalization (N.comap (algebraMap R S).toMonoidHom) T :=
  by 
...
instance (p : Ideal (Localization M)) [p.is_prime] : Algebra R (Localization.AtPrime p) :=
  ((algebraMap (Localization M) _).comp (algebraMap R _)).toAlgebra
...
theorem is_unit_mk'_iff (x : R) (y : I.prime_compl) : IsUnit (mk' S x y) ↔ x ∈ I.prime_compl :=
  ⟨fun h hx => mk'_mem_iff.mpr ((to_map_mem_maximal_iff S I x).mpr hx) h,
    fun h => is_unit_iff_exists_inv.mpr ⟨mk' S (↑y) ⟨x, h⟩, mk'_mul_mk'_eq_one ⟨x, h⟩ y⟩⟩
