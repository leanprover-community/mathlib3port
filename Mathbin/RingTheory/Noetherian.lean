...
theorem fg_top (N : Submodule R M) : (⊤ : Submodule R N).Fg ↔ N.fg :=
  ⟨fun h => N.range_subtype ▸ map_top N.subtype ▸ h.map _,
    fun h =>
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » t1)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » («expr↑ »(t) : set M))
...
theorem fg_iff_compact (s : Submodule R M) : s.fg ↔ CompleteLattice.IsCompactElement s :=
  by 
    ...
    have supr_rw : ∀ t : Finset M, (⨆ (x : _)(_ : x ∈ t), sp x) = ⨆ (x : _)(_ : x ∈ (↑t : Set M)), sp x 
    exact
      fun t =>
        by 
          rfl 
...
theorem is_noetherian_submodule {N : Submodule R M} : IsNoetherian R N ↔ ∀ s : Submodule R M, s ≤ N → s.fg :=
  by 
    ...
    have h₂ : (⊤ : Submodule R (s.map N.subtype)).map (↑f : _ →ₗ[R] s) = ⊤ :=
      by 
        simp 
  ...
  :=
    by
      refine'
        ...
        have : ∀ n , coeₓ ∘ f '' { m | m ≤ n } ⊆ s
        · rintro n x ⟨ y , hy₁ , hy₂ ⟩ subst hy₂ exact f y . 2
        have : ∀ a b : ℕ , a ≤ b ↔ span R coeₓ ∘ f '' { m | m ≤ a } ≤ span R coeₓ ∘ f '' { m | m ≤ b }
        ·
          intro a b
            rw
              [
