...
-- ./././Mathport/Syntax/Translate/Command.lean:326:30: infer kinds are unsupported in Lean 4: #[`principal] []
...
instance (priority := 100) DivisionRing.is_principal_ideal_ring (K : Type u) [DivisionRing K] :
    IsPrincipalIdealRing
      K where principal := fun S => by rcases Ideal.eq_bot_or_top S with (rfl | rfl) <;> infer_instance
...
theorem is_maximal_of_irreducible [CommRingₓ R] [IsPrincipalIdealRing R] {p : R} (hp : Irreducible p) :
  ...
  ⟨⟨mt Ideal.span_singleton_eq_top.1 hp.1, fun I hI => by
      ...
      refine' (of_irreducible_mul hp).resolve_right (mt (fun hb => _) (not_le_of_ltₓ hI))
