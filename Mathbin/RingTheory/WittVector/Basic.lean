...
def map_fun (f : α → β) : 𝕎 α → 𝕎 β :=
  fun x => mk _ (f ∘ x.coeff)

namespace MapFun

theorem injective (f : α → β) (hf : injective f) : injective (map_fun f : 𝕎 α → 𝕎 β) :=
  fun x y h => ext$ fun n => hf (congr_argₓ (fun x => coeff x n) h : _)

theorem surjective (f : α → β) (hf : surjective f) : surjective (map_fun f : 𝕎 α → 𝕎 β) :=
  fun x =>
    ⟨mk _ fun n => Classical.some$ hf$ x.coeff n,
      by 
        ext n 
        dsimp [map_fun]
...
theorem zero : map_fun f (0 : 𝕎 R) = 0 :=
  by 
    runTac 
      map_fun_tac

theorem one : map_fun f (1 : 𝕎 R) = 1 :=
  by 
    runTac 
      map_fun_tac

theorem add : map_fun f (x+y) = map_fun f x+map_fun f y :=
  by 
    runTac 
      map_fun_tac

theorem sub : map_fun f (x - y) = map_fun f x - map_fun f y :=
  by 
    runTac 
      map_fun_tac

theorem mul : map_fun f (x*y) = map_fun f x*map_fun f y :=
  by 
    runTac 
      map_fun_tac

theorem neg : map_fun f (-x) = -map_fun f x :=
  by 
    runTac 
      map_fun_tac
...
private def ghost_fun : 𝕎 R → ℕ → R :=
  fun x n => aeval x.coeff (W_ ℤ n)
...
@[local simp]
theorem matrix_vec_empty_coeff {R} i j : @coeff p R (Matrix.vecEmpty i) j = (Matrix.vecEmpty i : ℕ → R) j :=
  by 
...
private theorem ghost_fun_zero : ghost_fun (0 : 𝕎 R) = 0 :=
  by 
    ghostFunTac 0, «expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"

-- ././Mathport/Syntax/Translate/Basic.lean:600:4: warning: unsupported notation `«expr![ , ]»
-- ././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»
private theorem ghost_fun_one : ghost_fun (1 : 𝕎 R) = 1 :=
  by 
    ghostFunTac 1, «expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"

-- ././Mathport/Syntax/Translate/Basic.lean:600:4: warning: unsupported notation `«expr![ , ]»
-- ././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»
private theorem ghost_fun_add : ghost_fun (x+y) = ghost_fun x+ghost_fun y :=
  by 
    ghostFunTac X 0+X 1,
...
-- ././Mathport/Syntax/Translate/Basic.lean:600:4: warning: unsupported notation `«expr![ , ]»
-- ././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»
private theorem ghost_fun_sub : ghost_fun (x - y) = ghost_fun x - ghost_fun y :=
  by 
    ghostFunTac X 0 - X 1,
      «expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"
...
-- ././Mathport/Syntax/Translate/Basic.lean:600:4: warning: unsupported notation `«expr![ , ]»
-- ././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»
private theorem ghost_fun_mul : ghost_fun (x*y) = ghost_fun x*ghost_fun y :=
  by 
    ghostFunTac X 0*X 1,
      «expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"
...
-- ././Mathport/Syntax/Translate/Basic.lean:600:4: warning: unsupported notation `«expr![ , ]»
-- ././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»
private theorem ghost_fun_neg : ghost_fun (-x) = -ghost_fun x :=
  by 
    ghostFunTac -X 0, «expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:601:61: unsupported notation `«expr![ , ]»"
...
def map (f : R →+* S) : 𝕎 R →+* 𝕎 S :=
  { toFun := map_fun f, map_zero' := map_fun.zero f, map_one' := map_fun.one f, map_add' := map_fun.add f,
...
def ghost_map : 𝕎 R →+* ℕ → R :=
  { toFun := ghost_fun, map_zero' := ghost_fun_zero, map_one' := ghost_fun_one, map_add' := ghost_fun_add,
