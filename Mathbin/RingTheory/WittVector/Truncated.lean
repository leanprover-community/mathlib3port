...
#print WittVector.truncateFun_natCast /-
theorem truncateFun_natCast (m : ℕ) : truncateFun n (m : 𝕎 R) = m :=
  rfl
#align witt_vector.truncate_fun_nat_cast WittVector.truncateFun_natCast
-/

#print WittVector.truncateFun_intCast /-
theorem truncateFun_intCast (m : ℤ) : truncateFun n (m : 𝕎 R) = m :=
  rfl
...
theorem coeff_truncate {m : ℕ} (hm : n ≤ m) (i : Fin n) (x : TruncatedWittVector p m R) :
    (truncate hm x).coeff i = x.coeff (Fin.castLEOrderEmb hm i) :=
  by
