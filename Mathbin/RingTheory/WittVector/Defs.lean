...
@[simp]
theorem constant_coeff_witt_add (n : ℕ) : constant_coeff (witt_add p n) = 0 :=
  by 
    apply constant_coeff_witt_structure_int p _ _ n 
    simp only [add_zeroₓ, RingHom.map_add, constant_coeff_X]

@[simp]
theorem constant_coeff_witt_sub (n : ℕ) : constant_coeff (witt_sub p n) = 0 :=
  by 
    apply constant_coeff_witt_structure_int p _ _ n 
    simp only [sub_zero, RingHom.map_sub, constant_coeff_X]

@[simp]
theorem constant_coeff_witt_mul (n : ℕ) : constant_coeff (witt_mul p n) = 0 :=
  by 
    apply constant_coeff_witt_structure_int p _ _ n 
    simp only [mul_zero, RingHom.map_mul, constant_coeff_X]

@[simp]
theorem constant_coeff_witt_neg (n : ℕ) : constant_coeff (witt_neg p n) = 0 :=
  by 
    apply constant_coeff_witt_structure_int p _ _ n 
...
theorem one_coeff_zero : (1 : 𝕎 R).coeff 0 = 1 :=
  show (aeval _ (witt_one p 0) : R) = 1by 
...
{p' R'}
(x y : witt_vector p' R')
(i : fin 2) : «expr = »((«expr![ , ]»([x, y]) i).coeff, «expr![ , ]»([x.coeff, y.coeff]) i) :=
by fin_cases [ident i] []; simp [] [] [] [] [] []
...
by simp [] [] [] ["[", expr has_neg.neg, ",", expr eval, ",", expr matrix.cons_fin_one, "]"] [] []
