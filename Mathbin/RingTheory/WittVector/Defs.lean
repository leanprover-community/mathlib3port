...
instance : Functor ( WittVector p ) where map α β f v := mk p ( f ∘ v.coeff ) mapConst α β a v := mk p fun _ => a

-- failed to format: format: uncaught backtrack exception
instance
  : IsLawfulFunctor ( WittVector p )
  where map_const_eq α β := rfl id_map α ⟨ v , _ ⟩ := rfl comp_map α β γ f g v := rfl
...
theorem v2_coeff {p' R'} (x y : WittVector p' R') (i : Finₓ 2) :
    ((«expr![ , ]» "././Mathport/Syntax/Translate/Basic.lean:681:61: unsupported notation `«expr![ , ]»") i).coeff =
