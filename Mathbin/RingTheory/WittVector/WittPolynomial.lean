...
lean 3 declaration is
  forall (p : Nat) (R : Type.{u_1}) [_inst_1 : CommRing.{u_1} R] [_inst_2 : Invertible.{u_1} R (Distrib.toHasMul.{u_1} R (Ring.toDistrib.{u_1} R (CommRing.toRing.{u_1} R _inst_1))) (AddMonoidWithOne.toHasOne.{u_1} R (AddGroupWithOne.toAddMonoidWithOne.{u_1} R (NonAssocRing.toAddGroupWithOne.{u_1} R (Ring.toNonAssocRing.{u_1} R (CommRing.toRing.{u_1} R _inst_1))))) ((fun (a : Type) (b : Type.{u_1}) [self : HasLiftT.{1 succ u_1} a b] => self.0) Nat R (HasLiftT.mk.{1 succ u_1} Nat R (CoeTCₓ.coe.{1 succ u_1} Nat R (Nat.castCoe.{u_1} R (AddMonoidWithOne.toHasNatCast.{u_1} R (AddGroupWithOne.toAddMonoidWithOne.{u_1} R (NonAssocRing.toAddGroupWithOne.{u_1} R (Ring.toNonAssocRing.{u_1} R (CommRing.toRing.{u_1} R _inst_1)))))))) p)], Nat -> (MvPolynomial.{0 u_1} Nat R (CommRing.toCommSemiring.{u_1} R _inst_1))
...
theorem bind₁_witt_polynomial_X_in_terms_of_W [Invertible (p : R)] (n : ℕ) : bind₁ (W_ R) (xInTermsOfW p R n) = x n :=
  ...
  have : (W_ R n - ∑ i in range n, C (p ^ i : R) * X i ^ p ^ (n - i)) = C (p ^ n : R) * X n := by
    simp only [witt_polynomial_eq_sum_C_mul_X_pow, tsub_self, sum_range_succ_comm, pow_one, add_sub_cancel, pow_zero]
