...
theorem map_eq_C_mul_X_pow_of_forall_coeff_mem {f : Polynomial R} {P : Ideal R}
  (hfP : ∀ n : ℕ, «expr↑ » n < f.degree → f.coeff n ∈ P) (hf0 : f ≠ 0) :
  map (mk P) f = C ((mk P) f.leading_coeff)*X ^ f.nat_degree :=
  Polynomial.ext
    fun n =>
      by 
        rcases lt_trichotomyₓ («expr↑ » n) (degree f) with (h | h | h)
        ·
        ...
        ·
          have  : nat_degree f = n 
          exact nat_degree_eq_of_degree_eq_some h.symm 
...
theorem irreducible_of_eisenstein_criterion {f : Polynomial R} {P : Ideal R} (hP : P.is_prime)
  (hfl : f.leading_coeff ∉ P) (hfP : ∀ n : ℕ, «expr↑ » n < degree f → f.coeff n ∈ P) (hfd0 : 0 < degree f)
    ...
    by 
      ...
      rcases
        mul_eq_mul_prime_pow
          (show Prime (X : Polynomial (Ideal.Quotient P)) from monic_X.prime_of_degree_eq_one degree_X) hf with
      ...
      ·
        ...
        exact
          Ideal.mul_mem_mul (eval_zero_mem_ideal_of_eq_mul_X_pow hp hm0) (eval_zero_mem_ideal_of_eq_mul_X_pow hq hn0)
      have hpql0 : (mk P) (p*q).leadingCoeff ≠ 0
      ·
      ...
      ·
        applyFun degree  at hbc 
        rwa [degree_C hpql0, degree_mul, eq_comm, Nat.WithBot.add_eq_zero_iff] at hbc 
      have hmp : m ≤ nat_degree p 
      exact le_nat_degree_of_map_eq_mul_X_pow hP hp hbc0.1
      have hnq : n ≤ nat_degree q 
      exact le_nat_degree_of_map_eq_mul_X_pow hP hq hbc0.2
