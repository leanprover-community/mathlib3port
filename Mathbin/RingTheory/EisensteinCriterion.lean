...
theorem irreducible_of_eisenstein_criterion {f : Polynomial R} {P : Ideal R} (hP : P.is_prime)
  ...
  have hf : f.map (mk P) = C (mk P (leading_coeff f))*X^nat_degree f := map_eq_C_mul_X_pow_of_forall_coeff_mem hfP hf0 
  ...
  ⟨mt degree_eq_zero_of_is_unit
    ...
    by 
      ...
      rw [map_mul] at hf 
      rcases
        mul_eq_mul_prime_pow (show Prime (X : Polynomial (R ⧸ P)) from monic_X.prime_of_degree_eq_one degree_X) hf with
