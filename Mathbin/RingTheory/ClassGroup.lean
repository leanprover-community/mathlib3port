...
irreducible_def toPrincipalIdeal : (K)ˣ →* (FractionalIdeal R⁰ K)ˣ :=
  { toFun := fun x =>
...
@[simp]
theorem coe_to_principal_ideal (x : (K)ˣ) : (toPrincipalIdeal R K x : FractionalIdeal R⁰ K) = span_singleton _ x :=
  rfl

@[simp]
theorem to_principal_ideal_eq_iff {I : (FractionalIdeal R⁰ K)ˣ} {x : (K)ˣ} :
