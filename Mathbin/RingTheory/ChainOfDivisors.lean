...
theorem exists_chain_of_prime_pow {p : Associates M} {n : ℕ} (hn : n ≠ 0) (hp : Prime p) :
    ∃ c : Finₓ (n + 1) → Associates M, c 1 = p ∧ StrictMonoₓ c ∧ ∀ {r : Associates M}, r ≤ p ^ n ↔ ∃ i, r = c i := by
...
theorem element_of_chain_not_is_unit_of_index_ne_zero {n : ℕ} {i : Finₓ (n + 1)} (i_pos : i ≠ 0)
    {c : Finₓ (n + 1) → Associates M} (h₁ : StrictMonoₓ c) : ¬IsUnit (c i) :=
  DvdNotUnit.not_unit
    (Associates.dvd_not_unit_iff_lt.2 (h₁ <| show (0 : Finₓ (n + 1)) < i from i.pos_iff_ne_zero.mpr i_pos))

theorem first_of_chain_is_unit {q : Associates M} {n : ℕ} {c : Finₓ (n + 1) → Associates M} (h₁ : StrictMonoₓ c)
...
theorem second_of_chain_is_irreducible {q : Associates M} {n : ℕ} (hn : n ≠ 0) {c : Finₓ (n + 1) → Associates M}
    (h₁ : StrictMonoₓ c) (h₂ : ∀ {r}, r ≤ q ↔ ∃ i, r = c i) (hq : q ≠ 0) : Irreducible (c 1) := by
  cases n
  · contradiction
...
theorem eq_second_of_chain_of_prime_dvd {p q r : Associates M} {n : ℕ} (hn : n ≠ 0) {c : Finₓ (n + 1) → Associates M}
    (h₁ : StrictMonoₓ c) (h₂ : ∀ {r : Associates M}, r ≤ q ↔ ∃ i, r = c i) (hp : Prime p) (hr : r ∣ q) (hp' : p ∣ r) :
    p = c 1 := by
  cases n
  · contradiction
...
theorem card_subset_divisors_le_length_of_chain {q : Associates M} {n : ℕ} {c : Finₓ (n + 1) → Associates M}
    (h₂ : ∀ {r}, r ≤ q ↔ ∃ i, r = c i) {m : Finsetₓ (Associates M)} (hm : ∀ r, r ∈ m → r ≤ q) : m.card ≤ n + 1 := by
  classical
  have mem_image : ∀ r : Associates M, r ≤ q → r ∈ finset.univ.image c := by
    intro r hr
    obtain ⟨i, hi⟩ := h₂.1 hr
    exact Finsetₓ.mem_image.2 ⟨i, Finsetₓ.mem_univ _, hi.symm⟩
  rw [← Finsetₓ.card_fin (n + 1)]
  exact (Finsetₓ.card_le_of_subset fun x hx => mem_image x <| hm x hx).trans Finsetₓ.card_image_le

variable [UniqueFactorizationMonoid M]

theorem element_of_chain_eq_pow_second_of_chain {q r : Associates M} {n : ℕ} (hn : n ≠ 0)
    {c : Finₓ (n + 1) → Associates M} (h₁ : StrictMonoₓ c) (h₂ : ∀ {r}, r ≤ q ↔ ∃ i, r = c i) (hr : r ∣ q)
    (hq : q ≠ 0) : ∃ i : Finₓ (n + 1), r = c 1 ^ (i : ℕ) := by
  classical
  ...
  have : (finset.univ.image fun m : Finₓ (i + 1) => c 1 ^ (m : ℕ)).card = i + 1 := by
    conv_rhs => rw [← Finsetₓ.card_fin (i + 1)]
    cases n
    · contradiction
      
    rw [Finsetₓ.card_image_iff]
...
theorem eq_pow_second_of_chain_of_has_chain {q : Associates M} {n : ℕ} (hn : n ≠ 0) {c : Finₓ (n + 1) → Associates M}
    (h₁ : StrictMonoₓ c) (h₂ : ∀ {r : Associates M}, r ≤ q ↔ ∃ i, r = c i) (hq : q ≠ 0) : q = c 1 ^ n := by
  classical
  obtain ⟨i, hi'⟩ := element_of_chain_eq_pow_second_of_chain hn h₁ (fun r => h₂) (dvd_refl q) hq
  convert hi'
  refine' (Nat.lt_succ_iff.1 i.prop).antisymm' (Nat.le_of_succ_le_succₓ _)
  calc
    n + 1 = (Finsetₓ.univ : Finsetₓ (Finₓ (n + 1))).card := (Finsetₓ.card_fin _).symm
    _ = (finset.univ.image c).card := (finset.card_image_iff.mpr (h₁.injective.inj_on _)).symm
    _ ≤ (finset.univ.image fun m : Finₓ (i + 1) => c 1 ^ (m : ℕ)).card := Finsetₓ.card_le_of_subset _
    _ ≤ (Finsetₓ.univ : Finsetₓ (Finₓ (i + 1))).card := Finsetₓ.card_image_le
    _ = i + 1 := Finsetₓ.card_fin _
    
  intro r hr
  obtain ⟨j, -, rfl⟩ := Finsetₓ.mem_image.1 hr
  have := h₂.2 ⟨j, rfl⟩
  rw [hi'] at this
  obtain ⟨u, hu, hu'⟩ := (dvd_prime_pow (show Prime (c 1) from _) i).1 this
  refine' finset.mem_image.mpr ⟨u, Finsetₓ.mem_univ _, _⟩
...
theorem is_prime_pow_of_has_chain {q : Associates M} {n : ℕ} (hn : n ≠ 0) {c : Finₓ (n + 1) → Associates M}
    (h₁ : StrictMonoₓ c) (h₂ : ∀ {r : Associates M}, r ≤ q ↔ ∃ i, r = c i) (hq : q ≠ 0) : IsPrimePow q :=
...
theorem pow_image_of_prime_by_factor_order_iso_dvd [DecidableEq (Associates M)] {m p : Associates M} {n : Associates N}
  ...
  set c₂ : Finₓ (s + 1) → Associates N := fun t => d ⟨c₁ t, le_transₓ (hc₁''.2 ⟨t, by simp⟩) hs'⟩
...
def mkFactorOrderIsoOfFactorDvdEquiv {m : M} {n : N} {d : { l : M // l ∣ m } ≃ { l : N // l ∣ n }}
  ...
  left_inv := fun ⟨l, hl⟩ => by
    simp only [Subtype.coe_eta, Equivₓ.symm_apply_apply, Subtype.coe_mk, associates_equiv_of_unique_units_apply, mk_out,
      out_mk, normalize_eq]
  right_inv := fun ⟨l, hl⟩ => by
    simp only [Subtype.coe_eta, Equivₓ.apply_symm_apply, Subtype.coe_mk, associates_equiv_of_unique_units_apply, out_mk,
      normalize_eq, mk_out]
  map_rel_iff' := by
    rintro ⟨a, ha⟩ ⟨b, hb⟩ <;>
...
theorem mem_normalized_factors_factor_dvd_iso_of_mem_normalized_factors [DecidableEq N] {m p : M} {n : N} (hm : m ≠ 0)
  ...
  have :
    Associates.mk
        ↑(d
            ⟨associatesEquivOfUniqueUnits (associates_equiv_of_unique_units.symm p), by
              simp only [dvd_of_mem_normalized_factors hp, associates_equiv_of_unique_units_apply, out_mk, normalize_eq,
