...
@[simp]
theorem is_primitive_one : is_primitive (1 : Polynomial R) := fun r h => is_unit_C.mp (is_unit_of_dvd_one (C r) h)

theorem monic.is_primitive {p : Polynomial R} (hp : p.monic) : p.is_primitive := by
  rintro r ⟨q, h⟩
  exact
...
theorem is_primitive.ne_zero [Nontrivial R] {p : Polynomial R} (hp : p.is_primitive) : p ≠ 0 := by
  rintro rfl
...
theorem content_dvd_coeff {p : Polynomial R} (n : ℕ) : p.content ∣ p.coeff n := by
...
@[simp]
theorem content_zero : content (0 : Polynomial R) = 0 := by
  rw [← C_0, content_C, normalize_zero]

@[simp]
theorem content_one : content (1 : Polynomial R) = 1 := by
  rw [← C_1, content_C, normalize_one]

theorem content_X_mul {p : Polynomial R} : content (X*p) = content p := by
...
@[simp]
theorem content_X_pow {k : ℕ} : content ((X : Polynomial R)^k) = 1 := by
...
@[simp]
theorem content_X : content (X : Polynomial R) = 1 := by
  rw [← mul_oneₓ X, content_X_mul, content_one]

theorem content_C_mul (r : R) (p : Polynomial R) : (C r*p).content = normalize r*p.content := by
  by_cases' h0 : r = 0
  ·
    simp [h0]
  rw [content]
...
theorem content_eq_zero_iff {p : Polynomial R} : content p = 0 ↔ p = 0 := by
...
theorem content_eq_gcd_leading_coeff_content_erase_lead (p : Polynomial R) :
    p.content = GcdMonoid.gcd p.leading_coeff (erase_lead p).content := by
  by_cases' h : p = 0
  ·
    simp [h]
...
theorem dvd_content_iff_C_dvd {p : Polynomial R} {r : R} : r ∣ p.content ↔ C r ∣ p := by
...
theorem is_primitive_iff_content_eq_one {p : Polynomial R} : p.is_primitive ↔ p.content = 1 := by
...
theorem eq_C_content_mul_prim_part (p : Polynomial R) : p = C p.content*p.prim_part := by
  by_cases' h : p = 0
  ·
    simp [h]
...
theorem is_primitive_prim_part (p : Polynomial R) : p.prim_part.is_primitive := by
  by_cases' h : p = 0
  ·
    simp [h]
...
theorem nat_degree_prim_part (p : Polynomial R) : p.prim_part.nat_degree = p.nat_degree := by
...
theorem content_mul_aux {p q : Polynomial R} :
    GcdMonoid.gcd (p*q).eraseLead.content p.leading_coeff = GcdMonoid.gcd (p.erase_lead*q).content p.leading_coeff := by
...
theorem is_primitive.mul {p q : Polynomial R} (hp : p.is_primitive) (hq : q.is_primitive) : (p*q).IsPrimitive := by
  rw [is_primitive_iff_content_eq_one, content_mul, hp.content_eq_one, hq.content_eq_one, mul_oneₓ]

@[simp]
theorem prim_part_mul {p q : Polynomial R} (h0 : (p*q) ≠ 0) : (p*q).primPart = p.prim_part*q.prim_part := by
...
theorem degree_gcd_le_left {p : Polynomial R} (hp : p ≠ 0) q : (gcd p q).degree ≤ p.degree := by
  have := nat_degree_le_iff_degree_le.mp (nat_degree_le_of_dvd (gcd_dvd_left p q) hp)
  rwa [degree_eq_nat_degree hp]

theorem degree_gcd_le_right p {q : Polynomial R} (hq : q ≠ 0) : (gcd p q).degree ≤ q.degree := by
  rw [gcd_comm]
