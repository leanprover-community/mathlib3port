...
theorem IsPrimitive.irreducible_iff_irreducible_map_fraction_map {p : R[X]} (hp : p.IsPrimitive) :
  ...
  obtain ⟨u, hu⟩ :
    Associated (c * d)
      (content (integerNormalization R⁰ a) * content (integerNormalization R⁰ b)) :=
    by
  ...
  · right
    apply
      isUnit_or_eq_zero_of_isUnit_integerNormalization_primPart h0.2 (isUnit_of_mul_isUnit_right h)
  · left
...
theorem IsPrimitive.dvd_of_fraction_map_dvd_fraction_map {p q : R[X]} (hp : p.IsPrimitive)
  ...
  have h : p ∣ q * c s := by
    ...
    simp
  rw [← hp.dvd_prim_part_iff_dvd, primPart_mul, hq.prim_part_eq, Associated.dvd_iff_dvd_right] at h
  · exact h
  · symm
