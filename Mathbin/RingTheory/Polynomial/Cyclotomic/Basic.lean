...
theorem X_pow_sub_one_eq_prod {ζ : R} {n : ℕ} (hpos : 0 < n) (h : IsPrimitiveRoot ζ n) :
    X ^ n - 1 = ∏ ζ in nth_roots_finset n R, X - C ζ := by
...
theorem int_coeff_of_cyclotomic' {K : Type _} [CommRingₓ K] [IsDomain K] {ζ : K} {n : ℕ} (h : IsPrimitiveRoot ζ n) :
  ...
  · use 1
    simp only [hzero, cyclotomic'_zero, Set.mem_univ, Subsemiring.coe_top, eq_self_iff_true, coe_map_ring_hom, map_one,
      and_selfₓ]
  ...
  have huniq : 0 + B * cyclotomic' k K = X ^ k - 1 ∧ (0 : Polynomial K).degree < B.degree := by
    constructor
...
theorem unique_int_coeff_of_cycl {K : Type _} [CommRingₓ K] [IsDomain K] [CharZero K] {ζ : K} {n : ℕ+}
    (h : IsPrimitiveRoot ζ n) : ∃! P : Polynomial ℤ, map (Int.castRingHom K) P = cyclotomic' n K := by
...
theorem cyclotomic_eq_prod_X_pow_sub_one_pow_moebius {n : ℕ} (R : Type _) [CommRingₓ R] [IsDomain R] :
    algebraMap _ (Ratfunc R) (cyclotomic n R) =
      ∏ i in n.divisors_antidiagonal, algebraMap (Polynomial R) _ (X ^ i.snd - 1) ^ μ i.fst :=
  by
...
theorem is_root_cyclotomic {n : ℕ} {K : Type _} [CommRingₓ K] [IsDomain K] (hpos : 0 < n) {μ : K}
    (h : IsPrimitiveRoot μ n) : is_root (cyclotomic n K) μ := by
...
private theorem is_root_cyclotomic_iff' {n : ℕ} {K : Type _} [Field K] {μ : K} [NeZero (n : K)] :
    is_root (cyclotomic n K) μ ↔ IsPrimitiveRoot μ n := by
...
theorem cyclotomic_injective {R : Type _} [CommRingₓ R] [CharZero R] : Function.Injective fun n => cyclotomic n R := by
  ...
  · rw [cyclotomic_zero] at hnm
    replace hnm := congr_argₓ nat_degree hnm
  ...
  · have := NeZero.mk hzero
    ...
    replace hnm := congr_argₓ (map (Int.castRingHom ℂ)) hnm
...
theorem coprime_of_root_cyclotomic {n : ℕ} (hpos : 0 < n) {p : ℕ} [hprime : Fact p.prime] {a : ℕ}
    (hroot : is_root (cyclotomic n (Zmod p)) (Nat.castRingHom (Zmod p) a)) : a.coprime p := by
...
theorem _root_.minpoly_dvd_cyclotomic {n : ℕ} {K : Type _} [Field K] {μ : K} (h : IsPrimitiveRoot μ n) (hpos : 0 < n)
    [CharZero K] : minpoly ℤ μ ∣ cyclotomic n ℤ := by
...
theorem cyclotomic_expand_eq_cyclotomic_mul {p n : ℕ} (hp : Nat.Prime p) (hdiv : ¬p ∣ n) (R : Type _) [CommRingₓ R] :
  ...
  refine'
    eq_of_monic_of_dvd_of_nat_degree_le (monic_mul (cyclotomic.monic _ _) (cyclotomic.monic _ _))
  ...
  · rw [nat_degree_expand, nat_degree_cyclotomic, nat_degree_mul (cyclotomic_ne_zero _ ℤ) (cyclotomic_ne_zero _ ℤ),
      nat_degree_cyclotomic, nat_degree_cyclotomic, mul_commₓ n,
      Nat.totient_mul ((Nat.Prime.coprime_iff_not_dvd hp).2 hdiv), Nat.totient_prime hp, mul_commₓ (p - 1), ←
...
theorem cyclotomic_expand_eq_cyclotomic {p n : ℕ} (hp : Nat.Prime p) (hdiv : p ∣ n) (R : Type _) [CommRingₓ R] :
  ...
  · have hpos := Nat.mul_posₓ hzero hp.pos
    ...
    refine'
      @minpoly.gcd_domain_dvd ℤ ℂ ℚ _ _ _ _ _ _ _ _ Complex.algebra (algebraInt ℂ) _ _
        (IsPrimitiveRoot.is_integral hprim hpos) _ ((cyclotomic.monic n ℤ).expand hp.pos).IsPrimitive _
    rw [aeval_def, ← eval_map, map_expand, map_cyclotomic, expand_eval, ← is_root.def, is_root_cyclotomic_iff]
    · convert IsPrimitiveRoot.pow_of_dvd hprim hp.ne_zero (dvd_mul_left p n)
      rw [Nat.mul_div_cancelₓ _ hp.pos]
      
    
  · rw [nat_degree_expand, nat_degree_cyclotomic, nat_degree_cyclotomic, mul_commₓ n,
      Nat.totient_mul_of_prime_of_dvd hp hdiv, mul_commₓ]
