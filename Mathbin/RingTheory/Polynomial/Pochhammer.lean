...
theorem Polynomial.mul_X_add_nat_cast_comp {p q : Polynomial S} {n : ℕ} : (p*X+n).comp q = p.comp q*q+n := by
...
theorem pochhammer_eval_succ (r n : ℕ) : ((n : S)*(pochhammer S r).eval (n+1 : S)) = (n+r)*(pochhammer S r).eval n := by
