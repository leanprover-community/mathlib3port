...
def degreeLtEquiv (R) [Semiring R] (n : ℕ) :
    degreeLt R n ≃ₗ[R] Fin n →
        R where 
...
def ofPolynomial (I : Ideal R[X]) :
    Submodule R R[X] where 
  carrier := I.carrier
  zero_mem' := I.zero_mem
...
instance (priority := 100) {R : Type _} [CommRing R] [IsDomain R] [WfDvdMonoid R] :
    WfDvdMonoid
