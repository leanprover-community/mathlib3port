...
theorem U_eq_X_mul_U_add_T : ∀ n : ℕ, U R (n+1) = (X*U R n)+T R (n+1)
  ...
  | 1 => by
    ...
    ring
  | n+2 =>
    calc U R ((n+2)+1) = ((2*X)*(X*U R (n+1))+T R (n+2)) - (X*U R n)+T R (n+1) := by
...
theorem T_derivative_eq_U : ∀ n : ℕ, derivative (T R (n+1)) = (n+1)*U R n
  ...
  | 1 => by
    ...
    norm_num
  | n+2 =>
    calc derivative (T R ((n+2)+1)) = ((2*T R (n+2))+(2*X)*derivative (T R ((n+1)+1))) - derivative (T R (n+1)) := by
...
theorem one_sub_X_sq_mul_derivative_T_eq_poly_in_T (n : ℕ) :
  ...
  calc ((1 - X ^ 2)*derivative (T R (n+1))) = (1 - X ^ 2)*(n+1)*U R n := by
