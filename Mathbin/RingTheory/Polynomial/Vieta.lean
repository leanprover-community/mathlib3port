...
theorem prod_X_add_C_eval (r : σ → R) :
  ...
  by 
    ...
    applyFun Polynomial.map (eval r)  at h 
    rw [map_prod, map_sum] at h 
    convert h 
    simp only [eval_X, map_add, Polynomial.map_C, Polynomial.map_X, eq_self_iff_true]
    funext 
    simp only [Function.funext_iffₓ, esymm, Polynomial.map_C, map_sum, polynomial.C.map_sum, Polynomial.map_C, map_pow,
...
theorem prod_X_add_C_coeff (r : σ → R) (k : ℕ) (h : k ≤ card σ) :
  ...
  by 
    have hk : filter (fun x : ℕ => k = card σ - x) (range (card σ+1)) = {card σ - k} :=
      by 
