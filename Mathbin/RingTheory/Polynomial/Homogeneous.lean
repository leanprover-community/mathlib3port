...
theorem homogeneous_submodule_eq_finsupp_supported [CommSemiringₓ R] (n : ℕ) :
    homogeneous_submodule σ R n = Finsupp.supported _ R { d | (∑ i in d.support, d i) = n } := by
  ext
...
theorem homogeneous_submodule_mul [CommSemiringₓ R] (m n : ℕ) :
    homogeneous_submodule σ R m * homogeneous_submodule σ R n ≤ homogeneous_submodule σ R (m + n) := by
  rw [Submodule.mul_le]
...
theorem is_homogeneous_X (i : σ) : is_homogeneous (X i : MvPolynomial σ R) 1 := by
  apply is_homogeneous_monomial
...
theorem coeff_eq_zero (hφ : is_homogeneous φ n) (d : σ →₀ ℕ) (hd : (∑ i in d.support, d i) ≠ n) : coeff d φ = 0 := by
  have aux := mt (@hφ d) hd
  classical
  rwa [not_not] at aux

theorem inj_right (hm : is_homogeneous φ m) (hn : is_homogeneous φ n) (hφ : φ ≠ 0) : m = n := by
...
theorem total_degree (hφ : is_homogeneous φ n) (h : φ ≠ 0) : total_degree φ = n := by
  rw [total_degree]
  apply le_antisymmₓ
  · apply Finset.sup_le
    intro d hd
...
instance homogeneous_submodule.gcomm_semiring : SetLike.GradedMonoid (homogeneous_submodule σ R) where
  one_mem := is_homogeneous_one σ R
  mul_mem := fun i j xi xj => is_homogeneous.mul
...
theorem homogeneous_component_is_homogeneous : (homogeneous_component n φ).IsHomogeneous n := by
  intro d hd
  contrapose! hd
...
theorem homogeneous_component_eq_zero (h : φ.total_degree < n) : homogeneous_component n φ = 0 := by
...
theorem sum_homogeneous_component : (∑ i in range (φ.total_degree + 1), homogeneous_component i φ) = φ := by
  ext1 d
