...
instance
  ( priority := 100 )
  of_is_lawful_functor
  ( f : Type u₀ → Type u₁ ) [ Functor f ] [ IsLawfulFunctor f ] : EquivFunctor f
  where
    map α β e := Functor.map e
      map_refl' α := by ext apply IsLawfulFunctor.id_map
      map_trans' α β γ k h := by ext x apply IsLawfulFunctor.comp_map k h x
