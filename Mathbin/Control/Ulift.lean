...
instance : Monadₓ Plift :=
  { map := @Plift.map, pure := @Plift.pure, seq := @Plift.seq, bind := @Plift.bind }

instance : IsLawfulFunctor Plift :=
  { id_map := fun α ⟨x⟩ => rfl, comp_map := fun α β γ g h ⟨x⟩ => rfl }

instance : IsLawfulApplicative Plift :=
  { pure_seq_eq_map := fun α β g ⟨x⟩ => rfl, map_pure := fun α β g x => rfl, seq_pure := fun α β ⟨g⟩ x => rfl,
    seq_assoc := fun α β γ ⟨x⟩ ⟨g⟩ ⟨h⟩ => rfl }

instance : IsLawfulMonad Plift :=
  { bind_pure_comp_eq_map := fun α β f ⟨x⟩ => rfl, bind_map_eq_seq := fun α β ⟨a⟩ ⟨b⟩ => rfl,
    pure_bind := fun α β x f => rfl, bind_assoc := fun α β γ ⟨x⟩ f g => rfl }
...
instance : Monadₓ Ulift :=
  { map := @Ulift.map, pure := @Ulift.pure, seq := @Ulift.seq, bind := @Ulift.bind }

instance : IsLawfulFunctor Ulift :=
  { id_map := fun α ⟨x⟩ => rfl, comp_map := fun α β γ g h ⟨x⟩ => rfl }

instance : IsLawfulApplicative Ulift :=
  { to_is_lawful_functor := Ulift.is_lawful_functor, pure_seq_eq_map := fun α β g ⟨x⟩ => rfl,
    map_pure := fun α β g x => rfl, seq_pure := fun α β ⟨g⟩ x => rfl, seq_assoc := fun α β γ ⟨x⟩ ⟨g⟩ ⟨h⟩ => rfl }

instance : IsLawfulMonad Ulift :=
  { bind_pure_comp_eq_map := fun α β f ⟨x⟩ => rfl, bind_map_eq_seq := fun α β ⟨a⟩ ⟨b⟩ => rfl,
    pure_bind :=
      fun α β x f =>
        by 
