...
instance : IsLawfulFunctor Plift where id_map α ⟨ x ⟩ := rfl comp_map α β γ g h ⟨ x ⟩ := rfl

-- failed to format: format: uncaught backtrack exception
instance
  : IsLawfulApplicative Plift
  where
    pure_seq_eq_map α β g ⟨ x ⟩ := rfl
      map_pure α β g x := rfl
      seq_pure α β ⟨ g ⟩ x := rfl
      seq_assoc α β γ ⟨ x ⟩ ⟨ g ⟩ ⟨ h ⟩ := rfl
...
instance
  ...
  where
    bind_pure_comp_eq_map α β f ⟨ x ⟩ := rfl
      bind_map_eq_seq α β ⟨ a ⟩ ⟨ b ⟩ := rfl
      pure_bind α β x f := rfl
      bind_assoc α β γ ⟨ x ⟩ f g := rfl
...
-- failed to format: format: uncaught backtrack exception
instance : IsLawfulFunctor Ulift where id_map α ⟨ x ⟩ := rfl comp_map α β γ g h ⟨ x ⟩ := rfl

-- failed to format: format: uncaught backtrack exception
instance
  : IsLawfulApplicative Ulift
  where
...
instance
  ...
  where
    bind_pure_comp_eq_map α β f ⟨ x ⟩ := rfl
      ...
      bind_assoc α β γ ⟨ x ⟩ f g := by dsimp only [ bind , pure , Ulift.pure , Ulift.bind ] cases f x rfl
