...
/- ./././Mathport/Syntax/Translate/Command.lean:379:30: infer kinds are unsupported in Lean 4: #[`congr] [] -/
...
def adaptUp (F : Type v₀ → Type v₁) (G : Type max v₀ u₀ → Type u₁) [Uliftable F G] [Monad G] {α β} (x : F α)
    (f : α → G β) : G β :=
  up x >>= f ∘ ULift.down
