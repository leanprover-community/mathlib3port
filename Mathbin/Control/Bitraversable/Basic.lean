...
class IsLawfulBitraversable (t : Type u → Type u → Type u) [Bitraversable t] extends IsLawfulBifunctor t where 
  ...
  comp_bitraverse :
  ∀ {F G} [Applicativeₓ F] [Applicativeₓ G] [IsLawfulApplicative F] [IsLawfulApplicative G] {α α' β β' γ γ'} f : β → F γ
...
attribute [higherOrder bitraverse_id_id] id_bitraverse

attribute [higherOrder bitraverse_comp] comp_bitraverse

attribute [higherOrder] binaturality bitraverse_eq_bimap_id
