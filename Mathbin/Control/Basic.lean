...
@[reducible]
def fish {m} [Monadₓ m] {α β γ} (f : α → m β) (g : β → m γ) :=
  fun x => f x >>= g

infixl:55 " >=> " => fish

@[functor_norm]
theorem fish_pure {α β} (f : α → m β) : f >=> pure = f :=
  by 
    simp' only [· >=> ·] with functor_norm

@[functor_norm]
theorem fish_pipe {α β} (f : α → m β) : pure >=> f = f :=
  by 
    simp' only [· >=> ·] with functor_norm

@[functor_norm]
theorem fish_assoc {α β γ φ} (f : α → m β) (g : β → m γ) (h : γ → m φ) : f >=> g >=> h = f >=> (g >=> h) :=
  by 
    simp' only [· >=> ·] with functor_norm
...
theorem mjoin_map_mjoin {α : Type u} (a : m (m (m α))) : mjoin (mjoin <$> a) = mjoin (mjoin a) :=
  by 
    simp only [mjoin, · ∘ ·, id.def, map_bind, (bind_pure_comp_eq_map _ _).symm, bind_assoc, pure_bind]

@[simp]
theorem mjoin_map_pure {α : Type u} (a : m α) : mjoin (pure <$> a) = a :=
  by 
...
@[simp]
theorem guard_true {h : Decidable True} : @guardₓ F _ True h = pure () :=
  by 
    simp [guardₓ]

@[simp]
theorem guard_false {h : Decidable False} : @guardₓ F _ False h = failure :=
  by 
    simp [guardₓ]
