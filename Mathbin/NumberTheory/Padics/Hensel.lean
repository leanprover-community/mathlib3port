...
theorem padic_polynomial_dist {p : ℕ} [Fact p.prime] (F : Polynomial ℤ_[p]) (x y : ℤ_[p]) :
  ...
  calc ∥F.eval x - F.eval y∥ = ∥z∥*∥x - y∥ := by
    simp [hz]
...
private def calc_eval_z' {z z' z1 : ℤ_[p]} (hz' : z' = z - z1) {n} (hz : ih n z)
  ...
  have : (F.derivative.eval z*-z1) = -F.eval z :=
    calc (F.derivative.eval z*-z1) = F.derivative.eval z*-⟨↑F.eval z / ↑F.derivative.eval z, h1⟩ := by
      rw [hzeq]
...
private theorem newton_seq_dist_aux (n : ℕ) : ∀ k : ℕ, ∥newton_seq (n+k) - newton_seq n∥ ≤ ∥F.derivative.eval a∥*T^2^n
  ...
  | k+1 =>
    ...
    calc ∥newton_seq (n+k+1) - newton_seq n∥ = ∥newton_seq ((n+k)+1) - newton_seq n∥ := by
      rw [add_assocₓ]
...
private theorem newton_seq_dist_to_a : ∀ n : ℕ, 0 < n → ∥newton_seq n - a∥ = ∥F.eval a∥ / ∥F.derivative.eval a∥
  ...
  | k+2, h =>
    ...
    calc ∥newton_seq (k+2) - a∥ = ∥(newton_seq (k+2) - newton_seq (k+1))+newton_seq (k+1) - a∥ := by
      rw [← sub_add_sub_cancel]
  ...
  have : h = 0 :=
    by_contradiction $ fun hne =>
      ...
      lt_irreflₓ ∥F.derivative.eval soln∥
        (calc ∥F.derivative.eval soln∥ = ∥(-q)*h∥ := by
          rw [this]
          _ ≤ 1*∥h∥ := by
  ...
  have : h = 0 :=
    by_contradiction $ fun hne =>
      ...
      lt_irreflₓ ∥F.derivative.eval a∥
        (calc ∥F.derivative.eval a∥ = ∥q∥*∥h∥ := by
          simp [this]
