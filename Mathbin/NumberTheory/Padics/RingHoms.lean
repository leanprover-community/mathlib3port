...
theorem norm_sub_mod_part_aux (r : ℚ) (h : ∥(r : ℚ_[p])∥ ≤ 1) :
  «expr↑ » p ∣ r.num - ((r.num*r.denom.gcd_a p) % p)*«expr↑ » r.denom :=
  by 
...
theorem appr_spec (n : ℕ) : ∀ x : ℤ_[p], x - appr x n ∈ (Ideal.span {p^n} : Ideal ℤ_[p]) :=
  by 
    ...
    rw
      [show (x - «expr↑ » (appr x n)).Valuation = ((«expr↑ » p^n)*c).Valuation by 
        rw [hc]]
...
theorem pow_dvd_nth_hom_sub (r : R) (i j : ℕ) (h : i ≤ j) : («expr↑ » p^i) ∣ nth_hom f r j - nth_hom f r i :=
  by 
...
theorem lift_sub_val_mem_span (r : R) (n : ℕ) :
  lift f_compat r - (f n r).val ∈ (Ideal.span {«expr↑ » p^n} : Ideal ℤ_[p]) :=
  by 
    obtain ⟨k, hk⟩ :=
    ...
    dsimp [lift]
    rw [sub_eq_sub_add_sub (lim_nth_hom f_compat r) _ («expr↑ » (nth_hom f r (max n k)))]
