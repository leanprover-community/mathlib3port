...
instance is_absolute_value : IsAbsoluteValue fun z : ℤ_[p] => ‖z‖
    where
...
theorem maximal_ideal_eq_span_p : maximalIdeal ℤ_[p] = Ideal.span {p} :=
  by
  apply le_antisymm
  · intro x hx
...
instance is_fraction_ring : IsFractionRing ℤ_[p] ℚ_[p]
    where
