...
instance :
    CharZero
      ℤ_[p] where cast_injective m n h :=
    Nat.cast_injective <|
      show (m : ℚ_[p]) = n by 
...
theorem unit_coeff_spec {x : ℤ_[p]} (hx : x ≠ 0) :
...
instance :
    IsAdicComplete (maximalIdeal ℤ_[p])
      ℤ_[p] where prec' x hx := by
