...
theorem admissible.one_lt_sum_inv {pqr : Multiset ℕ+} : admissible pqr → 1 < sum_inv pqr := by
  rw [admissible]
...
theorem lt_three {p q r : ℕ+} (hpq : p ≤ q) (hqr : q ≤ r) (H : 1 < sum_inv {p, q, r}) : p < 3 := by
  have h3 : (0 : ℚ) < 3 := by
    norm_num
  contrapose! H
  rw [sum_inv_pqr]
...
theorem lt_four {q r : ℕ+} (hqr : q ≤ r) (H : 1 < sum_inv {2, q, r}) : q < 4 := by
  have h4 : (0 : ℚ) < 4 := by
    norm_num
  contrapose! H
  rw [sum_inv_pqr]
...
theorem lt_six {r : ℕ+} (H : 1 < sum_inv {2, 3, r}) : r < 6 := by
  have h6 : (0 : ℚ) < 6 := by
    norm_num
  contrapose! H
  rw [sum_inv_pqr]
...
theorem admissible_of_one_lt_sum_inv {p q r : ℕ+} (H : 1 < sum_inv {p, q, r}) : admissible {p, q, r} := by
