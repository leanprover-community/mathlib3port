  ...
  by 
    ...
    ·
      ...
      rw [choose_eq_factorial_div_factorial h.le, eq_comm, div_eq_iff (hne q.succ), succ_eq_add_one,
        mul_assocₓ _ _ (↑q.succ !), mul_commₓ _ (↑q.succ !), ←mul_assocₓ, div_mul_eq_mul_div, mul_commₓ (↑n^(q - m)+1),
        ←mul_assocₓ _ _ (↑n^(q - m)+1), ←one_div, mul_one_div, div_div_eq_div_mul,
      ...
      ·
        simp [hne]
    have hps : (∑ k in range n, ↑k^p) = (∑ i in range (p+1), ((bernoulli i*(p+1).choose i)*n^(p+1) - i) / (p+1)!)*p !
    ·
      suffices  :
    ...
    rw [hps, sum_mul]
    refine' sum_congr rfl fun x hx => _ 
    fieldSimp [mul_right_commₓ _ (↑p !), ←mul_assocₓ _ _ (↑p !), cast_add_one_ne_zero, hne]
...
theorem sum_Ico_pow (n p : ℕ) :
  ...
  by 
    ...
    let f' := fun i => ((bernoulli' i*p.succ.succ.choose i)*n^p.succ.succ - i) / p.succ.succ 
    suffices  : (∑ k in Ico 1 n.succ, ↑k^p.succ) = ∑ i in range p.succ.succ, f' i
    ·
      convert this 
    ...
    calc (∑ k in Ico 1 n.succ, ↑k^p.succ) = ∑ k in range n.succ, ↑k^p.succ :=
      by 
