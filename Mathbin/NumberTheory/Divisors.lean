...
theorem proper_divisors.not_self_mem : ¬n ∈ proper_divisors n := by
  rw [proper_divisors]
  simp

@[simp]
theorem mem_proper_divisors {m : ℕ} : n ∈ proper_divisors m ↔ n ∣ m ∧ n < m := by
...
theorem divisors_eq_proper_divisors_insert_self_of_pos (h : 0 < n) :
    divisors n = HasInsert.insert n (proper_divisors n) := by
...
@[simp]
theorem mem_divisors_antidiagonal {x : ℕ × ℕ} : x ∈ divisors_antidiagonal n ↔ x.fst * x.snd = n ∧ n ≠ 0 := by
...
theorem proper_divisors_subset_divisors : proper_divisors n ⊆ divisors n := by
  cases n
  · simp
...
@[simp]
theorem proper_divisors_one : proper_divisors 1 = ∅ := by
  ext
...
@[simp]
theorem divisors_antidiagonal_zero : divisors_antidiagonal 0 = ∅ := by
  ext
  simp

@[simp]
theorem divisors_antidiagonal_one : divisors_antidiagonal 1 = {(1, 1)} := by
  ext
...
theorem snd_mem_divisors_of_mem_antidiagonal {x : ℕ × ℕ} (h : x ∈ divisors_antidiagonal n) : x.snd ∈ divisors n := by
  rw [mem_divisors_antidiagonal] at h
  simp [Dvd.intro_left _ h.1, h.2]

@[simp]
theorem map_swap_divisors_antidiagonal :
    (divisors_antidiagonal n).map ⟨Prod.swap, Prod.swap_right_inverseₓ.Injective⟩ = divisors_antidiagonal n := by
  ext
...
theorem sum_divisors_eq_sum_proper_divisors_add_self : (∑ i in divisors n, i) = (∑ i in proper_divisors n, i) + n := by
  cases n
  · simp
...
theorem eq_proper_divisors_of_subset_of_sum_eq_sum {s : Finset ℕ} (hsub : s ⊆ n.proper_divisors) :
    ((∑ x in s, x) = ∑ x in n.proper_divisors, x) → s = n.proper_divisors := by
  cases n
...
theorem sum_proper_divisors_dvd (h : (∑ x in n.proper_divisors, x) ∣ n) :
    (∑ x in n.proper_divisors, x) = 1 ∨ (∑ x in n.proper_divisors, x) = n := by
  cases n
  · simp
...
theorem prime.prod_proper_divisors {α : Type _} [CommMonoidₓ α] {p : ℕ} {f : ℕ → α} (h : p.prime) :
    (∏ x in p.proper_divisors, f x) = f 1 := by
...
theorem sum_proper_divisors_eq_one_iff_prime : (∑ x in n.proper_divisors, x) = 1 ↔ n.prime := by
  cases n
...
theorem mem_proper_divisors_prime_pow {p : ℕ} (pp : p.prime) (k : ℕ) {x : ℕ} :
    x ∈ proper_divisors (p ^ k) ↔ ∃ (j : ℕ)(H : j < k), x = p ^ j := by
...
theorem proper_divisors_prime_pow {p : ℕ} (pp : p.prime) (k : ℕ) :
    proper_divisors (p ^ k) = (Finset.range k).map ⟨pow p, pow_right_injective pp.two_le⟩ := by
  ext
...
theorem prime_divisors_eq_to_filter_divisors_prime (n : ℕ) : n.factors.to_finset = (divisors n).filter Prime := by
