...
theorem wilsons_lemma : ((p - 1)! : Zmod p) = -1 := by
  refine'
    calc
      ((p - 1)! : Zmod p) = ∏ x in Ico 1 (succ (p - 1)), x := by rw [← Finset.prod_Ico_id_eq_factorial, prod_nat_cast]
      _ = ∏ x : (Zmod p)ˣ, x := _
      _ = -1 := by
...
#align zmod.Ico_map_val_min_abs_nat_abs_eq_Ico_map_id Zmod.Ico_map_val_min_abs_nat_abs_eq_Ico_map_id
