...
localized [NumberTheorySymbols] notation "J(" a " | " b ")" => jacobiSym a b
...
@[simp]
theorem jacobi_sym_zero_right (a : ℤ) : J(a | 0) = 1 := by
  simp only [jacobi_sym, factors_zero, List.prod_nil, List.pmap]

/-- The symbol `J(a | 1)` has the value `1`. -/
@[simp]
theorem jacobi_sym_one_right (a : ℤ) : J(a | 1) = 1 := by
  simp only [jacobi_sym, factors_one, List.prod_nil, List.pmap]

/-- The Legendre symbol `legendre_sym p a` with an integer `a` and a prime number `p`
is the same as the Jacobi symbol `J(a | p)`. -/
theorem legendreSym.to_jacobi_sym {p : ℕ} [fp : Fact p.Prime] {a : ℤ} : legendreSym p a = J(a | p) := by
  simp only [jacobi_sym, factors_prime fp.1, List.prod_cons, List.prod_nil, mul_oneₓ, List.pmap]

/-- The Jacobi symbol is multiplicative in its second argument. -/
theorem jacobi_sym_mul_right' (a : ℤ) {b₁ b₂ : ℕ} (hb₁ : b₁ ≠ 0) (hb₂ : b₂ ≠ 0) :
...
/-- The Jacobi symbol is multiplicative in its second argument. -/
theorem jacobi_sym_mul_right (a : ℤ) (b₁ b₂ : ℕ) [NeZero b₁] [NeZero b₂] : J(a | b₁ * b₂) = J(a | b₁) * J(a | b₂) :=
  jacobi_sym_mul_right' a (NeZero.ne b₁) (NeZero.ne b₂)

/-- The Jacobi symbol takes only the values `0`, `1` and `-1`. -/
theorem jacobi_sym_trichotomy (a : ℤ) (b : ℕ) : J(a | b) = 0 ∨ J(a | b) = 1 ∨ J(a | b) = -1 :=
...
@[simp]
theorem jacobi_sym_one_left (b : ℕ) : J(1 | b) = 1 :=
  List.prod_eq_one fun z hz => by
    let ⟨p, hp, he⟩ := List.mem_pmap.1 hz
    rw [← he, legendre_sym_one]

/-- The Jacobi symbol is multiplicative in its first argument. -/
theorem jacobi_sym_mul_left (a₁ a₂ : ℤ) (b : ℕ) : J(a₁ * a₂ | b) = J(a₁ | b) * J(a₂ | b) := by
  simp_rw [jacobi_sym, List.pmap_eq_map_attach, legendre_sym_mul]
  exact List.prod_map_mul

/-- The symbol `J(a | b)` vanishes iff `a` and `b` are not coprime (assuming `b ≠ 0`). -/
theorem jacobi_sym_eq_zero_iff_not_coprime {a : ℤ} {b : ℕ} [NeZero b] : J(a | b) = 0 ↔ a.gcd b ≠ 1 :=
  List.prod_eq_zero_iff.trans
    (by
      rw [List.mem_pmap, Int.gcd_eq_nat_abs, Ne, prime.not_coprime_iff_dvd]
      simp_rw [legendre_sym_eq_zero_iff, int_coe_zmod_eq_zero_iff_dvd, mem_factors (NeZero.ne b), ←
        Int.coe_nat_dvd_left, Int.coe_nat_dvd, exists_propₓ, and_assocₓ, and_comm])

/-- The symbol `J(a | b)` is nonzero when `a` and `b` are coprime. -/
theorem jacobi_sym_ne_zero {a : ℤ} {b : ℕ} (h : a.gcd b = 1) : J(a | b) ≠ 0 := by
  cases' eq_zero_or_ne_zero b with hb
  · rw [hb, jacobi_sym_zero_right]
    exact one_ne_zero
    
  · contrapose! h
    exact jacobi_sym_eq_zero_iff_not_coprime.1 h
    

/-- The symbol `J(a | b)` vanishes if and only if `b ≠ 0` and `a` and `b` are not coprime. -/
theorem jacobi_sym_eq_zero_iff {a : ℤ} {b : ℕ} : J(a | b) = 0 ↔ b ≠ 0 ∧ a.gcd b ≠ 1 :=
  ⟨fun h => by
    cases' eq_or_ne b 0 with hb hb
    · rw [hb, jacobi_sym_zero_right] at h
      cases h
      
    exact ⟨hb, mt jacobi_sym_ne_zero <| not_not.2 h⟩, fun ⟨hb, h⟩ => by
    rw [← ne_zero_iff] at hb
    exact jacobi_sym_eq_zero_iff_not_coprime.2 h⟩

/-- The symbol `J(0 | b)` vanishes when `b > 1`. -/
theorem jacobi_sym_zero_left {b : ℕ} (hb : 1 < b) : J(0 | b) = 0 :=
  (@jacobi_sym_eq_zero_iff_not_coprime 0 b ⟨ne_zero_of_lt hb⟩).mpr <| by
    rw [Int.gcd_zero_left, Int.nat_abs_of_nat]
    exact hb.ne'

/-- The symbol `J(a | b)` takes the value `1` or `-1` if `a` and `b` are coprime. -/
theorem jacobi_sym_eq_one_or_neg_one {a : ℤ} {b : ℕ} (h : a.gcd b = 1) : J(a | b) = 1 ∨ J(a | b) = -1 :=
  (jacobi_sym_trichotomy a b).resolve_left <| jacobi_sym_ne_zero h

/-- We have that `J(a^e | b) = J(a | b)^e`. -/
theorem jacobi_sym_pow_left (a : ℤ) (e b : ℕ) : J(a ^ e | b) = J(a | b) ^ e :=
  (Nat.recOn e
      (by
        rw [pow_zeroₓ, pow_zeroₓ, jacobi_sym_one_left]))
    fun _ ih => by
    rw [pow_succₓ, pow_succₓ, jacobi_sym_mul_left, ih]

/-- We have that `J(a | b^e) = J(a | b)^e`. -/
theorem jacobi_sym_pow_right (a : ℤ) (b e : ℕ) : J(a | b ^ e) = J(a | b) ^ e := by
  induction' e with e ih
  · rw [pow_zeroₓ, pow_zeroₓ, jacobi_sym_one_right]
    
  · cases' eq_zero_or_ne_zero b with hb
    · rw [hb, zero_pow (succ_pos e), jacobi_sym_zero_right, one_pow]
      
    · rw [pow_succₓ, pow_succₓ, jacobi_sym_mul_right, ih]
      
    

/-- The square of `J(a | b)` is `1` when `a` and `b` are coprime. -/
theorem jacobi_sym_sq_one {a : ℤ} {b : ℕ} (h : a.gcd b = 1) : J(a | b) ^ 2 = 1 := by
  cases' jacobi_sym_eq_one_or_neg_one h with h₁ h₁ <;> rw [h₁] <;> rfl

/-- The symbol `J(a^2 | b)` is `1` when `a` and `b` are coprime. -/
theorem jacobi_sym_sq_one' {a : ℤ} {b : ℕ} (h : a.gcd b = 1) : J(a ^ 2 | b) = 1 := by
  rw [jacobi_sym_pow_left, jacobi_sym_sq_one h]

/-- The symbol `J(a | b)` depends only on `a` mod `b`. -/
theorem jacobi_sym_mod_left (a : ℤ) (b : ℕ) : J(a | b) = J(a % b | b) :=
  congr_arg List.prod <|
    List.pmap_congr _
      (by
        rintro p hp _ _
        conv_rhs =>
          rw [legendre_sym_mod, Int.mod_mod_of_dvd _ (Int.coe_nat_dvd.2 <| dvd_of_mem_factors hp), ← legendre_sym_mod])

/-- The symbol `J(a | b)` depends only on `a` mod `b`. -/
theorem jacobi_sym_mod_left' {a₁ a₂ : ℤ} {b : ℕ} (h : a₁ % b = a₂ % b) : J(a₁ | b) = J(a₂ | b) := by
  rw [jacobi_sym_mod_left, h, ← jacobi_sym_mod_left]

/-- If `J(a | b)` is `-1`, then `a` is not a square modulo `b`. -/
theorem nonsquare_of_jacobi_sym_eq_neg_one {a : ℤ} {b : ℕ} (h : J(a | b) = -1) : ¬IsSquare (a : Zmod b) :=
  fun ⟨r, ha⟩ => by
...
theorem nonsquare_iff_jacobi_sym_eq_neg_one {a : ℤ} {p : ℕ} [Fact p.Prime] : J(a | p) = -1 ↔ ¬IsSquare (a : Zmod p) :=
  by
  rw [← legendre_sym.to_jacobi_sym]
  exact legendre_sym_eq_neg_one_iff p
...
theorem jacobi_sym_value (a : ℤ) {R : Type _} [CommSemiringₓ R] (χ : R →* ℤ)
  ...
  rw [jacobi_sym, List.map_mapₓ, ← List.pmap_eq_map Nat.Prime _ _ fun _ => prime_of_mem_factors]
  congr 1
  apply List.pmap_congr
  exact fun p h pp _ => hp p pp (hb.factors_ne_two h)

/-- If `b` is odd, then `J(-1 | b)` is given by `χ₄ b`. -/
theorem jacobi_sym_neg_one {b : ℕ} (hb : Odd b) : J(-1 | b) = χ₄ b :=
  jacobi_sym_value (-1) χ₄ (fun p pp => @legendre_sym_neg_one p ⟨pp⟩) hb

/-- If `b` is odd, then `J(-a | b) = χ₄ b * J(a | b)`. -/
theorem jacobi_sym_neg (a : ℤ) {b : ℕ} (hb : Odd b) : J(-a | b) = χ₄ b * J(a | b) := by
  rw [neg_eq_neg_one_mul, jacobi_sym_mul_left, jacobi_sym_neg_one hb]

/-- If `b` is odd, then `J(2 | b)` is given by `χ₈ b`. -/
theorem jacobi_sym_two {b : ℕ} (hb : Odd b) : J(2 | b) = χ₈ b :=
  jacobi_sym_value 2 χ₈ (fun p pp => @legendre_sym_two p ⟨pp⟩) hb

/-- If `b` is odd, then `J(-2 | b)` is given by `χ₈' b`. -/
theorem jacobi_sym_neg_two {b : ℕ} (hb : Odd b) : J(-2 | b) = χ₈' b :=
  jacobi_sym_value (-2) χ₈' (fun p pp => @legendre_sym_neg_two p ⟨pp⟩) hb
...
theorem qr_sign_neg_one_pow {m n : ℕ} (hm : Odd m) (hn : Odd n) : qrSign m n = -1 ^ (m / 2 * (n / 2)) := by
  rw [qr_sign, pow_mulₓ, ← χ₄_eq_neg_one_pow (odd_iff.mp hm)]
...
theorem qr_sign_sq_eq_one {m n : ℕ} (hm : Odd m) (hn : Odd n) : qrSign m n ^ 2 = 1 := by
  rw [qr_sign_neg_one_pow hm hn, ← pow_mulₓ, mul_comm, pow_mulₓ, neg_one_sq, one_pow]

/-- `qr_sign` is multiplicative in the first argument. -/
theorem qr_sign_mul_left (m₁ m₂ n : ℕ) : qrSign (m₁ * m₂) n = qrSign m₁ n * qrSign m₂ n := by
  simp_rw [qr_sign, Nat.cast_mulₓ, map_mul, jacobi_sym_mul_left]

/-- `qr_sign` is multiplicative in the second argument. -/
theorem qr_sign_mul_right (m n₁ n₂ : ℕ) [NeZero n₁] [NeZero n₂] : qrSign m (n₁ * n₂) = qrSign m n₁ * qrSign m n₂ :=
  jacobi_sym_mul_right (χ₄ m) n₁ n₂

/-- `qr_sign` is symmetric when both arguments are odd. -/
theorem qr_sign_symm {m n : ℕ} (hm : Odd m) (hn : Odd n) : qrSign m n = qrSign n m := by
  rw [qr_sign_neg_one_pow hm hn, qr_sign_neg_one_pow hn hm, mul_comm (m / 2)]

/-- We can move `qr_sign m n` from one side of an equality to the other when `m` and `n` are odd. -/
theorem qr_sign_eq_iff_eq {m n : ℕ} (hm : Odd m) (hn : Odd n) (x y : ℤ) : qrSign m n * x = y ↔ x = qrSign m n * y := by
  refine'
      ⟨fun h' =>
        let h := h'.symm
        _,
        fun h => _⟩ <;>
    rw [h, ← mul_assoc, ← pow_two, qr_sign_sq_eq_one hm hn, one_mulₓ]

/-- The Law of Quadratic Reciprocity for the Jacobi symbol, version with `qr_sign` -/
theorem jacobi_sym_quadratic_reciprocity' {a b : ℕ} (ha : Odd a) (hb : Odd b) : J(a | b) = qrSign b a * J(b | a) := by
  -- define the right hand side for fixed `a` as a `ℕ →* ℤ`
  let rhs : ℕ → ℕ →* ℤ := fun a =>
    { toFun := fun x => qr_sign x a * J(x | a),
      map_one' := by
        convert ← mul_oneₓ _
        symm
        all_goals
          apply jacobi_sym_one_left,
      map_mul' := fun x y => by
        rw [qr_sign_mul_left, Nat.cast_mulₓ, jacobi_sym_mul_left, mul_mul_mul_commₓ] }
  have rhs_apply : ∀ a b : ℕ, rhs a b = qr_sign b a * J(b | a) := fun a b => rfl
  refine' jacobi_sym_value a (rhs a) (fun p pp hp => Eq.symm _) hb
  have hpo := pp.eq_two_or_odd'.resolve_left hp
  rw [@legendre_sym.to_jacobi_sym p ⟨pp⟩, rhs_apply, Nat.cast_id, qr_sign_eq_iff_eq hpo ha, qr_sign_symm hpo ha]
  refine' jacobi_sym_value p (rhs p) (fun q pq hq => _) ha
  have hqo := pq.eq_two_or_odd'.resolve_left hq
  rw [rhs_apply, Nat.cast_id, ← @legendre_sym.to_jacobi_sym p ⟨pp⟩, qr_sign_symm hqo hpo, qr_sign_neg_one_pow hpo hqo,
    @quadratic_reciprocity' p q ⟨pp⟩ ⟨pq⟩ hp hq]

/-- The Law of Quadratic Reciprocity for the Jacobi symbol -/
theorem jacobi_sym_quadratic_reciprocity {a b : ℕ} (ha : Odd a) (hb : Odd b) :
    J(a | b) = -1 ^ (a / 2 * (b / 2)) * J(b | a) := by
  rw [← qr_sign_neg_one_pow ha hb, qr_sign_symm ha hb, jacobi_sym_quadratic_reciprocity' ha hb]
...
theorem jacobi_sym_quadratic_reciprocity_one_mod_four {a b : ℕ} (ha : a % 4 = 1) (hb : Odd b) : J(a | b) = J(b | a) :=
  by
  rw [jacobi_sym_quadratic_reciprocity (odd_iff.mpr (odd_of_mod_four_eq_one ha)) hb, pow_mulₓ,
    neg_one_pow_div_two_of_one_mod_four ha, one_pow, one_mulₓ]

/-- The Law of Quadratic Reciprocity for the Jacobi symbol: if `a` and `b` are natural numbers
with `a` odd and `b % 4 = 1`, then `J(a | b) = J(b | a)`. -/
theorem jacobi_sym_quadratic_reciprocity_one_mod_four' {a b : ℕ} (ha : Odd a) (hb : b % 4 = 1) : J(a | b) = J(b | a) :=
...
theorem jacobi_sym_quadratic_reciprocity_three_mod_four {a b : ℕ} (ha : a % 4 = 3) (hb : b % 4 = 3) :
    J(a | b) = -J(b | a) := by
  let nop := @neg_one_pow_div_two_of_three_mod_four
  rw [jacobi_sym_quadratic_reciprocity, pow_mulₓ, nop ha, nop hb, neg_one_mul] <;>
    rwa [odd_iff, odd_of_mod_four_eq_three]

/-- The Jacobi symbol `J(a | b)` depends only on `b` mod `4*a` (version for `a : ℕ`). -/
theorem jacobi_sym_mod_right' (a : ℕ) {b : ℕ} (hb : Odd b) : J(a | b) = J(a | b % (4 * a)) := by
    ...
    hb.mod_even
      (Even.mul_right
        (by
          norm_num)
        _)
  rcases exists_eq_pow_mul_and_not_dvd ha₀ 2
      (by
        norm_num) with
    ⟨e, a', ha₁', ha₂⟩
  ...
  nth_rw 1[ha₂]
  nth_rw 0[ha₂]
  rw [Nat.cast_mulₓ, jacobi_sym_mul_left, jacobi_sym_mul_left, jacobi_sym_quadratic_reciprocity' ha₁ hb,
    jacobi_sym_quadratic_reciprocity' ha₁ hb', Nat.cast_powₓ, jacobi_sym_pow_left, jacobi_sym_pow_left,
    show ((2 : ℕ) : ℤ) = 2 by
      rfl,
    jacobi_sym_two hb, jacobi_sym_two hb']
  congr 1
  swap
  congr 1
  · simp_rw [qr_sign]
  ...
  · rw [jacobi_sym_mod_left ↑(b % _), jacobi_sym_mod_left b, Int.coe_nat_mod, Int.mod_mod_of_dvd b]
...
theorem jacobi_sym_mod_right (a : ℤ) {b : ℕ} (hb : Odd b) : J(a | b) = J(a | b % (4 * a.natAbs)) := by
  cases' Int.nat_abs_eq a with ha ha <;> nth_rw 1[ha] <;> nth_rw 0[ha]
