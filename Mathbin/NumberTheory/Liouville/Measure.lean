  ...
  { x : ℝ | ∃ (p : _)(_ : p > 2), LiouvilleWith p x } ⊆
    ⋃m : ℤ,
      (fun x : ℝ => x+m) ⁻¹'
        ⋃(n : _)(_ : n > (0 : ℕ)),
          { x : ℝ |
            ∃ᶠb : ℕ in at_top, ∃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), |x - (a : ℤ) / b| < 1 / (b^(2+1 / n : ℝ)) } :=
  by 
    rintro x ⟨p, hp, hxp⟩
    ...
    suffices  :
      ∀ y : ℝ,
        LiouvilleWith p y →
          y ∈ Ico (0 : ℝ) 1 →
            ∃ᶠb : ℕ in at_top, ∃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), |y - a / b| < 1 / (b^(2+1 / n+1 : ℝ))
    ·
      simp only [mem_Union, mem_preimage]
      have hx : (x+«expr↑ » (-⌊x⌋)) ∈ Ico (0 : ℝ) 1
      ·
...
theorem volume_Union_set_of_liouville_with : volume (⋃(p : ℝ)(hp : 2 < p), { x : ℝ | LiouvilleWith p x }) = 0 :=
  by 
    ...
    have hB : ∀ a b, volume (B a b) = «expr↑ » (2 / (b^r) :  ℝ≥0 )
    ·
      intro a b 
