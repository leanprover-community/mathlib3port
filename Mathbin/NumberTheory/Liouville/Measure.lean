...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » finset.Icc (0 : exprℤ()) b)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (p «expr > » 2)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (n «expr > » (0 : exprℕ()))
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » finset.Icc (0 : exprℤ()) b)
theorem set_of_liouville_with_subset_aux :
  { x : ℝ | ∃ (p : _)(_ : p > 2), LiouvilleWith p x } ⊆
    ⋃ m : ℤ,
      (fun x : ℝ => x+m) ⁻¹'
        ⋃ (n : _)(_ : n > (0 : ℕ)),
          { x : ℝ |
            ∃ᶠ b : ℕ in at_top, ∃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), |x - (a : ℤ) / b| < 1 / (b^(2+1 / n : ℝ)) } :=
  by 
    rintro x ⟨p, hp, hxp⟩
    ...
    suffices  :
      ∀ y : ℝ,
        LiouvilleWith p y →
          y ∈ Ico (0 : ℝ) 1 →
            ∃ᶠ b : ℕ in at_top, ∃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), |y - a / b| < 1 / (b^(2+1 / n+1 : ℝ))
    ·
...
measure zero. -/
@[simp]
theorem volume_Union_set_of_liouville_with : volume (⋃ (p : ℝ)(hp : 2 < p), { x : ℝ | LiouvilleWith p x }) = 0 :=
  by 
    ...
    have  : ∀ b : ℕ, volume (⋃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), B a b) ≤ (2*(b^1 - r)+b^-r :  ℝ≥0 )
    ·
      intro b 
      calc volume (⋃ (a : _)(_ : a ∈ Finset.icc (0 : ℤ) b), B a b) ≤ ∑ a in Finset.icc (0 : ℤ) b, volume (B a b) :=
        measure_bUnion_finset_le _ _ _ = ((b+1)*2 / (b^r) :  ℝ≥0 ) :=
        by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (p «expr > » (2 : exprℝ()))
theorem ae_not_liouville_with : ∀ᵐ x, ∀ p _ : p > (2 : ℝ), ¬LiouvilleWith p x :=
  by 
