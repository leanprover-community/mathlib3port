...
@[simp]
theorem bit1_re z : (bit1 z : ℤ√d).re = bit1 z.re := by
  simp [bit1]
...
@[simp]
theorem coe_nat_re (n : ℕ) : (n : ℤ√d).re = n := by
  induction n <;> simp

@[simp]
theorem coe_nat_im (n : ℕ) : (n : ℤ√d).im = 0 := by
  induction n <;> simp

theorem coe_nat_val (n : ℕ) : (n : ℤ√d) = ⟨n, 0⟩ := by
  simp [ext]
...
theorem coe_int_dvd_iff {d : ℤ} (z : ℤ) (a : ℤ√d) : ↑z ∣ a ↔ z ∣ a.re ∧ z ∣ a.im := by
  constructor
  ·
    rintro ⟨x, rfl⟩
...
theorem nonneg_add {a b : ℤ√d} (ha : nonneg a) (hb : nonneg b) : nonneg (a+b) := by
...
@[ext]
theorem hom_ext {d : ℤ} (f g : ℤ√d →+* R) (h : f sqrtd = g sqrtd) : f = g := by
  ext ⟨x_re, x_im⟩
  simp [decompose, h]
  ...
  { toFun := fun r =>
      { toFun := fun a => a.1+a.2*(r : R),
        ...
        map_one' := by
          simp ,
        map_mul' := fun a b => by
          have : ((a.re+a.im*r : R)*b.re+b.im*r) = ((a.re*b.re)+((a.re*b.im)+a.im*b.re)*r)+(a.im*b.im)*r*r := by
            ring
          simp [this, r.prop]
          ring },
    invFun := fun f =>
