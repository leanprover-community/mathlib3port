...
theorem norm_pos {x : ℤ[i]} : 0 < norm x ↔ x ≠ 0 := by
  rw [lt_iff_le_and_neₓ, Ne.def, eq_comm, norm_eq_zero] <;> simp [norm_nonneg]
...
theorem div_def (x y : ℤ[i]) : x / y = ⟨round ((x * conj y).re / norm y : ℚ), round ((x * conj y).im / norm y : ℚ)⟩ :=
  show Zsqrtd.mk _ _ = _ by simp [Ratₓ.of_int_eq_mk, Ratₓ.mk_eq_div, div_eq_mul_inv]
...
theorem nat_abs_norm_mod_lt (x : ℤ[i]) {y : ℤ[i]} (hy : y ≠ 0) : (x % y).norm.natAbs < y.norm.natAbs :=
  Int.coe_nat_ltₓ.1 (by simp [-Int.coe_nat_ltₓ, norm_mod_lt x hy])
  ...
  rw [Zsqrtd.norm_mul, Int.nat_abs_mul] <;>
    exact
      le_mul_of_one_le_right (Nat.zero_leₓ _)
        (Int.coe_nat_leₓ.1 (by rw [coe_nat_abs_norm] <;> exact Int.add_one_le_of_ltₓ (norm_pos.2 hy)))
  ...
  hp.1.eq_two_or_odd.elim
    ...
    by_contradiction fun hp3 : p % 4 ≠ 3 => by
      have hp41 : p % 4 = 1 := by
        ...
        have := Nat.mod_ltₓ p (show 0 < 4 by decide)
        revert this hp3 hp1
        generalize p % 4 = m
        decide!
      ...
      have hkltp : 1 + k * k < p * p :=
        calc
          1 + k * k ≤ k + k * k :=
            add_le_add_right (Nat.pos_of_ne_zeroₓ fun hk0 => by clear_aux_decl <;> simp_all [pow_succ'ₓ]) _
          _ = k * (k + 1) := by simp [add_commₓ, mul_addₓ]
      ...
      have hpk₁ : ¬(p : ℤ[i]) ∣ ⟨k, -1⟩ := fun ⟨x, hx⟩ =>
        lt_irreflₓ (p * x : ℤ[i]).norm.natAbs <|
          calc
            ...
            _ < (norm (p : ℤ[i])).natAbs := by simpa [add_commₓ, Zsqrtd.norm] using hkltp
      ...
      obtain ⟨y, hy⟩ := hpk
      have := hpi.2.2 ⟨k, 1⟩ ⟨k, -1⟩ ⟨y, by rw [← hkmul, ← Nat.cast_mulₓ p, ← hy] <;> simp⟩
      clear_aux_decl
      tauto
