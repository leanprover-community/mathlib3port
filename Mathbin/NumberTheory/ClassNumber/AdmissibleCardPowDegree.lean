...
theorem exists_approx_polynomial_aux [Ring Fq] {d : ℕ} {m : ℕ} (hm : Fintype.card Fq ^ d ≤ m)
  ...
  rw [not_le, degree_eq_natDegree hb, WithBot.coe_lt_coe] at hbj
  have hj : natDegree b - j.succ < d :=
    by
  ...
  have : j = b.nat_degree - (natDegree b - j.succ).succ := by
...
theorem exists_approx_polynomial {b : Fq[X]} (hb : b ≠ 0) {ε : ℝ} (hε : 0 < ε)
  ...
  suffices (natDegree (A i₁ % b - A i₀ % b) : ℝ) < b.nat_degree + log ε / log (Fintype.card Fq) by
...
theorem exists_partition_polynomial_aux (n : ℕ) {ε : ℝ} (hε : 0 < ε) {b : Fq[X]} (hb : b ≠ 0)
  ...
  obtain ⟨j, hj⟩ :
    ∃ j,
      ∀ i : Fin n, t' i = j → (cardPowDegree (A 0 % b - A i.succ % b) : ℝ) < cardPowDegree b • ε :=
    by
    by_contra this
    push_neg  at this
