...
theorem exists_partition_int (n : ℕ) {ε : ℝ} (hε : 0 < ε) {b : ℤ} (hb : b ≠ 0) (A : Finₓ n → ℤ) :
  ∃ t : Finₓ n → Finₓ ⌈1 / ε⌉₊, ∀ i₀ i₁, t i₀ = t i₁ → «expr↑ » (abs (A i₁ % b - A i₀ % b)) < abs b • ε :=
  by 
    have hb' : (0 : ℝ) < «expr↑ » (abs b) := int.cast_pos.mpr (abs_pos.mpr hb)
    have hbε : 0 < abs b • ε
    ·
      rw [Algebra.smul_def]
      exact mul_pos hb' hε 
    ...
    intro i₀ i₁ hi 
    have hi : (⌊«expr↑ » (A i₀ % b) / abs b • ε⌋.natAbs : ℤ) = ⌊«expr↑ » (A i₁ % b) / abs b • ε⌋.natAbs :=
