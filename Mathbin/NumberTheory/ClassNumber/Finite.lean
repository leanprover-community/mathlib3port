...
-- error in NumberTheory.ClassNumber.Finite: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contra: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem norm_bound_pos : «expr < »(0, norm_bound abv bS) :=
begin
  obtain ["⟨", ident i, ",", ident j, ",", ident k, ",", ident hijk, "⟩", ":", expr «expr∃ , »((i
     j
     k), «expr ≠ »(algebra.left_mul_matrix bS (bS i) j k, 0))],
  { by_contra [ident h],
    push_neg ["at", ident h],
    obtain ["⟨", ident i, "⟩", ":=", expr bS.index_nonempty],
    apply [expr bS.ne_zero i],
    apply [expr (algebra.left_mul_matrix bS).injective_iff.mp (algebra.left_mul_matrix_injective bS)],
    ext [] [ident j, ident k] [],
    simp [] [] [] ["[", expr h, ",", expr dmatrix.zero_apply, "]"] [] [] },
  simp [] [] ["only"] ["[", expr norm_bound, ",", expr algebra.smul_def, ",", expr ring_hom.eq_nat_cast, ",", expr int.nat_cast_eq_coe_nat, "]"] [] [],
  refine [expr mul_pos (int.coe_nat_pos.mpr (nat.factorial_pos _)) _],
  refine [expr pow_pos (mul_pos (int.coe_nat_pos.mpr (fintype.card_pos_iff.mpr ⟨i⟩)) _) _],
  refine [expr lt_of_lt_of_le (abv.pos hijk) (finset.le_max' _ _ _)],
  exact [expr finset.mem_image.mpr ⟨⟨i, j, k⟩, finset.mem_univ _, rfl⟩]
end
...
theorem exists_mem_finset_approx (a : S) {b} (hb : b ≠ (0 : R)) :
  ∃ (q : S)(r : _)(_ : r ∈ finset_approx bS adm),
  ...
  by 
    ...
    ·
      apply le_of_eqₓ 
      congr 
      simpRw [LinearEquiv.map_sum, LinearEquiv.map_sub, LinearEquiv.map_smul, Finset.sum_apply', Finsupp.sub_apply,
...
theorem exists_mem_finset_approx' (h : Algebra.IsAlgebraic R L) (a : S) {b : S} (hb : b ≠ 0) :
  ∃ (q : S)(r : _)(_ : r ∈ finset_approx bS adm), abv (Algebra.norm R (r • a - q*b)) < abv (Algebra.norm R b) :=
  by 
...
theorem exists_mk0_eq_mk0 [IsDedekindDomain S] [IsFractionRing S L] (h : Algebra.IsAlgebraic R L) (I : (Ideal S)⁰) :
  ∃ J : (Ideal S)⁰,
...
noncomputable def fintype_of_admissible_of_finite [IsDedekindDomain R] :
  Fintype (@ClassGroup S L _ _ _ (IsIntegralClosure.is_fraction_ring_of_finite_extension R K L S)) :=
  by 
    letI this := Classical.decEq L 
    letI this := IsIntegralClosure.is_fraction_ring_of_finite_extension R K L S 
    letI this := IsIntegralClosure.is_dedekind_domain R K L S 
    ...
    refine'
      fintype_of_admissible_of_algebraic L bS adm
        fun x => (IsFractionRing.is_algebraic_iff R K).mpr (Algebra.is_algebraic_of_finite x)
    ·
