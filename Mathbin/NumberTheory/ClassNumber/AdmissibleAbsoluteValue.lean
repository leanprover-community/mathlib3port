...
theorem exists_partition {ι : Type _} [Fintypeₓ ι] {ε : ℝ} (hε : 0 < ε) {b : R} (hb : b ≠ 0) (A : ι → R)
    (h : abv.IsAdmissible) :
    ∃ t : ι → Finₓ (h.card ε), ∀ i₀ i₁, t i₀ = t i₁ → (abv (A i₁ % b - A i₀ % b) : ℝ) < abv b • ε := by
  let e := Fintypeₓ.equivFin ι
  obtain ⟨t, ht⟩ := h.exists_partition' (Fintypeₓ.card ι) hε hb (A ∘ e.symm)
...
theorem exists_approx_aux (n : ℕ) (h : abv.IsAdmissible) :
  ...
  obtain ⟨s, s_inj, hs⟩ :
    ...
    obtain ⟨s, hs⟩ :=
      @Fintypeₓ.exists_lt_card_fiber_of_mul_lt_card _ _ _ _ _ t (M ^ n)
        (by simpa only [Fintypeₓ.card_fin, pow_succₓ] using Nat.lt_succ_selfₓ (M ^ n.succ))
    refine' ⟨fun i => (finset.univ.filter fun x => t x = s).toList.nthLe i _, _, fun i₀ i₁ => ht _ _ _⟩
    · refine' i.2.trans_le _
      rwa [Finsetₓ.length_to_list]
      
    · intro i j h
      ext
      exact list.nodup_iff_nth_le_inj.mp (Finsetₓ.nodup_to_list _) _ _ _ _ h
...
whose remainders are close together, pointwise. -/
theorem exists_approx {ι : Type _} [Fintypeₓ ι] {ε : ℝ} (hε : 0 < ε) {b : R} (hb : b ≠ 0) (h : abv.IsAdmissible)
    (A : Finₓ (h.card ε ^ Fintypeₓ.card ι).succ → ι → R) :
    ∃ i₀ i₁, i₀ ≠ i₁ ∧ ∀ k, (abv (A i₁ k % b - A i₀ k % b) : ℝ) < abv b • ε := by
  let e := Fintypeₓ.equivFin ι
  obtain ⟨i₀, i₁, ne, h⟩ := h.exists_approx_aux (Fintypeₓ.card ι) hε hb fun x y => A x (e.symm y)
