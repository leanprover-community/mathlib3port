...
theorem discr_zeta_eq_discr_zeta_sub_one (hζ : IsPrimitiveRoot ζ n) :
  ...
  have H₂ : (aeval (hζ.sub_one_power_basis ℚ).gen) (x + 1 : ℤ[X]) = (hζ.power_basis ℚ).gen := by
    simp
  refine'
    discr_eq_discr_of_toMatrix_coeff_isIntegral _ (fun i j => toMatrix_isIntegral H₁ _ _ _ _)
...
theorem discr_prime_pow_ne_two [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact (p : ℕ).Prime]
  ...
  rw [discr_powerBasis_eq_norm, finrank L hirr, hζ.power_basis_gen _, ←
  ...
  · have H := congr_arg derivative (cyclotomic_prime_pow_mul_x_pow_sub_one K p k)
    ...
    simp only [aeval_add, aeval_mul, minpoly.aeval, zero_mul, add_zero, aeval_nat_cast, map_sub,
...
theorem discr_prime_pow [hcycl : IsCyclotomicExtension {p ^ k} K L] [hp : Fact (p : ℕ).Prime]
  ...
  by
  cases k
  · simp only [coe_basis, pow_zero, powerBasis_gen, totient_one, mul_zero, mul_one,
  ...
  · by_cases hk : p ^ (k + 1) = 2
    · have hp : p = 2 :=
      ...
      simp only [hp, hk, show 1 / 2 = 0 by rfl, coe_basis, pow_one, powerBasis_gen, PNat.coe_bit0,
        PNat.one_coe, totient_two, pow_zero, mul_one, mul_zero]
      rw [powerBasis_dim, hζ.eq_neg_one_of_two_right, show (-1 : L) = algebraMap K L (-1) by simp,
        minpoly.eq_x_sub_c_of_algebraMap_inj _ (algebraMap K L).Injective, natDegree_x_sub_c]
      simp only [discr, traceMatrix, Matrix.det_unique, Fin.default_eq_zero, Fin.val_zero, pow_zero,
