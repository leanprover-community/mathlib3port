...
theorem norm_eq_one [IsDomain L] [IsCyclotomicExtension {n} K L] (hn : n ≠ 2)
  ...
  · replace h1 : 2 ≤ n
    ...
    rw [← hζ.power_basis_gen K, PowerBasis.norm_gen_eq_coeff_zero_minpoly, hζ.power_basis_gen K, ←
      hζ.minpoly_eq_cyclotomic_of_irreducible hirr, cyclotomic_coeff_zero _ h1, mul_one,
      hζ.power_basis_dim K, ← hζ.minpoly_eq_cyclotomic_of_irreducible hirr, natDegree_cyclotomic]
...
theorem sub_one_norm_eq_eval_cyclotomic [IsCyclotomicExtension {n} K L] (h : 2 < (n : ℕ))
  ...
  have : (finset.univ.prod fun σ : L →ₐ[K] E => 1 - σ ζ) = eval 1 (cyclotomic' n E) :=
    ...
    simp
  haveI : ne_zero ((n : ℕ) : E) := NeZero.of_noZeroSMulDivisors K _ (n : ℕ)
  rw [this, cyclotomic', ← cyclotomic_eq_prod_x_sub_primitiveRoots (isRoot_cyclotomic_iff.1 hz), ←
    map_cyclotomic_int, map_intCast, ← Int.cast_one, eval_int_cast_map, eq_intCast, Int.cast_id]
...
theorem pow_sub_one_norm_two {k : ℕ} (hζ : IsPrimitiveRoot ζ (2 ^ (k + 1)))
  ...
  have H : (-1 : L) - (1 : L) = algebraMap K L (-2) :=
    by
    simp only [map_neg, map_bit0, map_one]
    ring
