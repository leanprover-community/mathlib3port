    ...
    :=
      by
        let S : Set ℕ := { n | ∃ s : ℤ × ℤ × ℤ , Fermat42 s . 1 s . 2 . 1 s . 2 . 2 ∧ n = Int.natAbs s . 2 . 2 }
          ...
          · use Int.natAbs c rw [ Set.mem_set_of_eq ] use ⟨ a , ⟨ b , c ⟩ ⟩ tauto
