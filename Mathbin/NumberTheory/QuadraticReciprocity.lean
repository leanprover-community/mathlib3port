...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » Ico 1 «expr / »(p, 2).succ)
        ...
        have he : ∀ { x } , x ∈ Ico 1 p / 2 . succ → x ≠ 0 ∧ x ≤ p / 2
          ·
            simp
              ...
              [ Nat.lt_succ_iff , Nat.succ_le_iff , pos_iff_ne_zero ]
          ...
          have
            ...
            :
              ∀
                ...
                ,
                ∃ ( x : _ ) ( _ : x ∈ Ico 1 p / 2 . succ ) , b = ( a * x : Zmod p ) . valMinAbs . natAbs
          ·
            intro b hb
      ...
      calc
        ( a ^ p / 2 * p / 2 ! : Zmod p ) = ∏ x in Ico 1 p / 2 . succ , a * x
          ...
          _
            ...
            :=
            prod_congr rfl $ fun _ _ => by simp only [ nat_cast_nat_abs_val_min_abs ] splitIfs <;> simp
          _
              =
            ...
            have
              :
                ...
                prod_bij_ne_one
                  fun x _ _ => x
                    fun x => by splitIfs <;> simp_all ( config := { contextual := Bool.true._@._internal._hyg.0 } )
                    fun _ _ _ _ _ _ => id
                    fun b h _ => ⟨ b , by simp_all [ - not_leₓ ] ⟩
                    by intros <;> splitIfs at * <;> simp_all
...
private theorem eisenstein_lemma_aux₁ (p : ℕ) [Fact p.prime] [hp2 : Fact (p % 2 = 1)] {a : ℕ} (hap : (a : Zmod p) ≠ 0) :
  ((∑ x in Ico 1 (p / 2).succ, a*x : ℕ) : Zmod 2) =
    (((Ico 1 (p / 2).succ).filter
            fun x : ℕ =>
              p / 2 <
                (a*x : Zmod p).val).card+∑ x in Ico 1 (p / 2).succ, x)+(∑ x in Ico 1 (p / 2).succ, (a*x) / p : ℕ) :=
