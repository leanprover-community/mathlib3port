...
theorem legendre_sym_eq_one_or_neg_one (a p : ℕ) (ha : (a : Zmod p) ≠ 0) : legendreSym a p = -1 ∨ legendreSym a p = 1 :=
  by
  unfold legendre_sym
  split_ifs <;> simp_all only [eq_self_iff_true, or_trueₓ, true_orₓ, Int.cast_coe_nat]

theorem legendre_sym_eq_zero_iff (a p : ℕ) : legendreSym a p = 0 ↔ (a : Zmod p) = 0 := by
  constructor
  · classical
    contrapose
    intro ha
  ...
  · intro ha
    rw [legendre_sym, Int.cast_coe_nat, if_pos ha]
    

/-- Gauss' lemma. The legendre symbol can be computed by considering the number of naturals less
  than `p/2` such that `(a * x) % p > p / 2` -/
theorem gauss_lemma {a : ℕ} [Fact (p % 2 = 1)] (ha0 : (a : Zmod p) ≠ 0) :
    legendreSym a p = -1 ^ ((ico 1 (p / 2).succ).filter fun x : ℕ => p / 2 < (a * x : Zmod p).val).card := by
  have :
    (legendreSym a p : Zmod p) =
...
theorem legendre_sym_eq_one_iff {a : ℕ} (ha0 : (a : Zmod p) ≠ 0) : legendreSym a p = 1 ↔ ∃ b : Zmod p, b ^ 2 = a := by
...
theorem eisenstein_lemma [Fact (p % 2 = 1)] {a : ℕ} (ha1 : a % 2 = 1) (ha0 : (a : Zmod p) ≠ 0) :
    legendreSym a p = -1 ^ ∑ x in ico 1 (p / 2).succ, x * a / p := by
...
theorem legendre_sym_two [hp1 : Fact (p % 2 = 1)] : legendreSym 2 p = -1 ^ (p / 4 + p / 2) := by
  have hp2 : p ≠ 2 :=
    mt (congr_argₓ (· % 2))
      (by
        simpa using hp1.1)
  ...
  erw [gauss_lemma p (prime_ne_zero p 2 hp2), neg_one_pow_eq_pow_mod_two,
...
theorem exists_sq_eq_prime_iff_of_mod_four_eq_one (hp1 : p % 4 = 1) [hq1 : Fact (q % 2 = 1)] :
  ...
  if hpq : p = q then by
    ...
    rw [neg_one_pow_eq_pow_mod_two, h1, legendre_sym, legendre_sym, Int.cast_coe_nat, Int.cast_coe_nat, if_neg hqp0,
      if_neg hpq0] at this
...
theorem exists_sq_eq_prime_iff_of_mod_four_eq_three (hp3 : p % 4 = 3) (hq3 : q % 4 = 3) (hpq : p ≠ q) :
  ...
  have hqp0 : (q : Zmod p) ≠ 0 := prime_ne_zero p q hpq
  have := quadratic_reciprocity p q hpq
  rw [neg_one_pow_eq_pow_mod_two, h1, legendre_sym, legendre_sym, Int.cast_coe_nat, Int.cast_coe_nat, if_neg hpq0,
    if_neg hqp0] at this
  rw [euler_criterion q hpq0, euler_criterion p hqp0]
  split_ifs  at this <;> simp [*] <;> contradiction
