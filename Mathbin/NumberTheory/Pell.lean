...
@[simp]
theorem d_pos : 0 < d :=
  tsub_pos_of_lt (mul_lt_mul a1 (le_of_ltₓ a1) (by decide) (by decide) : 1 * 1 < a * a)
...
theorem asq_pos : 0 < a * a :=
  le_transₓ (le_of_ltₓ a1) (by have := @Nat.mul_le_mul_leftₓ 1 a a (le_of_ltₓ a1) <;> rwa [mul_oneₓ] at this)

theorem dz_val : ↑d = az * az - 1 :=
  have : 1 ≤ a * a := asq_pos
  show ↑(a * a - 1) = _ by rw [Int.coe_nat_subₓ this] <;> rfl
...
theorem is_pell_nat {x y : ℕ} : is_pell ⟨x, y⟩ ↔ x * x - d * y * y = 1 :=
  ⟨fun h => Int.coe_nat_inj (by rw [Int.coe_nat_subₓ (Int.le_of_coe_nat_le_coe_nat <| Int.Le.intro_sub h)] <;> exact h),
    fun h =>
...
theorem is_pell_mul {b c : ℤ√d} (hb : is_pell b) (hc : is_pell c) : is_pell (b * c) :=
  is_pell_norm.2 (by simp [mul_comm, mul_left_commₓ, Zsqrtd.conj_mul, Pell.is_pell_norm.1 hb, Pell.is_pell_norm.1 hc])
...
theorem pell_eq (n : ℕ) : xn n * xn n - d * yn n * yn n = 1 :=
  let pn := pell_eqz n
  have h : (↑(xn n * xn n) : ℤ) - ↑(d * yn n * yn n) = 1 := by repeat' rw [Int.coe_nat_mul] <;> exact pn
  have hl : d * yn n * yn n ≤ xn n * xn n :=
    Int.le_of_coe_nat_le_coe_nat <| Int.Le.intro <| add_eq_of_eq_sub' <| Eq.symm h
  Int.coe_nat_inj (by rw [Int.coe_nat_subₓ hl] <;> exact h)

instance dnsq : Zsqrtd.Nonsquare d :=
  ⟨fun n h =>
    have : n * n + 1 = a * a := by rw [← h] <;> exact Nat.succ_pred_eq_of_posₓ (asq_pos a1)
    have na : n < a := Nat.mul_self_lt_mul_self_iff.2 (by rw [← this] <;> exact Nat.lt_succ_selfₓ _)
    have : (n + 1) * (n + 1) ≤ n * n + 1 := by rw [this] <;> exact Nat.mul_self_le_mul_self na
    have : n + n ≤ 0 := @Nat.le_of_add_le_add_rightₓ (n * n + 1) _ _ (by ring_nf  at this⊢ <;> assumption)
    ne_of_gtₓ d_pos <| by rwa [Nat.eq_zero_of_le_zeroₓ ((Nat.le_add_leftₓ _ _).trans this)] at h⟩

theorem xn_ge_a_pow : ∀ n : ℕ, a ^ n ≤ xn n
  | 0 => le_reflₓ 1
  | n + 1 => by simp [pow_succ'ₓ] <;> exact le_transₓ (Nat.mul_le_mul_rightₓ _ (xn_ge_a_pow n)) (Nat.le_add_rightₓ _ _)
...
theorem eq_pell_lem : ∀ (n) (b : ℤ√d), 1 ≤ b → is_pell b → b ≤ pell_zd n → ∃ n, b = pell_zd n
  ...
  | n + 1, b => fun h1 hp h =>
    ...
    have am1p : (0 : ℤ√d) ≤ ⟨a, -1⟩ := show (_ : Nat) ≤ _ by simp <;> exact Nat.pred_leₓ _
    ...
    if ha : (⟨↑a, 1⟩ : ℤ√d) ≤ b then
      let ⟨m, e⟩ :=
        eq_pell_lem n (b * ⟨a, -1⟩) (by rw [← a1m] <;> exact mul_le_mul_of_nonneg_right ha am1p)
          ...
          (by have t := mul_le_mul_of_nonneg_right h am1p <;> rwa [pell_zd_succ, mul_assoc, a1m, mul_oneₓ] at t)
      ⟨m + 1, by
...
theorem pell_zd_add (m) : ∀ n, pell_zd (m + n) = pell_zd m * pell_zd n
  | 0 => (mul_oneₓ _).symm
  | n + 1 => by rw [← add_assocₓ, pell_zd_succ, pell_zd_succ, pell_zd_add n, ← mul_assoc]
...
theorem strict_mono_y : StrictMonoₓ yn
...
theorem strict_mono_x : StrictMonoₓ xn
  | m, 0, h => absurd h <| Nat.not_lt_zeroₓ _
  | m, n + 1, h => by
    have : xn m ≤ xn n :=
              ...
              mul_dvd_mul_right
                (dvd_mul_of_dvd_right
                  (modeq_zero_iff_dvd.1 <|
                    (hy.modeq_of_dvd <| by simp [pow_succ'ₓ]).trans <|
...
theorem ysq_dvd_yy (n) : yn n * yn n ∣ yn (n * yn n) :=
  modeq_zero_iff_dvd.1 <|
    ((xy_modeq_yn n (yn n)).right.modeq_of_dvd <| by simp [pow_succₓ]).trans
      ...
      Nat.dvd_of_mul_dvd_mul_rightₓ (strict_mono_y n0l) <|
        modeq_zero_iff_dvd.1 <| by
          have xm := (xy_modeq_yn a1 n k).right <;>
            rw [← ke] at xm <;> exact (xm.modeq_of_dvd <| by simp [pow_succₓ]).symm.trans h.modeq_zero_nat
...
theorem x_sub_y_dvd_pow (y : ℕ) : ∀ n, (2 * a * y - y * y - 1 : ℤ) ∣ yz n * (a - y) + ↑(y ^ n) - xz n
  ...
  | n + 2 => by
    have : (2 * a * y - y * y - 1 : ℤ) ∣ ↑(y ^ (n + 2)) - ↑(2 * a) * ↑(y ^ (n + 1)) + ↑(y ^ n) :=
      ⟨-↑(y ^ n), by
        simp [pow_succₓ, mul_addₓ, Int.coe_nat_mul, show ((2 : ℕ) : ℤ) = 2 from rfl, mul_comm, mul_left_commₓ]
        ring⟩
...
theorem xn_modeq_x2n_add_lem (n j) : xn n ∣ d * yn n * (yn n * xn j) + xn j := by
  have h1 : d * yn n * (yn n * xn j) + xn j = (d * yn n * yn n + 1) * xn j := by simp [add_mulₓ, mul_assoc]
  have h2 : d * yn n * yn n + 1 = xn n * xn n := by
    apply Int.coe_nat_inj <;>
      repeat' first |rw [Int.coe_nat_add]|rw [Int.coe_nat_mul] <;> exact add_eq_of_eq_sub' (Eq.symm <| pell_eqz _ _)
      ...
      exact
        dvd_sub
          (by
            delta xz <;>
              delta yz <;>
                repeat' first |rw [← Int.coe_nat_add]|rw [← Int.coe_nat_mul] <;>
...
theorem xn_modeq_x2n_sub {n j} (h : j ≤ 2 * n) : xn (2 * n - j) + xn j ≡ 0 [MOD xn n] :=
  (le_totalₓ j n).elim xn_modeq_x2n_sub_lem fun jn => by
    have : 2 * n - j + j ≤ n + j := by rw [tsub_add_cancel_of_le h, two_mul] <;> exact Nat.add_le_add_leftₓ jn _
...
theorem xn_modeq_x4n_add (n j) : xn (4 * n + j) ≡ xn j [MOD xn n] :=
  Modeq.add_right_cancel' (xn (2 * n + j)) <| by
    refine' @modeq.trans _ _ 0 _ _ (by rw [add_commₓ] <;> exact (xn_modeq_x2n_add _ _ _).symm) <;>
...
theorem xn_modeq_x4n_sub {n j} (h : j ≤ 2 * n) : xn (4 * n - j) ≡ xn j [MOD xn n] :=
  have h' : j ≤ 2 * n := le_transₓ h (by rw [Nat.succ_mul] <;> apply Nat.le_add_leftₓ)
  Modeq.add_right_cancel' (xn (2 * n - j)) <| by
    refine' @modeq.trans _ _ 0 _ _ (by rw [add_commₓ] <;> exact (xn_modeq_x2n_sub _ h).symm) <;>
  ...
  | j + 1, ij, jn => by
    suffices xn j % xn n < xn (j + 1) % xn n from
      (lt_or_eq_of_leₓ (Nat.le_of_succ_le_succₓ ij)).elim
        (fun h => lt_transₓ (eq_of_xn_modeq_lem1 h (le_of_ltₓ jn)) this) fun h => by rw [h] <;> exact this
...
theorem eq_of_xn_modeq_lem3 {i n} (npos : 0 < n) :
  ...
  | j + 1, ij, j2n, jnn, ntriv =>
    have lem2 : ∀ k > n, k ≤ 2 * n → (↑(xn k % xn n) : ℤ) = xn n - xn (2 * n - k) := fun k kn k2n => by
      ...
      suffices xn k % xn n = xn n - xn (2 * n - k) by rw [this, Int.coe_nat_subₓ xle]
    ...
    (lt_or_eq_of_leₓ ij).resolve_left fun ij' =>
      if jn : j = n then by
        ...
        have x0 : 0 < xn a1 0 % xn a1 n := by
          rw [Nat.mod_eq_of_ltₓ (strict_mono_x a1 (Nat.pos_of_ne_zeroₓ npos))] <;> exact by decide
        cases' i with i
        exact x0
        rw [jn] at ij'
        exact
...
theorem eq_of_xn_modeq' {i j n} (ipos : 0 < i) (hin : i ≤ n) (j4n : j ≤ 4 * n) (h : xn j ≡ xn i [MOD xn n]) :
    j = i ∨ j + i = 4 * n :=
  have i2n : i ≤ 2 * n := by apply le_transₓ hin <;> rw [two_mul] <;> apply Nat.le_add_leftₓ
  (le_or_gtₓ j (2 * n)).imp
    (fun j2n : j ≤ 2 * n =>
      (eq_of_xn_modeq j2n i2n h) fun a2 n1 =>
        ⟨fun j0 i2 => by rw [n1, i2] at hin <;> exact absurd hin (by decide), fun j2 i0 => ne_of_gtₓ ipos i0⟩)
    fun j2n : 2 * n < j =>
...
theorem modeq_of_xn_modeq {i j n} (ipos : 0 < i) (hin : i ≤ n) (h : xn j ≡ xn i [MOD xn n]) :
  ...
  have jj : j ≡ j' [MOD 4 * n] := by delta modeq <;> rw [Nat.mod_eq_of_ltₓ jl]
...
theorem matiyasevic {a k x y} :
  ...
  ⟨fun ⟨a1, hx, hy⟩ => by
    rw [← hx, ← hy] <;>
      refine'
          ⟨a1,
            (Nat.eq_zero_or_posₓ k).elim (fun k0 => by rw [k0] <;> exact ⟨le_rflₓ, Or.inl ⟨rfl, rfl⟩⟩) fun kpos =>
              _⟩ <;>
        exact
          let x := xn a1 k
          let y := yn a1 k
          ...
          have tk : t ≡ k [MOD 4 * y] :=
            have : 4 * y ∣ b - 1 := Int.coe_nat_dvd.1 <| by rw [Int.coe_nat_subₓ (le_of_ltₓ b1)] <;> exact bm1.symm.dvd
            (yn_modeq_a_sub_one _ _).modeq_of_dvd this
          ⟨ky, Or.inr ⟨u, v, s, t, b, pell_eq _ _, pell_eq _ _, pell_eq _ _, b1, bm1, ba, vp, yv, sx, tk⟩⟩,
    fun ⟨a1, ky, o⟩ =>
    ⟨a1,
      match o with
      | Or.inl ⟨x1, y0⟩ => by rw [y0] at ky <;> rw [Nat.eq_zero_of_le_zeroₓ ky, x1, y0] <;> exact ⟨rfl, rfl⟩
      | Or.inr ⟨u, v, s, t, b, xy, uv, st, b1, rem⟩ =>
        ...
        | _, _, ⟨i, rfl, rfl⟩, _, _, ⟨n, rfl, rfl⟩, _, _, ⟨j, rfl, rfl⟩,
          ...
          (ky : k ≤ yn a1 i) =>
          ((Nat.eq_zero_or_posₓ i).elim fun i0 => by simp [i0] at ky <;> rw [i0, ky] <;> exact ⟨rfl, rfl⟩) fun ipos =>
            by
            ...
            clear _x o rem xy uv st _match _match _fun_match <;>
              exact by
                ...
                have jk : j ≡ k [MOD 4 * yn a1 i] :=
                  have : 4 * yn a1 i ∣ b - 1 :=
                    Int.coe_nat_dvd.1 <| by rw [Int.coe_nat_subₓ (le_of_ltₓ b1)] <;> exact bm1.symm.dvd
                ...
                have ki : k + i < 4 * yn a1 i :=
                  lt_of_le_of_ltₓ (add_le_add ky (yn_ge_n a1 i)) <| by
                    rw [← two_mul] <;> exact Nat.mul_lt_mul_of_pos_rightₓ (by decide) (strict_mono_y a1 ipos)
                have ji : j ≡ i [MOD 4 * n] :=
  ...
  have : y < a → a + (y * y + 1) ≤ 2 * a * y := by
    ...
    · rw [Nat.mul_succ, Nat.mul_succ, Nat.succ_mul y]
      have : y + Nat.succ y ≤ 2 * a := by
        change y + y < 2 * a
        rw [← two_mul]
        exact mul_lt_mul_of_pos_left (Nat.lt_of_succ_ltₓ ya) (by decide)
...
theorem eq_pow_of_pell {m n k} :
  ...
  ⟨fun e => by
    rw [← e] <;>
      refine'
          (Nat.eq_zero_or_posₓ k).elim (fun k0 => by rw [k0] <;> exact Or.inl ⟨rfl, rfl⟩) fun kpos =>
            Or.inr ⟨kpos, _⟩ <;>
        refine'
            (Nat.eq_zero_or_posₓ n).elim (fun n0 => by rw [n0, zero_pow kpos] <;> exact Or.inl ⟨rfl, rfl⟩) fun npos =>
              Or.inr ⟨npos, _⟩ <;>
          exact
            let w := max n k
            ...
            have mt : n ^ k < t := Int.lt_of_coe_nat_lt_coe_nat <| by rw [← te] <;> exact nt
            have zp : a * a - ((w + 1) * (w + 1) - 1) * (w * z) * (w * z) = 1 := by rw [← ze] <;> exact pell_eq w1 w
        ...
        exact by
          have jpos : 0 < j :=
            (Nat.eq_zero_or_posₓ j).resolve_left fun j0 => by
              ...
              have n1 : n = 1 :=
                have : n * n < n * 2 := by rw [mul_comm n 2, this] <;> apply Nat.le_add_leftₓ
          ...
          have na : n ≤ a := by rw [xj] <;> exact le_transₓ (le_transₓ nw wj) (le_of_ltₓ <| n_lt_xn _ _)
