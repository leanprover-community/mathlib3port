...
theorem is_pell_norm : ∀ {b : ℤ√d}, is_pell b ↔ (b*b.conj) = 1
  | ⟨x, y⟩ => by
    simp [Zsqrtd.ext, is_pell, mul_commₓ] <;> ring_nf
...
theorem xy_coprime n : (xn n).Coprime (yn n) :=
  Nat.coprime_of_dvd' $ fun k kp kx ky =>
    let p := pell_eq n
    by
...
theorem y_dvd_iff m n : yn m ∣ yn n ↔ m ∣ n :=
  ⟨fun h =>
    Nat.dvd_of_mod_eq_zeroₓ $
      (Nat.eq_zero_or_posₓ _).resolve_right $ fun hp =>
...
theorem xn_modeq_x2n_sub {n j} (h : j ≤ 2*n) : (xn ((2*n) - j)+xn j) ≡ 0 [MOD xn n] :=
  (le_totalₓ j n).elim xn_modeq_x2n_sub_lem fun jn =>
    have : (((2*n) - j)+j) ≤ n+j := by
  ...
  ⟨fun e => by
    ...
    Or.inr
      ...
      clear _match o _let_match <;>
        exact
          have jpos : 0 < j :=
            (Nat.eq_zero_or_posₓ j).resolve_left $ fun j0 =>
              have a1 : a = 1 := by
