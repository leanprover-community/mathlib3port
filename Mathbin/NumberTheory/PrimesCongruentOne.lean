...
theorem exists_prime_ge_modeq_one (k n : ℕ) (hpos : 0 < k) : ∃ p : ℕ, Nat.Prime p ∧ n ≤ p ∧ p ≡ 1 [MOD k] :=
  by 
    have hli : tendsto (abs ∘ fun a : ℕ => |(a : ℚ)|) at_top at_top
    ·
      simp only [· ∘ ·, abs_cast]
      exact nat.strict_mono_cast.monotone.tendsto_at_top_at_top exists_nat_ge 
    have hcff : Int.castRingHom ℚ (cyclotomic k ℤ).leadingCoeff ≠ 0
    ·
    ...
    have hgt : 1 < (eval (↑a*k*n.factorial) (cyclotomic k ℤ)).natAbs
    ·
    ...
    ·
      ...
      exactModCast min_fac_dvd (eval (↑b) (cyclotomic k ℤ)).natAbs 
    ...
    ·
      ...
      have  : k = orderOf (b : Zmod p) := ((is_root_cyclotomic_iff this).mp hroot).eq_order_of 
      rw [←order_of_units, Zmod.coe_unit_of_coprime, ←this] at hdiv 
      exact ((modeq_iff_dvd' hprime.1.Pos).2 hdiv).symm

theorem frequently_at_top_modeq_one (k : ℕ) (hpos : 0 < k) : ∃ᶠ p in at_top, Nat.Prime p ∧ p ≡ 1 [MOD k] :=
  by 
