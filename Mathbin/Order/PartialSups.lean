...
theorem le_partial_sups (f : ℕ → α) : f ≤ partialSups f :=
  fun n => le_partial_sups_of_le f le_rfl

theorem partial_sups_le (f : ℕ → α) (n : ℕ) (a : α) (w : ∀ m, m ≤ n → f m ≤ a) : partialSups f n ≤ a :=
  by 
    induction' n with n ih
    ·
      apply w 0 le_rfl
...
theorem partial_sups_mono : Monotone (partialSups : (ℕ → α) → ℕ →ₘ α) :=
  by 
    rintro f g h n 
    induction' n with n ih
    ·
...
theorem partial_sups_eq_sup'_range (f : ℕ → α) (n : ℕ) :
  ...
  by 
    ...
    ·
      ...
      simpRw [@Finset.range_succ n.succ]
