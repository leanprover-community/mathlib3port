...
@[simp]
theorem closure_le_closed_iff_le (x : α) {y : α} (hy : c.closed y) : c x ≤ y ↔ x ≤ y :=
  by 
...
theorem closure_sup_closure_right (x y : α) : c (x⊔c y) = c (x⊔y) :=
  by 
    rw [sup_comm, closure_sup_closure_left, sup_comm]

theorem closure_sup_closure (x y : α) : c (c x⊔c y) = c (x⊔y) :=
  by 
...
instance : CoeFun (LowerAdjoint u) fun _ => α → β :=
  { coe := to_fun }
...
theorem not_mem_of_not_mem_closure {s : Set β} {P : β} (hP : P ∉ l s) : P ∉ s :=
  fun h => hP (subset_closure _ s h)
...
@[simp]
theorem closure_operator_gi_self [PartialOrderₓ α] (c : ClosureOperator α) : c.gi.gc.closure_operator = c :=
  by 
    ext x 
    rfl
