...
@[simp]
theorem biimp_eq_iff (p q : Prop) : p ⇔ₒ q = (p ↔ q) :=
  by 
    simp [biimp, ←iff_def]
...
theorem inf_imp_eq (a b c : α) : a⊓(b ⇒ₒ c) = a ⇒ₒ b ⇒ₒ a⊓c :=
  by 
    unfold imp <;> simp [inf_sup_left]

@[simp]
theorem imp_eq_top_iff : a ⇒ₒ b = ⊤ ↔ a ≤ b :=
  by 
    rw [←compl_sdiff, compl_eq_top, sdiff_eq_bot_iff]

@[simp]
theorem imp_eq_bot_iff : a ⇒ₒ b = ⊥ ↔ a = ⊤ ∧ b = ⊥ :=
  by 
    simp [imp]
...
theorem imp_inf_le {α : Type _} [BooleanAlgebra α] (a b : α) : (a ⇒ₒ b)⊓a ≤ b :=
  by 
    unfold imp 
    rw [inf_sup_right]
    simp 

theorem inf_imp_eq_imp_imp (a b c : α) : a⊓b ⇒ₒ c = a ⇒ₒ (b ⇒ₒ c) :=
  by 
    simp [imp, sup_assoc]

theorem le_imp_iff : a ≤ b ⇒ₒ c ↔ a⊓b ≤ c :=
  by 
...
theorem biimp_comm (a b : α) : a ⇔ₒ b = b ⇔ₒ a :=
  by 
    unfold Lattice.biimp 
    rw [inf_comm]

@[simp]
theorem biimp_eq_top_iff : a ⇔ₒ b = ⊤ ↔ a = b :=
  by 
...
theorem biimp_symm : a ≤ b ⇔ₒ c ↔ a ≤ c ⇔ₒ b :=
  by 
    rw [biimp_comm]
