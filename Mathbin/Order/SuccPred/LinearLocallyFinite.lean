...
theorem is_glb_Ioc_of_is_glb_Ioi {i j k : ι} (hij_lt : i < j) (h : IsGlb (Set.ioi i) k) : IsGlb (Set.ioc i j) k := by
...
#align linear_locally_finite_order.is_succ_archimedean LinearLocallyFiniteOrder.is_succ_archimedean

instance (priority := 100) LinearOrder.pred_archimedean_of_succ_archimedean [SuccOrder ι] [PredOrder ι]
    [IsSuccArchimedean ι] :
    IsPredArchimedean ι where exists_pred_iterate_of_le i j hij := by
...
theorem to_Z_of_ge (hi : i0 ≤ i) : toZ i0 i = Nat.find (exists_succ_iterate_of_le hi) :=
  dif_pos hi
#align to_Z_of_ge to_Z_of_ge

theorem to_Z_of_lt (hi : i < i0) : toZ i0 i = -Nat.find (exists_pred_iterate_of_le hi.le) :=
  dif_neg (not_le.mpr hi)
#align to_Z_of_lt to_Z_of_lt

@[simp]
theorem to_Z_of_eq : toZ i0 i0 = 0 := by
...
theorem iterate_succ_to_Z (i : ι) (hi : i0 ≤ i) : (succ^[(toZ i0 i).toNat]) i0 = i := by
  rw [to_Z_of_ge hi, Int.to_nat_coe_nat]
  exact Nat.find_spec (exists_succ_iterate_of_le hi)
#align iterate_succ_to_Z iterate_succ_to_Z

theorem iterate_pred_to_Z (i : ι) (hi : i < i0) : (pred^[(-toZ i0 i).toNat]) i0 = i := by
...
theorem to_Z_nonneg (hi : i0 ≤ i) : 0 ≤ toZ i0 i := by
  rw [to_Z_of_ge hi]
  exact Nat.cast_nonneg _
#align to_Z_nonneg to_Z_nonneg

theorem to_Z_neg (hi : i < i0) : toZ i0 i < 0 := by
  refine' lt_of_le_of_ne _ _
  ...
  · by_contra
    ...
    rw [← h_eq, h] at hi
    simpa only [neg_zero, Int.to_nat_zero, Function.iterate_zero, id.def, lt_self_iff_false] using hi
    
#align to_Z_neg to_Z_neg

theorem to_Z_iterate_succ_le (n : ℕ) : toZ i0 ((succ^[n]) i0) ≤ n := by
...
theorem to_Z_iterate_pred_ge (n : ℕ) : -(n : ℤ) ≤ toZ i0 ((pred^[n]) i0) := by
...
theorem to_Z_iterate_pred_of_not_is_min (n : ℕ) (hn : ¬IsMin ((pred^[n]) i0)) : toZ i0 ((pred^[n]) i0) = -n := by
  cases n
...
theorem to_Z_le_iff (i j : ι) : toZ i0 i ≤ toZ i0 j ↔ i ≤ j :=
...
noncomputable def orderIsoRangeToZOfLinearSuccPredArch [hι : Nonempty ι] : ι ≃o Set.range (toZ hι.some) where
  toEquiv := Equiv.ofInjective _ injective_to_Z
...
noncomputable def orderIsoIntOfLinearSuccPredArch [NoMaxOrder ι] [NoMinOrder ι] [hι : Nonempty ι] : ι ≃o ℤ where
  toFun := toZ hι.some
  invFun n := if 0 ≤ n then (succ^[n.toNat]) hι.some else (pred^[(-n).toNat]) hι.some
  left_inv i := by
    cases' le_or_lt hι.some i with hi hi
    · have h_nonneg : 0 ≤ toZ hι.some i := to_Z_nonneg hi
      simp_rw [if_pos h_nonneg]
      exact iterate_succ_to_Z i hi
      
    · have h_neg : toZ hι.some i < 0 := to_Z_neg hi
...
def orderIsoNatOfLinearSuccPredArch [NoMaxOrder ι] [OrderBot ι] : ι ≃o ℕ where
  ...
  invFun n := (succ^[n]) ⊥
  left_inv i := by
    simp_rw [if_pos (to_Z_nonneg bot_le)]
    exact iterate_succ_to_Z i bot_le
  right_inv n := by
...
def orderIsoRangeOfLinearSuccPredArch [OrderBot ι] [OrderTop ι] : ι ≃o Finset.range ((toZ ⊥ (⊤ : ι)).toNat + 1) where
  toFun i := ⟨(toZ ⊥ i).toNat, Finset.mem_range_succ_iff.mpr (Int.to_nat_le_to_nat ((to_Z_le_iff _ _).mpr le_top))⟩
  invFun n := (succ^[n]) ⊥
