...
theorem refl_trans_gen_of_succ_of_le (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IcoCat n m, r i (succ i)) (hnm : n ≤ m) :
...
/-- For `m ≤ n`, `(n, m)` is in the reflexive-transitive closure of `~` if `succ i ~ i`
  for all `i` between `n` and `m`. -/
theorem refl_trans_gen_of_succ_of_ge (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IcoCat m n, r (succ i) i) (hmn : m ≤ n) :
    ReflTransGen r n m := by
...
/-- For `n < m`, `(n, m)` is in the transitive closure of a relation `~` if `i ~ succ i`
  for all `i` between `n` and `m`. -/
theorem trans_gen_of_succ_of_lt (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IcoCat n m, r i (succ i)) (hnm : n < m) :
    TransGen r n m :=
...
/-- For `m < n`, `(n, m)` is in the transitive closure of a relation `~` if `succ i ~ i`
  for all `i` between `n` and `m`. -/
theorem trans_gen_of_succ_of_gt (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IcoCat m n, r (succ i) i) (hmn : m < n) :
    TransGen r n m :=
...
/-- `(n, m)` is in the reflexive-transitive closure of `~` if `i ~ succ i` and `succ i ~ i`
  for all `i` between `n` and `m`. -/
theorem refl_trans_gen_of_succ (r : α → α → Prop) {n m : α} (h1 : ∀ i ∈ IcoCat n m, r i (succ i))
    (h2 : ∀ i ∈ IcoCat m n, r (succ i) i) : ReflTransGen r n m :=
...
theorem trans_gen_of_succ_of_ne (r : α → α → Prop) {n m : α} (h1 : ∀ i ∈ IcoCat n m, r i (succ i))
    (h2 : ∀ i ∈ IcoCat m n, r (succ i) i) (hnm : n ≠ m) : TransGen r n m :=
...
theorem trans_gen_of_succ_of_reflexive (r : α → α → Prop) {n m : α} (hr : Reflexive r)
    (h1 : ∀ i ∈ IcoCat n m, r i (succ i)) (h2 : ∀ i ∈ IcoCat m n, r (succ i) i) : TransGen r n m := by
...
/-- For `m ≤ n`, `(n, m)` is in the reflexive-transitive closure of `~` if `i ~ pred i`
  for all `i` between `n` and `m`. -/
theorem refl_trans_gen_of_pred_of_ge (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IocCat m n, r i (pred i)) (hnm : m ≤ n) :
    ReflTransGen r n m :=
...
/-- For `n ≤ m`, `(n, m)` is in the reflexive-transitive closure of `~` if `pred i ~ i`
  for all `i` between `n` and `m`. -/
theorem refl_trans_gen_of_pred_of_le (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IocCat n m, r (pred i) i) (hmn : n ≤ m) :
    ReflTransGen r n m :=
...
/-- For `m < n`, `(n, m)` is in the transitive closure of a relation `~` for `n ≠ m` if `i ~ pred i`
  for all `i` between `n` and `m`. -/
theorem trans_gen_of_pred_of_gt (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IocCat m n, r i (pred i)) (hnm : m < n) :
    TransGen r n m :=
...
/-- For `n < m`, `(n, m)` is in the transitive closure of a relation `~` for `n ≠ m` if `pred i ~ i`
  for all `i` between `n` and `m`. -/
theorem trans_gen_of_pred_of_lt (r : α → α → Prop) {n m : α} (h : ∀ i ∈ IocCat n m, r (pred i) i) (hmn : n < m) :
    TransGen r n m :=
...
/-- `(n, m)` is in the reflexive-transitive closure of `~` if `i ~ pred i` and `pred i ~ i`
  for all `i` between `n` and `m`. -/
theorem refl_trans_gen_of_pred (r : α → α → Prop) {n m : α} (h1 : ∀ i ∈ IocCat m n, r i (pred i))
    (h2 : ∀ i ∈ IocCat n m, r (pred i) i) : ReflTransGen r n m :=
...
theorem trans_gen_of_pred_of_ne (r : α → α → Prop) {n m : α} (h1 : ∀ i ∈ IocCat m n, r i (pred i))
    (h2 : ∀ i ∈ IocCat n m, r (pred i) i) (hnm : n ≠ m) : TransGen r n m :=
...
theorem trans_gen_of_pred_of_reflexive (r : α → α → Prop) {n m : α} (hr : Reflexive r)
    (h1 : ∀ i ∈ IocCat m n, r i (pred i)) (h2 : ∀ i ∈ IocCat n m, r (pred i) i) : TransGen r n m :=
