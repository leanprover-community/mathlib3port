...
/- ./././Mathport/Syntax/Translate/Basic.lean:611:2: warning: expanding binder collection (x «expr ≠ » «expr⊤»()) -/
...
theorem IsCoatomic.of_is_chain_bounded {α : Type _} [PartialOrder α] [OrderTop α]
    (h : ∀ c : Set α, IsChain (· ≤ ·) c → c.Nonempty → ⊤ ∉ c → ∃ (x) (_ : x ≠ ⊤), x ∈ upperBounds c) : IsCoatomic α :=
  by
...
theorem IsAtomic.of_is_chain_bounded {α : Type _} [PartialOrder α] [OrderBot α]
    (h : ∀ c : Set α, IsChain (· ≤ ·) c → c.Nonempty → ⊥ ∉ c → ∃ (x) (_ : x ≠ ⊥), x ∈ lowerBounds c) : IsAtomic α :=
