...
theorem Monotone.disjointed_eq {f : ℕ → α} (hf : Monotone f) (n : ℕ) : disjointed f (n+1) = f (n+1) \ f n :=
  by 
...
theorem disjointed_eq_inf_compl (f : ℕ → α) (n : ℕ) : disjointed f n = f n⊓⨅(i : _)(_ : i < n), «expr ᶜ» (f i) :=
  by 
    ...
    simpRw [disjointed_succ, partial_sups_eq_bsupr, sdiff_eq, compl_supr]
    congr 
    ext i 
    rw [Nat.lt_succ_iff]
...
theorem preimage_find_eq_disjointed (s : ℕ → Set α) (H : ∀ x, ∃ n, x ∈ s n) [∀ x n, Decidable (x ∈ s n)] (n : ℕ) :
  (fun x => Nat.findₓ (H x)) ⁻¹' {n} = disjointed s n :=
  by 
    ext x 
