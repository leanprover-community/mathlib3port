...
theorem Disjoint.mapOrderIso [SemilatticeInf α] [OrderBot α] [SemilatticeInf β] [OrderBot β] {a b : α} (f : α ≃o β)
...
theorem Codisjoint.mapOrderIso [SemilatticeSup α] [OrderTop α] [SemilatticeSup β] [OrderTop β] {a b : α} (f : α ≃o β)
...
theorem disjoint_map_order_iso_iff [SemilatticeInf α] [OrderBot α] [SemilatticeInf β] [OrderBot β] {a b : α}
  ...
  ⟨fun h => f.symm_apply_apply a ▸ f.symm_apply_apply b ▸ h.mapOrderIso f.symm, fun h => h.mapOrderIso f⟩
...
theorem OrderIso.isCompl {x y : α} (h : IsCompl x y) : IsCompl (f x) (f y) :=
  ⟨h.1.mapOrderIso _, h.2.mapOrderIso _⟩
