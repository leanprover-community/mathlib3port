...
theorem le_iterate_comp_of_le (hf : Monotone f) (H : h ∘ g ≤ f ∘ h) (n : ℕ) : h ∘ g^[n] ≤ f^[n] ∘ h :=
  fun x =>
    by 
...
`f^[n]` of `f`. -/
theorem id_le_iterate_of_id_le (h : id ≤ f) (n : ℕ) : id ≤ f^[n] :=
  by 
...
theorem antitone_iterate_of_le_id (h : f ≤ id) : Antitone fun m => f^[m] :=
  fun m n hmn => @monotone_iterate_of_id_le (OrderDual α) _ f h m n hmn
...
theorem strict_anti_iterate_of_map_lt (hf : StrictMono f) (hx : f x < x) : StrictAnti fun n => (f^[n]) x :=
