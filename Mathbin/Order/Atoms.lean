...
theorem IsAtom.inf_eq_bot_of_ne [SemilatticeInfBot α] {a b : α} (ha : IsAtom a) (hb : IsAtom b) (hab : a ≠ b) :
  a⊓b = ⊥ :=
...
theorem IsAtom.disjoint_of_ne [SemilatticeInfBot α] {a b : α} (ha : IsAtom a) (hb : IsAtom b) (hab : a ≠ b) :
  Disjoint a b :=
  disjoint_iff.mpr (IsAtom.inf_eq_bot_of_ne ha hb hab)

theorem IsCoatom.sup_eq_top_of_ne [SemilatticeSupTop α] {a b : α} (ha : IsCoatom a) (hb : IsCoatom b) (hab : a ≠ b) :
  a⊔b = ⊤ :=
...
class IsSimpleLattice(α : Type _)[BoundedLattice α] extends Nontrivial α : Prop where 
...
variable[BoundedLattice α][IsSimpleLattice α]
...
instance (priority := 100) : BoundedDistribLattice α :=
  { (inferInstance : BoundedLattice α) with
    le_sup_inf :=
      fun x y z =>
        by 
...
variable[DecidableEq α][BoundedLattice α][IsSimpleLattice α]
...
protected def BooleanAlgebra : BooleanAlgebra α :=
  { IsSimpleLattice.boundedDistribLattice with Compl := fun x => if x = ⊥ then ⊤ else ⊥,
    ...
    inf_inf_sdiff :=
      fun x y =>
        by 
          rcases eq_bot_or_eq_top x with (rfl | rfl) <;> rcases eq_bot_or_eq_top y with (rfl | rfl) <;> simp  }
...
protected noncomputable def CompleteLattice : CompleteLattice α :=
  { (inferInstance : BoundedLattice α) with sup := fun s => if ⊤ ∈ s then ⊤ else ⊥,
...
variable[BoundedLattice α][IsSimpleLattice α][DecidableEq α]
...
theorem is_simple_lattice_iff_is_atom_top [BoundedLattice α] : IsSimpleLattice α ↔ IsAtom (⊤ : α) :=
  ⟨fun h => @is_atom_top _ _ h,
    fun h =>
      { exists_pair_ne := ⟨⊤, ⊥, h.1⟩, eq_bot_or_eq_top := fun a => ((eq_or_lt_of_le le_top).imp_right (h.2 a)).symm }⟩

theorem is_simple_lattice_iff_is_coatom_bot [BoundedLattice α] : IsSimpleLattice α ↔ IsCoatom (⊥ : α) :=
  is_simple_lattice_iff_is_simple_lattice_order_dual.trans is_simple_lattice_iff_is_atom_top

namespace Set

theorem is_simple_lattice_Iic_iff_is_atom [BoundedLattice α] {a : α} : IsSimpleLattice (Iic a) ↔ IsAtom a :=
...
theorem is_simple_lattice_Ici_iff_is_coatom [BoundedLattice α] {a : α} : IsSimpleLattice (Ici a) ↔ IsCoatom a :=
...
variable{β : Type _}[PartialOrderₓ α][PartialOrderₓ β]
...
theorem is_simple_lattice_iff {α β} [BoundedLattice α] [BoundedLattice β] (f : α ≃o β) :
...
theorem IsSimpleLattice {α β} [BoundedLattice α] [BoundedLattice β] [h : IsSimpleLattice β] (f : α ≃o β) :
  IsSimpleLattice α :=
  f.is_simple_lattice_iff.mpr h

theorem is_atomic_iff [OrderBot α] [OrderBot β] (f : α ≃o β) : IsAtomic α ↔ IsAtomic β :=
  by 
...
theorem is_coatomic_iff [OrderTop α] [OrderTop β] (f : α ≃o β) : IsCoatomic α ↔ IsCoatomic β :=
  by 
...
theorem is_atomic_iff_is_coatomic : IsAtomic α ↔ IsCoatomic α :=
  ⟨fun h => @is_coatomic_of_is_atomic_of_is_complemented_of_is_modular _ _ _ _ h,
    fun h => @is_atomic_of_is_coatomic_of_is_complemented_of_is_modular _ _ _ _ h⟩
