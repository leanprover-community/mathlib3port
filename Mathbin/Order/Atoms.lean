...
@[simp]
theorem Set.ici.is_atom_iff {b : Set.ici a} : IsAtom b ↔ a ⋖ b := by
  rw [← bot_covby_iff]
...
@[simp]
theorem Set.iic.is_coatom_iff {a : Set.iic b} : IsCoatom a ↔ ↑a ⋖ b := by
  rw [← covby_top_iff]
...
instance : IsAtomistic α :=
  ⟨fun b =>
    (eq_bot_or_eq_top b).elim
      ...
      fun h =>
      ⟨{⊤}, h.trans Sup_singleton.symm, fun a ha =>
