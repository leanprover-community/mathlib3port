...
theorem IsAtom.inf_eq_bot_of_ne [SemilatticeInf α] [OrderBot α] {a b : α} (ha : IsAtom a) (hb : IsAtom b)
    (hab : a ≠ b) : a⊓b = ⊥ :=
  Or.elim (eq_bot_or_eq_of_le_atom ha inf_le_left) id fun h1 =>
    Or.elim (eq_bot_or_eq_of_le_atom hb inf_le_right) id fun h2 =>
      False.ndrec _ (hab (le_antisymmₓ (inf_eq_left.mp h1) (inf_eq_right.mp h2)))
...
theorem IsCoatom.sup_eq_top_of_ne [SemilatticeSup α] [OrderTop α] {a b : α} (ha : IsCoatom a) (hb : IsCoatom b)
    (hab : a ≠ b) : a⊔b = ⊤ :=
  Or.elim (eq_top_or_eq_of_coatom_le ha le_sup_left) id fun h1 =>
    Or.elim (eq_top_or_eq_of_coatom_le hb le_sup_right) id fun h2 =>
      False.ndrec _ (hab (le_antisymmₓ (sup_eq_right.mp h2) (sup_eq_left.mp h1)))
...
protected def IsSimpleOrder.preorder {α} [LE α] [BoundedOrder α] [IsSimpleOrder α] : Preorderₓ α :=
  { le := · ≤ ·,
    le_refl := fun a => by
...
protected def IsSimpleOrder.linearOrder [DecidableEq α] : LinearOrderₓ α :=
  { (inferInstance : PartialOrderₓ α) with
    le_total := fun a b => by
      rcases eq_bot_or_eq_top a with (rfl | rfl) <;> simp ,
    decidableLe := fun a b =>
      if ha : a = ⊥ then is_true (ha.le.trans bot_le)
      else
        if hb : b = ⊤ then is_true (le_top.trans hb.ge)
        else is_false fun H => hb (top_unique (le_transₓ (top_le_iff.mpr (Or.resolve_left (eq_bot_or_eq_top a) ha)) H)),
    DecidableEq := by
      assumption }
...
theorem bot_covers_top : (⊥ : α) ⋖ ⊤ :=
  is_atom_top.bot_covers
...
protected def Lattice {α} [DecidableEq α] [PartialOrderₓ α] [BoundedOrder α] [IsSimpleOrder α] : Lattice α :=
  @latticeOfLinearOrder α IsSimpleOrder.linearOrder
...
@[simps]
def equiv_bool {α} [DecidableEq α] [LE α] [BoundedOrder α] [IsSimpleOrder α] : α ≃ Bool :=
...
instance (priority := 200) {α} [DecidableEq α] [LE α] [BoundedOrder α] [IsSimpleOrder α] : Fintype α :=
  Fintype.ofEquiv Bool equiv_bool.symm
