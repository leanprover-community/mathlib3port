...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » upper_bounds s)
theorem le_Sup_iff : a ≤ Sup s ↔ ∀ b _ : b ∈ UpperBounds s, a ≤ b :=
...
theorem Sup_le_Sup_of_forall_exists_le (h : ∀ x _ : x ∈ s, ∃ (y : _)(_ : y ∈ t), x ≤ y) : Sup s ≤ Sup t :=
  le_Sup_iff.2$
    fun b hb =>
      Sup_le$
        fun a ha =>
...
theorem
  ...
  ( h : ∀ x _ : x ∈ s , ∃ ( y : _ ) ( _ : y ∈ t ) , y ≤ x ) : Inf t ≤ Inf s
  :=
    le_of_forall_le
      by simp only [ le_Inf_iff ] introv h₀ h₁ rcases h _ h₁ with ⟨ y , hy , hy' ⟩ solveByElim [ le_transₓ _ hy' ]
...
/--
  ...
  def
    ...
    ( α : Type _ ) [ H1 : PartialOrderₓ α ] [ H2 : HasInfₓ α ] ( is_glb_Inf : ∀ s : Set α , IsGlb s Inf s )
      : CompleteLattice α
    :=
      {
        H1 , H2 with
        bot := Inf univ ,
          bot_le := fun x => is_glb_Inf univ . 1 trivialₓ ,
          top := Inf ∅ ,
          le_top := fun a => is_glb_Inf ∅ . 2 $ by simp ,
          sup := fun a b => Inf { x | a ≤ x ∧ b ≤ x } ,
          inf := fun a b => Inf { a , b } ,
          le_inf := fun a b c hab hac => by apply is_glb_Inf _ . 2 simp ,
...
/--
  ...
  def
    ...
    ( α : Type _ ) [ H1 : PartialOrderₓ α ] [ H2 : HasSupₓ α ] ( is_lub_Sup : ∀ s : Set α , IsLub s Sup s )
      : CompleteLattice α
    :=
      {
        H1 , H2 with
        top := Sup univ ,
          le_top := fun x => is_lub_Sup univ . 1 trivialₓ ,
          bot := Sup ∅ ,
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr < » «expr⊤»())
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s)
...
theorem OrderIso.map_supr [CompleteLattice β] (f : α ≃o β) (x : ι → α) : f (⨆ i, x i) = ⨆ i, f (x i) :=
  eq_of_forall_ge_iff$
    f.surjective.forall.2$
      fun x =>
        by 
          simp only [f.le_iff_le, supr_le_iff]

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s)
theorem OrderIso.map_Sup [CompleteLattice β] (f : α ≃o β) (s : Set α) : f (Sup s) = ⨆ (a : _)(_ : a ∈ s), f a :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (a «expr ∈ » s)
theorem OrderIso.map_Inf [CompleteLattice β] (f : α ≃o β) (s : Set α) : f (Inf s) = ⨅ (a : _)(_ : a ∈ s), f a :=
...
theorem infi_subtype' {p : ι → Prop} {f : ∀ i, p i → α} :
  (⨅ (i : _)(h : p i), f i h) = ⨅ x : Subtype p, f x x.property :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (b «expr ∈ » range f)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » («expr∅»() : set β))
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » (univ : set β))
...
theorem
  infi_split
  ( f : β → α ) ( p : β → Prop ) : ⨅ i , f i = ⨅ ( i : _ ) ( h : p i ) , f i ⊓ ⨅ ( i : _ ) ( h : ¬ p i ) , f i
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » insert b s)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » s)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » (singleton b : set β))
theorem infi_singleton {f : β → α} {b : β} : (⨅ (x : _)(_ : x ∈ (singleton b : Set β)), f x) = f b :=
  by 
    simp 

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (x «expr ∈ » ({a, b} : set β))
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (c «expr ∈ » «expr '' »(f, t))
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ≥ » n)
@[simp]
theorem supr_infi_ge_nat_add (f : ℕ → α) (k : ℕ) :
  (⨆ n, ⨅ (i : _)(_ : i ≥ n), f (i+k)) = ⨆ n, ⨅ (i : _)(_ : i ≥ n), f i :=
  by 
...
theorem
  independent_def'
  { ι : Type _ } { t : ι → α } : independent t ↔ ∀ i , Disjoint t i Sup t '' { j | j ≠ i }
  := by simpRw [ Sup_image ] rfl
...
theorem
  independent_def''
  { ι : Type _ } { t : ι → α } : independent t ↔ ∀ i , Disjoint t i Sup { a | ∃ ( j : _ ) ( _ : j ≠ i ) , t j = a }
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » y)
