...
theorem partially_well_ordered_on {r : α → α → Prop} [IsRefl α r] (f : Finset α) :
  Set.PartiallyWellOrderedOn (↑f : Set α) r :=
  by 
    intro g hg 
...
@[simp]
theorem well_founded_on {r : α → α → Prop} [IsStrictOrder α r] (f : Finset α) : Set.WellFoundedOn (↑f : Set α) r :=
  by 
...
/--
  ...
  def
    mul_antidiagonal
    [ Monoidₓ α ] ( s t : Set α ) ( a : α ) : Set α × α
...
theorem finite_of_is_pwo (hs : s.is_pwo) (ht : t.is_pwo) a : (mul_antidiagonal s t a).Finite :=
  by 
    ...
    have h1 : (mul_antidiagonal s t a).PartiallyWellOrderedOn (Prod.fst ⁻¹'o (· ≤ ·))
    ·
      intro f hf 
    ...
    have h2 : (mul_antidiagonal s t a).PartiallyWellOrderedOn (Prod.snd ⁻¹'o (· ≤ ·))
    ·
      intro f hf 
