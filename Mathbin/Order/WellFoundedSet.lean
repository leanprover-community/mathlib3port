...
theorem well_founded_on_iff_no_descending_seq {s : Set α} {r : α → α → Prop} [IsStrictOrder α r] :
  ...
  refine'
    ⟨fun h f con => by
      ...
      simp only [con (mem_range_self a), con (mem_range_self b), and_trueₓ, gt_iff_ltₓ, Function.Embedding.coe_fn_mk,
        f.map_rel_iff],
      fun h => ⟨fun con => _⟩⟩
...
theorem eq_of_fst_le_fst_of_snd_le_snd {x y : MulAntidiagonal s t a} (h1 : (x : α × α).fst ≤ (y : α × α).fst)
  ...
  cases' eq_or_lt_of_leₓ h1 with heq hlt
  · exact HEq
...
theorem mul_antidiagonal_min_mul_min {α} [LinearOrderedCancelCommMonoid α] {s t : Set α} (hs : s.IsWf) (ht : t.IsWf)
  ...
  constructor
  · rintro ⟨hast, has, hat⟩
    cases' eq_or_lt_of_leₓ (hs.min_le hns has) with heq hlt
    · refine' ⟨HEq.symm, _⟩
      rw [HEq] at hast
