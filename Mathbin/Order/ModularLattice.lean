...
theorem inf_sup_assoc_of_le {x : α} (y : α) {z : α} (h : z ≤ x) : x⊓y⊔z = x⊓(y⊔z) :=
  by 
...
theorem sup_lt_sup_of_lt_of_inf_le_inf (hxy : x < y) (hinf : y⊓z ≤ x⊓z) : x⊔z < y⊔z :=
  lt_of_le_of_neₓ (sup_le_sup_right (le_of_ltₓ hxy) _)
    fun hsup => ne_of_ltₓ hxy$ eq_of_le_of_inf_le_of_sup_le (le_of_ltₓ hxy) hinf (le_of_eqₓ hsup.symm)
...
theorem well_founded_lt_exact_sequence {β γ : Type _} [PartialOrderₓ β] [PartialOrderₓ γ]
  (h₁ : WellFounded (· < · : β → β → Prop)) (h₂ : WellFounded (· < · : γ → γ → Prop)) (K : α) (f₁ : β → α) (f₂ : α → β)
  (g₁ : γ → α) (g₂ : α → γ) (gci : GaloisCoinsertion f₁ f₂) (gi : GaloisInsertion g₂ g₁) (hf : ∀ a, f₁ (f₂ a) = a⊓K)
  (hg : ∀ a, g₁ (g₂ a) = a⊔K) : WellFounded (· < · : α → α → Prop) :=
  Subrelation.wfₓ
    (fun A B hAB =>
...
theorem is_modular_lattice_iff_inf_sup_inf_assoc [Lattice α] :
  ...
  ⟨fun h => @IsModularLattice.inf_sup_inf_assoc _ _ h,
    fun h =>
      ⟨fun x y z xz =>
          by 
