...
protected theorem _root_.has_lt.lt.covers_succ {a b : α} (h : a < b) : a ⋖ succ a :=
...
theorem _root_.covers_iff_succ_eq : a ⋖ b ↔ succ a = b :=
  ⟨Covers.succ_eq, by
    rintro rfl
    exact covers_succ _⟩
...
protected theorem _root_.has_lt.lt.pred_covers {a b : α} (h : b < a) : pred a ⋖ a :=
...
theorem _root_.covers_iff_pred_eq : a ⋖ b ↔ pred b = a :=
  ⟨Covers.pred_eq, by
    rintro rfl
    exact pred_covers _⟩
...
theorem Succ.rec {p : α → Prop} (hsucc : ∀ a, p a → p (succ a)) {a b : α} (h : a ≤ b) (ha : p a) : p b := by
