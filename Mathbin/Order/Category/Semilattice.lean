...
instance hasForgetToPartialOrder : HasForget₂ SemilatticeSupCat PartialOrderₓₓ where
...
instance hasForgetToPartialOrder : HasForget₂ SemilatticeInfCat PartialOrderₓₓ where
  forget₂ := { obj := fun X => ⟨X⟩, map := fun X Y f => f }
