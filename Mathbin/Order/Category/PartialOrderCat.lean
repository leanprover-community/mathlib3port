...
def preorderToPartialOrder : PreorderCat.{u} ⥤ PartialOrderCat where
  ...
  map_id' X := by
    ext
    exact Quotient.induction_on' x fun x => Quotient.map'_mk' _ (fun a b => id) _
  map_comp' X Y Z f g := by
    ext
    exact Quotient.induction_on' x fun x => OrderHom.antisymmetrization_apply_mk _ _
        ...
        { toFun := fun f => ⟨f ∘ toAntisymmetrization (· ≤ ·), f.mono.comp to_antisymmetrization_mono⟩,
          invFun := fun f =>
            ⟨fun a => (Quotient.liftOn' a f) fun a b h => (AntisymmRel.image h f.mono).Eq, fun a b =>
              (Quotient.induction_on₂' a b) fun a b h => f.mono h⟩,
          left_inv := fun f => OrderHom.ext _ _ <| funext fun x => (Quotient.induction_on' x) fun x => rfl,
          right_inv := fun f => OrderHom.ext _ _ <| funext fun x => rfl },
      hom_equiv_naturality_left_symm' := fun X Y Z f g =>
        OrderHom.ext _ _ <| funext fun x => (Quotient.induction_on' x) fun x => rfl,
