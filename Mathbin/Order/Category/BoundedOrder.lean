...
instance large_category : LargeCategory.{u} BoundedOrderCat where
...
instance concrete_category : ConcreteCategory BoundedOrderCat where
  forget := ⟨coeSort, fun X Y => coeFn, fun X => rfl, fun X Y Z f g => rfl⟩
  forget_faithful :=
    ⟨fun X Y => by
      convert FunLike.coe_injective⟩

instance has_forget_to_PartialOrder : HasForget₂ BoundedOrderCat PartialOrderₓₓ where
  forget₂ := { obj := fun X => X.toPartialOrder, map := fun X Y => BoundedOrderHom.toOrderHom }

instance has_forget_to_Bipointed : HasForget₂ BoundedOrderCat Bipointed where
...
def dual_equiv : BoundedOrderCat ≌ BoundedOrderCat :=
