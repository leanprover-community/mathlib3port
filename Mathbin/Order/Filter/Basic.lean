...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in exacts: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem mem_infi'
{ι}
{s : ι → filter α}
{U : set α} : «expr ↔ »(«expr ∈ »(U, «expr⨅ , »((i), s i)), «expr∃ , »((I : set ι), «expr ∧ »(finite I, «expr∃ , »((V : ι → set α), «expr ∧ »(∀
     i, «expr ∈ »(V i, s i), «expr ∧ »(∀
      i «expr ∉ » I, «expr = »(V i, univ), «expr ∧ »(«expr = »(U, «expr⋂ , »((i «expr ∈ » I), V i)), «expr = »(U, «expr⋂ , »((i), V i))))))))) :=
begin
  simp [] [] ["only"] ["[", expr mem_infi, ",", expr set_coe.forall', ",", expr bInter_eq_Inter, "]"] [] [],
  refine [expr ⟨_, λ ⟨I, If, V, hVs, _, hVU, _⟩, ⟨I, If, λ i, V i, λ i, hVs i, hVU⟩⟩],
  rintro ["⟨", ident I, ",", ident If, ",", ident V, ",", ident hV, ",", ident rfl, "⟩"],
  refine [expr ⟨I, If, λ i, if hi : «expr ∈ »(i, I) then V ⟨i, hi⟩ else univ, λ i, _, λ i hi, _, _⟩],
  { split_ifs [] [],
    exacts ["[", expr hV _, ",", expr univ_mem, "]"] },
  { exact [expr dif_neg hi] },
  { simp [] [] [] ["[", expr Inter_dite, ",", expr bInter_eq_Inter, "]"] [] [] }
end
  ...
  ⟨fun ⟨⟨f, g, hfg⟩⟩ =>
      by_contra$
        fun h =>
          hfg$
            by 
              haveI  : IsEmpty α := not_nonempty_iff.1 h <;> exact Subsingleton.elimₓ _ _,
    fun ⟨x⟩ =>
      ⟨⟨⊤, ⊥,
          ne_bot.ne$
...
instance  : BoundedDistribLattice (Filter α) :=
...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
theorem frequently.exists
{p : α → exprProp()}
{f : filter α}
(hp : «expr∃ᶠ in , »((x), f, p x)) : «expr∃ , »((x), p x) :=
begin
  by_contradiction [ident H],
  replace [ident H] [":", expr «expr∀ᶠ in , »((x), f, «expr¬ »(p x))] [],
  from [expr eventually_of_forall (not_exists.1 H)],
  exact [expr hp H]
end
...
theorem comap_const_of_not_mem {x : α} {f : Filter α} {V : Set α} (hV : V ∈ f) (hx : x ∉ V) :
  ...
  by 
    ext W 
    suffices  : ∃ (t : _)(_ : t ∈ f), (fun y : α => x) ⁻¹' t ⊆ W
    ·
      simpa 
    use V, hV 
...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:176:17: failed to parenthesize: parenthesize: uncaught backtrack exception
...
{p : α → exprProp()}
[∀ x, decidable (p x)]
(h₀ : tendsto f «expr ⊓ »(l₁, expr𝓟() {x | p x}) l₂)
(h₁ : tendsto g «expr ⊓ »(l₁, expr𝓟() {x | «expr¬ »(p x)}) l₂) : tendsto (λ x, if p x then f x else g x) l₁ l₂ :=
begin
  simp [] [] ["only"] ["[", expr tendsto_def, ",", expr mem_inf_principal, "]"] [] ["at", "*"],
  intros [ident s, ident hs],
  filter_upwards ["[", expr h₀ s hs, ",", expr h₁ s hs, "]"] [],
  simp [] [] ["only"] ["[", expr mem_preimage, "]"] [] [],
  intros [ident x, ident hp₀, ident hp₁],
  split_ifs [] [],
  exacts ["[", expr hp₀ h, ",", expr hp₁ h, "]"]
end
...
theorem prod_comap_comap_eq {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x} {f₁ : Filter α₁} {f₂ : Filter α₂}
  {m₁ : β₁ → α₁} {m₂ : β₂ → α₂} : comap m₁ f₁ ×ᶠ comap m₂ f₂ = comap (fun p : β₁ × β₂ => (m₁ p.1, m₂ p.2)) (f₁ ×ᶠ f₂) :=
  by 
  ...
  le_antisymmₓ
    (fun s hs =>
      ...
      Filter.sets_of_superset _ (prod_mem_prod (image_mem_map hs₁) (image_mem_map hs₂))$
        calc Set.Prod (m₁ '' s₁) (m₂ '' s₂) = (fun p : α₁ × α₂ => (m₁ p.1, m₂ p.2)) '' Set.Prod s₁ s₂ :=
...
theorem map_prod_map_const_id_principal_coprod_principal {α β ι : Type _} (a : α) (b : β) (i : ι) :
  map (Prod.mapₓ (fun _ : α => b) id) ((𝓟 {a}).coprod (𝓟 {i})) = 𝓟 (({b} : Set β).Prod (univ : Set ι)) :=
  by 
...
variable{δ : Type _}{κ : δ → Type _}

/-- Coproduct of filters. -/
protected def Coprod (f : ∀ d, Filter (κ d)) : Filter (∀ d, κ d) :=
  ⨆d : δ, (f d).comap fun k => k d

theorem mem_Coprod_iff {s : Set (∀ d, κ d)} {f : ∀ d, Filter (κ d)} :
  s ∈ Filter.coprodₓ f ↔ ∀ d : δ, ∃ (t₁ : _)(_ : t₁ ∈ f d), (fun k : ∀ d, κ d => k d) ⁻¹' t₁ ⊆ s :=
  by 
    simp [Filter.coprodₓ]

theorem Coprod_ne_bot_iff' {f : ∀ d, Filter (κ d)} :
  ne_bot (Filter.coprodₓ f) ↔ (∀ d, Nonempty (κ d)) ∧ ∃ d, ne_bot (f d) :=
  by 
    simp only [Filter.coprodₓ, supr_ne_bot, ←exists_and_distrib_left, ←comap_eval_ne_bot_iff']

@[simp]
theorem Coprod_ne_bot_iff [∀ d, Nonempty (κ d)] {f : ∀ d, Filter (κ d)} :
  ne_bot (Filter.coprodₓ f) ↔ ∃ d, ne_bot (f d) :=
  by 
    simp [Coprod_ne_bot_iff']

theorem ne_bot.Coprod [∀ d, Nonempty (κ d)] {f : ∀ d, Filter (κ d)} {d : δ} (h : ne_bot (f d)) :
  ne_bot (Filter.coprodₓ f) :=
  Coprod_ne_bot_iff.2 ⟨d, h⟩

@[instance]
theorem Coprod_ne_bot [∀ d, Nonempty (κ d)] [Nonempty δ] (f : ∀ d, Filter (κ d)) [H : ∀ d, ne_bot (f d)] :
  ne_bot (Filter.coprodₓ f) :=
  (H (Classical.arbitrary δ)).coprod

@[mono]
theorem Coprod_mono {f₁ f₂ : ∀ d, Filter (κ d)} (hf : ∀ d, f₁ d ≤ f₂ d) : Filter.coprodₓ f₁ ≤ Filter.coprodₓ f₂ :=
  supr_le_supr$ fun d => comap_mono (hf d)

theorem map_pi_map_Coprod_le {μ : δ → Type _} {f : ∀ d, Filter (κ d)} {m : ∀ d, κ d → μ d} :
  map (fun k : ∀ d, κ d => fun d => m d (k d)) (Filter.coprodₓ f) ≤ Filter.coprodₓ fun d => map (m d) (f d) :=
  by 
    intro s h 
...
theorem tendsto.pi_map_Coprod {μ : δ → Type _} {f : ∀ d, Filter (κ d)} {m : ∀ d, κ d → μ d} {g : ∀ d, Filter (μ d)}
  (hf : ∀ d, tendsto (m d) (f d) (g d)) :
  tendsto (fun k : ∀ d, κ d => fun d => m d (k d)) (Filter.coprodₓ f) (Filter.coprodₓ g) :=
