...
instance inhabited_mem : Inhabited { s : Set α // s ∈ f } :=
  ⟨⟨univ, f.univ_sets⟩⟩
...
theorem monotone_mem {f : Filter α} : Monotone fun s => s ∈ f :=
  fun s t hst h => mem_of_superset h hst
...
-/
unsafe def filter_upwards (s : parse types.pexpr_list) (e' : parse$ optionalₓ types.texpr) : tactic Unit :=
  do 
...
instance  : PartialOrderₓ (Filter α) :=
...
def generate (g : Set (Set α)) : Filter α :=
  { Sets := generate_sets g, univ_sets := generate_sets.univ, sets_of_superset := fun x y => generate_sets.superset,
    inter_sets := fun s t => generate_sets.inter }
...
protected def mk_of_closure (s : Set (Set α)) (hs : (generate s).Sets = s) : Filter α :=
  { Sets := s, univ_sets := hs ▸ (univ_mem : univ ∈ generate s),
...
def gi_generate (α : Type _) : @GaloisInsertion (Set (Set α)) (OrderDual (Filter α)) _ _ Filter.generate Filter.Sets :=
  { gc := fun s f => sets_iff_generate, le_l_u := fun f u h => generate_sets.basic h,
    choice := fun s hs => Filter.mkOfClosure s (le_antisymmₓ hs$ sets_iff_generate.1$ le_rfl),
    choice_eq := fun s hs => mk_of_closure_sets }

-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
/-- The infimum of filters is the filter generated by intersections
  of elements of the two filters. -/ instance : has_inf (filter α) :=
⟨λ
 f
 g : filter α, { sets := {s | «expr∃ , »((a «expr ∈ » f) (b «expr ∈ » g), «expr = »(s, «expr ∩ »(a, b)))},
   univ_sets := ⟨_, univ_mem, _, univ_mem, by simp [] [] [] [] [] []⟩,
   sets_of_superset := begin
     rintro [ident x, ident y, "⟨", ident a, ",", ident ha, ",", ident b, ",", ident hb, ",", ident rfl, "⟩", ident xy],
     refine [expr ⟨«expr ∪ »(a, y), mem_of_superset ha (subset_union_left a y), «expr ∪ »(b, y), mem_of_superset hb (subset_union_left b y), _⟩],
     rw ["[", "<-", expr inter_union_distrib_right, ",", expr union_eq_self_of_subset_left xy, "]"] []
   end,
   inter_sets := begin
     rintro [ident x, ident y, "⟨", ident a, ",", ident ha, ",", ident b, ",", ident hb, ",", ident rfl, "⟩", "⟨", ident c, ",", ident hc, ",", ident d, ",", ident hd, ",", ident rfl, "⟩"],
     refine [expr ⟨«expr ∩ »(a, c), inter_mem ha hc, «expr ∩ »(b, d), inter_mem hb hd, _⟩],
     ac_refl
   end }⟩
...
@[simp]
theorem mem_top {s : Set α} : s ∈ (⊤ : Filter α) ↔ s = univ :=
  by 
...
private def original_complete_lattice : CompleteLattice (Filter α) :=
  @OrderDual.completeLattice _ (gi_generate α).liftCompleteLattice

attribute [local instance] original_complete_lattice

instance  : CompleteLattice (Filter α) :=
  original_complete_lattice.copy Filter.partialOrder.le rfl Filter.hasTop.1
    (top_unique$
      fun s hs =>
        by 
          simp [mem_top.1 hs])
    _ rfl _ rfl Filter.hasInf.1
    (by 
      ext f g : 2 
      exact
...
class ne_bot(f : Filter α) : Prop where 
  ne' : f ≠ ⊥

theorem ne_bot_iff {f : Filter α} : ne_bot f ↔ f ≠ ⊥ :=
  ⟨fun h => h.1, fun h => ⟨h⟩⟩

theorem ne_bot.ne {f : Filter α} (hf : ne_bot f) : f ≠ ⊥ :=
  ne_bot.ne'
...
theorem ne_bot.mono {f g : Filter α} (hf : ne_bot f) (hg : f ≤ g) : ne_bot g :=
  ⟨ne_bot_of_le_ne_bot hf.1 hg⟩

theorem ne_bot_of_le {f g : Filter α} [hf : ne_bot f] (hg : f ≤ g) : ne_bot g :=
  hf.mono hg

@[simp]
theorem sup_ne_bot {f g : Filter α} : ne_bot (f⊔g) ↔ ne_bot f ∨ ne_bot g :=
  by 
...
@[simp]
theorem mem_supr {x : Set α} {f : ι → Filter α} : x ∈ supr f ↔ ∀ i, x ∈ f i :=
  by 
...
theorem principal_mono {s t : Set α} : 𝓟 s ≤ 𝓟 t ↔ s ⊆ t :=
  by 
    simp only [le_principal_iff, iff_selfₓ, mem_principal]

@[mono]
theorem monotone_principal : Monotone (𝓟 : Set α → Filter α) :=
  fun _ _ => principal_mono.2

@[simp]
theorem principal_eq_iff_eq {s t : Set α} : 𝓟 s = 𝓟 t ↔ s = t :=
  by 
...
theorem nonempty_of_mem {f : Filter α} [hf : ne_bot f] {s : Set α} (hs : s ∈ f) : s.nonempty :=
  s.eq_empty_or_nonempty.elim (fun h => absurd hs (h.symm ▸ mt empty_mem_iff_bot.mp hf.1)) id

theorem ne_bot.nonempty_of_mem {f : Filter α} (hf : ne_bot f) {s : Set α} (hs : s ∈ f) : s.nonempty :=
  @nonempty_of_mem α f hf s hs

@[simp]
theorem empty_not_mem (f : Filter α) [ne_bot f] : ¬∅ ∈ f :=
  fun h => (nonempty_of_mem h).ne_empty rfl
...
@[local instance]
protected def Unique [IsEmpty α] : Unique (Filter α) :=
...
theorem mem_infi_finset {s : Finset α} {f : α → Filter β} {t : Set β} :
  ...
  by 
    ...
    ·
      ...
      refine'
        ⟨fun a => if h : a ∈ s then p ⟨a, h⟩ else univ,
          fun a ha =>
            by 
...
(hd : directed ((«expr ≥ »)) f)
(hb : ∀ i, ne_bot (f i)) : ne_bot (infi f) :=
if hι : nonempty ι then @infi_ne_bot_of_directed' _ _ _ hι hd hb else ⟨λ
 h : «expr = »(infi f, «expr⊥»()), have «expr ⊆ »(univ, («expr∅»() : set α)), begin
   rw ["[", "<-", expr principal_mono, ",", expr principal_univ, ",", expr principal_empty, ",", "<-", expr h, "]"] [],
   exact [expr «expr $ »(le_infi, λ i, «expr $ »(false.elim, hι ⟨i⟩))]
 end,
 let ⟨x⟩ := hn in
 this (mem_univ x)⟩
...
@[mono]
theorem join_mono {f₁ f₂ : Filter (Filter α)} (h : f₁ ≤ f₂) : join f₁ ≤ join f₂ :=
  fun s hs => h hs
...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp]
theorem eventually_const {f : filter α} [t : ne_bot f] {p : exprProp()} : «expr ↔ »(«expr∀ᶠ in , »((x), f, p), p) :=
classical.by_cases (λ
 h : p, by simp [] [] [] ["[", expr h, "]"] [] []) (λ h, by simpa [] [] [] ["[", expr h, "]"] [] ["using", expr t.ne])
...
{f : filter α}
{p : exprProp()}
{q : α → exprProp()} : «expr ↔ »(«expr∀ᶠ in , »((x), f, «expr ∨ »(p, q x)), «expr ∨ »(p, «expr∀ᶠ in , »((x), f, q x))) :=
classical.by_cases (λ h : p, by simp [] [] [] ["[", expr h, "]"] [] []) (λ h, by simp [] [] [] ["[", expr h, "]"] [] [])
...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp] theorem frequently_const {f : filter α} [ne_bot f] {p : exprProp()} : «expr ↔ »(«expr∃ᶠ in , »((x), f, p), p) :=
classical.by_cases (λ
 h : p, by simpa [] [] [] ["[", expr h, "]"] [] []) (λ h, by simp [] [] [] ["[", expr h, "]"] [] [])
...
@[simp]
theorem eventually_eq_univ {s : Set α} {l : Filter α} : s =ᶠ[l] univ ↔ s ∈ l :=
  by 
...
theorem eventually_eq.filter_mono {l l' : Filter α} {f g : α → β} (h₁ : f =ᶠ[l] g) (h₂ : l' ≤ l) : f =ᶠ[l'] g :=
  h₂ h₁

@[refl]
theorem eventually_eq.refl (l : Filter α) (f : α → β) : f =ᶠ[l] f :=
  eventually_of_forall$ fun x => rfl

theorem eventually_eq.rfl {l : Filter α} {f : α → β} : f =ᶠ[l] f :=
  eventually_eq.refl l f

@[symm]
theorem eventually_eq.symm {f g : α → β} {l : Filter α} (H : f =ᶠ[l] g) : g =ᶠ[l] f :=
  H.mono$ fun _ => Eq.symm

@[trans]
theorem eventually_eq.trans {f g h : α → β} {l : Filter α} (H₁ : f =ᶠ[l] g) (H₂ : g =ᶠ[l] h) : f =ᶠ[l] h :=
  H₂.rw (fun x y => f x = y) H₁

theorem eventually_eq.prod_mk {l} {f f' : α → β} (hf : f =ᶠ[l] f') {g g' : α → γ} (hg : g =ᶠ[l] g') :
...
theorem eventually_eq.fun_comp {f g : α → β} {l : Filter α} (H : f =ᶠ[l] g) (h : β → γ) : h ∘ f =ᶠ[l] h ∘ g :=
  H.mono$ fun x hx => congr_argₓ h hx

theorem eventually_eq.comp₂ {δ} {f f' : α → β} {g g' : α → γ} {l} (Hf : f =ᶠ[l] f') (h : β → γ → δ) (Hg : g =ᶠ[l] g') :
  (fun x => h (f x) (g x)) =ᶠ[l] fun x => h (f' x) (g' x) :=
  (Hf.prod_mk Hg).fun_comp (uncurry h)

@[toAdditive]
theorem eventually_eq.mul [Mul β] {f f' g g' : α → β} {l : Filter α} (h : f =ᶠ[l] g) (h' : f' =ᶠ[l] g') :
  (fun x => f x*f' x) =ᶠ[l] fun x => g x*g' x :=
  h.comp₂ (·*·) h'

@[toAdditive]
theorem eventually_eq.inv [HasInv β] {f g : α → β} {l : Filter α} (h : f =ᶠ[l] g) :
  (fun x => f x⁻¹) =ᶠ[l] fun x => g x⁻¹ :=
  h.fun_comp HasInv.inv

theorem eventually_eq.div [GroupWithZeroₓ β] {f f' g g' : α → β} {l : Filter α} (h : f =ᶠ[l] g) (h' : f' =ᶠ[l] g') :
  (fun x => f x / f' x) =ᶠ[l] fun x => g x / g' x :=
  by 
    simpa only [div_eq_mul_inv] using h.mul h'.inv

theorem eventually_eq.div' [Groupₓ β] {f f' g g' : α → β} {l : Filter α} (h : f =ᶠ[l] g) (h' : f' =ᶠ[l] g') :
  (fun x => f x / f' x) =ᶠ[l] fun x => g x / g' x :=
  by 
    simpa only [div_eq_mul_inv] using h.mul h'.inv

theorem eventually_eq.sub [AddGroupₓ β] {f f' g g' : α → β} {l : Filter α} (h : f =ᶠ[l] g) (h' : f' =ᶠ[l] g') :
  (fun x => f x - f' x) =ᶠ[l] fun x => g x - g' x :=
  by 
    simpa only [sub_eq_add_neg] using h.add h'.neg

theorem eventually_eq.inter {s t s' t' : Set α} {l : Filter α} (h : s =ᶠ[l] t) (h' : s' =ᶠ[l] t') :
  (s ∩ s' : Set α) =ᶠ[l] (t ∩ t' : Set α) :=
  h.comp₂ (· ∧ ·) h'

theorem eventually_eq.union {s t s' t' : Set α} {l : Filter α} (h : s =ᶠ[l] t) (h' : s' =ᶠ[l] t') :
  (s ∪ s' : Set α) =ᶠ[l] (t ∪ t' : Set α) :=
  h.comp₂ (· ∨ ·) h'
...
@[simp]
theorem eventually_eq_principal {s : Set α} {f g : α → β} : f =ᶠ[𝓟 s] g ↔ eq_on f g s :=
  Iff.rfl
...
theorem eventually_le.congr {f f' g g' : α → β} (H : f ≤ᶠ[l] g) (hf : f =ᶠ[l] f') (hg : g =ᶠ[l] g') : f' ≤ᶠ[l] g' :=
  H.mp$
    hg.mp$
      hf.mono$
        fun x hf hg H =>
...
theorem eventually_eq.le (h : f =ᶠ[l] g) : f ≤ᶠ[l] g :=
  h.mono$ fun x => le_of_eqₓ

@[refl]
theorem eventually_le.refl (l : Filter α) (f : α → β) : f ≤ᶠ[l] f :=
  eventually_eq.rfl.le

theorem eventually_le.rfl : f ≤ᶠ[l] f :=
  eventually_le.refl l f

@[trans]
theorem eventually_le.trans (H₁ : f ≤ᶠ[l] g) (H₂ : g ≤ᶠ[l] h) : f ≤ᶠ[l] h :=
  H₂.mp$ H₁.mono$ fun x => le_transₓ

@[trans]
theorem eventually_eq.trans_le (H₁ : f =ᶠ[l] g) (H₂ : g ≤ᶠ[l] h) : f ≤ᶠ[l] h :=
  H₁.le.trans H₂

@[trans]
theorem eventually_le.trans_eq (H₁ : f ≤ᶠ[l] g) (H₂ : g =ᶠ[l] h) : f ≤ᶠ[l] h :=
  H₁.trans H₂.le

end Preorderₓ

theorem eventually_le.antisymm [PartialOrderₓ β] {l : Filter α} {f g : α → β} (h₁ : f ≤ᶠ[l] g) (h₂ : g ≤ᶠ[l] f) :
  f =ᶠ[l] g :=
...
theorem eventually_le.le_iff_eq [PartialOrderₓ β] {l : Filter α} {f g : α → β} (h : f ≤ᶠ[l] g) :
  g ≤ᶠ[l] f ↔ g =ᶠ[l] f :=
...
@[mono]
theorem eventually_le.inter {s t s' t' : Set α} {l : Filter α} (h : s ≤ᶠ[l] t) (h' : s' ≤ᶠ[l] t') :
  (s ∩ s' : Set α) ≤ᶠ[l] (t ∩ t' : Set α) :=
  h'.mp$ h.mono$ fun x => And.imp

@[mono]
theorem eventually_le.union {s t s' t' : Set α} {l : Filter α} (h : s ≤ᶠ[l] t) (h' : s' ≤ᶠ[l] t') :
  (s ∪ s' : Set α) ≤ᶠ[l] (t ∪ t' : Set α) :=
...
@[mono]
theorem eventually_le.diff {s t s' t' : Set α} {l : Filter α} (h : s ≤ᶠ[l] t) (h' : t' ≤ᶠ[l] s') :
  (s \ s' : Set α) ≤ᶠ[l] (t \ t' : Set α) :=
  h.inter h'.compl
...
def map (m : α → β) (f : Filter α) : Filter β :=
  { Sets := preimage m ⁻¹' f.sets, univ_sets := univ_mem,
...
theorem eventually_comap {f : Filter β} {φ : α → β} {P : α → Prop} :
  ...
  by 
    ...
    ·
      ...
      apply ht y_in
    ·
      exact ⟨_, h, fun _ x_in => x_in _ rfl⟩
...
with this definition we have `s ∈ pure a` defeq `a ∈ s`. -/ instance : has_pure filter :=
⟨λ
 (α : Type u)
 (x), { sets := {s | «expr ∈ »(x, s)},
...
@[simp]
theorem pure_bind (a : α) (m : α → Filter β) : bind (pure a) m = m a :=
  by 
...
protected def Monadₓ : Monadₓ Filter :=
  { map := @Filter.map }

attribute [local instance] Filter.monad

protected theorem IsLawfulMonad : IsLawfulMonad Filter :=
...
instance  : Applicativeₓ Filter :=
  { map := @Filter.map, seq := @Filter.seq }

instance  : Alternativeₓ Filter :=
  { failure := fun α => ⊥, orelse := fun α x y => x⊔y }
...
{V : set α}
(hV : «expr ∈ »(V, f))
(hx : «expr ∉ »(x, V)) : «expr = »(comap (λ y : α, x) f, «expr⊥»()) :=
begin
  ext [] [ident W] [],
  suffices [] [":", expr «expr∃ , »((t «expr ∈ » f), «expr ⊆ »(«expr ⁻¹' »(λ y : α, x, t), W))],
  by simpa [] [] [] [] [] [],
  use ["[", expr V, ",", expr hV, "]"],
  simp [] [] [] ["[", expr preimage_const_of_not_mem hx, "]"] [] []
end
...
{x : α}
{f : filter α}
(h : ∀ V «expr ∈ » f, «expr ∈ »(x, V)) : «expr = »(comap (λ y : α, x) f, «expr⊤»()) :=
begin
  ext [] [ident W] [],
  suffices [] [":", expr «expr ↔ »(«expr∃ , »((t : set α), «expr ∧ »(«expr ∈ »(t, f), «expr ⊆ »(«expr ⁻¹' »(λ
        y : α, x, t), W))), «expr = »(W, univ))],
  by simpa [] [] [] [] [] [],
  split,
  { rintro ["⟨", ident V, ",", ident V_in, ",", ident hW, "⟩"],
    simpa [] [] [] ["[", expr preimage_const_of_mem (h V V_in), ",", expr univ_subset_iff, "]"] [] ["using", expr hW] },
  { rintro [ident rfl],
    use [expr univ],
    simp [] [] [] ["[", expr univ_mem, "]"] [] [] }
end
...
theorem comap_comap {m : γ → β} {n : β → α} : comap m (comap n f) = comap (n ∘ m) f :=
  le_antisymmₓ (fun c ⟨b, hb, (h : preimage (n ∘ m) b ⊆ c)⟩ => ⟨preimage n b, preimage_mem_comap hb, h⟩)
...
theorem map_comm (F : Filter α) : map ψ (map φ F) = map ρ (map θ F) :=
  by 
    rw [Filter.map_map, H, ←Filter.map_map]

theorem comap_comm (G : Filter δ) : comap φ (comap ψ G) = comap θ (comap ρ G) :=
  by 
...
@[simp]
theorem comap_pure {b : β} : comap m (pure b) = 𝓟 (m ⁻¹' {b}) :=
  by 
...
theorem comap_sup : comap m (g₁⊔g₂) = comap m g₁⊔comap m g₂ :=
  by 
...
theorem map_comap_of_mem {f : Filter β} {m : α → β} (hf : range m ∈ f) : (f.comap m).map m = f :=
  by 
...
theorem subtype_coe_map_comap (s : Set α) (f : Filter α) : map (coeₓ : s → α) (comap (coeₓ : s → α) f) = f⊓𝓟 s :=
  by 
    rw [map_comap, Subtype.range_coe]

theorem subtype_coe_map_comap_prod (s : Set α) (f : Filter (α × α)) :
  map (coeₓ : s × s → α × α) (comap (coeₓ : s × s → α × α) f) = f⊓𝓟 (s.prod s) :=
  have  : (coeₓ : s × s → α × α) = fun x => (x.1, x.2) :=
    by 
      ext ⟨x, y⟩ <;> rfl 
  by 
...
theorem comap_map {f : Filter α} {m : α → β} (h : injective m) : comap m (map m f) = f :=
  le_antisymmₓ
    (fun s hs =>
      mem_of_superset (preimage_mem_comap$ image_mem_map hs)$
        by 
...
theorem map_inj {f g : Filter α} {m : α → β} (hm : injective m) (h : map m f = map m g) : f = g :=
  have  : comap m (map m f) = comap m (map m g) :=
    by 
      rw [h]
  by 
...
theorem comap_ne_bot_iff {f : Filter β} {m : α → β} : ne_bot (comap m f) ↔ ∀ t (_ : t ∈ f), ∃ a, m a ∈ t :=
  by 
    ...
    exact
      ⟨fun h t t_in => h (m ⁻¹' t) ⟨t, t_in, subset.rfl⟩,
        fun h s ⟨u, u_in, hu⟩ =>
          let ⟨x, hx⟩ := h u u_in
          ⟨x, hu hx⟩⟩
...
@[instance]
theorem comap_fst_ne_bot [Nonempty β] {f : Filter α} [ne_bot f] : (f.comap (Prod.fst : α × β → α)).ne_bot :=
...
@[instance]
theorem comap_eval_ne_bot {ι : Type _} {α : ι → Type _} [∀ j, Nonempty (α j)] (i : ι) (f : Filter (α i)) [ne_bot f] :
  (comap (eval i) f).ne_bot :=
  comap_eval_ne_bot_iff.2 ‹_›

theorem comap_inf_principal_ne_bot_of_image_mem {f : Filter β} {m : α → β} (hf : ne_bot f) {s : Set α}
  (hs : m '' s ∈ f) : ne_bot (comap m f⊓𝓟 s) :=
  by 
...
theorem ne_bot.comap_of_image_mem {f : Filter β} {m : α → β} (hf : ne_bot f) {s : Set α} (hs : m '' s ∈ f) :
  ne_bot (comap m f) :=
...
theorem map_ne_bot_iff (f : α → β) {F : Filter α} : ne_bot (map f F) ↔ ne_bot F :=
  by 
    simp only [ne_bot_iff, Ne, map_eq_bot_iff]

theorem ne_bot.map (hf : ne_bot f) (m : α → β) : ne_bot (map m f) :=
...
{m : α → β}
(hf : directed ((«expr ≥ »)) f)
[nonempty ι] : «expr = »(map m (infi f), «expr⨅ , »((i), map m (f i))) :=
map_infi_le.antisymm (λ (s) (hs : «expr ∈ »(preimage m s, infi f)), let ⟨i, hi⟩ := (mem_infi_of_directed hf _).1 hs in
 have «expr ≤ »(«expr⨅ , »((i), map m (f i)), expr𝓟() s), from «expr $ »(infi_le_of_le i, by { simp [] [] ["only"] ["[", expr le_principal_iff, ",", expr mem_map, "]"] [] [],
    assumption }),
...
{n : β → α}
(h₁ : «expr = »(«expr ∘ »(m, n), id))
(h₂ : «expr = »(«expr ∘ »(n, m), id)) : «expr = »(map m f, comap n f) :=
le_antisymm (λ
 (b)
 ⟨a, ha, (h : «expr ⊆ »(preimage n a, b))⟩, «expr $ »(f.sets_of_superset ha, calc
    «expr = »(a, preimage «expr ∘ »(n, m) a) : by simp [] [] ["only"] ["[", expr h₂, ",", expr preimage_id, ",", expr eq_self_iff_true, "]"] [] []
    «expr ⊆ »(..., preimage m b) : preimage_mono h)) (λ
 (b)
 (hb : «expr ∈ »(preimage m b, f)), ⟨preimage m b, hb, show «expr ⊆ »(preimage «expr ∘ »(m, n) b, b), by simp [] [] ["only"] ["[", expr h₁, "]"] [] []; apply [expr subset.refl]⟩)
  ...
  by 
    ...
    ·
      ...
      calc f '' V ∩ Z = f '' (V ∩ f ⁻¹' Z) :=
        ...
        image_subset _ (inter_subset_inter_right _ ‹_›)_ = f '' (f ⁻¹' U) :=
        by 
          rw [h]_ ⊆ U :=
        image_preimage_subset f U

protected theorem push_pull' (f : α → β) (F : Filter α) (G : Filter β) : map f (comap f G⊓F) = G⊓map f F :=
  by 
...
theorem mem_map_seq_iff {f : Filter α} {g : Filter β} {m : α → β → γ} {s : Set γ} :
  ...
  Iff.intro (fun ⟨t, ht, s, hs, hts⟩ => ⟨s, m ⁻¹' t, hs, ht, fun a => hts _⟩)
    fun ⟨t, s, ht, hs, hts⟩ => ⟨m '' s, image_mem_map hs, t, ht, fun f ⟨a, has, Eq⟩ => Eq ▸ hts _ has⟩
...
theorem le_seq {f : Filter (α → β)} {g : Filter α} {h : Filter β}
  (hh : ∀ t (_ : t ∈ f), ∀ u (_ : u ∈ g), Set.Seq t u ∈ h) : h ≤ seq f g :=
...
-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
@[simp] theorem seq_pure (f : filter (α → β)) (a : α) : «expr = »(seq f (pure a), map (λ g : α → β, g a) f) :=
begin
  refine [expr le_antisymm «expr $ »(le_map, λ s hs, _) «expr $ »(le_seq, λ s hs t ht, _)],
  { rw ["<-", expr seq_singleton] [],
    exact [expr seq_mem_seq hs singleton_mem_pure] },
  { refine [expr sets_of_superset (map (λ g : α → β, g a) f) (image_mem_map hs) _],
    rintro [ident b, "⟨", ident g, ",", ident hg, ",", ident rfl, "⟩"],
    exact [expr ⟨g, hg, a, ht, rfl⟩] }
end
...
instance  : IsLawfulFunctor (Filter : Type u → Type u) :=
  { id_map := fun α f => map_id, comp_map := fun α β γ f g a => map_map.symm }

instance  : IsLawfulApplicative (Filter : Type u → Type u) :=
  { pure_seq_eq_map := fun α β => pure_seq_eq_map, map_pure := fun α β => map_pure, seq_pure := fun α β => seq_pure,
...
theorem sup_bind {f g : Filter α} {h : α → Filter β} : bind (f⊔g) h = bind f h⊔bind g h :=
  by 
...
(fs : list β')
(t : set (list α')) : «expr ↔ »(«expr ∈ »(t, traverse f fs), «expr∃ , »((us : list (set α')), «expr ∧ »(forall₂ (λ
    (b)
    (s : set α'), «expr ∈ »(s, f b)) fs us, «expr ⊆ »(sequence us, t)))) :=
begin
  split,
  { induction [expr fs] [] [] ["generalizing", ident t],
    case [ident nil] { simp [] [] ["only"] ["[", expr sequence, ",", expr mem_pure, ",", expr imp_self, ",", expr forall₂_nil_left_iff, ",", expr exists_eq_left, ",", expr set.pure_def, ",", expr singleton_subset_iff, ",", expr traverse_nil, "]"] [] [] },
    case [ident cons, ":", ident b, ident fs, ident ih, ident t] { intro [ident ht],
      rcases [expr mem_seq_iff.1 ht, "with", "⟨", ident u, ",", ident hu, ",", ident v, ",", ident hv, ",", ident ht, "⟩"],
      rcases [expr mem_map_iff_exists_image.1 hu, "with", "⟨", ident w, ",", ident hw, ",", ident hwu, "⟩"],
      rcases [expr ih v hv, "with", "⟨", ident us, ",", ident hus, ",", ident hu, "⟩"],
      exact [expr ⟨[«expr :: »/«expr :: »/«expr :: »](w, us), forall₂.cons hw hus, (set.seq_mono hwu hu).trans ht⟩] } },
  { rintro ["⟨", ident us, ",", ident hus, ",", ident hs, "⟩"],
    exact [expr mem_of_superset (mem_traverse _ _ hus) hs] }
end
...
@[simp]
theorem tendsto_bot {f : α → β} {l : Filter β} : tendsto f ⊥ l :=
  by 
    simp [tendsto]
...
theorem tendsto_of_is_empty [IsEmpty α] {f : α → β} {la : Filter α} {lb : Filter β} : tendsto f la lb :=
  by 
...
theorem tendsto.ne_bot {f : α → β} {x : Filter α} {y : Filter β} (h : tendsto f x y) [hx : ne_bot x] : ne_bot y :=
  (hx.map _).mono h
...
theorem tendsto_comap_iff {f : α → β} {g : β → γ} {a : Filter α} {c : Filter γ} :
  ...
  ⟨fun h => tendsto_comap.comp h,
    fun h =>
      map_le_iff_le_comap.mp$
        by 
          rwa [map_map]⟩
...
theorem tendsto.of_tendsto_comp {f : α → β} {g : β → γ} {a : Filter α} {b : Filter β} {c : Filter γ}
  ...
  by 
    ...
    calc a ≤ comap (g ∘ f) c := hfg _ ≤ comap f b :=
      by 
...
theorem prod_mem_prod_iff {s : Set α} {t : Set β} {f : Filter α} {g : Filter β} [f.ne_bot] [g.ne_bot] :
  ...
  ⟨fun h =>
      ...
      (prod_subset_prod_iff.1 H).elim (fun ⟨hs's, ht't⟩ => ⟨mem_of_superset hs' hs's, mem_of_superset ht' ht't⟩)
        fun h =>
          h.elim (fun hs'e => absurd hs'e (nonempty_of_mem hs').ne_empty)
...
theorem prod_top {f : Filter α} : f ×ᶠ (⊤ : Filter β) = f.comap Prod.fst :=
  by 
    rw [Filter.prod, comap_top, inf_top_eq]

theorem sup_prod (f₁ f₂ : Filter α) (g : Filter β) : f₁⊔f₂ ×ᶠ g = (f₁ ×ᶠ g)⊔(f₂ ×ᶠ g) :=
  by 
    rw [Filter.prod, comap_sup, inf_sup_right, ←Filter.prod, ←Filter.prod]

theorem prod_sup (f : Filter α) (g₁ g₂ : Filter β) : f ×ᶠ g₁⊔g₂ = (f ×ᶠ g₁)⊔(f ×ᶠ g₂) :=
  by 
...
{f₂ : filter α₂}
{m₁ : β₁ → α₁}
{m₂ : β₂ → α₂} : «expr = »(«expr ×ᶠ »(comap m₁ f₁, comap m₂ f₂), comap (λ
  p : «expr × »(β₁, β₂), (m₁ p.1, m₂ p.2)) «expr ×ᶠ »(f₁, f₂)) :=
by simp [] [] ["only"] ["[", expr filter.prod, ",", expr comap_comap, ",", expr eq_self_iff_true, ",", expr comap_inf, "]"] [] []

theorem prod_comm' : f ×ᶠ g = comap Prod.swap (g ×ᶠ f) :=
  by 
    simp only [Filter.prod, comap_comap, · ∘ ·, inf_comm, Prod.fst_swap, eq_self_iff_true, Prod.snd_swap, comap_inf]

-- error in Order.Filter.Basic: ././Mathport/Syntax/Translate/Basic.lean:177:17: failed to parenthesize: no declaration of attribute [parenthesizer] found for 'Lean.Parser.Term.explicitBinder'
theorem prod_comm : «expr = »(«expr ×ᶠ »(f, g), map (λ p : «expr × »(β, α), (p.2, p.1)) «expr ×ᶠ »(g, f)) :=
by { rw ["[", expr prod_comm', ",", "<-", expr map_swap_eq_comap_swap, "]"] [],
  refl }
...
{f₂ : filter α₂}
{m₁ : α₁ → β₁}
{m₂ : α₂ → β₂} : «expr = »(«expr ×ᶠ »(map m₁ f₁, map m₂ f₂), map (λ
  p : «expr × »(α₁, α₂), (m₁ p.1, m₂ p.2)) «expr ×ᶠ »(f₁, f₂)) :=
le_antisymm (λ s hs, let ⟨s₁, hs₁, s₂, hs₂, h⟩ := mem_prod_iff.mp hs in
 «expr $ »(filter.sets_of_superset _ (prod_mem_prod (image_mem_map hs₁) (image_mem_map hs₂)), calc
    «expr = »(set.prod «expr '' »(m₁, s₁) «expr '' »(m₂, s₂), «expr '' »(λ
      p : «expr × »(α₁, α₂), (m₁ p.1, m₂ p.2), set.prod s₁ s₂)) : set.prod_image_image_eq
    «expr ⊆ »(..., _) : by rwa ["[", expr image_subset_iff, "]"] [])) ((tendsto.comp le_rfl tendsto_fst).prod_mk (tendsto.comp le_rfl tendsto_snd))

theorem prod_map_map_eq' {α₁ : Type _} {α₂ : Type _} {β₁ : Type _} {β₂ : Type _} (f : α₁ → α₂) (g : β₁ → β₂)
  (F : Filter α₁) (G : Filter β₁) : map f F ×ᶠ map g G = map (Prod.mapₓ f g) (F ×ᶠ G) :=
  prod_map_map_eq

theorem tendsto.prod_map {δ : Type _} {f : α → γ} {g : β → δ} {a : Filter α} {b : Filter β} {c : Filter γ}
  {d : Filter δ} (hf : tendsto f a c) (hg : tendsto g b d) : tendsto (Prod.mapₓ f g) (a ×ᶠ b) (c ×ᶠ d) :=
  by 
    erw [tendsto, ←prod_map_map_eq]
    exact Filter.prod_mono hf hg

theorem map_prod (m : α × β → γ) (f : Filter α) (g : Filter β) : map m (f ×ᶠ g) = (f.map fun a b => m (a, b)).seq g :=
  by 
...
theorem prod_eq {f : Filter α} {g : Filter β} : f ×ᶠ g = (f.map Prod.mk).seq g :=
  have h := map_prod id f g 
  by 
    rwa [map_id] at h

theorem prod_inf_prod {f₁ f₂ : Filter α} {g₁ g₂ : Filter β} : (f₁ ×ᶠ g₁)⊓(f₂ ×ᶠ g₂) = f₁⊓f₂ ×ᶠ g₁⊓g₂ :=
  by 
    simp only [Filter.prod, comap_inf, inf_comm, inf_assoc, inf_left_comm]

@[simp]
theorem prod_bot {f : Filter α} : f ×ᶠ (⊥ : Filter β) = ⊥ :=
  by 
    simp [Filter.prod]

@[simp]
theorem bot_prod {g : Filter β} : (⊥ : Filter α) ×ᶠ g = ⊥ :=
  by 
    simp [Filter.prod]

@[simp]
theorem prod_principal_principal {s : Set α} {t : Set β} : 𝓟 s ×ᶠ 𝓟 t = 𝓟 (Set.Prod s t) :=
  by 
    simp only [Filter.prod, comap_principal, principal_eq_iff_eq, comap_principal, inf_principal] <;> rfl

@[simp]
theorem pure_prod {a : α} {f : Filter β} : pure a ×ᶠ f = map (Prod.mk a) f :=
  by 
    rw [prod_eq, map_pure, pure_seq_eq_map]

@[simp]
theorem prod_pure {f : Filter α} {b : β} : f ×ᶠ pure b = map (fun a => (a, b)) f :=
  by 
    rw [prod_eq, seq_pure, map_map]

theorem prod_pure_pure {a : α} {b : β} : pure a ×ᶠ pure b = pure (a, b) :=
  by 
    simp 
...
theorem prod_ne_bot {f : Filter α} {g : Filter β} : ne_bot (f ×ᶠ g) ↔ ne_bot f ∧ ne_bot g :=
  by 
    simp only [ne_bot_iff, Ne, prod_eq_bot, not_or_distrib]

theorem ne_bot.prod {f : Filter α} {g : Filter β} (hf : ne_bot f) (hg : ne_bot g) : ne_bot (f ×ᶠ g) :=
  prod_ne_bot.2 ⟨hf, hg⟩
...
@[instance]
theorem coprod_ne_bot_left [ne_bot f] [Nonempty β] : (f.coprod g).ne_bot :=
...
theorem map_prod_map_coprod_le {α₁ : Type u} {α₂ : Type v} {β₁ : Type w} {β₂ : Type x} {f₁ : Filter α₁} {f₂ : Filter α₂}
  {m₁ : α₁ → β₁} {m₂ : α₂ → β₂} : map (Prod.mapₓ m₁ m₂) (f₁.coprod f₂) ≤ (map m₁ f₁).coprod (map m₂ f₂) :=
  by 
    intro s 
...
(b : β)
(i : ι) : «expr = »((map (λ
   _ : α, b) (expr𝓟() {a})).coprod (map id (expr𝓟() {i})), expr𝓟() «expr ∪ »(({b} : set β).prod (univ : set ι), (univ : set β).prod {i})) :=
begin
  rw ["[", expr map_principal, ",", expr map_principal, ",", expr principal_coprod_principal, "]"] [],
  congr,
  ext [] ["⟨", ident b', ",", ident i', "⟩"] [],
  simp [] [] [] [] [] [],
  tauto []
...
theorem map_prod_map_const_id_principal_coprod_principal
{α β ι : Type*}
(a : α)
(b : β)
(i : ι) : «expr = »(map (prod.map (λ
   _ : α, b) id) ((expr𝓟() {a}).coprod (expr𝓟() {i})), expr𝓟() (({b} : set β).prod (univ : set ι))) :=
begin
  rw ["[", expr principal_coprod_principal, ",", expr map_principal, "]"] [],
  congr,
  ext [] ["⟨", ident b', ",", ident i', "⟩"] [],
  split,
  { rintro ["⟨", "⟨", ident a'', ",", ident i'', "⟩", ",", ident h₁, ",", ident h₂, ",", ident h₃, "⟩"],
    simp [] [] [] [] [] [] },
...
theorem tendsto.prod_map_coprod {δ : Type _} {f : α → γ} {g : β → δ} {a : Filter α} {b : Filter β} {c : Filter γ}
  {d : Filter δ} (hf : tendsto f a c) (hg : tendsto g b d) : tendsto (Prod.mapₓ f g) (a.coprod b) (c.coprod d) :=
...
theorem Set.EqOn.eventually_eq {α β} {s : Set α} {f g : α → β} (h : eq_on f g s) : f =ᶠ[𝓟 s] g :=
  h

theorem Set.EqOn.eventually_eq_of_mem {α β} {s : Set α} {l : Filter α} {f g : α → β} (h : eq_on f g s) (hl : s ∈ l) :
  f =ᶠ[l] g :=
...
theorem Set.MapsTo.tendsto {α β} {s : Set α} {t : Set β} {f : α → β} (h : maps_to f s t) :
