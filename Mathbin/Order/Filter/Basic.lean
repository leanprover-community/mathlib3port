...
@[simp]
theorem bInter_finset_mem {β : Type v} {s : β → Set α} (is : Finset β) : (⋂ i ∈ is, s i) ∈ f ↔ ∀ i ∈ is, s i ∈ f :=
...
theorem mem_generate_iff {s : Set $ Set α} {U : Set α} : U ∈ generate s ↔ ∃ (t) (_ : t ⊆ s), Set.Finite t ∧ ⋂₀ t ⊆ U :=
  by
  constructor <;> intro h
  · induction h
...
@[simp]
theorem mem_top {s : Set α} : s ∈ (⊤ : Filter α) ↔ s = univ := by rw [mem_top_iff_forall, eq_univ_iff_forall]
...
theorem neBotOfLe {f g : Filter α} [hf : NeBot f] (hg : f ≤ g) : NeBot g :=
  hf.mono hg
#align filter.ne_bot_of_le Filter.neBotOfLe

@[simp]
theorem sup_ne_bot {f g : Filter α} : NeBot (f ⊔ g) ↔ NeBot f ∨ NeBot g := by simp [ne_bot_iff, not_and_or]
...
@[simp]
theorem supr_ne_bot {f : ι → Filter α} : (⨆ i, f i).ne_bot ↔ ∃ i, (f i).ne_bot := by simp [ne_bot_iff]
...
#align filter._root_.pairwise.exists_mem_filter_of_disjoint filter._root_.pairwise.exists_mem_filter_of_disjoint

theorem _root_.set.pairwise_disjoint.exists_mem_filter {ι : Type _} {l : ι → Filter α} {t : Set ι}
    (hd : t.PairwiseDisjoint l) (ht : t.Finite) : ∃ s : ι → Set α, (∀ i, s i ∈ l i) ∧ t.PairwiseDisjoint s := by
  cases ht
  obtain ⟨s, hd⟩ : ∃ s : ∀ i : t, { s : Set α // s ∈ l i }, Pairwise (Disjoint on fun i => (s i : Set α)) := by
...
instance [Nonempty α] : Nontrivial (Filter α) :=
  ⟨⟨⊤, ⊥,
      ne_bot.ne $ forall_mem_nonempty_iff_ne_bot.1 $ fun s hs => by rwa [mem_top.1 hs, ← nonempty_iff_univ_nonempty]⟩⟩
...
theorem eq_Inf_of_mem_iff_exists_mem {S : Set (Filter α)} {l : Filter α} (h : ∀ {s}, s ∈ l ↔ ∃ f ∈ S, s ∈ f) :
    l = inf S :=
  le_antisymm (le_Inf $ fun f hf s hs => h.2 ⟨f, hf, hs⟩) fun s hs =>
    let ⟨f, hf, hs⟩ := h.1 hs
...
theorem eq_binfi_of_mem_iff_exists_mem {f : ι → Filter α} {p : ι → Prop} {l : Filter α}
    (h : ∀ {s}, s ∈ l ↔ ∃ (i) (_ : p i), s ∈ f i) : l = ⨅ (i) (_ : p i), f i := by
  rw [infi_subtype']
...
theorem infi_sets_eq {f : ι → Filter α} (h : Directed (· ≥ ·) f) [ne : Nonempty ι] : (infi f).sets = ⋃ i, (f i).sets :=
  ...
  have : u = infi f := eq_infi_of_mem_iff_exists_mem fun s => by simp only [Filter.mem_mk, mem_Union, Filter.mem_sets]
...
theorem infiNeBotOfDirected' {f : ι → Filter α} [Nonempty ι] (hd : Directed (· ≥ ·) f) (hb : ∀ i, NeBot (f i)) :
    NeBot (infi f) :=
  ⟨by
    intro h
...
#align filter.infi_ne_bot_of_directed' Filter.infiNeBotOfDirected'
...
theorem infiNeBotOfDirected {f : ι → Filter α} [hn : Nonempty α] (hd : Directed (· ≥ ·) f) (hb : ∀ i, NeBot (f i)) :
    NeBot (infi f) := by
...
theorem infNeBotOfDirected' {s : Set (Filter α)} (hne : s.Nonempty) (hd : DirectedOn (· ≥ ·) s) (hbot : ⊥ ∉ s) :
    NeBot (inf s) :=
  (Inf_eq_infi' s).symm ▸
    @infiNeBotOfDirected' _ _ _ hne.to_subtype hd.directed_coe fun ⟨f, hf⟩ => ⟨ne_of_mem_of_not_mem hf hbot⟩
#align filter.Inf_ne_bot_of_directed' Filter.infNeBotOfDirected'

theorem infNeBotOfDirected [Nonempty α] {s : Set (Filter α)} (hd : DirectedOn (· ≥ ·) s) (hbot : ⊥ ∉ s) :
    NeBot (inf s) :=
  (Inf_eq_infi' s).symm ▸ infiNeBotOfDirected hd.directed_coe fun ⟨f, hf⟩ => ⟨ne_of_mem_of_not_mem hf hbot⟩
#align filter.Inf_ne_bot_of_directed Filter.infNeBotOfDirected

theorem infi_ne_bot_iff_of_directed' {f : ι → Filter α} [Nonempty ι] (hd : Directed (· ≥ ·) f) :
    NeBot (infi f) ↔ ∀ i, NeBot (f i) :=
  ⟨fun H i => H.mono (infi_le _ i), infiNeBotOfDirected' hd⟩
...
@[simp]
theorem infi_principal {ι : Type w} [Finite ι] (f : ι → Set α) : (⨅ i, 𝓟 (f i)) = 𝓟 (⋂ i, f i) := by
...
theorem eventually_of_mem {f : Filter α} {P : α → Prop} {U : Set α} (hU : U ∈ f) (h : ∀ x ∈ U, P x) : ∀ᶠ x in f, P x :=
...
theorem eventually_or_distrib_right {f : Filter α} {p : α → Prop} {q : Prop} :
    (∀ᶠ x in f, p x ∨ q) ↔ (∀ᶠ x in f, p x) ∨ q := by simp only [or_comm' _ q, eventually_or_distrib_left]
...
theorem eventually_imp_distrib_left {f : Filter α} {p : Prop} {q : α → Prop} :
    (∀ᶠ x in f, p → q x) ↔ p → ∀ᶠ x in f, q x := by simp only [imp_iff_not_or, eventually_or_distrib_left]
...
theorem frequently_and_distrib_right {f : Filter α} {p : α → Prop} {q : Prop} :
    (∃ᶠ x in f, p x ∧ q) ↔ (∃ᶠ x in f, p x) ∧ q := by simp only [and_comm' _ q, frequently_and_distrib_left]
...
@[simp]
theorem eventually_eq_univ {s : Set α} {l : Filter α} : s =ᶠ[l] univ ↔ s ∈ l := by simp [eventually_eq_set]
...
theorem EventuallyEq.sub_eq [AddGroup β] {f g : α → β} {l : Filter α} (h : f =ᶠ[l] g) : f - g =ᶠ[l] 0 := by
  simpa using (eventually_eq.sub (eventually_eq.refl l f) h).symm
#align filter.eventually_eq.sub_eq Filter.EventuallyEq.sub_eq

theorem eventually_eq_iff_sub [AddGroup β] {f g : α → β} {l : Filter α} : f =ᶠ[l] g ↔ f - g =ᶠ[l] 0 :=
...
theorem EventuallyLe.antisymm [PartialOrder β] {l : Filter α} {f g : α → β} (h₁ : f ≤ᶠ[l] g) (h₂ : g ≤ᶠ[l] f) :
    f =ᶠ[l] g :=
  h₂.mp $ h₁.mono $ fun x => le_antisymm
#align filter.eventually_le.antisymm Filter.EventuallyLe.antisymm

theorem eventually_le_antisymm_iff [PartialOrder β] {l : Filter α} {f g : α → β} : f =ᶠ[l] g ↔ f ≤ᶠ[l] g ∧ g ≤ᶠ[l] f :=
...
theorem EventuallyLe.le_iff_eq [PartialOrder β] {l : Filter α} {f g : α → β} (h : f ≤ᶠ[l] g) : g ≤ᶠ[l] f ↔ g =ᶠ[l] f :=
...
theorem Eventually.ne_top_of_lt [PartialOrder β] [OrderTop β] {l : Filter α} {f g : α → β} (h : ∀ᶠ x in l, f x < g x) :
    ∀ᶠ x in l, f x ≠ ⊤ :=
  h.mono fun x hx => hx.ne_top
#align filter.eventually.ne_top_of_lt Filter.Eventually.ne_top_of_lt

theorem Eventually.lt_top_of_ne [PartialOrder β] [OrderTop β] {l : Filter α} {f : α → β} (h : ∀ᶠ x in l, f x ≠ ⊤) :
    ∀ᶠ x in l, f x < ⊤ :=
...
theorem EventuallyLe.mul_le_mul [MulZeroClass β] [PartialOrder β] [PosMulMono β] [MulPosMono β] {l : Filter α}
    {f₁ f₂ g₁ g₂ : α → β} (hf : f₁ ≤ᶠ[l] f₂) (hg : g₁ ≤ᶠ[l] g₂) (hg₀ : 0 ≤ᶠ[l] g₁) (hf₀ : 0 ≤ᶠ[l] f₂) :
    f₁ * g₁ ≤ᶠ[l] f₂ * g₂ := by filter_upwards [hf, hg, hg₀, hf₀] with x using mul_le_mul
...
theorem EventuallyLe.mul_le_mul' [Mul β] [Preorder β] [CovariantClass β β (· * ·) (· ≤ ·)]
    ...
    (hg : g₁ ≤ᶠ[l] g₂) : f₁ * g₁ ≤ᶠ[l] f₂ * g₂ := by filter_upwards [hf, hg] with x hfx hgx using mul_le_mul' hfx hgx
#align filter.eventually_le.mul_le_mul' Filter.EventuallyLe.mul_le_mul'

theorem EventuallyLe.mul_nonneg [OrderedSemiring β] {l : Filter α} {f g : α → β} (hf : 0 ≤ᶠ[l] f) (hg : 0 ≤ᶠ[l] g) :
    0 ≤ᶠ[l] f * g := by filter_upwards [hf, hg] with x using mul_nonneg
...
theorem EventuallyLe.sup_le [SemilatticeSup β] {l : Filter α} {f g h : α → β} (hf : f ≤ᶠ[l] h) (hg : g ≤ᶠ[l] h) :
    f ⊔ g ≤ᶠ[l] h := by filter_upwards [hf, hg] with x hfx hgx using sup_le hfx hgx
...
theorem EventuallyLe.le_sup_of_le_left [SemilatticeSup β] {l : Filter α} {f g h : α → β} (hf : h ≤ᶠ[l] f) :
    h ≤ᶠ[l] f ⊔ g := by filter_upwards [hf] with x hfx using le_sup_of_le_left hfx
...
theorem EventuallyLe.le_sup_of_le_right [SemilatticeSup β] {l : Filter α} {f g h : α → β} (hg : h ≤ᶠ[l] g) :
    h ≤ᶠ[l] f ⊔ g := by filter_upwards [hg] with x hgx using le_sup_of_le_right hgx
...
theorem _root_.function.semiconj.filter_map {f : α → β} {ga : α → α} {gb : β → β} (h : Function.Semiconj f ga gb) :
  ...
  map_comm h.comp_eq
#align filter._root_.function.semiconj.filter_map filter._root_.function.semiconj.filter_map

theorem _root_.commute.filter_map {f g : α → α} (h : Function.Commute f g) : Function.Commute (map f) (map g) :=
  h.filterMap
#align filter._root_.commute.filter_map filter._root_.commute.filter_map
...
#align filter._root_.function.semiconj.filter_comap filter._root_.function.semiconj.filter_comap

theorem _root_.commute.filter_comap {f g : α → α} (h : Function.Commute f g) : Function.Commute (comap f) (comap g) :=
  h.filter_comap
#align filter._root_.commute.filter_comap filter._root_.commute.filter_comap
...
@[simp]
theorem comap_pure {b : β} : comap m (pure b) = 𝓟 (m ⁻¹' {b}) := by rw [← principal_singleton, comap_principal]
...
@[simp]
theorem map_top (f : α → β) : map f ⊤ = 𝓟 (range f) := by rw [← principal_univ, map_principal, image_univ]
...
theorem neBotOfComap (h : (comap m g).ne_bot) : g.ne_bot := by
  rw [ne_bot_iff] at *
  contrapose! h
  rw [h]
  exact comap_bot
#align filter.ne_bot_of_comap Filter.neBotOfComap
...
theorem comap_supr {ι} {f : ι → Filter β} {m : α → β} : comap m (supr f) = ⨆ i, comap m (f i) :=
  le_antisymm
    (fun s hs =>
      have : ∀ i, ∃ t, t ∈ f i ∧ m ⁻¹' t ⊆ s := by simpa only [mem_comap, exists_prop, mem_supr] using mem_supr.1 hs
...
instance canLift (c) (p) [CanLift α β c p] :
    CanLift (Filter α) (Filter β) (map c) fun f =>
      ∀ᶠ x : α in f, p x where prf f hf := ⟨comap c f, map_comap_of_mem $ hf.mono CanLift.prf⟩
...
#align filter._root_.function.surjective.filter_map_top filter._root_.function.surjective.filter_map_top
...
theorem comapNeBot {f : Filter β} {m : α → β} (hm : ∀ t ∈ f, ∃ a, m a ∈ t) : NeBot (comap m f) :=
  comap_ne_bot_iff.mpr hm
#align filter.comap_ne_bot Filter.comapNeBot
...
theorem NeBot.comapOfRangeMem {f : Filter β} {m : α → β} (hf : NeBot f) (hm : range m ∈ f) : NeBot (comap m f) :=
  comap_ne_bot_iff_frequently.2 $ Eventually.frequently hm
#align filter.ne_bot.comap_of_range_mem Filter.NeBot.comapOfRangeMem
...
@[instance]
theorem comapFstNeBot [Nonempty β] {f : Filter α} [NeBot f] : (f.comap (Prod.fst : α × β → α)).ne_bot :=
  comap_fst_ne_bot_iff.2 ⟨‹_›, ‹_›⟩
#align filter.comap_fst_ne_bot Filter.comapFstNeBot
...
@[instance]
theorem comapSndNeBot [Nonempty α] {f : Filter β} [NeBot f] : (f.comap (Prod.snd : α × β → β)).ne_bot :=
  comap_snd_ne_bot_iff.2 ⟨‹_›, ‹_›⟩
#align filter.comap_snd_ne_bot Filter.comapSndNeBot
...
@[instance]
theorem comapEvalNeBot {ι : Type _} {α : ι → Type _} [∀ j, Nonempty (α j)] (i : ι) (f : Filter (α i)) [NeBot f] :
    (comap (eval i) f).ne_bot :=
  comap_eval_ne_bot_iff.2 ‹_›
#align filter.comap_eval_ne_bot Filter.comapEvalNeBot

theorem comapInfPrincipalNeBotOfImageMem {f : Filter β} {m : α → β} (hf : NeBot f) {s : Set α} (hs : m '' s ∈ f) :
    NeBot (comap m f ⊓ 𝓟 s) := by
  refine' ⟨compl_compl s ▸ mt mem_of_eq_bot _⟩
  rintro ⟨t, ht, hts⟩
  rcases hf.nonempty_of_mem (inter_mem hs ht) with ⟨_, ⟨x, hxs, rfl⟩, hxt⟩
  exact absurd hxs (hts hxt)
#align filter.comap_inf_principal_ne_bot_of_image_mem Filter.comapInfPrincipalNeBotOfImageMem

theorem comapCoeNeBotOfLePrincipal {s : Set γ} {l : Filter γ} [h : NeBot l] (h' : l ≤ 𝓟 s) :
    NeBot (comap (coe : s → γ) l) :=
  h.comapOfRangeMem $ (@Subtype.range_coe γ s).symm ▸ h' (mem_principal_self s)
#align filter.comap_coe_ne_bot_of_le_principal Filter.comapCoeNeBotOfLePrincipal

theorem NeBot.comapOfSurj {f : Filter β} {m : α → β} (hf : NeBot f) (hm : Surjective m) : NeBot (comap m f) :=
  hf.comapOfRangeMem $ univ_mem' hm
#align filter.ne_bot.comap_of_surj Filter.NeBot.comapOfSurj

theorem NeBot.comapOfImageMem {f : Filter β} {m : α → β} (hf : NeBot f) {s : Set α} (hs : m '' s ∈ f) :
    NeBot (comap m f) :=
  hf.comapOfRangeMem $ mem_of_superset hs (image_subset_range _ _)
#align filter.ne_bot.comap_of_image_mem Filter.NeBot.comapOfImageMem
...
theorem NeBot.ofMap : NeBot (f.map m) → NeBot f :=
  (map_ne_bot_iff m).1
#align filter.ne_bot.of_map Filter.NeBot.ofMap

instance mapNeBot [hf : NeBot f] : NeBot (f.map m) :=
  hf.map m
#align filter.map_ne_bot Filter.mapNeBot
...
theorem inf_principal_eq_bot_iff_comap {F : Filter α} {s : Set α} : F ⊓ 𝓟 s = ⊥ ↔ comap (coe : s → α) F = ⊥ := by
...
instance pureNeBot {α : Type u} {a : α} : NeBot (pure a) :=
  ⟨mt empty_mem_iff_bot.2 $ not_mem_empty a⟩
#align filter.pure_ne_bot Filter.pureNeBot
...
theorem Tendsto.neBot {f : α → β} {x : Filter α} {y : Filter β} (h : Tendsto f x y) [hx : NeBot x] : NeBot y :=
  (hx.map _).mono h
...
theorem tendsto_inf {f : α → β} {x : Filter α} {y₁ y₂ : Filter β} :
    Tendsto f x (y₁ ⊓ y₂) ↔ Tendsto f x y₁ ∧ Tendsto f x y₂ := by simp only [tendsto, le_inf_iff, iff_self_iff]
...
theorem tendsto_infi {f : α → β} {x : Filter α} {y : ι → Filter β} : Tendsto f x (⨅ i, y i) ↔ ∀ i, Tendsto f x (y i) :=
  by simp only [tendsto, iff_self_iff, le_infi_iff]
...
@[simp]
theorem tendsto_principal_principal {f : α → β} {s : Set α} {t : Set β} : Tendsto f (𝓟 s) (𝓟 t) ↔ ∀ a ∈ s, f a ∈ t := by
