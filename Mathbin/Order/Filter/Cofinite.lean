    ...
    :=
      {
        Sets := { s | finite s ᶜ } ,
          ...
          sets_of_superset := fun s t hs : finite s ᶜ st : s ⊆ t => hs.subset $ compl_subset_compl . 2 st ,
          inter_sets
            :=
            fun
              s t hs : finite s ᶜ ht : finite t ᶜ
                =>
...
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
@[ simp ] theorem eventually_cofinite { p : α → Prop } : ∀ᶠ x in cofinite , p x ↔ finite { x | ¬ p x } := Iff.rfl
...
theorem
  ...
  { p : ℕ → Prop } : ∃ᶠ n in at_top , p n ↔ Set.Infinite { n | p n }
  ...
  :=
    by
      rcases em ∃ ( y : _ ) ( _ : y ∈ s ) , ∃ x , f y < x with ( ⟨ y , hys , x , hx ⟩ | not_all_top )
        ...
        · pushNeg at not_all_top obtain ⟨ a₀ , ha₀s ⟩ := hs exact ⟨ a₀ , ha₀s , fun a ha => not_all_top a ha f a₀ ⟩
