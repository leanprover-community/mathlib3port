...
theorem comap_lift_eq {m : γ → β} (hg : Monotone g) : comap m (f.lift g) = f.lift (comap m ∘ g) :=
...
theorem lift_assoc {h : Set β → Filter γ} (hg : Monotone g) : (f.lift g).lift h = f.lift fun s => (g s).lift h :=
  le_antisymmₓ
    (le_infi$
      fun s =>
        le_infi$
          fun hs => le_infi$ fun t => le_infi$ fun ht => infi_le_of_le t$ infi_le _$ (mem_lift_sets hg).mpr ⟨_, hs, ht⟩)
    (le_infi$
      fun t =>
        le_infi$
          fun ht =>
...
@[simp]
theorem lift_inf {f : Filter α} {g h : Set α → Filter β} : (f.lift fun x => g x⊓h x) = f.lift g⊓f.lift h :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (t «expr ∈ » infi f)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (s «expr ∈ » f)
theorem tendsto_lift' {m : γ → β} {l : Filter γ} : tendsto m l (f.lift' h) ↔ ∀ s _ : s ∈ f, ∀ᶠ a in l, m a ∈ h s :=
  by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (t «expr ∈ » f)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (y «expr ∈ » h t)
theorem eventually_lift'_iff (hh : Monotone h) {p : β → Prop} :
  (∀ᶠ y in f.lift' h, p y) ↔ ∃ (t : _)(_ : t ∈ f), ∀ y _ : y ∈ h t, p y :=
  mem_lift'_sets hh
...
theorem map_lift'_eq {m : β → γ} (hh : Monotone h) : map m (f.lift' h) = f.lift' (image m ∘ h) :=
  calc map m (f.lift' h) = f.lift (map m ∘ 𝓟 ∘ h) := map_lift_eq$ monotone_principal.comp hh 
    _ = f.lift' (image m ∘ h) :=
    by 
...
theorem comap_lift'_eq {m : γ → β} (hh : Monotone h) : comap m (f.lift' h) = f.lift' (preimage m ∘ h) :=
  calc comap m (f.lift' h) = f.lift (comap m ∘ 𝓟 ∘ h) := comap_lift_eq$ monotone_principal.comp hh 
    _ = f.lift' (preimage m ∘ h) :=
    by 
...
theorem lift'_pure {a : α} (hh : Monotone h) : (pure a : Filter α).lift' h = 𝓟 (h {a}) :=
  by 
    rw [←principal_singleton, lift'_principal hh]

theorem lift'_bot (hh : Monotone h) : (⊥ : Filter α).lift' h = 𝓟 (h ∅) :=
  by 
...
theorem monotone_lift' [Preorderₓ γ] {f : γ → Filter α} {g : γ → Set α → Set β} (hf : Monotone f) (hg : Monotone g) :
  Monotone fun c => (f c).lift' (g c) :=
  fun a b h => lift'_mono (hf h) (hg h)

theorem lift_lift'_assoc {g : Set α → Set β} {h : Set β → Filter γ} (hg : Monotone g) (hh : Monotone h) :
  (f.lift' g).lift h = f.lift fun s => h (g s) :=
  calc (f.lift' g).lift h = f.lift fun s => (𝓟 (g s)).lift h := lift_assoc (monotone_principal.comp hg)
    _ = f.lift fun s => h (g s) :=
    by 
...
theorem lift'_inf_principal_eq {h : Set α → Set β} {s : Set β} : f.lift' h⊓𝓟 s = f.lift' fun t => h t ∩ s :=
  by 
...
theorem eventually_lift'_powerset {f : Filter α} {p : Set α → Prop} :
  (∀ᶠ s in f.lift' powerset, p s) ↔ ∃ (s : _)(_ : s ∈ f), ∀ t _ : t ⊆ s, p t :=
  eventually_lift'_iff monotone_powerset

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (s «expr ∈ » f)
theorem eventually_lift'_powerset' {f : Filter α} {p : Set α → Prop} (hp : ∀ ⦃s t⦄, s ⊆ t → p t → p s) :
  (∀ᶠ s in f.lift' powerset, p s) ↔ ∃ (s : _)(_ : s ∈ f), p s :=
...
theorem tendsto_lift'_powerset_mono {la : Filter α} {lb : Filter β} {s t : α → Set β}
  (ht : tendsto t la (lb.lift' powerset)) (hst : ∀ᶠ x in la, s x ⊆ t x) : tendsto s la (lb.lift' powerset) :=
  by 
...
theorem eventually_lift'_powerset_forall {f : Filter α} {p : α → Prop} :
  (∀ᶠ s in f.lift' powerset, ∀ x _ : x ∈ s, p x) ↔ ∀ᶠ x in f, p x :=
...
theorem prod_same_eq : f ×ᶠ f = f.lift' fun t => Set.Prod t t :=
  by 
    rw [prod_def] <;>
      exact
        lift_lift'_same_eq_lift' (fun s => Set.monotone_prod monotone_const monotone_id)
          fun t => Set.monotone_prod monotone_id monotone_const

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (t «expr ∈ » f)
theorem mem_prod_same_iff {s : Set (α × α)} : s ∈ f ×ᶠ f ↔ ∃ (t : _)(_ : t ∈ f), Set.Prod t t ⊆ s :=
  by 
    rw [prod_same_eq, mem_lift'_sets] <;> exact Set.monotone_prod monotone_id monotone_id

-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (W «expr ∈ » y)
...
theorem prod_lift'_lift' {f₁ : Filter α₁} {f₂ : Filter α₂} {g₁ : Set α₁ → Set β₁} {g₂ : Set α₂ → Set β₂}
  ...
  f₁.lift' g₁ ×ᶠ f₂.lift' g₂ = f₁.lift fun s => f₂.lift' fun t => (g₁ s).Prod (g₂ t) :=
  by 
