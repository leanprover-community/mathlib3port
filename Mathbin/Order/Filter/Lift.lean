...
theorem HasBasis.mem_lift_iff {ι} {p : ι → Prop} {s : ι → Set α} {f : Filter α} (hf : f.HasBasis p s) {β : ι → Type _}
    ...
    (gm : Monotoneₓ g) {s : Set γ} : s ∈ f.lift g ↔ ∃ (i : ι)(hi : p i)(x : β i)(hx : pg i x), sg i x ⊆ s := by
...
theorem HasBasis.lift {ι} {p : ι → Prop} {s : ι → Set α} {f : Filter α} (hf : f.HasBasis p s) {β : ι → Type _}
    {pg : ∀ i, β i → Prop} {sg : ∀ i, β i → Set γ} {g : Set α → Filter γ} (hg : ∀ i, (g <| s i).HasBasis (pg i) (sg i))
    (gm : Monotoneₓ g) : (f.lift g).HasBasis (fun i : Σi, β i => p i.1 ∧ pg i.1 i.2) fun i : Σi, β i => sg i.1 i.2 := by
  refine' ⟨fun t => (hf.mem_lift_iff hg gm).trans _⟩
  simp [Sigma.exists, and_assocₓ, exists_and_distrib_leftₓ]

theorem mem_lift_sets (hg : Monotoneₓ g) {s : Set β} : s ∈ f.lift g ↔ ∃ t ∈ f, s ∈ g t :=
  (f.basis_sets.mem_lift_iff (fun s => (g s).basis_sets) hg).trans <| by simp only [id, exists_mem_subset_iff]

theorem sInter_lift_sets (hg : Monotoneₓ g) : ⋂₀ { s | s ∈ f.lift g } = ⋂ s ∈ f, ⋂₀ { t | t ∈ g s } := by
  simp only [sInter_eq_bInter, mem_set_of_eq, Filter.mem_sets, mem_lift_sets hg, Inter_exists, @Inter_comm _ (Set β)]
...
theorem map_lift_eq {m : β → γ} (hg : Monotoneₓ g) : map m (f.lift g) = f.lift (map m ∘ g) :=
  have : Monotoneₓ (map m ∘ g) := map_mono.comp hg
  Filter.ext fun s => by simp only [mem_lift_sets hg, mem_lift_sets this, exists_propₓ, mem_map, Function.comp_app]

theorem comap_lift_eq {m : γ → β} : comap m (f.lift g) = f.lift (comap m ∘ g) := by simp only [Filter.lift, comap_infi]

theorem comap_lift_eq2 {m : β → α} {g : Set β → Filter γ} (hg : Monotoneₓ g) :
...
theorem map_lift_eq2 {g : Set β → Filter γ} {m : α → β} (hg : Monotoneₓ g) : (map m f).lift g = f.lift (g ∘ Image m) :=
  le_antisymmₓ
    (infi_mono' fun s =>
      ⟨Image m s, infi_mono' fun hs => ⟨(f.sets_of_superset hs) fun a h => mem_image_of_mem _ h, le_rflₓ⟩⟩)
    (infi_mono' fun t =>
      ⟨Preimage m t,
        infi_mono' fun ht =>
          ⟨ht,
...
theorem lift_assoc {h : Set β → Filter γ} (hg : Monotoneₓ g) : (f.lift g).lift h = f.lift fun s => (g s).lift h :=
  le_antisymmₓ
    (le_infi fun s =>
      le_infi fun hs =>
...
theorem lift_lift_same_le_lift {g : Set α → Set α → Filter β} :
  ...
  le_lift.2 fun s hs => lift_le hs <| lift_le hs le_rflₓ

theorem lift_lift_same_eq_lift {g : Set α → Set α → Filter β} (hg₁ : ∀ s, Monotoneₓ fun t => g s t)
    (hg₂ : ∀ t, Monotoneₓ fun s => g s t) : (f.lift fun s => f.lift (g s)) = f.lift fun s => g s s :=
...
theorem lift_principal {s : Set α} (hg : Monotoneₓ g) : (𝓟 s).lift g = g s :=
  (lift_le (mem_principal_self _) le_rflₓ).antisymm (le_lift.2 fun t ht => hg ht)

theorem monotone_lift [Preorderₓ γ] {f : γ → Filter α} {g : γ → Set α → Filter β} (hf : Monotoneₓ f)
    (hg : Monotoneₓ g) : Monotoneₓ fun c => (f c).lift (g c) := fun a b h => lift_mono (hf h) (hg h)

theorem lift_ne_bot_iff (hm : Monotoneₓ g) : (ne_bot <| f.lift g) ↔ ∀ s ∈ f, NeBot (g s) := by
...
theorem mem_lift' {t : Set α} (ht : t ∈ f) : h t ∈ f.lift' h :=
  le_principal_iff.mp <| show f.lift' h ≤ 𝓟 (h t) from infi_le_of_le t <| infi_le_of_le ht <| le_rflₓ
...
theorem HasBasis.lift' {ι} {p : ι → Prop} {s} (hf : f.HasBasis p s) (hh : Monotoneₓ h) :
...
theorem mem_lift'_sets (hh : Monotoneₓ h) {s : Set β} : s ∈ f.lift' h ↔ ∃ t ∈ f, h t ⊆ s :=
  mem_lift_sets <| monotone_principal.comp hh

theorem eventually_lift'_iff (hh : Monotoneₓ h) {p : β → Prop} : (∀ᶠ y in f.lift' h, p y) ↔ ∃ t ∈ f, ∀ y ∈ h t, p y :=
  mem_lift'_sets hh

theorem sInter_lift'_sets (hh : Monotoneₓ h) : ⋂₀ { s | s ∈ f.lift' h } = ⋂ s ∈ f, h s :=
...
theorem map_lift'_eq {m : β → γ} (hh : Monotoneₓ h) : map m (f.lift' h) = f.lift' (Image m ∘ h) :=
  calc
...
theorem map_lift'_eq2 {g : Set β → Set γ} {m : α → β} (hg : Monotoneₓ g) : (map m f).lift' g = f.lift' (g ∘ Image m) :=
...
theorem comap_lift'_eq2 {m : β → α} {g : Set β → Set γ} (hg : Monotoneₓ g) :
    (comap m f).lift' g = f.lift' (g ∘ Preimage m) :=
  comap_lift_eq2 <| monotone_principal.comp hg

theorem lift'_principal {s : Set α} (hh : Monotoneₓ h) : (𝓟 s).lift' h = 𝓟 (h s) :=
  lift_principal <| monotone_principal.comp hh

theorem lift'_pure {a : α} (hh : Monotoneₓ h) : (pure a : Filter α).lift' h = 𝓟 (h {a}) := by
  rw [← principal_singleton, lift'_principal hh]

theorem lift'_bot (hh : Monotoneₓ h) : (⊥ : Filter α).lift' h = 𝓟 (h ∅) := by rw [← principal_empty, lift'_principal hh]

theorem le_lift' {f : Filter α} {h : Set α → Set β} {g : Filter β} : g ≤ f.lift' h ↔ ∀ s ∈ f, h s ∈ g :=
  le_lift.trans <| forall₂_congrₓ fun s hs => le_principal_iff

theorem principal_le_lift' {t : Set β} : 𝓟 t ≤ f.lift' h ↔ ∀ s ∈ f, t ⊆ h s :=
  le_lift'

theorem monotone_lift' [Preorderₓ γ] {f : γ → Filter α} {g : γ → Set α → Set β} (hf : Monotoneₓ f) (hg : Monotoneₓ g) :
    Monotoneₓ fun c => (f c).lift' (g c) := fun a b h => lift'_mono (hf h) (hg h)

theorem lift_lift'_assoc {g : Set α → Set β} {h : Set β → Filter γ} (hg : Monotoneₓ g) (hh : Monotoneₓ h) :
...
theorem lift'_lift'_assoc {g : Set α → Set β} {h : Set β → Set γ} (hg : Monotoneₓ g) (hh : Monotoneₓ h) :
    (f.lift' g).lift' h = f.lift' fun s => h (g s) :=
  lift_lift'_assoc hg (monotone_principal.comp hh)

theorem lift'_lift_assoc {g : Set α → Filter β} {h : Set β → Set γ} (hg : Monotoneₓ g) :
...
theorem lift_lift'_same_eq_lift' {g : Set α → Set α → Set β} (hg₁ : ∀ s, Monotoneₓ fun t => g s t)
    (hg₂ : ∀ t, Monotoneₓ fun s => g s t) : (f.lift fun s => f.lift' (g s)) = f.lift' fun s => g s s :=
...
theorem lift'_ne_bot_iff (hh : Monotoneₓ h) : NeBot (f.lift' h) ↔ ∀ s ∈ f, (h s).Nonempty :=
  calc
...
theorem prod_same_eq : f ×ᶠ f = f.lift' fun t : Set α => t ×ˢ t :=
  prod_def.trans <|
    lift_lift'_same_eq_lift' (fun s => monotone_constₓ.set_prod monotone_idₓ) fun t =>
      monotone_idₓ.set_prod monotone_constₓ

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem mem_prod_same_iff {s : Set (α × α)} : s ∈ f ×ᶠ f ↔ ∃ t ∈ f, t ×ˢ t ⊆ s := by
  rw [prod_same_eq, mem_lift'_sets]
  exact monotone_id.set_prod monotone_idₓ
