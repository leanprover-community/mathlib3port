...
theorem
  ...
  [ Encodable ι ] { p : α → ι → Prop } : ∀ᶠ x in l , ∀ i , p x i ↔ ∀ i , ∀ᶠ x in l , p x i
  :=
    by
...
theorem
  ...
  { S : Set ι } ( hS : countable S ) { p : ∀ x : α i _ : i ∈ S , Prop }
    : ∀ᶠ x in l , ∀ i _ : i ∈ S , p x i ‹ _ › ↔ ∀ i _ : i ∈ S , ∀ᶠ x in l , p x i ‹ _ ›
  :=
    by
      simpa
        only
        [ Filter.Eventually , set_of_forall ]
        using @ countable_bInter_mem _ _ l _ _ hS fun i hi => { x | p x i hi }
