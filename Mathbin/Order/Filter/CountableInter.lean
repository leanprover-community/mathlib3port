...
class CountableInterFilter (l : Filter α) : Prop where
  countable_sInter_mem' : ∀ {S : Set (Set α)} hSc : Countable S hS : ∀, ∀ s ∈ S, ∀, s ∈ l, ⋂₀ S ∈ l

variable {l : Filter α} [CountableInterFilter l]

theorem countable_sInter_mem {S : Set (Set α)} (hSc : Countable S) : ⋂₀ S ∈ l ↔ ∀, ∀ s ∈ S, ∀, s ∈ l :=
...
def Filter.ofCountableInter (l : Set (Set α)) (hp : ∀ S : Set (Set α), Countable S → S ⊆ l → ⋂₀ S ∈ l)
...
instance Filter.countable_Inter_of_countable_Inter (l : Set (Set α))
    (hp : ∀ S : Set (Set α), Countable S → S ⊆ l → ⋂₀ S ∈ l) (h_mono : ∀ s t, s ∈ l → s ⊆ t → t ∈ l) :
    CountableInterFilter (Filter.ofCountableInter l hp h_mono) :=
  ⟨hp⟩

@[simp]
theorem Filter.mem_of_countable_Inter {l : Set (Set α)} (hp : ∀ S : Set (Set α), Countable S → S ⊆ l → ⋂₀ S ∈ l)
