...
theorem eventually_countable_ball {ι : Type _} {S : Set ι} (hS : S.Countable) {p : ∀ (x : α), ∀ i ∈ S, Prop} :
  ...
  simpa only [Filter.Eventually, set_of_forall] using @countable_bInter_mem _ l _ _ _ hS fun i hi => { x | p x i hi }
...
theorem EventuallyLe.countable_bUnion {ι : Type _} {S : Set ι} (hS : S.Countable) {s t : ∀ i ∈ S, Set α}
...
theorem EventuallyLe.countable_bInter {ι : Type _} {S : Set ι} (hS : S.Countable) {s t : ∀ i ∈ S, Set α}
    (h : ∀ i ∈ S, s i ‹_› ≤ᶠ[l] t i ‹_›) : (⋂ i ∈ S, s i ‹_›) ≤ᶠ[l] ⋂ i ∈ S, t i ‹_› := by
