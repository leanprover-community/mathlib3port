...
protected def FilterBasis {p : ι → Prop} {s : ι → Set α} (h : is_basis p s) : FilterBasis α where
  Sets := { t | ∃ i, p i ∧ s i = t }
  Nonempty :=
    let ⟨i, hi⟩ := h.nonempty
    ⟨s i, ⟨i, hi, rfl⟩⟩
...
protected theorem mem_filter_iff (h : is_basis p s) {U : Set α} : U ∈ h.filter ↔ ∃ i, p i ∧ s i ⊆ U := by
...
theorem filter_eq_generate (h : is_basis p s) : h.filter = generate { U | ∃ i, p i ∧ s i = U } := by
...
theorem of_sets_filter_eq_generate (s : Set (Set α)) : (filter_basis.of_sets s).filter = generate s := by
...
theorem has_basis.to_has_basis' (hl : l.has_basis p s) (h : ∀ i, p i → ∃ i', p' i' ∧ s' i' ⊆ s i)
    (h' : ∀ i', p' i' → s' i' ∈ l) : l.has_basis p' s' := by
...
theorem has_basis.frequently_iff (hl : l.has_basis p s) {q : α → Prop} : (∃ᶠ x in l, q x) ↔ ∀ i, p i → ∃ x ∈ s i, q x :=
  by
...
theorem has_basis_self {l : Filter α} {P : Set α → Prop} :
    has_basis l (fun s => s ∈ l ∧ P s) id ↔ ∀, ∀ t ∈ l, ∀, ∃ r ∈ l, P r ∧ r ⊆ t := by
...
theorem has_basis.restrict (h : l.has_basis p s) {q : ι → Prop} (hq : ∀ i, p i → ∃ j, p j ∧ q j ∧ s j ⊆ s i) :
    l.has_basis (fun i => p i ∧ q i) s := by
...
theorem has_basis.sup_pure (hl : l.has_basis p s) (x : α) : (l⊔pure x).HasBasis p fun i => s i ∪ {x} := by
...
theorem inf_ne_bot_iff_frequently_right {f g : Filter α} :
    ne_bot (f⊓g) ↔ ∀ {p : α → Prop}, (∀ᶠ x in g, p x) → ∃ᶠ x in f, p x := by
  rw [inf_comm]
  exact inf_ne_bot_iff_frequently_left

theorem has_basis.eq_binfi (h : l.has_basis p s) : l = ⨅ (i) (_ : p i), 𝓟 (s i) :=
  eq_binfi_of_mem_iff_exists_mem $ fun t => by
    simp only [h.mem_iff, mem_principal]

theorem has_basis.eq_infi (h : l.has_basis (fun _ => True) s) : l = ⨅ i, 𝓟 (s i) := by
...
theorem has_basis.tendsto_left_iff (hla : la.has_basis pa sa) :
    tendsto f la lb ↔ ∀, ∀ t ∈ lb, ∀, ∃ (i : _)(hi : pa i), maps_to f (sa i) t := by
...
theorem tendsto.basis_left (H : tendsto f la lb) (hla : la.has_basis pa sa) :
    ∀, ∀ t ∈ lb, ∀, ∃ (i : _)(hi : pa i), maps_to f (sa i) t :=
...
theorem has_basis.exists_antitone_subbasis {f : Filter α} [h : f.is_countably_generated] {p : ι → Prop} {s : ι → Set α}
    (hs : f.has_basis p s) : ∃ x : ℕ → ι, (∀ i, p (x i)) ∧ f.has_antitone_basis fun i => s (x i) := by
...
@[instance]
theorem is_countably_generated_seq [Encodable β] (x : β → Set α) : is_countably_generated (⨅ i, 𝓟 $ x i) := by
...
theorem is_countably_generated_iff_exists_antitone_basis {f : Filter α} :
    is_countably_generated f ↔ ∃ x : ℕ → Set α, f.has_antitone_basis x := by
  constructor
  · intro h
