...
theorem frequently_modeq {n : ℕ} (h : n ≠ 0) (d : ℕ) : ∃ᶠm in at_top, m ≡ d [MOD n] :=
...
theorem frequently_mod_eq {d n : ℕ} (h : d < n) : ∃ᶠm in at_top, m % n = d :=
  by 
    simpa only [Nat.Modeq, mod_eq_of_lt h] using frequently_modeq h.ne_bot d

theorem frequently_even : ∃ᶠm : ℕ in at_top, Even m :=
  by 
    simpa only [even_iff] using frequently_mod_eq zero_lt_two

theorem frequently_odd : ∃ᶠm : ℕ in at_top, Odd m :=
  by 
