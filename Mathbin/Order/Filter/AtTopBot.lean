...
def at_top [Preorderₓ α] : Filter α :=
  ⨅a, 𝓟 (Ici a)
...
theorem at_top_basis' [SemilatticeSup α] (a : α) : (@at_top α _).HasBasis (fun x => a ≤ x) Ici :=
  ⟨fun t =>
      (@at_top_basis α ⟨a⟩ _).mem_iff.trans
        ⟨fun ⟨x, _, hx⟩ => ⟨x⊔a, le_sup_right, fun y hy => hx (le_transₓ le_sup_left hy)⟩,
          fun ⟨x, _, hx⟩ => ⟨x, trivialₓ, hx⟩⟩⟩
...
theorem eventually_le_at_bot [Preorderₓ α] (a : α) : ∀ᶠx in at_bot, x ≤ a :=
  mem_at_bot a

theorem eventually_gt_at_top [Preorderₓ α] [NoTopOrder α] (a : α) : ∀ᶠx in at_top, a < x :=
  Ioi_mem_at_top a

theorem eventually_lt_at_bot [Preorderₓ α] [NoBotOrder α] (a : α) : ∀ᶠx in at_bot, x < a :=
  Iio_mem_at_bot a

theorem at_top_basis_Ioi [Nonempty α] [SemilatticeSup α] [NoTopOrder α] : (@at_top α _).HasBasis (fun _ => True) Ioi :=
  (at_top_basis.to_has_basis fun a ha => ⟨a, ha, Ioi_subset_Ici_self⟩)$
    fun a ha => (no_top a).imp$ fun b hb => ⟨ha, Ici_subset_Ioi.2 hb⟩
...
theorem frequently.forall_exists_of_at_top [SemilatticeSup α] [Nonempty α] {p : α → Prop} (h : ∃ᶠx in at_top, p x) :
  ∀ a, ∃ (b : _)(_ : b ≥ a), p b :=
  frequently_at_top.mp h

theorem frequently.forall_exists_of_at_bot [SemilatticeInf α] [Nonempty α] {p : α → Prop} (h : ∃ᶠx in at_bot, p x) :
  ∀ a, ∃ (b : _)(_ : b ≤ a), p b :=
  frequently_at_bot.mp h

theorem map_at_top_eq [Nonempty α] [SemilatticeSup α] {f : α → β} : at_top.map f = ⨅a, 𝓟$ f '' { a' | a ≤ a' } :=
  (at_top_basis.map _).eq_infi

theorem map_at_bot_eq [Nonempty α] [SemilatticeInf α] {f : α → β} : at_bot.map f = ⨅a, 𝓟$ f '' { a' | a' ≤ a } :=
  @map_at_top_eq (OrderDual α) _ _ _ _

theorem tendsto_at_top [Preorderₓ β] {m : α → β} {f : Filter α} : tendsto m f at_top ↔ ∀ b, ∀ᶠa in f, b ≤ m a :=
  by 
    simp only [at_top, tendsto_infi, tendsto_principal, mem_Ici]

theorem tendsto_at_bot [Preorderₓ β] {m : α → β} {f : Filter α} : tendsto m f at_bot ↔ ∀ b, ∀ᶠa in f, m a ≤ b :=
...
theorem extraction_of_frequently_at_top {P : ℕ → Prop} (h : ∃ᶠn in at_top, P n) :
  ∃ φ : ℕ → ℕ, StrictMono φ ∧ ∀ n, P (φ n) :=
  by 
    rw [frequently_at_top'] at h 
    exact extraction_of_frequently_at_top' h

theorem extraction_of_eventually_at_top {P : ℕ → Prop} (h : ∀ᶠn in at_top, P n) :
  ∃ φ : ℕ → ℕ, StrictMono φ ∧ ∀ n, P (φ n) :=
  extraction_of_frequently_at_top h.frequently

theorem extraction_forall_of_frequently {P : ℕ → ℕ → Prop} (h : ∀ n, ∃ᶠk in at_top, P n k) :
...
theorem extraction_forall_of_eventually {P : ℕ → ℕ → Prop} (h : ∀ n, ∀ᶠk in at_top, P n k) :
  ∃ φ : ℕ → ℕ, StrictMono φ ∧ ∀ n, P n (φ n) :=
...
theorem exists_lt_of_tendsto_at_top [SemilatticeSup α] [Preorderₓ β] [NoTopOrder β] {u : α → β}
  (h : tendsto u at_top at_top) (a : α) (b : β) : ∃ (a' : _)(_ : a' ≥ a), b < u a' :=
  by 
...
theorem tendsto_at_bot_add_nonpos_left' (hf : ∀ᶠx in l, f x ≤ 0) (hg : tendsto g l at_bot) :
...
theorem tendsto_at_top_add_nonneg_right' (hf : tendsto f l at_top) (hg : ∀ᶠx in l, 0 ≤ g x) :
  tendsto (fun x => f x+g x) l at_top :=
  tendsto_at_top_mono' l (monotone_mem (fun x => le_add_of_nonneg_right) hg) hf

theorem tendsto_at_bot_add_nonpos_right' (hf : tendsto f l at_bot) (hg : ∀ᶠx in l, g x ≤ 0) :
  tendsto (fun x => f x+g x) l at_bot :=
...
theorem tendsto_at_top_of_add_bdd_above_left' C (hC : ∀ᶠx in l, f x ≤ C) (h : tendsto (fun x => f x+g x) l at_top) :
  tendsto g l at_top :=
  tendsto_at_top_of_add_const_left C (tendsto_at_top_mono' l (hC.mono fun x hx => add_le_add_right hx (g x)) h)

theorem tendsto_at_bot_of_add_bdd_below_left' C (hC : ∀ᶠx in l, C ≤ f x) (h : tendsto (fun x => f x+g x) l at_bot) :
  tendsto g l at_bot :=
...
theorem tendsto_at_top_of_add_bdd_above_right' C (hC : ∀ᶠx in l, g x ≤ C) (h : tendsto (fun x => f x+g x) l at_top) :
  tendsto f l at_top :=
  tendsto_at_top_of_add_const_right C (tendsto_at_top_mono' l (hC.mono fun x hx => add_le_add_left hx (f x)) h)

theorem tendsto_at_bot_of_add_bdd_below_right' C (hC : ∀ᶠx in l, C ≤ g x) (h : tendsto (fun x => f x+g x) l at_bot) :
  tendsto f l at_bot :=
...
theorem tendsto_at_bot_add_left_of_ge' (C : β) (hf : ∀ᶠx in l, f x ≤ C) (hg : tendsto g l at_bot) :
  tendsto (fun x => f x+g x) l at_bot :=
...
theorem tendsto_at_top_add_right_of_le' (C : β) (hf : tendsto f l at_top) (hg : ∀ᶠx in l, C ≤ g x) :
  tendsto (fun x => f x+g x) l at_top :=
...
theorem tendsto_at_bot_add_right_of_ge' (C : β) (hf : tendsto f l at_bot) (hg : ∀ᶠx in l, g x ≤ C) :
  tendsto (fun x => f x+g x) l at_bot :=
...
theorem tendsto_neg_at_top_at_bot : tendsto (Neg.neg : β → β) at_top at_bot :=
  by 
...
theorem nonneg_of_eventually_pow_nonneg [LinearOrderedRing α] {a : α} (h : ∀ᶠn in at_top, 0 ≤ a ^ (n : ℕ)) : 0 ≤ a :=
...
theorem tendsto_at_top_embedding [Preorderₓ β] [Preorderₓ γ] {f : α → β} {e : β → γ} {l : Filter α}
  (hm : ∀ b₁ b₂, e b₁ ≤ e b₂ ↔ b₁ ≤ b₂) (hu : ∀ c, ∃ b, c ≤ e b) : tendsto (e ∘ f) l at_top ↔ tendsto f l at_top :=
  by 
    refine' ⟨_, (tendsto_at_top_at_top_of_monotone (fun b₁ b₂ => (hm b₁ b₂).2) hu).comp⟩
...
theorem at_top_finset_eq_infi : (at_top : Filter$ Finset α) = ⨅x : α, 𝓟 (Ici {x}) :=
  by 
...
theorem eventually_at_bot_prod_self [SemilatticeInf α] [Nonempty α] {p : α × α → Prop} :
  (∀ᶠx in at_bot, p x) ↔ ∃ a, ∀ k l, k ≤ a → l ≤ a → p (k, l) :=
  by 
...
theorem eventually_at_top_prod_self [SemilatticeSup α] [Nonempty α] {p : α × α → Prop} :
  (∀ᶠx in at_top, p x) ↔ ∃ a, ∀ k l, a ≤ k → a ≤ l → p (k, l) :=
  by 
...
@[simp]
theorem map_coe_Ioi_at_top [SemilatticeSup α] [NoTopOrder α] (a : α) : map (coeₓ : Ioi a → α) at_top = at_top :=
  by 
...
order. -/
theorem at_top_Ioi_eq [SemilatticeSup α] (a : α) : at_top = comap (coeₓ : Ioi a → α) at_top :=
  by 
    nontriviality 
...
order. -/
theorem at_top_Ici_eq [SemilatticeSup α] (a : α) : at_top = comap (coeₓ : Ici a → α) at_top :=
  by 
...
theorem tendsto_iff_seq_tendsto {f : α → β} {k : Filter α} {l : Filter β} [k.is_countably_generated] :
  ...
  by 
    ...
    split 
    ·
      exact (at_top_basis.tendsto_iff gbasis).2 fun i _ => ⟨i, trivialₓ, fun j hj => gmon trivialₓ trivialₓ hj (h j).1⟩
    ·
...
theorem exists_le_mul_self (a : R) : ∃ (x : _)(_ : x ≥ 0), a ≤ x*x :=
...
@[simp]
theorem map_at_top (e : α ≃o β) : map (e : α → β) at_top = at_top :=
  by 
...
theorem tendsto_at_top_iff {l : Filter γ} {f : γ → α} (e : α ≃o β) :
  tendsto (fun x => e (f x)) l at_top ↔ tendsto f l at_top :=
  by 
