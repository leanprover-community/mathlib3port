...
theorem disjointAtBotPrincipalIoi [Preorder α] (x : α) : Disjoint atBot (𝓟 (IoiCat x)) :=
  disjointOfDisjointOfMem (iicDisjointIoi le_rfl) (Iic_mem_at_bot x) (mem_principal_self _)

theorem disjointAtTopPrincipalIio [Preorder α] (x : α) : Disjoint atTop (𝓟 (IioCat x)) :=
  @disjointAtBotPrincipalIoi αᵒᵈ _ _

theorem disjointAtTopPrincipalIic [Preorder α] [NoMaxOrder α] (x : α) : Disjoint atTop (𝓟 (IicCat x)) :=
  disjointOfDisjointOfMem (iicDisjointIoi le_rfl).symm (Ioi_mem_at_top x) (mem_principal_self _)

theorem disjointAtBotPrincipalIci [Preorder α] [NoMinOrder α] (x : α) : Disjoint atBot (𝓟 (IciCat x)) :=
...
theorem not_tendsto_const_at_top [Preorder α] [NoMaxOrder α] (x : α) (l : Filter β) [l.ne_bot] :
  ...
  tendsto_const_pure.not_tendsto (disjointPureAtTop x)

theorem not_tendsto_const_at_bot [Preorder α] [NoMinOrder α] (x : α) (l : Filter β) [l.ne_bot] :
    ¬Tendsto (fun _ => x) l atBot :=
  tendsto_const_pure.not_tendsto (disjointPureAtBot x)

theorem disjointAtBotAtTop [PartialOrder α] [Nontrivial α] : Disjoint (atBot : Filter α) atTop := by
...
theorem disjointAtTopAtBot [PartialOrder α] [Nontrivial α] : Disjoint (atTop : Filter α) atBot :=
  disjointAtBotAtTop.symm
...
theorem not_tendsto_pow_at_top_at_bot [LinearOrderedRing α] : ∀ {n : ℕ}, ¬Tendsto (fun x : α => x ^ n) atTop atBot
  ...
  | n + 1 => (tendsto_pow_at_top n.succ_ne_zero).not_tendsto disjointAtTopAtBot
