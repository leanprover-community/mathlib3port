...
variable [HasScalar α β] {f f₁ f₂ : Filter α} {g g₁ g₂ h : Filter β} {s : Set α} {t : Set β} {a : α} {b : β}

/-- The filter `f • g` is generated by `{s • t | s ∈ f, t ∈ g}` in locale `pointwise`. -/
@[to_additive Filter.hasVadd "The filter `f +ᵥ g` is generated by `{s +ᵥ t | s ∈ f, t ∈ g}` in locale `pointwise`."]
protected def hasScalar : HasScalar (Filter α) (Filter β) :=
  ⟨/- This is defeq to `map₂ (•) f g`, but the hypothesis unfolds to `t₁ • t₂ ⊆ s` rather than all the
  way to `set.image2 (•) t₁ t₂ ⊆ s`. -/
  fun f g => { map₂ (· • ·) f g with Sets := { s | ∃ t₁ t₂, t₁ ∈ f ∧ t₂ ∈ g ∧ t₁ • t₂ ⊆ s } }⟩

localized [Pointwise] attribute [instance] Filter.hasScalar Filter.hasVadd
...
variable [HasScalar α β] {f f₁ f₂ : Filter β} {s : Set β} {a : α}
...
protected def hasScalarFilter : HasScalar α (Filter β) :=
  ⟨fun a => map ((· • ·) a)⟩

localized [Pointwise] attribute [instance] Filter.hasScalarFilter Filter.hasVaddFilter
...
@[to_additive]
instance smul_comm_class_filter [HasScalar α γ] [HasScalar β γ] [SmulCommClass α β γ] :
