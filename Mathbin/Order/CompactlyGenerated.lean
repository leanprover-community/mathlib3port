...
def IsSupFiniteCompact : Prop :=
  ∀ s : Set α, ∃ t : Finsetₓ α, ↑t ⊆ s ∧ sup s = t.sup id
...
def IsCompactElement {α : Type _} [CompleteLattice α] (k : α) :=
  ∀ s : Set α, k ≤ sup s → ∃ t : Finsetₓ α, ↑t ⊆ s ∧ k ≤ t.sup id

theorem is_compact_element_iff.{u} {α : Type u} [CompleteLattice α] (k : α) :
    CompleteLattice.IsCompactElement k ↔ ∀ (ι : Type u) (s : ι → α), k ≤ supr s → ∃ t : Finsetₓ ι, k ≤ t.sup s := by
  classical
  constructor
  · intro H ι s hs
    ...
    · rw [Finsetₓ.sup_le_iff]
      intro b hb
      rw [← show s (f ⟨b, hb⟩) = id b from hf _]
      exact Finsetₓ.le_sup (Finsetₓ.mem_image_of_mem f <| Finsetₓ.mem_univ ⟨b, hb⟩)
  ...
  · intro H s hs
    ...
    rw [Finsetₓ.sup_le_iff]
    exact fun x hx => @Finsetₓ.le_sup _ _ _ _ _ id _ (Finsetₓ.mem_image_of_mem coe hx)
...
theorem is_compact_element_iff_le_of_directed_Sup_le (k : α) :
  ...
  · intro hk s hne hdir hsup
    ...
    have t_below_s : ∀ x ∈ t, ∃ y ∈ s, x ≤ y := fun x hxt => ⟨x, ht.left hxt, le_rflₓ⟩
    obtain ⟨x, ⟨hxs, hsupx⟩⟩ := Finsetₓ.sup_le_of_le_directed s hne hdir t t_below_s
    exact ⟨x, ⟨hxs, le_transₓ ht.right hsupx⟩⟩
    
  · intro hk s hsup
    -- Consider the set of finite joins of elements of the (plain) set s.
    let S : Set α := { x | ∃ t : Finsetₓ α, ↑t ⊆ s ∧ x = t.sup id }
    ...
    have dir_US : DirectedOn (· ≤ ·) S := by
      ...
      · use c ∪ d
        constructor
        · simp only [hc.left, hd.left, Set.union_subset_iff, Finsetₓ.coe_union, and_selfₓ]
          
        · simp only [hc.right, hd.right, Finsetₓ.sup_union]
    ...
    have sup_S : Sup s ≤ Sup S := by
      ...
      intro x hx
      use {x}
      simpa only [and_trueₓ, id.def, Finsetₓ.coe_singleton, eq_self_iff_true, Finsetₓ.sup_singleton,
        Set.singleton_subset_iff]
    have Sne : S.nonempty := by
      suffices : ⊥ ∈ S
      exact Set.nonempty_of_mem this
      use ∅
      simp only [Set.empty_subset, Finsetₓ.coe_empty, Finsetₓ.sup_empty, eq_self_iff_true, and_selfₓ]
...
theorem IsCompactElement.exists_finset_of_le_supr {k : α} (hk : IsCompactElement k) {ι : Type _} (f : ι → α)
    (h : k ≤ ⨆ i, f i) : ∃ s : Finsetₓ ι, k ≤ ⨆ i ∈ s, f i := by
  classical
  let g : Finsetₓ ι → α := fun s => ⨆ i ∈ s, f i
  have h1 : DirectedOn (· ≤ ·) (Set.Range g) := by
    rintro - ⟨s, rfl⟩ - ⟨t, rfl⟩
    exact
      ⟨g (s ∪ t), ⟨s ∪ t, rfl⟩, supr_le_supr_of_subset (Finsetₓ.subset_union_left s t),
        supr_le_supr_of_subset (Finsetₓ.subset_union_right s t)⟩
  have h2 : k ≤ Sup (Set.Range g) :=
    h.trans
      (supr_le fun i =>
        le_Sup_of_le ⟨{i}, rfl⟩ (le_supr_of_le i (le_supr_of_le (Finsetₓ.mem_singleton_self i) le_rflₓ)))
  obtain ⟨-, ⟨s, rfl⟩, hs⟩ :=
...
theorem finset_sup_compact_of_compact {α β : Type _} [CompleteLattice α] {f : β → α} (s : Finsetₓ β)
    (h : ∀ x ∈ s, IsCompactElement (f x)) : IsCompactElement (s.sup f) := by
  classical
  rw [is_compact_element_iff_le_of_directed_Sup_le]
  intro d hemp hdir hsup
  change f with id ∘ f
  rw [← Finsetₓ.sup_finset_image]
  apply Finsetₓ.sup_le_of_le_directed d hemp hdir
  rintro x hx
  obtain ⟨p, ⟨hps, rfl⟩⟩ := finset.mem_image.mp hx
  specialize h p hps
  rw [is_compact_element_iff_le_of_directed_Sup_le] at h
  specialize h d hemp hdir (le_transₓ (Finsetₓ.le_sup hps) hsup)
  simpa only [exists_propₓ]

theorem WellFounded.is_Sup_finite_compact (h : WellFounded ((· > ·) : α → α → Prop)) : IsSupFiniteCompact α := by
  intro s
  let p : Set α := { x | ∃ t : Finsetₓ α, ↑t ⊆ s ∧ t.sup id = x }
  ...
  · apply Sup_le
    ...
    have hm' : m ≤ (insert y t).sup id := by
      rw [← ht₂]
      exact Finsetₓ.sup_mono (t.subset_insert y)
    rw [← hm _ hy' hm']
    simp
    
  · rw [← ht₂, Finsetₓ.sup_id_eq_Sup]
    exact Sup_le_Sup ht₁
...
theorem is_Sup_finite_compact_iff_all_elements_compact : IsSupFiniteCompact α ↔ ∀ k : α, IsCompactElement k := by
  ...
  · obtain ⟨t, ⟨hts, htsup⟩⟩ := h (Sup s) s (by rfl)
    have : Sup s = t.sup id := by
      suffices t.sup id ≤ Sup s by apply le_antisymmₓ <;> assumption
      simp only [id.def, Finsetₓ.sup_le_iff]
      intro x hx
      exact le_Sup (hts hx)
    use t, hts, this
...
theorem WellFounded.finite_of_set_independent (h : WellFounded ((· > ·) : α → α → Prop)) {s : Set α}
  ...
  · have : (s \ (insert ⊥ t : Finsetₓ α)).Infinite := contra.diff (Finsetₓ.finite_to_set _)
...
theorem inf_Sup_eq_supr_inf_sup_finset : a ⊓ sup s = ⨆ (t : Finsetₓ α) (H : ↑t ⊆ s), a ⊓ t.sup id :=
  le_antisymmₓ
    (by
      rw [le_iff_compact_le_imp]
      intro c hc hcinf
    ...
    (supr_le fun t => supr_le fun h => inf_le_inf_left _ ((Finsetₓ.sup_id_eq_Sup t).symm ▸ Sup_le_Sup h))

theorem CompleteLattice.set_independent_iff_finite {s : Set α} :
    CompleteLattice.SetIndependent s ↔ ∀ t : Finsetₓ α, ↑t ⊆ s → CompleteLattice.SetIndependent (↑t : Set α) :=
  ⟨fun hs t ht => hs.mono ht, fun h a ha => by
    rw [disjoint_iff, inf_Sup_eq_supr_inf_sup_finset, supr_eq_bot]
    intro t
    rw [supr_eq_bot, Finsetₓ.sup_id_eq_Sup]
    intro ht
    classical
    have h' := (h (insert a t) _ (t.mem_insert_self a)).eq_bot
    · rwa [Finsetₓ.coe_insert, Set.insert_diff_self_of_not_mem] at h'
      exact fun con => ((Set.mem_diff a).1 (ht con)).2 (Set.mem_singleton a)
      
    · rw [Finsetₓ.coe_insert, Set.insert_subset]
...
instance (priority := 100) is_atomistic_of_complemented_lattice [ComplementedLattice α] : IsAtomistic α :=
  ⟨fun b =>
    ⟨{ a | IsAtom a ∧ a ≤ b }, by
      symm
