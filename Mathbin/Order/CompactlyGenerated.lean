...
theorem is_compact_element_iff_le_of_directed_Sup_le (k : α) :
    IsCompactElement k ↔
      ∀ s : Set α, s.Nonempty → DirectedOn (· ≤ ·) s → k ≤ sup s → ∃ x : α, x ∈ s ∧ k ≤ x :=
  by classical
  constructor
...
theorem IsCompactElement.exists_finset_of_le_supr {k : α} (hk : IsCompactElement k) {ι : Type _}
  ...
  have h2 : k ≤ Sup (Set.range g) :=
    h.trans
      (supr_le fun i =>
        le_Sup_of_le ⟨{i}, rfl⟩
          (le_supr_of_le i (le_supr_of_le (Finset.mem_singleton_self i) le_rfl)))
  obtain ⟨-, ⟨s, rfl⟩, hs⟩ :=
...
#align
  complete_lattice.well_founded.is_Sup_finite_compact CompleteLattice.WellFounded.isSupFiniteCompact
...
theorem inf_Sup_eq_supr_inf_sup_finset : a ⊓ sup s = ⨆ (t : Finset α) (H : ↑t ⊆ s), a ⊓ t.sup id :=
  le_antisymm
    ...
    (supr_le fun t =>
      supr_le fun h => inf_le_inf_left _ ((Finset.sup_id_eq_Sup t).symm ▸ Sup_le_Sup h))
...
theorem CompleteLattice.setIndependentUnionOfDirected {η : Type _} {s : η → Set α}
  ...
  · skip
    ...
    exact
      (h i).mono (Set.Subset.trans hI <| Set.Union₂_subset fun j hj => hi j (fi.mem_to_finset.2 hj))
...
#align
  complete_lattice.set_independent_Union_of_directed CompleteLattice.setIndependentUnionOfDirected
...
theorem CompleteLattice.independentSUnionOfDirected {s : Set (Set α)} (hs : DirectedOn (· ⊆ ·) s)
  ...
  rw [Set.sUnion_eq_Union] <;>
    exact CompleteLattice.setIndependentUnionOfDirected hs.directed_coe (by simpa using h)
...
instance (priority := 100) isAtomisticOfComplementedLattice [ComplementedLattice α] :
    IsAtomistic α :=
  ⟨fun b =>
    ⟨{ a | IsAtom a ∧ a ≤ b }, by
      symm
...
theorem complementedLatticeOfSupAtomsEqTop (h : sup { a : α | IsAtom a } = ⊤) :
  ...
  ⟨fun b => by
    ...
    · intro c hc1 hc2
      refine'
        ⟨⋃₀c,
          ⟨CompleteLattice.independentSUnionOfDirected hc2.directed_on fun s hs => (hc1 hs).1, _,
            fun a ha => _⟩,
