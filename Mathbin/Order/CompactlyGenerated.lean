...
theorem complementedLatticeOfSupAtomsEqTop (h : sup { a : α | IsAtom a } = ⊤) : ComplementedLattice α :=
  ⟨fun b => by
    ...
    · refine' ⟨Sup s, le_of_eq b_inf_Sup_s, h.symm.trans_le <| Sup_le_iff.2 fun a ha => _⟩
      rw [← inf_eq_left]
      ...
      · rw [Set.mem_union, Set.mem_singleton_iff] at hx
        ...
        · have h : (s ∪ {a}) \ {x} = s \ {x} ∪ {a} := by
          ...
          apply (s_ind (hx.resolve_right xa)).disjointSupRightOfDisjointSupLeft (a_dis_Sup_s.mono_right _).symm
