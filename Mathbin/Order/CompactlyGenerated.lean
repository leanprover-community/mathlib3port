...
/- ./././Mathport/Syntax/Translate/Basic.lean:610:2: warning: expanding binder collection (a b «expr ∈ » s) -/
...
theorem IsCompactElement.exists_finset_of_le_supr {k : α} (hk : IsCompactElement k) {ι : Type _} (f : ι → α)
  ...
  classical let g : Finset ι → α := fun s => ⨆ i ∈ s, f i
    · rintro - ⟨s, rfl⟩ - ⟨t, rfl⟩
      exact
    ...
    · exact
        h.trans
          (supr_le fun i =>
            le_Sup_of_le ⟨{i}, rfl⟩ (le_supr_of_le i (le_supr_of_le (Finset.mem_singleton_self i) le_rfl)))
...
theorem finsetSupCompactOfCompact {α β : Type _} [CompleteLattice α] {f : β → α} (s : Finset β)
    (h : ∀ x ∈ s, IsCompactElement (f x)) : IsCompactElement (s.sup f) := by
  classical rw [is_compact_element_iff_le_of_directed_Sup_le]
    change f with id ∘ f
    apply Finset.sup_le_of_le_directed d hemp hdir
    obtain ⟨p, ⟨hps, rfl⟩⟩ := finset.mem_image.mp hx
    rw [is_compact_element_iff_le_of_directed_Sup_le] at h
    simpa only [exists_prop]
#align complete_lattice.finset_sup_compact_of_compact CompleteLattice.finsetSupCompactOfCompact

theorem WellFounded.isSupFiniteCompact (h : WellFounded ((· > ·) : α → α → Prop)) : IsSupFiniteCompact α := by
  intro s
  ...
  · apply Sup_le
    intro y hy
    classical have hy' : (insert y t).sup id ∈ p
...
theorem IsSupFiniteCompact.isSupClosedCompact (h : IsSupFiniteCompact α) : IsSupClosedCompact α := by
  intro s hne hsc
  obtain ⟨t, ht₁, ht₂⟩ := h s
  clear h
...
#align
  complete_lattice.is_Sup_finite_compact.is_sup_closed_compact CompleteLattice.IsSupFiniteCompact.isSupClosedCompact
         ...
         (Tactic.tacticSeq1Indented
          [(Tactic.tfaeHave "tfae_have" [] (num "1") "→" (num "2"))
           ...
           («tactic___;_»
            ...
            [(group (Tactic.exact "exact" (Term.app `well_founded.is_Sup_finite_compact [`α])) [])])
           []
           (Tactic.tfaeHave "tfae_have" [] (num "2") "→" (num "3"))
           []
           («tactic___;_»
            (cdotTk (patternIgnore (token.«·» "·")))
            [(group (Tactic.exact "exact" (Term.app `is_Sup_finite_compact.is_sup_closed_compact [`α])) [])])
           []
           (Tactic.tfaeHave "tfae_have" [] (num "3") "→" (num "1"))
           []
           («tactic___;_»
            (cdotTk (patternIgnore (token.«·» "·")))
            [(group (Tactic.exact "exact" (Term.app `is_sup_closed_compact.well_founded [`α])) [])])
           []
           (Tactic.tfaeHave "tfae_have" [] (num "2") "↔" (num "4"))
           []
           («tactic___;_»
            (cdotTk (patternIgnore (token.«·» "·")))
            [(group (Tactic.exact "exact" (Term.app `is_Sup_finite_compact_iff_all_elements_compact [`α])) [])])
           []
...
theorem WellFounded.finite_of_set_independent (h : WellFounded ((· > ·) : α → α → Prop)) {s : Set α}
  ...
  classical refine' set.not_infinite.mp fun contra => _
...
theorem WellFounded.finite_of_independent (hwf : WellFounded ((· > ·) : α → α → Prop)) {ι : Type _} {t : ι → α}
    (ht : Independent t) (h_ne_bot : ∀ i, t i ≠ ⊥) : Finite ι :=
...
theorem Sup_compact_le_eq (b) : sup { c : α | CompleteLattice.IsCompactElement c ∧ c ≤ b } = b := by
  ...
  exact le_antisymm (Sup_le fun c hc => hc.2) (Sup_le_Sup fun c cs => ⟨hs c cs, le_Sup cs⟩)
...
theorem le_iff_compact_le_imp {a b : α} : a ≤ b ↔ ∀ c : α, CompleteLattice.IsCompactElement c → c ≤ a → c ≤ b :=
  ⟨fun ab c hc ca => le_trans ca ab, fun h => by
...
theorem CompleteLattice.set_independent_iff_finite {s : Set α} :
  ...
  ⟨fun hs t ht => hs.mono ht, fun h a ha => by
    ...
    intro ht
    classical have h' := (h (insert a t) _ (t.mem_insert_self a)).eq_bot
...
theorem CompleteLattice.setIndependentUnionOfDirected {η : Type _} {s : η → Set α} (hs : Directed (· ⊆ ·) s)
...
#align complete_lattice.set_independent_Union_of_directed CompleteLattice.setIndependentUnionOfDirected

theorem CompleteLattice.independentSUnionOfDirected {s : Set (Set α)} (hs : DirectedOn (· ⊆ ·) s)
    (h : ∀ a ∈ s, CompleteLattice.SetIndependent a) : CompleteLattice.SetIndependent (⋃₀s) := by
  rw [Set.sUnion_eq_Union] <;> exact CompleteLattice.setIndependentUnionOfDirected hs.directed_coe (by simpa using h)
#align complete_lattice.independent_sUnion_of_directed CompleteLattice.independentSUnionOfDirected

end

namespace CompleteLattice

theorem compactlyGeneratedOfWellFounded (h : WellFounded ((· > ·) : α → α → Prop)) : IsCompactlyGenerated α := by
...
#align complete_lattice.compactly_generated_of_well_founded CompleteLattice.compactlyGeneratedOfWellFounded
...
instance (priority := 100) isAtomisticOfComplementedLattice [ComplementedLattice α] : IsAtomistic α :=
  ⟨fun b =>
    ⟨{ a | IsAtom a ∧ a ≤ b }, by
      symm
      ...
      · apply ha.1
        ...
        ,
      fun _ => And.left⟩⟩
#align is_atomistic_of_complemented_lattice isAtomisticOfComplementedLattice

/-- See Theorem 6.6, Călugăreanu -/
theorem complementedLatticeOfSupAtomsEqTop (h : sup { a : α | IsAtom a } = ⊤) : ComplementedLattice α :=
  ⟨fun b => by
    ...
    · refine'
        ⟨Sup s, disjoint_iff.mpr b_inf_Sup_s,
          codisjoint_iff_le_sup.mpr <| h.symm.trans_le <| Sup_le_iff.2 fun a ha => _⟩
      rw [← inf_eq_left]
    ...
    · intro c hc1 hc2
      refine'
        ⟨⋃₀c, ⟨CompleteLattice.independentSUnionOfDirected hc2.directed_on fun s hs => (hc1 hs).1, _, fun a ha => _⟩,
      ...
      ⟩
#align complemented_lattice_of_Sup_atoms_eq_top complementedLatticeOfSupAtomsEqTop

/-- See Theorem 6.6, Călugăreanu -/
theorem complementedLatticeOfIsAtomistic [IsAtomistic α] : ComplementedLattice α :=
  complementedLatticeOfSupAtomsEqTop Sup_atoms_eq_top
#align complemented_lattice_of_is_atomistic complementedLatticeOfIsAtomistic

theorem complemented_lattice_iff_is_atomistic : ComplementedLattice α ↔ IsAtomistic α := by
  constructor <;> intros
  · exact isAtomisticOfComplementedLattice
    
  · exact complementedLatticeOfIsAtomistic
