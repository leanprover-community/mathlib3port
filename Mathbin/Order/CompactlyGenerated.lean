...
-- error in Order.CompactlyGenerated: ././Mathport/Syntax/Translate/Basic.lean:340:40: in by_contradiction: ././Mathport/Syntax/Translate/Tactic/Basic.lean:41:45: missing argument
...
theorem well_founded.is_Sup_finite_compact (h : WellFounded (· > · : α → α → Prop)) : is_Sup_finite_compact α :=
  by 
    intro s 
    let p : Set α := { x | ∃ t : Finset α, «expr↑ » t ⊆ s ∧ t.sup id = x }
    have hp : p.nonempty
    ·
      use ⊥, ∅
      simp 
...
theorem CompleteLattice.set_independent_iff_finite {s : Set α} :
  CompleteLattice.SetIndependent s ↔
    ∀ t : Finset α, «expr↑ » t ⊆ s → CompleteLattice.SetIndependent («expr↑ » t : Set α) :=
