...
[PrettyPrinter.parenthesize.input] (Command.declaration
 (Command.declModifiers
  [(Command.docComment "/--" " The supremum of a bounded, well-founded order -/")]
  []
 ...
 (Command.def
  ...
  (Command.optDeclSig
   [(Term.implicitBinder "{" [`α] [] "}")
    ...
    (Term.explicitBinder "(" [`wf] [":" (Term.app `WellFounded [`r])] [] ")")
    (Term.explicitBinder "(" [`s] [":" (Term.app `Set [`α])] [] ")")
    (Term.explicitBinder "(" [`h] [":" (Term.app `Bounded [`r `s])] [] ")")]
   [(Term.typeSpec ":" `α)])
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  ...
  (Command.declSig
   ...
   (Term.typeSpec ":" (Term.app `r [`x (Term.app `wf.sup [`s `h])])))
  (Command.declValSimple
   ":="
   (Term.app
    `min_mem
  ...
  (Command.declValSimple
   ...
   (termDepIfThenElse
    ...
    («term∃_,_»
     ...
     (Term.app `r [`x `y]))
    "then"
    (Term.app `wf.min [(Set.«term{_|_}» "{" `y "|" (Term.app `r [`x `y]) "}") `h])
    "else"
    `x)
   [])
  []
