...
instance (priority := 100)is_maximal.is_prime [is_maximal I] : is_prime I :=
  by 
    ...
    have hyJ : y ∈ «expr↑ » J := set.eq_univ_iff_forall.mp hJuniv y 
...
theorem is_prime_of_mem_or_compl_mem [is_proper I] (h : ∀ {x : P}, x ∈ I ∨ «expr ᶜ» x ∈ I) : is_prime I :=
  by 
    simp only [is_prime_iff_mem_or_mem, or_iff_not_imp_left]
    intro x y hxy hxI 
    have hxcI : «expr ᶜ» x ∈ I := h.resolve_left hxI 
    have ass : x⊓y⊔y⊓«expr ᶜ» x ∈ I := sup_mem _ _ hxy (mem_of_le I inf_le_right hxcI)
