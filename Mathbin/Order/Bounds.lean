...
theorem not_bdd_above_iff' : ¬BddAbove s ↔ ∀ x, ∃ (y : _)(_ : y ∈ s), ¬y ≤ x :=
  by 
...
theorem is_lub_le_iff (h : IsLub s a) : a ≤ b ↔ b ∈ UpperBounds s :=
  by 
    rw [h.upper_bounds_eq]
    rfl

theorem le_is_glb_iff (h : IsGlb s a) : b ≤ a ↔ b ∈ LowerBounds s :=
  by 
...
theorem BddAbove.union [SemilatticeSup γ] {s t : Set γ} : BddAbove s → BddAbove t → BddAbove (s ∪ t) :=
  by 
    rintro ⟨bs, hs⟩ ⟨bt, ht⟩
    use bs⊔bt 
...
theorem IsLub.union [SemilatticeSup γ] {a b : γ} {s t : Set γ} (hs : IsLub s a) (ht : IsLub t b) :
  IsLub (s ∪ t) (a⊔b) :=
  ⟨fun c h => h.cases_on (fun h => le_sup_of_le_left$ hs.left h) fun h => le_sup_of_le_right$ ht.left h,
    fun c hc => sup_le (hs.right$ fun d hd => hc$ Or.inl hd) (ht.right$ fun d hd => hc$ Or.inr hd)⟩
...
theorem IsLeast.union [LinearOrderₓ γ] {a b : γ} {s t : Set γ} (ha : IsLeast s a) (hb : IsLeast t b) :
  IsLeast (s ∪ t) (min a b) :=
  ⟨by 
      cases' le_totalₓ a b with h h <;> simp [h, ha.1, hb.1],
...
theorem is_lub_Ico {a b : γ} (hab : a < b) : IsLub (Ico a b) b :=
  by 
...
theorem bdd_below_bdd_above_iff_subset_Icc : BddBelow s ∧ BddAbove s ↔ ∃ a b, s ⊆ Icc a b :=
  by 
...
@[simp]
theorem OrderTop.upper_bounds_univ [PartialOrderₓ γ] [OrderTop γ] : UpperBounds (univ : Set γ) = {⊤} :=
  by 
...
@[simp]
theorem not_bdd_above_univ [NoTopOrder α] : ¬BddAbove (univ : Set α) :=
  by 
    simp [BddAbove]
...
@[simp]
theorem upper_bounds_empty : UpperBounds (∅ : Set α) = univ :=
  by 
...
@[simp]
theorem bdd_above_empty [Nonempty α] : BddAbove (∅ : Set α) :=
  by 
    simp only [BddAbove, upper_bounds_empty, univ_nonempty]

@[simp]
theorem bdd_below_empty [Nonempty α] : BddBelow (∅ : Set α) :=
  by 
    simp only [BddBelow, lower_bounds_empty, univ_nonempty]

theorem is_glb_empty [Preorderₓ γ] [OrderTop γ] : IsGlb ∅ (⊤ : γ) :=
  by 
...
@[simp]
theorem bdd_above_insert [SemilatticeSup γ] (a : γ) {s : Set γ} : BddAbove (insert a s) ↔ BddAbove s :=
  by 
...
@[simp]
theorem bdd_below_insert [SemilatticeInf γ] (a : γ) {s : Set γ} : BddBelow (insert a s) ↔ BddBelow s :=
  by 
...
theorem IsLub.insert [SemilatticeSup γ] a {b} {s : Set γ} (hs : IsLub s b) : IsLub (insert a s) (a⊔b) :=
  by 
    rw [insert_eq]
    exact is_lub_singleton.union hs

theorem IsGlb.insert [SemilatticeInf γ] a {b} {s : Set γ} (hs : IsGlb s b) : IsGlb (insert a s) (a⊓b) :=
  by 
    rw [insert_eq]
...
theorem IsLeast.insert [LinearOrderₓ γ] a {b} {s : Set γ} (hs : IsLeast s b) : IsLeast (insert a s) (min a b) :=
  by 
    rw [insert_eq]
    exact is_least_singleton.union hs

@[simp]
theorem upper_bounds_insert (a : α) (s : Set α) : UpperBounds (insert a s) = Ici a ∩ UpperBounds s :=
  by 
    rw [insert_eq, upper_bounds_union, upper_bounds_singleton]

@[simp]
theorem lower_bounds_insert (a : α) (s : Set α) : LowerBounds (insert a s) = Iic a ∩ LowerBounds s :=
  by 
...
theorem is_glb_lt_is_lub_of_ne (Ha : IsGlb s a) (Hb : IsLub s b) {x y} (Hx : x ∈ s) (Hy : y ∈ s) (Hxy : x ≠ y) :
  ...
  lt_iff_le_not_leₓ.2
    ⟨lower_bounds_le_upper_bounds Ha.1 Hb.1 ⟨x, Hx⟩,
      fun hab => Hxy$ Set.subsingleton_of_is_lub_le_is_glb Ha Hb hab Hx Hy⟩
...
theorem image_upper_bounds_subset_upper_bounds_image (hf : Monotone f) : f '' UpperBounds s ⊆ UpperBounds (f '' s) :=
  by 
    rintro _ ⟨a, ha, rfl⟩
...
theorem IsGlb.of_image [Preorderₓ α] [Preorderₓ β] {f : α → β} (hf : ∀ {x y}, f x ≤ f y ↔ x ≤ y) {s : Set α} {x : α}
  ...
  ⟨fun y hy => hf.1$ hx.1$ mem_image_of_mem _ hy,
    fun y hy => hf.1$ hx.2$ Monotone.mem_lower_bounds_image (fun x y => hf.2) hy⟩
...
theorem is_lub_pi {π : α → Type _} [∀ a, Preorderₓ (π a)] {s : Set (∀ a, π a)} {f : ∀ a, π a} :
  IsLub s f ↔ ∀ a, IsLub (Function.eval a '' s) (f a) :=
  by 
    classical 
...
theorem is_lub_image {s : Set α} {x : β} : IsLub (f '' s) x ↔ IsLub s (f.symm x) :=
  ⟨fun h => IsLub.of_image (fun _ _ => f.le_iff_le) ((f.apply_symm_apply x).symm ▸ h),
    fun h => (IsLub.of_image fun _ _ => f.symm.le_iff_le)$ (f.symm_image_image s).symm ▸ h⟩

theorem is_lub_image' {s : Set α} {x : α} : IsLub (f '' s) (f x) ↔ IsLub s x :=
  by 
...
@[simp]
theorem is_lub_preimage {s : Set β} {x : α} : IsLub (f ⁻¹' s) x ↔ IsLub s (f x) :=
  by 
    rw [←f.symm_symm, ←image_eq_preimage, is_lub_image]

theorem is_lub_preimage' {s : Set β} {x : β} : IsLub (f ⁻¹' s) (f.symm x) ↔ IsLub s x :=
  by 
