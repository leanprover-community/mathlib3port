...
@[simp]
theorem symm_diff_self : a Δ a = ⊥ :=
  by 
    rw [· Δ ·, sup_idem, sdiff_self]

@[simp]
theorem symm_diff_bot : a Δ ⊥ = a :=
  by 
    rw [· Δ ·, sdiff_bot, bot_sdiff, sup_bot_eq]

@[simp]
theorem bot_symm_diff : ⊥ Δ a = a :=
  by 
    rw [symm_diff_comm, symm_diff_bot]

theorem symm_diff_eq_sup_sdiff_inf : a Δ b = (a⊔b) \ (a⊓b) :=
  by 
    simp [sup_sdiff, sdiff_inf, sup_comm, · Δ ·]

theorem disjoint_symm_diff_inf : Disjoint (a Δ b) (a⊓b) :=
  by 
    rw [symm_diff_eq_sup_sdiff_inf]
    exact disjoint_sdiff_self_left

theorem symm_diff_le_sup : a Δ b ≤ a⊔b :=
  by 
    rw [symm_diff_eq_sup_sdiff_inf]
    exact sdiff_le
...
theorem sdiff_symm_diff' : c \ a Δ b = c⊓a⊓b⊔c \ (a⊔b) :=
  by 
    rw [sdiff_symm_diff, sdiff_sup, sup_comm]

theorem symm_diff_sdiff : a Δ b \ c = a \ (b⊔c)⊔b \ (a⊔c) :=
  by 
    rw [symm_diff_def, sup_sdiff, sdiff_sdiff_left, sdiff_sdiff_left]

@[simp]
theorem symm_diff_sdiff_left : a Δ b \ a = b \ a :=
  by 
    rw [symm_diff_def, sup_sdiff, sdiff_idem, sdiff_sdiff_self, bot_sup_eq]

@[simp]
theorem symm_diff_sdiff_right : a Δ b \ b = a \ b :=
  by 
    rw [symm_diff_comm, symm_diff_sdiff_left]

@[simp]
theorem sdiff_symm_diff_self : a \ a Δ b = a⊓b :=
  by 
...
theorem symm_diff_assoc : a Δ b Δ c = a Δ (b Δ c) :=
  by 
...
@[simp]
theorem symm_diff_symm_diff_self : a Δ (a Δ b) = b :=
  by 
    simp [←symm_diff_assoc]

@[simp]
theorem symm_diff_symm_diff_self' : a Δ b Δ a = b :=
  by 
...
theorem symm_diff_eq_top_iff : a Δ b = ⊤ ↔ IsCompl a b :=
  by 
