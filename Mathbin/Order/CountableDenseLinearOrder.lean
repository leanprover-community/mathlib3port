...
theorem exists_between_finsets {α : Type _} [LinearOrderₓ α] [DenselyOrdered α] [NoBotOrder α] [NoTopOrder α]
  ...
  if nlo : lo.nonempty then
    if nhi : hi.nonempty then
      ...
      Exists.elim (no_top (Finset.max' lo nlo)) fun m hm =>
        ⟨m, fun x hx => lt_of_le_of_ltₓ (Finset.le_max' lo x hx) hm, fun y hy => (nhi ⟨y, hy⟩).elim⟩
  else
    if nhi : hi.nonempty then
      Exists.elim (no_bot (Finset.min' hi nhi)) fun m hm =>
...
-- ././Mathport/Syntax/Translate/Basic.lean:477:2: warning: expanding binder collection (p q «expr ∈ » f)
...
protected def comm : partial_iso α β → partial_iso β α :=
  Subtype.map (Finset.image (Equivₓ.prodComm _ _)) $ fun f hf p q hp hq =>
    Eq.symm $
