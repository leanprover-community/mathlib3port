...
noncomputable def HasLines.hasPoints [HasLines P L] [Fintype P] [Fintype L] (h : Fintype.card P = Fintype.card L) :
  ...
  let this : ∀ l₁ l₂ : L, l₁ ≠ l₂ → ∃ p : P, p ∈ l₁ ∧ p ∈ l₂ := fun l₁ l₂ hl => by
    ...
    haveI := fintype.one_lt_card_iff_nontrivial.mp ((congr_argₓ _ h).mpr Fintype.one_lt_card)
...
theorem line_count_eq_line_count [Finite P] [Finite L] (p q : P) : lineCount L p = lineCount L q := by
  ...
  rwa [(eq_or_eq h₂ h₂₂ h₃ h₂₃).resolve_right fun h => h₃₃ ((congr_argₓ (Membership.Mem p₃) h).mp h₃₂)]
...
theorem card_points [Fintype P] [Finite L] : Fintype.card P = order P L ^ 2 + order P L + 1 := by
  ...
  have h1 : Fintype.card { q // q ≠ p } + 1 = Fintype.card P := by
    ...
    convert (Fintype.card_subtype_compl _).trans (congr_argₓ _ (Fintype.card_subtype_eq p))
