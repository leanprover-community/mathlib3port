...
theorem Nondegenerate.exists_injective_of_card_le [Nondegenerate P L] [Fintype P] [Fintype L]
  ...
  classical let t : L → Finset P := fun l => Set.toFinset { p | p ∉ l }
...
theorem sum_line_count_eq_sum_point_count [Fintype P] [Fintype L] :
  ...
  classical simp only [line_count, point_count, Nat.card_eq_fintype_card, ← Fintype.card_sigma]
    calc
...
#align configuration.has_lines.point_count_le_line_count Configuration.HasLines.point_count_le_line_count
...
#align configuration.has_points.line_count_le_point_count Configuration.HasPoints.line_count_le_point_count
      ...
      calc
        ...
        _ = ∑ p in finset.univ.image f, line_count L p :=
          Finset.sum_bij (fun l hl => f l) (fun l hl => Finset.mem_image_of_mem f hl) (fun l hl => rfl)
            (fun l₁ l₂ hl₁ hl₂ hl₃ => hf₁ hl₃) fun p => by simp_rw [Finset.mem_image, eq_comm, imp_self]
...
theorem HasLines.exists_bijective_of_card_eq [HasLines P L] [Fintype P] [Fintype L]
  ...
  classical obtain ⟨f, hf1, hf2⟩ := nondegenerate.exists_injective_of_card_le (ge_of_eq h)
    refine'
      ⟨f, hf3, fun l =>
...
theorem HasLines.line_count_eq_point_count [HasLines P L] [Fintype P] [Fintype L]
  ...
  classical obtain ⟨f, hf1, hf2⟩ := has_lines.exists_bijective_of_card_eq hPL
...
#align configuration.has_lines.line_count_eq_point_count Configuration.HasLines.line_count_eq_point_count
...
#align configuration.has_points.line_count_eq_point_count Configuration.HasPoints.line_count_eq_point_count
...
noncomputable def HasLines.hasPoints [HasLines P L] [Fintype P] [Fintype L] (h : Fintype.card P = Fintype.card L) :
  ...
  let this : ∀ l₁ l₂ : L, l₁ ≠ l₂ → ∃ p : P, p ∈ l₁ ∧ p ∈ l₂ := fun l₁ l₂ hl => by
    classical obtain ⟨f, hf1, hf2⟩ := has_lines.exists_bijective_of_card_eq h
...
#align configuration.projective_plane.line_count_eq_line_count Configuration.ProjectivePlane.line_count_eq_line_count
...
#align configuration.projective_plane.line_count_eq_point_count Configuration.ProjectivePlane.line_count_eq_point_count
...
theorem line_count_eq [Finite P] [Finite L] (p : P) : lineCount L p = order P L + 1 := by
  classical obtain ⟨q, -, -, l, -, -, -, -, h, -⟩ := Classical.choose_spec (@exists_config P L _ _)
...
theorem one_lt_order [Finite P] [Finite L] : 1 < order P L := by
  ...
  classical cases nonempty_fintype { p : P // p ∈ l₂ }
...
#align configuration.projective_plane.two_lt_line_count Configuration.ProjectivePlane.two_lt_line_count
...
#align configuration.projective_plane.two_lt_point_count Configuration.ProjectivePlane.two_lt_point_count
    ...
    { toFun := fun q => ⟨⟨mk_line q.2, (mk_line_ax q.2).2⟩, q, (mk_line_ax q.2).1, q.2⟩,
      ...
      right_inv := fun lq =>
        Sigma.subtype_ext
          ...
          rfl }
  classical have h1 : Fintype.card { q // q ≠ p } + 1 = Fintype.card P
