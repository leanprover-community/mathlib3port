...
instance [this : Fintypeₓ P] : Fintypeₓ (Dual P) :=
  this
...
theorem Nondegenerate.exists_injective_of_card_le [Nondegenerate P L] [Fintypeₓ P] [Fintypeₓ L]
    (h : Fintypeₓ.card L ≤ Fintypeₓ.card P) : ∃ f : L → P, Function.Injective f ∧ ∀ l, f l ∉ l := by
  classical
  let t : L → Finsetₓ P := fun l => Set.toFinset { p | p ∉ l }
  suffices ∀ s : Finsetₓ L, s.card ≤ (s.bUnion t).card by
    -- Hall's marriage theorem
    obtain ⟨f, hf1, hf2⟩ := (Finsetₓ.all_card_le_bUnion_card_iff_exists_injective t).mp this
  ...
  · obtain ⟨l, rfl⟩ := finset.card_eq_one.mp hs₁
    ...
    rw [Finsetₓ.card_singleton, Finsetₓ.singleton_bUnion, Nat.one_le_iff_ne_zero]
    exact Finsetₓ.card_ne_zero_of_mem (set.mem_to_finset.mpr hl)
    
  suffices s.bUnion tᶜ.card ≤ sᶜ.card by
    -- Rephrase in terms of complements (uses `h`)
    rw [Finsetₓ.card_compl, Finsetₓ.card_compl, tsub_le_iff_left] at this
  ...
  have hs₂ : s.bUnion tᶜ.card ≤ 1 := by
    ...
    simp_rw [Finsetₓ.mem_compl, Finsetₓ.mem_bUnion, exists_propₓ, not_exists, not_and, Set.mem_to_finset,
  ...
  · rw [hs₃, le_zero_iff]
    rw [Finsetₓ.card_compl, tsub_eq_zero_iff_le, LE.le.le_iff_eqₓ (Finsetₓ.card_le_univ _), eq_comm,
      Finsetₓ.card_eq_iff_eq_univ] at hs₃⊢
    rw [hs₃]
    rw [Finsetₓ.eq_univ_iff_forall] at hs₃⊢
    exact fun p =>
      Exists.elim (exists_line p)-- If `s = univ`, then show `s.bUnion t = univ`
      fun l hl => finset.mem_bUnion.mpr ⟨l, Finsetₓ.mem_univ l, set.mem_to_finset.mpr hl⟩
...
theorem sum_line_count_eq_sum_point_count [Fintypeₓ P] [Fintypeₓ L] :
    (∑ p : P, lineCount L p) = ∑ l : L, pointCount P l := by
  classical
  simp only [line_count, point_count, Nat.card_eq_fintype_card, ← Fintypeₓ.card_sigma]
  apply Fintypeₓ.card_congr
  calc
...
theorem HasLines.point_count_le_line_count [HasLines P L] {p : P} {l : L} (h : p ∉ l) [Finite { l : L // p ∈ l }] :
  ...
  exact
    Fintypeₓ.card_le_of_injective (fun p' => ⟨mk_line (this p'), (mk_line_ax (this p')).1⟩) fun p₁ p₂ hp =>
      Subtype.ext
...
theorem HasLines.card_le [HasLines P L] [Fintypeₓ P] [Fintypeₓ L] : Fintypeₓ.card P ≤ Fintypeₓ.card L := by
  classical
  by_contra hc₂
  ...
  have :=
    calc
      ...
      _ ≤ ∑ l, line_count L (f l) := Finsetₓ.sum_le_sum fun l hl => has_lines.point_count_le_line_count (hf₂ l)
      _ = ∑ p in finset.univ.image f, line_count L p :=
        Finsetₓ.sum_bij (fun l hl => f l) (fun l hl => Finsetₓ.mem_image_of_mem f hl) (fun l hl => rfl)
          (fun l₁ l₂ hl₁ hl₂ hl₃ => hf₁ hl₃) fun p => by simp_rw [Finsetₓ.mem_image, eq_comm, imp_self]
      _ < ∑ p, line_count L p := _
      
  · exact lt_irreflₓ _ this
    
  · obtain ⟨p, hp⟩ := not_forall.mp (mt (Fintypeₓ.card_le_of_surjective f) hc₂)
    refine'
      Finsetₓ.sum_lt_sum_of_subset (finset.univ.image f).subset_univ (Finsetₓ.mem_univ p) _ _ fun p hp₁ hp₂ =>
        zero_le (line_count L p)
    · simpa only [Finsetₓ.mem_image, exists_propₓ, Finsetₓ.mem_univ, true_andₓ]
      
    · rw [line_count, Nat.card_eq_fintype_card, Fintypeₓ.card_pos_iff]
...
theorem HasPoints.card_le [HasPoints P L] [Fintypeₓ P] [Fintypeₓ L] : Fintypeₓ.card L ≤ Fintypeₓ.card P :=
  @HasLines.card_le (Dual L) (Dual P) _ _ _ _

variable {P L}

theorem HasLines.exists_bijective_of_card_eq [HasLines P L] [Fintypeₓ P] [Fintypeₓ L]
    (h : Fintypeₓ.card P = Fintypeₓ.card L) :
  ...
  have hf3 := (Fintypeₓ.bijective_iff_injective_and_card f).mpr ⟨hf1, h.symm⟩
  refine'
    ⟨f, hf3, fun l =>
      (Finsetₓ.sum_eq_sum_iff_of_le fun l hl => has_lines.point_count_le_line_count (hf2 l)).mp
        ((sum_line_count_eq_sum_point_count P L).symm.trans
          (Finsetₓ.sum_bij (fun l hl => f l) (fun l hl => Finsetₓ.mem_univ (f l))
              (fun l hl => refl (line_count L (f l))) (fun l₁ l₂ hl₁ hl₂ hl => hf1 hl) fun p hp => _).symm)
        l (Finsetₓ.mem_univ l)⟩
  obtain ⟨l, rfl⟩ := hf3.2 p
  exact ⟨l, Finsetₓ.mem_univ l, rfl⟩

theorem HasLines.line_count_eq_point_count [HasLines P L] [Fintypeₓ P] [Fintypeₓ L]
    (hPL : Fintypeₓ.card P = Fintypeₓ.card L) {p : P} {l : L} (hpl : p ∉ l) : lineCount L p = pointCount P l := by
  classical
  obtain ⟨f, hf1, hf2⟩ := has_lines.exists_bijective_of_card_eq hPL
  let s : Finsetₓ (P × L) := Set.toFinset { i | i.1 ∈ i.2 }
  have step1 : (∑ i : P × L, line_count L i.1) = ∑ i : P × L, point_count P i.2 := by
    rw [← Finsetₓ.univ_product_univ, Finsetₓ.sum_product_right, Finsetₓ.sum_product]
    simp_rw [Finsetₓ.sum_const, Finsetₓ.card_univ, hPL, sum_line_count_eq_sum_point_count]
  have step2 : (∑ i in s, line_count L i.1) = ∑ i in s, point_count P i.2 := by
    rw [s.sum_finset_product Finsetₓ.univ fun p => Set.toFinset { l | p ∈ l }]
    rw [s.sum_finset_product_right Finsetₓ.univ fun l => Set.toFinset { p | p ∈ l }]
    refine'
      (Finsetₓ.sum_bij (fun l hl => f l) (fun l hl => Finsetₓ.mem_univ (f l)) (fun l hl => _) (fun _ _ _ _ h => hf1.1 h)
          fun p hp => _).symm
    · simp_rw [Finsetₓ.sum_const, Set.to_finset_card, ← Nat.card_eq_fintype_card]
      change point_count P l • point_count P l = line_count L (f l) • line_count L (f l)
      rw [hf2]
      
    · obtain ⟨l, hl⟩ := hf1.2 p
      exact ⟨l, Finsetₓ.mem_univ l, hl.symm⟩
      
    all_goals
    simp_rw [Finsetₓ.mem_univ, true_andₓ, Set.mem_to_finset]
    exact fun p => Iff.rfl
  ...
  exact
    ((Finsetₓ.sum_eq_sum_iff_of_le fun i hi => has_lines.point_count_le_line_count (set.mem_to_finset.mp hi)).mp
        step3.symm (p, l) (set.mem_to_finset.mpr hpl)).symm

theorem HasPoints.line_count_eq_point_count [HasPoints P L] [Fintypeₓ P] [Fintypeₓ L]
    (hPL : Fintypeₓ.card P = Fintypeₓ.card L) {p : P} {l : L} (hpl : p ∉ l) : lineCount L p = pointCount P l :=
...
noncomputable def HasLines.hasPoints [HasLines P L] [Fintypeₓ P] [Fintypeₓ L] (h : Fintypeₓ.card P = Fintypeₓ.card L) :
    HasPoints P L :=
  let this : ∀ l₁ l₂ : L, l₁ ≠ l₂ → ∃ p : P, p ∈ l₁ ∧ p ∈ l₂ := fun l₁ l₂ hl => by
    ...
    haveI := fintype.one_lt_card_iff_nontrivial.mp ((congr_arg _ h).mpr Fintypeₓ.one_lt_card)
    ...
    by_cases hl₂:p ∈ l₂
    exact ⟨p, hl₁, hl₂⟩
    have key' : Fintypeₓ.card { q : P // q ∈ l₂ } = Fintypeₓ.card { l : L // p ∈ l } :=
    ...
    have key' := ((Fintypeₓ.bijective_iff_injective_and_card f).mpr ⟨hf, key'⟩).2
...
noncomputable def HasPoints.hasLines [HasPoints P L] [Fintypeₓ P] [Fintypeₓ L] (h : Fintypeₓ.card P = Fintypeₓ.card L) :
    HasLines P L :=
...
theorem card_points_eq_card_lines [Fintypeₓ P] [Fintypeₓ L] : Fintypeₓ.card P = Fintypeₓ.card L :=
  le_antisymmₓ (HasLines.card_le P L) (HasPoints.card_le P L)

variable {P} (L)

theorem line_count_eq_line_count [Finite P] [Finite L] (p q : P) : lineCount L p = lineCount L q := by
...
theorem point_count_eq_point_count [Finite P] [Finite L] (l m : L) : pointCount P l = pointCount P m :=
...
theorem point_count_eq [Finite P] [Finite L] (l : L) : pointCount P l = order P L + 1 :=
...
theorem one_lt_order [Finite P] [Finite L] : 1 < order P L := by
  ...
  simp_rw [Fintypeₓ.two_lt_card_iff, Ne, Subtype.ext_iff]
  have h := mk_point_ax fun h => h₂₁ ((congr_arg _ h).mpr h₂₂)
  exact
    ⟨⟨mk_point _, h.2⟩, ⟨p₂, h₂₂⟩, ⟨p₃, h₃₂⟩, ne_of_mem_of_not_memₓ h.1 h₂₁, ne_of_mem_of_not_memₓ h.1 h₃₁,
      ne_of_mem_of_not_memₓ h₂₃ h₃₃⟩
...
theorem card_points [Fintypeₓ P] [Finite L] : Fintypeₓ.card P = order P L ^ 2 + order P L + 1 := by
  cases nonempty_fintype L
    ...
    { toFun := fun q => ⟨⟨mk_line q.2, (mk_line_ax q.2).2⟩, q, (mk_line_ax q.2).1, q.2⟩,
      ...
      right_inv := fun lq =>
        Sigma.subtype_ext
          ...
          rfl }
  classical
  have h1 : Fintypeₓ.card { q // q ≠ p } + 1 = Fintypeₓ.card P := by
    apply (eq_tsub_iff_add_eq_of_le (Nat.succ_le_of_ltₓ (fintype.card_pos_iff.mpr ⟨p⟩))).mp
    convert (Fintypeₓ.card_subtype_compl _).trans (congr_arg _ (Fintypeₓ.card_subtype_eq p))
  have h2 : ∀ l : { l : L // p ∈ l }, Fintypeₓ.card { q // q ∈ l.1 ∧ q ≠ p } = order P L := by
    intro l
    rw [← Fintypeₓ.card_congr (Equivₓ.subtypeSubtypeEquivSubtypeInter _ _),
      Fintypeₓ.card_subtype_compl fun x : Subtype (· ∈ l.val) => x.val = p, ← Nat.card_eq_fintype_card]
    refine' tsub_eq_of_eq_add ((point_count_eq P l.1).trans _)
    rw [← Fintypeₓ.card_subtype_eq (⟨p, l.2⟩ : { q : P // q ∈ l.1 })]
    simp_rw [Subtype.ext_iff_val]
  simp_rw [← h1, Fintypeₓ.card_congr ϕ, Fintypeₓ.card_sigma, h2, Finsetₓ.sum_const, Finsetₓ.card_univ]
  rw [← Nat.card_eq_fintype_card, ← line_count, line_count_eq, smul_eq_mul, Nat.succ_mul, sq]

theorem card_lines [Finite P] [Fintypeₓ L] : Fintypeₓ.card L = order P L ^ 2 + order P L + 1 :=
