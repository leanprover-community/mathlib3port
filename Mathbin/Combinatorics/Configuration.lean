...
theorem Nondegenerate.exists_injective_of_card_le [Nondegenerate P L] [Fintype P] [Fintype L]
    (h : Fintype.card L ≤ Fintype.card P) : ∃ f : L → P, Function.Injective f ∧ ∀ l, f l ∉ l := by classical
...
theorem HasLines.card_le [HasLines P L] [Fintype P] [Fintype L] : Fintype.card P ≤ Fintype.card L := by classical
  by_contra hc₂
...
theorem HasLines.line_count_eq_point_count [HasLines P L] [Fintype P] [Fintype L]
  ...
  have step2 : (∑ i in s, line_count L i.1) = ∑ i in s, point_count P i.2 := by
    ...
    all_goals
    simp_rw [Finset.mem_univ, true_and_iff, Set.mem_to_finset]
    exact fun p => Iff.rfl
