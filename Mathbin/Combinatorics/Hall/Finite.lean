...
theorem hall_hard_inductive_zero (t : ι → Finset α) (hn : Fintype.card ι = 0) :
  ...
  by 
    ...
    exactI ⟨isEmptyElim, isEmptyElim, isEmptyElim⟩
    ...
    ·
      ...
      ·
        ...
        simp only [mem_image, mem_bUnion, exists_prop, SetCoe.exists, exists_and_distrib_right, exists_eq_right,
          Subtype.coe_mk]
      rw [←erase_bUnion]
      byCases' hb : a ∈ s'.bUnion fun x => t x
      ·
...
-/
theorem hall_hard_inductive_step_A {n : ℕ} (hn : Fintype.card ι = n+1) (ht : ∀ s : Finset ι, s.card ≤ (s.bUnion t).card)
  (ih :
  ...
  by 
    ...
    ·
      ...
      ·
        ...
        rw [←h] at hfr 
        simpa using hfr 
      byCases' h₁ : z₁ = x <;> byCases' h₂ : z₂ = x <;> simp [h₁, h₂, hfinj.eq_iff, key, key.symm]
    ·
      intro z 
      splitIfs with hz
      ·
...
theorem hall_cond_of_restrict {ι : Type u} {t : ι → Finset α} {s : Finset ι}
  ...
  s'.card ≤ (s'.bUnion fun a' => t a').card :=
  by 
    haveI  := Classical.decEq ι 
...
theorem hall_cond_of_compl {ι : Type u} {t : ι → Finset α} {s : Finset ι} (hus : s.card = (s.bUnion t).card)
  ...
  by 
    haveI  := Classical.decEq ι 
    have  : s'.card = (s ∪ s'.image coeₓ).card - s.card
    ·
    ...
    rw [←card_sdiff]
    ·
      have  : (s ∪ s'.image Subtype.val).bUnion t \ s.bUnion t ⊆ s'.bUnion fun x' => t x' \ s.bUnion t
      ·
        intro t 
...
`ι'` of cardinality ≤ `n`, then it is true for `ι` of cardinality `n + 1`.
-/
theorem hall_hard_inductive_step_B {n : ℕ} (hn : Fintype.card ι = n+1) (ht : ∀ s : Finset ι, s.card ≤ (s.bUnion t).card)
  (ih :
    ∀ {ι' : Type u} [Fintype ι'] t' : ι' → Finset α,
      by 
        exactI
  ...
  by 
    ...
    ·
      ...
      convert Fintype.card_coe («expr ᶜ» s)
...
theorem hall_hard_inductive_step {n : ℕ} (hn : Fintype.card ι = n+1) (ht : ∀ s : Finset ι, s.card ≤ (s.bUnion t).card)
  ...
  ∃ f : ι → α, Function.Injective f ∧ ∀ x, f x ∈ t x :=
  by 
    byCases' h : ∀ s : Finset ι, s.nonempty → s ≠ univ → s.card < (s.bUnion t).card
    ·
