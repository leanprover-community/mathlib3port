...
theorem mem_mk (c : ℕ × ℕ) (cells) (is_lower_set) :
    c ∈ YoungDiagram.mk cells is_lower_set ↔ c ∈ cells :=
  Iff.rfl
...
theorem rowLens_length_ofRowLens {w : List ℕ} {hw : w.Sorted (· ≥ ·)} (hpos : ∀ x ∈ w, 0 < x) :
  ...
  by
  simp only [length_rowLens, colLen, Nat.find_eq_iff, mem_cells, mem_ofRowLens, lt_self_iff_false,
...
but is expected to have type
  forall {w : List.{0} Nat} {hw : List.Sorted.{0} Nat (fun (x._@.Mathlib.Combinatorics.Young.YoungDiagram._hyg.2894 : Nat) (x._@.Mathlib.Combinatorics.Young.YoungDiagram._hyg.2896 : Nat) => GE.ge.{0} Nat instLENat x._@.Mathlib.Combinatorics.Young.YoungDiagram._hyg.2894 x._@.Mathlib.Combinatorics.Young.YoungDiagram._hyg.2896) w} (i : Fin (List.length.{0} Nat w)), Eq.{1} Nat (YoungDiagram.rowLen (YoungDiagram.ofRowLens w hw) (Fin.val (List.length.{0} Nat w) i)) (List.get.{0} Nat w i)
...
theorem rowLen_ofRowLens {w : List ℕ} {hw : w.Sorted (· ≥ ·)} (i : ℕ) (hi : i < w.length) :
    (ofRowLens w hw).rowLen i = w.nthLe i hi := by simp [rowLen, Nat.find_eq_iff, mem_ofRowLens, hi]
