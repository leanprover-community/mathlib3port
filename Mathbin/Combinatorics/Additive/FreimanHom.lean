...
#print IsMulFreimanHom.map_prod_eq_map_prod /-
@[to_additive]
theorem IsMulFreimanHom.map_prod_eq_map_prod [FreimanHomClass F A β n] (f : F) {s t : Multiset α}
...
#print IsMulFreimanHom.mul_eq_mul /-
@[to_additive]
theorem IsMulFreimanHom.mul_eq_mul [FreimanHomClass F A β 2] (f : F) (ha : a ∈ A) (hb : b ∈ A)
    (hc : c ∈ A) (hd : d ∈ A) (h : a * b = c * d) : f a * f b = f c * f d :=
  by
  simp_rw [← prod_pair] at h ⊢
  refine' IsMulFreimanHom.map_prod_eq_map_prod f _ _ (card_pair _ _) (card_pair _ _) h <;>
    simp [ha, hb, hc, hd]
...
@[to_additive]
instance funLike : DFunLike (A →*[n] β) α fun _ => β
    where
  coe := toFun
...
protected def comp (f : B →*[n] γ) (g : A →*[n] β) (hAB : A.MapsTo g B) : A →*[n] γ
  ...
  map_prod_eq_map_prod' s t hsA htA hs ht h :=
    by
    rw [← map_map,
      IsMulFreimanHom.map_prod_eq_map_prod f _ _ ((s.card_map _).trans hs) ((t.card_map _).trans ht)
        (IsMulFreimanHom.map_prod_eq_map_prod g hsA htA hs ht h),
      map_map]
  ...
  ⟨fun f g =>
    { toFun := fun x => f x * g x
      map_prod_eq_map_prod' := fun s t hsA htA hs ht h => by
        rw [prod_map_mul, prod_map_mul, IsMulFreimanHom.map_prod_eq_map_prod f hsA htA hs ht h,
          IsMulFreimanHom.map_prod_eq_map_prod g hsA htA hs ht h] }⟩

@[simp, to_additive]
theorem hMul_apply (f g : A →*[n] β) (x : α) : (f * g) x = f x * g x :=
  rfl
#align freiman_hom.mul_apply FreimanHom.hMul_apply
#align add_freiman_hom.add_apply AddFreimanHom.add_apply

@[to_additive]
theorem hMul_comp (g₁ g₂ : B →*[n] γ) (f : A →*[n] β) {hg hg₁ hg₂} :
    (g₁ * g₂).comp f hg = g₁.comp f hg₁ * g₂.comp f hg₂ :=
  rfl
#align freiman_hom.mul_comp FreimanHom.hMul_comp
...
instance : Inv (A →*[n] G) :=
  ⟨fun f =>
    { toFun := fun x => (f x)⁻¹
      map_prod_eq_map_prod' := fun s t hsA htA hs ht h => by
        rw [prod_map_inv, prod_map_inv, IsMulFreimanHom.map_prod_eq_map_prod f hsA htA hs ht h] }⟩
...
instance : Div (A →*[n] G) :=
  ⟨fun f g =>
    { toFun := fun x => f x / g x
      map_prod_eq_map_prod' := fun s t hsA htA hs ht h => by
        rw [prod_map_div, prod_map_div, IsMulFreimanHom.map_prod_eq_map_prod f hsA htA hs ht h,
          IsMulFreimanHom.map_prod_eq_map_prod g hsA htA hs ht h] }⟩
...
instance : CommMonoid (A →*[n] β) where
  ...
  npow m f :=
    { toFun := fun x => f x ^ m
      map_prod_eq_map_prod' := fun s t hsA htA hs ht h => by
        rw [prod_map_pow, prod_map_pow, IsMulFreimanHom.map_prod_eq_map_prod f hsA htA hs ht h] }
...
instance {β} [CommGroup β] : CommGroup (A →*[n] β) :=
  { FreimanHom.commMonoid with
    ...
    zpow := fun n f =>
      { toFun := fun x => f x ^ n
        map_prod_eq_map_prod' := fun s t hsA htA hs ht h => by
          rw [prod_map_zpow, prod_map_zpow,
            IsMulFreimanHom.map_prod_eq_map_prod f hsA htA hs ht h] }
...
@[to_additive AddMonoidHom.to_add_freiman_hom
      "An `add_monoid_hom` is naturally an\n`add_freiman_hom`"]
def MonoidHom.toFreimanHom (A : Set α) (n : ℕ) (f : α →* β) : A →*[n] β
    where
  toFun := f
  map_prod_eq_map_prod' s t hsA htA :=
    IsMulFreimanHom.map_prod_eq_map_prod f (fun _ _ => Set.mem_univ _) fun _ _ => Set.mem_univ _
...
#print IsMulFreimanHom.mono /-
@[to_additive]
theorem IsMulFreimanHom.mono [FreimanHomClass F A β n] (f : F) {s t : Multiset α}
  ...
  replace ha := hsA _ ha
  refine' IsMulFreimanHom.map_prod_eq_map_prod f (fun x hx => _) (fun x hx => _) _ _ _
...
@[to_additive AddFreimanHom.to_add_freiman_hom
      "`α →+[n] β` is naturally included in  `α →+[m] β`\nfor any `m ≤ n`"]
def FreimanHom.toFreimanHom (h : m ≤ n) (f : A →*[n] β) : A →*[m] β
    where
  toFun := f
  map_prod_eq_map_prod' s t hsA htA hs ht hst := IsMulFreimanHom.mono f hsA htA hs ht hst h
#align freiman_hom.to_freiman_hom FreimanHom.toFreimanHom
#align add_freiman_hom.to_add_freiman_hom AddFreimanHom.to_add_freiman_hom
...
theorem FreimanHom.freimanHomClassOfLe [FreimanHomClass F A β n] (h : m ≤ n) :
    FreimanHomClass F A β m :=
  {
    map_prod_eq_map_prod' := fun f s t hsA htA hs ht hst =>
      IsMulFreimanHom.mono f hsA htA hs ht hst h }
