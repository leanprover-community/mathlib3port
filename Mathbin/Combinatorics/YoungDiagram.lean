...
instance : SetLike YoungDiagram (ℕ × ℕ) where
  ...
  coe_injective' := fun μ ν h => by rwa [YoungDiagram.ext_iff, ← Finsetₓ.coe_inj]
...
def transpose (μ : YoungDiagram) : YoungDiagram where
  ...
  IsLowerSet := fun _ _ h => by
    simp only [Finsetₓ.mem_coe, Equivₓ.finset_congr_apply, Finsetₓ.mem_map_equiv]
    intro hcell
...
def row (μ : YoungDiagram) (i : ℕ) : Finsetₓ (ℕ × ℕ) :=
...
theorem row_eq_prod {μ : YoungDiagram} {i : ℕ} : μ.row i = {i} ×ˢ Finsetₓ.range (μ.rowLen i) := by
  ext ⟨a, b⟩
  simp only [Finsetₓ.mem_product, Finsetₓ.mem_singleton, Finsetₓ.mem_range, mem_row_iff, mem_iff_lt_row_len, and_comm,
    And.congr_right_iff]
  rintro rfl
  rfl
...
def col (μ : YoungDiagram) (j : ℕ) : Finsetₓ (ℕ × ℕ) :=
...
-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem col_eq_prod {μ : YoungDiagram} {j : ℕ} : μ.col j = Finsetₓ.range (μ.colLen j) ×ˢ {j} := by
  ext ⟨a, b⟩
  simp only [Finsetₓ.mem_product, Finsetₓ.mem_singleton, Finsetₓ.mem_range, mem_col_iff, mem_iff_lt_col_len, and_comm,
    And.congr_right_iff]
  rintro rfl
  rfl
