...
variable {α : Type _} [DecidableEq α] {𝒜 ℬ : Finsetₓ (Finsetₓ α)} {s : Finsetₓ α} {a : α}

theorem IsLowerSet.non_member_subfamily (h : IsLowerSet (𝒜 : Set (Finsetₓ α))) :
    IsLowerSet (𝒜.nonMemberSubfamily a : Set (Finsetₓ α)) := fun s t hts => by
  simp_rw [mem_coe, mem_non_member_subfamily]
  exact And.impₓ (h hts) (mt <| @hts _)

theorem IsLowerSet.member_subfamily (h : IsLowerSet (𝒜 : Set (Finsetₓ α))) :
    IsLowerSet (𝒜.memberSubfamily a : Set (Finsetₓ α)) := by
  rintro s t hts
  simp_rw [mem_coe, mem_member_subfamily]
  exact And.impₓ (h <| insert_subset_insert _ hts) (mt <| @hts _)

theorem IsLowerSet.member_subfamily_subset_non_member_subfamily (h : IsLowerSet (𝒜 : Set (Finsetₓ α))) :
...
theorem IsLowerSet.le_card_inter_finset' (h𝒜 : IsLowerSet (𝒜 : Set (Finsetₓ α))) (hℬ : IsLowerSet (ℬ : Set (Finsetₓ α)))
    (h𝒜s : ∀ t ∈ 𝒜, t ⊆ s) (hℬs : ∀ t ∈ ℬ, t ⊆ s) : 𝒜.card * ℬ.card ≤ 2 ^ s.card * (𝒜 ∩ ℬ).card := by
  induction' s using Finsetₓ.induction with a s hs ih generalizing 𝒜 ℬ
  ...
  have h₀ : ∀ 𝒞 : Finsetₓ (Finsetₓ α), (∀ t ∈ 𝒞, t ⊆ insert a s) → ∀ t ∈ 𝒞.nonMemberSubfamily a, t ⊆ s := by
    rintro 𝒞 h𝒞 t ht
    rw [mem_non_member_subfamily] at ht
    exact (subset_insert_iff_of_not_mem ht.2).1 (h𝒞 _ ht.1)
  have h₁ : ∀ 𝒞 : Finsetₓ (Finsetₓ α), (∀ t ∈ 𝒞, t ⊆ insert a s) → ∀ t ∈ 𝒞.memberSubfamily a, t ⊆ s := by
    rintro 𝒞 h𝒞 t ht
...
/-- **Harris-Kleitman inequality**: Any two lower sets of finsets correlate. -/
theorem IsLowerSet.le_card_inter_finset (h𝒜 : IsLowerSet (𝒜 : Set (Finsetₓ α)))
    (hℬ : IsLowerSet (ℬ : Set (Finsetₓ α))) : 𝒜.card * ℬ.card ≤ 2 ^ Fintypeₓ.card α * (𝒜 ∩ ℬ).card :=
...
theorem IsUpperSet.card_inter_le_finset (h𝒜 : IsUpperSet (𝒜 : Set (Finsetₓ α)))
    (hℬ : IsLowerSet (ℬ : Set (Finsetₓ α))) : 2 ^ Fintypeₓ.card α * (𝒜 ∩ ℬ).card ≤ 𝒜.card * ℬ.card := by
  rw [← is_lower_set_compl, ← coe_compl] at h𝒜
  have := h𝒜.le_card_inter_finset hℬ
  rwa [card_compl, Fintypeₓ.card_finset, tsub_mul, tsub_le_iff_tsub_le, ← mul_tsub, ←
...
theorem IsLowerSet.card_inter_le_finset (h𝒜 : IsLowerSet (𝒜 : Set (Finsetₓ α)))
    (hℬ : IsUpperSet (ℬ : Set (Finsetₓ α))) : 2 ^ Fintypeₓ.card α * (𝒜 ∩ ℬ).card ≤ 𝒜.card * ℬ.card := by
...
theorem IsUpperSet.le_card_inter_finset (h𝒜 : IsUpperSet (𝒜 : Set (Finsetₓ α)))
    (hℬ : IsUpperSet (ℬ : Set (Finsetₓ α))) : 𝒜.card * ℬ.card ≤ 2 ^ Fintypeₓ.card α * (𝒜 ∩ ℬ).card := by
  rw [← is_lower_set_compl, ← coe_compl] at h𝒜
  have := h𝒜.card_inter_le_finset hℬ
  rwa [card_compl, Fintypeₓ.card_finset, tsub_mul, le_tsub_iff_le_tsub, ← mul_tsub, ←
    card_sdiff (inter_subset_right _ _), sdiff_inter_self_right, sdiff_compl, _root_.inf_comm] at this
  · exact mul_le_mul_left' (card_le_of_subset <| inter_subset_right _ _) _
    
  · rw [← Fintypeₓ.card_finset]
