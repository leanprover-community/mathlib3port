...
theorem size_up_to_index_le (j : Finₓ n) : c.size_up_to (c.index j) ≤ j :=
  by 
    byContra H 
...
def blocks_fin_equiv : (Σ i : Finₓ c.length, Finₓ (c.blocks_fun i)) ≃ Finₓ n :=
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » c.blocks)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (j «expr ∈ » c.blocks)
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ∈ » c.blocks)
-- failed to parenthesize: parenthesize: uncaught backtrack exception
-- failed to format: format: uncaught backtrack exception
theorem
  ne_ones_iff
  { c : Composition n } : c ≠ ones n ↔ ∃ ( i : _ ) ( _ : i ∈ c.blocks ) , 1 < i
  :=
    by
      refine' not_congr eq_ones_iff . trans _
        have : ∀ j _ : j ∈ c.blocks , j = 1 ↔ j ≤ 1 := fun j hj => by simp [ le_antisymm_iffₓ , c.one_le_blocks hj ]
...
theorem ne_single_iff {n : ℕ} (hn : 0 < n) {c : Composition n} : c ≠ single n hn ↔ ∀ i, c.blocks_fun i < n :=
  by 
    ...
    ·
      ...
      have  : ∀ j : Finₓ c.length, j = i
      ·
        intro j 
        byContra ji 
        apply lt_irreflₓ (∑ k, c.blocks_fun k)
        calc (∑ k, c.blocks_fun k) ≤ c.blocks_fun i :=
          by 
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr < » c.boundaries.card)
...
-- ././Mathport/Syntax/Translate/Basic.lean:452:2: warning: expanding binder collection (i «expr ≤ » d.blocks.length)
@[simp]
