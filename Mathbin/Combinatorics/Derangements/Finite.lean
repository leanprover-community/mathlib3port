...
theorem card_derangements_fin_add_two (n : ℕ) :
    card (Derangements (Finₓ (n+2))) = ((n+1)*card (Derangements (Finₓ n)))+(n+1)*card (Derangements (Finₓ (n+1))) := by
...
[PrettyPrinter.parenthesize.input] (Command.declaration
 ...
 (Command.theorem
  "theorem"
  (Command.declId `num_derangements_sum [])
  (Command.declSig
   [(Term.explicitBinder "(" [`n] [":" (termℕ "ℕ")] [] ")")]
   (Term.typeSpec
    ":"
    («term_=_»
     (Term.paren "(" [(Term.app `numDerangements [`n]) [(Term.typeAscription ":" (termℤ "ℤ"))]] ")")
     "="
     (Algebra.BigOperators.Basic.«term∑_in_,_»
      ...
      (Finset.Data.Finset.Fold.«term_*_»
       («term_^_» (Term.paren "(" [(«term-_» "-" (numLit "1")) [(Term.typeAscription ":" (termℤ "ℤ"))]] ")") "^" `k)
       "*"
       (Term.app `Nat.ascFactorial [`k («term_-_» `n "-" `k)]))))))
        ...
        (Tactic.rwSeq
         ...
         (Tactic.rwRuleSeq
          ...
          [(Tactic.rwRule [] `Finset.sum_range_succ)
           ","
           (Tactic.rwRule [] `num_derangements_succ)
           ","
           (Tactic.rwRule [] `hn)
           ","
           ...
           ","
           (Tactic.rwRule [] `Nat.asc_factorial_zero)
           ","
       ...
       (group
        (Tactic.tacticHave_
         ...
         (Term.haveDecl
          (Term.haveIdDecl
           [`h_le []]
           [(Term.typeSpec ":" («term_≤_» `x "≤" `n))]
           ":="
           (Term.app `finset.mem_range_succ_iff.mp [`hx]))))
        [])
       (group
        (Tactic.rwSeq
         "rw"
         ...
         (Tactic.rwRuleSeq
          "["
          [(Tactic.rwRule [] (Term.app `Nat.succ_subₓ [`h_le]))
           ","
           (Tactic.rwRule [] `Nat.asc_factorial_succ)
           ","
           (Tactic.rwRule [] (Term.app `add_tsub_cancel_of_le [`h_le]))
           ","
           (Tactic.rwRule [] `Int.coe_nat_mul)
           ","
...
theorem
  ...
  ( n : ℕ ) : ( numDerangements n : ℤ ) = ∑ k in Finset.range n + 1 , ( - 1 : ℤ ) ^ k * Nat.ascFactorial k n - k
  :=
    by
      induction' n with n hn
        · rfl
