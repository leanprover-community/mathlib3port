...
variable (r : α → β → Prop) [∀ a, DecidablePred (r a)] {s s₁ s₂ : Finsetₓ α} {t t₁ t₂ : Finsetₓ β} {a : α} {b : β}
  {δ : ℚ}

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
/-- Finset of edges of a relation between two finsets of vertices. -/
def interedges (s : Finsetₓ α) (t : Finsetₓ β) : Finsetₓ (α × β) :=
  (s ×ˢ t).filter fun e => r e.1 e.2

/-- Edge density of a relation between two finsets of vertices. -/
def edgeDensity (s : Finsetₓ α) (t : Finsetₓ β) : ℚ :=
  (interedges r s t).card / (s.card * t.card)

variable {r}

theorem mem_interedges_iff {x : α × β} : x ∈ interedges r s t ↔ x.1 ∈ s ∧ x.2 ∈ t ∧ r x.1 x.2 := by
  simp only [interedges, and_assocₓ, mem_filter, Finsetₓ.mem_product]

theorem mk_mem_interedges_iff : (a, b) ∈ interedges r s t ↔ a ∈ s ∧ b ∈ t ∧ r a b :=
  mem_interedges_iff

@[simp]
theorem interedges_empty_left (t : Finsetₓ β) : interedges r ∅ t = ∅ := by
  rw [interedges, Finsetₓ.empty_product, filter_empty]
...
theorem card_interedges_add_card_interedges_compl (s : Finsetₓ α) (t : Finsetₓ β) :
...
theorem interedges_disjoint_left {s s' : Finsetₓ α} (hs : Disjoint s s') (t : Finsetₓ β) :
...
theorem interedges_disjoint_right (s : Finsetₓ α) {t t' : Finsetₓ β} (ht : Disjoint t t') :
    Disjoint (interedges r s t) (interedges r s t') := by
  rintro x hx
  rw [inf_eq_inter, mem_inter, mem_interedges_iff, mem_interedges_iff] at hx
  exact ht (mem_inter.2 ⟨hx.1.2.1, hx.2.2.1⟩)

theorem interedges_bUnion_left (s : Finsetₓ ι) (t : Finsetₓ β) (f : ι → Finsetₓ α) :
    interedges r (s.bUnion f) t = s.bUnion fun a => interedges r (f a) t :=
  ext fun a => by simp only [mem_bUnion, mem_interedges_iff, exists_and_distrib_rightₓ]

theorem interedges_bUnion_right (s : Finsetₓ α) (t : Finsetₓ ι) (f : ι → Finsetₓ β) :
    interedges r s (t.bUnion f) = t.bUnion fun b => interedges r s (f b) :=
  ext fun a => by simp only [mem_interedges_iff, mem_bUnion, ← exists_and_distrib_leftₓ, ← exists_and_distrib_rightₓ]

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem interedges_bUnion (s : Finsetₓ ι) (t : Finsetₓ κ) (f : ι → Finsetₓ α) (g : κ → Finsetₓ β) :
...
theorem card_interedges_le_mul (s : Finsetₓ α) (t : Finsetₓ β) : (interedges r s t).card ≤ s.card * t.card :=
  (card_filter_le _ _).trans (card_product _ _).le

theorem edge_density_nonneg (s : Finsetₓ α) (t : Finsetₓ β) : 0 ≤ edgeDensity r s t := by
  apply div_nonneg <;> exact_mod_cast Nat.zero_leₓ _

theorem edge_density_le_one (s : Finsetₓ α) (t : Finsetₓ β) : edgeDensity r s t ≤ 1 :=
  div_le_one_of_le (by exact_mod_cast card_interedges_le_mul _ _ _) <| by exact_mod_cast Nat.zero_leₓ _
...
@[simp]
theorem edge_density_empty_left (t : Finsetₓ β) : edgeDensity r ∅ t = 0 := by
  rw [edge_density, Finsetₓ.card_empty, Nat.cast_zeroₓ, zero_mul, div_zero]

@[simp]
theorem edge_density_empty_right (s : Finsetₓ α) : edgeDensity r s ∅ = 0 := by
  rw [edge_density, Finsetₓ.card_empty, Nat.cast_zeroₓ, mul_zero, div_zero]

theorem card_interedges_finpartition_left [DecidableEq α] (P : Finpartition s) (t : Finsetₓ β) :
...
theorem card_interedges_finpartition_right [DecidableEq β] (s : Finsetₓ α) (P : Finpartition t) :
...
theorem abs_edge_density_sub_edge_density_le_two_mul_sub_sq (hs : s₂ ⊆ s₁) (ht : t₂ ⊆ t₁) (hδ₀ : 0 ≤ δ) (hδ₁ : δ < 1)
  ...
  · rw [Finsetₓ.card_empty, Nat.cast_zeroₓ] at hs₂
    simpa [(nonpos_of_mul_nonpos_right hs₂ hδ₁).antisymm (Nat.cast_nonneg _)] using hδ'
    
  obtain rfl | ht₂' := t₂.eq_empty_or_nonempty
  · rw [Finsetₓ.card_empty, Nat.cast_zeroₓ] at ht₂
...
variable (r : α → α → Prop) [DecidableRel r] {s s₁ s₂ t t₁ t₂ : Finsetₓ α} {a b : α}
...
theorem card_interedges_comm (s t : Finsetₓ α) : (interedges r s t).card = (interedges r t s).card :=
  Finsetₓ.card_congr (fun (x : α × α) _ => x.swap) (fun x => (swap_mem_interedges_iff hr).2)
    (fun _ _ _ _ h => Prod.swap_injectiveₓ h) fun x h => ⟨x.swap, (swap_mem_interedges_iff hr).2 h, x.swap_swap⟩

theorem edge_density_comm (s t : Finsetₓ α) : edgeDensity r s t = edgeDensity r t s := by
...
variable (G : SimpleGraph α) [DecidableRel G.Adj] {s s₁ s₂ t t₁ t₂ : Finsetₓ α} {a b : α}

/-- Finset of edges of a relation between two finsets of vertices. -/
def interedges (s t : Finsetₓ α) : Finsetₓ (α × α) :=
  interedges G.Adj s t

/-- Density of edges of a graph between two finsets of vertices. -/
def edgeDensity : Finsetₓ α → Finsetₓ α → ℚ :=
  edgeDensity G.Adj

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem interedges_def (s t : Finsetₓ α) : G.interedges s t = (s ×ˢ t).filter fun e => G.Adj e.1 e.2 :=
  rfl

theorem edge_density_def (s t : Finsetₓ α) : G.edgeDensity s t = (G.interedges s t).card / (s.card * t.card) :=
  rfl

@[simp]
theorem card_interedges_div_card (s t : Finsetₓ α) :
...
@[simp]
theorem interedges_empty_left (t : Finsetₓ α) : G.interedges ∅ t = ∅ :=
...
theorem interedges_disjoint_left (hs : Disjoint s₁ s₂) (t : Finsetₓ α) :
    Disjoint (G.interedges s₁ t) (G.interedges s₂ t) :=
  interedges_disjoint_left _ hs _

theorem interedges_disjoint_right (s : Finsetₓ α) (ht : Disjoint t₁ t₂) :
    Disjoint (G.interedges s t₁) (G.interedges s t₂) :=
  interedges_disjoint_right _ _ ht

theorem interedges_bUnion_left (s : Finsetₓ ι) (t : Finsetₓ α) (f : ι → Finsetₓ α) :
    G.interedges (s.bUnion f) t = s.bUnion fun a => G.interedges (f a) t :=
  interedges_bUnion_left _ _ _ _

theorem interedges_bUnion_right (s : Finsetₓ α) (t : Finsetₓ ι) (f : ι → Finsetₓ α) :
    G.interedges s (t.bUnion f) = t.bUnion fun b => G.interedges s (f b) :=
  interedges_bUnion_right _ _ _ _

-- ./././Mathport/Syntax/Translate/Expr.lean:177:8: unsupported: ambiguous notation
theorem interedges_bUnion (s : Finsetₓ ι) (t : Finsetₓ κ) (f : ι → Finsetₓ α) (g : κ → Finsetₓ α) :
...
theorem card_interedges_le_mul (s t : Finsetₓ α) : (G.interedges s t).card ≤ s.card * t.card :=
  card_interedges_le_mul _ _ _

theorem edge_density_nonneg (s t : Finsetₓ α) : 0 ≤ G.edgeDensity s t :=
  edge_density_nonneg _ _ _

theorem edge_density_le_one (s t : Finsetₓ α) : G.edgeDensity s t ≤ 1 :=
  edge_density_le_one _ _ _

@[simp]
theorem edge_density_empty_left (t : Finsetₓ α) : G.edgeDensity ∅ t = 0 :=
  edge_density_empty_left _ _

@[simp]
theorem edge_density_empty_right (s : Finsetₓ α) : G.edgeDensity s ∅ = 0 :=
...
theorem edge_density_comm (s t : Finsetₓ α) : G.edgeDensity s t = G.edgeDensity t s :=
...
unsafe def positivity_edge_density : expr → tactic strictness
  | quote.1 (Rel.edgeDensity (%%ₓr) (%%ₓs) (%%ₓt)) =>
    nonnegative <$> mk_mapp `` Rel.edge_density_nonneg [none, none, r, none, s, t]
  | quote.1 (SimpleGraph.edgeDensity (%%ₓG) (%%ₓs) (%%ₓt)) =>
    nonnegative <$> mk_mapp `` SimpleGraph.edge_density_nonneg [none, G, none, s, t]
  | e =>
    pp e >>=
      fail ∘
