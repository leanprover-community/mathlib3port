...
@[simp]
theorem apply_diag_ne [MulZeroOneClass α] [Nontrivial α] (h : is_adj_matrix A) (i : V) : ¬A i i = 1 := by
  simp [h.apply_diag i]
...
@[simps]
def to_graph [MulZeroOneClass α] [Nontrivial α] (h : is_adj_matrix A) : SimpleGraph V where
  Adj := fun i j => A i j = 1
  symm := fun i j hij => by
...
instance [MulZeroOneClass α] [Nontrivial α] [DecidableEq α] (h : is_adj_matrix A) : DecidableRel h.to_graph.adj := by
  simp only [to_graph]
  infer_instance
...
theorem adj_matrix_dot_product [NonAssocSemiring α] (v : V) (vec : V → α) :
    dot_product (G.adj_matrix α v) vec = ∑ u in G.neighbor_finset v, vec u := by
  simp [neighbor_finset_eq_filter, dot_product, sum_filter]

@[simp]
theorem dot_product_adj_matrix [NonAssocSemiring α] (v : V) (vec : V → α) :
    dot_product vec (G.adj_matrix α v) = ∑ u in G.neighbor_finset v, vec u := by
...
theorem adj_matrix_mul_vec_apply [NonAssocSemiring α] (v : V) (vec : V → α) :
    ((G.adj_matrix α).mulVec vec) v = ∑ u in G.neighbor_finset v, vec u := by
  rw [mul_vec, adj_matrix_dot_product]

@[simp]
theorem adj_matrix_vec_mul_apply [NonAssocSemiring α] (v : V) (vec : V → α) :
    ((G.adj_matrix α).vecMul vec) v = ∑ u in G.neighbor_finset v, vec u := by
...
theorem adj_matrix_mul_apply [NonAssocSemiring α] (M : Matrix V V α) (v w : V) :
    (G.adj_matrix α ⬝ M) v w = ∑ u in G.neighbor_finset v, M u w := by
  simp [mul_apply, neighbor_finset_eq_filter, sum_filter]

@[simp]
theorem mul_adj_matrix_apply [NonAssocSemiring α] (M : Matrix V V α) (v w : V) :
    (M ⬝ G.adj_matrix α) v w = ∑ u in G.neighbor_finset w, M v u := by
  simp [mul_apply, neighbor_finset_eq_filter, sum_filter, adj_comm]

variable (α)

theorem trace_adj_matrix [NonAssocSemiring α] [Semiringₓ β] [Module β α] : Matrix.trace _ β _ (G.adj_matrix α) = 0 := by
  simp

variable {α}

theorem adj_matrix_mul_self_apply_self [NonAssocSemiring α] (i : V) :
    (G.adj_matrix α ⬝ G.adj_matrix α) i i = degree G i := by
  simp [degree]
...
theorem adj_matrix_mul_vec_const_apply [Semiringₓ α] {a : α} {v : V} :
    (G.adj_matrix α).mulVec (Function.const _ a) v = G.degree v * a := by
  simp [degree]

theorem adj_matrix_mul_vec_const_apply_of_regular [Semiringₓ α] {d : ℕ} {a : α} (hd : G.is_regular_of_degree d)
    {v : V} : (G.adj_matrix α).mulVec (Function.const _ a) v = d * a := by
  simp [hd v]
...
theorem adj_matrix_to_graph_eq [DecidableEq α] : h.to_graph.adj_matrix α = A := by
  ext i j
  obtain h' | h' := h.zero_or_one i j <;> simp [h']
