...
theorem complete_strongly_regular (m : ℕ) :
    (⊤ : SimpleGraph V).IsSRGOf (Fintype.card V) (Fintype.card V - 1) (Fintype.card V - 2) m :=
  { card := rfl, regular := complete_graph_degree,
    adj_common := fun v w h : v ≠ w => by
      simp only [Fintype.card_of_finset, mem_common_neighbors, filter_not, ← not_or_distrib, filter_eq, filter_or,
        card_univ_diff, mem_univ, if_pos, ← insert_eq, top_adj]
      rw [card_insert_of_not_mem, card_singleton]
      simp [h],
    nadj_common := fun v w h : ¬v ≠ w ∧ _ => (h.1 h.2).elim }
