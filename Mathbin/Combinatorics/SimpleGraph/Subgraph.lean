...
@[simps]
def spanning_coe (G' : subgraph G) : SimpleGraph V where
  Adj := G'.adj
  symm := G'.symm
...
@[simps]
def spanning_coe_equiv_coe_of_spanning (G' : subgraph G) (h : G'.is_spanning) : G'.spanning_coe ≃g G'.coe where
  toFun := fun v => ⟨v, h v⟩
  invFun := fun v => v
  left_inv := fun v => rfl
...
def coe_neighbor_set_equiv {G' : subgraph G} (v : G'.verts) : G'.coe.neighbor_set v ≃ G'.neighbor_set v where
  toFun := fun w =>
    ⟨w, by
      obtain ⟨w', hw'⟩ := w
      simpa using hw'⟩
...
@[simp]
theorem edge_set_top : (⊤ : subgraph G).EdgeSet = G.edge_set :=
  rfl
...
def top_equiv : (⊤ : subgraph G).coe ≃g G where
  toFun := fun v => ↑v
...
vertex type. -/
def bot_equiv : (⊥ : subgraph G).coe ≃g (⊥ : SimpleGraph Empty) where
...
theorem _root_.disjoint.edge_set {H₁ H₂ : subgraph G} (h : Disjoint H₁ H₂) : Disjoint H₁.edge_set H₂.edge_set := by
...
def map_top (x : subgraph G) : x.coe →g G where
  toFun := fun v => v
...
@[simps]
def map_spanning_top (x : subgraph G) : x.spanning_coe →g G where
  toFun := id
  map_rel' := fun v w hvw => x.adj_sub hvw
