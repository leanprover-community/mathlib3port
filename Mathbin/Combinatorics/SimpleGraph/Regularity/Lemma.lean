...
theorem szemeredi_regularity (hε : 0 < ε) (hl : l ≤ card α) :
  ...
  obtain ⟨dum, hdum₁, hdum₂⟩ :=
    exists_equipartition_card_eq (univ : Finset α) (initial_bound_pos _ _).ne' htα
  obtain hε₁ | hε₁ := le_total 1 ε
  ---If `ε ≥ 1`, then this dummy equipartition is `ε`-uniform, so we're done.
  ·
    exact
      ⟨dum, hdum₁, (le_initial_bound ε l).trans hdum₂.ge,
  ...
  suffices h :
    ∀ i,
      ∃ P : Finpartition (univ : Finset α),
        P.IsEquipartition ∧
          t ≤ P.parts.card ∧
            P.parts.card ≤ (step_bound^[i]) t ∧ (P.IsUniform G ε ∨ ε ^ 5 / 4 * i ≤ P.energy G)
