...
theorem IsMatching.to_edge_eq_of_adj {M : Subgraph G} (h : M.IsMatching) {v w : V}
    (hv : v ∈ M.verts) (hvw : M.Adj v w) : h.toEdge ⟨v, hv⟩ = ⟨⟦(v, w)⟧, hvw⟩ := by
  simp only [is_matching.to_edge, Subtype.mk_eq_mk]
  congr
  exact ((h (M.edge_vert hvw)).some_spec.2 w hvw).symm
#align
  simple_graph.subgraph.is_matching.to_edge_eq_of_adj SimpleGraph.Subgraph.IsMatching.to_edge_eq_of_adj
...
#align
  simple_graph.subgraph.is_matching.to_edge.surjective SimpleGraph.Subgraph.IsMatching.toEdge.surjective
...
#align
  simple_graph.subgraph.is_matching.to_edge_eq_to_edge_of_adj SimpleGraph.Subgraph.IsMatching.to_edge_eq_to_edge_of_adj
...
theorem IsMatching.support_eq_verts {M : Subgraph G} (h : M.IsMatching) : M.support = M.verts := by
  ...
  exact ⟨_, hvw⟩
#align
  simple_graph.subgraph.is_matching.support_eq_verts SimpleGraph.Subgraph.IsMatching.support_eq_verts
...
#align
  simple_graph.subgraph.is_matching_iff_forall_degree SimpleGraph.Subgraph.is_matching_iff_forall_degree
...
#align
  simple_graph.subgraph.is_perfect_matching_iff_forall_degree SimpleGraph.Subgraph.is_perfect_matching_iff_forall_degree
...
#align
  simple_graph.subgraph.is_perfect_matching.even_card SimpleGraph.Subgraph.IsPerfectMatching.even_card
