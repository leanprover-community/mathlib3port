...
theorem exists_upperCrossingTime_eq (f : ℕ → Ω → ℝ) (N : ℕ) (ω : Ω) (hab : a < b) :
  ...
  obtain ⟨_, ⟨k, rfl⟩, hk⟩ :
    ∃ (m : _)(hm : m ∈ Set.range fun n => upperCrossingTime a b f N n ω), N < m :=
...
theorem Adapted.isStoppingTime_crossing (hf : Adapted ℱ f) :
  ...
  · obtain ⟨ih₁, ih₂⟩ := ih
    have : IsStoppingTime ℱ (upperCrossingTime a b f N (k + 1)) :=
      ...
      exact
        isStoppingTime_hitting_isStoppingTime ih₂ (fun _ => lowerCrossingTime_le) measurableSet_Ici
          hf _
    refine' ⟨this, _⟩
    · intro n
      exact
        isStoppingTime_hitting_isStoppingTime this (fun _ => upperCrossingTime_le) measurableSet_Iic
          hf _
...
theorem upcrossingStrat_le_one : upcrossingStrat a b f N n ω ≤ 1 :=
  by
  rw [upcrossingStrat, ← Set.indicator_finset_bunionᵢ_apply]
  ...
  · intro i hi j hj hij
    ...
    · rw [min_eq_left
        ...
        max_eq_right
          (lowerCrossingTime_mono hij'.le :
            lowerCrossingTime a b f N _ _ ≤ lowerCrossingTime _ _ _ _ _ _)]
      refine'
        le_trans upperCrossingTime_le_lowerCrossingTime
...
theorem Adapted.upcrossingStrat_adapted (hf : Adapted ℱ f) : Adapted ℱ (upcrossingStrat a b f N) :=
  ...
  change
    strongly_measurable[ℱ n] fun ω =>
      ∑ k in Finset.range N,
        ({ n | lowerCrossingTime a b f N k ω ≤ n } ∩
              { n | n < upperCrossingTime a b f N (k + 1) ω }).indicator
          1 n
  refine'
...
theorem Submartingale.sum_mul_upcrossingStrat_le [IsFiniteMeasure μ] (hf : Submartingale f ℱ μ) :
  ...
  by
  have h₁ :
    (0 : ℝ) ≤ μ[∑ k in Finset.range n, (1 - upcrossingStrat a b f N k) * (f (k + 1) - f k)] :=
    by
  ...
  have h₂ :
    μ[∑ k in Finset.range n, (1 - upcrossingStrat a b f N k) * (f (k + 1) - f k)] =
    ...
    refine'
      integral_sub
        (Integrable.sub (integrableFinsetSum _ fun i hi => hf.integrable _)
          (integrableFinsetSum _ fun i hi => hf.integrable _))
        _
...
theorem upperCrossingTime_eq_of_upcrossingsBefore_lt (hab : a < b)
  ...
  convert not_mem_of_csupₛ_lt hn (upperCrossingTime_lt_bddAbove hab)
...
theorem crossing_eq_crossing_of_lowerCrossingTime_lt {M : ℕ} (hNM : N ≤ M)
  ...
  · simp only [Nat.zero_eq, upperCrossingTime_zero, bot_eq_zero', eq_self_iff_true,
    ...
    simp only [lowerCrossingTime, hitting_lt_iff] at h
    obtain ⟨j, hj₁, hj₂⟩ := h
  ...
  · specialize
    ...
    have : upperCrossingTime a b f M k.succ ω = upperCrossingTime a b f N k.succ ω :=
      by
      simp only [upperCrossingTime_succ_eq, hitting_lt_iff] at h'⊢
      obtain ⟨j, hj₁, hj₂⟩ := h'
      rw [eq_comm, ih.2]
...
theorem crossing_eq_crossing_of_upperCrossingTime_lt {M : ℕ} (hNM : N ≤ M)
  ...
  by
  have :=
    (crossing_eq_crossing_of_lowerCrossingTime_lt hNM
...
theorem upcrossingsBefore_mono (hab : a < b) : Monotone fun N ω => upcrossingsBefore a b f N ω :=
  ...
  · refine' csupₛ_le_csupₛ (upperCrossingTime_lt_bddAbove hab) hemp fun n hn => _
...
theorem upcrossingsBefore_lt_of_exists_upcrossing (hab : a < b) {N₁ N₂ : ℕ} (hN₁ : N ≤ N₁)
  ...
  by
  refine' lt_of_lt_of_le (Nat.lt_succ_self _) (le_csupₛ (upperCrossingTime_lt_bddAbove hab) _)
  ...
  · refine' ⟨N₂, ⟨_, Nat.lt_succ_self _⟩, hN₂'.le⟩
    ...
    · have : upperCrossingTime a b f N (upcrossingsBefore a b f N ω) ω < N :=
        Nat.supₛ_mem (upperCrossingTime_lt_nonempty hN) (upperCrossingTime_lt_bddAbove hab)
...
theorem sub_eq_zero_of_upcrossingsBefore_lt (hab : a < b) (hn : upcrossingsBefore a b f N ω < n) :
  ...
  have : N ≤ upperCrossingTime a b f N n ω :=
    ...
    rw [← not_lt]
    exact fun h => not_le.2 hn (le_csupₛ (upperCrossingTime_lt_bddAbove hab) h)
...
theorem mul_upcrossingsBefore_le (hf : a ≤ f N ω) (hab : a < b) :
  ...
  classical
    by_cases hN : N = 0
    · simp [hN]
    simp_rw [upcrossingStrat, Finset.sum_mul, ← Set.indicator_mul_left, Pi.one_apply, Pi.sub_apply,
      one_mul]
    rw [Finset.sum_comm]
    have h₁ :
      ∀ k,
        (∑ n in Finset.range N,
            (Set.Ico (lowerCrossingTime a b f N k ω)
                  (upperCrossingTime a b f N (k + 1) ω)).indicator
              (fun m => f (m + 1) ω - f m ω) n) =
          stoppedValue f (upperCrossingTime a b f N (k + 1)) ω -
            stoppedValue f (lowerCrossingTime a b f N k) ω :=
      by
      intro k
      rw [Finset.sum_indicator_eq_sum_filter,
        (_ :
          Finset.filter
              (fun i =>
                i ∈ Set.Ico (lowerCrossingTime a b f N k ω) (upperCrossingTime a b f N (k + 1) ω))
              (Finset.range N) =
    ...
    simp_rw [h₁]
    have h₂ :
      (∑ k in Finset.range (upcrossingsBefore a b f N ω), b - a) ≤
        ∑ k in Finset.range N,
          stoppedValue f (upperCrossingTime a b f N (k + 1)) ω -
            stoppedValue f (lowerCrossingTime a b f N k) ω :=
      by
      calc
        (∑ k in Finset.range (upcrossingsBefore a b f N ω), b - a) ≤
          ...
          by
          refine'
            Finset.sum_le_sum fun i hi => le_sub_of_le_upcrossingsBefore (zero_lt_iff.2 hN) hab _
...
theorem crossing_pos_eq (hab : a < b) :
  ...
  induction' n with k ih
  · refine' ⟨rfl, _⟩
    simp only [lowerCrossingTime_zero, hitting, Set.mem_Icc, Set.mem_Iic]
    ext ω
    ...
    · rfl
  · have :
      upperCrossingTime 0 (b - a) (fun n ω => (f n ω - a)⁺) N (k + 1) =
        upperCrossingTime a b f N (k + 1) :=
      by
      ext ω
...
theorem mul_integral_upcrossingsBefore_le_integral_pos_part_aux [IsFiniteMeasure μ]
  ...
  refine'
    le_trans (le_of_eq _)
      (integral_mul_upcrossingsBefore_le_integral (hf.sub_martingale (martingaleConst _ _ _)).pos
...
theorem upcrossingsBefore_eq_sum (hab : a < b) :
  ...
  have h₁ :
    ...
    ·
      exact
        upperCrossingTime_lt_of_le_upcrossingsBefore (zero_lt_iff.2 hN) hab (Nat.lt_succ_iff.1 hk.2)
  have h₂ :
...
theorem Adapted.integrableUpcrossingsBefore [IsFiniteMeasure μ] (hf : Adapted ℱ f) (hab : a < b) :
  ...
  ⟨Measurable.aeStronglyMeasurable
    ...
    hasFiniteIntegralOfBounded this⟩
...
theorem Submartingale.mul_lintegral_upcrossings_le_lintegral_pos_part [IsFiniteMeasure μ] (a b : ℝ)
  ...
  · simp_rw [upcrossings]
    have : ∀ N, (∫⁻ ω, Ennreal.ofReal ((f N ω - a)⁺) ∂μ) = Ennreal.ofReal (∫ ω, (f N ω - a)⁺ ∂μ) :=
      ...
      · exact (hf.sub_martingale (martingaleConst _ _ _)).pos.integrable _
