...
theorem IndepSets.piUnionInterOfNotMem {π : ι → Set (Set Ω)} {a : ι} {S : Finset ι} (hp_ind : IndepSets π μ)
  ...
  classical let f n := ite (n = a) t2 (ite (n ∈ s) (ft1 n) Set.univ)
    ...
    exact fun x hxS => h_f_mem x (by simp [hxS])
...
theorem IndepSets.indep [IsProbabilityMeasure μ] (m : ι → MeasurableSpace Ω) (h_le : ∀ i, m i ≤ m0)
  ...
  classical refine' Finset.induction _ _
...
#align probability_theory.indep_set_iff_indep_sets_singleton ProbabilityTheory.indep_set_iff_indep_sets_singleton
...
#align probability_theory.indep_set_iff_measure_inter_eq_mul ProbabilityTheory.indep_set_iff_measure_inter_eq_mul
...
#align probability_theory.indep_fun_iff_indep_set_preimage ProbabilityTheory.indep_fun_iff_indep_set_preimage
...
theorem IndepFun.indepFunFinset [IsProbabilityMeasure μ] {ι : Type _} {β : ι → Type _} {m : ∀ i, MeasurableSpace (β i)}
...
theorem IndepFun.indepFunProd [IsProbabilityMeasure μ] {ι : Type _} {β : ι → Type _} {m : ∀ i, MeasurableSpace (β i)}
  ...
  classical have h_right :
      f k =
        (fun p : ∀ j : ({k} : Finset ι), β j => p ⟨k, Finset.mem_singleton_self k⟩) ∘ fun a (j : ({k} : Finset ι)) =>
          f j a :=
      rfl
    exact measurablePiApply ⟨k, Finset.mem_singleton_self k⟩
    have h_left :
      (fun ω => (f i ω, f j ω)) =
        (fun p : ∀ l : s, β l =>
  ...
  classical have h_right :
      f i =
        (fun p : ∀ j : ({i} : Finset ι), β => p ⟨i, Finset.mem_singleton_self i⟩) ∘ fun a (j : ({i} : Finset ι)) =>
          f j a :=
      rfl
    exact measurablePiApply ⟨i, Finset.mem_singleton_self i⟩
    · ext1 a
      simp only [Function.comp_apply]
      have : (∏ j : ↥s, f (↑j) a) = (∏ j : ↥s, f ↑j) a := by rw [Finset.prod_apply]
      rw [this, Finset.prod_coe_sort]
      
    exact finset.univ.measurable_prod fun (j : ↥s) (H : j ∈ Finset.univ) => measurablePiApply j
    exact
...
theorem IndepSet.indepFunIndicator [Zero β] [One β] {m : MeasurableSpace β} {s : ι → Set Ω} (hs : IndepSet s μ) :
  ...
  classical rw [Indep_fun_iff_measure_inter_preimage_eq_mul]
