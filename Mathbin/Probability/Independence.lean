...
#align
  probability_theory.indep_sets_of_indep_sets_of_le_left ProbabilityTheory.indepSetsOfIndepSetsOfLeLeft
...
#align
  probability_theory.indep_sets_of_indep_sets_of_le_right ProbabilityTheory.indepSetsOfIndepSetsOfLeRight
...
#align probability_theory.indep_sets_singleton_iff ProbabilityTheory.indep_sets_singleton_iff
...
theorem IndepSets.indepSets {s : ι → Set (Set Ω)} [MeasurableSpace Ω] {μ : Measure Ω}
    (h_indep : IndepSets s μ) {i j : ι} (hij : i ≠ j) : IndepSets (s i) (s j) μ := by classical
  intro t₁ t₂ ht₁ ht₂
...
theorem IndepCat.indep {m : ι → MeasurableSpace Ω} [MeasurableSpace Ω] {μ : Measure Ω}
    (h_indep : IndepCat m μ) {i j : ι} (hij : i ≠ j) : Indep (m i) (m j) μ := by
...
theorem IndepCat.indepSets [MeasurableSpace Ω] {μ : Measure Ω} {m : ι → MeasurableSpace Ω}
  ...
  (h_indep S) fun x hxS =>
    ((hms x).symm ▸ measurableSetGenerateFrom (hfs x hxS) : measurable_set[m x] (f x))
...
theorem Indep.indepSets [MeasurableSpace Ω] {μ : Measure Ω} {s1 s2 : Set (Set Ω)}
  ...
  fun t1 t2 ht1 ht2 => h_indep t1 t2 (measurableSetGenerateFrom ht1) (measurableSetGenerateFrom ht2)
...
private theorem indep_sets.indep_aux {m2 : MeasurableSpace Ω} {m : MeasurableSpace Ω}
    ...
    (ht1 : t1 ∈ p1) (ht2m : measurable_set[m2] t2) : μ (t1 ∩ t2) = μ t1 * μ t2 := by
...
theorem indepSetsPiUnionInterOfDisjoint [IsProbabilityMeasure μ] {s : ι → Set (Set Ω)} {S T : Set ι}
    ...
    IndepSets (piUnionInter s S) (piUnionInter s T) μ := by
...
#align
  probability_theory.indep_sets_pi_Union_Inter_of_disjoint ProbabilityTheory.indepSetsPiUnionInterOfDisjoint
...
theorem IndepSet.indepGenerateFromOfDisjoint [IsProbabilityMeasure μ] {s : ι → Set Ω}
  ...
  refine'
    indep_sets.indep'
      (fun t ht => generate_from_pi_Union_Inter_le _ _ _ _ (measurable_set_generate_from ht))
...
#align
  probability_theory.Indep_set.indep_generate_from_of_disjoint ProbabilityTheory.IndepSet.indepGenerateFromOfDisjoint
...
theorem indepSuprOfDirectedLe {Ω} {m : ι → MeasurableSpace Ω} {m' m0 : MeasurableSpace Ω}
    ...
    (h_le : ∀ i, m i ≤ m0) (h_le' : m' ≤ m0) (hm : Directed (· ≤ ·) m) : Indep (⨆ i, m i) m' μ := by
...
#align
  probability_theory.Indep_set.indep_generate_from_le_nat ProbabilityTheory.IndepSet.indepGenerateFromLeNat
...
theorem IndepSets.piUnionInterOfNotMem {π : ι → Set (Set Ω)} {a : ι} {S : Finset ι}
    (hp_ind : IndepSets π μ) (haS : a ∉ S) : IndepSets (piUnionInter π S) (π a) μ := by
...
#align
  probability_theory.Indep_sets.pi_Union_Inter_of_not_mem ProbabilityTheory.IndepSets.piUnionInterOfNotMem
...
theorem IndepSets.indep [IsProbabilityMeasure μ] (m : ι → MeasurableSpace Ω) (h_le : ∀ i, m i ≤ m0)
  ...
  · simp only [measure_univ, imp_true_iff, Set.Inter_false, Set.Inter_univ, Finset.prod_empty,
      eq_self_iff_true]
...
#align
  probability_theory.indep_set_iff_indep_sets_singleton ProbabilityTheory.indep_set_iff_indep_sets_singleton
...
#align
  probability_theory.indep_set_iff_measure_inter_eq_mul ProbabilityTheory.indep_set_iff_measure_inter_eq_mul
...
#align
  probability_theory.indep.indep_set_of_measurable_set ProbabilityTheory.Indep.indepSetOfMeasurableSet

theorem indep_iff_forall_indep_set (m₁ m₂ : MeasurableSpace Ω) {m0 : MeasurableSpace Ω}
    (μ : Measure Ω) :
  ...
  ⟨fun h => fun s t hs ht => h.indepSetOfMeasurableSet hs ht, fun h s t hs ht =>
    h s t hs ht s t (measurableSetGenerateFrom (Set.mem_singleton s))
      (measurableSetGenerateFrom (Set.mem_singleton t))⟩
#align probability_theory.indep_iff_forall_indep_set ProbabilityTheory.indep_iff_forall_indep_set
...
theorem indep_fun_iff_measure_inter_preimage_eq_mul {mβ : MeasurableSpace β}
...
#align
  probability_theory.indep_fun_iff_measure_inter_preimage_eq_mul ProbabilityTheory.indep_fun_iff_measure_inter_preimage_eq_mul

theorem Indep_fun_iff_measure_inter_preimage_eq_mul {ι : Type _} {β : ι → Type _}
...
theorem indep_fun_iff_indep_set_preimage {mβ : MeasurableSpace β} {mβ' : MeasurableSpace β'}
...
#align
  probability_theory.indep_fun_iff_indep_set_preimage ProbabilityTheory.indep_fun_iff_indep_set_preimage
...
theorem IndepFun.indepFunFinset [IsProbabilityMeasure μ] {ι : Type _} {β : ι → Type _}
  ...
  have h_meas_t' : ∀ i ∈ T, MeasurableSet (sets_t' i) := by
    ...
    exact ht1 _
  have h_eq_inter_S :
    (fun (ω : Ω) (i : ↥S) => f (↑i) ω) ⁻¹' Set.pi Set.univ sets_s = ⋂ i ∈ S, f i ⁻¹' sets_s' i := by
    ext1 x
  ...
  have h_eq_inter_T :
    (fun (ω : Ω) (i : ↥T) => f (↑i) ω) ⁻¹' Set.pi Set.univ sets_t = ⋂ i ∈ T, f i ⁻¹' sets_t' i := by
    ext1 x
    simp only [Set.mem_preimage, Set.mem_univ_pi, Set.mem_Inter]
    constructor <;> intro h
    · intro i hi
...
#align
  probability_theory.measure_eq_zero_or_one_or_top_of_indep_set_self ProbabilityTheory.measure_eq_zero_or_one_or_top_of_indep_set_self

theorem measure_eq_zero_or_one_of_indep_set_self [IsFiniteMeasure μ] {t : Set Ω}
...
#align
  probability_theory.measure_eq_zero_or_one_of_indep_set_self ProbabilityTheory.measure_eq_zero_or_one_of_indep_set_self
...
#align
  probability_theory.measure_zero_or_one_of_measurable_set_limsup ProbabilityTheory.measure_zero_or_one_of_measurable_set_limsup
...
#align
  probability_theory.measure_zero_or_one_of_measurable_set_limsup_at_top ProbabilityTheory.measure_zero_or_one_of_measurable_set_limsup_at_top
...
#align
  probability_theory.measure_zero_or_one_of_measurable_set_limsup_at_bot ProbabilityTheory.measure_zero_or_one_of_measurable_set_limsup_at_bot
