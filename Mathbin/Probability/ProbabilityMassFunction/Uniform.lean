...
def uniformOfFinset (s : Finsetₓ α) (hs : s.Nonempty) : Pmf α :=
  ofFinset (fun a => if a ∈ s then (s.card : ℝ≥0)⁻¹ else 0) s
    (Exists.rec_on hs fun x hx =>
      calc
        (∑ a : α in s, ite (a ∈ s) (s.card : ℝ≥0)⁻¹ 0) = ∑ a : α in s, (s.card : ℝ≥0)⁻¹ :=
          Finsetₓ.sum_congr rfl fun x hx => by simp [hx]
        _ = s.card • (s.card : ℝ≥0)⁻¹ := Finsetₓ.sum_const _
        _ = (s.card : ℝ≥0) * (s.card : ℝ≥0)⁻¹ := by rw [nsmul_eq_mul]
        _ = 1 := div_self (Nat.cast_ne_zero.2 <| Finsetₓ.card_ne_zero_of_mem hx)
        )
    fun x hx => by simp only [hx, if_false]

variable {s : Finsetₓ α} (hs : s.Nonempty) {a : α}

@[simp]
theorem uniform_of_finset_apply (a : α) : uniformOfFinset s hs a = if a ∈ s then (s.card : ℝ≥0)⁻¹ else 0 :=
  rfl

theorem uniform_of_finset_apply_of_mem (ha : a ∈ s) : uniformOfFinset s hs a = s.card⁻¹ := by simp [ha]
...
theorem support_uniform_of_finset : (uniformOfFinset s hs).Support = s :=
  Set.ext
    (by
      let ⟨a, ha⟩ := hs
      simp [mem_support_iff, Finsetₓ.ne_empty_of_mem ha])
...
theorem to_outer_measure_uniform_of_finset_apply :
  ...
  calc
    ...
    _ = ↑(∑ x in s.filter (· ∈ t), if x ∈ s ∧ x ∈ t then (s.card : ℝ≥0)⁻¹ else 0) := by
      ...
      have : ¬(x ∈ s ∧ x ∈ t) := fun h => hx (Finsetₓ.mem_filter.2 h)
      simp [this]
    _ = ↑(∑ x in s.filter (· ∈ t), (s.card : ℝ≥0)⁻¹) :=
      Ennreal.coe_eq_coe.2
        ((Finsetₓ.sum_congr rfl) fun x hx => by
          let this : x ∈ s ∧ x ∈ t := by simpa using hx
          simp [this])
    _ = (s.filter (· ∈ t)).card / s.card := by
      let this : (s.card : ℝ≥0) ≠ 0 := Nat.cast_ne_zero.2 (hs.recOn fun _ => Finsetₓ.card_ne_zero_of_mem)
...
def uniformOfFintype (α : Type _) [Fintypeₓ α] [Nonempty α] : Pmf α :=
  uniformOfFinset Finsetₓ.univ Finsetₓ.univ_nonempty

variable [Fintypeₓ α] [Nonempty α]

@[simp]
theorem uniform_of_fintype_apply (a : α) : uniformOfFintype α a = (Fintypeₓ.card α)⁻¹ := by
  simpa only [uniform_of_fintype, Finsetₓ.mem_univ, if_true, uniform_of_finset_apply]

@[simp]
theorem support_uniform_of_fintype (α : Type _) [Fintypeₓ α] [Nonempty α] : (uniformOfFintype α).Support = ⊤ :=
  Set.ext fun x => by simpa [mem_support_iff] using Fintypeₓ.card_ne_zero
...
theorem to_outer_measure_uniform_of_fintype_apply :
    (uniformOfFintype α).toOuterMeasure s = Fintypeₓ.card s / Fintypeₓ.card α := by simpa [uniform_of_fintype]

theorem to_measure_uniform_of_fintype_apply [MeasurableSpace α] (hs : MeasurableSet s) :
    (uniformOfFintype α).toMeasure s = Fintypeₓ.card s / Fintypeₓ.card α := by simpa [uniform_of_fintype, hs]
...
def ofMultiset (s : Multiset α) (hs : s ≠ 0) : Pmf α :=
  ⟨fun a => s.count a / s.card, by
    have : (∑ a in s.toFinset, (s.count a : ℝ) / s.card) = 1 := by
      simp only [div_eq_inv_mul, ← Finsetₓ.mul_sum, ← Nat.cast_sum, Multiset.to_finset_sum_count_eq]
      rw [inv_mul_cancel]
      simp [hs]
