...
theorem finite_of_condCount_ne_zero {s t : Set Ω} (h : condCount s t ≠ 0) : s.Finite :=
  by
  by_contra hs'
  simpa [condCount, cond, Measure.count_apply_infinite hs'] using h
...
theorem condCountIsProbabilityMeasure {s : Set Ω} (hs : s.Finite) (hs' : s.Nonempty) :
    IsProbabilityMeasure (condCount s) :=
  {
    measure_univ :=
      by
      rw [condCount, cond_apply _ hs.measurable_set, Set.inter_univ, Ennreal.inv_mul_cancel]
...
theorem condCount_inter_self (hs : s.Finite) : condCount s (s ∩ t) = condCount s t := by
  rw [condCount, cond_inter_self _ hs.measurable_set]
...
theorem condCount_self (hs : s.Finite) (hs' : s.Nonempty) : condCount s s = 1 :=
  by
  rw [condCount, cond_apply _ hs.measurable_set, Set.inter_self, Ennreal.inv_mul_cancel]
  · exact fun h => hs'.ne_empty <| Measure.empty_of_count_eq_zero h
  · exact (Measure.count_apply_lt_top.2 hs).ne
...
theorem condCount_eq_one_of (hs : s.Finite) (hs' : s.Nonempty) (ht : s ⊆ t) : condCount s t = 1 :=
  by
  haveI := condCountIsProbabilityMeasure hs hs'
  ...
  have hsf :=
    finite_of_condCount_ne_zero
      (by
        rw [h]
        exact one_ne_zero)
  rw [condCount, cond_apply _ hsf.measurable_set, mul_comm] at h
...
theorem condCount_eq_zero_iff (hs : s.Finite) : condCount s t = 0 ↔ s ∩ t = ∅ := by
  simp [condCount, cond_apply _ hs.measurable_set, Measure.count_apply_eq_top,
...
theorem condCount_inter (hs : s.Finite) :
  ...
  rw [condCount, condCount, cond_apply _ hs.measurable_set, cond_apply _ hs.measurable_set,
...
theorem condCount_union (hs : s.Finite) (htu : Disjoint t u) :
  ...
  by
  rw [condCount, cond_apply _ hs.measurable_set, cond_apply _ hs.measurable_set,
...
theorem condCount_compl (t : Set Ω) (hs : s.Finite) (hs' : s.Nonempty) :
  ...
  rw [← condCount_union hs disjoint_compl_right, Set.union_compl_self,
    (condCountIsProbabilityMeasure hs hs').measure_univ]
...
theorem condCount_disjoint_union (hs : s.Finite) (ht : t.Finite) (hst : Disjoint s t) :
  ...
  rw [condCount, condCount, condCount, cond_apply _ hs.measurable_set,
